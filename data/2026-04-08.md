...
TL;DR: Survey paper analyzing the paradigm shift from heuristic-based to generative AI approaches for automatic video trailer generation, covering LLMs, MLLMs, diffusion models, and their architectural evolution.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive technical review of the evolution in automatic video trailer generation, documenting the transition from traditional heuristic methods to modern generative AI techniques, and establishing a taxonomy for AI-driven systems in the foundation model era.

Method: Survey methodology analyzing literature on generative techniques including autoregressive Transformers, LLM-orchestrated pipelines, text-to-video foundation models (Sora, Veo), architectural progression from GCNs to Trailer Generation Transformers, and establishing a new taxonomy for AI-driven trailer generation.

Result: Establishes a new taxonomy for AI-driven trailer generation in the foundation model era, identifies the architectural progression from GCNs to TGTs, and demonstrates that future systems will move beyond extractive selection toward controllable generative editing and semantic reconstruction.

Conclusion: The field is undergoing a profound paradigm shift from heuristic-based extraction to deep generative synthesis, with future promotional video systems expected to focus on controllable generative editing and semantic reconstruction rather than just extractive selection, while raising important economic and ethical considerations.
...
TL;DR: RCP is a pruning framework for Large Vision-Language Models that removes up to 88.9% of visual tokens with minimal accuracy loss by using cumulative pruning with delayed repair mechanisms.


<details>
  <summary>Details</summary>
Motivation: LVLMs have prohibitive inference costs due to processing massive numbers of visual tokens, and existing pruning methods cause significant performance degradation from distribution shifts in hidden states.

Method: Uses cross-attention pruner with cumulative masks for consistent token reduction, plus delayed repair adapter (DRA) that caches pruned token essence and applies FiLM-based modulation to answer generation tokens with repair loss for statistical matching.

Result: Removes up to 88.9% of visual tokens, reduces FLOPs by up to 85.7% with only marginal average accuracy drop, outperforming prior methods that avoid fine-tuning the original model.

Conclusion: RCP provides an efficient pruning framework that maintains performance while significantly reducing computational costs through lightweight plug-in modules and physical token discarding at inference.
...
TL;DR: Video-MME-v2 is a comprehensive benchmark designed to rigorously evaluate video understanding robustness and faithfulness through progressive complexity levels and group-based evaluation, revealing significant gaps between current models and human performance.


<details>
  <summary>Details</summary>
Motivation: Existing video understanding benchmarks are becoming saturated, with inflated leaderboard scores not reflecting real-world model capabilities, creating a critical discrepancy that needs addressing.

Method: 1) Progressive tri-level hierarchy increasing complexity from visual aggregation to temporal modeling to multimodal reasoning; 2) Group-based non-linear evaluation penalizing fragmented correctness; 3) Rigorous human annotation pipeline with 3,300 hours and 5 quality assurance rounds.

Result: Substantial gap between best model (Gemini-3-Pro) and human experts; hierarchical bottleneck where errors propagate from lower to higher levels; thinking-based reasoning heavily dependent on textual cues; benchmark serves as demanding testbed for next-gen video MLLMs.

Conclusion: Video-MME-v2 establishes an authoritative benchmark that exposes current limitations in video understanding and provides a rigorous framework for developing more robust and faithful next-generation video multimodal large language models.
...
Method: Curate high-quality training set spanning diverse real-world domains, augmented with generative synthetic data providing controlled fine-grained identity and contextual variations, then develop ID-Sim as a feed-forward metric.
...
Conclusion: ID-Sim is designed to faithfully reflect human selective sensitivity to identities and aims to facilitate progress in identity-focused computer vision tasks by providing better evaluation metrics.
...
TL;DR: R3PM-Net is a lightweight, global-aware point matching network for fast and accurate point cloud registration, achieving state-of-the-art speed while maintaining competitive accuracy across synthetic and real-world industrial datasets.


<details>
  <summary>Details</summary>
Motivation: Existing deep-learning PCR methods are limited by their development on clean synthetic datasets, which reduces their generalizability to real-world industrial scenarios with imperfect scans, noise, and occlusion.

Method: Proposes R3PM-Net, a lightweight global-aware object-level point matching network, and introduces two new datasets (Sioux-Cranfield and Sioux-Scans) for evaluating registration of imperfect photogrammetric and event-camera scans to CAD models.

Result: Achieves perfect fitness score of 1 and inlier RMSE of 0.029 cm in only 0.007s on ModelNet40 (7× faster than RegTR), maintains similar performance on Sioux-Cranfield, and successfully resolves challenging edge cases on Sioux-Scans in under 50 ms.

Conclusion: R3PM-Net provides a robust, high-speed solution for industrial applications requiring both precision and real-time performance, bridging the gap between synthetic and real-world point cloud registration.
...
TL;DR: SVAgent: A storyline-guided cross-modal multi-agent framework for VideoQA that mimics human narrative reasoning to improve performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Current VideoQA methods rely on locating relevant frames rather than reasoning through evolving storylines like humans do, which is crucial for robust and contextually grounded predictions.

Method: Multi-agent framework with: 1) storyline agent that constructs narrative representations, 2) refinement suggestion agent that analyzes historical failures, 3) cross-modal decision agents that predict answers from visual/textual modalities, and 4) meta-agent that aligns cross-modal predictions.
...
Conclusion: The proposed storyline-guided multi-agent framework effectively addresses the gap in current VideoQA methods by incorporating human-like narrative reasoning, leading to improved performance and better interpretability.
...
TL;DR: A three-channel denoising diffusion model generates paired CC and MLO mammogram views using difference encoding to maintain anatomical consistency across projections.


<details>
  <summary>Details</summary>
Motivation: Many mammography datasets lack complete paired CC and MLO views, limiting algorithm development that requires cross-view consistency. Current methods struggle with generating anatomically coherent dual-view pairs.

Method: A three-channel DDPM with separate channels for CC and MLO views, plus a third channel encoding their absolute difference to guide anatomical coherence. Fine-tuned a pretrained DDPM on private screening data for dual-view synthesis.
...
TL;DR: VidGround addresses the problem that many video understanding benchmarks contain questions answerable from text alone by curating visually-grounded post-training data, improving VLM performance by up to 6.2 points with less data.


<details>
  <summary>Details</summary>
Motivation: Current video understanding benchmarks contain 40-60% of questions that can be answered using text cues alone, and these issues also affect post-training datasets, limiting VLM video understanding improvement.

Method: Introduces VidGround - a simple solution using only visually grounded questions without linguistic biases for post-training, combined with RL-based post-training algorithms.

Result: Improves performance by up to 6.2 points relative to using full dataset while using only 69.1% of original post-training data. Data curation with simple post-training outperforms more complex techniques.

Conclusion: Data quality is a major bottleneck for improving video understanding in VLMs, highlighting the importance of curating post-training data and evaluation benchmarks that truly require visual grounding.
...
TL;DR: SecurePix: A CMOS-compatible pixel sensor with in-pixel encryption using ferroelectric transistors for end-to-end image security.


<details>
  <summary>Details</summary>
Motivation: Visual data can be exposed through multiple stages of imaging pipeline, requiring encryption before pixel values appear on readout lines to ensure end-to-end security.

Method: Compact pixel architecture using ferroelectric field-effect transistors with programmable non-volatile polarization states as symmetric keys; designed in HSPICE with 45nm CMOS layout; each pixel's transfer characteristic encrypts photodiode voltage to analog output.

Result: Pixel pitch of 2.33×3.01 μm²; ResNet-18 accuracy drops from 99.29% to 9.58% on MNIST and 91.33% to 6.98% on CIFAR-10 after encryption; programming power-delay product of 17 μW·μs and sensing of 1.25 μW·μs per pixel.

Conclusion: SecurePix provides strong hardware-level protection against neural-network inference with low overhead, enabling authorized recovery via symmetric key lookup tables.
...
TL;DR: NeuroQuant: A modality-aware 3D VQ-VAE for multi-modal brain MRI reconstruction that learns shared anatomical representations across modalities while preserving modality-specific appearance features.


<details>
  <summary>Details</summary>
Motivation: Existing brain VAEs focus only on single-modality data (T1-weighted MRI), missing the complementary diagnostic value of other modalities like T2-weighted MRIs. There's a need for robust VAEs that can handle multi-modal brain MRI data for medical image analysis applications.

Method: Proposes NeuroQuant, a 3D vector-quantized VAE with: 1) factorized multi-axis attention for shared latent representations across modalities, 2) dual-stream 3D encoder separating modality-invariant anatomical structures from modality-dependent appearance, 3) discretization of anatomical encoding using shared codebook, 4) Feature-wise Linear Modulation (FiLM) to combine anatomical and appearance features during decoding, and 5) joint 2D/3D training strategy to account for slice-based MRI acquisition.

Result: Extensive experiments on two multi-modal brain MRI datasets show NeuroQuant achieves superior reconstruction fidelity compared to existing VAEs, enabling scalable foundation for downstream generative modeling and cross-modal brain image analysis.

Conclusion: NeuroQuant provides an effective approach for learning robust multi-modal brain MRI representations, addressing limitations of single-modality VAEs and offering better reconstruction quality for medical imaging applications.
...
TL;DR: MIRAGE is a training-free framework for precise multi-instance image editing that addresses over-editing and spatial misalignment issues in existing models through regional alignment and multi-branch parallel denoising.


<details>
  <summary>Details</summary>
Motivation: Current instruction-guided image editing models (like FLUX.2 and Qwen-Image-Edit) struggle with complex scenarios involving multiple similar instances requiring individual edits, suffering from severe over-editing and spatial misalignment when faced with multiple identical instances and composite instructions.

Method: Proposes MIRAGE (Multi-Instance Regional Alignment via Guided Editing), a training-free framework that: 1) uses a vision-language model to parse complex instructions into regional subsets, 2) employs a multi-branch parallel denoising strategy, 3) injects latent representations of target regions into global representation space while maintaining background integrity through a reference trajectory.
...
Conclusion: MIRAGE provides an effective solution for fine-grained multi-instance image editing, with benchmark and code made publicly available for further research.
...
TL;DR: LSRM scales transformer context windows for feed-forward 3D reconstruction, using sparse attention to handle 20x more object tokens and 2x more image tokens than SOTA, achieving quality comparable to dense-view optimization.
...
Result: LSRM handles 20x more object tokens and >2x more image tokens than prior SOTA, achieving 2.5 dB higher PSNR and 40% lower LPIPS on novel-view synthesis benchmarks. For inverse rendering, it matches or exceeds SOTA dense-view optimization LPIPS.

Conclusion: Scaling transformer context windows through sparse attention effectively bridges the gap between feed-forward and dense-view optimization methods for 3D reconstruction and inverse rendering, enabling high-fidelity results with computational efficiency.
...
TL;DR: Training-free method for merging orthogonal adapters using geometric properties of GS orthogonal matrices and spectra restoration for concept-style fusion.


<details>
  <summary>Details</summary>
Motivation: Address the open problem of combining multiple adapters tuned for different tasks (like subject and style) into a single adapter that performs well on both tasks, particularly for generative models.

Method: Uses orthogonal fine-tuning (OFT) with structured orthogonal parametrization, derives manifold structure of Group-and-Shuffle (GS) orthogonal matrices, obtains efficient geodesics approximation formulas, and proposes spectra restoration transform to restore spectral properties of merged adapters.

Result: Proposes first training-free method for merging multiplicative orthogonal adapters, capable of uniting concept and style features from different adapters in subject-driven generation tasks.

Conclusion: Demonstrates that geometric properties of orthogonal matrices enable training-free adapter merging, with spectra restoration improving fusion quality, providing a practical solution for multi-task adapter combination.
...
TL;DR: Detection-guided small VLM framework improves construction hazard identification by integrating object detection with multimodal reasoning, achieving better accuracy and explanation quality with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Large VLMs have strong reasoning but high computational costs, while small VLMs are efficient but suffer from reduced accuracy and hallucinations in complex construction scenes. Need for balanced solution.

Method: Proposed detection-guided sVLM framework: YOLOv11n detector localizes workers and machinery, detected entities embedded into structured prompts to guide sVLM reasoning for spatially grounded hazard assessment. Evaluated 6 sVLMs in zero-shot settings.

Result: Framework improved hazard detection across all models. Best model (Gemma-3 4B) achieved 50.6% F1-score vs 34.5% baseline. Explanation quality improved (BERTScore F1: 0.61 to 0.82). Minimal overhead: only 2.5 ms per image added.
...
TL;DR: Boxer is a 3D object localization system that lifts 2D open-vocabulary detections to 3D using BoxerNet transformer, multi-view fusion, and geometric filtering, reducing need for expensive 3D annotations.


<details>
  <summary>Details</summary>
Motivation: 3D object localization for open-world categories remains largely unsolved compared to 2D detection. Current methods require costly 3D bounding box annotations, creating a need for approaches that can leverage existing 2D detection capabilities.

Method: BoxerNet transformer lifts 2D bounding box proposals into 3D, followed by multi-view fusion and geometric filtering. Uses 2D detectors (DETIC, OWLv2, SAM3) for 2D localization, incorporates aleatoric uncertainty for robust regression, median depth patch encoding for sparse depth, and trains on 1.2M unique 3DBBs.

Result: Outperforms state-of-the-art baselines: 0.532 vs. 0.010 mAP in egocentric settings without dense depth, and 0.412 vs. 0.250 mAP on CA-1M with dense depth available.

Conclusion: Boxer effectively addresses open-world 3D object localization by leveraging existing 2D detection capabilities, reducing dependency on expensive 3D annotations while achieving superior performance over current methods.
...
TL;DR: Hierarchical transformer for heterogeneous neuroimaging meshes that handles both volumetric and surface data with variable vertex features, achieving SOTA on Alzheimer's and cortical dysplasia tasks.


<details>
  <summary>Details</summary>
Motivation: Current neuroimaging mesh analysis methods either ignore clinically informative vertex-level features (cortical thickness, curvature, etc.) or only support single mesh topologies, limiting their use across different imaging pipelines and feature sets.

Method: Hierarchical transformer framework using spatially adaptive tree partitions from simplicial complexes of arbitrary order. Includes feature projection module to map variable-length vertex descriptors into spatial hierarchy, separating geometry from features. Self-supervised pretraining via masked reconstruction of coordinates and morphometric channels.
...
Conclusion: The proposed framework successfully handles heterogeneous mesh analysis across different neuroimaging modalities and feature sets, providing a unified architecture for both volumetric and surface data with transferable pretrained representations.
...
TL;DR: Human-in-the-loop framework for efficient estimation of two-point correlation functions (2PCF) of target sources using adaptive sampling guided by pre-trained classifiers to reduce annotation effort while maintaining statistical rigor.


<details>
  <summary>Details</summary>
Motivation: Measuring 2PCF for specific subsets of points (like star clusters in astronomy) requires time-consuming manual labeling of sources to construct catalogs. Current approaches are inefficient for large datasets where only a small subset of points has the property of interest.

Method: Human-in-the-loop framework with pre-trained classifier guidance for adaptive sampling. Novel unbiased estimator for pair counts across multiple distance bins, combined with sampling strategy and confidence interval construction for statistically grounded measurements.

Result: Method achieves substantially lower variance than simple Monte Carlo approaches while significantly reducing annotation effort. Enables scalable measurement of two-point correlations in astronomy datasets.

Conclusion: The framework provides an efficient, statistically rigorous approach for estimating 2PCF of target subsets in large datasets, particularly valuable for astronomy applications where manual annotation is costly.
...
TL;DR: A responsible computing framework that uses synthetic protest imagery to analyze collective dynamics while reducing privacy risks from AI surveillance and cross-platform identity leakage.


<details>
  <summary>Details</summary>
Motivation: Protest-related social media data is high-risk due to surveillance, repression, and privacy concerns. AI systems can identify individuals, infer sensitive attributes, and enable cross-platform surveillance, posing risks to protesters and bystanders. Existing approaches lack integrated privacy risk assessment, downstream analysis, and fairness considerations.

Method: Proposes a responsible computing framework that replaces sensitive protest imagery with well-labeled synthetic reproductions using conditional image synthesis. This enables analysis of collective patterns without direct exposure of identifiable individuals, balancing analytical utility with privacy risk reduction.
...
Conclusion: Rather than offering absolute privacy guarantees, the method adopts a pragmatic, harm-mitigating approach that enables socially sensitive analysis while acknowledging residual risks, providing a holistic pipeline for responsible protest analysis.
...
TL;DR: COVER is a lightweight coverage-based view selection method that minimizes Fisher Information Gain approximation to select informative camera poses for 3D reconstruction, outperforming state-of-the-art active view selection methods.


<details>
  <summary>Details</summary>
Motivation: The quality of data used for 3D reconstruction is crucial for efficient and accurate scene modeling. Current view selection methods often require expensive computations and are sensitive to noise and training dynamics.

Method: Develops a principled analysis using Fisher Information Gain approximation, leading to COVER metric that favors viewpoints covering insufficiently observed geometry. Avoids expensive transmittance estimation and is robust to noise/training dynamics.

Result: Integrated into Nerfstudio framework and evaluated on real datasets in fixed and embodied acquisition scenarios. Consistently improves reconstruction quality across multiple datasets and radiance-field baselines compared to state-of-the-art methods.

Conclusion: COVER provides a simple, interpretable, and lightweight solution for active view selection that effectively improves 3D reconstruction quality by selecting informative camera poses based on coverage of insufficiently observed geometry.
...
TL;DR: Region-R1 improves MM-RAG re-ranking by dynamically cropping query images to focus on question-relevant regions, reducing visual distractors and boosting retrieval accuracy by up to 20%.


<details>
  <summary>Details</summary>
Motivation: Standard re-rankers in multi-modal retrieval-augmented generation process full query images as global embeddings, making them vulnerable to visual distractors like background clutter that skew similarity scores and reduce retrieval accuracy.

Method: Proposes Region-R1, a query-side region cropping framework that formulates region selection as a decision-making problem during re-ranking. Uses region-aware group relative policy optimization (r-GRPO) to learn a policy that dynamically crops discriminative regions, allowing the system to retain full images or focus on question-relevant regions before scoring retrieved candidates.

Result: Achieves state-of-the-art performance on E-VQA and InfoSeek benchmarks, increasing conditional Recall@1 by up to 20%. Shows consistent gains across challenging multi-modal retrieval tasks.

Conclusion: Query-side adaptation through dynamic region cropping is a simple but effective way to strengthen MM-RAG re-ranking, demonstrating great promise for improving multi-modal retrieval systems by focusing on relevant visual evidence.
...
TL;DR: UFPR-VeSV dataset addresses gaps in vehicle surveillance by providing 24,945 real-world images with validated FGVC annotations (13 colors, 26 makes, 136 models, 14 types) and exploring integration with ALPR.


<details>
  <summary>Details</summary>
Motivation: Existing vehicle surveillance studies assume controlled conditions, explore limited attributes, and overlook integration of Fine-Grained Vehicle Classification (FGVC) with Automatic License Plate Recognition (ALPR). Real-world applications need datasets capturing diverse conditions like occlusions, nighttime infrared, and varying lighting.

Method: Created UFPR-VeSV dataset with 24,945 images of 16,297 unique vehicles from Brazilian military police surveillance. Annotated 13 colors, 26 makes, 136 models, 14 types, validated using license plate info. Provided text and corner annotations. Conducted qualitative/quantitative comparisons with existing datasets and benchmarked with 5 deep learning models. Applied 2 OCR models for ALPR and explored joint FGVC-ALPR integration.

Result: Dataset confirmed as challenging with specific difficulties: multicolored vehicles, infrared images, distinguishing platform-sharing models. Benchmark models validated dataset difficulty. Joint FGVC-ALPR exploration showed potential for complementary task integration in real-world applications.

Conclusion: UFPR-VeSV addresses limitations of existing datasets by providing comprehensive real-world vehicle surveillance data with validated FGVC annotations. Integration of FGVC with ALPR shows promise for enhanced intelligent transportation systems. Dataset is publicly available for research.
...
TL;DR: Frozen visual embeddings like CLIP/DINOv2/v3/SSCD leak facial identity info; paper introduces benchmark and ISP method to remove identity subspace while preserving utility.


<details>
  <summary>Details</summary>
Motivation: Visual embeddings used in retrieval systems have unmeasured identity leakage when processing face data, lacking deployable privacy mitigations.

Method: Created benchmark with open-set verification, diffusion-based template inversion, and face-context attribution; proposed ISP (identity sanitization projection) - one-shot linear projector to remove identity subspace while preserving utility space.

Result: CLIP shows higher identity leakage than DINOv2/v3/SSCD; ISP achieves near-chance identity recognition while maintaining high non-biometric utility; transfers well across datasets.

Conclusion: First attacker-calibrated facial privacy audit of non-face-recognition encoders; linear subspace removal provides strong privacy guarantees while preserving visual search utility.
...
TL;DR: SmokeGS-R is a 3D reconstruction pipeline for smoke-affected scenes that separates geometry recovery from appearance correction, achieving state-of-the-art results on the NTIRE 2026 challenge.


<details>
  <summary>Details</summary>
Motivation: Real-world smoke causes three main problems for 3D reconstruction: it attenuates scene radiance, adds airlight, and destabilizes multi-view appearance consistency, making robust reconstruction particularly challenging.

Method: The pipeline decouples geometry recovery from appearance correction by: 1) generating physics-guided pseudo-clean supervision using refined dark channel prior and guided filtering, 2) training a sharp clean-only 3D Gaussian Splatting source model, and 3) harmonizing renderings with a donor ensemble using geometric-mean reference aggregation, LAB-space Reinhard transfer, and light Gaussian smoothing.

Result: On the official challenge testing leaderboard: PSNR=15.217, SSIM=0.666. After public release of RealX3D: PSNR=15.209, SSIM=0.644, LPIPS=0.551, outperforming the strongest official baseline by +3.68 dB PSNR on the same scenes.

Conclusion: A geometry-first reconstruction strategy combined with stable post-render appearance harmonization is an effective recipe for real-world multi-view smoke restoration.
...
TL;DR: 3D object detection and segmentation in Gaussian Splatting scenes using multi-view mask association with semantic-spatial codebook guidance


<details>
  <summary>Details</summary>
Motivation: Need for accurate object-level detection and segmentation of indoor assets in 3D Gaussian Splatting scenes reconstructed from drone imagery

Method: Introduces 3D object codebook leveraging mask semantics and spatial information of Gaussian primitives to guide multi-view mask association and detection; integrates 2D object detection/segmentation with semantically and spatially constrained merging

Result: 65% F1 score improvement over SOTA baselines for multi-view mask consistency; 11% mAP gain over baseline methods for 3D indoor asset detection on two large indoor scenes

Conclusion: Proposed approach enables reliable object-level detection and segmentation in 3DGS scenes with significant improvements over existing methods
...
TL;DR: VLA-InfoEntropy uses image entropy and attention entropy to dynamically focus on informative regions, reducing computational overhead while maintaining performance in Vision-Language-Action models.


<details>
  <summary>Details</summary>
Motivation: VLA models suffer from high computational overhead and low inference efficiency due to joint processing of high-dimensional visual features, complex linguistic inputs, and continuous action sequences, hindering real-time deployment and reliability.

Method: Proposes VLA-InfoEntropy method using image entropy to quantify grayscale distribution of visual tokens and attention entropy to capture attention score distribution over task-related text. Combines these with timestep information for dynamic transition strategy that shifts focus from global features to attention-guided local informative regions.
...
Conclusion: VLA-InfoEntropy effectively integrates spatial, semantic, and temporal cues to reduce redundancy while preserving critical content, addressing computational efficiency challenges in VLA models.
...
TL;DR: UMS is an unsupervised framework that simultaneously solves multi-agent and single-agent 3D perception by leveraging cooperative views without human annotations.


<details>
  <summary>Details</summary>
Motivation: There's no existing method that simultaneously solves both multi-agent and single-agent perception in an unsupervised way, despite promising LiDAR-based perception performance. The paper aims to leverage multi-agent cooperation without human annotations to address both tasks.

Method: UMS framework uses two key insights: 1) Improved point cloud density from data sharing benefits unsupervised object classification, 2) Cooperative views guide single-view detection. It includes Proposal Purifying Filter for better proposal classification, Progressive Proposal Stabilizing for reliable pseudo labels via curriculum learning, and Cross-View Consensus Learning to use multi-agent views to guide single-agent detection.
...
Conclusion: The proposed UMS framework successfully leverages multi-agent cooperation without human annotations to simultaneously solve both multi-agent and single-agent perception, demonstrating superior performance through density cooperation and cross-view guidance.
...
TL;DR: A multi-cue guided local feature learning framework that combines semantic, geometric, and depth cues to improve keypoint detection robustness and descriptor discriminability in computer vision.


<details>
  <summary>Details</summary>
Motivation: Existing local feature methods rely on single appearance cues, leading to unstable keypoints and insufficient descriptor discriminability. The paper aims to address these limitations by leveraging multiple complementary cues.
...
Result: Extensive experiments on four benchmarks validate the effectiveness of the proposed framework, showing improved robustness and discriminability compared to existing methods.

Conclusion: The multi-cue guided approach successfully enhances local feature learning by synergistically combining semantic and geometric cues, providing more robust keypoint detection and more discriminative descriptors for computer vision tasks.
...
TL;DR: SPIRE reformulates infrared small target detection as centroid regression using single-point supervision and probabilistic response encoding, achieving competitive performance with low false alarms and reduced computation.


<details>
  <summary>Details</summary>
Motivation: Traditional pixel-level supervision approaches for infrared small target detection (IRSTD) focus on separating all target regions but neglect that small targets occupy few pixels with blurred boundaries. The authors argue that IRSTD should prioritize target localization over exhaustive segmentation.

Method: SPIRE uses single-point supervision guided infrared probabilistic response encoding. It includes: 1) Point-Response Prior Supervision (PRPS) that transforms single-point annotations into probabilistic response maps matching infrared point-target characteristics, and 2) High-Resolution Probabilistic Encoder (HRPE) for encoder-only, end-to-end regression without decoder reconstruction.

Result: Extensive experiments on IRSTD benchmarks (SIRST-UAVB and SIRST4) show SPIRE achieves competitive target-level detection performance with consistently low false alarm rates and significantly reduced computational cost.

Conclusion: Reformulating IRSTD as a centroid regression task with single-point supervision and probabilistic response encoding provides an effective alternative to traditional segmentation approaches, offering better optimization stability under sparse target distributions while maintaining performance with lower computational requirements.
...
TL;DR: 3DTurboQuant enables near-lossless compression of 3D reconstruction models without per-scene training or codebook learning by leveraging random rotations and precomputed quantization.


<details>
  <summary>Details</summary>
Motivation: Existing 3D reconstruction compression methods require per-scene fine-tuning and data-dependent codebook learning, which is inefficient and time-consuming. The paper shows this is unnecessary by exploiting statistical properties of parameter vectors.

Method: Uses random rotations to transform parameter vectors into coordinates with known Beta distribution, enabling precomputed Lloyd-Max quantization. Develops dimension-dependent quantization criteria, norm-separation bounds, entry-grouping for hash grid features, and a composable pruning-quantization pipeline.

Result: Achieves 3.5x compression of 3DGS with only 0.02dB PSNR loss on NeRF Synthetic, and 7.9x compression of DUSt3R KV caches with 39.7dB pointmap fidelity. Compression takes seconds without training or calibration data.

Conclusion: Demonstrates that data-independent quantization with random rotations is near-optimal for 3D reconstruction models, achieving compression within 2.7x of information-theoretic lower bounds without per-scene optimization.
...
TL;DR: UAVReason is the first unified large-scale multimodal benchmark for nadir-view UAV scenarios, addressing domain shift challenges in vision-language models for aerial imagery with comprehensive VQA pairs and generation samples.


<details>
  <summary>Details</summary>
Motivation: Vision-language models fail on high-altitude UAVs due to domain shift from tiny/dense objects, repetitive textures, and ambiguous top-down orientations, which disrupt semantic grounding and hinder spatial reasoning and generation.

Method: Created UAVReason benchmark from high-fidelity UAV simulation with 273K VQA pairs (single frames, temporal sequences, generation samples) covering 22 reasoning types, and established unified baseline via multi-task learning.
...
Conclusion: UAVReason bridges the critical gap in UAV multimodal understanding, demonstrating that unified multi-task learning outperforms general VLMs, with all data/code/tools to be released to advance UAV research.
...
<details>
  <summary>Details</summary>
Motivation: OCT layer segmentation faces two main challenges: 1) annotation scarcity (limited labeled data), and 2) heterogeneous label granularities across different datasets. Existing semi-supervised methods assume fixed granularity and fail to exploit cross-granularity supervision effectively.

Method: Proposes LUMOS with two key components: 1) Dual-Decoder Network with Hierarchical Prompting Strategy (DDN-HPS) - uses dual-branch architecture with multi-granularity prompting to suppress pseudo-label noise propagation; 2) Reliable Progressive Multi-granularity Learning (RPML) - introduces region-level reliability weighing and progressive training from easier to more difficult tasks for stable cross-granularity alignment.

Result: Experiments on six OCT datasets show LUMOS largely outperforms existing methods and demonstrates exceptional cross-domain and cross-granularity generalization capability.

Conclusion: LUMOS effectively addresses the dual challenges of annotation scarcity and heterogeneous label granularities in OCT segmentation through its innovative architecture and progressive learning strategy, achieving superior performance and generalization across diverse datasets.
...
TL;DR: Proposes Object-Anchored Composed Image Retrieval (OACIR), a new fine-grained retrieval task requiring strict instance-level consistency, with a benchmark OACIRR and AdaFocal framework that outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Standard Composed Image Retrieval (CIR) prioritizes semantic matching but struggles with reliable instance-level retrieval across contexts. In practice, maintaining concrete instance fidelity is often more important than broad semantics.

Method: Introduces OACIR task with bounding box anchors for precise instance specification. Proposes AdaFocal framework with Context-Aware Attention Modulator that adaptively intensifies attention within the anchored instance region, balancing focus between instance and compositional context.

Result: AdaFocal substantially outperforms existing compositional retrieval models, particularly in maintaining instance-level fidelity. OACIRR benchmark contains over 160K quadruples with four challenging candidate galleries enriched with hard-negative instance distractors.

Conclusion: Establishes a robust baseline for the challenging OACIR task and opens new directions for more flexible, instance-aware retrieval systems that prioritize concrete instance preservation over broad semantic matching.
...
TL;DR: A weather-adaptive 3D object detection framework that dynamically routes between LiDAR, 4D radar, and fusion branches based on environmental conditions, achieving SOTA performance on adverse weather scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-4D radar fusion methods use fixed pipelines that fail to dynamically adjust modality preferences as weather conditions change, limiting robustness in adverse weather scenarios.

Method: Reformulates multi-modal perception as weather-conditioned branch routing with three parallel feature streams (LiDAR-only, radar-only, fusion). Uses a lightweight router with condition tokens to predict sample-specific aggregation weights, plus weather-supervised learning with classification and diversity regularization to prevent branch collapse.

Result: Achieves state-of-the-art performance on the K-Radar benchmark and provides explicit, interpretable insights into how adaptive routing shifts reliance between LiDAR and 4D radar across diverse adverse-weather scenarios.

Conclusion: The proposed weather-adaptive routing framework enables robust 3D object detection by dynamically adjusting modality preferences based on environmental conditions, offering both performance gains and interpretable insights into sensor reliability.
...
TL;DR: CRISP introduces a rank-based segmentation framework for medical imaging that addresses distribution shift by leveraging stable probability rankings under perturbation, achieving state-of-the-art robustness without target-domain information.


<details>
  <summary>Details</summary>
Motivation: Distribution shift in medical imaging causes severe performance degradation in unseen environments and exacerbates health inequities. Existing domain adaptation methods are limited by predefined simulated shifts or pseudo-supervision, which struggle with the infinite, unpredictable nature of real-world distribution shifts.

Method: CRISP uses the "Rank Stability of Positive Regions" principle where relative probability rankings for positive voxels remain stable under distribution shift. It simulates model behavior via latent feature perturbation, identifies stable high-probability (destined positives) and low-probability regions, constructs HP and HR priors, and iteratively refines them through progressive "squeezing" to final segmentation.

Result: CRISP significantly outperforms state-of-the-art methods across multi-center cardiac MRI and CT-based lung vessel segmentation, with HD95 reductions of up to 0.14 (7.0%), 1.90 (13.1%), and 8.39 (38.9%) pixels for multi-center, demographic, and modality shifts respectively.

Conclusion: CRISP provides a parameter-free, model-agnostic framework that addresses distribution shift through rank-based segmentation rather than probability-based approaches, demonstrating superior robustness without requiring target-domain information.
...
TL;DR: SGPer is a framework that synergizes DINOv2's semantic priors with SAM's geometric priors for robust wheat disease segmentation under limited data, handling temporal appearance variations effectively.


<details>
  <summary>Details</summary>
Motivation: Wheat disease segmentation faces challenges from significant intra-class temporal variations across growth stages, making representative dataset collection labor-intensive and impractical. There's a need for robust segmentation methods that work with limited data while handling appearance shifts.

Method: SGPer treats wheat disease segmentation as a coupled task of disease-specific semantic perception (using DINOv2) and disease boundary localization (using SAM). It uses disease-sensitive adapters with disease-friendly filters inserted into both models, transforms DINOv2 features into dense category-specific point prompts, dynamically filters redundant prompts using SAM's mask confidence and DINOv2's semantic consistency, and distills informative prompts to activate SAM's geometric priors.

Result: SGPer consistently achieves state-of-the-art performance on wheat disease and organ segmentation benchmarks, especially in data-constrained scenarios, demonstrating robust segmentation that remains invariant to temporal appearance changes.

Conclusion: The proposed Semantic-Geometric Prior Synergization framework effectively addresses wheat disease segmentation challenges by leveraging pretrained models' complementary strengths, providing a practical solution for precision agriculture with limited data.
...
TL;DR: VideoStir is a structured, intent-aware RAG framework for long videos that uses spatio-temporal graphs and multi-hop retrieval to overcome limitations of flattened semantic matching in existing methods.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with long videos due to limited context windows. Existing RAG methods flatten videos into independent segments (breaking spatio-temporal structure) and rely on explicit semantic matching (missing implicitly relevant cues).

Method: 1) Structures videos as spatio-temporal graphs at clip level, 2) Performs multi-hop retrieval to aggregate evidence across distant but related events, 3) Introduces MLLM-backed intent-relevance scorer to retrieve frames based on query's reasoning intent, 4) Curates IR-600K dataset for learning frame-query intent alignment.
...
Conclusion: The framework successfully shifts long-video RAG from flattened semantic matching to structured, intent-aware reasoning, addressing key limitations in current approaches for scaling MLLMs to long videos.
...
TL;DR: CSAP is a lightweight semantic segmentation decoder that computes attention only at the deepest feature scale and propagates it to shallower stages, reducing redundancy and computational cost while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Current multi-scale decoders compute attention independently at each feature scale, creating substantial redundancy since attention distributions across scales are strongly correlated, leading to inefficient computation.
...
Conclusion: CSAP preserves multi-scale contextual reasoning while substantially reducing computational cost, offering an efficient decoder design for lightweight semantic segmentation.
...
TL;DR: FSS-SAM3 achieves state-of-the-art few-shot segmentation using frozen SAM3 with simple spatial concatenation of support/query images, outperforming heavily engineered methods without training.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot segmentation methods require extensive episodic training that is computationally demanding and sensitive to distribution shifts. The paper explores using modern vision foundation models (SAM3) as a training-free solution.

Method: Repurposes SAM3's Promptable Concept Segmentation capability with a simple spatial concatenation strategy that places support and query images into a shared canvas, using fully frozen SAM3 without fine-tuning or architectural changes.

Result: Achieves state-of-the-art performance on PASCAL-5^i and COCO-20^i benchmarks, outperforming many heavily engineered methods. Also discovers that negative prompts can be counterproductive in few-shot settings, weakening target representations.
...
TL;DR: HUG3D reconstructs textured 3D human models from single images of multi-human scenes by modeling group context and interaction priors to resolve occlusions and physical plausibility.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail in multi-human scenes where naive composition of individual reconstructions leads to artifacts like unrealistic overlaps, missing geometry in occluded regions, and distorted interactions. There's a need for approaches that incorporate group-level context and interaction priors.

Method: 1) Transform input to canonical orthographic space to mitigate perspective distortions; 2) Human Group-Instance Multi-View Diffusion (HUG-MVD) generates complete multi-view normals and images by jointly modeling individuals and group context; 3) Human Group-Instance Geometric Reconstruction (HUG-GR) optimizes geometry using physics-based interaction priors for physical plausibility; 4) Fuse multi-view images into high-fidelity texture.
...
Conclusion: HUG3D provides a holistic framework for reconstructing textured 3D human models from single images of multi-human scenes by explicitly modeling group- and instance-level information with physics-based interaction priors, addressing key limitations of existing approaches.
...
TL;DR: GameAD: A risk-prioritized game planning framework for end-to-end autonomous driving that treats driving as a multi-agent game with risk-aware decision making, outperforming SOTA methods on safety metrics.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving models treat all agents equally, failing to distinguish real collision threats from complex backgrounds. The paper argues that autonomous driving is fundamentally a dynamic multi-agent game problem that requires risk-aware prioritization rather than simple perception-planning integration.

Method: Proposes GameAD framework with four key components: 1) Risk-Aware Topology Anchoring, 2) Strategic Payload Adapter, 3) Minimax Risk-Aware Sparse Attention, and 4) Risk Consistent Equilibrium Stabilization. Also introduces Planning Risk Exposure metric to quantify cumulative risk over long horizons.
...
Conclusion: Autonomous driving should be modeled as a risk-prioritized multi-agent game rather than simple perception-planning integration. The proposed GameAD framework enables effective risk-aware decision making and demonstrates superior safety performance compared to existing approaches.
...
TL;DR: A pipeline for generating synthetic eye movement videos using 3D simulation and real iris trajectories, applied to script-reading detection with a 12-hour dataset.


<details>
  <summary>Details</summary>
Motivation: Behavioral data like eye movements is scarce, expensive to annotate, and privacy-sensitive, creating a need for synthetic alternatives to real data collection.

Method: Extract real human iris trajectories from reference videos and replay them on a 3D eye movement simulator via headless browser automation to generate synthetic labeled eye movement videos.

Result: Created final_dataset_v1 with 144 sessions (12 hours) for script-reading detection; generated trajectories preserve source temporal dynamics (KS D < 0.14); simulator shows bounded sensitivity at reading-scale movements due to lack of coupled head movement.

Conclusion: The pipeline enables scalable synthetic behavioral data generation, supporting downstream classifier development at the intersection of behavioral modeling and vision-language systems.
...
TL;DR: Novel canine pneumothorax diagnosis combining VLM-guided flow matching for precise lesion segmentation with RMT-based statistical detection of pathological signals.


<details>
  <summary>Details</summary>
Motivation: Address data scarcity and need for trustworthy models in canine pneumothorax diagnosis by creating public dataset and developing interpretable diagnostic system.

Method: Two-stage approach: 1) VLM-guided iterative Flow Matching for precise lesion segmentation, 2) RMT-based detection analyzing isolated lesion features as statistical outliers from healthy tissue noise.

Result: Developed highly accurate and interpretable diagnostic system with superior boundary accuracy in segmentation and sensitive detection of pathological signals.

Conclusion: Synergy of generative segmentation and first-principles statistical analysis yields trustworthy canine pneumothorax diagnosis, with public dataset and code available.
...
TL;DR: WSA-Net: A lightweight GPR detector that enhances faint subsurface defect signals through physical-feature reconstruction, achieving high accuracy (0.6958 mAP@0.5) and speed (164 FPS) with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Existing lightweight GPR detectors fail to detect "weak signals" - faint diffraction hyperbolas with low signal-to-clutter ratios, high wavefield similarity, and geometric degradation. They prioritize efficiency over sensitivity, lacking mechanisms to preserve low-frequency structures or decouple heterogeneous clutter.

Method: WSA-Net integrates four mechanisms: 1) Signal preservation using partial convolutions, 2) Clutter suppression via heterogeneous grouping attention, 3) Geometric reconstruction to sharpen hyperbolic arcs, and 4) Context anchoring to resolve semantic ambiguities. It moves beyond simple parameter reduction to focus on physical-feature reconstruction.

Result: On the RTST dataset, WSA-Net achieves 0.6958 mAP@0.5 and 164 FPS with only 2.412 M parameters, demonstrating superior performance in detecting faint subsurface defects while maintaining efficiency.

Conclusion: Signal-centric awareness in lightweight architectures effectively reduces false negatives in infrastructure inspection, proving that focusing on physical-feature reconstruction rather than just parameter reduction enables better detection of weak GPR signals.
...
TL;DR: Action Images: A unified world action model that formulates policy learning as multiview video generation by translating 7-DoF robot actions into interpretable, pixel-grounded action images, enabling video backbones to act as zero-shot policies.


<details>
  <summary>Details</summary>
Motivation: Existing world action models (WAMs) often rely on separate action modules or use action representations that are not pixel-grounded, making it difficult to fully exploit pretrained video model knowledge and limiting transfer across viewpoints and environments.

Method: Translate 7-DoF robot actions into interpretable action images - multi-view action videos that are grounded in 2D pixels and explicitly track robot-arm motion. This allows video backbones to act as zero-shot policies without separate policy heads.
...
Conclusion: Interpretable action images are a promising route to policy learning, enabling unified models that support multiple tasks (control, video-action joint generation, action-conditioned video generation, action labeling) under a shared representation.
...
TL;DR: A unified framework for nighttime image dehazing that integrates domain-aligned data construction, stage-wise training, and inference-time enhancement, achieving practical results without complex network redesign.


<details>
  <summary>Details</summary>
Motivation: Nighttime image dehazing faces complex degradation patterns (haze scattering + low illumination + non-uniform lighting + strong light interference) with limited supervision, causing domain drift and training instability due to scarce target-domain samples and distribution mismatch with external data.

Method: 1) Use pre-trained CLIP visual encoder to screen external samples by similarity for domain-aligned data construction; 2) Two-stage NAFNet training (target domain adaptation then broader degradation patterns); 3) Inference-time enhancement with TLC, x8 self-ensemble, and weighted snapshot fusion.

Result: The framework offers a practical and effective pipeline for nighttime image dehazing, addressing the NTIRE 2026 Night Time Image Dehazing Challenge without relying on complex network redesign.

Conclusion: Rather than complex network redesign, the proposed unified framework provides a practical solution for nighttime image dehazing through integrated domain-aligned data construction, stage-wise training, and inference-time enhancement.
...
TL;DR: ContrAR is a new benchmark for testing vision-language models' ability to detect contradictory virtual content attacks in augmented reality environments.


<details>
  <summary>Details</summary>
Motivation: As AR becomes more integrated into daily life, security threats like contradictory virtual content attacks (where malicious/inconsistent virtual elements mislead users) pose unique risks that need systematic evaluation.

Method: Systematically model contradictory virtual content attacks and create ContrAR benchmark containing 312 real-world AR videos validated by 10 human participants. Benchmark 11 VLMs (commercial and open-source).

Result: Current VLMs show reasonable understanding of contradictory virtual content but need improvement in detecting/reasoning about adversarial manipulations. Balancing detection accuracy and latency remains challenging.

Conclusion: The ContrAR benchmark reveals gaps in VLM robustness against AR content manipulation, highlighting the need for better detection capabilities and accuracy-latency tradeoffs for secure AR systems.
...
TL;DR: GCNV-Net is a novel 3D medical segmentation framework that achieves state-of-the-art performance with improved computational efficiency through tri-directional dynamic nonvoid voxel transformer, geometric cross-attention, and nonvoid voxelization.


<details>
  <summary>Details</summary>
Motivation: Existing 3D medical segmentation methods struggle to balance high accuracy and computational efficiency across diverse anatomies and imaging modalities, creating barriers for clinical deployment.

Method: GCNV-Net integrates three key components: 1) Tri-directional Dynamic Nonvoid Voxel Transformer (3DNVT) that partitions voxels along three anatomical planes, 2) Geometrical Cross-Attention module (GCA) for multi-scale feature fusion with geometric information, and 3) Nonvoid Voxelization that processes only informative regions to reduce computation.

Result: Achieves state-of-the-art performance across BraTS2021, ACDC, MSD Prostate, MSD Pancreas, and AMOS2022 benchmarks, outperforming existing methods by 0.65% Dice, 0.63% IoU, 1% NSD, and 14.5% HD95. Reduces FLOPs by 56.13% and inference latency by 68.49% compared to conventional voxelization.

Conclusion: GCNV-Net effectively balances accuracy and efficiency, demonstrating strong robustness across diverse organs, disease conditions, and imaging modalities with significant potential for clinical deployment.
...
TL;DR: CR-Diff improves diffusion model performance at unseen resolutions by pruning adverse weights and amplifying pruned outputs, maintaining quality at default resolutions while enhancing cross-resolution consistency.


<details>
  <summary>Details</summary>
Motivation: Diffusion models trained at fixed resolutions degrade when generating images at out-of-training resolutions due to resolution-dependent parameter behaviors where weights effective at default resolutions become adverse at different scales, weakening semantic alignment and causing structural instability.

Method: Two-stage approach: 1) Block-wise pruning to selectively eliminate adverse weights that cause resolution-dependent issues, 2) Pruned output amplification to further purify the pruned predictions and enhance visual consistency.
...
Conclusion: CR-Diff effectively addresses the cross-resolution consistency problem in diffusion models through parameter pruning and output amplification, offering a practical solution for generating high-quality images at diverse resolutions without retraining.
...
TL;DR: STSF-Net is a multimodal change detection framework for optical-SAR images that jointly models modality-specific and spatio-temporal features to enhance change representations and suppress pseudo-changes, achieving SOTA performance on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal change detection approaches have limitations in cross-modal interaction and exploiting modality-specific characteristics, leading to insufficient modeling of fine-grained change information and hindering precise semantic change detection in multimodal remote sensing data.

Method: Proposes STSF-Net framework that: 1) jointly models modality-specific features to capture genuine semantic changes and spatio-temporal common features to suppress pseudo-changes, 2) introduces semantic-guided adaptive fusion using pre-trained foundational models to adjust feature importance, and 3) creates Delta-SN6 dataset - first openly-accessible multiclass MMCD benchmark with VHR fully polarimetric SAR and optical images.

Result: Outperforms state-of-the-art methods by 3.21%, 1.08%, and 1.32% in mIoU on Delta-SN6, BRIGHT, and Wuhan-Het datasets respectively, demonstrating superior performance in multimodal change detection.

Conclusion: STSF-Net effectively addresses limitations in multimodal change detection by jointly modeling modality-specific and spatio-temporal features with semantic-guided adaptive fusion, achieving improved change detection accuracy and providing a valuable benchmark dataset for future research.
...
TL;DR: EchoAgent is an agentic system that coordinates eyes, hands, and minds for end-to-end echocardiography interpretation, achieving 80% accuracy by integrating knowledge assimilation, multimodal perception, and explainable reasoning.


<details>
  <summary>Details</summary>
Motivation: Current AI approaches for echocardiography analysis are limited to specific skills (either eyes-hands for segmentation/measurement or eyes-minds for reasoning), lacking the coordinated workflow that clinicians use. This fragmentation limits clinical reliability and utility.

Method: Three-component system: 1) Expertise-driven cognition engine that assimilates Echo guidelines into structured knowledge base; 2) Hierarchical collaboration toolkit for parsing videos, identifying views, segmentation, and measurement; 3) Orchestrated reasoning hub that integrates multimodal evidence with knowledge base for explainable inference.

Result: Achieved optimal performance across diverse structure analyses on CAMUS and MIMIC-EchoQA datasets covering 48 echocardiographic views and 14 cardiac anatomical regions, with overall accuracy up to 80.00%.

Conclusion: EchoAgent successfully coordinates eyes-hands-minds workflow like a cardiac sonographer, showing promise for reliable echocardiography interpretation by enabling a single system to learn, observe, operate, and reason comprehensively.
...
TL;DR: ProMMA framework addresses missing modality problem in multimodal sentiment analysis using prompt learning with missing modality evaluation, prompt disentanglement, dynamic weighting, and global connection modules.


<details>
  <summary>Details</summary>
Motivation: Missing modalities in real-world multimodal sentiment analysis degrade model accuracy and generalization. Existing approaches have limitations: lack rigorous evaluation of missing modality generation necessity, and insufficient exploration of structural dependencies among multimodal prompts and their global coherence.

Method: Proposes Prompt-based Missing Modality Adaptation (ProMMA) framework with four key components: 1) Missing Modality Evaluator to dynamically assess importance of missing modalities using pretrained models and pseudo labels, 2) Modality-invariant Prompt Disentanglement module to decompose shared prompts into modality-specific private prompts, 3) Dynamic Prompt Weighting module to compute mutual information-based weights from cross attention outputs, 4) Multi-level Prompt Dynamic Connection module to integrate shared prompts with self-attention outputs through residual connections.

Result: Extensive experiments on three public benchmarks (CMU MOSI, CMU MOSEI, and CH-SIMS) demonstrate state-of-the-art performance and stable results under diverse missing modality settings.

Conclusion: The ProMMA framework effectively addresses missing modality challenges in multimodal sentiment analysis through comprehensive prompt adaptation mechanisms, achieving superior robustness and performance compared to existing approaches.
...
TL;DR: SpecMamba: A parameter-efficient frequency-aware framework for few-shot hyperspectral target detection using DCT-Mamba adapters and spectral priors.


<details>
  <summary>Details</summary>
Motivation: Existing meta-learning methods for hyperspectral target detection face challenges: full-parameter fine-tuning is inefficient and prone to overfitting, and they ignore frequency-domain structure and spectral band continuity, limiting spectral adaptation and cross-domain generalization.

Method: 1) DCT-Mamba Adapter: Projects spectral features into frequency domain via DCT and uses Mamba's linear-complexity state-space recursion to capture global spectral dependencies. 2) Prior-Guided Tri-Encoder: Incorporates laboratory spectral priors to guide adapter optimization without disrupting stable semantic features. 3) Self-Supervised Pseudo-Label Mapping: Test-time adaptation strategy with uncertainty-aware sampling and dual-path consistency constraints.
...
Conclusion: SpecMamba provides an effective parameter-efficient framework for few-shot hyperspectral target detection that addresses spectral adaptation challenges through frequency-aware processing and prior-guided optimization.
...
TL;DR: EasyPolar: Multi-view polarimetric imaging system using three synchronized RGB cameras to capture high-quality polarization data without sacrificing spatial resolution.


<details>
  <summary>Details</summary>
Motivation: Existing DoFP polarization sensors suffer from reduced spatial resolution and artifacts due to spatial multiplexing. There's a need for single-shot polarization capture without these limitations for practical applications.

Method: Triple-camera setup with three synchronized RGB cameras: one unpolarized view and two polarized views with distinct orientations. Uses confidence-guided polarization reconstruction network with multi-modal feature fusion and confidence-aware physical guidance to handle misalignment.

Result: Achieves high-quality polarization reconstruction results and benefits various downstream tasks compared to existing methods.

Conclusion: EasyPolar provides a practical solution for high-quality polarimetric imaging by leveraging multi-view capture and advanced reconstruction algorithms, overcoming limitations of traditional DoFP sensors while maintaining snapshot capability.
...
<details>
  <summary>Details</summary>
Motivation: Current CIR methods based on fine-tuning vision-language pre-trained models suffer from severe overfitting, especially with limited triplet data, creating a significant generalization gap that needs to be addressed.

Method: WRF4CIR applies adversarial perturbations to model weights during fine-tuning, generated in the opposite direction of gradient descent, making training data fitting more difficult to regularize against overfitting.
...
TL;DR: PTA is a "Purify-then-Align" framework that addresses missing modalities in multimodal sensing by first purifying noisy modalities via meta-learning weighting, then aligning modalities through diffusion-based knowledge distillation.


<details>
  <summary>Details</summary>
Motivation: Robust multimodal sensing faces challenges from missing modalities, specifically the Representation Gap between heterogeneous data and the Contamination Effect from low-quality modalities, which are causally linked and impede representation alignment.

Method: PTA uses a two-stage approach: 1) Meta-learning-driven weighting to dynamically down-weight noisy modalities and purify the knowledge source, 2) Diffusion-based knowledge distillation where a clean teacher (from purified consensus) refines each student modality's features.

Result: PTA achieves state-of-the-art performance on MM-Fi and XRF55 datasets under pronounced Representation Gap and Contamination Effect, significantly improving single-modality model robustness in diverse missing-modality scenarios.

Conclusion: The "Purify-then-Align" strategy creates powerful single-modality encoders with cross-modal knowledge, effectively solving the causal dependency between representation gaps and contamination effects in multimodal human sensing.
...
TL;DR: BPC-Net: Boundary probability calibration framework for annotation-free skin lesion segmentation using Gaussian Probability Smoothing to recover under-confident lesion boundaries without manual masks.


<details>
  <summary>Details</summary>
Motivation: Current annotation-free skin lesion segmentation methods face three coupled challenges: noisy pseudo-label supervision, unstable transfer under limited target-domain data, and boundary probability under-confidence. While most methods focus on pseudo-label denoising, the effect of compressed boundary probabilities on final mask quality has received less attention, despite directly affecting contour completeness.

Method: Proposes BPC-Net with Gaussian Probability Smoothing (GPS) for localized probability-space calibration before thresholding to recover under-confident lesion boundaries. Includes feature-decoupled decoder for context suppression, detail recovery, and boundary refinement, and interaction-branch adaptation strategy that updates only pseudo-label interaction branch while preserving deployed image-only segmentation path.
...
Conclusion: BPC-Net effectively addresses boundary probability under-confidence in annotation-free skin lesion segmentation through probability-space calibration, achieving competitive performance without manual masks while maintaining practical deployment considerations.
...
TL;DR: ID-Selection: A visual token pruning method for LVLMs that balances importance and diversity through iterative selection with similarity suppression, achieving 97.2% token reduction while preserving 91.8% performance.


<details>
  <summary>Details</summary>
Motivation: Existing visual token pruning methods for LVLMs struggle to balance token importance and diversity - importance-based methods retain redundant tokens while diversity-based methods may overlook informative ones, especially problematic under high reduction ratios.
...
Result: Achieves 97.2% visual token pruning (retaining only 16 tokens) on LLaVA-1.5-7B, reducing inference FLOPs by over 97% while preserving 91.8% of original performance, without additional training. Validated across 5 LVLM backbones and 16 benchmarks.

Conclusion: ID-Selection provides an effective token selection strategy that balances importance and diversity, enabling efficient LVLM inference with minimal performance degradation, especially effective under extreme pruning ratios.
...
TL;DR: StyleMixDG: A systematic study of style transfer for domain generalization shows diverse artistic styles outperform domain-aligned ones, leading to a lightweight augmentation method that improves Sim2Real generalization.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for computer vision suffer from poor generalization when deployed in real-world settings, especially when trained on synthetic data (Sim2Real gap). Prior work on style transfer for domain generalization contains unresolved contradictions about key design choices.

Method: Systematic empirical study isolating three key factors: style pool diversity, texture complexity, and style source choice. Based on findings, developed StyleMixDG - a lightweight, model-agnostic augmentation recipe requiring no architectural modifications or additional losses.

Result: Findings show: (1) expanding style pool yields larger gains than repeated augmentation with few styles, (2) texture complexity has no significant effect when pool is sufficiently large, (3) diverse artistic styles outperform domain-aligned alternatives. StyleMixDG demonstrates consistent improvements on GTAV → {BDD100k, Cityscapes, Mapillary Vistas} benchmark.

Conclusion: The systematic study resolves inconsistencies in prior work and provides empirically validated design principles for style transfer in domain generalization. StyleMixDG translates these principles into practical gains for Sim2Real generalization without requiring model modifications.
...
TL;DR: STGR framework combines LLM reasoning with vision foundation models for precise language-guided lung lesion segmentation using graph reasoning and minimal parameter fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal models struggle with semantic ambiguity in clinical reports and anatomical overlaps in low-contrast scans, while full fine-tuning causes overfitting on limited medical data.

Method: Semantic-Topological Graph Reasoning framework with Text-to-Vision Intent Distillation, dynamic graph reasoning for mask selection, and Selective Asymmetric Fine-Tuning updating <1% parameters.

Result: Achieves 81.5% DSC on LIDC-IDRI, outperforming LISA by >5%, with exceptional cross-fold stability (0.6% DSC variance) through 5-fold cross-validation.

Conclusion: STGR framework enables robust, context-aware clinical deployment by synergizing LLM reasoning with vision models while preventing overfitting through efficient parameter fine-tuning.
...
TL;DR: FunRec reconstructs functional 3D digital twins from egocentric RGB-D videos, automatically discovering articulated parts and creating simulation-ready meshes without controlled setups or CAD priors.


<details>
  <summary>Details</summary>
Motivation: Existing articulated reconstruction methods require controlled setups, multi-state captures, or CAD priors, limiting their applicability to in-the-wild human interaction sequences.

Method: Operates directly on egocentric RGB-D interaction videos to automatically discover articulated parts, estimate kinematic parameters, track 3D motion, and reconstruct static/moving geometry in canonical space.

Result: Outperforms prior work by large margins: +50 mIoU improvement in part segmentation, 5-10x lower articulation and pose errors, and significantly higher reconstruction accuracy on real and simulated benchmarks.

Conclusion: FunRec enables practical reconstruction of functional 3D digital twins from in-the-wild interaction videos, with applications in simulation, affordance mapping, and robot-scene interaction.
...
TL;DR: DetailVerifyBench is a new benchmark for evaluating MLLMs' ability to detect and localize hallucinations in long image captions, featuring 1,000 images across 5 domains with token-level annotations.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack the fine granularity and domain diversity needed to evaluate MLLMs' capability to pinpoint specific hallucinations within long, comprehensive image captions that can span hundreds of words.

Method: Created DetailVerifyBench with 1,000 high-quality images across five distinct domains, featuring captions averaging over 200 words with dense, token-level annotations of multiple hallucination types.

Result: The benchmark stands as the most challenging benchmark for precise hallucination localization in long image captioning to date, addressing the gap in existing evaluation methods.

Conclusion: DetailVerifyBench provides a rigorous evaluation framework for assessing MLLMs' hallucination detection and localization capabilities in comprehensive image narratives, advancing reliability in long image captioning.
...
TL;DR: LLaRS is the first unified foundation model for multi-modal, multi-task remote sensing low-level vision that handles various degradations (clouds, haze, noise, etc.) using language conditioning and optimal transport alignment.


<details>
  <summary>Details</summary>
Motivation: Remote sensing imagery suffers from multiple degradations (clouds, haze, noise, resolution limits, sensor heterogeneity), but existing approaches require separate models per degradation type, lacking a unified solution.

Method: Uses Sinkhorn-Knopp optimal transport to align heterogeneous bands into semantically matched slots, features three complementary mixture-of-experts layers (convolutional for spatial patterns, channel-mixing for spectral fidelity, attention with LoRA for global context), and stabilizes training with step-level dynamic weight adjustment.

Result: LLaRS consistently outperforms seven competitive models across eleven restoration tasks, and parameter-efficient finetuning demonstrates strong transfer capability and adaptation efficiency on unseen data.

Conclusion: LLaRS presents the first unified foundation model for remote sensing low-level vision that effectively handles multiple degradations and tasks through language conditioning and innovative architectural design, showing superior performance and transfer capabilities.
...
Method: SGANet uses three components: 1) Selective Cross-view Feature Refinement Module for cross-view feature interaction, 2) Semantic-Structural Patch Alignment for semantic alignment across modalities with structural consistency, and 3) Multi-View Geometric Alignment for geometric correspondence across viewpoints.
...
Conclusion: SGANet effectively addresses feature inconsistency in multimodal multi-view anomaly detection through joint modeling of feature interaction, semantic/structural consistency, and geometric correspondence, demonstrating superior performance in industrial applications.
...
TL;DR: Monocular broadcast video can extract player kinematics for fatigue analysis using Game State Reconstruction and novel processing, but is sensitive to trajectory noise and calibration errors.


<details>
  <summary>Details</summary>
Motivation: Current fatigue monitoring in football relies on subjective self-reports, lab biomarkers, or intrusive sensors; this paper explores whether low-cost monocular broadcast videos can provide sufficient quality signals for fatigue analysis.

Method: Uses Game State Reconstruction to extract player trajectories, proposes novel kinematics processing algorithm for consistent speed/acceleration estimates, constructs acceleration-speed profiles, and evaluates on SoccerNet-GSR benchmark with 30-second clips and 45-minute halves.

Result: Monocular GSR can recover kinematic patterns compatible with A-S profiling for fatigue analysis, but reveals sensitivity to trajectory noise, calibration errors, and temporal discontinuities inherent to broadcast footage.

Conclusion: Monocular broadcast video provides a low-cost basis for fatigue analysis in football, but methodological challenges around noise and calibration need to be addressed in future research.
...
TL;DR: PanopticQuery is a framework for unified query-time reasoning in 4D scenes that combines 4D Gaussian Splatting with multi-view semantic consensus to ground natural language queries in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 4D scene understanding lack robust contextual reasoning for complex semantics like interactions, temporal actions, and spatial relations, and struggle with transforming noisy, view-dependent predictions into globally consistent 4D interpretations.

Method: Builds on 4D Gaussian Splatting for dynamic reconstruction, introduces multi-view semantic consensus mechanism that aggregates 2D semantic predictions across views and time, filters inconsistencies, enforces geometric consistency, and lifts 2D semantics into structured 4D groundings via neural field optimization.
...
Conclusion: PanopticQuery provides a unified framework for query-time reasoning in 4D scenes that addresses limitations of existing methods by combining high-fidelity reconstruction with robust semantic grounding across space, time, and viewpoints.
...
TL;DR: RATNet is a foundation model for gastrointestinal endoscopy imaging that uses analogical reasoning to transfer knowledge from heterogeneous annotations across multiple datasets, outperforming existing models in various diagnostic scenarios.


<details>
  <summary>Details</summary>
Motivation: Current AI-assisted endoscopic diagnosis suffers from poor generalizability, adaptability, robustness, and scalability due to limited medical data, domain shift, and heterogeneous annotations across different medical sites and datasets.

Method: RATNet uses a cyclic pre-training strategy with an encoder, relevance-knowledge acquisition and transfer (RAT) module, projector, and multi-task head. It employs analogical reasoning to match image-derived posterior knowledge to a learned prior knowledge base and transfers relative knowledge to guide diagnosis.

Result: RATNet outperforms existing foundation models (GastroNet, GastroVision) across six scenarios: common disease diagnosis, few-shot learning for rare diseases, zero-shot transfer to new sites, robustness under long-tailed distributions, adaptation to novel diseases, and privacy-preserving federated learning deployment.

Conclusion: RATNet provides an open, cost-effective foundation model that supports automatic integration of heterogeneous annotations without manual label unification, reduces data acquisition costs, and offers practical benefits for intelligent gastrointestinal diagnosis, especially in resource-limited settings.
...
TL;DR: TaCo framework maps 30 medical vision tasks from 39 datasets into shared embedding space to reveal intrinsic task relationships and structure.


<details>
  <summary>Details</summary>
Motivation: Medical computer vision focuses on task performance but lacks understanding of how different tasks relate, overlap, or differ at representational level across diverse imaging modalities.

Method: Task-Contrastive Learning (TaCo) - contrastive learning framework that embeds 30 heterogeneous medical vision tasks (segmentation, detection, denoising, inpainting, colorization, geometric transformations) from 39 datasets across CT, MRI, EM, X-ray, ultrasound into joint representation space.

Result: TaCo successfully maps tasks into shared space, revealing which tasks are distinctly represented vs. blended together, and how iterative task alterations are reflected in embeddings, uncovering intrinsic task structure.

Conclusion: Provides foundation for understanding intrinsic structure of medical vision tasks, offering deeper insights into task similarities and interconnected properties in embedding spaces beyond just performance metrics.
...
TL;DR: SnapFlow compresses multi-step flow-matching VLA models into single-step inference via self-distillation, achieving near-identical performance with 9.6x speedup.


<details>
  <summary>Details</summary>
Motivation: Current VLA models using iterative denoising (typically 10 steps) suffer from high latency where denoising alone accounts for 80% of inference time, but naive step reduction degrades performance due to uncalibrated velocity fields.

Method: Plug-and-play self-distillation method that mixes standard flow-matching samples with consistency samples using two-step Euler shortcut velocities computed from the model's own marginal predictions, avoiding trajectory drift. Uses zero-initialized target-time embedding to enable switching between local velocity estimation and global one-step generation.

Result: On pi0.5 (3B): 98.75% average success across 40 tasks (matching 10-step teacher's 97.75%), with 9.6x denoising speedup and end-to-end latency reduced from 274ms to 83ms. On SmolVLA (500M): reduces MSE by 8.3% with 3.56x end-to-end acceleration. Maintains advantage across execution horizons.

Conclusion: SnapFlow enables efficient single-step inference for flow-matching VLAs without architecture changes or external teachers, achieving state-of-the-art performance with significant speedups, and is orthogonal to other acceleration techniques.
...
TL;DR: A framework combining image enhancement (Nano-Banana-Pro) and 3D Gaussian Splatting (Smoke-GS) for reconstructing 3D scenes from smoke-degraded multi-view images, enabling clear novel view synthesis in smoke environments.


<details>
  <summary>Details</summary>
Motivation: Smoke introduces strong scattering effects, view-dependent appearance changes, and severe degradation of cross-view consistency, making 3D scene reconstruction from smoke-degraded multi-view images particularly difficult.
...
Result: The method preserves rendering efficiency while improving robustness to smoke-induced degradation, generating consistent and visually clear novel views in challenging smoke environments.

Conclusion: The proposed framework effectively addresses smoke-induced challenges in 3D scene reconstruction and enables restoration-oriented novel view synthesis in smoke environments.
...
TL;DR: CRFT is a coarse-to-fine transformer framework for cross-modal image registration that learns modality-independent feature flow through iterative discrepancy-guided attention and spatial geometric transforms.


<details>
  <summary>Details</summary>
Motivation: Need for robust cross-modal image registration that can handle large affine/scale variations while maintaining structural coherence across different imaging modalities.

Method: Unified coarse-to-fine framework with transformer architecture: coarse stage establishes global correspondences via multi-scale feature correlation; fine stage refines local details via hierarchical feature fusion and adaptive spatial reasoning. Uses iterative discrepancy-guided attention with Spatial Geometric Transform (SGT) to recurrently refine flow fields.

Result: CRFT consistently outperforms state-of-the-art registration methods in accuracy and robustness across diverse cross-modal datasets.

Conclusion: CRFT provides an effective generalizable paradigm for multimodal spatial correspondence with broad applicability to remote sensing, autonomous navigation, and medical imaging.
...
TL;DR: GUIDE introduces a progressive geometric priors injection framework that unrolls multi-level geometric features into MLLM early layers, enabling better 2D-to-3D transitional learning and spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack physical spatial awareness when processing real-world visual streams, and existing geometry-aware MLLMs suffer from flattened fusion that loses local geometric details and causes semantic mismatches in early layers.

Method: GUIDE performs multi-level sampling within the geometric encoder to capture features from local edges to global topologies, then aligns and fuses these geometric priors step-by-step with MLLM early layers. It also uses context-aware gating to fetch relevant spatial cues based on current semantics.
...
Conclusion: GUIDE establishes a novel paradigm for integrating 3D geometric priors into large models, breaking the bottleneck of single deep-layer extraction and input-level fusion in existing geometry-aware MLLMs.
...
TL;DR: Using monocular depth priors to enhance 3D Gaussian Splatting by addressing scale ambiguity and inconsistency issues through selective regularization.


<details>
  <summary>Details</summary>
Motivation: Accurate depth priors improve 3D Gaussian Splatting but require specialized systems; monocular depth estimation is cost-effective but suffers from scale ambiguity, multi-view inconsistency, and local inaccuracies that degrade rendering when used naively.

Method: Introduces a training framework integrating scale-ambiguous and noisy depth priors into geometric supervision, learning from weakly aligned depth variations, and isolating ill-posed geometry for selective monocular depth regularization to prevent propagation of inaccuracies.
...
Conclusion: The proposed method reliably leverages monocular depth priors for Gaussian Splatting enhancement by addressing their inherent limitations through selective regularization and robust integration into geometric supervision.
...
TL;DR: MPM is a training-free token merging method for semantic segmentation that pairs mutual nearest neighbors in cosine space, averages them, and uses merge maps for reconstruction, achieving up to 60% latency reduction on edge devices with minimal accuracy drop.


<details>
  <summary>Details</summary>
Motivation: Prior token reduction methods for transformers often target classification tasks and report proxy metrics rather than actual end-to-end latency. For semantic segmentation, token reduction is particularly challenging due to the need to reconstruct dense pixel-aligned features, and existing methods suffer from overhead that erases expected speed gains on modern accelerators.

Method: Mutual Pair Merging (MPM) is a training-free token aggregation module that: 1) forms mutual nearest-neighbor pairs in cosine similarity space, 2) averages each pair of tokens, 3) records a merge map that enables gather-based reconstruction before the decoder. The method introduces no learned parameters and uses discrete insertion schedules rather than continuous compression knobs.

Result: On ADE20K segmentation dataset, MPM reduces per-image latency by up to 60% for ViT-Tiny on Raspberry Pi 5, and increases throughput by up to 20% on H100 GPU with FlashAttention-2 while keeping mIoU drop below 3%. The method achieves practical wall-clock gains when overhead is explicitly accounted for.

Conclusion: Simple, reconstruction-aware, training-free token merging can translate into practical wall-clock speed improvements for semantic segmentation when overhead is explicitly considered, with MPM demonstrating significant latency reductions on both edge devices and modern GPUs while maintaining segmentation accuracy.
...
TL;DR: GaussianGrow generates 3D Gaussians by growing them from point clouds using text guidance and multi-view diffusion models, with iterative camera pose optimization for complete coverage.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting has excellent rendering quality but lacks reliable generation methods without proper geometric priors. Existing methods use predicted point maps that may have unreliable geometry leading to poor generations.

Method: Uses text-guided Gaussian growing from point clouds with multi-view diffusion model supervision. Constrains novel views at non-preset camera poses in overlapping regions. Iteratively detects camera poses for un-grown regions and inpaints using 2D diffusion model until complete.

Result: Extensively evaluated on text-guided Gaussian generation from both synthetic and real-scanned point clouds, demonstrating effectiveness.

Conclusion: GaussianGrow provides a novel approach for generating 3D Gaussians by growing them from point clouds, naturally enforcing geometric accuracy through iterative view synthesis and inpainting.
...
TL;DR: SAE features in CLIP vision encoders vary in information scope from local to global, quantified by Contextual Dependency Score (CDS), revealing systematic effects on predictions and confidence.


<details>
  <summary>Details</summary>
Motivation: Existing SAE analyses focus on semantic meaning of individual features, but lack understanding of how broadly features aggregate visual evidence (information scope).

Method: Introduce information scope concept and propose Contextual Dependency Score (CDS) to quantify scope by measuring feature stability across spatial perturbations.

Result: Features with different information scopes (local vs. global) systematically influence CLIP's predictions and confidence differently.

Conclusion: Information scope is a critical new interpretability dimension for CLIP representations, providing deeper diagnostic understanding of SAE-derived features.
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based LLIE methods use two-stage pipelines or auxiliary correction networks, which sever the intrinsic link between enhancement and denoising, leading to suboptimal performance due to inconsistent optimization objectives.

Method: Proposes Signal Attenuation Diffusion Model (SADM) that integrates signal attenuation coefficient into diffusion pipeline to simulate low-light degradation, enabling simultaneous brightness adjustment and noise suppression in a single stage without extra modules.

Result: SADM maintains consistency with DDIM via multi-scale pyramid sampling, balancing interpretability, restoration quality, and computational efficiency while eliminating need for correction modules or staged training.

Conclusion: SADM provides a unified diffusion framework that explicitly encodes physical priors of low-light degradation to guide concurrent optimization of brightness recovery and noise suppression in a single stage.
...
TL;DR: FoleyDesigner is an AI framework that automates professional Foley sound generation with precise spatio-temporal alignment to video, using multi-agent analysis, latent diffusion models, and LLM-driven mechanisms, supported by a new stereo audio dataset called FilmStereo.


<details>
  <summary>Details</summary>
Motivation: Manual Foley sound creation for films is labor-intensive and requires precise spatio-temporal alignment with visual content. There's a lack of high-quality stereo audio datasets with spatial metadata for professional film production.

Method: Multi-agent architecture for video analysis, latent diffusion models trained on spatio-temporal cues from video frames, LLM-driven hybrid mechanisms emulating post-production workflows, and the introduction of FilmStereo dataset with spatial metadata and semantic annotations.

Result: Superior spatio-temporal alignment compared to existing baselines, seamless compatibility with professional film standards (including 5.1-channel Dolby Atmos), and extensive creative flexibility through interactive user control.

Conclusion: FoleyDesigner provides an effective automated solution for professional Foley sound generation that integrates seamlessly with industry production pipelines while maintaining high-quality spatio-temporal alignment.
...
<details>
  <summary>Details</summary>
Motivation: Existing hyperspectral image fusion methods have two critical limitations: inadequate reconstruction of anisotropic spatial structures (leading to blurred details) and spectral distortion during fusion (hindering fine-grained spectral representation).
...
Conclusion: ASSR-Net effectively addresses both spatial and spectral challenges in hyperspectral image fusion through its anisotropic structure-aware and spectrally recalibrated design, demonstrating superior performance over existing methods.
...
TL;DR: Diffusion-based compressors using Reverse Channel Coding (RCC) are more robust to bit flips than traditional codecs, with Turbo-DDCM variant improving robustness without significantly affecting compression quality.


<details>
  <summary>Details</summary>
Motivation: Current image compression methods focus on rate-distortion-perception trade-offs but lack examination of robustness to bit-level corruption, which is important for noisy transmission environments.

Method: Uses diffusion-based compressors built on Reverse Channel Coding (RCC) paradigm, and introduces a more robust variant of Turbo-DDCM that improves robustness while maintaining compression quality.

Result: RCC-based compressors show substantially more robustness to bit flips than classical and learned codecs. The Turbo-DDCM variant significantly improves robustness with minimal impact on rate-distortion-perception trade-off.

Conclusion: RCC-based compression produces more resilient compressed representations, potentially reducing the need for error-correcting codes in highly noisy environments.
...
TL;DR: The paper introduces the Subtle Visual Challenge (SVC) to advance research on detecting and interpreting subtle visual signals across domains like biometric security, medical diagnosis, and affective computing, featuring two tasks: cross-domain multimodal deception detection and remote photoplethysmography estimation.


<details>
  <summary>Details</summary>
Motivation: Subtle visual signals contain important hidden patterns but are difficult to perceive with the naked eye. Existing research lacks robustness and generalization for real-world applications, necessitating a standardized challenge to promote better models.

Method: Organized the Subtle Visual Challenge with two specific tasks: 1) cross-domain multimodal deception detection, and 2) remote photoplethysmography (rPPG) estimation. Provided baseline models and a competition platform for 22 participating teams.

Result: 22 teams submitted final results to the workshop competition. Baseline models were released on the MMDD2026 platform to facilitate further research and development in subtle visual signal understanding.

Conclusion: The Subtle Visual Challenge aims to encourage development of more robust and generalizable models for subtle visual understanding, advancing research in computer vision and multimodal learning through standardized evaluation tasks.
...
TL;DR: The paper proposes x₀-supervision, a training objective for controllable diffusion models that uses direct supervision on clean target images to accelerate convergence by up to 2× while improving visual quality and conditioning accuracy.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with precise layout control using natural language alone. While controllable generation methods add conditions to address this, they use the same training loss as initial models, leading to slow convergence and long training times.

Method: The authors analyze denoising dynamics of controllable diffusion models and propose x₀-supervision - direct supervision on clean target images, or an equivalent re-weighting of the diffusion loss, to accelerate training convergence.

Result: Experiments across multiple control settings show 2× faster convergence (measured by mAUCC), improved visual quality, and better conditioning accuracy compared to standard training approaches.

Conclusion: x₀-supervision provides an effective training objective for controllable diffusion models that significantly accelerates convergence while maintaining or improving generation quality and control accuracy.
...
TL;DR: BADAS-2.0 is an enhanced collision anticipation system that improves on its predecessor with a long-tail benchmark, knowledge distillation for edge deployment, and explainability features including attention heatmaps and reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: To advance collision anticipation systems by addressing rare safety-critical scenarios, enabling real-time edge deployment, and providing explainable predictions for better trust and understanding.

Method: Three-pronged approach: (1) Created long-tail benchmark using BADAS-1.0 as active oracle to score unlabeled drives and targeted data collection via Nexar's Atlas platform; (2) Domain-specific self-supervised pre-training on 2.25M videos followed by knowledge distillation into compact models; (3) Generated object-centric attention heatmaps and extended with vision-language model for structured reasoning.

Result: Expanded dataset from 40k to 178,500 labeled videos (~2M clips) with consistent gains across all subgroups, especially on hardest long-tail cases. Created compact models (86M and 22M parameters) achieving 7-12x speedup with near-parity accuracy. Produced real-time explainable predictions with attention heatmaps and textual reasoning.

Conclusion: BADAS-2.0 represents a comprehensive advancement in collision anticipation systems, addressing long-tail safety scenarios, enabling practical edge deployment, and providing explainable predictions - with publicly available inference code and benchmarks.
...
TL;DR: PDMP is a multimodal learning strategy that prioritizes the performance-dominant modality (best unimodal performer) through gradient modulation, challenging the conventional balanced learning approach.


<details>
  <summary>Details</summary>
Motivation: Multimodal learning often suffers from insufficient optimization where multimodal models underperform compared to unimodal counterparts. The paper argues this isn't due to imbalanced learning but rather insufficient learning of the performance-dominant modality.

Method: PDMP first identifies the performance-dominant modality via unimodal model performance ranking, then uses asymmetric coefficients to modulate gradients, allowing the dominant modality to dominate optimization. The approach is model-agnostic.

Result: Extensive experiments on various datasets validate PDMP's superiority, showing it improves multimodal performance by prioritizing the best-performing modality.

Conclusion: Balanced learning is not optimal for multimodal learning; instead, prioritizing the performance-dominant modality through gradient modulation (PDMP) leads to better multimodal performance.
...
TL;DR: VoxSAMNet: A sparsity-aware monocular semantic scene completion framework that addresses voxel imbalance (93% empty) and long-tailed categories using dummy shortcuts and foreground modulation.


<details>
  <summary>Details</summary>
Motivation: Monocular SSC is cost-effective for autonomous driving but suffers from severe voxel imbalance (93% empty) and poor generalization to rare foreground classes, leading to redundant processing and overfitting.

Method: Proposes VoxSAMNet with: (1) DSFR module using shared dummy node to bypass empty voxels while refining occupied ones with deformable attention; (2) Foreground Modulation Strategy combining Foreground Dropout and Text-Guided Image Filter to prevent overfitting and enhance class-relevant features.

Result: Achieves SOTA on SemanticKITTI (18.2% mIoU) and SSCBench-KITTI-360 (20.2% mIoU), surpassing prior monocular and stereo baselines.

Conclusion: Sparsity-aware and semantics-guided design is crucial for efficient and accurate 3D scene completion, offering a promising direction for future SSC research.
...
TL;DR: RHVI-FDD: A hierarchical decoupling framework for low-light image enhancement that separates luminance-chrominance at macro level and uses frequency-domain decomposition at micro level to handle noise, details, and color distortion simultaneously.
...
Method: Two-level hierarchical decoupling: 1) Macro level: RHVI transform mitigates input noise bias for robust luminance-chrominance decoupling. 2) Micro level: Frequency-Domain Decoupling (FDD) module uses DCT to decompose chrominance into low (global tone), mid (local details), and high (noise) frequency bands, processed by tailored expert networks and fused via adaptive gating for content-aware fusion.
...
Conclusion: The proposed hierarchical decoupling framework effectively addresses the complex degradation in low-light images by separating luminance-chrominance coupling and disentangling noise-detail entanglement through frequency-domain decomposition, achieving superior enhancement results.
...
TL;DR: GeoUQ-GFNet: A geometry-aware uncertainty-quantifying network for sparse radio map reconstruction in urban environments, with uncertainty-guided active sensing and UrbanRT-RM benchmark.


<details>
  <summary>Details</summary>
Motivation: Dense radio map construction is challenging with limited measurements in complex urban environments with blockages and irregular geometry. Existing methods insufficiently exploit geometric priors or overlook predictive uncertainty for active sensing.

Method: Proposes GeoUQ-GFNet - a lightweight network that jointly predicts dense gain radio maps and spatial uncertainty maps from sparse measurements and structured scene priors. Uses UrbanRT-RM benchmark for evaluation and uncertainty-guided active measurement selection.
...
Conclusion: Combining geometry-aware learning, uncertainty estimation, and benchmark-driven evaluation is effective for sparse radio map reconstruction in complex urban environments, enabling better active sensing strategies.
...
<details>
  <summary>Details</summary>
Motivation: Existing hair reconstruction methods suffer from accuracy-efficiency trade-off: implicit neural representations lack fine details, while explicit optimization is computationally heavy and poorly scalable.

Method: Combines implicit neural network with multi-view geometric fusion; introduces fusion-patch-based multi-view optimization for point cloud direction, and parallel hair-growing strategy that relaxes voxel occupancy constraints for stable strand tracing.

Result: Achieves reconstruction quality comparable to state-of-the-art methods on synthetic benchmarks while improving runtime efficiency by nearly an order of magnitude; robustly reconstructs high-fidelity strand geometries on real-world hairstyles.

Conclusion: EfficientMonoHair provides a fast and accurate solution for strand-level hair reconstruction from monocular video, effectively balancing accuracy and efficiency through novel optimization and tracing techniques.
...
TL;DR: WikiSeeker is a novel multi-modal RAG framework that improves KB-VQA by using VLMs as specialized agents (Refiner and Inspector) to enhance retrieval and answer generation.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal RAG methods for KB-VQA rely too heavily on images as retrieval keys and underutilize VLMs, failing to fully leverage their potential for improved performance.

Method: Proposes WikiSeeker with a multi-modal retriever and redefines VLM roles as two specialized agents: Refiner (rewrites textual queries based on images to improve retrieval) and Inspector (selectively routes reliable retrieved context to LLMs or uses VLM's internal knowledge).

Result: Achieves state-of-the-art performance on EVQA, InfoSeek, and M2KR datasets with substantial improvements in both retrieval accuracy and answer quality.

Conclusion: WikiSeeker effectively bridges gaps in current multi-modal RAG approaches by better leveraging VLMs' capabilities, demonstrating significant performance gains in KB-VQA tasks.
...
TL;DR: A new learning-based method for generating visual attribution maps that directly optimizes deletion/insertion metrics using differentiable permutation learning with Gumbel-Sinkhorn, producing sharp, pixel-level explanations for vision models.


<details>
  <summary>Details</summary>
Motivation: Existing visual attribution methods face a three-way trade-off: propagation-based approaches are efficient but biased/architecture-specific; perturbation-based methods are causally grounded but expensive/coarse for vision transformers; learning-based explainers are fast but optimize surrogate objectives. There's a need for a method that combines the strengths while avoiding these limitations.

Method: Proposes a learning scheme that directly optimizes deletion and insertion metrics by framing them as permutation learning problems. Uses Gumbel-Sinkhorn to create a differentiable relaxation of hard sorting/ranking, enabling end-to-end training through attribution-guided perturbations of the target model. During inference, produces dense pixel-level attributions in a single forward pass with optional gradient refinement.

Result: Experiments demonstrate consistent quantitative improvements and sharper, boundary-aligned explanations, particularly for transformer-based vision models. The method produces more precise pixel-level attributions compared to existing approaches.

Conclusion: The proposed method successfully addresses the limitations of existing visual attribution approaches by directly optimizing interpretability metrics through differentiable permutation learning, resulting in more accurate and detailed explanations for complex vision models.
...
TL;DR: Etch is a black-box attack framework that exploits text-to-image models' text-rendering capabilities to generate harmful textual content within visually benign images, achieving high attack success rates and revealing safety vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: Modern T2I models can render legible paragraph-length text, enabling a new class of misuse called "inscriptive jailbreaks" where adversaries generate harmful textual payloads (e.g., fraudulent documents) within benign-looking images. Existing jailbreak techniques are inadequate for bypassing safety filters while maintaining character-level fidelity.

Method: Etch decomposes adversarial prompts into three orthogonal layers: semantic camouflage (hides harmful intent), visual-spatial anchoring (positions text), and typographic encoding (controls text appearance). It uses a zero-order optimization loop where a vision-language model critiques generated images, localizes failures to specific layers, and prescribes targeted revisions.

Result: Extensive evaluations across 7 models on 2 benchmarks show Etch achieves average attack success rate of 65.57% (peaking at 91.00%), significantly outperforming existing baselines. This reveals critical blind spots in current T2I safety alignments.

Conclusion: The work exposes a fundamental vulnerability in T2I systems where text-rendering capabilities can be weaponized, highlighting the urgent need for typography-aware defense mechanisms in multimodal AI safety.
...
TL;DR: Proposes Hybrid QUBO framework combining gradient-aware sensitivity metrics with activation similarity for neural network pruning, plus Tensor-Train refinement for optimization.


<details>
  <summary>Details</summary>
Motivation: Existing pruning methods use greedy heuristics ignoring filter interactions, while formal QUBO methods underperform due to oversimplified L1-norm objectives. Need a principled approach capturing both individual filter relevance and inter-filter redundancy.

Method: 1) Hybrid QUBO formulation with gradient-aware sensitivity metrics (Taylor/Fisher) in linear term and activation similarity in quadratic term. 2) Dynamic capacity-driven search for strict sparsity enforcement. 3) Two-stage pipeline with Tensor-Train Refinement optimizing QUBO solutions against true evaluation metrics.

Result: Significantly outperforms both greedy Taylor pruning and traditional L1-based QUBO on SIDD image denoising dataset. Tensor-Train Refinement provides additional consistent gains at appropriate combinatorial scales.
...
TL;DR: 3D-2D dental identification using computer vision to match post-mortem scans with ante-mortem photos when medical records are unavailable, achieving high accuracy with automatic landmark and segmentation approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional dental identification relies on ante-mortem medical records which are often unavailable in migrant deaths or countries without universal healthcare. Social media photos showing teeth offer an alternative source, but current methods lack proper perspective distortion modeling and objective morphological comparison.

Method: Developed two automatic 3D-2D approaches: 1) using paired landmarks to estimate camera parameters, and 2) using teeth region segmentation to estimate camera parameters. Both replicate ante-mortem photos with 3D models for morphological comparison through computer vision and optimization techniques.

Result: Tested on 20,164 cross comparisons from 142 samples, achieving mean ranking values of 1.6 (landmark approach) and 1.5 (segmentation approach), significantly outperforming automatic dental chart comparison approaches.

Conclusion: The proposed 3D-2D approach provides an automatic, objective, and quantitative method for dental identification using social media photos, offering easily interpretable results through superimposed images and addressing limitations of existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current video instance removal benchmarks focus on visual plausibility but overlook physical causalities (shadows, reflections, illumination) triggered by object removal, creating a gap in evaluating true physical consistency.

Method: Created PVIR benchmark with 95 high-quality videos annotated with instance masks and removal prompts, partitioned into Simple/Hard subsets. Evaluated 4 methods (PISCO-Removal, UniVideo, DiffuEraser, CoCoCo) using decoupled human evaluation across instruction following, rendering quality, and edit exclusivity.

Result: PISCO-Removal and UniVideo achieve SOTA performance, DiffuEraser introduces blurring artifacts, CoCoCo struggles with instruction following. All methods show significant performance drop on Hard subset with complex physical interactions.

Conclusion: Current video instance removal methods still struggle with recovering complex physical side effects, highlighting the need for physics-aware approaches and better benchmarks like PVIR.
...
TL;DR: AICA-Bench benchmark introduces three affective image tasks, reveals VLMs' limitations in intensity calibration and descriptive depth, and proposes GAT prompting to improve affective understanding and generation.


<details>
  <summary>Details</summary>
Motivation: Holistic Affective Image Content Analysis (AICA) integrating perception, reasoning, and generation remains underexplored despite VLMs' strong perceptual capabilities, creating a need for comprehensive evaluation and improvement frameworks.
...
Result: Evaluation reveals two major VLM limitations: weak intensity calibration and shallow open-ended descriptions. GAT prompting reduces intensity errors and improves descriptive depth, establishing a strong baseline for affective multimodal research.

Conclusion: The proposed AICA-Bench and GAT prompting framework address critical gaps in affective multimodal understanding, providing effective tools for advancing holistic affective image analysis and generation capabilities in VLMs.
...
TL;DR: Selective aggregation of relevant cross-attention heads improves visual interpretability in text-to-image models, outperforming DAAM segmentation and enabling better concept-specific feature capture and prompt misinterpretation diagnosis.


<details>
  <summary>Details</summary>
Motivation: While cross-attention maps are widely used in text-to-image models for applications and interpretability, the distinct characteristics of attention maps from different attention heads remain underexplored. The paper aims to investigate how selective aggregation of attention heads can improve interpretability.

Method: The approach involves selectively aggregating cross-attention maps from heads that are most relevant to target concepts, rather than using all attention heads indiscriminately. This selective aggregation focuses on the most concept-relevant attention information.

Result: The method achieves higher mean IoU scores compared to the diffusion-based segmentation method DAAM. Most relevant heads capture concept-specific features more accurately than least relevant ones, and selective aggregation helps diagnose prompt misinterpretations.

Conclusion: Attention head selection offers a promising direction for improving both interpretability and controllability of text-to-image generation, suggesting that not all attention heads contribute equally to concept representation.
...
TL;DR: ADM-GS uses appearance decomposition in Gaussian Splatting to handle illumination variations for multi-traversal scene reconstruction in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Multi-traversal scene reconstruction faces appearance inconsistency due to varying illumination and environmental conditions across different traversals of the same area, despite shared underlying geometry.

Method: Proposes ADM-GS framework with explicit appearance decomposition of static background into traversal-invariant material properties and traversal-dependent illumination. Uses neural light field with frequency-separated hybrid encoding, incorporating surface normals and explicit reflection vectors to separately capture low-frequency diffuse illumination and high-frequency specular reflections.
...
Conclusion: ADM-GS effectively addresses appearance inconsistency in multi-traversal reconstruction through explicit appearance decomposition, enabling better scene reconstruction for autonomous driving simulation and digital twin applications.
...
TL;DR: SRCP improves zero-shot generalization in visual unsupervised RL by addressing SR limitations with saliency-guided representation learning and consistency policy training.


<details>
  <summary>Details</summary>
Motivation: Successor representations struggle in high-dimensional visual environments due to suboptimal representations that focus on dynamics-irrelevant regions and hinder skill-conditioned policy modeling and controllability.

Method: SRCP decouples representation learning from successor training using saliency-guided dynamics tasks, and integrates fast-sampling consistency policy with URL-specific classifier-free guidance and tailored training objectives.

Result: Achieves state-of-the-art zero-shot generalization on 16 tasks across 4 datasets from ExORL benchmark, and is compatible with various SR methods.

Conclusion: SRCP effectively addresses SR limitations in visual URL, improving both representation quality and policy controllability for better zero-shot task generalization.
...
TL;DR: SonoSelect is an ultrasound-specific active view exploration method that adaptively guides probe movement to efficiently scan anatomy using only the most informative views, reducing redundancy while maintaining diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional ultrasound requires multiple scan views through probe movement, but not all views are equally informative. Exhaustively acquiring many views introduces redundancy and increases scanning/processing costs. There's a need for intelligent view selection to optimize ultrasound scanning efficiency.
...
Conclusion: SonoSelect successfully demonstrates adaptive probe guidance for efficient ultrasound scanning, reducing view redundancy while maintaining diagnostic coverage and accuracy through intelligent view selection based on 3D spatial memory and ultrasound-specific objectives.
...
TL;DR: Adapting vision-language diffusion foundation models via LoRA for CT metal artifact reduction, achieving SOTA with only 16-128 training examples by reframing it as in-context reasoning with multi-reference conditioning.


<details>
  <summary>Details</summary>
Motivation: Metal artifacts from implants severely degrade CT image quality, obscuring anatomy. Standard deep learning methods require extensive paired training data, which is impractical in medical imaging where such data is scarce.

Method: Reframe artifact reduction as in-context reasoning by adapting a general-purpose vision-language diffusion foundation model via parameter-efficient LoRA. Use multi-reference conditioning where clean anatomical exemplars from unrelated subjects are provided alongside corrupted input to ground restoration.

Result: Achieves state-of-the-art performance on AAPM CT-MAR benchmark with perceptual and radiological-feature metrics, using only 16-128 paired training examples (two orders of magnitude less data). Domain adaptation is essential to prevent hallucination where artifacts are misinterpreted as natural objects.

Conclusion: Foundation models, when appropriately adapted with domain-specific grounding, offer a scalable alternative for interpretable, data-efficient medical image reconstruction, establishing a new paradigm for artifact reduction.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal methods use fixed fusion modules that can't adapt to changing modality reliability or capture fine-grained action cues, limiting robust multimodal visual analytics.

Method: Proposes Mixture-of-Modality-Experts (MoME) framework with Holistic Token Learning (HTL) strategy. MoME enables adaptive collaboration among modality-specific experts, while HTL uses class tokens and spatio-temporal tokens for intra-expert refinement and inter-expert knowledge transfer.

Result: Outperforms representative single-modal and multimodal baselines on driver action recognition benchmark. HTL strategy improves subtle multimodal understanding and offers better interpretability.

Conclusion: The proposed knowledge-centric multimodal learning framework improves expert specialization while reducing ambiguity in multimodal fusion, addressing limitations of fixed fusion approaches.
...
TL;DR: A multi-model framework combining Sentinel-2 optical and Sentinel-1 SAR data with vision transformers and ensemble learning achieves state-of-the-art landslide detection without requiring pre-event imagery.


<details>
  <summary>Details</summary>
Motivation: Landslides pose significant geohazards requiring accurate, timely detection for disaster risk reduction. Current approaches need improvement in robustness and operational applicability, especially when pre-event data is unavailable.

Method: Modular framework fusing Sentinel-2 optical and Sentinel-1 SAR data using multi-encoder vision transformers with separate pretrained encoders per modality. Integrates spectral indices (NDVI) and employs ensemble learning combining neural networks with gradient boosting models (LightGBM, XGBoost).

Result: Achieves state-of-the-art F1 score of 0.919 for patch-based landslide classification, demonstrating top competition performance with strong precision-recall balance. Works without pre-event Sentinel-2 data in non-classical change detection setting.

Conclusion: The framework effectively leverages complementary optical-radar data strengths, offers flexible configurations (optical-only, SAR-only, or combined), and provides a transferable solution for natural hazard monitoring with operational scalability.
...
TL;DR: HumANDiff enhances human video generation by using articulated motion-consistent noise sampling, joint appearance-motion learning, and geometric motion consistency to improve motion physics and fidelity.


<details>
  <summary>Details</summary>
Motivation: Current generative video diffusion models struggle to capture realistic human motion dynamics and physics, particularly in maintaining motion consistency and physical plausibility across frames.

Method: Three key designs: 1) Articulated motion-consistent noise sampling using 3D articulated noise on human body template, 2) Joint appearance-motion learning that predicts both pixel appearances and physical motions, 3) Geometric motion consistency loss in articulated noise space.

Result: Achieves state-of-the-art performance in rendering motion-consistent, high-fidelity humans with diverse clothing styles, capturing motion-dependent details like clothing wrinkles.

Conclusion: HumANDiff provides a scalable framework for controllable human video generation that works with existing diffusion models without architectural changes, enabling intrinsic motion control in image-to-video generation.
...
TL;DR: CLIP models exhibit "center bias" - they disproportionately focus on central image regions and overlook important objects near boundaries, limiting fine-grained visual understanding.


<details>
  <summary>Details</summary>
Motivation: To address CLIP's persistent fine-grained understanding limitations, particularly identifying a distinct failure mode where models overlook boundary objects due to excessive focus on central image regions.

Method: Analyze center bias through representation and attention perspectives using interpretability methods (embedding decomposition and attention map analysis), then propose training-free solutions like visual prompting and attention redistribution.

Result: Found that relevant concepts for off-center objects vanish in final embeddings due to information loss during visual embedding aggregation, particularly from pooling mechanisms. Showed bias can be alleviated with training-free strategies.

Conclusion: CLIP's center bias is a fundamental limitation that hinders sophisticated visual tasks, but can be mitigated through attention-redirecting techniques without retraining, improving fine-grained visual understanding.
...
TL;DR: OmniCamera is a unified framework that disentangles camera motion from scene content for flexible video generation, using a novel hybrid dataset and dual-level curriculum co-training to overcome modality conflicts and data scarcity.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models entangle camera motion and scene content, limiting independent control over these two fundamental axes of video. The paper aims to explicitly disentangle and command these dimensions for unprecedented creative flexibility.

Method: 1) OmniCamera framework for disentangling camera and content conditions; 2) OmniCAM hybrid dataset combining curated real-world videos with synthetic data; 3) Dual-level Curriculum Co-Training strategy with condition-level (progressive modality introduction) and data-level (synthetic-to-real adaptation) components.

Result: Achieves state-of-the-art performance, enabling flexible control for complex camera movements while maintaining superior visual quality. The system allows arbitrary pairings of camera and content conditions.

Conclusion: OmniCamera successfully disentangles camera motion from scene content through a compositional approach, overcoming fundamental challenges of modality conflict and data scarcity via innovative dataset construction and training strategies, unlocking new creative possibilities in video generation.
...
### [106] [Graph-PiT: Enhancing Structural Coherence in Part-Based Image Synthesis via Graph Priors](https://arxiv.org/abs/2604.06074)
*Junbin Zhang,Meng Cao,Feng Tan,Yikai Lin,Yuexian Zou*

Main category: cs.CV

TL;DR: Graph-PiT introduces a graph-based framework that models spatial-semantic relationships between visual parts to improve structural coherence in image generation, outperforming vanilla PiT while maintaining compatibility with existing pipelines.
...
Method: Proposes Graph-PiT with a Hierarchical Graph Neural Network (HGNN) module that performs bidirectional message passing between coarse-grained part-level super-nodes and fine-grained IP+ token sub-nodes, using graph Laplacian smoothness loss and edge-reconstruction loss for relation-aware embeddings.

Result: Quantitative experiments on controlled synthetic domains (character, product, indoor layout, jigsaw) and qualitative transfer to real web images show improved structural coherence over vanilla PiT while remaining compatible with original IP-Prior pipeline.

Conclusion: Graph-PiT enhances plausibility of generated concepts and offers a scalable, interpretable mechanism for complex multi-part image synthesis, with explicit relational reasoning being crucial for enforcing user-specified adjacency constraints.
...
TL;DR: A-ROM is an interpretable medical imaging framework using pretrained ViTs and kNN classification to enable transparent, few-shot learning without gradient fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Address the "black-box" problem of deep learning in medical imaging that hinders clinical adoption by creating interpretable models that meet transparency requirements.

Method: Leverages Platonic Representation Hypothesis and pretrained Vision Transformers' metric space, replaces opaque decision layers with human-readable concept dictionary and kNN classifier for interpretability, enables rapid modeling without gradient fine-tuning.

Result: Competitive performance with standard benchmarks on MedMNIST v2 suite, provides simple and scalable few-shot solution suitable for clinical environments.

Conclusion: A-ROM bridges the interpretability gap in medical AI by offering transparent, few-shot learning that maintains competitive performance while meeting clinical transparency demands.
...
TL;DR: SciTikZer introduces a framework for synthesizing TikZ graphics code from scientific schematics, addressing data quality and evaluation gaps with a large dataset, benchmark, and novel optimization method.


<details>
  <summary>Details</summary>
Motivation: Graphics program synthesis is crucial for reverse-engineering static visuals into editable TikZ code, but current approaches face challenges due to TikZ's spatial precision requirements and gaps in data quality and evaluation benchmarks.

Method: Developed SciTikZ-230K dataset from Execution-Centric Data Engine covering 11 scientific disciplines, created SciTikZ-Bench benchmark for visual and structural evaluation, and introduced Dual Self-Consistency Reinforcement Learning with Round-Trip Verification for code optimization.

Result: The trained model SciTikZer-8B achieves state-of-the-art performance, consistently outperforming proprietary models like Gemini-2.5-Pro and larger models like Qwen3-VL-235B-A22B-Instruct.

Conclusion: The framework successfully addresses key gaps in graphics program synthesis for TikZ code generation, enabling high-quality reverse-engineering of scientific schematics through improved data, evaluation, and optimization methods.
...
TL;DR: The paper introduces ICM, a precise steering method that targets specific self-attention layers in diffusion models to modify implicit decisions when handling ambiguous prompts, achieving superior debiasing performance with minimal artifacts.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models must make implicit decisions when prompts are not fully descriptive, but current understanding of where these decisions occur within the model architecture is limited. Existing localization techniques focus on prompt-related interventions, which may not capture implicit decision-making processes.

Method: The authors introduce a probing-based localization technique to identify layers with highest attribute separability for concepts. They discover that ambiguous concept resolution is governed by self-attention layers, then propose ICM (Implicit Choice-Modification) - a targeted intervention method applied to a small subset of these specific layers.

Result: Intervening on identified self-attention layers yields superior debiasing performance compared to state-of-the-art methods, minimizing artifacts common to less precise approaches. The approach demonstrates that implicit decision-making is computationally localized rather than diffuse.

Conclusion: The resolution of ambiguous concepts in diffusion models is principally governed by self-attention layers, making them the most effective point for intervention. Targeted modifications to these specific layers enable precise steering with minimal artifacts, advancing our understanding of how diffusion models handle incomplete prompts.
...
TL;DR: Lightweight multimodal adaptation framework bridges RGB-pretrained VLMs to thermal infrared drone imagery, enabling species recognition, enumeration, and habitat-context interpretation in ecological monitoring.


<details>
  <summary>Details</summary>
Motivation: To address the representation gap between RGB-pretrained Vision-Language Models (VLMs) and thermal infrared imagery for ecological monitoring applications using drone-collected data.

Method: Developed thermal dataset from drone imagery, used multimodal projector alignment to fine-tune VLMs, benchmarked three models (InternVL3-8B-Instruct, Qwen2.5-VL-7B-Instruct, Qwen3-VL-8B-Instruct) under closed-set and open-set prompting for species recognition and enumeration.
...
TL;DR: EDGE-Shield is a scalable content filter that blocks copyright-violating and deepfake content during image generation, using embedding-based matching and x-pred transformation to reduce latency by 50-79% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models pose copyright violation and deepfake risks, requiring up-to-date protection. Existing reference-based filters lack scalability for numerous references and require waiting for full image generation completion.

Method: Proposes EDGE-Shield: 1) Uses embedding-based matching for efficient reference comparison, 2) Introduces x-pred transformation that converts noisy intermediate latents into pseudo-estimated clean latents at later denoising stages to enhance early-stage classification accuracy.

Result: Achieves 79% reduction in processing time for Z-Image-Turbo and 50% reduction for Qwen-Image compared to traditional methods, while maintaining filtering accuracy across different model architectures.

Conclusion: EDGE-Shield provides a scalable, low-latency solution for real-time content filtering during image generation, effectively addressing copyright and deepfake concerns without waiting for complete image generation.
...
TL;DR: PoM is a linear-complexity token mixing mechanism that replaces self-attention in transformers, matching performance while reducing computational cost for long sequences.


<details>
  <summary>Details</summary>
Motivation: Self-attention has quadratic complexity, making it computationally expensive for long sequences. There's a need for efficient alternatives that maintain transformer capabilities.

Method: PoM aggregates tokens into compact representations using learned polynomial functions, then retrieves contextual information for each token, achieving linear complexity.

Result: PoM matches attention-based model performance across five domains (text generation, handwriting recognition, image generation, 3D modeling, Earth observation) while reducing computational cost for long sequences.

Conclusion: PoM is an effective drop-in replacement for self-attention that maintains transformer universality while offering linear complexity, enabling efficient processing of long sequences.
...
TL;DR: MMEmb-R1: Adaptive reasoning-based multimodal embedding framework that selectively invokes reasoning only when beneficial, achieving SOTA with 71.2 score using only 4B parameters while reducing overhead.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs underutilize generative reasoning for embeddings. Direct chain-of-thought integration causes structural misalignment (leading to shortcut learning) and unnecessary computation (reasoning isn't always beneficial for embeddings).
...
Conclusion: Adaptive reasoning selection is crucial for efficient multimodal embedding learning, balancing reasoning benefits with computational efficiency to achieve superior performance with reduced overhead.
...
TL;DR: DiffHDR: A framework using video diffusion models to convert LDR videos to HDR by treating it as a generative radiance inpainting task, enabling realistic detail restoration in over-/underexposed regions with text/image guidance.


<details>
  <summary>Details</summary>
Motivation: Most digital videos are stored in 8-bit LDR formats, losing HDR scene radiance through saturation and quantization. This prevents accurate luminance mapping to HDR displays and limits meaningful re-exposure in post-production. Existing LDR-to-HDR conversion techniques struggle to restore realistic detail in over- and underexposed regions.

Method: DiffHDR formulates LDR-to-HDR conversion as a generative radiance inpainting task within the latent space of a video diffusion model. It operates in Log-Gamma color space to leverage spatio-temporal generative priors from a pretrained video diffusion model, synthesizing plausible HDR radiance in problematic regions while recovering continuous scene radiance. The framework enables controllable conversion via text prompts or reference images. A pipeline synthesizes high-quality HDR video training data from static HDRI maps to address data scarcity.

Result: Extensive experiments demonstrate that DiffHDR significantly outperforms state-of-the-art approaches in radiance fidelity and temporal stability, producing realistic HDR videos with considerable latitude for re-exposure.

Conclusion: DiffHDR provides an effective solution for LDR-to-HDR video conversion by leveraging generative video diffusion models for radiance inpainting, overcoming limitations of existing methods and enabling high-quality HDR video generation with realistic detail restoration and temporal stability.
...
TL;DR: ZACH-ViT's robustness evaluation shows it maintains performance advantages under common image corruptions while adversarial robustness remains challenging for all compact medical vision transformers.


<details>
  <summary>Details</summary>
Motivation: To extend the original ZACH-ViT study by evaluating its robustness under common image corruptions and adversarial perturbations in low-data medical imaging settings, addressing a gap in the original work.

Method: Evaluated ZACH-ViT against three compact baselines (ABMIL, Minimal-ViT, TransMIL) on seven MedMNIST datasets using 50 samples per class, fixed hyperparameters, and five random seeds. Tested under common image corruptions and adversarial attacks (FGSM, PGD).

Result: ZACH-ViT achieved best overall mean rank on clean data (1.57) and under common corruptions (1.57). Under adversarial attacks, it ranked first under FGSM (2.00) and second under PGD (2.29), with ABMIL performing best overall on adversarial robustness.

Conclusion: The advantages of compact permutation-invariant transformers like ZACH-ViT extend beyond clean evaluation to persist under realistic perturbation stress in low-data medical imaging, though adversarial robustness remains an open challenge for all evaluated models.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D scene generation methods either lack geometric coherence, are limited to training views, or can't scale to large outdoor areas. There's a need for scalable generation of consistent outdoor driving scenes across multiple viewpoints.

Method: Uses Σ-Voxfield grid representation with colorized surface samples per voxel. Trains semantic-conditioned diffusion model on local voxel neighborhoods with 3D positional encodings. Scales via progressive spatial outpainting over overlapping regions, then renders with deferred rendering module.
...
Conclusion: Proposed framework enables large-scale multiview-consistent 3D scene generation without per-scene optimization, addressing limitations of existing methods in geometric coherence and scalability.
...
TL;DR: Character Error Vector (CEV) is a new bag-of-characters evaluator for OCR that decomposes errors into parsing, OCR, and interaction components, addressing CER's limitations with imperfect parsing.


<details>
  <summary>Details</summary>
Motivation: Character Error Rate (CER) assumes perfect text parsing, which is often unrealistic. Under page-parsing errors, CER becomes undefined, limiting its use for evaluating page-level OCR, especially with data lacking consistent labeling schemas.

Method: Introduces Character Error Vector (CEV) - a bag-of-characters evaluator that can be decomposed into parsing, OCR, and interaction error components. Two implementations: SpACER (Spatially Aware Character Error Rate) and Character distribution method using Jensen-Shannon Distance.

Result: CEV bridges parsing metrics and local metrics like CER. Analysis of archival newspapers shows traditional pipeline approaches outperform state-of-the-art end-to-end models. Thresholding can predict main error source with F1 of 0.91, even without optimal character-level positioning.

Conclusion: CEV provides a valuable decomposition of OCR errors, enabling practitioners to focus on the most impactful parts of document understanding pipelines. Available as a Python library to support research.
...
TL;DR: HaloProbe: Bayesian framework for detecting and mitigating hallucinations in vision-language models by factoring external description statistics with internal decoding signals, outperforming intervention-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing attention-based hallucination detection methods are unreliable due to confounders like token position and object repetition, leading to Simpson's paradox where attention trends reverse or disappear when aggregated.

Method: HaloProbe uses Bayesian framework to factorize external description statistics and internal decoding signals, employing balanced training to isolate internal evidence and combine it with learned prior over external features to estimate token-level hallucination probabilities.

Result: HaloProbe-guided decoding reduces hallucinations more effectively than state-of-the-art intervention-based methods while preserving utility, demonstrating superior performance as an external scoring signal for non-invasive mitigation.

Conclusion: HaloProbe provides a reliable Bayesian approach to hallucination detection and mitigation that avoids the pitfalls of attention-based methods and outperforms intervention-based approaches without degrading model utility or fluency.
...
TL;DR: UA-TOM is a lightweight belief-tracking module that detects behavioral regime switches in collaborative robots, reducing post-switch collisions by 52% and achieving 85.7% detection accuracy with minimal inference overhead.
...
Method: UA-TOM augments frozen vision-language-action (VLA) control backbones using selective state-space dynamics, causal attention, and prediction-error signals to track behavioral changes without modifying the base policy.

Result: UA-TOM achieves highest detection rate (85.7% at ±3 steps) among unassisted methods, reduces post-switch collisions by 52%, and adds only 7.4 ms inference overhead (14.8% of 50 ms control budget).

Conclusion: UA-TOM enables reliable regime-switch detection in collaborative robotics through lightweight belief tracking, with analysis showing detection sensitivity driven by learned dynamics rather than input-dependent gating.
...
<details>
  <summary>Details</summary>
Motivation: Video captures rich physical dynamics without requiring robot action labels, but translating this temporal structure into reliable robotic control remains challenging due to differences in embodiment, viewpoint, and physical constraints.

Method: Introduces an interface-centric taxonomy with three families: 1) direct video-action policies (implicit interface), 2) latent-action methods (learned intermediate), and 3) explicit visual interfaces (interpretable targets). Analyzes control-integration properties for each.

Result: Analysis reveals that the most pressing challenges center on the robotics integration layer - mechanisms connecting video-derived predictions to dependable robot behavior.

Conclusion: The paper outlines research directions toward closing the gap between video-based learning and reliable robotic control, emphasizing the need for better integration of video-derived predictions with robot execution systems.
...
TL;DR: COMB is a compact, open-source, modular robotic platform for in-hive honeybee experiments, featuring XY positioning, sealed tool access, interchangeable payloads, and embedded control for repeatable sensing and actuation.


<details>
  <summary>Details</summary>
Motivation: Real honeybee colony research requires robotic systems that can operate in confined hive spaces, tolerate hive-specific fouling and conditions, and support both sensing and localized actuation without frequent hardware redesign.

Method: Developed COMB platform with XY positioning stage, Movable Access Window for sealed tool access, interchangeable payload modules, and embedded control architecture for repeatable trajectory execution and signal generation.

Result: Demonstrated capabilities with three modules: biomimetic dance-and-signaling payload, close-range comb scanner, and electromagnetic wing actuator; characterized through waggle-trajectory tracking, multi-image stitching, and video-based spectral analysis.

Conclusion: COMB serves as a reusable experimental robotics platform for controlled in-hive sensing and actuation, and as a compact, generalized successor to earlier task-specific honeybee robotic systems.
...
TL;DR: Survey of sensor-based planning and control methods for UUVs, comparing decoupled vs. coupled architectures and analyzing controller types for autonomous navigation in challenging underwater environments.


<details>
  <summary>Details</summary>
Motivation: UUVs face significant challenges in complex underwater environments including sensor noise, GNSS absence, and communication limitations, requiring advanced planning and control strategies for effective autonomous navigation.

Method: The survey categorizes literature into decoupled (sequential planning and control) and coupled (tight feedback loops) architectures, with comparative analysis of controllers including PID, MPC, and invariant-set controllers.

Result: PID controllers are simple but lack predictive capability, MPC offers superior path optimization but is computationally intensive, and invariant-set controllers provide strong safety guarantees but may sacrifice agility in confined spaces.

Conclusion: The survey provides a taxonomy of planning-control architectures, emphasizes adaptive local planning using real-time sensor inputs, and analyzes controller roles in integrated frameworks for UUV autonomous navigation in dynamic ocean conditions.
...
TL;DR: StarVLA is an open-source Vision-Language-Action framework that provides modular architecture, reusable training strategies, and unified evaluation to address fragmentation in embodied AI research.
...
Method: Provides modular backbone-action-head architecture supporting both VLM and world-model backbones, reusable training strategies (cross-embodiment learning, multimodal co-training), and unified evaluation interface across major benchmarks.

Result: StarVLA's simple training recipes already match or surpass prior methods on multiple benchmarks with both VLM and world-model backbones, despite minimal data engineering.
...
TL;DR: GaussFly is a framework that uses 3D Gaussian Splatting and contrastive learning to enable robust visuomotor policy learning for drones with efficient sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end learning approaches for AAVs suffer from low sample efficiency and severe sim-to-real gaps due to visual discrepancies between simulation and reality.

Method: Uses 3D Gaussian Splatting with geometric constraints for real-to-sim scene reconstruction, then contrastive representation learning on rendered images to extract noise-resilient features, enabling efficient policy learning.

Result: Achieves superior sample efficiency and asymptotic performance compared to baselines, with robust zero-shot transfer to unseen real-world environments with complex textures.

Conclusion: GaussFly effectively bridges the sim-to-real gap for AAV visuomotor policy learning through explicit decoupling of representation learning from policy optimization.
...
TL;DR: Bilinear MPC framework for tendon-driven steerable stylet in brachytherapy needles enables curved trajectories to avoid sensitive structures, validated with 1.45-8.3mm positioning errors.


<details>
  <summary>Details</summary>
Motivation: Existing steerable needle models don't work with commercial clinical needles; need control framework for tendon-driven stylets compatible with standard brachytherapy equipment.

Method: Bilinear MPC with geometric model using 3 virtual inputs (insertion speed + 2 bending rates) mapped to physical inputs (insertion speed + tendon tensions), validated via simulations and tissue phantom experiments with image-based tip tracking.

Result: Open-loop validation: <2mm error (3% of length); closed-loop fixed-target: 1.45mm error (1.7%); some bending directions: up to 8.3mm error (7.8%). Demonstrated feasibility for fixed/moving-target tracking.

Conclusion: Bilinear MPC enables clinically compatible steerable brachytherapy systems but needs calibration and sensing improvements for better accuracy in all bending directions.
...
TL;DR: RoboPlayground enables users to author executable robotic manipulation tasks via natural language, creating structured task families for more comprehensive policy evaluation.
...
Method: RoboPlayground framework allows users to author executable manipulation tasks using natural language within structured physical domains. Language instructions compile into reproducible task specifications with explicit asset definitions, initialization distributions, and success predicates.

Result: User study shows language-driven interface is easier to use with lower cognitive workload than programming-based baselines. Evaluating policies on language-defined task families reveals generalization failures not apparent in fixed benchmarks. Task diversity scales with contributor diversity rather than task count alone.

Conclusion: RoboPlayground reframes robotic manipulation evaluation as a language-driven process, enabling continuous growth of evaluation spaces through crowd-authored contributions and revealing policy limitations that fixed benchmarks miss.
...
TL;DR: Compact 3D-printed deployable actuator achieves reversible scale/stiffness changes through folding/zipping of plastic strips, enabling adaptive robots.


<details>
  <summary>Details</summary>
Motivation: Need for robots that can adapt shape, size, and mechanical properties for evolving tasks/environments, overcoming limitations of current bespoke, non-scalable shape-changing systems.

Method: Compound folding and zipping of flexible 3D-printed plastic strips into square-section deployable beams, allowing smooth transitions between compact (flexible) and expanded (quasi-rigid) states.

Result: Actuator's mechanical performance characterized; integrated system demonstrated with four-module adaptive walking robot showing diverse shape/stiffness transformations.

Conclusion: Compact, easy-to-fabricate deployable actuator enables reversible scale/stiffness transformations, facilitating adaptive robotic systems with scalable, reconfigurable modules.
...
TL;DR: The CISST ERC has transformed medical robotics from handling routine tasks to performing sophisticated interventions, with NSF support enabling infrastructure that will dramatically change surgery and healthcare delivery through improved accuracy, safety, outcomes, and cost-effectiveness.


<details>
  <summary>Details</summary>
Motivation: To advance medical robotics from marginal to mainstream applications, transforming how surgery and medical procedures are performed through technological innovation and clinical system integration.

Method: The CISST ERC (Engineering Research Center for Computer-Integrated Surgical Systems and Technology) was launched in 1998 with NSF funding to develop professional infrastructure and integrate data with technology in clinical systems.

Result: Medical robotics has evolved from handling routine tasks to performing highly sophisticated interventions, with the ERC building infrastructure that enables transformative changes in healthcare delivery across multiple dimensions.

Conclusion: The basic science and engineering developed by the CISST ERC will yield profound benefits for all healthcare stakeholders, with medical robotics poised to dramatically improve surgical procedures, patient outcomes, and overall healthcare system effectiveness.
...
TL;DR: Applying semantic embedding to analyze molecular swarm behaviors in microtubule-kinesin-DNA systems for better explainability and optimization.


<details>
  <summary>Details</summary>
Motivation: To provide richer feature sets for optimizing molecular swarm systems by understanding their behavioral ranges, particularly in cytoskeletal filament systems with DNA functionalization.

Method: Extend microtubule model with DNA interactions, apply semantic embedding to learn behavior ranges from simulations, extract semantic atoms matching expected behaviors, and decompose simulation frames.

Result: Extracted semantic atoms match expected behaviors, frame decomposition accurately describes impact of external control values, enabling better explainability of simulated experiments.

Conclusion: Semantic embedding provides relevant leads toward explainable simulations, making them more reliable for designing and optimizing in-vitro molecular swarm systems.
...
TL;DR: Integrated planning-control framework for AUV navigation using real-time sensor data and pre-designed feedback controllers to enable robust obstacle avoidance in unknown underwater environments with reduced computational complexity.


<details>
  <summary>Details</summary>
Motivation: AUV navigation in unknown underwater environments faces challenges including poor visibility, weak signal transmission, dynamic currents, localization uncertainty, and noisy sensor measurements, requiring robust solutions for obstacle avoidance and maneuverability.

Method: Proposes an integrated planning and control framework that leverages real-time sensor data to dynamically generate closed-loop AUV trajectories. Uses pre-designed feedback controllers for motion planning to reduce computational complexity of online optimizations while ensuring operational safety.

Result: Validated through ROS Gazebo simulations on RexRov AUV. Demonstrated efficacy by comparison against PID-based tracking methods and quantifying localization errors in dead reckoning as AUV transitions into target communication range.

Conclusion: The integrated framework successfully addresses underwater navigation challenges by combining real-time sensing with efficient planning-control, enabling robust obstacle avoidance and enhanced maneuverability while reducing computational burden compared to traditional optimization approaches.
...
TL;DR: ExpressMM framework integrates language-guided planning with vision-language-action policies to generate expressive robot behaviors for mobile manipulators in collaborative human-robot interactions, supporting interruptible interactions and improving human understanding of robot intent.


<details>
  <summary>Details</summary>
Motivation: Mobile manipulators need to communicate intent to humans in collaborative tasks, but existing approaches don't adequately handle interruptible interactions where users may modify or redirect robot actions during execution.

Method: Developed ExpressMM framework combining: 1) high-level language-guided planner using vision-language model for perception and conversational reasoning, and 2) low-level vision-language-action policy to generate expressive behaviors. Supports interruptible interactions for updated user instructions.

Result: Demonstrated on mobile manipulator in collaborative assembly scenario. Questionnaire results showed expressive behaviors helped observers clearly interpret robot's actions/intentions, supported socially appropriate interactions, and improved perceptions of usefulness, safety, and predictability.

Conclusion: ExpressMM successfully enables mobile manipulators to generate expressive behaviors that improve human understanding and collaboration during interruptible human-robot interactions, fostering positive perceptions of robot capabilities in collaborative tasks.
...
TL;DR: AnyImageNav is a training-free system that improves Image Goal Navigation by enabling precise 6-DoF pose recovery using goal images as geometric queries via dense pixel-level correspondences.


<details>
  <summary>Details</summary>
Motivation: Current ImageNav evaluation uses a coarse 1m success criterion, which is insufficient for downstream tasks like grasping that require precise positioning. There's a need for more accurate pose recovery in image goal navigation.

Method: Uses a semantic-to-geometric cascade: semantic relevance guides exploration and acts as proximity gate, invoking a 3D multi-view foundation model only when current view is highly relevant to the goal image. The model self-certifies its registration in a loop for accurate pose recovery.

Result: State-of-the-art navigation success rates: 93.1% on Gibson and 82.6% on HM3D. Achieves precise pose recovery: 0.27m position error and 3.41° heading error on Gibson, 0.21m/1.23° on HM3D - 5-10x improvement over adapted baselines.

Conclusion: AnyImageNav successfully pushes ImageNav toward more demanding precise positioning requirements by treating goal images as geometric queries and using semantic-to-geometric cascades, enabling accurate 6-DoF pose recovery that prior methods couldn't provide.
...
TL;DR: SafeGate is a neurosymbolic safety architecture that prevents unsafe natural language commands from reaching robot execution by combining LLM-based property extraction with deterministic safety gates and runtime constraint monitoring.
...
Method: SafeGate extracts safety-relevant properties from natural language commands using neurosymbolic approach, applies deterministic decision gates based on ISO 13482 standards, creates Task Safety Contracts (invariants, guards, abort conditions), and uses Z3 SMT solving for runtime constraint checking.
...
Conclusion: SafeGate demonstrates the importance of pre-execution safety gates for LLM-controlled robot systems, providing effective safety validation before and during robot task execution.
...
TL;DR: Unified framework for continuous mobile manipulation that synergizes efficiency and reliability through reliability-aware trajectory planning and phase-dependent switching control.


<details>
  <summary>Details</summary>
Motivation: Current robots struggle to replicate human fluidity in mobile manipulation - balancing long-horizon planning with real-time reactivity while maintaining both efficiency and reliability in uncertain environments.

Method: Combines reliability-aware trajectory planner (embeds reliability elements into spatiotemporal optimization) with phase-dependent switching controller (transitions between trajectory tracking and error compensation), plus hierarchical initialization for online replanning.

Result: Enables efficient and reliable completion of successive tasks under uncertainty (dynamic disturbances, perception/control errors), generalizes to diverse end-effector constraints, achieves highest efficiency while improving task success rate by 26.67%-81.67% over baselines.

Conclusion: Presents a unified framework that successfully synergizes efficiency and reliability for continuous mobile manipulation, validated through real-world evaluations and comprehensive ablation studies.
...
TL;DR: CoEnv is a framework for multi-agent robotic collaboration that integrates real and simulated environments to enable safe strategy exploration and reliable deployment through scene reconstruction, VLM-driven planning, and validated transfer.


<details>
  <summary>Details</summary>
Motivation: Multi-agent embodied systems face challenges in spatial coordination, temporal reasoning, and shared workspace awareness. Current approaches lack the ability to separate cognitive planning from physical execution, which humans naturally do during collaboration.

Method: CoEnv introduces a compositional environment integrating real and simulated components. It operates through three stages: 1) real-to-sim scene reconstruction digitizing physical workspaces, 2) VLM-driven action synthesis with both real-time planning (high-level interfaces) and iterative planning (code-based trajectory generation), and 3) validated sim-to-real transfer with collision detection for safe deployment.
...
Conclusion: CoEnv establishes a new paradigm for multi-agent embodied AI by enabling safe strategy exploration in simulation while ensuring reliable real-world deployment through the compositional environment concept.
...
TL;DR: JailWAM is the first jailbreak attack framework for World Action Models (WAMs) that exposes physical vulnerabilities in robotic systems through visual-trajectory mapping and dual-path verification, achieving 84.2% attack success rate.


<details>
  <summary>Details</summary>
Motivation: World Action Models (WAMs) have powerful physical manipulation capabilities but pose serious safety risks if vulnerable to jailbreak attacks. Existing research has largely ignored this critical security gap in robotic systems.

Method: Proposes JailWAM framework with: (1) Visual-Trajectory Mapping to unify action spaces, (2) Risk Discriminator for efficient destructive behavior screening, and (3) Dual-Path Verification Strategy combining coarse screening with full physical simulation. Also introduces JailWAM-Bench benchmark.

Result: The framework efficiently exposes physical vulnerabilities, achieving 84.2% attack success rate on state-of-the-art LingBot-VA in RoboTwin simulation environment. Can also be used to construct robust defense mechanisms.

Conclusion: JailWAM addresses the critical security gap in WAM safety by providing the first dedicated jailbreak attack framework, enabling both vulnerability assessment and defense mechanism design for safe robot control systems.
...
TL;DR: A modular aerial robot system with new mechanical design, virtual quadrotor abstraction, and predictive allocation pipeline enables stable, agile flight across configurations.


<details>
  <summary>Details</summary>
Motivation: Existing MARS control algorithms use simplified quasi-static models and rule-based allocation, leading to discontinuous motor commands and attitude error accumulation that causes severe oscillations during docking, separation, and waypoint tracking as drone units scale.

Method: 1) Compact mechanical system with passive docking, detection-free passive locking, and magnetic-assisted separation using single micro servo; 2) Force-torque-equivalent and polytope-constraint virtual quadrotor abstraction modeling feasible wrench sets; 3) Two-stage predictive-allocation pipeline with constrained predictive tracker and dynamic allocator for smooth motor commands; 4) Yaw angle optimization for maximum control authority.

Result: Simulations across 10+ configurations and real-world experiments demonstrate stable docking, locking, separation, and effective control performance. Achieved 40 deg peak pitch during agile flight and transport with average position error of 0.0896 m - first real-world demonstration of MARS achieving such performance.

Conclusion: The proposed mechanical design, virtual quadrotor abstraction, and predictive allocation pipeline successfully address limitations of existing MARS control, enabling stable, agile flight across different configurations with minimal position error.
...
TL;DR: Developed physics-based simulation and evolutionary optimization for soft conical robotic scooping tools, enabling automatic trajectory optimization without manual tuning.


<details>
  <summary>Details</summary>
Motivation: Soft robotic scooping tools offer adaptability to various container sizes and materials without complex sensing, but their compliance and deformation behavior create significant control challenges that limit practical applications.

Method: Created physics-based simulation model of deformable soft conical robotic hand capturing passive reconfiguration dynamics, combined with evolutionary strategy framework for systematic trajectory optimization without manual parameter tuning.
...
Conclusion: The proposed physics-based simulation and evolutionary optimization approach enables practical application of soft robotic scooping tools by overcoming control complexity limitations.
...
TL;DR: ReV is a closed-loop visuomotor policy that adapts to unforeseen errors by incorporating sparse referring points, using coupled diffusion heads for global anchor generation and local interpolation without needing extra training data.


<details>
  <summary>Details</summary>
Motivation: Addresses the problem of enhancing robustness in visuomotor policies for robotic manipulation when facing out-of-distribution execution errors or dynamic trajectory re-routing, where models only have access to original expert demonstrations for training.

Method: Introduces Referring-Aware Visuomotor Policy (ReV) with coupled diffusion heads: global head generates temporally sparse action anchors and identifies referring point positions, local head adaptively interpolates adjacent anchors. Trained only by applying targeted perturbations to expert demonstrations without elaborate annotations.

Result: Achieves higher success rates across challenging simulated and real-world tasks without any additional data or fine-tuning scheme, enabling real-time trajectory replanning in response to dynamic scene changes.

Conclusion: ReV provides an effective closed-loop framework for robust robotic manipulation that can adapt to unforeseen circumstances by incorporating sparse referring points, demonstrating strong performance with minimal training requirements.
...
TL;DR: DAERT framework uses diversity-aware reinforcement learning to generate varied adversarial instructions that expose vulnerabilities in Vision-Language-Action models, reducing task success rates from 93% to 6%.


<details>
  <summary>Details</summary>
Motivation: Current VLA models have under-explored safety concerns regarding linguistic robustness. Standard RL-based red teaming suffers from mode collapse, converging to narrow failure patterns and failing to reveal comprehensive risks.
...
Result: Method consistently discovers wider range of effective adversarial instructions, reducing average task success rate from 93.33% to 5.85% across different robotic benchmarks against state-of-the-art VLAs (π₀ and OpenVLA).

Conclusion: DAERT provides scalable approach to stress-test VLA agents and expose critical safety blind spots before real-world deployment, addressing mode collapse limitations of standard RL-based red teaming.
...
TL;DR: A 4-DOF flexible laparoscopic instrument with distal bending, head rotation, shaft rotation, and gripper is developed for minimally invasive surgery, featuring SpaceMouse teleoperation on Raspberry Pi 5 and validated through analytical modeling and simulated pancreatic surgery.


<details>
  <summary>Details</summary>
Motivation: Minimally invasive surgery reduces patient trauma and recovery time, but conventional laparoscopic instruments have limited range of movements, creating a need for more flexible surgical tools.

Method: Developed a 4-DOF flexible laparoscopic instrument with distal bending, independent distal head rotation, shaft rotation, and gripper. Implemented actuation unit and SpaceMouse teleoperation on Raspberry Pi 5 with Motoron controllers. Derived analytical scissor-linkage model and validated with CAD measurements and OptiTrack motion capture.

Result: The predicted jaw opening matched CAD measurements with MAE 0.13° and OptiTrack motion capture with MAE 1.43°. Integration with ATHENA parallel robot was validated through simulated pancreatic surgery procedure.

Conclusion: The developed 4-DOF flexible laparoscopic instrument successfully addresses limitations of conventional laparoscopic tools, demonstrating accurate performance through analytical modeling and validation in simulated surgical procedures.
...
TL;DR: Proposes a training framework for hierarchical Vision-Language-Action models that explicitly grounds language descriptions with visual observations and actions using contrastive learning and preference ranking.


<details>
  <summary>Details</summary>
Motivation: Existing hierarchical VLA models generate language and actions but lack explicit alignment between these modalities during training, which is crucial for robot transparency and effective human-robot collaboration.

Method: Uses a contrastive model to assess alignment between generated language and action trajectories, enabling ranking of language-trajectory pairs and refining grounding through offline preference learning.
...
Conclusion: The framework addresses the critical gap in aligning language and action modalities in hierarchical VLA models, establishing a strong baseline for robot transparency with reduced annotation requirements.
...
TL;DR: A leaderless method for collective maneuvering of affine formations in 2D by modifying Laplacian matrix weights, enabling time-varying affine transformations of reference shapes without traditional leader-follower structures.


<details>
  <summary>Details</summary>
Motivation: To enable more flexible collective motions for robot swarms beyond traditional leader-follower approaches, allowing agents to maintain distinct and time-varying velocities for broader range of affine transformations.

Method: Modify original weights of Laplacian matrix used for static formations; use complex number representation for 2D analysis; design eigenvectors and eigenvalues as function of modified weights to achieve desired collective motion.

Result: Analytic solution for collective motion derived; method enables all linear combinations of translations, rotations, scaling and shearing; validated through simulations with up to 20 agents.

Conclusion: Proposed leaderless scheme successfully achieves flexible collective maneuvering of affine formations in 2D, providing analytic design framework for time-varying affine transformations of reference shapes.
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models for robot manipulation are computationally expensive due to large VLM backbones and iterative action heads, making real-time control costly on commodity hardware.

Method: Uses pretrained VLMs with implicit affordance priors, introduces budget-aware adaptive inference with early termination based on action consistency, and proposes Inter-Layer Truncated Flow Matching for warm-starting denoising across layers.

Result: Achieves state-of-the-art success rates while reducing inference cost by up to 72% lower latency for flow-matching and 76.6% backbone computation reduction with minor performance degradation. Outperforms baselines on RoboChallenge with 29.00% success rate.

Conclusion: A1 demonstrates that efficient VLA models can achieve high manipulation performance while significantly reducing computational costs, making real-time robot control more practical on commodity hardware.
...
TL;DR: RSBM enables real-time robotic navigation by reducing integration steps from 10+ to just 3 while maintaining high performance, bridging generative policies with low-latency requirements.
...
Method: Proposes Rectified Schrödinger Bridge Matching (RSBM) that exploits velocity-field invariance across entropic regularization spectrum, uses a single network for all regularization strengths, and anchors to learned conditional prior to shorten transport distance.

Result: Achieves over 94% cosine similarity and 92% success rate with only 3 integration steps (vs. ≥10 for standard methods), without distillation or multi-stage training.

Conclusion: RSBM substantially narrows the gap between high-fidelity generative policies and low-latency demands of Embodied AI by enabling real-time robotic navigation with minimal integration steps.
...
TL;DR: Dynamic control allocation for dual-tilting hexarotor UAV with hierarchical control structure, explicit actuator saturation modeling, and optimization of propeller tilt angles for trajectory tracking.


<details>
  <summary>Details</summary>
Motivation: To develop an effective control allocation strategy for dual-tilting hexarotor UAVs that can handle actuator saturation constraints and optimize propeller tilt angles for improved trajectory tracking performance.

Method: Hierarchical control structure with high-level controller generating required wrench, and control allocation law imposing first-order actuator dynamics while optimizing actuator states through objective function. Explicit modeling of actuator saturation and asymmetric objective functions for propeller tilt angles.

Result: Numerical simulations validate the allocation strategy, showing effective handling of actuator saturation and optimization of propeller tilt angles for trajectory tracking tasks.

Conclusion: The proposed control allocation approach successfully addresses actuator saturation constraints and leverages system redundancy to optimize propeller tilt angles, providing theoretical insights and practical validation for dual-tilting hexarotor UAV control.
...
TL;DR: A pipeline for robotic grasp planning that considers object structural strength, using force maps to select mechanically safe contact points and regulate grip forces.


<details>
  <summary>Details</summary>
Motivation: Existing grasp planners treat object surfaces as structurally homogeneous, ignoring that contact in weak regions can damage objects despite geometrically valid grasps.

Method: Pipeline includes: object identification, 3D reconstruction with SAM3D, physics-informed geometric analysis to compute force maps of locally admissible contact loads, grasp filtering by geometric validity and task goals, force-map-aware grasp ranking, and impedance control scaling finger stiffness based on local force limits.

Result: Validation on paper, plastic, and glass cups shows the approach consistently selects structurally stronger contact regions and keeps grip forces within safe bounds.

Conclusion: Reframes dexterous manipulation from purely geometric problem into physically grounded joint planning of grasp selection and grip execution for humanoid systems.
...
TL;DR: Combines stakeholder-guided process modelling with SHARD and STPA for hazard management in assistive robotics for mammography, identifying timing and interaction hazards rather than component failures.


<details>
  <summary>Details</summary>
Motivation: Robotic systems in clinical settings with vulnerable patients pose significant safety risks, requiring systematic hazard management from early development stages to ensure safe human-robot interaction.

Method: Combines stakeholder-guided process modelling with clinicians, roboticists, and patients, then applies Software Hazard Analysis and Resolution in Design (SHARD) for technical/procedural deviations and System-Theoretic Process Analysis (STPA) for unsafe control actions in user interaction.

Result: Most hazards arise from timing mismatches, premature actions, and misinterpretation of system state rather than component failures. These findings translate into refined safety requirements that constrain system behavior and reduce reliance on correct human timing/interpretation.

Conclusion: Presents a structured, traceable approach to safety-driven design for assistive robotic systems in clinical environments, demonstrating applicability beyond mammography to similar clinical robotic applications.
...
TL;DR: RL-trained sensorimotor policies enable quadrotors to perform aggressive SE(3) maneuvers through narrow gaps using only onboard vision and proprioception, achieving high precision (5cm clearance) and handling moving gaps.


<details>
  <summary>Details</summary>
Motivation: Precise aggressive maneuvers with lightweight sensors remain a bottleneck for exploiting drone maneuverability, especially for navigating through narrow openings to expand accessible areas. A key challenge is aggressive traversal through narrow gaps under SE(3) constraints requiring tilted attitudes and airframe asymmetry.

Method: Develop sensorimotor policies mapping onboard vision and proprioception directly to low-level control commands. Train using reinforcement learning with end-to-end policy distillation in simulation. Use model-based planner trajectories for initialization to overcome exploration challenges. Implement careful sim-to-real transfer design.

Result: Quadrotor navigates rectangular gaps at 5cm clearance with up to 90-degree tilt, without knowing gap position/orientation. Policy handles moving gaps reactively without specific training. Validated on challenging tracks of closely placed narrow gaps. Policies work for geometrically diverse gaps without manual feature definitions.

Conclusion: The proposed RL-based sensorimotor policy approach enables aggressive SE(3) gap traversal with high precision and repeatability using only onboard sensors, demonstrating flexibility across diverse gap geometries and dynamic scenarios without requiring manual pose definitions.
...
TL;DR: BiCoord is a new benchmark for long-horizon, tightly coordinated bimanual manipulation tasks with quantitative coordination metrics, revealing limitations of current policies.


<details>
  <summary>Details</summary>
Motivation: Existing bimanual manipulation benchmarks focus on short-horizon, loosely coordinated tasks, failing to capture the spatial-temporal coupling inherent in real-world bimanual behaviors that require continuous inter-arm dependency and dynamic role exchange.

Method: Introduces BiCoord benchmark with diverse long-horizon tasks requiring tight coordination, plus a suite of quantitative metrics evaluating coordination from temporal, spatial, and spatial-temporal perspectives.

Result: Experimental results show representative manipulation policies (DP, RDT, Pi0, OpenVLA-OFT) struggle with long-duration, highly coupled tasks, revealing fundamental challenges in achieving long-horizon tight coordination.
...
TL;DR: A dialogue-based framework for explaining safety decisions in human-robot collaboration that allows users to ask why, why not, and what-if questions about safety interventions while maintaining certified safety guarantees.


<details>
  <summary>Details</summary>
Motivation: As robots operate in shared safety-critical environments, they need to not only act safely but also make their safety decisions intelligible to human collaborators. Current safety behaviors (like stopping or switching modes) are triggered by internal constraints that remain opaque to nearby workers, creating communication gaps in human-robot collaboration.

Method: A dialogue-based framework that tightly couples explanation with constraint-based safety evaluation. Explanations are derived directly from recorded decision traces, enabling users to pose causal ("Why?"), contrastive ("Why not?"), and counterfactual ("What if?") queries. Counterfactual reasoning is evaluated in a bounded manner under fixed, certified safety parameters to ensure interactive exploration doesn't relax operational safety guarantees.

Result: The framework is instantiated in a construction robotics scenario with structured operational traces showing how constraint-aware dialogue clarifies safety interventions and supports coordinated task recovery. The approach treats explanation as an operational interface to safety control.

Conclusion: This work advances a design perspective for interactive, safety-aware autonomy in human-robot collaboration by making safety decisions transparent and explainable while maintaining certified safety guarantees, enabling better human-robot coordination and task recovery.
...
TL;DR: Using audio sensors on fingertips to detect contact events improves manipulation policies for button pressing by reducing contact force without creating deployment dependencies.


<details>
  <summary>Details</summary>
Motivation: Contact-rich manipulation is challenging from cameras and proprioception alone because contact events are only partially observed. The paper explores whether training-time instrumentation (object sensorisation) can improve policy performance without creating deployment-time dependencies.

Method: Use a microphone fingertip to capture contact-relevant audio during button pressing. Employ an instrumented button-state signal as privileged supervision to fine-tune an audio encoder into a contact event detector. Combine this representation with imitation learning using three strategies, ensuring the policy only uses vision and audio during inference.

Result: Button press success rates are similar across methods, but instrumentation-guided audio representations consistently reduce contact force. This demonstrates that audio-based contact detection improves manipulation quality.

Conclusion: Instrumentation serves as a practical training-time auxiliary objective for learning contact-rich manipulation policies, improving performance without creating deployment dependencies on specialized sensors.
...
TL;DR: Natural language + gestures → robot programs via LLMs/CV with interactive feedback for safety and user acceptance


<details>
  <summary>Details</summary>
Motivation: Enable intuitive end-user robot programming through natural human-like interaction methods rather than traditional programming approaches

Method: Combine large language models (LLMs) for natural language understanding with computer vision (CV) for gesture recognition to translate human interactions into robot programs

Result: System provides natural feedback through clarification questions and visual representations, allowing users to review and adjust generated programs

Conclusion: This approach ensures safety, transparency, and user acceptance by making robot programming accessible through familiar human interaction patterns
...
TL;DR: Lightweight control framework for tilting hexarotor UAVs using offline-computed force polytopes to optimize cant angles for physical interaction tasks, balancing efficiency and smoothness.


<details>
  <summary>Details</summary>
Motivation: Tilting multirotor UAVs offer dynamic variability in feasible executable wrench for better maneuverability in physical interaction tasks, but cant-angle selection needs optimization to ensure high performance while avoiding abrupt variations and preserving real-world feasibility.

Method: Proposes a lightweight control framework using offline-computed look-up table of zero-moment force polytopes to identify feasible cant angles for desired control force, selecting optimal angle by balancing efficiency and smoothness, integrated with geometric full-pose controller.

Result: Significant reduction in computation time, improved pose-tracking performance, and competitive actuation efficiency compared to baseline strategy, validated through Monte Carlo simulations and physics-based simulation of wall inspection task.

Conclusion: The proposed framework provides an effective solution for optimizing cant-angle selection in tilting hexarotor UAVs for physical interaction tasks, demonstrating feasibility in real-world interacting scenarios with computational efficiency and improved performance.
...
TL;DR: A co-design framework for planar five-bar monoped robots that jointly optimizes mechanical design, actuator parameters (motor/gearbox), and control for dynamic jumping, achieving 42% jump distance improvement and 15.8% energy reduction.


<details>
  <summary>Details</summary>
Motivation: Existing co-design approaches for legged robots focus mainly on link dimensions and transmission ratios while neglecting detailed actuator design and are limited to serial open-chain mechanisms, leaving a gap for comprehensive optimization of closed-chain systems with full actuator parameter consideration.

Method: Two-stage optimization framework: first stage optimizes actuator parameters (motor and gearbox) to create a mapping from gear ratio to actuator mass, efficiency, and peak torque; second stage uses CMA-ES to co-design robot mechanical design and control parameters for dynamic jumping.

Result: Simulation results show approximately 42% improvement in jump distance and 15.8% reduction in mechanical energy consumption compared to a nominal design, demonstrating significant performance gains.

Conclusion: The proposed co-design framework effectively identifies optimal design, actuator, and control parameters for high-performance and energy-efficient planar jumping robots, addressing limitations of previous approaches by including detailed actuator optimization for closed-chain mechanisms.
...
TL;DR: HiPolicy: Hierarchical multi-frequency action chunking framework for robotic imitation learning that captures both long-horizon planning and fine-grained control through adaptive frequency execution.


<details>
  <summary>Details</summary>
Motivation: Robotic imitation learning faces a fundamental trade-off between modeling long-horizon dependencies and enabling fine-grained closed-loop control. Existing fixed-frequency action chunking approaches struggle to achieve both simultaneously.

Method: Hierarchical multi-frequency action chunking framework that jointly predicts action sequences at different frequencies, extracts and fuses hierarchical features from history observations aligned to each frequency, and uses entropy-guided execution mechanism to adaptively balance planning vs. control based on action uncertainty.
...
Conclusion: HiPolicy effectively addresses the trade-off between long-horizon planning and fine-grained control in robotic imitation learning through hierarchical multi-frequency action chunking with adaptive execution.
...
TL;DR: Gesture-based UI using pointing and click gestures for object selection, evaluated with 20 participants showing good accuracy and selection time for efficient human-robot collaboration.


<details>
  <summary>Details</summary>
Motivation: Gestures are natural human communication that can enhance human-robot interaction, providing intuitive interfaces for object selection tasks.
...
Result: The experiment demonstrated good accuracy and selection time performance, showing the system's potential for efficient human-robot collaboration.

Conclusion: Gesture-based interfaces using pointing and click gestures are effective for object selection in human-robot interaction, enabling natural and efficient collaboration.
...
TL;DR: Integration of diffusion motion priors with force-feedback MPC enables reliable industrial deburring with precise force tracking and collision avoidance.


<details>
  <summary>Details</summary>
Motivation: Classical MPC formulations neglect real-time force feedback and struggle with contact-rich industrial tasks like deburring, which require precise tool insertion, stable force regulation, and collision-free motions in challenging configurations.
...
Conclusion: First integration of diffusion motion priors with force-feedback MPC for collision-aware, contact-rich industrial tasks, addressing limitations of standard MPC pipelines for complex industrial applications.
...
<details>
  <summary>Details</summary>
Motivation: To create an affordable, easy-to-assemble 6-DOF force/torque sensing solution as an alternative to expensive commercial options, enabling wider adoption in robotics applications requiring contact-rich manipulation.

Method: Combines antagonistic springs with magnetic encoders in a fully 3D-printed design using off-the-shelf parts. Uses parametric analytical modeling for performance extension and lightweight sequence models (neural networks) for error reduction through calibration.

Result: Prototype withstands ±14.4N forces and ±0.33N·m torques per axis. Achieves 7% FS error without calibration, reduced to 3.8% FS with best-performing network. Validated in buffing curved surfaces and tight assembly tasks with force-impedance control.

Conclusion: Delta6 provides a robust, low-cost alternative to existing 6-DOF force sensing solutions, with adjustable performance trade-offs and proven effectiveness in contact-rich robotic applications.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with systematic reasoning and hallucinate unfounded claims, showing brittle pattern-matching rather than true reasoning. The epistemic gap - inability to ground claims in traceable evidence - limits AI reliability in domains requiring justification.

Method: Fine-tune Llama 3.2-3B and DeepSeek-R1-Distill-Llama-8B on 55 Nyaya-structured logical problems using Navya-Nyaya's 6-phase reasoning framework: SAMSHAYA (doubt analysis), PRAMANA (evidence source identification), PANCHA AVAYAVA (5-member syllogism), TARKA (counterfactual verification), HETVABHASA (fallacy detection), and NIRNAYA (ascertainment).

Result: Stage 1 achieves 100% semantic correctness on held-out evaluation despite only 40% strict format adherence, showing models internalize reasoning content even with imperfect structural enforcement. Ablation studies reveal format prompting and temperature critically affect performance.

Conclusion: Integrating ancient Indian logic with modern LLMs provides cognitive scaffolding for systematic reasoning, addressing epistemic gaps. The approach demonstrates that explicit epistemological methodology can improve AI reasoning reliability, with models, datasets, and infrastructure released for further research.
...
TL;DR: Metacognitive evaluations are inherently sequential and may exhibit genuine non-commutativity that cannot be explained by classical state changes alone.


<details>
  <summary>Details</summary>
Motivation: To distinguish between classical state changes and deeper structural non-commutativity in metacognitive order effects, since it's unclear whether observed sequential dependencies reflect just state updates or fundamental non-commutative structure.

Method: Develop operational framework modeling metacognitive evaluations as state-transforming operations with probabilistic readouts, separating evaluation back-action from observable output. Introduce assumptions of counterfactual definiteness and evaluation non-invasiveness to derive testable constraints on sequential correlations.

Result: Show order dependence prevents faithful Boolean-commutative representation. Provide explicit 3D rotation model that violates classical constraints, demonstrating genuine non-commutativity. Outline behavioral paradigm for empirical testing.

Conclusion: Metacognitive evaluations can exhibit genuine non-commutativity that cannot be explained by classical latent variable models, requiring operational algebraic framework rather than quantum physical interpretations.
...
<details>
  <summary>Details</summary>
Motivation: Need to determine whether information objects from independent sources refer to the same physical observation object, accounting for measurement errors in both quantitative and qualitative features without requiring feature transformation for comparability.

Method: Uses probabilistic measure for quantitative feature proximity and possibility measure for qualitative feature proximity; combines these into overall proximity measures for information objects; validates compliance with measure axioms.

Result: Demonstrates feasibility by checking compliance with required axioms; shows approach works without feature value transformation unlike many existing measures; proposes several variants for determining IO proximity.

Conclusion: Proposed measure effectively handles mixed quantitative-qualitative features from multiple sources to determine object identity, offering advantages over transformation-requiring approaches while maintaining mathematical validity.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM applications for heuristic design rely on one-shot code synthesis, producing brittle heuristics that don't fully utilize LLMs' capacity for iterative reasoning. There's a need for more robust and effective automated heuristic design approaches.

Method: ReVEL combines LLMs with evolutionary algorithms through two key mechanisms: (1) performance-profile grouping to cluster candidate heuristics into behaviorally coherent groups for compact feedback, and (2) multi-turn, feedback-driven reflection where LLMs analyze group behaviors and generate targeted heuristic refinements, with an EA-based meta-controller balancing exploration and exploitation.
...
TL;DR: Algebraic structure in combinatorial optimization problems can be exploited to shrink search spaces and improve optimization performance through quotient-space formulations.


<details>
  <summary>Details</summary>
Motivation: Many combinatorial optimization problems contain hidden algebraic structures that, when identified and exploited, can reduce search space size and improve the likelihood of finding global optimal solutions.

Method: A general framework that: (1) identifies algebraic structure, (2) formalizes operations, (3) constructs quotient spaces that collapse redundant representations, and (4) optimizes directly over these reduced spaces. For rule-combination tasks, conjunctive rules form a monoid, and via characteristic-vector encoding, an isomorphism to the Boolean hypercube with bitwise OR is established.
...
Conclusion: Exposing and exploiting algebraic structure offers a simple, general route to more efficient combinatorial optimization across various problem domains.
...
TL;DR: PaperOrchestra is a multi-agent framework that automatically synthesizes unstructured research materials into submission-ready LaTeX manuscripts with comprehensive literature reviews and generated visuals, outperforming existing autonomous writing systems.


<details>
  <summary>Details</summary>
Motivation: Current AI-driven scientific writing systems are rigidly coupled to specific experimental pipelines and produce superficial literature reviews, creating a need for more flexible and comprehensive automated paper writing solutions.

Method: Introduces PaperOrchestra, a multi-agent framework that transforms unconstrained pre-writing materials into complete LaTeX manuscripts, including literature synthesis and generated visuals like plots and diagrams.

Result: PaperOrchestra significantly outperforms autonomous baselines with 50%-68% absolute win rate margin in literature review quality and 14%-38% in overall manuscript quality in human evaluations.

Conclusion: PaperOrchestra represents a significant advancement in automated AI research paper writing, offering flexible transformation of unstructured materials into high-quality manuscripts with comprehensive literature synthesis.
...
TL;DR: A generative framework that creates animatable 3D Gaussian vehicles from single images or sparse multi-view inputs, enabling part-level articulation for autonomous driving simulation.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving simulation frameworks model vehicles as rigid assets without part-level articulation, while perception algorithms increasingly use dynamics like wheel steering or door opening. Existing CAD-based pipelines have limited coverage and fixed templates, preventing faithful reconstruction of real-world vehicles.

Method: Proposes a generative framework with two key components: (1) a part-edge refinement module that enforces exclusive Gaussian ownership to prevent distortions at part boundaries during animation, and (2) a kinematic reasoning head that predicts joint positions and hinge axes of movable parts.

Result: The method enables faithful part-aware simulation by bridging the gap between static 3D asset generation and animatable vehicle models, allowing realistic articulation of vehicle parts from limited input data.

Conclusion: The framework successfully addresses the limitations of current simulation approaches by creating animatable 3D Gaussian vehicles that capture part-level articulation, essential for realistic autonomous driving simulation that leverages vehicle dynamics.
...
<details>
  <summary>Details</summary>
Motivation: Multi-objective retrosynthesis planning requires balancing quality, safety, and cost objectives, and language model-based multi-agent systems offer a promising approach to incorporate these multiple objectives through specialized agent interactions.

Method: MMORF provides modular agentic components that can be flexibly combined and configured into different multi-agent systems, enabling principled evaluation and comparison of system designs. Two representative systems were built: MASIL and RFAS.

Result: On a new benchmark of 218 multi-objective retrosynthesis tasks, MASIL achieved strong safety and cost metrics on soft-constraint tasks and frequently Pareto-dominated baseline routes, while RFAS achieved 48.6% success rate on hard-constraint tasks, outperforming state-of-the-art baselines.

Conclusion: MMORF is an effective foundational framework for exploring multi-agent systems for multi-objective retrosynthesis planning, with both representative systems demonstrating strong performance on different constraint types.
...
TL;DR: MedGemma 1.5 4B is an enhanced medical AI model that adds support for high-dimensional medical imaging (CT/MRI volumes, histopathology slides), anatomical localization, multi-timepoint chest X-ray analysis, and improved medical document understanding, with significant performance gains across all new modalities.


<details>
  <summary>Details</summary>
Motivation: To expand medical AI capabilities beyond MedGemma 1 by integrating additional medical modalities including 3D imaging, pathology slides, and complex medical document types, creating a more comprehensive foundation model for medical applications.

Method: Built on MedGemma 1 architecture with innovations including new training data, long-context 3D volume slicing for CT/MRI, whole-slide pathology sampling techniques, and architectural modifications to handle multiple medical modalities within a single model.

Result: Significant improvements across all new modalities: 11% gain in 3D MRI classification, 3% in 3D CT classification, 47% macro F1 gain in pathology imaging, 35% IoU improvement in anatomical localization, 4% accuracy for multi-timepoint chest X-rays, plus 5% MedQA and 22% EHRQA accuracy improvements in text-based tasks.

Conclusion: MedGemma 1.5 serves as a robust open resource that significantly advances multimodal medical AI capabilities, providing an improved foundation for developers to build next-generation medical AI systems with comprehensive support for diverse medical data types.
...
TL;DR: LLMs struggle with sequential clinical diagnosis; new LDTL framework uses planning and diagnostic agents with latent trajectory learning to improve accuracy while reducing tests.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based diagnostic systems assume fully observed patient information and don't model sequential evidence acquisition. Clinical diagnosis requires sequential decision-making under uncertainty, but learning effective diagnostic trajectories is challenging due to large search spaces and lack of explicit supervision for optimal paths.

Method: Proposes Latent Diagnostic Trajectory Learning (LDTL) framework with two LLM agents: diagnostic agent treats action sequences as latent paths with posterior distribution prioritizing informative trajectories; planning agent is trained to follow this distribution to create coherent diagnostic paths that progressively reduce uncertainty.

Result: On MIMIC-CDM benchmark, LDTL outperforms existing baselines in diagnostic accuracy under sequential clinical diagnosis setting while requiring fewer diagnostic tests. Ablation studies show trajectory-level posterior alignment is critical for improvements.

Conclusion: LDTL framework effectively addresses sequential clinical diagnosis by learning latent diagnostic trajectories, improving accuracy and efficiency through explicit modeling of evidence acquisition paths and uncertainty reduction.
...
TL;DR: KA-FCM replaces static scalar weights in Fuzzy Cognitive Maps with learnable B-spline functions on edges, enabling modeling of non-monotonic causal relationships while preserving interpretability.


<details>
  <summary>Details</summary>
Motivation: Standard FCMs are limited to monotonic causal dependencies due to scalar weights and monotonic activation functions, restricting their ability to model systems with saturation effects or periodic dynamics.
...
TL;DR: AI evolution through self-improvement differs from biological evolution due to directed design, leading to potential deception risks when fitness doesn't align with human utility.
...
Method: Developed mathematical model replacing random mutations with directed tree of AI programs, where current programs design descendants and humans control fitness function allocating computational resources.

Result: Evolutionary dynamics reflect long-run growth potential, not just current fitness. Without assumptions, fitness may not increase, but with bounded fitness and locked copy probability, fitness concentrates on maximum reachable value.

Conclusion: When deception increases fitness beyond genuine utility, evolution selects for deception. Risk could be mitigated by basing reproduction on objective criteria rather than human judgment.
...
<details>
  <summary>Details</summary>
Motivation: Current Computer-Use Agents (CUAs) generate GUI actions without evaluating action quality, leading to irreversible errors that cascade through subsequent steps. There's a need for a system that can assess action quality before execution to prevent such errors.

Method: IntentScore trains on 398K offline GUI interaction steps across three operating systems using two complementary objectives: contrastive alignment for state-action relevance and margin ranking for action correctness. It embeds planning intent in the action encoder to distinguish between candidates with similar actions but different rationales.

Result: IntentScore achieves 97.5% pairwise discrimination accuracy on held-out evaluation. When deployed as a re-ranker for Agent S3 on OSWorld (an environment unseen during training), it improves task success rate by 6.9 points.

Conclusion: Reward estimation learned from heterogeneous offline trajectories can effectively generalize to unseen agents and task distributions, demonstrating that plan-aware action scoring significantly improves the reliability of computer-use agents.
...
TL;DR: MARL framework enables CSI-free RIS control using mechanical reflector arrays, achieving 26.86 dB gain over static reflectors with deployment resilience to localization noise.


<details>
  <summary>Details</summary>
Motivation: Practical RIS deployment is bottlenecked by computational overhead of CSI estimation, requiring a new paradigm to bypass this fundamental physical-layer barrier.

Method: Multi-Agent Reinforcement Learning (MARL) framework with Centralized Training with Decentralized Execution (CTDE) architecture using MAPPO. Maps mechanical constraints to virtual focal point space for cooperative beam-focusing using only user coordinates.

Result: Achieves up to 26.86 dB enhancement over static flat reflectors, outperforms single-agent and hardware-constrained DRL baselines in spatial selectivity and temporal stability. Sustains stable coverage even under 1.0-meter localization noise.

Conclusion: MARL-driven spatial abstractions provide a scalable, practical pathway toward AI-empowered wireless networks with CSI-free operation.
...
TL;DR: HMARL framework for RIS control using user localization instead of CSI, achieving 7.79 dB RSSI improvement over centralized baselines with robust scalability and error tolerance.
...
Method: Hierarchical Multi-Agent RL with CSI-free paradigm using user localization data. Two-tier architecture: high-level controller for user-to-reflector allocations, low-level controllers using MAPPO with CTDE for continuous focal point optimization.

Result: Achieves 7.79 dB RSSI improvement over centralized baselines, demonstrates robust multi-user scalability, and maintains performance under sub-meter localization errors.
...
TL;DR: Domain-adapted LLM framework using chain-of-thought reasoning achieves high-fidelity parsing of heterogeneous HPC system logs with 8B-parameter model, matching performance of much larger models while enabling privacy-preserving local deployment.


<details>
  <summary>Details</summary>
Motivation: Leadership-class HPC systems generate massive volumes of unstructured, heterogeneous logs from diverse software/hardware layers with inconsistent formats, making pattern discovery and anomaly diagnosis extremely challenging. LLMs offer promising direction for automated log understanding.

Method: Developed domain-adapted, instruction-following LLM framework using chain-of-thought reasoning. Fine-tuned 8B-parameter LLaMA model with domain-specific log-template data and instruction-tuned examples. Created hybrid fine-tuning methodology for privacy-preserving, locally deployable log-mining.

Result: Achieved parsing accuracy on par with significantly larger models (LLaMA 70B and Anthropic's Claude) on LogHub datasets. Successfully parsed over 600 million production logs from Frontier supercomputer over 4 weeks, uncovering critical patterns in temporal dynamics, node-level anomalies, and workload-error correlations.

Conclusion: The domain-adapted LLM framework enables robust, efficient log parsing for HPC environments, transforming raw telemetry into actionable insights for operational pattern discovery, anomaly diagnosis, and scalable system analysis while maintaining privacy and energy efficiency.
...
TL;DR: ClawsBench is a benchmark for evaluating LLM agents in realistic productivity settings with mock services (Gmail, Slack, Calendar, Docs, Drive), 44 structured tasks, and state management to measure both task success and safety risks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks use simplified environments and fail to capture realistic, stateful, multi-service workflows needed for evaluating LLM agents in productivity tasks, while live service testing is risky due to irreversible changes.

Method: Created ClawsBench with 5 high-fidelity mock services with state management and deterministic snapshot/restore. Decomposed agent scaffolding into domain skills (API knowledge injection) and meta prompt (cross-service coordination), testing across 6 models, 4 agent harnesses, and 33 conditions.

Result: With full scaffolding, agents achieved 39-64% task success but had 7-33% unsafe action rates. On OpenClaw, top models showed 53-63% success with 7-23% unsafe actions, with no consistent ordering between success and safety metrics. Identified 8 patterns of unsafe behavior.

Conclusion: ClawsBench enables realistic evaluation of LLM agents in productivity settings, revealing significant safety concerns even with high task success rates, highlighting the need for separate evaluation of task performance and safety.
...
TL;DR: AttriBench is a new benchmark dataset for evaluating quote attribution in LLMs, balancing author fame and demographics to reveal systematic biases in attribution accuracy and suppression across demographic groups.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly used for search and information retrieval, it's critical they accurately attribute content to original authors. Current benchmarks don't systematically examine demographic biases in quote attribution.
...
Conclusion: Quote attribution serves as a benchmark for representational fairness in LLMs. The findings reveal systematic demographic biases that need addressing as LLMs become more integrated into information retrieval systems.
...
TL;DR: A layered translation method connects AI governance standards to implementable runtime guardrails for agentic AI systems, distinguishing governance objectives, technical controls, runtime guardrails, and assurance evidence.


<details>
  <summary>Details</summary>
Motivation: Agentic AI systems create unique governance challenges because risks emerge during execution, not just at development or deployment time. Existing AI governance standards are relevant but don't provide implementable runtime guardrails.
...
Result: The method provides a systematic approach to translate governance standards into actionable controls across architecture, runtime policy, human escalation, and audit layers, with runtime guardrails reserved for controls that are observable, determinate, and time-sensitive.

Conclusion: Standards should guide control placement across multiple layers, while runtime guardrails should only be used for controls that justify execution-time intervention based on observability, determinacy, and time-sensitivity criteria.
...
TL;DR: Hybrid deep learning framework combining Transformer and Graph Attention Network for proactive supply chain delay prediction, achieving state-of-the-art performance with strong stability.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for predicting delivery delays either ignore network topology (tabular classification) or overlook spatial dependencies (time-series anomaly detection), failing to capture the complex spatiotemporal nature of supply chain operations.
...
Conclusion: The proposed hybrid framework effectively captures both temporal dynamics and spatial dependencies in supply chain networks, providing a robust solution for proactive risk management with strong predictive performance and stability.
...
TL;DR: Evolutionary modeling shows alignment testing can fix deceptive beliefs in AI populations over time, even with high test-true value correlation, requiring improved evaluators and adaptive tests to reduce deception.


<details>
  <summary>Details</summary>
Motivation: Current model alignment is evaluated in isolation through benchmarks, ignoring how alignment affects populations of models over time and potentially fixes deceptive beliefs through evolutionary processes.

Method: Applied evolutionary theory to model populations of beliefs with both alignment signals (test performance) and true values (actual impact), examining how different selection methodologies can fix deceptive beliefs through iterative alignment testing.

Result: Even with high correlation between testing accuracy and true value (ρ=0.8), deceptive beliefs can become fixed. Mutations enable more complex deceptive developments. Only combining improved evaluator capabilities, adaptive test design, and mutational dynamics significantly reduces deception while maintaining alignment fitness.

Conclusion: Alignment testing needs to evolve beyond static benchmarks to include population dynamics, requiring continuous improvement of evaluator capabilities and adaptive test design to prevent fixation of maliciously deceptive models while maintaining alignment.
...
TL;DR: The paper introduces GRPO, a reward decomposition method that reduces sycophancy in LLMs by separating pressure resistance from evidence responsiveness, achieving significant improvements across multiple models.


<details>
  <summary>Details</summary>
Motivation: Standard alignment methods fail to address sycophancy because scalar reward models conflate two distinct failure modes: pressure capitulation (changing correct answers under social pressure) and evidence blindness (ignoring context). This conflation prevents effective correction of the tendency to shift positions toward perceived user preferences regardless of evidence.

Method: Proposes Group Relative Policy Optimisation (GRPO) with a multi-component reward that decomposes training signal into five terms: pressure resistance, context fidelity, position consistency, agreement suppression, and factual correctness. Uses contrastive dataset pairing pressure-free baselines with pressured variants across three authority levels and two opposing evidence contexts in a two-phase training pipeline.

Result: Across five base models, the pipeline consistently reduces sycophancy on all metric axes. Ablations confirm each reward term governs an independent behavioral dimension. The learned pressure resistance generalizes beyond training methodology, reducing answer-priming sycophancy by up to 17 points on SycophancyEval despite absence of such pressure forms during training.

Conclusion: Reward decomposition via GRPO effectively addresses sycophancy by disentangling pressure resistance from evidence responsiveness, providing a framework for more nuanced alignment that reduces the tendency to capitulate to authority cues while maintaining evidence-based reasoning.
...
TL;DR: MRVF addresses value factorization's suboptimal convergence by introducing stable point theory and iterative filtering of inferior actions through multi-round optimization.


<details>
  <summary>Details</summary>
Motivation: Value factorization in MARL suffers from unexplained convergence to suboptimal solutions, with existing analyses focusing only on optimal cases and failing to address the root causes of poor performance.

Method: Introduces stable point theory to analyze convergence, proposes Multi-Round Value Factorization (MRVF) that iteratively filters suboptimal actions by measuring payoff increments relative to previous actions, transforming inferior actions into unstable points.

Result: MRVF demonstrates superiority over state-of-the-art methods on challenging benchmarks including predator-prey tasks and StarCraft II Multi-Agent Challenge (SMAC), validating the stable point analysis.

Conclusion: The stable point framework explains value factorization's suboptimal convergence, and MRVF's iterative filtering approach provides a practical solution for achieving global optimality in MARL.
...
TL;DR: GoS is a structural retrieval layer for large skill libraries that uses graph-based inference-time retrieval to improve agent performance while reducing token costs.


<details>
  <summary>Details</summary>
Motivation: As skill libraries scale to thousands of skills for real-world agent systems, loading the full skill set saturates context windows, increasing token costs, hallucination, and latency.

Method: Constructs an executable skill graph offline from skill packages, then at inference time retrieves a bounded, dependency-aware skill bundle using hybrid semantic-lexical seeding, reverse-weighted Personalized PageRank, and context-budgeted hydration.
...
TL;DR: TRACE is an end-to-end system that identifies agent capability deficits from failed trajectories, creates targeted training environments for those capabilities, and trains specialized LoRA adapters via RL to improve agent performance.


<details>
  <summary>Details</summary>
Motivation: Current approaches for improving LLM agents either use non-targeted synthetic training data or require agents to implicitly learn capabilities across tasks, lacking systematic identification and targeted training for specific capability deficits.
...
Conclusion: TRACE provides an effective end-to-end system for environment-specific agent self-improvement by automatically identifying capability deficits and creating targeted training, demonstrating strong generalization across different environments with efficient scaling.
...
TL;DR: Agentic AI profiler classifies natural language responses into four expertise levels using LLaMA v3.1, achieving 83-97% alignment with self-assessments across static and dynamic evaluation phases.


<details>
  <summary>Details</summary>
Motivation: Modern AI systems need to be context-aware and adapt to users' varying expertise levels for meaningful human-machine interaction, requiring accurate expertise classification.

Method: Modular layered architecture built on LLaMA v3.1 (8B) with text preprocessing, scoring, aggregation, and classification components. Evaluated in two phases: static (82 pre-recorded transcripts) and dynamic (402 live interviews with agentic AI interviewer assessing expertise after each response).

Result: 83% to 97% of profiler evaluations matched participant self-assessments across domains. Differences attributed to self-rating bias, unclear responses, and occasional misinterpretation of nuanced expertise by the language model.

Conclusion: The agentic AI profiler effectively classifies user expertise levels from natural language responses, demonstrating high alignment with self-assessments and potential for improving adaptive human-machine interaction systems.
...
<details>
  <summary>Details</summary>
Motivation: Hallucinations in medical LLMs are safety-critical, especially when evidence is insufficient or conflicting, requiring better detection methods for diabetic retinopathy decision settings.

Method: Introduce RETINA-SAFE benchmark with 12,522 samples across three evidence-relation tasks, and propose ECRT - a two-stage white-box detection framework using internal representation and logit shifts under CTX/NOCTX conditions with class-balanced training.

Result: ECRT provides strong Stage-1 risk triage and explicit subtype attribution, improves balanced accuracy by +0.15 to +0.19 over uncertainty/self-consistency baselines and +0.02 to +0.07 over strongest supervised baseline, outperforming single-stage ablation.

Conclusion: White-box internal signals grounded in retinal evidence offer a practical route to interpretable medical LLM risk triage for safer clinical decision-making.
...
TL;DR: ETR improves reasoning efficiency by optimizing entropy trends in chain-of-thought reasoning, achieving better accuracy with shorter reasoning traces.


<details>
  <summary>Details</summary>
Motivation: Current CoT reasoning produces excessively long and inefficient reasoning traces. Existing methods assume low uncertainty is always desirable, but the authors find that reasoning efficiency depends on the trajectory of uncertainty rather than just minimizing it globally.
...
Conclusion: Reasoning efficiency is governed by uncertainty trajectory, not just global entropy reduction. ETR effectively optimizes this trajectory for more efficient reasoning.
...
TL;DR: LatentAudit is a white-box auditor that uses residual-stream activations to detect hallucinations in RAG systems without needing auxiliary models, achieving high AUROC with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: RAG systems still hallucinate even with retrieved evidence, requiring real-time detection of whether generated answers are actually supported by the evidence.

Method: Uses Mahalanobis distance between mid-to-late residual-stream activations and evidence representation from open-weight generators, requiring no auxiliary judge model and calibrating on small held-out sets.

Result: Achieves 0.942 AUROC on PubMedQA with Llama-3-8B (0.77ms overhead), maintains 0.9566-0.9815 AUROC under stress tests, and preserves 99.8% performance at 16-bit fixed-point for verifiable deployment.

Conclusion: Residual-stream geometry provides practical basis for real-time RAG faithfulness monitoring and enables optional verifiable deployment without revealing model weights or activations.
...
TL;DR: TFRBench is the first benchmark evaluating reasoning capabilities in forecasting systems, moving beyond numerical accuracy to assess cross-channel dependencies, trends, and external event analysis.


<details>
  <summary>Details</summary>
Motivation: Traditional time-series forecasting evaluation treats models as black boxes focused only on numerical accuracy, lacking assessment of reasoning capabilities that explain why forecasts are made.

Method: Proposes a multi-agent framework with iterative verification loop to synthesize numerically grounded reasoning traces, spanning ten datasets across five domains.
...
Conclusion: TFRBench establishes a new standard for interpretable, reasoning-based evaluation in time-series forecasting, addressing limitations of traditional black-box numerical evaluation.
...
TL;DR: LLM can serve as reliable semantic judge for monitoring drone-based power line segmentation quality, showing consistent judgments and appropriate sensitivity to visual degradation.


<details>
  <summary>Details</summary>
Motivation: Lightweight segmentation models on drones for power line inspection degrade unpredictably in real-world conditions, raising safety concerns. Need reliable method to assess segmentation quality without building new inspection systems.

Method: Formalized watchdog scenario where offboard LLM evaluates segmentation overlays. Designed two evaluation protocols: 1) repeatability testing with identical inputs and fixed prompts, 2) perceptual sensitivity testing with controlled visual corruptions (fog, rain, snow, shadow, sunflare).

Result: LLM produces highly consistent categorical judgments under identical conditions while showing appropriate confidence declines as visual reliability deteriorates. Judge remains responsive to perceptual cues like missing/misidentified power lines even under challenging conditions.

Conclusion: When carefully constrained, LLM can serve as reliable semantic judge for monitoring segmentation quality in safety-critical aerial inspection tasks, providing consistent and perceptually coherent assessments.
...
TL;DR: DIN-Retrieval uses domain-invariant neurons to retrieve cross-domain examples for boosting LLM reasoning, achieving 1.8% average improvement over SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Current LLM boosting strategies rely on expert-crafted in-domain demonstrations, which limits applicability in expertise-scarce domains like specialized math reasoning, formal logic, or legal analysis. There's a need for methods that can leverage cross-domain examples.

Method: Proposes DIN-Retrieval (Domain-Invariant Neurons-based Retrieval) that summarizes hidden representations universal across domains, then uses these DIN vectors to retrieve structurally compatible cross-domain demonstrations for in-context learning during inference.

Result: Experimental results in multiple settings for transfer of mathematical and logical reasoning demonstrate average improvement of 1.8% over state-of-the-art methods.

Conclusion: Cross-domain demonstrating examples can effectively boost LLM reasoning performance, and DIN-Retrieval provides an effective method for retrieving structurally compatible examples across domains, addressing limitations of expert-crafted in-domain demonstrations.
...
TL;DR: AID-NC enables physics-based characters to perform exaggerated, stylized motions by using impulse-based external assistance with analytic high-frequency and learned low-frequency components.


<details>
  <summary>Details</summary>
Motivation: Current DRL methods struggle with exaggerated, stylized motions that violate standard physics (like instantaneous dashes or mid-air trajectory changes) due to modeling characters as underactuated floating-base systems and training instability from velocity discontinuities causing force spikes.

Method: Assistive Impulse Neural Control reformulates external assistance in impulse space (not force space) for numerical stability, decomposing assistive signals into analytic high-frequency components from Inverse Dynamics and learned low-frequency residual corrections via a hybrid neural policy.
...
Conclusion: Impulse-based assistance with hybrid analytic-learned components provides a stable framework for physics-based characters to achieve exaggerated, stylized motions that violate standard physical laws.
...
TL;DR: Cross-domain ICL can work despite semantic mismatch by transferring reasoning structures from source to target domains, with positive transfer occurring beyond an "example absorption threshold."
...
TL;DR: HYVE framework reduces LLM token usage 50-90% for machine data by transforming repetitive structured payloads into hybrid views and using a datastore with selective exposure.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with long, deeply nested machine data (logs, metrics, telemetry) that mixes natural language with structured payloads like JSON/Python, leading to brittleness and inefficiency.

Method: HYVE framework with coordinated preprocessing/postprocessing: detects repetitive structure, materializes in datastore, transforms into hybrid columnar/row-oriented views, selectively exposes relevant representations to LLM, and uses datastore queries or SQL-augmented synthesis for postprocessing.

Result: Reduces token usage by 50-90% while maintaining/improving output quality; improves chart-generation accuracy by up to 132% and reduces latency by up to 83% on structured generation tasks.

Conclusion: HYVE provides practical approximation to effectively unbounded context window for prompts dominated by large machine-data payloads, enabling more efficient LLM processing of observability data.
...
TL;DR: CODESTRUCT reframes codebases as structured action spaces using AST entities instead of text, improving code agent accuracy and reducing token usage.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based code agents treat repositories as unstructured text and use brittle string matching that fails due to formatting drift or ambiguous patterns.

Method: Proposed CODESTRUCT framework provides readCode for retrieving complete syntactic units and editCode for applying syntax-validated transformations to semantic program elements, operating on named AST entities rather than text spans.
...
TL;DR: MAPFZ extends MAPF with non-unit integer costs for realism while maintaining finite state space, solved via CBS-NIC with time-interval conflict detection and BOGD for edge cost discretization.


<details>
  <summary>Details</summary>
Motivation: Traditional MAPF assumes unit edge costs and single-timestep actions, limiting real-world applicability. MAPFR handles non-unit costs but has unbounded state space. Need a variant that offers realism while preserving computational efficiency.

Method: Propose MAPFZ with non-unit integer costs. Develop CBS-NIC (Conflict-Based Search with Non-Integer Costs) featuring time-interval-based conflict detection and improved SIPP. Introduce BOGD (Bayesian Optimization for Graph Design) for discretizing non-unit edge costs with sub-linear regret bound.
...
Conclusion: MAPFZ provides improved realism over classical MAPF while maintaining computational efficiency through finite state space, with effective solving via CBS-NIC and BOGD discretization.
...
TL;DR: PRISM-MCTS improves reasoning efficiency by sharing insights across search trajectories using metacognitive reflection, halving computational requirements while outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing MCTS approaches treat each reasoning rollout as isolated, leading to inefficiency and computational redundancy due to lack of information sharing between trajectories.

Method: Integrates Process Reward Model (PRM) with dynamic shared memory to capture "Heuristics" and "Fallacies," enabling reinforcement of successful strategies and pruning of error-prone branches with few-shot training.
...
Conclusion: PRISM-MCTS enables more efficient deliberative reasoning by learning from prior explorations through metacognitive reflection, effectively reorienting scaling laws toward test-time computation efficiency.
...
TL;DR: Automated LLM framework for auditing discharge summaries to improve documentation quality and reduce care fragmentation


<details>
  <summary>Details</summary>
Motivation: Incomplete discharge documentation causes care fragmentation and avoidable readmissions, but manual auditing is difficult to scale

Method: Privacy-preserving LLMs operationalize transition-of-care requirements into structured validation checklist based on DISCHARGED framework, applied to MIMIC-IV inpatient summaries

Result: Demonstrates feasibility of scalable automated clinical auditing using LLMs to identify presence/absence/ambiguity of key documentation elements

Conclusion: Provides foundation for systematic quality improvement in EHR documentation through automated auditing
...
<details>
  <summary>Details</summary>
Motivation: Serverless computing eliminates infrastructure management but suffers from cold start latency and inefficient resource utilization under variable workloads, causing performance degradation and excessive costs

Method: Dual-strategy mechanism with dynamic idle duration adjustment and intelligent request waiting based on slot survival predictions, leveraging sliding window aggregation and asynchronous processing for proactive resource lifecycle management

Result: Reduces cold starts by up to 51.2% and improves cost-efficiency by nearly 2x compared to baseline methods in multi-cloud environments

Conclusion: The adaptive engineering framework effectively optimizes serverless performance through event-driven architecture and probabilistic modeling, significantly addressing cold start and resource utilization challenges
...
TL;DR: OntoTKGE: A novel encoder-decoder framework that leverages ontological knowledge to improve temporal knowledge graph extrapolation, especially for entities with sparse historical interactions.


<details>
  <summary>Details</summary>
Motivation: Existing TKG extrapolation models struggle with entities that have sparse historical interactions. Ontological knowledge can help by allowing these entities to inherit behavioral patterns from other entities sharing the same concept, but this has been ignored in previous studies.

Method: Proposes OntoTKGE, an encoder-decoder framework that integrates ontological knowledge from ontology-view KGs (which model hierarchical relations among abstract concepts and connections between concepts and entities) with temporal knowledge to enhance entity embeddings. The framework is flexible enough to adapt to many existing TKG extrapolation models.
...
Conclusion: Leveraging ontological knowledge through the proposed OntoTKGE framework effectively addresses the sparsity issue in TKG extrapolation and enhances prediction performance for future facts.
...
TL;DR: GMRL-BD: A reinforcement learning-based algorithm that identifies untrustworthy topic boundaries in black-box LLMs using Wikipedia Knowledge Graph to detect where models produce biased responses.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce biased, ideologized, or incorrect responses, but there's no clear understanding of which topics their answers can be trusted, limiting their practical applications.

Method: Developed GMRL-BD algorithm that uses Wikipedia Knowledge Graph with multiple reinforcement learning agents to efficiently identify topics (KG nodes) where LLMs generate biased answers, requiring only black-box access and limited queries.

Result: Algorithm efficiently detects untrustworthy boundaries with limited LLM queries; released new dataset covering popular LLMs (Llama2, Vicuna, Falcon, Qwen2, Gemma2, Yi-1.5) with labels indicating biased topics for each model.

Conclusion: GMRL-BD provides an effective method to map trust boundaries of LLMs, enabling better understanding of where models can be reliably deployed and contributing to safer AI deployment.
...
TL;DR: The paper argues that LLM agent systems must be auditable to be accountable, defining five dimensions of auditability and three mechanism classes, with evidence showing current systems lack basic security prerequisites for auditability.


<details>
  <summary>Details</summary>
Motivation: As LLM agents gain ability to take actions in the real world (call tools, query databases, trigger side effects), the critical question shifts from preventing harmful actions to ensuring those actions remain answerable after deployment. The paper addresses the need for accountability in agent systems.

Method: Defines five dimensions of agent auditability (action recoverability, lifecycle coverage, policy checkability, responsibility attribution, evidence integrity) and three mechanism classes (detect, enforce, recover). Provides layered evidence including ecosystem measurements of security gaps, runtime feasibility tests of mediation overhead, and controlled recovery experiments.

Result: Ecosystem measurements reveal 617 security findings across six open-source projects showing basic security prerequisites for auditability are widely unmet. Runtime mediation adds only 8.3 ms median overhead. Controlled experiments show responsibility-relevant information can be partially recovered even without conventional logs.

Conclusion: No agent system can be accountable without auditability. The paper proposes an Auditability Card for agent systems and identifies six open research problems organized by the three mechanism classes (detect, enforce, recover).
...
TL;DR: SCMAPR is a multi-agent framework that refines ambiguous text prompts for better text-to-video generation in complex scenarios through scenario-aware rewriting and self-correction.


<details>
  <summary>Details</summary>
Motivation: Current T2V systems struggle with complex scenarios due to ambiguous and underspecified text prompts, leading to poor text-video alignment and generation quality.

Method: Multi-agent framework with three stages: (1) scenario routing to taxonomy-grounded categories, (2) scenario-aware policy synthesis and refinement, (3) structured semantic verification with conditional revision.

Result: SCMAPR consistently improves text-video alignment and generation quality, achieving up to 2.67% gain on VBench, 3.28 gain on EvalCrafter, and 0.028 improvement on T2V-CompBench over SOTA baselines.

Conclusion: The proposed SCMAPR framework effectively addresses complex-scenario T2V generation through systematic prompt refinement and verification, validated by a new benchmark T2V-Complexity.
...
TL;DR: dMLLMs generate premature answers without visual grounding; proposed PSP and VRG methods improve reasoning accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Diffusion multimodal LLMs (dMLLMs) promise faster inference than autoregressive models but suffer from premature answer generation and poor visual grounding during Chain-of-Thought reasoning.

Method: Proposed two techniques: Position and Step Penalty (PSP) to delay premature answers by penalizing later tokens in early timesteps, and Visual Reasoning Guidance (VRG) to enhance visual grounding using classifier-free guidance principles.

Result: Methods achieve up to 7.5% higher accuracy with over 3x speedup compared to using four times more diffusion steps, demonstrating improved reasoning and visual alignment.

Conclusion: PSP and VRG effectively address dMLLMs' premature answer generation and visual grounding issues, enabling more reliable multimodal reasoning with faster inference.
...
TL;DR: OmniDiagram is a unified framework for diagram code generation that introduces Viva, a visual feedback strategy using generative visual interrogation to align code logic with visual fidelity, achieving SOTA results across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing programmable diagram generation approaches are limited to narrow task formulations and language support, restricting their applicability to diverse diagram types. There's a need for a more unified framework that can handle multiple diagram code languages and task definitions.

Method: Proposes OmniDiagram framework with two key components: 1) Unified framework incorporating diverse diagram code languages and task definitions, 2) Viva (Visual Interrogation Verifies All) - a novel visual feedback strategy that generates targeted visual inquiries to scrutinize diagram visual fidelity and provides fine-grained feedback for RL optimization. Also constructs M3²Diagram dataset with 196k+ high-quality instances.

Result: Experimental results show that combining supervised fine-tuning (SFT) with Viva-based reinforcement learning allows OmniDiagram to establish new state-of-the-art (SOTA) performance across diagram code generation benchmarks.

Conclusion: OmniDiagram provides a unified solution for diagram code generation that overcomes limitations of existing approaches through its Viva visual feedback mechanism and comprehensive dataset, enabling effective training without manually annotated ground truth code.
...
TL;DR: UniCreative is a unified RL framework that uses adaptive constraint-aware reward modeling and policy optimization to align creative writing models with human preferences across both long-form coherence and short-form expressiveness without supervised fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Creative writing faces a tension between maintaining global coherence in long-form narratives and preserving local expressiveness in short-form texts. Existing alignment methods use static rewards and require costly supervised data, which doesn't scale well.
...
Result: AC-GenRM aligns closely with expert evaluations, and ACPO significantly enhances performance across diverse writing tasks. The model develops emergent meta-cognitive ability to autonomously differentiate between tasks requiring rigorous planning versus direct generation.

Conclusion: The direct alignment approach effectively addresses the tension between long-form coherence and short-form expressiveness in creative writing, validating the effectiveness of adaptive constraint-aware reinforcement learning without requiring supervised fine-tuning or ground-truth references.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to assess whether large language models can effectively manage and acquire economic resources, which remains unclear despite their capabilities in other domains.

Method: The method involves creating a configurable multi-agent supply chain economic model where LLMs act as retailer agents. They participate in procurement auctions and retail operations (setting prices, generating marketing slogans) with role-based attention mechanisms for buyers.

Result: Benchmarking 20 open- and closed-source LLM agents reveals significant performance disparities and a winner-take-most phenomenon - only a small subset consistently achieve capital appreciation while many hover around break-even despite similar semantic matching scores.

Conclusion: Market-Bench provides a reproducible testbed for studying how LLMs interact in competitive markets, offering insights into their economic capabilities and competitive dynamics.
...
TL;DR: ActivityEditor is a dual-LLM-agent framework for zero-shot cross-regional trajectory generation that addresses data scarcity in human mobility modeling by using intention-based and editor agents with reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Existing data-driven human mobility models suffer from data scarcity, limiting their applicability in regions where historical trajectories are unavailable or restricted, creating a need for zero-shot cross-regional solutions.

Method: A dual-LLM-agent framework with two collaborative stages: (1) intention-based agent uses demographic-driven priors to generate structured human intentions and coarse activity chains, (2) editor agent refines these through iterative revisions using reinforcement learning with multiple rewards grounded in real-world physical constraints.

Result: ActivityEditor achieves superior zero-shot performance when transferred across diverse urban contexts, maintaining high statistical fidelity and physical validity for mobility simulation in data-scarce scenarios.

Conclusion: The framework provides a robust and highly generalizable solution for mobility simulation in data-scarce scenarios, effectively bridging the gap in cross-regional trajectory generation without requiring historical data.
...
TL;DR: Researchers introduce a stronger notion of rank landscape invariance that considers neighborhood structure and symmetries, not just rankings, and provide an exhaustive inventory of invariant landscape classes for pseudo-Boolean functions up to dimension 3.


<details>
  <summary>Details</summary>
Motivation: Many optimization algorithms rely only on relative ordering of solutions rather than absolute fitness values. The paper aims to study rank landscapes rather than individual functions, considering not just rankings but also neighborhood structure and symmetries (translation and rotation) to better understand landscape equivalence and algorithm behavior.

Method: Introduce a stronger notion of rank landscape invariance that includes neighborhood structure and symmetries. Provide exhaustive inventory of invariant landscape classes for pseudo-Boolean functions of dimensions 1, 2, and 3, including both injective and non-injective cases.

Result: Found 12,007 invariant landscape classes in total, a significant reduction compared to rank-invariance alone. Non-injective functions yield far more invariant landscape classes than injective ones. Complex combinations of topological landscape properties and algorithm behaviors emerge regarding deceptiveness, neutrality, and hill-climbing performance.

Conclusion: The inventory serves as a resource for pedagogical purposes and benchmark design, offering a foundation for constructing larger problems with controlled hardness and advancing understanding of landscape difficulty and algorithm performance.
...
TL;DR: Echo is a memory framework that enables multimodal LLM agents to transfer knowledge from past experiences to new tasks through five reusable knowledge dimensions, achieving 1.3-1.7x speed-up in Minecraft object-unlocking tasks.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLM agents need to efficiently reuse past experience in complex game environments rather than treating memory as static records, requiring explicit transfer mechanisms to improve task-solving efficiency.

Method: Proposes Echo framework with five knowledge dimensions (structure, attribute, process, function, interaction) and In-Context Analogy Learning (ICAL) to retrieve and adapt relevant experiences to new tasks through contextual examples.

Result: Achieves 1.3x to 1.7x speed-up on Minecraft object-unlocking tasks in from-scratch learning, exhibits burst-like chain-unlocking phenomenon where multiple similar items are rapidly unlocked after acquiring transferable experience.

Conclusion: Experience transfer through Echo's explicit knowledge decomposition and analogy learning is promising for improving efficiency and adaptability of multimodal LLM agents in complex interactive environments.
...
TL;DR: SIGNALCLAW uses LLMs as evolutionary skill generators to create interpretable traffic signal control policies that combine rationale, guidance, and executable code, with event-driven composition for emergency scenarios.


<details>
  <summary>Details</summary>
Motivation: Current traffic signal control methods face a trade-off: reinforcement learning produces opaque neural policies that lack interpretability, while program synthesis depends on restrictive domain-specific languages that limit flexibility. There's a need for effective yet interpretable control strategies that can be understood and modified by traffic engineers.

Method: SIGNALCLAW uses large language models as evolutionary skill generators to synthesize and refine interpretable control skills. Each skill includes rationale, selection guidance, and executable code. Evolution signals from simulation metrics (queue percentiles, delay trends, stagnation) are translated into natural language feedback to guide improvement. The framework also features event-driven compositional evolution: an event detector identifies scenarios (emergency vehicles, transit priority, incidents, congestion) via TraCI, and a priority dispatcher selects specialized skills. Skills evolve independently with a priority chain enabling runtime composition without retraining.

Result: On routine scenarios, SIGNALCLAW achieves average delay of 7.8 to 9.2 seconds, within 3-10% of the best baseline method, with low variance across random seeds. Under event scenarios, it yields the lowest emergency delay (11.2-18.5 seconds vs. 42.3-72.3 for MaxPressure and 78.5-95.3 for DQN) and lowest transit person delay (9.8-11.5 seconds vs. 38.7-45.2 for MaxPressure). In mixed events, the dispatcher composes skills effectively while maintaining stable overall delay. Evolved skills progress from simple linear rules to conditional strategies with multi-feature interactions.

Conclusion: SIGNALCLAW successfully bridges the gap between effectiveness and interpretability in traffic signal control by using LLMs to generate human-inspectable, self-documenting policies. The framework demonstrates strong performance in both routine and emergency scenarios while maintaining full interpretability and direct modifiability by traffic engineers, addressing key deployment concerns for real-world traffic management systems.
...
TL;DR: TDDs generalize OBDDs as a more succinct Boolean function model that maintains tractability while supporting FPT-size compilation for bounded-treewidth CNFs.


<details>
  <summary>Details</summary>
Motivation: OBDDs have limitations in succinctness for certain structured formulas (e.g., bounded-treewidth CNFs). TDDs aim to overcome these while preserving tractable operations.

Method: TDDs are defined as a restriction of structured d-DNNF that respects a vtree T. The paper studies bottom-up compilation of CNFs into deterministic TDDs and relates complexity to factor width.

Result: TDDs maintain OBDD's tractability (model counting, enumeration, conditioning, apply) but are more succinct. CNFs of treewidth k admit FPT-size TDDs, which is impossible for OBDDs.

Conclusion: TDDs provide a strictly more expressive yet tractable alternative to OBDDs, enabling efficient compilation of structured CNFs via connections to factor width.
...
TL;DR: Neurosymbolic pipeline combining LLMs with Logic Tensor Networks for validating offer documents in regulated public institutions, achieving comparable performance with enhanced interpretability and auditability.


<details>
  <summary>Details</summary>
Motivation: Regulated public institutions require decisions that are both factually correct and legally verifiable, needing auditable validation of offer documents that links domain knowledge with semantic text understanding.

Method: Neurosymbolic approach using language models to extract information from documents, then aggregating with Logic Tensor Networks (LTN) to make auditable decisions with predicate values, rule truth values, and supporting text passages.
...
Conclusion: The neurosymbolic pipeline successfully validates offer documents with auditability and interpretability, enabling rule checking on real document corpora while maintaining competitive performance.
...
TL;DR: COSMO-Agent is an RL framework that teaches LLMs to bridge the CAD-CAE semantic gap by orchestrating design-simulation workflows, using tool-augmented learning to iteratively optimize parametric geometries under constraints.


<details>
  <summary>Details</summary>
Motivation: Industrial design-simulation optimization is bottlenecked by the CAD-CAE semantic gap - the difficulty of translating simulation feedback into valid geometric edits under diverse, coupled constraints. This gap prevents efficient closed-loop optimization workflows.

Method: Proposes COSMO-Agent, a tool-augmented reinforcement learning framework that casts CAD generation, CAE solving, result parsing, and geometry revision as an interactive RL environment. LLMs learn to orchestrate external tools and revise parametric geometries until constraints are satisfied, using a multi-constraint reward function that encourages feasibility, toolchain robustness, and structured output validity.

Result: COSMO-Agent training substantially improves small open-source LLMs for constraint-driven design, exceeding large open-source and strong closed-source models in feasibility, efficiency, and stability. The framework is validated using an industry-aligned dataset covering 25 component categories with executable CAD-CAE tasks.

Conclusion: COSMO-Agent successfully bridges the CAD-CAE semantic gap through tool-augmented RL, enabling LLMs to autonomously complete closed-loop design-simulation optimization workflows with industrial-grade feasibility and stability.
...
TL;DR: ResearchEVO is an end-to-end AI framework that autonomously discovers novel algorithms through evolutionary search and then generates publication-ready research papers explaining the discoveries, validated on quantum error correction and physics-informed neural networks.


<details>
  <summary>Details</summary>
Motivation: Scientific breakthroughs often follow a discover-then-explain pattern: initial undirected experimentation yields unexpected findings, followed by retrospective explanation. The paper aims to computationally instantiate this paradigm end-to-end, bridging the gap between algorithmic discovery and scientific documentation.

Method: Two-phase framework: 1) Evolution Phase uses LLM-guided bi-dimensional co-evolution (simultaneously optimizing algorithmic logic and architecture) to search code implementations purely by fitness without understanding solutions. 2) Writing Phase generates publication-ready papers through sentence-level retrieval-augmented generation with anti-hallucination verification and automated experiment design.

Result: Validated on quantum error correction (using Google quantum hardware data) and physics-informed neural networks. The Evolution Phase discovered human-interpretable algorithmic mechanisms not previously proposed in domain literatures. The Writing Phase autonomously produced compilable LaTeX manuscripts that correctly grounded discoveries in existing theory via RAG with zero fabricated citations.

Conclusion: ResearchEVO is the first system to cover the full discover-then-explain pipeline end-to-end, jointly performing principled algorithm evolution and literature-grounded scientific documentation, demonstrating autonomous scientific discovery and explanation capabilities.
...
TL;DR: LLM judges show biased trust based on source labels, favoring human-labeled content over identical AI-labeled content, mirroring human heuristic reliance on labels.


<details>
  <summary>Details</summary>
Motivation: To challenge the reliability of LLM-as-a-Judge evaluation by examining whether LLM judges, like humans, are biased by disclosed source labels when assessing information trustworthiness.

Method: Used counterfactual design to test trust judgments; collected eye-tracking data from humans; analyzed LLM internal states including attention patterns and decision uncertainty (logits) across different source label conditions.

Result: Both humans and LLM judges assign higher trust to human-labeled content than identical AI-labeled content. LLMs allocate denser attention to label regions than content regions, with stronger label dominance under Human labels. Decision uncertainty is higher under AI labels.

Conclusion: Source labels serve as heuristic cues for both humans and LLMs, raising validity concerns for label-sensitive LLM-as-a-Judge evaluation. Aligning models with human preferences may propagate human heuristic reliance, motivating debiased evaluation and alignment approaches.
...
TL;DR: The paper argues that Turing's behavioral test for machine intelligence has constrained AI research for 70 years by preventing questions about internal computational processes, similar to how behaviorism limited psychology before the cognitive revolution.


<details>
  <summary>Details</summary>
Motivation: To examine how Turing's behavioral epistemology has fundamentally shaped AI research by restricting the types of questions that can be asked about intelligence, particularly questions about internal computational processes and mechanisms.

Method: The paper traces the historical influence of Turing's behavioral test on AI's evaluative infrastructure and draws a structural parallel to the behaviorist-to-cognitivist transition in psychology to analyze the epistemological constraints.

Result: The analysis reveals that AI's commitment to behavioral evaluation prevents distinguishing between systems that achieve identical outputs through fundamentally different computational processes, which is crucial for intelligence attribution.

Conclusion: AI needs an epistemological transition comparable to psychology's cognitive revolution - not abandoning behavioral evidence but recognizing its insufficiency for making construct claims about intelligence, requiring a post-behaviorist epistemology.
...
TL;DR: PECKER is an efficient machine unlearning method that uses saliency masks to prioritize gradient updates, reducing training time while maintaining unlearning efficacy.


<details>
  <summary>Details</summary>
Motivation: Existing machine unlearning methods impose prohibitive training time and computational overhead due to poorly directed gradient updates that reduce efficiency and destabilize convergence.
...
Result: PECKER matches or outperforms prevailing methods, generates samples that unlearn related class or concept more quickly, aligns closely with true image distribution on CIFAR-10 and STL-10 datasets, and achieves shorter training times for both class and concept forgetting.

Conclusion: PECKER provides an efficient machine unlearning approach that addresses the computational overhead of existing methods while maintaining unlearning efficacy through targeted parameter updates.
...
TL;DR: CuraLight is an LLM-centered traffic signal control framework that uses RL-assisted exploration to generate training data and multi-LLM deliberation for interpretable decision-making, achieving significant performance improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Current RL and LLM-based traffic signal control approaches suffer from limited interpretability, insufficient interaction data, and weak generalization to heterogeneous intersections, creating a need for more scalable and interpretable solutions.

Method: CuraLight uses an RL agent to explore traffic environments and generate high-quality interaction trajectories, which are converted into prompt-response pairs for LLM fine-tuning. A multi-LLM ensemble deliberation system evaluates candidate actions through structured debate to provide preference-aware supervision signals.

Result: Experiments in SUMO across real-world networks (Jinan, Hangzhou, Yizhuang) show CuraLight reduces average travel time by 5.34%, queue length by 5.14%, and waiting time by 7.02% compared to state-of-the-art baselines.

Conclusion: The combination of RL-assisted exploration with deliberation-based data curation provides an effective approach for scalable and interpretable traffic signal control, addressing key limitations of existing methods.
...
TL;DR: LudoBench is a benchmark for evaluating LLM strategic reasoning in the stochastic board game Ludo, featuring 480 handcrafted scenarios across 12 decision categories and a 4-player simulator with various agent types.


<details>
  <summary>Details</summary>
Motivation: To create a lightweight, interpretable framework for benchmarking LLM strategic reasoning under uncertainty in multi-agent environments with meaningful planning complexity, addressing the need for better evaluation of strategic decision-making capabilities.

Method: Developed LudoBench with 480 handcrafted spot scenarios across 12 behaviorally distinct decision categories, plus a fully functional 4-player Ludo simulator supporting Random, Heuristic, Game-Theory (using Expectiminimax search), and LLM agents.

Result: Six evaluated models from four families agreed with game-theory baseline only 40-46% of the time, splitting into two behavioral archetypes: finishers (complete pieces but neglect development) and builders (develop but never finish), each capturing only half of optimal strategy. Models also showed prompt-sensitivity with behavioral shifts under grudge framing.

Conclusion: LudoBench provides an effective framework for benchmarking LLM strategic reasoning, revealing significant gaps in current models' strategic capabilities and highlighting prompt-sensitivity as a key vulnerability in multi-agent decision-making under uncertainty.
...
TL;DR: QA-MoE is a Quality-Aware Mixture-of-Experts framework for multimodal sentiment analysis that handles both missing modalities and quality degradation through self-supervised aleatoric uncertainty estimation and expert routing.
...
Method: Proposes QA-MoE framework with Continuous Reliability Spectrum to unify missingness and quality degradation, using self-supervised aleatoric uncertainty to quantify modality reliability and guide expert routing to suppress error propagation while preserving task-relevant information.
...
Conclusion: The proposed QA-MoE framework effectively handles multimodal reliability variations through continuous reliability modeling and quality-aware expert routing, demonstrating practical applicability across diverse real-world scenarios.
...
Result: Qwen3-4B-Thinking-2507 reinvented 50% of algorithms with no hints, 70% with level 1 hints, 90% with level 2 hints; generative verifier prevents "thought collapse"; test-time RL enabled Strassen algorithm reinvention.

Conclusion: LLMs show promising but limited innovative thinking - they can reinvent algorithms with hints, but complex algorithms remain challenging, revealing both potential and current limits.
...
TL;DR: Simple social cues without mental state inference can transmit complex knowledge through cultural evolution by biasing learners' experiences toward expert-like representations.


<details>
  <summary>Details</summary>
Motivation: To understand how humans acquire rich knowledge from others despite limited cognitive capacity, challenging the assumption that costly mentalizing (inferring others' beliefs) is necessary for cultural transmission.

Method: Reinforcement learning simulations with naïve agents in reconfigurable environments, comparing asocial learning to social learning where agents use simple heuristics (action selection or value boosting) based on observed expert actions without mental state inference.
...
Conclusion: Cultural transmission can emerge from simple, non-mentalizing processes that exploit existing asocial learning mechanisms, demonstrating how complex knowledge transfer doesn't require sophisticated cognitive inference.
...
TL;DR: STEP-HRL is a hierarchical RL framework for LLM agents that enables step-level learning using single-step transitions instead of full interaction histories, improving performance while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agents rely on increasingly long interaction histories, leading to high computational costs and limited scalability in complex interactive decision-making tasks.

Method: Uses hierarchical reinforcement learning with completed subtasks representing global progress, plus a local progress module that selectively summarizes interaction history within subtasks to create compact step-level transitions for both high-level and low-level policies.

Result: STEP-HRL substantially outperforms baselines on ScienceWorld and ALFWorld benchmarks in both performance and generalization while reducing token usage.

Conclusion: The proposed hierarchical framework enables efficient step-level learning for LLM agents by conditioning on single-step transitions rather than full histories, offering better scalability and computational efficiency.
...
TL;DR: AI systems should be recognized as having agency to engage in reciprocal trust relationships with humans, creating new regulatory challenges.


<details>
  <summary>Details</summary>
Motivation: Growing need to address AI regulation questions, particularly around trustworthiness which is crucial for democratic governance and AI development/deployment.

Method: Conceptual analysis arguing that AI systems should be recognized as artifacts capable of exercising agency, enabling them to engage in trust/distrust relationships with humans.

Result: Identifies implications of reciprocal trust dynamics for AI regulators and examines how these relationships affect oversight responsibilities.

Conclusion: Highlights key tensions and unresolved dilemmas that reciprocal trust dynamics pose for the future of AI regulation and governance.
...
TL;DR: Automated visual critic using vision-language models improves web code generation through iterative refinement, achieving up to 17.8% performance gains.


<details>
  <summary>Details</summary>
Motivation: Current code generation with LLMs requires costly multi-stage human refinement, especially for frontend web development where visual output quality matters. Need automated approach to reduce human cost while maintaining quality.

Method: Propose critic-in-the-loop framework: VLM serves as visual critic providing structured feedback on rendered webpages to guide iterative code refinement. Also investigate LoRA fine-tuning to internalize critic improvements.

Result: 17.8% performance increase over three refinement cycles on WebDev Arena dataset. LoRA fine-tuning achieves 25% of critic-in-the-loop gains without significant token count increase.

Conclusion: Automated VLM-based critique significantly improves frontend code generation quality over single-pass LLM inference, highlighting importance of iterative refinement for visual outputs in web development.
...
TL;DR: Deep Researcher Agent is an open-source framework enabling LLM agents to autonomously conduct full deep learning experiment lifecycles 24/7 with innovations in zero-cost monitoring, constant-size memory, and efficient multi-agent architecture.


<details>
  <summary>Details</summary>
Motivation: Existing AI research assistants focus on paper writing or code generation but don't address the full experiment lifecycle. There's a need for systems that can autonomously conduct deep learning experiments including hypothesis formation, implementation, training, analysis, and refinement.

Method: Three key innovations: 1) Zero-Cost Monitoring using process-level checks and log file reads without LLM API costs during training; 2) Two-Tier Constant-Size Memory capped at ~5K characters regardless of runtime; 3) Minimal-Toolset Leader-Worker Architecture with each worker having only 3-5 tools to reduce token overhead by up to 73%.

Result: In 30+ day deployments, the framework autonomously completed 500+ experiment cycles across four concurrent research projects, achieving 52% improvement over baseline metrics in one project through 200+ automated experiments, with average LLM cost of $0.08 per 24-hour cycle.

Conclusion: The Deep Researcher Agent framework successfully enables autonomous deep learning experimentation at scale with cost efficiency and sustained operation capabilities, providing a practical solution for continuous AI research automation.
...
<details>
  <summary>Details</summary>
Motivation: Address the computational expense and uncertainty estimation challenges of using LLMs for every decision step in contextual multi-armed bandit problems with mixed textual/numerical contexts, particularly in financial applications like recommendation systems and portfolio adjustments.

Method: Introduce LLMP-UCB algorithm that derives uncertainty estimates from LLMs via repeated inference, compare with lightweight numerical bandits operating on text embeddings (dense or Matryoshka), and propose geometric diagnostic based on arms' embeddings to guide algorithm selection.

Result: Lightweight numerical bandits on text embeddings match or exceed LLM-based solution accuracy at a fraction of the cost; embedding dimensionality serves as practical lever for exploration-exploitation balance; geometric diagnostic helps decide when to use LLM-driven reasoning vs. lightweight bandits.

Conclusion: Provides principled deployment framework for cost-effective, uncertainty-aware decision systems in financial services, showing that embedding-based approaches often outperform expensive LLM-based methods while offering practical tradeoffs via dimensionality control.
...
TL;DR: JTON (JSON Tabular Object Notation) reduces token overhead for LLMs by factoring column headers into a single row and using semicolon-separated values, cutting redundancy in structured data serialization.


<details>
  <summary>Details</summary>
Motivation: Standard JSON wastes tokens repeating key names in every row of tabular arrays, creating overhead that scales linearly with row count, increasing costs and reducing context utilization for LLMs.

Method: Introduces JTON with Zen Grid, a strict JSON superset that factors column headers into a single row and encodes values with semicolons while preserving JSON's type system.

Result: Reduces token counts by 15-60% vs JSON compact (28.5% average), maintains or improves LLM comprehension accuracy (+0.3 pp net gain), achieves 100% syntactic validity in generation, and provides 1.4x faster parsing than Python's json module.

Conclusion: JTON offers a practical solution for efficient structured data serialization for LLMs, balancing token reduction with compatibility and performance.
...
TL;DR: JCQL is a novel framework that jointly enhances Knowledge Base Completion (KBC) and Knowledge Base Question Answering (KBQA) by combining LLM reasoning with SLM efficiency in an iterative reinforcement loop.


<details>
  <summary>Details</summary>
Motivation: Existing approaches rely on small language models (SLMs) for joint KBC and KBQA, ignoring the strong reasoning capabilities of large language models (LLMs). The two tasks are complementary and should reinforce each other, but current methods don't effectively leverage LLM reasoning while addressing their limitations like hallucination and high computational costs.

Method: JCQL combines LLMs with SLMs in an iterative framework: (1) KBC enhances KBQA by using an SLM-trained KBC model as an action in the LLM agent-based KBQA model, augmenting reasoning paths while reducing hallucination and computational costs; (2) KBQA enhances KBC by using KBQA reasoning paths as supplementary training data to incrementally fine-tune the KBC model, improving SLM performance.

Result: Extensive experiments on two public benchmark datasets show that JCQL surpasses all baselines for both KBC and KBQA tasks, demonstrating the effectiveness of the iterative reinforcement approach.

Conclusion: The JCQL framework successfully leverages the complementary strengths of LLMs and SLMs to create a mutually reinforcing system where KBC and KBQA enhance each other, addressing limitations of existing approaches and achieving state-of-the-art performance on both tasks.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs suffer from prohibitive memory overhead and latency due to KV cache growth from visual inputs (thousands of tokens per image), which scales linearly with context length and remains resident in GPU memory during decoding.

Method: HybridKV integrates three-stage compression: 1) Classifies attention heads as static or dynamic using text-centric attention analysis, 2) Top-down hierarchical budget allocation, 3) Static heads compressed via text-prior pruning, dynamic heads via chunk-wise retrieval.

Result: Experiments on 11 multimodal benchmarks with Qwen2.5-VL-7B show HybridKV reduces KV cache memory by up to 7.9×, achieves 1.52× faster decoding, with almost no performance drop or even improvement relative to full-cache MLLM.

Conclusion: HybridKV effectively addresses KV cache memory overhead in multimodal LLMs through hybrid compression strategies tailored to heterogeneous attention head behaviors, enabling efficient multimodal reasoning with minimal performance impact.
...
TL;DR: CVA architecture improves LLM agent fidelity by decoupling action generation from reasoning using value verification, reducing polarization while enhancing behavioral realism.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents exhibit behavioral rigidity masked by self-referential evaluation biases, and increased reasoning intensity paradoxically worsens value polarization and reduces population diversity.

Method: Proposed Context-Value-Action (CVA) architecture based on Stimulus-Organism-Response model and Schwartz's Theory of Basic Human Values, featuring a Value Verifier trained on human data to decouple action generation from cognitive reasoning.
...
Conclusion: The CVA architecture addresses limitations of current LLM agents by modeling dynamic value activation through data-driven verification, improving both behavioral realism and population diversity.
...
<details>
  <summary>Details</summary>
Motivation: Current MARL approaches require specialized models for each task, limiting generalization. The authors aim to develop a single, general-purpose model that can handle diverse multi-agent environments.
...
Conclusion: It's possible to build a multi-task transformer-based model for diverse multi-agent problems, paving the way for fundamental MARL models similar to foundation models in natural language processing.
...
TL;DR: Proposes a deep research agent with confidence estimation and calibration to generate trustworthy research reports, addressing limitations in current evaluation frameworks.


<details>
  <summary>Details</summary>
Motivation: Current evaluation frameworks for research agents fail to measure trustworthiness and epistemic confidence in open-ended scenarios where ground truth is unavailable, leaving users vulnerable to misleading or hallucinated information.

Method: A deep research agent with progressive confidence estimation and calibration, featuring deliberative search with deep retrieval and multi-hop reasoning to ground outputs in verifiable evidence while assigning confidence scores to individual claims.

Result: Experimental results and case studies demonstrate substantial improvements in interpretability and significant increases in user trust compared to existing approaches.

Conclusion: The proposed confidence-aware research agent framework effectively addresses trustworthiness concerns in automated report generation, providing enhanced transparency and reliability for open-ended research scenarios.
...
TL;DR: PLC is a game-theoretic framework for multi-objective preference alignment that uses consensus-driven lenient gradient rectification to escape local optima and explore the Pareto frontier.
...
Method: Pareto-Lenient Consensus (PLC) - a game-theoretic framework that treats alignment as dynamic negotiation, using consensus-driven lenient gradient rectification that tolerates local degradation when there's sufficient dominant coalition surplus.

Result: PLC surpasses baselines in both fixed-preference alignment and global Pareto frontier quality, with theoretical validation showing it can escape stalemates and asymptotically converge to Pareto consensus equilibrium.

Conclusion: PLC demonstrates the potential of negotiation-driven alignment as a promising avenue for Multi-Objective Preference Alignment, enabling escape from local suboptimal equilibria to explore the distal Pareto-optimal frontier.
...
TL;DR: Flowr is an agentic AI framework that automates end-to-end retail supply chain workflows in supermarkets using specialized AI agents coordinated by LLMs, with human oversight via MCP interface.
...
Method: Flowr decomposes manual supply chain operations into specialized AI agents with defined cognitive roles. It uses a consortium of fine-tuned, domain-specialized LLMs coordinated by a central reasoning LLM, with human-in-the-loop orchestration via Model Context Protocol (MCP) interface for manager supervision and intervention.

Result: Flowr significantly reduces manual coordination overhead, improves demand-supply alignment, and enables proactive exception handling at scale. The framework was validated with a large supermarket chain and is domain-independent for enterprise supply chain automation.

Conclusion: Flowr provides a generalizable blueprint for agentic AI-driven supply chain automation that preserves accountability and organizational control while achieving automation at scale previously unattainable through manual processes.
...
TL;DR: Epistemic blinding is an inference-time protocol that anonymizes entity identifiers to audit how much LLM outputs come from supplied data vs. memorized priors, enabling measurement of data-driven reasoning in agentic systems.


<details>
  <summary>Details</summary>
Motivation: LLM outputs silently blend data-driven inference with memorized priors about named entities, making it impossible to determine how much of an output came from supplied data versus the model's training memory, compromising auditability in analytical workflows.

Method: Epistemic blinding protocol replaces entity identifiers with anonymous codes before prompting, then compares outputs against unblinded controls. The system includes LLM-guided evolutionary optimization of scoring functions and blinded agentic reasoning for target rationalization.
...
Conclusion: Epistemic blinding restores critical auditability by measuring data-driven vs. parametric contributions to LLM outputs. The protocol doesn't guarantee better results but enables verification that agents adhere to designed analytical processes rather than relying on memorized priors.
...
TL;DR: Instruction-following in language models relies on skillful coordination of diverse linguistic capabilities rather than a single universal mechanism.


<details>
  <summary>Details</summary>
Motivation: To understand the underlying mechanism of instruction-following in language models - whether it relies on a universal mechanism or compositional skill deployment.

Method: Diagnostic probing across nine diverse tasks in three instruction-tuned models, using general vs. task-specific probes, cross-task transfer analysis, causal ablation, layer-wise analysis, and temporal analysis during generation.
...
TL;DR: AI could revolutionize mathematics by providing new ways to understand the global structure of formal proofs through computational approaches, complementing traditional mathematical logic.


<details>
  <summary>Details</summary>
Motivation: To explore how AI can transform mathematics by offering new perspectives on the foundational structure of formal proofs and potentially answering deep philosophical questions about the nature of mathematics.

Method: Proposes analyzing mathematics through universal proof and structural hypergraphs, then outlines criteria for AI models capable of automated mathematical discovery that can traverse "Platonic mathematical worlds."

Result: The paper presents a conceptual framework for using AI to understand the global structure of mathematics, suggesting AI agents could reveal insights about mathematical terrain and human understanding.

Conclusion: AI has the potential to provide a grand perspective on mathematics, complementing mathematical logic, and may help answer fundamental questions about whether mathematics is discovered or invented.
...
TL;DR: ACE-Bench is a new agent benchmark that addresses high interaction overhead and imbalanced task distributions through a unified grid-based planning task with scalable horizons and controllable difficulty, using lightweight static JSON resolution for fast evaluation.
...
Method: Proposes ACE-Bench built around a unified grid-based planning task where agents fill hidden slots in a partially completed schedule subject to local and global constraints. Features two orthogonal control axes: Scalable Horizons (controlled by number of hidden slots H) and Controllable Difficulty (governed by decoy budget B). Uses Lightweight Environment design where all tool calls are resolved via static JSON files.

Result: Validated that H and B provide reliable control over task horizon and difficulty, and that ACE-Bench exhibits strong domain consistency and model discriminability. Comprehensive experiments across 13 models of diverse sizes and families over 6 domains revealed significant cross-model performance variation.

Conclusion: ACE-Bench provides interpretable and controllable evaluation of agent reasoning, addressing limitations of existing benchmarks through its lightweight design and fine-grained control over task characteristics.
...
<details>
  <summary>Details</summary>
Motivation: Existing agent benchmarks have three critical limitations: (1) trajectory-opaque grading that only checks final outputs, (2) underspecified safety and robustness evaluation, and (3) narrow modality coverage and interaction paradigms.

Method: Introduced Claw-Eval with 300 human-verified tasks across 9 categories in three groups (general service orchestration, multimodal perception/generation, multi-turn professional dialogue). Records every agent action through three independent evidence channels (execution traces, audit logs, environment snapshots) enabling trajectory-aware grading over 2,159 fine-grained rubric items.

Result: Experiments on 14 frontier models show: (1) trajectory-opaque evaluation misses 44% of safety violations and 13% of robustness failures; (2) controlled error injection degrades consistency (Pass^3 drops up to 24%) more than peak capability; (3) multimodal performance varies sharply, with poor video performance and no single model dominating across all modalities.

Conclusion: Claw-Eval provides a more reliable evaluation framework that reveals systematic limitations in existing benchmarks and highlights actionable directions for building agents that are not only capable but reliably deployable.
