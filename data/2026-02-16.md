...
TL;DR: Thermal imaging enables contactless estimation of EDA, HR, and BR from facial video using signal processing pipelines, achieving moderate correlation with palm EDA and reasonable accuracy for BR/HR given low frame rate constraints.


<details>
  <summary>Details</summary>
Motivation: Thermal imaging captures skin temperature changes from autonomic regulation, potentially providing contactless estimation of EDA (which visible-light methods cannot access), HR, and BR for driver monitoring applications.

Method: Signal-processing pipeline tracks facial anatomical regions, applies spatial aggregation, separates slow sudomotor trends from faster cardiorespiratory components. Uses OMIT decomposition for HR across multiple ROIs, averages nasal/cheek signals for BR, evaluates 288 EDA configurations.

Result: Best EDA configuration (nose region, exponential moving average) achieves mean absolute correlation 0.40±0.23 against palm EDA (individual sessions up to 0.89). BR estimation: 3.1±1.1 bpm MAE, HR estimation: 13.8±7.5 bpm MAE (limited by 7.5 Hz frame rate).

Conclusion: Thermal imaging provides baseline performance for contactless biosignal estimation, with signal polarity alternation, short thermodynamic latency, and condition/demographic effects on quality, offering design guidance for future applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for motion-language models suffer from catastrophic forgetting of linguistic capabilities when fine-tuning LLMs on limited motion-text pairs, and they introduce jitter artifacts from discrete motion tokenization. There's a need for a unified framework that preserves language understanding while enabling scalable multimodal adaptation.

Method: Proposes LLaMo with a modality-specific Mixture-of-Transformers (MoT) architecture that extends pretrained LLMs. Encodes human motion into a causal continuous latent space and uses a lightweight flow-matching head to maintain the next-token prediction paradigm, enabling streaming motion generation in real-time (>30 FPS).
...
Conclusion: LLaMo represents a significant step toward a general unified motion-language large model by addressing key limitations of previous approaches through its Mixture-of-Transformers architecture and continuous motion representation, enabling both generation and understanding tasks while preserving linguistic capabilities.
...
TL;DR: CLIP-based synthetic image detectors perform well but rely on photographic style cues rather than generator artifacts, showing limited generalization across different generative models.


<details>
  <summary>Details</summary>
Motivation: With generative models producing near-photorealistic images, synthetic image detection (SID) is crucial for trustworthiness, but existing methods struggle with generalization to novel models and practical settings.

Method: Introduces SynthCLIC dataset to reduce semantic bias, uses interpretable linear head with de-correlated activations and text-grounded concept-model to analyze what CLIP-based detectors learn.

Result: CLIP-based detectors achieve 0.96 mAP on GAN benchmarks but only 0.92 on SynthCLIC, with cross-generator generalization dropping to 0.37 mAP. Detectors rely on photographic attributes rather than generator artifacts.

Conclusion: CLIP-based approaches are strong foundation for SID but need continual updates and broader training exposure due to uneven generalization across generative architectures.
...
TL;DR: Reproducibility study confirms DragDiffusion's core claims about point-based image editing via diffusion latent optimization, but finds performance is sensitive to specific hyperparameters like timestep selection and feature supervision levels.


<details>
  <summary>Details</summary>
Motivation: To conduct an independent reproducibility study of DragDiffusion's claims about interactive point-based image editing using diffusion models, verifying whether the method's reported capabilities and ablation trends can be reliably reproduced.

Method: Used authors' released implementation and DragBench benchmark to reproduce main ablation studies on diffusion timestep selection, LoRA-based fine-tuning, mask regularization strength, and UNet feature supervision. Also evaluated a multi-timestep latent optimization variant.

Result: Successfully reproduced qualitative and quantitative trends from original work, confirming DragDiffusion's central claims. Found performance is sensitive to optimized timestep and feature level for motion supervision, while other components have broader operating ranges. Multi-timestep optimization didn't improve spatial accuracy but increased computational cost.

Conclusion: The reproducibility study supports DragDiffusion's core claims while clarifying that reliable reproduction depends on specific hyperparameter conditions, particularly timestep selection and feature supervision levels, with other components being more robust to variation.
...
TL;DR: RL in vision-language models primarily refines mid-to-late transformer layers for better vision-to-reasoning alignment, not uniform visual perception enhancement.
...
Method: Proposed a Frankenstein-style analysis framework with: (1) functional localization via causal probing, (2) update characterization via parameter comparison, and (3) transferability test via model merging.

Result: RL induces consistent inference-time shift primarily in mid-to-late layers; these mid-to-late refinements are both transferable (via merging) and necessary (via freezing) for RL gains.

Conclusion: RL's reliable contribution in visual reasoning is not uniform enhancement of visual perception, but systematic refinement of mid-to-late transformer computation that improves vision-to-reasoning alignment and reasoning performance, highlighting limitations of benchmark-only evaluation.
...
TL;DR: ZeroDiff++ is a diffusion-based generative ZSL framework that addresses spurious visual-semantic correlations and data scarcity through diffusion augmentation, contrastive representations, and test-time adaptation to synthesize better unseen class features.


<details>
  <summary>Details</summary>
Motivation: Existing generative ZSL methods suffer from spurious visual-semantic correlations worsened by scarce seen class samples, and unadaptive generators produce features disconnected from real test samples, limiting zero-shot recognition performance.
...
Conclusion: ZeroDiff++ effectively addresses spurious correlations and data scarcity in ZSL through diffusion-based generation and test-time adaptation, achieving state-of-the-art performance on zero-shot learning benchmarks.
...
TL;DR: MonoLoss: A plug-in training objective that improves monosemanticity in sparse autoencoders by using an efficient single-pass MonoScore computation, achieving better interpretability and classification performance.


<details>
  <summary>Details</summary>
Motivation: Standard SAE training weakly encourages monosemantic feature decomposition, and existing monosemanticity metrics require inefficient pairwise comparisons across all dataset samples, making them impractical for training-time use.

Method: Derived a single-pass algorithm for MonoScore computation with linear rather than quadratic cost, then introduced Monosemanticity Loss (MonoLoss) as a plug-in objective that directly rewards semantically consistent activations.

Result: Achieved up to 1200x speedup in evaluation and 159x during training with only ~4% per-epoch overhead. MonoLoss increased MonoScore for most latents and improved class purity from 0.152 to 0.723. Also yielded up to 0.6% accuracy gains on ImageNet-1K when used as auxiliary regularizer.

Conclusion: MonoLoss enables efficient training-time optimization for monosemantic representations, improving both interpretability and downstream task performance across various SAE architectures and pretrained features.
...
TL;DR: PathoSpatial is an interpretable multimodal framework that fuses whole slide images (WSI) and spatial transcriptomics (ST) data for cancer prognosis, using task-guided prototype learning and multi-level experts architecture to achieve superior performance with enhanced biological interpretability.


<details>
  <summary>Details</summary>
Motivation: As paired WSI-ST cohorts scale to population level, there's a need for principled cross-modal fusion strategies that leverage complementary spatial signals from histopathology images and molecular data for improved cancer prognosis, but current methods remain limited.

Method: PathoSpatial uses task-guided prototype learning within a multi-level experts architecture that adaptively orchestrates unsupervised within-modality discovery with supervised cross-modal aggregation, integrating co-registered WSIs and ST data in an end-to-end framework.

Result: PathoSpatial delivers strong and consistent performance across five survival endpoints in triple-negative breast cancer, achieving superior or comparable performance to leading unimodal and multimodal methods while enabling post-hoc prototype interpretation and molecular risk decomposition.

Conclusion: PathoSpatial serves as a proof-of-concept for scalable and interpretable multimodal learning for spatial omics-pathology fusion, providing quantitative, biologically grounded explanations and highlighting candidate prognostic factors.
...
TL;DR: SAFT improves CLIP's adversarial robustness by fine-tuning with semantic-aware adversarial examples generated using refined textual descriptions instead of simple hand-crafted templates.


<details>
  <summary>Details</summary>
Motivation: Current adversarial fine-tuning methods for CLIP use adversarial examples generated by minimizing cosine similarity with simple hand-crafted templates, which may fail when using more semantically enriched similarity metrics, limiting robustness improvements.

Method: Proposes semantic-ensemble attack that generates semantic-aware adversarial examples by minimizing average similarity between original images and an ensemble of refined textual descriptions generated by foundation models. Uses these to perform Semantic-aware Adversarial Fine-Tuning (SAFT) of CLIP's image encoder.
...
Conclusion: Semantic-aware adversarial examples generated using refined textual descriptions are more effective for adversarial fine-tuning of CLIP, leading to significantly improved zero-shot adversarial robustness compared to methods using simple hand-crafted templates.
...
TL;DR: Proposes Optimized DenseNet 121 for grape leaf disease classification with 99.27% accuracy, using domain-specific preprocessing and Grad-CAM for interpretability.
...
Method: Uses Optimized DenseNet 121 with domain-specific preprocessing to extract disease-relevant features (veins, edges, lesions). Employs transfer learning, cross-validation, and Grad-CAM for interpretability.

Result: Achieves 99.27% accuracy, 99.28% F1 score, 99.71% specificity, 98.86% Kappa with 9-second inference time. Outperforms ResNet18, VGG16, AlexNet, SqueezeNet. Cross-validation shows 99.12% mean accuracy.

Conclusion: The framework is scalable, precise, computationally inexpensive for grape leaf disease detection, with model optimization for real-time deployment and interpretable outputs for practical use.
...
<details>
  <summary>Details</summary>
Motivation: Humans use coarse volumetric bodies for intuitive physics, while segmentation models optimize pixel-accurate masks. The paper investigates whether and when these models acquire human-like coarse representations despite their different optimization objectives.

Method: Used time-to-collision behavioral paradigm with comparison pipeline and alignment metric. Varied model training time, size, and effective capacity via pruning to examine how different resource constraints affect representation granularity.
...
Conclusion: Human-like coarse bodies emerge from resource constraints rather than bespoke biases. Simple adjustments like early checkpoints, modest architectures, and light pruning can elicit physics-efficient representations, supporting resource-rational accounts balancing recognition detail with physical affordances.
...
TL;DR: A novel method for establishing correspondence between 2D image sequences using slice insertion learning and attention mechanisms, achieving 5.4 mm localization error vs 8.4 mm for state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Slice-level content navigation is crucial for localizing specific 2D slices within 3D volumes or determining anatomical coverage, serving as important preprocessing for diagnostic tasks, registration, and segmentation pipelines.

Method: Trains a network to learn how to insert a slice from one sequence into the appropriate position in another using contextual representations and slice-to-slice attention mechanisms, leveraging sequence context rather than treating slices independently.

Result: Reduces slice localization errors from 8.4 mm to 5.4 mm in supervised settings, demonstrating substantial improvement over body part regression which predicts anatomical position scores for individual slices.

Conclusion: The insertion network approach outperforms current state-of-the-art body part regression by effectively leveraging contextual information from entire sequences, providing more accurate slice localization for medical imaging applications.
...
TL;DR: NAST improves medical VLMs' ability to distinguish affirmative from negated clinical statements using interpretability-guided selective training based on causal tracing effects.


<details>
  <summary>Details</summary>
Motivation: Vision-language models frequently fail to distinguish affirmative from negated medical statements, which is critical for safety in clinical reporting where negation is fundamental.
...
Result: Experiments show improved discrimination of affirmative and negated clinical statements without degrading general vision-language alignment, using a radiology-specific diagnostic benchmark and contextual clinical negation dataset.

Conclusion: Causal interpretability can enable targeted model adaptation in safety-critical medical settings, transforming mechanistic interpretability signals into principled optimization rules for better negation processing.
...
TL;DR: Proposes a novel approach for optical-SAR image matching by transforming both image types to a shared modality, enabling use of pre-trained RoMa matching models without modality-specific retraining.


<details>
  <summary>Details</summary>
Motivation: Optical and SAR images have fundamental physical differences that make precise co-registration difficult due to their different acquisition principles.

Method: Transform optical and SAR images to a new common modality with equal channels, similarity after co-registration, and preservation of original features, then use pre-trained RoMa model for matching.

Result: Superior performance over alternative image translation and feature matching approaches on MultiSenGE dataset, with better matching quality and versatility.

Conclusion: The approach enables high-quality optical-SAR matching using off-the-shelf models without retraining, offering both improved performance and practical versatility.
...
TL;DR: A self-supervised method called Collaborative Distillation uses 3D LiDAR to improve 2D image encoder robustness to adverse weather while maintaining original capabilities.
...
Method: Collaborative Distillation leverages 3D LiDAR as self-supervision to distill robustness into 2D image encoders while preserving their original feature extraction capabilities.
...
Conclusion: The method demonstrates practicality and adaptability for real-world scenarios by enhancing 2D image encoder robustness to adverse conditions using LiDAR self-supervision.
...
TL;DR: Complete geometric stratification of P3P singular configurations based on multiplicity of camera center, revealing relationships with danger cylinder, Morley triangle, and circumcircle.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate and classify singular configurations in the P3P (Perspective-3-Point) problem, which are critical for understanding when camera pose estimation becomes ambiguous or has infinite solutions.

Method: Uses local dual space and an algebraic-computational framework to analyze singular configurations, establishing geometric stratification based on multiplicity (μ) of the camera center O.

Result: For μ≥2: O lies on the "danger cylinder"; for μ≥3: O lies on one of three generatrices of the danger cylinder associated with Morley triangle or circumcircle; for μ≥4: O lies on circumcircle (infinite P3P solutions). Also characterizes complementary configuration O' on deltoidal surfaces and cuspidal curves.

Conclusion: Provides complete geometric characterization of P3P singular configurations, revealing hierarchical structure from danger cylinder to circumcircle, with practical implications for avoiding ambiguous camera pose estimation scenarios.
...
TL;DR: LGIP benchmark evaluates VLMs' linguistic robustness by testing invariance to paraphrases and sensitivity to semantic flips, revealing performance differences invisible to standard metrics.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models show strong zero-shot performance but lack understanding of how reliably they respond to controlled linguistic perturbations, creating a need for better diagnostic tools.

Method: Language-Guided Invariance Probing (LGIP) benchmark using 40k MS COCO images with human captions, automatically generating paraphrases and rule-based semantic flips (object category, color, count changes).

Result: EVA02-CLIP and large OpenCLIP variants show favorable invariance-sensitivity balance, while SigLIP/SigLIP2 have high invariance error and often prefer flipped captions. These failures are invisible to standard retrieval metrics.

Conclusion: LGIP provides a model-agnostic diagnostic tool for assessing linguistic robustness of VLMs beyond conventional accuracy scores, revealing important performance differences in how models handle linguistic variations.
...
<details>
  <summary>Details</summary>
Motivation: Autonomous driving requires world models that capture environment evolution for long-term planning, and scalability demands self-supervised learning without expensive human annotations. JEPA enables learning world models from large volumes of unlabeled data.

Method: Proposes AD-LiST-JEPA, a self-supervised world model that predicts future spatiotemporal evolution from LiDAR data using a Joint-Embedding Predictive Architecture (JEPA) framework.

Result: Proof of concept experiments show better LiDAR-based occupancy completion and forecasting (OCF) performance with pretrained encoder after JEPA-based world model learning, demonstrating improved representation quality.

Conclusion: AD-LiST-JEPA successfully learns world models for autonomous driving in a self-supervised manner using JEPA, enabling better perception and prediction capabilities for downstream tasks like occupancy completion and forecasting.
...
TL;DR: PLLM: Self-training framework for CAD program synthesis from unlabeled 3D shapes using iterative sampling, selection, and augmentation to create synthetic training data.


<details>
  <summary>Details</summary>
Motivation: Existing CAD program synthesis methods require supervised training with paired shape-program data, which is often unavailable. There's a need for approaches that can work with unlabeled 3D shapes.

Method: Self-training framework that iteratively: 1) samples candidate CAD programs from a pre-trained LLM, 2) selects high-fidelity program executions, 3) augments programs to construct synthetic program-shape pairs, and 4) fine-tunes the LLM on this synthetic data.
...
Conclusion: PLLM demonstrates that self-training with synthetic program-shape pairs enables effective CAD program synthesis from unlabeled 3D shapes, overcoming the data limitation problem in supervised approaches.
...
TL;DR: Researchers identify decoupling failure in autonomous driving research between appearance shifts (weather/lighting) and structural changes, create navdream benchmark to isolate appearance impact, find existing planners degrade under OOD appearance, and propose universal perception interface using DINOv3 for zero-shot generalization across planning paradigms.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving algorithms are fragile under Out-of-Distribution (OOD) conditions, but research fails to distinguish between appearance-based shifts (weather, lighting) and structural scene changes. This leaves a fundamental question unanswered: Is the planner failing because of complex road geometry, or simply because it is raining?

Method: 1) Establish navdream benchmark using generative pixel-aligned style transfer to create visual stress tests with negligible geometric deviation, isolating appearance impact. 2) Propose universal perception interface leveraging frozen visual foundation model (DINOv3) to extract appearance-invariant features as stable interface for planners.

Result: Evaluation reveals existing planning algorithms show significant degradation under OOD appearance conditions even when scene structure remains consistent. The proposed DINOv3-based interface achieves exceptional zero-shot generalization across diverse planning paradigms (regression-based, diffusion-based, scoring-based) and maintains consistent performance across extreme appearance shifts without fine-tuning.

Conclusion: The work addresses critical decoupling failure in autonomous driving research by distinguishing appearance vs. structural changes, provides navdream benchmark for robustness evaluation, and offers plug-and-play solution using DINOv3 for appearance-invariant perception that generalizes across planning paradigms without additional training.
...
<details>
  <summary>Details</summary>
Motivation: Current event-based vision methods either bin events into frames (losing gradients) or learn directly from raw events (suffering from biased gradient estimation due to binning discontinuities), limiting learning efficiency.

Method: Uses integration by parts to lift target functions to functionals, deriving weak derivatives of binning functions during backpropagation while keeping forward output unchanged. Reconstructs cotangent function from sampled cotangent vectors to compute provably accurate weak derivatives.
...
Conclusion: The proposed framework enables unbiased gradient estimation for event binning functions, significantly improving learning efficiency and performance across various event-based visual perception tasks.
...
TL;DR: QuEPT is an efficient post-training quantization scheme for LLMs that enables dynamic multi-bit deployment with one-shot calibration, supporting real-time switching between uniform and mixed precision quantization via cascaded low-rank adapters.


<details>
  <summary>Details</summary>
Motivation: Transformer architectures have high storage and optimization costs, making elastic quantization research limited for large language models despite its potential for multi-bit deployment flexibility.

Method: Uses one-shot calibration on small data to reconstruct block-wise multi-bit errors, cascades different low-rank adapters for bit-width adaptation, introduces Multi-Bit Token Merging (MB-ToMe) for cross-bit-width feature fusion, and employs Multi-Bit Cascaded Low-Rank adapters (MB-CLoRA) to strengthen bit-width group correlations.
...
Conclusion: QuEPT provides an efficient solution for elastic quantization of LLMs with dynamic bit-width adaptation and real-time quantization mode switching without repeated optimization.
...
TL;DR: ADSC reduces MLLM computational costs by using the LLM's own attention mechanism to progressively compress vision tokens through uniform downsampling at selected layers, achieving 53.7% FLOP reduction while preserving 98.2% performance.
...
Method: Attention-Driven Self-Compression (ADSC) treats the LLM as the optimal guide for compression. It applies uniform token downsampling at selected layers, forming bottlenecks that encourage the model to reorganize and compress information into remaining tokens using only the LLM's attention mechanism.

Result: Applied to LLaVA-1.5, ADSC reduces FLOPs by 53.7% and peak KV-cache memory by 56.7%, while preserving 98.2% of original model performance. Outperforms prior pruning approaches across multiple benchmarks and remains robust under high compression ratios.

Conclusion: ADSC provides a simple, broadly applicable compression method that leverages the LLM's own attention mechanism without requiring score computation, auxiliary modules, or attention modification, maintaining full FlashAttention compatibility.
...
TL;DR: ImageRAGTurbo improves few-step diffusion models by using retrieval-augmented generation with trainable adapters to maintain image quality while reducing sampling steps.


<details>
  <summary>Details</summary>
Motivation: Few-step diffusion models reduce latency but compromise image quality and prompt alignment, especially in one-step generation, and require expensive training. Need efficient method to maintain quality while reducing steps.

Method: Retrieve relevant text-image pairs for prompts, use them to condition generation. Edit denoiser's latent H-space with retrieved content, then augment UNet with trainable adapter in H-space using cross-attention to blend retrieved content with target prompt.

Result: Approach produces high-fidelity images without compromising latency compared to existing methods, improves prompt fidelity even without finetuning.

Conclusion: Retrieval augmentation with trainable adapters enables efficient few-step diffusion models that maintain image quality while reducing sampling latency.
...
TL;DR: Additive skip fusion in U-Net architectures improves denoising and multi-task learning by replacing concatenative skips with gated additive fusion, acting as an architectural regularizer for stable optimization.


<details>
  <summary>Details</summary>
Motivation: To improve training stability and performance in image denoising and denoising-centric multi-task learning by constraining shortcut capacity while preserving fixed feature dimensionality across network depth.

Method: Proposed Additive U-Net (AddUNet) replaces concatenative skip connections with gated additive fusion, which constrains shortcut capacity and maintains consistent feature dimensionality throughout the network.

Result: AddUNet achieves competitive reconstruction performance with improved training stability in both single-task denoising and joint denoising-classification settings. In MTL, learned skip weights show task-aware redistribution: shallow skips favor reconstruction while deeper features support discrimination.

Conclusion: Simple constraints on skip connections serve as an effective architectural regularizer for stable and scalable multi-task learning without increasing model complexity, enabling implicit task decoupling through additive fusion.
...
TL;DR: Researchers created CloudyBigEarthNet dataset with cloudy optical-radar image pairs to develop cloud-robust methods, showing existing clear-sky trained models drop 23-33pp on cloudy images, but adaptation improves performance by 17-28pp.


<details>
  <summary>Details</summary>
Motivation: Clouds distort optical satellite imagery, forcing exclusion of cloudy images from ML datasets, which fails for time-sensitive applications like disaster response. Cloud removal methods have artifacts, so cloud-robust methods using radar-optical fusion are needed.

Method: Created CloudyBigEarthNet (CBEN) dataset with paired optical and radar images containing cloud occlusion for training/evaluation. Adapted state-of-the-art methods to train on cloudy optical data combined with radar instead of only clear-sky data.

Result: Methods trained on clear-sky optical+radar suffer 23-33 percentage point performance drops on cloudy images. When adapted to train on cloudy optical data, achieved 17.2-28.7 percentage point relative improvements on cloudy test cases.

Conclusion: Excluding cloudy images from ML training/evaluation limits real-world applicability. CloudyBigEarthNet enables development of cloud-robust methods, showing significant performance improvements when models are trained with cloudy optical data alongside radar.
...
TL;DR: IndicFairFace is a new balanced face dataset addressing geographical bias in VLMs by representing India's 28 states and 8 Union Territories, used to quantify and reduce intra-national bias in CLIP models.


<details>
  <summary>Details</summary>
Motivation: Current fairness datasets treat Indian as a single monolithic category, ignoring India's vast intra-national diversity across 28 states and 8 Union Territories, leading to representational and geographical bias in VLMs.

Method: Created IndicFairFace dataset with 14,400 images ethically sourced from Wikimedia Commons and open-license repositories, uniformly balanced across states and gender. Used Iterative Nullspace Projection debiasing approach on CLIP-based VLMs.

Result: Successfully quantified intra-national geographical bias in prominent CLIP-based VLMs and reduced it using debiasing, with less than 1.5% average drop in retrieval accuracy on benchmark datasets.

Conclusion: IndicFairFace establishes the first benchmark to study geographical bias in VLMs for the Indian context, addressing the oversimplification of Indian as a single category and enabling more nuanced fairness evaluation.
...
TL;DR: MPD is an inference-time distillation technique that improves video inbetweening by aligning forward and backward paths using motion prior distillation to reduce temporal discontinuities.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time sampling methods for image-to-video inbetweening suffer from temporal discontinuities and visual artifacts due to misalignment between forward and backward generated paths, as each path follows different motion priors from their respective conditioning frames.

Method: Motion Prior Distillation (MPD) - an inference-time distillation technique that suppresses bidirectional mismatch by distilling the motion residual of the forward path into the backward path, avoiding denoising the end-conditioned path to reduce ambiguity.
...
TL;DR: CAP improves probing performance for multi-channel imaging by using channel-aware feature encoding and pooling to better leverage pre-trained encoders without full fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Multi-channel imaging data has varying channel configurations across datasets, making it hard to reuse pre-trained encoders. Existing probing methods perform poorly on MCI data, often worse than training from scratch, creating a need for better ways to leverage fixed representations.

Method: Channel-Aware Probing (CAP) uses Independent Feature Encoding (IFE) to encode each channel separately and Decoupled Pooling (DCP) to pool within channels before aggregating across channels, exploiting inter-channel diversity in MCI data.

Result: CAP consistently improves probing performance over default protocols, matches fine-tuning from scratch, and significantly reduces the gap to full fine-tuning across three MCI benchmarks.

Conclusion: CAP provides an effective probing strategy for multi-channel imaging that better leverages pre-trained encoders without requiring full fine-tuning, addressing the challenge of varying channel configurations in MCI data.
...
TL;DR: ART3mis is a user-friendly, general-purpose 3D annotation tool for cultural heritage professionals that enables non-technical users to segment and annotate 3D digital artifacts with metadata stored in JSON format.


<details>
  <summary>Details</summary>
Motivation: Archaeologists and cultural heritage professionals need more advanced 3D visualization tools with annotation capabilities beyond simplistic visualizations. Existing solutions are often domain-specific and not accessible to non-technical users like conservators and curators.

Method: ART3mis uses a user-driven, direct-on-surface approach that allows real-time handling of detailed 3D cultural objects. The tool enables segmentation and annotation of 3D digital replicas with textual metadata stored in JSON format.

Result: The tool successfully provides a general-purpose solution that works in real-time with detailed 3D cultural objects, allowing storage of textual annotations for multiple complex regions in JSON format.

Conclusion: ART3mis addresses the need for accessible 3D annotation tools in cultural heritage by providing a user-friendly, interactive solution that empowers non-technical professionals to work with 3D digital artifacts.
...
TL;DR: RADAR is an evaluation framework for pre-trained MLLMs that uses Soft Discrimination Score and Multi-Modal Mixture Benchmark to assess perception/reasoning abilities without fine-tuning, revealing asymmetric development patterns.


<details>
  <summary>Details</summary>
Motivation: Current MLLM evaluation lacks efficiency (requires fine-tuning) and cannot disentangle perception vs reasoning abilities. Existing benchmarks are limited in scale and misaligned with pre-training objectives.

Method: Two components: (1) Soft Discrimination Score - novel metric quantifying model preference for correct answers over distractors without fine-tuning; (2) Multi-Modal Mixture Benchmark - 15K+ sample benchmark unifying authoritative datasets and collecting new ones for comprehensive 0-shot evaluation.

Result: RADAR reveals asymmetric development of perceptual vs reasoning capabilities in pre-trained MLLMs across factors like data volume, model size, and pretraining strategy.

Conclusion: The framework enables decomposed analysis of pre-training ability bottlenecks, informing targeted interventions to advance MLLMs more efficiently.
...
TL;DR: VimRAG is a multimodal RAG framework that models reasoning as a dynamic graph to better handle visual data in long-context tasks, using graph-modulated memory encoding and graph-guided policy optimization.


<details>
  <summary>Details</summary>
Motivation: Traditional RAG methods struggle with long-context tasks involving visual data because they use linear interaction histories, which are inefficient for information-sparse yet token-heavy visual content in iterative reasoning scenarios.

Method: Models reasoning as a dynamic directed acyclic graph structuring agent states and retrieved multimodal evidence. Uses Graph-Modulated Visual Memory Encoding to evaluate memory node significance via topological position, dynamically allocating high-resolution tokens to pivotal evidence while compressing/discarding trivial clues. Implements Graph-Guided Policy Optimization to disentangle step-wise validity from trajectory-level rewards by pruning memory nodes associated with redundant actions.
...
TL;DR: Proposes a method to evaluate object detection model robustness in autonomous vehicles under adverse weather/lighting conditions using synthetic data augmentation to find failure thresholds, with Faster R-CNN showing highest robustness.
...
Method: Employs data augmentation operators to generate synthetic data simulating different severance degrees of adverse operation conditions at progressive intensity levels to find the lowest intensity at which object detection models fail. Measures robustness using average first failure coefficients (AFFC) over input images. Tests four models (YOLOv5s, YOLOv11s, Faster R-CNN, Detectron2) with seven augmentation operators simulating fog, rain, snow, dark, bright, flaring, and shadow conditions.

Result: Method is feasible, effective, and efficient for evaluating and comparing robustness. Faster R-CNN achieved highest robustness with overall average AFFC of 71.9% over all seven adverse conditions, while YOLO variants showed AFFC values of 43%. Training with synthetic data can improve robustness but may suffer from diminishing returns and forgetting phenomena if overtrained.

Conclusion: The proposed method provides a systematic approach to evaluate object detection model robustness in adverse conditions, enabling comparison between models and assessment of training strategies. Faster R-CNN demonstrates superior robustness, and targeted training with synthetic data can enhance performance but requires careful optimization to avoid negative effects.
...
<details>
  <summary>Details</summary>
Motivation: Existing rigging methods fail for sequential data (animal motion capture, AIGC/video-derived mesh sequences) because they assume a canonical rest pose (like T-pose) which doesn't exist in such data. When applied frame-by-frame, these methods produce topological inconsistencies and lack pose invariance.

Method: SPRig is a general fine-tuning framework that adds cross-frame consistency losses on top of existing rigging models to learn pose-invariant rigs. It uses a new permutation-invariant stability protocol for validation.

Result: Experiments show state-of-the-art temporal stability: SPRig produces coherent rigs from challenging sequences and dramatically reduces artifacts that plague baseline methods.
...
TL;DR: EPRBench: A comprehensive benchmark for event stream-based Visual Place Recognition with 10K sequences, 65K frames, and LLM-generated scene descriptions, plus a novel multi-modal fusion framework for interpretable VPR.


<details>
  <summary>Details</summary>
Motivation: Event stream-based VPR addresses limitations of conventional cameras in challenging conditions, but lacks dedicated datasets. Current scarcity of benchmarks hinders research progress in this emerging field.

Method: 1) Created EPRBench dataset with handheld/vehicle-mounted setups across diverse conditions; 2) Provided LLM-generated scene descriptions with human refinement; 3) Benchmarked 15 SOTA VPR algorithms; 4) Proposed novel multi-modal fusion framework using LLMs to generate textual descriptions from event streams for guided feature fusion.

Result: Established comprehensive benchmark with 10K event sequences and 65K frames. Implemented systematic evaluation of 15 VPR algorithms. Proposed framework achieves accurate place recognition with interpretable reasoning processes, enhancing model transparency.

Conclusion: EPRBench fills critical gap in event-based VPR research, enabling systematic algorithm evaluation. The proposed multi-modal fusion framework demonstrates promising approach for accurate and interpretable place recognition, advancing event-based perception with LLM integration.
...
TL;DR: Annotation-free framework for painting craquelure restoration using synthetic crack generation and detector-guided refinement with SegFormer-LoRA, followed by anisotropic diffusion inpainting.


<details>
  <summary>Details</summary>
Motivation: Cultural heritage preservation needs non-invasive digital restoration methods, but identifying fine craquelure patterns from complex brushstrokes is challenging due to scarce pixel-level annotations.

Method: 1) Synthetic craquelure generator using Bézier trajectories for realistic branching patterns; 2) Coupled morphological detector with SegFormer-LoRA refinement module; 3) Detector-guided strategy using morphological map as spatial prior; 4) Masked hybrid loss and logit adjustment for focused crack refinement; 5) Anisotropic Diffusion inpainting guided by refined masks.

Result: Significantly outperforms state-of-the-art photographic restoration models in zero-shot settings while faithfully preserving original paint brushwork.
...
TL;DR: A deep learning deformable registration framework aligns pathological brains with metastases to a common atlas without requiring lesion masks, enabling standardized spatial analysis of melanoma brain metastases across multiple centers.


<details>
  <summary>Details</summary>
Motivation: Melanoma brain metastases (MBM) are common but spatially heterogeneous, making cohort-level analysis difficult due to anatomical variability and different MRI protocols across centers. Standard registration methods struggle with pathological brains containing metastatic lesions that disrupt anatomical correspondences.

Method: A fully differentiable deep-learning-based deformable registration framework that aligns individual pathological brains to a common atlas while preserving metastatic tissue. Uses a forward-model similarity metric based on distance-transformed anatomical labels to handle missing correspondences from metastases, combined with volume-preserving regularization for deformation plausibility.

Result: Achieved high registration accuracy across 209 MBM patients from three centers (DSC 0.89-0.92, HD 6.79-7.60 mm, ASSD 0.63-0.77 mm) while preserving metastatic volumes. Spatial analysis showed significant over-representation of MBM in cerebral cortex and putamen, under-representation in white matter, and consistent localization near gray-white matter junction. No arterial territory showed increased metastasis frequency after volume correction.

Conclusion: The framework enables robust atlas registration of pathological brain MRI without lesion masks and supports reproducible multi-center analyses. It confirms and refines known spatial predilections of MBM, particularly preferential seeding near gray-white matter junction and cortical regions. The publicly available implementation facilitates extension to other brain tumors and neurological pathologies.
...
<details>
  <summary>Details</summary>
Motivation: Whole brain age (WBA) is too coarse for tasks like disease characterization and aging research since brain changes are region-selective. Robust regional brain age (ReBA) estimation is critical but lacks widely generalizable models.

Method: Proposes ReBA-Pred-Net, a Teacher-Student framework for fine-grained brain age estimation. Teacher produces soft ReBA to guide Student, with clinical-prior consistency constraint (regions within same function should change similarly).

Result: Introduces two indirect evaluation metrics: Healthy Control Similarity (HCS) for statistical consistency and Neuro Disease Correlation (NDC) for factual consistency. Experiments across multiple backbones demonstrate statistical and factual validity of the method.

Conclusion: ReBA-Pred-Net provides a robust framework for regional brain age estimation with validated statistical and factual consistency, addressing limitations of whole brain age approaches for disease characterization and aging research.
...
TL;DR: Diffusion priors for sparse-view CT show domain shift and forward model mismatch complicate real-world application, requiring careful validation.


<details>
  <summary>Details</summary>
Motivation: Most studies use synthetic data, unclear if domain shift or forward model mismatch affect diffusion-based priors for experimental CT data.

Method: Measured CT data from physical phantom, trained diffusion priors on synthetic data with varying domain shift, used Decomposed Diffusion Sampling on sparse-view CT data.

Result: Domain shift causes nuanced effects - severe mismatch leads to model collapse, but diverse priors outperform narrow ones. Forward model mismatch causes artifacts but can be mitigated with annealed likelihood schedules.

Conclusion: Performance gains don't immediately translate from synthetic to experimental data; future development must validate against real-world benchmarks.
...
TL;DR: Proposes sequential hypothesis testing as a safety mechanism for real-time object tracking, providing statistical guarantees on failure detection while limiting false alerts.


<details>
  <summary>Details</summary>
Motivation: Current object tracking systems lack formal safety assurances about when tracking is reliable versus when it may fail, relying only on heuristic confidence measures. There's a need for statistically grounded methods to detect tracking failures with provable guarantees.

Method: Interprets object tracking as a sequential hypothesis test using e-processes to accumulate evidence for tracking failures over time. Proposes both supervised (using ground-truth) and unsupervised (using internal tracking information) variants that are computationally light-weight, require no extra training, and are model-agnostic.

Result: Demonstrates effectiveness across two established tracking models and four video benchmarks. The approach quickly identifies tracking failures while provably containing false alerts at a desired rate, limiting costly re-calibration or intervention steps.

Conclusion: Sequential testing offers a statistically grounded and efficient mechanism to incorporate safety assurances into real-time tracking systems, providing formal guarantees about tracking reliability without requiring additional training or fine-tuning.
...
TL;DR: ART3mis is a web-based 3D annotation tool for cultural heritage that enables non-technical users to annotate and attach metadata to specific regions of 3D digital artifacts, with W3C Web Annotation Data Model compliance for interoperability.


<details>
  <summary>Details</summary>
Motivation: Current 3D visualization tools for cultural heritage lack advanced annotation capabilities, are domain-specific, lack generalization and interoperability, and are not accessible to non-technical users like conservators and curators.

Method: Developed ART3mis - a general-purpose, user-friendly, feature-rich, interactive web-based textual annotation tool for 3D objects that complies with W3C Web Annotation Data Model standards.

Result: Created a tool that enables communication, distribution, and reuse of information through standardized annotations, specifically designed for cultural heritage professionals without technical 3D expertise.

Conclusion: ART3mis addresses the limitations of existing 3D visualization tools by providing an accessible, interoperable annotation solution that empowers cultural heritage professionals to work with 3D digital artifacts effectively.
...
Method: Two-stage approach: 1) Data-level curriculum organizing image pairs by difficulty, 2) Model-level curriculum with progressive capacity increase via layer unfreezing and dynamic LoRA rank scheduling. Also proposes alternative ranking strategy.
...
Conclusion: Curriculum-DPO++ effectively addresses the varying difficulty of preference learning through combined data and model-level curriculum, achieving superior performance in text-to-image generation preference optimization.
...
TL;DR: PixelRush is a training-free framework for high-resolution text-to-image generation that achieves 10-35× speedup over SOTA methods, generating 4K images in ~20 seconds while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: Pre-trained diffusion models are limited by their native training resolution, and existing training-free approaches for high-resolution generation incur substantial computational overhead (5+ minutes for 4K images).
...
Result: Achieves exceptional efficiency: 4K images in ~20 seconds (10-35× speedup over SOTA methods) while maintaining superior visual fidelity. Extensive experiments validate performance gains and output quality.

Conclusion: PixelRush presents the first tuning-free framework for practical high-resolution text-to-image generation, offering dramatic speed improvements without compromising quality.
...
TL;DR: VideoLMs that use video codec primitives (motion vectors & residuals) instead of full images to reduce computational overhead while maintaining or improving performance on video understanding tasks.
...
Method: Leverage video codec primitives (motion vectors and residuals) that natively encode video redundancy and sparsity without expensive full-image encoding. Introduce lightweight transformer-based encoders to aggregate codec primitives and align their representations with image encoder embeddings through pre-training.
...
Conclusion: Using video codec primitives provides an efficient alternative to full-image encoding for VideoLMs, significantly reducing computational overhead while maintaining or improving video understanding performance across diverse tasks.
...
TL;DR: WS-COC is the first MLLM-driven weakly-supervised framework for class-agnostic object counting that uses only image-level counts as supervision, achieving performance comparable to fully-supervised methods with much lower annotation costs.


<details>
  <summary>Details</summary>
Motivation: Fully-supervised counting methods require costly point-level annotations per object, while existing weakly-supervised methods are limited to single categories. There's a need for a class-agnostic counting approach that reduces annotation burden while maintaining performance.

Method: Three key strategies: 1) Divide-and-discern dialogue tuning to guide MLLM through multi-round range estimation; 2) Compare-and-rank count optimization to train MLLM on relative ranking of images by object counts; 3) Global-and-local counting enhancement to fuse local and global predictions for dense scenes.

Result: Extensive experiments on FSC-147, CARPK, PUCPR+, and ShanghaiTech show WS-COC matches or surpasses many state-of-the-art fully-supervised methods while significantly reducing annotation costs.

Conclusion: WS-COC demonstrates that MLLMs can be effectively adapted for class-agnostic object counting with only image-level supervision, achieving strong performance comparable to fully-supervised approaches with dramatically lower annotation requirements.
...
TL;DR: GSM-GS improves 3D Gaussian Splatting by combining single-view adaptive region weighting and multi-view spatial structure refinement to enhance reconstruction accuracy and preserve high-frequency details.


<details>
  <summary>Details</summary>
Motivation: The unstructured nature of Gaussian point clouds in 3D Gaussian Splatting causes high-frequency detail loss in complex surface microstructures, limiting reconstruction accuracy despite the method's fast training and high-fidelity rendering capabilities.

Method: Proposes GSM-GS with two components: 1) Single-view optimization using image gradient features to partition scenes into texture-rich/texture-less regions with adaptive filtering and dual-branch constraints; 2) Multi-view optimization with geometry-guided cross-view point cloud association and dynamic weight sampling to create 3D structural normal constraints across frames.

Result: Extensive experiments on public datasets show the method achieves competitive rendering quality and geometric reconstruction, effectively addressing the detail loss problem in complex surface microstructures.

Conclusion: GSM-GS successfully enhances 3D Gaussian Splatting reconstruction by synergistically combining single-view adaptive constraints with multi-view spatial refinement, improving both geometric detail characterization and multi-view consistency.
...
TL;DR: MMRad-IVL-22K is the first large-scale dataset for interleaved vision-language reasoning in chest X-ray interpretation, enabling AI models to mimic radiologists' repeated visual inspection and language reasoning cycles.


<details>
  <summary>Details</summary>
Motivation: Current medical LVLMs rely on text-only chain-of-thought reasoning after a single visual inspection, leading to hallucinations. Pseudo-visual solutions using coordinates fail to preserve rich visual details like texture and density, unlike the interleaved visual-language reasoning process used by radiologists.

Method: Created MMRad-IVL-22K dataset with 21,994 diagnostic traces that capture radiologists' repeated cycles of reasoning and visual inspection across 35 anatomical regions, where visual rationales complement textual descriptions to ground each reasoning step.

Result: Report generation with multimodal CoT outperforms text-only CoT by 6% in RadGraph metric. Models fine-tuned on MMRad-IVL-22K achieve superior reasoning consistency and report quality compared to both general-purpose and medical-specific LVLMs.

Conclusion: High-fidelity interleaved vision-language evidence is essential for reliable medical AI, and the MMRad-IVL-22K dataset enables AI models to better mimic radiologists' diagnostic workflow, improving clinical accuracy and report quality.
...
TL;DR: Roadscapes is a multimodal dataset of 9,000 Indian driving scene images with bounding boxes and generated QA pairs for scene understanding tasks.


<details>
  <summary>Details</summary>
Motivation: To advance research on visual scene understanding in unstructured environments, particularly for autonomous driving systems that need to interpret diverse Indian driving conditions including urban/rural areas, highways, village paths, and congested streets.

Method: Collected 9,000 images from diverse Indian driving environments, manually verified bounding boxes, used rule-based heuristics to infer scene attributes, and generated question-answer pairs for object grounding, reasoning, and scene understanding tasks.

Result: Created Roadscapes dataset with diverse Indian driving scenes (urban/rural, day/night), provided key dataset statistics, and established initial baselines for image QA tasks using vision-language models.

Conclusion: Roadscapes facilitates scalable scene understanding research for autonomous driving in unstructured environments, particularly focusing on the unique challenges of Indian driving conditions.
...
TL;DR: A framework that converts static completed SDFs into editable, structurally coherent objects through part-aware scaling, enabling proportional deformation without structural distortions.
...
Method: Uses SDFs and Texture Fields from completion models, performs automatic part segmentation, defines user-controlled scaling zones, applies smooth interpolation of SDFs, color, and part indices for proportional deformation, and incorporates repetition-based strategy for large-scale deformations while preserving repeating patterns.

Result: Experiments on Matterport3D and ShapeNet objects show the method overcomes inherent rigidity of completed SDFs and is visually more appealing than global and naive selective scaling, especially for complex shapes and repetitive structures.

Conclusion: The part-aware scaling framework successfully transforms static completed SDFs into editable, structurally coherent objects, enabling flexible manipulation while preserving geometric integrity and visual quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing Thinking with Images (TWI) methods assume perfect interleaved image-text reasoning chains, but real-world multimodal understanding often contains errors in visual cue mining and reasoning, leading to error accumulation that degrades MLLM performance.

Method: RTWI estimates reliability of both visual cues and textual Chain-of-Thought in a unified text-centric manner, then employs robust filtering and voting modules to prevent noisy thinking from contaminating final answers.
...
Conclusion: RTWI provides a reliable solution to the noisy thinking problem in multimodal reasoning by addressing error accumulation through reliability estimation and robust filtering mechanisms.
...
TL;DR: The Intrapartum Ultrasound Grand Challenge (IUGC) addresses high maternal/neonatal mortality during labor by creating a multi-task framework for automated ultrasound measurements and releasing the largest multi-center intrapartum ultrasound dataset to overcome sonographer shortages in resource-limited settings.
...
Method: Created Intrapartum Ultrasound Grand Challenge with clinically-oriented multi-task framework integrating standard plane classification, fetal head-pubic symphysis segmentation, and biometry. Released largest multi-center dataset (774 videos, 68,106 frames from 3 hospitals). Analyzed submissions from 8 teams across preprocessing, data augmentation, learning strategy, model architecture, and post-processing.
...
Conclusion: Further in-depth investigation required before large-scale clinical deployment. Public release of dataset and solutions aims to facilitate reproducible research and promote advances in automatic intrapartum ultrasound biometry.
...
TL;DR: MLLMEmbed-ReID: A unified cloud-edge framework adapting MLLMs for cross-modal re-identification across RGB, infrared, sketch, and text modalities using instruction-based prompting and novel distillation.


<details>
  <summary>Details</summary>
Motivation: Practical cloud-edge CM-ReID faces challenges with fragmented specialized models for diverse modalities. MLLMs offer unification potential but existing approaches fail to adapt them into end-to-end backbones and lack effective distillation for edge deployment.

Method: 1) Adapt foundational MLLM into cloud model using instruction-based prompting for unified embedding space across modalities, trained with hierarchical LoRA-SFT under cross-modal alignment. 2) Novel distillation strategy using Principal Component Mapping loss (prioritizes essential info) and Feature Relation loss (preserves relational structures) for edge-native student.

Result: Lightweight edge model achieves SOTA on multiple visual CM-ReID benchmarks; cloud counterpart excels across all CM-ReID benchmarks. Framework enables unified MLLM-level intelligence on resource-constrained devices.

Conclusion: MLLMEmbed-ReID presents a complete and effective solution for deploying unified MLLM-level intelligence on resource-constrained devices, addressing fragmentation and distillation challenges in cloud-edge CM-ReID deployment.
...
TL;DR: Training-free acceleration method for VLM-based document parsing using speculative decoding with lightweight draft models and parallel region-based processing, achieving 2.42-4.89x speedup.
...
Method: Uses speculative decoding with lightweight document parsing pipeline as draft model to predict token batches, verified by accurate VLM in parallel. Exploits document layout by partitioning pages into independent regions for parallel decoding, then assembling predictions in reading order.
...
Conclusion: Proposed training-free acceleration method effectively reduces inference latency for VLM-based document parsing while maintaining accuracy, with code to be released for reproducibility.
...
TL;DR: MASAR is a differentiable framework for joint 3D detection and trajectory forecasting that uses object-centric spatio-temporal encoding to capture long-term dependencies by predicting and refining past trajectories with appearance cues.


<details>
  <summary>Details</summary>
Motivation: Traditional autonomous driving systems use hand-crafted bounding-box interfaces that limit information flow and propagate errors. End-to-end models often fail to fully exploit synergy between appearance and motion cues, relying mainly on short-term visual features.

Method: MASAR employs an object-centric spatio-temporal mechanism that jointly encodes appearance and motion features. It predicts past trajectories and refines them using guidance from appearance cues to capture long-term temporal dependencies for better future trajectory forecasting.
...
Conclusion: MASAR effectively enhances trajectory forecasting by capturing long-term temporal dependencies through joint encoding of appearance and motion features, demonstrating significant performance improvements in autonomous driving tasks.
...
TL;DR: ASID introduces a million-scale structured audiovisual instruction dataset, a scalable curation pipeline, and a video understanding model that achieves SOTA performance on fine-grained video tasks.


<details>
  <summary>Details</summary>
Motivation: Existing video understanding models are limited by coarse, incomplete video-instruction data that lacks fine-grained organization and reliable annotation, failing to capture complex audiovisual content.

Method: Three components: (1) ASID-1M dataset with 1M structured fine-grained audiovisual instructions, (2) ASID-Verify scalable curation pipeline with automatic verification for semantic/temporal consistency, (3) ASID-Captioner model trained via SFT on ASID-1M.

Result: ASID-Captioner improves fine-grained caption quality, reduces hallucinations, improves instruction following, achieves SOTA among open-source models, and is competitive with Gemini-3-Pro across 7 benchmarks covering captioning, QA, and temporal grounding.

Conclusion: The structured fine-grained annotation approach with automatic verification enables superior video understanding, addressing limitations of existing video-instruction data and advancing universal video understanding capabilities.
...
Method: Proposes TCMax method that maximizes total correlation between multimodal features and labels. Introduces Total Correlation Neural Estimation (TCNE) to derive a lower bound for total correlation, then presents TCMax as a hyperparameter-free loss function that maximizes total correlation through variational bound optimization.

Result: Extensive experiments demonstrate that TCMax outperforms state-of-the-art joint and unimodal learning approaches.

Conclusion: By maximizing total correlation between multimodal features and labels, TCMax effectively alleviates modality competition while capturing inter-modal interactions, providing a theoretically grounded solution to multimodal learning challenges.
...
Method: Dual-guidance strategy combining global pseudo-labels from zero-shot models (DiffSeg/SegFormer) with local boundary refinement via lightweight CNN, plus dynamic multi-component loss balancing feature similarity, Huber-smoothed spatial continuity, and semantic alignment.

Result: Achieves state-of-the-art performance: 17.5% mIoU improvement on BSD500, 3.1% on PASCAL VOC2012, and 11.66% on COCO, with plug-and-play integration and minimal computational footprint.

Conclusion: DynaGuide offers a scalable, practical solution for unsupervised segmentation with strong generalization, modular design, and no need for ground-truth labels in target domains.
...
TL;DR: Using ALS-derived pseudo-labels enhanced by SAM 2 to train deep learning models for individual tree crown segmentation from aerial imagery without manual annotation.


<details>
  <summary>Details</summary>
Motivation: Individual tree crown mapping is essential for urban tree inventories and forest health monitoring, but automatic separation in aerial imagery is challenging due to texture variations and crown overlaps.
...
Conclusion: ALS-derived pseudo-labels enhanced by SAM 2 provide a cost-effective way to obtain domain-specific training annotations for optical image-based tree crown segmentation models.
...
TL;DR: FedHENet extends FedHEONN to image classification using fixed pre-trained features and single-layer learning with homomorphic encryption, achieving competitive accuracy with better stability, energy efficiency, and no hyperparameter tuning.


<details>
  <summary>Details</summary>
Motivation: Federated Learning enables privacy-preserving collaborative training but faces challenges: expensive iterative optimization, privacy risks from shared gradients, and high energy consumption from hyperparameter tuning.

Method: FedHENet uses a fixed pre-trained feature extractor, learns only a single output layer, and aggregates client knowledge analytically in one communication round using homomorphic encryption to protect privacy.

Result: Achieves competitive accuracy compared to iterative FL baselines, superior stability performance, up to 70% better energy efficiency, and eliminates hyperparameter tuning carbon footprint.

Conclusion: FedHENet provides an efficient, privacy-preserving FL solution for image classification with minimal communication, no hyperparameter tuning, and significant energy savings while maintaining competitive accuracy.
...
TL;DR: Proposes a fine-grained MLLM-as-a-Judge framework for evaluating image editing models, decomposing evaluation into 12 interpretable factors across image preservation, edit quality, and instruction fidelity, with human-validated benchmark showing MLLM judges align closely with human evaluations.


<details>
  <summary>Details</summary>
Motivation: Traditional image editing metrics are coarse, limited in interpretability, and fail to capture important human perception aspects like controllability, edit localization, and faithfulness to user instructions. They often reward visually plausible outputs while overlooking critical evaluation dimensions.
...
Result: MLLM judges align closely with human evaluations at fine granularity, supporting their use as reliable and scalable evaluators. Traditional metrics are poor proxies for these factors, failing to distinguish over-edited or semantically imprecise outputs, while MLLM judges provide more intuitive and informative assessments.

Conclusion: The work introduces a benchmark, principled factorization, and empirical evidence positioning fine-grained MLLM judges as a practical foundation for studying, comparing, and improving image editing approaches, offering more reliable evaluation than traditional metrics.
...
TL;DR: A benchmark dataset for implicit-scale 3D reconstruction from monocular food images to advance geometry-based food portion estimation, addressing scale ambiguity in real dining scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing dietary assessment methods rely on single-image analysis or appearance-based inference (including vision-language models) which lack explicit geometric reasoning and are sensitive to scale ambiguity. Real-world food portion estimation needs geometric reasoning without explicit physical references.

Method: Reframes food portion estimation as implicit-scale 3D reconstruction problem under monocular observations. Creates benchmark dataset with multi-food scenes, diverse geometries, occlusions, and complex arrangements. Removes explicit physical references and metric annotations, instead providing contextual objects (plates, utensils) requiring algorithms to infer scale from implicit cues and prior knowledge.

Result: Benchmark adopted as challenge at MetaFood 2025 Workshop. Multiple teams proposed reconstruction-based solutions. Geometry-based reconstruction methods outperformed vision-language baselines, with top-performing approach achieving 0.21 MAPE in volume estimation and 5.7 L1 Chamfer Distance in geometric accuracy.

Conclusion: Geometry-based reconstruction methods provide both improved accuracy and greater robustness for food portion estimation compared to appearance-based methods, demonstrating the value of explicit geometric reasoning even when scale must be inferred implicitly from context.
...
TL;DR: Proposes a calibrated per-sample metric using MRI foundation model features to detect memorization/duplication in medical image generation, achieving near-perfect duplicate detection.
...
Conclusion: The proposed metric effectively addresses privacy concerns in medical image generation by reliably detecting memorization and duplication of training data.
...
TL;DR: SIEFormer uses spectral analysis to reinterpret ViT attention, enhancing feature adaptability for Generalized Category Discovery tasks through implicit and explicit spectral branches.


<details>
  <summary>Details</summary>
Motivation: To improve Vision Transformers for challenging Generalized Category Discovery tasks by leveraging spectral analysis to reinterpret attention mechanisms and enhance feature adaptability.

Method: Two-branch architecture: implicit branch uses graph Laplacians and Band-adaptive Filter layer for local structure; explicit branch uses Maneuverable Filtering Layer with Fourier transform for global dependencies.

Result: Achieves state-of-the-art performance on multiple image recognition datasets, validated through ablation studies and visualizations.

Conclusion: SIEFormer's spectral approach effectively enhances ViT for GCD tasks, demonstrating superior performance through joint optimization of implicit and explicit spectral perspectives.
...
TL;DR: A dataset folding method transforms any one-class classifier anomaly detector into a fully unsupervised method by leveraging weak assumptions about anomaly rarity and heterogeneity, enabling unsupervised anomaly detection without modifying the underlying detector.


<details>
  <summary>Details</summary>
Motivation: One-class classification for anomaly detection assumes training data contains only nominal values, making methods vulnerable to training label noise. There's a need for robust unsupervised methods that don't require clean training data.

Method: Dataset folding method that uses multiple independently trained one-class classifiers to filter training datasets for anomalies. Assumes anomalies are uncommon and heterogeneous in training data. No modifications to underlying anomaly detectors - only algorithmic selection of training data subsets.

Result: Successfully transforms various one-class classifiers into unsupervised anomaly detectors for images and videos. Creates first unsupervised logical anomaly detectors. Achieves state-of-the-art performance on MVTec AD, ViSA, and MVTec Loco AD datasets.

Conclusion: The method bridges one-class classification and unsupervised anomaly detection domains, allowing improvements in one-class classifiers to directly transfer to unsupervised settings, establishing a link between these two domains.
...
TL;DR: FEM framework uses KAN networks with Identity-Preserving diffusion models to reconstruct high-res faces from FR/PPFR embeddings, demonstrating privacy risks and evaluation capabilities.


<details>
  <summary>Details</summary>
Motivation: Limited studies exist on reconstructing realistic high-resolution faces from privacy-preserving face recognition (PPFR) embeddings, despite growing adoption of these systems. Need to verify privacy risks and evaluate system safety.

Method: Proposes Face Embedding Mapping (FEM) framework using Kolmogorov-Arnold Network (KAN) with pre-trained Identity-Preserving diffusion model to conduct embedding-to-face attacks against SOTA FR and PPFR systems.

Result: Reconstructed faces can access other real-world FR systems; method shows robustness with partial and protected embeddings; FEM serves as privacy leakage evaluation tool for FR/PPFR systems.

Conclusion: FEM demonstrates significant privacy risks in FR/PPFR systems through successful face reconstruction from embeddings, providing a valuable tool for system safety evaluation against privacy leakage.
...
TL;DR: LongStream is a novel gauge-decoupled streaming 3D reconstruction model that achieves stable, metric-scale reconstruction over kilometer-scale sequences at 18 FPS by addressing attention decay, scale drift, and extrapolation errors in long sequences.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive models for streaming 3D reconstruction fail on long sequences due to first-frame anchoring that causes attention decay, scale drift, and extrapolation errors, limiting their practical application for kilometer-scale scenes.

Method: Three key innovations: 1) Discard first-frame anchor and predict keyframe-relative poses to reformulate long-range extrapolation as local tasks; 2) Orthogonal scale learning to disentangle geometry from scale estimation and suppress drift; 3) Cache-consistent training with periodic cache refresh to address Transformer cache issues like attention-sink reliance and KV-cache contamination.

Result: State-of-the-art performance with stable, metric-scale reconstruction over kilometer-scale sequences at 18 FPS, significantly outperforming existing methods on long-sequence streaming 3D reconstruction tasks.

Conclusion: LongStream successfully addresses fundamental limitations in long-sequence 3D reconstruction by decoupling gauge constraints, enabling practical kilometer-scale scene reconstruction with real-time performance, representing a significant advancement for streaming visual geometry models.
...
TL;DR: Monocular camera with AI markerless motion capture can accurately assess upper extremity reachable workspace, with frontal view showing strong agreement with gold-standard marker-based systems.
...
Method: Nine unimpaired adults performed standardized UERW task reaching targets on virtual sphere using VR headset. Movements captured simultaneously with marker-based system and eight FLIR cameras. Monocular video analysis performed on frontal and offset camera views for comparison.

Result: Frontal camera showed strong agreement with marker-based reference (minimal bias: 0.61 ± 0.12% reachspace per octant). Offset camera underestimated percent workspace reached (-5.66 ± 0.45% reachspace). Frontal view particularly accurate for anterior workspace evaluation.

Conclusion: Frontal monocular camera configuration is feasible for UERW assessment, demonstrating clinical potential for practical single-camera assessments. First validation of monocular markerless motion capture for UERW task, enabling broader implementation of quantitative upper extremity mobility assessment.
...
TL;DR: FlexAM introduces a 3D point cloud control signal with multi-frequency and depth-aware encoding to disentangle appearance and motion for superior video generation and editing.


<details>
  <summary>Details</summary>
Motivation: Current video generation methods struggle with effective and generalizable control, often relying on ambiguous or task-specific signals. The authors argue that fundamental disentanglement of appearance and motion provides a more robust and scalable pathway for video control.

Method: FlexAM is a unified framework built on a novel 3D control signal that represents video dynamics as a point cloud. It introduces three key enhancements: 1) multi-frequency positional encoding to distinguish fine-grained motion, 2) depth-aware positional encoding, and 3) a flexible control signal for balancing precision and generative quality.
...
Conclusion: The 3D point cloud representation effectively disentangles appearance and motion, enabling a wide range of video generation and editing tasks with improved control and quality.
...
TL;DR: Introduces Conversational Image Segmentation (CIS) and ConverSeg benchmark for functional/physical reasoning, plus ConverSeg-Net model trained with AI-generated data.


<details>
  <summary>Details</summary>
Motivation: Prior referring image grounding focuses only on categorical/spatial queries, overlooking functional and physical reasoning needed for real-world conversational AI applications.

Method: Created ConverSeg benchmark spanning multiple reasoning types, developed ConverSeg-Net that fuses segmentation priors with language understanding, and built AI-powered data engine for unsupervised prompt-mask generation.

Result: Current models inadequate for CIS, but ConverSeg-Net trained on AI-generated data achieves significant gains on ConverSeg while maintaining strong performance on existing benchmarks.

Conclusion: Conversational image segmentation requires new benchmarks and models for functional/physical reasoning, and AI-generated data can effectively train models for this task.
...
TL;DR: GT-HarmBench is a benchmark of 2,009 high-stakes multi-agent scenarios testing frontier AI models' ability to choose socially beneficial actions in game-theoretic situations, revealing they succeed only 62% of the time.


<details>
  <summary>Details</summary>
Motivation: Existing AI safety benchmarks focus on single agents, leaving multi-agent risks like coordination failure and conflict poorly understood, despite frontier AI systems increasingly operating in high-stakes multi-agent environments.

Method: Created GT-HarmBench with 2,009 scenarios spanning game-theoretic structures (Prisoner's Dilemma, Stag Hunt, Chicken) drawn from realistic AI risk contexts in the MIT AI Risk Repository. Tested 15 frontier models, measured sensitivity to prompt framing/ordering, analyzed reasoning patterns, and tested game-theoretic interventions.

Result: Agents chose socially beneficial actions in only 62% of cases, frequently leading to harmful outcomes. Game-theoretic interventions improved socially beneficial outcomes by up to 18%.

Conclusion: The benchmark reveals substantial reliability gaps in frontier AI models' multi-agent alignment and provides a standardized testbed for studying alignment in multi-agent environments, with interventions showing promise for improvement.
...
TL;DR: The paper proposes a theoretical framework that reimagines AI benchmarking as a multilayer adaptive network connecting evaluation metrics, model components, and stakeholder groups, enabling dynamic, context-aware evaluation that incorporates human tradeoffs.


<details>
  <summary>Details</summary>
Motivation: Traditional benchmarking practices in AI/ML focus on shared tasks, metrics, and leaderboards but fail to account for the sociotechnical contexts where AI systems operate. There's a need for more holistic evaluation that considers multiple stakeholders and their priorities to determine meaningful model behavior.

Method: Introduces a theoretical framework conceptualizing benchmarking as a multilayer adaptive network with weighted interactions between evaluation metrics, model components, and stakeholder groups. Uses conjoint-derived utilities and human-in-the-loop update rules to embed human tradeoffs into benchmark structure, allowing dynamic evolution while preserving stability and interpretability.

Result: The framework generalizes classical leaderboards as a special case and provides tools for analyzing benchmark structural properties. It enables creation of evaluation protocols that are more context-aware and human-aligned.

Conclusion: This approach opens a path toward more accountable and human-aligned AI evaluation by making benchmarks adaptive to stakeholder priorities and sociotechnical contexts, moving beyond traditional static metrics toward dynamic, multi-stakeholder assessment frameworks.
...
TL;DR: EST introduces persistent entity states for TKG forecasting to overcome episodic amnesia in stateless methods, improving long-term dependency modeling.


<details>
  <summary>Details</summary>
Motivation: Existing TKG forecasting methods are stateless and recompute entity representations at each timestamp, causing episodic amnesia and rapid decay of long-term dependencies.

Method: Entity State Tuning (EST) maintains global state buffer with closed-loop design: topology-aware state perceiver injects entity-state priors, unified temporal context module aggregates state-enhanced events, and dual-track evolution writes updated context back to memory.

Result: EST consistently improves diverse backbones and achieves state-of-the-art performance on multiple benchmarks, demonstrating effectiveness for long-horizon TKG forecasting.

Conclusion: Persistent entity states are crucial for TKG forecasting, and EST provides an encoder-agnostic framework that effectively maintains long-term dependencies through state evolution.
...
TL;DR: LLM+KG framework translates natural language intents into executable manufacturing actions with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Smart manufacturing needs interfaces that can convert high-level human intents into machine-executable actions, especially in Manufacturing-as-a-Service (MaaS) ecosystems.

Method: Fine-tune Mistral-7B-Instruct-V02 on domain-specific data to translate natural language intents into structured JSON models, then map these to a Neo4j knowledge graph based on ISA-95 standard for manufacturing alignment.
...
Conclusion: The integrated LLM+KG framework provides a foundation for scalable, explainable, and adaptive human-machine interaction in smart manufacturing.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of efficiently creating diverse, realistic web interaction datasets for training web agents, with a particular focus on the difficulty of identifying high-quality training instances through trajectory evaluation.

Method: Introduces a novel constraint-based evaluation framework for fine-grained assessment of progress towards task completion, enabling the use of partially successful trajectories. Presents a scalable pipeline for automatically generating training data and evaluates on BookingArena benchmark with complex booking tasks across 20 popular websites.

Result: The distilled student model outperforms open-source approaches and matches or exceeds commercial systems on the BookingArena benchmark, while being a significantly smaller model. The method significantly expands the amount of usable training data by leveraging partially successful trajectories.

Conclusion: The work provides an effective solution for creating diverse web interaction datasets and offers a systematic evaluation methodology for complex structured web tasks, demonstrating that constraint-based trajectory evaluation enables more efficient training data generation for web agents.
...
<details>
  <summary>Details</summary>
Motivation: To understand how Reinforcement Learning with Verifiable Rewards (RLVR) training paradigms affect multi-domain expert-level LLM performance, as current approaches lack detailed comparison between mixed multi-task RLVR and separate RLVR with model merging.

Method: Used multiple high-level tasks (math, coding, science, instruction following) with open-source datasets, conducted extensive qualitative and quantitative experiments, and analyzed internal mechanisms through weight space geometry, model prediction behavior, and information constraints.

Result: RLVR across domains shows few mutual interferences, with reasoning-intensive domains demonstrating mutually synergistic effects. The analysis reveals insights into the mechanisms behind these mutual gains.

Conclusion: The M2RL project provides systematic comparison of RLVR training paradigms, showing that multi-domain RLVR can be effectively implemented with minimal interference and potential synergistic benefits between reasoning domains.
...
TL;DR: McDiffuSE uses Monte Carlo Tree Search to optimize slot infilling order in Masked Diffusion Models, improving reasoning performance by 3.2-8.0% over baselines.


<details>
  <summary>Details</summary>
Motivation: Plan-and-infill decoding in Masked Diffusion Models shows promise for reasoning tasks but suffers from high sensitivity to slot infilling order, causing substantial output variance.

Method: Formulates slot selection as decision making and optimizes infilling orders through Monte Carlo Tree Search with look-ahead simulations to evaluate partial completions before commitment.

Result: Average improvement of 3.2% over autoregressive baselines and 8.0% over baseline plan-and-infill, with notable gains of 19.5% on MBPP and 4.9% on MATH500.

Conclusion: MCTS-based planning effectively enhances generation quality in MDMs, with larger exploration constants (not more simulations) needed to overcome model confidence biases and discover effective orderings.
...
TL;DR: GeoAgent is a model for fine-grained address reasoning that improves on previous RL methods by using expert-annotated geographic data and novel training rewards to ensure geographic consistency.


<details>
  <summary>Details</summary>
Motivation: Previous RL-based methods for geographic reasoning rely on AI-generated chain-of-thought data and training strategies that conflict with geographic characteristics, raising concerns about their reliability and geographic accuracy.

Method: 1) Introduce GeoSeek dataset with CoT data annotated by geographic experts and professional players; 2) Propose geo-similarity reward and consistency reward assessed by a consistency agent to guide training toward geographic correctness and reasoning integrity.

Result: GeoAgent outperforms existing methods and general VLLMs across multiple grains while generating reasoning that closely aligns with human geographic understanding.

Conclusion: The approach of using expert-annotated geographic data combined with specialized training rewards enables models to reason more accurately and consistently from a geographic perspective, bridging the gap between AI reasoning and human geographic understanding.
...
<details>
  <summary>Details</summary>
Motivation: Traditional OR algorithms for inventory control are rigid and perform poorly with demand shifts or missing contextual information, while LLMs offer flexible reasoning but lack established integration methods into decision-making pipelines.

Method: Created InventoryBench with 1,000+ inventory instances using synthetic and real-world demand data to test under demand shifts, seasonality, and uncertain lead times. Conducted controlled classroom experiment with human-in-the-loop decision pipeline using LLM recommendations.
...
Conclusion: OR algorithms and LLMs are complementary rather than substitutes in inventory control. Human-AI collaboration can be beneficial despite prior findings suggesting degradation, with formal individual-level complementarity effects demonstrated.
...
TL;DR: CogRouter trains LLM agents to dynamically adjust cognitive depth per step using hierarchical cognitive levels, achieving SOTA performance with 62% fewer tokens.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents use fixed cognitive patterns (either immediate responses or uniform deep reasoning), which is inefficient for long-horizon tasks where cognitive demands vary significantly across steps.

Method: Based on ACT-R theory, designs four hierarchical cognitive levels from instinctive to strategic planning. Uses two-stage training: Cognition-aware Supervised Fine-tuning (CoSFT) for level-specific patterns, and Cognition-aware Policy Optimization (CoPO) with confidence-aware advantage reweighting for step-level credit assignment.

Result: Achieves 82.3% success rate with Qwen2.5-7B on ALFWorld and ScienceWorld, outperforming GPT-4o (+40.3%), OpenAI-o3 (+18.3%), and GRPO (+14.0%) while using 62% fewer tokens.

Conclusion: Dynamic cognitive depth adaptation based on maximizing action confidence enables more efficient and effective autonomous agents for multi-turn decision-making tasks.
...
TL;DR: A diagnostic benchmark for 2-SAT that isolates structural phenomena (not surface features) to evaluate LLM reasoning capabilities through parameterized formula families.


<details>
  <summary>Details</summary>
Motivation: Standard SAT benchmarks conflate surface difficulty (length, wording, clause order) with structural phenomena that actually determine satisfiability, making it hard to diagnose LLM reasoning failures.

Method: Introduce parameterized families of structured 2-CNF formulas with generators that isolate specific competencies: contradiction-cycle UNSAT cores, SAT instances with controlled free variables, planted backbones, late bridge clauses, and symmetry/duplication variants.
...
Conclusion: The benchmark reveals hidden failure modes in LLM reasoning by isolating structural phenomena, providing more diagnostic power than standard SAT benchmarks for evaluating reasoning capabilities.
...
TL;DR: SkillsBench benchmark shows curated Skills boost agent performance by 16.2pp on average, but self-generated Skills provide no benefit, and smaller models with Skills can match larger models without them.


<details>
  <summary>Details</summary>
Motivation: Despite rapid adoption of Agent Skills (structured procedural knowledge packages for LLM agents), there's no standard way to measure whether they actually help improve agent performance at inference time.

Method: Created SkillsBench - a benchmark of 86 tasks across 11 domains with curated Skills and deterministic verifiers. Evaluated 7 agent-model configurations over 7,308 trajectories under three conditions: no Skills, curated Skills, and self-generated Skills.
...
Conclusion: Models cannot reliably author the procedural knowledge they benefit from consuming. Smaller models with Skills can match larger models without them, showing Skills are valuable but must be carefully curated rather than self-generated.
...
TL;DR: X-SYS: A reference architecture for interactive explanation systems that treats explainability as an information systems problem, organizing around STAR quality attributes and five-component decomposition to decouple UI evolution from backend computation.


<details>
  <summary>Details</summary>
Motivation: Despite numerous XAI technical methods, deploying explainability as systems remains challenging due to requirements for maintaining usability across repeated queries, evolving models/data, and governance constraints. The paper argues that operationalizing XAI requires treating explainability as an information systems problem.
...
Result: Implements X-SYS through SemanticLens, a system for semantic search and activation steering in vision-language models. Demonstrates how contract-based service boundaries enable independent evolution, offline/online separation ensures responsiveness, and persistent state management supports traceability.

Conclusion: Provides a reusable blueprint and concrete instantiation for interactive explanation systems supporting end-to-end design under operational constraints, helping XAI researchers, developers and practitioners connect interactive explanation user interfaces with system capabilities.
...
TL;DR: WebClipper: A framework that compresses web agent search trajectories via graph-based pruning to improve search efficiency while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Current deep research web agents suffer from inefficient search patterns with long tool-call trajectories, cyclic reasoning loops, and exploration of unproductive branches, leading to poor search efficiency despite strong potential for complex information-seeking tasks.

Method: Models the agent's search process as a state graph and formulates trajectory optimization as a minimum-necessary Directed Acyclic Graph (DAG) mining problem. Uses graph-based pruning to compress trajectories while preserving essential reasoning, then continues training on refined trajectories to evolve more efficient search patterns.

Result: Reduces tool-call rounds by about 20% while improving accuracy. Introduces F-AE Score metric to measure balance between accuracy and efficiency. Experiments show WebClipper compresses tool-call rounds while maintaining excellent performance.

Conclusion: WebClipper provides practical insight into balancing effectiveness and efficiency in web agent design through trajectory compression and continued training on refined search patterns.
...
TL;DR: BrowseComp-V³ is a challenging multimodal web browsing benchmark with 300 complex questions requiring cross-modal reasoning across publicly searchable web pages, revealing significant gaps in current MLLM capabilities (only 36% accuracy).


<details>
  <summary>Details</summary>
Motivation: Existing multimodal browsing benchmarks lack task complexity, evidence accessibility, and evaluation granularity, limiting comprehensive assessment of deep search capabilities in real-world open environments.

Method: Created BrowseComp-V³ benchmark with 300 curated questions requiring multi-level, cross-modal reasoning across textual and visual evidence within/across web pages. Introduced expert-validated subgoal-driven process evaluation for fine-grained analysis. Proposed OmniSeeker framework integrating diverse web search and visual perception tools.

Result: State-of-the-art models achieve only 36% accuracy on the benchmark, revealing critical bottlenecks in multimodal information integration and fine-grained perception. The process evaluation enables systematic characterization of capability boundaries.

Conclusion: There's a fundamental gap between current MLLM capabilities and robust multimodal deep search in real-world settings. The benchmark provides a comprehensive framework for reproducible assessment and highlights areas needing improvement in multimodal reasoning.
...
TL;DR: Optimal policies in Markov decision processes reveal exactly n log m bits of information about the environment's transition dynamics.


<details>
  <summary>Details</summary>
Motivation: To understand how much internal world representation is needed for optimal behavior in AI systems, quantifying the information an optimal policy provides about the environment.

Method: Analyze Controlled Markov Processes with n states and m actions under uniform prior over transition dynamics, proving mutual information between environment and optimal deterministic policy.

Result: Optimal deterministic policy for any non-constant reward function conveys exactly n log m bits of information about the environment.

Conclusion: Provides precise information-theoretic lower bound on implicit world models needed for optimality across various reward objectives.
...
TL;DR: Reasoning models show meaningful but incomplete robustness to adversarial attacks, with distinct vulnerability patterns and failure modes. Confidence-based defenses fail due to overconfidence from reasoning traces.
...
Method: Evaluated nine frontier reasoning models under adversarial attacks, conducted trajectory analysis to identify failure modes, and tested Confidence-Aware Response Generation (CARG) defense on reasoning models.

Result: Reasoning models significantly outperform instruction-tuned baselines but show distinct vulnerabilities. Five failure modes identified (Self-Doubt, Social Conformity, Suggestion Hijacking, Emotional Susceptibility, Reasoning Fatigue) with first two accounting for 50% of failures. CARG defense fails for reasoning models due to overconfidence from extended reasoning traces.

Conclusion: Reasoning capabilities do not automatically confer adversarial robustness, and confidence-based defenses require fundamental redesign for reasoning models due to overconfidence induced by reasoning traces.
...
TL;DR: Introduces Constrained ABA (CABA) to extend Assumption-based Argumentation beyond propositional atoms by allowing constrained variables over infinite domains, with non-ground semantics that conservatively generalize standard ABA.


<details>
  <summary>Details</summary>
Motivation: Standard ABA frameworks are limited to ground (variable-free) arguments and attacks built from propositional atoms, restricting their representational power and applicability.

Method: Proposes Constrained ABA (CABA) where components and arguments may include constrained variables ranging over possibly infinite domains, and defines non-ground semantics in terms of various notions of non-ground attacks.

Result: The new CABA framework lifts the representational restriction of standard ABA, allowing more expressive argumentation with variables, while maintaining compatibility through conservative generalization.

Conclusion: CABA provides a more expressive framework for structured argumentation that extends beyond propositional atoms while preserving the core semantics of standard ABA through conservative generalization.
...
TL;DR: Hybrid obstacle avoidance architecture combining optimal control with fuzzy logic for unmanned aircraft, achieving 2-3 second computation times but encountering solver software incompatibility issues.
...
Method: Three-stage Takagi-Sugeno-Kang fuzzy layer modulates constraint radii, urgency levels, and activation decisions based on aviation regulations, integrated as soft constraints into optimal control problem solved with FALCON toolbox and IPOPT.

Result: Proof-of-concept shows optimal trajectories with 2-3 second computation times per iteration, suggesting near real-time feasibility, but reveals critical software incompatibility where Lagrangian penalty term remained zero, preventing proper constraint enforcement.

Conclusion: The hybrid architecture demonstrates potential for adaptive obstacle avoidance, but software regression issues need resolution. Future work includes validating with earlier software versions, optimizing fuzzy membership functions, and extending to higher-fidelity models.
...
TL;DR: LatentAM is an online 3D Gaussian Splatting mapping framework that builds scalable latent feature maps from RGB-D streams for open-vocabulary robotic perception, achieving near-real-time performance with plug-and-play VLM integration.


<details>
  <summary>Details</summary>
Motivation: Current methods for open-vocabulary robotic perception often rely on distilling high-dimensional VLM embeddings using model-specific decoders, which limits flexibility and requires pretraining. There's a need for a model-agnostic, pretraining-free approach that can integrate different VLMs at test time while maintaining scalability for long trajectories and large environments.

Method: LatentAM uses an online dictionary learning approach where each Gaussian primitive has a compact query vector converted to approximate VLM embeddings via attention with a learnable dictionary. The dictionary is initialized from streaming observations and optimized online with trust-region regularization. For scalability, it employs voxel hashing with GPU-based active local map optimization and CPU-based global map storage.

Result: Experiments show LatentAM achieves significantly better feature reconstruction fidelity than state-of-the-art methods while maintaining near-real-time speed (12-35 FPS) on public benchmarks and a large-scale custom dataset.

Conclusion: LatentAM provides an efficient, scalable, and flexible framework for open-vocabulary robotic perception that is model-agnostic, pretraining-free, and achieves high performance with plug-and-play VLM integration at test time.
...
TL;DR: ForeAct introduces visual foresight planning for VLA models, using imagined future observations to guide step-by-step execution, achieving 87.4% success rate on real-world tasks.


<details>
  <summary>Details</summary>
Motivation: VLA models struggle with converting high-level language instructions into executable actions in open-world environments, requiring better planning capabilities for improved accuracy and generalization.

Method: ForeAct combines an efficient foresight image generator (predicts 640×480 future observations in 0.33s) with a vision-language model that reasons over tasks and produces subtask descriptions, pretrained on 1M+ multi-task episodes.

Result: Achieves 87.4% average success rate on 11 diverse real-world tasks, with +40.9% improvement over baseline (46.5%) and +30.3% improvement over baseline with textual guidance (57.1%).

Conclusion: Visual foresight planning enables VLAs to focus on visuo-motor inference rather than high-level reasoning, leading to significant performance improvements that can be integrated seamlessly into existing VLA architectures.
...
TL;DR: Schur-MI is a faster Gaussian process mutual information formulation for robotic information gathering that reduces computational cost from O(|V|³) to O(|A|³) using precomputation and Schur complements.


<details>
  <summary>Details</summary>
Motivation: Mutual information is theoretically strong for robotic information gathering but computationally expensive for real-time planning due to repeated log-determinant evaluations.

Method: Uses iterative structure of RIG to precompute/reuse intermediate quantities across planning steps and employs Schur-complement factorization to avoid large determinant computations.

Result: Achieves up to 12.7× speedup over standard MI formulation on real-world bathymetry datasets and is validated in field trials with autonomous surface vehicles.

Conclusion: Schur-MI makes mutual information computation tractable for online planning, bridging the gap between information-theoretic objectives and real-time robotic exploration.
...
TL;DR: LongNav-R1: A multi-turn RL framework for VLA models that optimizes long-horizon navigation through continuous conversation with environment and horizon-adaptive policy optimization.


<details>
  <summary>Details</summary>
Motivation: Existing single-turn paradigms for VLA navigation models lack the ability to reason about historical interactions and sequential future outcomes, and often suffer from behavioral rigidity imposed by human demonstrations.
...
Result: Boosts Qwen3-VL-2B success rate from 64.3% to 73.0% with only 4,000 rollout trajectories, demonstrating superior sample efficiency and outperforming state-of-the-art methods. Shows strong zero-shot performance in real-world navigation.

Conclusion: LongNav-R1 effectively addresses long-horizon navigation challenges through multi-turn RL, enabling diverse behavior generation and robust performance while avoiding behavioral rigidity from human demonstrations.
...
TL;DR: Using deep learning to predict full dynamic map states from limited field-of-view time series data by converting sensor data into single-image representations.


<details>
  <summary>Details</summary>
Motivation: Autonomous systems often face limited field-of-view constraints due to design limitations, occlusions, or sensor failures, requiring the ability to infer environmental information from partial data to maintain safe operation.
...
Result: The approach achieves high accuracy in predicting map states across diverse sensing scenarios using limited FOV time series data.

Conclusion: Deep learning with single-image representations of spatiotemporal sensor data enables effective dynamic map state prediction from limited field-of-view observations.
...
TL;DR: ZLIK uses natural language descriptions of structural damage to learn kinodynamic models that achieve zero-shot adaptation to various damages with up to 81% error reduction.


<details>
  <summary>Details</summary>
Motivation: Autonomous mobile robots suffer mechanical degradation during real-world operations, causing changes in kinodynamic behavior. Structural failures are heterogeneous and challenging to quantify for kinodynamic modeling.

Method: Proposes Zero-shot Language Informed Kinodynamics (ZLIK) using self-supervised learning to ground semantic damage descriptions in kinodynamic behaviors. Collects data from structurally compromised vehicles in BeamNG.tech simulator.

Result: Achieves zero-shot adaptation to different damages with up to 81% reduction in kinodynamics error. Generalizes across sim-to-real and full-to-1/10th scale gaps.

Conclusion: Natural language can effectively describe structural damage variations, enabling data-driven learning of forward kinodynamics models that adapt to different damages without retraining.
...
TL;DR: ARMOR is a multi-task self-refinement model for robotic failure detection and reasoning that learns from sparse binary labels and small-scale rich annotations, achieving SOTA performance with up to 30% improvement in detection and 100% in reasoning.


<details>
  <summary>Details</summary>
Motivation: Real-world robotic failures are subtle, combinatorial, and difficult to enumerate, while rich reasoning labels are expensive to acquire. Prior approaches either treat failure reasoning as closed-set classification or require ample human annotations.

Method: ARMOR formulates detection and reasoning as a multi-task self-refinement process where the model iteratively predicts detection outcomes and natural language reasoning conditioned on past outputs. It learns from heterogeneous supervision (large-scale sparse binary labels + small-scale rich reasoning annotations) via offline and online imitation learning. At inference, it generates multiple refinement trajectories and selects the most confident prediction via self-certainty metric.

Result: ARMOR achieves state-of-the-art performance across diverse environments, improving over previous approaches by up to 30% on failure detection rate and up to 100% in reasoning measured through LLM fuzzy match score. It demonstrates robustness to heterogeneous supervision and open-ended reasoning beyond predefined failure modes.

Conclusion: ARMOR effectively addresses the challenge of robotic failure reasoning by combining multi-task self-refinement with heterogeneous supervision, enabling robust detection and reasoning without requiring extensive human annotations or predefined failure modes.
...
Method: MiDAS integrates electromagnetic and RGB-D hand tracking, foot pedal sensing, and surgical video capturing without requiring proprietary interfaces. Validated on Raven-II and da Vinci Xi with peg transfer and hernia repair suturing tasks performed by surgical residents.
...
TL;DR: Audio-enhanced CBF safety filter for construction robots uses jackhammer detection to dynamically adjust safety margins during navigation.


<details>
  <summary>Details</summary>
Motivation: Construction sites are dynamic and visually occluded, making robust autonomy challenging. Audio information is underutilized in current autonomy stacks despite providing valuable risk cues.

Method: Proposes a CBF-based safety filter with a lightweight real-time jackhammer detector using signal envelope and periodicity. The audio-derived risk cue modulates barrier functions in two CBF formulations (circular and goal-aligned elliptical) for a unicycle robot.

Result: CBF safety filter eliminated safety violations across all trials. Goal-aligned elliptical formulation achieved 76.5% success rate reaching target vs 40.2% for circular, as elliptical formulation better avoids deadlock.

Conclusion: Integration of audio perception into CBF-based controllers demonstrates a pathway toward richer multimodal safety reasoning for autonomous robots in safety-critical dynamic environments.
...
TL;DR: CORTEX is a real-time autonomous guidance framework for satellite rendezvous that combines deep learning perception with convex optimization for trajectory design, validated in simulation and hardware tests.


<details>
  <summary>Details</summary>
Motivation: Autonomous satellite servicing requires safe, computationally efficient close-range rendezvous that can handle uncertainty in sensing, actuation, and dynamics while being robust to sensor faults and engine failures.

Method: Integrates deep-learning perception pipeline with convex-optimization-based trajectory design and guidance, including reference regeneration and abort-to-safe-orbit logic for recovery from large deviations.

Result: In simulation Monte-Carlo tests: terminal docking errors of 36.85±44.46 mm position and 1.25±2.26 mm/s velocity. In planar air-bearing hardware tests: 8.09±5.29 mm position and 2.23±1.72 mm/s velocity errors across 18 cases (10 nominal, 8 off-nominal).

Conclusion: CORTEX successfully demonstrates autonomous, perception-enabled, real-time trajectory guidance for satellite rendezvous with robust performance in both simulation and hardware validation under various uncertainties and failure scenarios.
...
TL;DR: Real-time quadrotor dynamics prediction using partitioned Gaussian Processes with gradient information and aerodynamic effects from potential flow simulations


<details>
  <summary>Details</summary>
Motivation: Traditional GP-based approaches provide reliable Bayesian predictions with uncertainty quantification but are computationally expensive and unsuitable for real-time simulations, especially when capturing complex aerodynamic effects in quadrotor dynamics.

Method: Developed a partitioned GP with gradient conditioning: 1) Integrated gradient information via finite differences to improve accuracy, 2) Partitioned state-space into non-overlapping regions with local GPs, 3) Used Schur complements to pre-compute matrix inversions offline, 4) Generated training data using CHARM mid-fidelity aerodynamic solver to capture rotor-rotor interactions and apparent wind direction.

Result: Achieved real-time inference at frequencies above 30 Hz on standard desktop hardware, with higher accuracy than standard partitioned GPs without gradient information, while greatly reducing computational time.

Conclusion: The partitioned GP with gradient conditioning provides an efficient foundation for real-time aerodynamic prediction and control algorithms in complex and unsteady environments, enabling practical deployment of Bayesian methods for quadrotor dynamics.
...
TL;DR: Model-free RL method learns value functions for individual environment elements and composes them online via QCQP for safe navigation with formal guarantees.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots need to navigate complex dynamic environments safely in real time, but anticipating all possible behaviors is infeasible. Existing methods lack formal guarantees for safety in model-free settings.

Method: Composable model-free RL with continuous-time HJB formulation. Learns value functions and policies for individual elements (goals/obstacles) using actor-critic algorithm, then composes via quadratically constrained quadratic program (QCQP).

Result: Method provides formal obstacle-avoidance guarantees through value-function level sets, offering model-free alternative to CLF/CBF controllers. Simulations show improved performance over PPO baseline on discrete-time approximation.

Conclusion: Proposed composable model-free RL framework enables safe navigation with formal guarantees, bridging model-free learning with control-theoretic safety assurances for autonomous systems in complex environments.
...
TL;DR: Neural tactile fields predict 3D tactile responses from single RGB images, enabling robots to plan paths that avoid high-resistance objects while routing through low-resistance regions.


<details>
  <summary>Details</summary>
Motivation: Robots need to navigate deformable, yielding environments that reconfigure under contact, requiring interaction-aware 3D representations beyond static geometric occupancy.

Method: Introduce neural tactile fields - a 3D representation mapping spatial locations to expected tactile response upon contact, predicted from a single monocular RGB image.

Result: Improves volumetric 3D reconstruction by 85.8% and surface reconstruction by 26.7% compared to state-of-the-art monocular 3D reconstruction methods (LRM and Direct3D).

Conclusion: Neural tactile fields enable robots to generate intelligent paths that distinguish between different types of occupied space, rather than treating all occupied space as equally impassable.
...
TL;DR: CRAFT introduces a force-aware curriculum fine-tuning framework with variational information bottleneck to address VLA models' struggles with contact-rich manipulation by prioritizing force signals before restoring full multimodal access.


<details>
  <summary>Details</summary>
Motivation: VLA models struggle with contact-rich manipulation tasks requiring precise alignment, stable contact maintenance, and handling deformable objects due to imbalance between high-entropy vision/language inputs and low-entropy but critical force signals, leading to over-reliance on perception and unstable control.

Method: CRAFT framework integrates variational information bottleneck module to regulate vision/language embeddings during early training using curriculum strategy that prioritizes force signals first, then progressively restores full multimodal access. Uses homologous leader-follower teleoperation system to collect synchronized vision, language, and force data across diverse contact-rich tasks.
...
Conclusion: CRAFT's force-aware curriculum fine-tuning with variational information bottleneck effectively addresses VLA models' limitations in contact-rich manipulation by balancing multimodal inputs and prioritizing critical force signals, leading to improved performance and generalization.
...
TL;DR: Eva-Tracker: ESDF-free visibility-aware aerial tracking framework with FoV-ESDF for efficient occlusion avoidance and target reacquisition


<details>
  <summary>Details</summary>
Motivation: Traditional ESDF-based visibility evaluation for aerial tracking requires frequent updates that cause high computational overhead, limiting real-time performance

Method: Three key components: 1) Target trajectory prediction with visibility-aware initial path generation, 2) Precomputed FoV-ESDF for rapid visibility evaluation without updates, 3) Differentiable FoV-ESDF-based trajectory optimization for continuous visibility

Result: Extensive simulations and real-world experiments show more robust tracking with lower computational effort than state-of-the-art methods

Conclusion: Eva-Tracker eliminates ESDF update overhead while maintaining visibility-aware tracking, enabling efficient aerial tracking with target recovery capabilities
...
TL;DR: A hemispherical angular received-power mapping method for in-situ EM validation of installed mmWave modules under realistic deployment constraints.


<details>
  <summary>Details</summary>
Motivation: Characterizing angular radiation behavior of installed mmWave radar modules is important because packaging, mounting hardware, and nearby structures can significantly alter effective emission profiles, but conventional chamber-based antenna measurements become impractical once devices are embedded in host environments.

Method: Hemispherical angular received-power mapping methodology that samples accessible half-space around stationary device-under-test by placing calibrated receiving probe at prescribed (phi, theta, r) locations using geometry-consistent positioning and quasi-static acquisition, recording amplitude-only received-power with standard RF instrumentation.

Result: Proof-of-concept measurements on 60-GHz radar module demonstrate repeatable hemispherical mapping with angular trends in good agreement with full-wave simulation.

Conclusion: The approach supports practical on-site characterization of embedded mmWave transmitters, enabling in-situ EM validation under realistic deployment constraints.
...
TL;DR: PISHYAR is a socially intelligent smart cane combining social navigation with multimodal AI interaction for mobility assistance.


<details>
  <summary>Details</summary>
Motivation: To create an assistive mobility aid that goes beyond basic navigation to provide socially aware navigation and interactive assistance for visually impaired users.

Method: Two-component system: (1) Social navigation framework with Raspberry Pi 5, RGB-D perception, YOLOv8 detection, COMPOSER activity recognition, D* Lite path planning, and haptic feedback; (2) Agentic multimodal LLM-VLM interaction framework with speech recognition, vision/language models, and dynamic routing between voice/vision modes.

Result: 80% overall system accuracy in social navigation, robust group activity recognition, and positive user feedback from 8 visually impaired participants showing high acceptance, usability, trust, and perceived sociability.

Conclusion: PISHYAR demonstrates potential as a multimodal assistive mobility aid that extends beyond navigation to provide socially interactive support for visually impaired users.
...
TL;DR: RL-based sim-real co-training framework (RL-Co) improves VLA policies by combining supervised fine-tuning with reinforcement learning in simulation while anchoring with real-world data, achieving higher success rates and better generalization than real-only or SFT-based approaches.


<details>
  <summary>Details</summary>
Motivation: Current sim-real co-training methods rely on supervised fine-tuning which treats simulation as static and doesn't exploit interactive learning, limiting real-world gains and generalization. There's a need to leverage large-scale closed-loop simulation interaction while preserving real-world capabilities.

Method: Two-stage framework: 1) Warm-start policy with supervised fine-tuning on mixture of real and simulated demonstrations, 2) Fine-tune with reinforcement learning in simulation while adding auxiliary supervised loss on real-world data to prevent catastrophic forgetting and anchor the policy.

Result: Consistent improvements over real-only fine-tuning and SFT-based co-training: +24% real-world success on OpenVLA and +20% on π₀.₅. Better generalization to unseen task variations and substantially improved real-world data efficiency across four tabletop manipulation tasks.

Conclusion: RL-based sim-real co-training provides a practical and scalable pathway for leveraging simulation to enhance real-robot deployment, offering higher success rates, stronger generalization, and improved data efficiency compared to existing approaches.
...
TL;DR: Physics-constrained Real-to-Sim pipeline reconstructs physically consistent 3D scenes from single-view RGB-D data using differentiable optimization with contact graphs for robotic manipulation.


<details>
  <summary>Details</summary>
Motivation: Standard perception pipelines for 3D scene reconstruction neglect physical constraints, resulting in invalid states (floating objects, inter-penetration) that make downstream simulation unreliable for contact-rich robotic manipulation tasks.

Method: Differentiable optimization pipeline that explicitly models spatial dependencies via a contact graph, jointly refining object poses and physical properties through differentiable rigid-body simulation.

Result: Extensive evaluations in both simulation and real-world settings demonstrate reconstructed scenes achieve high physical fidelity and faithfully replicate real-world contact dynamics.
...
TL;DR: PMG is a parameterized motion generator that synthesizes human-like locomotion from compact motion data and high-dimensional control commands, enabling natural humanoid control with efficient sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: Current humanoid locomotion methods are brittle, require large datasets, are difficult to adapt to diverse tasks, and sensitive to robot-specific calibration. Need for practical, deployable humanoid control.

Method: Parameterized Motion Generator (PMG) based on human motion structure analysis, combined with imitation learning pipeline and optimization-based sim-to-real motor parameter identification.

Result: PMG produces natural human-like locomotion, responds precisely to high-dimensional control inputs (including VR teleoperation), and enables efficient verifiable sim-to-real transfer on ZERITH Z1 humanoid.

Conclusion: PMG establishes a practical, experimentally validated pathway toward natural and deployable humanoid control that addresses limitations of existing methods.
...
TL;DR: Xiaomi-Robotics-0 is a vision-language-action model optimized for real-time robot execution with state-of-the-art performance in simulation and real-world bimanual manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: To create a high-performance VLA model that can execute smoothly in real-time on robots while maintaining visual-semantic knowledge and addressing inference latency issues during deployment.

Method: Two-stage approach: 1) Pre-training on large-scale cross-embodiment robot trajectories and vision-language data to prevent catastrophic forgetting, 2) Post-training with techniques for asynchronous execution to address inference latency, plus careful timestep alignment during deployment for seamless real-time rollouts.

Result: Achieves state-of-the-art performance across all simulation benchmarks and demonstrates high success rates and throughput on real-robot bimanual manipulation tasks using consumer-grade GPUs.

Conclusion: Xiaomi-Robotics-0 successfully combines high performance with real-time execution capabilities, making it practical for real-world robotic applications, with code and models open-sourced for community use.
...
TL;DR: Robots can use navigational signs for mapless navigation by interpreting signs through vision-language models with a specialized spatial-semantic representation, achieving 88% grounding accuracy and enabling real-world robot navigation.


<details>
  <summary>Details</summary>
Motivation: Humans navigate unfamiliar environments using signs without maps, but robots struggle to interpret diverse real-world signs that need semantic grounding in 3D scenes for navigation.

Method: Proposes SignScene, a sign-centric spatial-semantic representation that captures navigation-relevant scene elements and sign information, presented to Vision-Language Models (VLMs) for effective reasoning about sign grounding.
...
Conclusion: The SignScene representation enables effective sign grounding for robots, allowing them to exploit navigational signs for mapless navigation in open-world environments, similar to how humans navigate unfamiliar spaces.
...
TL;DR: ALOE enables stable off-policy RL for real-world VLA systems by using chunking-based temporal-difference bootstrapping to evaluate action sequences instead of final outcomes.


<details>
  <summary>Details</summary>
Motivation: Current VLA systems use conservative on-policy estimation for stability, avoiding direct evaluation of high-capacity policies and limiting learning effectiveness. Off-policy evaluation from mixed data sources is needed but challenging.

Method: ALOE framework uses chunking-based temporal-difference bootstrapping to evaluate individual action sequences (action-level evaluation) rather than predicting final task outcomes, improving credit assignment under sparse rewards.
...
Conclusion: Off-policy RL can be reliably reintroduced for real-world VLA post-training through action-level evaluation, enabling more effective learning from mixed data sources including historical policies and human interventions.
...
TL;DR: Constrained PSO optimizes 6-parameter fuzzy PID for underwater vehicle depth control, balancing performance metrics with actuator constraints.


<details>
  <summary>Details</summary>
Motivation: Traditional fuzzy PID tuning relies on empirical methods, making it difficult to achieve stable equilibrium between performance enhancement and control cost in underwater vehicle depth control applications.

Method: Proposes constrained particle swarm optimization (PSO) for tuning six-parameter fuzzy PID controllers, adjusting benchmark PID parameters alongside fuzzy controller's input quantization factor and output proportional gain.

Result: Significant performance improvements: time-weighted absolute error integral decreased from 0.2631 to 0.1473, settling time shortened from 2.301s to 1.613s, relative overshoot reduced from 0.1494 to 0.01839, while maintaining control energy constraints.

Conclusion: The constrained six-parameter joint tuning strategy effectively enhances depth control performance for underwater vehicles while satisfying engineering constraints, validating its effectiveness and engineering significance.
...
TL;DR: TRANS is a DRL framework for quadrupedal social navigation over unstructured terrains using a two-stage training approach with three pipelines for locomotion, navigation, and their integration.


<details>
  <summary>Details</summary>
Motivation: Existing quadrupedal navigation methods either separate motion planning from locomotion (neglecting whole-body constraints and terrain awareness) or use end-to-end approaches requiring high-frequency sensing that's noisy and computationally costly. Most also assume static environments, limiting use in human-populated settings.

Method: Two-stage training framework with three DRL pipelines: (1) TRANS-Loco uses asymmetric actor-critic model for quadrupedal locomotion without explicit terrain/contact observations; (2) TRANS-Nav uses symmetric AC framework for social navigation mapping LiDAR to actions; (3) TRANS integrates both for terrain-aware navigation in uneven and socially interactive environments.

Result: Comprehensive benchmarks against locomotion and social navigation baselines demonstrate TRANS's effectiveness. Hardware experiments confirm its potential for sim-to-real transfer.

Conclusion: TRANS addresses limitations of conventional quadrupedal navigation by providing an integrated, terrain-aware framework for social navigation in unstructured environments with demonstrated effectiveness and sim-to-real transfer potential.
...
TL;DR: Real2Gen enables training robot manipulation policies from a single human demonstration by transferring it to simulation for unlimited synthetic data generation via flow matching.


<details>
  <summary>Details</summary>
Motivation: Collecting robot demonstrations is tedious and time-consuming, while human demonstrations are abundant but difficult to transfer to robots. There's a need to efficiently leverage human demonstrations for robot policy learning.

Method: Extracts information from a single human demonstration, transfers it to simulation, uses a programmable expert agent to generate unlimited synthetic demonstrations, and trains a flow matching policy on this abundant data.

Result: Achieves 26.6% average increase in success rate over baseline, shows better generalization due to diverse training data, and enables zero-shot real-world deployment of simulation-trained policies.

Conclusion: Real2Gen successfully bridges the sim-to-real gap by leveraging human demonstrations to generate unlimited synthetic data, enabling efficient robot policy training with strong real-world performance.
...
TL;DR: SafeFlowMPC combines flow matching with online optimization to create safe, real-time robot policies that generalize from demonstrations while providing rigorous safety guarantees.


<details>
  <summary>Details</summary>
Motivation: Robots in everyday life need flexibility and real-time reactivity. Learning methods (like imitation learning) provide generalization but lack interpretability and safety guarantees. Optimization methods provide safety guarantees but lack flexibility and generalization capabilities.

Method: SafeFlowMPC combines flow matching (a learning-based approach for trajectory generation) with online optimization using a suboptimal model-predictive control formulation. This hybrid approach maintains safety at all times while enabling real-time execution.

Result: Achieved strong performance in three real-world experiments on a KUKA 7-DoF manipulator: two grasping experiments and a dynamic human-robot object handover experiment.

Conclusion: SafeFlowMPC successfully bridges the gap between learning-based and optimization-based methods, providing both generalization capabilities from demonstrations and rigorous safety guarantees for real-world robot applications.
...
TL;DR: Proposes a local-global behavioral management approach for autonomous soaring-capable UAVs to extend surveillance persistence by extracting energy from rising air, with cooperative agents, collision avoidance, and optimized path tracking.


<details>
  <summary>Details</summary>
Motivation: Extend UAV surveillance persistence by overcoming limited on-board power through energy extraction from rising buoyant air (soaring), enabling longer aloft times without landing.

Method: Local-global behavioral management with non-deterministic finite state-based rational agents, mission planning with dynamic waypoints, visibility/prediction-based collision avoidance, and delayed learning for path tracking controller optimization.

Result: Maintains surveillance persistency (staying aloft longer), detects targets 2x better than non/semi-cooperative approaches, with only 6% battery consumption in 6 hours, outperforming 15 evolutionary algorithms and 3 baselines.

Conclusion: The proposed approach effectively enables autonomous soaring-capable UAVs to achieve extended surveillance persistence through cooperative energy harvesting from atmospheric thermals, significantly improving target detection while minimizing power consumption.
...
TL;DR: Robotic system combines high-resolution force sensing (Minsight) and high-frequency vibration sensing (Minsound) for fabric classification, achieving 97% accuracy on 20 fabrics using transformer-based method.


<details>
  <summary>Details</summary>
Motivation: Human tactile perception integrates both spatial force patterns and texture-induced vibrations, but robotic tactile sensors struggle to achieve both high spatial resolution and high temporal sampling rate simultaneously.

Method: Uses robotic hand with two soft tactile sensors: Minsight (camera-based, 50Hz force sensing) and Minsound (MEMS microphone, 50Hz-15kHz vibration sensing). Robot actively encloses and rubs fabric samples between fingers, inspired by human exploration movements. Employs transformer-based classification method.

Result: Achieves 97% fabric classification accuracy on 20 common fabrics. Audio-based sensor shows high utility for classification. External microphone improves robustness in loud ambient noise. System learns general representations of fabric properties (stretchiness, thickness, roughness) beyond training data.

Conclusion: Combining high-resolution force sensing with high-frequency vibration sensing enables robust robotic tactile perception of fabrics, with audio-based sensing proving particularly valuable for classification tasks.
...
TL;DR: INHerit-SG is a semantic scene graph system for robot navigation that uses explicit natural-language anchors and asynchronous processing to create interpretable, RAG-ready knowledge bases that better align with human intent.


<details>
  <summary>Details</summary>
Motivation: Existing semantic scene graph approaches for robot navigation are misaligned with embodied task needs - they rely on offline batch processing or implicit feature embeddings, making them poor for interpretable human-intent reasoning in complex environments.

Method: Redefines maps as structured, RAG-ready knowledge bases with natural-language descriptions as explicit semantic anchors. Uses asynchronous dual-process architecture with Floor-Room-Area-Object hierarchy to decouple geometric segmentation from semantic reasoning. Implements event-triggered map updates and employs multi-role LLMs for query decomposition with hard-to-soft filtering strategy.

Result: Achieves state-of-the-art performance on complex queries in HM3DSem-SQR dataset and real-world environments. Demonstrates scalability for downstream navigation tasks with improved success rate and reliability of complex retrievals.

Conclusion: INHerit-SG's explicit interpretability enables adaptation to broader spectrum of human interaction tasks while maintaining long-term consistency with low computational overhead, making it suitable for practical robot navigation applications.
...
TL;DR: Legato is a training-time continuation method for action-chunked VLA policies that improves trajectory smoothness and reduces multimodal switching by initializing denoising from schedule-shaped action-noise mixtures and reshaping flow dynamics.


<details>
  <summary>Details</summary>
Motivation: Existing real-time chunking (RTC) methods for VLA models are external to the policy, causing spurious multimodal switching and non-smooth trajectories at chunk boundaries during execution.

Method: Legato uses three key techniques: 1) initializes denoising from schedule-shaped mixture of known actions and noise to expose partial action information, 2) reshapes learned flow dynamics for consistency between training/inference under per-step guidance, and 3) uses randomized schedule conditions during training to handle varying inference delays and achieve controllable smoothness.

Result: Legato produces smoother trajectories, reduces spurious multimodal switching, leads to less hesitation and shorter task completion time. Extensive real-world experiments show ~10% improvements in both trajectory smoothness and task completion time across five manipulation tasks compared to RTC.

Conclusion: Legato effectively addresses chunk boundary discontinuities in VLA models through training-time continuation, outperforming external RTC methods by making smoothness intrinsic to the policy rather than an external fix.
...
TL;DR: Investigates feature sets and similarity measures for quantifying swarm behavior similarity, showing some combinations better distinguish behaviors and proposing a self-organizing map approach to identify ambiguous regions.


<details>
  <summary>Details</summary>
Motivation: Swarm behavior analysis relies on numerical features for classification/imitation learning, but existing ad-hoc feature sets lack robustness across contexts. Automatic swarm behavior design requires reliable quantitative similarity measurement.

Method: Selects existing swarm feature sets and similarity measures from prior works, assesses their robustness, analyzes feature-similarity measure interplay, and proposes self-organizing map approach to identify ambiguous feature space regions.

Result: Demonstrates that specific combinations of feature sets and similarity measures are more effective at distinguishing similar behavior groups. Shows some combinations are more suitable for behavior discrimination.

Conclusion: The interplay between feature sets and similarity measures significantly impacts swarm behavior discrimination. Proposed self-organizing map approach helps identify where behaviors cannot be easily distinguished in feature space.
...
TL;DR: Learning-based control for soft quadruped robots using tactile suction-cup feet enables robust closed-loop locomotion on flat and inclined surfaces.
...
Method: A learning-based control framework with tactile suction-cup feet, trained in simulation through staged learning from a reference gait under randomized environmental conditions, mapping proprioceptive and tactile feedback to pneumatic actuation and suction-cup commands.

Result: Closed-loop policy outperforms open-loop baseline by 41% faster on flat surfaces and 91% faster on 5-degree inclines; tactile force estimates and inertial feedback improve performance by up to 56% compared to configurations without sensory feedback.

Conclusion: The learning-based control framework with tactile feedback enables robust closed-loop locomotion for soft quadruped robots, significantly improving performance on various terrains compared to open-loop approaches.
...
TL;DR: LLM-based agentic control system for robots shows flexibility across domains but suffers from fragility and non-deterministic behavior.


<details>
  <summary>Details</summary>
Motivation: To leverage generative models' commonsense reasoning for robot control in real-world settings with uncertainty, partial observability, and natural language commands.

Method: Agentic control system with reasoning-capable LLM that plans and executes tasks via iterative planner-executor loop, selecting robot skills, with structured introspection, event checks, and operator intervention support.

Result: System deployed on two platforms (Mobipick for indoor manipulation, Valdemar for agricultural navigation) showed substantial fragility including non-deterministic behavior, instruction errors, and prompt sensitivity, but demonstrated flexibility with domain transfer requiring only prompt updates and API binding.

Conclusion: LLM-based robot control offers promising flexibility for cross-domain transfer but requires addressing fragility issues like non-determinism and prompt sensitivity for reliable deployment.
...
TL;DR: UniManip is a robotic manipulation framework using a Bi-level Agentic Operational Graph to unify semantic reasoning with physical grounding, enabling robust zero-shot generalization on unseen objects and tasks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle with zero-shot generalization: end-to-end Vision-Language-Action models lack precision for long-horizon tasks, while traditional hierarchical planners are too rigid for open-world variations. There's a need to bridge high-level semantic intent with low-level physical interaction in unstructured environments.
...
Result: The system achieves 22.5% and 25.0% higher success rates compared to state-of-the-art VLA and hierarchical baselines, respectively. It demonstrates robust zero-shot capability on unseen objects and tasks, and enables direct zero-shot transfer from fixed-base setups to mobile manipulation without fine-tuning.

Conclusion: UniManip successfully bridges semantic reasoning with physical grounding through its bi-level agentic architecture, enabling robust zero-shot generalization in unstructured environments. The framework represents a significant advancement toward general-purpose robotic manipulation by dynamically aligning abstract planning with geometric constraints.
...
TL;DR: TSEASL is a regional planner arbitration architecture that improves navigation safety in partially observable off-road environments by considering optimized versions of previous trajectories alongside current ones, reducing unsafe behavior from rapidly changing guidance.


<details>
  <summary>Details</summary>
Motivation: Traditional navigation architectures in partially observable off-road environments suffer from rapidly changing regional plan guidance due to continuous map updates, causing unsafe navigation behavior that requires manual safety interventions during autonomous traversals.

Method: Proposed Temporally-Sampled Efficiently Adaptive State Lattices (TSEASL), a regional planner arbitration architecture that considers updated and optimized versions of previously generated trajectories against the currently generated trajectory to provide more stable guidance.

Result: Testing on a Clearpath Robotics Warthog UGV with real map data showed that TSEASL eliminated manual interventions in locations where the baseline planner required them, and recorded higher levels of planner stability compared to the baseline.

Conclusion: TSEASL successfully addresses the problem of rapidly changing guidance in partially observable off-road environments, improving navigation safety and stability, with potential for further improvements to make it more generalizable to various off-road autonomy scenarios.
...
TL;DR: Soft robotics combined with EEG brain signals (alpha waves) creates emotion-responsive art installations - a soft character and flower that move based on human emotional states.


<details>
  <summary>Details</summary>
Motivation: To explore the intersection of human emotions, soft robotics, and art to create new forms of human emotion-mediated soft robotic art, leveraging the flexibility and lifelike movement of soft robots for immersive emotional expression.

Method: Developed two soft embodiments (character and flower) that respond to EEG brain signals; measured human emotions via alpha waves from EEG, mapped alpha wave patterns to dynamic movements of soft robots, and conducted experimental demonstrations.

Result: Successfully created emotion-responsive soft robotic art displays that dynamically respond to brain signals, demonstrating how soft robotics can embody human emotional states through movement.

Conclusion: Soft robotics offers a new medium for artistic expression by embodying human emotions, enabling insightful interactive art displays that respond to emotional states.
...
TL;DR: Steerable Policies improve robot control by training vision-language-action models on rich synthetic commands at multiple abstraction levels, enabling better grounding of pretrained vision-language model knowledge in low-level behaviors.


<details>
  <summary>Details</summary>
Motivation: Current hierarchical approaches using VLMs for high-level reasoning and VLAs for low-level execution are limited by natural language interfaces, restricting how much VLM reasoning can steer low-level robot behavior.

Method: Train VLAs on synthetic commands at various abstraction levels (subtasks, motions, grounded pixel coordinates) to create Steerable Policies that improve low-level controllability and enable better utilization of VLM knowledge.

Result: Steerable Policies outperform prior embodied reasoning VLAs and VLM-based hierarchical baselines in real-world manipulation experiments, including challenging generalization and long-horizon tasks.

Conclusion: By training VLAs on multi-level synthetic commands, Steerable Policies unlock pretrained VLM knowledge for improved robot task generalization and control, bridging the gap between high-level reasoning and low-level execution.
...
TL;DR: PSI framework trains modular robot manipulation policies using human video data with simulation-based grasp filtering, enabling task-compatible grasping without robot data.


<details>
  <summary>Details</summary>
Motivation: Human videos provide scalable data for robot learning but are insufficient for teaching grasping behaviors to robots with non-human hands, especially for task-compatible grasps.

Method: Perceive-Simulate-Imitate (PSI) framework processes human video motion data through paired grasp-trajectory filtering in simulation to generate grasp suitability labels for supervised learning of task-oriented grasping.

Result: Real-world experiments show PSI enables efficient learning of precise manipulation skills without robot data, achieving significantly more robust performance than naive grasp generator approaches.

Conclusion: PSI successfully bridges the gap between human video data and robot manipulation by using simulation to generate task-compatible grasp labels, enabling effective modular policy training.
