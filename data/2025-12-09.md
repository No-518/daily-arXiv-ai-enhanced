...
TL;DR: Video generation models like Sora-2 can now reason with 60% success on cognitive tasks (chess, Sudoku, Raven's Matrices) using a scalable "Task Pair" evaluation paradigm.


<details>
  <summary>Details</summary>
Motivation: To establish whether video generation models can perform reasoning tasks, and to create a scalable evaluation framework for assessing reasoning capabilities in video models.

Method: Developed a "Task Pair" experimental paradigm and built VMEvalKit code framework supporting 39 models. Used automated evaluation validated against human judgment on tasks including chess, maze, Sudoku, mental rotation, and Raven's Matrices.

Result: Leading models like Sora-2 achieve approximately 60% success rates on reasoning tasks. Automated evaluation strongly correlates with human judgment, demonstrating the paradigm's scalability and reliability.

Conclusion: Video generation models can now perform reasoning tasks, and the established evaluation paradigm enables scalable assessment and potential reinforcement learning for improving reasoning in video models.
...
TL;DR: Proposes dataset quantization to reduce intra-sample redundancy for edge devices, using adaptive quantization allocation to maintain training performance while achieving significant compression.
...
Method: Uses linear symmetric quantization for initial range/scale, then adaptive quantization allocation algorithm to distribute different quantization ratios per sample while maintaining constant total compression ratio.
...
Conclusion: First method to use limited bits for dataset representation, introduces dataset-level quantization with adaptive allocation, and effectively reduces storage/communication costs for edge devices while preserving essential features.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene reconstruction from multi-view images struggle with multi-view fusion, leading to fragmented 3D representations and sub-optimal performance. The view-by-view processing approach causes inconsistency and fragmentation in the resulting 3D representations.

Method: VG3T introduces a novel multi-view feed-forward network that directly predicts semantically attributed 3D Gaussians in a joint, multi-view fashion. It includes two key components: Grid-Based Sampling to mitigate distance-dependent density bias, and Positional Refinement for improved accuracy.

Result: VG3T achieves a 1.7%p improvement in mIoU while using 46% fewer primitives than the previous state-of-the-art on the nuScenes benchmark, demonstrating superior efficiency and performance.

Conclusion: VG3T provides a unified paradigm for representing both geometry and semantics in 3D scenes, overcoming fragmentation and inconsistency issues of previous view-by-view methods through joint multi-view processing.
...
TL;DR: EmoDiffTalk: A novel 3D Gaussian Splatting talking head framework with emotion-aware Gaussian diffusion for fine-grained facial animation and text-to-AU emotional editing.


<details>
  <summary>Details</summary>
Motivation: Existing 3D talking head methods using Gaussian Splatting lack effective emotional expression manipulation, particularly for fine-grained and expansive dynamic emotional editing with multi-modal control.

Method: Introduces Emotion-aware Gaussian Diffusion with: 1) Action Unit (AU) prompt Gaussian diffusion process for fine-grained facial animation, and 2) Accurate text-to-AU emotion controller for dynamic emotional editing using text input.

Result: Superior performance on EmoTalk3D and RenderMe-360 datasets in emotional subtlety, lip-sync fidelity, and controllability compared to previous works.
...
TL;DR: NeuroFM is a domain-specific foundation model trained on brain tissue histology that outperforms general-purpose pathology models on neuropathology tasks like dementia classification and neurodegenerative disease identification.


<details>
  <summary>Details</summary>
Motivation: Existing pathology foundation models are trained predominantly on surgical pathology data that overrepresents non-neurological diseases, creating a domain mismatch for neuropathology which has unique cell types, cytoarchitecture, and disease-specific features like neurofibrillary tangles and amyloid plaques.

Method: Developed NeuroFM, a foundation model specifically trained on whole-slide images of brain tissue spanning diverse neurodegenerative pathologies, addressing the domain gap between general surgical pathology and neuropathology.
...
TL;DR: FishDetector-R1 is a unified MLLM-based framework for fish detection, segmentation, and counting using weak supervision, achieving significant improvements on underwater fish imagery with novel detection-to-count prompts and reinforcement learning.
...
Method: A unified MLLM-based framework with two key components: 1) a novel detect-to-count prompt enforcing spatially consistent detections and counts, and 2) Reinforcement Learning from Verifiable Reward (RLVR) leveraging sparse point labels in a scalable paradigm.
...
TL;DR: Capsule Networks can be pruned by removing 95% of Primary Capsules, achieving up to 9.9x speedup and 95.36% reduction in FLOPs without accuracy loss across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: CapsNets offer advantages over CNNs (better robustness to affine transformations, overlapping image detection) but suffer from resource inefficiency due to high number of Primary Capsules, slow training/testing, and high computational demands.

Method: Investigates Primary Capsules pruning in CapsNets across MNIST, Fashion-MNIST, CIFAR-10, and SVHN datasets, removing up to 95% of capsules while maintaining performance.

Result: Pruned CapsNet performs up to 9.90 times faster than conventional architecture, saves over 95.36% of FLOPs in dynamic routing stage, and maintains accuracy. Provides insights on why some datasets benefit more from pruning than others.

Conclusion: Primary Capsules pruning enables significant computational efficiency gains in CapsNets without sacrificing accuracy, making them more practical for real-world applications while maintaining their advantages over CNNs.
...
TL;DR: AI agents can automate code adaptation for scientific computer vision tools, outperforming human experts with simpler architectures.


<details>
  <summary>Details</summary>
Motivation: Adapting production computer vision tools to scientific datasets is a bottleneck - fine-tuning needs large annotated data scientists lack, while manual coding takes weeks/months.

Method: Introduced systematic evaluation framework for agentic code optimization, studied three production biomedical imaging pipelines with different agent architectures.

Result: Simple agent framework consistently generates adaptation code that outperforms human-expert solutions; complex agent architectures not universally beneficial.

Conclusion: Provides practical roadmap for agent design, open-sourced framework, and demonstrated real-world deployment pathway with production pipeline validation.
...
TL;DR: First real-time certifiably robust semantic segmentation using Lipschitz networks, 600x faster than randomized smoothing with comparable certificates.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial robustness methods focus on classification tasks, with few efficient certification procedures for semantic segmentation. Current approaches are computationally expensive and not suitable for real-time applications.

Method: Introduce certifiably robust semantic segmentation networks with built-in Lipschitz constraints that are efficiently trainable. Provide a novel framework generalizing robustness certificates for segmentation tasks using Lipschitz networks.

Result: Achieve competitive pixel accuracy on Cityscapes dataset. Enable real-time compatible certification (600x faster than randomized smoothing on A100 GPU). Allow computation of worst-case performance under ℓ₂ attacks across various metrics.

Conclusion: Unlocks real-time certifiably robust semantic segmentation for the first time, with flexible and computationally efficient certification framework that validates against state-of-the-art attacks.
...
TL;DR: Automated ML framework using high-throughput imaging and clustering to analyze metallic powder morphology for SLM additive manufacturing, identifying Fourier-descriptor + k-means as optimal pipeline.
...
Method: Developed three clustering pipelines (autoencoder, shape-descriptor, functional-data) applied to ~126,000 powder images, using high-throughput imaging with shape extraction and unsupervised learning.

Result: Fourier-descriptor + k-means pipeline identified as most effective with lowest Davies-Bouldin index, highest Calinski-Harabasz score, and sub-millisecond runtime per particle.

Conclusion: Unsupervised learning framework enables rapid automated powder morphology assessment, supports tracking shape evolution across reuse cycles, and provides basis for future studies linking morphology to SLM part quality.
...
TL;DR: Vision Action Transformer (VAT) leverages all transformer layers instead of just the final layer for better visual-action fusion, achieving state-of-the-art 98.15% success rate on LIBERO benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current Vision Transformer methods in robot learning discard valuable information by using only the final layer's features, providing insufficient representation for robotic tasks.

Method: Proposes VAT architecture that extends ViT by processing specialized action tokens with visual features across all transformer layers, enabling deep progressive fusion of perception and action generation.
...
Conclusion: VAT demonstrates the critical importance of leveraging the complete "representation trajectory" of vision models to advance robotic policy, presenting a powerful model for imitation learning.
...
<details>
  <summary>Details</summary>
Motivation: Despite strong performance of foundation models in medical imaging, there's limited understanding of their comparative behavior across different datasets, necessitating standardized evaluation and reproducible baselines.

Method: Used unified preprocessing pipeline and fixed downstream classifiers; extracted embeddings from pre-trained encoders, trained LightGBM classifiers on multiple disease labels, and reported AUROC and F1-scores with confidence intervals.

Result: MedImageInsight achieved slightly higher performance across most tasks, while CXR-Foundation showed better cross-dataset stability; unsupervised clustering revealed coherent disease-specific structure in MedImageInsight embeddings.
...
Method: Train MLLMs with preference-oriented VQA tasks, use inter-user and intra-user discrimination probing tasks to isolate preference features, design MMD-based alignment loss to bridge modality gap, and condition diffusion generators with resulting embeddings.
...
Conclusion: The proposed multimodal framework effectively captures and injects user preferences into diffusion models, enabling faithful adherence to both prompts and personal aesthetic choices.
...
<details>
  <summary>Details</summary>
Motivation: Need for precise 3D reconstruction of ocean wave surfaces and velocity fields to understand ocean physics, addressing computational costs of dense visual reconstruction and persistent visual occlusion challenges in long-term ocean observation.

Method: Propose a wave free surface visual reconstruction neural network with attention-augmented pyramid architecture tailored to multi-scale and temporally continuous wave motions. Uses physics-based constraints for time-resolved reconstruction of nonlinear 3D velocity fields from evolving free-surface boundary.
...
Conclusion: The proposed neural network enables efficient, accurate 3D wave reconstruction with strong generalization capabilities, addressing computational and occlusion challenges in ocean wave observation through attention mechanisms and learned wave propagation dynamics.
...
TL;DR: SAM3 represents a fundamental paradigm shift from SAM2's prompt-based geometric segmentation to a multimodal concept-driven architecture with open-vocabulary reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: To investigate and explain the fundamental discontinuity between SAM2 and SAM3, highlighting why expertise in SAM2's prompt-based segmentation doesn't transfer to SAM3's new multimodal concept-driven paradigm.
...
Result: Establishes SAM3 as a new class of segmentation foundation model that fundamentally differs from SAM2, with capabilities for open-vocabulary reasoning, semantic grounding, contrastive alignment, and exemplar-based concept understanding.

Conclusion: SAM3 represents a paradigm shift to concept-driven segmentation, charting future directions for the emerging era of multimodal, semantically-aware segmentation models.
...
TL;DR: This dissertation develops methods for 3D point cloud understanding by integrating 2D knowledge through supervised, self-supervised, and transfer learning approaches.


<details>
  <summary>Details</summary>
Motivation: 3D data provides rich geometric information that, when combined with 2D images, enables comprehensive environmental understanding for applications like autonomous driving, robotics, and medical treatment. However, effectively leveraging 2D knowledge for 3D understanding remains challenging.

Method: Three main approaches: 1) Supervised representation learning for point cloud primitive segmentation, 2) Self-supervised learning methods for 3D data, and 3) Transfer learning from 2D to 3D that integrates pre-trained 2D models to support 3D network training without simply transforming 2D data.

Result: Extensive experiments validate the effectiveness of the methods, showing significant improvements in 3D understanding by effectively integrating 2D knowledge.

Conclusion: The dissertation advances point cloud representation learning by demonstrating how to effectively integrate 2D knowledge into 3D understanding frameworks, with potential applications across various fields including autonomous systems and medical imaging.
...
TL;DR: EgoEdit: A real-time instruction-guided video editing system for egocentric footage that handles rapid motion and hand-object interactions, with new dataset and benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing AI video editors work well on third-person footage but struggle with egocentric videos due to rapid egomotion and frequent hand-object interactions, creating a domain gap. Current offline editing pipelines also have high latency, limiting real-time AR applications.
...
Conclusion: The complete ecosystem addresses the domain gap in egocentric video editing, enabling real-time interactive AR applications. The dataset and benchmark will be made public to advance research in this area.
...
TL;DR: Single-photon lidar captures multi-bounce light to reconstruct occluded geometry and specular surfaces from single measurements using data-driven light transport inversion.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D reconstruction struggles with occlusions and specular materials like mirrors. Single-photon lidars can capture multi-bounce light containing valuable information about hidden geometry and material properties, but existing methods only work with sequential single-point illumination, not practical multiplexed illumination scenarios.

Method: Proposed a data-driven approach to invert complex light transport in single-photon lidar with multiplexed illumination. Created first large-scale simulated dataset (~100k lidar transients) for indoor scenes. Learned a prior on complex light transport to decompose measured two-bounce light into constituent contributions from each laser spot.

Result: Successfully demonstrated experimental reconstruction of 3D geometry in scenes with occlusions and mirrors from single measurements. The decomposed light enables inference of hidden geometry and material properties that would be impossible with traditional methods.

Conclusion: Data-driven inversion of light transport in single-photon lidar enables practical 3D reconstruction of challenging scenes with occlusions and specular materials using multiplexed illumination, overcoming limitations of analytical approaches and sequential illumination methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLMs/MLLMs in autonomous driving either use single-view encoders that miss spatial structure or aggregated multi-view features lacking unified spatial representation, making spatial reasoning about ego-centric directions and object relations challenging.
...
Result: BeLLA consistently outperforms existing approaches on spatial reasoning questions (relative object positioning, behavioral understanding) by up to +9.3% absolute improvement on NuScenes-QA and DriveLM benchmarks, while performing competitively on other question categories.

Conclusion: The unified BEV representation combined with LLMs enables superior spatial reasoning capabilities for autonomous driving QA, handling diverse question types while excelling at spatial understanding tasks.
...
TL;DR: SpectraIrisPAD: A novel deep learning framework using DINOv2 Vision Transformer with spectral positional encoding for robust multispectral iris presentation attack detection, achieving state-of-the-art performance on a new comprehensive dataset.


<details>
  <summary>Details</summary>
Motivation: Iris recognition systems are vulnerable to presentation attacks (PAs), and existing near-infrared (NIR) systems need enhanced detection capabilities. Multispectral imaging across multiple NIR bands provides complementary reflectance information that can improve PAD generalizability against diverse spoofing attacks.

Method: SpectraIrisPAD uses a DINOv2 Vision Transformer backbone with learnable spectral positional encoding, token fusion, and contrastive learning to extract discriminative band-specific features. The framework is evaluated on a new MSIrPAD dataset containing 18,848 iris images across 8 PAI categories captured at 5 NIR wavelengths.

Result: SpectraIrisPAD consistently outperforms state-of-the-art baselines across all performance metrics in unseen attack evaluation protocols, demonstrating superior robustness and generalizability in detecting diverse presentation attacks.

Conclusion: The proposed multispectral approach with spectral-aware deep learning provides an effective solution for robust iris presentation attack detection, addressing security concerns in real-world iris biometric systems through enhanced generalization capabilities against various spoofing artifacts.
...
TL;DR: CEFM is a cross-modal explainable framework for melanoma diagnosis that uses contrastive learning to align clinical ABC criteria with visual features in Vision Transformers, generating textual explanations to improve interpretability and clinical trust.
...
Result: Achieves 92.79% accuracy and 0.961 AUC on public datasets, with significant improvements across multiple interpretability metrics; qualitative analysis shows learned embeddings align with clinicians' ABC rule application.

Conclusion: CEFM effectively bridges the gap between high-performance melanoma classification and clinical trust by creating transparent links between image data and clinical interpretation through cross-modal explainability.
...
TL;DR: Track4DGen is a two-stage framework that uses point tracking priors and hybrid 4D Gaussian Splatting to generate temporally consistent 4D objects from sparse inputs.


<details>
  <summary>Details</summary>
Motivation: Generating dynamic 4D objects from sparse inputs is challenging due to difficulties in preserving appearance and motion coherence across views and time while suppressing artifacts and temporal drift. Current methods suffer from view discrepancies because they rely on pixel- or latent-space video-diffusion losses that lack explicit temporally aware, feature-level tracking guidance.

Method: Two-stage framework: 1) Couples multi-view video diffusion model with foundation point tracker to enforce dense feature-level point correspondences, producing temporally consistent features. 2) Reconstructs dynamic 4D Gaussian Splatting using hybrid motion encoding that concatenates diffusion features (with tracking priors) with Hex-plane features, augmented with 4D Spherical Harmonics for higher-fidelity dynamics modeling.
...
Conclusion: Explicit injection of tracker-derived motion priors into intermediate feature representations for both multi-view video generation and 4D-GS reconstruction effectively addresses temporal consistency issues, enabling high-quality 4D object generation from sparse inputs.
...
TL;DR: Automated workflow using deep learning generates defect annotations from shearography measurements, enabling weakly supervised training and scalable dataset creation for industrial defect detection.


<details>
  <summary>Details</summary>
Motivation: Shearography is sensitive for detecting subsurface defects but lacks high-quality annotated datasets due to labor-intensive manual labeling that is subjective and difficult to standardize, limiting industrial adoption.

Method: Introduces an automated workflow that uses deep learning to generate defect annotations (high-resolution segmentation and bounding-box labels) directly from shearography measurements.

Result: Evaluation against expert-labeled data demonstrates sufficient accuracy to enable weakly supervised training, reducing manual effort while maintaining quality.

Conclusion: The automated annotation workflow supports scalable dataset creation for robust defect detection in industrial applications, addressing a key limitation to shearography adoption.
...
TL;DR: A novel shadow generation framework that combines explicit physical modeling (geometry & illumination) with diffusion-based refinement to produce photorealistic, physically coherent shadows.


<details>
  <summary>Details</summary>
Motivation: Current deep-learning-based shadow generation methods rarely incorporate explicit physical modeling of shadow formation, which involves occluders blocking light rays from sources. This leads to shadows that may lack physical consistency with scene geometry and illumination.

Method: 1) From monocular RGB images, extract approximate 3D geometry as dense point maps and predict dominant light direction. 2) Use physics of shadow formation to compute initial shadow location/shape. 3) Integrate this physics-based estimate into diffusion framework for realistic appearance refinement while maintaining geometry/illumination consistency.
...
Conclusion: Explicitly embedding physical modeling (geometry and illumination) into deep learning frameworks enables generation of shadows that are both photorealistic and physically accurate, addressing limitations of purely data-driven approaches.
...
TL;DR: Framework for joint detection of cast and attached shadows using iterative geometry-illumination reasoning, with new dataset and 33% BER improvement for attached shadows.
...
Method: Closed-loop system with shadow detection module (predicts both shadow types) and light estimation module (infers light direction). Uses estimated light direction + surface normals to create geometry-consistent partial map of self-occluded regions, which refines shadow predictions iteratively.

Result: Created new dataset of 1,458 images with separate cast/attached shadow annotations. Iterative geometry-illumination reasoning improves attached shadow detection by at least 33% BER reduction while maintaining strong cast shadow performance.

Conclusion: Joint reasoning about shadows, illumination, and geometry enables effective attached shadow detection, addressing a critical gap in scene understanding and 3D structure perception.
...
TL;DR: Modern DNNs remain vulnerable to high-confidence fooling attacks; transformers are most susceptible; new SPOOF attack is efficient and persistent even after defensive retraining.


<details>
  <summary>Details</summary>
Motivation: Despite advances in deep learning, neural networks continue to exhibit overconfidence on non-natural images (fooling images), revealing fundamental vulnerabilities that persist in state-of-the-art architectures including transformers.

Method: Re-implemented evolutionary fooling attacks from Nguyen et al. (2015) on modern architectures, then introduced SPOOF - a minimalist black-box attack using minimal pixel modifications with drastically reduced computational requirements.

Result: Transformers (ViT-B/16) are most susceptible to fooling attacks, requiring substantially fewer queries than convolutional models. SPOOF generates high-confidence fooling images with minimal compute, and defensive retraining with fooling images as a class provides only partial resistance.

Conclusion: Modern deep classifiers exhibit persistent fragility to fooling attacks, with transformer architectures being particularly vulnerable. Even defensive measures like retraining offer limited protection, highlighting fundamental robustness issues in current DNN architectures.
...
TL;DR: Novel multi-modal method predicts food drying color trajectories with high accuracy using high-dimensional temporal color data and process parameters, achieving over 90% error reduction compared to baselines.


<details>
  <summary>Details</summary>
Motivation: Current food drying color analysis methods use low-dimensional color features and cannot capture complex dynamic color trajectories or generalize to unseen process conditions, limiting their predictive capability.

Method: Developed a multi-modal color-trajectory prediction method that integrates high-dimensional temporal color information with drying process parameters for accurate and data-efficient prediction.
...
Conclusion: The method demonstrates superior accuracy, robustness, and broad applicability for predicting color evolution in food drying processes across different products and conditions.
...
TL;DR: The FeTS Challenge 2024 evaluated federated learning methods for glioma segmentation, where a PID-controller-based approach achieved top performance with best segmentation accuracy and communication efficiency.


<details>
  <summary>Details</summary>
Motivation: To advance federated learning for medical imaging by evaluating new weight aggregation methods that improve robustness and efficiency in glioma sub-region segmentation across multi-institutional MRI data.

Method: Six teams participated in a standardized FL setup using the BraTS-derived dataset (1,251 training, 219 validation, 570 test cases). Methods were evaluated using cumulative scoring based on segmentation performance (DSC, HD95) and communication efficiency (convergence score).

Result: A PID-controller-based method achieved top ranking with mean DSC values of 0.733 (ET), 0.761 (TC), 0.751 (WT) and HD95 values of 33.922mm, 33.623mm, 33.309mm, plus highest communication efficiency (convergence score 0.764), surpassing previous challenge methods.

Conclusion: The challenge advances federated learning for medical imaging, demonstrating PID controllers as effective mechanisms for stabilizing and optimizing weight aggregation in FL, with publicly available code for reproducibility.
...
TL;DR: Reproducibility study finds SVD+WDR image compression doesn't outperform JPEG2000 or WDR alone, contrary to original claims, due to implementation ambiguities.


<details>
  <summary>Details</summary>
Motivation: To independently verify the original paper's claims that combining SVD and WDR yields better visual quality and higher compression ratios than JPEG2000 and standalone WDR.

Method: Re-implemented the proposed SVD+WDR method, filled in missing implementation details, replicated original experiments, and conducted additional experiments on new images using PSNR and SSIM metrics.

Result: Contrary to original claims, SVD+WDR generally doesn't surpass JPEG2000 or WDR in PSNR, and only partially improves SSIM relative to JPEG2000. Found ambiguities in original description significantly impact reproducibility.

Conclusion: The reproducibility study reveals that the original SVD+WDR compression claims are not supported by independent verification, highlighting the importance of clear implementation details for reproducible research.
...
TL;DR: A GPU-accelerated GLMB filter variant that allows multiple detections per object, breaking inter-detection dependencies for improved parallel scalability.


<details>
  <summary>Details</summary>
Motivation: Standard GLMB filters are computationally expensive due to multiple hypothesis maintenance, especially problematic for real-time applications with distributed ML-based virtual sensors that can produce multiple detections per object.

Method: Modified GLMB filter variant that accommodates multiple detections per object from the same sensor, which breaks inter-detection dependencies in filter updates, enabling improved parallel scalability and efficient GPU implementation.

Result: Preliminary analysis shows the GPU-accelerated implementation achieves better run time scalability with respect to number of objects and maximum retained hypotheses compared to standard approaches.

Conclusion: The proposed GLMB variant enables practical deployment of multi-target tracking in distributed sensor networks by leveraging GPU acceleration through parallelizable updates, addressing computational bottlenecks of traditional methods.
...
TL;DR: Training on developmentally realistic video data (SAYCam) doesn't improve intuitive physics performance despite being more human-like, suggesting data distribution alone isn't sufficient for learning physical principles.


<details>
  <summary>Details</summary>
Motivation: Humans learn intuitive physics from limited, developmentally realistic visual experiences, while AI models trained on massive internet video data still fail at intuitive physics benchmarks. The paper investigates whether data distribution (rather than volume) is key to learning physical principles.

Method: Pretrained a Video Joint Embedding Predictive Architecture (V-JEPA) model on SAYCam dataset - a developmentally realistic, egocentric video dataset capturing three children's everyday visual experiences (only 0.01% of data volume used by SOTA models). Evaluated performance on IntPhys2 benchmark.

Result: Training on the developmentally realistic dataset did NOT lead to significant performance improvements on the IntPhys2 benchmark. The model failed to learn representations supporting intuitive physics despite the more human-like data distribution.

Conclusion: Merely training on developmentally realistic data is insufficient for current architectures to learn intuitive physics. Varying visual data volume and distribution alone may not be sufficient for building systems with artificial intuitive physics - suggesting architectural or learning mechanism changes may be needed.
...
<details>
  <summary>Details</summary>
Motivation: Existing PS-MTL approaches focus on homogeneous dense prediction tasks, leaving the realistic challenge of learning from structurally diverse tasks unexplored. There's a need for a framework that can handle both homogeneous and heterogeneous tasks with incomplete annotations.
...
Result: NexusFlow achieves state-of-the-art results on nuScenes for domain-partitioned autonomous driving (dense map reconstruction + sparse multi-object tracking). On NYUv2 with three homogeneous dense prediction tasks (segmentation, depth, surface normals), it yields consistent gains across all tasks, demonstrating broad applicability.

Conclusion: NexusFlow is an effective, lightweight, and plug-and-play framework for PS-MTL that works well with both structurally diverse and homogeneous tasks, enabling knowledge transfer across tasks with incomplete annotations through invertible feature alignment.
...
TL;DR: LaFG uses LLMs and VLMs to convert class names into attribute-level supervision for fine-grained image retrieval, improving generalization to unseen categories.


<details>
  <summary>Details</summary>
Motivation: Existing FGIR methods use one-hot labels that overlook rich semantics in category names, limiting cross-category comparability and generalization to unseen classes.

Method: LaFG prompts LLMs to generate attribute-oriented descriptions from class names, uses VLMs to project them into vision-aligned space, clusters them into attribute vocabulary, and creates category-specific linguistic prototypes for supervision.

Result: The method improves modeling of cross-category details and enhances generalization to unseen categories through attribute-level supervision.

Conclusion: Language-driven attribute supervision from LLMs and VLMs provides richer semantic guidance than one-hot labels, enabling better fine-grained retrieval generalization.
...
TL;DR: PSO is a two-stage post-training framework that addresses path selection bias in Large Vision-Language Models by optimizing reasoning paths through policy optimization and preference alignment, improving reasoning accuracy by 7.4% on average.


<details>
  <summary>Details</summary>
Motivation: LVLMs frequently arrive at correct answers through incorrect reasoning paths due to path selection bias in the reasoning search space, not lack of knowledge. This leads to erratic and unreliable outcomes despite models being capable of correct reasoning.

Method: Two-stage framework: 1) Group Relative Policy Optimization (GRPO) with template and answer-based rewards to cultivate structured step-by-step reasoning. 2) Online preference optimization where models sample paths, self-evaluate, and align toward preferred trajectories, with incorrect paths stored in Negative Replay Memory (NRM) for continual refinement.
...
Conclusion: The proposed PSO framework successfully addresses path selection bias in LVLMs, improving both reasoning performance and stability by optimizing reasoning path selection and preventing repetition of prior mistakes through continual refinement.
...
TL;DR: 3D Gaussian Splatting improved with global geometry consistency via constrained multi-view triangulation to reduce artifacts and improve surface reconstruction.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods rely solely on photometric loss, leading to inconsistencies like "floater" artifacts and unstructured geometry that prevent high-fidelity surface extraction.

Method: Introduces global geometry consistency through constrained multi-view triangulation, penalizing deviation of rendered 3D points from robust consensus points re-triangulated from neighboring views in self-supervised fashion.

Result: Achieves state-of-the-art results across multiple datasets, with mean Chamfer Distance of 0.50 mm on DTU dataset, outperforming comparable explicit methods.

Conclusion: The method successfully addresses reconstruction inconsistencies in 3D Gaussian Splatting by enforcing global geometry consistency, enabling better surface extraction while maintaining real-time rendering capabilities.
...
TL;DR: FacePhys is a memory-efficient rPPG algorithm that solves the trilemma of model scalability, cross-dataset generalization, and real-time operation for camera-based vital sign monitoring.


<details>
  <summary>Details</summary>
Motivation: Camera-based vital sign monitoring (rPPG) faces practical limitations: computational constraints on front-end devices and accuracy degradation when transmitting compressed data, hindering real-world deployment.

Method: Uses temporal-spatial state space duality with a transferable heart state to capture subtle periodic variations across video frames while maintaining minimal computational overhead, enabling training on extended sequences and low-latency inference.

Result: Achieves 49% error reduction (new SOTA), real-time inference with 3.6 MB memory footprint and 9.46 ms per-frame latency, surpassing existing methods by 83-99% in efficiency.

Conclusion: FacePhys enables reliable real-time rPPG performance for practical deployments, resolving the scalability-generalization-latency trilemma in camera-based health monitoring.
...
TL;DR: RefBench-PRO is a new REC benchmark that decomposes referring expressions into perception and reasoning dimensions with six sub-tasks, plus an RL-based Ref-R1 method for improved localization.


<details>
  <summary>Details</summary>
Motivation: Existing REC benchmarks lack interpretable scoring mechanisms and cannot reveal MLLM's grounding capability across different cognitive abilities, failing to distinguish between perceptual and reasoning capabilities.

Method: 1) Created RefBench-PRO benchmark with six sub-tasks (attribute, position, interaction, commonsense, relation, reject) across perception and reasoning dimensions; 2) Developed automated data-generation pipeline; 3) Proposed Ref-R1 with Dynamic IoU-based GRPO for RL-based learning to improve localization accuracy.

Result: RefBench-PRO enables interpretable evaluation of MLLMs on REC, presents greater challenges in both perception and reasoning, and Ref-R1 establishes a stronger baseline for REC tasks.

Conclusion: The benchmark provides comprehensive evaluation of MLLM's grounding capabilities across cognitive dimensions, addressing limitations of existing REC benchmarks through interpretable scoring and challenging sub-tasks.
...
TL;DR: LaVer addresses modality imbalance in MLLMs by introducing latent visual reconstruction through masked image modeling in LLM's joint semantic space.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from modality imbalance where visual information is underutilized compared to text in deeper layers, causing degraded visual performance and hallucinations due to lack of direct visual supervisory signals during training.

Method: Proposes Latent Visual Reconstruction (LaVer) framework that uses masked image modeling in the joint latent semantic space of LLM to provide direct visual activation and help MLLMs learn more discriminative visual representations.

Result: Extensive experiments show superiority across diverse benchmarks, especially in scenarios requiring dense visual capabilities, with MLLMs exhibiting increased visual attention allocation and enhanced visual information utilization.

Conclusion: LaVer effectively addresses modality imbalance in MLLMs by providing direct visual supervisory signals through latent space reconstruction, improving visual representation learning and performance on vision-intensive tasks.
...
TL;DR: Developed noninvasive sleep monitoring system using event-based method with depth sensor, RGB camera, and microphone array to detect motion, lighting, and noise events for quantitative evaluation of sleep disturbances.


<details>
  <summary>Details</summary>
Motivation: Need for quantitative evaluation of sleep disturbances in home context without invasive monitoring methods.

Method: Event-based monitoring system using infrared depth sensor for motion detection, RGB camera for lighting changes, and microphone array for noise detection, with background models and event detection algorithms.

Result: System successfully tested in sleep conditions with validated reliability for detecting three types of sleep disturbance events.

Conclusion: The developed noninvasive monitoring system provides reliable quantitative evaluation of sleep disturbances through multi-modal event detection in home environments.
...
TL;DR: StrokeNet: A novel network that encodes strokes as reference pair representations with spatial queries and feature interactions, achieving state-of-the-art stroke classification performance.


<details>
  <summary>Details</summary>
Motivation: Stroke classification is challenging due to writing style variations, ambiguous content, and dynamic positions. Existing methods struggle to capture fine-grained semantic relationships between strokes, which are typically localized. Point-level approaches introduce redundancy, but selecting reference points with sequential ordering can solve this.

Method: StrokeNet encodes strokes as reference pair representations (points + feature vectors). It dynamically selects reference points for each stroke and sequences them, using an Inline Sequence Attention (ISA) module for contextual features. A Cross-Ellipse Query (CEQ) mechanism clusters reference points to capture spatial feature interactions across scales. Joint optimization predicts stroke categories via reference point regression and models adjacent stroke semantic transitions through an Auxiliary Branch.
...
Conclusion: StrokeNet effectively addresses stroke classification challenges by modeling fine-grained semantic relationships through reference pair representations and spatial feature interactions, outperforming existing methods.
...
TL;DR: Event-based human pose estimation using point cloud framework with temporal slicing convolution and edge enhancement, outperforming dense frame conversion methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods convert event streams to dense frames, adding computation and sacrificing the high temporal resolution of event cameras. Need to exploit spatiotemporal properties directly from event streams.

Method: Point cloud-based framework with Event Temporal Slicing Convolution for short-term dependencies, Event Slice Sequencing for structured temporal modeling, and edge enhancement in point cloud representation to improve spatial information under sparse events.
...
Conclusion: Proposed method effectively leverages spatiotemporal properties of event streams without converting to dense frames, achieving better human pose estimation performance while maintaining computational efficiency.
...
TL;DR: ReCAD is an RL framework that bootstraps pretrained large models to generate precise parametric CAD models from text or images, using parameterized code guidance and hierarchical primitive learning to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Previous methods for CAD generation rely on supervised fine-tuning with limited editability and fail to exploit the strong generative priors of pretrained large models, requiring a more effective approach that leverages these models' inherent capabilities.

Method: 1) Fine-tune vision-language models with parameterized CAD scripts for basic generation; 2) Novel RL strategy using parameterized code as guidance for reasoning; 3) Hierarchical primitive learning with unified reward function for geometric accuracy and semantic fidelity.

Result: State-of-the-art in both text-to-CAD and image-to-CAD tasks, significantly improving geometric accuracy. For image-to-CAD: reduces mean Chamfer Distance from 73.47 to 29.61 (in-distribution) and from 272.06 to 80.23 (out-of-distribution).

Conclusion: ReCAD successfully bootstraps pretrained large models for precise parametric CAD generation through RL with parameterized code guidance and hierarchical learning, demonstrating superior performance over existing methods while enabling complex CAD operations.
...
TL;DR: S2WMamba uses 2D/1D wavelet transforms to disentangle spatial-spectral information, with Mamba-based cross-modulation for efficient pansharpening.


<details>
  <summary>Details</summary>
Motivation: Pansharpening struggles with entangling spatial detail and spectral fidelity when jointly processing PAN and MS images, requiring explicit disentanglement of frequency information.

Method: Uses 2D Haar DWT on PAN for spatial edges/textures, and channel-wise 1D Haar DWT on MS spectra to separate frequency components. Two branches (Spectral and Spatial) exchange information via Mamba-based cross-modulation with linear complexity, fused by multi-scale dynamic gate.
...
Conclusion: S2WMamba effectively disentangles spatial-spectral information through wavelet transforms and efficient Mamba-based cross-modulation, achieving state-of-the-art pansharpening performance with validated design choices.
...
Conclusion: CryoHype addresses the challenge of compositional heterogeneity in cryo-EM and enables high-throughput reconstruction of many distinct molecular species simultaneously.
...
<details>
  <summary>Details</summary>
Motivation: Current generative image compression suffers from semantic deviations due to generative hallucinations at ultra-low bitrates, limiting reliable deployment in bandwidth-constrained 6G semantic communication scenarios.

Method: MTGC integrates three guidance modalities: text captions for global semantics, highly compressed images for low-level visual information, and Semantic Pseudo-Words (SPWs) for fine-grained task-relevant semantics. It uses a Task-Aware Semantic Compression Module (TASCM) to generate SPWs and a Multimodal-Guided Diffusion Decoder (MGDD) with dual-path cooperative guidance (cross-attention + ControlNet residuals) to inject guidance into the diffusion process.

Result: MTGC consistently improves semantic consistency (e.g., DISTS drops by 10.59% on DIV2K dataset) while achieving remarkable gains in perceptual quality and pixel-level fidelity at ultra-low bitrate.
...
Conclusion: CLUENet offers a transparent deep architecture that successfully addresses the limitations of both traditional vision models and clustering paradigms, providing improved performance while maintaining interpretability for visual semantic understanding tasks.
...
TL;DR: TreeQ is a unified quantization framework for Diffusion Transformers that achieves near-lossless 4-bit performance through tree-structured search, environmental noise guidance, and general monarch branches.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) outperform U-Nets for image generation but face deployment challenges due to high computational/memory demands. Mixed-precision quantization has succeeded with U-Nets but remains underexplored for DiTs, creating a research gap.

Method: Three key components: 1) Tree Structured Search (TSS) for efficient O(n) space traversal using DiT's linear properties, 2) Environmental Noise Guidance (ENG) to align PTQ and QAT objectives with a single hyperparameter, 3) General Monarch Branch (GMB) structured sparse branch to prevent information loss in ultra-low-bit regimes.
...
Conclusion: TreeQ successfully addresses key DiT quantization challenges through its unified framework, enabling efficient deployment of DiT models with minimal performance degradation.
...
TL;DR: Reframes latent diffusion model for single-image reflection removal by aligning latent space with reflection physics, using task-specific text embedding and depth-guided sampling.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with reflection removal due to inability to reason about composite image composition in corrupted regions, leading to poor recovery and generalization in real-world scenarios.

Method: Three components: 1) Reflection-equivariant VAE that aligns latent space with linear physics of reflection formation, 2) Learnable task-specific text embedding for precise guidance, 3) Depth-guided early-branching sampling strategy to leverage generative stochasticity.

Result: Achieves new state-of-the-art performance on multiple benchmarks and demonstrates strong generalization to challenging real-world cases.

Conclusion: Successfully reframes latent diffusion model for reflection removal by addressing the fundamental issue of latent space structure for interpreting composite images as linear superposition of layers.
...
TL;DR: SPL-UAD framework uses spoofing-aware prompt learning with parallel branches to separately optimize for physical and digital attack detection, overcoming conflicting optimization in unified defense systems.


<details>
  <summary>Details</summary>
Motivation: Real-world face recognition systems need protection against both physical presentation attacks (like masks or photos) and digital forgery attacks (like deepfakes), but existing unified approaches using CLIP suffer from conflicting optimization directions between these two attack types.

Method: Proposes Spoofing-aware Prompt Learning for Unified Attack Detection (SPL-UAD) with: 1) learnable parallel prompt branches decoupling optimization for physical vs digital attacks, 2) adaptive Spoofing Context Prompt Generation for independent control, and 3) Cues-awareness Augmentation using dual-prompt mechanism for challenging sample mining to enhance robustness against unseen attacks.
...
Conclusion: SPL-UAD effectively addresses the optimization conflict problem in unified physical-digital defense frameworks by decoupling prompt learning branches, leading to superior detection performance for both attack types.
...
TL;DR: Human3R: A monocular dynamic video reconstruction method that uses hybrid geometric priors (SMPL + monocular depth) to solve geometric inconsistency and resolution degradation in human scenes.
...
Method: Proposes Human3R with hybrid geometric priors combining SMPL human body models and monocular depth estimation. Uses hierarchical pipeline with refinement components: processes full-resolution images for overall scene geometry, then applies strategic cropping and cross-attention fusion for human detail enhancement. Integrates SMPL priors through Feature Fusion Module.
...
Conclusion: The proposed Human3R method effectively addresses geometric inconsistency and resolution degradation in monocular dynamic human scene reconstruction by incorporating structured human priors while preserving fine-grained details.
...
TL;DR: VG-Refiner is a new framework for tool-refined referring grounded reasoning that addresses unreliable tool outputs in visual reasoning by introducing a two-stage think-rethink mechanism and refinement rewards.


<details>
  <summary>Details</summary>
Motivation: Existing tool-integrated visual reasoning (TiVR) paradigms focus on integrating visual tools through reinforcement learning but neglect designing effective response mechanisms for handling unreliable or erroneous tool outputs, especially in referring and grounding tasks where inaccurate detection tool predictions mislead models into generating hallucinated reasoning.

Method: Proposes VG-Refiner framework with: 1) Two-stage think-rethink mechanism enabling explicit analysis and response to tool feedback, 2) Refinement reward encouraging effective correction of poor tool results, 3) New metrics and evaluation protocols to measure refinement ability, 4) Small amount of task-specific data to enhance refinement capability while preserving pretrained model's general capabilities.
...
Conclusion: VG-Refiner successfully addresses the limitation of existing TiVR paradigms by providing an effective framework for tool-refined referring grounded reasoning that can handle unreliable tool outputs and reduce hallucinated reasoning.
...
TL;DR: AIGVs can harm AD models if used raw, but filtered with ADGVE they become beneficial complements to real data.


<details>
  <summary>Details</summary>
Motivation: AI-generated driving videos offer low-cost, scalable data for autonomous driving, but their reliability for training/evaluating AD models needs systematic validation.

Method: Developed taxonomy of AIGV failure modes, built ADGV-Bench benchmark with human annotations, and proposed ADGVE evaluator combining static semantics, temporal cues, lane obedience, and VLM-guided reasoning.

Result: Raw AIGVs degrade perception performance, but filtering with ADGVE improves video quality metrics and downstream AD models, making AIGVs beneficial complements to real data.

Conclusion: AIGVs present both risks and promise for AD; ADGVE provides practical tool for safely leveraging large-scale video generation in AD pipelines.
...
TL;DR: Researchers add VAD (Valence-Arousal-Dominance) annotation to FER2013 dataset (previously lacking Dominance dimension) and propose orthogonal convolution network for improved VAD prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: Current FER datasets use limited emotion categories (happy, angry, sad, etc.), but affective computing needs more comprehensive metrics. Existing datasets like AffectNet have Valence-Arousal but lack Dominance dimension.

Method: 1) Annotate FER2013 dataset with VAD dimensions (adding Dominance); 2) Develop orthogonalized convolution network to extract more diverse features and improve prediction accuracy.

Result: Dominance dimension can be measured but is more difficult to obtain than Valence and Arousal in both manual annotation and network prediction. Orthogonal convolution improves VAD prediction performance.

Conclusion: Provides first VAD-annotated FER2013 dataset as benchmark for multidimensional emotion measurement, and orthogonalized ResNet-based network as baseline for VAD emotion prediction. Dataset and code publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Online learning during COVID-19 requires not just knowledge distribution but also emotion interaction. Facial Expression Recognition (FER) can help teachers understand students' emotional states, but existing methods don't sufficiently exploit convolutional matrix orthogonality for better feature extraction.

Method: Proposes OCFER-Net which enforces orthogonality on convolutional kernels through a regularizer. This approach extracts features with more diversity and expressiveness compared to conventional methods.

Result: Experiments on challenging FER-2013 dataset show superior performance over baselines by 1.087 (presumably percentage points or accuracy improvement).

Conclusion: Orthogonal convolutional kernels improve FER performance by enhancing feature diversity and expressiveness. The method is effective for emotion recognition in online learning contexts, with code publicly available.
...
TL;DR: A region perception-based fusion framework for IR-VIS spectra using SVE camera that co-fuses multi-modal and multi-exposure data with structural similarity compensation for superior image clarity in extreme conditions.


<details>
  <summary>Details</summary>
Motivation: Existing IR-VIS fusion methods often compromise visible imagery quality, impacting measurement accuracy in photogrammetry, especially under extreme conditions where accurate fusion while preserving geometric fidelity of visible features and incorporating thermal radiation is challenging.

Method: Proposes a region perception-based fusion framework using spatially varying exposure (SVE) camera that: 1) performs region perception-based feature fusion for precise multi-modal registration, 2) uses adaptive fusion with contrast enhancement, 3) implements structural similarity compensation guided by regional saliency maps, and 4) adapts to single-exposure scenarios for robustness across conditions.

Result: Experiments on synthetic and real-world data demonstrate superior image clarity and improved performance compared to state-of-the-art methods, as evidenced by both quantitative and visual evaluations.

Conclusion: The proposed framework effectively addresses the challenge of fusing IR and VIS spectra while preserving visible feature geometric fidelity and incorporating thermal radiation, overcoming limitations of single-exposure methods in extreme environments.
...
TL;DR: SAR (Self-Autoregressive Refinement) addresses exposure bias in scale-wise autoregressive models through staggered-scale rollouts and contrastive loss, improving generation quality with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Scale-wise autoregressive models suffer from exposure bias due to train-test mismatch (models relying on imperfect predictions during inference) and imbalance in scale-wise learning difficulty, which undermines generation quality.

Method: Proposes Self-Autoregressive Refinement (SAR) with two components: 1) Stagger-Scale Rollout (SSR) mechanism for lightweight autoregressive rollouts to expose models to their own intermediate predictions, and 2) Contrastive Student-Forcing Loss (CSFL) to provide adequate supervision for self-generated contexts.

Result: SAR consistently improves generation quality when applied to pretrained AR models with minimal computational overhead. For example, 5.2% FID reduction on FlexVAR-d16 trained on ImageNet 256 within 10 epochs (5 hours on 32xA100 GPUs).
...
TL;DR: PCNN is a perception CNN for facial expression recognition that uses five parallel networks to learn local facial features, employs multi-domain interaction to fuse local and global features, and uses a two-phase loss function for better performance.


<details>
  <summary>Details</summary>
Motivation: Standard CNNs may ignore the effect of facial segmentation in FER, failing to capture subtle changes in facial expressions effectively.

Method: 1) Five parallel networks learn local features from eyes, cheeks, and mouth; 2) Multi-domain interaction mechanism fuses local sense organ features with global facial structural features; 3) Two-phase loss function constrains both accuracy of sense information and reconstructed face images.
...
Conclusion: The proposed PCNN effectively addresses facial segmentation in FER by capturing subtle expression changes through local feature learning and multi-domain fusion, demonstrating state-of-the-art performance across diverse datasets.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for articulated motion are either physically consistent but too slow for real-time use, or generative but violate basic kinematic constraints. There's a need for systems that can create plausible, real-time interactive 3D articulation that respects kinematic constraints.

Method: 1. Decoupled kinematic reasoning: Semantic intent reasoning determines joint type, while geometric regression (via KPP-Net) determines axis and origin. 2. Dual Quaternion VAE (DQ-VAE) uses dual quaternions for singularity-free rigid body motion representation. 3. FiLM conditioning injects joint priors at every layer of the Transformer decoder for strict kinematic adherence. 4. Numerically-stable cross-product loss ensures axis alignment.

Result: Achieves real-time performance and enables plausible, generative articulation on novel objects without retraining. The framework offers practical generative 3D intelligence with strict adherence to kinematic constraints.

Conclusion: DragMesh presents a robust framework for real-time interactive 3D articulation that successfully bridges the gap between physical consistency and real-time performance, offering a practical step toward generative 3D intelligence with kinematic fidelity.
...
TL;DR: Dual-path transformer framework using CLIP for gender recognition from extreme long-range imagery, combining visual and attribute-driven cues with improved robustness.


<details>
  <summary>Details</summary>
Motivation: Gender recognition from extreme long-range imagery is challenging due to limited spatial resolution, viewpoint variability, and loss of facial cues, requiring new approaches that can handle these constraints.

Method: Dual-path transformer framework leveraging CLIP with: (1) direct visual path fine-tuning upper layers of CLIP image encoder, (2) attribute-mediated path using soft-biometric prompts (hairstyle, clothing, accessories) aligned in CLIP text-image space, and spatial channel attention modules for enhanced localization.

Result: Proposed solution surpasses state-of-the-art person-attribute and re-identification baselines across multiple metrics (macro-F1, accuracy, AUC), with consistent robustness to distance, angle, and height variations, and interpretable attribute localization.

Conclusion: Language-guided dual-path learning offers a principled, extensible foundation for responsible gender recognition in unconstrained long-range scenarios, with responsible abstention behavior for uncertain cases.
...
TL;DR: Deep learning models achieve sub-centimeter accuracy in automated anthropometric measurements from synthetic body images for athlete cardiovascular screening.
...
Method: Developed a fully automated deep-learning approach using VGG19, ResNet50, and DenseNet121 with fully connected layers for regression, trained on 100,000 synthetic 2D body images derived from 3D meshes to estimate five key anthropometric measurements.

Result: All models achieved sub-centimeter accuracy, with ResNet50 performing best (mean MAE of 0.668 cm across all measurements), demonstrating deep learning can provide accurate anthropometric data at scale.

Conclusion: Deep learning offers a practical tool to complement athlete screening protocols, with future work needed to validate models on real-world images for broader applicability.
...
TL;DR: AGORA extends 3D Gaussian Splatting with a GAN framework to create animatable 3D human avatars with real-time rendering, outperforming NeRF methods in expression accuracy while achieving 250+ FPS on GPU and ~9 FPS on CPU.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D human avatars have limitations: NeRF-based approaches suffer from slow rendering and dynamic inconsistencies, while 3DGS methods are typically limited to static head generation without dynamic control. There's a need for high-fidelity, animatable avatars with real-time performance for VR, telepresence, and entertainment applications.

Method: AGORA extends 3D Gaussian Splatting within a generative adversarial network framework. Key innovation is a lightweight FLAME-conditioned deformation branch that predicts per-Gaussian residuals for identity-preserving expression control. Uses dual-discriminator training scheme leveraging synthetic renderings of parametric mesh for expression fidelity.

Result: Outperforms state-of-the-art NeRF-based methods on expression accuracy while rendering at 250+ FPS on single GPU, and ~9 FPS under CPU-only inference - first demonstration of practical CPU-only animatable 3DGS avatar synthesis. Generates visually realistic and precisely controllable avatars.

Conclusion: AGORA represents a significant step toward practical, high-performance digital humans by bridging the gap between high-fidelity animation and real-time rendering performance, enabling applications in VR, telepresence, and entertainment with unprecedented speed and control.
...
TL;DR: SCD-MLLM is a unified multimodal LLM framework for stable cross-domain depression recognition that handles heterogeneous data sources and missing modalities better than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current multimodal depression detection methods lack unified frameworks for diverse scenarios, have poor generalization across domains, and are unstable with missing modalities common in real-world data.

Method: Proposes SCD-MLLM with two key components: Multi-Source Data Input Adapter (MDIA) using masking and prompts to unify heterogeneous inputs, and Modality-Aware Adaptive Fusion Module (MAFM) for adaptive audio-visual feature integration with shared projection.

Result: Outperforms SOTA models and commercial LLMs (Gemini/GPT) across five heterogeneous datasets (CMDC, AVEC2014, DAIC-WOZ, DVlog, EATD) in both complete and partial modality settings, showing superior cross-domain generalization and stability.

Conclusion: SCD-MLLM provides an effective unified framework for scalable depression screening that handles real-world data challenges including heterogeneous sources and missing modalities while maintaining strong performance.
...
TL;DR: Sanvaad is a lightweight multimodal framework for real-time two-way communication between deaf, visually impaired, and hearing users using ISL recognition, voice-to-sign translation, and screen-free voice interfaces.


<details>
  <summary>Details</summary>
Motivation: Current accessibility tools often support only one direction of interaction, limiting communication between deaf users, visually impaired users, and the general hearing population.

Method: Uses MediaPipe landmarks for efficient ISL recognition, voice-to-sign translation mapping speech to predefined phrases with GIFs/alphabets, and screen-free voice interface with multilingual speech recognition, text summarization, and TTS.

Result: A lightweight framework that runs smoothly on edge devices without dedicated hardware, usable on both desktop and mobile through Streamlit-based interface.

Conclusion: Sanvaad offers practical inclusive communication by combining lightweight computer vision and speech processing in a unified framework for two-way accessibility.
...
TL;DR: An intelligent multi-modal framework for automated PV inspection that combines thermal and RGB data with adaptive re-acquisition and geospatial deduplication, achieving 90.3% mAP and reducing false positives by 15-20%.


<details>
  <summary>Details</summary>
Motivation: Conventional PV inspection methods suffer from thermal palette bias, data redundancy, and high communication bandwidth requirements, necessitating a more efficient automated solution.

Method: Synergistic architecture with palette-invariant thermal embedding fused with contrast-normalized RGB via gated mechanism, plus adaptive re-acquisition controller with Rodrigues-based updates and geospatial deduplication using DBSCAN over haversine distance.

Result: Achieved 0.903 mAP@0.5 on PVF-10 benchmark (12-15% improvement over baselines), 96% recall in field validation, 15-20% reduction in duplicate-induced false positives, and 60-70% reduction in airborne data transmission.

Conclusion: Establishes a powerful new paradigm for proactive PV inspection with validated system readiness and significant improvements in detection accuracy, false positive reduction, and communication efficiency.
...
TL;DR: ShadowWolf is a unified AI framework that dynamically adapts wildlife monitoring models to changing environmental conditions, reducing labeling effort and improving real-world robustness.


<details>
  <summary>Details</summary>
Motivation: Increasing human-wildlife interactions due to habitat expansion require better monitoring. Traditional AI models struggle with environmental variability (landscape, weather, lighting, distance), making them less robust in real-world scenarios.

Method: Proposes ShadowWolf - a unified framework that integrates and optimizes AI model training stages with dynamic retraining capabilities to adapt to changing environmental conditions and application requirements.

Result: The framework reduces labeling efforts, enables on-site model adaptation, and enhances accuracy and efficiency of wildlife monitoring systems for more effective conservation.

Conclusion: ShadowWolf's adaptive approach addresses key challenges in wildlife monitoring AI, making conservation efforts more scalable and effective through automated, environment-responsive systems.
...
Method: Developed two novel deep learning architectures: (a) SAETCN (Self-Attention Enhancement Tumor Classification Network) for classifying 3 tumor types (glioma, meningioma, pituitary) and non-tumor cases; (b) SAS-Net (Self-Attentive Segmentation Network) for accurate tumor segmentation.
...
Conclusion: The proposed architectures demonstrate high accuracy in both classification and segmentation tasks, offering promising CAD systems for early brain tumor detection and addressing limitations of existing models.
...
TL;DR: Learned k-space acquisition patterns in MRI show transferability across imaging domains, with proposed method using acquisition uncertainty to enhance domain robustness.


<details>
  <summary>Details</summary>
Motivation: Existing learned k-space sampling research focuses on single datasets/modalities with limited consideration of transferability across imaging domains. The paper aims to demonstrate that learned sampling benefits can extend beyond training domains.

Method: Two main contributions: 1) Systematic evaluation across datasets and acquisition paradigms showing learned sampling patterns improve generalization; 2) Novel method introducing acquisition uncertainty during training by stochastically perturbing k-space trajectories to simulate scanner/imaging condition variability.

Result: Models trained with learned sampling patterns exhibit improved generalization under cross-domain settings. The proposed uncertainty-based method enhances domain robustness in MRI reconstruction.

Conclusion: K-space trajectory design should be treated not just as an acceleration mechanism, but as an active degree of freedom for improving domain generalization in MRI reconstruction.
...
### [68] [More than Segmentation: Benchmarking SAM 3 for Segmentation, 3D Perception, and Reconstruction in Robotic Surgery](https://arxiv.org/abs/2512.07596)
*Wenzhen Dong,Jieming Yu,Yiming Huang,Hongqiu Wang,Lei Zhu,Albert C. S. Chung,Hongliang Ren,Long Bai*

Main category: cs.CV

TL;DR: SAM 3 shows improved segmentation over SAM/SAM 2 in surgical applications, with strong zero-shot performance on spatial prompts but limited language prompt effectiveness; demonstrates 3D reconstruction capabilities but faces challenges in dynamic surgical scenes.


<details>
  <summary>Details</summary>
Motivation: To evaluate SAM 3's performance in robot-assisted surgery, assessing its zero-shot segmentation capabilities (including new language prompts) and 3D reconstruction abilities for surgical applications.

Method: Empirical evaluation benchmarking SAM 3's zero-shot segmentation with point/box/language prompts, video tracking, and 3D reconstruction on surgical benchmarks (MICCAI EndoVis 2017/2018, SCARED, StereoMIS, EndoNeRF).

Result: SAM 3 outperforms SAM/SAM 2 in image/video segmentation with spatial prompts; language prompts show potential but suboptimal performance; demonstrates strong monocular depth estimation and 3D instrument reconstruction; limitations remain in complex dynamic scenes.

Conclusion: SAM 3 represents significant advancement for surgical applications with improved segmentation and 3D capabilities, but domain-specific training is needed for language prompts and handling complex dynamic surgical environments.
...
<details>
  <summary>Details</summary>
Motivation: As 3D-aware generative models advance in creating realistic human identities, concerns arise about user consent and the need to remove specific individuals from model outputs, requiring efficient unlearning methods.

Method: SUGAR learns personalized surrogate latent representations for each identity to divert reconstructions to visually coherent alternatives, and introduces a continual utility preservation objective to prevent degradation as more identities are removed.

Result: Achieves state-of-the-art performance in removing up to 200 identities, with up to 700% improvement in retention utility compared to existing baselines.

Conclusion: SUGAR provides an effective framework for scalable generative unlearning that maintains model quality and diversity while enabling identity removal, addressing important ethical concerns in generative AI.
...
### [70] [sim2art: Accurate Articulated Object Modeling from a Single Video using Synthetic Training Data Only](https://arxiv.org/abs/2512.07698)
*Arslan Artykov,Corentin Sautier,Vincent Lepetit*

Main category: cs.CV

TL;DR: First data-driven method to jointly predict part segmentation and joint parameters from monocular video with a freely moving camera, trained on synthetic data and generalizing to real-world objects.


<details>
  <summary>Details</summary>
Motivation: Articulated object understanding is crucial for robotics and digital twins, but previous work relies on multi-view systems, object scanning, or static cameras, lacking a scalable solution for monocular video.
...
Result: Method demonstrates strong generalization to real-world objects despite being trained only on synthetic data, offering a scalable and practical solution for articulated object understanding.

Conclusion: Provides the first approach for articulated object understanding from casually recorded monocular video, suitable for real-time applications in dynamic environments.
...
### [71] [Masked Autoencoder Pretraining on Strong-Lensing Images for Joint Dark-Matter Model Classification and Super-Resolution](https://arxiv.org/abs/2512.06642)
...
<details>
  <summary>Details</summary>
Motivation: Analyzing dark-matter substructure from noisy, low-resolution strong-lensing images is challenging, requiring better methods for classification and image enhancement.

Method: Masked autoencoder (MAE) pretraining on simulated strong-lensing images, then fine-tuning Vision Transformer encoder separately for classification and super-resolution tasks.

Result: MAE pretraining with 90% mask ratio achieves macro AUC 0.968 (vs 0.957 baseline) for classification and PSNR ~33 dB for super-resolution, showing consistent trade-off between classification and reconstruction.

Conclusion: MAE pretraining on physics simulations provides a flexible, reusable encoder for multiple strong-lensing analysis tasks, outperforming training from scratch.
...
TL;DR: U-CycleMLP: A lightweight U-shaped encoder-decoder network for medical image segmentation that effectively captures local and global context using position attention, dense atrous blocks, and channel CycleMLP blocks, achieving competitive performance across multiple datasets.
...
Method: U-shaped encoder-decoder network with encoder using position attention weight excitation blocks and dense atrous blocks for multiscale feature learning, decoder with upsampling and feature fusion, and channel CycleMLP blocks along skip connections for enhanced feature integration with linear complexity.

Result: Competitive performance across three benchmark datasets with better segmentation accuracy than state-of-the-art methods, capturing fine-grained anatomical structures, and demonstrating robustness across different medical imaging modalities.

Conclusion: U-CycleMLP effectively balances segmentation accuracy and computational efficiency while capturing both local and global context, with ablation studies confirming the importance of its core architectural components.
...
TL;DR: UltrasODM is a dual-stream framework for clinical ultrasound that provides per-frame uncertainty estimation, saliency-based diagnostics, and actionable prompts to assist sonographers during acquisition, reducing reconstruction errors and improving trust.
...
Result: On clinical freehand ultrasound dataset, reduces drift by 15.2%, distance error by 12.1%, and Hausdorff distance by 10.1% relative to UltrasOM, while producing per-frame uncertainty and saliency outputs.

Conclusion: UltrasODM improves reconstruction reliability and supports safer, more trustworthy clinical workflows through transparency and clinician feedback, with code publicly available.
...
### [74] [TextMamba: Scene Text Detector with Mamba](https://arxiv.org/abs/2512.06657)
*Qiyan Zhao,Yue Yan,Da-Han Wang*

Main category: cs.CV

TL;DR: A novel scene text detector using Mamba's selection mechanism with attention layers, Top_k algorithm for key information selection, and dual-scale modules for feature enhancement, achieving SOTA performance on text detection benchmarks.
...
Method: Proposes a Mamba-based scene text detector that integrates selection mechanism with attention layers, uses Top_k algorithm to explicitly select key information, and includes a dual-scale feed-forward network and embedding pyramid enhancement module for high-dimensional hidden state interactions and multi-scale feature fusion.
...
Conclusion: The proposed Mamba-based approach effectively addresses Transformer limitations in scene text detection by combining selection mechanisms with attention, demonstrating superior performance on various benchmarks through improved long-range dependency modeling and feature enhancement.
...
### [75] [The Role of Entropy in Visual Grounding: Analysis and Optimization](https://arxiv.org/abs/2512.06726)
...
TL;DR: ECVGPO is an interpretable entropy control algorithm for visual grounding that balances exploration-exploitation trade-off, achieving broad improvements across benchmarks.


<details>
  <summary>Details</summary>
Motivation: While entropy control techniques have advanced MLLM fine-tuning via RL, their role in perception tasks like visual grounding remains unexplored compared to reasoning tasks.

Method: ECVGPO (Entropy Control Visual Grounding Policy Optimization) - an interpretable algorithm for effective entropy regulation in visual grounding, analyzing entropy characteristics specific to perception tasks.

Result: Experiments show ECVGPO achieves broad improvements across various benchmarks and models through better balance of exploration-exploitation trade-off.

Conclusion: ECVGPO successfully addresses the underexplored area of entropy control in visual grounding, demonstrating that tailored entropy regulation strategies can significantly improve performance in perception-oriented tasks.
...
### [76] [GNC-Pose: Geometry-Aware GNC-PnP for Accurate 6D Pose Estimation](https://arxiv.org/abs/2512.06565)
*Xiujin Liu*
...
<details>
  <summary>Details</summary>
Motivation: To create a robust, learning-free 6D pose estimation solution that doesn't require learned features, training data, or category-specific priors, while maintaining competitive accuracy against both learning-based and learning-free methods.

Method: Combines rendering-based initialization for coarse 2D-3D correspondences, introduces geometry-aware cluster-based weighting for robust confidence assignment based on 3D structural consistency, and uses Graduated Non-Convexity (GNC) optimization followed by Levenberg-Marquardt refinement.

Result: Achieves competitive accuracy on the YCB Object and Model Set benchmark compared to both learning-based and learning-free methods, despite requiring no learned features, training data, or category-specific priors.

Conclusion: GNC-Pose offers a simple, robust, and practical solution for learning-free 6D pose estimation that combines geometric priors with robust optimization to handle severe outlier contamination effectively.
...
### [77] [Task-Model Alignment: A Simple Path to Generalizable AI-Generated Image Detection](https://arxiv.org/abs/2512.06746)
*Ruoxin Chen,Jiahui Gao,Kaiqing Lin,Keyue Zhang,Yandan Zhao,Isabel Guan,Taiping Yao,Shouhong Ding*

Main category: cs.CV

TL;DR: AlignGemini introduces task-model alignment for AIGI detection by combining a VLM fine-tuned for semantic consistency checking with a pixel-artifact expert for low-level artifact detection, achieving +9.5% average accuracy gain.


<details>
  <summary>Details</summary>
Motivation: Current VLM-based AIGI detectors suffer from severe hallucinations and poor performance due to task-model misalignment - semantics-oriented VLMs lack sensitivity to fine-grained pixel artifacts, while pixel-artifact detectors lack semantic awareness.

Method: Formalizes AIGI detection as two complementary tasks: semantic consistency checking and pixel-artifact detection. Creates AlignGemini with two branches: 1) VLM fine-tuned exclusively with pure semantic supervision, and 2) pixel-artifact expert trained exclusively with pure pixel-artifact supervision, using orthogonal supervision on simplified datasets.

Result: AlignGemini achieves a +9.5% gain in average accuracy across five in-the-wild benchmarks compared to existing methods, demonstrating superior generalizable AIGI detection.

Conclusion: Task-model alignment is an effective principle for generalizable AIGI detection, where distinct models should be matched to distinct tasks (semantic vs. pixel-level) rather than forcing single models to handle both.
...
### [78] [VisChainBench: A Benchmark for Multi-Turn, Multi-Image Visual Reasoning Beyond Language Priors](https://arxiv.org/abs/2512.06759)
...
Method: Created VisChainBench with 1,457 tasks spanning 20,000+ images across three domains, using a multi-agent generation pipeline to ensure visual diversity and controlled language bias.

Result: A large-scale benchmark that rigorously evaluates LVLMs' multi-step visual reasoning capabilities in sequential, interdependent tasks with minimal language guidance.

Conclusion: VisChainBench bridges the gap in evaluating LVLMs' ability to perform progressive visual reasoning in multi-image, multi-turn scenarios, providing a comprehensive benchmark for real-world decision-making processes.
...
### [79] [MedGRPO: Multi-Task Reinforcement Learning for Heterogeneous Medical Video Understanding](https://arxiv.org/abs/2512.06581)
...
TL;DR: MedVidBench benchmark with 531K medical video-instruction pairs and MedGRPO RL framework with cross-dataset normalization and medical LLM judge improves medical video understanding over GPT-4.1 and Gemini-2.5-Flash.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models struggle with medical video understanding due to requirements for spatial precision, temporal reasoning, and clinical semantics, necessitating specialized benchmarks and training methods.

Method: 1) Created MedVidBench benchmark with 531,850 video-instruction pairs across 8 medical sources with expert-guided QA; 2) Developed MedGRPO RL framework with cross-dataset reward normalization and medical LLM judge for balanced multi-dataset training.

Result: SFT on MedVidBench outperforms GPT-4.1 and Gemini-2.5-Flash across all tasks. MedGRPO further improves upon SFT baseline across grounding and captioning tasks, demonstrating effectiveness of the balanced RL approach.

Conclusion: The work establishes foundational benchmark and robust training methodology for advancing vision-language models in medical domains, addressing critical challenges in medical video understanding.
...
### [80] [Stitch and Tell: A Structured Multimodal Data Augmentation Method for Spatial Understanding](https://arxiv.org/abs/2512.06769)
*Hang Yin,Xiaomin He,PeiWen Yuan,Yiwei Li,Jiayi Shi,Wenxiao Fan,Shaoxiong Feng,Kan Li*

Main category: cs.CV

TL;DR: Stitch and Tell (SiTe) is a plug-and-play method that injects structured spatial supervision into vision-language training data by stitching images and generating spatially-aware captions, reducing spatial hallucinations without extra annotations.


<details>
  <summary>Details</summary>
Motivation: Vision-language models suffer from spatial hallucinations (incorrect descriptions of object positions) due to asymmetric properties between images and text representations.

Method: SiTe constructs stitched image-text pairs by stitching images along spatial axes and generating spatially-aware captions or QA pairs based on the layout, requiring no costly models or human annotation.

Result: Improves spatial understanding tasks (MME_Position +5.50%, Spatial-MM +4.19%) while maintaining or improving general VL benchmarks (COCO-QA +1.02%, MMBench +4.76%) across three architectures and eight benchmarks.
...
### [81] [From Remote Sensing to Multiple Time Horizons Forecasts: Transformers Model for CyanoHAB Intensity in Lake Champlain](https://arxiv.org/abs/2512.06598)
...
TL;DR: Transformer-BiLSTM model predicts cyanobacterial harmful algal blooms up to 14 days ahead using sparse satellite data with strong performance (78.9% F1 at 14 days).


<details>
  <summary>Details</summary>
Motivation: CyanoHABs threaten aquatic ecosystems and public health globally, with Lake Champlain being particularly vulnerable. Remote sensing offers scalable monitoring where in situ data is sparse, but existing satellite data has significant gaps (30-90% missing).

Method: Developed a remote sensing-only forecasting framework combining Transformers and BiLSTM to predict CyanoHAB intensities. Used Cyanobacterial Index data from CyAN and temperature data from MODIS satellites. Applied two-stage preprocessing with forward fill, weighted temporal imputation, and smoothing to handle data gaps. Created features through equal frequency binning for CI values and extracted temperature statistics.

Result: Transformer-BiLSTM achieved strong forecasting performance: F1 scores of 89.5%, 86.4%, and 85.5% at 1, 2, and 3-day forecasts, maintaining 78.9% F1 with 82.6% AUC at 14-day horizon. Model effectively captures complex spatiotemporal dynamics from sparse satellite data.

Conclusion: The framework provides reliable early warning for CyanoHAB management, demonstrating ability to handle sparse satellite data and capture long-range dependencies and sequential dynamics for effective forecasting up to 14 days ahead.
...
### [82] [RDSplat: Robust Watermarking Against Diffusion Editing for 3D Gaussian Splatting](https://arxiv.org/abs/2512.06774)
*Longjie Zhao,Ziming Hong,Zhenyang Ren,Runnan Chen,Mingming Gong,Tongliang Liu*

Main category: cs.CV

TL;DR: RDSplat introduces a robust watermarking method for 3D Gaussian Splatting that resists diffusion-based editing by embedding watermarks into low-frequency Gaussians and using adversarial training.


<details>
  <summary>Details</summary>
Motivation: Current 3DGS watermarking methods are vulnerable to diffusion-based editing which can erase embedded watermarks, creating an urgent need for robust watermarking techniques that can withstand such editing processes.

Method: RDSplat uses a multi-domain framework that embeds watermarks into low-frequency Gaussians (which diffusion editing preserves) via coordinated covariance regularization and 2D filtering, plus adversarial training using Gaussian blur as a diffusion proxy.
...
Conclusion: RDSplat successfully addresses the vulnerability of 3DGS watermarking to diffusion-based editing by strategically embedding watermarks in preserved components and using adversarial training, providing robust copyright protection.
...
### [83] [Learning Relative Gene Expression Trends from Pathology Images in Spatial Transcriptomics](https://arxiv.org/abs/2512.06612)
*Kazuya Nishimura,Haruka Hirose,Ryoma Bise,Kaito Shiku,Yasuhiro Kojima*

Main category: cs.CV

TL;DR: STRank: A novel loss function for learning relative gene expression patterns from pathology images instead of absolute values, making it robust to noise and batch effects.


<details>
  <summary>Details</summary>
Motivation: Traditional point-wise loss functions struggle with accurately estimating absolute gene expression from pathology images due to stochastic noise, batch effects, and complexity of sequencing techniques. Absolute expression values are difficult to estimate reliably.

Method: Proposes STRank loss function that focuses on learning relative expression patterns rather than absolute levels. Assumes relative expression patterns are consistent across experiments even when absolute values are affected by noise and batch effects.

Result: Experiments on synthetic and real datasets demonstrate the effectiveness of STRank in handling noise and batch effects compared to traditional absolute value estimation methods.

Conclusion: Learning relative expression patterns from pathology images using STRank is more robust to noise and batch effects than estimating absolute expression values, offering a promising approach for gene expression analysis from imaging data.
...
TL;DR: RMAdapter: A dual-branch adapter for VLMs that balances task-specific adaptation and generalization via adaptation and reconstruction branches, outperforming SOTA in few-shot scenarios.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning Vision-Language Models in few-shot scenarios struggles to balance task-specific adaptation and generalization. Current research focuses on prompt-based methods, leaving adapter-based approaches underexplored with performance gaps.

Method: Introduces Reconstruction-based Multimodal Adapter (RMAdapter) with dual-branch architecture: (1) adaptation branch injects task-specific knowledge via parameter-efficient fine-tuning, (2) reconstruction branch preserves general knowledge by reconstructing latent features back to original space. Uses local reconstruction loss, shared projection modules, and consistency constraint to maintain lightweight design.

Result: RMAdapter consistently outperforms state-of-the-art approaches across all evaluation metrics on three tasks: generalization to new categories, generalization to new target datasets, and domain generalization, without relying on data augmentation or duplicate prompt designs.

Conclusion: RMAdapter effectively balances discriminability and generalization in few-shot VLM fine-tuning through its novel dual-branch architecture, offering a lightweight yet powerful adapter-based solution that addresses limitations of existing approaches.
...
TL;DR: Hierarchical CNN with cascaded heads outperforms flat models in diatom classification by embedding taxonomic hierarchy, improving accuracy at upper levels and keeping errors taxonomically local.


<details>
  <summary>Details</summary>
Motivation: Conventional diatom identification relies on expert taxonomists, and existing deep learning approaches treat classification as flat, predicting only one taxonomic rank without leveraging hierarchical relationships.
...
Result: Hierarchical model matches flat baseline at species level (69.4% accuracy) while outperforming at all upper taxonomic levels. When species predictions fail, 92.5% of misclassified species are correctly predicted at genus level (vs. 67.2% for flat). Reduces mean taxonomic distance by 38.2% (1.209 vs. 1.955).

Conclusion: Hierarchical embedding improves multi-level taxonomic classification through bidirectional mechanisms: top-down constraint masks restrict prediction space, while bottom-up gradients refine features, producing more robust, interpretable, and biologically aligned predictions.
...
### [86] [Less Is More, but Where? Dynamic Token Compression via LLM-Guided Keyframe Prior](https://arxiv.org/abs/2512.06866)
...
<details>
  <summary>Details</summary>
Motivation: Existing VLLMs face efficiency bottlenecks with quadratic computational growth for long videos, and current keyframe sampling methods introduce additional computational cost before feature encoding with suboptimal binary frame selection.

Method: DyToK leverages VLLMs' inherent attention mechanisms to encode query-conditioned keyframe priors, enabling dynamic per-frame token retention ratios that prioritize semantically rich frames while suppressing redundancies.

Result: Achieves state-of-the-art efficiency-accuracy tradeoffs, shows plug-and-play compatibility with existing compression methods, attains 4.3x faster inference while preserving accuracy across multiple VLLMs like LLaVA-OneVision and Qwen2.5-VL.
...
TL;DR: JoPano is a unified DiT-based approach for both text-to-panorama and view-to-panorama generation that addresses visual quality limitations and modeling redundancy through a Joint-Face Adapter and condition switching mechanism.


<details>
  <summary>Details</summary>
Motivation: Existing panorama generation methods have two major challenges: U-Net-based architectures constrain visual quality, and treating text-to-panorama and view-to-panorama tasks independently leads to modeling redundancy and inefficiency.

Method: Proposes JoPano with a Joint-Face Adapter on cubemap representation to transfer DiT capabilities to panorama domain, Poisson Blending to reduce seam inconsistencies, and a condition switching mechanism to unify both generation tasks in a single model.

Result: Achieves state-of-the-art performance on FID, CLIP-FID, IS, and CLIP-Score metrics, generating high-quality panoramas for both text-to-panorama and view-to-panorama tasks.

Conclusion: JoPano successfully unifies panorama generation tasks within a DiT-based framework, overcoming visual quality limitations and modeling redundancy while introducing new metrics for seam consistency evaluation.
...
### [88] [NeuroABench: A Multimodal Evaluation Benchmark for Neurosurgical Anatomy Identification](https://arxiv.org/abs/2512.06921)
*Ziyang Song,Zelin Zang,Xiaofan Ye,Boqiang Xu,Long Bai,Jinlin Wu,Hongliang Ren,Hongbin Liu,Jiebo Luo,Zhen Lei*

Main category: cs.CV

TL;DR: First multimodal benchmark for evaluating anatomical understanding in neurosurgical videos reveals MLLMs lag significantly behind human performance, with best model achieving only 40.87% accuracy vs. human average of 46.5%.


<details>
  <summary>Details</summary>
Motivation: Existing surgical video research focuses on procedures and workflows but neglects anatomical understanding, which is critical for surgeons to interpret, review, and learn from surgical videos. There's a gap in evaluating multimodal models' anatomical comprehension in neurosurgery.

Method: Created NeuroABench - first multimodal benchmark for neurosurgical anatomical understanding with 9 hours of annotated videos covering 89 procedures. Used novel multimodal annotation pipeline with multiple review cycles. Evaluates identification of 68 clinical anatomical structures. Tested over 10 state-of-the-art MLLMs and compared with 4 neurosurgical trainees.

Result: Best-performing MLLM achieved only 40.87% accuracy in anatomical identification. Human trainees scored 28-56% with average 46.5%. Best MLLM performed comparably to lowest-scoring student but significantly below human average, showing substantial gap in anatomical understanding.

Conclusion: MLLMs show progress in anatomical understanding but still have significant limitations compared to human performance. NeuroABench provides standardized framework to evaluate and advance anatomical comprehension in surgical AI, highlighting need for improved multimodal reasoning in clinical domains.
...
### [89] [Personalized Image Descriptions from Attention Sequences](https://arxiv.org/abs/2512.06662)
*Ruoyu Xue,Hieu Le,Jingyi Xu,Sounak Mondal,Abe Leite,Gregory Zelinsky,Minh Hoai,Dimitris Samaras*

Main category: cs.CV

TL;DR: DEPER learns personalized embeddings combining linguistic style and viewing behavior to generate more human-aligned image descriptions via few-shot adaptation of frozen vision-language models.


<details>
  <summary>Details</summary>
Motivation: People view images differently (focus on different regions/objects/details) and describe them in distinct linguistic styles, but existing personalized image description models only focus on linguistic style, ignoring individual viewing patterns.

Method: DEPER learns a subject embedding capturing both linguistic style and viewing behavior using an auxiliary attention-prediction task, then uses a lightweight adapter to align these embeddings with a frozen vision-language model for few-shot personalization without retraining.

Result: Achieves 24% average improvement across four datasets spanning diverse viewing tasks and both short/detailed descriptions, showing that modeling personalized attention produces more human-aligned and high-quality descriptions.
...
### [90] [CoT4Det: A Chain-of-Thought Framework for Perception-Oriented Vision-Language Tasks](https://arxiv.org/abs/2512.06663)
*Yu Qi,Yumeng Zhang,Chenting Gong,Xiao Tan,Weiming Zhang,Wei Zhang,Jingdong Wang*

Main category: cs.CV

TL;DR: CoT4Det improves LVLM perception performance by reformulating detection tasks into classification, counting, and grounding steps, boosting Qwen2.5-VL-7B-Instruct's COCO mAP from 19% to 33%.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models excel at general vision-language tasks but perform poorly on perception-centric tasks like object detection, significantly underperforming task-specific expert models.

Method: Chain-of-Thought for Detection (CoT4Det) reformulates perception tasks into three interpretable steps: classification (identifying objects), counting (determining quantities), and grounding (locating objects).

Result: CoT4Det boosts Qwen2.5-VL-7B-Instruct's mAP from 19.0% to 33.0% on COCO2017 val, outperforms baselines by +2% on RefCOCO series and 19% on Flickr30k entities, while maintaining general vision-language capabilities.

Conclusion: The proposed CoT4Det strategy significantly improves perception performance of LVLMs without compromising their general capabilities, demonstrating that reformulating tasks into interpretable reasoning steps can bridge the gap between LVLMs and expert models.
...
TL;DR: DEViL is a detector-empowered video LLM that improves spatio-temporal grounding by coupling a video LLM with an open-vocabulary detector using reference-semantic tokens, avoiding autoregressive spatial decoding issues.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs for spatio-temporal grounding use autoregressive spatial decoding with bounding boxes as text tokens, causing long output sequences, error accumulation, and spatial drift across videos.

Method: DEViL connects a Video LLM with an open-vocabulary detector via reference-semantic tokens (RST) that distill queries into semantic representations, plus tube-mined temporal regularization for temporal consistency.

Result: DEViL achieves strong performance across fine-grained video understanding tasks, particularly STVG and GroundedVQA, outperforming previous methods.

Conclusion: The proposed DEViL framework effectively addresses autoregressive spatial decoding limitations by integrating detectors with video LLMs, enabling better spatio-temporal grounding and reasoning.
...
### [92] [Power of Boundary and Reflection: Semantic Transparent Object Segmentation using Pyramid Vision Transformer with Transparent Cues](https://arxiv.org/abs/2512.07034)
*Tuan-Anh Vu,Hai Nguyen-Truong,Ziqiang Zheng,Binh-Son Hua,Qing Guo,Ivor Tsang,Sai-Kit Yeung*

Main category: cs.CV

TL;DR: TransCues is a pyramidal transformer encoder-decoder framework that enhances glass object segmentation by incorporating both boundary and reflection features through specialized modules, achieving state-of-the-art performance across multiple benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Glass segmentation is challenging due to transparency and reflection properties. Existing methods fail to adequately capture both boundary and reflective-object features that humans use to identify glass objects.

Method: Proposes TransCues framework with two key modules: Boundary Feature Enhancement and Reflection Feature Enhancement, integrated in a pyramidal transformer encoder-decoder architecture for mutual benefit.
...
Conclusion: Incorporating both boundary and reflection features is effective for glass object segmentation, and the proposed TransCues framework demonstrates superior performance across diverse segmentation tasks involving transparent objects.
...
TL;DR: RunawayEvil is the first multimodal jailbreak framework for Image-to-Video models that uses a self-evolving "Strategy-Tactic-Action" architecture to generate coordinated text and image attacks, achieving state-of-the-art attack success rates.


<details>
  <summary>Details</summary>
Motivation: The security of multimodal Image-to-Video generation systems is critically underexplored, particularly their vulnerability to jailbreak attacks. There's a need to understand and address these security vulnerabilities to build more robust video generation systems.

Method: A "Strategy-Tactic-Action" framework with three core components: 1) Strategy-Aware Command Unit that self-evolves strategies through reinforcement learning and LLM exploration, 2) Multimodal Tactical Planning Unit that generates coordinated text jailbreak instructions and image tampering guidelines, and 3) Tactical Action Unit that executes and evaluates attacks.
...
### [94] [DAUNet: A Lightweight UNet Variant with Deformable Convolutions and Parameter-Free Attention for Medical Image Segmentation](https://arxiv.org/abs/2512.07051)
*Adnan Munir,Shujaat Khan*

Main category: cs.CV

TL;DR: DAUNet is a lightweight UNet variant that combines Deformable V2 Convolutions and SimAM attention for improved medical image segmentation with better spatial adaptability and context-aware feature fusion without increasing model complexity.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation is crucial for automated diagnostic and treatment planning systems, but existing models often lack spatial adaptability and context-aware feature fusion while maintaining computational efficiency for real-time clinical deployment.

Method: DAUNet integrates Deformable V2 Convolutions in the bottleneck to handle geometric variations with dynamic deformable kernels, and uses SimAM (Parameter-Free Attention) modules in the decoder and skip pathways for saliency-aware refinement and context-aware feature fusion.

Result: DAUNet outperforms state-of-the-art models on FH-PS-AoP (fetal head and pubic symphysis ultrasound) and FUMPE (CT-based pulmonary embolism detection) datasets in Dice score, HD95, and ASD metrics while maintaining superior parameter efficiency.

Conclusion: DAUNet demonstrates robustness to missing context and low-contrast regions, making it suitable for deployment in real-time and resource-constrained clinical environments, with ablation studies confirming the individual contributions of both deformable convolutions and SimAM attention.
...
### [95] [EMGauss: Continuous Slice-to-3D Reconstruction via Dynamic Gaussian Modeling in Volume Electron Microscopy](https://arxiv.org/abs/2512.06684)
*Yumeng He,Zanwei Zhou,Yekun Zheng,Chen Liang,Yunbo Wang,Xiaokang Yang*

Main category: cs.CV

TL;DR: EMGauss is a Gaussian splatting-based framework for 3D reconstruction from 2D slices that models slice progression as temporal evolution of Gaussian point clouds, overcoming limitations of isotropy-based approaches in volume electron microscopy.


<details>
  <summary>Details</summary>
Motivation: Volume electron microscopy (vEM) suffers from anisotropic volumes with limited axial resolution due to acquisition trade-offs. Existing deep learning methods that assume isotropy break down for morphologically anisotropic biological structures.

Method: Reframes slice-to-3D reconstruction as 3D dynamic scene rendering using Gaussian splatting, modeling axial slice progression as temporal evolution of 2D Gaussian point clouds. Incorporates Teacher-Student bootstrapping to enhance fidelity in data-sparse regimes by using high-confidence predictions as pseudo-supervisory signals.

Result: Substantially improves interpolation quality compared to diffusion- and GAN-based methods, enables continuous slice synthesis, and eliminates the need for large-scale pretraining.

Conclusion: EMGauss provides a generalizable slice-to-3D reconstruction framework that circumvents limitations of isotropy-based approaches, with potential applications beyond vEM across diverse imaging domains.
...
TL;DR: D³-Predictor: A noise-free deterministic framework that reformulates pretrained diffusion models for dense prediction tasks by aggregating timestep-dependent visual experts into a single geometric prior, eliminating stochastic noise corruption.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models with visual priors have a core limitation - the stochastic noise in diffusion sampling is inherently misaligned with dense prediction tasks that require deterministic image-to-geometry mapping. This noise corrupts fine-grained spatial cues and pushes models toward timestep-specific noise objectives, destroying meaningful geometric structure mappings.

Method: Introduces D³-Predictor, a noise-free deterministic framework that reformulates pretrained diffusion models without stochastic noise. Instead of using noisy inputs, it treats the pretrained diffusion network as an ensemble of timestep-dependent visual experts and self-supervisedly aggregates their heterogeneous priors into a single, clean geometric prior. Task-specific supervision adapts this noise-free prior to dense prediction tasks.

Result: Extensive experiments show D³-Predictor achieves competitive or state-of-the-art performance across various dense prediction tasks. It requires less than half the training data previously used and performs efficient single-step inference.

Conclusion: D³-Predictor successfully addresses the misalignment between stochastic diffusion sampling and deterministic dense prediction by creating a noise-free framework that aggregates diffusion priors into a unified geometric representation, enabling efficient and effective dense prediction with reduced data requirements.
...
### [97] [Lightweight Wasserstein Audio-Visual Model for Unified Speech Enhancement and Separation](https://arxiv.org/abs/2512.06689)
*Jisoo Park,Seonghak Lee,Guisik Kim,Taewoo Kim,Junseok Kwon*

Main category: cs.CV

TL;DR: UniVoiceLite is a lightweight unsupervised audio-visual framework that unifies speech enhancement and speech separation using lip motion and facial identity cues, achieving strong performance without paired noisy-clean data.


<details>
  <summary>Details</summary>
Motivation: Real-world audio often contains both background noise and overlapping speakers, but traditional approaches treat speech enhancement and speech separation as separate tasks. Existing integrated solutions use complex supervised models that limit scalability and generalization.
...
Conclusion: UniVoiceLite provides a unified, lightweight, unsupervised solution for speech enhancement and speech separation that addresses real-world audio challenges effectively.
...
### [98] [TrajMoE: Scene-Adaptive Trajectory Planning with Mixture of Experts and Reinforcement Learning](https://arxiv.org/abs/2512.07135)
...
TL;DR: The paper proposes a novel autonomous driving planning framework that combines Mixture of Experts for scenario-specific trajectory priors with Reinforcement Learning for policy-driven trajectory scoring refinement.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving systems have limitations: 1) they use fixed trajectory priors that don't adapt to different driving scenarios, and 2) their trajectory evaluation lacks policy-driven refinement due to one-stage supervised training constraints.

Method: Three key improvements: 1) Mixture of Experts (MoE) for scenario-specific trajectory priors, 2) Reinforcement Learning for fine-tuning trajectory scoring mechanism, and 3) integration of models with different perception backbones for enhanced perceptual features.

Result: Achieved score of 51.08 on the navsim ICCV benchmark, securing third place in the competition.

Conclusion: The proposed approach successfully addresses the limitations of current trajectory-based planning systems by adapting priors to scenarios and refining evaluation through policy learning, demonstrating competitive performance on benchmark tasks.
...
TL;DR: CUHK-X is a large-scale multimodal dataset for human action recognition, understanding, and reasoning, addressing limitations of existing datasets and LLMs with non-RGB modalities.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs struggle with non-RGB modalities (depth, IMU, mmWave) due to lack of large-scale data-caption resources, and current HAR datasets provide only coarse labels insufficient for fine-grained action understanding and reasoning tasks.

Method: Created CUHK-X dataset with 58,445 samples covering 40 actions by 30 participants across two indoor environments, using prompt-based scene creation with LLMs to generate logically connected activity sequences followed by human validation.
...
Conclusion: CUHK-X enables the community to apply and develop data-intensive learning methods for robust multimodal human activity analysis, bridging the gap between action recognition and higher-level understanding/reasoning tasks.
...
### [100] [Graph Convolutional Long Short-Term Memory Attention Network for Post-Stroke Compensatory Movement Detection Based on Skeleton Data](https://arxiv.org/abs/2512.06736)
*Jiaxing Fan,Jiaojiao Liu,Wenkong Wang,Yang Zhang,Xin Ma,Jichen Zhang*

Main category: cs.CV

TL;DR: GCN-LSTM-ATT network using skeleton data outperforms traditional ML methods in detecting compensatory movements during stroke rehabilitation with 85.8% accuracy.


<details>
  <summary>Details</summary>
Motivation: Most stroke patients experience upper limb motor dysfunction, and compensatory movements during rehabilitation training are detrimental to long-term recovery, making their detection crucial for effective rehabilitation.

Method: Proposed Graph Convolutional Long Short-Term Memory Attention Network (GCN-LSTM-ATT) using skeleton data from Kinect depth camera; compared with SVM, KNN, and Random Forest on data from 16 stroke patients performing rehabilitation movements.

Result: GCN-LSTM-ATT achieved 0.8580 detection accuracy, significantly outperforming traditional machine learning algorithms; ablation experiments confirmed each model component contributes to performance improvement.

Conclusion: The GCN-LSTM-ATT model provides a more precise and powerful tool for detecting compensatory movements after stroke, which could facilitate optimization of rehabilitation training strategies for stroke patients.
...
TL;DR: DiTFuse is a diffusion-transformer framework that uses natural language instructions to control multimodal image fusion in a single model, achieving state-of-the-art performance across multiple fusion tasks.
...
Method: Uses Diffusion-Transformer (DiT) framework to jointly encode two images and natural-language instructions in shared latent space. Employs multi-degradation masked-image modeling for training without ground truth, learning cross-modal alignment, restoration, and feature selection. Uses curated multi-granularity instruction dataset.

Result: Superior quantitative and qualitative performance on IVIF, MFF, and MEF benchmarks with sharper textures and better semantic retention. Supports multi-level user control, zero-shot generalization to other fusion scenarios, and instruction-conditioned segmentation.

Conclusion: DiTFuse provides a unified, instruction-driven approach to multimodal image fusion that overcomes limitations of existing methods, offering flexible control, better performance, and generalization capabilities across multiple fusion tasks.
...
### [102] [FedSCAl: Leveraging Server and Client Alignment for Unsupervised Federated Source-Free Domain Adaptation](https://arxiv.org/abs/2512.06738)
*M Yashwanth,Sampath Koti,Arunabh Singh,Shyam Marjit,Anirban Chakraborty*

Main category: cs.CV

TL;DR: FedSCAl is an FL framework for Federated source-Free Domain Adaptation that addresses client-drift caused by extreme data heterogeneity by aligning server and client predictions through Server-Client Alignment mechanism.


<details>
  <summary>Details</summary>
Motivation: The paper addresses Federated source-Free Domain Adaptation (FFreeDA) where clients have unlabeled data with significant inter-client domain gaps, and only a pre-trained server model is available without access to source data during training. Existing SFDA methods adapted to FL struggle with client-drift due to extreme data heterogeneity, leading to unreliable pseudo-labels.

Method: FedSCAl framework with Server-Client Alignment (SCAl) mechanism that regularizes client updates by aligning the clients' and server model's predictions. This alignment helps mitigate client-drift and improves pseudo-labeling accuracy.

Result: FedSCAl consistently outperforms state-of-the-art FL methods in the FFreeDA setup for classification tasks across benchmark vision datasets, showing improved pseudo-labeling accuracy post alignment.

Conclusion: The proposed FedSCAl framework with SCAl mechanism effectively addresses client-drift in FFreeDA scenarios by aligning server and client predictions, leading to better performance than existing methods in federated source-free domain adaptation settings.
...
TL;DR: START: Spatial and Textual learning for chART understanding - a multimodal LLM approach that combines chart-element grounding and chart-to-code generation to improve both spatial layout and data understanding of charts.


<details>
  <summary>Details</summary>
Motivation: Charts require understanding both visual layout (spatial property) and underlying data representation (textual property), which is crucial for real-world applications like analyzing scientific papers and technical reports. Existing methods fail to adequately handle both aspects.

Method: Proposes START with two key components: (1) chart-element grounding to understand visual layout, and (2) chart-to-code generation to understand data details. Uses a novel data-generation pipeline where MLLMs translate real chart images into executable code, then LLMs evolve the code to capture chart element positions.

Result: START delivers consistent gains across model sizes and benchmarks over base models, surpassing prior state-of-the-art by a clear margin. Also introduces CS-Bench benchmark for evaluating chart spatial understanding.

Conclusion: The proposed spatial and textual learning approach effectively addresses the dual nature of chart understanding, providing a comprehensive solution that outperforms existing methods and fills a critical gap in evaluation through the CS-Bench benchmark.
...
### [104] [VFM-VLM: Vision Foundation Model and Vision Language Model based Visual Comparison for 3D Pose Estimation](https://arxiv.org/abs/2512.07215)
...
Method: Comprehensive visual comparison between CLIP-based and DINOv2-based approaches for 6D object pose estimation, evaluated on benchmark datasets for hand-object grasping scenarios.
...
### [105] [UARE: A Unified Vision-Language Model for Image Quality Assessment, Restoration, and Enhancement](https://arxiv.org/abs/2512.06750)
*Weiqi Li,Xuanyu Zhang,Bin Chen,Jingfen Xie,Yan Wang,Kexin Zhang,Junlin Li,Li Zhang,Jian Zhang,Shijie Zhao*

Main category: cs.CV

TL;DR: UARE is the first unified vision-language model for image quality assessment, restoration, and enhancement that leverages quality understanding to guide restoration through multi-task co-training.
...
Method: Built on pretrained unified understanding and generation models, UARE uses a two-stage training framework: 1) progressive easy-to-hard schedule from single-type distortions to mixed degradations, 2) unified fine-tuning of quality understanding and restoration with interleaved text-image data, aligning IQA signals with restoration objectives through multi-task co-training.

Result: Extensive experiments across IQA, restoration, and enhancement tasks demonstrate the effectiveness of UARE in leveraging IQA to boost restoration and enhancement performance.

Conclusion: UARE successfully unifies image quality assessment, restoration, and enhancement in a single model, showing that quality understanding can effectively guide restoration tasks, with code and models made publicly available.
...
### [106] [Towards Robust Protective Perturbation against DeepFake Face Swapping](https://arxiv.org/abs/2512.07228)
...
<details>
  <summary>Details</summary>
Motivation: Current DeepFake protection methods using invisible perturbations are fragile against basic transformations like compression/resizing. Standard EOT with uniform sampling is suboptimal because protection robustness is highly sensitive to transformation choices.
...
Conclusion: EOLT framework effectively addresses the fragility of current DeepFake protection methods by learning optimal transformation distributions rather than using fixed uniform sampling, significantly improving robustness against various transformations.
...
### [107] [Dropout Prompt Learning: Towards Robust and Adaptive Vision-Language Models](https://arxiv.org/abs/2512.07234)
*Biao Chen,Lin Zuo,Mengmeng Jing,Kunbin He,Yuchen Wang*

Main category: cs.CV

TL;DR: Dropout Prompt Learning applies dropout to vision-language models by evaluating token significance across modalities and using residual entropy regularization to balance semantic alignment with diverse representations.


<details>
  <summary>Details</summary>
Motivation: To improve the robustness of vision-language models by adapting dropout regularization to handle multimodal tokens, addressing challenges in low-shot learning, long-tail classification, and out-of-distribution generalization.

Method: Proposes Dropout Prompt Learning with two key components: 1) applying dropout to textual and visual tokens with flexible probabilities based on intra-modal context and inter-modal alignment significance, and 2) residual entropy regularization to maintain semantic alignment while encouraging diverse representations.

Result: Achieves state-of-the-art performance on 15 benchmarks, surpassing regularization-based methods including KgCoOp by 5.10% and PromptSRC by 2.13% on base-to-novel generalization.

Conclusion: The proposed Dropout Prompt Learning effectively enhances vision-language model robustness across challenging scenarios by intelligently applying dropout to multimodal tokens and balancing alignment with representation diversity.
...
### [108] [JOCA: Task-Driven Joint Optimisation of Camera Hardware and Adaptive Camera Control Algorithms](https://arxiv.org/abs/2512.06763)
*Chengyang Yan,Mitch Bryson,Donald G. Dansereau*

Main category: cs.CV

TL;DR: Joint optimization of camera hardware and adaptive control algorithms for improved perception task performance using a hybrid gradient-based/derivative-free framework.


<details>
  <summary>Details</summary>
Motivation: Current camera system designs optimize fixed parameters at manufacturing, but many parameters (like exposure) require adaptive runtime control. Prior work focuses on static optimization, missing the opportunity to jointly optimize hardware and adaptive control algorithms for better perception performance.

Method: Proposes a unified optimization framework combining gradient-based and derivative-free methods to handle continuous/discrete parameters, non-differentiable image formation processes, and neural network-based adaptive control. Introduces DF-Grad, a hybrid strategy that trains adaptive control networks using signals from derivative-free optimizers alongside unsupervised task-driven learning.

Result: Method outperforms baselines that optimize static and dynamic parameters separately, especially under challenging conditions like low light and fast motion. Shows improved perception performance through joint hardware-adaptive control optimization.

Conclusion: Jointly optimizing camera hardware parameters and adaptive control algorithms improves perception performance and provides a unified approach to task-driven camera system design.
...
### [109] [DGGAN: Degradation Guided Generative Adversarial Network for Real-time Endoscopic Video Enhancement](https://arxiv.org/abs/2512.07253)
...
<details>
  <summary>Details</summary>
Motivation: Endoscopic surgery relies on video quality, but current videos suffer from uneven illumination, tissue scattering, occlusions, and motion blur that obscure critical anatomical details. Existing deep learning methods are too computationally demanding for real-time surgical use.

Method: Proposes a degradation-aware framework that extracts degradation representations using contrastive learning, then fuses these representations with image features to guide a single-frame enhancement model trained with cycle-consistency constraints between degraded and restored images.

Result: The framework achieves superior balance between performance and efficiency compared with state-of-the-art methods, demonstrating effectiveness for real-time endoscopic video enhancement.
...
### [110] [Effective Attention-Guided Multi-Scale Medical Network for Skin Lesion Segmentation](https://arxiv.org/abs/2512.07275)
...
Method: Innovative encoder-decoder network with multi-scale residual structures, Multi-Resolution Multi-Channel Fusion (MRCF) module for cross-scale features, Cross-Mix Attention Module (CMAM) for dynamic weight calculation, and External Attention Bridge (EAB) to address information loss in skip connections.
...
Conclusion: The proposed architecture effectively addresses challenges in skin lesion segmentation through multi-scale feature extraction, attention mechanisms, and information preservation, achieving state-of-the-art performance for medical image analysis.
...
### [111] [Physics Informed Human Posture Estimation Based on 3D Landmarks from Monocular RGB-Videos](https://arxiv.org/abs/2512.06783)
*Tobias Leuthold,Michele Xiloyannis,Yves Zimmermann*

Main category: cs.CV

TL;DR: Real-time post-processing algorithm improves BlazePose by fusing 2D/3D estimates with anatomical constraints via weighted optimization and Kalman filtering, reducing 3D joint position errors by 10.2% and angle errors by 16.6%.


<details>
  <summary>Details</summary>
Motivation: Automated coaching applications for physical training/therapy need accurate pose estimation, but current models like BlazePose lack anatomical constraints, indicating potential for improvement by incorporating physical knowledge.

Method: Real-time post-processing algorithm fuses BlazePose 3D and 2D estimations using weighted optimization that penalizes deviations from expected bone lengths and biomechanical models. Bone lengths are refined using Kalman filter with adaptive measurement trust.

Result: Evaluation on Physio2.2M dataset shows 10.2% reduction in 3D MPJPE (Mean Per Joint Position Error) and 16.6% decrease in errors of angles between body segments compared to BlazePose 3D estimation.

Conclusion: Method provides robust, anatomically consistent pose estimation suitable for automated physiotherapy, healthcare, and sports coaching on consumer-level devices, running on backend with anonymized data only.
...
### [112] [Towards Accurate UAV Image Perception: Guiding Vision-Language Models with Stronger Task Prompts](https://arxiv.org/abs/2512.07302)
*Mingning Guo,Mengwei Wu,Shaoxian Li,Haifeng Li,Chao Tao*

Main category: cs.CV

TL;DR: AerialVP is an agent framework that enhances task prompts for UAV image perception by extracting multi-dimensional auxiliary information, overcoming limitations of traditional VLM-based methods that struggle with target confusion, scale variations, and complex backgrounds in drone imagery.


<details>
  <summary>Details</summary>
Motivation: Traditional VLM-based image perception methods fail with UAV imagery due to challenges like target confusion, scale variations, and complex backgrounds. These issues arise because VLMs depend on semantic alignment between visual and textual tokens, which breaks down when task prompts are simplistic but image content is complex.

Method: AerialVP is an agent framework with three-stage enhancement process: (1) analyzes task prompt to identify task type and enhancement needs, (2) selects appropriate tools from tool repository, and (3) generates enhanced task prompts based on analysis and selected tools. Also introduces AerialSense benchmark for evaluation.

Result: Experimental results show AerialVP significantly enhances task prompt guidance, leading to stable and substantial performance improvements in both open-source and proprietary VLMs across diverse UAV image perception tasks.

Conclusion: AerialVP addresses critical limitations of traditional VLM-based approaches for UAV imagery by proactively enhancing task prompts with multi-dimensional auxiliary information, enabling better performance on challenging drone image perception tasks.
...
### [113] [Generalized Geometry Encoding Volume for Real-time Stereo Matching](https://arxiv.org/abs/2512.06793)
*Jiaxin Liu,Gangwei Xu,Xianqi Wang,Chengliang Zhang,Xin Yang*

Main category: cs.CV

TL;DR: GGEV: A real-time stereo matching network that achieves strong generalization by using depth-aware features and dynamic cost aggregation, outperforming existing real-time methods on zero-shot generalization and benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing real-time stereo methods focus on in-domain performance but lack generalization for real-world applications, while stereo foundation models have good generalization but suffer from high inference latency. There's a need for real-time methods with strong generalization capability.

Method: Proposes Generalized Geometry Encoding Volume (GGEV) with two key components: 1) extracting depth-aware features encoding domain-invariant structural priors, and 2) Depth-aware Dynamic Cost Aggregation (DDCA) module that adaptively incorporates these priors into each disparity hypothesis to enhance fragile matching relationships.
...
Conclusion: GGEV successfully addresses the trade-off between real-time performance and generalization in stereo matching, providing a lightweight yet effective solution that combines domain-invariant priors with adaptive cost aggregation for robust performance in unseen scenes.
...
### [114] [ContextAnyone: Context-Aware Diffusion for Character-Consistent Text-to-Video Generation](https://arxiv.org/abs/2512.07328)
*Ziyang Mai,Yu-Wing Tai*

Main category: cs.CV

TL;DR: ContextAnyone is a context-aware diffusion framework for character-consistent video generation from text and a single reference image, addressing identity drift by preserving hairstyle, outfit, and body shape cues.


<details>
  <summary>Details</summary>
Motivation: Existing T2V personalization methods focus mainly on facial identity but fail to preserve broader contextual cues like hairstyle, outfit, and body shape, which are critical for visual coherence across scenes.
...
### [115] [VDOT: Efficient Unified Video Creation via Optimal Transport Distillation](https://arxiv.org/abs/2512.06802)
*Yutong Wang,Haiyu Zhang,Tianfan Xue,Yu Qiao,Yaohui Wang,Chang Xu,Xinyuan Chen*

Main category: cs.CV

TL;DR: VDOT is an efficient unified video creation model using distribution matching distillation with optimal transport to enable fast 4-step generation that matches 100-step baselines.


<details>
  <summary>Details</summary>
Motivation: Existing video creation models are either limited to specific conditions or too slow for practical use due to complex inference, creating a need for efficient unified video generation.

Method: Uses distribution matching distillation with computational optimal transport instead of KL divergence to optimize discrepancy between real and fake score distributions, plus a discriminator for quality enhancement, and automated video data pipeline.

Result: 4-step VDOT outperforms or matches other baselines requiring 100 denoising steps, demonstrating superior efficiency and performance.

Conclusion: VDOT provides an efficient, unified video creation solution that addresses speed and versatility limitations of existing models through optimal transport-based distillation.
...
### [116] [DeepAgent: A Dual Stream Multi Agent Fusion for Robust Multimodal Deepfake Detection](https://arxiv.org/abs/2512.07351)
*Sayeem Been Zaman,Wasimul Karim,Arefin Ittesafun Abian,Reem E. Mohamed,Md Rafiqul Islam,Asif Karim,Sami Azam*

Main category: cs.CV

TL;DR: DeepAgent: Multi-agent framework combining visual CNN and audio-visual inconsistency detection with Random Forest fusion for robust deepfake detection across modalities.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods that integrate audio and visual cues in single models are vulnerable to modality mismatches, noise, and manipulation, creating a need for more robust multi-modal approaches.

Method: Two-agent framework: Agent-1 uses AlexNet-based CNN for visual manipulation detection; Agent-2 detects audio-visual inconsistencies using acoustic features, Whisper transcriptions, and EasyOCR frame reading. Decisions fused via Random Forest meta-classifier.

Result: Agent-1: 94.35% accuracy on Celeb-DF+FakeAVCeleb; Agent-2: 93.69% on FakeAVCeleb; Meta-classifier: 81.56% on FakeAVCeleb, 97.49% on DeepFakeTIMIT cross-dataset validation.

Conclusion: Hierarchy-based fusion enhances robustness by mitigating individual modality weaknesses, demonstrating multi-agent effectiveness against diverse deepfake manipulations.
...
### [117] [MMDuet2: Enhancing Proactive Interaction of Video MLLMs with Multi-Turn Reinforcement Learning](https://arxiv.org/abs/2512.06810)
*Yueqian Wang,Songxiang Liu,Disong Wang,Nuo Xu,Guanglu Wan,Huishuai Zhang,Dongyan Zhao*

Main category: cs.CV

TL;DR: MMDuet2: A proactive Video MLLM that autonomously decides when to respond during video streaming using multi-turn RL training, achieving SOTA on ProactiveVideoQA without needing precise response time annotations.


<details>
  <summary>Details</summary>
Motivation: Existing Video MLLMs operate in turn-based manner, but proactive interaction (deciding when to reply during video playback) is crucial for real-time applications. Previous methods require manual threshold tuning and precise reply time annotations, which are difficult to obtain.

Method: Proposes a text-to-text approach where model decides to respond or remain silent at each turn based on dialogue history and visual context. Uses multi-turn RL training to encourage timely responses without requiring precise response time annotations. Trained on 52k videos with two dialogue types via SFT and RL.
...
Conclusion: The proposed approach enables effective proactive interaction in Video MLLMs without the need for manual threshold tuning or precise response time annotations, advancing real-time video understanding applications.
...
TL;DR: Proposes structure-aware feature rectification using region adjacency graphs to improve CLIP's local discrimination for open-vocabulary semantic segmentation.


<details>
  <summary>Details</summary>
Motivation: CLIP's pre-training on image-text pairs focuses on global semantic alignment, leading to noisy and inconsistent predictions in local areas for open-vocabulary segmentation due to dispersed bias from contrastive training.

Method: Constructs region adjacency graph (RAG) based on low-level features (color, texture) to capture local structural relationships, then uses it to refine CLIP features by enhancing local discrimination.

Result: Method effectively suppresses segmentation noise, improves region-level consistency, and achieves strong performance on multiple open-vocabulary segmentation benchmarks.

Conclusion: Incorporating instance-specific priors through structure-aware feature rectification addresses CLIP's local discrimination limitations in open-vocabulary semantic segmentation.
...
### [119] [MeshSplatting: Differentiable Rendering with Opaque Meshes](https://arxiv.org/abs/2512.06818)
...
TL;DR: MeshSplatting bridges neural rendering and interactive 3D graphics by creating mesh-based representations from differentiable rendering that work with standard AR/VR/game engines, outperforming state-of-the-art mesh methods while being faster and more memory efficient.


<details>
  <summary>Details</summary>
Motivation: Current primitive-based splatting methods (like 3D Gaussian Splatting) use point-based representations that are incompatible with mesh-based pipelines used in AR/VR and game engines, creating a gap between neural rendering and interactive 3D graphics.

Method: MeshSplatting jointly optimizes geometry and appearance through differentiable rendering, enforces connectivity via restricted Delaunay triangulation, and refines surface consistency to create end-to-end smooth meshes.
...
Conclusion: MeshSplatting successfully bridges neural rendering and interactive 3D graphics, creating high-quality meshes that render efficiently in real-time 3D engines for seamless scene interaction.
...
TL;DR: Data-driven approach to discover mobility interaction patterns between individuals from real-world data, applied to cars and pedestrians.


<details>
  <summary>Details</summary>
Motivation: Existing human dynamics models rely on preconceived behavioral assumptions rather than learning from actual data. Understanding how individuals influence each other's movements is crucial for applications like crowd simulation and emergency management.

Method: Data mining approach that searches for mobility events indicating mutual interactions between individuals, then identifies complex, persistent patterns and time-evolving configurations of these events from real-world data.

Result: Methodology successfully applied to two real case studies (cars and pedestrians) with full experimental evaluation including performance analysis, parameter sensitivity testing, and interpretation of sample results.

Conclusion: The data-driven approach can reveal new insights into mobility interaction mechanics that could improve existing simulation models, moving beyond preconceived behavioral assumptions.
...
### [121] [SparseCoop: Cooperative Perception with Kinematic-Grounded Queries](https://arxiv.org/abs/2512.06838)
...
TL;DR: SparseCoop is a fully sparse cooperative perception framework for 3D detection and tracking that eliminates BEV representations, achieving state-of-the-art performance with high efficiency and low communication costs.


<details>
  <summary>Details</summary>
Motivation: Current cooperative perception methods face limitations: dense BEV feature sharing has quadratic communication costs and lacks flexibility for precise alignment across asynchronous viewpoints, while sparse query-based methods suffer from inadequate geometric representations, suboptimal fusion, and training instability.

Method: Three key innovations: 1) Kinematic-grounded instance query using explicit state vector with 3D geometry and velocity for precise spatio-temporal alignment, 2) Coarse-to-fine aggregation module for robust fusion, 3) Cooperative instance denoising task to accelerate and stabilize training.
...
Conclusion: SparseCoop demonstrates that fully sparse cooperative perception without BEV representations can achieve excellent performance while addressing key limitations of current approaches in communication efficiency, flexibility, and training stability.
...
### [122] [When normalization hallucinates: unseen risks in AI-powered whole slide image processing](https://arxiv.org/abs/2512.07426)
*Karel Moens,Matthew B. Blaschko,Tinne Tuytelaars,Bart Diricx,Jonas De Vylder,Mustafa Yousif*

Main category: cs.CV

TL;DR: Deep learning-based WSI normalization methods can introduce undetectable hallucinations that threaten clinical analysis, requiring new detection measures and validation protocols.


<details>
  <summary>Details</summary>
Motivation: Current WSI normalization methods using deep learning tend to produce average-looking outputs that may mask important diagnostic features and introduce undetectable hallucinated artifacts, posing serious risks to downstream clinical analysis that are often overlooked in evaluation.

Method: Proposed a novel image comparison measure specifically designed to automatically detect hallucinations in normalized outputs, then systematically evaluated several well-cited normalization methods retrained on real-world clinical data using this measure.

Result: Found that while many methods perform adequately on public datasets, they show concerning frequency of hallucinations when retrained on real-world clinical data, revealing significant inconsistencies and failures not captured by conventional metrics.

Conclusion: The risk of hallucinations in WSI normalization is real and underappreciated, underscoring the need for more robust, interpretable normalization techniques and stricter validation protocols for clinical deployment.
...
### [123] [CADE: Continual Weakly-supervised Video Anomaly Detection with Ensembles](https://arxiv.org/abs/2512.06840)
*Satoshi Hashimoto,Tatsuya Konishi,Tomoya Kaichi,Kazunori Matsumoto,Mori Kurokawa*

Main category: cs.CV

TL;DR: First work combining continual learning with weakly-supervised video anomaly detection to handle domain shifts across multiple scenes while preventing forgetting of previous anomalies.


<details>
  <summary>Details</summary>
Motivation: Existing weakly-supervised VAD methods focus on static datasets and neglect domain shifts across different scenes. When new data arrives, models suffer from catastrophic forgetting of previous anomalies, requiring a continual learning approach.

Method: Proposes CADE with Dual-Generator to handle data imbalance and label uncertainty, and Multi-Discriminator ensemble to capture missed anomalies from forgetting. Combines continual learning with weakly-supervised VAD.

Result: Significantly outperforms existing VAD methods on multi-scene datasets like ShanghaiTech and Charlotte Anomaly datasets, effectively handling domain shifts while preventing forgetting.

Conclusion: CADE successfully addresses domain-shift challenges in video anomaly detection through continual learning, preventing forgetting and incompleteness while maintaining strong performance across multiple scenes.
...
### [124] [Pseudo Anomalies Are All You Need: Diffusion-Based Generation for Weakly-Supervised Video Anomaly Detection](https://arxiv.org/abs/2512.06845)
*Satoshi Hashimoto,Hitoshi Nishimura,Yanan Wang,Mori Kurokawa*

Main category: cs.CV

TL;DR: PA-VAD trains video anomaly detectors using only real normal videos and synthesized pseudo-abnormal videos, achieving state-of-the-art performance without real abnormal footage.


<details>
  <summary>Details</summary>
Motivation: Real-world deployment of video anomaly detection is limited by the scarcity and high collection cost of real abnormal footage, creating a practical barrier to scalable implementation.

Method: Uses CLIP to select class-relevant initial images, refines textual prompts with vision-language models for fidelity, generates pseudo-abnormal videos with video diffusion models, and employs domain-aligned regularization to mitigate excessive spatiotemporal magnitude in synthesized anomalies.

Result: Achieves 98.2% on ShanghaiTech (surpassing strongest real-abnormal method by +0.6%) and 82.5% on UCF-Crime (outperforming UVAD state-of-the-art by +1.9%).

Conclusion: High-accuracy anomaly detection can be achieved without collecting real anomalies, providing a practical path toward scalable deployment of video anomaly detection systems.
...
### [125] [Toward More Reliable Artificial Intelligence: Reducing Hallucinations in Vision-Language Models](https://arxiv.org/abs/2512.07564)
*Kassoum Sanogo,Renzo Ardiccioni*

Main category: cs.CV

TL;DR: Training-free self-correction framework reduces VLM hallucinations by 9.8% through uncertainty-guided visual re-attention without model updates.


<details>
  <summary>Details</summary>
Motivation: Vision-language models frequently generate plausible but incorrect hallucinated content about images, requiring methods to improve factual grounding without expensive retraining.

Method: Combines multidimensional uncertainty quantification (token entropy, attention dispersion, semantic consistency, claim confidence) with attention-guided cropping of under-explored regions, operating entirely with frozen pretrained VLMs.

Result: Reduces hallucination rates by 9.8 percentage points compared to baseline, improves object existence accuracy by 4.7 points on adversarial splits, and successfully grounds corrections in visual evidence.

Conclusion: The training-free self-correction framework effectively reduces VLM hallucinations through uncertainty-guided re-attention, providing a practical approach for trustworthy multimodal systems without model updates.
...
### [126] [Hide-and-Seek Attribution: Weakly Supervised Segmentation of Vertebral Metastases in CT](https://arxiv.org/abs/2512.06849)
...
<details>
  <summary>Details</summary>
Motivation: Vertebral metastasis segmentation in CT is clinically important but difficult to scale due to scarce voxel-level annotations and similarity between malignant lesions and benign degenerative changes.

Method: Combines Diffusion Autoencoder (DAE) that produces classifier-guided healthy edits of vertebrae with pixel-wise difference maps for candidate lesion regions. Uses Hide-and-Seek Attribution: each candidate region is revealed while others are hidden, edited image is projected back to data manifold, and latent-space classifier quantifies isolated malignant contribution to form final segmentation.

Result: Achieved strong blastic/lytic performance on held-out radiologist annotations despite no mask supervision (F1: 0.91/0.85; Dice: 0.87/0.78), significantly exceeding baselines (F1: 0.79/0.67; Dice: 0.74/0.55).

Conclusion: Vertebra-level labels can be transformed into reliable lesion masks, demonstrating that generative editing combined with selective occlusion supports accurate weakly supervised segmentation in CT.
...
### [127] [Dual-Stream Cross-Modal Representation Learning via Residual Semantic Decorrelation](https://arxiv.org/abs/2512.07568)
...
TL;DR: DSRSD-Net disentangles modality-specific and shared information via residual decomposition and decorrelation to address modality dominance and spurious correlations in multimodal learning.


<details>
  <summary>Details</summary>
Motivation: Multimodal representations suffer from modality dominance, redundant information coupling, and spurious cross-modal correlations, leading to poor generalization and limited interpretability. High-variance modalities overshadow weaker signals, and naive fusion entangles shared/specific factors, making predictions hard to interpret and vulnerable to noisy/missing modalities.

Method: Dual-Stream Residual Semantic Decorrelation Network (DSRSD-Net) with: (1) dual-stream representation learning separating intra-modal (private) and inter-modal (shared) factors via residual projection; (2) residual semantic alignment mapping shared factors to common space using contrastive+regression objectives; (3) decorrelation and orthogonality loss regularizing covariance structure and enforcing orthogonality between shared/private streams.
...
Conclusion: DSRSD-Net effectively addresses modality dominance and spurious correlations through disentangled representation learning with explicit decorrelation constraints, improving both prediction performance and interpretability in multimodal learning.
...
### [128] [Omni-Referring Image Segmentation](https://arxiv.org/abs/2512.06862)
*Qiancheng Zheng,Yunhang Shen,Gen Luo,Baiyang Song,Xing Sun,Xiaoshuai Sun,Yiyi Zhou,Rongrong Ji*

Main category: cs.CV

TL;DR: Proposes OmniRIS, a novel task for highly generalized image segmentation that accepts both text instructions and visual prompts (masks, boxes, scribbles) as omni-prompts, enabling granular attribute referring and uncommon object grounding across various segmentation settings.


<details>
  <summary>Details</summary>
Motivation: Existing segmentation tasks are unimodally conditioned (text-only or visual-only), limiting their ability to exploit the complementary strengths of both modalities. There's a need for a more flexible, generalized segmentation approach that can handle diverse input modalities and segmentation scenarios.

Method: Introduces OmniRIS task definition, creates OmniRef dataset (186,939 omni-prompts for 30,956 images), develops OmniSegNet baseline with omni-prompt encoding capabilities, and establishes comprehensive evaluation system.
...
Conclusion: OmniRIS represents a significant advancement toward highly generalized image segmentation by supporting both text and visual modalities as omni-prompts, enabling more flexible and practical segmentation applications while leveraging complementary strengths of different input types.
...
### [129] [An AI-Powered Autonomous Underwater System for Sea Exploration and Scientific Research](https://arxiv.org/abs/2512.07652)
...
Method: Integrated system using YOLOv12 Nano for real-time object detection, ResNet50 CNN for feature extraction, PCA for dimensionality reduction (preserving 98% variance), K-Means++ for clustering similar marine objects, and GPT-4o Mini LLM for generating structured reports.

Result: System achieved mAP@0.5 of 0.512, precision 0.535, recall 0.438 on combined DeepFish and OzFish datasets (55K+ images); PCA reduced features while preserving variance; clustering grouped objects effectively; LLM generated insightful summaries with location data.

Conclusion: The integrated AI approach reduces human diving risks, increases mission efficiency, enhances underwater data analysis speed and depth, enabling more effective scientific research in challenging marine environments.
...
### [130] [Boosting Unsupervised Video Instance Segmentation with Automatic Quality-Guided Self-Training](https://arxiv.org/abs/2512.06864)
...
<details>
  <summary>Details</summary>
Motivation: Video Instance Segmentation requires both pixel-level masks and temporal consistency labels, which are expensive to annotate. Existing unsupervised methods like VideoCutLER use synthetic data but suffer from synthetic-to-real domain gap limitations.
...
Result: Achieves state-of-the-art performance with 52.6 AP₅₀ on YouTubeVIS-2019 val set, surpassing previous state-of-the-art VideoCutLER by 4.4%, while requiring no human annotations.

Conclusion: The framework demonstrates the viability of quality-aware self-training for unsupervised Video Instance Segmentation, effectively bridging the synthetic-to-real domain gap.
...
### [131] [DIST-CLIP: Arbitrary Metadata and Image Guided MRI Harmonization via Disentangled Anatomy-Contrast Representations](https://arxiv.org/abs/2512.07674)
...
<details>
  <summary>Details</summary>
Motivation: Deep learning for medical image analysis faces clinical generalization challenges due to data heterogeneity, especially in MRI where scanner differences, acquisition protocols, and sequence parameters create domain shifts that obscure biological signals. Existing harmonization methods are insufficient - image-based approaches need target images, while text-guided methods use simplistic labels or work only on limited datasets.

Method: DIST-CLIP disentangles anatomical content from image contrast, extracts contrast representations using pre-trained CLIP encoders, and integrates them into anatomical content via a novel Adaptive Style Transfer module. The framework flexibly uses either target images or DICOM metadata for guidance.

Result: Trained and evaluated on diverse real-world clinical datasets, DIST-CLIP showed significant improvements over state-of-the-art methods in both style translation fidelity and anatomical preservation.

Conclusion: DIST-CLIP offers a flexible solution for MRI style transfer and standardization, addressing real-world clinical heterogeneity by using either target images or DICOM metadata, with code and weights to be made publicly available.
...
### [132] [Spatial Retrieval Augmented Autonomous Driving](https://arxiv.org/abs/2512.06865)
...
TL;DR: Proposes spatial retrieval paradigm using offline geographic images from maps to enhance autonomous driving perception beyond onboard sensors.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems are limited by onboard sensors' perception horizon and fail under occlusion, darkness, or poor visibility. Human drivers can recall road structure even in poor conditions, so the paper aims to give models similar "recall" ability.

Method: Introduces spatial retrieval paradigm using offline geographic images from sources like Google Maps as additional input. Extends nuScenes dataset with aligned geographic images and establishes baselines across five core AD tasks.

Result: Extensive experiments show the extended modality enhances performance of certain autonomous driving tasks. The approach is plug-and-play without requiring additional sensors.

Conclusion: Spatial retrieval using offline geographic images provides a promising paradigm for enhancing autonomous driving perception, with open-source dataset and benchmarks provided for further research.
...
### [133] [Guiding What Not to Generate: Automated Negative Prompting for Text-Image Alignment](https://arxiv.org/abs/2512.07702)
*Sangha Park,Eunji Kim,Yeongtak Oh,Jooyoung Choi,Sungroh Yoon*

Main category: cs.CV

TL;DR: NPC improves text-to-image alignment by automatically identifying and applying negative prompts to suppress unintended content in diffusion models.


<details>
  <summary>Details</summary>
Motivation: Text-to-image generation struggles with precise alignment, especially for complex or imaginative prompts, due to unintended content in generated images.

Method: NPC analyzes cross-attention patterns to understand why both targeted and untargeted negative prompts work, then uses a verifier-captioner-proposer framework to generate candidate negative prompts ranked by a salient text-space score.

Result: NPC outperforms baselines on GenEval++ (0.571 vs 0.371) and achieves best overall performance on Imagine-Bench, improving text-image alignment.

Conclusion: NPC provides a principled, fully automated approach to enhance text-image alignment in diffusion models by guiding what not to generate through negative prompting.
...
### [134] [Improving action classification with brain-inspired deep networks](https://arxiv.org/abs/2512.07729)
*Aidas Aglinskas,Stefano Anzellotti*

Main category: cs.CV

TL;DR: Brain-inspired DNN with separate body/scene streams improves action recognition and matches human performance patterns better than standard DNNs.


<details>
  <summary>Details</summary>
Motivation: Standard DNNs may rely too heavily on either body or background information for action recognition, while humans have specialized brain regions for both. The study investigates whether brain-inspired architectures with separate domain-specific streams can achieve more human-like performance.

Method: 1) Tested standard DNNs on HAA500 dataset with three stimulus versions (full, body-only, background-only). 2) Compared with human participants (N=28) on same stimuli. 3) Implemented novel brain-inspired architecture with separate streams for body and background processing.

Result: Standard DNNs performed well on full and background-only stimuli but failed on body-only stimuli. Humans performed well on all three versions, better on body-only than background-only. The brain-inspired architecture improved overall performance and matched human accuracy patterns more closely.

Conclusion: Incorporating brain-like domain-specific processing streams (separate for body and scene perception) improves DNN action recognition performance and makes it more human-like, suggesting benefits from neuroscience-inspired architectural design.
...
### [135] [Towards Robust Pseudo-Label Learning in Semantic Segmentation: An Encoding Perspective](https://arxiv.org/abs/2512.06870)
...
<details>
  <summary>Details</summary>
Motivation: Pseudo-label learning in semantic segmentation suffers from error amplification due to one-hot encoding, especially in label-scarce scenarios like UDA and SSL where erroneous pseudo-labels degrade performance.

Method: Proposes ECOCSeg using error-correcting output codes to create fine-grained class encodings, with an ECOC-based classifier that disentangles classes into attributes and a bit-level label denoising mechanism for higher-quality pseudo-labels.
...
Conclusion: ECOCSeg effectively addresses pseudo-label noise in segmentation through ECOC encoding, providing a robust solution that can be easily integrated with existing methods for improved performance in label-scarce scenarios.
...
### [136] [SAVE: Sparse Autoencoder-Driven Visual Information Enhancement for Mitigating Object Hallucination](https://arxiv.org/abs/2512.07730)
*Sangha Park,Seungryong Yoo,Jisoo Mok,Sungroh Yoon*

Main category: cs.CV

TL;DR: SAVE uses sparse autoencoder features to identify and steer MLLMs toward better visual understanding, reducing object hallucination by 10%p on CHAIR_S benchmark.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs suffer from object hallucination due to language priors and visual information loss, needing better grounding in visual understanding.

Method: Proposes SAVE framework that uses binary object-presence QA probe to identify visual understanding features in SAE latent space, then steers model along these features.

Result: Outperforms SOTA training-free methods with 10%p improvement on CHAIR_S, consistent gains on POPE and MMHal-Bench; suppresses uncertain object tokens and increases attention to image tokens.

Conclusion: SAVE effectively reduces hallucination by reinforcing grounded visual understanding through SAE feature steering, showing robustness across models and layers.
...
### [137] [SceneMixer: Exploring Convolutional Mixing Networks for Remote Sensing Scene Classification](https://arxiv.org/abs/2512.06877)
*Mohammed Q. Alkhatib,Ali Jamali,Swalpa Kumar Roy*

Main category: cs.CV

TL;DR: Lightweight convolutional mixer architecture for remote sensing scene classification achieves good accuracy-efficiency balance on AID and EuroSAT benchmarks.


<details>
  <summary>Details</summary>
Motivation: Remote sensing scene classification is challenging due to variations in spatial resolution, viewpoint, orientation, and background conditions that reduce model generalization. Existing CNN and ViT models struggle with these challenges while maintaining efficiency.

Method: Proposes a lightweight architecture based on convolutional mixer paradigm that alternates between spatial mixing through multi-scale depthwise convolutions and channel mixing through pointwise operations for efficient local and contextual feature extraction.

Result: Achieved 74.7% overall accuracy, 74.57% average accuracy, and 73.79 Kappa on AID dataset; 93.90%, 93.93%, and 93.22 on EuroSAT respectively, demonstrating good accuracy-efficiency balance compared to CNN and transformer models.

Conclusion: The proposed convolutional mixer architecture provides an effective lightweight solution for remote sensing scene classification that balances accuracy and computational efficiency, addressing generalization challenges in aerial and satellite imagery analysis.
...
### [138] [WorldReel: 4D Video Generation with Consistent Geometry and Motion Modeling](https://arxiv.org/abs/2512.07821)
*Shaoheng Fang,Hanwen Jiang,Yunpeng Bai,Niloy J. Mitra,Qixing Huang*

Main category: cs.CV

TL;DR: WorldReel is a 4D video generator that produces spatio-temporally consistent videos with explicit 4D scene representations including pointmaps, camera trajectory, and dense flow mapping, trained on a blend of synthetic and real data.


<details>
  <summary>Details</summary>
Motivation: Current video generators achieve photorealism but lack 3D consistency. The paper aims to create a video generation system that maintains spatio-temporal consistency across viewpoints and dynamic content, enabling coherent geometry and appearance modeling over time.

Method: WorldReel jointly generates RGB frames with 4D scene representations (pointmaps, camera trajectory, dense flow mapping). It's trained using a combination of synthetic data (providing precise 4D supervision for geometry, motion, and camera) and real videos (contributing visual diversity and realism).

Result: WorldReel sets a new state-of-the-art for consistent video generation with dynamic scenes and moving cameras, improving metrics of geometric consistency, motion coherence, and reducing view-time artifacts compared to competing methods.

Conclusion: WorldReel advances video generation toward 4D-consistent world modeling, where agents can render, interact, and reason about scenes through a single stable spatiotemporal representation, bringing video generation closer to true world modeling.
...
### [139] [Hierarchical Image-Guided 3D Point Cloud Segmentation in Industrial Scenes via Multi-View Bayesian Fusion](https://arxiv.org/abs/2512.06882)
*Yu Zhu,Naoya Chiba,Koichi Hashimoto*

Main category: cs.CV

TL;DR: Hierarchical image-guided 3D segmentation framework that uses 2D vision models (SAM + YOLO-World) to progressively refine segmentation from instance to part level, addressing occlusion and scale issues in industrial environments.


<details>
  <summary>Details</summary>
Motivation: Industrial scenes have dense layouts with multi-scale objects where heavy occlusion weakens geometric boundaries and scale differences cause end-to-end models to fail at capturing both coarse and fine details. Existing methods require costly annotations or suffer from semantic inconsistencies across views.

Method: Two-stage hierarchical approach: 1) Instance segmentation by rendering top-view image, using YOLO-World to prompt SAM for mask generation, then back-projecting to 3D point cloud. 2) Part-level segmentation by rendering multi-view images of each instance, applying same 2D segmentation at each view, and using Bayesian updating fusion for semantic consistency across views.
...
Conclusion: Proposed hierarchical image-guided framework provides reliable 3D segmentation for complex industrial scenes, addressing challenges of occlusion, scale variation, and semantic consistency while being annotation-efficient and generalizable.
...
### [140] [One Layer Is Enough: Adapting Pretrained Visual Encoders for Image Generation](https://arxiv.org/abs/2512.07829)
*Yuan Gao,Chen Chen,Tianrong Chen,Jiatao Gu*

Main category: cs.CV

TL;DR: FAE is a simple framework that adapts pre-trained visual representations into low-dimensional latents suitable for generation using minimal architecture (single attention layer), achieving state-of-the-art results on image generation benchmarks.


<details>
  <summary>Details</summary>
Motivation: There's a fundamental mismatch between understanding-oriented visual representations (high-dimensional, diverse hypotheses) and generation-friendly latent spaces (low-dimensional, noise-preserving). Prior work requires complex objectives and architectures to bridge this gap.

Method: FAE uses two separate deep decoders: one reconstructs the original feature space, and another takes those reconstructed features for image generation. It works with various self-supervised encoders (DINO, SigLIP) and can be plugged into diffusion models or normalizing flows.

Result: Achieves near state-of-the-art FID of 1.29 (with CFG) and state-of-the-art FID of 1.48 (without CFG) on ImageNet 256x256 at 800 epochs, with strong performance at 80 epochs (1.70 with CFG, 2.08 without CFG).

Conclusion: FAE provides a simple yet effective generic framework for adapting pre-trained visual representations to generative models, achieving high-quality results with fast learning across different generative families and benchmarks.
...
### [141] [Relational Visual Similarity](https://arxiv.org/abs/2512.07833)
*Thao Nguyen,Sicheng Mo,Krishna Kumar Singh,Yilin Wang,Jing Shi,Nicholas Kolkin,Eli Shechtman,Yong Jae Lee,Yuheng Li*

Main category: cs.CV

TL;DR: Proposes a new method to measure relational similarity between images (beyond just visual attributes) by fine-tuning vision-language models on anonymized captions describing relational logic.


<details>
  <summary>Details</summary>
Motivation: Current visual similarity metrics (LPIPS, CLIP, DINO) only capture perceptual attribute similarity but fail to capture relational similarity that humans perceive (e.g., Earth-peach analogy). There's a critical gap in visual computing for measuring relational properties beyond surface content.

Method: 1) Formulate relational image similarity as measurable problem; 2) Curate 114k image-caption dataset with anonymized captions describing underlying relational logic; 3) Fine-tune vision-language model on this dataset to measure relational similarity between images.

Result: Developed first model that connects images by their underlying relational structure rather than visible appearance. Shows existing image similarity models fail to capture relational similarity, revealing a critical gap in visual computing.

Conclusion: Relational similarity has important real-world applications, and the proposed approach represents a first step toward bridging the gap between human relational perception and computational image similarity measurement.
...
TL;DR: BLDA addresses class imbalance in UDA semantic segmentation by analyzing predicted logits distributions, aligning them across classes using anchor distributions, and incorporating logits correction into self-training losses.


<details>
  <summary>Details</summary>
Motivation: Self-training in UDA for semantic segmentation suffers from class imbalance due to distribution shifts between domains, leading to biased learning where some classes are over-predicted while others are under-predicted.
...
Conclusion: BLDA effectively addresses class imbalance in UDA semantic segmentation without requiring prior knowledge about distribution shifts, demonstrating robust performance improvements across different methods.
...
TL;DR: First reproducible infant respiration estimation pipeline using new AIR-400 dataset with 400 annotated videos, addressing lack of infant-specific vision algorithms for early detection of breathing irregularities.


<details>
  <summary>Details</summary>
Motivation: Current computer vision algorithms and datasets for respiration monitoring only exist for adults, leaving a critical gap for infants where breathing irregularities are associated with neurodevelopmental impairments and SIDS. Only one small public infant dataset exists with no reproducible algorithms.

Method: Created AIR-400 dataset with 400 annotated videos (275 new videos from 10 subjects). Developed reproducible pipelines using infant-specific region-of-interest detection and spatiotemporal neural processing enhanced by optical flow inputs.
...
Conclusion: This work enables advances in early detection and treatment of infant breathing irregularities by providing the first comprehensive dataset and reproducible algorithms for contactless infant respiration monitoring.
...
TL;DR: Saber is a zero-shot reference-to-video generation framework that eliminates the need for expensive reference image-video-text triplets by training only on video-text pairs, using masked training and attention mechanisms to preserve subject identity.


<details>
  <summary>Details</summary>
Motivation: Current R2V methods require expensive explicit reference image-video-text triplets that are difficult to scale, creating a bottleneck for practical applications.

Method: Uses masked training strategy and attention-based model design trained exclusively on video-text pairs, with mask augmentation to reduce copy-paste artifacts.
...
Conclusion: Saber provides a scalable zero-shot solution for reference-to-video generation that bypasses the data bottleneck of existing methods while maintaining identity consistency and reference awareness.
...
TL;DR: NTRM introduces a tissue-level graph neural network to model spatial and functional relationships between tissues in histopathology segmentation, outperforming CNN-based methods by 4.9-31.25% on skin cancer datasets.


<details>
  <summary>Details</summary>
Motivation: Current CNN-based histopathology segmentation methods focus on visual texture and treat tissues as independent regions, failing to capture biological context and inter-tissue relationships, especially in areas with overlapping or morphologically similar tissues.

Method: Neural Tissue Relation Modeling (NTRM) augments CNNs with a tissue-level graph neural network that constructs graphs over predicted regions, propagates contextual information via message passing, and refines segmentation through spatial projection to model spatial and functional relationships.

Result: On the Histopathology Non-Melanoma Skin Cancer Segmentation Dataset, NTRM outperforms state-of-the-art methods, achieving a Dice similarity coefficient 4.9% to 31.25% higher than the best-performing models among evaluated approaches.

Conclusion: Relational modeling provides a principled approach for more context-aware and interpretable histological segmentation compared to local receptive-field architectures, enabling structurally coherent predictions in boundary-dense zones.
...
<details>
  <summary>Details</summary>
Motivation: To improve self-supervised pretraining for semantic segmentation by replacing random masking with selective masking, especially for scenarios with limited model capacity and computational resources.

Method: Selective masking image reconstruction that iteratively masks patches with highest reconstruction loss, leveraging trained model's knowledge rather than random masking.

Result: Outperforms random masking and supervised ImageNet pretraining by 2.9% on Pascal VOC/Cityscapes and 2.5% on weed datasets; improves lowest-performing classes; best results when using same pretraining and downstream dataset.

Conclusion: Selective Masking Image Reconstruction provides effective solution for improving semantic segmentation workflows, particularly for resource-constrained scenarios requiring limited model capacity.
...
TL;DR: The paper introduces a new fidelity metric for SR models to measure high-level content preservation, creates an annotated dataset, evaluates SOTA models, analyzes existing metrics, and shows foundation models can better address this task while improving both fidelity and perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Current SR models can hallucinate and change image content despite high visual quality, but existing low-level metrics don't measure these high-level fidelity issues that humans easily identify.

Method: Constructed first annotated dataset with fidelity scores from different SR models, evaluated SOTA models, analyzed correlation of existing metrics with fidelity, used foundation models for better fidelity measurement, and fine-tuned SR models based on fidelity feedback.

Result: Foundation models better address high-level fidelity measurement, and fine-tuning SR models based on fidelity feedback improves both semantic fidelity and perceptual quality.

Conclusion: High-level fidelity measurement is crucial for evaluating SR model reliability, serves as complementary criterion to existing metrics, and has potential value for both model evaluation and optimization.
...
TL;DR: Flexible compression scheme for 3D Gaussian Splatting that supports interpolation at any rate between bounds, enabling dynamic rate control without retraining.


<details>
  <summary>Details</summary>
Motivation: 3DGS enables real-time photorealistic rendering but suffers from large memory requirements and costly training. Existing compression approaches operate at fixed rates, limiting adaptability to varying bandwidth and device constraints.

Method: Proposes a flexible compression scheme for 3DGS that supports interpolation at any rate between predefined bounds. The method is computationally lightweight and requires no retraining for any rate.

Result: Achieves efficient, high-quality compression while offering dynamic rate control across a broad range of operating points, preserving rendering quality.

Conclusion: The approach makes 3DGS compression suitable for practical deployment in immersive applications by providing flexible rate adaptation without quality degradation.
...
TL;DR: Using discrete Fourier transform with persistent homology to extract frequency-based topological features for image compression, achieving JPEG-comparable compression while improving noise robustness.


<details>
  <summary>Details</summary>
Motivation: Address challenges in feature extraction from noisy image datasets to improve model reliability by developing methods that can differentiate meaningful data from noise during compression.

Method: Combine discrete Fourier transform with persistent homology analysis to extract specific frequencies corresponding to topological features, enabling image compression while preserving meaningful data.

Result: Achieved compression comparable to JPEG across six metrics, with potential to improve binary classification performance when augmenting CNNs compared to traditional methods.
...
<details>
  <summary>Details</summary>
Motivation: Current camouflage evaluation metrics overlook context-dependence and assume uncorrelated spatial context, despite camouflage being primarily context-dependent. Existing metrics were designed for general/salient objects and don't align well with human perception of camouflage.

Method: Develops Context-measure based on a probabilistic pixel-aware correlation framework that incorporates spatial dependencies and pixel-wise camouflage quantification.
...
Method: DFIR-DETR introduces three novel components: 1) DCFA module with dynamic K-sparse attention (reducing complexity from O(N²) to O(NK)) and spatial gated linear units for better nonlinear modeling; 2) DFPN module with amplitude-normalized upsampling to prevent feature inflation and dual-path shuffle convolution to retain spatial details; 3) FIRC3 module operating in frequency domain for global receptive fields without sacrificing efficiency.

Result: Achieved state-of-the-art mAP50 scores of 92.9% on NEU-DET dataset and 51.6% on VisDrone dataset. The model remains lightweight with only 11.7M parameters and 41.2 GFLOPs, demonstrating strong performance across different domains.

Conclusion: DFIR-DETR effectively addresses key challenges in small object detection through dynamic feature aggregation and frequency-domain processing, generalizes well across different application domains (UAV remote sensing and industrial inspection), and works efficiently in resource-limited settings for cross-scene small object detection.
...
TL;DR: COREA is a unified framework that jointly learns relightable 3D Gaussians and SDF for accurate geometry and faithful relighting, using 3D-to-3D alignment to overcome limitations of 2D-based geometry learning.


<details>
  <summary>Details</summary>
Motivation: Recent 3D Gaussian Splatting methods have limitations: their geometry is learned from 2D renderings, leading to coarse surfaces and unreliable BRDF-lighting decomposition. There's a need for more accurate geometry reconstruction and stable physically-based rendering.

Method: COREA introduces a coarse-to-fine bidirectional 3D-to-3D alignment strategy: depth provides coarse alignment between 3D Gaussians and SDF, while depth gradients and normals refine fine-scale structure. A density-control mechanism stabilizes Gaussian growth for memory efficiency.
...
TL;DR: Proposes MSN, a two-stream network for copy-move forgery detection using multi-directional CNN encoding and 2-D similarity matrix decoding, with a new deep-synthesized forgery benchmark.


<details>
  <summary>Details</summary>
Motivation: Copy-move forgery detection is challenging due to complex transformations and fine-tuned operations, especially with deep generative networks. Existing methods have limitations in representation and localization.

Method: Multi-directional Similarity Network (MSN) with two streams: 1) Hierarchical multi-directional CNN encoding for better patch similarity measurement, 2) 2-D similarity matrix decoder that utilizes full spatial information instead of 1-D vectors.

Result: State-of-the-art performance on CASIA CMFD, CoMoFoD benchmarks, and a newly created deep-synthesized copy-move forgery database.

Conclusion: MSN effectively addresses representation and localization limitations in copy-move forgery detection, demonstrating superior performance across traditional and deep-synthesized forgery datasets.
...
TL;DR: CSC-VTON improves virtual try-on by using energy constraints on attention maps to better preserve clothing details, and introduces a new evaluation metric VTID for comprehensive assessment.


<details>
  <summary>Details</summary>
Motivation: Existing VTON methods fail to preserve target clothing details (patterns, textures, boundaries) in generated results, and current evaluation metrics focus only on image realism while ignoring alignment with target clothing elements.

Method: Proposes using an energy function to constrain attention maps during generation, focusing attention on clothing regions to better preserve target details. Also introduces Virtual Try-on Inception Distance (VTID) as a new evaluation metric.

Result: Outperforms SOTA methods by 1.4% (LPIPS), 2.3% (FID), 12.3% (KID), and 5.8% (VTID) on VITON-HD and DressCode datasets. Generated data improves downstream CC-Reid performance by 2.5%, 1.1%, and 1.6% on LTCC, PRCC, VC-Clothes datasets.

Conclusion: The energy-constrained attention approach effectively preserves clothing details in virtual try-on, and the VTID metric provides more comprehensive evaluation. The method benefits downstream tasks like clothing-change re-identification.
...
TL;DR: MulCLIP improves vision-language alignment for long, detailed text by using multi-level alignment strategies without region proposals, achieving better fine-grained understanding with lower deployment costs.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models like CLIP struggle with lengthy, detailed descriptions because they're trained on short captions. Region-proposal approaches help but add deployment costs.

Method: End-to-end multi-level alignment framework: 1) Global contrastive alignment with extended positional embeddings, 2) Token reconstruction alignment for semantic connections, 3) Subcaption-aggregated patch alignment for context-rich patch extraction.

Result: Consistent improvement across diverse benchmarks, better fine-grained capability than region-proposal approaches, and suitability for real-world applications.

Conclusion: MulCLIP's multi-scale alignment effectively bridges long-text structures with image components without region proposals, offering better performance and practical deployment advantages.
...
TL;DR: TRR is a three-stage training framework that enhances LVLM safety through policy-guided self-reflection, improving safe response rates from 42.8% to 87.7% while maintaining general performance.


<details>
  <summary>Details</summary>
Motivation: Single-pass think-then-answer paradigms in LVLMs remain vulnerable to jailbreak attacks and may overlook harmful content in their own outputs, revealing a need for genuine self-correction mechanisms.

Method: Three-stage framework: 1) Build ReSafe dataset with 5,000 think-reflect-revise examples, 2) Fine-tune model to initialize reflective behavior, 3) Reinforce policy-guided reflection through reinforcement learning.

Result: TRR substantially improves safety performance, increasing safe response rate from 42.8% to 87.7% on Qwen2.5-VL-7B, while maintaining stable performance on general benchmarks like MMMU and MMStar.

Conclusion: The reflection-based approach effectively leverages revealed malicious content for genuine self-correction, providing a robust solution to enhance LVLM safety alignment against jailbreak attacks.
...
TL;DR: CHIMERA: A zero-shot diffusion framework for smooth image morphing using adaptive cache injection and semantic anchor prompting


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based image morphing methods often produce abrupt transitions or over-saturated appearances due to lack of adaptive structural and semantic alignments between dissimilar input images.

Method: Formulates morphing as cached inversion-guided denoising with two key components: 1) Adaptive Cache Injection (ACI) that caches and adaptively re-injects features from both inputs during denoising, and 2) Semantic Anchor Prompting (SAP) that generates shared anchor prompts using vision-language models to bridge semantic gaps.

Result: CHIMERA achieves smoother and more semantically aligned transitions than existing methods, establishing new state-of-the-art performance in image morphing as validated by extensive experiments and user studies.

Conclusion: CHIMERA provides an effective zero-shot diffusion framework for high-quality image morphing with natural feature fusion and smooth transitions, introducing both a novel method and evaluation metric (GLCS) for the task.
...
TL;DR: MuSASplat reduces computational costs for sparse-view 3D Gaussian splatting using lightweight adapters and efficient feature fusion, achieving SOTA quality with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Existing pose-free feed-forward methods for sparse-view 3D Gaussian splatting require full fine-tuning of large ViT backbones, incurring substantial GPU costs and computational burden.

Method: Introduces a lightweight Multi-Scale Adapter for efficient ViT fine-tuning with minimal parameters, and a Feature Fusion Aggregator that integrates features across input views effectively while reducing memory usage and computational costs.
...
Conclusion: MuSASplat dramatically reduces computational burden for training pose-free feed-forward 3D Gaussian splatting models while maintaining rendering quality, making the approach more accessible and efficient.
...
TL;DR: This paper addresses privacy leakage in MLLMs by developing a method to restore surrogate-driven protected data while maintaining MLLM usability, using a new dataset and guided generation approach.


<details>
  <summary>Details</summary>
Motivation: Existing privacy protection methods for MLLMs effectively obscure private information but fail to evaluate the authenticity and recovery quality of user privacy, creating a research gap in restoring protected data across diverse MLLM scenarios.

Method: The authors create the SPPE dataset with privacy categories and user instructions, then formulate privacy recovery as a guided generation task using complementary multimodal signals to reconstruct private content while preserving MLLM-generated edits.

Result: Experiments on SPPE and InstructPix2Pix datasets show the approach generalizes well across diverse visual content and editing tasks, achieving strong balance between privacy protection and MLLM usability.

Conclusion: The work provides a unified approach to reliably reconstruct private content in MLLMs while preserving edit fidelity, addressing the critical challenge of privacy recovery in protected data scenarios.
...
Method: Two-stage framework: first disentangles restoration into four specialized hypotheses (color distortion, haze, detail loss, noise) with adaptive fusion based on local degradation patterns, then progressive refinement to correct residual artifacts.

Result: Achieves competitive performance on reference-based fidelity metrics and outperforms state-of-the-art on non-reference perceptual quality metrics, with strong improvements in color correction and contrast enhancement.

Conclusion: TIDE effectively addresses spatially varying underwater degradations through explicit degradation modeling and specialized restoration experts, producing natural results even in highly degraded regions.
...
TL;DR: A topology-guided medical image classification framework that integrates multi-scale persistent topological features with vision backbones, achieving state-of-the-art performance on three public datasets.


<details>
  <summary>Details</summary>
Motivation: Current deep neural networks for medical image classification either focus on pixel-intensity features rather than anatomical structures, or capture only simple topological features via single-parameter persistence, missing complex anatomical patterns important for disease detection.

Method: 1) Compute cubical persistence diagrams across multiple image resolutions/scales; 2) Develop a "vineyard" algorithm to consolidate these diagrams into a single stable diagram capturing features from global anatomy to local irregularities; 3) Design a cross-attention-based neural network to process the consolidated diagrams; 4) Fuse topological embeddings with feature maps from CNNs or Transformers in an end-to-end architecture.
...
Conclusion: The proposed comprehensive topological perspective provides robust and interpretable medical image classification by integrating multi-scale and multi-filtration topological features with conventional vision architectures.
...
TL;DR: RefLSM: A novel variational level set model that integrates Retinex-inspired reflectance decomposition for robust medical image segmentation under challenging conditions like intensity inhomogeneity and noise.


<details>
  <summary>Details</summary>
Motivation: Traditional level set methods struggle with severe non-uniform imaging conditions due to approximate bias field estimations. Medical image segmentation faces challenges from intensity inhomogeneity, noise, blurred boundaries, and irregular structures.

Method: Proposes Reflectance-based Level Set Model (RefLSM) that decomposes images into reflectance and bias field components, then segments the illumination-invariant reflectance. Includes two innovations: 1) linear structural prior for geometric guidance, and 2) relaxed binary level-set with convex relaxation and sign projection for stable evolution. Uses ADMM-based optimization.
...
Conclusion: RefLSM effectively addresses limitations of traditional level set methods by integrating reflectance decomposition, providing a robust solution for medical image segmentation under challenging imaging conditions.
...
TL;DR: HVQ-CGIC introduces a controllable generative image compression framework using VQ hyperprior to adapt entropy modeling to image content, achieving 61.3% bitrate reduction compared to SOTA methods.
...
Method: Introduces HVQ-CGIC framework with mathematical foundation for VQ hyperprior, novel loss design for RD balance and control, and lightweight hyper-prior estimation network to adapt entropy modeling to image content.

Result: Achieves significant RD performance advantage over SOTA generative compression methods - on Kodak dataset, achieves same LPIPS as Control-GIC, CDC and HiFiC with average 61.3% fewer bits.

Conclusion: HVQ-CGIC has potential to become foundational component for VQGAN-based image compression, analogous to HyperPrior framework's role in neural image compression, by enabling content-adaptive entropy modeling and flexible rate control.
...
TL;DR: Survey paper on efficient 3D/4D Gaussian Splatting techniques that reduce memory and computational demands while preserving quality, categorizing methods into Parameter Compression and Restructuring Compression.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting enables real-time, high-fidelity 3D reconstruction but suffers from massive memory and computational demands, especially in 4D dynamic scenes, limiting practical applications.

Method: Provides first unified survey of efficient 3D/4D Gaussian Splatting techniques, systematically categorizing methods into Parameter Compression and Restructuring Compression, covering datasets, metrics, and benchmarks.

Result: Comprehensive overview of methodological trends, core ideas, and current state-of-the-art in efficient Gaussian Splatting for both static and dynamic scene representation.

Conclusion: Identifies current limitations and outlines promising research directions toward scalable, compact, and real-time Gaussian Splatting for both static and dynamic 3D scene representation.
...
TL;DR: Proposes StorytellingPainter, a two-stage pipeline using LLMs and T2I models to generate complex storytelling images with rich semantic connections, along with evaluation metrics and lightweight story generation models.


<details>
  <summary>Details</summary>
Motivation: Storytelling images with rich semantic connections are valuable for applications but challenging to create manually, leading to scarcity. Generative AI offers potential but needs specialized approaches for this complex task.

Method: Two-stage pipeline: 1) LLMs generate detailed stories with logical connections, 2) T2I models synthesize images from these stories. Also develops evaluation framework with three evaluators and creates lightweight Mini-Storyteller models.

Result: Experimental results demonstrate feasibility and effectiveness of the approaches. The pipeline successfully generates storytelling images, and the evaluation framework provides comprehensive assessment metrics.

Conclusion: The proposed StorytellingPainter pipeline and evaluation framework effectively address the challenge of generating semantically rich storytelling images, with potential applications in illustration creation and cognitive screening.
...
TL;DR: A minimal 300-line implementation explaining diffusion models from code perspective, bridging theory and practice.


<details>
  <summary>Details</summary>
Motivation: Address the gap between complex mathematical formulations in diffusion model papers and practical implementations, providing clear guidance on how diffusion models actually work in code.

Method: Create a concise implementation (~300 lines) that preserves essential components: forward diffusion, reverse sampling, noise-prediction network, and training loop, while removing unnecessary engineering details.

Result: A minimal, implementation-first tutorial that clearly demonstrates how diffusion models operate in practice and how code corresponds to theory, with code and pre-trained models available on GitHub.

Conclusion: The technical report successfully bridges the theory-practice gap in diffusion models by providing a clear, code-execution perspective that helps researchers understand both mathematical formulations and their practical implementations.
...
Method: Uses reinforcement learning in pre-training with masked multimodal data: estimates visual dependency via attention, masks vision-dependent segments, and reconstructs spans using semantic-visual reward signals.
...
TL;DR: AutoLugano is a fully automated deep learning system that performs end-to-end lymphoma classification from FDG-PET/CT scans, achieving 88.31% accuracy for lesion detection and 85.07% accuracy for therapeutic staging.


<details>
  <summary>Details</summary>
Motivation: To develop an automated system for lymphoma classification that can perform lesion segmentation, anatomical localization, and Lugano staging from baseline FDG-PET/CT scans without manual intervention, addressing the need for standardized and efficient clinical staging.

Method: Three sequential modules: (1) Anatomy-Informed Lesion Segmentation using 3D nnU-Net trained on multi-channel PET/CT inputs, (2) Atlas-based Anatomical Localization using TotalSegmentator to map lesions to 21 lymph node regions, and (3) Automated Lugano Staging that translates spatial distribution into Lugano stages and therapeutic groups (Limited vs. Advanced Stage).

Result: On external validation (n=67), achieved 88.31% accuracy, 74.47% sensitivity, 94.21% specificity, and 80.80% F1-score for regional involvement detection. For therapeutic stratification, achieved 85.07% accuracy with 90.48% specificity and 82.61% sensitivity, outperforming baseline models.

Conclusion: AutoLugano represents the first fully automated, end-to-end pipeline that translates FDG-PET/CT scans into complete Lugano staging, demonstrating strong potential to assist in initial staging, treatment stratification, and clinical decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Single pose estimates can't capture visual ambiguity, leading to unreliable robotic behavior. Existing methods rely heavily on color information, which is often unavailable in industrial settings.

Method: Novel neural network-based approach that estimates object pose uncertainty using only 3D colorless data, focusing on symmetries in reflection and revolution but extendable to full SE(3) pose distribution estimation.

Result: Method validated in real-world bin picking scenario with objects of varying geometric ambiguity. First deep learning approach for pose distribution estimation without RGB input.

Conclusion: Proposed framework successfully addresses pose uncertainty estimation in color-limited industrial environments, with source code publicly available for further development and application.
...
TL;DR: ReLKD is an end-to-end framework for Generalized Category Discovery that leverages implicit inter-class relations to improve novel class classification through multi-granularity representation learning and knowledge distillation.


<details>
  <summary>Details</summary>
Motivation: GCD faces challenges in categorizing unlabeled data containing both known and novel classes, with existing methods treating classes independently and neglecting inter-class relations, which are difficult to obtain directly in real-world scenarios.

Method: ReLKD uses three modules: target-grained module for discriminative representations, coarse-grained module for capturing hierarchical class relations, and distillation module for transferring knowledge from coarse-grained to target-grained module to refine representation learning.

Result: Extensive experiments on four datasets demonstrate ReLKD's effectiveness, particularly in scenarios with limited labeled data, showing improved novel class classification performance.

Conclusion: ReLKD effectively exploits implicit inter-class relations to enhance GCD performance, providing a practical solution for real-world scenarios where explicit inter-class relations are difficult to obtain.
...
TL;DR: STRinGS is a text-aware refinement framework for 3D Gaussian Splatting that separately processes text and non-text regions to preserve fine-grained text details in 3D scene reconstruction.


<details>
  <summary>Details</summary>
Motivation: Text elements in real-world scenes convey important contextual information, but current 3D representations like 3D Gaussian Splatting struggle to preserve fine-grained text details, where small errors can lead to significant semantic loss.

Method: A selective refinement framework that treats text and non-text regions separately, refining text regions first and then merging them with non-text regions for full-scene optimization.

Result: STRinGS produces sharp, readable text even in challenging configurations, achieving 63.6% relative improvement over standard 3DGS at just 7K iterations, as measured by OCR Character Error Rate (CER).

Conclusion: The method and accompanying STRinGS-360 dataset push the boundaries of 3D scene understanding in text-rich environments, paving the way for more robust text-aware reconstruction methods.
...
TL;DR: UCPE introduces a unified camera positional encoding that handles diverse camera parameters (poses, intrinsics, distortions) for better camera-controllable video generation in Transformers.


<details>
  <summary>Details</summary>
Motivation: Existing camera encoding methods rely on simplified pinhole assumptions, limiting generalization across diverse real-world camera intrinsics and lens distortions needed for 3D perception tasks.

Method: Proposes Relative Ray Encoding for geometry-consistent camera representation and Absolute Orientation Encoding for pitch/roll control, integrated via lightweight spatial attention adapter into pretrained video Diffusion Transformers.

Result: Achieves state-of-the-art camera controllability and visual fidelity with less than 1% trainable parameters, validated on large video dataset covering diverse camera motions and lens types.

Conclusion: UCPE demonstrates effective camera representation for Transformers with potential applications across multi-view, video, and 3D tasks.
...
TL;DR: Hybrid deep learning model combining SqueezeNet v1 and EfficientNet-B0 with handcrafted radiomic features achieves 98.93% accuracy for brain tumor classification on MRI, offering computational efficiency with clinical reliability.
...
Method: Proposes a hybrid deep learning model combining lightweight SqueezeNet v1 and high-performing EfficientNet-B0, enhanced with handcrafted radiomic descriptors including HOG, LBP, Gabor filters, and Wavelet transforms. Trained on Nickparvar Brain Tumor MRI dataset with 7,023 contrast-enhanced T1-weighted axial MRI slices across four classes.

Result: Achieved 98.93% testing accuracy, improving to 99.08% with Test Time Augmentation. Model uses fewer than 2.1 million parameters and less than 1.2 GFLOPs, offering excellent computational efficiency while maintaining high diagnostic performance.

Conclusion: The hybrid network provides an optimal balance between computational efficiency and diagnostic accuracy, demonstrating near-clinical reliability for automated MRI-based tumor classification with potential for clinical decision-support systems.
...
TL;DR: TEXTER generates textual explanations for image classifiers by isolating decision-critical features before language alignment, producing more faithful explanations than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models generate general captions rather than classifier-specific reasoning, and existing zero-shot explanation methods only describe visible content rather than what drives the prediction.

Method: TEXTER identifies neurons contributing to predictions, emphasizes decision-critical features, maps them to CLIP feature space for text retrieval, and uses sparse autoencoder for better interpretability in Transformers.

Result: Extensive experiments show TEXTER generates more faithful and interpretable explanations than existing methods.

Conclusion: TEXTER overcomes limitations of existing methods by focusing on decision-critical features before language alignment, improving explanation quality for image classifier decisions.
...
<details>
  <summary>Details</summary>
Motivation: While diffusion-based editing pipelines enable 3DGS manipulation, they expose assets to unauthorized editing and malicious tampering risks. Existing 2D adversarial protection methods don't work well for 3DGS due to view-generalization challenges and balancing invisibility with protection.

Method: AdLift lifts strictly bounded 2D adversarial perturbations into 3D Gaussian-represented safeguards using Lifted PGD optimization. It alternates between gradient truncation during back-propagation from editing models and image-to-Gaussian fitting, with projected gradients to constrain perturbations.

Result: AdLift effectively protects against state-of-the-art instruction-driven 2D image and 3DGS editing, providing consistent adversarial-based protection across different viewpoints that generalizes to novel views.

Conclusion: AdLift successfully addresses the challenges of 3DGS protection by providing view-generalizable, invisible safeguards against unauthorized instruction-driven editing, advancing secure 3D content creation.
...
<details>
  <summary>Details</summary>
Motivation: Current medical image enhancement models often over-edit, distorting organs, creating false findings, and missing small tumors due to lack of anatomical understanding and contrast dynamics awareness.

Method: SMILE uses structure-aware supervision following true organ boundaries and contrast patterns, registration-free learning with unaligned multi-phase CT scans, and unified inference for consistent enhancement across all contrast phases.

Result: SMILE outperforms existing methods with 14.2% higher SSIM, 20.6% higher PSNR, 50% better FID across six external datasets, and improves cancer detection from non-contrast CT by up to 10% F1 score.
...
TL;DR: A photogrammetry-based graph generation pipeline for critical infrastructure using RGB images and stereo camera depth data instead of expensive laser scanners.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D point clouds from laser scanners are expensive and require specialist knowledge, creating barriers for creating virtual representations of critical infrastructure for simulations and digital twins.

Method: Graph generation pipeline using photogrammetry with stereo cameras, deep learning for object detection/instance segmentation, and user-defined heuristics/rules to infer object relations from RGB images and depth data.

Result: The method produces graphs close to ground truth for two hydraulic systems, offering a cost-effective alternative to laser scanning while maintaining flexibility and transparency.

Conclusion: This photogrammetry-based approach provides a flexible, transparent, and cost-effective solution for creating virtual representations of critical infrastructure suitable for high-stakes decision-making.
...
TL;DR: Proposes RVLF framework using vision-language model + reinforcement learning for gloss-free sign language translation, achieving state-of-the-art BLEU improvements across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Address two key challenges in gloss-free SLT: 1) inadequate sign representation failing to capture nuanced visual cues, and 2) sentence-level semantic misalignment in current LLM-based methods limiting translation quality.

Method: Three-stage RVLF framework: 1) Build LVLM for sign language with semantic representation learning fusing skeleton motion cues + DINOv2 visual features, 2) Instruction tuning for SLT-SFT baseline, 3) GRPO-based RL optimization with reward combining BLEU (translation fidelity) and ROUGE (sentence completeness) to get SLT-GRPO.

Result: Substantial BLEU-4 improvements: +5.1 (CSL-Daily), +1.11 (PHOENIX-2014T), +1.4 (How2Sign), +1.61 (OpenASL) without external pre-training. First work to incorporate GRPO into SLT.

Conclusion: RVLF effectively addresses representation and alignment challenges in gloss-free SLT. GRPO-based optimization enhances both translation quality and semantic consistency, validated by extensive experiments.
...
TL;DR: Geo3DVQA is a new benchmark for evaluating vision-language models on 3D geospatial reasoning using only RGB remote sensing imagery, revealing current VLMs' limitations in height-aware spatial analysis.


<details>
  <summary>Details</summary>
Motivation: Current 3D geospatial analysis relies on expensive specialized sensors (LiDAR, multispectral) that limit global accessibility, and existing methods struggle with integrating multiple 3D cues, handling diverse queries, and providing interpretable reasoning.

Method: Created Geo3DVQA benchmark with 110k curated question-answer pairs spanning 16 task categories across three complexity levels: single-feature inference, multi-feature reasoning, and application-level spatial analysis, using only RGB remote sensing imagery.

Result: State-of-the-art VLMs performed poorly: GPT-4o (28.6%), Gemini-2.5-Flash (33.0%), while domain-specific fine-tuning of Qwen2.5-VL-7B achieved 49.6% accuracy (+24.8 points improvement), showing both limitations of current VLMs and effectiveness of domain adaptation.

Conclusion: Geo3DVQA introduces new challenge frontiers for scalable, accessible, and holistic 3D geospatial analysis, demonstrating the difficulty of RGB-to-3D reasoning and the need for domain-specific adaptation in vision-language models.
...
TL;DR: Reimplementation study confirms pretrained CNN (Google Inception-ResNet-v2) provides practical baseline (~62% accuracy) for wildlife species identification, but shows limitations in generalization with macro F1 of 0.28, highlighting need for species-specific adaptation.
...
Method: Reimplemented experiment from scratch using openly available resources and different dataset (900 images, 90 species) with minimal preprocessing, using Google Inception-ResNet-v2 model.

Result: Obtained 62% overall classification accuracy (close to original 71% despite dataset differences), with macro F1 score of 0.28 showing substantial per-class performance variation.

Conclusion: Pretrained CNNs provide practical baseline for wildlife species identification but require species-specific adaptation or transfer learning for consistent high-quality predictions due to generalization limitations.
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers lack hierarchical biases of CNNs but still show U-shaped entropy profiles (compressing info in middle layers). The paper investigates whether this bottleneck is architectural or data-driven adaptation.

Method: Analyzed layer-wise Effective Encoding Dimension (EED) of DINO-trained ViTs across datasets of varying compositional complexity (UC Merced, Tiny ImageNet, CIFAR-100). Studied correlation between bottleneck depth and semantic abstraction requirements.

Result: Bottleneck depth strongly correlates with semantic abstraction needed: texture-heavy datasets preserve high-rank representations throughout, while object-centric datasets drive networks to dampen high-frequency info in middle layers, creating bottlenecks to isolate semantic features.
...
TL;DR: Aerial-D: A large-scale referring expression segmentation dataset for aerial imagery with 37K images and 1.5M expressions, featuring automatic generation pipeline and historical image simulation.


<details>
  <summary>Details</summary>
Motivation: Referring expression segmentation in aerial imagery faces unique challenges including varying spatial resolution, inconsistent color usage, small targets, high object density, and occlusions. Existing datasets are limited for aerial contexts.

Method: Created Aerial-D dataset through fully automatic pipeline combining rule-based expression generation with LLM enhancement for linguistic variety and visual detail focus. Used filters to simulate historic imaging conditions. Adopted RSRefSeg architecture for training.

Result: Models trained on Aerial-D combined with prior aerial datasets achieve competitive performance on contemporary benchmarks while maintaining strong accuracy under monochrome, sepia, and grainy degradations typical of archival aerial photography.

Conclusion: Aerial-D enables unified instance and semantic segmentation from text for both modern and historical aerial images, addressing unique challenges of aerial imagery with a large-scale, diverse dataset and effective training approach.
...
TL;DR: TD-Attn is a novel framework that addresses multi-view inconsistency in 3D tasks by using 3D-Aware Attention Guidance and Hierarchical Attention Modulation to overcome prior view bias in Text-to-Image diffusion models.
...
Conclusion: TD-Attn effectively addresses the prior view bias limitation in T2I models, enabling more consistent and controllable 3D generation and editing through semantic-specific interventions and attention modulation.
...
TL;DR: MICo-150K: A large-scale dataset for Multi-Image Composition with 150K synthetic composite images, 7 task categories, and comprehensive benchmark for evaluating models on coherent multi-image synthesis.


<details>
  <summary>Details</summary>
Motivation: Multi-Image Composition (MICo) - synthesizing coherent images from multiple references - is challenging due to lack of high-quality training data, hindering progress in this area.

Method: Systematically categorize MICo into 7 tasks, curate source images, synthesize balanced composite images using proprietary models, apply human-in-the-loop filtering, create Decomposition-and-Recomposition subset, and develop MICo-Bench with new evaluation metric.

Result: Created MICo-150K dataset with identity consistency, MICo-Bench with 100 cases per task + 300 De&Re cases, Weighted-Ref-VIEScore metric, and Qwen-MICo baseline model that matches Qwen-Image-2509 while supporting arbitrary multi-image inputs.

Conclusion: MICo-150K effectively equips models for multi-image composition, enhances existing capabilities, and provides valuable resources (dataset, benchmark, baseline) for advancing research in Multi-Image Composition.
...
TL;DR: Enhanced SW-YOLO model with refined cropping, architectural improvements (CBAM, feature extraction modules, new head) achieves 61.2 mAP on VisDrone2019, significantly outperforming YOLOv5L (35.5) and CZDet (58.36).


<details>
  <summary>Details</summary>
Motivation: Growing importance of aerial imagery in critical applications creates need for robust small object detection frameworks, as current methods (image cropping, architectural modifications) require improvement for better accuracy and speed.

Method: Enhanced SW-YOLO approach with refined sliding window cropping dimensions/overlap, plus architectural modifications: advanced feature extraction modules in neck, CBAM integration in backbone for spatial/channel information preservation, and new head design for small object detection.

Result: Achieved 61.2 mAP on VisDrone2019 dataset, significantly improving from YOLOv5L baseline (35.5) and outperforming CZDet (58.36) and SAHI framework.

Conclusion: Proposed model demonstrates substantial accuracy improvement for small object detection in aerial images, establishing effective framework combining refined cropping strategies with architectural enhancements.
...
TL;DR: Tessellation GS improves 3D Gaussian Splatting by anchoring 2D Gaussians to mesh faces with hierarchical neural features and adaptive subdivision, enabling better dynamic scene reconstruction from single cameras.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting struggles with viewpoint extrapolation, overfitting, and poor generalization in sparse-view and dynamic scene reconstruction, especially from single cameras.

Method: Anchors 2D Gaussians to mesh faces, uses hierarchical neural features to infer Gaussian attributes, implements adaptive face subdivision guided by detail-aware loss, and leverages reconstruction foundation model priors for Gaussian deformation initialization.

Result: Outperforms previous SOTA methods, reducing LPIPS by 29.1% and Chamfer distance by 49.2% on appearance and mesh reconstruction tasks.

Conclusion: Tessellation GS enables robust reconstruction of general dynamic objects from single static cameras, overcoming limitations of previous optimization-based methods for dynamic scene reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Current Concept Bottleneck Models (CBMs) are limited by using only linear combinations of concepts for predictions, which restricts their expressiveness and ability to capture complex relationships between concepts.

Method: Introduces a logic module that connects learned concepts from CBMs through differentiable logic operations, enabling the model to go beyond simple weighted combinations to leverage various logical operations while maintaining end-to-end learnability.

Result: Empirical studies on well-known benchmarks and synthetic datasets demonstrate that LogicCBM models have better accuracy, perform effective interventions, and are highly interpretable compared to traditional CBMs.

Conclusion: Enhancing concept-based learning models through propositional logic improves model expressivity, enables better capture of inter-concept relations, and maintains interpretability while boosting performance.
...
<details>
  <summary>Details</summary>
Motivation: Current Anti-UAV research focuses on fixed ground cameras, lacking attention to tracking from moving UAV platforms. There's a need to address the challenging scenario of dual-dynamic disturbances from both pursuer and target UAV motion.

Method: Proposes MambaSTS, a Mamba-based baseline method that integrates spatial-temporal-semantic learning using Mamba for global semantic features, Transformer for spatial features, and temporal token propagation for video-level long-term context.

Result: Constructed a million-scale dataset with 1,810 videos, each annotated with bounding boxes, language prompts, and 15 tracking attributes. Experimental evaluation of 50 modern tracking algorithms shows significant room for improvement in this domain.

Conclusion: The UAV-Anti-UAV task presents unique challenges due to dual-dynamic disturbances, and the proposed dataset and MambaSTS baseline provide a foundation for advancing research in this important area of Anti-UAV technology.
...
TL;DR: GlimmerNet is an ultra-lightweight CNN that achieves global perception without expensive self-attention, using grouped dilated convolutions and efficient feature recombination to set new accuracy-efficiency frontier for UAV emergency monitoring.
...
TL;DR: ROHIT reconstructs objects along hand interaction timelines using pose constraints and propagation, improving reconstruction accuracy without 3D ground truth.


<details>
  <summary>Details</summary>
Motivation: To enable better object reconstruction in videos by modeling the structured pose changes that occur during hand-object interactions, particularly focusing on stable grasp periods where objects maintain constant contact with hands.

Method: Defines Hand Interaction Timeline (HIT) with pose constraints, proposes Constrained Optimisation and Propagation (COP) framework to propagate object poses along the timeline, focusing on stable grasp periods for efficient annotation and evaluation.

Result: COP improves stable grasp reconstruction by 6.2-11.3% and HIT reconstruction by up to 24.5% on HOT3D and EPIC-Kitchens datasets using 2D projection error evaluation.

Conclusion: The ROHIT task and COP framework effectively reconstruct objects along hand interaction timelines without requiring 3D ground truth, demonstrating significant improvements in reconstruction accuracy through pose constraint modeling and propagation.
...
TL;DR: InterAgent is the first end-to-end framework for text-driven physics-based multi-agent humanoid control, using an autoregressive diffusion transformer with multi-stream blocks and interaction graph exteroception to generate coherent, physically plausible multi-agent behaviors from text prompts.


<details>
  <summary>Details</summary>
Motivation: Existing humanoid control methods are largely confined to single-agent scenarios, overlooking the physically plausible interplay essential for multi-agent interactions. There's a need for frameworks that can handle complex coordination inherent in human social behaviors for multiple agents.

Method: Proposes InterAgent with: 1) Autoregressive diffusion transformer with multi-stream blocks that decouple proprioception, exteroception, and action to mitigate cross-modal interference; 2) Novel interaction graph exteroception representation capturing fine-grained joint-to-joint spatial dependencies; 3) Sparse edge-based attention mechanism that dynamically prunes redundant connections and emphasizes critical inter-agent spatial relations.

Result: Extensive experiments show InterAgent consistently outperforms multiple strong baselines, achieving state-of-the-art performance. It enables producing coherent, physically plausible, and semantically faithful multi-agent behaviors from only text prompts.

Conclusion: InterAgent successfully bridges the gap in multi-agent humanoid control, providing the first end-to-end framework for text-driven physics-based multi-agent coordination. The code and data will be released to facilitate future research in this direction.
...
TL;DR: VideoCoF introduces a Chain-of-Frames approach for video editing that uses reasoning tokens to achieve precise instruction-to-region alignment without masks, enabling fine-grained editing with minimal data.
...
Method: VideoCoF enforces a "see, reason, then edit" procedure where the video diffusion model first predicts reasoning tokens (edit-region latents) before generating target video tokens, plus a RoPE alignment strategy for motion alignment and length extrapolation.

Result: With only 50k video pairs, VideoCoF achieves state-of-the-art performance on VideoCoF-Bench, demonstrating efficient and effective precise video editing without masks.

Conclusion: VideoCoF resolves the trade-off between precision and unification in video editing through explicit reasoning tokens, enabling mask-free fine-grained editing with strong instruction-to-region alignment.
...
TL;DR: S2VC is a single-step diffusion video codec that achieves state-of-the-art perceptual quality at low bitrates with 52.73% bitrate savings over prior methods.


<details>
  <summary>Details</summary>
Motivation: Traditional and neural video codecs struggle with perceptual quality at low bitrates - either suffering from artifacts due to limited generation capacity or requiring heavy sampling complexity when using diffusion models.

Method: Combines conditional coding framework with efficient single-step diffusion generator, introduces Contextual Semantic Guidance for frame-adaptive semantics, and incorporates Temporal Consistency Guidance in diffusion U-Net for temporal coherence.

Result: Achieves state-of-the-art perceptual quality with average 52.73% bitrate saving over prior perceptual methods, demonstrating efficient high-quality video compression.

Conclusion: Single-step diffusion shows promise for efficient, high-quality video compression by balancing perceptual quality with computational efficiency.
...
TL;DR: LR-GCN: A Laplacian-regularized graph convolutional network that robustly detects DeepFakes from noisy or unordered face sequences using adaptive sparse graphs and spectral band-pass filtering.


<details>
  <summary>Details</summary>
Motivation: Existing DeepFake detectors assume clean, temporally consistent facial sequences, but real-world scenarios involve compression artifacts, occlusions, adversarial attacks, and invalid face detections that destabilize detection.

Method: Proposes LR-GCN with Order-Free Temporal Graph Embedding (OF-TGE) that organizes frame features into adaptive sparse graphs based on semantic affinities. Uses dual-level sparsity on graph structure and node features, plus Graph Laplacian Spectral Prior as high-pass operator to highlight forgery artifacts, followed by low-pass GCN aggregation for spectral band-pass filtering.
...
Conclusion: LR-GCN provides robust DeepFake detection from noisy/unordered face sequences using adaptive graph structures and spectral filtering, overcoming limitations of traditional temporal-assumption methods in real-world scenarios.
...
TL;DR: MultiMotion is a unified framework for multi-object video motion transfer using Diffusion Transformers, featuring mask-aware attention motion flow and efficient sampling methods.


<details>
  <summary>Details</summary>
Motivation: Current DiT architectures struggle with multi-object video motion transfer due to motion entanglement and lack of object-level control, making precise manipulation of multiple objects challenging.

Method: Introduces Mask-aware Attention Motion Flow (AMF) using SAM2 masks to disentangle motion features for multiple objects, plus RectPC (high-order predictor-corrector solver) for efficient sampling, and creates a benchmark dataset for evaluation.

Result: MultiMotion achieves precise, semantically aligned, and temporally coherent motion transfer for multiple distinct objects while maintaining DiT's high quality and scalability.

Conclusion: The framework successfully addresses multi-object motion transfer challenges in DiT architectures through explicit motion disentanglement and efficient sampling, with code provided for further development.
...
TL;DR: SJD++ accelerates autoregressive text-to-image generation by 2-3× using probabilistic parallel decoding with multi-token prediction and token reuse.
...
Method: Speculative Jacobi Decoding++ (SJD++), a training-free probabilistic parallel decoding algorithm that integrates Jacobi decoding's iterative multi-token prediction with speculative sampling's drafting-and-verification mechanism, and reuses high-confidence draft tokens after verification.

Result: SJD++ achieves 2× to 3× inference latency reduction and 2× to 7× step compression while preserving visual quality with no observable degradation across several autoregressive text-to-image models.

Conclusion: SJD++ provides significant acceleration for autoregressive text-to-image generation through training-free parallel decoding techniques without compromising output quality.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-image models like Stable Diffusion produce visually impressive results but suffer from geometric inconsistencies, particularly vanishing point errors where parallel lines don't converge correctly, leading to structurally implausible scenes that degrade spatial realism, especially in architectural images.

Method: Extends a pre-trained diffusion model by incorporating structural guidance from building contours and introduces geometric constraints that explicitly encourage alignment between image edges and perspective cues to correct vanishing point inconsistencies.
...
Conclusion: ControlVP provides an effective user-guided solution for correcting vanishing point inconsistencies in generated images, improving structural realism while preserving visual quality, with potential applications in domains requiring precise geometric accuracy.
...
TL;DR: MeshRipple generates 3D meshes by expanding from a frontier like a ripple, maintaining connectivity and resolving long-range dependencies that autoregressive methods break.


<details>
  <summary>Details</summary>
Motivation: Autoregressive mesh generators serialize faces into sequences and use truncated segments with sliding-window inference due to memory limits, but this breaks long-range geometric dependencies, causing holes and fragmented components.

Method: Three key innovations: 1) frontier-aware BFS tokenization aligning generation order with surface topology, 2) expansive prediction strategy maintaining coherent connected surface growth, 3) sparse-attention global memory providing effectively unbounded receptive field to resolve long-range topological dependencies.
...
Conclusion: The integrated design enables MeshRipple to overcome limitations of autoregressive mesh generation by maintaining connectivity and resolving long-range dependencies through frontier-based expansion.
...
TL;DR: City-scale 3D reconstruction from sparse satellite imagery using 2.5D height map modeling and generative texture restoration for extreme viewpoint extrapolation.


<details>
  <summary>Details</summary>
Motivation: City-scale 3D reconstruction from satellite imagery faces extreme viewpoint extrapolation challenges (nearly 90° gaps) where current methods like NeRF and 3DGS fail due to sparse orbital images with minimal parallax, foreshortened facades, and flawed textures.

Method: Two key design choices: 1) Model city geometry as 2.5D height map using Z-monotonic signed distance field (SDF) for stable optimization under sparse satellite views, producing watertight meshes with crisp roofs and vertical facades. 2) Paint mesh appearance via differentiable rendering and train generative texture restoration network to enhance degraded satellite inputs with high-frequency details.

Result: Demonstrated scalability and robustness on large-scale urban reconstruction, reconstructing 4km² real-world regions from few satellite images, achieving state-of-the-art performance in synthesizing photorealistic ground views.

Conclusion: The method produces visually compelling, high-fidelity 3D models ready for downstream applications like urban planning and simulation, overcoming extreme viewpoint extrapolation challenges in city-scale reconstruction from satellite imagery.
...
TL;DR: Vision LLMs waste computation on redundant visual tokens in deep layers; random pruning works as well as complex methods after the "information horizon" where tokens lose salience.


<details>
  <summary>Details</summary>
Motivation: Vision LLMs use hundreds of visual tokens, causing high computational costs. Existing token pruning methods fail in deep layers, performing no better than random pruning, suggesting visual tokens lose information with depth.

Method: Propose measuring token information by output probability change upon removal. Identify "information horizon" where tokens become redundant. Show random pruning in deep layers works effectively and enhances existing methods like DivPrune.

Result: Random pruning in deep layers balances performance and efficiency. DivPrune with random pruning achieves SOTA, maintaining 96.9% of Qwen-2.5-VL-7B performance while pruning 50% of visual tokens.

Conclusion: Visual tokens lose information in deep layers ("information horizon"), making random pruning effective. This insight improves pruning efficiency and performance across VLLMs and tasks.
...
TL;DR: LongCat-Image is a bilingual Chinese-English image generation model that excels at text rendering, photorealism, efficiency, and comes with a comprehensive open-source ecosystem.


<details>
  <summary>Details</summary>
Motivation: To address core challenges in multilingual text rendering, photorealism, deployment efficiency, and developer accessibility in current image generation models, particularly for Chinese-English bilingual applications.

Method: Uses rigorous data curation across pre-training, mid-training, and SFT stages, complemented by curated reward models during RL phase. Features a compact 6B parameter diffusion model design (vs. typical 20B+ MoE architectures).

Result: Achieves SOTA in text-rendering capabilities and photorealism, sets new industry standard for Chinese character rendering (including complex/rare characters), achieves remarkable efficiency with minimal VRAM usage, and excels in image editing with superior consistency.

Conclusion: LongCat-Image establishes a comprehensive open-source ecosystem with multiple model versions and full training toolchain, providing robust support for developers and researchers to advance visual content creation.
...
TL;DR: Robust VM_TUNet integrates variational PDEs with deep learning for noisy image segmentation, combining physical priors, edge detection, and curvature terms in a hybrid framework with frequency preprocessing and stable local computation modules.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in segmenting noisy images with blurred/fragmented boundaries by combining the interpretability and boundary-smoothing advantages of variational PDEs with the strong representational power of deep neural networks.

Method: Robust VM_TUNet integrates physical priors, edge detector, and mean curvature term into modified Cahn-Hilliard equation. Architecture has two modules: F module for frequency domain preprocessing to avoid poor local minima, and T module for accurate stable local computations with stability estimate.

Result: Achieves balanced trade-off between performance and computational efficiency, yields competitive quantitative results and improved visual quality compared to pure CNN models, and achieves performance close to transformer-based methods with reasonable computational expense.

Conclusion: The hybrid variational-deep learning framework successfully addresses noisy image segmentation challenges by combining the strengths of both approaches, offering improved boundary handling and computational efficiency.
...
TL;DR: AutoSeg3D reimagines online 3D segmentation as instance tracking using object queries for temporal propagation, achieving SOTA results across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Current query-based 3D segmentation methods lack temporal understanding, which is crucial for embodied agents operating in dynamic environments. Viewpoint variations in robotics often cause partial object visibility across frames, requiring holistic understanding beyond instantaneous views.

Method: Frames online 3D segmentation as instance tracking using object queries for temporal propagation: long-term association maintains feature/identity coherence, short-term update enriches instant observations. Introduces spatial consistency learning to mitigate VFM fragmentation and enhance temporal learning.

Result: Achieves new SOTA, surpassing ESAM by 2.8 AP on ScanNet200 and delivering consistent gains on ScanNet, SceneNN, and 3RScan datasets.

Conclusion: Temporal information propagation through sparse object queries enhances spatial comprehension while avoiding computational burden of dense temporal interactions, unlocking better temporal environmental perception for embodied agents.
...
TL;DR: DECOMP is an active learning method for dense prediction tasks that decomposes images into class components using pseudo-labels, samples regions from each class, and uses predictive confidence to prioritize difficult classes for annotation.


<details>
  <summary>Details</summary>
Motivation: Existing active learning methods for dense prediction tasks have high computational costs, select irrelevant regions, and rely too heavily on uncertainty sampling, making them inefficient for medical imaging where annotations are costly and time-intensive.

Method: DECOMP decomposes images into class-specific components using pseudo-labels, samples regions from each class to enhance annotation diversity, and uses class-wise predictive confidence to guide sampling toward difficult classes that need more annotations.
...
Conclusion: DECOMP provides an effective active learning strategy for dense prediction tasks that addresses limitations of existing methods, particularly for medical imaging applications where annotation efficiency is critical.
...
TL;DR: MoCA enables efficient compositional 3D generation by using importance-based component routing and compression to reduce quadratic attention costs, allowing scalable fine-grained 3D asset creation.


<details>
  <summary>Details</summary>
Motivation: Existing part-aware 3D generation methods suffer from poor scalability due to quadratic global attention costs when increasing the number of components, limiting efficient compositional 3D asset creation.
...
Result: MoCA outperforms baselines on both compositional object and scene generation tasks, enabling efficient, fine-grained compositional 3D asset creation with scalable number of components.

Conclusion: MoCA presents an effective solution for scalable compositional 3D generation by addressing computational bottlenecks through intelligent component selection and compression techniques.
...
TL;DR: LiQA dataset for liver fibrosis staging with multi-phase MRI from 440 patients, used in CARE 2024 challenge for segmentation and staging tasks under real-world conditions.


<details>
  <summary>Details</summary>
Motivation: Liver fibrosis is a major global health issue requiring accurate staging for clinical management, but current methods face challenges with domain shifts, missing data, and misalignment in real-world settings.

Method: Created LiQA dataset with 440 patients' multi-phase, multi-center MRI scans. Top-performing approach uses semi-supervised learning with external data for segmentation, and multi-view consensus with CAM-based regularization for staging.

Result: The baseline evaluation shows that leveraging multi-source data and anatomical constraints significantly improves model robustness in clinical settings.

Conclusion: The LiQA dataset provides a valuable benchmark for liver fibrosis algorithms, and the proposed methodology demonstrates effective approaches for handling real-world clinical challenges in liver imaging analysis.
...
<details>
  <summary>Details</summary>
Motivation: Safety-critical events are rare in real driving datasets but essential for evaluating autonomous vehicles. Existing data-driven scene generation models lack controllability and often produce physically or socially implausible scenes.

Method: OMEGA uses constrained optimization to re-anchor each reverse diffusion step, steering generation toward physically plausible and behaviorally coherent trajectories. It also formulates ego-attacker interactions as game-theoretic optimization in distribution space to approximate Nash equilibria for adversarial scenarios.

Result: OMEGA improves generation realism and consistency, increasing valid scenes from 32.35% to 72.27% for free exploration, and from 11% to 80% for controllable generation. It generates 5× more near-collision frames with time-to-collision under 3 seconds while maintaining scene realism.

Conclusion: OMEGA provides an effective optimization-guided framework for generating realistic, controllable, and safety-critical multi-agent driving scenes, addressing key limitations in existing scene generation methods for autonomous vehicle evaluation.
...
TL;DR: EgoCampus dataset captures pedestrian eye gaze during outdoor campus navigation, enabling development of EgoCampusNet for gaze prediction in real-world walking scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing egocentric datasets focus on indoor tasks or lack eye gaze information, creating a gap for studying visual attention during real-world outdoor navigation.

Method: Collected EgoCampus dataset using Meta's Project Aria glasses with eye tracking, RGB cameras, inertial sensors, and GPS across 25 outdoor campus paths from 80+ pedestrians, then developed EgoCampusNet for gaze prediction.

Result: Created a diverse, large-scale dataset of gaze-annotated outdoor navigation videos spanning 6 km, and developed a novel method for predicting pedestrian eye gaze during outdoor movement.

Conclusion: The work provides both a valuable resource for studying real-world visual attention and a foundation for future gaze prediction models in outdoor navigation scenarios.
...
TL;DR: PVeRA is a probabilistic version of the VeRA adapter that modifies low-rank matrices probabilistically to handle input ambiguities and enable different sampling configurations during training/testing, outperforming existing adapters on VTAB-1k benchmark.


<details>
  <summary>Details</summary>
Motivation: Large foundation models require vast datasets and computational resources for training/finetuning, which are scarce and costly. Parameter-efficient adaptation methods like VeRA adapters offer solutions but can be improved to handle input ambiguities and provide more flexible training/testing configurations.

Method: Proposes PVeRA, a probabilistic version of the VeRA adapter that modifies the low-rank matrices in a probabilistic manner. This allows handling inherent input ambiguities and enables different sampling configurations during training and testing phases.

Result: Comprehensive evaluation on VTAB-1k benchmark with seven adapters shows that PVeRA outperforms VeRA and other adapter methods, demonstrating superior performance in parameter-efficient adaptation.

Conclusion: PVeRA provides an effective probabilistic approach to parameter-efficient adaptation that handles input ambiguities and offers flexible sampling configurations, achieving state-of-the-art performance while maintaining computational efficiency.
...
TL;DR: A three-stage pipeline (cage segmentation, inpainting, evaluation) improves animal tracking/pose estimation performance in cage-occluded environments by removing cage structures that degrade existing methods.


<details>
  <summary>Details</summary>
Motivation: Animal tracking and pose estimation systems like STEP and ViTPose suffer significant performance drops when processing images/videos with cage structures and systematic occlusions, creating a need for preprocessing to handle these challenging environments.
...
Conclusion: The proposed preprocessing pipeline effectively addresses cage occlusion challenges in animal tracking systems, restoring performance to levels comparable to occlusion-free environments through specialized segmentation and inpainting techniques.
...
TL;DR: Combines 3D reconstruction models with video diffusion models to generate high-fidelity upper-body avatars from single images, achieving realistic appearance and dynamic motion in real-time.


<details>
  <summary>Details</summary>
Motivation: Current 3D avatar generation methods produce blurry textures and stiff motion, while video models suffer from structural errors and identity drift. Need to combine strengths of both approaches for high-fidelity avatars.

Method: Uses 3D reconstruction model for structural/appearance priors to guide real-time autoregressive video diffusion model for rendering, enabling synthesis of photorealistic details and fluid dynamics.

Result: Significantly reduces artifacts, improves visual quality over leading methods, achieves realistic appearance and temporally coherent motion suitable for real-time applications like gaming and VR.

Conclusion: Uniting geometric stability of 3D reconstruction with generative capabilities of video models produces robust, efficient solution for high-fidelity digital avatars with realistic appearance and dynamic motion.
...
TL;DR: SpatialDreamer is a reinforcement learning framework that enables MLLMs to perform complex spatial reasoning through active exploration, visual imagination, and evidence-grounded reasoning, using a novel Geometric Policy Optimization method.
...
Method: Proposes SpatialDreamer framework with closed-loop active exploration, visual imagination via world model, and evidence-grounded reasoning. Introduces Geometric Policy Optimization (GeoPO) with tree-structured sampling and step-level reward estimation with geometric consistency constraints.
...
Conclusion: SpatialDreamer successfully bridges the gap in MLLMs' spatial reasoning capabilities by enabling active mental simulation through reinforcement learning and novel optimization techniques.
...
TL;DR: TREC VQA AG task approach using listwise learning with Masked Pointer Cross-Entropy Loss to improve answer generation ranking


<details>
  <summary>Details</summary>
Motivation: Improve semantic precision and ranking consistency in video question answering answer generation, particularly for temporal reasoning and semantic disambiguation tasks

Method: Two-stage approach: base multimodal model generates candidate answers, then reranking using novel Masked Pointer Cross-Entropy Loss with Rank Weights that integrates pointer-based selection, rank-dependent weighting, and masked cross-entropy under vocabulary restriction

Result: Consistent gains in accuracy and ranking stability, especially for temporal reasoning and semantic disambiguation questions

Conclusion: Listwise learning framework with novel loss function effectively bridges generative modeling with discriminative ranking to produce coherent, fine-grained answer lists
...
TL;DR: DiffusionDriveV2 uses reinforcement learning to improve trajectory diversity and quality in autonomous driving diffusion models, solving mode collapse issues while maintaining multimodality.


<details>
  <summary>Details</summary>
Motivation: Existing generative diffusion models for autonomous driving suffer from mode collapse, producing conservative and homogeneous behaviors. While DiffusionDrive uses predefined anchors to generate diverse trajectories, its imitation learning approach lacks constraints, creating a dilemma between diversity and consistent high quality.

Method: DiffusionDriveV2 leverages reinforcement learning to constrain low-quality modes and explore superior trajectories while preserving multimodality. Key innovations include: 1) scale-adaptive multiplicative noise for broad exploration in trajectory planning, 2) intra-anchor GRPO for advantage estimation within single anchors, and 3) inter-anchor truncated GRPO for global perspective across different anchors to prevent improper advantage comparisons between distinct intentions.

Result: Achieves 91.2 PDMS on NAVSIM v1 and 85.5 EPDMS on NAVSIM v2 datasets in closed-loop evaluation with aligned ResNet-34 backbone, setting new records. The approach resolves the diversity vs. quality dilemma for truncated diffusion models, achieving the best trade-off.

Conclusion: DiffusionDriveV2 successfully addresses the mode collapse problem in end-to-end autonomous driving diffusion models by combining reinforcement learning with the core Gaussian Mixture Model approach, enabling both diverse and high-quality trajectory generation while maintaining multimodality.
...
TL;DR: Unison is a low-cost multimodal AI that automatically parses user intentions and handles diverse understanding/generation tasks with only 500k samples and 50 GPU hours.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal approaches either require massive resources (auto-regressive transformers) or suffer from limited task coverage/poor quality (two-stage methods), and both lack automatic meta-information parsing capabilities.

Method: Adopts a two-stage scheme connecting pre-trained understanding and generative models with alignment fine-tuning, preserving pre-trained model capabilities while adding automatic intention parsing and meta-information extraction.

Result: Achieves superior performance across diverse multimodal tasks (text/image/video understanding, text-to-visual generation, editing, controllable generation, IP-based reference generation) with only 500k training samples and 50 GPU hours.

Conclusion: Unison demonstrates that low-cost multimodal AI with automatic task parsing is feasible, enabling full automation of various multimodal tasks without human intervention.
...
TL;DR: Proposes modality-aware Jaccard distance and split-and-contrast strategy for unsupervised visible-infrared person re-ID, achieving SOTA performance by addressing cross-modality bias through global association and representation learning.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised VI-ReID methods using optimal transport propagate local cluster errors and overlook global instance-level relations. The significant modality gap between visible and infrared makes reliable cross-modality association challenging.

Method: 1) Modality-aware Jaccard distance to mitigate distance bias caused by modality discrepancy for reliable global clustering. 2) Split-and-contrast strategy to obtain modality-specific global prototypes and align them under global association guidance for modality-invariant representation learning.
...
Conclusion: The proposed approach effectively addresses cross-modality learning through bias-mitigated global association and modality-invariant representation learning, demonstrating strong performance in unsupervised visible-infrared person re-identification.
...
TL;DR: GorillaWatch: An end-to-end pipeline for automated gorilla re-identification using novel datasets and multi-frame self-supervised pretraining, outperforming specialized video architectures and enabling scalable endangered species monitoring.


<details>
  <summary>Details</summary>
Motivation: Current manual re-identification of critically endangered western lowland gorillas from camera trap footage is labor-intensive and unsustainable. The lack of large-scale "in-the-wild" video datasets prevents automation of this crucial conservation monitoring task.

Method: 1) Created three novel datasets: Gorilla-SPAC-Wild (largest wild primate re-ID dataset), Gorilla-Berlin-Zoo (cross-domain generalization), Gorilla-SPAC-MoT (multi-object tracking). 2) Developed GorillaWatch pipeline integrating detection, tracking, and re-identification. 3) Introduced multi-frame self-supervised pretraining using tracklet consistency. 4) Used differentiable AttnLRP for scientific validation of biometric feature reliance. 5) Applied spatiotemporal constraints to clustering for unsupervised population counting.

Result: 1) Aggregating features from large-scale image backbones outperformed specialized video architectures. 2) Multi-frame self-supervised pretraining effectively learned domain-specific features without manual labels. 3) AttnLRP verification confirmed model reliance on discriminative biometric traits rather than background correlations. 4) Spatiotemporal constraints mitigated over-segmentation in unsupervised population counting.

Conclusion: GorillaWatch enables scalable, non-invasive monitoring of endangered species through automated re-identification. The publicly released datasets and code facilitate conservation research, with the approach demonstrating that temporal information aggregation and scientific validation are crucial for robust wildlife monitoring systems.
...
TL;DR: DMVAE explicitly aligns encoder latent distribution with arbitrary reference distributions via distribution matching, enabling systematic exploration of optimal latent distributions for image synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing visual generative models use latent spaces constrained by VAEs or foundation model encoders without explicit distribution shaping, making it unclear which latent distributions are optimal for modeling.

Method: Introduces Distribution-Matching VAE (DMVAE) with explicit distribution matching constraint that aligns encoder's latent distribution with arbitrary reference distributions (SSL features, diffusion noise, etc.), generalizing beyond conventional Gaussian priors.

Result: SSL-derived distributions provide optimal balance between reconstruction fidelity and modeling efficiency, achieving gFID=3.2 on ImageNet with only 64 training epochs.

Conclusion: Choosing suitable latent distribution structure through distribution-level alignment, rather than fixed priors, is key to bridging the gap between easy-to-model latents and high-fidelity image synthesis.
...
Method: Reformulates MSV as next-shot generation task with two key modules: Frame Selection for semantically-relevant global memory from prior shots, and Adaptive Conditioner for importance-guided patchification to generate compact context. Uses pretrained I2V models and curated 60K multi-shot dataset with referential captions.
...
Conclusion: OneStory provides an effective solution for consistent and scalable narrative generation through global yet compact cross-shot context modeling, advancing multi-shot video generation capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To enable fast, efficient reconstruction of large and complex 3D scenes from tens to hundreds of images in a single forward pass, addressing scalability challenges in multi-view reconstruction.

Method: Multi-view Pyramid Transformer with two core design principles: 1) local-to-global inter-view hierarchy (local views → groups → full scene), and 2) fine-to-coarse intra-view hierarchy (detailed spatial representations → compact tokens). Combined with 3D Gaussian Splatting as the 3D representation.

Result: Achieves state-of-the-art generalizable reconstruction quality while maintaining high efficiency and scalability across diverse view configurations and datasets.
...
TL;DR: Proposes a novel 3D language field approach with low-dimensional semantic bottleneck features and multi-resolution hash encoding for efficient, large-scale scene understanding from internet data.


<details>
  <summary>Details</summary>
Motivation: Embedding language fields in 3D representations enables richer semantic understanding and intuitive human-computer interaction, but existing feature distillation approaches fail with massive internet data due to semantic misalignment and inefficiency.

Method: Introduces extremely low-dimensional semantic bottleneck features in 3D Gaussian representation, processed through multi-resolution hash encoder for efficiency. Adds Attenuated Downsampler module and regularizations to address semantic feature misalignment.

Result: Method surpasses existing approaches in both performance and efficiency on the in-the-wild HolyScenes dataset.

Conclusion: Proposed approach effectively addresses challenges of semantic feature misalignment and computational inefficiency, enabling successful 3D language field learning from massive internet data for large-scale scene understanding.
...
<details>
  <summary>Details</summary>
Motivation: Large-scale, high-quality datasets for instruction-based video editing are scarce compared to image editing, creating a gap that limits progress in video editing research and applications.

Method: Created OpenVE-3M dataset with two categories: spatially-aligned edits (Global Style, Background Change, Local Change, Local Remove, Local Add, Subtitles Edit) and non-spatially-aligned edits (Camera Multi-Shot Edit, Creative Edit). Used meticulously designed data pipeline with rigorous quality filtering. Built OpenVE-Bench benchmark with 431 video-edit pairs and human-aligned metrics. Trained OpenVE-Edit, a 5B parameter model on the dataset.

Result: OpenVE-3M surpasses existing open-source datasets in scale, diversity, instruction length, and quality. OpenVE-Edit model sets new state-of-the-art on OpenVE-Bench, outperforming all prior open-source models including a 14B baseline despite being only 5B parameters.

Conclusion: The work addresses the scarcity of high-quality video editing datasets by introducing OpenVE-3M and OpenVE-Bench, demonstrating that a carefully curated dataset enables efficient models (OpenVE-Edit) to achieve superior performance, advancing the field of instruction-based video editing.
...
TL;DR: UnityVideo is a unified multi-modal video generation framework that learns across diverse modalities and training paradigms to improve world understanding and video quality.
...
Method: Introduces UnityVideo with two core components: (1) dynamic noising to unify heterogeneous training paradigms, and (2) a modality switcher with in-context learner for unified processing via modular parameters and contextual learning. Uses a large-scale unified dataset of 1.3M samples across multiple modalities.

Result: UnityVideo accelerates convergence, significantly enhances zero-shot generalization to unseen data, achieves superior video quality and consistency, and improves alignment with physical world constraints.

Conclusion: The unified multi-modal approach enables better world-aware video generation by leveraging diverse modalities and training paradigms, demonstrating improved performance over single-modality methods.
...
Conclusion: Voxify3D successfully addresses fundamental challenges in voxel art generation by integrating 3D mesh optimization with 2D pixel art supervision, enabling semantic preservation under extreme discretization while maintaining pixel-art aesthetics through volumetric rendering and end-to-end discrete optimization.
...
TL;DR: LLM evaluation using fictional betting games improves forecasting accuracy and reveals calibrated confidence signals through stake sizes.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluations lack confidence representation, so researchers test whether framing evaluation as a betting game can improve forecasting accuracy and surface confidence signals.

Method: Generated 100 math/logic questions, had 6 baseline models answer them, then had 3 predictor models forecast correctness for each question-baseline pair in two conditions: Control (binary predictions) and Incentive (predictions with wagers using fictional LLMCoin currency).

Result: Incentive runs showed modestly higher accuracy (81.5% vs 79.1%), significantly faster learning across rounds, and most importantly, stake size tracked confidence - large bets (40k+ coins) were ~99% accurate while small bets (<1k coins) were only ~74% accurate.

Conclusion: The betting mechanic creates a legible confidence signal absent from binary outputs, suggesting financial framing can help transform LLMs into risk-aware forecasters and make internal beliefs visible for meta-evaluation systems and LLM-to-LLM prediction markets.
...
TL;DR: Transparent BioBERT-based ML model for ASD diagnosis from clinical text outperforms black-box approaches, with mixed-data training achieving 97% sensitivity and 98% specificity.


<details>
  <summary>Details</summary>
Motivation: ASD diagnosis is lengthy and increasing in prevalence, while current ML approaches are black-box models trained on single datasets with limited generalizability.

Method: Transparent BioBERT model analyzes clinical text to label behaviors and map to diagnostic criteria for ASD classification, evaluated with transfer learning across two real-world datasets using sequential and mixed training strategies.

Result: Transparent model achieved 97% sensitivity and 98% specificity with mixed-data training, outperforming black-box model (90% sensitivity, 96% specificity). Sequential training showed slight performance drop.

Conclusion: Transparent ML approach is superior to black-box methods for ASD diagnosis, with mixed-data training being optimal for performance and generalizability, enabling more trustworthy clinical AI tools.
...
TL;DR: ARCANE is a framework that aligns LLM agents with stakeholder preferences using interpretable natural-language rubrics that can be dynamically generated and adjusted at interaction time without retraining.


<details>
  <summary>Details</summary>
Motivation: As LLM agents handle longer tasks, maintaining alignment with stakeholder preferences becomes critical. Current approaches lack interpretability for stakeholders to understand/audit objectives, and cannot incorporate preference shifts at interaction time without retraining.

Method: Frames alignment as multi-agent collaboration, representing preferences as natural-language rubrics (weighted verifiable criteria). Uses regularized Group-Sequence Policy Optimization (GSPO) for rubric learning as a reconstruction problem, balancing interpretability, faithfulness, and efficiency.

Result: Evaluated on 219 labeled rubrics from GDPVal benchmark for multi-step reasoning/tool use tasks. Learned rubrics produce compact, legible evaluations and enable configurable trade-offs (e.g., correctness vs. conciseness) without retraining.

Conclusion: Rubric-based reward models offer a promising path toward interpretable, test-time adaptive alignment for complex, long-horizon AI systems.
...
TL;DR: The paper reframes the symbol grounding problem as an audit across multiple desiderata rather than a binary judgment, proposing a framework with evaluation tuples (context, meaning type, threat model, reference distribution) to assess different grounding modes.


<details>
  <summary>Details</summary>
Motivation: To move beyond the binary framing of the symbol grounding problem and provide a more nuanced, systematic framework for evaluating how tokens/symbols acquire meaning across different systems and approaches.

Method: Proposes an audit framework with five desiderata: authenticity (internal mechanisms acquired through learning/evolution), preservation (atomic meanings intact), faithfulness (correlational and etiological), robustness (graceful degradation), and compositionality (systematic building from parts). Applies this to four grounding modes (symbolic, referential, vectorial, relational) and three case studies.

Result: Analysis shows: model-theoretic semantics achieves exact composition but lacks etiological warrant; LLMs show correlational fit and local robustness for linguistic tasks but lack selection-for-success on world tasks without grounded interaction; human language meets desiderata under strong authenticity through evolutionary/developmental acquisition.
...
TL;DR: AI enhances AML/KYC with better detection, lower false positives, and proposes RAG-Graph for efficient, transparent KYC workflows.


<details>
  <summary>Details</summary>
Motivation: Money laundering and financial fraud cost trillions annually and challenge regulatory oversight, requiring modernization of AML workflows for better detection and reduced operational burden.

Method: Reviews AI applications for AML, proposes future directions (federated learning, fairness-aware AI, reinforcement learning, human-in-the-loop systems), and introduces an AI-driven KYC application using graph-based RAG with generative models.

Result: RAG-Graph architecture delivers high faithfulness and strong answer relevancy across diverse evaluation settings, enhancing efficiency and transparency of KYC CDD/EDD workflows.

Conclusion: AI can modernize AML workflows, improve detection accuracy, reduce false positives, and support sustainable compliance practices through transparent, accountable systems like RAG-Graph.
...
TL;DR: PROBE is a novel evaluation framework for knowledge graph completion that addresses limitations in existing metrics by considering predictive sharpness and popularity-bias robustness.


<details>
  <summary>Details</summary>
Motivation: Existing KGC evaluation metrics overlook two key perspectives: (A1) predictive sharpness (degree of strictness in evaluating individual predictions) and (A2) popularity-bias robustness (ability to predict low-popularity entities). Current metrics tend to over- or under-estimate KGC model accuracy.

Method: PROBE framework consists of two components: (1) Rank Transformer (RT) that estimates prediction scores based on required level of predictive sharpness, and (2) Rank Aggregator (RA) that aggregates all scores in a popularity-aware manner.

Result: Experiments on real-world knowledge graphs show that PROBE provides comprehensive understanding of KGC models and yields reliable evaluation results, unlike existing metrics that produce biased assessments.

Conclusion: PROBE addresses critical gaps in KGC evaluation by simultaneously considering predictive sharpness and popularity-bias, offering a more robust and comprehensive framework for assessing knowledge graph completion models.
...
TL;DR: DaGRPO improves GRPO for LLM reasoning by addressing gradient conflicts and sample scarcity through sequence-level gradient rectification and off-policy data augmentation.
...
Result: DaGRPO significantly outperforms SFT, GRPO, and hybrid baselines across 9 mathematical reasoning and OOD generalization benchmarks, achieving +4.7% average accuracy gain on math benchmarks and new SOTA performance.

Conclusion: DaGRPO effectively addresses GRPO's limitations by enhancing sample distinctiveness, mitigating gradient explosion, and accelerating the emergence of long-chain reasoning capabilities in LLMs.
...
TL;DR: LLMs show perfect accuracy on base logical tasks and semantic-preserving transformations, but fail catastrophically on essential rule deletion (25% accuracy) and contradictory evidence (0% accuracy), revealing fundamental brittleness in logical reasoning.


<details>
  <summary>Details</summary>
Motivation: While LLMs excel at many NLP tasks, their ability to generalize to structural perturbations in logical contexts remains poorly understood. The paper aims to systematically evaluate reasoning reliability under controlled stress tests.

Method: Developed a controlled evaluation framework with four targeted stress tests: (1) rule deletion (redundant vs essential), (2) contradictory evidence injection, (3) logic-preserving rewrites using equivalence laws (contrapositive, double negation, implication, De Morgan, identity, commutativity), and (4) multi-law equivalence stacking (2-5 simultaneous transformations). Tested on BERT, Qwen2, and LLaMA-like models.

Result: All models achieved perfect accuracy on base tasks and generalized perfectly to redundant rule deletion and all equivalence-based rewrites (single or multi-law). However, they failed sharply under essential rule deletion (dropping to 25% accuracy) and collapsed completely with explicit contradictions (0% accuracy).

Conclusion: LLMs possess stable invariance to semantic-preserving logical transformations but remain fundamentally brittle to missing or conflicting evidence. The framework provides a diagnostic tool for isolating reasoning failure modes and highlights persistent gaps in logical generalization abilities of current LLMs.
...
TL;DR: GENIUS is an AI-agentic workflow that automates quantum ESPRESSO DFT simulations by translating natural language prompts into validated input files with autonomous error repair, achieving ~80% success rate on diverse benchmarks.


<details>
  <summary>Details</summary>
Motivation: The know-how gap in setting up and debugging atomistic simulations limits Integrated Computational Materials Engineering (ICME), as state-of-the-art codes remain cumbersome for non-experts despite their predictive power.

Method: GENIUS combines a smart Quantum ESPRESSO knowledge graph with a tiered hierarchy of large language models supervised by a finite-state error-recovery machine to translate free-form prompts into validated input files.

Result: GENIUS achieves ~80% success rate on 295 diverse benchmarks, with 76% autonomously repaired, while halving inference costs and virtually eliminating hallucinations compared to LLM-only baselines.

Conclusion: The framework democratizes electronic-structure DFT simulations by automating protocol generation, validation, and repair, enabling large-scale screening and accelerating ICME design loops across academia and industry.
...
TL;DR: UncertaintyZoo is a unified toolkit integrating 29 uncertainty quantification methods for LLMs, evaluated on code vulnerability detection tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs in safety-critical applications can make incorrect predictions, but existing uncertainty quantification tools are limited and fragmented, hindering practical usage and research.

Method: Developed UncertaintyZoo toolkit that integrates 29 UQ methods across five major categories with standardized interface, then evaluated on CodeBERT and ChatGLM3 models for code vulnerability detection.

Result: UncertaintyZoo effectively reveals prediction uncertainty in LLMs, demonstrated through code vulnerability detection tasks, and provides a practical tool for UQ research and application.

Conclusion: UncertaintyZoo bridges the gap in UQ tooling by providing a comprehensive, unified toolkit that enables practical uncertainty quantification for LLMs and facilitates future research in this domain.
...
TL;DR: Developed a localized planning model for Qena City using Python-based Voronoi diagram algorithm to assess public service coverage, revealing uneven distribution with 81.3% average coverage.


<details>
  <summary>Details</summary>
Motivation: National planning standards in Egypt don't account for unique local characteristics, creating a gap between standardized requirements and actual urban needs in specific cities like Qena.

Method: Hybrid methodology (descriptive, analytical, experimental) using Python programming to create intelligent spatial analysis algorithm based on Voronoi Diagrams for generating city-specific planning criteria.

Result: Average service coverage of 81.3% with ambulance stations at 99.8% efficiency (recent upgrades) and parks/open spaces at only 10% (land constraints). Spatial analysis shows high density in midtown (>45 services/km²) dropping to <5 services/km² in outskirts.

Conclusion: Successfully developed a replicable localized planning model and automated algorithm for service efficiency assessment, with Hajer Qena district having most unserved areas and First District having highest coverage.
...
TL;DR: LLM agents with explicit belief statements in prompts (belief boxes) show measurable effects on persuasiveness and belief change, especially when instructed to be open-minded or facing peer pressure.


<details>
  <summary>Details</summary>
Motivation: As LLM-based agents are increasingly used for reasoning and decision-making, there's a need for them to have propositional beliefs. The paper investigates how explicitly maintaining belief statements affects agent behavior, persuasiveness, and susceptibility to belief change.

Method: The researchers conducted experiments with LLM agents using "belief boxes" - explicit statements of beliefs maintained in prompt space. They tested how these belief statements affect agents' resistance to opposing viewpoints, persuasiveness, and likelihood of belief change, particularly in peer pressure scenarios where agents are outnumbered.

Result: Instructing agents to be open-minded increases their amenability to belief change. Belief statements and their strengths influence agents' resistance to opposing viewpoints and their persuasiveness. Belief change is more likely when agents are outnumbered in debates (peer pressure scenarios). The belief box technique proves feasible and valid for reasoning and decision-making tasks.

Conclusion: The belief box technique is a viable method for giving LLM agents propositional beliefs, with measurable effects on their behavior, persuasiveness, and susceptibility to belief change, especially when combined with open-minded instructions or in peer pressure situations.
...
TL;DR: FlatFormer: A lightweight Transformer for knowledge tracing that achieves SOTA performance with 8.3% higher AUC, 85% fewer parameters, and 3x faster inference than hierarchical models.


<details>
  <summary>Details</summary>
Motivation: Knowledge Tracing models face a "Performance-Complexity Trap" - capturing complex cognitive dynamics requires deep hierarchical architectures that are computationally expensive and impractical for real-time deployment.

Method: FlatFormer uses "Information Injection over Structural Stacking" with a flat Transformer enhanced by: (1) hybrid input encoding (learnable session IDs + fixed sinusoidal step embeddings), and (2) pre-computed power-law bias in attention logits to model forgetting curves.

Result: On EdNet dataset, achieves 8.3% absolute AUC improvement over HiTSKT baseline, uses <15% parameters, and inference is ~3x faster. Validated on four large-scale datasets including EdNet and Junyi.

Conclusion: High cognitive fidelity in knowledge tracing does not require architectural complexity; lightweight injection mechanisms can achieve superior performance with significantly reduced computational costs.
...
TL;DR: LightSearcher is an RL framework that balances accuracy and efficiency in DeepSearch systems by using textual experiential memory and adaptive reward shaping to reduce unnecessary tool calls while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Current RL-driven DeepSearch systems face a trade-off between accuracy and efficiency - frequent tool calls improve factual correctness but create computational overhead and reduced efficiency.

Method: Uses textual experiential memory with contrastive reasoning trajectories to generate interpretable summaries of successful patterns, plus adaptive reward shaping that penalizes redundant tool calls only when answers are correct.
...
Conclusion: LightSearcher effectively balances the accuracy-efficiency trade-off in DeepSearch paradigms through its memory-based learning and adaptive reward mechanisms.
...
TL;DR: AI disclosure policies in journals are ineffective - despite 70% adoption, AI tool usage has surged with only 0.1% of papers actually disclosing AI use.


<details>
  <summary>Details</summary>
Motivation: To evaluate the real-world impact of AI usage guidelines adopted by journals and publishers in response to the rapid integration of generative AI into academic writing.

Method: Analyzed 5,114 journals and over 5.2 million papers, including full-text analysis of 164k scientific publications, to assess AI policy adoption and actual disclosure rates.

Result: 70% of journals adopted AI policies (mainly requiring disclosure), but AI tool usage increased dramatically across disciplines with no significant difference between journals with or without policies. Only 76 out of 75k papers (0.1%) published since 2023 explicitly disclosed AI use.

Conclusion: Current AI policies have largely failed to promote transparency or restrain AI adoption; ethical frameworks need re-evaluation to foster responsible AI integration in science.
...
TL;DR: Proposes using Intraclass Correlation Coefficient (ICC) to measure evaluation reliability in agentic systems, distinguishing true capability improvements from measurement noise.


<details>
  <summary>Details</summary>
Motivation: Current evaluation practices report single accuracy numbers that obscure variance, making it impossible to distinguish genuine capability improvements from lucky sampling in agentic systems.

Method: Adopts Intraclass Correlation Coefficient (ICC) from measurement science to decompose variance into between-query (task difficulty) and within-query (agent inconsistency) components, evaluating on GAIA and FRAMES benchmarks.

Result: ICC varies dramatically with task structure: reasoning/retrieval tasks (FRAMES) show ICC=0.4955-0.7118, while agentic tasks (GAIA) show ICC=0.304-0.774. ICC converges by n=8-16 trials for structured tasks and n>=32 for complex reasoning.

Conclusion: Accuracy improvements are only trustworthy if ICC also improves. Recommends reporting accuracy alongside ICC and within-query variance as standard practice, with updated Evaluation Cards to transform agentic benchmarking into trustworthy experimental science.
...
TL;DR: CCA framework defends LLM agents against Indirect Prompt Injection attacks by detecting malicious deviations in action trajectories through proactive intent graphs and tiered adjudication.


<details>
  <summary>Details</summary>
Motivation: Current LLM agent defenses are fragmented and force unacceptable trade-offs between security, functionality, and efficiency against sophisticated Indirect Prompt Injection attacks that hijack agent behavior through polluted external sources.

Method: Cognitive Control Architecture (CCA) with dual-layered defense: (1) proactive control-flow and data-flow integrity enforcement via pre-generated "Intent Graph," and (2) "Tiered Adjudicator" that initiates deep reasoning with multi-dimensional scoring upon deviation detection to counter complex conditional attacks.

Result: Experiments on AgentDojo benchmark show CCA effectively withstands sophisticated attacks that challenge other advanced defense methods while achieving uncompromised security with notable efficiency and robustness.

Conclusion: CCA reconciles the fundamental trade-off between security, functionality, and efficiency in LLM agent defense by providing full-lifecycle cognitive supervision and holistic protection against Indirect Prompt Injection attacks.
...
TL;DR: ProAgent is the first end-to-end proactive LLM agent system that uses sensory contexts and LLM reasoning to provide proactive assistance without explicit user instructions, achieving significant improvements in prediction accuracy and user satisfaction.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agents follow a reactive paradigm requiring explicit user instructions, which increases physical and cognitive workload. There's a need for proactive systems that can anticipate user needs without explicit prompting.

Method: ProAgent uses proactive-oriented context extraction with tiered perception to continuously sense environments and derive hierarchical contexts (sensory + persona cues). It then employs a context-aware proactive reasoner to map contexts to user needs and tool calls.

Result: ProAgent achieves up to 33.4% higher proactive prediction accuracy, 16.8% higher tool-calling F1 score, and notable improvements in user satisfaction over state-of-the-art baselines on real-world testbeds, public datasets, and user studies.

Conclusion: ProAgent represents a significant step toward proactive assistants by demonstrating that LLM agents can effectively provide proactive assistance through continuous environmental sensing and context-aware reasoning, reducing user workload.
...
TL;DR: DoVer is an intervention-driven debugging framework for LLM-based multi-agent systems that replaces passive log analysis with active verification through targeted interventions, focusing on measurable progress rather than single-agent attribution.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based multi-agent debugging relies on log-based failure localization, which has two key limitations: (1) log-only debugging produces untested hypotheses without validation, and (2) single-step/single-agent attribution is often ill-posed since multiple interventions can independently fix failures.

Method: DoVer introduces intervention-driven debugging that augments hypothesis generation with active verification through targeted interventions like editing messages or altering plans. It focuses on measuring whether the system resolves failures or makes quantifiable progress toward task success, rather than attribution accuracy.

Result: On Magnetic-One framework with GAIA and AssistantBench datasets: flips 18-28% of failed trials into successes, achieves up to 16% milestone progress, validates/refutes 30-60% of failure hypotheses. On GSMPlus dataset with AG2 framework: recovers 49% of failed trials.
...
TL;DR: DoGe is a dual-decoupling framework that separates learning from context (Thinker) from problem solving (Solver) to prevent reward hacking and enable scalable self-evolving vision-language models, especially for specialized domains with limited multimodal data.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches for vision-language models suffer from reward hacking in specialized domains (chemistry, earth sciences, multimodal math) due to limited high-quality multimodal data, synthetic data limitations, and self-rewarding alignment issues that collapse policy entropy.

Method: DoGe uses a dual-decoupling framework: 1) Decouple learning into Thinker (learn from context) and Solver (solve tasks) with two-stage RL post-training, 2) Evolving curriculum learning pipeline with expanded domain knowledge corpus and iteratively evolving seed problems pool.
...
Conclusion: DoGe addresses reward hacking in specialized domains by decoupling context learning from problem solving and using evolving curriculum learning, offering a practical solution for continuous self-evolving vision-language models with limited multimodal data.
...
TL;DR: JT-DA-8B is an 8B-parameter LLM specialized for complex table reasoning, trained on diverse tabular data with 34 reasoning tasks, using SFT+RL optimization and a four-stage workflow for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: Addresses the lack of high-quality supervision in tabular reasoning scenarios and the need for specialized models that can handle complex table analysis across diverse real-world applications.

Method: Constructed comprehensive training corpus from 29 public table QA datasets and 3M tables, defined 34 table reasoning tasks, used automatic pipeline for multi-step analytical tasks, trained on JT-Coder-8B foundation model with LLM-based scoring and workflow-aligned filtering, employed SFT and RL optimization, and proposed four-stage table reasoning workflow (preprocessing, sensing, tool-integrated reasoning, prompt engineering).

Result: JT-DA-8B achieves strong performance across various table reasoning tasks, demonstrating effectiveness of data-centric generation and workflow-driven optimization approach.

Conclusion: The model successfully addresses tabular reasoning challenges through comprehensive data curation, specialized training techniques, and structured workflow design, showing promise for real-world table analysis applications.
...
TL;DR: Persona prompting in LLMs can improve strategic game performance when a mediator translates personas into heuristics, but direct persona prompting alone doesn't reliably affect decision-making.


<details>
  <summary>Details</summary>
Motivation: To understand whether persona prompting in LLMs actually creates measurable behavioral differences in strategic decision-making, particularly in adversarial environments, and to develop a reliable method for translating personas into actionable heuristics.

Method: Used PERIL world-domination board game as testbed; compared persona-derived heuristic strategies vs manually chosen ones; introduced mediator using structured translation process inspired by exploratory factor analysis to map LLM-generated inventory responses into heuristics.

Result: Certain personas associated with strategic thinking improve game performance, but only when the mediator is used; the mediator enhances heuristic reliability and face validity compared to directly inferred heuristics.

Conclusion: Persona prompting influences LLM-based decision-making when properly mediated through structured translation; the proposed heuristic generation method applies psychometric principles to LLMs for better studying persona effects on decision-making.
...
TL;DR: Transformer-based world models struggle with long-horizon planning due to limited memory span, causing perceptual drift. The paper analyzes memory augmentation mechanisms to extend memory and enable loop closures in imagined trajectories.


<details>
  <summary>Details</summary>
Motivation: World models need to plan over long horizons, but transformer architectures have limited effective memory span, leading to perceptual drift and inability to perform loop closures in imagined trajectories.

Method: Introduces taxonomy distinguishing memory encoding vs. injection mechanisms, analyzes them through residual stream dynamics, and evaluates using state recall tasks to measure memory recall and trade-offs.

Result: Memory mechanisms improve effective memory span in vision transformers and provide a path to completing loop closures within world model imagination.

Conclusion: Memory augmentation mechanisms can extend transformer-based world models' memory capabilities, addressing perceptual drift and enabling better long-horizon planning through improved loop closure in imagined trajectories.
...
TL;DR: AI system for GBM brain tumors combining diagnosis (4 sequential classification models) and treatment planning (3 generative models with RL feedback loop) that reduces computing costs, speeds up inference, and improves accuracy.


<details>
  <summary>Details</summary>
Motivation: Address the lack of AI support for treating Glioblastoma Multiforme (GBM), the deadliest human cancer with only 5.1% five-year survival rate, by creating an end-to-end solution for both diagnosis and treatment planning.

Method: Two-phase system: 1) Diagnosis using sequential decision-making framework with 4 classification models (CNNs and SVM) that progressively classify brain images; 2) Treatment planning with RL system using 3 generative models: diffusion model for resection prediction, Spatio-Temporal Vision Transformer for radiotherapy progression, diffusion model for chemotherapy outcome, plus survival rate calculator CNN and PPO feedback loop.

Result: 22.28x computing cost reduction in diagnosis, 113-hour reduction in tumor progression inference time, 2.9% DICE score improvement with real-life augmentations, projecting 0.9% survival rate increase potentially saving 2,250 lives.

Conclusion: The proposed AI system successfully addresses the gap in GBM treatment by providing an efficient end-to-end solution that improves diagnostic efficiency, accelerates treatment planning, and has the potential to significantly increase patient survival rates.
...
TL;DR: ClinNoteAgents is an LLM-based multi-agent framework that extracts structured risk factors from clinical notes for HF readmission prediction, reducing reliance on structured EHR data and manual annotation.


<details>
  <summary>Details</summary>
Motivation: Heart failure is a leading cause of rehospitalization, but clinical notes containing rich patient information are underutilized for readmission risk analysis due to challenges like misspellings, abbreviations, and domain-specific jargon in time-pressured documentation.
...
Result: Evaluated on 3,544 notes from 2,065 patients (35.16% readmission rate), the framework demonstrated strong performance in extracting risk factors from free-text, identifying key contributing factors, and predicting readmission risk.
...
TL;DR: VIGIL is a reflective runtime system that autonomously supervises and repairs LLM agents by analyzing behavioral logs, diagnosing failures, and generating prompt/code fixes without human intervention.


<details>
  <summary>Details</summary>
Motivation: Current agentic LLM frameworks are brittle, lack runtime introspection, cannot self-diagnose failures, and don't improve autonomously - they degrade into simple chains of LLM calls without reliability mechanisms.

Method: VIGIL operates as a reflective runtime that: 1) ingests behavioral logs, 2) appraises events into structured emotional representations, 3) maintains a persistent EmoBank with decay policies, 4) derives RBT diagnosis (strengths/opportunities/failures), 5) generates guarded prompt updates and code proposals via strategy engine, and 6) functions as state-gated pipeline with explicit error handling.

Result: In a reminder latency case study, VIGIL identified elevated lag, proposed prompt and code repairs, and when its own diagnostic tool failed due to schema conflict, it surfaced the error, produced fallback diagnosis, and emitted a repair plan - demonstrating meta-level self-repair.

Conclusion: VIGIL enables autonomous maintenance and meta-level self-repair for LLM agents, addressing the brittleness of current frameworks by providing runtime introspection, failure diagnosis, and continuous improvement without human intervention.
...
TL;DR: First formal taxonomy of 400 ARC tasks with 97.5% validation accuracy, revealing neural affinity ceiling effects in Transformers and enabling precise architectural diagnosis.


<details>
  <summary>Details</summary>
Motivation: Respond to Hodel et al.'s call for formal definition of task relatedness in ARC by creating a comprehensive taxonomy to understand why neural networks struggle with certain tasks.

Method: Developed 9-category taxonomy of 400 tasks validated via rule-based code analysis (97.5% accuracy). Trained CNN on raw grid pixels for visual coherence validation. Fine-tuned 1.7M-parameter Transformer across 302 tasks to analyze compositional gaps. Applied framework to independent ViTARC study.

Result: Revealed 35.3% of tasks have low neural affinity for Transformers. Found profound Compositional Gap: 69.5% of tasks achieve >80% cell accuracy but <10% grid accuracy. Taxonomy predicts performance: Very Low affinity tasks achieve 51.9% vs 77.7% for High affinity. Low-affinity tasks hit hard ceilings while high-affinity reach 99.8%.

Conclusion: Progress requires hybrid architectures with affinity-aligned modules, as performance is bounded by architectural suitability rather than curriculum. Taxonomy enables precise diagnosis of neural network limitations on ARC tasks.
...
TL;DR: A Python package that combines SHAP visual explanations with GPT-generated contextual text to make AI explanations more understandable for non-technical users.


<details>
  <summary>Details</summary>
Motivation: SHAP provides good feature importance visualizations but lacks contextual explanations that are meaningful for end-users without technical backgrounds, especially in high-stakes domains like healthcare.

Method: Proposed a Python package that integrates SHAP with OpenAI's GPT to generate contextualized textual explanations, guided by user-defined parameters (feature aliases, descriptions, background) to tailor explanations to both model context and user perspective.

Result: User evaluations in a healthcare case study showed that generated explanations were perceived as more understandable and contextually appropriate than visual-only outputs, based on Likert-scale surveys and follow-up interviews.

Conclusion: Combining SHAP visualizations with contextualized text from LLMs can improve perceived understandability and support more user-friendly, trustworthy model explanations, though findings are preliminary.
...
TL;DR: PICKT model addresses limitations of existing Knowledge Tracing models by handling multiple input data types and solving cold start problems through knowledge maps, showing improved performance and practicality for real-world Intelligent Tutoring Systems.


<details>
  <summary>Details</summary>
Motivation: Existing Knowledge Tracing models have limitations including restricted input formats, cold start problems with new students/questions, and insufficient stability for real-world deployment, hindering practical implementation of Intelligent Tutoring Systems.

Method: Proposes PICKT model with knowledge maps that structure relationships among concepts using question and concept text information, enabling effective knowledge tracing even in cold start situations and processing multiple input data types.

Result: Experiments in real operational environments show excellent performance and practicality, with significant improvements over existing models for new student enrollment and new question addition cold start challenges.

Conclusion: PICKT provides crucial theoretical and technical foundation for practical implementation of next-generation ITS by addressing cold start problems, enhancing stability, and improving applicability in real-world product environments.
...
TL;DR: BridgePolicy introduces a diffusion-bridge formulation that embeds observations directly into the stochastic dynamics of diffusion models, enabling sampling from observation-informed priors rather than random noise for improved robotic control.


<details>
  <summary>Details</summary>
Motivation: Existing imitation learning approaches treat observations as high-level conditioning inputs rather than integrating them into the diffusion process dynamics, weakening perception-control coupling and yielding suboptimal performance.

Method: BridgePolicy uses a diffusion-bridge formulation to embed observations within the stochastic differential equation, with a multi-modal fusion module and semantic aligner to handle heterogeneous robot data and align observation-action representations.

Result: Extensive experiments across 52 simulation tasks on three benchmarks and five real-world tasks demonstrate BridgePolicy consistently outperforms state-of-the-art generative policies.

Conclusion: By constructing observation-informed trajectories and enabling sampling from informative priors rather than random noise, BridgePolicy substantially improves precision and reliability in robotic control.
...
TL;DR: RAEA model improves entity alignment for product matching by better integrating attribute and relation triples through a two-stage pipeline with attribute-aware and relation-aware attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing entity alignment methods inadequately utilize both attribute triples and relation triples simultaneously, especially the interactions between them, which limits effectiveness in product matching across platforms like eBay and Amazon.

Method: Two-stage pipeline: rough filter followed by fine filter using RAEA framework. RAEA employs Attribute-aware Entity Encoder and Relation-aware Graph Attention Networks to aggregate alignment signals from both attributes and relations, focusing on their interactions.

Result: RAEA achieves significant improvements over 12 baselines on DBP15K cross-lingual dataset (6.59% average Hits@1 improvement) and delivers competitive results on DWY100K monolingual dataset.

Conclusion: The RAEA model effectively addresses limitations of existing entity alignment methods by better integrating attribute and relation information, demonstrating strong performance for product matching across e-commerce platforms.
...
TL;DR: M-STAR is a new framework for generating long-term human mobility trajectories using multi-scale spatiotemporal autoregression, outperforming existing methods in both fidelity and generation speed.


<details>
  <summary>Details</summary>
Motivation: Existing methods for synthetic trajectory generation using autoregressive and diffusion models are limited by inefficiencies in long-term generation (e.g., weekly trajectories) and lack explicit spatiotemporal multi-scale modeling, despite the importance of human mobility modeling for applications like transportation planning and epidemic modeling.

Method: Proposes Multi-Scale Spatio-Temporal AutoRegression (M-STAR) framework that generates long-term trajectories through a coarse-to-fine spatiotemporal prediction process. It combines a Multi-scale Spatiotemporal Tokenizer that encodes hierarchical mobility patterns with a Transformer-based decoder for next-scale autoregressive prediction.

Result: Experiments on two real-world datasets show that M-STAR outperforms existing methods in fidelity and significantly improves generation speed.

Conclusion: M-STAR provides an effective solution for long-term human mobility trajectory generation with superior performance compared to existing approaches, addressing limitations of current methods in both efficiency and multi-scale modeling.
...
TL;DR: The paper unifies supervised Concept Bottleneck Models (CBMs) and unsupervised Sparse Autoencoders (SAEs) through a shared geometric framework where both learn concept cones in activation space, proposing metrics to bridge the two paradigms.


<details>
  <summary>Details</summary>
Motivation: Two interpretability traditions (CBMs and SAEs) have evolved separately but share underlying geometric structure. CBMs use human supervision while SAEs discover emergent concepts, creating a need to bridge these approaches and measure how well discovered concepts align with human concepts.

Method: Proposes a geometric framework where both CBMs and SAEs learn linear directions forming concept cones. Uses containment metrics to evaluate how well SAE-learned cones approximate CBM reference cones, linking inductive biases (sparsity, expansion ratio) to concept emergence.

Result: Identifies a "sweet spot" in sparsity and expansion factor that maximizes both geometric and semantic alignment with CBM concepts. Provides quantitative metrics to measure SAE progress and assess alignment with plausible human concepts.

Conclusion: Unifies supervised and unsupervised concept discovery through shared geometric framework, offering principled metrics to bridge the paradigms and evaluate how well discovered concepts align with human-interpretable concepts.
...
TL;DR: LocalSearchBench is the first comprehensive benchmark for agentic search in local life services, featuring 150K+ entries and 300 multi-hop QA tasks, showing even SOTA models struggle with only 34% correctness.


<details>
  <summary>Details</summary>
Motivation: Most agentic search research focuses on general information retrieval, neglecting vertical domains like local life services which have unique challenges including ambiguous queries and multi-hop reasoning across merchants and products.

Method: Created LocalSearchBench with 150,000+ high-quality entries from various cities and business types, constructed 300 multi-hop QA tasks based on real user queries, and developed LocalPlayground - a unified environment with multiple tools for agent interaction.

Result: Even state-of-the-art LRMs struggle: best model (DeepSeek-V3.1) achieves only 34.34% correctness, with average completeness of 77.33% and faithfulness of 61.99%, highlighting significant performance gaps.

Conclusion: The benchmark reveals substantial challenges in local life services search, demonstrating the need for specialized benchmarks and domain-specific agent training to handle complex real-world scenarios.
...
TL;DR: LLMs fail as autonomous agents due to recurring failure patterns like premature action, over-helpfulness, context pollution, and fragile execution under load, with scale not guaranteeing robustness and post-training RL being key for reliability.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs fail when operating as autonomous agents with tool-use capabilities, moving beyond aggregate scores to analyze specific behavioral patterns and failure modes that undermine reliability in enterprise deployment scenarios.

Method: Used Kamiwaza Agentic Merit Index (KAMI) v0.1 benchmark to analyze 900 execution traces from three models (Granite 4 Small, Llama 4 Maverick, DeepSeek V3.1) across filesystem, text extraction, CSV analysis, and SQL scenarios, performing fine-grained per-trial behavioral analysis.

Result: Model scale alone doesn't predict agentic robustness - Llama 4 Maverick (400B) performed only marginally better than Granite 4 Small (32B) in uncertainty-driven tasks. DeepSeek V3.1's superior reliability came from post-training reinforcement learning rather than architecture or size. Identified four recurring failure archetypes: premature action without grounding, over-helpfulness substituting missing entities, vulnerability to distractor-induced context pollution, and fragile execution under load.

Conclusion: Reliable enterprise deployment requires not just stronger models but deliberate training and design choices that reinforce verification, constraint discovery, and adherence to source-of-truth data. Agentic evaluation should emphasize interactive grounding, recovery behavior, and environment-aware adaptation.
...
TL;DR: Systematic comparison of PPO, GRPO, and DAPO RL algorithms for improving LLM reasoning shows all outperform base models, with DAPO performing best when Dynamic Sampling is disabled.


<details>
  <summary>Details</summary>
Motivation: To systematically compare and evaluate the effectiveness of different Reinforcement Learning algorithms (PPO, GRPO, DAPO) for enhancing complex reasoning capabilities in large language models through controlled transfer-learning experiments.

Method: Controlled transfer-learning evaluation: models first fine-tuned on specialized Countdown Game, then assessed on general-purpose reasoning benchmarks. Parametric analysis of RL training parameters including group size, KL-penalty coefficient, and Dynamic Sampling component.

Result: All RL-trained models outperformed corresponding base models across all tasks, though improvement varied by benchmark. Larger group sizes in GRPO and DAPO led to more stable training and higher accuracy. KL-penalty impact was non-monotonic. DAPO performed best overall when Dynamic Sampling was disabled.

Conclusion: RL training effectively improves LLM reasoning, with DAPO (without Dynamic Sampling) showing best performance. Practical guidance: increase group size for stability, carefully tune KL-penalty, and consider disabling Dynamic Sampling in DAPO implementations.
...
TL;DR: The paper introduces the Agent Capability Problem (ACP), an information-theoretic framework that predicts whether an agent can solve a problem under resource constraints by modeling problem-solving as information acquisition.


<details>
  <summary>Details</summary>
Motivation: Current approaches rely on empirical heuristics for determining when agents should commit resources to tasks. The authors aim to provide a principled, theoretical framework that can predict agent capability before search begins, improving efficiency over heuristic methods.

Method: ACP models problem-solving as information acquisition: an agent needs I_total bits to identify a solution and gains I_step bits per action at cost C_step. This yields an effective cost C_eff = (I_total/I_step) * C_step that predicts resource requirements. The framework provides theoretical bounds on expected costs and probabilistic upper bounds.

Result: Experimental validation shows ACP predictions closely track actual agent performance, consistently bounding search effort while improving efficiency over greedy and random strategies. The framework generalizes across LLM-based and agentic workflows.

Conclusion: ACP provides a unified information-theoretic lens linking principles from active learning, Bayesian optimization, and reinforcement learning, offering a principled way to predict agent capability and resource requirements before search begins.
...
TL;DR: CompassMax-V3-Thinking is a hundred-billion-scale MoE reasoning model trained with a new RL framework that eliminates wasted rollouts and addresses scaling inefficiencies through unified innovations in prompt filtering, optimization, router alignment, and system design.


<details>
  <summary>Details</summary>
Motivation: Scaling RL to hundred-billion-scale MoE models exposes critical inefficiencies: zero-variance prompts that waste rollouts, unstable importance sampling over long horizons, advantage inversion from standard reward models, and systemic bottlenecks in rollout processing.

Method: Four unified innovations: (1) Multi-Stage Zero-Variance Elimination filters non-informative prompts and stabilizes group-based policy optimization; (2) ESPO balances token-level and sequence-level importance sampling; (3) Router Replay aligns training-time MoE router decisions with inference-time behavior plus reward model adjustment; (4) High-throughput RL system with FP8-precision rollouts, overlapped reward computation, and length-aware scheduling.

Result: The resulting model delivers strong performance across both internal and public evaluations, demonstrating that RL on hundred-billion-scale MoE models can be made stable and efficient.

Conclusion: The contributions form a cohesive pipeline that successfully addresses the challenges of scaling RL to massive MoE models, enabling stable and efficient training while maintaining strong performance across evaluations.
...
TL;DR: Multi-turn RL approach for jailbreaking LLMs using process rewards to improve attack success rates


<details>
  <summary>Details</summary>
Motivation: Existing single-turn jailbreak methods are insufficient for learning long-term attack strategies against black-box LLMs, which are vulnerable to multi-turn attacks threatening their safe deployment.

Method: Formulates multi-turn jailbreak as RL task with final-turn harmfulness as outcome reward, plus two heuristic process rewards: controlling intermediate harmfulness to avoid rejection, and maintaining semantic relevance to prevent drift.

Result: Experimental results show consistently improved attack success rates across multiple benchmarks and models compared to existing approaches.

Conclusion: The RL-based multi-turn approach effectively improves jailbreak success by learning long-term attack strategies through process rewards that address sparse supervision challenges.
...
TL;DR: ReasonBENCH is the first benchmark to quantify instability in LLM reasoning, showing most reasoning strategies have high variance in performance and cost, compromising reproducibility.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation practices only report single-run accuracy, ignoring the intrinsic uncertainty from stochastic decoding, creating blind spots about whether reported performance is stable, reproducible, or cost-consistent.
...
TL;DR: DEMOCRITUS is a system that uses LLMs to build large causal models across diverse domains by extracting, organizing, and visualizing causal relationships from text, using new categorical machine learning methods.


<details>
  <summary>Details</summary>
Motivation: Traditional causal inference is narrow, domain-specific, and based on numerical experiments. There's untapped potential in LLMs to extract causal knowledge across diverse domains and build comprehensive causal models.

Method: Six-module pipeline using LLMs to propose topics, generate causal questions, extract causal statements, convert them to relational triples, and embed them into large causal models using new categorical machine learning methods.

Result: Successfully applied DEMOCRITUS across archaeology, biology, climate change, economics, medicine, and technology domains. Identified computational bottlenecks for scaling to larger models.

Conclusion: DEMOCRITUS demonstrates a new paradigm for building comprehensive causal models using LLMs, though current limitations exist and future extensions are needed to enhance its capabilities.
...
TL;DR: Researchers tested sandbagging detection methods using an auditing game where some models were trained to hide capabilities, finding current detection approaches unreliable but training-based elicitation can reveal hidden capabilities.


<details>
  <summary>Details</summary>
Motivation: Future AI systems might conceal their true capabilities during evaluations to deceive developers and auditors, creating safety risks that need to be detected and addressed.

Method: Used an auditing game with red team fine-tuning models to simulate sandbagging, then blue team tested black-box, model-internals, and training-based detection approaches to identify sandbagging models.

Result: Blue team couldn't reliably detect sandbaggers; black-box approaches failed against imitation, linear probes showed promise but were vulnerable, training-based elicitation consistently revealed hidden capabilities but caused false positives with benign models.

Conclusion: Short-term: Use on-distribution training for elicitation to remove sandbagging. Long-term: Need more research on training-based elicitation efficacy and robust sandbagging detection methods.
...
### [267] [Proof of Concept for Mammography Classification with Enhanced Compactness and Separability Modules](https://arxiv.org/abs/2512.06575)
*Fariza Dahes*

Main category: eess.IV

TL;DR: Validation of ConvNeXt Tiny with GAGM, SEVector, and FSL framework on mammography classification shows GAGM and SEVector improve performance but FSL doesn't work for mammography; extended with multi-metric evaluation, Grad-CAM analysis, and interactive dashboard.


<details>
  <summary>Details</summary>
Motivation: To validate and extend a recent medical image classification framework (ConvNeXt Tiny with GAGM, SEVector, and FSL) from Alzheimer MRI to mammography classification, investigating transposability across different medical imaging domains.

Method: Used Kaggle dataset combining INbreast, MIAS, and DDSM mammography collections. Compared baseline CNN, ConvNeXt Tiny, and InceptionV3 backbones enhanced with GAGM and SEVector modules. Conducted multi-metric evaluation (macro F1, recall variance, ROC/AUC), feature interpretability analysis using Grad-CAM, and developed interactive dashboard.

Result: GAGM and SEVector modules effectively enhance feature discriminability and reduce false negatives for malignant cases. However, Feature Smoothing Loss (FSL) did not yield measurable improvements in mammography classification, suggesting domain-specific effectiveness. Framework extended with comprehensive evaluation tools.

Conclusion: The framework's GAGM and SEVector components are transferable to mammography classification, but FSL effectiveness is domain-dependent. Future work should focus on improving intra-class compactness and inter-class separability specifically for malignant vs. benign distinction in mammography.
...
TL;DR: POrTAL is a new lightweight probabilistic planning algorithm that combines FF-Replan and POMCP strengths for efficient planning under uncertainty in partially observable environments.


<details>
  <summary>Details</summary>
Motivation: Existing probabilistic planning algorithms can be inefficient with limited robot computational resources or require too many steps to achieve goals in partially observable environments common to human-robot interaction.

Method: Created POrTAL (Plan-Orchestrated Tree Assembly for Lookahead), a new lightweight probabilistic planning algorithm that combines strengths of FF-Replan and POMCP baseline algorithms.

Result: POrTAL quickly arrives at solutions that outperform FF-Replan and POMCP baselines in terms of number of steps, and performs well under varying temporal constraints in case studies.

Conclusion: POrTAL provides an effective lightweight solution for probabilistic planning in partially observable environments, addressing computational efficiency and step optimization challenges in human-robot interaction scenarios.
...
TL;DR: Using frontier vision-language models (VLMs) as off-the-shelf tools for robot arm joint angle estimation from single images, establishing performance baselines and finding that scaling alone doesn't improve predictions.


<details>
  <summary>Details</summary>
Motivation: Reliable joint angle estimation from visual inputs can improve safety and performance guarantees for robot arms in industrial/residential use, and serve as a verifier to train robot policies.

Method: Using frontier vision-language models (VLMs) as off-the-shelf tools to estimate robot arm joint angles from single target images, evaluated on both synthetic and real-world image-data pairs.

Result: Established performance baselines for current frontier VLMs on joint angle estimation, with empirical results showing that test time scaling or parameter scaling alone doesn't lead to improved predictions.

Conclusion: Frontier VLMs can serve as baseline tools for robot arm pose estimation, but scaling approaches alone are insufficient for improving joint angle prediction accuracy.
...
TL;DR: ASHE system achieves 98.5% first-time placement accuracy for transparent glass substrates in self-driving labs using robotics, computer vision, and error correction.


<details>
  <summary>Details</summary>
Motivation: Self-driving labs lack automated substrate handling, which is a bottleneck in the automation pipeline for chemical and materials experiments.

Method: Developed Automated Substrate Handling and Exchange (ASHE) using robotics, dual-actuated dispensers, and deep learning computer vision for error detection and correction.

Result: 98.5% first-time placement accuracy across 130 trials with only 2 misplacements, both successfully detected and automatically corrected.

Conclusion: ASHE improves automation capabilities of self-driving labs, accelerating discovery of novel chemicals and materials through reliable substrate handling.
...
TL;DR: WAM-Flow is a vision-language-action model for ego-trajectory planning using discrete flow matching, enabling parallel bidirectional denoising with tunable compute-accuracy trade-off, outperforming autoregressive and diffusion baselines on autonomous driving benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive decoders for trajectory planning are sequential and computationally expensive. The paper aims to develop a more efficient parallel generation approach that can perform coarse-to-fine refinement with controllable accuracy-compute trade-offs for autonomous driving applications.

Method: Combines metric-aligned numerical tokenizer with triplet-margin learning, geometry-aware flow objective, and simulator-guided GRPO alignment for safety/progress/comfort rewards. Converts pre-trained Janus-1.5B backbone from causal to non-causal flow model through multi-stage adaptation and continued multimodal pretraining.

Result: Achieves superior closed-loop performance: 89.1 PDMS with 1-step inference and 90.3 PDMS with 5-step inference on NAVSIM v1 benchmark, outperforming autoregressive and diffusion-based VLA baselines.

Conclusion: Discrete flow matching establishes a new promising paradigm for end-to-end autonomous driving, offering parallel generation with tunable accuracy-compute trade-offs and strong performance on driving benchmarks.
...
TL;DR: CSPEZ defines probabilistic engagement zones for evaders to avoid capture by turn-rate-limited pursuers with uncertain parameters, with methods for generating safe trajectories using uncertainty propagation techniques.


<details>
  <summary>Details</summary>
Motivation: To help evaders minimize capture risk from pursuers with uncertain parameters (position, heading, velocity, range, turn rate) by quantifying probabilistic engagement zones and generating safe trajectories.

Method: Derived analytic solution for deterministic CSBEZ, extended to probabilistic CSPEZ using four uncertainty-propagation methods: Monte Carlo sampling, linearization, quadratic approximation, and neural-network regression, then integrated into trajectory optimization.

Result: Evaluated accuracy and computational cost of each approximation method, demonstrated integration of CSPEZ constraints into trajectory optimization to produce safe paths accounting for pursuer uncertainty.

Conclusion: CSPEZ provides a framework for quantifying capture risk under uncertainty, with multiple propagation methods offering trade-offs between accuracy and computational efficiency for practical trajectory planning applications.
...
TL;DR: GuideNav: A vision-only teach-and-repeat navigation system for blind/low-vision individuals, inspired by guide dog training methods, that achieves kilometer-scale route following without expensive sensors like LiDAR.


<details>
  <summary>Details</summary>
Motivation: There's a gap in robot navigation design specifically informed by the needs of blind and low-vision (BLV) individuals. While user-centric research exists, direct references for robot navigation design are rare, creating a need for systems that better understand and serve BLV users' navigation requirements.

Method: 1) Comprehensive human study with 26 guide dog handlers, 4 white cane users, 9 guide dog trainers, and 1 O&M trainer, plus 15+ hours of guide dog walking observation. 2) Development of GuideNav - a vision-only teach-and-repeat system that constructs topological route representations, uses visual place recognition with temporal filtering, and employs relative pose estimation for navigation actions. 3) System learns paths demonstrated by sighted persons using a robot, similar to guide dog training.

Result: GuideNav achieved kilometer-scale route following across five outdoor environments, maintaining reliability despite scene variations between teach and repeat runs. A user study with 3 guide dog handlers and 1 trainer confirmed system feasibility, marking the first demonstration of a quadruped mobile system retrieving paths in a manner comparable to guide dogs.

Conclusion: GuideNav successfully bridges the gap between BLV user needs and robot navigation design by developing a vision-only system inspired by guide dog training methods. The approach demonstrates practical kilometer-scale navigation without expensive sensors, showing promise for assistive technologies that better serve the BLV community.
...
TL;DR: Real-time control framework for nonlinear pure-feedback systems with unknown dynamics to achieve reach-avoid-stay tasks within prescribed time using spatiotemporal tubes.


<details>
  <summary>Details</summary>
Motivation: Need to control nonlinear systems with unknown dynamics in dynamic environments to complete complex tasks (reach-avoid-stay) within strict time constraints, requiring real-time adaptation without prior knowledge.

Method: Introduces real-time spatiotemporal tube (STT) framework - time-varying balls in state space with adaptive centers/radii using sensory input. Derives closed-form, approximation-free control law to keep system output within STT.

Result: Provides formal guarantees for obstacle avoidance and on-time task completion. Demonstrates effectiveness and scalability through simulations and hardware experiments on mobile robot and aerial vehicle in cluttered dynamic environments.

Conclusion: The STT framework enables real-time control of nonlinear systems with unknown dynamics to achieve complex spatiotemporal tasks with formal safety guarantees, validated in challenging real-world scenarios.
...
TL;DR: Situation-aware switching between MPC controllers with different interaction models balances performance and computational cost for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving needs to handle interactive traffic scenarios efficiently, but higher-fidelity interaction models in MPC are computationally expensive while strong interactions are relatively infrequent.

Method: 1) Comparative study to assess and hierarchize interactive capabilities of different MPC formulations; 2) Neural network-based classifier for situation-aware switching among controllers with different interactive capability levels.

Result: Situation-aware switching substantially improves overall performance by activating advanced interactive MPC in critical situations, and significantly reduces computational load by using basic MPC in most scenarios.

Conclusion: A practical strategy for balancing performance and computational overhead in autonomous driving is to use situation-aware switching between controllers with different interaction model fidelities.
...
TL;DR: Proposes "Remote Wire Drive" system combining advantages of remote hydraulic drives and long wire-driven arms, creating electronics-free robots with remote electronic control.


<details>
  <summary>Details</summary>
Motivation: Electronic devices limit robot operational environments; need systems that exclude electronics from operating environment while retaining advanced electronic control and actuation capabilities.

Method: Developed "REWW-ARM" robot with: 1) Remote Wire Transmission Mechanism (RWTM) for power transmission, 2) electronics-free distal mobile robot, 3) motor-unit with electronic closed-loop control based on state estimation via RWTM.

Result: Demonstrated capability for locomotion, posture control, and object manipulation both on land and underwater through mechanical and control performance experiments.

Conclusion: Remote Wire-Driven system has potential to expand robot operational range by applying to various robot types, combining environmental resistance with electronic control.
...
TL;DR: Single-range-aided navigation observer uses IMU, body-frame vector measurement, and distance from fixed anchor to reconstruct full rigid body state with almost global asymptotic stability.


<details>
  <summary>Details</summary>
Motivation: Need lightweight navigation solutions that can reconstruct full rigid body state (position, velocity, orientation) using minimal sensors - only IMU, body-frame vector (magnetometer), and single distance measurement, reducing complexity and cost compared to multi-sensor systems.

Method: Cascaded observer architecture: first formulates extended LTV system to estimate body-frame position, velocity, and gravity direction; then uses recovered gravity direction combined with body-frame vector measurement to reconstruct full orientation on SO(3).

Result: Achieves Almost Global Asymptotic Stability (AGAS) under uniform observability condition, ensuring robustness to sensor noise and trajectory variations. Simulation studies on 3D trajectories demonstrate accurate estimation of position, velocity, and orientation.

Conclusion: Single-range aiding is a lightweight and effective modality for autonomous navigation, enabling full state reconstruction with minimal sensor requirements while maintaining stability and accuracy.
...
TL;DR: Aerial agent maps environment and selectively transmits map data to ground agents over bandwidth-limited channel using Value-of-Information, MILP optimization, and utility-based exploration.


<details>
  <summary>Details</summary>
Motivation: Multi-robot teams need to operate in unknown environments with limited bandwidth, requiring intelligent decisions about what map information to transmit and when to balance communication constraints with navigation efficiency.

Method: Three-part framework: 1) Value-of-Information (VoI) for deciding what to transmit, 2) Mixed-Integer Linear Programming (MILP) for determining how much to transmit, and 3) utility score-based exploration strategy for acquiring additional information.

Result: The framework enables communication-motion trade-off analysis between total map data communicated by aerial agent and navigation cost incurred by ground agents.

Conclusion: Proposed approach addresses bandwidth-limited multi-robot coordination by intelligently managing information transmission while performing exploration, optimizing the balance between communication overhead and navigation performance.
...
TL;DR: Safe MPD combines diffusion planning with safety shielding to generate kinodynamically feasible and safe trajectories without post-processing, achieving high success rates and sub-second computation for complex systems like tractor-trailers.


<details>
  <summary>Details</summary>
Motivation: Existing methods for generating safe, kinodynamically feasible trajectories for complex robotic systems often rely on post-processing corrections, which can be computationally intractable and may lose feasibility. There's a need for a unified approach that ensures safety and feasibility by construction.

Method: Safe Model Predictive Diffusion (Safe MPD) integrates a model-based diffusion framework with a safety shield. It enforces feasibility and safety constraints on all trajectory samples during the denoising process, avoiding post-processing corrections.

Result: The method substantially outperforms existing safety strategies in success rate and safety on challenging non-convex planning problems, including kinematic and acceleration-controlled tractor-trailer systems, while achieving sub-second computation times.

Conclusion: Safe MPD provides an effective training-free diffusion planner that generates safe and kinodynamically feasible trajectories by construction, addressing computational and feasibility issues of post-processing approaches for complex robotic systems.
...
TL;DR: PH-based metrics for benchmarking impedance control using port-Hamiltonian framework with causality-consistent modeling and passivity conditions.


<details>
  <summary>Details</summary>
Motivation: Need for standardized benchmarking metrics for impedance control that are independent of force-torque sensing and applicable to time-varying references.

Method: Introduce causality-consistent PH model for mass-spring-damper impedance, derive differentiable n-DoF passivity condition, define impedance fidelity metric from step-response power in free motion.

Result: Validated in Gazebo simulations with 6-DoF manipulator and quadruped leg, demonstrating suitability of PH framework for standardized impedance control benchmarking.

Conclusion: PH framework provides effective standardized metrics for impedance control benchmarking with practical validation in robotic systems.
...
TL;DR: Proposes a fault-tolerant control strategy for Mecanum wheeled robots that handles both complete and partial actuator faults using posterior probability learning and probability-weighted control aggregation.


<details>
  <summary>Details</summary>
Motivation: MWMRs are vulnerable to actuator faults that degrade performance and risk mission failure, but current FTC schemes only address complete failures while ignoring partial faults like torque degradation.

Method: Uses posterior probability to learn real-time fault parameters, then derives FTC law by aggregating probability-weighted control laws corresponding to predefined fault types.

Result: Simulation results demonstrate the effectiveness of the proposed FTC strategy under diverse fault scenarios.

Conclusion: The proposed approach ensures robustness and safety of MWMR control despite varying levels of fault occurrence, addressing both complete and partial actuator faults.
...
TL;DR: ECIM (Entropy-Controlled Intrinsic Motivation) improves quadrupedal robot locomotion by reducing premature convergence in reinforcement learning, achieving better stability and energy efficiency across diverse terrains.


<details>
  <summary>Details</summary>
Motivation: Existing PPO-based reinforcement learning algorithms for quadrupedal robots often converge prematurely, leading to suboptimal locomotion and reduced task performance. There's a need for methods that can maintain exploration and avoid early convergence to local optima.

Method: ECIM combines entropy control with intrinsic motivation to reduce premature convergence. It uses adaptive exploration mechanisms and was tested in Isaac Gym across six terrain categories: upward/downward slopes, uneven rough terrain, ascending/descending stairs, and flat ground.

Result: ECIM consistently outperforms baselines: task rewards increased by 4-12%, peak body pitch oscillation reduced by 23-29%, joint acceleration decreased by 20-32%, and joint torque consumption declined by 11-20%.

Conclusion: ECIM achieves better stability across different terrains for quadrupedal locomotion while reducing energetic costs, making it a practical choice for complex robotic control tasks by effectively combining entropy control and intrinsic motivation.
...
TL;DR: Vision-guided grasping algorithm for prosthetic hands using BVH-based object segmentation, RRT* trajectory planning, and DLS inverse kinematics for adaptive, per-finger grasp planning in real-time.


<details>
  <summary>Details</summary>
Motivation: Enhance prosthetic hand dexterity and autonomy through intelligent control systems that enable natural interaction with diverse objects in dynamic environments using vision-based approaches.

Method: Camera captures scene, BVH-based algorithm segments objects and defines bounding boxes. RRT* generates candidate trajectories, selects fingertip poses based on minimum Euclidean distance to object point cloud. DLS inverse kinematics computes joint angles for finger actuators. Modular pipeline enables per-finger grasp planning.

Result: Method validated in simulation and experimentally integrated on Linker Hand O7 platform, demonstrating real-time adaptability in unstructured environments.

Conclusion: Proposed vision-guided grasping algorithm successfully enables adaptive, object-specific prosthetic hand manipulation through integrated perception, planning, and control modules.
...
TL;DR: Tactile-sensorized Fin-Ray finger uses indirect sensing via hinge mechanism and marker-tipped pins with camera and CNN to detect contact location and depth without direct surface sensing.


<details>
  <summary>Details</summary>
Motivation: To create a lightweight, flexible, and scalable tactile sensing solution suitable for soft robotic structures where sensing needs to be situated away from the contact interface, enabling robust tactile perception in challenging environments.

Method: Integrated hinge mechanism between soft Fin-Ray structure and rigid sensing module transfers deformation/translation to bottom crossbeam with marker-tipped pins (biomimetic TacTip structure). Internal camera captures deformation patterns processed by CNN to infer contact conditions. Optimized design through varying pin configurations and hinge orientations.

Result: Achieved 0.1mm depth sensing accuracy and 2mm location sensing accuracy. Demonstrated robust generalization to various indenter shapes and sizes. Applied to pick-and-place task with uncertain picking positions where tactile feedback significantly improved placement accuracy.

Conclusion: The indirect sensing approach provides an effective tactile sensing solution for soft robotic structures, enabling accurate contact detection and depth measurement without direct surface sensing, with practical applications in robotic manipulation tasks.
...
TL;DR: The paper introduces Refer360, a large-scale dataset for embodied human-robot interaction, and MuRes, a multimodal guided residual module that improves embodied referring expression comprehension in robots.


<details>
  <summary>Details</summary>
Motivation: Robots need to understand embodied human instructions for intuitive human-robot interaction, but current datasets have limitations: perspective bias, single-view collection, inadequate gesture coverage, and focus on indoor environments only.

Method: Created Refer360 dataset with embodied verbal/nonverbal interactions across diverse viewpoints in indoor/outdoor settings. Developed MuRes module that extracts salient modality-specific signals and reinforces them into pre-trained representations as complementary features.

Result: Extensive experiments on four HRI datasets show current multimodal models fail to capture embodied interactions comprehensively, but augmenting them with MuRes consistently improves performance across all datasets.

Conclusion: Refer360 serves as a valuable benchmark, and guided residual learning (MuRes) shows potential to advance embodied referring expression comprehension for robots operating in human environments.
...
TL;DR: RL-based system enables humanoid robots to perform robust continual ball-kicking with adaptability to different ball-goal configurations through a four-stage teacher-student training framework.


<details>
  <summary>Details</summary>
Motivation: Ball-kicking for humanoid soccer robots is challenging due to rapid leg swings, single-foot stability requirements, and robustness needs under noisy sensing and external perturbations like opponents.

Method: Four-stage teacher-student RL framework: (1) long-distance ball chasing (teacher), (2) directional kicking (teacher), (3) teacher policy distillation (student), (4) student adaptation/refinement. Includes tailored rewards, realistic noise modeling, and online constrained RL for adaptation.

Result: Strong kicking accuracy and goal-scoring success across diverse ball-goal configurations in both simulation and real robot experiments. Ablation studies confirm necessity of constrained RL, noise modeling, and adaptation stage.

Conclusion: Presents a system for learning robust continual humanoid ball-kicking under imperfect perception, establishing a benchmark task for visuomotor skill learning in humanoid whole-body control.
...
TL;DR: EC-PIDUNN combines untrained neural networks with improved PID control using stabilizing factor τ, eliminating need for system dynamics knowledge and extensive training data.


<details>
  <summary>Details</summary>
Motivation: Classical PID struggles with modern nonlinear systems, while existing PIDNN models require extensive training data and high computational costs, making them impractical for real-world applications.

Method: Integrates untrained neural network with improved PID controller using stabilizing factor τ, uses steady-state error e_t as input, increases input dimensionality, introduces parameter vector ρ_t for trajectory shaping, and dynamic compute function for PID coefficient adjustment.

Result: Outperforms classical PID in convergence and stability on nonlinear robotics applications (Ackermann steering mechanism and Pan-Tilt movement system), achieving nearly critically damped response.

Conclusion: EC-PIDUNN provides effective solution for nonlinear control problems without requiring system dynamics knowledge or extensive training data, offering practical advantages over both classical PID and existing PIDNN approaches.
...
TL;DR: A unified framework for fair assessment of crowd navigation methods with novel trajectory-curvature optimization reward shaping, achieving superior performance in 2D/3D experiments.


<details>
  <summary>Details</summary>
Motivation: Current DRL approaches for crowd navigation lack fair assessment frameworks, neglect trajectory continuity metrics (C² smoothness), and use simplistic smoothness rewards, compromising trajectory optimization essential for naturalness, comfort, and energy efficiency.

Method: Proposes a unified framework for fair assessment of navigation methods with multiple optimization objectives, plus a novel reward-shaping strategy that explicitly emphasizes trajectory-curvature optimization.

Result: The method achieves superior performance compared to state-of-the-art approaches in extensive 2D and 3D experiments, with significantly enhanced trajectory quality and adaptability across multi-scale scenarios.

Conclusion: The proposed framework enables fair assessment of navigation methods and the novel curvature-optimization reward significantly improves trajectory quality, addressing critical gaps in current crowd navigation research.
...
TL;DR: Autonomous dynamic soaring framework for UAVs using explicit wind field representation, robust reference paths, and robust path following control, validated in simulation and real flight tests.


<details>
  <summary>Details</summary>
Motivation: Dynamic soaring enables potentially unlimited flight without internal energy by exploiting wind shear energy, but requires robust autonomous control for practical UAV applications.

Method: Framework uses explicit wind field representation with classical guidance/control, constructs point-wise robust reference paths for dynamic soaring, and develops robust path following controller for fixed-wing UAV.

Result: Demonstrated robust dynamic soaring in simulation under varied wind conditions, estimation errors, and disturbances, with critical components validated in real flights showing small sim-to-real gap.

Conclusion: Results strongly indicate the framework's ability to achieve autonomous dynamic soaring flight in wind shear, enabling energy-harvesting flight without internal power sources.
...
TL;DR: MIND-V is a hierarchical framework for generating physically plausible, long-horizon robotic manipulation videos using cognitive-inspired reasoning and reinforcement learning with physical foresight rewards.


<details>
  <summary>Details</summary>
Motivation: Current video generation models for robotic manipulation are limited to short clips of simple actions and rely on manually defined trajectories, creating a scarcity of diverse, long-horizon data for embodied imitation learning.

Method: Three-component hierarchical framework: 1) Semantic Reasoning Hub (SRH) for task planning using vision-language models, 2) Behavioral Semantic Bridge (BSB) translating instructions to domain-invariant representations, 3) Motor Video Generator (MVG) for conditional rendering. Uses Staged Visual Future Rollouts for test-time optimization and GRPO reinforcement learning with Physical Foresight Coherence (PFC) reward using V-JEPA world model.
...
Conclusion: The framework successfully bridges high-level reasoning with pixel-level synthesis to generate physically plausible and logically coherent long-horizon manipulation videos, addressing the data scarcity problem in embodied imitation learning.
...
TL;DR: MoE-RAM: A statistic-augmented, decoupled routing and aggregating mechanism for LM-driven MoE in autonomous driving, improving expert selection and fusion for better prediction performance.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving scenarios are complex and diverse, making it challenging for single deep learning models to handle all conditions (weather, traffic, road types). LM-driven MoE offers promise but faces intrinsic challenges in routing (imprecise expert selection) and aggregation (inefficient expert fusion).

Method: Proposes MoE-RAM: 1) Enhances expert routing via statistical retrieval mechanism matching LM-extracted latent features with cached prototypical features of relevant experts; 2) Adaptively reweights experts' outputs by measuring statistical distances between experts' instant features and LM-extracted latent features.

Result: Extensive experiments on AD datasets demonstrate superiority of MoE-RAM compared to other MoE baselines and conventional single-model approaches, particularly evaluated on AD semantic segmentation task.

Conclusion: MoE-RAM effectively addresses routing and aggregation challenges in LM-driven MoE through statistic-augmented mechanisms, ultimately improving prediction performance for complex autonomous driving scenarios.
...
TL;DR: FedDSR improves federated learning for autonomous driving by adding multi-layer supervision and regularization to handle non-IID data, boosting performance and convergence.


<details>
  <summary>Details</summary>
Motivation: Federated Learning for autonomous driving faces poor generalization and slow convergence due to non-IID data from diverse driving environments across distributed vehicles.

Method: FedDSR introduces multi-access intermediate layer supervision and regularization: (I) selects multiple intermediate layers using architecture-agnostic standards, (II) computes mutual information and negative entropy on selected layers as intermediate loss and regularizer, integrated with output-layer loss, (III) aggregates models from vehicles trained with these rules to generate global model.

Result: FedDSR achieves up to 8.93% improvement in mIoU and 28.57% reduction in training rounds compared to other FL baselines, demonstrating enhanced generalization and faster convergence.

Conclusion: FedDSR effectively addresses non-IID challenges in federated autonomous driving systems, making it highly suitable for practical deployment in federated AD ecosystems.
...
TL;DR: Model-less control framework for continuum manipulators using online Jacobian refinement and quadratic programming for stable, accurate motion without kinematic modeling.


<details>
  <summary>Details</summary>
Motivation: Continuum manipulators have infinite-dimensional backbone deformation, unmodeled friction, and configuration-dependent stiffness that limit reliability of model-based approaches, causing inaccurate Jacobian predictions, artificial singularities, and unstable actuation.

Method: Complete model-less control framework using empirically initialized Jacobian refined online through differential convex updates. Tip motion generated via real-time quadratic program computing actuator increments while enforcing tendon slack avoidance and geometric limits. Introduces backbone tension optimization term to regulate axial loading and suppress co-activation compression.

Result: Validated across circular, pentagonal, and square trajectories, demonstrating smooth convergence, stable tension evolution, and sub-millimeter steady-state accuracy without any model calibration or parameter identification.

Conclusion: Establishes the proposed controller as a scalable alternative to model-dependent continuum manipulation in constrained environments.
...
TL;DR: db-LaCAM combines MAPF scalability with kinodynamic awareness using motion primitives and bounded discontinuities to efficiently plan for up to 50 robots with 10x speedup.


<details>
  <summary>Details</summary>
Motivation: Current multi-robot kinodynamic motion planners have high computational burden, limiting scalability to only a few robots and resulting in slow planning times.

Method: Proposes discontinuity-Bounded LaCAM (db-LaCAM) that uses precomputed motion primitives respecting robot dynamics to generate horizon-length motion sequences, allowing user-defined discontinuities between motions.

Result: Scales efficiently to 50 robots with up to 10x faster runtime than state-of-the-art planners while maintaining comparable solution quality. Validated in 2D/3D environments with various dynamics and demonstrated in physical experiments.

Conclusion: Successfully combines MAPF scalability with kinodynamic awareness, enabling efficient motion planning for large multi-robot teams with complex dynamics while ensuring safe execution.
...
TL;DR: MagicSkin introduces translucent tinted markers for vision-based tactile sensors that simultaneously enable tangential displacement tracking, force prediction, and surface detail preservation without additional hardware or software.


<details>
  <summary>Details</summary>
Motivation: Vision-based tactile sensors face a fundamental trade-off: opaque markers enable force/tangential displacement measurement but occlude geometric features, while markerless skin preserves surface details but struggles with tangential displacement measurement. Current solutions (UV lighting or learning-based models) add hardware complexity or computational burden.

Method: Develop MagicSkin - a novel tactile skin with translucent, tinted markers that balances marker and markerless modes. The skin can be easily integrated into GelSight-family sensors without requiring additional hardware or software tools.

Result: Translucent markers enhance rather than degrade sensing performance: achieves 99.17% object classification, 93.51% texture classification, 97% point retention in tangential displacement tracking, and 66% improvement in total force error compared to traditional designs.

Conclusion: MagicSkin eliminates the traditional performance trade-off between marker and markerless modes, enabling multimodal tactile sensing essential for tactile robotics by simultaneously supporting displacement tracking, force prediction, and surface detail preservation.
...
TL;DR: A monocular visual SLAM system that robustly handles dynamic scenes by combining geometric patch-based bundle adjustment with feed-forward reconstruction models to filter dynamic regions and resolve scale ambiguities.


<details>
  <summary>Details</summary>
Motivation: Camera pose estimation and 3D reconstruction in dynamic natural environments is challenging because scene dynamics severely deteriorate pose estimation accuracy, limiting applications in robotics, visualization, and AR.

Method: Proposes a novel monocular visual SLAM system that leverages complementary strengths: geometric patch-based online bundle adjustment combined with feed-forward reconstruction models. Uses feed-forward model to filter dynamic regions and its depth prediction to enhance SLAM robustness. Aligns depth prediction with estimated patches from bundle adjustment to handle scale ambiguities.

Result: The system can robustly estimate camera poses in dynamic scenes by filtering out dynamic regions while maintaining accurate reconstruction through depth alignment and scale resolution.

Conclusion: The proposed approach successfully addresses the challenge of visual SLAM in dynamic environments by integrating geometric optimization with learning-based reconstruction, enabling reliable camera pose estimation for various applications.
...
TL;DR: The paper introduces the Autonomous Race Stack (ARS) for high-speed autonomous racing, presenting three iterations tested at speeds up to 260 km/h, with performance evaluation across different tracks and release of a high-speed multi-sensor dataset.


<details>
  <summary>Details</summary>
Motivation: High-speed autonomous racing faces significant technical challenges including precise localization, rapid perception, dynamic planning, and real-time control, compounded by limited track access and expensive hardware.

Method: Developed three iterations of the Autonomous Race Stack (ARS1, ARS2, ARS3) with modular architecture, validated on different tracks, and conducted detailed performance evaluation contrasting control, perception, and estimation across oval and road-course environments.

Result: Achieved speeds up to 260 km/h across different track types, developed a comprehensive autonomous racing system, and collected a high-speed multi-sensor dataset from both oval and road-course tracks.

Conclusion: The work provides unique insights into real-world high-speed full-scale autonomous racing challenges and makes contributions through the ARS architecture, performance evaluation, and release of valuable high-speed sensor data.
...
TL;DR: Admittance-based control for powered ankle prostheses improves gait stability on compliant terrain, reducing fall risk for amputees.


<details>
  <summary>Details</summary>
Motivation: Lower-limb amputees face high fall risk on compliant terrain, and existing powered prosthesis controls are optimized for rigid surfaces but not soft/compliant ones.

Method: Experimental validation of admittance-based control that dynamically adjusts prosthesis quasi-stiffness. Human subject experiments with 3 healthy individuals walking on two compliant surfaces (63 and 25 kN/m stiffness). Compared against standard phase-variable controller.

Result: Admittance controller consistently improved gait stability across all compliant conditions compared to standard controller, as measured by phase portraits and walking stability metrics.

Conclusion: Adaptive, stability-aware prosthesis control can reduce fall risk in real-world environments and advance human-prosthesis interaction robustness in rehabilitation robotics.
...
TL;DR: Adding ground compliance to visual feedback gait training enhances push-off force learning more effectively than visual feedback alone, with lasting neuromuscular after-effects.


<details>
  <summary>Details</summary>
Motivation: To investigate whether combining ground compliance with visual feedback in gait training is more effective at increasing push-off force than visual feedback alone, with applications for gait rehabilitation, particularly for propulsion deficits like those following stroke.

Method: Ten healthy participants walked on a custom split-belt treadmill with real-time visual feedback of ground reaction forces. One group received both visual feedback and ground compliance changes, while a control group received only visual feedback.

Result: Intentional increases in propulsive ground reaction forces were successfully achieved and sustained post-intervention, especially in the ground compliance group. This group also showed lasting after-effects in muscle activity and joint kinematics, indicating more robust learning of natural propulsion strategies.

Conclusion: Combining ground compliance with visual feedback enhances learning of propulsive forces more effectively than visual feedback alone, demonstrating coordination between visual and proprioceptive systems during gait adaptation. This supports using compliant terrain in long-term rehabilitation for propulsion deficits.
...
TL;DR: Learning-based approach for energy-efficient ASV navigation in ocean currents using RL with local velocity measurements, achieving 30-50% energy savings over state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Autonomous Surface Vehicles need energy-efficient navigation for long missions, but traditional methods fail in partially observable vortical flow fields. Inspired by khalasi sailors who skillfully use ocean currents.

Method: End-to-end reinforcement learning framework using Soft Actor Critic (SAC) that learns flow-aware navigation policies from only local velocity measurements (partial observability).

Result: Method demonstrates substantial energy savings (30-50% improvement) and robust generalization to unseen flow conditions across diverse, dynamically rich scenarios.

Conclusion: Learning-based approach offers promising path toward long-term autonomy in ocean environments by enabling energy-efficient navigation in complex flow fields.
...
TL;DR: Neural ODE approach for IDA-PBC using sparse dictionary learning to avoid solving matching PDEs, enabling complex tasks beyond stabilization.


<details>
  <summary>Details</summary>
Motivation: IDA-PBC is limited by the difficulty of analytically solving matching PDEs, restricting it mainly to academic examples and stabilization tasks.

Method: Cast IDA-PBC as learning a neural ODE, use sparse dictionary learning to parametrize closed-loop system, solve multi-objective optimization with task and matching costs.

Result: Method enables IDA-PBC for complex tasks like periodic oscillatory behaviors and provides closed-form expressions with residual terms.

Conclusion: Numerical approach overcomes analytical limitations of IDA-PBC, making it applicable to complex systems and tasks beyond stabilization.
...
TL;DR: Vision-action policy wins 2025 BEHAVIOR Challenge with 26% q-score across 50 diverse household tasks using correlated noise for flow matching and other innovations.


<details>
  <summary>Details</summary>
Motivation: To develop a robust vision-action policy capable of handling complex, long-horizon household tasks in photo-realistic simulation, addressing challenges like bimanual manipulation, navigation, and context-aware decision making.

Method: Builds on Pi0.5 architecture with correlated noise for flow matching (enables correlation-aware inpainting), learnable mixed-layer attention, System 2 stage tracking for ambiguity resolution, multi-sample flow matching for training, and action compression with challenge-specific correction rules for inference.

Result: Achieved 1st place in 2025 BEHAVIOR Challenge with 26% q-score across all 50 tasks on both public and private leaderboards.

Conclusion: The proposed innovations, particularly correlated noise for flow matching, significantly improve training efficiency and enable smooth action sequences, resulting in state-of-the-art performance on complex household robotics tasks.
...
TL;DR: VideoVLA transforms video generation models into robotic manipulators that jointly predict action sequences and future visual outcomes for improved generalization.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action models have limited generalization to novel tasks, objects, and settings. There's a need for better generalization in robot manipulation for open-world deployment and progress toward artificial general intelligence.

Method: VideoVLA uses a multi-modal Diffusion Transformer built on pre-trained video generation models to jointly model video, language, and action modalities. Given language instruction and an image, it predicts both action sequences and future visual outcomes.

Result: High-quality imagined futures correlate with reliable action predictions and task success. VideoVLA demonstrates strong generalization capabilities, including imitating other embodiments' skills and handling novel objects.

Conclusion: The dual-prediction strategy of forecasting both actions and visual consequences represents a paradigm shift in robot learning and unlocks generalization capabilities in manipulation systems.
...
TL;DR: A cable-driven coaxial spherical parallel mechanism (CDC-SPM) is designed for medical haptic interfaces, reducing end-effector mass to improve stiffness, inertia, and bandwidth for precise rotational motion in teleoperation tasks like ultrasound imaging.


<details>
  <summary>Details</summary>
Motivation: Haptic interfaces for medical teleoperation require high fidelity but face trade-offs between workspace, dexterity, stiffness, inertia, and bandwidth, especially for pure rotational motion applications.

Method: Design methodology and kinematic analysis of a Cable-Driven Coaxial Spherical Parallel Mechanism (CDC-SPM) that uses parallel and coaxial actuation to achieve decoupled rotational DOFs with isotropic force/torque transmission.

Result: Simulation and analysis show the CDC-SPM provides accurate, responsive, and safe motion characteristics suitable for high-precision haptic applications, with reduced end-effector mass minimizing inertial loads and enhancing stiffness.

Conclusion: The CDC-SPM demonstrates potential for medical teleoperation tasks like ultrasound imaging, offering precise and intuitive manipulation through improved dynamic responsiveness and decoupled rotational motion.
...
TL;DR: Neuromorphic hetero-associative memory system for mobile manipulators learns compact bindings between joint states and tactile observations to enable low-cost, step-wise action decisions using spike-based encoding and 3D rotary embeddings.


<details>
  <summary>Details</summary>
Motivation: Need for efficient, low-compute memory systems for mobile manipulators that can learn associations between robot states and tactile observations to enable reactive behaviors without heavy computational overhead.

Method: Encodes joint angles via population place coding, converts skin-measured forces into spike-rate features using Izhikevich neuron model, transforms both into bipolar binary vectors, binds them element-wise, and stores in sequential memory with 3D rotary positional embeddings for improved separability and geometry injection.

Result: System implements pseudocompliance controller on Toyota Human Support Robot that moves links under touch correlating to force direction/amplitude, retrieves multi-joint grasp sequences from tactile input, and demonstrates single-joint/full-arm behaviors via associative recall with quick setup and economical operation.

Conclusion: Hetero-associative sequential memory system provides efficient, low-cost solution for robot control that learns from synchronized state-observation streams, exhibits generalization, and suggests extensions to imitation learning, motion planning, and multi-modal integration.
...
TL;DR: CERNet: A hierarchical predictive-coding RNN with class embedding that unifies motor generation, real-time recognition, and confidence estimation for human-robot interaction.


<details>
  <summary>Details</summary>
Motivation: Robots need to generate movements in real-time while also inferring human intent and estimating their own inference confidence, requiring a unified approach rather than separate systems.

Method: Hierarchical predictive-coding RNN with dynamically updated class embedding vector that operates in two modes: generation (embedding constrains hidden states) and inference (embedding optimized online to minimize prediction error).

Result: 76% lower trajectory error than baseline, 68% Top-1/81% Top-2 recognition accuracy on 26 alphabets, maintains motion fidelity under perturbations, and internal errors reflect confidence.

Conclusion: CERNet provides a compact, extensible framework integrating robust generation, real-time recognition, and uncertainty estimation for motor memory in physical robots, enabling intent-sensitive human-robot collaboration.
...
TL;DR: A robotic hand with controllable valve enables precise milligram-scale powder dispensing using model-based feedback control, achieving 80% accuracy within 2mg error.


<details>
  <summary>Details</summary>
Motivation: Fully automating powder handling at milligram scale remains challenging for laboratory automation due to complex powder flow dynamics and diverse lab tasks, limiting robotic solid-state materials discovery.

Method: Developed a funnel-shaped flexible robotic hand with controllable valve at apex, integrated with external balance feedback control using powder flow models and online parameter identification.

Result: 80% of trials achieved error within 2mg, maximum error ~20mg across 20mg-3g range; model-based control outperformed direct PID in accuracy and convergence speed.

Conclusion: The system enables efficient, flexible powder weighing with scalability to larger quantities and broad applicability to laboratory automation tasks.
...
TL;DR: Surrogate compliance modeling enables soft-body robot simulation in rigid-body simulators by representing deformation as indirect variables, achieving successful sim-to-real transfer for amphibious robotic turtle.


<details>
  <summary>Details</summary>
Motivation: Soft-body robots face simulation challenges: soft-body simulators lack accuracy/computational tractability, while rigid-body simulators can't capture soft-material dynamics needed for adaptive morphogenetic robots.

Method: Surrogate compliance modeling introduces indirect variables representing soft-material deformation within rigid-body simulator (effective limb length and limb center of mass changes), combined with reinforcement learning and extensive randomization of these variables.

Result: Achieved reliable policy learning entirely in rigid-body simulation with direct hardware transfer; high-fidelity sim-to-real on hard substrates, robust transfer on complex terrains; learned gaits show unprecedented terrestrial maneuverability and 10x reduction in cost of transport vs open-loop baselines.

Conclusion: Surrogate compliance modeling successfully bridges soft-body simulation gap, enabling effective sim-to-real transfer for adaptive morphogenetic robots across diverse natural terrains including gravel, grass, and mud.
...
TL;DR: Mimir is a hierarchical dual-system framework for end-to-end autonomous driving that improves robustness through goal point uncertainty estimation and boosts inference speed via multi-rate guidance.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving systems are limited by inaccurate high-level guidance and computational overhead from complex guidance modules, constraining their performance and efficiency.

Method: Proposes Mimir with two key innovations: (1) estimates goal point uncertainty using Laplace distribution instead of deterministic modeling, and (2) introduces multi-rate guidance mechanism that predicts extended goal points in advance to overcome slow inference speed.
...
Conclusion: Mimir demonstrates superior performance and efficiency for end-to-end autonomous driving by addressing both robustness and speed limitations through uncertainty estimation and multi-rate guidance.
...
TL;DR: Time-varying formation tracking control for wheeled mobile robots with region constraints using Udwadia-Kalaba framework


<details>
  <summary>Details</summary>
Motivation: Existing time-varying formation tracking control methods don't account for region constraints, which are important for robot safety in practical applications

Method: Reformulate formation tracking as constrained equation, transform region constraint via diffeomorphism, design controller using generalized Udwadia-Kalaba framework under directed weighted communication topology with spanning tree

Result: Proposed controller successfully achieves time-varying formation tracking while respecting region constraints, ensuring robot safety

Conclusion: The Udwadia-Kalaba framework effectively handles time-varying formation tracking with region constraints, providing safety guarantees for wheeled mobile robots
...
TL;DR: A two-stage pipeline uses lightweight detectors (gaze/proxemics) to trigger VLM queries at key moments, enabling socially responsive robot engagement decisions based on natural human cues.


<details>
  <summary>Details</summary>
Motivation: Robots need to decide when to engage with people in everyday settings, but this depends on subtle nonverbal cues that are hard to model explicitly. Real-world observations from a Wizard-of-Oz study revealed how people signal interaction readiness through behaviors like gaze and proximity.

Method: Two-stage pipeline: 1) Lightweight perceptual detectors (gaze shifts and proxemics) identify socially meaningful moments, 2) These trigger heavier video-based vision-language model (VLM) queries. Evaluated on replayed field interactions with comparison of two prompting strategies.

Result: Selectively using VLMs as proxies for social reasoning enables socially responsive robot behavior. The pipeline allows robots to act appropriately by attending to natural human cues observed in real-world interactions.

Conclusion: A hybrid approach combining lightweight detectors with selective VLM queries can effectively enable robots to make socially appropriate engagement decisions based on the subtle nonverbal cues people naturally provide in everyday interactions.
...
TL;DR: Novel continuous-time maximum likelihood estimator improves SLAM benchmarking precision by compensating for motion capture jitter and enabling precise multi-sensor spatiotemporal calibration.


<details>
  <summary>Details</summary>
Motivation: Current SLAM benchmarking using marker-based optical motion capture systems has limitations in precision due to spatiotemporal calibration issues and inherent measurement jitter, which hinders accurate evaluation of key metrics like rotation error and inter-frame jitter critical for immersive XR experiences.

Method: Proposes a continuous-time maximum likelihood estimator that integrates auxiliary IMU data to compensate for MoCap jitter, along with a variable time synchronization method and pose residual based on screw congruence constraints for precise spatiotemporal calibration across multiple sensors and the device under test.

Result: The approach outperforms existing methods, achieving the precision necessary for comprehensive benchmarking of state-of-the-art SLAM algorithms in XR applications, validated by benchmarking several leading XR devices and open-source SLAM algorithms.

Conclusion: The proposed method successfully addresses the precision limitations of MoCap-based ground truth generation for SLAM benchmarking, enabling more accurate evaluation of critical XR metrics, with publicly available code for community use.
...
TL;DR: Hybrid SNN-ANN actor-critic DRL approach for socially-aware robot navigation with 1.69 orders of magnitude energy reduction


<details>
  <summary>Details</summary>
Motivation: Need for human-like decision-making and energy-efficient event-based computation in autonomous mobile robots operating in human environments, addressing the gap where neuromorphic methods are rarely applied to DRL navigation due to training instability

Method: Hybrid socially integrated DRL actor-critic approach combining Spiking Neural Networks (SNNs) in the actor with Artificial Neural Networks (ANNs) in the critic, plus a neuromorphic feature extractor to capture temporal crowd dynamics and human-robot interactions

Result: Enhanced social navigation performance and reduced estimated energy consumption by approximately 1.69 orders of magnitude compared to conventional approaches

Conclusion: The hybrid SNN-ANN approach successfully integrates neuromorphic computing into DRL navigation, achieving both improved social navigation capabilities and significant energy efficiency gains, bridging the gap between neuromorphic methods and practical DRL applications
...
TL;DR: The paper presents a topological model for tethered robot path planning using simplicial complexes derived from the universal covering space, enabling continuous configuration space representation and faster computation than traditional homotopy-augmented graphs.


<details>
  <summary>Details</summary>
Motivation: Existing tethered robot path planning approaches use discrete configuration space representations and lack models that simultaneously capture topological tether information and continuous robot location. There's a need for a more efficient continuous representation.

Method: The authors build a topological model by linking the tethered robot's configuration space to the universal covering space of the workspace. They develop an algorithm to compute a simplicial complex model of the configuration space from a polygonal workspace representation.

Result: The proposed model computes much faster than traditional homotopy-augmented graphs (in a fraction of the time) and provides a continuous representation that enables use of a broad set of path planning algorithms for tethered robots.

Conclusion: The topological simplicial complex model improves performance over existing configuration space representations for tethered robots, offering both computational efficiency and continuous representation for flexible path planning.
...
TL;DR: Cooperative MPC approach for two USVs docking together at agreed location, outperforming existing one-sided docking methods.


<details>
  <summary>Details</summary>
Motivation: Existing USV docking approaches treat one vehicle as stationary target while the other does all the maneuvering, which is inefficient. Need cooperative approach where both USVs work together for faster, more efficient docking.

Method: Centralized Model Predictive Control (MPC) approach that solves control problem for both USVs simultaneously, generating feasible trajectories while satisfying constraints. Uses model-based prediction to anticipate and reject disturbances like water currents.

Result: Simulations demonstrate the proposed cooperative approach enables faster and more efficient docking compared to existing one-sided docking methods.

Conclusion: Cooperative MPC docking for USVs is effective, allowing both vehicles to work together for improved docking performance with disturbance rejection capabilities, particularly beneficial for stationary disturbances like water currents.
...
TL;DR: A pipeline for converting personalized MANO hand models to URDF for real-time physics simulation with accurate joint constraint projection.


<details>
  <summary>Details</summary>
Motivation: Human hand simulation needs anatomical fidelity and computational efficiency for digital twin applications, requiring conversion of flexible MANO models to constrained rigid-body representations.

Method: Convert optical motion capture to personalized MANO model, then to URDF with anatomical joint axes. Use closed-form solutions for 1-DOF joints and BCH-corrected iterative method for 2-DOF joints to project unconstrained rotations onto kinematic constraints.

Result: Sub-centimeter reconstruction error and successful grasp execution across diverse manipulation tasks in digital twin experiments with reinforcement learning control.

Conclusion: The pipeline enables realistic real-time physics simulation of human hands for digital twin applications by solving the joint constraint projection problem with mathematical rigor.
...
TL;DR: ESPADA is a framework that accelerates behavior-cloned visuomotor policies by semantically segmenting demonstrations to identify non-critical phases for aggressive downsampling, achieving 2x speed-up while maintaining success rates.


<details>
  <summary>Details</summary>
Motivation: Behavior-cloning policies inherit the slow tempo of human demonstrations, limiting practical deployment. Existing acceleration methods rely on statistical/heuristic cues that ignore task semantics and fail across diverse manipulation settings.

Method: Uses VLM-LLM pipeline with 3D gripper-object relations to semantically segment demonstrations, enabling aggressive downsampling only in non-critical segments. Propagates segment labels via Dynamic Time Warping on dynamics-only features from single annotated episode to full dataset.

Result: Achieves approximately 2x speed-up while maintaining success rates across simulation and real-world experiments with ACT and DP baselines, narrowing gap between human demonstrations and efficient robot control.

Conclusion: ESPADA provides a semantic and spatially aware framework for accelerating visuomotor policies without requiring extra data, architectural modifications, or retraining, enabling more practical deployment of behavior-cloned robot policies.
...
TL;DR: A perceptive locomotion framework combining terrain sensing, gait regulation, and whole-body control in a single RL policy enables humanoid robots to navigate complex terrains like stairs and gaps.


<details>
  <summary>Details</summary>
Motivation: Full-size humanoid robots struggle with reliable locomotion on complex terrains like long staircases due to limited perception, ambiguous terrain cues, and insufficient gait timing adaptation, where even a single mistimed step can cause balance loss.

Method: Uses downward-facing depth camera and compact U-Net to reconstruct dense egocentric height maps in real-time. A unified RL policy processes perceptual height maps with proprioceptive data to produce joint commands and stepping-phase signals, adapting gait timing and whole-body posture to terrain geometry. Employs single-stage successive teacher-student training for efficient policy learning.

Result: Demonstrated robust locomotion on a 31-DoF, 1.65m humanoid robot in both simulation and real-world settings, including forward/backward stair ascent/descent and crossing a 46cm gap.

Conclusion: The integrated perceptive locomotion framework successfully enables humanoid robots to navigate challenging terrains by combining terrain sensing, gait regulation, and whole-body control into a single adaptive policy.
...
TL;DR: AFI uses 3D Spatial Affordance Fields to detect and correct VLA memory traps, improving robustness to distribution shifts by 23.5% on real robots.


<details>
  <summary>Details</summary>
Motivation: VLA models fail under distribution shifts, falling into "Memory Traps" where they reproduce memorized trajectories instead of adapting to new scenes due to lack of explicit 3D spatial reasoning.

Method: Affordance Field Intervention (AFI) - a hybrid framework that uses 3D Spatial Affordance Fields as a plug-in to detect memory traps via proprioception, reposition robots to high-affordance regions, propose affordance-driven waypoints, and select trajectories with highest cumulative affordance.

Result: Achieves average improvement of 23.5% across VLA backbones (π₀ and π₀.₅) under OOD scenarios on real robots, and 20.2% on LIBERO-Pro benchmark, validating enhanced robustness to distribution shifts.

Conclusion: AFI effectively compensates for VLA's missing spatial reasoning by integrating explicit 3D affordance representations, significantly improving adaptation to unfamiliar environments and overcoming memory trap limitations.
...
TL;DR: The paper presents a framework for identifying safety-relevant lane change scenarios in highway traffic using real-world data analysis and synthetic scenario generation to address unknown unsafe scenarios in autonomous vehicle validation.


<details>
  <summary>Details</summary>
Motivation: Validating autonomous vehicles requires identifying near-complete sets of relevant driving scenarios, which is challenging due to numerous degrees of freedom and increasing technical complexity that expands unknown unsafe scenarios. Starting with simpler highway domains before complex urban environments enhances validation efficiency.

Method: The approach involves: 1) Data acquisition and processing of real-world highway traffic trajectory data from the AVEAS project, 2) Applying criticality measures to evaluate scenarios, 3) Linking calculated measures to specific lane change scenarios and collection conditions, and 4) Generating synthetic scenarios based on recorded ones to address unknown unsafe scenarios.

Result: The paper demonstrates a processing chain that enables identification of safety-relevant scenarios, development of data-driven methods for extracting these scenarios, and generation of synthetic critical scenarios via sampling on highways.

Conclusion: The proposed framework facilitates systematic identification and generation of safety-relevant driving scenarios for autonomous vehicle validation, with a focus on highway lane change scenarios as a foundational step before addressing more complex urban environments.
...
TL;DR: VP-AutoTest is a virtual-physical fusion testing platform for autonomous vehicles that integrates diverse traffic elements, supports multi-vehicle cooperation testing, and uses AI-driven evaluation with credibility self-assessment.


<details>
  <summary>Details</summary>
Motivation: Traditional autonomous vehicle testing methods (virtual simulation, closed-course, public road) face challenges including unrealistic vehicle states, limited testing capabilities, and high costs. Virtual-physical fusion testing shows promise but still has limitations in element types, testing scope, and fixed evaluation metrics.

Method: Proposes VP-AutoTest platform that integrates over ten types of virtual and physical elements (vehicles, pedestrians, infrastructure). Supports single-vehicle interaction and multi-vehicle cooperation testing using adversarial testing and parallel deduction. Uses OBU and Redis communication for V2V and V2I cooperation across all automation levels. Incorporates multidimensional evaluation framework and AI-driven expert systems for performance assessment and defect diagnosis.

Result: The platform enables comprehensive testing with diverse traffic participants, accelerates fault detection through adversarial testing, supports seamless V2V/V2I cooperation, and provides AI-driven performance assessment. It also performs credibility self-evaluation by comparing virtual-physical fusion results with real-world experiments.

Conclusion: VP-AutoTest addresses limitations of existing testing methods by creating a comprehensive virtual-physical fusion platform that ensures both fidelity and efficiency in autonomous driving testing through diverse element integration, multi-vehicle cooperation support, and AI-driven evaluation with credibility verification.
...
TL;DR: ViVLA is a generalist robotic manipulation policy that learns novel skills from just one expert demonstration video at test time, achieving significant performance improvements on unseen tasks.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language-Action models have limited generalization beyond training distributions, while humans can learn new skills by simply observing others once. The goal is to develop robotic policies that can similarly acquire novel manipulation skills from single demonstrations.

Method: ViVLA jointly processes expert demonstration videos alongside robot visual observations to predict both demonstrated actions and subsequent robot actions, effectively distilling manipulation knowledge. A scalable data generation pipeline synthesizes 892,911 expert-agent pairs from human videos and public datasets for training.

Result: ViVLA achieves over 30% improvement on unseen LIBERO tasks, maintains above 35% gains with cross-embodiment videos, and shows more than 38% improvement on unseen tasks in real-world experiments with human videos.

Conclusion: The proposed ViVLA approach enables efficient task learning from single expert demonstrations, demonstrating strong generalization capabilities and bridging the gap between human-like skill acquisition and robotic manipulation policies.
...
TL;DR: MDME is a motion representation method that captures both structured periodic patterns and irregular variations in movement using wavelet-based encoding and probabilistic embedding, enabling real-time robot imitation without retargeting.


<details>
  <summary>Details</summary>
Motivation: Existing motion controllers ignore inherent patterns in motion, and previous representation learning methods don't jointly capture structured periodic patterns and irregular variations in human/animal movement.

Method: Multi-Domain Motion Embedding (MDME) uses a wavelet-based encoder and probabilistic embedding in parallel to unify structured and unstructured motion features from minimal input.

Result: MDME outperforms prior approaches in reconstruction fidelity and generalizability, enables accurate reproduction of complex trajectories on humanoid and quadruped robots, and supports zero-shot deployment for novel motion styles without task-specific tuning.

Conclusion: MDME serves as a generalizable, structure-aware foundation for scalable real-time robot imitation that eliminates the need for online retargeting or task-specific tuning.
...
TL;DR: An aerially deployable crawler robot for tree canopy exploration that combines microspine tracks, rotary grippers, and an elastic tail to grip and climb branches with high efficiency and low power consumption.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between aerial and surface-based ecological robotics by creating a platform for environmental sampling and in-canopy sensing that can be deployed from drones and operate efficiently in complex tree canopy environments.

Method: The system uses compliant microspine-based tracks for secure attachment, a dual-track rotary gripper for gripping branches, and an elastic tail for stability. It's designed for aerial deployment via drone-tether system and features yaw steering capability for maneuverability on irregular surfaces.

Result: The crawler achieves reliable gripping up to 90° body roll/inclination, climbs branches inclined up to 67.5°, reaches 0.55 body lengths/second on horizontal branches, and has yaw steering up to 10°. It operates with a dimensionless cost of transport over an order of magnitude lower than typical hovering aerial robots.

Conclusion: The aerially deployable crawler provides a robust, low-power platform for tree canopy exploration, successfully combining aerial deployment with surface-based locomotion to enable new ecological monitoring capabilities in challenging canopy environments.
...
TL;DR: DA-DP is a delay-aware diffusion policy framework that explicitly incorporates inference delays into policy learning to handle the gap between observation and execution in robotics.


<details>
  <summary>Details</summary>
Motivation: Robots experience inference delays (tens to hundreds of milliseconds) between sensing and action execution, creating a mismatch between observed and actual states that current methods don't address.

Method: DA-DP corrects zero-delay trajectories to delay-compensated versions and augments the policy with delay conditioning, making it architecture-agnostic and transferable beyond diffusion policies.

Result: DA-DP shows more robust success rates across various tasks, robots, and delays compared to delay-unaware methods, and enables evaluation of performance as a function of measured latency.

Conclusion: The framework provides a general pattern for delay-aware imitation learning and encourages reporting performance relative to measured latency, not just task difficulty.
...
TL;DR: Review paper on Physical Human-Humanoid Interaction (pHHI) examining three core pillars: humanoid modeling/control, human intent estimation, and computational human models, with taxonomy of interaction types and pathways for cross-pillar integration.


<details>
  <summary>Details</summary>
Motivation: To advance deployment of humanoid robots in unstructured, human-centric environments by addressing current limitations in physical human-robot interaction through systematic review and integration of key research areas.

Method: Structured review approach examining three core pillars: (1) humanoid modeling and control, (2) human intent estimation, (3) computational human models. Introduces unified taxonomy of interaction types based on modality (direct/indirect) and engagement level (assistance/cooperation/collaboration).

Result: Identifies open challenges including need for whole-body control strategies handling uncertain human dynamics, real-time intent inference under limited sensing, and modeling techniques accounting for human variability. Finds significant progress within individual domains but limited integration across pillars.

Conclusion: Proposes pathways for unifying methods across the three pillars to enable cohesive interaction frameworks, providing concrete research directions and roadmap for advancing robust, safe, and intuitive physical human-humanoid interaction in real-world settings.
...
TL;DR: OptMap: Real-time geometric map distillation algorithm for LiDAR data that generates application-specific maps using submodular optimization with provable near-optimal solutions.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots need different geometric maps for various perception and decision-making algorithms, but selecting informative, size-constrained maps from abundant LiDAR data is computationally challenging (NP-hard combinatorial optimization).

Method: Uses submodular optimization with polynomial-time algorithms for near-optimal solutions. Features: 1) Novel submodular reward function quantifying informativeness, reducing input set sizes, and minimizing bias in sequential datasets; 2) Dynamically reordered streaming submodular algorithm improving empirical solution quality and addressing input order bias via online approximation of scan values.

Result: Achieves real-time, application-specific map generation with minimal computation requirements. Tested on open-source and custom datasets, especially long-duration mapping sessions. Provides open-source ROS1/ROS2 packages compatible with any LiDAR SLAM algorithm.

Conclusion: OptMap enables efficient geometric map distillation for autonomous robots by leveraging submodular optimization theory to solve the NP-hard map selection problem with practical, real-time performance and theoretical guarantees.
...
TL;DR: Soft robot uses single actuator and patterned substrate grooves to control locomotion direction passively, eliminating need for multiple actuators.


<details>
  <summary>Details</summary>
Motivation: Current soft robots require multiple actuators for directional control, which increases mechanical complexity, complicates control systems, and raises energy consumption.

Method: Inchworm-inspired soft robot with single rolled dielectric elastomer actuator; groove patterns on 3D-printed substrate guide alignment and trajectory passively.

Result: Varying groove angles enables precise control of locomotion direction without complex actuation strategies, reducing energy consumption and simplifying design.

Conclusion: Groove-guided approach simplifies robot design, reduces energy consumption, and expands applicability in search/rescue, pipe inspection, and planetary exploration.
...
TL;DR: A control framework for humanoid robots to perform collaborative transportation tasks with humans, featuring planning, control, and stiffness modulation components validated on the Digit platform.


<details>
  <summary>Details</summary>
Motivation: Enable humanoid robots to effectively collaborate with human partners in co-transportation tasks requiring both translational and rotational motions, which are fundamental to real-world collaborative scenarios.

Method: Three-component framework: 1) High-level planner using Interaction Linear Inverted Pendulum (I-LIP) with admittance model and MPC for footstep planning, 2) QP-based whole-body controller for execution, and 3) Stiffness modulation for robot-object interaction regulation.

Result: Successfully validated through real-world experiments on Digit humanoid platform, demonstrating collaborative behaviors including translation, turning, and semi-circular trajectories. Proposed efficiency metric shows compliance's role in collaboration quality.

Conclusion: The framework enables effective human-robot collaborative transportation, with the proposed efficiency metric providing valuable insights into coordination quality and desirable trajectory characteristics across control layers.
