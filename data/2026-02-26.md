...
TL;DR: StoryTailor is a zero-shot pipeline that generates multi-frame visual narratives with improved action faithfulness, subject identity fidelity, and background continuity using three novel modules.


<details>
  <summary>Details</summary>
Motivation: There's a threefold tension in generating multi-frame visual narratives without fine-tuning: achieving action text faithfulness, maintaining subject identity fidelity, and ensuring cross-frame background continuity simultaneously.

Method: Three synergistic modules: 1) Gaussian-Centered Attention (GCA) for dynamic subject focus and grounding-box overlap handling, 2) Action-Boost Singular Value Reweighting (AB-SVR) to amplify action-related text embeddings, and 3) Selective Forgetting Cache (SFC) for background cue retention and cross-scene semantic ties.

Result: CLIP-T improves by 10-15%, DreamSim lower than strong baselines, CLIP-I stays competitive. Inference is faster than FluxKontext on 24GB GPU with matched resolution and steps. Qualitatively delivers expressive interactions and evolving yet stable scenes.

Conclusion: StoryTailor successfully addresses the threefold tension in zero-shot visual narrative generation, producing temporally coherent, identity-preserving image sequences with improved performance and efficiency.
...
TL;DR: HorizonForge is a unified framework for controllable driving scene generation that combines editable Gaussian Splats and Meshes with noise-aware video diffusion for photorealistic, consistent scene editing and vehicle insertion.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for controllable driving scene generation struggle to jointly achieve photorealism and precise control, limiting realistic and scalable autonomous driving simulation.

Method: Reconstructs scenes as editable Gaussian Splats and Meshes for fine-grained 3D manipulation, uses language-driven vehicle insertion, and renders edits through noise-aware video diffusion that enforces spatial and temporal consistency in a single feed-forward pass.

Result: Gaussian-Mesh representation delivers substantially higher fidelity than alternative 3D representations, and temporal priors from video diffusion are essential for coherent synthesis. Achieves 83.4% user-preference gain and 25.19% FID improvement over second-best state-of-the-art method.

Conclusion: HorizonForge establishes a simple yet powerful paradigm for photorealistic, controllable driving simulation, with the proposed HorizonSuite benchmark standardizing evaluation for ego- and agent-level editing tasks.
...
TL;DR: Geometry-free view synthesis transformers scale well with compute; encoder-decoder architectures can be compute-optimal, achieving SOTA with less training compute.


<details>
  <summary>Details</summary>
Motivation: While geometry-free view synthesis transformers achieve SOTA in Novel View Synthesis, the factors governing their scaling with compute remain unclear. The paper aims to systematically study scaling laws and derive design principles for compute-optimal NVS models.

Method: Presents systematic study of scaling laws for view synthesis transformers. Develops encoder-decoder architecture called Scalable View Synthesis Model (SVSM) that addresses suboptimal architectural choices identified in prior work. Compares across equal training compute budgets to ensure fair evaluation.

Result: Encoder-decoder architectures can be compute-optimal (contrary to prior findings). SVSM scales as effectively as decoder-only models, achieves superior performance-compute Pareto frontier, and surpasses previous SOTA on real-world NVS benchmarks with substantially reduced training compute.

Conclusion: Proper architectural design enables encoder-decoder transformers to be compute-optimal for view synthesis, challenging previous assumptions and providing more efficient scaling solutions for NVS tasks.
...
TL;DR: OR video diffusion framework synthesizes rare/safety-critical surgical events from geometric abstractions, enabling synthetic dataset creation for AI model training.


<details>
  <summary>Details</summary>
Motivation: Curating real OR datasets with rare/safety-critical events is operationally and ethically challenging, creating a data bottleneck for developing ambient intelligence systems to detect and mitigate such events.

Method: OR video diffusion framework with geometric abstraction module, conditioning module, and fine-tuned diffusion model that transforms OR scenes into abstract geometric representations, conditions synthesis, and generates realistic OR event videos.

Result: Outperforms baseline video diffusion models on routine events (lower FVD/LPIPS, higher SSIM/PSNR), enables controlled synthesis of counterfactual events, and AI model trained on synthetic data achieves 70.13% recall for detecting near safety-critical events.

Conclusion: The framework enables controlled synthesis of routine and rare OR events from geometric abstractions, demonstrating potential to support ambient intelligence model development by generating rare/safety-critical scenarios.
...
TL;DR: MoMKD is a cross-modal knowledge distillation framework that uses momentum memory to transfer genomic supervision to histopathology models, enabling accurate cancer diagnosis using histology alone without paired data limitations.


<details>
  <summary>Details</summary>
Motivation: Clinical translation of multimodal learning (genomics + histopathology) is hindered by limited paired data. Knowledge distillation offers a solution but existing methods suffer from batch-local alignment instability and performance degradation.

Method: Proposes Momentum Memory Knowledge Distillation (MoMKD) with momentum-updated memory that aggregates genomic and histopathology information across batches. Decouples gradients of genomics and histology branches to prevent genomic signal domination and eliminate modality-gap issues.

Result: Outperforms state-of-the-art MIL and multimodal KD baselines on TCGA-BRCA benchmark (HER2, PR, ODX classification) and independent in-house dataset. Delivers strong performance and generalization under histology-only inference.

Conclusion: MoMKD establishes a robust and generalizable knowledge distillation paradigm for computational pathology, enabling accurate cancer diagnosis using histology alone by effectively transferring genomic supervision.
...
TL;DR: MMLoP is a parameter-efficient multi-modal prompt tuning method that uses low-rank factorization to achieve deep prompting with only 11.5K parameters, matching early text-only methods while outperforming many larger models.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal prompt tuning methods require millions of parameters, abandoning the parameter efficiency that makes prompt tuning attractive. There's a need for methods that maintain both performance and efficiency.

Method: MMLoP uses low-rank factorization to parameterize vision and text prompts at each transformer layer. It includes three components: self-regulating consistency loss to anchor to frozen CLIP features, uniform drift correction to preserve class structure, and shared up-projection for cross-modal alignment.

Result: MMLoP achieves highly favorable accuracy-efficiency tradeoff, outperforming most existing methods including those with orders of magnitude more parameters, with 79.70% harmonic mean on base-to-novel generalization across 11 datasets.

Conclusion: MMLoP demonstrates that deep multi-modal prompting can be achieved with extreme parameter efficiency (11.5K parameters) while maintaining strong performance, making prompt tuning truly parameter-efficient again.
...
TL;DR: FlowFixer is an image refinement framework that restores fine details lost in subject-driven generation by using direct image-to-image translation from visual references, avoiding language prompt ambiguities.


<details>
  <summary>Details</summary>
Motivation: Subject-driven generation (SDG) often loses fine details when subjects change in scale and perspective. Current methods struggle with detail fidelity, and existing metrics like CLIP/DINO don't properly assess fine detail preservation.

Method: Uses image-to-image translation from visual references. Introduces one-step denoising to generate self-supervised training data that removes high-frequency details while preserving global structure. Proposes keypoint matching-based metric for detail fidelity assessment.

Result: Outperforms state-of-the-art SDG methods in both qualitative and quantitative evaluations, setting a new benchmark for high-fidelity subject-driven generation.

Conclusion: FlowFixer provides an effective refinement framework for SDG that restores lost fine details through direct image-to-image translation, with better detail fidelity assessment through keypoint matching metrics.
...
TL;DR: Training-free pipeline using Vision-Language Models for open-vocabulary zero-shot temporal action segmentation without task-specific supervision.


<details>
  <summary>Details</summary>
Motivation: Traditional temporal action segmentation methods are limited to closed vocabularies and fixed label sets, making comprehensive dataset collection infeasible due to vast activity spaces and alternative breakdowns.

Method: Two-stage training-free pipeline: 1) Frame-Action Embedding Similarity (FAES) matches video frames to candidate action labels using VLMs, 2) Similarity-Matrix Temporal Segmentation (SMTS) enforces temporal consistency in the segmentation.

Result: Achieves strong results on standard benchmarks without task-specific supervision, with systematic analysis across 14 diverse VLMs showing their suitability for open-vocabulary action segmentation.

Conclusion: Demonstrates the potential of Vision-Language Models for structured temporal understanding in open-vocabulary zero-shot settings, providing first broad analysis of VLMs for this task.
...
TL;DR: Introduces SVG extraction task and WildSVG benchmark for translating real-world images to scalable vector graphics, showing current models underperform but iterative refinement shows promise.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal models work well with clean renderings or text descriptions but fail with real-world images containing noise, clutter, and domain shifts. There's a lack of suitable benchmarks for SVG extraction from natural images.
...
Result: Benchmarked state-of-the-art multimodal models and found they perform well below what's needed for reliable SVG extraction in real scenarios. Current approaches are insufficient for practical use.

Conclusion: Iterative refinement methods point to a promising path forward, and model capabilities are steadily improving. The WildSVG Benchmark provides the first foundation for systematic benchmarking of SVG extraction from real-world images.
...
TL;DR: ECHOSAT is a global, temporally consistent 10m-resolution tree height map using multi-sensor satellite data and a specialized vision transformer with self-supervised growth loss to capture both gradual growth and abrupt disturbances over multiple years.


<details>
  <summary>Details</summary>
Motivation: Existing global tree height maps only provide static snapshots and don't capture temporal forest dynamics, which are essential for accurate carbon accounting and climate change mitigation.

Method: Uses multi-sensor satellite data to train a specialized vision transformer model that performs pixel-level temporal regression, with a self-supervised growth loss that regularizes predictions to follow natural tree development patterns (gradual growth and abrupt declines from disturbances).
...
TL;DR: tugturn.py is a Python workflow for 3D markerless Timed Up and Go analysis that provides phase segmentation, gait event detection, spatiotemporal metrics, coordination analysis, and stability metrics with reproducible outputs.


<details>
  <summary>Details</summary>
Motivation: Current markerless TUG analysis pipelines lack robustness and reproducibility, limiting their clinical and research utility despite the potential of instrumented TUG to support decision-making.

Method: A Python-based workflow using spatial thresholds for phase segmentation (stand, first gait, turning, second gait, sit), relative-distance strategy for gait event detection, and includes Vector Coding and XCoM-based stability metrics, configured via TOML files.

Result: The software produces reproducible artifacts including HTML reports, CSV tables, and quality-assurance visual outputs, with a complete runnable example provided for validation and implementation.

Conclusion: tugturn.py represents a focused software contribution that addresses the need for robust, reproducible markerless biomechanical TUG analysis pipelines for both clinical and research applications.
...
TL;DR: Medical VLMs show high sensitivity to question paraphrasing (8-58% flip rates), but low flip rates don't guarantee visual grounding. The study introduces PSF-Med benchmark, analyzes flip mechanisms via sparse autoencoders, and shows intervention can reduce flips by 31% with minimal accuracy cost.


<details>
  <summary>Details</summary>
Motivation: Medical Vision Language Models (VLMs) exhibit concerning inconsistency when clinicians rephrase the same question, creating deployment risks. Current evaluations don't adequately measure both paraphrase stability and visual grounding.

Method: Created PSF-Med benchmark with 19,748 chest X-ray questions and ~92,000 paraphrases. Evaluated six medical VLMs on yes/no flip rates. Used text-only baselines to test visual grounding. Applied GemmaScope 2 Sparse Autoencoders to MedGemma 4B to analyze flip mechanisms, identifying specific sparse features correlated with decision flips.

Result: Found flip rates from 8% to 58% across models. Text-only baselines revealed some models rely on language priors rather than visual grounding. Identified a sparse feature at layer 17 that predicts decision margin shifts. Causal patching recovered 45% of logit margin and reversed 15% of flips. Feature clamping reduced flip rates by 31% with only 1.3pp accuracy cost.

Conclusion: Flip rate alone is insufficient for robustness evaluation; assessments must test both paraphrase stability and image reliance. Identified mechanisms can be targeted for intervention, showing promise for improving model consistency in clinical settings.
...
TL;DR: AD-Loop introduces an interleaved Analyzing-Drafting paradigm for UVLMs that dynamically alternates between understanding and generation tasks, enabling synergistic multimodal learning through iterative refinement.


<details>
  <summary>Details</summary>
Motivation: Existing UVLMs focus on architectural unification but treat understanding and generation as parallel skills rather than synergistic processes, lacking explicit interaction between these capabilities during task solving.

Method: Proposes AD-Loop: an interleaved Analyzing-Drafting problem-solving loop that dynamically alternates between analytic and drafting operations, interleaving textual and visual thoughts. Uses two-stage training: supervised learning on interleaved thought data followed by reinforcement learning for adaptive control.

Result: AD-Loop consistently improves performance across standard benchmarks for both understanding and generation, with strong transferability to various UVLM architectures. Visual analyses validate effectiveness of implicit visual thoughts.

Conclusion: AD-Loop represents a principled and broadly applicable strategy for synergizing comprehension and creation in multimodal learning, moving beyond architectural unification to achieve genuine capability synergy.
...
TL;DR: Adversarial attacks on thyroid ultrasound segmentation show spatial-domain perturbations are partially mitigable, while frequency-domain attacks resist current defenses.


<details>
  <summary>Details</summary>
Motivation: Deep learning segmentation models in clinical imaging need better characterization of adversarial robustness, especially for ultrasound images where this remains understudied.

Method: Developed two black-box attacks: SSAA (boundary-targeted noise) and FDUA (Fourier-domain perturbations). Evaluated three inference-time defenses: randomized preprocessing, deterministic denoising, and stochastic ensemble inference on U-Net trained on thyroid nodule ultrasound.

Result: SSAA reduced DSC by 0.29 with high visual similarity; FDUA reduced DSC by 0.11 with lower visual fidelity. Defenses significantly improved DSC against SSAA (denoising best at +0.10) but none worked against FDUA.

Conclusion: Spatial-domain ultrasound adversarial perturbations are partially mitigable with preprocessing, but frequency-domain attacks resist current defenses, revealing modality-specific adversarial robustness challenges.
...
TL;DR: Dynamic VPR mapping approach automatically selects optimal map density to meet user-defined local recall requirements across environment regions, outperforming fixed-density benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current VPR systems use fixed, engineering-driven map densities that don't guarantee performance across specific environment regions. There's a need to ensure systems meet user-specified local performance requirements rather than just global averages.

Method: Uses pairs of reference traverses to model match patterns across different map densities, predicting the density needed to achieve target Local Recall@1 and Recall Achievement Rate (RAR) on unseen deployment data.

Result: System consistently achieves/exceeds specified local recall levels over user-specified environment proportions across multiple VPR methods and benchmarks (Nordland, Oxford RobotCar). Outperforms baselines by avoiding unnecessary over-densification.

Conclusion: Dynamic density selection enables reliable local performance guarantees, conventional global Recall@1 is poor predictor of operationally meaningful RAR, and the approach provides practical solution for real-world VPR deployment.
...
TL;DR: SPL is a unified framework for unsupervised and sparsely-supervised 3D object detection that uses semantic pseudo-labeling and prototype learning to reduce annotation dependency.


<details>
  <summary>Details</summary>
Motivation: 3D object detection relies heavily on large-scale manual annotations, limiting scalability and adaptability. Existing unsupervised and sparsely-supervised methods face challenges with low-quality pseudo-labels, unstable feature mining, and lack of unified training frameworks.

Method: SPL generates high-quality pseudo-labels by integrating image semantics, point cloud geometry, and temporal cues. These are used as probabilistic priors in a multi-stage prototype learning strategy with memory-based initialization and momentum-based prototype updating to stabilize feature representation learning.
...
TL;DR: Training-free plug-and-play framework reduces visual hallucination propagation in multimodal reasoning by supervising each reasoning step with visual evidence at test time.


<details>
  <summary>Details</summary>
Motivation: Chain-of-thought reasoning in multimodal contexts is vulnerable to visual hallucination propagation - once an intermediate step becomes inconsistent with visual evidence, subsequent steps can lead to incorrect answers even if logically valid. Existing RL-based solutions are costly, model-specific, and difficult to generalize.

Method: Lightweight, training-free framework that supervises each reasoning step at test time with visual evidence. Uses a textual visual-evidence pool to guide reasoning generation, with a visual decider module that dynamically extracts additional relevant evidence from images when existing evidence is insufficient.
...
Conclusion: Proposes an effective, lightweight, training-free approach to mitigate visual hallucination propagation in multimodal reasoning that outperforms existing methods and works across different model architectures without requiring costly retraining.
...
TL;DR: A feedforward 3D editing framework using TRELLIS generative backbone that enables fast, globally consistent editing from single view with Voxel FlowEdit for geometry and normal-guided generation for high-fidelity textures.


<details>
  <summary>Details</summary>
Motivation: Existing 3D editing methods are computationally intensive (require scene-by-scene iterative optimization) and suffer from multi-view inconsistency, making them inefficient and unreliable for practical 3D editing applications.

Method: Proposes a fully feedforward 3D editing framework based on TRELLIS generative backbone. Uses Voxel FlowEdit for edit-driven flow in sparse voxel latent space to achieve globally consistent 3D deformation in single pass. Develops normal-guided single to multi-view generation module as external appearance prior to restore high-fidelity textures.

Result: Experiments demonstrate the method enables fast, globally consistent, and high-fidelity 3D model editing, overcoming limitations of previous iterative optimization approaches.

Conclusion: The proposed framework provides an effective solution for 3D editing that addresses geometric consistency and appearance fidelity challenges, offering a practical alternative to computationally intensive existing methods.
...
TL;DR: AHAN improves twin face verification by 3.4% using hierarchical attention and facial asymmetry analysis


<details>
  <summary>Details</summary>
Motivation: Current face recognition systems fail on identical twins (dropping from 99.8% to 88.9% accuracy), exposing critical biometric security vulnerabilities. The challenge is learning subtle non-genetic variations that uniquely identify individuals despite overwhelming genetic similarity.

Method: Proposes Asymmetric Hierarchical Attention Network (AHAN) with three key components: 1) Hierarchical Cross-Attention (HCA) for multi-scale analysis of semantic facial regions, 2) Facial Asymmetry Attention Module (FAAM) that computes cross-attention between left and right facial halves to capture subtle asymmetric patterns, and 3) Twin-Aware Pair-Wise Cross-Attention (TA-PWCA) training regularization using each subject's own twin as the hardest distractor.
...
Conclusion: The proposed AHAN architecture effectively addresses the extreme fine-grained challenge of identical twin verification through specialized attention mechanisms that capture subtle, non-genetic facial variations, significantly improving biometric security system robustness against genetic look-alikes.
...
TL;DR: TEA-CXA: A tool-expertise-aware chest X-ray agent that learns which AI tools to trust for different multimodal queries through reinforcement learning, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Medical AI tools are error-prone and can produce contradictory responses, but existing medical agents lack understanding of tools' realistic reliability and cannot effectively resolve tool conflicts.

Method: Introduces a framework where agents interact with tools to empirically learn practical trustworthiness via agentic learning. Specifically presents TEA-CXA for chest X-ray analysis, extending RL codebases to support multimodal contexts, multiple tool calls per turn, parallel inference, and multi-image queries.

Result: TEA-CXA outperforms state-of-the-art methods and comprehensive baselines in experiments.

Conclusion: The framework enables effective resolution of tool conflicts in medical AI by learning which tools to trust for different query types, with code applicable to general medical research on multi-turn tool-calling RL in multimodal settings.
...
TL;DR: A Virtual Biopsy framework using MRI and vision-language models achieves over 90% accuracy for non-invasive diagnosis of deep intracranial tumors, addressing data scarcity and sampling bias issues in traditional biopsy methods.


<details>
  <summary>Details</summary>
Motivation: Deep intracranial tumors in eloquent brain regions present diagnostic challenges. Traditional stereotactic biopsy carries risks of hemorrhage, neurological deficits, and suffers from sampling bias due to tumor heterogeneity. There's a need for non-invasive MRI-based pathology prediction for holistic tumor assessment and better clinical decision-making.

Method: Proposes a Virtual Biopsy framework with three components: MRI-Processor for standardization, Tumor-Localizer using vision-language models for coarse-to-fine localization via weak supervision, and Adaptive-Diagnoser with a Masked Channel Attention mechanism that fuses local discriminative features with global contexts. Built on the ICT-MRI dataset - first public biopsy-verified benchmark with 249 cases across four categories.

Result: Achieves over 90% accuracy in pathology prediction, outperforming baseline methods by more than 20%.

Conclusion: The Virtual Biopsy framework provides an effective non-invasive alternative to traditional biopsy, addressing data scarcity and sampling bias while achieving high diagnostic accuracy for deep intracranial tumors.
...
TL;DR: Novel framework for sparse-view 3D outdoor scene reconstruction using bidirectional pseudo frame restoration and scene perception Gaussian management to handle extreme view sparsity.


<details>
  <summary>Details</summary>
Motivation: 3D scene reconstruction from unposed sparse viewpoints is challenging in outdoor scenes due to complex lighting and scale variation. Existing methods using diffusion models for pseudo frame synthesis introduce unreasonable geometry that harms reconstruction quality.

Method: Proposes bidirectional pseudo frame restoration (diffusion-based synthesis guided by adjacent frames with lightweight pseudo-view deblur model and confidence mask inference) and scene perception Gaussian management (optimizing Gaussians based on joint depth-density information).

Result: Experiments on outdoor benchmarks demonstrate substantial gains over existing methods in both fidelity and stability. The approach enhances reconstruction completeness, suppresses floating artifacts, and improves geometric consistency under extreme view sparsity.

Conclusion: The proposed framework effectively addresses sparse-view outdoor reconstruction challenges through innovative bidirectional restoration and Gaussian management strategies, achieving high-quality results even with extremely limited input views.
...
TL;DR: IHF-Harmony is an invertible hierarchy flow framework for multi-modality MRI harmonization using unpaired data, ensuring bijective mapping and lossless reconstruction to prevent anatomical distortion.


<details>
  <summary>Details</summary>
Motivation: Current MRI harmonization methods have poor scalability across modalities and rely on traveling subject datasets, limiting their practical application in large-scale multi-site studies.
...
Result: Outperforms existing methods in both anatomical fidelity and downstream task performance across multiple MRI modalities, enabling robust harmonization for large-scale multi-site imaging studies.

Conclusion: IHF-Harmony provides a scalable, unpaired data solution for multi-modality MRI harmonization that preserves anatomical integrity while transferring target characteristics, facilitating large-scale imaging research.
...
TL;DR: CCCaption introduces a reinforcement learning framework with dual rewards for completeness and correctness to generate better image captions beyond human-annotation limitations.


<details>
  <summary>Details</summary>
Motivation: Human-annotated captions are often incomplete or incorrect due to subjective preferences, limiting caption model performance. The paper argues caption quality should be assessed objectively by completeness (covering all salient visual facts) and correctness (truthfulness to the image).

Method: CCCaption uses dual-reward reinforcement learning: 1) For completeness, diverse LVLMs disentangle images into visual queries, rewarding captions that answer more queries with dynamic sampling for efficiency; 2) For correctness, penalizes hallucinations by validating sub-caption query authenticity through caption decomposition.

Result: Extensive experiments across standard captioning benchmarks show consistent improvements, demonstrating the framework's effectiveness in generating more complete and correct captions.

Conclusion: The approach offers a principled path to training caption models beyond human-annotation imitation by explicitly optimizing for objective completeness and correctness criteria through symmetric dual-reward optimization.
...
TL;DR: VasGuideNet: A Couinaud liver segmentation framework that explicitly models vascular topology using GCNs and cross-attention fusion, achieving state-of-the-art performance on liver vessel segmentation tasks.
...
Method: Proposes VasGuideNet with: 1) Graph Convolutional Networks encoding skeletonized vessels, Euclidean distance transform geometry, and kNN connectivity into topology features; 2) Cross-attention fusion module injecting topology features into 3D encoder-decoder backbone; 3) Structural Contrastive Loss with global memory bank for improved inter-class separability and anatomical consistency.

Result: Achieves Dice scores of 83.68% on Task08_HepaticVessel and 76.65% on private LASSD dataset with RVDs of 1.68 and 7.08 respectively. Consistently outperforms UNETR, Swin UNETR, and G-UNETR++ across metrics.

Conclusion: VasGuideNet demonstrates that explicit vascular topology guidance significantly improves Couinaud liver segmentation, producing anatomically consistent results with better generalization under anatomical variability.
...
TL;DR: VCC-Net is a visual cognition-guided collaborative network that integrates radiologists' visual search patterns with AI models for improved chest X-ray diagnosis through human-AI collaboration.
...
Method: Proposes VCC-Net that captures radiologists' visual search traces via eye-tracking/mouse interfaces, uses visual cognition as spatial guide to learn hierarchical search strategies, and employs cognition-graph co-editing to integrate radiologist VC with model inference, constructing disease-aware graphs.
...
Conclusion: VCC-Net successfully bridges the gap between radiologists' cognitive processes and AI models, enabling complementary and transparent decision-making in chest X-ray diagnosis through effective human-AI collaboration.
...
TL;DR: GPOcc: A framework that leverages visual geometry priors for monocular 3D occupancy prediction using Gaussian primitives and incremental updates for streaming input.


<details>
  <summary>Details</summary>
Motivation: Existing occupancy prediction methods rely on depth priors but make limited use of 3D cues, while visual geometry models like VGGT provide rich 3D priors but only operate at surface level rather than volumetric interiors. The paper aims to better leverage these powerful geometry priors for 3D occupancy prediction.
...
Result: Significant improvements on Occ-ScanNet and EmbodiedOcc-ScanNet: +9.99 mIoU in monocular setting, +11.79 mIoU in streaming setting over prior SOTA. Under same depth prior, achieves +6.73 mIoU while running 2.65× faster.

Conclusion: GPOcc demonstrates more effective and efficient use of geometry priors for 3D occupancy prediction, showing substantial performance gains in both monocular and streaming settings while being computationally efficient.
...
TL;DR: Proposes a DiT-based multi-character image animation framework with Identifier Assigner and Adapter components that handles multiple characters without identity confusion or implausible occlusions.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based human image animation methods are limited to single-character scenarios, and naive extensions to multi-character settings cause identity confusion and unrealistic occlusions between characters.

Method: Built on Diffusion Transformers (DiTs), introduces two key components: Identifier Assigner (captures per-person positional cues) and Identifier Adapter (handles inter-person spatial relationships). Uses mask-driven scheme with scalable training strategy for flexibility and generalization.

Result: Achieves state-of-the-art performance in multi-character image animation, surpassing existing diffusion-based baselines. Model trained on only two-character data generalizes to multi-character scenarios while maintaining compatibility with single-character cases.

Conclusion: Proposed framework successfully addresses multi-character animation challenges with identity preservation and realistic spatial relationships, demonstrating extensibility and strong generalization capabilities beyond training data.
...
TL;DR: Dynamic Multimodal Activation Steering: A training-free method that mitigates hallucination in Large Vision-Language Models by using semantic-based truthfulness steering vectors and visual perception steering vectors applied to specific attention heads.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models (LVLMs) show strong performance but suffer from hallucination problems. The paper aims to address this issue by analyzing activation patterns to understand how truthfulness and visual perception capabilities are distributed within the model architecture.

Method: Dynamic Multimodal Activation Steering: 1) Constructs a semantic-based truthfulness steering vector database, 2) Computes visual perception steering vectors, 3) Dynamically selects relevant steering vectors based on input semantic similarity during inference, and 4) Applies them to the most influential attention heads identified through analysis.

Result: The approach significantly enhances model performance across multiple models and datasets, outperforming existing state-of-the-art methods for hallucination mitigation.

Conclusion: The paper demonstrates that truthfulness and visual perception capabilities engage different subsets of attention heads, and that context-aware interventions using dynamic steering vectors can effectively mitigate hallucination in LVLMs without requiring additional training.
...
TL;DR: SEFMAP is a robust multimodal HD map prediction framework that disentangles BEV features into four semantic subspaces with dedicated experts, uses uncertainty-aware gating, and employs distribution-aware masking to handle degraded conditions.


<details>
  <summary>Details</summary>
Motivation: HD maps are crucial for autonomous driving, but current multi-modal fusion methods suffer from inconsistency between camera and LiDAR modalities, leading to performance degradation under challenging conditions like low-light, occlusions, or sparse point clouds.

Method: Disentangles BEV features into four semantic subspaces (LiDAR-private, Image-private, Shared, and Interaction) with dedicated experts. Uses uncertainty-aware gating at BEV-cell level to adaptively combine expert outputs, plus distribution-aware masking during training with EMA-statistical surrogate features and specialization loss to enhance robustness.
...
Conclusion: SEFMAP provides a robust and effective solution for multi-modal HD map prediction under diverse and degraded conditions by explicitly modeling modality relationships and adaptively fusing information.
...
TL;DR: SurGo-R1 is a surgical AI system that identifies safe operative zones in laparoscopic surgery by first determining surgical phase, then generating reasoning and Go Zone coordinates, significantly outperforming generalist vision-language models.


<details>
  <summary>Details</summary>
Motivation: Current AI systems for minimally invasive surgery offer binary safety verification or static detection, ignoring the phase-dependent nature of intraoperative reasoning where surgeons must integrate visual cues, procedural phase, and anatomical context under high cognitive load.

Method: Introduces ResGo benchmark with annotated laparoscopic frames, then presents SurGo-R1 model optimized via RLHF with multi-turn phase-then-go architecture: first identifies surgical phase, then generates reasoning and Go Zone coordinates conditioned on that context.
...
Conclusion: The phase-then-go architecture effectively addresses the phase-dependent nature of surgical safety reasoning, with the ResGo benchmark and SurGo-R1 model advancing AI-assisted surgical decision-making beyond binary verification to context-aware reasoning.
...
TL;DR: A content-adaptive diffusion-based image codec with three innovations: adaptive quantization using uncertainty maps, auxiliary decoder-guided information concentration, and bitrate-free textual conditioning from reconstructed images.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based image compression methods lack effective content adaptation due to three limitations: isotropic quantization misaligns with content complexity, information concentration bottleneck prevents adaptive semantic preservation, and textual conditioning strategies are inefficient or generic.

Method: Three technical innovations: 1) Uncertainty-Guided Adaptive Quantization using spatial uncertainty maps, 2) Auxiliary Decoder-Guided Information Concentration with lightweight auxiliary decoder, 3) Bitrate-Free Adaptive Textual Conditioning deriving descriptions from reconstructed images.

Result: The proposed method enables effective content adaptation in diffusion-based image compression, addressing the three critical limitations of existing approaches.

Conclusion: The content-adaptive diffusion-based image codec with three innovations overcomes key limitations in existing methods, enabling realistic reconstruction at ultra-low bitrates through dynamic alignment with image content characteristics.
...
TL;DR: FAQ benchmark enables VLMs to detect deepfakes by analyzing temporal inconsistencies in videos through a three-level hierarchical multiple-choice task.


<details>
  <summary>Details</summary>
Motivation: Current VLMs for deepfake detection focus only on spatial artifacts and overlook temporal inconsistencies in video forgeries, creating a gap in dynamic cue analysis.

Method: Proposes Forensic Answer-Questioning (FAQ) benchmark with three-level hierarchy: Facial Perception, Temporal Deepfake Grounding, and Forensic Reasoning, plus FAQ-IT instruction-tuning set.

Result: Models fine-tuned on FAQ-IT achieve advanced performance on both in-domain and cross-dataset detection benchmarks, with ablation studies confirming FAQ's impact.

Conclusion: FAQ successfully bridges the temporal reasoning gap in VLMs for deepfake detection, enabling them to analyze dynamic forgery artifacts across video frames.
...
TL;DR: Diffusion Transformers have redundant conditional embeddings with extreme angular similarity (>99%), where semantic info concentrates in few dimensions, enabling pruning of 2/3 of embedding space without quality loss.


<details>
  <summary>Details</summary>
Motivation: To systematically study the structure of learned conditional embeddings in Diffusion Transformers, which achieve SOTA performance but whose embedding structure remains poorly understood.

Method: Analyzed class-conditioned and continuous-condition embeddings, measured angular similarity, identified semantic concentration patterns, and performed dimension pruning experiments on low-magnitude dimensions.

Result: Found extreme angular similarity (>99% for ImageNet-1K, >99.9% for continuous tasks), semantic info concentrated in head dimensions, and pruning up to 2/3 of embedding space maintains or improves generation quality.

Conclusion: Reveals a semantic bottleneck in Transformer-based diffusion models, provides insights into semantic encoding, and suggests opportunities for more efficient conditioning mechanisms through dimension pruning.
...
<details>
  <summary>Details</summary>
Motivation: Current fMRI-to-video reconstruction approaches suffer from two major shortcomings: (1) inconsistent visual representations of salient objects across frames leading to appearance mismatches, and (2) poor temporal coherence resulting in motion misalignment or abrupt frame transitions.

Method: Introduces SemVideo with three key components: 1) Semantic Alignment Decoder aligning fMRI signals with CLIP-style embeddings from SemMiner (hierarchical guidance module), 2) Motion Adaptation Decoder reconstructing dynamic motion patterns using tripartite attention fusion, and 3) Conditional Video Render leveraging hierarchical semantic guidance.

Result: Experiments on CC2017 and HCP datasets show SemVideo achieves superior performance in both semantic alignment and temporal consistency, setting new state-of-the-art in fMRI-to-video reconstruction.

Conclusion: SemVideo successfully addresses the limitations of current fMRI-to-video reconstruction approaches by incorporating hierarchical semantic guidance, leading to improved visual consistency and temporal coherence in reconstructed videos from brain activity.
...
TL;DR: StoryMovie dataset aligns movie scripts with subtitles to improve visual storytelling models' dialogue attribution and character relationship accuracy, with fine-tuned Qwen Storyteller3 showing significant improvements over baseline models.


<details>
  <summary>Details</summary>
Motivation: Visual storytelling models often hallucinate semantic relationships even when correctly grounding entities in images, leading to incorrect dialogue attribution, character interactions, and emotional states.

Method: Created StoryMovie dataset (1,757 stories) by aligning movie scripts with subtitles using LCS matching, synchronizing screenplay dialogue with subtitle timestamps to link character names to temporal positions. Fine-tuned Qwen Storyteller3 on this aligned dataset.

Result: Storyteller3 achieves 89.9% win rate against base Qwen2.5-VL 7B on subtitle alignment, and 48.5% vs 38.0% for Storyteller without script grounding, showing semantic alignment progressively improves dialogue attribution beyond visual grounding alone.

Conclusion: Semantic alignment through script-subtitle synchronization significantly improves dialogue attribution and character relationship accuracy in visual storytelling models, going beyond basic visual grounding.
...
TL;DR: Unified semantic segmentation approach using language modeling to output masks as discrete RLE tokens, extended to videos and panoptic segmentation.


<details>
  <summary>Details</summary>
Motivation: To create a unified approach for semantic segmentation in both images and videos using language modeling techniques, overcoming computational limitations while maintaining competitive performance.

Method: Uses run length encoding (RLE) to discretize segmentation masks, trains modified Pix2Seq model to output RLE tokens autoregressively, develops novel tokenization strategies for video compression, and incorporates instance information for panoptic segmentation.

Result: Proposed models are competitive with state-of-the-art methods despite limited computational resources, validated on two datasets.

Conclusion: Language modeling with RLE tokenization provides an effective unified framework for semantic and panoptic segmentation across images and videos, demonstrating viability even with computational constraints.
...
TL;DR: L2RP framework improves endoscopic video annotation by learning when to seek expert input to balance accuracy and human effort, outperforming baseline methods on dysplasia datasets.


<details>
  <summary>Details</summary>
Motivation: Endoscopic video annotation is time-consuming, especially for challenging cases like Barrett's esophagus dysplasia with irregular boundaries. Semi-automatic tools like SAM2 help but suffer from error accumulation requiring expert correction.

Method: Proposed Learning-to-Re-Prompt (L2RP), a cost-aware framework that learns when and where to seek expert input. Systematically studied error propagation across different prompt types (masks, boxes, points) and tunes a human-cost parameter to balance annotation effort and segmentation accuracy.
...
Conclusion: L2RP effectively balances annotation accuracy and human effort by intelligently deciding when to request expert input, improving endoscopic video annotation for challenging medical cases.
...
TL;DR: UniHand is a unified diffusion framework that combines hand motion estimation and generation into a single conditional synthesis model, handling diverse inputs like MANO parameters, 2D skeletons, and visual observations through shared latent space alignment.


<details>
  <summary>Details</summary>
Motivation: Current hand motion research is divided into separate estimation and generation tasks, limiting the use of heterogeneous condition signals and preventing knowledge transfer between tasks. This separation fails to handle practical scenarios with occlusions or incomplete inputs.

Method: UniHand uses a unified diffusion-based framework with: 1) joint variational autoencoder to embed structured signals (MANO parameters, 2D skeletons) into shared latent space, 2) frozen vision backbone for visual observations, 3) hand perceptron for hand-specific feature extraction without detection/cropping, and 4) latent diffusion model for motion synthesis from diverse conditions.

Result: Extensive experiments show UniHand delivers robust and accurate hand motion modeling, maintaining performance under severe occlusions and temporally incomplete inputs across multiple benchmarks.

Conclusion: UniHand successfully unifies hand motion estimation and generation through a conditional diffusion framework, enabling effective handling of heterogeneous inputs and demonstrating strong performance in challenging scenarios.
...
TL;DR: DynamicGTR framework dynamically selects optimal graph topology representations for VLMs to improve zero-shot graph QA with customizable accuracy-brevity trade-off.


<details>
  <summary>Details</summary>
Motivation: Current VLM approaches for graph QA use a single fixed graph representation that ignores model-specific and task-specific preferences, leading to inaccurate or verbose responses.

Method: Proposes DynamicGTR framework that dynamically selects the optimal graph topology representation for each query during inference to enhance VLM graph QA capabilities.

Result: Improves VLM-based graph algorithm QA performance, successfully transfers from synthetic tasks to real-world applications (link prediction, node classification) without additional training, and shows strong cross-task/domain/model transferability.

Conclusion: DynamicGTR offers a flexible solution for broad graph scenarios by addressing the limitations of fixed graph representations and enabling customizable accuracy-brevity trade-offs for VLM-based graph QA.
...
TL;DR: Proposes an axial-centric cross-plane attention architecture for 3D medical image classification that mimics clinical interpretation workflows where axial plane is primary, outperforming existing methods on MedMNIST3D benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing 3D deep learning methods don't reflect how clinicians actually interpret medical images - they use axial plane as primary reference with coronal/sagittal as complementary views, but current methods either process volumes holistically or treat all planes equally.

Method: Uses MedDINOv3 (frozen medical vision foundation model) as feature extractor for all three planes. Adds RICA blocks and intra-plane transformers for plane-specific features, plus axial-centric cross-plane transformers that condition axial features on auxiliary plane information.

Result: Outperforms existing 3D and multi-plane models on six MedMNIST3D datasets in accuracy and AUC. Ablation studies confirm importance of axial-centric query-key-value allocation and directional cross-plane fusion.

Conclusion: Aligning architectural design with clinical interpretation workflows (axial-centric approach) leads to more robust and data-efficient 3D medical image analysis.
...
TL;DR: Automated framework for coronary artery calcium scoring works on both ECG-gated and non-gated CT scans using self-supervised Vision Transformer trained only on gated data.


<details>
  <summary>Details</summary>
Motivation: CAC scoring is crucial for cardiovascular risk assessment but currently requires specialized ECG-gated CT scans, limiting its use to cardiac imaging settings. There's a need to enable CAC scoring from routine non-gated chest CT scans.

Method: Developed CARD-ViT, a self-supervised Vision Transformer trained exclusively on gated CT data using DINO framework. The automated framework performs CAC detection and lesion-specific Agatston scoring across both gated and non-gated CT scans without requiring non-gated training data.

Result: On non-gated Stanford dataset: 0.707 accuracy, Cohen's kappa 0.528 (matching models trained directly on non-gated scans). On gated test sets: 0.910 accuracy, Cohen's kappa 0.871-0.874 across independent datasets, demonstrating robust risk stratification.

Conclusion: The framework enables cross-domain CAC scoring from gated to non-gated domains, supporting scalable cardiovascular screening in routine chest imaging without requiring additional scans or annotations, potentially expanding access to cardiovascular risk assessment.
...
TL;DR: CARE is a pathology foundation model that automatically partitions WSIs into morphologically relevant regions using molecular guidance, achieving superior performance with less data.


<details>
  <summary>Details</summary>
Motivation: Existing pathology foundation models rely on natural image backbones not tailored for tissue morphology, failing to capture coherent tissue architecture and limiting interpretability and clinical relevance.

Method: Two-stage pretraining: (1) self-supervised unimodal pretraining on 34,277 WSIs without annotations, (2) cross-modal alignment using RNA/protein profiles to refine adaptive region construction and representation.

Result: Using only 1/10 of typical pretraining data, CARE achieves superior average performance across 33 downstream benchmarks (morphological classification, molecular prediction, survival analysis) and outperforms other foundation models.

Conclusion: CARE effectively addresses limitations of existing pathology models by incorporating molecular guidance to identify biologically relevant tissue regions, enabling better interpretability and clinical utility with less data.
...
TL;DR: PatchDenoiser is a lightweight multi-scale patch-based denoising framework that effectively removes noise while preserving anatomical details, outperforming CNN/GAN methods with significantly fewer parameters and lower energy consumption.


<details>
  <summary>Details</summary>
Motivation: Medical image quality is degraded by noise from various sources, affecting clinical interpretation and analysis. Traditional filters over-smooth details, while deep learning methods struggle with detail preservation or require large, expensive models that limit clinical practicality.

Method: A lightweight, energy-efficient multi-scale patch-based denoising framework that decomposes denoising into local texture extraction and global context aggregation, fused via spatially aware patch fusion strategy.

Result: Outperforms state-of-the-art CNN- and GAN-based methods on Mayo Low-Dose CT dataset in PSNR/SSIM, robust to variations in slice thickness/kernels/HU windows, generalizes across scanners without fine-tuning, reduces parameters by ~9x and energy consumption by ~27x.

Conclusion: Provides a practical, scalable, computationally efficient solution for medical image denoising that balances performance, robustness, and clinical deployability.
...
TL;DR: LieFlow: A dynamic radiance representation framework using SE(3) Lie group transformations to model 4D scenes with physically consistent rigid and non-rigid motions.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for 4D scene modeling rely on translational displacements that struggle with rotations and articulated transformations, leading to spatial inconsistency and physically implausible motion.

Method: LieFlow explicitly models motion within the SE(3) Lie group, enabling coherent learning of translation and rotation in a unified geometric space with physically inspired constraints for motion continuity and geometric consistency.

Result: Across synthetic rigid-body datasets and real-world datasets with complex motion under natural lighting and occlusions, LieFlow consistently improves view-synthesis fidelity, temporal coherence, and physical realism over NeRF-based baselines.

Conclusion: SE(3)-based motion modeling offers a robust and physically grounded framework for representing dynamic 4D scenes, confirming the advantages of geometric motion representation over purely translational approaches.
...
TL;DR: Proposes an RGB-Event fusion approach for Kilometer Marker Recognition in metro systems, introduces EvMetro5K dataset, and shows improved robustness in challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Metro visual perception faces challenges from illumination variations, high-speed motion, and adverse weather. Event cameras offer advantages in low-light, high-speed scenarios with low power consumption, making them suitable for GNSS-denied autonomous metro localization.

Method: Uses a pre-trained RGB OCR foundation model enhanced through multi-modal adaptation for Kilometer Marker Recognition. Creates the first large-scale RGB-Event dataset (EvMetro5K) with 5,599 synchronized RGB-Event sample pairs.

Result: Extensive experiments on EvMetro5K and other benchmarks demonstrate the effectiveness of the proposed approach for Kilometer Marker Recognition in challenging metro environments.

Conclusion: Integration of event cameras with RGB systems improves robustness for metro perception tasks. The released EvMetro5K dataset and source code will facilitate further research in multi-modal perception for autonomous metro systems.
...
TL;DR: Proposes a large-scale PDE pre-trained neural operator using nested Mixture-of-Experts (MoE) framework to capture both global and local dependencies, improving generalization for diverse PDE systems.


<details>
  <summary>Details</summary>
Motivation: Existing neural operators use single network architectures that limit their ability to capture heterogeneous features and complex dependencies in diverse PDE systems, creating a bottleneck for large-scale PDE pre-training.

Method: Nested Mixture-of-Experts framework with image-level MoE for global dependencies and token-level Sub-MoE for local dependencies, enabling selective activation of expert networks based on input.

Result: Successfully conducted large-scale pre-training on twelve diverse PDE datasets and demonstrated effective transfer to downstream tasks through extensive experiments.

Conclusion: The proposed nested MoE framework enhances neural operator capacity for PDE solving by better capturing heterogeneous features and dependencies, enabling more effective large-scale pre-training and transfer learning.
...
TL;DR: NoLan is a training-free framework that reduces object hallucinations in LVLMs by suppressing language decoder priors based on multimodal vs text-only output differences.


<details>
  <summary>Details</summary>
Motivation: To determine which component (vision encoder vs language decoder) primarily causes object hallucinations in Large Vision-Language Models, and develop a solution based on the findings.

Method: Systematic experiments to analyze hallucination sources, revealing language decoder priors as main cause. Proposed NoLan framework dynamically suppresses language priors by modulating output distribution differences between multimodal and text-only inputs.
...
Conclusion: Object hallucinations in LVLMs are primarily caused by language decoder priors, and the proposed training-free NoLan framework successfully mitigates this issue by dynamically suppressing these priors.
...
TL;DR: HybridINR-PCGC: A hybrid point cloud compression framework combining pretrained models with implicit neural representations to achieve distribution-agnostic compression with faster convergence and reduced model overhead.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based point cloud compression methods have limitations: pretrained methods suffer from training data dependency and poor generalization, while INR methods are distribution-agnostic but require time-consuming online training and have bitstream overhead from overfitted models.

Method: Proposes a hybrid framework with two components: 1) Pretrained Prior Network (PPN) for fast inference and stable performance to generate robust priors, and 2) Distribution Agnostic Refiner (DAR) decomposed into base and enhancement layers (only enhancement layer packed into bitstream). Includes supervised model compression module to minimize enhancement layer bitrate.

Result: Achieves significant compression improvements: 20.43% Bpp reduction vs G-PCC on 8iVFB, 57.85% Bpp reduction vs UniPCGC on Cat1B (out-of-distribution), and 15.193% average Bpp reduction vs LINR-PCGC on 8iVFB with superior time-rate trade-off.

Conclusion: HybridINR-PCGC successfully bridges pretrained models and INR, retaining distribution-agnostic properties while accelerating convergence and reducing model overhead, achieving state-of-the-art compression performance across both in-distribution and challenging out-of-distribution scenarios.
...
TL;DR: Off-the-shelf image-to-image GenAI models can be repurposed as generic denoisers to remove protective perturbations from images, bypassing 6 diverse protection schemes and outperforming specialized attacks.


<details>
  <summary>Details</summary>
Motivation: Current image protection methods rely on imperceptible perturbations to prevent unauthorized use, but existing attacks require specialized methods. The paper aims to show that generic attacks using readily available GenAI models are now possible, revealing vulnerabilities in current protection schemes.

Method: Repurpose off-the-shelf image-to-image GenAI models as generic "denoisers" using simple text prompts to remove protective perturbations. Tested across 8 case studies spanning 6 diverse protection schemes.

Result: The general-purpose attack successfully circumvents all tested defenses, outperforms existing specialized attacks, and preserves image utility for adversaries while revealing widespread vulnerabilities in current protection methods.

Conclusion: Current image protection schemes provide a false sense of security, and there's an urgent need for robust defenses. Future protection mechanisms must be benchmarked against attacks from off-the-shelf GenAI models.
...
TL;DR: QPoint2Comm is a quantized point-cloud communication framework for collaborative perception that reduces bandwidth while maintaining high accuracy and robustness to packet loss through shared codebook quantization and masked training.


<details>
  <summary>Details</summary>
Motivation: Existing collaborative perception approaches struggle with high bandwidth requirements and vulnerability to transmission packet loss, limiting their practical deployment in connected vehicle systems.

Method: Uses quantized point-cloud indices with shared codebook instead of intermediate features; employs masked training to simulate packet loss for robustness; includes cascade attention fusion module for multi-vehicle information integration.

Result: Sets new state-of-the-art in accuracy, communication efficiency, and resilience to packet loss on both simulated and real-world datasets.

Conclusion: QPoint2Comm provides an effective solution for bandwidth-efficient and robust collaborative perception in connected vehicles, addressing key practical deployment challenges.
...
TL;DR: UNet-based neural network for cone keypoint detection in autonomous racing, using large custom dataset to improve accuracy over traditional methods and enable real-time performance.


<details>
  <summary>Details</summary>
Motivation: Accurate 3D cone localization is crucial for autonomous racing navigation. Traditional computer vision methods are sensitive to environmental variations, while existing neural networks are often trained on limited data and cannot run in real time.

Method: UNet-based neural network for keypoint detection on cones, trained on the largest custom-labeled dataset assembled for this purpose. The approach enables accurate cone position estimation and potential color prediction.
...
Conclusion: The approach is effective and has potential for adoption in competitive autonomous racing systems, showing robust performance in real-world racing scenarios.
...
TL;DR: MoGaF is a framework for long-term scene forecasting that uses motion-aware Gaussian grouping with 4D Gaussian Splatting to achieve physically consistent motion and stable temporal evolution.
...
Method: Motion Group-aware Gaussian Forecasting (MoGaF) uses 4D Gaussian Splatting with motion-aware Gaussian grouping and group-wise optimization to enforce physically consistent motion across rigid and non-rigid regions, plus a lightweight forecasting module for future motion prediction.

Result: MoGaF consistently outperforms existing baselines on synthetic and real-world datasets in rendering quality, motion plausibility, and long-term forecasting stability.

Conclusion: MoGaF provides an effective framework for long-term scene extrapolation with physically consistent motion and stable temporal evolution through structured space-time representation.
...
TL;DR: E-comIQ-ZH: A framework for evaluating Chinese e-commerce posters with specialized models and datasets that address quality assessment gaps in generative AI for commercial design.


<details>
  <summary>Details</summary>
Motivation: Existing AI quality assessment models for commercial posters focus on generic aesthetics or low-level distortions, lacking functional criteria needed for e-commerce design. This is especially problematic for Chinese content where complex characters often produce subtle but critical textual artifacts that current methods overlook.

Method: Created E-comIQ-18k dataset with multi-dimensional scores and expert-calibrated Chain of Thought rationales. Trained E-comIQ-M, a specialized evaluation model aligned with human expert judgment. Built E-comIQ-Bench as the first automated and scalable benchmark for Chinese e-commerce poster generation.

Result: Extensive experiments show E-comIQ-M aligns more closely with expert standards than existing methods and enables scalable automated assessment of e-commerce posters. The framework successfully addresses the gap in quality assessment for Chinese commercial content.

Conclusion: E-comIQ-ZH provides a comprehensive solution for evaluating Chinese e-commerce posters, addressing critical gaps in automated quality assessment. The release of datasets, models, and evaluation tools will support future research in this important area of generative AI for commercial design.
...
TL;DR: SF3D-RGB: A deep learning architecture for sparse scene flow estimation that fuses 2D monocular images and 3D point clouds to achieve better accuracy with fewer parameters than single-modality or other fusion methods.


<details>
  <summary>Details</summary>
Motivation: Scene flow estimation is crucial for dynamic scene perception, but existing learning-based methods focus on single modalities (either image-based or LiDAR-based), limiting robustness and accuracy.

Method: End-to-end architecture that encodes features from both 2D images and 3D point clouds, fuses them, uses graph matching for initial scene flow estimation, and refines with a residual scene flow module.
...
Conclusion: SF3D-RGB successfully demonstrates that multimodal fusion of 2D images and 3D point clouds leads to more robust and accurate scene flow estimation with efficient parameter usage.
...
TL;DR: U-Net architecture for brain tumor segmentation in MRI, focusing on non-enhancing tumor compartment often overlooked in recent challenges but important for survival prediction and tumor growth monitoring.


<details>
  <summary>Details</summary>
Motivation: Non-enhancing tumor compartment has been neglected in recent brain tumor segmentation challenges (like MICCAI), yet it's clinically important as it indicates patient survival time and areas of potential tumor growth, necessitating automated segmentation methods.

Method: U-Net based deep learning architecture designed specifically for segmenting brain tumors across various MRI modalities, with special emphasis on the non-enhancing tumor compartment.

Result: The paper presents a deep learning approach capable of automatically delineating the non-enhancing tumor extension within brain tumors from MRI data.

Conclusion: Automated segmentation of non-enhancing tumor compartments is essential for clinical applications despite being overlooked in recent challenges, and U-Net architecture provides an effective solution for this task.
...
TL;DR: Deep learning forest stand delineation works equally well with ALS or DAP canopy height models, and adding DTM doesn't improve performance, showing framework resilience to input data variations.


<details>
  <summary>Details</summary>
Motivation: Current forest stand delineation is manual and subjective. While deep learning with ALS data works, temporal misalignment limits scalability. Need to test if DAP-derived CHMs (better temporal alignment) can replace ALS-CHMs, and if DTM inclusion helps.

Method: Used U-Net semantic segmentation with municipality-level cross-validation across 6 Norwegian municipalities. Compared three data combinations: (1) aerial imagery + ALS-CHM, (2) aerial imagery + DAP-CHM, (3) aerial imagery + DAP-CHM + DTM. Used expert-delineated stands as reference.

Result: All three data combinations performed comparably (0.90-0.91 overall accuracy). Model predictions agreed more with each other than with reference data, showing both model consistency and delineation subjectivity. DAP-CHMs performed similarly despite reduced structural detail, and DTM provided no improvement.

Conclusion: Deep learning framework is resilient to input data variations. Large datasets for stand delineation can be assembled using temporally aligned ALS and DAP data, enabling operational scalability despite data source differences.
...
TL;DR: Proposes a three-stage encoder with hierarchical feature representation and bidirectional sequence modeling for more accurate and efficient tooth image segmentation, achieving 1.1% mIoU improvement on OralVision dataset.


<details>
  <summary>Details</summary>
Motivation: Traditional dental image segmentation suffers from discontinuous results and poor target-background discrimination due to insufficient environmental/global context modeling. Transformer-based approaches have quadratic computational complexity that makes them inefficient for high-resolution dental images.

Method: Three-stage encoder with hierarchical feature representation to capture scale-adaptive information, cross-scale feature fusion to combine low-level details with high-level semantics, and bidirectional sequence modeling strategy to enhance global spatial context understanding without high computational cost.

Result: Validated on two dental datasets, achieving 1.1% improvement in mean intersection over union (mIoU) on the OralVision dataset, demonstrating superiority over existing approaches.

Conclusion: The proposed method effectively addresses the limitations of traditional dental image segmentation by providing better contextual awareness while maintaining computational efficiency, leading to improved segmentation accuracy.
...
TL;DR: TranX-Adapter improves AIGI detection in MLLMs by addressing attention dilution through optimal-transport and cross-attention fusion of semantic and artifact features.


<details>
  <summary>Details</summary>
Motivation: AI-generated images (AIGI) threaten information integrity. While combining texture-level artifact features with semantic features in MLLMs helps detection, artifact features have high intra-feature similarity causing attention dilution that hinders effective feature fusion.

Method: Proposes TranX-Adapter with two components: 1) Task-aware Optimal-Transport Fusion using Jensen-Shannon divergence between artifact/semantic prediction probabilities as cost matrix to transfer artifact info to semantic features, and 2) X-Fusion using cross-attention to transfer semantic info to artifact features.

Result: Experiments on standard AIGI detection benchmarks show TranX-Adapter brings consistent and significant improvements of up to +6% accuracy across several advanced MLLMs.

Conclusion: The proposed lightweight fusion adapter effectively addresses attention dilution in multimodal feature fusion, significantly enhancing AIGI detection capability in MLLMs through bidirectional feature transfer.
...
TL;DR: SigVLP is a vision-language model for medical imaging that uses Rotary Position Embeddings to handle variable-sized CT volumes by treating them as sequences of 3D chunks, enabling chunkwise alignment with localized textual observations.


<details>
  <summary>Details</summary>
Motivation: Medical imaging datasets have highly variable resolution and slice thicknesses across different vendors/devices. Traditional approaches require cropping or interpolating to fixed sizes, causing information loss. There's a need for models that can handle variable input sizes while maintaining fine-grained alignment between imaging data and textual observations.

Method: Proposes SigVLP vision-language model with Rotary Position Embeddings applied directly within attention operations, allowing treatment of z-axis as unconstrained temporal dimension. Trains on CT volumes sampled in chunks paired with localized organ-wise textual observations (chunkwise alignment). Uses Muon optimizer for training.

Result: Model evaluated on diverse downstream tasks including zero-shot abnormality and organ classification, segmentation, and retrieval tasks. Chunkwise alignment provides finer-grained supervision, enabling stronger correlations between text and volume representations and improving text-to-volume alignment precision.

Conclusion: SigVLP's approach of using Rotary Position Embeddings and chunkwise alignment effectively handles variable-sized medical imaging data while establishing precise correlations between volumetric data and localized textual observations, overcoming limitations of traditional fixed-size approaches.
...
TL;DR: Proposes Structure-to-Image paradigm for colonoscopy depth estimation, using phase congruency and cross-level constraints to generate realistic synthetic images while preserving geometric structures and vascular details.


<details>
  <summary>Details</summary>
Motivation: Monocular depth estimation in colonoscopy suffers from domain gap between simulated and real images. Existing image translation methods cause structural distortions and specular highlights by failing to balance realism with structure consistency.

Method: Introduces Structure-to-Image paradigm that transforms depth maps from passive constraint to active generative foundation. Uses phase congruency for colonoscopic domain adaptation and designs cross-level structure constraint to co-optimize geometric structures and fine-grained details like vascular textures.

Result: In zero-shot evaluations on public phantom dataset, MDE model fine-tuned on generated data achieved maximum 44.18% reduction in RMSE compared to competing methods.

Conclusion: The Structure-to-Image approach effectively bridges domain gap for colonoscopy depth estimation by generating structurally consistent synthetic data while preserving important anatomical details.
...
TL;DR: Proposes Durian, a difficulty-aware group normalization method for multimodal RL that addresses instability from extreme reward samples by grouping samples based on perceptual complexity and reasoning uncertainty.


<details>
  <summary>Details</summary>
Motivation: Extending RLVR and GRPO methods to multimodal settings faces instability from std-based normalization, which is easily distorted by extreme reward samples. Multimodal models are particularly sensitive to such distortions due to both perceptual and reasoning errors influencing responses.

Method: Characterizes samples by difficulty using perceptual complexity (measured via visual entropy) and reasoning uncertainty (captured by model confidence). Proposes difficulty-aware group normalization (Durian) that re-groups samples by difficulty levels and shares the standard deviation within each group.
...
Conclusion: Durian effectively addresses the instability of std-based normalization in multimodal RL settings by incorporating difficulty-aware grouping, leading to improved performance on multimodal reasoning tasks.
...
TL;DR: LiREC-Net is a target-free, learning-based multi-sensor calibration network that jointly calibrates LiDAR, RGB, and event data in a unified framework, outperforming single-pair methods.


<details>
  <summary>Details</summary>
Motivation: Advanced autonomous systems need accurate multi-sensor fusion from natural driving scenes. Existing learning-based calibration methods only handle single sensor pairs, creating inefficiency for multi-modal systems.

Method: Proposes LiREC-Net with shared LiDAR representation leveraging 3D features and projected depth maps to reduce redundant computation and ensure consistency across LiDAR, RGB, and event modalities.

Result: Achieves competitive performance to bi-modal models on KITTI and DSEC datasets, establishing a new strong baseline for tri-modal sensor calibration.

Conclusion: LiREC-Net provides an efficient, unified framework for multi-sensor calibration that enables better sensor alignment for autonomous driving systems.
...
TL;DR: Hybrid parallelism framework combining condition-based data partitioning and adaptive pipeline switching for efficient conditional diffusion model inference with 2x+ latency reduction while preserving quality.


<details>
  <summary>Details</summary>
Motivation: Current diffusion acceleration methods based on distributed parallelism suffer from generation artifacts and fail to achieve substantial acceleration proportional to GPU count, creating a need for more efficient inference approaches.

Method: Hybrid parallelism framework with: (1) condition-based partitioning that leverages conditional and unconditional denoising paths as a new data-partitioning perspective, and (2) adaptive parallelism switching that enables optimal pipeline parallelism based on denoising discrepancy between paths.

Result: Achieves 2.31× latency reduction on SDXL and 2.07× on SD3 using two NVIDIA RTX 3090 GPUs while preserving image quality, outperforms existing methods in high-resolution synthesis acceleration, and demonstrates generality across U-Net-based diffusion and DiT-based flow-matching architectures.

Conclusion: The proposed hybrid parallelism framework effectively reduces inference latency in conditional diffusion models while maintaining generation quality, offering a practical solution for efficient diffusion model deployment across different architectures.
...
TL;DR: SAPNet++ addresses granularity ambiguity and boundary uncertainty in point-prompted instance segmentation by integrating semantic awareness, distance guidance, box mining, and multi-level affinity refinement.


<details>
  <summary>Details</summary>
Motivation: Single-point annotation reduces labeling costs but introduces challenges for high-precision tasks like instance segmentation. Point annotations cause granularity ambiguity (difficulty distinguishing object vs. parts) and boundary uncertainty (imprecise delineation). Existing methods relying on proposal selection with only category information fail to resolve granularity ambiguity, and mask generators provide limited discrete solutions that deviate from actual boundaries.

Method: Proposes SAPNet++ with three key components: 1) Point Distance Guidance and Box Mining Strategy to address granularity ambiguity at group and local levels, 2) S-MIL (enhanced multiple instance learning) with completeness scores for spatial granularity awareness in proposal selection, and 3) Multi-level Affinity Refinement that conveys pixel and semantic clues to narrow boundary uncertainty during mask refinement.
...
Conclusion: SAPNet++ successfully addresses the core challenges of point-prompted instance segmentation - granularity ambiguity and boundary uncertainty - through semantic-aware integration of distance guidance, enhanced MIL, and multi-level refinement, demonstrating potential to advance PPIS while maintaining the cost benefits of single-point annotation.
...
TL;DR: PhysicEdit: A physics-aware image editing framework that treats editing as predictive physical state transitions rather than discrete image mappings, achieving better physical realism through a dual-thinking mechanism and large-scale video supervision.


<details>
  <summary>Details</summary>
Motivation: Current instruction-based image editing models fail to produce physically plausible results when editing involves complex causal dynamics like refraction or material deformation. This is because they treat editing as discrete image-to-image mapping, which only provides boundary conditions without specifying transition dynamics.

Method: Reformulates physics-aware editing as predictive physical state transitions. Introduces PhysicTran38K dataset with 38K transition trajectories across 5 physical domains. Proposes PhysicEdit framework with textual-visual dual-thinking mechanism: frozen Qwen2.5-VL for physically grounded reasoning + learnable transition queries for timestep-adaptive visual guidance to diffusion backbone.

Result: PhysicEdit improves over Qwen-Image-Edit by 5.9% in physical realism and 10.1% in knowledge-grounded editing. Sets new state-of-the-art for open-source methods while remaining competitive with leading proprietary models.

Conclusion: Treating image editing as predictive physical state transitions rather than discrete mappings enables more physically plausible results, especially for complex causal dynamics. The proposed PhysicEdit framework demonstrates significant improvements in physical realism through its dual-thinking approach and large-scale video supervision.
...
TL;DR: XStreamVGGT introduces KV cache compression via pruning and quantization for memory-efficient streaming 3D reconstruction, reducing memory 4.42× and speeding inference 5.48× with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: StreamVGGT's unbounded KV cache growth from multi-image/long-video inputs causes excessive memory consumption and latency, limiting scalability for long-horizon streaming 3D applications.

Method: Tuning-free approach combining pruning (token-importance identification to fit fixed KV budget) and dimension-adaptive quantization of KV tensors, maintaining compatibility with high-performance attention kernels like FlashAttention.

Result: Achieves mostly negligible performance degradation while reducing memory usage by 4.42× and accelerating inference by 5.48× compared to baseline StreamVGGT.

Conclusion: XStreamVGGT enables practical and scalable streaming 3D applications through efficient KV cache compression, with code publicly available for implementation.
...
TL;DR: A learning-based motion segmentation method that bypasses explicit correspondence estimation and uses attention mechanisms to directly infer moving objects from latent features, achieving state-of-the-art performance with high efficiency.


<details>
  <summary>Details</summary>
Motivation: Conventional motion segmentation methods rely on noisy camera pose and point correspondence estimation, leading to cumulative errors in multi-stage pipelines with limited performance or high computational cost.

Method: A fully learning-based approach that directly infers moving objects from latent feature representations via attention mechanisms, bypassing explicit correspondence estimation. Leverages reliable camera poses and spatial-temporal priors from 4D scene geometry reconstruction.

Result: Extensive experiments demonstrate state-of-the-art motion segmentation performance with high efficiency by eliminating complex pre-processing and iterative refinement.

Conclusion: The proposed end-to-end feed-forward approach successfully bypasses explicit correspondence estimation, enabling robust motion segmentation through implicit disentanglement of object and camera motion.
...
TL;DR: SkyReels V4 is a unified multimodal video foundation model that jointly generates synchronized video and audio, supports inpainting/editing, handles rich multimodal inputs, and achieves cinema-quality 1080p, 32FPS, 15-second outputs through efficient joint low-res/high-res generation.


<details>
  <summary>Details</summary>
Motivation: To create a unified video foundation model that can simultaneously handle joint video-audio generation, inpainting, and editing with rich multimodal instructions, while achieving cinema-quality outputs at high resolutions and durations that are computationally feasible.

Method: Dual-stream Multimodal Diffusion Transformer (MMDiT) architecture with separate video and audio branches sharing a powerful MMLM text encoder; uses channel concatenation to unify inpainting/editing tasks; employs joint low-resolution full-sequence and high-resolution keyframe generation with super-resolution and frame interpolation for efficiency.

Result: First video foundation model supporting simultaneous multimodal input, joint video-audio generation, and unified generation/inpainting/editing; achieves 1080p resolution, 32 FPS, 15-second duration with synchronized audio; maintains strong efficiency and quality at cinematic scales.

Conclusion: SkyReels V4 represents a significant advancement as the first unified multimodal video foundation model capable of high-quality, cinema-level video-audio generation with comprehensive editing capabilities, making high-resolution, long-duration video synthesis computationally practical.
...
TL;DR: LGI maps encode light-aware occlusion from monocular depth, providing physics-inspired priors for joint shadow generation and relighting, outperforming prior methods in realism and consistency.


<details>
  <summary>Details</summary>
Motivation: Current generative models often produce floating shadows, inconsistent illumination, and implausible shadow geometry due to lack of physics-inspired priors tying illumination direction to geometry.

Method: Propose Light-Geometry Interaction (LGI) maps computed from 2.5D depth predictions, embedded into bridge-matching generative backbone for joint shadow generation and relighting, with curated large-scale benchmark dataset.

Result: Significant gains in realism and consistency across synthetic and real images, enabling physically consistent shadow generation and relighting without full 3D reconstruction.

Conclusion: LGI bridges geometry-inspired rendering with generative modeling, providing efficient, physically consistent approach for joint shadow and relighting tasks.
...
TL;DR: UniVBench is a comprehensive benchmark for evaluating video foundation models across four core abilities (understanding, generation, editing, reconstruction) using 200 high-quality, diverse multi-shot videos with detailed annotations and a unified evaluation system.


<details>
  <summary>Details</summary>
Motivation: Existing video model benchmarks are fragmented, limited to single tasks, use simple metrics, and don't capture the unified capabilities that video foundation models are designed to deliver. There's a need for a comprehensive evaluation framework that assesses integrated video intelligence.

Method: Created UniVBench with 200 human-created, high-quality multi-shot videos with detailed captions, editing instructions, and reference images. Developed UniV-Eval, a unified agentic evaluation system that standardizes prompting, instruction parsing, and scoring across all tasks for fair comparisons.

Result: Provides the first framework for measuring integrated capabilities of video foundation models through instruction-based multi-shot video tasks. Extensive human annotations ensure evaluation aligns with human judgment, enabling rigorous assessment of video intelligence.

Conclusion: UniVBench addresses the critical gap in evaluating video foundation models by providing a comprehensive, standardized benchmark that captures their unified capabilities, accelerating progress toward robust video intelligence through fair, scalable, and reproducible comparisons.
...
TL;DR: Proposes Meta-FC training strategy using meta-learning with feature consistency to improve robustness and generalization of deep watermarking models, addressing limitations of single random distortion training.


<details>
  <summary>Details</summary>
Motivation: Current deep watermarking methods use single random distortion (SRD) training which treats distortions independently, causing optimization conflicts and limiting robustness/generalization across different distortion types.

Method: Meta-learning approach with feature consistency (Meta-FC): samples multiple distortions for meta-training tasks, holds out one as "unknown" distortion for meta-testing, uses feature consistency loss to maintain decoded feature consistency across different distortions.

Result: Meta-FC improves robustness and generalization by average of 1.59%, 4.71%, and 2.38% under high-intensity, combined, and unknown distortions compared to SRD training across various watermarking models.

Conclusion: Meta-FC training strategy effectively addresses optimization conflicts in SRD training, enhancing watermarking model robustness and generalization through meta-learning and feature consistency constraints.
...
TL;DR: GFPL framework uses generative prototype learning to address class imbalance and communication overhead in federated learning, improving accuracy by 3.6% while maintaining low communication costs.
...
Method: Proposes Generative Federated Prototype Learning (GFPL) framework with: 1) GMM-based prototype generation capturing class-wise feature statistics, 2) Bhattacharyya distance-based prototype aggregation for semantic knowledge fusion, 3) pseudo-feature generation to mitigate distribution imbalance, and 4) dual-classifier architecture with hybrid Dot Regression and Cross-Entropy loss.

Result: Extensive experiments show GFPL improves model accuracy by 3.6% under imbalanced data settings while maintaining low communication cost compared to conventional approaches.

Conclusion: GFPL effectively addresses both knowledge fusion bias and communication overhead in federated learning through brain-inspired prototype learning, demonstrating superior performance in imbalanced data scenarios.
...
TL;DR: MemCoach: A training-free MLLM-based system that provides natural language feedback to improve image memorability at capture time, shifting from prediction to actionable guidance.


<details>
  <summary>Details</summary>
Motivation: Existing image memorability research focuses on passive prediction or generative alteration, but lacks support for users at capture time when they need actionable guidance to improve photo memorability.

Method: MemCoach uses Multimodal Large Language Models with a teacher-student steering strategy, aligning model activations toward more memorable patterns learned from a teacher model progressing along least-to-most memorable samples.

Result: MemCoach consistently outperforms zero-shot models on the new MemBench benchmark, demonstrating effective memorability improvement guidance through natural language suggestions.

Conclusion: Memorability can be taught and instructed, not just predicted, enabling a shift from passive prediction to actionable feedback for human creators at image capture time.
...
TL;DR: EndoDDC: A diffusion-based depth completion method for endoscopic surgery that combines images, sparse depth, and depth gradient features to address weak textures and lighting issues, outperforming SOTA models.


<details>
  <summary>Details</summary>
Motivation: Accurate depth estimation is critical for endoscopic surgical robot navigation, but existing self-supervised methods degrade in endoscopic environments with weak textures and variable lighting, leading to sparse reconstruction. Depth completion can help but hasn't been well explored in endoscopy.

Method: EndoDDC integrates images, sparse depth information with depth gradient features, and optimizes depth maps through a diffusion model to address weak texture and light reflection issues in endoscopic environments.

Result: Extensive experiments on two publicly available endoscopy datasets show that EndoDDC outperforms state-of-the-art models in both depth accuracy and robustness.

Conclusion: The method demonstrates potential to reduce visual errors in complex endoscopic environments and could improve 3D reconstruction and safe instrument guidance for surgical robots.
...
TL;DR: TIRAuxCloud: A multi-modal thermal infrared dataset with auxiliary layers for day/night cloud segmentation, addressing challenges in nighttime cloud detection due to limited spectral information and low TIR resolution.


<details>
  <summary>Details</summary>
Motivation: Clouds obstruct Earth observation critical for disaster response and environmental monitoring. While optical bands work for daytime, they fail at night when thermal infrared becomes essential. However, accurate nighttime cloud detection is challenging due to limited spectral information and low TIR resolution.

Method: Created TIRAuxCloud dataset combining multispectral data (TIR, optical, NIR) from Landsat and VIIRS with auxiliary layers including elevation, land cover, meteorological variables, and cloud-free references. Includes both automated cloud masks and manually annotated subsets.
...
Conclusion: TIRAuxCloud facilitates development of innovative methods for 24/7 cloud detection by providing multi-modal thermal spectral data with auxiliary information to reduce surface-cloud ambiguity and cloud formation uncertainty.
...
TL;DR: Geometry-aware GNN autodecoder for heterogeneous cryo-EM reconstruction that predicts atomic backbone conformations using protein-structure priors.


<details>
  <summary>Details</summary>
Motivation: To improve heterogeneous single-particle cryo-EM reconstruction by incorporating protein-structure priors and geometry-aware modeling for more accurate atomic backbone conformation prediction.

Method: Represent backbone as graph, use GNN autodecoder mapping per-image latent variables to 3D displacements of template conformation. Combines differentiable cryo-EM forward model with geometric regularization, supports unknown orientations via ellipsoidal support lifting pose estimation.

Result: On synthetic datasets from molecular dynamics trajectories, the GNN achieves higher accuracy compared to MLP of comparable size, demonstrating benefits of geometry-informed inductive bias.

Conclusion: Geometry-aware GNN approach with protein-structure priors improves cryo-EM reconstruction accuracy for heterogeneous particle analysis.
...
TL;DR: C²SSM introduces a cluster-serial scanning approach for UHD image restoration, replacing pixel-wise operations with semantic cluster processing to achieve linear complexity and state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing UHD image restoration models suffer from unsustainable computational costs due to pixel-wise operations, and while SSMs offer linear complexity, their pixel-serial scanning remains a bottleneck for millions of pixels in UHD content.
...
Result: Achieves decisive efficiency improvements by slashing computational costs while establishing new state-of-the-art results across five UHD restoration tasks.

Conclusion: C²SSM charts a new course for efficient large-scale vision by demonstrating that scanning clusters rather than pixels enables effective UHD image restoration with dramatically reduced computational requirements.
...
TL;DR: Scene-consistent video generation using "geometry-as-context" approach with autoregressive camera-controlled model that estimates geometry and restores novel views, outperforming previous methods.
...
Method: Introduces "geometry-as-context" approach using autoregressive camera-controlled video generation model that iteratively: (1) estimates geometry for 3D reconstruction, and (2) simulates/restores novel view images. Uses camera gated attention module to leverage camera poses effectively. During training, text contexts determine whether to generate geometric or RGB images, with geometry context randomly dropped from training sequences to enable RGB-only inference.

Result: Tested on scene video generation with one-direction and forth-and-back trajectories, showing superiority over previous approaches in maintaining scene consistency and camera control.

Conclusion: The geometry-as-context approach effectively overcomes limitations of previous methods by integrating geometry estimation and novel view restoration in a unified framework, achieving better scene consistency and camera control in generated videos.
...
TL;DR: D-ODR introduces directed ordinal diffusion regularization to model diabetic retinopathy progression as a directed flow, preventing biologically implausible reverse transitions in feature space.


<details>
  <summary>Details</summary>
Motivation: Existing ordinal regression methods treat DR severity as static symmetric ranks, ignoring the unidirectional nature of disease progression, which can lead to biologically implausible feature representations that allow reverse transitions or proximity between non-consecutive stages.
...
Result: Extensive experiments show D-ODR achieves superior grading performance compared to state-of-the-art ordinal regression and DR-specific grading methods, providing more clinically reliable disease severity assessment.

Conclusion: D-ODR effectively aligns feature representations with the natural trajectory of DR worsening by modeling disease progression as a directed flow, preventing biologically inconsistent reverse transitions and improving clinical reliability.
...
TL;DR: Lightweight MobileNetV3 with CORAL head achieves 0.9019 QWK for diabetic retinopathy severity classification, optimized for mobile deployment.


<details>
  <summary>Details</summary>
Motivation: Manual DR diagnosis is time-consuming and error-prone, causing screening delays. Need for automated, efficient system for resource-constrained environments.

Method: MobileNetV3 architecture with CORAL head for ordinal classification, trained on APTOS 2019 + IDRiD with preprocessing (circular cropping, illumination normalization), 3-fold cross-validation, model calibration, mobile optimization.

Result: Achieves Quadratic Weighted Kappa of 0.9019 and accuracy of 80.03%, demonstrating strong performance for DR severity assessment.

Conclusion: Proposed lightweight framework provides scalable, practical tool for early-stage diabetic retinopathy screening in resource-constrained settings.
...
TL;DR: MVGFDR: Multi-view graph fusion framework for diabetic retinopathy grading that disentangles shared and view-specific features using frequency-domain graph initialization and cross-view reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view DR grading methods overlook inter-view correlations and fail to exploit inherent consistency across views from the same patient, limiting their ability to fully leverage complementary information.

Method: End-to-end framework with Multi-View Graph Fusion module: 1) Graph initialization via residual-guided connections and DCT frequency anchors, 2) Graph fusion based on frequency-domain relevance, 3) Masked cross-view reconstruction for view-invariant learning.

Result: Superior performance on MFIDDR (largest multi-view fundus dataset) compared to state-of-the-art approaches, demonstrating effectiveness of explicit inter-view correlation modeling.

Conclusion: MVGFDR effectively captures complementary view-specific information while learning view-invariant representations, advancing multi-view learning for diabetic retinopathy grading through explicit graph-based feature disentanglement.
...
TL;DR: MindDriver is a progressive multimodal reasoning framework that enables Vision-Language Models to imitate human-like thinking for autonomous driving by bridging semantic understanding, imagination, and physical trajectory planning.


<details>
  <summary>Details</summary>
Motivation: Existing Chain-of-Thought approaches for VLMs in autonomous driving have limitations: textual CoT creates a gap between text semantics and physical trajectory space, while image-based CoT lacks planning-oriented guidance for accurate scene evolution.

Method: MindDriver uses a progressive multimodal reasoning framework with three stages: semantic understanding, semantic-to-physical space imagination, and physical-space trajectory planning. It includes a feedback-guided automatic data annotation pipeline for aligned multimodal reasoning data and progressive reinforcement fine-tuning with high-level reward-based learning.

Result: MindDriver demonstrates superior performance in both nuScenes open-loop and Bench2Drive closed-loop evaluations, showing improved reasoning capabilities for autonomous driving applications.

Conclusion: MindDriver successfully addresses the limitations of existing CoT approaches by providing a progressive multimodal reasoning framework that better aligns semantic understanding with physical trajectory planning for autonomous driving systems.
...
Method: GLoTran integrates a low-resolution global image with multi-scale region-level text image slices under an instruction-guided alignment strategy, conditioning MLLMs to maintain scene-level contextual consistency while capturing fine-grained textual details.

Result: Extensive experiments demonstrate that GLoTran substantially improves translation completeness and accuracy over state-of-the-art MLLMs, and the authors construct GLoD, a large-scale 510K dataset of high-resolution global-local image-text pairs.

Conclusion: GLoTran offers a new paradigm for fine-grained TIMT under high-resolution and text-rich conditions by addressing the limitations of existing methods through dual visual perception.
...
TL;DR: A method for globally consistent pose graph initialization in SfM using edge prioritization with GNN-based reliability prediction and connectivity-aware construction.


<details>
  <summary>Details</summary>
Motivation: Existing SfM methods use image retrieval to connect each image to k nearest neighbors, treating pairs independently and ignoring global consistency, which is problematic for sparse and ambiguous scenes.

Method: Three components: (1) GNN trained with SfM-derived supervision to predict globally consistent edge reliability, (2) multi-minimal-spanning-tree-based pose graph construction guided by edge ranks, (3) connectivity-aware score modulation to reinforce weak regions and reduce graph diameter.

Result: Produces more reliable and compact pose graphs, improving reconstruction accuracy in sparse and high-speed settings, outperforming SOTA retrieval methods on ambiguous scenes.

Conclusion: Globally informed initialization through edge prioritization yields better pose graphs than independent pairwise methods, with code and models publicly available.
...
TL;DR: MasqLoRA is a backdoor attack framework that uses LoRA modules to stealthily inject malicious behavior into text-to-image diffusion models, achieving 99.8% attack success rate with minimal training overhead.


<details>
  <summary>Details</summary>
Motivation: While LoRA's modular flexibility enables efficient fine-tuning and sharing of diffusion models, this same plug-and-play nature creates a new attack surface in the AI supply chain that hasn't been adequately addressed.

Method: Freezes base model parameters and trains only low-rank adapter weights using "trigger word-target image" pairs to create a standalone backdoor LoRA module that embeds hidden cross-modal mapping.

Result: Achieves 99.8% attack success rate with minimal resource overhead, demonstrating the attack's effectiveness while maintaining stealthiness (model behaves normally without trigger words).

Conclusion: MasqLoRA reveals a severe and unique threat in the AI supply chain, highlighting the urgent need for dedicated defense mechanisms for the LoRA-centric sharing ecosystem.
...
TL;DR: PanoEnv is a new VQA benchmark for 360° panoramic images with 14.8K questions and 3D annotations, revealing VLMs' poor 3D spatial reasoning (49% accuracy). A curriculum RL framework with geometry-aware rewards improves 7B model performance to SOTA levels.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models struggle with 3D spatial reasoning on 360° panoramic images due to geometric distortion in Equirectangular Projection and limited 3D supervision, hindering applications in VR, autonomous driving, and robotics.

Method: Created PanoEnv benchmark from synthetic 3D environments with 14.8K questions across 5 categories and 3D annotations. Proposed reinforcement learning post-training with Group Relative Policy Optimization using ground-truth-guided reward with 5 geometry-aware strategies, plus two-stage curriculum to prevent catastrophic forgetting.

Result: 14 SOTA VLMs achieved only 49.34% overall accuracy and 8.36% on open-ended questions. Their 7B model improved to 52.93% overall (+3.59%) and 14.83% on open-ended questions, achieving top semantic evaluation scores (Q-Score 6.24, P-Score 5.95) surpassing 32B models.

Conclusion: PanoEnv-QA benchmark and curriculum-based RL framework effectively instill 3D spatial intelligence in VLMs for omnidirectional perception, demonstrating significant improvements in 3D reasoning capabilities for panoramic images.
...
TL;DR: RobustVisRAG improves vision-based retrieval-augmented generation robustness against visual distortions using a causality-guided dual-path framework that separates semantics from degradations.


<details>
  <summary>Details</summary>
Motivation: Existing VisRAG models degrade when visual inputs have distortions (blur, noise, low light, shadow) because semantic and degradation factors become entangled in pretrained visual encoders, causing errors in both retrieval and generation.

Method: Introduces RobustVisRAG with a causality-guided dual-path framework: non-causal path captures degradation signals via unidirectional attention, and causal path learns purified semantics guided by these signals. Uses Non-Causal Distortion Modeling and Causal Semantic Alignment objectives to separate semantics from degradations.

Result: Improves retrieval by 7.35%, generation by 6.35%, and end-to-end performance by 12.40% on real-world degradations while maintaining comparable accuracy on clean inputs. Introduces Distortion-VisRAG benchmark with synthetic and real-world degraded documents across 7 domains with 12 synthetic and 5 real distortion types.

Conclusion: RobustVisRAG effectively addresses VisRAG robustness issues under challenging visual conditions by separating semantics from degradations through a causality-guided approach, enabling stable performance on distorted inputs while preserving efficiency and zero-shot generalization.
...
TL;DR: Olbedo is a large-scale aerial dataset for outdoor albedo-shading decomposition, enabling diffusion-based IID models to generalize from synthetic indoor to real outdoor scenes.
...
Method: Created Olbedo dataset with 5,664 UAV images across four landscape types with multi-view consistent annotations derived from an inverse-rendering refinement pipeline over multi-view stereo reconstructions and calibrated sky illumination.

Result: Fine-tuning on Olbedo significantly improves single-view outdoor albedo prediction on the MatrixCity benchmark and enables applications to multi-view consistent relighting, material editing, and scene change analysis.

Conclusion: Olbedo enables state-of-the-art diffusion-based IID models to generalize to real outdoor imagery and supports future research in outdoor intrinsic decomposition and illumination-aware aerial vision.
...
TL;DR: RT-RMOT: RGB-Thermal Referring Multi-Object Tracking using multimodal LLM with reinforcement learning optimization for all-day tracking.


<details>
  <summary>Details</summary>
Motivation: Existing referring multi-object tracking struggles in low-visibility conditions (nighttime, smoke). Need fusion of RGB appearance with thermal modality's illumination robustness for all-day tracking.

Method: Propose RTrack framework based on multimodal LLM integrating RGB, thermal, and textual features. Use Group Sequence Policy Optimization (GSPO) with Clipped Advantage Scaling (CAS) for stable RL fine-tuning. Design Structured Output Reward and Comprehensive Detection Reward for balanced exploration-exploitation.

Result: Created RefRT dataset (first RGB-Thermal RMOT dataset) with 388 descriptions, 1,250 tracked targets, 166,147 L-RGB-T triplets. Extensive experiments on RefRT demonstrate RTrack effectiveness.

Conclusion: RT-RMOT enables all-day referring multi-object tracking by fusing RGB and thermal modalities. RTrack framework with GSPO and CAS strategies effectively addresses low-visibility challenges in RMOT.
...
TL;DR: SPGen is a deep learning model that predicts eye movement sequences (scanpaths) when viewing paintings, using domain adaptation to bridge the gap between natural scenes and artworks.
...
Method: Uses a Fully Convolutional Neural Network with differentiable fixation selection, learnable Gaussian priors for natural viewing biases, unsupervised domain adaptation via gradient reversal layer to transfer knowledge from natural scenes to paintings, and random noise sampler for eyetracking stochasticity.

Result: Extensive testing shows SPGen outperforms existing methods in predicting scanpaths for paintings.

Conclusion: SPGen provides a powerful tool for analyzing gaze behavior, advancing both the preservation and appreciation of artistic treasures.
...
TL;DR: AutoSew is a geometry-based approach using Graph Neural Networks and optimal transport to automatically predict stitch correspondences from 2D sewing patterns, achieving 96% F1-score and assembling 73.3% of garments without error.
...
Method: Formulates stitch prediction as graph matching using Graph Neural Networks to capture geometric context, with differentiable optimal transport solver to infer stitching relationships including multi-edge connections. Updated GarmentCodeData dataset with realistic multi-edge annotations.
...
Conclusion: Geometry alone can robustly guide stitching prediction, enabling scalable garment assembly without manual input, demonstrating the effectiveness of geometry-based approaches for automated garment manufacturing.
...
TL;DR: AdaSpot: Adaptive region selection for precise event spotting, using low-res global features + high-res ROI processing to improve efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods process all frames uniformly, causing redundant computation on non-informative regions while losing fine details through spatial downsampling, limiting both efficiency and precision for fast-paced event localization.

Method: Process low-resolution videos for global features while adaptively selecting the most informative region-of-interest per frame for high-resolution processing, using an unsupervised task-aware strategy that maintains spatio-temporal consistency.

Result: State-of-the-art performance on PES benchmarks (+3.96 and +2.26 mAP@0 frames on Tennis and FineDiving), preserving fine details with marginal computational overhead vs low-res baselines while being far more efficient than uniform high-res processing.

Conclusion: AdaSpot effectively addresses spatio-temporal redundancy in video processing, achieving superior precision for event spotting with efficient computation by adaptively focusing on informative regions.
...
TL;DR: LFG: Label-free teacher-guided framework for autonomous driving representation learning from unposed YouTube videos, using multi-modal pseudo-supervision to learn unified pseudo-4D representations.


<details>
  <summary>Details</summary>
Motivation: Ego-centric driving videos are abundant online but lack annotations, making it difficult to learn representations that capture both semantic structure and 3D geometry. Current self-supervised approaches focus too much on frame-to-frame consistency while safe driving requires temporal context.

Method: Uses a feedforward architecture with lightweight autoregressive module, trained with multi-modal supervisory signals from teachers that provide sequence-level pseudo-supervision. Predicts current/future point maps, camera poses, semantic segmentation, and motion masks from raw unposed videos without labels or LiDAR.

Result: The encoder transfers effectively to downstream autonomous driving planning on NAVSIM benchmark, surpassing multi-camera and LiDAR baselines with only single monocular camera. Also yields strong performance on semantic, geometric, and motion prediction tasks.

Conclusion: LFG learns geometry and motion-aware features from unlabeled videos, positioning it as a compelling video-centric foundation model for autonomous driving that doesn't require expensive annotations or sensors.
...
TL;DR: CXR-LT 2026 challenge addresses long-tailed pathology distribution and open-world clinical environments using multi-center dataset with 145K+ images, featuring robust multi-label classification and open-world generalization tasks.


<details>
  <summary>Details</summary>
Motivation: Chest X-ray interpretation is hindered by long-tailed distribution of pathologies and open-world nature of clinical environments. Existing benchmarks rely on closed-set classes from single institutions, failing to capture rare diseases or novel findings.

Method: Created CXR-LT 2026 challenge with multi-center dataset (PadChest + NIH Chest X-ray, 145K+ images). Two tasks: 1) Robust Multi-Label Classification on 30 known classes, 2) Open-World Generalization to 6 unseen rare disease classes. Evaluated using mAP, AUROC, and F1-score.

Result: Winning solutions achieved mAP of 0.5854 on Task 1 and 0.4315 on Task 2. Large-scale vision-language pre-training significantly mitigates performance drop typically associated with zero-shot diagnosis.

Conclusion: The CXR-LT 2026 benchmark successfully addresses real-world challenges in chest X-ray interpretation, demonstrating that advanced pre-training approaches can handle both common and rare pathologies in open-world clinical settings.
...
TL;DR: WeatherCity: A framework for 4D urban scene reconstruction with flexible weather editing using Gaussian representations and physics-driven effects.


<details>
  <summary>Details</summary>
Motivation: Existing 4D scene reconstruction methods lack weather simulation capabilities, while image-level weather editing introduces artifacts and poor controllability, limiting their use in autonomous driving training and simulation.

Method: Combines text-guided image editing for weather backgrounds, introduces weather Gaussian representation with shared scene features and weather-specific decoders, adds content consistency optimization, and implements physics-driven model for dynamic weather effects using particles and motion patterns.

Result: Achieves flexible controllability, high fidelity, and temporal consistency in 4D reconstruction and weather editing across multiple datasets and scenes, supporting fine-grained weather control and object-level manipulation.

Conclusion: WeatherCity enables high-quality 4D urban scene reconstruction with diverse weather simulation capabilities, addressing limitations of existing methods for autonomous driving applications.
...
TL;DR: Brain3D is a 3D vision-language model for automated radiology report generation from brain tumor MRI, outperforming 2D slice-based approaches by preserving spatial context crucial for neuroradiology.


<details>
  <summary>Details</summary>
Motivation: Current medical VLMs use 2D slice approximations that fragment spatial context needed for accurate neuroradiological interpretation, especially for brain tumor analysis where hemispheric laterality, tumor infiltration patterns, and anatomical localization are critical.

Method: A staged vision-language framework that inflates a pretrained 2D medical encoder into native 3D architecture, then aligns it with a causal language model through three stages: contrastive grounding, supervised projector warmup, and LoRA-based linguistic specialization.

Result: Achieves Clinical Pathology F1 of 0.951 vs 0.413 for strong 2D baseline on 468 subjects (BraTS pathological cases plus healthy controls), while maintaining perfect specificity on healthy scans.

Conclusion: Brain3D demonstrates superior performance for automated neuroradiology report generation by preserving 3D spatial context, with staged alignment proving essential for establishing visual-textual correspondence, stabilizing conditioning, and producing structured clinical reports.
...
TL;DR: GeoDiv is a framework using LLMs and VLMs to measure geographical diversity in text-to-image models along socio-economic (SEVI) and visual diversity (VDI) axes, revealing systematic biases where models disproportionately depict certain countries as impoverished and lacking diversity.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models lack geographical diversity, reinforce stereotypes, and misrepresent regions, but existing diversity metrics are limited by curated datasets or focus only on visual similarity without interpretability.

Method: GeoDiv framework leverages large language models and vision-language models to assess geographical diversity through two indices: Socio-Economic Visual Index (SEVI) capturing economic/condition cues, and Visual Diversity Index (VDI) measuring variation in primary entities and backgrounds.

Result: Applied to Stable Diffusion and FLUX.1-dev across 10 entities and 16 countries, GeoDiv reveals consistent lack of diversity and identifies fine-grained biases - countries like India, Nigeria, and Colombia are disproportionately depicted as impoverished and worn.
...
TL;DR: WeaveTime addresses Video-LLMs' time-agnosticism in streaming settings by teaching temporal order perception and implementing dynamic focus caching for efficient online video understanding.


<details>
  <summary>Details</summary>
Motivation: Current Video-LLMs suffer from Time-Agnosticism, treating videos as unordered evidence rather than causally ordered sequences, leading to temporal order ambiguity and past-current focus blindness in streaming scenarios where frames arrive sequentially.

Method: WeaveTime uses a lightweight Temporal Reconstruction objective (Streaming Order Perception) to instill order-aware representations with minimal finetuning, plus a Past-Current Dynamic Focus Cache that performs uncertainty-triggered, coarse-to-fine retrieval to expand history only when needed.

Result: WeaveTime delivers consistent gains on representative streaming benchmarks, improving accuracy while reducing latency when plugged into existing Video-LLMs without architectural changes.

Conclusion: WeaveTime establishes a practical path toward time-aware streaming Video-LLMs under strict online, time-causal constraints, offering an efficient, model-agnostic framework for temporal understanding in sequential video streams.
...
TL;DR: MedTri is a structured text normalization framework for medical vision-language pretraining that converts free-text reports into standardized [Anatomical Entity: Radiologic Description + Diagnosis Category] triplets, improving pretraining quality and enabling modular text augmentation.


<details>
  <summary>Details</summary>
Motivation: Raw medical reports have substantial stylistic heterogeneity, variable length, and image-irrelevant content, but existing text normalization approaches lack systematic examination of their design principles and impact on vision-language pretraining.

Method: MedTri normalizes free-text medical reports into a unified [Anatomical Entity: Radiologic Description + Diagnosis Category] triplet structure that preserves essential morphological and spatial information while removing stylistic noise and irrelevant content. The framework also supports modular text-level augmentation strategies including knowledge enrichment and anatomy-grounded counterfactual supervision.

Result: Across multiple X-ray and CT datasets, structured anatomy-grounded text normalization consistently improves medical vision-language pretraining quality over raw reports and existing normalization baselines. The framework enables complementary gains in robustness and generalization through text augmentation without altering the core normalization process.

Conclusion: Structured text normalization is a critical and generalizable preprocessing component for medical vision-language learning, and MedTri provides a deployable normalization platform that enables consistent, image-grounded textual supervision at scale.
...
TL;DR: CoLoGen is a unified diffusion framework that addresses concept-localization representational conflict in conditional image generation by progressively learning and reconciling this duality through a staged curriculum and Progressive Representation Weaving module.
...
Method: CoLoGen uses a staged curriculum: 1) builds core conceptual and localization abilities, 2) adapts them to diverse visual conditions, 3) refines their synergy for complex instruction-driven tasks. The Progressive Representation Weaving (PRW) module dynamically routes features to specialized experts and integrates their outputs across stages.
...
TL;DR: CASR: A cyclic super-resolution framework using single model for arbitrary-scale SR by reformulating ultra-magnification as in-distribution scale transitions, addressing cross-scale distribution shift.


<details>
  <summary>Details</summary>
Motivation: Arbitrary-Scale Super-Resolution (ASISR) suffers from cross-scale distribution shift where performance degrades sharply when inference scale leaves training range, causing noise, blur, and artifacts accumulation.

Method: Proposes CASR, a cyclic SR framework that treats ultra-magnification as sequence of in-distribution scale transitions. Uses SDAM module for structural distribution alignment via superpixel aggregation to prevent error accumulation, and SARM module for high-frequency texture restoration using autocorrelation and LR self-similarity priors.

Result: Significantly reduces distribution drift, preserves long-range texture consistency, achieves superior generalization even at extreme magnification, all with only a single model.

Conclusion: CASR provides an efficient solution to arbitrary-scale SR by addressing cross-scale distribution shift through cyclic framework and specialized modules, enabling stable inference at arbitrary scales with single model.
...
TL;DR: Proposes a region-level mixing encoder that fuses multi-resolution tile representations from foundation models to capture histologic features requiring different magnifications, reducing tile count while improving biomarker prediction.


<details>
  <summary>Details</summary>
Motivation: Current computational pathology workflows use single-magnification (20×) foundation models, but histologic features require multi-resolution analysis. Large numbers of tiles per slide are computationally expensive, and important context is lost with single magnification.

Method: Region-level mixing encoder that jointly fuses image tile representations from mixed-magnification foundation models using masked embedding modeling pretraining. Explores design space for pretraining mixed-magnification region aggregators.

Result: Shows cancer-dependent improvements in predictive performance for biomarker prediction tasks across various cancer types, highlighting the importance of spatial context and multi-resolution understanding.

Conclusion: Multi-resolution analysis through region-level mixing encoders improves biomarker prediction by capturing features requiring different magnifications, addressing limitations of single-magnification foundation models in computational pathology.
...
TL;DR: Solaris is a multiplayer video world model that simulates consistent multi-view observations, addressing the limitation of existing single-agent video generation models in capturing multi-agent interactions.


<details>
  <summary>Details</summary>
Motivation: Existing action-conditioned video generation models (video world models) are limited to single-agent perspectives and fail to capture the multi-agent interactions present in real-world environments.

Method: Developed a multiplayer data system for automated data collection on games like Minecraft, collected 12.64M multiplayer frames, and trained Solaris using a staged pipeline with bidirectional, causal, and Self Forcing training, including Checkpointed Self Forcing for memory efficiency.

Result: Solaris outperforms existing baselines and enables evaluation of multiplayer movement, memory, grounding, building, and view consistency through a proposed evaluation framework.

Conclusion: Solaris lays the groundwork for a new generation of multi-agent world models by addressing the multi-view consistency challenge in multiplayer environments.
...
Method: WHOLE learns a generative prior over hand-object motion to jointly reason about interactions, then guides this prior at test time to generate trajectories that conform to video observations.

Result: WHOLE achieves state-of-the-art performance on hand motion estimation, 6D object pose estimation, and their relative interaction reconstruction, substantially outperforming separate processing approaches.

Conclusion: Holistic joint reconstruction using generative priors is superior to independent hand/object processing for egocentric manipulation videos, enabling consistent hand-object relations and handling out-of-sight cases.
...
TL;DR: Neu-PiG: Fast deformation optimization for dynamic 3D surface reconstruction using preconditioned latent-grid encoding, achieving drift-free results 60x faster than training-free methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for temporally consistent surface reconstruction of dynamic 3D objects from unstructured point clouds either suffer from drift and slow optimization (incremental methods) or require complex category-specific training (learned models).

Method: Uses novel preconditioned latent-grid encoding that distributes spatial features based on keyframe surface position and normal direction. Encodes entire deformations across all time steps into multi-resolution latent grid, augmented for time modulation and decoded into per-frame 6-DoF deformations via lightweight MLP. Employs Sobolev preconditioning during gradient-based training.

Result: Outperforms state-of-the-art approaches across diverse human and animal datasets, offering superior accuracy and scalability to long sequences. Runs at least 60x faster than existing training-free methods and achieves inference speeds comparable to heavy pretrained models.

Conclusion: Neu-PiG provides a fast, drift-free solution for dynamic 3D surface reconstruction that avoids explicit correspondences or additional priors, making it practical for long sequences while maintaining high fidelity.
...
<details>
  <summary>Details</summary>
Motivation: Soft set theory provides a framework for parameterized decision modeling with uncertainty, and has expanded into numerous variants with connections to diverse mathematical areas

Method: Survey-style overview covering core definitions, representative constructions, and key development directions of soft sets and their major extensions

Result: Comprehensive presentation of soft set theory including hypersoft sets, superhypersoft sets, TreeSoft sets, bipolar soft sets, dynamic soft sets, and connections to topology and matroid theory

Conclusion: The book provides a systematic survey of soft set theory's evolution, extensions, and current research directions in uncertainty modeling
...
TL;DR: PANGAEA-GPT: A hierarchical multi-agent framework for autonomous Earth science data discovery and analysis in large repositories like PANGAEA, addressing data underutilization through coordinated agent workflows.
...
Method: PANGAEA-GPT uses a hierarchical multi-agent framework with Supervisor-Worker topology, featuring data-type-aware routing, sandboxed deterministic code execution, and self-correction via execution feedback to diagnose and resolve runtime errors.

Result: The system demonstrates capacity to execute complex, multi-step workflows with minimal human intervention across use-case scenarios in physical oceanography and ecology.

Conclusion: The framework provides a methodology for querying and analyzing heterogeneous repository data through coordinated agent workflows, addressing the scalability challenge of Earth science data accumulation.
...
TL;DR: SemSIEdit is an inference-time framework that uses an agentic "Editor" to iteratively critique and rewrite sensitive spans in LLM outputs, reducing Semantic Sensitive Information leakage by 34.6% with only 9.8% utility loss.


<details>
  <summary>Details</summary>
Motivation: LLMs pose new threats through Semantic Sensitive Information (SemSI) - inferring sensitive identity attributes, generating reputation-harmful content, or hallucinating wrong information. Current defenses for structured PII are mature, but handling these complex, context-dependent sensitive information leaks while preserving utility remains an open challenge.

Method: SemSIEdit framework with an agentic "Editor" that iteratively critiques and rewrites sensitive spans during inference time. Instead of simply refusing to answer, it preserves narrative flow by rewriting problematic content while maintaining utility.

Result: Achieves Privacy-Utility Pareto Frontier: reduces SemSI leakage by 34.6% across all three categories (identity inference, reputation-harmful content, hallucinations) with only 9.8% utility loss. Reveals Scale-Dependent Safety Divergence: large models use constructive expansion (adding nuance), while capacity-constrained models use destructive truncation (deleting text). Identifies Reasoning Paradox: inference-time reasoning both increases baseline risk and empowers defense.

Conclusion: Agentic rewriting at inference time can effectively mitigate Semantic Sensitive Information leaks in LLMs while preserving utility, revealing important patterns in how different model scales approach safety and the dual nature of reasoning for both risk and defense.
...
TL;DR: ARLArena provides stable training recipe and analysis framework for agentic RL, decomposing policy gradient into 4 dimensions to identify instability sources, leading to SAMPO method for stable agentic policy optimization.


<details>
  <summary>Details</summary>
Motivation: Agentic reinforcement learning (ARL) shows promise for complex multi-step tasks but suffers from high instability and training collapse, limiting scalability and systematic exploration of algorithmic design choices.

Method: Proposes ARLArena: 1) clean standardized testbed, 2) decomposes policy gradient into 4 core design dimensions for systematic analysis, 3) distills unified perspective leading to SAMPO method that mitigates dominant instability sources.
...
Conclusion: Study provides unifying policy gradient perspective for ARL and offers practical framework for building stable, reproducible agent training systems through systematic analysis and targeted optimization methods.
...
TL;DR: Aggregation of multiple AI models can expand the set of achievable outputs beyond what's possible with a single model, through three specific mechanisms: feasibility expansion, support expansion, and binding set contraction.


<details>
  <summary>Details</summary>
Motivation: Compound AI systems often query multiple copies of the same model and aggregate responses, but it's unclear whether this aggregation actually expands the achievable output space beyond what a single model can produce.

Method: The paper uses a principal-agent framework to analyze how system designers can steer agent outputs through reward functions, while accounting for prompt engineering limitations and model capabilities. The analysis identifies three mechanisms through which aggregation expands elicitable outputs.

Result: The analysis proves that any aggregation operation must implement one of three mechanisms (feasibility expansion, support expansion, or binding set contraction) to be elicitability-expanding. Strengthened versions of these mechanisms provide necessary and sufficient conditions for elicitability-expansion.

Conclusion: Aggregation in compound AI systems can overcome limitations in both model capabilities and prompt engineering through specific mechanisms, providing a theoretical foundation for understanding when and how multi-model systems outperform single models.
...
TL;DR: The ASIR Courage Model is a phase-dynamic framework that treats truth-disclosure as a state transition (from suppression to expression) rather than a personality trait, applicable to both humans and AI systems under constraints.
...
Method: Develops a phase-dynamic framework with mathematical inequality (lambda(1+gamma)+psi > theta+phi) representing facilitative vs inhibitory forces, with feedback extension for recursive parameter recalibration and path dependence across interactions.

Result: Creates a formal model that characterizes truth-disclosure as geometric consequences of interacting forces within constrained phase space, applicable to both human truth-telling under asymmetric stakes and AI systems under policy constraints.

Conclusion: The ASIR Courage Model offers a unified dynamical perspective on truth-disclosure across human and artificial systems, reframing courage and alignment as emergent properties of interacting forces rather than intentional traits.
...
TL;DR: Extended fuzzy Ethical Decision-Making framework (fEDM+) adds explainability modules and pluralistic validation to address principled explainability and ethical pluralism challenges.


<details>
  <summary>Details</summary>
Motivation: The original fEDM framework lacked principled explainability of decisions and robustness under ethical pluralism, limiting its practical application for ethically sensitive AI systems.

Method: Two major extensions: 1) Explainability and Traceability Module (ETM) that links ethical rules to moral principles and computes principle-contribution profiles; 2) Pluralistic semantic validation framework replacing single-referent validation with multiple stakeholder referents.

Result: The extended fEDM+ framework preserves formal verifiability while achieving enhanced interpretability and stakeholder-aware validation, making it suitable as an oversight layer for ethically sensitive AI systems.

Conclusion: fEDM+ addresses critical limitations of the original model by enabling transparent explanations and formally representing principled disagreement, increasing robustness and contextual sensitivity for ethical AI governance.
...
TL;DR: STAR reasoning framework boosts car wash problem accuracy from 0% to 85%, with full context reaching 100% success.
...
Method: Variable isolation study (n=20 per condition, 6 conditions, 120 trials) using Claude 3.5 Sonnet with controlled hyperparameters, testing different prompt architecture layers including STAR framework, user profile context via vector DB, and RAG context.

Result: STAR framework alone raised accuracy from 0% to 85% (p=0.001, odds ratio 13.22). Adding user profile context provided 10% gain, and RAG context added 5%, achieving 100% accuracy in full-stack condition.
...
TL;DR: RL approach using GRPO jointly optimizes claim decomposition quality and verification alignment, achieving state-of-the-art verification performance with smaller models.
...
Method: Reinforcement learning with Group Relative Policy Optimization (GRPO) integrating structured sequential reasoning, supervised finetuning on teacher-distilled exemplars, and multi-objective reward balancing format compliance, verifier alignment, and decomposition quality.

Result: 8B decomposer achieves 71.75% macro-F1 across six evaluation settings, outperforming prompt-based approaches (+1.99, +6.24) and existing RL methods (+5.84). Human evaluation confirms high-quality subclaims.
...
TL;DR: ProactiveMobile is a new benchmark for evaluating proactive intelligence in mobile agents, featuring 3,660 instances across 14 real-world scenarios with 63 API functions, showing current MLLMs lack proactivity but it's learnable.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs are mostly reactive (executing explicit commands), but proactive intelligence (anticipating needs and initiating actions) is the next frontier for mobile agents. Development is bottlenecked by lack of benchmarks that address real-world complexity and enable objective, executable evaluation.

Method: Created ProactiveMobile benchmark that formalizes proactive tasks as inferring latent user intent across four dimensions of on-device contextual signals and generating executable function sequences from 63 APIs. Features 3,660 instances across 14 scenarios with multi-answer annotations. Team of 30 experts conducted final audit for factual accuracy, logical consistency, and action feasibility.

Result: Fine-tuned Qwen2.5-VL-7B-Instruct achieved 19.15% success rate, outperforming o1 (15.71%) and GPT-5 (7.39%). Shows proactivity is widely lacking in current MLLMs but is learnable.

Conclusion: ProactiveMobile addresses the critical bottleneck in proactive intelligence research by providing a comprehensive benchmark for evaluation. The results demonstrate that proactivity is a learnable but currently lacking capability in MLLMs, emphasizing the importance of the proposed benchmark for advancing proactive mobile agents.
...
TL;DR: AI decision support can worsen outcomes when users have misaligned prior beliefs, requiring better model documentation and training.


<details>
  <summary>Details</summary>
Motivation: Despite widespread AI adoption in decision support, we lack understanding of its effects on human decision making and outcomes.

Method: Developed 2-Step Agent framework using Bayesian causal inference to model how AI predictions affect agent beliefs and subsequent decisions/outcomes.

Result: Simulations show a single misaligned prior belief can cause AI decision support to produce worse outcomes than no support at all.

Conclusion: AI decision support has potential pitfalls; requires thorough model documentation and proper user training to avoid negative outcomes.
...
TL;DR: SPG-LLM uses LLMs to analyze PDDL files and identify irrelevant objects/actions/predicates before grounding, dramatically speeding up classical planning grounding.


<details>
  <summary>Details</summary>
Motivation: Grounding in classical planning suffers from exponential growth in grounded actions/atoms as task size increases, creating computational bottlenecks. Existing partial grounding approaches don't leverage textual and structural cues in PDDL descriptions.

Method: Uses LLMs to analyze domain and problem PDDL files to heuristically identify potentially irrelevant objects, actions, and predicates before grounding, significantly reducing the size of the grounded task.
...
Conclusion: LLM-based analysis of PDDL descriptions can effectively identify irrelevant components for grounding, dramatically improving grounding efficiency in classical planning without sacrificing plan quality.
...
TL;DR: LLMs show inconsistent biases: they rate human experts as more trustworthy but disproportionately choose algorithms in incentivized decisions, even when algorithms perform worse.


<details>
  <summary>Details</summary>
Motivation: To investigate how LLMs weigh information from different sources (human experts vs. algorithmic agents) and whether they exhibit algorithm aversion bias similar to humans, which is important for understanding LLM decision-making in high-stakes scenarios.
...
Result: LLMs gave higher trust ratings to human experts (matching human bias), but in incentivized betting tasks, they disproportionately chose algorithms even when algorithms performed demonstrably worse, showing inconsistent biases.

Conclusion: LLMs encode inconsistent biases toward humans and algorithms that vary by task presentation format, highlighting important considerations for AI safety and evaluation robustness in high-stakes deployments.
...
<details>
  <summary>Details</summary>
Motivation: Plans often change due to situational changes or new understanding, and sometimes feasible plans don't exist. Traditional planning approaches focus on efficient one-shot planning in feasible cases rather than handling domain updates or detecting infeasibility.

Method: Proposes a Petri net reachability relaxation approach for robust invariant synthesis, goal-unreachability detection, and infeasibility explanations. Uses incremental constraint solvers to support goal and constraint updates.
...
Conclusion: The proposed approach effectively addresses the limitations of traditional planning by enabling robust invariant synthesis, efficient infeasibility detection, and better handling of plan updates.
...
TL;DR: HUAUVs use DRL+Transfer Learning for cross-domain persistent monitoring with unified Lidar/Sonar policy


<details>
  <summary>Details</summary>
Motivation: HUAUVs enable operations in both aerial and underwater environments for inspection, mapping, search and rescue, but face challenges due to distinct domain dynamics and constraints

Method: Combines Deep Reinforcement Learning and Transfer Learning with shared DRL architecture trained on Lidar data (air) and Sonar data (underwater) for unified cross-domain policy
...
<details>
  <summary>Details</summary>
Motivation: Urban navigation suffers from GNSS signal blockages, and existing methods like non-holonomic constraints impose rigid equality assumptions that fail under dynamic driving conditions when aiding is most needed.
...
Result: Tested on 4 urban datasets (4.3 hours total): Under full GNSS, reduces vertical position error by 16.7% and improves altitude accuracy by 50.1%; under GNSS-denied conditions, reduces vertical drift by 24.2% and improves altitude accuracy by 20.2%.

Conclusion: Replacing hard motion equality assumptions with physically motivated inequality bounds is a practical, cost-free strategy for improving navigation resilience, continuity, and drift robustness without additional sensors, maps, or learned models.
...
TL;DR: A Task-Level Iterative Learning Control method enables robots to dynamically manipulate various ropes to tie knots using minimal human demonstration and no simulation, achieving 100% success within 10 trials.


<details>
  <summary>Details</summary>
Motivation: Dynamic manipulation of deformable objects like ropes is challenging due to infinite degrees of freedom and underactuated dynamics, requiring methods that work directly on hardware without extensive demonstrations or simulation.

Method: Task-Level Iterative Learning Control that uses a single human demonstration and simplified rope model to learn directly on hardware, constructing local inverse models via quadratic programming to propagate task-space errors into action updates.

Result: Achieved 100% success rate within 10 trials across 7 different rope types (chain, latex surgical tubing, braided/twisted ropes, 7-25mm thickness, 0.013-0.5 kg/m density), with transfer learning between rope types in 2-5 trials.

Conclusion: The method enables robust dynamic manipulation of diverse deformable objects with minimal prior knowledge, demonstrating effective hardware learning and transfer capabilities for real-world applications.
...
TL;DR: CUSP: A unified complementarity-based framework for contact modeling and planning in soft robots that addresses numerical challenges through conditioning techniques and enables dynamic trajectory optimization.


<details>
  <summary>Details</summary>
Motivation: Soft robots need safe, adaptive interaction with environments through contact, but modeling and planning contact-rich interactions is challenging due to redundant constraints, rank-deficient LCPs, and ill-conditioning from stiffness/friction disparity.

Method: Developed a robust LCP model for discretized soft robots with a three-stage conditioning pipeline: inertial rank selection to remove redundant contacts, Ruiz equilibration to correct scale disparity, and Tikhonov regularization on normal blocks. Also introduced kinematically guided warm-start strategy for MPCC-based dynamic trajectory optimization.

Result: Demonstrated effectiveness on contact-rich ball manipulation tasks, providing a physically consistent formulation that unifies contact modeling, manipulation, and planning.

Conclusion: CUSP provides a new foundation for unifying contact modeling, simulation, and planning in soft robotics, addressing fundamental numerical challenges in contact-rich interactions.
...
TL;DR: CableRobotGraphSim: A GNN-based simulator for cable-driven robots that uses graph representations for fast, accurate simulation with partial observability, combined with sim-and-real co-training and MPPI control.


<details>
  <summary>Details</summary>
Motivation: Traditional physics-based simulators require full-state observability or parameter search for system identification, limiting their applicability for cable-driven robots where partial observability is common.

Method: Represents cable-driven robots as graphs (rigid-bodies as nodes, cables/contacts as edges) using Graph Neural Networks, with sim-and-real co-training for generalization and robustness to noisy real data.

Result: The model can quickly and accurately match properties of other simulation models and real robots while ingesting only partially observable inputs, and integrates with MPPI controller for closed-loop navigation.

Conclusion: CableRobotGraphSim addresses limitations of traditional simulators for cable-driven robots through graph-based representation and co-training, enabling fast, accurate simulation with partial observability for practical robotic applications.
...
TL;DR: LACE: Learning-based framework for modeling temporal dynamics of GNSS measurement covariance with stability guarantees, improving localization in autonomous racing.
...
Method: Models covariance evolution as exponentially stable dynamical system using DNN with attention mechanism to predict process noise from environmental features, with contraction-based stability and spectral constraints for formal guarantees.

Result: Validated on AV-24 autonomous racecar, demonstrating improved localization performance and smoother covariance estimates in challenging GNSS-degraded environments.

Conclusion: Shows promise for dynamically modeling perceived uncertainty in state estimation problems tightly coupled with control sensitivity.
...
TL;DR: A sea turtle-inspired autonomous underwater robot combines bioinspired locomotion with field-ready autonomy for close-range monitoring in marine ecosystems, demonstrating successful obstacle avoidance and animal tracking in real aquarium environments.


<details>
  <summary>Details</summary>
Motivation: Current autonomous underwater vehicles struggle with close-range operation in cluttered reef habitats due to challenges with fragile structures, currents, variable illumination, and limited sensing. Previous bioinspired platforms lack deployable autonomy for practical field applications.

Method: Developed a sea turtle-inspired autonomous underwater robot with a tightly integrated, vision-driven control stack combining depth-heading stabilization, obstacle avoidance, and target-centric control. The system uses novel hardware and low-compute onboard tracking for field deployment.

Result: Validated in pool experiments and live coral reef exhibit at New England Aquarium, achieving 91% success rate in obstacle avoidance and reliable tracking of fast-moving marine animals and human divers. First integrated biomimetic system to track real marine animals in natural environment.

Conclusion: The work establishes a practical route toward soft-rigid hybrid, bioinspired underwater robots capable of minimally disruptive exploration and close-range monitoring in sensitive marine ecosystems, bridging the gap between bioinspired locomotion and field-ready autonomy.
...
TL;DR: Wearable HAR system performs real-time inference directly inside shank-mounted IMU using embedded Machine Learning Core, enabling low-latency exoskeleton control with minimal microcontroller involvement.
...
Method: Uses STMicroelectronics LSM6DSV16X IMU with embedded Machine Learning Core to execute activity recognition at sensor level. A lightweight decision-tree model was configured and deployed for on-sensor execution using ST MEMS Studio, focusing on three locomotion modes: stance, level walking, and stair ascent.

Result: The system reduces computation and communication overhead while preserving battery energy and improving robustness in distinguishing level walking from stair ascent for torque-assist control. The interrupt-driven architecture allows the microcontroller to remain in low-power state.

Conclusion: On-sensor inference architecture enables efficient, low-latency human activity recognition for exoskeleton control by moving computation to the sensor level, reducing system complexity and power consumption while maintaining classification accuracy.
...
TL;DR: AutoHorizon: A test-time method that dynamically estimates optimal execution horizons for action chunks in flow-based VLA models to adapt to changing perceptual conditions.


<details>
  <summary>Details</summary>
Motivation: Action chunking is standard in flow-based VLA models, but the execution horizon (number of actions executed per chunk) is underexplored. Performance varies significantly with different horizons, initially improving then declining as horizon increases.

Method: Analyzed cross- and self-attention weights in flow-based VLAs, discovered: (1) intra-chunk actions attend invariantly to vision-language tokens, limiting adaptability; (2) initial and terminal action tokens serve as stable anchors. Used action self-attention weights as proxy for model's predictive limit to develop AutoHorizon - a test-time method that dynamically estimates execution horizon for each predicted action chunk.

Result: AutoHorizon is performant across simulated and real-world robotic manipulation tasks, incurs negligible computational overhead, and generalizes across diverse tasks and flow-based models.

Conclusion: The execution horizon significantly impacts VLA model performance, and AutoHorizon provides an effective solution for dynamically adapting execution horizons to changing perceptual conditions, improving performance without substantial computational cost.
...
TL;DR: Novel vector field control for fully-actuated systems on matrix Lie groups that ensures convergence to and traversal along curves, with non-redundant control inputs matching group dimension rather than embedding space.


<details>
  <summary>Details</summary>
Motivation: To extend previous vector field control methods (Rezende et al., 2022) from Euclidean translation groups to general connected matrix Lie groups, enabling more practical control inputs and applications to systems like omnidirectional drones.

Method: Generalizes previous work by leveraging Lie group properties to maintain orthogonality between convergent and traversal components, enabling non-redundant control inputs matching the Lie group dimension. Provides efficient algorithm for SE(3) case.

Result: Theoretical extension of vector field control to matrix Lie groups with non-redundant inputs, experimental validation using robotic manipulator demonstrating effectiveness.

Conclusion: The proposed method successfully generalizes vector field control to matrix Lie groups with practical advantages, particularly for SE(3) applications like omnidirectional drone control, validated experimentally.
...
TL;DR: LiLo-VLA is a modular VLA framework that decouples transport from interaction to enable zero-shot generalization to novel long-horizon manipulation tasks without training on them.


<details>
  <summary>Details</summary>
Motivation: General-purpose robots need to handle long-horizon manipulation with multiple kinematic structure changes in unstructured environments. Current VLA models struggle with combinatorial complexity of sequencing atomic skills and suffer from cascading failures due to environmental sensitivity.

Method: Proposes LiLo-VLA (Linked Local VLA) with modular architecture: Reaching Module handles global motion, while Interaction Module uses object-centric VLA to process isolated objects. This decoupling ensures robustness against irrelevant visual features and spatial invariance. Enables dynamic replanning and skill reuse for failure recovery.

Result: Achieves 69% average success rate on 21-task simulation benchmark (LIBERO-Long++ and Ultra-Long), outperforming Pi0.5 by 41% and OpenVLA-OFT by 67%. Real-world evaluations across 8 long-horizon tasks show 85% average success rate.

Conclusion: LiLo-VLA's modular approach effectively addresses cascading failures in long-horizon manipulation through decoupled transport-interaction architecture, enabling zero-shot generalization and robust failure recovery, significantly outperforming existing methods.
...
TL;DR: Reinforcement learning framework for tilt-rotor aerial robots achieves robust SE(3) motion control with sim-to-real transfer via system identification and domain randomization.


<details>
  <summary>Details</summary>
Motivation: Tilt-rotor robots enable omnidirectional maneuvering but face control challenges due to strong coupling between joint and rotor dynamics. Model-based controllers degrade under disturbances and uncertainties.

Method: Learning-based control framework using reinforcement learning for coordinated rotor-joint behaviors. Integrates system identification with minimal, physically consistent domain randomization for reliable sim-to-real transfer.

Result: Achieves comparable 6-DOF pose tracking accuracy to state-of-the-art NMPC, with superior robustness and generalization across diverse tasks. Enables zero-shot deployment on real hardware.

Conclusion: Reinforcement learning with proper sim-to-real transfer techniques provides a robust solution for tilt-rotor aerial control, outperforming traditional model-based approaches in challenging conditions.
...
TL;DR: SPOC is a new benchmark for safety-aware embodied task planning that addresses partial observability and physical constraints, evaluating LLMs on household hazard scenarios with state and constraint-based metrics.
...
Conclusion: SPOC enables rigorous safety assessment for embodied task planning, revealing limitations in current LLMs and providing a benchmark for developing safer planning systems.
...
TL;DR: Proposes closed-loop automated motion data generation framework to overcome fixed difficulty distribution limitations and high motion capture costs, enabling policy improvement through iterative difficulty scaling.


<details>
  <summary>Details</summary>
Motivation: Physics-based humanoid control faces two main challenges: 1) Fixed difficulty distribution in motion datasets limits policy performance ceiling, and 2) High-quality motion capture data is expensive and difficult to scale, restricting dataset diversity and size.

Method: Closed-loop automated motion data generation and iterative framework that generates high-quality motion data with rich action semantics (martial arts, dance, combat, sports, gymnastics). Enables difficulty iteration through physical metrics and objective evaluations to break through original difficulty limits.

Result: On PHC single-primitive tracker, using only ~1/10 of AMASS dataset size, reduced average failure rate on test set (2201 clips) by 45% compared to baseline. Comprehensive ablation and comparative experiments demonstrate framework's rationality and advantages.

Conclusion: The proposed framework successfully addresses limitations of fixed difficulty distributions and high motion capture costs, enabling scalable generation of diverse, high-quality motion data and iterative policy improvement through difficulty scaling.
...
TL;DR: A novel motion control framework combining NMPC for locomotion and DE-based trajectory optimization for jumping enables quadrupedal wheeled-legged robots to perform dynamic jumps including vertical jumps, forward jumps over obstacles, and backflips.


<details>
  <summary>Details</summary>
Motivation: Quadrupedal wheeled-legged robots offer superior mobility by combining legged and wheeled locomotion, but executing dynamic jumps remains challenging due to the additional degrees of freedom from wheeled legs.

Method: Developed a mini-sized wheeled-legged robot with a novel motion control framework integrating Nonlinear Model Predictive Control (NMPC) for locomotion and Differential Evolution (DE)-based trajectory optimization for jumping, utilizing wheel motion to enhance jumping performance.

Result: The framework successfully enables versatile jumping maneuvers including vertical jumps reaching 0.5 m and forward jumps over 0.12 m obstacles, validated through extensive simulations and real-world experiments.

Conclusion: The proposed integrated control framework effectively addresses the challenge of dynamic jumping in quadrupedal wheeled-legged robots, demonstrating superior jumping capabilities that combine the advantages of both legged and wheeled locomotion.
...
<details>
  <summary>Details</summary>
Motivation: Multi-agent robotic manipulation is challenging due to coordination demands, grasp stability requirements, and collision avoidance in shared workspaces. Existing approaches struggle with integrating multiple sensory modalities effectively for coordinated control.

Method: ADM-DP integrates vision (RGB + point cloud via FiLM modulation), tactile (FSR feedback for grasp refinement), and graph-based (multi-agent TCP positions) modalities. It uses an Adaptive Modality Attention Mechanism (AMAM) for dynamic fusion and employs decoupled training where agents learn independent policies while sharing spatial information.

Result: Achieved 12-25% performance gains over state-of-the-art baselines across seven multi-agent tasks. Ablation studies show greatest improvements in tasks requiring multiple sensory modalities, validating the adaptive fusion strategy.

Conclusion: ADM-DP effectively addresses multi-agent manipulation challenges through integrated multi-modal perception, adaptive fusion, and decoupled training, demonstrating robustness across diverse manipulation scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Current tactile simulations face a dilemma: simplified geometric projections lack physical authenticity, while high-fidelity FEM methods are too computationally expensive for large-scale reinforcement learning, creating a fundamental bottleneck for dexterous robotic manipulation.

Method: Tacmap uses volumetric penetration depth to compute 3D intersection volumes as depth maps in simulation. In the real world, an automated data-collection rig learns a mapping from raw tactile images to ground-truth depth maps, creating a shared deform map representation that aligns simulation and reality.

Result: Quantitative evaluations show Tacmap's deform maps closely mirror real-world measurements across diverse contact scenarios. A policy trained exclusively in simulation for an in-hand rotation task achieves zero-shot transfer to a physical robot.

Conclusion: Tacmap successfully bridges the tactile sim-to-real gap by unifying simulation and reality through a shared geometric representation, enabling efficient simulation for reinforcement learning while maintaining physical consistency for zero-shot transfer.
...
TL;DR: SC-VLA introduces self-correcting vision-language-action models that use sparse imagination and online action refinement to achieve self-improvement without external rewards, outperforming baselines in robot manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Current VLA models lack robust physical dynamics understanding, RL needs external rewards isolated from internal states, and world action models lack explicit self-improvement mechanisms.

Method: Designs sparse world imagination with auxiliary predictive heads for task progress and future trajectory forecasting, plus online action refinement module that reshapes progress-dependent dense rewards based on predicted sparse future states.

Result: Achieves SOTA on robot manipulation tasks with 16% fewer steps, 9% higher success rate than best baselines, and 14% gain in real-world experiments.

Conclusion: SC-VLA enables self-improvement through intrinsic guidance of action refinement via sparse imagination, demonstrating superior performance in both simulation and real-world robot manipulation.
...
TL;DR: DAGS-SLAM: Dynamic-aware 3D Gaussian Splatting SLAM system with spatiotemporal motion probability states and on-demand semantic triggering for efficient mobile deployment.


<details>
  <summary>Details</summary>
Motivation: Mobile robots and IoT devices need real-time localization and dense reconstruction under tight compute/energy budgets. Existing dynamic 3DGS-SLAM methods rely on heavy optical flow and per-frame segmentation, which are too costly for mobile deployment and brittle under challenging illumination.

Method: Maintains spatiotemporal motion probability (MP) state per Gaussian, triggers semantics on demand via uncertainty-aware scheduler. Fuses lightweight YOLO instance priors with geometric cues to estimate and update MP, propagates MP to front-end for dynamic-aware correspondence selection, and suppresses dynamic artifacts in back-end via MP-guided optimization.

Result: Experiments on public dynamic RGB-D benchmarks show improved reconstruction and robust tracking while sustaining real-time throughput on commodity GPU. Demonstrates practical speed-accuracy tradeoff with reduced semantic invocations.

Conclusion: DAGS-SLAM provides an efficient dynamic-aware 3DGS-SLAM system suitable for mobile deployment with reduced computational overhead compared to existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current imitation learning approaches for legged robots fail to capture fundamental principles of human motion, simply replicating joint angles without understanding underlying biomechanics. There's a need for systematic evaluation of human-robot locomotion discrepancies.

Method: Proposed Gait Divergence Analysis Framework (GDAF) - a unified biomechanical evaluation framework that systematically quantifies kinematic and kinetic discrepancies. Applied to compare human and humanoid locomotion across 28 walking speeds, with open-source implementation including analysis, visualization, and MuJoCo-based tools.
...
Conclusion: GDAF provides a quantitative benchmark for evaluating humanoid locomotion and offers data/tools to develop more human-like, energetically efficient controllers. The framework enables reproducible, interpretable biomechanical analysis to bridge the gap between biological and mechanical locomotion.
...
TL;DR: Hierarchical LLM-based planner with prompt optimization for multi-robot task planning, combining LLM interpretation with PDDL planning and improving success rates on MAT-THOR benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing approaches have limitations: PDDL planners struggle with ambiguous/long-horizon missions, while LLMs can hallucinate or produce infeasible actions. Need a system that combines strengths of both for robust multi-robot task planning.

Method: Hierarchical multi-agent LLM-based planner with prompt optimization. Upper layer decomposes tasks and assigns to lower-layer agents, which generate PDDL problems solved by classical planner. When plans fail, applies TextGrad-inspired textual-gradient updates to optimize prompts. Meta-prompts are learned and shared across agents within same layer.

Result: Achieves success rates of 0.95 on compound tasks, 0.84 on complex tasks, and 0.60 on vague tasks on MAT-THOR benchmark, improving over previous state-of-the-art LaMMA-P by 2, 7, and 15 percentage points respectively.

Conclusion: The hierarchical structure, prompt optimization, and meta-prompt sharing significantly improve planning accuracy, with contributions of +59, +37, and +4 percentage points to overall success rate respectively. The approach effectively combines LLM interpretation with rigorous PDDL planning.
...
TL;DR: SunnyParking is a dual-branch E2E architecture for autonomous parking that jointly predicts spatial trajectories and discrete motion states, addressing the "dimensionality deficiency" in existing methods by incorporating motion state awareness and using Fourier feature-based parking slot representation.


<details>
  <summary>Details</summary>
Motivation: Existing E2E planning methods for autonomous parking oversimplify the task as geometric path regression, neglecting explicit modeling of vehicle kinematic states. This "dimensionality deficiency" leads to physically infeasible trajectories and deviates from human driving behavior, especially at critical gear-shift points in multi-shot parking scenarios.

Method: Proposes SunnyParking, a novel dual-branch End-to-End architecture that achieves motion state awareness by jointly predicting spatial trajectories and discrete motion state sequences (forward/reverse). Introduces Fourier feature-based representation of target parking slots to overcome resolution limitations of traditional BEV approaches, enabling high-precision target interactions.

Result: The framework generates more robust and human-like trajectories in complex multi-shot parking scenarios while significantly improving gear-shift point localization accuracy compared to state-of-the-art methods. Also open-sources a new parking dataset from CARLA simulator specifically designed to evaluate full prediction capabilities under complex maneuvers.

Conclusion: SunnyParking addresses the critical limitation of existing E2E parking methods by incorporating motion state awareness, resulting in more physically feasible and human-like parking trajectories, particularly for complex multi-shot parking scenarios requiring frequent direction changes.
...
TL;DR: PF-DAG is a two-stage imitation learning framework that decouples coarse action modes from fine-grained variations to address multi-modal robotic manipulation challenges, outperforming SOTA baselines across 56 tasks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for multi-modal robotic manipulation imitation learning face trade-offs: discrete tokenization loses fine-grained action variations, while continuous latent-variable methods produce unstable mode transitions. Need a method that maintains both coarse action consistency and fine-grained control.

Method: Two-stage framework: (1) Compress action chunks into discrete modes for lightweight policy to select consistent coarse modes, avoiding mode bouncing; (2) Mode-conditioned MeanFlow policy generates high-fidelity continuous actions conditioned on selected modes.

Result: Theoretically proves lower MSE bound than single-stage generative policies. Empirically outperforms SOTA baselines across 56 tasks from Adroit, DexArt, and MetaWorld benchmarks. Successfully generalizes to real-world tactile dexterous manipulation tasks.

Conclusion: Explicit mode-level decoupling enables both robust multi-modal modeling and reactive closed-loop control for robotic manipulation, addressing limitations of existing discrete and continuous action representation approaches.
...
<details>
  <summary>Details</summary>
Motivation: Traditional jerk-based heuristic trajectory sampling causes unstable terminal behavior and state discontinuities under frequent regeneration in structured, low-speed dynamic environments for visually impaired scenarios.

Method: Integrates endpoint regulation to stabilize terminal states within each segment and momentum-aware dynamics to regularize velocity/acceleration evolution for segment consistency.

Result: Reduced acceleration peaks and jerk levels with decreased dispersion, smoother velocity/acceleration profiles, more stable endpoint distributions, and fewer infeasible trajectory candidates compared to baseline.

Conclusion: The proposed approach effectively addresses stability and consistency issues in trajectory generation for visually impaired scenarios, improving motion quality and reliability.
...
TL;DR: Hybrid aerodynamic model for winged blimps combining fixed-wing and drag models with neural network mixer, validated on extensive real-world flight data.


<details>
  <summary>Details</summary>
Motivation: Winged blimps operate across distinct aerodynamic regimes (high-speed fixed-wing behavior vs low-speed drag-dominated dynamics) that cannot be captured by single models, with transitions between regimes being a fundamental challenge.

Method: Hybrid framework integrating fixed-wing Aerodynamic Coupling Model (ACM) and Generalized Drag Model (GDM) using learned neural network mixer with physics-based regularization, identified through structured three-phase pipeline.

Result: Validated on RGBlimp platform with 1,320 real-world flight trajectories across 330 configurations; hybrid model consistently outperforms single-model and predefined-mixer baselines.

Conclusion: Establishes practical and robust aerodynamic modeling solution for winged blimps that enables smooth transitions between regimes while retaining explicit physics-based representation.
...
TL;DR: LessMimic uses Distance Fields as a geometric representation to enable reference-free humanoid robot policies that generalize across object scales, compose long-horizon skills, and transfer to vision-only deployment.
...
Method: LessMimic conditions a single whole-body policy on Distance Field (DF) derived geometric cues (surface distances, gradients, velocity decompositions). It uses a Variational Auto-Encoder (VAE) to encode interaction latents, post-trained with Adversarial Interaction Priors (AIP) under Reinforcement Learning. The method includes DAgger-style distillation to align DF latents with egocentric depth features for vision-only deployment.

Result: A single LessMimic policy achieves 80-100% success across object scales from 0.4x to 1.6x on PickUp and SitStand tasks where baselines degrade sharply. It attains 62.1% success on 5 task instance trajectories and remains viable up to 40 sequentially composed tasks.

Conclusion: By grounding interaction in local geometry rather than demonstrations, LessMimic offers a scalable path toward humanoid robots that can generalize, compose skills, and recover from failures in unstructured environments.
...
TL;DR: JALA learns joint-aligned latent actions from human videos to improve robot manipulation, using a 7.5M video dataset blending lab and in-the-wild footage.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models suffer from limited robot data; human videos offer rich alternatives but existing methods face trade-offs between small labeled datasets and large unlabeled footage with unreliable tracking.

Method: JALA learns predictive action embeddings aligned with both inverse dynamics and real actions, creating transition-aware latent space without full visual reconstruction. Uses UniHand-Mix dataset of 7.5M videos blending lab and in-the-wild footage.

Result: Generates more realistic hand motions in both controlled and unconstrained scenarios, significantly improves downstream robot manipulation performance in simulation and real-world tasks.

Conclusion: Jointly-aligned latent actions provide a scalable pathway for VLA pretraining from human data, overcoming limitations of existing human video datasets.
...
TL;DR: Haptic teleoperation system enables trainers to remotely guide trainees wearing arm exoskeletons through intuitive physical interaction, reducing completion time and verbal instructions compared to visual demonstration.


<details>
  <summary>Details</summary>
Motivation: Robotic systems for motor training have limited real-world adoption due to non-intuitive trainer-trainee interactions. The paper aims to address this gap by creating a more intuitive remote guidance system.

Method: Developed a haptic teleoperation system where trainers use a commercial handheld haptic device to physically interact with a trainee's arm exoskeleton through virtual contact points at elbow and wrist. Tested with 32 participants in trainer-trainee paradigm comparing haptic vs visual demonstration for arm pose guidance.

Result: Haptic demonstration significantly reduced movement completion time, improved movement smoothness, and required fewer verbal instructions. Trainers reported greater competence and trainees reported lower physical demand, with no increase in mental/physical effort for trainers compared to visual demonstration.

Conclusion: The haptic teleoperation interface is feasible for effective remote human-robot physical interaction. Future work should assess clinical usability and efficacy for restoring clinicians' sense of agency in robot-assisted therapy.
...
TL;DR: DexRep is a novel hand-object interaction representation that improves generalization in dexterous manipulation tasks by capturing object surface features and spatial relations, achieving high success rates on unseen objects with minimal sim-to-real gap.


<details>
  <summary>Details</summary>
Motivation: Existing DRL methods for dexterous manipulation focus on sample efficiency but overlook representation's role in generalization across complex hand-object interactions, limiting policy transfer to unseen objects and real-world deployment.

Method: Proposes DexRep, a novel representation that captures object surface features and spatial relations between hands and objects. Uses this representation to learn policies for three dexterous manipulation tasks: grasping, in-hand reorientation, and bimanual handover.

Result: For grasping: 87.9% success rate on 5000+ unseen diverse objects (trained with only 40 objects), significantly surpassing existing work. For reorientation and handover: 20-40% improvement over existing representations. Real-world deployment shows small sim-to-real gap under multi/single-camera setups.

Conclusion: DexRep effectively addresses generalization challenges in dexterous manipulation by providing a robust hand-object interaction representation that enables high performance on unseen objects and facilitates real-world deployment with minimal sim-to-real gap.
...
TL;DR: Two-stage RL + visual servoing framework for precise DLO shape control, combining model-based RL for large deformations with Jacobian-based visual servoing for fine adjustments, achieving high success rates and zero-shot sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Deformable Linear Object (DLO) shape control struggle with complex large deformations (especially opposite curvatures), lack efficiency and precision, and have difficulty transferring from simulation to real-world applications.

Method: Two-stage framework: 1) Large-deformation stage uses model-based RL with ensemble dynamics models for sample efficiency, plus self-curriculum goal generation for diverse intermediate goals; 2) Small-deformation stage uses Jacobian-based visual servoing for high-precision convergence.

Result: Simulation shows efficient policy learning and outperforms baselines in success rate and precision. Zero-shot transfer to real-world tasks succeeds in all 30 cases with diverse shapes, sizes, and materials.

Conclusion: The proposed framework effectively addresses DLO shape control challenges by combining RL for large deformations with visual servoing for precision, achieving robust performance and successful sim-to-real transfer.
...
TL;DR: A novel SAR framework optimizes robot deployment for search-and-rescue using 5G/6G networks, balancing robot count, area coverage, and response time while considering terrain, energy, and connectivity constraints.


<details>
  <summary>Details</summary>
Motivation: Cellular-enabled collaborative robots are crucial for SAR operations but face limitations due to battery constraints and the need for persistent low-latency communication, requiring optimization of deployment strategies.

Method: Proposes a two-phase SAR framework (Mission Planning and Mission Execution) that optimizes robot deployment by considering exploration area size, terrain elevation, fleet size, energy profiles, exploration rate, and target response time to determine minimum robots needed and optimal paths.

Result: Demonstrates trade-offs between number of robots, explored area, and response time for different robot types (wheeled and quadruped), and quantifies impact of terrain elevation on mission time and energy consumption.

Conclusion: The framework provides critical insights for leveraging next-generation mobile networks to enhance autonomous SAR operations by incorporating real-world environmental factors affecting connectivity and mission planning.
...
TL;DR: Dream-SLAM: A monocular active SLAM method that uses "dreaming" of cross-spatio-temporal images and semantic structures to improve localization, mapping, and long-horizon planning in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Existing active SLAM methods have three main limitations: 1) Inherit restrictions of underlying SLAM modules, 2) Shortsighted motion planning lacking long-term vision, 3) Struggle with dynamic scenes. The authors aim to overcome these limitations.

Method: Proposes Dream-SLAM which generates "dreamed" cross-spatio-temporal images and semantically plausible structures of partially observed dynamic environments. These dreamed images are fused with real observations to reduce noise and data incompleteness. Also integrates dreamed and observed scene structures for long-horizon planning.
...
Conclusion: Dream-SLAM successfully addresses limitations of existing active SLAM methods by leveraging dreamed cross-spatio-temporal images and semantic structures, enabling more accurate pose estimation, coherent 3D representation, and farsighted trajectory planning for efficient exploration.
...
TL;DR: RGS framework integrates VLM-based gaze reasoning with conditional VQ-VAE motion generation for natural human-robot gaze shifts.


<details>
  <summary>Details</summary>
Motivation: Enabling humanoid robots to perform natural, context-appropriate gaze shifts in unconstrained human-robot interaction remains challenging, requiring coupling of cognitive attention mechanisms with biomimetic motion generation.

Method: Two-component framework: 1) VLM-based gaze reasoning pipeline to infer context-appropriate gaze targets from multimodal cues, 2) conditional VQ-VAE model for eye-head coordinated gaze-shift motion generation.

Result: Experiments validate that RGS effectively replicates human-like target selection and generates realistic, diverse gaze-shift motions.

Conclusion: The proposed RGS framework successfully integrates cognitive attention mechanisms with biomimetic motion generation for natural human-robot gaze interaction.
...
TL;DR: Foundation models (LLMs, VLMs, VLAs) enable multi-level transfer learning in robotics, bringing robots closer to "full-stack transfer" across linguistic, visual, and motor skill domains.


<details>
  <summary>Details</summary>
Motivation: To explore how foundation models and transformers facilitate transfer learning at different abstraction levels in robotics, from high-level linguistic knowledge to low-level motor skills, and assess their potential for achieving comprehensive "full-stack transfer."

Method: The paper provides an overview and analysis of foundation models (LLMs, VLMs, VLAs) from a robotic transfer learning perspective, examining recurring transfer concepts beyond specific implementations, and considering data collection and benchmark challenges.

Result: Foundation models demonstrate significant potential for enabling transfer learning across multiple levels of abstraction in robotics, bringing robots closer to achieving "full-stack transfer" capabilities.

Conclusion: Foundation models are expected to remain a key technology on the route to achieving full-stack transfer in robotics, despite challenges in data collection and benchmarking.
...
TL;DR: CT-RIO: A continuous-time relative-inertial odometry framework using clamped B-splines for high-accuracy, low-latency multi-robot localization with parallel optimization.


<details>
  <summary>Details</summary>
Motivation: Multi-robot cooperation requires accurate relative localization, but existing continuous-time methods struggle with query-time delays, high computational costs, and asynchronous measurements with clock time-offsets in swarm settings.

Method: Uses Clamped Non-Uniform B-splines (C-NUBS) to eliminate query-time delay, with closed-form extension/shrinkage operations for online estimation. Introduces knot-keyknot strategy for high-frequency spline extension. Formulates sliding-window relative localization problem and decomposes optimization into parallel robot-wise sub-problems solved with incremental asynchronous block coordinate descent.

Result: Converges from time-offsets up to 263 ms to sub-millisecond within 3 seconds, achieves RMSEs of 0.046 m and 1.8°. Outperforms state-of-the-art methods by up to 60% under high-speed motion.

Conclusion: CT-RIO successfully addresses challenges in multi-robot continuous-time localization by combining clamped B-splines with parallel optimization, achieving high accuracy, low latency, and computational efficiency suitable for robot swarms.
...
TL;DR: WoG framework uses compressed future observation conditions to guide VLA action generation, balancing efficiency with fine-grained control.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models struggle to balance efficient future representations with preserving fine-grained information needed for precise action generation.

Method: Proposes WoG framework that maps future observations into compact conditions, injects them into action inference pipeline, and trains VLA to predict both conditions and actions simultaneously.

Result: Outperforms existing future prediction methods in simulation and real-world experiments, showing better generalization and learning from human manipulation videos.

Conclusion: Modeling compressed condition space enables effective world modeling for action inference, facilitating fine-grained action generation with superior generalization.
...
TL;DR: FlowCorrect enables real-time correction of robot failures using sparse human nudges via VR, adapting policies without retraining to fix near-miss scenarios.


<details>
  <summary>Details</summary>
Motivation: Generative manipulation policies often fail catastrophically under distribution shift, but many failures are near-misses that could be fixed with small corrective motions rather than requiring full policy retraining.
...
TL;DR: Force Policy: A global-local vision-force policy that decouples vision-guided free-space motion from high-frequency force feedback control for contact-rich manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches either entangle vision and force control in monolithic networks (trading off generalization vs local refinement) or assume known task structure. Need better integration of global perception with local force regulation for robust contact-rich manipulation.

Method: Formalizes a physically grounded "interaction frame" that decouples force regulation from motion execution. Proposes a method to recover this frame from demonstrations. Uses a global policy with vision for free-space guidance, and upon contact, a high-frequency local policy with force feedback estimates the interaction frame and executes hybrid force-position control.

Result: Real-world experiments across diverse contact-rich tasks show consistent gains over strong baselines: more robust contact establishment, more accurate force regulation, and reliable generalization to novel objects with varied geometries and physical properties.

Conclusion: The proposed Force Policy framework improves both contact stability and execution quality by effectively integrating global vision guidance with local force feedback control through the novel interaction frame concept.
...
TL;DR: Behavioral cloning approach for robotic connector insertion using force-torque sensing and vision achieves over 90% success rate across varied connector geometries.


<details>
  <summary>Details</summary>
Motivation: Automating wire harness assembly is challenging due to deformable cables, varying connector geometries, and the need for delicate force control. While humans can do this intuitively with visual and haptic feedback, programming industrial robots for adaptable connector insertion remains difficult.

Method: Empirical study using behavioral cloning to learn action prediction models for connector insertion. Combines force-torque sensing with fixed position camera. Evaluates multiple network architectures using 300 human demonstrations collected via teleoperation of UR5e robot with SpaceMouse under varying connector poses.

Result: System achieves over 90% overall insertion success rate when evaluated against five different connector geometries under varying poses.

Conclusion: Behavioral cloning with fused force-torque and visual sensing is an effective approach for automating connector insertion tasks, demonstrating high success rates across diverse connector geometries and poses.
...
TL;DR: A robotic bicycle platform (UMV) inspired by trials cyclists that combines minimal actuation with reaction mass for dynamic athletic behaviors like jumping, balancing, and flipping.


<details>
  <summary>Details</summary>
Motivation: Inspired by trials cyclists and mountain bikers who can hop, jump, balance, and drive on wheels, the researchers aim to create a robotic platform that achieves similar athletic versatility with minimal actuated degrees of freedom.

Method: Used simulation-driven design optimization to create a spatial linkage topology combining a bicycle with reaction mass. Employed constrained Reinforcement Learning framework to train diverse athletic behaviors, enabling zero-shot transfer to physical robot.

Result: Created a 23.5 kg robot capable of high speeds (8 m/s) and jumping over obstacles up to 1 m tall (130% of robot's height). Demonstrated track-stands, jumps, wheelies, rear wheel hopping, and front flips with zero-shot transfer from simulation.

Conclusion: The UMV platform successfully demonstrates that combining minimal actuation with reaction mass and advanced control methods can achieve versatile athletic behaviors comparable to human trials cyclists, enabling dynamic mobility in challenging terrain.
...
TL;DR: Position-based flocking model achieves velocity alignment without velocity sensing by using position changes over time, enabling cohesive swarm motion in real robots.


<details>
  <summary>Details</summary>
Motivation: Real-world robotic swarms often lack reliable velocity measurements, making traditional velocity-based flocking models impractical. Inspired by collective motion in nature, the paper aims to develop a flocking algorithm that works with only position sensing.

Method: Uses position-based flocking model that approximates relative velocity differences from changes between current and initial relative positions. Incorporates time- and density-dependent alignment gain with non-zero minimum threshold to maintain persistent alignment without velocity sensing.

Result: Simulations with 50 agents show faster and more sustained directional alignment, and more compact formations than velocity-alignment baseline. Experimental validation with 9 real wheeled mobile robots demonstrates practical applicability.

Conclusion: The position-based flocking model successfully enables cohesive collective motion without velocity sensing, making it particularly suitable for real-world robotic swarms where velocity measurements are unreliable or unavailable.
