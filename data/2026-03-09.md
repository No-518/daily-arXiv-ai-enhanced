...
TL;DR: SKETCHGAIT introduces sketch as a new visual modality for gait recognition, combining label-free structural cues with parsing for improved performance.


<details>
  <summary>Details</summary>
Motivation: Existing gait recognition methods rely on silhouette- and parsing-based representations with limitations: silhouettes are sparse and miss internal details, while parsing depends heavily on upstream human parsers with unstable performance across datasets.

Method: Proposes SKETCH as a new visual modality that extracts high-frequency structural cues (limb articulations, occlusion contours) via edge-based detectors without semantic labels. Introduces SKETCHGAIT framework with two independent streams for modality-specific learning and lightweight early-stage fusion to capture structural complementarity between label-free sketch and label-guided parsing.

Result: Achieves 92.9% Rank-1 accuracy on SUSTech1K and 93.1% mean Rank-1 on CCPG datasets, demonstrating superior performance.

Conclusion: Sketch provides a valuable new modality for gait recognition that captures dense part-level structure without semantic labels, complementing parsing-based approaches and enabling more robust performance across datasets.
...
TL;DR: Thinking with Spatial Code transforms RGB video into explicit 3D representations for physical-world visual QA, enabling LLMs to reason over spatial variables and achieving SOTA on VSI-Bench.


<details>
  <summary>Details</summary>
Motivation: To enable large language models to perform physical-world visual question answering by providing them with explicit, temporally coherent 3D representations rather than raw RGB video data.

Method: Proposes a spatial encoder that parses videos into structured spatial code with 3D oriented bounding boxes and semantic labels, unifying 6D object parsing and tracking with geometric prediction. LLMs are further finetuned with reinforcement learning using a spatial rubric reward for perspective-aware, geometrically grounded inference.

Result: Outperforms proprietary vision-language models on VSI-Bench, setting a new state-of-the-art for physical-world visual question answering.

Conclusion: The Thinking with Spatial Code framework successfully enables LLMs to reason directly over explicit spatial variables from video data, demonstrating the value of structured 3D representations for visual question answering tasks.
...
TL;DR: First coupled robustness verification framework for heatmap-based keypoint detectors that bounds joint deviation across all keypoints using MILP formulation.


<details>
  <summary>Details</summary>
Motivation: Keypoint detection is crucial for vision tasks but neural models are vulnerable to perturbations. Existing verification approaches treat keypoints independently, missing interdependencies and yielding conservative guarantees.

Method: Formulates verification as falsification using mixed-integer linear program (MILP) that combines reachable heatmap sets with polytope encoding joint deviation constraints. Infeasibility certifies robustness, feasibility provides counterexamples.

Result: Coupled approach achieves high verified rates and remains effective under strict error thresholds where decoupled methods fail. Method is sound - certification guarantees model robustness.

Conclusion: Proposed framework enables formal robustness verification for keypoint detectors by capturing interdependencies between keypoints, addressing limitations of independent verification approaches.
...
TL;DR: DreamCAD generates editable BRep CAD models from point clouds without CAD annotations, using parametric patches and differentiable tessellation, achieving SOTA results with 75% user preference.
...
Method: DreamCAD represents BReps as parametric patches (e.g., Bézier surfaces) and uses differentiable tessellation to generate meshes, enabling large-scale training on 3D datasets without CAD-specific annotations. Also introduces CADCap-1M dataset with 1M+ GPT-5 generated descriptions.
...
Conclusion: DreamCAD enables scalable CAD generation from point-level supervision without CAD annotations, and the CADCap-1M dataset advances text-to-CAD research. Code and dataset will be publicly available.
...
TL;DR: ABRA is a domain generalization method that uses adversarial representation augmentation to mitigate biological batch effects in cell painting images, achieving SOTA on siRNA perturbation classification.


<details>
  <summary>Details</summary>
Motivation: High-content screening generates massive cell painting images, but technical variations create biological batch effects that cause covariate shifts and degrade deep learning model generalization on unseen data. Existing batch correction methods either require additional prior knowledge or struggle with unseen bio-batches.

Method: Frames bio-batch mitigation as Domain Generalization and proposes Adversarial Batch Representation Augmentation (ABRA). Models batch-wise statistical fluctuations by parameterizing feature statistics as structured uncertainties. Uses min-max optimization to synthesize worst-case bio-batch perturbations in representation space with strict angular geometric margin to preserve class discriminability. Introduces synergistic distribution alignment objective to prevent representation collapse.

Result: Extensive evaluations on large-scale RxRx1 and RxRx1-WILDS benchmarks demonstrate that ABRA establishes new state-of-the-art performance for siRNA perturbation classification.

Conclusion: ABRA effectively addresses biological batch effects in cell painting images through adversarial representation augmentation, providing a robust solution for domain generalization in high-content screening applications.
...
TL;DR: PFS is a lightweight post-fusion stabilizer module that improves robustness of existing BEV fusion detectors against domain shift and sensor failures without architectural changes or retraining.


<details>
  <summary>Details</summary>
Motivation: BEV fusion detectors degrade under domain shift and sensor failures, limiting real-world reliability. Existing robustness approaches require architectural modifications or specialized retraining, making them hard to integrate into deployed systems.

Method: PFS operates on intermediate BEV representations, stabilizing feature statistics under domain shift, suppressing degraded sensor regions, and adaptively restoring weakened cues through residual correction. Designed as near-identity transformation to preserve performance.

Result: Achieves SOTA results on nuScenes benchmark: +1.2% mAP for camera dropout robustness, +4.4% mAP for low-light performance, while maintaining lightweight footprint of only 3.3M parameters.

Conclusion: PFS provides an effective, lightweight solution to improve robustness of existing BEV fusion detectors against sensor failures and domain shifts without requiring architectural changes or retraining.
...
TL;DR: CBM-Suite addresses four key limitations of Concept Bottleneck Models: no metric for concept relevance evaluation, the "linearity problem" causing bypass of concept bottleneck, accuracy gap vs opaque models, and lack of systematic study on visual backbones/VLMs impact.


<details>
  <summary>Details</summary>
Motivation: Concept Bottleneck Models (CBMs) have fundamental limitations including: 1) absence of metric to pre-evaluate concept relevance, 2) "linearity problem" causing recent CBMs to bypass concept bottleneck, 3) accuracy gap compared to opaque models, and 4) lack of systematic study on impact of different visual backbones and VLMs.

Method: CBM-Suite framework with four components: 1) entropy-based metric to quantify concept set suitability, 2) non-linear layer between concept activations and classifier to resolve linearity problem, 3) distillation loss guided by linear teacher probe to narrow accuracy gap, and 4) comprehensive analyses of vision encoders, VLMs, and concept sets interactions.

Result: Extensive evaluations show CBM-Suite yields more accurate models and provides insights for improving concept-based interpretability.

Conclusion: CBM-Suite systematically addresses key limitations of CBMs through a methodological framework that improves both accuracy and interpretability while providing tools for concept evaluation and systematic analysis of model components.
...
TL;DR: iFID is a new metric that correlates strongly with diffusion generation FID by interpolating nearest neighbors in latent space, unlike reconstruction FID which correlates poorly.


<details>
  <summary>Details</summary>
Motivation: Reconstruction FID (rFID) of VAEs is poorly correlated with generation FID (gFID) of latent diffusion models, creating a need for a better predictive metric.
...
Result: iFID achieves strong correlation with diffusion gFID (Pearson ~0.85, Spearman ~0.85), outperforming rFID which correlates with diffusion refinement phase quality instead.

Conclusion: iFID is the first metric demonstrating strong correlation with diffusion gFID, providing insights into why reconstruction metrics fail and connecting to diffusion generalization/hallucination phenomena.
...
TL;DR: IEC (Implicit Error Counting) addresses the gap in post-training methods for tasks with multiple valid outputs by counting errors instead of checking correctness against rubrics, outperforming RaR on virtual try-on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current methods like RLVR and RaR work well with clear correctness signals or ideal reference answers, but many real-world tasks have multiple valid outputs and lack single ideal answers, creating a gap in post-training methods.

Method: IEC enumerates what responses get wrong rather than what they get right, applying severity-weighted scores across task-relevant axes and converting them into calibrated per-aspect rewards. Key innovations include implicit score emission and group calibration to reduce noise.

Result: IEC outperforms RaR on MDressBench (CEC: 5.31 vs. 5.60 on flat references; 5.20 vs. 5.53 on non-flat) and matches or surpasses six baselines on 6 of 8 perceptual metrics on VITON-HD and DressCode. CEC metric tracks human preferences well (60% top-1 vs. 30% others).

Conclusion: When ideal answers are unavailable, counting errors provides a stronger signal than constructing rubrics, making IEC effective for domains like virtual try-on that are too constrained for holistic scoring but too permissive for rubric-based evaluation.
...
TL;DR: SemVID is a training-free visual token pruning framework for Video Temporal Grounding that preserves query-critical evidence and cross-frame connectivity, achieving 95.4% mIoU with only 12.5% tokens and 5.8x speedup.


<details>
  <summary>Details</summary>
Motivation: Video Temporal Grounding requires processing long videos with VLMs, which is computationally expensive. Existing pruning methods degrade VTG performance because they don't preserve boundary-sensitive evidence and cross-frame reasoning needed for accurate temporal localization.

Method: SemVID uses two VTG-specific pruning principles: Evidence Retention (keeps query-critical patches around boundaries) and Connectivity Strength (preserves token-level cross-frame connectivity). It allocates per-frame token budgets balancing query relevance and inter-frame variation, then selects three token types: object tokens (query-critical evidence), motion tokens (transitions and cross-frame relays), and context tokens (scene continuity).

Result: Extensive experiments show SemVID achieves strong accuracy-efficiency trade-off: retains up to 95.4% mIoU with only 12.5% visual tokens, delivers up to 5.8x prefill speedup, and consistently outperforms prior methods under same budgets.

Conclusion: SemVID effectively addresses VTG's unique challenges by preserving both boundary-sensitive evidence and cross-frame connectivity through semantic role-based token selection, enabling efficient yet accurate temporal grounding without training.
...
TL;DR: OWL is a perception function that uses visual motion cues (looming and rotation) to enable scaled 3D mapping and camera heading from motion sequences without prior knowledge of environment or motion parameters.


<details>
  <summary>Details</summary>
Motivation: To address complex 3D perception challenges during motion by developing a unified approach that doesn't require explicit measurement of physical quantities like range and translation, and works without prior knowledge of stationary environments, moving objects, or camera motion.

Method: Derives values from two fundamental visual motion cues: 1) perceived local visual looming near fixation points, and 2) perceived rotation of rigid objects relative to fixation points. Uses minimalistic, pixel-based parallel computations on raw visual motion image sequences.

Result: OWL achieves geometric constancy of 3D objects over time and enables scaled 3D scene reconstruction from visual motion cues alone. Simulations demonstrate it can obtain values without prior knowledge of environment or motion parameters.

Conclusion: OWL bridges theory and practice in robotics/autonomous navigation, offers real-time 3D point representation, and provides an alternative perspective on machine perception with implications for natural perception, behavioral psychology, and neural functionality.
...
TL;DR: MultiHaystack is a new benchmark that evaluates multimodal retrieval and reasoning on large-scale, heterogeneous corpora, revealing that current MLLMs struggle with evidence retrieval despite strong reasoning when given evidence.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate multimodal understanding in simplified settings with small, single-modality candidate sets, which doesn't reflect real-world requirements for retrieving evidence from large, heterogeneous multimodal corpora before reasoning.

Method: Created MultiHaystack benchmark with over 46,000 multimodal retrieval candidates (documents, images, videos) and 747 open yet verifiable questions, each requiring evidence localization across modalities and fine-grained reasoning.

Result: Models perform well when given evidence (80.86% accuracy for GPT-5) but drop sharply when required to retrieve evidence (51.4% accuracy). Best retriever (E5-V) achieves only 40.8% Recall@1, showing multimodal retrieval remains a major bottleneck.

Conclusion: Multimodal retrieval over heterogeneous pools is a primary bottleneck for MLLMs, and MultiHaystack provides a valuable testbed to highlight limitations obscured by small-scale evaluations and promote retrieval-centric advances.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with precise global geolocation due to visual ambiguity and underutilized auditory cues. The paper aims to address geographic ambiguity by leveraging both visual and audio information for more accurate localization.

Method: Three-stage framework: (1) Perception stage uses mixture-autoregressive sparse autoencoder to decompose audio into "acoustic atoms"; (2) Multimodal Reasoning stage employs MLLM finetuned with GRPO to synthesize audio and visual features; (3) Precision Prediction stage uses Riemannian Flow Matching on S² manifold for final localization.

Result: The framework significantly outperforms unimodal baselines, demonstrating that interpretable soundscape perception provides critical orthogonal signals that enable high-precision global localization when combined with multimodal reasoning.

Conclusion: Audiovisual geolocation using interpretable acoustic perception and multimodal reasoning enables precise global localization, with audio providing complementary signals to visual information that overcome geographic ambiguity.
...
TL;DR: Any2Full is a one-stage depth completion framework that adapts pretrained monocular depth estimation models using scale prompts from sparse depth inputs, achieving domain generalization and pattern-agnostic performance.


<details>
  <summary>Details</summary>
Motivation: Existing RGBD-fused depth completion methods have limited domain generalization and robustness to various depth patterns, while current two-stage MDE integration strategies incur additional computation and introduce structured distortions.

Method: Reformulates depth completion as scale-prompting adaptation of pretrained MDE models using a Scale-Aware Prompt Encoder that distills scale cues from sparse inputs into unified scale prompts, preserving geometric priors while achieving global scale consistency.

Result: Outperforms OMNI-DC by 32.2% in average AbsREL and delivers 1.4× speedup over PriorDA with the same MDE backbone, demonstrating superior robustness and efficiency.

Conclusion: Any2Full establishes a new paradigm for universal depth completion with one-stage, domain-general, and pattern-agnostic capabilities, offering improved performance and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: ImageNet's single-label assumption creates label noise and limits learning signal, while real-world images contain multiple objects; multi-label annotations better reflect visual scenes for richer representation learning.

Method: Use self-supervised Vision Transformers for unsupervised object discovery, select regions aligned with original labels to train lightweight classifier, apply it to all regions to generate coherent multi-label annotations across dataset.

Result: Models trained with multi-label supervision achieve better in-domain accuracy (+2.0 top-1 on ReaL, +1.5 on ImageNet-V2) and stronger transferability (+4.2 mAP on COCO, +2.3 mAP on VOC).

Conclusion: Multi-label annotations enhance both classification performance and representation learning; automated pipeline provides scalable, high-quality multi-label annotations without human annotation.
...
TL;DR: Automatically creates surgical timelines from videos using CLIP-based multi-modal alignment between video frames and textual gesture descriptions.


<details>
  <summary>Details</summary>
Motivation: Current methods for creating surgical timelines are either vague (post-OP reports) or time-consuming (manual video annotation). There's a need for an automated solution that sits between these extremes.

Method: Uses CLIP-based multi-modal framework to align surgical video frames with textual gesture descriptions. Fine-tunes CLIP visual and text encoders in shared embedding space to improve alignment, then predicts gestures and phases for video frames to construct structured timeline.

Result: Enables automatic creation of surgical timeline and narrative directly from surgical video by bridging visual gestures and textual narratives.

Conclusion: Leverages pretrained multi-modal representations to reduce manual video review and annotation by surgeons while creating structured surgical timelines.
...
TL;DR: Icarus is a deep learning sky-model that generates photorealistic, full dynamic range environment maps with user-controlled solar/cloud positioning and atmospheric texturing, addressing limitations in existing DNN sky-models.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning sky-models fail to accurately recreate photorealistic natural skies, especially at higher resolutions where they struggle with the 14EV+ class-imbalanced solar region, resulting in poor visual quality, skewed lighting, shadows, and tones.

Method: Icarus is an all-weather sky-model that learns the exposure range of Full Dynamic Range (FDR) physically captured outdoor imagery, enabling conditional generation of environment maps with intuitive user-positioning of solar/cloud formations and user-controlled texturing of atmospheric formations.
...
Conclusion: Icarus addresses critical limitations in current DNN sky-models by providing a comprehensive solution for generating photorealistic environment maps with user control over atmospheric elements, enabling more accurate and realistic outdoor scene illumination.
...
TL;DR: LayerBind is a training-free, plug-and-play method for precise region and occlusion control in text-to-image generation by modeling regional generation as distinct layers and binding them during denoising.


<details>
  <summary>Details</summary>
Motivation: Existing region-instructed layout control methods have limitations: training-based approaches inherit data bias and degrade quality, while current techniques struggle with occlusion order, limiting real-world usability.

Method: Two-phase approach: (1) Layer-wise Instance Initialization creates per-instance branches attending to their own regions while anchoring to shared background, fused early to establish layout; (2) Layer-wise Semantic Nursing reinforces regional details via sequential layered attention path with layer-transparency scheduler.

Result: Qualitative and quantitative results demonstrate LayerBind's effectiveness for precise regional and occlusion controllability, supporting editable workflows for flexible modifications like changing instances or rearranging visible orders.

Conclusion: LayerBind provides training-free, plug-and-play regional and occlusion control across Diffusion Transformers, showing strong potential for creative applications with its ability to handle occlusion order and support editable workflows.
...
TL;DR: BM25-V applies text retrieval's BM25 scoring to visual words from Vision Transformers, achieving high recall as an efficient first-stage retriever for dense reranking pipelines.


<details>
  <summary>Details</summary>
Motivation: Dense image retrieval has accuracy but lacks interpretability/attribution and is computationally expensive at scale. Need an efficient, interpretable alternative that can work with sparse visual representations.

Method: Apply Okapi BM25 scoring to sparse visual-word activations from a Sparse Auto-Encoder (SAE) on Vision Transformer patch features. Use BM25's IDF weighting to suppress common visual words and emphasize rare discriminative ones. Retrieve via sparse inverted-index operations as first-stage retriever.

Result: Achieves Recall@200 ≥ 0.993 across seven benchmarks, enabling two-stage pipeline reranking only 200 candidates per query with near-dense accuracy (within 0.2% on average). SAE trained once on ImageNet-1K transfers zero-shot to seven fine-grained benchmarks without fine-tuning.

Conclusion: BM25-V provides an efficient, interpretable first-stage retriever for image retrieval that achieves high recall, enables efficient dense reranking, offers attribution via visual words, and demonstrates strong zero-shot transfer capability.
...
TL;DR: Learnable upsamplers often produce sharper features but don't outperform classical interpolation for 3D reconstruction; spectral consistency matters more than spatial detail enhancement.


<details>
  <summary>Details</summary>
Motivation: While recent learnable upsampling methods focus on enhancing spatial details like sharper geometry or richer textures, their impact on 3D awareness remains underexplored. The authors aim to understand how upsampling affects 3D reconstruction quality.

Method: Introduces a spectral diagnostic framework with six complementary metrics that characterize amplitude redistribution, structural spectral alignment, and directional stability. Evaluates classical interpolation and learnable upsampling methods on CLIP and DINO backbones.

Result: Three key findings: 1) Structural spectral consistency is the strongest predictor of novel view synthesis quality, while high-frequency emphasis often correlates negatively with reconstruction; 2) Geometry and texture respond to different spectral properties; 3) Learnable upsamplers rarely outperform classical interpolation in reconstruction quality.

Conclusion: Reconstruction quality is more closely related to preserving spectral structure than to enhancing spatial detail, highlighting spectral consistency as an important principle for designing upsampling strategies in 2D-to-3D pipelines.
...
TL;DR: EventGeM: A global-to-local feature fusion pipeline for event-based Visual Place Recognition using ViT for global features, MaxViT for local keypoints, and depth estimation for refinement, achieving SOTA performance with real-time capability.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer sparse activation and high-temporal resolution ideal for robotic navigation and localization, especially when energy efficiency and frequent positioning updates are crucial. Existing event-based place recognition methods need improvement in accuracy and robustness across varying conditions.

Method: Three-stage pipeline: 1) Pre-trained ViT-S/16 backbone extracts global features from event histogram images for initial match predictions. 2) Pre-trained MaxViT backbone detects local feature keypoints for 2D-homography based re-ranking with RANSAC. 3) Pre-trained vision foundation model for depth estimation provides additional structural similarity comparison between references and queries.

Result: Achieves state-of-the-art localization performance compared to best available event-based place recognition methods across multiple benchmark datasets and lighting conditions. Fully capable of real-time operation on various compute architectures. Successfully demonstrated in real-world robotic deployment for online localization using event camera streams.

Conclusion: EventGeM presents an effective global-to-local feature fusion approach for event-based Visual Place Recognition that combines multiple pre-trained vision models to achieve superior localization accuracy while maintaining real-time performance suitable for robotic applications.
...
### [22] [Margin and Consistency Supervision for Calibrated and Robust Vision Models](https://arxiv.org/abs/2603.05812)
*Salim Khazem*

Main category: cs.CV

TL;DR: MaCS is a regularization framework that adds margin penalty and consistency supervision to cross-entropy loss, improving calibration and robustness without architectural changes or inference overhead.
...
Method: MaCS adds two components to cross-entropy: (1) hinge-squared margin penalty enforcing target logit gap between correct class and strongest competitor, and (2) consistency regularizer minimizing KL divergence between predictions on clean inputs and mildly perturbed views.
...
Conclusion: MaCS provides an effective drop-in replacement for standard training objectives that requires no additional data, no architectural changes, and negligible inference overhead, offering improved generalization guarantees and provable robustness bounds.
...
<details>
  <summary>Details</summary>
Motivation: Current video generation models have high computational latency that makes real-time applications too expensive, due to temporal redundancy in video latent patches.

Method: Proposed Latent Inter-frame Pruning with Attention Recovery (LIPAR) framework that detects and skips recomputing duplicated latent patches, plus an Attention Recovery mechanism to approximate attention values of pruned tokens to avoid visual artifacts.

Result: Increases video editing throughput by 1.45×, achieving 12.2 FPS on NVIDIA A6000 vs baseline 8.4 FPS, with no generation quality compromise and no additional training required.

Conclusion: The method effectively bridges the gap between traditional compression algorithms and modern generative pipelines, enabling faster video generation without quality degradation.
...
TL;DR: Proposes an ensemble fusion model combining CNNs and Vision Transformers for remote sensing image classification, achieving state-of-the-art accuracy on three benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Remote sensing requires accurate classification, but CNNs struggle with global context while ViTs handle long-range dependencies. Simple CNN-ViT integration creates bottlenecks from redundant features, limiting performance improvements.

Method: Trains four independent fusion models that integrate CNN and ViT backbones, then combines their outputs through ensembling at the final prediction stage to overcome performance bottlenecks from redundant feature representations.

Result: Achieves 98.10% accuracy on UC Merced, 94.46% on RSSCN7, and 95.45% on MSRSI datasets, outperforming competing architectures with efficient computational resource usage during training.

Conclusion: The ensemble fusion approach effectively combines CNN and ViT strengths for remote sensing classification, overcoming performance bottlenecks and achieving state-of-the-art results with computational efficiency.
...
TL;DR: CORE-Seg is an end-to-end framework that integrates reasoning with segmentation for complex medical lesions, achieving state-of-the-art results with 37.06% mean Dice score and 18.42% failure rate.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation needs to shift from visual pattern matching to cognitive reasoning. Current MLLMs lack specialized visual reasoning for complex lesions, while traditional segmentation models lack logical interpretability.

Method: Proposed CORE-Seg framework with Semantic-Guided Prompt Adapter, progressive training from SFT to GRPO, and adaptive dual-granularity reward mechanism to mitigate reward sparsity. Also introduced ComLesion-14K benchmark.

Result: Achieved state-of-the-art with 37.06% mean Dice (14.89% higher than second-best baseline) and reduced failure rate to 18.42%.

Conclusion: The framework successfully integrates reasoning with segmentation for complex medical lesions, demonstrating significant improvements over existing approaches and advancing the paradigm toward cognitive reasoning analysis.
...
TL;DR: BlackMirror is a novel black-box detection framework for backdoored text-to-image models that identifies partial semantic pattern manipulation rather than relying on image-level similarity, achieving accurate detection across diverse backdoor attacks.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor detection methods fail against emerging attacks where backdoored generations appear visually diverse, as they rely on image-level similarity assuming consistent outputs. The key insight is that across attacks, only partial semantic patterns are steadily manipulated while other content remains diverse or benign.
...
Conclusion: BlackMirror provides an effective solution for black-box detection of backdoored text-to-image models by focusing on partial semantic pattern manipulation rather than image-level similarity, making it robust against emerging attacks with diverse visual outputs.
...
### [27] [Cog2Gen3D: Sculpturing 3D Semantic-Geometric Cognition for 3D Generation](https://arxiv.org/abs/2603.05845)
*Haonan Wang,Hanyu Zhou,Haoyue Liu,Tao Gu,Luxin Yan*

Main category: cs.CV

TL;DR: Cog2Gen3D: A 3D cognition-guided diffusion framework that integrates semantic and absolute geometric representations for physically plausible 3D generation, addressing scale inconsistency issues in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generation methods struggle with spatial geometry constraints, often relying on relative geometric features that lead to scale inconsistency. The authors argue that combining semantic information with absolute geometry is crucial for enabling controllable 3D generation that aligns with physical world constraints.

Method: Three key designs: 1) Cognitive Feature Embeddings encoding different modalities into semantic, geometric, and logical representations; 2) 3D Latent Cognition Graph structuring representations into dual-stream semantic-geometric graphs fused via cross-attention; 3) Cognition-Guided Latent Diffusion using the fused 3D cognition graph to guide latent diffusion for 3D Gaussian generation.

Result: Extensive experiments on a validation subset based on Marble World Labs demonstrate that Cog2Gen3D significantly outperforms existing methods in both semantic fidelity and geometric plausibility.

Conclusion: The unified framework with 3D cognition graph ensures physical plausibility and structural rationality of 3D generation, successfully addressing the limitations of existing methods by integrating semantic information with absolute geometry for better 3D cognition.
...
TL;DR: RAC replaces traditional VAE with Rectified Flow-based architecture, achieving better reconstruction/generation with 70% lower compute and 41% fewer parameters via bidirectional inference.


<details>
  <summary>Details</summary>
Motivation: To address limitations of traditional VAEs including the reconstruction-generation gap, high computational cost, and parameter inefficiency by developing a more efficient and effective generative model architecture.

Method: Proposes Rectified Flow Auto Coder (RAC) that uses flow timesteps for multi-step decoding with straight/correctable paths, supports bidirectional inference where decoder serves as encoder through time reversal, enabling parameter sharing and step-by-step refinement.

Result: RAC surpasses state-of-the-art VAEs in both reconstruction and generation quality with approximately 70% lower computational cost and nearly 41% parameter reduction.

Conclusion: RAC provides an efficient alternative to traditional VAEs with improved performance, reduced computational requirements, and partial mitigation of the reconstruction-generation gap through its flow-based architecture and bidirectional inference capabilities.
...
TL;DR: VS3R: A video stabilization framework combining 3D reconstruction with generative video diffusion to achieve robust, full-frame stabilization across diverse camera motions.


<details>
  <summary>Details</summary>
Motivation: Video stabilization faces a fundamental trade-off between geometric robustness and full-frame consistency. 2D methods suffer from aggressive cropping, while 3D techniques often fail under extreme motions due to fragile optimization pipelines.

Method: VS3R synergizes feed-forward 3D reconstruction with generative video diffusion. It jointly estimates camera parameters, depth, and masks for reliability, uses Hybrid Stabilized Rendering to fuse semantic and geometric cues, and employs a Dual-Stream Video Diffusion Model to restore disoccluded regions and rectify artifacts.
...
Conclusion: The proposed VS3R framework successfully bridges the gap between geometric robustness and full-frame consistency in video stabilization by combining 3D reconstruction with generative video diffusion, delivering superior performance across challenging scenarios.
...
### [30] [Facial Expression Recognition Using Residual Masking Network](https://arxiv.org/abs/2603.05937)
*Luan Pham,The Huynh Vu,Tuan Anh Tran*

Main category: cs.CV

TL;DR: A novel Residual Masking Network combining ResNet and U-Net architectures with attention mechanism achieves SOTA accuracy on FER2013 and VEMO datasets for facial expression recognition.


<details>
  <summary>Details</summary>
Motivation: To improve facial expression recognition performance by developing a deep architecture with attention mechanism that focuses on relevant facial features while ignoring irrelevant information.

Method: Proposes a Masking approach using segmentation network to refine feature maps, combining Deep Residual Network with Unet-like architecture to create Residual Masking Network that enables attention to relevant facial regions.

Result: Achieves state-of-the-art accuracy on both public FER2013 dataset and private VEMO dataset, demonstrating superior performance over existing methods.

Conclusion: The Residual Masking Network effectively improves facial expression recognition by focusing attention on relevant facial features through masking mechanism, establishing new SOTA benchmarks.
...
### [31] [TumorChain: Interleaved Multimodal Chain-of-Thought Reasoning for Traceable Clinical Tumor Analysis](https://arxiv.org/abs/2603.05867)
...
TL;DR: TumorChain: A multimodal reasoning framework for clinical tumor analysis using Chain-of-Thought reasoning with 3D CT scans, improving lesion detection, impression generation, and pathology classification.


<details>
  <summary>Details</summary>
Motivation: Accurate tumor analysis is crucial for clinical radiology and precision oncology, requiring reliable lesion characterization and pathology-level risk assessment. Chain-of-Thought reasoning is important for step-by-step interpretation from imaging findings to clinical conclusions, improving traceability and reducing diagnostic errors.

Method: Proposes TumorChain, a multimodal interleaved reasoning framework that couples 3D imaging encoders, clinical text understanding, and organ-level vision-language alignment. Uses cross-modal alignment and iterative interleaved causal reasoning to ground visual evidence, aggregate conclusions, and issue pathology predictions after multiple rounds of self-refinement.

Result: Experiments show consistent improvements over strong baselines in lesion detection, impression generation, and pathology classification. Demonstrates strong generalization on the DeepTumorVQA benchmark.

Conclusion: Highlights the potential of multimodal reasoning for reliable and interpretable tumor analysis in clinical practice, with improved traceability and reduced hallucination risk.
...
### [32] [Energy-Driven Adaptive Visual Token Pruning for Efficient Vision-Language Models](https://arxiv.org/abs/2603.05950)
*Jialuo He,Huangxun Chen*

Main category: cs.CV

TL;DR: E-AdaPrune: Energy-driven adaptive pruning framework for VLMs that dynamically allocates token budgets based on image information density using singular value spectrum analysis, achieving better performance with same average token count.


<details>
  <summary>Details</summary>
Motivation: Existing visual token reduction methods use fixed token budgets for all images, ignoring that images vary greatly in information density - some scenes are information-dense while others have redundant content.

Method: Uses singular value spectrum of visual features to determine token budget by preserving a certain proportion of spectral energy. Allocates more tokens to information-dense scenes and aggressively compresses redundant ones without additional learnable parameters.

Result: Tested on 9 benchmarks with 3 VLM backbones (LLaVA-1.5-7B, LLaVA-1.5-13B, LLaVA-NeXT-8B). Under matched average token budgets, achieves average improvement up to 0.6%, including +5.1% relative boost on MMVet reasoning task. Additional latency limited to 8ms per image using randomized SVD.

Conclusion: E-AdaPrune provides an effective adaptive pruning approach that dynamically adjusts token allocation based on image content, improving VLM performance while maintaining efficiency with minimal computational overhead.
...
TL;DR: PatchCue introduces patch-based visual cues to enhance VLM reasoning, outperforming pixel-level approaches through better alignment with human perception and VLM architecture.


<details>
  <summary>Details</summary>
Motivation: Existing VLM reasoning paradigms like Chain-of-Thought rely only on text and underutilize visual cues, while pixel-level visual cues require precise spatial localization and add learning complexity.

Method: PatchCue partitions images into patches and represents cues at patch level, using two-stage training: cold-start supervised fine-tuning for patch-level cue output, followed by reinforcement learning with process-supervised cue reward.

Result: PatchCue consistently improves performance across multiple VLMs and diverse benchmarks (general VQA, complex reasoning, document understanding), outperforming both pixel-level bounding boxes and point-based cues.

Conclusion: Patch-level cues provide a more effective and cognitively aligned visual reasoning paradigm that better leverages visual information while reducing localization complexity.
...
TL;DR: Skeleton-to-Image Encoding (S2I) transforms 3D human skeleton sequences into image-like data, enabling use of pretrained vision models for self-supervised skeleton representation learning across heterogeneous formats.


<details>
  <summary>Details</summary>
Motivation: Direct application of pretrained vision models to 3D skeleton data is challenging due to format differences. There's scarcity of large-scale skeleton datasets and need to incorporate skeleton data into multi-modal action recognition without adding model branches. Existing methods overlook structural heterogeneity from diverse data sources.

Method: Skeleton-to-Image Encoding (S2I) transforms skeleton sequences into image-like data by partitioning and arranging joints based on body-part semantics, then resizing to standardized image dimensions. This creates a unified image-like format that accommodates heterogeneous skeleton data.
...
Conclusion: S2I enables first-time use of powerful vision-pretrained models for self-supervised skeleton representation learning, effectively transferring visual-domain knowledge to skeleton analysis while handling heterogeneous skeleton formats.
...
### [35] [Shifting Adaptation from Weight Space to Memory Space: A Memory-Augmented Agent for Medical Image Segmentation](https://arxiv.org/abs/2603.05873)
*Bowen Chen,Qiaohui Gao,Shaowen Wan,Shanhui Sun,Wei Liu,Xiang Li,Tianming Liu,Lin Zhao*

Main category: cs.CV

TL;DR: MemSeg-Agent is a memory-augmented segmentation agent that enables few-shot learning, federated learning, and test-time adaptation by shifting adaptation from weight space to memory space, reducing communication overhead and improving generalization.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation models trained on single datasets fail to generalize across institutions/scanners/patient populations. Vision foundation models require task-specific fine-tuning which introduces substantial communication overhead in federated learning and prevents continuous knowledge evolution during deployment.

Method: Proposes MemSeg-Agent that conditions a fixed backbone with lightweight static, few-shot, and test-time working memories dynamically composed by an agentic controller. Updates compact memory units instead of model parameters in federated settings, reducing communication overhead.

Result: Experiments on four public datasets show strong performance and robustness to domain shift: Static memory alone matches or surpasses strong supervised baselines with high parameter efficiency, and test-time working memory further improves in-domain and cross-domain performance without fine-tuning.

Conclusion: MemSeg-Agent introduces a new paradigm for scalable and adaptive medical image segmentation in the era of agentic AI by shifting adaptation from weight space to memory space, enabling unified few-shot learning, federated learning, and test-time adaptation.
...
TL;DR: ProCap reformulates change captioning from static image comparison to dynamic procedure modeling by learning latent change procedures from keyframes and using learnable queries for caption generation.


<details>
  <summary>Details</summary>
Motivation: Existing change captioning methods only work on static image pairs, ignoring the temporal dynamics of how changes occur. Understanding the change procedure is crucial for comprehensive change understanding beyond just what changed.

Method: Two-stage framework: 1) Train a procedure encoder on keyframes from generated intermediate frames using caption-conditioned masked reconstruction. 2) Integrate encoder with decoder using learnable procedure queries to infer latent procedure representations for caption generation, trained end-to-end.

Result: Experiments on three datasets demonstrate ProCap's effectiveness in dynamic change procedure modeling for captioning.

Conclusion: ProCap successfully shifts change modeling from static comparison to dynamic procedure understanding, enabling better comprehension of how changes occur through latent procedure learning and efficient query-based inference.
...
### [37] [Systematic Evaluation of Novel View Synthesis for Video Place Recognition](https://arxiv.org/abs/2603.05876)
*Muhammad Zawad Mahmud,Samiha Islam,Damian Lyons*

Main category: cs.CV

TL;DR: Synthetic novel views improve Video Place Recognition (VPR) performance, especially with small additions; viewpoint change magnitude matters less than number of views added and dataset imagery type.


<details>
  <summary>Details</summary>
Motivation: Synthetic novel views can enhance robot navigation by enabling cross-platform guidance (ground-to-air and air-to-ground) and improving place recognition across different robot perspectives.

Method: Systematic evaluation using five public VPR image databases and seven typical image similarity methods to assess synthetic novel views in VPR.

Result: Small synthetic additions improve VPR recognition statistics; for larger additions, the number of views added and dataset imagery type are more important than viewpoint change magnitude.

Conclusion: Synthetic novel views are beneficial for VPR, with optimal benefits achieved through strategic addition of views rather than maximizing viewpoint changes.
...
TL;DR: This paper analyzes surgical instrument manufacturing defects in Pakistan using deep learning models (YOLOv8, ResNet-152, EfficientNet-b4) on a dataset of 4,414 images to improve quality assurance through automated optical inspection.


<details>
  <summary>Details</summary>
Motivation: Surgical instrument defects can have fatal consequences in healthcare, making precision manufacturing critical. The paper aims to address quality issues in Pakistani-made instruments to prevent financial losses for manufacturers and, more importantly, protect patient safety during surgical procedures.

Method: The research uses a newly developed dataset of 4,414 high-resolution images of surgical instruments. It applies deep learning architectures including YOLOv8, ResNet-152, and EfficientNet-b4 for automated defect detection. The study collaborates with industry leaders Daddy D Pro and Dr. Frigz International from the Sialkot surgical cluster.

Result: The paper develops automated defect detection methodologies for identifying critical defects such as cracks, rust, and structural irregularities in surgical instruments. The approach enables quality assurance through Automated Optical Inspection (AOI) tools.

Conclusion: The research contributes to enhancing surgical instrument standards by integrating industry expertise with cutting-edge deep learning technology, ultimately creating a more reliable healthcare environment through improved quality control in manufacturing.
...
### [39] [CylinderSplat: 3D Gaussian Splatting with Cylindrical Triplanes for Panoramic Novel View Synthesis](https://arxiv.org/abs/2603.05882)
*Qiwei Wang,Xianghui Ze,Jingyi Yu,Yujiao Shi*

Main category: cs.CV

TL;DR: CylinderSplat introduces a feed-forward framework for panoramic 3D Gaussian Splatting using cylindrical Triplane representation to handle sparse-view panoramic scenes better than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods struggle with panoramic imagery due to poor handling of occlusions in sparse-view scenarios and distortion from standard Cartesian Triplane representations that don't align well with 360° scene geometry.

Method: Proposes CylinderSplat with cylindrical Triplane representation aligned with panoramic data and Manhattan-world assumption. Uses dual-branch architecture: pixel-based branch for well-observed regions and volume-based branch with cylindrical Triplane for occluded/sparse areas.
...
Conclusion: CylinderSplat effectively addresses limitations of existing panoramic 3DGS methods through cylindrical representation and dual-branch architecture, enabling flexible handling of variable input views from single to multiple panoramas.
...
### [40] [PixARMesh: Autoregressive Mesh-Native Single-View Scene Reconstruction](https://arxiv.org/abs/2603.05888)
*Xiang Zhang,Sohyun Yoo,Hongrui Wu,Chuan Li,Jianwen Xie,Zhuowen Tu*

Main category: cs.CV

TL;DR: PixARMesh is an autoregressive method that reconstructs complete 3D indoor scene meshes from a single RGB image, producing artist-ready meshes in one forward pass without post-processing.


<details>
  <summary>Details</summary>
Motivation: Prior methods for 3D scene reconstruction rely on implicit signed distance fields and separate layout optimization, which can be complex and require post-processing. There's a need for a unified approach that directly produces coherent, high-quality meshes from single images.

Method: The method builds on mesh generative models, using a point-cloud encoder augmented with pixel-aligned image features and global scene context via cross-attention. Scenes are generated autoregressively from a unified token stream containing context, pose, and mesh information.

Result: Experiments on synthetic and real-world datasets show PixARMesh achieves state-of-the-art reconstruction quality while producing lightweight, high-quality meshes ready for downstream applications.

Conclusion: PixARMesh successfully demonstrates that joint prediction of object layout and geometry in a unified autoregressive model can produce coherent, artist-ready 3D scene meshes directly from single RGB images, outperforming previous approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing ISTS forecasting methods rely solely on historical observations, lacking contextual semantics and fine-grained temporal pattern learning. Real-world ISTS data has asynchronous observations on uneven intervals across variables.

Method: Two-stage encoding: 1) Cross-modal vision-text encoding generates informative images/text for temporal patterns using MLLMs, 2) ISTS encoding extracts enriched temporal features. Includes adaptive query-based feature extractor to compress MLLM tokens and multimodal alignment with modality-aware gating.
...
### [42] [InnoAds-Composer: Efficient Condition Composition for E-Commerce Poster Generation](https://arxiv.org/abs/2603.05898)
...
TL;DR: InnoAds-Composer: A single-stage diffusion framework for e-commerce poster generation with efficient tri-conditional control over subject, text, and style, addressing poor fidelity, inaccurate text, and inconsistent style issues in multi-stage pipelines.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models for e-commerce poster generation rely on multi-stage pipelines with three key issues: poor subject fidelity, inaccurate text rendering, and inconsistent style. Simultaneous control over subject, text, and style remains underexplored, and naive multi-stage approaches introduce quadratic overhead from token concatenation.

Method: Proposes InnoAds-Composer, a single-stage framework with efficient tri-conditional control tokens. Uses importance analysis over layers and timesteps to route each condition only to responsive positions, shortening active token sequences. Includes Text Feature Enhancement Module (TFEM) for Chinese text rendering by integrating glyph image and crop features. Also constructs a new high-quality e-commerce poster dataset with joint subject, text, and style conditions.

Result: Extensive experiments show InnoAds-Composer significantly outperforms existing product poster methods without obvious inference latency increase. The framework effectively addresses the three key issues while maintaining efficiency.

Conclusion: InnoAds-Composer provides an effective single-stage solution for e-commerce poster generation with simultaneous tri-conditional control, overcoming limitations of multi-stage pipelines while maintaining computational efficiency through intelligent token routing and text feature enhancement.
...
### [43] [Mitigating Bias in Concept Bottleneck Models for Fair and Interpretable Image Classification](https://arxiv.org/abs/2603.05899)
...
<details>
  <summary>Details</summary>
Motivation: Concept bottleneck models (CBMs) theoretically support fairness by masking sensitive attributes, but in practice they leak information and show only marginal bias reduction, requiring better debiasing methods.

Method: Three techniques: 1) top-k concept filter to reduce information leakage, 2) removal of biased concepts, and 3) adversarial debiasing applied to CBMs.

Result: Outperforms prior work in fairness-performance tradeoffs, providing significant improvement towards fair and interpretable image classification.

Conclusion: The proposed debiased CBM represents a significant step forward for achieving both fairness and interpretability in image classification systems.
...
TL;DR: RepKAN combines CNNs with Kolmogorov-Arnold Networks (KANs) for interpretable remote sensing image classification, achieving SOTA performance while providing explicit physical reasoning.


<details>
  <summary>Details</summary>
Motivation: Standard CNNs and Transformers for remote sensing image classification are uninterpretable black-boxes, lacking explicit physical reasoning about spectral features and interactions.

Method: RepKAN integrates CNN structural efficiency with KAN non-linear power using a dual-path design: Spatial Linear path and Spectral Non-linear path to discover class-specific spectral fingerprints and physical interaction manifolds.

Result: Outperforms state-of-the-art models on EuroSAT and NWPU-RESISC45 datasets while providing explicit physically interpretable reasoning about spectral features.

Conclusion: RepKAN has significant potential as a backbone for future interpretable visual foundation models, bridging the gap between performance and interpretability in remote sensing.
...
### [45] [CollabOD: Collaborative Multi-Backbone with Cross-scale Vision for UAV Small Object Detection](https://arxiv.org/abs/2603.05905)
*Xuecheng Bai,Yuxiang Wang,Chuanzhi Xu,Boyu Hu,Kang Han,Ruijie Pan,Xiaowei Niu,Xiaotian Guan,Liqiang Fu,Pengfei Ye*

Main category: cs.CV

TL;DR: CollabOD is a lightweight collaborative detection framework for small object detection in UAV imagery that preserves structural details and aligns features before multi-scale fusion.


<details>
  <summary>Details</summary>
Motivation: Small object detection in UAV imagery faces challenges due to scale variation, structural detail degradation, and limited computational resources. In high-altitude scenarios, fine-grained features are weakened during hierarchical downsampling and cross-scale fusion, leading to unstable localization and reduced robustness.

Method: Proposes CollabOD framework with three key strategies: 1) Structural Detail Preservation, 2) Cross-Path Feature Alignment, and 3) Localization-Aware Lightweight Design. It integrates these approaches to optimize conventional UAV perception models from image processing, channel structure, and lightweight design perspectives. Includes a unified detail-aware detection head for improved regression robustness.

Result: The framework enhances representation stability while maintaining efficient inference. The detail-aware detection head improves regression robustness without adding deployment overhead.

Conclusion: CollabOD effectively addresses small object detection challenges in UAV imagery by preserving structural details and aligning heterogeneous features, achieving better localization stability and robustness while remaining lightweight for practical deployment.
...
TL;DR: VLMs fail on simple driving-relevant visual questions despite having reasoning capabilities; researchers analyze intermediate activations to identify two failure modes: perceptual (visual info not linearly encoded) and cognitive (info present but misaligned with language).


<details>
  <summary>Details</summary>
Motivation: VLMs are increasingly used in automated driving for handling long-tail scenarios, but they often fail on simple visual questions relevant to driving. The reasons for these failures remain poorly understood, prompting investigation into how visual information flows through these models.

Method: Researchers examine intermediate activations of four SOTA VLMs, create counterfactual image sets that differ only in targeted visual concepts, and train linear probes to distinguish between them. They assess which concepts are linearly encoded versus implicitly encoded through spatial structure.

Result: Concepts like object presence are explicitly linearly encoded, while spatial concepts like orientation are only implicitly encoded. Two failure modes identified: perceptual failure (visual info not linearly encoded) and cognitive failure (info present but misaligned with language). Object distance degrades linear separability.

Conclusion: The study improves understanding of VLM failures on simple driving-relevant visual tasks by identifying specific bottlenecks in visual information flow and distinguishing between perceptual and cognitive failure modes, with implications for improving VLM reliability in automated driving applications.
...
### [47] [Beyond Geometry: Artistic Disparity Synthesis for Immersive 2D-to-3D](https://arxiv.org/abs/2603.05906)
...
TL;DR: Art3D proposes artistic disparity synthesis for 2D-to-3D conversion, focusing on replicating cinematic artistic intent rather than geometric accuracy.


<details>
  <summary>Details</summary>
Motivation: Current 2D-to-3D conversion methods achieve geometric accuracy but fail artistically, missing the immersive experience of professional 3D cinema by mistaking artistic intent (like zero-plane shifts for pop-out effects) for data noise.

Method: Art3D uses a dual-path architecture to decouple global depth parameters (macro-intent) from local artistic effects (visual brushstrokes), learning from professional 3D film data via indirect supervision.

Result: Experiments show Art3D demonstrates potential in replicating key local out-of-screen effects and aligning with global depth styles of cinematic 3D content.

Conclusion: The work lays groundwork for a new class of artistically-driven conversion tools through the Artistic Disparity Synthesis paradigm.
...
TL;DR: TempoSyncDiff is a distilled latent diffusion framework for efficient audio-driven talking-head generation that reduces inference latency while maintaining quality through teacher-student distillation, identity anchoring, and temporal regularization.
...
Method: Reference-conditioned latent diffusion framework with teacher-student distillation: a diffusion teacher trained with standard noise prediction guides a lightweight student denoiser for few-step inference. Incorporates identity anchoring and temporal regularization to mitigate identity drift and flicker, plus viseme-based audio conditioning for coarse lip motion control.

Result: Experiments on LRS3 dataset show distilled models retain much reconstruction behavior of stronger teacher while enabling substantially lower latency inference. Component-level metrics relative to VAE reconstructions and preliminary latency characterization (CPU-only and edge computing) suggest feasibility for edge deployment.

Conclusion: Distilled diffusion models can achieve practical talking-head generation under constrained computational settings, representing an initial step toward efficient diffusion-based synthesis with reduced latency and improved stability.
...
### [49] [Pano3DComposer: Feed-Forward Compositional 3D Scene Generation from Single Panoramic Image](https://arxiv.org/abs/2603.05908)
*Zidian Qiu,Ancong Wu*

Main category: cs.CV

TL;DR: Pano3DComposer is a fast feed-forward framework that generates complete 360-degree 3D scenes from panoramic images by decoupling object generation from layout estimation using a novel Object-World Transformation Predictor.


<details>
  <summary>Details</summary>
Motivation: Current methods for compositional image-to-3D scene generation are slow (iterative optimization) or inflexible (joint generation), and limited to narrow field-of-view images, preventing creation of complete 360-degree environments.

Method: 1) Object-World Transformation Predictor: Adapts VGGT to Alignment-VGGT using object crops, multi-view renderings, and camera parameters to transform 3D objects from local to world coordinates. 2) Coarse-to-Fine alignment: Iteratively refines geometric consistency for unseen domains using scene rendering feedback. 3) Uses pseudo-geometric supervision to handle shape discrepancies between generated and ground-truth objects.

Result: Achieves superior geometric accuracy on synthetic and real-world datasets for image/text-to-3D tasks, generating high-fidelity 3D scenes in approximately 20 seconds on RTX 4090 GPU.

Conclusion: Pano3DComposer provides an efficient, feed-forward solution for panoramic 3D scene generation that overcomes limitations of existing methods through decoupled object-layout processing and novel transformation prediction.
...
TL;DR: Proposes EC-TFG task for generating talking faces with continuous emotional changes, and TIE-TFG model that uses temporal-intensive emotion fluctuation modeling to create smooth emotion transitions in synthetic videos.


<details>
  <summary>Details</summary>
Motivation: Existing talking face generation methods produce fixed target emotions in videos, lacking the ability to show continuously changing natural expressions like humans do when conveying information.

Method: Introduces Temporal-Intensive Emotion Modulated Talking Face Generation (TIE-TFG) with Temporal-Intensive Emotion Fluctuation Modeling that generates emotion variation sequences corresponding to input text to drive continuous facial expression changes.
...
Conclusion: The proposed EC-TFG task and TIE-TFG model successfully address the limitation of fixed emotional expressions in talking face generation, enabling realistic videos with continuous emotional variations.
...
### [51] [Place-it-R1: Unlocking Environment-aware Reasoning Potential of MLLM for Video Object Insertion](https://arxiv.org/abs/2603.06140)
...
<details>
  <summary>Details</summary>
Motivation: Current video editing techniques prioritize visual fidelity over physical causality, resulting in physically inconsistent object insertions that don't interact properly with their environment.

Method: Uses MLLM Chain-of-Thought reasoning to understand physical scenes and infer valid insertion regions, guides video diffusion with environment-aware tokens, implements MLLM-guided Spatial DPO for visual naturalness, and creates closed-loop refinement cycles with two user modes (plausibility-oriented flexible and fidelity-oriented standard).
...
Conclusion: The framework successfully bridges cognitive reasoning and generative execution for environment-aware video editing, offering users explicit control over the plausibility-fidelity trade-off through its dual-mode design.
...
### [52] [Towards Driver Behavior Understanding: Weakly-Supervised Risk Perception in Driving Scenes](https://arxiv.org/abs/2603.05926)
...
Method: Introduce RAID dataset with diverse traffic scenario annotations, then propose a weakly supervised risk object identification framework that models relationship between driver's intended maneuver and responses to identify risk sources.
...
TL;DR: VLM-RobustBench evaluates vision-language models on 49 image distortions across 133 settings, finding VLMs are semantically strong but spatially fragile, with geometric perturbations causing largest performance drops.


<details>
  <summary>Details</summary>
Motivation: While VLMs perform well on standard datasets, we lack understanding of their robustness to real-world image distortions. The paper aims to systematically evaluate VLM performance across diverse corruption types and severities.

Method: Created VLM-RobustBench with 49 augmentation types across noise, blur, weather, digital, and geometric perturbations, evaluated under graded severities and binary transforms (133 settings). Tested VLMs from four families (Qwen, InternVL, Molmo, Gemma) on MMBench (visually grounded) and MMMU-Pro (reasoning-oriented) benchmarks.

Result: Visual severity poorly predicts difficulty - low-severity spatial perturbations degrade performance more than severe photometric corruptions. Glass_blur (low severity) reduces MMBench accuracy by ~8pp average. Largest drops from resampling and geometric distortions (upsample, elastic_transform) up to 34pp. Current VLMs are semantically strong but spatially fragile.

Conclusion: Findings motivate new robustness evaluation protocols and training regimes emphasizing resampling and geometric invariances, as current VLMs show significant vulnerability to spatial perturbations despite strong semantic understanding.
...
### [54] [Beyond Static Frames: Temporal Aggregate-and-Restore Vision Transformer for Human Pose Estimation](https://arxiv.org/abs/2603.05929)
*Hongwei Fang,Jiahang Cai,Xun Wang,Wenwu Yang*

Main category: cs.CV

TL;DR: TAR-ViTPose: A temporal Vision Transformer for video pose estimation that aggregates temporal cues across frames to improve accuracy and stability over single-frame methods.


<details>
  <summary>Details</summary>
Motivation: Existing ViT-based pose estimators process each frame independently, ignoring temporal coherence in videos, leading to unstable predictions in challenging scenes with motion blur, occlusion, or defocus.

Method: Proposes TAR-ViTPose with two key components: 1) Joint-centric temporal aggregation (JTA) using learnable query tokens for each joint to attend to corresponding regions across frames, and 2) Global restoring attention (GRA) to restore aggregated temporal features back into current frame tokens while preserving global context.

Result: Achieves +2.3 mAP gain over single-frame ViTPose baseline on PoseTrack2017, outperforms existing state-of-the-art video-based methods, and achieves higher runtime frame rate in real-world applications.

Conclusion: TAR-ViTPose effectively leverages temporal information in videos for more robust and accurate 2D human pose estimation while maintaining computational efficiency.
...
TL;DR: RF-Sampling is a training-free inference enhancement framework for flow matching text-to-image models like FLUX that improves generation quality and prompt alignment through reflective sampling and flow inversion.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time enhancement strategies work well for conventional diffusion models but fail on flow models like FLUX, creating a gap that needs to be addressed for flow-based text-to-image generation.

Method: Reflective Flow Sampling (RF-Sampling) uses a theoretically-grounded approach with formal derivation showing it performs gradient ascent on text-image alignment score. It combines textual representations with flow inversion to explore noise spaces more consistent with input prompts.

Result: Extensive experiments show RF-Sampling consistently improves generation quality and prompt alignment across multiple benchmarks. It's the first inference enhancement method to exhibit test-time scaling ability on FLUX models.

Conclusion: RF-Sampling successfully bridges the gap for inference enhancement in flow models, providing a training-free solution that improves both quality and alignment while offering theoretical grounding and practical benefits for CFG-distilled flow models.
...
### [56] [FTSplat: Feed-forward Triangle Splatting Network](https://arxiv.org/abs/2603.05932)
*Xiong Jinlin,Li Can,Shen Jiawei,Qi Zhigang,Sun Lei,Zhao Dongyang*

Main category: cs.CV

TL;DR: A feed-forward framework that directly predicts continuous triangle surfaces from multi-view images for simulation-ready 3D reconstruction without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Current 3D reconstruction methods like NeRF and 3DGS require time-consuming per-scene optimization and lack explicit, manifold geometry needed for direct simulation in robotics and graphics applications.

Method: Proposes a feed-forward framework with pixel-aligned triangle generation module and relative 3D point cloud supervision to directly predict continuous triangle surfaces from calibrated multi-view images in a single forward pass.

Result: Achieves efficient reconstruction while maintaining seamless compatibility with standard graphics and robotic simulators, producing simulation-ready models without per-scene optimization or post-processing.

Conclusion: The method enables real-time deployment of high-fidelity 3D reconstruction for robotics and simulation by providing explicit triangle geometry in a feed-forward manner, overcoming limitations of optimization-based approaches.
...
TL;DR: Two-stage framework for learning glyph similarities: supervised contrastive learning on invented alphabets, then teacher-student distillation to historical scripts for unsupervised discovery of cross-script relationships.


<details>
  <summary>Details</summary>
Motivation: Learning similarity metrics for glyphs and writing systems is challenging because while individual graphemes in invented alphabets can be reliably labeled, historical relationships between different scripts remain uncertain and contested.
...
Conclusion: The approach bridges supervised contrastive learning and unsupervised discovery, enabling both hard boundaries between distinct systems and soft similarities reflecting potential historical influences.
...
### [58] [OD-RASE: Ontology-Driven Risk Assessment and Safety Enhancement for Autonomous Driving](https://arxiv.org/abs/2603.05936)
*Kota Shimomura,Masaki Nambata,Atsuya Ishikawa,Ryota Mimura,Takayuki Kawabuchi,Takayoshi Yamashita,Koki Inoue*

Main category: cs.CV

TL;DR: OD-RASE is a framework that uses ontology and large visual language models to proactively identify accident-prone road structures and generate infrastructure improvement proposals for autonomous driving safety.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems have limitations in handling rare situations and complex road structures. Road infrastructure improvements are typically reactive (after accidents), but autonomous systems need proactive risk mitigation.

Method: 1) Formalize ontology based on road traffic domain knowledge; 2) Generate infrastructure improvement proposals using LVLM; 3) Use ontology-driven data filtering for reliability; 4) Automatically annotate proposals on pre-accident images to build dataset; 5) Develop OD-RASE model combining LVLM and diffusion model for proposals and generated improved road images.

Result: Ontology-driven data filtering enables highly accurate prediction of accident-causing road structures and corresponding improvement plans. The framework contributes to safer traffic environments.

Conclusion: OD-RASE represents an important step toward broader adoption of autonomous driving by enabling proactive safety improvements through systematic detection of risky road structures and infrastructure enhancement proposals.
...
TL;DR: CoE is a training-free multimodal summarization framework that uses a Chain-of-Events guided by a Hierarchical Event Graph to address challenges in domain-specific supervision, weak cross-modal grounding, and flat temporal modeling.
...
TL;DR: TaPD is a plug-and-play framework for trajectory prediction that handles variable history lengths using progressive knowledge distillation and temporal backfilling.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory predictors assume fixed-length histories and degrade with variable or extremely short observations in real-world scenarios (occlusions, limited sensing).

Method: Two cooperative modules: Observation-Adaptive Forecaster (OAF) with progressive knowledge distillation to transfer knowledge from long to short observations, and Temporal Backfilling Module (TBM) to reconstruct missing historical segments. Uses cosine-annealed distillation weighting and decoupled pretrain-reconstruct-finetune protocol.

Result: Outperforms strong baselines across all observation lengths on Argoverse 1 and 2, with especially large gains under very short inputs. Works as plug-and-play improvement for other predictors like HiVT.

Conclusion: TaPD provides a unified, observation-adaptive solution for trajectory forecasting that handles variable history lengths effectively through progressive distillation and explicit temporal backfilling.
...
### [61] [SLER-IR: Spherical Layer-wise Expert Routing for All-in-One Image Restoration](https://arxiv.org/abs/2603.05940)
*Peng Shurui,Xin Lin,Shi Luo,Jincen Ou,Dizhe Zhang,Lu Qi,Truong Nguyen,Chao Ren*

Main category: cs.CV

TL;DR: SLER-IR is a unified image restoration framework using spherical layer-wise expert routing to handle diverse degradations by dynamically activating specialized experts across network layers.


<details>
  <summary>Details</summary>
Motivation: Image restoration under diverse degradations is challenging for unified frameworks due to feature interference and insufficient expert specialization. Existing approaches struggle with degradation representation bias and spatially non-uniform degradations.

Method: Proposes SLER-IR with three key components: 1) Spherical Uniform Degradation Embedding using contrastive learning to map degradation representations onto a hypersphere, eliminating geometry bias; 2) Layer-wise expert routing that dynamically activates specialized experts; 3) Global-Local Granularity Fusion module to integrate global semantics and local degradation cues for spatially non-uniform degradations.

Result: Experiments on three-task and five-task benchmarks show consistent improvements over state-of-the-art methods in both PSNR and SSIM metrics.

Conclusion: SLER-IR effectively addresses challenges in unified image restoration by eliminating degradation representation bias, enabling specialized expert activation, and handling spatially non-uniform degradations through granularity fusion.
...
### [62] [GazeMoE: Perception of Gaze Target with Mixture-of-Experts](https://arxiv.org/abs/2603.06256)
*Zhuangzhuang Dai,Zhongxi Lu,Vincent G. Zakka,Luis J. Manso,Jose M Alcaraz Calero,Chen Li*

Main category: cs.CV

TL;DR: GazeMoE: A Mixture-of-Experts framework that adaptively leverages gaze-related cues from frozen foundation models to achieve state-of-the-art gaze target estimation.


<details>
  <summary>Details</summary>
Motivation: Gaze target estimation is critical for robots to understand human attention, but current methods struggle with integrating multi-modal cues (eyes, head poses, gestures, context) and achieving generalizability. While foundation models offer promise, they need adaptive decoding mechanisms to selectively leverage relevant gaze cues.

Method: Proposes GazeMoE, an end-to-end framework using Mixture-of-Experts modules to selectively extract gaze-target-related cues from a frozen foundation model. Incorporates class-balancing auxiliary loss to address in-frame vs. out-of-frame class imbalance, and uses strategic data augmentations including region-specific cropping and photometric transformations.

Result: Extensive experiments on benchmark datasets show GazeMoE achieves state-of-the-art performance, outperforming existing methods on challenging gaze estimation tasks.

Conclusion: GazeMoE successfully integrates Mixture-of-Experts with foundation models for gaze target estimation, demonstrating superior performance through adaptive cue selection and robust training strategies. The framework is publicly available with code and pre-trained models.
...
### [63] [Adaptive Radial Projection on Fourier Magnitude Spectrum for Document Image Skew Estimation](https://arxiv.org/abs/2603.05942)
*Luan Pham,Phu Hao Hoang,Xuan Toan Mai,Tuan Anh Tran*

Main category: cs.CV

TL;DR: Proposes a novel skew estimation method using Adaptive Radial Projection on Fourier spectrum, introduces DISE-2021 dataset, and shows superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Skew estimation is crucial for document processing systems, especially for scanned documents, as it directly impacts subsequent processing steps. There's a need for robust methods in the digitization age.

Method: Uses Adaptive Radial Projection on the 2D Discrete Fourier Magnitude spectrum to extract dominant skew angle from document images.

Result: The proposed method is robust, reliable, and outperforms all compared methods in comprehensive analyses.

Conclusion: The method provides effective skew estimation, supported by the new DISE-2021 dataset for evaluation, with open-source implementation available.
...
<details>
  <summary>Details</summary>
Motivation: Pruning VLMs is challenging because compression affects both task utility and visual grounding, often amplifying object hallucinations even at the same sparsity level. Existing methods don't adequately address VLM-specific failure modes like hallucination.

Method: Hierarchical preference-conditioned structured pruning framework that makes plan-level decisions: a single policy outputs global pruning blueprint by factorizing decisions into overall sparsity budget and layer-wise allocation. Uses visual sensitivity signal from attention flow between vision tokens and language states, and optimizes with plan-level Group Relative Policy Optimization under multi-objective return.

Result: Experiments on LLaVA with POPE and ScienceQA show HiPP-Prune discovers diverse non-dominated pruning plans and provides controllable robustness-utility trade-offs under matched sparsity budgets.

Conclusion: HiPP-Prune effectively addresses VLM pruning challenges by enabling preference-conditioned resource allocation with consideration of visual grounding and hallucination robustness, offering practical trade-off control for efficient deployment.
...
### [65] [LucidNFT: LR-Anchored Multi-Reward Preference Optimization for Generative Real-World Super-Resolution](https://arxiv.org/abs/2603.05947)
*Song Fei,Tian Ye,Sixiang Chen,Zhaohu Xing,Jianyu Lai,Lei Zhu*

Main category: cs.CV

TL;DR: LucidNFT is a multi-reward RL framework for flow-matching real-world image super-resolution that addresses hallucination issues through degradation-robust faithfulness evaluation and improved optimization.


<details>
  <summary>Details</summary>
Motivation: Real-ISR suffers from semantic/structural hallucination where outputs look sharp but are unfaithful to LR evidence, and existing RL approaches face three key challenges: lack of degradation-robust faithfulness signals, rollout-group optimization bottlenecks causing advantage collapse, and limited real degradation coverage.

Method: Proposes LucidNFT with three components: 1) LucidConsistency - degradation-robust semantic evaluator for LR-anchored faithfulness measurement, 2) decoupled advantage normalization to preserve objective-wise contrasts and prevent advantage collapse, and 3) LucidLR - large-scale collection of real-world degraded images for robust RL fine-tuning.
...
Conclusion: The proposed framework effectively addresses key challenges in Real-ISR alignment through robust faithfulness evaluation, improved RL optimization, and comprehensive real degradation coverage, enabling more reliable and faithful super-resolution results.
...
### [66] [DEX-AR: A Dynamic Explainability Method for Autoregressive Vision-Language Models](https://arxiv.org/abs/2603.06302)
*Walid Bousselham,Angie Boggust,Hendrik Strobelt,Hilde Kuehne*

Main category: cs.CV

TL;DR: DEX-AR is a novel explainability method for autoregressive VLMs that generates per-token and sequence-level 2D heatmaps to visualize which image regions influence textual responses, addressing challenges of complex token-by-token generation and multimodal interactions.
...
Method: DEX-AR computes layer-wise gradients with respect to attention maps during token generation, featuring dynamic head filtering to identify visually-focused attention heads and sequence-level filtering to aggregate per-token explanations while distinguishing visually-grounded from linguistic tokens.

Result: Evaluation on ImageNet, VQAv2, and PascalVOC shows consistent improvement in both perturbation-based metrics (using novel normalized perplexity measure) and segmentation-based metrics compared to existing methods.

Conclusion: DEX-AR successfully addresses the explainability challenges of autoregressive VLMs by providing both per-token and sequence-level visual explanations, enabling better understanding of how these models integrate visual and textual information during generation.
...
TL;DR: K-MaT is a prompt-learning framework that transfers biomedical VLMs from high-end to low-end imaging modalities without low-end training data, using knowledge-anchored prompts and optimal transport for cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Biomedical VLMs adapted to high-end imaging (CT) fail to transfer to frontline low-end modalities (radiography) due to modality-specific shortcuts, creating a deployment gap for medical applications.

Method: K-MaT factorizes prompts, anchors them to clinical text descriptions, and aligns low-end prompt manifolds to visually-grounded high-end spaces using Fused Gromov-Wasserstein optimal transport, enabling zero-shot cross-modal transfer.

Result: State-of-the-art results on four cross-modal benchmarks: average harmonic mean accuracy 44.1% (vs BiomedCoOp's 42.0%), macro-F1 36.2%. Notably mitigates catastrophic forgetting on breast imaging (vs CoOp's 27.0% accuracy drop).

Conclusion: Aligning prompt manifolds via optimal transport provides an effective route for zero-shot cross-modal deployment of medical VLMs, enabling robust performance across imaging modalities without low-end training data.
...
### [68] [Unify the Views: View-Consistent Prototype Learning for Few-Shot Segmentation](https://arxiv.org/abs/2603.05952)
*Hongli Liu,Yu Wang,Shengjie Zhao*

Main category: cs.CV

TL;DR: VINE introduces a view-informed network for few-shot segmentation that jointly models structural consistency and foreground discrimination to refine class-specific prototypes, addressing challenges of structural misalignment and cross-view inconsistency.


<details>
  <summary>Details</summary>
Motivation: Few-shot segmentation struggles with structural misalignment and cross-view inconsistency under large appearance or viewpoint variations, limiting its generalization to novel classes with limited supervision.

Method: VINE uses a spatial-view graph on backbone features (spatial graph for local geometry, view graph for cross-perspective connections) and derives a discriminative prior from support-query feature discrepancy to capture category-specific contrast. It progressively integrates foreground-enhanced SAM features and structurally enriched ResNet features through masked cross-attention.
...
Conclusion: VINE provides a unified framework that successfully addresses structural misalignment and cross-view inconsistency in few-shot segmentation by jointly modeling structural consistency and foreground discrimination.
...
### [69] [Dynamic Chunking Diffusion Transformer](https://arxiv.org/abs/2603.06351)
*Akash Haridas,Utkarsh Saxena,Parsa Ashrafi Fashi,Mehdi Rezagholizadeh,Vikram Appia,Emad Barsoum*

Main category: cs.CV

TL;DR: DC-DiT improves diffusion transformers by adaptively compressing images into variable-length token sequences based on visual complexity and diffusion timestep, reducing compute while improving quality.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion transformers use fixed-length token sequences with uniform compute across all image regions, ignoring that images have varying detail levels and that denoising progresses from coarse to fine structure.

Method: Adds a learned encoder-router-decoder scaffold to DiT that adaptively compresses 2D input into variable-length token sequences using chunking mechanism learned end-to-end with diffusion training. Compresses uniform regions into fewer tokens and detail-rich regions into more tokens.

Result: Consistently improves FID and Inception Score over parameter-matched and FLOP-matched DiT baselines on ImageNet 256×256 across 4× and 16× compression. Can be upcycled from pretrained DiT checkpoints with minimal training and composes with other dynamic computation methods.

Conclusion: DC-DiT is a promising technique for adaptive compute allocation in diffusion models that improves both quality and efficiency, with potential applications to pixel-space, video, and 3D generation.
...
### [70] [OVGGT: O(1) Constant-Cost Streaming Visual Geometry Transformer](https://arxiv.org/abs/2603.05959)
*Si-Yu Lu,Po-Ting Chen,Hui-Che Hsu,Sin-Ye Jhong,Wen-Huang Cheng,Yung-Yao Chen*

Main category: cs.CV

TL;DR: OVGGT enables streaming 3D reconstruction from arbitrarily long videos with constant memory/compute budgets via selective KV cache compression and anchor protection.


<details>
  <summary>Details</summary>
Motivation: Existing geometric foundation models have quadratic attention costs limiting them to short offline sequences, while causal-attention variants accumulate unbounded KV cache that exhausts GPU memory within hundreds of frames, preventing long-horizon streaming deployment.

Method: Training-free framework combining Self-Selective Caching (uses FFN residual magnitudes to compress KV cache while maintaining FlashAttention compatibility) and Dynamic Anchor Protection (shields coordinate-critical tokens from eviction to suppress geometric drift).
...
Conclusion: OVGGT enables practical long-horizon streaming 3D reconstruction by bounding both memory and compute to fixed budgets regardless of sequence length, overcoming limitations of previous approaches.
...
### [71] [Prompt Group-Aware Training for Robust Text-Guided Nuclei Segmentation](https://arxiv.org/abs/2603.06384)
*Yonghuang Wu,Zhenyang Liang,Wenwen Zeng,Xuan Xie,Jinhua Yu*

Main category: cs.CV

TL;DR: A method to reduce prompt sensitivity in text-guided medical image segmentation by treating semantically equivalent prompts as groups and enforcing consistency through quality-guided regularization and logit-level constraints.
...
Method: Reformulates prompt sensitivity as a group-wise consistency problem, organizing semantically related prompts into prompt groups sharing the same ground-truth mask. Uses a prompt group-aware training framework with: (1) quality-guided group regularization using segmentation loss as implicit ranking signal, and (2) logit-level consistency constraint with stop-gradient strategy to align predictions within each group.

Result: Extensive experiments on multi-dataset nuclei benchmarks show consistent gains under textual prompting and markedly reduced performance variance across prompt quality levels. On six zero-shot cross-dataset tasks, the method improves Dice by an average of 2.16 points.

Conclusion: The approach demonstrates improved robustness and generalization for vision-language segmentation in computational pathology without requiring architectural modifications or changes to inference.
...
### [72] [Exploring Open-Vocabulary Object Recognition in Images using CLIP](https://arxiv.org/abs/2603.05962)
*Wei Yu Chen,Ying Dai*

Main category: cs.CV

TL;DR: Proposes a two-stage open-vocabulary object recognition framework using object segmentation followed by recognition, with CLIP-based or CNN/MLP-based encoding and SVD for shared representation space.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing open-vocabulary object recognition methods: high system complexity, substantial training costs, and limited generalization capabilities.

Method: Two-stage strategy: 1) object segmentation, 2) recognition. Two encoding approaches: CLIP-based (object-level image + category-level text embeddings) and CNN/MLP-based (CNN feature maps + MLP alignment). Embeddings concatenated and processed via SVD for shared representation space. Recognition via embedding similarity matching.
...
Conclusion: Proposed framework eliminates need for complex retraining and labor-intensive annotation while maintaining strong performance. CLIP-based approach works best, but CNN/MLP-based method offers promising alternative with reduced CLIP dependency.
...
### [73] [Physical Simulator In-the-Loop Video Generation](https://arxiv.org/abs/2603.06408)
*Lin Geng Foo,Mark He Huang,Alexandros Lattas,Stylianos Moschoglou,Thabo Beeler,Christian Theobalt*

Main category: cs.CV

TL;DR: PSIVG integrates physical simulation into video diffusion to generate physically plausible videos by correcting motion trajectories and optimizing texture consistency.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based video generation produces visually realistic results but often violates basic physical laws like gravity, inertia, and collision, limiting realism and reliability.

Method: PSIVG reconstructs 4D scenes and object meshes from template videos, initializes them in a physical simulator to generate consistent trajectories, then guides video generation with these trajectories. Includes TTCO for texture consistency optimization.

Result: Comprehensive experiments show PSIVG produces videos that better adhere to real-world physics while preserving visual quality and diversity compared to existing methods.

Conclusion: Integrating physical simulation into video diffusion generation significantly improves physical plausibility while maintaining visual realism, addressing a key limitation in current video generation approaches.
...
### [74] [CLoPA: Continual Low Parameter Adaptation of Interactive Segmentation for Medical Image Annotation](https://arxiv.org/abs/2603.06426)
*Parhom Esmaeili,Chayanin Tangwiriyasakul,Eli Gibson,Sebastien Ourselin,M. Jorge Cardoso*

Main category: cs.CV

TL;DR: CLoPA: Continual adaptation strategy that tunes a small fraction of nnInteractive's parameters on annotation cache to rapidly achieve expert-level segmentation performance across diverse medical imaging tasks.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot models like nnInteractive fail to consistently reach expert-level performance across diverse medical imaging tasks, and annotation campaigns produce growing streams of task-specific labeled data that could be leveraged for online adaptation.

Method: CLoPA is a continual adaptation strategy that tunes a small fraction of nnInteractive's parameters on the annotation cache, triggered by lightweight episode scheduling. It requires no new parameters or changes to the inference pipeline and operates within existing annotation workflows.
...
Conclusion: CLoPA effectively bridges the gap between zero-shot inference and expert-level performance through continual adaptation, though for targets with complex geometries, deeper feature-representation alignment may be needed beyond instance normalization and low-level feature tuning.
...
### [75] [CR-QAT: Curriculum Relational Quantization-Aware Training for Open-Vocabulary Object Detection](https://arxiv.org/abs/2603.05964)
*Jinyeong Park,Donghwa Kim,Brent ByungHoon Kang,Hyeongboo Baek,Jibum Kim*

Main category: cs.CV

TL;DR: CR-QAT framework combines curriculum QAT and text-centric relational KD to enable effective 4-bit quantization for open-vocabulary object detection while preserving vision-language alignment.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary object detection models are too large for resource-constrained devices, but naive extreme low-bit quantization (e.g., 4-bit) severely degrades vision-language alignment and relational structures.

Method: Proposes curriculum relational quantization-aware training (CR-QAT) with two components: curriculum QAT (CQAT) for progressive quantization with error isolation, and text-centric relational KD (TRKD) that transfers teacher's relational knowledge using text-anchored pairwise similarity matrices.

Result: CR-QAT outperforms existing QAT baselines on LVIS and COCO zero-shot benchmarks under aggressive low-bit settings, achieving relative AP improvements of up to 38.9% and 40.9% respectively.

Conclusion: The integrated CR-QAT framework effectively enables extreme low-bit quantization for OVOD while preserving fine-grained vision-language alignment and relational structures through progressive optimization and comprehensive knowledge distillation.
...
### [76] [Do Foundation Models Know Geometry? Probing Frozen Features for Continuous Physical Measurement](https://arxiv.org/abs/2603.06459)
*Yakov Pyotr Shkolnikov*

Main category: cs.CV

TL;DR: Vision-language models encode geometric information that their text pathways fail to express, creating a 3.3x bottleneck. Training objective matters more than architecture, with different encoders achieving similar functional accuracy despite low representational similarity.


<details>
  <summary>Details</summary>
Motivation: To understand why vision-language models struggle to express geometric information through text pathways despite encoding it in their visual features, and to investigate the relationship between training objectives, architecture, and geometric representation fidelity.

Method: Used linear probes (6,000 parameters) to extract hand joint angles from frozen vision-language model features, compared text output accuracy, performed LoRA fine-tuning (r=16, 2,000 images), analyzed five different encoder architectures across self-supervised, contrastive, and hybrid paradigms, conducted layer-wise analysis, and examined attention heads in specific layers.

Result: Linear probes achieved 6.1° MAE for hand joint angles vs. 20.0° for text output (3.3x bottleneck). LoRA fine-tuning reduced gap to 6.5°. Different encoder architectures converged to statistically equivalent accuracy (R²≈0.55) despite low representational similarity (CKA=0.41). Autoregressive generation damages geometric fidelity, but LLM layers can improve accuracy over raw vision encoders. Mid-network layers (18-22) carry disproportionate geometric signal.

Conclusion: Vision-language models encode geometric information that text pathways fail to express due to pathway-training deficits rather than representational limitations. Training objective determines accuracy more than architecture, enabling frozen backbones to function as multi-task geometric sensors through lightweight probes without fine-tuning or text generation.
...
TL;DR: AI system achieves 93%+ sensitivity and 95%+ specificity for prenatal orofacial cleft detection, matching senior radiologists and boosting junior radiologists' performance by 6% when used as a copilot.


<details>
  <summary>Details</summary>
Motivation: Prenatal detection of orofacial clefts is challenging due to scarcity of experienced specialists and rarity of the condition, while early diagnosis is crucial for timely intervention and reduced morbidity.

Method: Developed an AI system trained on 45,139 ultrasound images from 9,215 fetuses across 22 hospitals, tested as a diagnostic copilot with 24 radiologists and trainees.

Result: AI achieved >93% sensitivity and >95% specificity, matching senior radiologists and outperforming junior radiologists. As a copilot, it increased junior radiologists' sensitivity by >6% and accelerated expertise development for rare conditions.

Conclusion: The dual-purpose AI system offers a scalable solution for improving both diagnostic accuracy and specialist training in settings with limited experienced radiologists.
...
### [78] [Breaking Smooth-Motion Assumptions: A UAV Benchmark for Multi-Object Tracking in Complex and Adverse Conditions](https://arxiv.org/abs/2603.05970)
...
<details>
  <summary>Details</summary>
Motivation: Existing UAV-perspective MOT benchmarks lack the complexities of real-world UAV operations, featuring predictable camera dynamics and linear motion patterns, failing to capture the challenges of rapid UAV movements and agile maneuvers.

Method: Introduced DynUAV benchmark with 42 video sequences containing over 1.7 million bounding box annotations covering vehicles, pedestrians, and industrial equipment (excavators, bulldozers, cranes), characterized by intense ego-motion and resulting complex apparent trajectories.

Result: Comprehensive evaluation of state-of-the-art trackers reveals their limitations in managing detection and association challenges under dynamic conditions, establishing DynUAV as a rigorous benchmark that exposes tracker weaknesses.

Conclusion: DynUAV serves as a demanding testbed to spur progress in real-world UAV-perspective MOT, addressing the gap in existing benchmarks and providing resources for future research.
...
### [79] [SUREON: A Benchmark and Vision-Language-Model for Surgical Reasoning](https://arxiv.org/abs/2603.06570)
*Alejandra Perez,Anita Rau,Lee White,Busisiwe Mlambo,Chinedu Nwoye,Muhammad Abdullah Jamal,Omid Mohareri*

Main category: cs.CV

TL;DR: SUREON is a large-scale surgical video QA dataset extracted from expert lecture narrations, enabling AI models to learn surgical reasoning about intent, rationale, and anticipation.


<details>
  <summary>Details</summary>
Motivation: Current surgical AI lacks reasoning capabilities (understanding why instruments are chosen, risks, next steps) because explicit surgical reasoning data is hard to annotate at scale. Surgical video lectures contain expert explanations of intent and rationale that could provide this missing training signal.

Method: Created SUREON dataset by harvesting surgical reasoning from academic video lectures using a multi-agent pipeline to extract and structure supervision. Defined 12 question categories covering safety, decision rationale, and forecasting. Developed two models: SureonVLM (vision-language model fine-tuned on SUREON) and SureonVLM-R1 (reasoning model trained with Group Relative Policy Optimization).

Result: SUREON contains 134.7K clips across 170 procedure types with 206.8K QA pairs and 354 expert-validated benchmark examples. Both models substantially outperform larger general-domain models, achieving >84% accuracy on SUREON benchmark while also outperforming on standard surgical perception tasks. SureonVLM-R1 shows explicit reasoning behavior like inferring operative intent.

Conclusion: Surgical video lectures provide valuable but underutilized training signal for surgical reasoning AI. SUREON enables models to learn complex surgical reasoning about intent, rationale, and anticipation, bridging a critical gap in surgical AI capabilities.
...
### [80] [Towards High-resolution and Disentangled Reference-based Sketch Colorization](https://arxiv.org/abs/2603.05971)
*Dingkun Yan,Xinrui Wang,Ru Wang,Zhuoru Li,Jinze Yu,Yusuke Iwasawa,Yutaka Matsuo,Jiaxian Guo*

Main category: cs.CV

TL;DR: A novel sketch colorization framework that directly addresses distribution shift between training and test data using dual-branch architecture with Gram Regularization Loss, achieving SOTA performance in quality and controllability.


<details>
  <summary>Details</summary>
Motivation: Previous sketch colorization methods only mitigated artifacts from distribution shift rather than solving the core problem of semantic misalignment between training and diverse test data.

Method: Dual-branch framework with semantic-aligned and semantic-misaligned branches, Gram Regularization Loss for cross-domain coherence, anime-specific Tagger Network for fine-grained attribution extraction, and plugin module for texture transfer.

Result: Superior quality, resolution, and controllability of colorization; SOTA performance across quality and controllability metrics; effective overcoming of distribution shift challenge confirmed by quantitative/qualitative comparisons and user studies.

Conclusion: The proposed framework directly minimizes distribution shift in sketch colorization, achieving state-of-the-art results through explicit modeling of training/inference distributions and enhanced control mechanisms.
...
TL;DR: BEVLM connects spatially consistent BEV representations with LLMs for autonomous driving, improving reasoning accuracy by 46% and end-to-end driving performance by 29%.


<details>
  <summary>Details</summary>
Motivation: Existing methods feed LLMs with tokens from multi-view/multi-frame images independently, causing redundant computation and limited spatial consistency. BEV representations provide spatial structure but lack semantic richness from foundation vision encoders.

Method: Propose BEVLM framework that connects spatially consistent and semantically distilled BEV representations with LLMs, using BEV features as unified inputs and distilling semantic knowledge from LLMs into BEV representations.

Result: BEVLM enables LLMs to reason more effectively in cross-view driving scenes, improving accuracy by 46%. It also significantly improves closed-loop end-to-end driving performance by 29% in safety-critical scenarios.

Conclusion: BEVLM bridges the gap between spatial consistency and semantic richness in autonomous driving, enabling more effective LLM reasoning and improved driving performance through unified BEV representations.
...
### [82] [RePer-360: Releasing Perspective Priors for 360$^\circ$ Depth Estimation via Self-Modulation](https://arxiv.org/abs/2603.05999)
*Cheng Guan,Chunyu Lin,Zhijie Shen,Junsong Zhang,Jiyuan Wang*

Main category: cs.CV

TL;DR: RePer-360 adapts depth foundation models to panoramic images using distortion-aware self-modulation, preserving pretrained perspective priors while requiring only 1% of training data.
...
Method: Uses a lightweight geometry-aligned guidance module with ERP and CP projections, Self-Conditioned AdaLN-Zero for pixel-wise scaling, and cubemap-domain consistency loss for training stability.

Result: Surpasses standard fine-tuning methods using only 1% of training data, achieves ~20% RMSE improvement under same in-domain training settings.

Conclusion: RePer-360 effectively adapts depth foundation models to panoramic domain while preserving pretrained perspective knowledge, enabling efficient panoramic depth estimation with minimal data.
...
### [83] [EffectMaker: Unifying Reasoning and Generation for Customized Visual Effect Creation](https://arxiv.org/abs/2603.06014)
*Shiyuan Yang,Ruihuang Li,Jiale Tao,Shuai Shao,Qinglin Lu,Jing Liao*

Main category: cs.CV

TL;DR: EffectMaker is a unified reasoning-generation framework for reference-based visual effects customization that uses multimodal LLM reasoning and diffusion transformer in-context learning to generate high-quality VFX without per-effect fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current VFX production requires expert knowledge and costly pipelines, while existing AIGC systems struggle with VFX generation due to data scarcity, difficulty modeling supernatural/stylized effects, and reliance on per-effect fine-tuning that limits scalability and generalization.

Method: Two-component framework: 1) multimodal LLM interprets high-level effect semantics and reasons about adaptation to target subjects, 2) diffusion transformer uses in-context learning to capture fine-grained visual cues from reference videos. This semantic-visual dual-path guidance enables accurate, controllable synthesis without per-effect fine-tuning. Also created EffectData dataset with 130k videos across 3k VFX categories.

Result: EffectMaker achieves superior visual quality and effect consistency over state-of-the-art baselines, offering a scalable and flexible paradigm for customized VFX generation.

Conclusion: EffectMaker presents an effective solution to VFX generation challenges by combining semantic reasoning with visual in-context learning, eliminating the need for per-effect fine-tuning while maintaining high quality and consistency.
...
### [84] [MOSIV: Multi-Object System Identification from Videos](https://arxiv.org/abs/2603.06022)
...
TL;DR: MOSIV is a new framework for multi-object system identification from videos that optimizes continuous per-object material parameters using differentiable simulation guided by geometric objectives.


<details>
  <summary>Details</summary>
Motivation: Prior methods are ill-suited for multi-object system identification as they focus on single-object scenes or discrete material classification with fixed prototypes, creating a gap for handling complex multi-object interactions.

Method: MOSIV directly optimizes continuous, per-object material parameters using a differentiable simulator guided by geometric objectives derived from video data, enabling fine-grained supervision at the object level.
...
### [85] [Devil is in Narrow Policy: Unleashing Exploration in Driving VLA Models](https://arxiv.org/abs/2603.06049)
...
TL;DR: Curious-VLA addresses the narrow policy limitation in autonomous VLA models by introducing a two-stage framework with trajectory expansion in IL and diversity-aware sampling in RL, achieving state-of-the-art performance on Navsim benchmark.


<details>
  <summary>Details</summary>
Motivation: The paper identifies a fundamental limitation in autonomous VLA models where Imitation Learning collapses exploration, limiting subsequent Reinforcement Learning stages that saturate prematurely due to insufficient feedback diversity.
...
Result: Achieves state-of-the-art results on Navsim benchmark with PDMS 90.3, EPDMS 85.4, and Best-of-N PDMS of 94.8, demonstrating effectiveness in unlocking exploratory potential of VLA models.

Conclusion: Curious-VLA successfully addresses the exploit-explore dilemma in autonomous VLA models through its two-stage design, significantly improving performance by enhancing exploration diversity and reward sensitivity.
...
### [86] [StruVis: Enhancing Reasoning-based Text-to-Image Generation via Thinking with Structured Vision](https://arxiv.org/abs/2603.06032)
*Yuanhuiyi Lyu,Kaiyu Lei,Ziqiao Weng,Xu Zheng,Lutao Jiang,Teng Li,Yangfu Li,Ziyuan Huang,Linfeng Zhang,Xuming Hu*

Main category: cs.CV

TL;DR: StruVis enhances text-to-image generation by using text-based structured visual representations instead of intermediate images, improving reasoning accuracy while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning frameworks for T2I generation have limitations: text-only reasoning lacks visual context, while text-image interleaved reasoning is computationally expensive and constrained by generator limitations.

Method: Proposes StruVis framework that uses text-based structured visual representations as intermediate reasoning states, enabling MLLMs to perceive visual structure in a purely text-based process without intermediate image generation.

Result: Achieves significant performance improvements: 4.61% gain on T2I-ReasonBench and 4% gain on WISE benchmarks for reasoning-based T2I generation.

Conclusion: StruVis provides a generator-agnostic reasoning framework that enhances T2I generation through structured-vision-guided reasoning, balancing computational efficiency with visual accuracy.
...
### [87] [Transforming Omnidirectional RGB-LiDAR data into 3D Gaussian Splatting](https://arxiv.org/abs/2603.06061)
*Semin Bae,Hansol Lim,Jongseong Brad Choi*

Main category: cs.CV

TL;DR: A pipeline that repurposes discarded omnidirectional RGB-LiDAR logs into initialization assets for 3D Gaussian Splatting, overcoming distortion and computational bottlenecks to create digital twins from archived sensor data.


<details>
  <summary>Details</summary>
Motivation: Deployed platforms collect extensive RGB and LiDAR data that is largely discarded due to transmission constraints and lack of scalable reuse pipelines, while there's growing demand for large-scale digital twins in robotics and autonomous driving.

Method: An omnidirectional RGB-LiDAR reuse pipeline with ERP-to-cubemap conversion for spatial anchoring, PRISM color-stratified downsampling, FPFH-based global registration, and ICP to bridge multi-modal inputs for SfM geometry initialization.

Result: Successfully repurposes discarded data into usable SfM geometry, with LiDAR-reinforced initialization consistently enhancing 3DGS rendering fidelity in complex scenes compared to vision-only baselines.

Conclusion: Provides a deterministic workflow for creating simulation-grade digital twins from standard archived sensor logs, enabling scalable reuse of underutilized data for 3D Gaussian Splatting environments.
...
### [88] [Occlusion-Aware SORT: Observing Occlusion for Robust Multi-Object Tracking](https://arxiv.org/abs/2603.06034)
...
<details>
  <summary>Details</summary>
Motivation: 2D multi-object tracking faces challenges due to positional cost confusion arising from partial occlusion, which affects trajectory analysis and object counting in video sequences.

Method: OA-SORT includes three key components: 1) Occlusion-Aware Module (OAM) analyzes occlusion status using Gaussian Maps to reduce background influence, 2) Occlusion-Aware Offset (OAO) mitigates cost confusion, and 3) Bias-Aware Momentum (BAM) suppresses estimation instability. The framework is plug-and-play and training-free.

Result: On DanceTrack test set, OA-SORT achieves 63.1% HOTA and 64.2% IDF1. When integrated into four additional trackers, it improves HOTA by average 2.08% and IDF1 by 3.05%, demonstrating strong reusability and occlusion handling capabilities.

Conclusion: The paper demonstrates the importance of occlusion handling in MOT and presents a reusable occlusion-aware framework that effectively addresses positional cost confusion and estimation instability caused by partial occlusion.
...
TL;DR: VG3S enhances 3D semantic occupancy prediction by injecting geometric priors from Vision Foundation Models into Gaussian splatting, achieving significant performance gains on autonomous driving benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian splatting methods for occupancy prediction lack accurate geometric cues in vision-only systems, limiting their effectiveness. Vision Foundation Models (VFMs) have strong geometric grounding capabilities that could bridge this gap.

Method: Proposes VG3S (Visual Geometry Grounded Gaussian Splatting) with a plug-and-play hierarchical geometric feature adapter that transforms VFM tokens through feature aggregation, task-specific alignment, and multi-scale restructuring to inject geometric priors.

Result: Achieves 12.6% improvement in IoU and 7.5% improvement in mIoU over baseline on nuScenes occupancy benchmark. Generalizes well across different VFMs and consistently enhances prediction accuracy.

Conclusion: Integrating geometric priors from pre-trained Vision Foundation Models significantly improves 3D semantic occupancy prediction, demonstrating the value of leveraging powerful geometry-grounded VFMs for autonomous driving perception.
...
### [90] [Ensemble Learning with Sparse Hypercolumns](https://arxiv.org/abs/2603.06036)
*Julia Dietlmeier,Vayangi Ganepola,Oluwabukola G. Adegboro,Mayug Maniparambil,Claudia Mazo,Noel E. O'Connor*

Main category: cs.CV

TL;DR: Hypercolumns for pixel classification are computationally expensive; stratified subsampling and ensemble methods improve performance, with Logistic Regression being best for very small datasets (N≤20).


<details>
  <summary>Details</summary>
Motivation: Hypercolumns (multi-scale CNN feature vectors) show promise for image segmentation but are computationally expensive due to linear growth with training set size, limiting practical use.

Method: Applied stratified subsampling to VGG16-based hypercolumns to reduce computational cost, and investigated ensemble learning (stacking and voting) on sparse hypercolumns.

Result: For N≤20, Logistic Regression performed best with 10% stratified subsampling, achieving average Dice score of 0.66 - a 24.53% statistically significant improvement over UNet baseline (p=3.07e-11).

Conclusion: Stratified subsampling makes hypercolumns practical, and simple classifiers like Logistic Regression outperform complex ensembles in low-shot learning scenarios, significantly beating standard UNet baselines.
...
### [91] [FontUse: A Data-Centric Approach to Style- and Use-Case-Conditioned In-Image Typography](https://arxiv.org/abs/2603.06038)
*Xia Xin,Yuki Endo,Yoshihiro Kanamori*

Main category: cs.CV

TL;DR: FontUse: A data-centric approach using a structured annotation pipeline to create a typography-focused dataset for training text-to-image models to better control typographic appearance in generated images.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with controlling typography - requested typographic appearance is often ignored or weakly followed, limiting their practical applications where specific typographic styles are needed.

Method: Created FontUse dataset (~70K images) using automated annotation pipeline with segmentation models and MLLMs; annotated with prompts, text-region locations, and OCR strings; prompts combine font styles and use cases; fine-tuned existing generators without architectural changes.

Result: Models trained with FontUse produce text renderings more consistent with prompts than competitive baselines; introduced Long-CLIP-based metric for evaluation; effective across diverse prompts and layouts.

Conclusion: Data-centric approach with specialized typography annotation pipeline enables better typographic control in text-to-image generation; method works with existing models and provides intuitive prompt specification for users.
...
TL;DR: NOVA is a novel 3D multi-object tracking method that uses autoregressive LLMs to model trajectories as spatio-temporal semantic sequences, enabling open-vocabulary generalization across unknown targets.


<details>
  <summary>Details</summary>
Motivation: Existing 3D MOT methods are limited by closed-set assumptions and "semantic-blind" heuristics, failing to generalize to unknown targets in open-world perception scenarios.

Method: Reformulates 3D trajectories as structured spatio-temporal semantic sequences, leverages LLMs' autoregressive capabilities for next-step sequence completion, and uses hierarchical language space to resolve semantic ambiguities.
...
Conclusion: NOVA successfully shifts 3D tracking from traditional distance-based matching to generative spatio-temporal semantic modeling, enabling open-vocabulary generalization through language priors and commonsense reasoning.
...
### [93] [Learning to Generate via Understanding: Understanding-Driven Intrinsic Rewarding for Unified Multimodal Models](https://arxiv.org/abs/2603.06043)
...
<details>
  <summary>Details</summary>
Motivation: Unified multimodal models show promise for text-to-image tasks but suffer from a capability gap: strong visual understanding but weak generation due to decoupled processes. Need to leverage internal understanding to enhance generation quality.

Method: Proposed GvU - token-level intrinsic text-image alignment reward mechanism that enables UMM to use its understanding branch to evaluate and guide its own generations. Built self-supervised reinforcement learning framework for iterative improvement without external supervision.

Result: Method substantially boosts UMMs' generation quality, which in turn strengthens their fine-grained visual understanding, narrowing the capability gap between understanding and generation.

Conclusion: UMMs can leverage their own understanding capabilities to self-improve generation through intrinsic reward mechanisms, creating a virtuous cycle that enhances both understanding and generation without external supervision.
...
TL;DR: First benchmark (OccNL) for 3D occupancy prediction under noisy supervision, showing 2D noise methods fail in 3D; proposed DPR-Occ framework uses dual-source partial labels to handle extreme noise (90%) with significant performance gains.


<details>
  <summary>Details</summary>
Motivation: Real-world 3D semantic occupancy annotations are corrupted by structural artifacts and dynamic trailing effects, creating unreliable supervision for safety-critical robotic perception systems.

Method: Proposes DPR-Occ framework using dual-source partial label reasoning: combines temporal model memory with representation-level structural affinity to dynamically expand/prune candidate label sets, preserving true semantics while suppressing noise propagation.

Result: DPR-Occ prevents geometric/semantic collapse under extreme corruption; at 90% label noise, achieves up to 2.57% mIoU and 13.91% IoU gains over adapted 2D noise learning baselines on SemanticKITTI.

Conclusion: Bridges label noise learning with 3D perception, providing reliable foundation for safety-critical robotic perception in dynamic environments; benchmark and code will be publicly released.
...
Method: GenHOI introduces two key components: 1) Head-Sliding RoPE for temporal balancing (assigns head-specific temporal offsets to reference tokens to distribute influence evenly across frames), and 2) a two-level spatial attention gate that concentrates object-conditioned attention on HOI regions while preserving background realism.
...
Conclusion: GenHOI effectively addresses HOI challenges in video synthesis by combining temporal balancing and spatial selectivity, enabling robust generalization to complex real-world scenarios while maintaining object consistency and interaction fidelity.
...
TL;DR: A calibration method for diffuse direct time-of-flight LiDARs that maps each pixel's spatial footprint and sensitivity in RGB image coordinates, enabling accurate cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Standard LiDAR-RGB calibration fails for diffuse direct time-of-flight LiDARs because they violate the single-ray assumption by aggregating photon returns over wide fields of view, requiring spatial calibration of each pixel's effective support region.

Method: Uses a scanned retroreflective patch with background subtraction to recover per-pixel response maps that estimate each LiDAR pixel's footprint and relative spatial sensitivity in the co-located RGB image plane.

Result: Demonstrated on the ams OSRAM TMF8828 LiDAR, providing explicit LiDAR-to-RGB correspondence for cross-modal alignment and fusion applications.

Conclusion: The simple spatial calibration procedure enables accurate alignment between diffuse LiDAR and RGB sensors by explicitly modeling each pixel's spatial characteristics, overcoming limitations of standard calibration methods.
...
TL;DR: Proposes Lyapunov Probes for hallucination detection in LLMs/MLLMs using dynamical systems stability theory, treating factual knowledge as stable equilibrium points and hallucinations as occurring at knowledge-transition boundaries.


<details>
  <summary>Details</summary>
Motivation: Current approaches treat hallucination detection as a classification task, but the authors argue that hallucinations emerge from the dynamical nature of (M)LLMs and should be understood through stability theory, where factual knowledge corresponds to stable equilibrium points in representation space.

Method: Lyapunov Probes: lightweight networks trained with derivative-based stability constraints that enforce monotonic confidence decay under input perturbations. Uses systematic perturbation analysis and two-stage training to distinguish stable factual regions from unstable hallucination-prone regions.

Result: Experiments on diverse datasets and models show consistent improvements over existing baselines in hallucination detection performance.

Conclusion: Framing hallucination detection through dynamical systems stability theory provides a principled approach, with Lyapunov Probes effectively capturing the transition between stable factual regions and unstable hallucination-prone zones in (M)LLMs.
...
TL;DR: DeepSight is the first dedicated depth MLLM that enhances 3D scene understanding by leveraging depth images instead of RGB, using novel depth datasets and modified ViT architecture to capture continuous depth variations.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with accurate depth interpretation in visual data, limiting their 3D scene understanding capabilities despite strong performance on 2D tasks like image captioning and VQA.

Method: 1) Construct novel depth image-text pair dataset and depth instruction dataset using GLPN for depth map generation and GPT-4 for instruction curation; 2) Modify ViT encoder in CLIP to incorporate local object information for better capture of continuous depth variations; 3) Develop comprehensive depth QA benchmark for evaluation.

Result: DeepSight significantly enhances depth perception and downstream task performance, marking substantial progress in multimodal 3D understanding compared to conventional RGB-based MLLMs.

Conclusion: The work introduces a specialized depth-focused MLLM architecture that effectively addresses the limitations of existing models in 3D scene interpretation, providing a foundation for improved multimodal spatial reasoning.
...
TL;DR: FedARKS addresses limitations in federated domain generalization for person re-identification by introducing robust knowledge selection and integration mechanisms to better capture local details and handle client heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Existing FedDG-ReID methods have two key limitations: 1) reliance on global features fails to capture domain-invariant local details like accessories/textures, and 2) uniform parameter averaging ignores differences in clients' robust feature extraction capabilities, diluting contributions from high-quality clients.

Method: Proposes FedARKS framework with two mechanisms: RK (Robust Knowledge) for capturing subtle domain-invariant local details, and KS (Knowledge Selection) for selectively aggregating client knowledge based on their robust feature extraction capabilities rather than simple averaging.

Result: The paper claims FedARKS addresses the identified limitations, though specific experimental results are not provided in the abstract.

Conclusion: FedARKS offers a novel approach to federated domain generalization for person re-identification that better handles local feature representation and client heterogeneity through knowledge selection and robust knowledge mechanisms.
...
TL;DR: RMD is a diffusion distillation framework that bridges cross-resolution distribution gaps for accelerated multi-resolution cascaded inference, achieving up to 33.4X speedup while maintaining visual fidelity.
...
Method: Divides timestep intervals per resolution using logSNR curves, introduces logSNR-based mapping to compensate resolution shifts, performs distribution matching along resolution trajectories, and uses predicted-noise re-injection during upsampling.

Result: Achieves up to 33.4X speedup on SDXL and 25.6X on Wan2.1-14B while preserving high visual fidelity across various backbones.

Conclusion: RMD effectively bridges cross-resolution distribution gaps for high-fidelity, few-step multi-resolution cascaded inference, significantly accelerating diffusion models without quality degradation.
...
TL;DR: VLMs fail dramatically on structured color distortions that humans handle easily, and simple preprocessing can partially recover performance.


<details>
  <summary>Details</summary>
Motivation: Vision-language models achieve strong benchmark results but have systematic perceptual weaknesses - they make confident nonsensical predictions when faced with structured color distortions that humans easily recognize.

Method: Introduced Spatial Colour Mixing framework with 8 variants of structured color distortions in RGB and Ostwald systems. Evaluated 9 VLMs across 3 model families on 4 datasets. Conducted human study with 61 participants on animal recognition dataset.

Result: VLM accuracy degrades sharply with increasing distortion, scaling language models doesn't reliably help, humans substantially outperform VLMs under same distortions, and simple human-inspired preprocessing recovers meaningful performance for several distortion types.

Conclusion: VLMs have fundamental perceptual weaknesses to structured color distortions that humans handle easily, motivating perception-aware preprocessing and tool-use strategies for improving VLM robustness.
...
TL;DR: Virtual Treatment framework predicts NSCLC tumor evolution during radiotherapy using dose-aware multimodal image translation to synthesize follow-up CT scans.


<details>
  <summary>Details</summary>
Motivation: Predicting tumor evolution during radiotherapy is clinically critical but challenging due to anatomical changes driven by both anatomy and treatment effects, especially for non-small cell lung cancer patients.

Method: VT formulates NSCLC progression as dose-aware multimodal conditional image-to-image translation, using CT scans, baseline clinical variables, and radiation dose increments to synthesize follow-up CT images. Evaluated on 222 stage III NSCLC patients with 895 CT scans, comparing GAN-based and diffusion-based models in 2D/2.5D configurations.
...
Conclusion: VT shows potential as a tool for in-silico treatment monitoring and adaptive radiotherapy research in NSCLC, with diffusion models outperforming GANs for this dose-aware tumor evolution prediction task.
...
TL;DR: FreeOcc is a training-free pipeline that uses pretrained foundation models for semantic and panoptic occupancy prediction from multi-view images, achieving competitive results without 3D supervision or domain-specific training.


<details>
  <summary>Details</summary>
Motivation: Current camera-only approaches for occupancy prediction require costly dense 3D supervision or domain-specific training, limiting deployment in unseen environments. There's a need for training-free methods that can generalize across domains.

Method: Leverages pretrained foundation models: 1) extracts per-view panoptic priors using promptable foundation segmentation with prompt-to-taxonomy rules, 2) reconstructs metric 3D points with reconstruction foundation model, 3) uses depth- and confidence-aware filtering to lift labels into 3D, 4) fuses over time and voxelizes with deterministic refinement, 5) recovers instances by fitting and merging robust 3D box candidates.

Result: On Occ3D-nuScenes: 16.9 mIoU and 16.5 RayIoU train-free (competitive with state-of-the-art weakly supervised methods). As pseudo-label generator: 21.1 RayIoU (surpasses previous SOTA weakly supervised baseline). Sets new baselines for train-free (3.1 RayPQ) and weakly supervised (3.9 RayPQ) panoptic occupancy prediction.
...
TL;DR: A semi-supervised framework for breast ultrasound segmentation using vision-language models for training-free pseudo-label generation, achieving near-fully-supervised performance with only 2.5% labeled data.


<details>
  <summary>Details</summary>
Motivation: Current semi-supervised learning for breast ultrasound segmentation suffers from unstable pseudo labels under limited annotations, and vision-language models have limited effectiveness due to domain transfer challenges between natural and medical images.

Method: Proposes training-free pseudo-label generation using simple appearance-based descriptions (e.g., "dark oval") for cross-domain structural transfer, combined with static teacher warm-up, exponential moving average teacher, uncertainty entropy weighted fusion, and adaptive uncertainty-guided reverse contrastive learning.
...
Conclusion: The framework enables scalable semi-supervised medical image segmentation with minimal labeled data by requiring only global appearance descriptions, making it extensible to other imaging modalities and diseases.
...
TL;DR: JOPP-3D is an open-vocabulary semantic segmentation framework that jointly processes panoramic images and 3D point clouds using language queries to generate semantic masks across both modalities.


<details>
  <summary>Details</summary>
Motivation: Semantic segmentation across 3D point clouds and panoramic images is challenging due to limited annotated data and fixed-label models that lack adaptability to new concepts.

Method: Converts RGB-D panoramic images to tangential perspective images and 3D point clouds, extracts and aligns vision-language features from both modalities, and uses natural language queries to generate semantic masks.

Result: Achieves significant improvement over state-of-the-art methods in open and closed vocabulary 2D and 3D semantic segmentation on Stanford-2D-3D-s and ToF-360 datasets, producing coherent semantic segmentations.

Conclusion: JOPP-3D enables language-driven scene understanding across panoramic and 3D domains, addressing data scarcity and model adaptability limitations through joint multimodal processing.
...
TL;DR: Enhancing 3D medical image diffusion models with RL using multi-scale feedback to improve clinical relevance and downstream task performance.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion model training objectives don't align well with clinical relevance in 3D medical imaging, creating a gap between technical metrics and practical utility.

Method: Two-stage approach: 1) Pretrain 3D diffusion model on MRI volumes, 2) Fine-tune with PPO using novel reward system combining 2D slice-wise and 3D volumetric analysis for multi-scale optimization.

Result: RL feedback effectively steers generation toward higher quality distributions, showing significant FID improvements and enhanced utility in downstream tumor/disease classification tasks on BraTS 2019 and OASIS-1 datasets.

Conclusion: Multi-scale RL feedback successfully bridges the gap between standard training objectives and clinical relevance in 3D medical image generation, improving both quantitative metrics and practical utility.
...
<details>
  <summary>Details</summary>
Motivation: Existing training-free diffusion segmentors using cross-attention maps don't scale well with more powerful diffusion models, despite the expectation that stronger generative capability should lead to better segmentation. The authors identify two gaps: discrepancy between individual attention maps and unified global representation, and score imbalances among text tokens preventing accurate semantic correlation.

Method: Two techniques: (1) Auto aggregation to address the discrepancy between multiple attention heads/layers and unified global representation, and (2) Per-pixel rescaling to handle score imbalances among different text tokens that prevent accurate semantic correlation for segmentation.

Result: The approach improves performance on standard semantic segmentation benchmarks and demonstrates broad applicability when integrated into generative techniques. The method enables training-free segmentation to better leverage generative capability.

Conclusion: By addressing the identified gaps in cross-attention map utilization through auto aggregation and per-pixel rescaling, training-free diffusion segmentation can better scale with more powerful generative models, achieving improved performance and broader applicability.
...
TL;DR: Proposes Motion Turing Test framework and HHMotion dataset to evaluate if humanoid robot poses can be distinguished from human poses using only kinematic data, revealing current limitations in motion human-likeness.


<details>
  <summary>Details</summary>
Motivation: Inspired by the Turing Test, the paper aims to create a systematic framework to evaluate how human-like humanoid robot motions are, addressing the gap in objective assessment of motion naturalness despite visual appearance improvements.

Method: Creates HHMotion dataset with 1,000 motion sequences across 15 action categories performed by 11 humanoid models and 10 humans, converted to SMPL-X representations. Uses 30 annotators to rate human-likeness on 0-5 scale, then formulates automated evaluation task and proposes baseline model.

Result: Humanoid motions still show noticeable deviations from human movements, especially in dynamic actions like jumping, boxing, and running. Current multimodal LLMs are inadequate for motion human-likeness assessment, while the proposed baseline outperforms LLM-based methods.

Conclusion: The Motion Turing Test framework and HHMotion dataset provide valuable tools for evaluating motion human-likeness, revealing current limitations in robot motion generation and offering a benchmark for future research in humanoid robotics.
...
<details>
  <summary>Details</summary>
Motivation: Traditional cancer tissue region detection methods using histology images alone suffer from high false positives due to morphological similarities. Spatial transcriptomics provides detailed cellular phenotypes and spatial information but current methods can't effectively integrate both data types, especially in cross-sample and cross-platform settings.

Method: SpaCRD uses a category-regularized variational reconstruction-guided bidirectional cross-attention fusion network that adaptively captures latent co-expression patterns between histological features and gene expression from multiple perspectives. It's a transfer learning approach that can generalize to new samples after training on source data.
...
Conclusion: SpaCRD enables reliable cancer tissue region detection by effectively integrating histology and spatial transcriptomics data, addressing cross-sample and cross-platform challenges through transfer learning and advanced fusion techniques.
...
TL;DR: ALANet uses adaptive language-aware filtering and optimization to remove complex reflections even with inaccurate language descriptions, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing reflection removal methods struggle with complex reflections, and inaccurate machine-generated language descriptions from blurred reflected images harm language-guided approaches.

Method: ALANet integrates filtering and optimization strategies: filtering reduces negative effects of inaccurate language while preserving benefits; optimization enhances language-visual alignment; uses language cues to decouple layer content from feature maps.

Result: ALANet surpasses state-of-the-art methods for image reflection removal, demonstrated on the new CRLAV dataset with complex reflections and varying language accuracy.

Conclusion: ALANet effectively handles complex reflections even with inaccurate language inputs through adaptive language-aware strategies, with code and dataset publicly available.
...
TL;DR: Point-supervised skeleton action segmentation using single-frame labels per action, multimodal features, and integrated pseudo-label generation achieves competitive performance with minimal annotation.


<details>
  <summary>Details</summary>
Motivation: Fully-supervised skeleton action segmentation requires costly frame-level annotations and suffers from ambiguous action boundaries. Need to reduce annotation effort while maintaining performance.

Method: Point-supervised framework with single frame per action segment. Uses multimodal skeleton data (joint, bone, motion) encoded via pretrained unified model. Proposes prototype similarity method combined with energy function and constrained K-Medoids for pseudo-label generation, with multimodal integration.

Result: Establishes new benchmarks on PKU-MMD (X-Sub/X-View), MCFS-22, and MCFS-130. Achieves competitive performance, even surpassing some fully-supervised methods while significantly reducing annotation effort.

Conclusion: Point-supervised skeleton action segmentation with multimodal pseudo-label integration effectively reduces annotation requirements while achieving strong performance, making it practical for real-world applications.
...
TL;DR: EntON introduces an Eigenentropy-optimized neighborhood densification strategy for 3D Gaussian Splatting that improves geometric accuracy and rendering quality while reducing computational costs.
...
Method: Uses Eigenentropy (from covariance matrix eigenvalues) to quantify local structural order. Implements alternating optimization: standard gradient-based densification plus Eigenentropy-aware densification that splits Gaussians in low-Eigenentropy (ordered) regions and prunes those in high-Eigenentropy (disordered) regions.

Result: Improves geometric accuracy by up to 33%, rendering quality by up to 7%, reduces Gaussians by up to 50%, and cuts training time by up to 23% on DTU and TUM2TWIN datasets.
...
TL;DR: WAFL reformulates temporal forgery localization as discrete word-level binary classification instead of temporal regression, achieving better performance with higher efficiency through forensic feature realignment and artifact-centric loss.


<details>
  <summary>Details</summary>
Motivation: Current TFL approaches suffer from feature granularity misalignment and high computational costs due to their reliance on temporal boundary regression or continuous frame-level anomaly detection.

Method: 1) Shift to word-level binary classification using word tokens as minimum forgery units; 2) Forensic Feature Realignment (FFR) module to map pre-trained features to forensic manifold; 3) Artifact-Centric Asymmetric (ACA) loss to handle class imbalance by prioritizing forensic artifacts.

Result: WAFL significantly outperforms state-of-the-art approaches in localization performance under both in-dataset and cross-dataset settings, while requiring substantially fewer parameters and operating at high computational efficiency.

Conclusion: The word-anchored paradigm provides a more effective and efficient approach to temporal forgery localization by aligning with natural linguistic boundaries and leveraging powerful pre-trained models with specialized forensic adaptation.
...
<details>
  <summary>Details</summary>
Motivation: Event cameras offer low-latency visual data but existing async neural networks have bottlenecks: recurrent architectures are hard to train on long sequences, and accuracy improvements increase computation/latency. Linear attention supports parallel training but standard approaches update global states per event, creating poor accuracy-efficiency trade-off for object detection requiring fine-grained representations.

Method: Proposes Spatially-Sparse Linear Attention (SSLA) with mixture-of-spaces state decomposition and scatter-compute-gather training procedure, enabling state-level sparsity while preserving training parallelism. Built on SSLA, develops SSLA-Det for end-to-end asynchronous event-based object detection.

Result: On Gen1 and N-Caltech101 datasets, SSLA-Det achieves state-of-the-art accuracy among asynchronous methods (0.375 mAP and 0.515 mAP respectively) while reducing per-event computation by more than 20 times compared to strongest prior async baseline.

Conclusion: SSLA-Det demonstrates the potential of linear attention for low-latency event-based vision by introducing spatially-sparse state activation that exploits event sparsity while maintaining efficient parallel training, achieving superior accuracy-efficiency trade-off.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D-GRES methods rely only on sparse point clouds, lacking rich visual semantics needed for fine-grained natural language descriptions that may match multiple or zero targets.

Method: Proposes HCF-RES with: 1) Hierarchical Visual Semantic Decomposition using SAM instance masks to guide CLIP encoding at pixel and instance levels, preserving object boundaries during 2D-to-3D projection; 2) Progressive Multi-level Fusion integrating representations through intra-modal collaboration, cross-modal adaptive weighting between 2D semantic and 3D geometric features, and language-guided refinement.

Result: Achieves state-of-the-art results on both ScanRefer and Multi3DRefer benchmarks.

Conclusion: HCF-RES effectively addresses limitations of sparse point clouds by incorporating rich visual semantics through hierarchical decomposition and progressive fusion, enabling better 3D object localization from natural language descriptions.
...
TL;DR: Real-time drone detection system using spinning event camera for moving carriers, achieving 360° FOV and <2° angular error without motion compensation.


<details>
  <summary>Details</summary>
Motivation: Existing drone detection systems struggle with fast-moving targets, adverse lighting, and especially when deployed on moving platforms like robots or vehicles. Event cameras offer advantages but current solutions assume static cameras, limiting real-world applicability.

Method: Uses spinning event-based camera for 360° horizontal field of view, novel image-like event representation without motion compensation, and lightweight neural network for spatiotemporal learning. Implemented on Jetson Orin NX for real-time operation.

Result: System achieves real-time drone detection on moving carriers with mean angular error below 2° in challenging outdoor conditions, enabling bearing estimation of detected drones.

Conclusion: The system successfully addresses limitations of existing drone detection methods by enabling deployment on moving platforms with reliable performance, making it suitable for real-world surveillance applications. Complete pipeline will be open-sourced.
...
TL;DR: StrSR is a novel one-step adversarial distillation framework for diffusion transformer-based real-world image super-resolution that addresses trajectory mismatch and grid artifacts through spectral and trajectory regularization.


<details>
  <summary>Details</summary>
Motivation: Diffusion transformers (DiT) show promise for real-world image super-resolution but require computationally expensive iterative sampling, necessitating one-step distillation. Existing one-step distillation methods struggle with Real-ISR on DiT due to fundamental trajectory mismatch and severe grid-like periodic artifacts.

Method: StrSR proposes a one-step adversarial distillation framework with spectral and trajectory regularization. It uses an asymmetric discriminative distillation architecture to bridge the trajectory gap and a frequency distribution matching strategy to suppress DiT-specific periodic artifacts caused by high-frequency spectral leakage.
...
Conclusion: StrSR effectively addresses the challenges of one-step distillation for DiT-based Real-ISR through its novel regularization techniques, achieving superior performance while eliminating the need for computationally expensive iterative sampling.
...
TL;DR: ADiVA addresses generative ZSL challenges by modeling attribute distributions and aligning semantic-visual representations, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Generative ZSL faces two intrinsic challenges: (1) class-level attributes fail to capture instance-specific visual appearances due to intra-class variability (class-instance gap), and (2) substantial mismatch between semantic and visual feature distributions due to inter-class correlations (semantic-visual domain gap).

Method: Proposes ADiVA with two modules: Attribute Distribution Modeling (ADM) learns transferable attribute distributions for each class and samples instance-level attributes for unseen classes; Visual-Guided Alignment (VGA) refines semantic representations to better reflect visual structures.

Result: Significantly outperforms state-of-the-art methods on three benchmark datasets (e.g., 4.7% gain on AWA2 and 6.1% gain on SUN). The approach can also serve as a plugin to enhance existing generative ZSL methods.

Conclusion: ADiVA effectively addresses the class-instance and semantic-visual domain gaps in generative ZSL through joint attribute distribution modeling and semantic-visual alignment, demonstrating superior performance and versatility as a plugin enhancement.
...
TL;DR: FlowMotion: Training-free video motion transfer framework using flow-based T2V model predictions for efficient motion alignment


<details>
  <summary>Details</summary>
Motivation: Existing training-free approaches for video motion transfer have heavy computational overhead and limited flexibility due to reliance on intermediate outputs of pre-trained T2V models

Method: Proposes flow guidance that extracts motion representations from early latent predictions of flow-based T2V models, and introduces velocity regularization for stable optimization and smooth motion evolution

Result: Achieves superior time and resource efficiency with competitive performance compared to state-of-the-art methods

Conclusion: FlowMotion enables efficient and flexible motion transfer by directly leveraging predicted outputs of flow-based T2V models, operating purely on model predictions
...
<details>
  <summary>Details</summary>
Motivation: CBCT imaging suffers from artefacts from high-density dental implants, compromising diagnostic accuracy. Existing diffusion-based methods treat 2D projections independently, ignoring inter-projection correlations and causing reconstruction inconsistencies.

Method: Proposes a 3D implant inpainting approach using perpendicular score-based diffusion models trained in two different planes (projection domain). Combines two 2D diffusion models in sampling scheme to model 3D distribution of projection series.
...
Conclusion: The 3D diffusion-based approach successfully addresses limitations of independent 2D projection methods, offering a promising solution for clinical CBCT artefact reduction through consistent 3D implant inpainting.
...
TL;DR: LTA is a transfer-based adversarial attack that optimizes perturbations in Stable Diffusion's latent space instead of pixel space, producing more robust and transferable attacks with low-frequency, coherent perturbations.


<details>
  <summary>Details</summary>
Motivation: Pixel-space adversarial attacks produce high-frequency noise that is brittle to preprocessing (resizing, cropping) and transfers poorly across architectures. There's a need for more robust and transferable attacks that can better evaluate model robustness.

Method: LTA optimizes perturbations in the latent space of a pretrained Stable Diffusion VAE. It encodes clean images into latent codes, optimizes them to maximize surrogate classifier loss while enforcing pixel-space ℓ∞ budget after decoding. Uses Expectation Over Transformations (EOT) with randomized resizing, interpolation, cropping, and periodic latent Gaussian smoothing to improve robustness.

Result: LTA achieves strong transfer attack success across CNN and vision-transformer targets, producing spatially coherent, low-frequency perturbations that differ qualitatively from pixel-space baselines and occupy a distinct point in the transfer-quality trade-off.

Conclusion: Pretrained generative latent spaces are effective and structured domains for adversarial optimization, bridging robustness evaluation with modern generative priors, offering a new approach to creating more robust and transferable adversarial attacks.
...
TL;DR: Wavelet-enhanced mixture-of-experts prompt learning for zero-shot anomaly detection improves adaptability to diverse anomalies through semantic-aware prompts and multi-frequency feature extraction.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot anomaly detection methods rely on fixed textual prompts that struggle with complex semantics and focus only on spatial-domain features, limiting detection of subtle anomalies.

Method: Proposes wavelet-enhanced mixture-of-experts prompt learning: uses variational autoencoder for global semantic representations in prompts, wavelet decomposition for multi-frequency features, and semantic-aware mixture-of-experts module for contextual aggregation.

Result: Extensive experiments on 14 industrial and medical datasets demonstrate the effectiveness of the proposed method.

Conclusion: The proposed approach addresses limitations of fixed prompts and spatial-only features in zero-shot anomaly detection by integrating semantic representations, multi-frequency analysis, and contextual aggregation.
...
TL;DR: P-SLCR: A prototype library-driven unsupervised point cloud semantic segmentation method using structure learning and consistent reasoning, achieving state-of-the-art results without manual labels.


<details>
  <summary>Details</summary>
Motivation: Current point cloud semantic segmentation relies heavily on manual labeling, while unsupervised methods are underdeveloped. The absence of annotations and pre-training makes unsupervised point cloud learning particularly challenging, necessitating effective strategies.

Method: Proposes P-SLCR with two main components: 1) Consistent Structure Learning that establishes structural feature learning between consistent points and prototype libraries using high-quality features, and 2) Semantic Relation Consistent Reasoning that constructs prototype inter-relation matrices between consistent and ambiguous prototype libraries to preserve semantic consistency through constraints.

Result: Achieved state-of-the-art performance on S3DIS, SemanticKITTI, and Scannet datasets. Specifically achieved 47.1% mIoU on S3DIS Area-5, surpassing the classical fully supervised method PointNet by 2.5%.

Conclusion: The proposed P-SLCR method effectively addresses the challenges of unsupervised point cloud semantic segmentation through prototype library-driven structure learning and consistent reasoning, demonstrating superior performance over existing unsupervised methods and even outperforming some supervised approaches.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion-based world models are computationally expensive for interactive use and long-horizon rollouts due to iterative denoising. Existing feature caching methods don't work well because policies designed for single-modal diffusion fail to address world-model-specific challenges: token heterogeneity from multi-modal coupling and spatial variation, and non-uniform temporal dynamics where hard tokens drive error growth.

Method: WorldCache introduces two key techniques: 1) Curvature-guided Heterogeneous Token Prediction - uses physics-grounded curvature scores to estimate token predictability and applies Hermite-guided damped predictor for chaotic tokens with abrupt direction changes; 2) Chaotic-prioritized Adaptive Skipping - accumulates curvature-normalized, dimensionless drift signal and recomputes only when bottleneck tokens begin to drift.

Result: WorldCache delivers up to 3.7× end-to-end speedups while maintaining 98% rollout quality on diffusion world models, demonstrating practical advantages for resource-constrained scenarios.

Conclusion: WorldCache provides an effective caching framework tailored to diffusion world models that overcomes the limitations of existing caching approaches, enabling significant computational speedups without compromising rollout quality.
...
TL;DR: LATO introduces a topology-preserving latent representation for scalable flow matching-based synthesis of explicit 3D meshes, using Vertex Displacement Fields and sparse voxel VAEs to generate meshes without isosurface extraction.


<details>
  <summary>Details</summary>
Motivation: Prior methods for 3D mesh generation often rely on isosurface extraction or heuristic meshing, which can be inefficient and may not preserve topology well. There's a need for scalable, topology-aware mesh synthesis that directly generates explicit meshes with proper connectivity.

Method: LATO represents meshes as Vertex Displacement Fields anchored on surfaces, compressed via sparse voxel VAEs into structured voxel latents. The decoder progressively subdivides and prunes latent voxels to instantiate vertices, while a connection head predicts edge connectivity directly from voxel latents. For generation, it uses two-stage flow matching: first synthesizing structure voxels, then refining topology features.
...
Conclusion: LATO provides a novel topology-preserving latent representation that enables scalable, flow matching-based synthesis of explicit 3D meshes without requiring isosurface extraction or heuristic meshing, offering improved efficiency and topology quality.
...
Method: Two approaches: 1) Linear model with novel predictors (area and sinking speed) from BIODISCOVER imaging system, 2) End-to-end deep neural networks (single-view, multi-view, metadata-aware) trained on dry mass-image sequence pairs.

Result: Methods successfully estimate dry mass for diverse specimen morphologies with 10-20% median percentage error for individuals when combined with automatic taxonomic classification.

Conclusion: Computer vision enables accurate, non-destructive biomass estimation; proper evaluation requires both percentage and absolute error metrics; deep learning architectures and data augmentation methods are important considerations.
...
TL;DR: OralGPT-Plus is an agentic vision-language model for panoramic dental radiograph analysis that performs iterative, symmetry-aware reasoning using a reinforcement learning framework and specialized dataset.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models use static single-pass paradigms that lack the fine-grained spatial reasoning, bilateral symmetry understanding, and multi-step diagnostic verification needed for reliable clinical panoramic dental radiograph analysis.

Method: Developed OralGPT-Plus with iterative symmetry-aware reasoning, created DentalProbe dataset (5k images with expert diagnostic trajectories), implemented Reinspection-driven reinforcement learning with rubric-based and diagnostic-driven rewards, and built MMOral-X benchmark for holistic panoramic diagnosis.

Result: OralGPT-Plus shows consistent improvements over strong baselines on MMOral-X and established panoramic benchmarks, demonstrating effectiveness of interactive and symmetry-informed reasoning.

Conclusion: Agentic modeling with iterative reasoning and symmetry awareness is valuable for dental imaging, providing a foundation for clinically aligned panoramic radiograph analysis research.
...
TL;DR: Rewis3d improves weakly supervised semantic segmentation by using 3D scene reconstruction from 2D videos as auxiliary supervision to propagate sparse annotations across scenes.


<details>
  <summary>Details</summary>
Motivation: Dense pixel-level annotations for segmentation models are expensive to obtain, and while sparse annotations offer a weakly-supervised alternative, they still suffer from a performance gap compared to fully supervised methods.

Method: Uses 3D scene reconstruction from 2D videos as auxiliary supervision. Employs a dual student-teacher architecture that enforces semantic consistency between 2D images and reconstructed 3D point clouds, leveraging state-of-the-art feed-forward reconstruction for reliable geometric supervision.

Result: Achieves state-of-the-art performance in sparse supervision, outperforming existing approaches by 2-7% without requiring additional labels or inference overhead.

Conclusion: 3D geometric structure recovered from 2D videos provides strong cues for propagating sparse annotations, enabling significant improvements in weakly supervised semantic segmentation through geometric consistency.
...
TL;DR: MoEMambaMIL: A structure-aware State Space Model framework for whole-slide image analysis that organizes multi-resolution patches into region-aware sequences and uses mixture-of-experts for efficient hierarchical modeling.


<details>
  <summary>Details</summary>
Motivation: Existing MIL approaches treat WSIs as unordered patch collections, failing to capture structured dependencies between global tissue organization and local cellular patterns. Current SSMs can handle long sequences but lack proper structuring of WSI tokens to exploit spatial hierarchy.

Method: Organizes patch tokens into region-aware sequences preserving spatial containment across resolutions via multi-resolution preprocessing. Uses region-nested selective scanning with MoE modeling, combining static resolution-specific experts and dynamic sparse experts with learned routing for resolution-aware encoding and region-adaptive contextual modeling.

Result: Achieves best performance across 9 downstream tasks, demonstrating superior capability in WSI analysis compared to existing approaches.

Conclusion: MoEMambaMIL effectively addresses the hierarchical structure challenge in WSI analysis by integrating structured token organization with efficient SSM and MoE modeling, enabling better capture of spatial dependencies across resolutions.
...
TL;DR: CHMv2 is a global, meter-resolution canopy height map derived from optical satellite imagery using DINOv3-based depth estimation, offering improved accuracy over existing products.


<details>
  <summary>Details</summary>
Motivation: Accurate canopy height information is essential for forest carbon quantification, restoration monitoring, degradation assessment, and habitat structure evaluation, but high-fidelity ALS measurements remain unevenly available globally.

Method: Uses high-resolution optical satellite imagery with a depth-estimation model built on DINOv3, trained against ALS canopy height models. Features large expansion of geographically diverse training data, automated data curation and registration, and tailored loss formulation and data sampling strategy for canopy height distributions.

Result: CHMv2 substantially improves accuracy, reduces bias in tall forests, and better preserves fine-scale structure like canopy edges and gaps compared to existing products. Validated against independent ALS test sets and tens of millions of GEDI and ICESat-2 observations, demonstrating consistent performance across major forest biomes.

Conclusion: CHMv2 provides a globally consistent, high-resolution canopy height mapping solution that addresses limitations of existing products and enables better forest monitoring and carbon assessment worldwide.
...
TL;DR: REACT++ is a new SGG model that balances inference speed, relation prediction accuracy, and object detection performance, achieving 20% faster inference with 10% better relation accuracy than REACT.


<details>
  <summary>Details</summary>
Motivation: Current SGG methods focus on either relation prediction accuracy, object detection accuracy, or latency reduction, but don't balance all three objectives simultaneously for real-time applications.
...
Result: Achieves highest inference speed among SGG models with improved relation prediction accuracy without sacrificing object detection performance - 20% faster with 10% better relation accuracy than REACT.

Conclusion: REACT++ sets new state-of-the-art for real-time SGG by effectively balancing the trade-off between performance and inference speed.
...
TL;DR: Human-in-the-loop puzzle solving framework for real-world archaeological artifact reconstruction that combines automatic relaxation-labeling with interactive human guidance, outperforming fully automatic and manual baselines.


<details>
  <summary>Details</summary>
Motivation: Real-world archaeological artifact reassembly is extremely challenging due to erosion, missing regions, irregular shapes, and large-scale ambiguity. Traditional jigsaw puzzle solvers designed for clean synthetic scenarios fail when dealing with thousands of fragments as in the RePAIR benchmark.

Method: Proposes a human-in-the-loop framework integrating automatic relaxation-labeling solver with interactive human guidance. Introduces two interaction strategies: Iterative Anchoring (users lock verified placements) and Continuous Interactive Refinement (users correct errors and guide the system). The hybrid approach allows iterative refinement toward semantically and geometrically coherent assemblies.

Result: Experiments on several RePAIR groups demonstrate the hybrid approach substantially outperforms both fully automatic and manual baselines in accuracy and efficiency.

Conclusion: The human-in-the-loop framework offers a practical solution for large-scale expert-in-the-loop artifact reassembly, effectively addressing the complexity and scale of real-world cultural heritage reconstruction.
...
TL;DR: DiffInf uses self-influence scoring and diffusion-based image editing to fix annotation inconsistencies in facial attribute datasets, improving classification without discarding data.


<details>
  <summary>Details</summary>
Motivation: Facial attribute datasets have annotation inconsistencies due to subjective labeling and visual confounders (pose, illumination, expression, demographics), which create supervision errors that impair representation learning and degrade downstream prediction performance.

Method: 1. Train baseline classifier and compute self-influence scores using first-order approximation to identify influential training instances. 2. Apply targeted generative correction via latent diffusion autoencoder to align visual content with assigned labels while preserving identity/realism. 3. Train lightweight predictor of high-influence membership as surrogate influence regularizer for differentiable guidance. 4. Replace original samples with edited ones to create influence-refined dataset of same size.
...
Conclusion: Repairing influential annotation inconsistencies at the image level via generative correction enhances downstream facial attribute classification without sacrificing distributional coverage, offering a better alternative to discarding problematic samples.
...
TL;DR: BEiT vision transformers show a bias toward convex completions in perceptual ambiguity, governed by specific attention heads like L0H9 that act as early seeds for convexity preference.


<details>
  <summary>Details</summary>
Motivation: To understand where Gestalt perceptual priors (specifically convexity) are realized within vision transformer architectures, using BEiT as a case study for figure-ground organization in ambiguous visual stimuli.

Method: Used synthetic dart shapes with masked regions that equally admit concave or convex completions, creating controlled perceptual conflict. Projected internal activations into discrete visual codebook space via logit attribution to trace convexity preference through transformer layers.

Result: BEiT reliably favors convex completion under competition. Figure-ground organization remains ambiguous through early/intermediate layers but resolves abruptly in later layers. Head L0H9 acts as an early seed introducing weak convexity bias - downscaling this single head shifts perceptual decisions toward concave completions.

Conclusion: Gestalt perceptual priors like convexity are implemented by identifiable functional units within transformer substructures, with specific attention heads serving as early decision seeds that can be manipulated to alter perceptual outcomes.
...
TL;DR: Non-invasive stereo vision method using refraction-aware 3D triangulation to measure fish length in aquariums, validated on endangered Sulawesi ricefish.


<details>
  <summary>Details</summary>
Motivation: Monitoring fish growth is important for health assessment in aquaculture and home aquariums, but traditional methods are invasive or inaccurate due to refractive distortions in aquarium environments.

Method: Uses YOLOv11-Pose for fish detection and keypoint prediction in stereo images, refraction-aware epipolar constraint for robust matching, quality score filtering, and refraction-aware 3D triangulation to recover 3D keypoints for length measurement.

Result: Validated on new stereo dataset of endangered Sulawesi ricefish, demonstrating that filtering low-quality detections is essential for accurate length estimation.

Conclusion: Proposed system offers simple, practical solution for non-invasive growth monitoring that can be easily applied in home aquariums.
...
TL;DR: WanderDream is a large-scale dataset for mental exploration simulation, enabling AI models to reason about spatial what-if questions without physical exploration through panoramic videos and QA pairs.


<details>
  <summary>Details</summary>
Motivation: Real-world spatial reasoning often requires active exploration, but physical constraints (robot limitations, safety concerns for visually impaired users) make actual exploration infeasible. The paper aims to enable agents to mentally simulate future trajectories and answer spatial what-if questions from limited observations.

Method: Created two components: WanderDream-Gen (15.8K panoramic videos across 1,088 real scenes from HM3D, ScanNet++, and real-world captures showing imagined trajectories) and WanderDream-QA (158K question-answer pairs covering starting states, paths, and end states). Tested with world models and multimodal large language models (MLLMs).

Result: Experiments show: (1) mental exploration is essential for situated reasoning, (2) world models achieve compelling performance on WanderDream-Gen, (3) imagination substantially facilitates reasoning on WanderDream-QA, and (4) WanderDream data exhibits remarkable transferability to real-world scenarios.

Conclusion: WanderDream enables effective mental exploration simulation for spatial reasoning without physical constraints, demonstrating that imagination-based approaches can successfully handle spatial what-if questions in constrained environments.
...
TL;DR: CaTok introduces a 1D causal image tokenizer with MeanFlow decoder that enables autoregressive vision modeling by learning causal 1D representations supporting both fast one-step and high-fidelity multi-step image generation.


<details>
  <summary>Details</summary>
Motivation: Current visual tokenizers either flatten 2D patches into non-causal sequences or use heuristic orderings that misalign with next-token prediction patterns. Diffusion autoencoders also fail to provide proper causality - either conditioning on all tokens lacks causality, or nested dropout creates imbalance.

Method: CaTok uses a 1D causal image tokenizer with MeanFlow decoder that selects tokens over time intervals and binds them to the MeanFlow objective. Also introduces REPA-A regularization to align encoder features with Vision Foundation Models for training stability and acceleration.

Result: Achieves state-of-the-art results on ImageNet reconstruction: 0.75 FID, 22.53 PSNR, 0.674 SSIM with fewer training epochs. The AR model attains performance comparable to leading approaches.

Conclusion: CaTok successfully bridges the gap between autoregressive language modeling and vision by providing a proper causal tokenization framework that supports efficient image generation while capturing diverse visual concepts across token intervals.
...
TL;DR: Pinterest Canvas is a large-scale image generation system that uses a foundational diffusion model fine-tuned for specific editing tasks, achieving significant engagement lifts and outperforming third-party models.


<details>
  <summary>Details</summary>
Motivation: While recent image generation models are flexible, they're hard to control via prompting alone and unsuitable for product use cases with strict requirements. Pinterest needs a system that can reliably support specific image editing and enhancement tasks.

Method: Train a foundational diffusion model on diverse multimodal data, then rapidly fine-tune task-specific variants on specialized datasets for individual use cases. Key components include dataset curation, training, and inference best practices.

Result: Online A/B experiments show 18.0% and 12.5% engagement lifts for background enhancement and aspect-ratio outpainting tasks. Human raters validate that Canvas models outperform third-party models on these specific tasks.

Conclusion: The task-specific fine-tuning approach effectively addresses product requirements and generalizes to various downstream tasks including multi-image scene synthesis and image-to-video generation.
...
TL;DR: Systematic analysis of training objectives for denoising-based generative models, examining loss weighting, output parameterization, and their interactions with data geometry, architecture, and dataset size.


<details>
  <summary>Details</summary>
Motivation: To disentangle the various factors that matter when training flow matching models, providing practical insights on design choices rather than proposing new methods.

Method: Systematic numerical study comparing training objectives (noise-, clean image-, and velocity-based formulations) across synthetic datasets with controlled geometry and image data, using quantitative metrics like PSNR and FID.

Result: Analysis of how training choices interact with intrinsic dimensionality of data manifold, model architecture, and dataset size, providing insights on optimal design choices for denoising-based generative models.

Conclusion: The study provides practical guidance on training objective selection for flow matching models based on data characteristics and available resources, helping practitioners make informed design decisions.
...
<details>
  <summary>Details</summary>
Motivation: Current radiology foundation models rely on brute-force scaling, directly translating natural image methods that prioritize scale over precision, leading to brittle and expensive models unsuitable for clinical practice.

Method: GreenRFM uses a principled supervision design called MUST (More distilled, Ubiquitous, Semantic-enforcing, and Task-aligning) supervision that maximizes supervisory signals rather than simply scaling training data. Offers two configurations: performant (24GB GPU/24h) and lightweight (6GB VRAM/4h).

Result: Achieves superior performance on chest and abdominal CT datasets across public/private benchmarks, surpassing baseline models. Transfers successfully to musculoskeletal MRI. Reduces computational requirements by orders of magnitude while establishing new SOTA.

Conclusion: Challenges the "scale is all you need" dogma and democratizes equitable development of SOTA radiology foundation models, making them accessible to clinicians even on laptop hardware.
...
TL;DR: Match4Annotate: A lightweight framework using implicit neural representations and deformation fields for both intra- and inter-video propagation of point and mask annotations in specialized domains like medical imaging.


<details>
  <summary>Details</summary>
Motivation: Per-frame video annotation is slow and costly in specialized domains like medical imaging. Existing label propagation methods have limitations: video trackers require per-video initialization and lack cross-video generalization, correspondence pipelines struggle in low-texture scenes, and dense feature matching lacks spatiotemporal smoothness.

Method: Fits a SIREN-based implicit neural representation to DINOv3 features at test time to create a continuous, high-resolution spatiotemporal feature field. Learns a smooth implicit deformation field between frame pairs to guide correspondence matching for both point and mask annotations.
...
Conclusion: Lightweight, test-time-optimized feature matching pipelines like Match4Annotate offer an efficient and accessible solution for scalable annotation workflows in specialized domains where expert labeling is costly.
...
TL;DR: Self-Flow: self-supervised flow matching that learns semantic representations internally via dual-timestep scheduling, eliminating need for external models and improving multi-modal generation.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion/flow models rely on external models for semantic representations, which require separate training, have misaligned objectives, and show unexpected scaling behavior. This dependence stems from denoising objectives that don't incentivize learning semantic representations.

Method: Introduces Self-Flow with Dual-Timestep Scheduling: applies heterogeneous noise levels across tokens to create information asymmetry, forcing the model to infer missing information from corrupted inputs. This integrates representation learning within the generative framework without external supervision.

Result: Method generalizes across modalities, enables multi-modal training, follows expected scaling laws, and achieves superior image, video, and audio generation compared to approaches using external models.

Conclusion: Self-Flow successfully integrates representation learning with generative modeling through self-supervised flow matching, eliminating dependence on external models while improving performance and scaling behavior across multiple modalities.
...
TL;DR: SCAN is a universal XAI framework that generates high-resolution self-confidence maps for both CNNs and Transformers, overcoming the trade-off between fidelity and applicability in current methods.


<details>
  <summary>Details</summary>
Motivation: Current XAI methods face a critical trade-off: architecture-specific methods have high fidelity but limited applicability, while universal methods have broad applicability but often produce abstract/fragmented explanations. This makes it difficult to compare explanations across different model families like CNNs and Transformers.

Method: SCAN uses an AutoEncoder-based approach to reconstruct features from intermediate layers, guided by the Information Bottleneck principle. It generates high-resolution Self-Confidence Maps that identify information-rich regions in both CNN and Transformer architectures.

Result: Extensive experiments show SCAN consistently achieves outstanding performance on quantitative metrics (AUC-D, Negative AUC, Drop%, Win%) and produces significantly clearer, object-focused explanations than existing methods across diverse architectures and datasets.
...
TL;DR: AV-Unified is a unified framework for joint learning across diverse audio-visual scene understanding tasks by standardizing inputs/outputs into token sequences and using multi-scale spatiotemporal perception networks.


<details>
  <summary>Details</summary>
Motivation: Current audio-visual tasks (event localization, parsing, segmentation, question answering) are explored individually, making it hard to comprehensively understand complex scenes and explore inter-task relationships.

Method: 1) Standardize diverse task inputs/outputs into sequences of discrete tokens for shared representation; 2) Multi-scale temporal perception module for varying event granularity; 3) Cross-modal guidance-based spatial perception module for audio-visual associations; 4) Task-specific text prompts for adaptability.

Result: Extensive experiments on benchmark datasets (AVE, LLP, MUSIC-AVQA, VGG-SS, AVS) demonstrate effectiveness across temporal, spatial, and spatiotemporal tasks.

Conclusion: AV-Unified enables comprehensive audio-visual scene understanding through unified joint learning, capturing complex inter-task relationships that individual approaches miss.
...
TL;DR: Formal treatment of linguistic negation in diffusion models as structured feasibility constraints on semantic guidance, enabling training-free negation handling across diverse phenomena.


<details>
  <summary>Details</summary>
Motivation: Negation is a fundamental linguistic operator but remains inadequately modeled in diffusion-based generative systems, creating a gap in handling diverse negation phenomena like object absence, graded semantics, and scope-sensitive disambiguation.

Method: Reinterpret classifier-free guidance as semantic update direction and enforce negation by projecting updates onto convex constraint sets derived from linguistic structure. Training-free approach compatible with pretrained diffusion backbones, extending to both images and videos.

Result: Method achieves robust negation compliance while preserving visual fidelity and structural coherence. Introduces structured negation-centric benchmark suite to isolate distinct linguistic failure modes in generative systems.

Conclusion: Establishes first unified formulation of linguistic negation in diffusion-based generative models beyond representation-level evaluation, providing formal framework for diverse negation phenomena without retraining.
...
TL;DR: SurgFormer is a multiresolution gated transformer that predicts soft tissue deformation on volumetric meshes at near real-time rates, handling both standard deformation and cut-conditioned simulation with learned cut embeddings.


<details>
  <summary>Details</summary>
Motivation: High-fidelity biomechanical solvers are too computationally expensive for interactive surgical simulation, creating a need for data-driven surrogates that can predict tissue deformation quickly while handling complex scenarios like surgical cuts.

Method: Uses a multiresolution gated transformer architecture with fixed mesh hierarchy, multibranch blocks combining local message passing, coarse global self-attention, and pointwise feedforward updates, fused by learned per-node gates. For cut-conditioned simulation, encodes resection information as learned cut embeddings.

Result: Achieves strong accuracy with favorable efficiency compared to diverse baselines, making it practical for both standard deformation prediction and topology-altering cases. Introduces two surgical simulation datasets (cholecystectomy and appendectomy) with XFEM-based supervision.

Conclusion: SurgFormer provides a scalable, efficient backbone for interactive surgical simulation that can handle both standard tissue deformation and complex cut-conditioned scenarios within a unified volumetric pipeline, representing the first learned approach to study XFEM-supervised cut-conditioned deformation.
...
TL;DR: This paper analyzes redundancy as a data quality factor in autonomous vehicle datasets, showing that removing redundant labels from multisource camera data can improve object detection performance.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles rely on multisource multimodal data, but data quality varies across sources and modalities. While AV research has focused on algorithm design, redundancy as a fundamental data quality issue remains underexplored.

Method: The study models and measures redundancy in multisource camera data and multimodal image-LiDAR data using nuScenes and Argoverse 2 datasets. It evaluates how removing redundant labels affects YOLOv8 object detection performance.

Result: Selectively removing redundant multisource image object labels improves detection: in nuScenes, mAP50 gains from 0.66 to 0.70, 0.64 to 0.67, and 0.53 to 0.55 on three overlap regions. In AV2, 4.1-8.6% of labels are removed while mAP50 stays near the 0.64 baseline. Multimodal analysis reveals substantial redundancy between image and LiDAR data.
...
TL;DR: EgoReasoner: A two-stage framework for egocentric 4D reasoning that adapts reasoning scaffolds and rewards to different cognitive tasks, achieving state-of-the-art performance on HD-EPIC benchmark.


<details>
  <summary>Details</summary>
Motivation: Egocentric video understanding is complex due to dynamic 4D environments requiring continuous spatial re-evaluation. Existing task-agnostic approaches (generic Chain-of-Thought, uniform reinforcement learning) are insufficient for diverse cognitive operations needed for tasks like fixture interaction counting, viewpoint-relative fixture location, object movement tracking, and stationary object localization.

Method: Two-stage framework: 1) Task-Adaptive Thinking Templates guide structured CoT trace synthesis for supervised fine-tuning, teaching adaptive reasoning across task types. 2) Task-aware reward functions verify entity grounding, temporal alignment, and logical consistency for reinforcement fine-tuning with GRPO, selectively strengthening each reasoning pathway.

Result: 3B-parameter model trained on only 16K samples achieves 37.5% average accuracy on HD-EPIC benchmark, surpassing Qwen2.5-VL-7B (25.7%) by over 10 percentage points.

Conclusion: Aligning reasoning scaffolds and reward signals to each task's cognitive structure is crucial for effective egocentric 4D reasoning, enabling a single model to handle diverse cognitive operations with high efficiency and performance.
...
TL;DR: Penguin-VL challenges the need for contrastive-pretrained vision encoders in VLMs, showing that text-only LLM initialization enables better visual fidelity and efficiency for compact models.


<details>
  <summary>Details</summary>
Motivation: Current VLM development relies on scaling model size, which limits deployment on mobile/edge devices. There's an objective mismatch where contrastive pretraining (CLIP/SigLIP) suppresses fine-grained visual cues needed for dense captioning and complex reasoning.

Method: Penguin-VL uses a vision encoder initialized from a text-only LLM instead of contrastive-pretrained encoders. This approach preserves fine-grained spatial and temporal cues critical for dense perception and complex reasoning.

Result: Penguin-VL achieves performance comparable to leading VLMs (e.g., Qwen3-VL) in mathematical reasoning and surpasses them in document understanding, visual knowledge, and multi-perspective video understanding, all with lightweight architecture.

Conclusion: Improved visual representation rather than model scaling is the primary driver of performance. Penguin-Encoder serves as a superior drop-in alternative to contrastive-pretrained encoders for compute-efficient VLMs in resource-constrained settings.
...
TL;DR: SCOPE is a plug-and-play framework for 3D incremental few-shot segmentation that enriches novel class prototypes using background pseudo-instances from base training scenes, achieving state-of-the-art performance with minimal forgetting.


<details>
  <summary>Details</summary>
Motivation: 3D incremental few-shot segmentation is underexplored compared to 2D, and existing methods suffer from catastrophic forgetting, poor discriminative prototypes under sparse supervision, and overlook that novel categories often appear as unlabelled background in base-training scenes.

Method: SCOPE uses a class-agnostic segmentation model after base training to extract high-confidence pseudo-instances from background regions to build a prototype pool. When novel classes arrive with few samples, relevant background prototypes are retrieved and fused with few-shot prototypes to create enriched representations without retraining the backbone or adding parameters.
...
Conclusion: SCOPE effectively leverages background information from base training scenes to enrich few-shot prototypes for novel classes, providing a plug-and-play solution that significantly improves 3D incremental few-shot segmentation performance without architectural changes or retraining.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal large language models (MLLMs) predominantly use conventional autoregressive architectures, leaving room to explore more effective and efficient alternatives. Discrete diffusion models have shown promise in visual understanding and image generation, suggesting their potential as a backbone for multimodal systems.

Method: Omni-Diffusion employs a unified mask-based discrete diffusion model to directly capture the joint distribution over discrete multimodal tokens. This approach supports bimodal tasks and more complex scenarios involving multiple modalities.

Result: On diverse benchmarks, Omni-Diffusion outperforms or performs on par with existing multimodal systems that process two or more modalities.

Conclusion: The work highlights the significant promise of diffusion models in powering the next generation of multimodal foundation models, demonstrating that discrete diffusion models can serve as an effective alternative to conventional autoregressive architectures for multimodal systems.
...
TL;DR: MLLM classification performance gaps are largely due to flawed evaluation protocols and noisy ground truth, not genuine model deficiencies. Fixing these issues narrows the gap with supervised models, and MLLMs can assist in dataset curation.


<details>
  <summary>Details</summary>
Motivation: Conflicting conclusions about MLLM classification performance compared to supervised models stem from problematic evaluation protocols and poor ground truth quality. The paper aims to identify and fix these issues to provide accurate performance assessment.

Method: Identifies and fixes key issues in common evaluation protocols: handling model outputs outside class lists, addressing weak multiple-choice distractors, and improving output mapping. Quantifies impact of overlooked design choices (batch size, image ordering, text encoder). Evaluates on ReGT, a multilabel reannotation of 625 ImageNet-1k classes. Conducts controlled case study on MLLM-assisted human annotation.

Result: MLLMs benefit most from corrected labels (up to +10.8% improvement), substantially narrowing the perceived gap with supervised models. Much reported underperformance is artifact of noisy ground truth and flawed protocols. Models less reliant on supervised training are most sensitive to annotation quality. MLLMs can assist human annotators, with approximately 50% of difficult cases confirmed or integrated.

Conclusion: MLLM classification underperformance is largely due to evaluation artifacts rather than genuine model deficiencies. Proper evaluation protocols and high-quality ground truth reveal MLLMs perform much better than previously reported. MLLMs show promise for large-scale dataset curation through human-in-the-loop annotation assistance.
...
TL;DR: RoboLayout extends LayoutVLM with agent-aware reasoning and improved optimization stability to generate physically feasible indoor scene layouts for diverse embodied agents.


<details>
  <summary>Details</summary>
Motivation: Current VLMs can generate semantically coherent 3D scene layouts but struggle with creating layouts that are physically feasible for embodied agents to interact with in constrained indoor environments.

Method: Extends LayoutVLM by integrating explicit reachability constraints into differentiable layout optimization, supporting diverse agent types (robots, humans, animals), and adding a local refinement stage that selectively reoptimizes problematic object placements.

Result: Preserves semantic alignment and physical plausibility of LayoutVLM while enhancing applicability to agent-centric indoor scene generation across diverse scene configurations.

Conclusion: RoboLayout successfully bridges the gap between semantic scene understanding and physical feasibility for embodied agents, enabling more practical and interactive environment design.
...
TL;DR: Decentralized price-based resource allocation works reliably for hierarchical DAGs but fails for complex dependencies; hybrid architecture with cross-domain integrators stabilizes prices while maintaining allocation quality.


<details>
  <summary>Details</summary>
Motivation: Real-time AI services operate across device-edge-cloud continuum with latency-sensitive workloads, multi-stage pipelines, and shared resources under policy constraints. Need to understand when decentralized, price-based resource allocation can work reliably at scale.

Method: Model service-dependency graphs as DAGs, analyze price convergence for different topologies (hierarchical vs. complex). Propose hybrid architecture with cross-domain integrators that encapsulate complex sub-graphs into resource slices. Conduct systematic ablation study across 6 experiments (1,620 runs, 10 seeds each).

Result: 1) Dependency-graph topology determines price stability and scalability; 2) Hybrid architecture reduces price volatility by 70-75% without sacrificing throughput; 3) Governance constraints create efficiency-compliance trade-offs dependent on topology and load; 4) Under truthful bidding, decentralized market matches centralized value-optimal baseline.

Conclusion: Decentralized coordination can replicate centralized allocation quality when dependency graphs are hierarchical or when complex sub-graphs are encapsulated via cross-domain integrators. Hybrid management architecture bridges the gap between simple and complex dependency structures.
...
TL;DR: Chain-of-thought controllability is currently low in reasoning models, making CoT monitoring effective despite concerns about models manipulating their reasoning process.
...
Method: Developed CoT-Control evaluation suite with tasks requiring models to solve problems while adhering to specific CoT instructions (e.g., reasoning without certain words), tested across various models, training regimes, and conditions.

Result: Models show significantly lower CoT controllability (2.7% for Claude Sonnet 4.5) than output controllability (61.9%). Controllability decreases with RL training, test-time compute, and problem difficulty, and doesn't meaningfully improve with adversarial prompting or incentives.

Conclusion: Current CoT controllability is low, making CoT monitoring reliable, but the mechanism is not well understood. Frontier labs should track this capability in future models to maintain CoT monitorability.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based medical agents use static tool sets that become brittle under real-world domain shifts, evolving diagnostic requirements, and cross-task variations, requiring costly manual re-design.

Method: MACRO autonomously identifies effective multi-step tool sequences from verified execution trajectories, synthesizes them into reusable composite tools, uses image-feature memory for visual-clinical context, and employs GRPO-like training to reinforce reliable composite tool invocation.

Result: Extensive experiments across diverse medical imaging datasets show MACRO consistently improves multi-step orchestration accuracy and cross-domain generalization over strong baselines and state-of-the-art agentic methods.
...
TL;DR: ProEvolve is a graph-based framework for programmatically evolving LLM agent environments to test adaptability to real-world dynamics.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks assume static environments with fixed schemas and toolsets, neglecting the evolutionary nature of the real world and agents' robustness to environmental changes.

Method: ProEvolve uses a typed relational graph as a unified representation of environment components (data, tools, schema). Environment evolution is expressed as graph transformations that propagate updates coherently across tools, schemas, and data access.

Result: The framework evolved a single environment into 200 environments and 3,000 task sandboxes, enabling comprehensive benchmarking of representative agents' adaptability.

Conclusion: ProEvolve provides a scalable and controllable way to evolve agent environments, offering a better evaluation framework for agents' adaptability to real-world dynamics.
...
TL;DR: Researchers propose Evolving Benchmarking via Audit-then-Score (AtS) to create reliable benchmarks for verifying factuality in deep research reports, showing experts are better auditors than one-shot labelers.


<details>
  <summary>Details</summary>
Motivation: Existing fact-checkers are designed for simple atomic claims and don't work well for deep research reports (DRRs). There's no good benchmark for DRR factuality verification, and creating one is difficult because expert labeling is unreliable (only 60.8% accuracy).

Method: Proposed Evolving Benchmarking via Audit-then-Score (AtS): when a verifier disagrees with benchmark labels, it submits evidence; an auditor adjudicates; accepted revisions update the benchmark before scoring models. Created DeepFact-Bench (versioned DRR benchmark) and DeepFact-Eval (document-level verification agent).

Result: Expert accuracy improved from 60.8% to 90.9% across four AtS rounds, showing experts are much better as auditors than one-shot labelers. DeepFact-Eval outperforms existing verifiers on DeepFact-Bench and transfers well to external factuality datasets.

Conclusion: AtS enables creation of reliable benchmarks for DRR factuality verification. Experts are substantially more reliable as auditors than as initial labelers. The approach yields practical tools (DeepFact-Bench and DeepFact-Eval) that advance document-level fact verification.
...
TL;DR: LLM-based multi-agent system automates product concept evaluation using technical and market feasibility criteria, achieving expert-level consistency


<details>
  <summary>Details</summary>
Motivation: Traditional expert-led product concept evaluation suffers from subjective bias, high time/cost requirements, and limitations in objectivity

Method: Proposed LLM-based multi-agent system with 8 virtual agents representing specialized domains (R&D, marketing, etc.), using RAG and real-time search for evidence gathering, fine-tuned with professional product review data

Result: System's evaluation rankings for professional display monitor concepts were consistent with senior industry experts' rankings

Conclusion: Multi-agent-based evaluation approach is usable for supporting product development decisions, offering automated, objective assessment
...
TL;DR: LLM-based planning with PDDL simulation shows modest 3% advantage over direct planning but at 5.7× higher token cost, suggesting limited agentic gains in planning vs. coding tasks.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs can serve as viable planners alongside classical symbolic methods, and to understand the effectiveness of agentic approaches in planning tasks compared to coding tasks.

Method: Developed PyPDDLEngine, an open-source PDDL simulation engine that exposes planning operations as LLM tool calls via MCP interface. LLM acts as interactive search policy selecting one action at a time with state observation and reset capability. Evaluated four approaches on 102 IPC Blocksworld instances with 180-second budget: Fast Downward baselines, direct LLM planning, and agentic LLM planning via PyPDDLEngine.

Result: Fast Downward achieved 85.3% success. Direct LLM planning: 63.7%, agentic LLM planning: 66.7% (3% advantage). Agentic approach had 5.7× higher token cost per solution. Both LLM approaches produced shorter plans than seq-sat-lama-2011 across most co-solved difficulty blocks, suggesting training-data recall rather than generalizable planning.

Conclusion: Agentic gains depend on environmental feedback nature: coding agents benefit from externally grounded signals (compiler errors, test failures), while PDDL step feedback is self-assessed without external verification, limiting agentic advantages in planning tasks.
...
TL;DR: Introduces aggregative semantics for Quantitative Bipolar Argumentation Frameworks (QBAF) that separately aggregates attackers and supporters in a three-stage computation for more interpretable and parametrizable gradual semantics.


<details>
  <summary>Details</summary>
Motivation: To address limitations in existing gradual semantics for QBAFs where attackers and supporters don't play symmetric roles, and to create more interpretable and parametrizable semantics that better preserve bipolarity throughout computation.

Method: Proposes aggregative semantics with three-stage computation: 1) compute global weight for attackers, 2) compute global weight for supporters, 3) aggregate these two values with the argument's intrinsic weight using separate aggregation functions.

Result: Developed a novel family of gradual semantics that maintains bipolarity further than existing approaches, tested 500 aggregative semantics on examples showing diverse behaviors, and established properties for aggregation functions based on context.

Conclusion: Aggregative semantics provide more understandable, parametrizable gradual semantics for QBAFs by decomposing computation into three interpretable steps that preserve bipolarity and allow flexible adaptation to different contexts.
...
TL;DR: CliqueFlowmer is a new offline model-based optimization approach for computational materials discovery that outperforms generative methods by directly optimizing target properties during generation.


<details>
  <summary>Details</summary>
Motivation: Current generative modeling methods for materials discovery are ineffective at exploring optimal regions of materials space due to their maximum likelihood training, which doesn't directly optimize target properties.

Method: CliqueFlowmer combines transformer and flow generation with clique-based model-based optimization to fuse direct optimization of target material properties into the generation process.

Result: CliqueFlowmer strongly outperforms generative baselines in producing materials with optimized properties, demonstrating superior optimization capabilities.

Conclusion: The proposed offline model-based optimization approach offers an effective alternative to generative methods for computational materials discovery, with open-source code available for specialized applications and interdisciplinary research.
...
TL;DR: CDR introduces bidirectional natural language conversations between aggregators and prosumers using agentic AI for more transparent and participatory demand response coordination.


<details>
  <summary>Details</summary>
Motivation: Current demand response systems lack meaningful prosumer participation - they're either fully automated or use one-way signals that don't allow for informed decision-making, creating a coordination gap between aggregators and prosumers.

Method: Two-tier multi-agent architecture with aggregator agents dispatching flexibility requests and prosumer HEMS agents assessing deliverability via optimization tools, enabling bidirectional natural language conversations and prosumer-initiated upstream communication.

Result: Proof-of-concept shows interactions complete in under 12 seconds, demonstrating how agentic AI can bridge the aggregator-prosumer coordination gap while maintaining transparency, explainability, and user agency.

Conclusion: CDR provides the scalability of automated demand response while preserving necessary transparency and user agency for sustained prosumer participation, with all system components released as open source.
...
TL;DR: RL-based decision-support framework for long-term urban flood adaptation planning outperforms traditional methods by learning coordinated spatial-temporal strategies under climate uncertainty.


<details>
  <summary>Details</summary>
Motivation: Climate change intensifies rainfall and pluvial flooding, disrupting urban transportation. Designing adaptation strategies is challenging due to long-term sequential investments, deep climate uncertainty, and complex interactions between flooding, infrastructure, and mobility.

Method: Novel RL-based decision-support framework formulated as an integrated assessment model (IAM) combining rainfall projection/flood modeling, transport simulation, and quantification of direct/indirect impacts on infrastructure and mobility. Learns adaptive strategies balancing investment/maintenance costs against avoided impacts.

Result: Framework tested on Copenhagen case study (2024-2100) with multiple adaptation options and climate scenarios. Outperforms traditional optimization by discovering coordinated spatial-temporal adaptation pathways and learning trade-offs between impact reduction and investment, yielding more resilient strategies.

Conclusion: Reinforcement learning shows strong potential as a flexible decision-support tool for adaptive infrastructure planning under climate uncertainty, enabling more resilient urban flood adaptation strategies.
...
TL;DR: EpisTwin is a neuro-symbolic framework that builds Personal Knowledge Graphs from user data to enable verifiable, holistic reasoning for Personal AI, outperforming traditional vector-based approaches.


<details>
  <summary>Details</summary>
Motivation: Current Personal AI suffers from data fragmentation across isolated silos, and traditional Retrieval-Augmented Generation (RAG) approaches fail to capture semantic topology and temporal dependencies needed for comprehensive sensemaking.

Method: EpisTwin uses Multimodal Language Models to convert heterogeneous user data into semantic triples for a Personal Knowledge Graph. At inference, it employs an agentic coordinator combining Graph RAG with Online Deep Visual Refinement to dynamically re-ground symbolic entities in visual context.

Result: The framework demonstrates robust performance across state-of-the-art judge models on PersonalQA-71-100, a synthetic benchmark simulating realistic user digital footprints.

Conclusion: EpisTwin offers a promising direction for trustworthy Personal AI by enabling verifiable, user-centric reasoning grounded in structured knowledge graphs rather than unstructured vector similarity.
...
TL;DR: SAHOO is a framework for monitoring and controlling alignment drift during recursive self-improvement, using multi-signal detection, constraint preservation, and regression-risk quantification to enable safe iterative improvement.


<details>
  <summary>Details</summary>
Motivation: As recursive self-improvement moves from theory to practice, iterative self-modification risks subtle alignment drift where AI systems gradually deviate from intended goals and constraints during self-critique and revision cycles.

Method: Three safeguards: (1) Goal Drift Index (GDI) - learned multi-signal detector combining semantic, lexical, structural, and distributional measures; (2) constraint preservation checks for safety-critical invariants; (3) regression-risk quantification to flag improvement cycles that undo prior gains.

Result: Across 189 tasks in code generation, mathematical reasoning, and truthfulness, SAHOO produces substantial quality gains (18.3% improvement in code tasks, 16.8% in reasoning) while preserving constraints in two domains and maintaining low violations in truthfulness.
...
TL;DR: Schema-gated orchestration resolves tension between deterministic execution and conversational flexibility in LLM-driven scientific workflows by using schemas as mandatory execution boundaries.


<details>
  <summary>Details</summary>
Motivation: Scientific workflows require determinism, provenance, and governance, but LLMs introduce unpredictability when they decide what runs. There's a tension between deterministic execution and conversational flexibility that needs resolution.

Method: Proposed schema-gated orchestration where schemas become mandatory execution boundaries. Analyzed 20 systems across 5 architectural groups using multi-model LLM scoring (15 sessions across 3 LLM families) to assess execution determinism and conversational flexibility.

Result: Found empirical Pareto front showing no system achieves both high flexibility and high determinism, but convergence zone exists. Multi-model LLM scoring showed substantial-to-near-perfect agreement (α=0.80 for ED, α=0.98 for CF), validating it as alternative to human expert panels.

Conclusion: Schema-gated architecture separating conversational from execution authority can decouple the trade-off, guided by three principles: clarification-before-execution, constrained plan-act orchestration, and tool-to-workflow-level gating.
...
TL;DR: H²RL: Hybrid Hierarchical RL combines symbolic planning with neural RL to improve long-horizon decision-making and prevent reward over-exploitation.


<details>
  <summary>Details</summary>
Motivation: Deep RL agents often over-exploit early rewards and struggle with long-horizon tasks, while purely symbolic approaches don't scale well to continuous settings. Humans learn by combining symbolic reasoning with experiential learning.

Method: Two-stage hybrid framework using logical option-based pretraining to inject symbolic structure into neural RL agents. First stage uses symbolic planning to guide learning away from short-term reward loops, then refines policy via standard environment interaction.

Result: Consistently improves long-horizon decision-making and outperforms strong neural, symbolic, and neuro-symbolic baselines.

Conclusion: Hybrid approach successfully combines symbolic reasoning with neural RL to create more aligned, goal-directed agents without sacrificing deep policy expressivity.
...
### [169] [ProFocus: Proactive Perception and Focused Reasoning in Vision-and-Language Navigation](https://arxiv.org/abs/2603.05530)
*Wei Xue,Mingcheng Li,Xuecheng Wu,Jingqun Tang,Dingkang Yang,Lihua Zhang*

Main category: cs.RO

TL;DR: ProFocus is a training-free VLN framework that uses LLMs and VLMs for proactive perception (via semantic maps and targeted queries) and focused reasoning (via BD-MCTS to prioritize high-value historical waypoints), achieving SOTA zero-shot performance on R2R and REVERIE.


<details>
  <summary>Details</summary>
Motivation: Existing VLN methods inefficiently process redundant visual inputs and treat all historical contexts equally, leading to unfocused reasoning and suboptimal navigation performance.

Method: Progressive framework with: 1) Proactive perception - transforms panoramic views into ego-centric semantic maps, uses orchestration agent to identify missing info and generate targeted visual queries with focus regions; 2) Focused reasoning - Branch-Diverse Monte Carlo Tree Search (BD-MCTS) to identify top-k high-value waypoints from history, then focuses reasoning only on contexts associated with these waypoints.
...
Conclusion: ProFocus successfully addresses VLN challenges by unifying proactive perception and focused reasoning through LLM-VLM collaboration, demonstrating that training-free methods can achieve competitive performance by intelligently managing visual information and historical context.
...
### [170] [Digital-Twin Losses for Lane-Compliant Trajectory Prediction at Urban Intersections](https://arxiv.org/abs/2603.05546)
*Kuo-Yi Chao,Erik Leo Haß,Melina Gegg,Jiajie Zhang,Ralph Raßhofer,Alois Christian Knoll*

Main category: cs.RO

TL;DR: Digital twin-driven V2X trajectory prediction pipeline using Bi-LSTM generator with MSE + novel twin loss for safety-aware multi-agent motion forecasting at intersections.


<details>
  <summary>Details</summary>
Motivation: Need for accurate and safety-conscious trajectory prediction in V2X-enabled urban environments with complex multi-agent interactions, especially at signalized intersections where safety violations can have severe consequences.

Method: Digital twin-driven pipeline combining cooperative perception from vehicles and infrastructure. Uses Bi-LSTM-based generator with structured training objective: standard MSE loss + novel twin loss that encodes infrastructure constraints, collision avoidance, diversity across predicted modes, and rule-based priors from digital twin.

Result: Significantly reduces critical violations (infrastructure and rule violations) while maintaining comparable prediction accuracy (ADE, FDE) and real-time performance on real-world V2X data from urban corridors.

Conclusion: Digital twin-driven multi-loss learning shows strong potential for V2X-enabled intelligent transportation systems by ensuring safety-compliant predictions without sacrificing accuracy, addressing both point-wise accuracy and scene-consistency requirements.
...
### [171] [TEGA: A Tactile-Enhanced Grasping Assistant for Assistive Robotics via Sensor Fusion and Closed-Loop Haptic Feedback](https://arxiv.org/abs/2603.05552)
...
<details>
  <summary>Details</summary>
Motivation: Current teleoperation systems focus on finger positioning but overlook grasp force modulation, which is critical for handling diverse objects. This is especially challenging for users with upper limb disabilities who lack natural tactile feedback.

Method: TEGA integrates EMG-based intent-to-force inference with visuotactile sensing, mapping this information into real-time vibrotactile feedback delivered through a wearable haptic vest, creating a closed-loop assistive teleoperation framework.

Result: User studies demonstrate that the system substantially improves grasp stability and task success rates, validating its effectiveness for assistive robotic applications.

Conclusion: TEGA represents a significant advancement in assistive teleoperation by addressing the critical gap in grasp force modulation, offering intuitive proportional force adjustment through multimodal sensing and haptic feedback.
...
### [172] [PRISM: Personalized Refinement of Imitation Skills for Manipulation via Human Instructions](https://arxiv.org/abs/2603.05574)
*Arnau Boix-Granell,Alberto San-Miguel-Tello,Magí Dalmau-Moreno,Néstor García*

Main category: cs.RO

TL;DR: PRISM combines imitation learning and reinforcement learning with human feedback to refine robotic manipulation policies, enabling adaptation to new goals and constraints while improving robustness and efficiency.


<details>
  <summary>Details</summary>
Motivation: To create a seamless pipeline that bridges imitation learning and reinforcement learning for robotic manipulation, enabling imitation policies to be refined for new fine-grained behaviors and adapted to different goal configurations and constraints while maintaining policy reusability and data efficiency.

Method: PRISM uses instruction-conditioned refinement where an imitation policy trained on generic tasks is refined through RL with reward functions generated from natural language descriptions (Eureka paradigm). The approach incorporates human feedback correction on intermediate rollouts to adapt policies to new goals and constraints.

Result: In simulated pick-and-place tasks, PRISM outperforms policies without human feedback, demonstrating improved robustness during deployment and reduced computational burden.

Conclusion: PRISM successfully integrates IL and RL with human feedback to create adaptable robotic manipulation policies that are more robust and computationally efficient than approaches without human correction.
...
TL;DR: Novel joint learning approach for accurate and efficient robot skill transfer to novel conditions via task inversion learning


<details>
  <summary>Details</summary>
Motivation: Imitation learning fails outside training regions, while transfer learning is data-hungry and lacks zero-shot accuracy. Need for robust skill generalization to novel conditions.

Method: Frames problem as task inversion learning, constructs common representation of forward/inverse tasks, leverages auxiliary forward demonstrations from novel configurations to execute inverse tasks without direct supervision

Result: Outperforms diffusion-based alternatives in simulated and real-world manipulation tasks with diverse objects/tools, shows strong extrapolation capabilities

Conclusion: Proposed joint learning approach enables accurate and efficient knowledge transfer for robot skill generalization to novel conditions
...
TL;DR: RACAS is a robot-agnostic control system using LLM/VLM agents that communicate through natural language to enable autonomous behavior across diverse robotic platforms without retraining or code modification.


<details>
  <summary>Details</summary>
Motivation: Existing approaches require retraining for each new robot or only work on structurally similar platforms, creating a high barrier to prototyping robotic solutions. There's a need for a system that can work across radically different robotic embodiments without modification.

Method: RACAS uses three LLM/VLM-based modules: Monitors (observe sensors), Controller (decides actions), and Memory Curator (maintains context). They communicate exclusively through natural language and only require a robot description, available actions, and task specification - no code changes or model retraining.

Result: RACAS consistently solved all assigned tasks across three radically different platforms: a wheeled ground robot, a novel multi-jointed robotic limb, and an underwater vehicle, demonstrating successful cross-platform operation.

Conclusion: Agentic AI architectures like RACAS can substantially reduce the barrier to prototyping robotic solutions by enabling robot-agnostic control that works across diverse platforms without modification or retraining.
...
TL;DR: A control framework for underactuated soft robots with bounded inputs that enforces rapid exponential stabilization while satisfying dynamics and actuator constraints, validated on various platforms with improved task-space accuracy.


<details>
  <summary>Details</summary>
Motivation: Soft and hybrid soft-rigid robots are underactuated and operate under tight actuator limits, making stable task-space control challenging. Existing nonlinear control strategies often assume full actuation without limits, which doesn't match real-world constraints.

Method: The method enforces a rapidly exponentially stabilizing control Lyapunov function as a convex inequality constraint while simultaneously satisfying underactuated full-body dynamics and actuator bounds. This creates a general control framework for task-space regulation and tracking.

Result: Validated in simulation on three platforms with increasing underactuation: a tendon-driven finger, trimmed helicoid manipulator, and highly underactuated spiral robot. The approach showed improved task-space accuracy and consistent Lyapunov convergence under input limits compared to baseline methods.

Conclusion: The proposed control framework successfully addresses the challenge of task-space control for underactuated soft robots with bounded inputs, achieving superior set-point and trajectory-tracking performance while maintaining stability guarantees.
...
TL;DR: RFM-HRI Dataset captures human responses to robot failures in medical settings, showing failures degrade affective valence and increase negative emotions like frustration.


<details>
  <summary>Details</summary>
Motivation: Understanding how users respond to robot interaction failures is under-explored in HRI, especially in healthcare settings where failures affect task performance and user trust.

Method: Created RFM-HRI Dataset through Wizard-of-Oz studies with 41 participants across lab and hospital settings, systematically inducing four failure types during crash-cart item retrieval tasks, recording multimodal data including facial action units, head pose, speech transcripts, and self-reports.

Result: Failures significantly degrade affective valence and reduce perceived control compared to successful interactions; failures are associated with confusion, annoyance, and frustration, while successful interactions show surprise, relief, and confidence; emotional responses evolve across repeated failures with confusion decreasing and frustration increasing.

Conclusion: Provides foundation for failure detection and recovery methods in embodied HRI through (1) publicly available multimodal dataset, (2) analysis of user responses and recovery strategies, and (3) crash-cart retrieval scenario enabling systematic comparison of recovery strategies for safety-critical applications.
...
TL;DR: SCOUT is a scene graph-based method for open-world interactive object search that uses learned utility scores and relational heuristics, outperforming embedding similarity methods while matching LLM performance with computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Current methods for open-world interactive object search either rely on vision-language embeddings that don't reliably capture task-relevant relational semantics, or use large language models that are too slow and costly for real-time deployment on robots.

Method: SCOUT searches directly over 3D scene graphs by assigning utility scores to rooms, frontiers, and objects using relational exploration heuristics (room-object containment, object-object co-occurrence). Uses offline procedural distillation to extract structured relational knowledge from LLMs into lightweight models for on-robot inference.

Result: Extensive evaluations across symbolic and simulation environments show SCOUT outperforms embedding similarity-based methods and matches LLM-level performance while remaining computationally efficient. Real-world experiments demonstrate effective transfer to physical environments.

Conclusion: SCOUT enables efficient open-world interactive object search under realistic sensing and navigation constraints by combining scene graph-based exploration with learned utility models, bridging the gap between semantic reasoning and real-time deployment.
...
TL;DR: TransMASK: A self-supervised method that learns a mask to filter out irrelevant state components in imitation learning, improving policy generalization to new environments.


<details>
  <summary>Details</summary>
Motivation: Robots need to generalize learned tasks to new environments, but current policies often incorporate irrelevant state components (like background clutter) that hinder generalization. Humans naturally focus only on task-relevant aspects of the environment.

Method: Proposes TransMASK - a self-supervised mask that multiplies with observed state to create latent representations biased toward relevant elements. The mask is trained simultaneously with the policy by leveraging gradient information, where mask rows are normalized to align with the Jacobian of the expert policy.

Result: The method can be combined with various imitation learning frameworks (like diffusion policies) without additional labels or loss function modifications. It identifies relevant state components while suppressing irrelevant ones.

Conclusion: TransMASK enables robots to learn policies that focus only on task-relevant state components, improving generalization to new environments by ignoring irrelevant variables in control decisions.
...
TL;DR: CGP enables fine-grained dexterous manipulation by grounding multi-point contacts through state prediction and tactile feedback, using learned contact-consistency mapping for compliance control.


<details>
  <summary>Details</summary>
Motivation: Enable fine-grained, contact-rich dexterous manipulation by effectively utilizing tactile feedback and multi-point contact information for precise control.

Method: Contact-Grounded Policy (CGP) predicts robot state and tactile feedback, uses learned contact-consistency mapping to convert predictions into controller-executable targets for compliance control, supports both dense tactile arrays and vision-based tactile sensors.

Result: Evaluated across multiple dexterous manipulation tasks using demonstrations collected via teleoperation in both simulation and on physical robot.

Conclusion: CGP provides an effective framework for contact-rich dexterous manipulation by grounding multi-point contacts through tactile feedback and learned consistency mapping.
...
TL;DR: Introduces NC-tALC Dataset - high-fidelity trajectory data for studying transitional autonomous vehicle lane-changing interactions with human-driven vehicles.


<details>
  <summary>Details</summary>
Motivation: Transitional AVs (tAVs) operating between partial and full autonomy are increasingly sharing roads with human-driven vehicles, creating new interaction patterns during complex maneuvers like lane changes. There's a scarcity of high-resolution trajectory data capturing tAV lane-changing behavior needed to assess traffic stability and safety implications.

Method: Created NC-tALC Dataset through two controlled experimental series: (1) tAV lane-changing experiments where a tAV executes lane changes near ACC-equipped target vehicles, and (2) tAV responding experiments where two tAVs respond to cut-in maneuvers by another tAV. Collected 152 trials (72 lane-changing, 80 responding) at 20 Hz with centimeter-level RTK-GPS accuracy.

Result: Produced a comprehensive high-fidelity trajectory dataset that enables analysis of both lane-changing execution and follower response dynamics in mandatory lane-changing scenarios involving tAVs.

Conclusion: The NC-tALC dataset provides a rigorous empirical foundation for evaluating tAV decision-making and interaction dynamics in controlled mandatory lane-changing scenarios, addressing the data scarcity problem in this emerging research area.
...
TL;DR: Proposes environment-aware path generation framework for robotic additive manufacturing using four path planning algorithms to handle dynamic construction environments with obstacles.


<details>
  <summary>Details</summary>
Motivation: Current AM design relies on pre-conceived offline toolpaths, but dynamic construction environments (terrestrial/extraterrestrial) require online path generation methods that can handle obstacles.

Method: Environment-aware path generation framework using four path planning algorithms (two search-based, two sampling-based), evaluated in challenging obstacle arrangements (periodic, random) for closed/open structures with structural and computational metrics.

Result: Evaluated path planners' feasibility in dense obstacle environments, assessed structural performance metrics, identified most essential metrics for toolpath evaluation, and determined most promising path planners for challenging AM applications.

Conclusion: Proposed framework enables online path generation for robotic AM in dynamic environments, with identified best-performing path planners and essential evaluation metrics for obstacle-rich construction scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Effective communication across language barriers in healthcare requires non-verbal cues and gestures, but privacy concerns exist for medical interpreter systems.

Method: Locally deployed open-source models with LLM few-shot prompting for intent detection, plus novel clinical conversation dataset annotated for speech acts and paired with gesture clips.

Result: Achieved 0.90 accuracy, 0.93 weighted precision, and 0.91 weighted F1-Score for identification; outperformed baseline in human-likeness while maintaining comparable appropriateness in user studies.

Conclusion: The privacy-preserving framework significantly improves computational efficiency and generates more human-like gestures for medical interpreter robots while maintaining appropriate communication.
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models rely only on RGB perception, missing thermal signals, and lack explicit safety constraints, making them fragile in novel scenarios with obstacles.

Method: Integrates long-wave infrared thermal perception into pre-trained vision-language backbone for semantic reasoning based on thermodynamic properties, plus safety filter using control barrier functions for deterministic workspace constraint enforcement.

Result: Outperforms RGB-only baselines in real-world experiments on Franka manipulator with temperature-conditioned manipulation, subsurface target localization, and reflection disambiguation while maintaining constrained execution.

Conclusion: Foundation models can effectively leverage non-visible physical modalities for robust manipulation, and safety constraints can be successfully integrated for reliable operation in unstructured environments.
...
TL;DR: VGM2Robot: A data-free framework that uses vision-language models to extract compositional constraints for improving video generative model outputs for robotic manipulation, achieving 43.3% higher success rates.


<details>
  <summary>Details</summary>
Motivation: Video generative models (VGMs) can produce coherent rollout videos for robotic manipulation but often generate physically implausible motions, and converting them to robot actions introduces cumulative errors from imperfect depth estimation and keypoint tracking.

Method: Method uses VLMs to automatically extract compositional constraints from language instructions, then applies them in two stages: (1) constraint-guided rollout selection to filter VGM outputs for physical plausibility, and (2) constraint-based trajectory optimization to refine robot trajectories and correct retargeting errors.

Result: Method improves overall success rate by 43.3% points over the strongest baseline on six real-robot manipulation tasks requiring precise, constraint-sensitive execution, without any task-specific training data.

Conclusion: The framework successfully combines VGMs' motion generation capabilities with VLMs' structured spatial reasoning to achieve zero-shot robotic manipulation with improved physical plausibility and execution accuracy.
...
TL;DR: A two-stage framework combining sampling-based online learning with formal STL reasoning for multi-robot motion planning under STL specifications with kinodynamic constraints.


<details>
  <summary>Details</summary>
Motivation: Existing exact approaches face scalability bottlenecks and limited adaptability, while conventional sampling-based methods require excessive samples to construct optimal trajectories for multi-robot systems with Signal Temporal Logic specifications and kinodynamic constraints.
...
Result: Benchmarking demonstrates improved trajectory efficiency and safety over existing methods. Real-world experiments with autonomous surface vehicles validate robustness and practical applicability in uncertain environments.

Conclusion: The proposed STLcBOT Planner effectively addresses multi-robot motion planning under STL specifications with kinodynamic constraints, offering scalability, probabilistic completeness, and will be released as open-source software.
...
TL;DR: Hierarchical policy (TDGC) separates high-level navigation decisions from low-level gait control for robust quadruped navigation, enabling sim-to-real transfer with explicit adaptation interfaces.


<details>
  <summary>Details</summary>
Motivation: Real-world quadruped navigation faces scale mismatch between high-level decisions and low-level execution, plus instability under environmental changes that challenge sim-to-real transfer and cause falls without adaptation interfaces.

Method: Task-level Decision to Gait Control (TDGC) architecture: low-level RL policy for gait-conditioned locomotion with controllable behavior parameters; high-level policy makes task decisions from sparse terrain cues; structured curriculum with performance-driven progression.

Result: Higher task success rates on mixed terrains and out-of-distribution tests compared to end-to-end approaches.

Conclusion: Hierarchical architecture provides explicit interfaces for deployment tuning, fault diagnosis, and policy refinement, enabling robust quadruped navigation with traceable decision pipeline.
...
TL;DR: CDF-Glove is a lightweight, low-cost cable-driven force-feedback glove for dexterous teleoperation that improves demonstration quality for imitation learning, achieving 4x higher task success rates than no-feedback teleoperation.


<details>
  <summary>Details</summary>
Motivation: High-quality teleoperated demonstrations are a bottleneck for imitation learning in dexterous manipulation. Haptic feedback improves demonstration quality by enabling real-time finger posture adjustments, but existing dexterous teleoperation platforms are typically bulky, expensive, and lack haptic feedback.

Method: Developed CDF-Glove, a lightweight, low-cost cable-driven force-feedback glove with real-time state sensing for 20 finger DoFs (16 directly sensed, 4 passively coupled). Created kinematic model and control stack validated across multiple robotic hands with diverse kinematics. Collected bimanual teleoperation datasets and trained Diffusion Policy baselines.

Result: CDF-Glove achieves 0.4° distal joint repeatability, ~200ms force feedback latency, and 4x improvement in task success rate vs no-feedback teleoperation. Policies trained on teleoperated demonstrations increase success rate by 55% and reduce completion time by 15.2 seconds (47.2% reduction) compared to kinesthetic teaching. Costs only ~US$230.

Conclusion: CDF-Glove provides an affordable, effective solution for high-quality dexterous teleoperation with haptic feedback, significantly improving demonstration quality for imitation learning while being lightweight, low-cost, and open-source.
...
TL;DR: HiLAM is a hierarchical latent action model that discovers high-level skills from actionless videos by modeling long-term temporal structure, outperforming baseline methods.


<details>
  <summary>Details</summary>
Motivation: Existing Latent Action Models (LAMs) focus on short-horizon frame transitions and capture only low-level motion, missing the temporally extended, high-level skills present in actionless videos.

Method: HiLAM uses a pretrained LAM as a low-level extractor to capture latent action sequences, then aggregates these sequences into high-level latent skills through hierarchical modeling of long-term temporal dependencies.

Result: Experiments show HiLAM improves over baseline methods and demonstrates robust dynamic skill discovery capabilities.

Conclusion: HiLAM successfully addresses the limitation of existing LAMs by capturing long-term temporal structure to discover high-level skills from actionless video data.
...
TL;DR: A robust and sample-efficient framework for legged manipulators to open heterogeneous articulated objects using compact feature extraction and adaptive sensory mixing.


<details>
  <summary>Details</summary>
Motivation: Legged manipulators struggle with robust interaction with diverse articulated objects (doors, drawers, cabinets) due to varying articulation types and complex dynamics, while existing RL approaches suffer from sample inefficiency from high-dimensional inputs.

Method: Proposes SAFE (Sampling-based Abstracted Feature Extraction) for compact geometric encoding and ArtIEst (Articulation Information Estimator) to adaptively mix proprioception and exteroception for estimating opening direction and range of motion.

Result: Successfully deployed framework to manipulate various heterogeneous articulated objects in both simulation and real-world robot systems.

Conclusion: The proposed framework enables robust and sample-efficient manipulation of diverse articulated objects by legged manipulators through compact feature representation and adaptive sensory fusion.
...
TL;DR: Small-scale robots struggle with real-world terrain adaptation. SILA Bot uses lizard-inspired design to adapt to granular media by sensing depth via proprioception and adjusting body movement patterns from standing-wave to traveling-wave undulation.


<details>
  <summary>Details</summary>
Motivation: Small-scale robots are largely confined to labs due to poor understanding of robot-terrain interactions at small scales. There's a lack of systematic knowledge about what sensory information small robots need and how they should respond to complex natural terrains.

Method: Developed SILA Bot (Small-scale, Intelligent, Lizard-inspired, Adaptive Robot) that adapts to diverse substrates. Used granular media of varying depths as controlled terrain. Parameterized optimal body movement patterns as linear function of granular depth. Used proprioceptive signals (joint torque) with K-Nearest Neighbors classifier to estimate granular depth. Designed linear feedback controller to modulate body phase based on depth estimation.

Result: Optimal movement patterns range from standing-wave bending (flat ground) to traveling-wave undulation (deep granular media). KNN classifier achieved 95% accuracy in estimating granular depth from proprioceptive signals. Linear feedback controller substantially improved locomotion performance on terrains with unknown depth.

Conclusion: Established principled framework for perception and control in small-scale locomotion. Enables effective terrain-adaptive locomotion while maintaining low computational complexity, addressing key limitations preventing small-scale robots from real-world deployment.
...
TL;DR: TraD-RL method combines expert racing lines, vehicle dynamics constraints via control barrier functions, and multi-stage curriculum learning to improve autonomous racing performance and safety.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning for autonomous driving faces training instability and unsafe actions in high-dynamics racing environments with strong nonlinearities.

Method: 1) Use expert racing line for augmented state representation and reward shaping; 2) Embed vehicle dynamics priors via control barrier functions for safety constraints; 3) Multi-stage curriculum learning from expert guidance to autonomous exploration.

Result: TraD-RL improves both lap speed and driving stability in Tempelhof Airport Street Circuit simulation, achieving synergistic optimization of racing performance and safety.

Conclusion: The proposed TraD-RL method effectively addresses RL limitations in autonomous racing by integrating domain knowledge, safety constraints, and progressive learning strategies.
...
### [192] [DexEMG: Towards Dexterous Teleoperation System via EMG2Pose Generalization](https://arxiv.org/abs/2603.05861)
...
<details>
  <summary>Details</summary>
Motivation: Existing teleoperation systems face trade-offs between performance and portability - vision-based systems have cost/line-of-sight constraints, while mechanical exoskeletons are bulky and restrictive. There's a need for scalable, intuitive interfaces for robotic manipulation and assistive technologies.

Method: Collect synchronized sEMG signals and hand poses via MoCap glove, train EMG2Pose neural network to predict hand kinematics from muscle activity, develop robust hand retargeting algorithm to map predicted poses onto dexterous hand in real-time.

Result: DexEMG achieves high precision in diverse teleoperation tasks, exhibits strong generalization across novel objects and complex environments without intensive individual-specific recalibration.

Conclusion: DexEMG offers a scalable and intuitive interface bridging human intent and robotic execution, suitable for both general-purpose robotic manipulation and assistive technologies.
...
### [193] [AnyCamVLA: Zero-Shot Camera Adaptation for Viewpoint Robust Vision-Language-Action Models](https://arxiv.org/abs/2603.05868)
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models for robot manipulation are sensitive to camera viewpoint changes in unstructured environments, requiring fine-tuning that limits deployment flexibility.

Method: Use feed-forward novel view synthesis model to virtually adjust test-time camera observations in real-time to match training camera configuration, preserving pre-trained VLA capabilities without architectural changes.

Result: Consistently outperforms baselines using data augmentation or 3D-aware features on LIBERO benchmark; enhances viewpoint robustness in real-world scenarios with varying camera extrinsics, intrinsics, and handheld cameras.

Conclusion: Proposed plug-and-play approach enables zero-shot camera adaptation for VLAs, improving viewpoint robustness without additional demonstration data or policy fine-tuning.
...
### [194] [Improved hopping control on slopes for small robots using spring mass modeling](https://arxiv.org/abs/2603.05902)
*Heston Roberts,Pronoy Sarker,Sm Ashikul Islam,Min Gyu Kim*

Main category: cs.RO

TL;DR: Simple spring-mass model shows how slopes destabilize hopping robots; two fixes (touchdown angle adjustment + pre-takeoff torque) cancel unwanted rotation, enabling stable hopping on steep slopes with minimal sensing/computation.


<details>
  <summary>Details</summary>
Motivation: Hopping robots lose balance on slopes because tilted ground creates unwanted rotation at landing, limiting their ability to navigate natural uneven terrain like hills and rubble.

Method: 1) Analyze slope-induced destabilization using simple spring-mass model; 2) Introduce two fixes: adjust body's touchdown angle based on slope, and apply small corrective torque before takeoff to cancel unwanted rotation.

Result: Simulation confirms small slope-aware adjustments dramatically improve landing stability, allowing robots to land smoothly and maintain stable hopping even on steep slopes.

Conclusion: Combining analytical modeling with minimal control actions provides practical path toward reliable hopping on uneven terrain, suitable for low-cost robotic platforms without complex sensing/computation, enabling future autonomous field robots in natural environments.
...
### [195] [Iterative Convex Optimization with Control Barrier Functions for Obstacle Avoidance among Polytopes](https://arxiv.org/abs/2603.05916)
*Shuo Liu,Zhe Huang,Calin A. Belta*

Main category: cs.RO

TL;DR: A convex MPC-DCBF framework for polytopic obstacle avoidance using exact closest-point computations and iterative linearization for real-time safety-critical control.


<details>
  <summary>Details</summary>
Motivation: Existing methods for polytopic obstacle avoidance either use smooth geometric approximations that distort true geometry and restrict feasible sets, or use exact polytope distances in nonconvex MPC formulations that limit real-time performance.

Method: Construct linear discrete-time control barrier function (DCBF) constraints using supporting hyperplanes from exact closest-point computations between convex polytopes. Propose iterative convex MPC-DCBF framework with local linearization of system dynamics and robot geometry to ensure convexity at each iteration.

Result: Reduces computational complexity and enables fast online implementation for safety-critical control and trajectory planning of general nonlinear dynamics. Extends to multi-robot and 3D environments. Numerical experiments show collision-free navigation in cluttered maze scenarios with millisecond-level solve times.

Conclusion: The proposed iterative convex MPC-DCBF framework provides an efficient solution for polytopic obstacle avoidance that maintains exact geometry while achieving real-time performance through convex optimization.
...
### [196] [Swooper: Learning High-Speed Aerial Grasping With a Simple Gripper](https://arxiv.org/abs/2603.05935)
*Ziken Huang,Xinze Niu,Bowen Chai,Renbiao Jin,Danping Zou*

Main category: cs.RO

TL;DR: Swooper uses deep reinforcement learning with a two-stage training approach to achieve high-speed aerial grasping with a single lightweight neural network policy, achieving 84% success rate at speeds up to 1.5 m/s without real-world fine-tuning.


<details>
  <summary>Details</summary>
Motivation: High-speed aerial grasping requires precise flight control and coordinated gripper manipulation, which is challenging to achieve with traditional methods. The authors aim to develop a DRL-based approach that can seamlessly integrate both flight and grasping control in a single lightweight policy.

Method: Two-stage deep reinforcement learning: first pre-train a flight control policy, then fine-tune it to acquire grasping skills. Uses carefully designed reward functions and training framework. The trained policy is deployed zero-shot on a lightweight quadrotor platform with Raspberry Pi 4B.

Result: Training completes in under 60 minutes on standard desktop hardware. Real-world deployment achieves 84% grasp success rate in 25 trials at speeds up to 1.5 m/s without any fine-tuning. Each inference takes only about 1.0 ms on Raspberry Pi 4B.

Conclusion: Swooper demonstrates that DRL can learn robust control policies that seamlessly integrate high-speed flight and grasping, matching state-of-the-art classical systems with sophisticated grippers while using simple off-the-shelf hardware.
...
### [197] [How to Model Your Crazyflie Brushless](https://arxiv.org/abs/2603.05944)
*Alexander Gräfe,Christoph Scherer,Wolfgang Hönig,Sebastian Trimpe*

Main category: cs.RO

TL;DR: Presents a dynamics model for Crazyflie Brushless quadcopter, validates accuracy, demonstrates RL applications for position control and acrobatics, and investigates domain randomization for sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: The Crazyflie Brushless offers 50% more thrust than its predecessor, enabling new agile nano-quadcopter research. Researchers need an accurate dynamics model to develop controllers and test in simulation before hardware deployment.

Method: Developed a dynamics model for Crazyflie Brushless, identified key parameters, validated through simulations and hardware analyses. Used reinforcement learning to train end-to-end neural network position controller and backflip controller (two rotations with 1.8m vertical movement). Investigated domain randomization for sim-to-real transfer.

Result: Successfully created accurate dynamics model validated through simulations and hardware. Demonstrated RL controllers that transfer from simulation to hardware, including acrobatic maneuvers. Found insights on domain randomization effectiveness for bridging sim-to-real gap. Open-sourced the entire project.

Conclusion: The presented dynamics model enables effective simulation-to-hardware transfer for Crazyflie Brushless, supporting rapid controller development and testing. Domain randomization helps bridge the sim-to-real gap, and the open-source platform accelerates nano-quadcopter research.
...
### [198] [PROBE: Probabilistic Occupancy BEV Encoding with Analytical Translation Robustness for 3D Place Recognition](https://arxiv.org/abs/2603.05965)
*Jinseop Lee,Byoungho Lee,Gichul Yoo*

Main category: cs.RO

TL;DR: PROBE is a learning-free LiDAR place recognition descriptor that models BEV occupancy as Bernoulli random variables, analytically marginalizes over continuous translations via polar Jacobian, and achieves competitive performance across diverse LiDAR sensors without dataset-specific tuning.


<details>
  <summary>Details</summary>
Motivation: The paper aims to create a robust LiDAR place recognition descriptor that works across different sensor types without requiring learning or dataset-specific tuning, addressing the challenge of cross-sensor generalization in LiDAR-based localization.

Method: PROBE models each BEV cell's occupancy as a Bernoulli random variable and analytically marginalizes over continuous Cartesian translations using the polar Jacobian, yielding distance-adaptive angular uncertainty. It uses a primary parameter σ_t representing expected translational uncertainty, enabling sensor independence. Similarity matching combines Bernoulli-KL Jaccard with exponential uncertainty gating and FFT-based height cosine similarity for rotation alignment.
...
Conclusion: PROBE provides an effective, learning-free approach to LiDAR place recognition that generalizes across different sensor types without per-dataset tuning, offering a practical solution for cross-sensor localization applications.
...
### [199] [Proprioceptive Shape Estimation of Tensegrity Manipulators Using Energy Minimisation](https://arxiv.org/abs/2603.05976)
...
<details>
  <summary>Details</summary>
Motivation: Existing shape estimation methods for tensegrity manipulators have limitations: exteroceptive sensors are costly and environment-dependent, while proprioceptive approaches lack proven examples for large-scale structures used as manipulators.

Method: Proposes using only inclination angle information relative to gravity for each strut, obtained from IMUs attached to struts, to estimate the entire manipulator shape.

Result: Experiments on a 5-layer tensegrity manipulator with 20 struts (1160mm total length) show shape estimation accuracy of 2.1% of total length, works from arbitrary initial conditions under static conditions, and maintains stable estimation under external disturbances.

Conclusion: Demonstrates that accurate shape estimation of entire tensegrity manipulators can be achieved using only intrinsic inclination angle data from IMUs, providing a practical solution that overcomes limitations of previous approaches.
...
TL;DR: TADPO: A novel RL policy gradient method for off-road autonomous driving that combines off-policy teacher guidance with on-policy student exploration, enabling zero-shot sim-to-real transfer on full-scale vehicles.


<details>
  <summary>Details</summary>
Motivation: Off-road autonomous driving faces challenges with unmapped terrain, variable dynamics, and long-horizon planning with low-signal rewards. Standard RL methods struggle in this setting due to these complexities.

Method: TADPO extends Proximal Policy Optimization (PPO) with a novel policy gradient formulation that leverages off-policy trajectories for teacher guidance and on-policy trajectories for student exploration, creating a vision-based end-to-end RL system.

Result: The system successfully navigates extreme slopes and obstacle-rich terrain in simulation and achieves zero-shot sim-to-real transfer on a full-scale off-road vehicle, representing the first deployment of RL-based policies on such a platform.

Conclusion: TADPO enables effective RL-based autonomous off-road driving by addressing long-horizon planning challenges through combined off-policy guidance and on-policy exploration, with successful real-world deployment.
...
### [201] [HarvestFlex: Strawberry Harvesting via Vision-Language-Action Policy Adaptation in the Wild](https://arxiv.org/abs/2603.05982)
*Ziyang Zhao,Shuheng Wang,Zhonghua Miao,Ya Xiong*

Main category: cs.RO

TL;DR: First study transferring VLA policies to real greenhouse strawberry harvesting using end-to-end closed-loop system with three-view RGB sensing, achieving 74% success rate with less than 4 hours of real data.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of transferring vision-language-action policies to real-world long-horizon, unstructured tasks like greenhouse strawberry harvesting, which involves occlusion and specular reflections.

Method: Built end-to-end closed-loop system on HarvestFlex platform using three-view RGB sensing (two fixed scene views plus wrist-mounted view) without depth clouds or explicit geometric calibration. Collected 3.71h of VR teleoperated demonstrations (227 episodes) and fine-tuned pi_0, pi_0.5, and WALL-OSS models with full fine-tuning and LoRA.

Result: pi_0.5 with full fine-tuning achieved 74.0% success rate with 32.6 seconds per pick and 4.1% damage rate under 50-trial real-greenhouse protocol. Asynchronous inference-control decoupling improved performance over synchronous deployment.

Conclusion: Demonstrated non-trivial closed-loop strawberry picking with fewer than four hours of real data, though limited by close-range observability loss and contact-dynamics mismatch. Shows promise for VLA policy transfer to complex real-world agricultural tasks.
...
### [202] [MagRobot:An Open Simulator for Magnetically Navigated Robots](https://arxiv.org/abs/2603.05992)
*Heng Wang,Haoyu Song,Jiatao Zheng,Yuxiang Han,Kunli Wang*

Main category: cs.RO

TL;DR: First universal open-source simulation platform for designing, visualizing, and benchmarking magnetically navigated medical robots across diverse applications.


<details>
  <summary>Details</summary>
Motivation: Current design of magnetically navigated robots relies heavily on experimental prototyping (time-consuming and costly), and lacks consistent experimental environments for comparing hardware and algorithms across different magnetic navigation systems.

Method: Developed an intuitive GUI-based open-source simulation platform that can simulate both magnetic actuation and tracking tasks for rigid and soft robots, supports third-party anatomical models, and allows customization of hardware and algorithms.

Result: Simulator validated using phantom and ex vivo experiments with continuum and capsule robots; demonstrated functionality through three use cases (bronchoscopy, endovascular intervention, gastrointestinal endoscopy); enables flexible design and optimization of magnetic navigation systems.

Conclusion: The proposed universal open-source simulation platform addresses key challenges in magnetic navigation research by providing a versatile tool for efficient design, visualization, analysis, and benchmarking of magnetically navigated medical robots across diverse applications.
...
### [203] [Restoring Linguistic Grounding in VLA Models via Train-Free Attention Recalibration](https://arxiv.org/abs/2603.06001)
*Ninghao Zhang,Bin Zhu,Shijie Zhou,Jingjing Chen*

Main category: cs.RO

TL;DR: VLA models suffer from "linguistic blindness" - they execute visually plausible actions even when language instructions contradict the scene, prioritizing visual priors over instruction semantics.


<details>
  <summary>Details</summary>
Motivation: While VLA models are promising for generalist robotic policies, their reliability under Out-of-Distribution (OOD) instructions is underexplored. The paper aims to investigate a critical failure mode where VLA policies ignore contradictory language instructions in favor of visual cues.

Method: 1) Introduce ICBench diagnostic benchmark from LIBERO dataset to probe language-action coupling with controlled OOD instruction contradictions; 2) Evaluate three VLA architectures (Pi0, Pi0.5, OpenVLA OFT); 3) Propose Instruction-Guided Attention Recalibration (IGAR) - a train-free inference-time mechanism that rebalances attention distributions to restore language influence.

Result: VLA models frequently succeed at tasks despite logically impossible instructions, revealing strong visual bias. IGAR substantially reduces erroneous execution under OOD contradictory instructions while preserving baseline task performance. Validated on real Franka robotic arm where IGAR prevents manipulation triggered by inconsistent instructions.

Conclusion: VLA models exhibit linguistic blindness where they prioritize visual priors over instruction semantics. The proposed IGAR method effectively mitigates this issue without retraining or architectural modifications, improving reliability of VLA policies under OOD instructions.
...
<details>
  <summary>Details</summary>
Motivation: Current humanoid locomotion research focuses on open, flat environments, but real-world settings (homes, offices) are densely cluttered and geometrically constrained, requiring scene-aware whole-body coordination that remains underexplored.

Method: Developed a VR-based data collection framework that procedurally generates cluttered scenes, captures whole-body human motion through immersive VR navigation, and automatically retargets motion to humanoid robot models with benchmarks for clutter level and locomotion performance.

Result: Created a dataset of 348 trajectories across 145 diverse 3D cluttered scenes, providing the first systematic coupling of full-body human locomotion with scene geometry that shapes it.

Conclusion: MTC addresses the critical gap in scene-aware humanoid locomotion research and provides a foundation for studying geometry-induced adaptation and developing scene-aware planning and control methods for real-world applications.
...
### [205] [RODEO: RObotic DEcentralized Organization](https://arxiv.org/abs/2603.06058)
*Milan Groshev,Eduardo Castelló Ferrer*

Main category: cs.RO

TL;DR: Blockchain-based framework (RODEO) enables robots to operate as DAOs with auditable actions, proof verification, and token-based compensation for autonomous operations.


<details>
  <summary>Details</summary>
Motivation: Current robot autonomy lacks auditability, transparent decision processes, and proper human-robot interaction models needed for extended autonomy in organizational settings.

Method: Proposed RODEO framework with: 1) ROS-ETH bridge connecting DAOs to robots, 2) organizational templates for service robot integration, 3) proof-verification mechanisms for auditable actions, and 4) token-based compensation system.

Result: In 3-day trash collection experiment: robot doubled income, reinvested funds for battery charging, achieved ~1 minute proof validation times, and funded up to 88 hours of future autonomous operation.

Conclusion: Blockchain-based DAOs enable robots and organizations to coordinate tasks with auditable execution proofs and on-chain settlement, creating sustainable autonomous operation ecosystems.
...
### [206] [Lifelong Embodied Navigation Learning](https://arxiv.org/abs/2603.06073)
*Xudong Wang,Jiahua Dong,Baichen Liu,Qi Lyu,Lianqing Liu,Zhi Han*

Main category: cs.RO

TL;DR: Uni-Walker: A lifelong embodied navigation framework using Decoder Extension LoRA to prevent catastrophic forgetting by separating navigation knowledge into task-shared and task-specific components.


<details>
  <summary>Details</summary>
Motivation: Current embodied navigation agents powered by LLMs perform well on individual tasks but suffer from catastrophic forgetting when learning new navigation skills sequentially, preventing them from becoming universal navigation agents.

Method: Proposes Uni-Walker framework with Decoder Extension LoRA (DE-LoRA) to decouple knowledge into task-shared and task-specific components. Uses knowledge inheritance and experts co-activation for shared knowledge, and expert subspace orthogonality constraint with navigation-specific chain-of-thought reasoning for specific knowledge.

Result: Extensive experiments demonstrate Uni-Walker's superiority in building universal navigation agents with lifelong learning capabilities.

Conclusion: Uni-Walker effectively addresses lifelong embodied navigation learning by preventing catastrophic forgetting through knowledge decoupling and specialized strategies for shared and specific knowledge transfer.
...
### [207] [Multimodal Behavior Tree Generation: A Small Vision-Language Model for Robot Task Planning](https://arxiv.org/abs/2603.06084)
*Cristiano Battistini,Riccardo Andrea Izzo,Gianluca Bardaro,Matteo Matteucci*

Main category: cs.RO

TL;DR: Fine-tuned vision-language models can generate executable behavior trees for robotic task planning with 87% success rate, approaching closed-source model performance at lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Combine language models for robotic planning with vision-language models' visual understanding capabilities, addressing the lack of datasets linking visual observations to executable behavior trees.

Method: Construct dataset from existing robotic episodes using large model as teacher, then fine-tune VLMs (500M-4B parameters) via parameter-efficient fine-tuning to generate BehaviorTree.CPP-compatible behavior trees.

Result: Fine-tuned 4B-parameter VLM achieves 87% success rate on household tasks in embodied simulator, approaching state-of-the-art closed-source model performance with significantly lower computational resources.

Conclusion: Compact open-source multimodal models can effectively generate executable behavior trees for robotic task planning, bridging visual perception with structured robotic control.
...
### [208] [A Hazard-Informed Data Pipeline for Robotics Physical Safety](https://arxiv.org/abs/2603.06130)
*Alexei Odinokov,Rostislav Yavorskiy*

Main category: cs.RO

TL;DR: A robotics safety framework combining asset declaration, vulnerability analysis, and synthetic data generation to bridge classical risk engineering with ML safety learning.


<details>
  <summary>Details</summary>
Motivation: To create a systematic approach for robotics safety that connects traditional risk engineering methods with modern machine learning pipelines, addressing the gap between formal safety analysis and data-driven ML systems.

Method: Structured framework based on explicit asset declaration, systematic vulnerability enumeration, and hazard-driven synthetic data generation. Uses formalized hazard ontology to ground safety envelope learning and aligns classical safety engineering with digital twin simulation and ML training.

Result: The framework enables safety envelope learning grounded in formal hazard analysis and creates alignment between classical safety engineering, digital twin simulation, synthetic data generation, and machine learning model training.

Conclusion: The proposed framework successfully bridges classical risk engineering with modern ML pipelines, providing a structured approach to robotics physical safety through systematic vulnerability analysis and hazard-driven data generation.
...
TL;DR: Object-centric gaze grounding framework with sticky-glance algorithm for robust intent recognition in multi-object environments, achieving high tracking and selection accuracy with minimal gaze samples.
...
Result: Achieves tracking rate of 0.94 for dynamic targets and selection accuracy of 0.98 for static targets with minimal 3 gaze samples, reduces task duration by nearly 10%, and demonstrates improved robustness, efficiency, and reduced workload.

Conclusion: The proposed framework enables robust gaze-based intent recognition for impaired users in complex environments, providing high-readiness control and human-in-loop feedback for practical assistive applications.
...
### [210] [Dual-Agent Multiple-Model Reinforcement Learning for Event-Triggered Human-Robot Co-Adaptation in Decoupled Task Spaces](https://arxiv.org/abs/2603.06163)
*Yaqi Li,Zhengqi Han,Huifang Liu,Steven W. Su*

Main category: cs.RO

TL;DR: Event-driven shared-control rehabilitation robot uses DAMMRL to optimize human-robot co-adaptation for upper-limb reaching tasks


<details>
  <summary>Details</summary>
Motivation: Traditional fixed-frequency control in rehabilitation robots causes trajectory oscillations due to variable inverse-kinematics execution times, requiring better human-robot co-adaptation

Method: Event-driven progression with admission spheres, plus Dual Agent Multiple Model Reinforcement Learning (DAMMRL) where human selects sphere radius and robot adjusts Cartesian steps

Result: Suppresses waypoint chatter, balances spatial precision with temporal efficiency, and significantly improves success rates in object acquisition tasks

Conclusion: Event-triggered DAMMRL approach effectively optimizes human-robot co-adaptation for upper-limb rehabilitation tasks
...
### [211] [DreamToNav: Generalizable Navigation for Robots via Generative Video Planning](https://arxiv.org/abs/2603.06190)
*Valerii Serpiva,Jeffrin Sam,Chidera Simon,Hajira Amjad,Iana Zhura,Artem Lykov,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: DreamToNav uses generative video models to translate natural language instructions into robot motion by having the robot "dream" the task in synthetic video before execution.


<details>
  <summary>Details</summary>
Motivation: To enable intuitive human-robot interaction through natural language instructions instead of rigid waypoint navigation, allowing users to provide vague prompts like "Follow the person carefully" that the system can interpret and execute.

Method: Pipeline refines vague language prompts into precise visual descriptions using Qwen 2.5-VL-7B-Instruct, then uses NVIDIA Cosmos 2.5 to generate physically consistent videos of the robot performing the task. From synthetic videos, extracts kinematic paths using visual pose estimation, robot detection, and trajectory recovery.

Result: Achieves 76.7% success rate on indoor navigation tasks with wheeled mobile robot and quadruped robot, with final goal errors within 0.05-0.10 m and trajectory tracking errors below 0.15 m.
...
### [212] [KISS-IMU: Self-supervised Inertial Odometry with Motion-balanced Learning and Uncertainty-aware Inference](https://arxiv.org/abs/2603.06205)
*Jiwon Choi,Hogyun Kim,Geonmo Yang,Juhui Lee,Younggun Cho*

Main category: cs.RO

TL;DR: KISS-IMU is a self-supervised inertial odometry framework that eliminates ground truth dependency by using LiDAR-based ICP registration and pose graph optimization as supervisory signals, enabling robust performance across diverse environments.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for inertial odometry heavily rely on ground truth data during training, which limits scalability and generalization to unseen and diverse environments. There's a need for methods that can learn robust inertial odometry without ground truth supervision.

Method: The framework uses LiDAR-based ICP registration and pose graph optimization as a lightweight supervisory signal for self-supervised training of IMU networks. It employs two key principles: keeping the IMU stable through motion-aware balanced training, and keeping the IMU strong through uncertainty-driven adaptive weighting during inference. The approach trains only the IMU network without requiring additional learnable processes for LiDAR.

Result: Comprehensive experiments on various real-world platforms including quadruped robots demonstrate the framework's effectiveness across diverse motion patterns and scenarios. The method achieves robust performance without relying on joint multi-modal learning or ground truth supervision.

Conclusion: KISS-IMU presents a scalable and generalizable self-supervised inertial odometry framework that eliminates ground truth dependency while maintaining robustness across diverse environments, making it suitable for real-world robotic applications.
...
### [213] [Few-Shot Neural Differentiable Simulator: Real-to-Sim Rigid-Contact Modeling](https://arxiv.org/abs/2603.06218)
*Zhenhao Huang,Siyuan Luo,Bingyang Zhou,Ziqiu Zeng,Jason Pho,Fan Shi*

Main category: cs.RO

TL;DR: A few-shot real-to-sim approach combining analytical physics with GNNs to create differentiable simulators that learn from minimal real data and enable efficient policy learning.


<details>
  <summary>Details</summary>
Motivation: Analytical simulators fail to capture complex contact dynamics, while learning-based simulators require large amounts of costly real-world data. Need to bridge this gap for robotic learning and control.

Method: Proposes few-shot real-to-sim approach that calibrates analytical simulators with small real-world data to generate synthetic datasets. Uses mesh-based GNN to implicitly model rigid-body forward dynamics with surrogate gradients for collision detection, achieving full differentiability.

Result: Learning-based simulators outperform differentiable baselines in replicating real-world trajectories. Differentiable design supports gradient-based optimization, validated through simulation-based policy learning in multi-object interaction scenarios.

Conclusion: Differentiable simulation with few-shot real-world grounding improves simulation fidelity with minimal supervision and increases policy learning efficiency, providing a powerful direction for advancing robotic manipulation and control.
...
### [214] [Towards Robotic Lake Maintenance: Integrating SONAR and Satellite Data to Assist Human Operators](https://arxiv.org/abs/2603.06266)
*Ahmed H. Elsayed,Christoph Manss,Tarek A. El-Mihoub,Andrej Lejman,Frederic Stahl*

Main category: cs.RO

TL;DR: Two-stage approach using satellite imagery and USV with SONAR for targeted weed harvesting in artificial lakes


<details>
  <summary>Details</summary>
Motivation: Artificial water bodies require continuous monitoring and maintenance due to rapid underwater vegetation growth that must be harvested to preserve ecological balance

Method: 1) Initial SAV detection using satellite-derived APA index to identify areas of interest; 2) High-resolution bathymetric mapping with USV equipped with multibeam SONAR for precise vegetation location and quantification

Result: Preliminary results demonstrate feasibility of integrating satellite imagery and underwater acoustic sensing for improved vegetation management

Conclusion: Two-stage approach enables effective human-robot collaboration, narrows search space, reduces manual workload, and makes harvesting process less labor-intensive
...
### [215] [SuperSuit: An Isomorphic Bimodal Interface for Scalable Mobile Manipulation](https://arxiv.org/abs/2603.06280)
*Tongqing Chen,Hang Wu,Jiasen Wang,Xiaotao Li,Zhu Jin,Lu Fang*

Main category: cs.RO

TL;DR: SuperSuit is a bimodal data acquisition framework for mobile manipulators that supports both teleoperation and active demonstration with consistent kinematic representations, enabling scalable collection of long-horizon demonstrations.


<details>
  <summary>Details</summary>
Motivation: Acquiring high-quality, long-horizon demonstrations for wheeled mobile manipulators is challenging due to the need for continuous coordination between locomotion and manipulation, and limitations in existing teleoperation/wearable interfaces.

Method: A bimodal framework with shared kinematic interface that maps human stepping to continuous base velocities, uses isomorphic wearable arm for manipulation, and employs shift-invariant delta-joint representation for policy training to handle calibration offsets.

Result: 2.6× higher demonstration throughput in active mode vs teleoperation baseline, comparable policy performance when substituting teleoperation data with active demonstrations, and monotonic performance improvement with increased active data volume.

Conclusion: Consistent kinematic representations across collection modalities enable scalable data acquisition for long-horizon mobile manipulation, addressing the fundamental bottleneck in embodied AI.
...
### [216] [Open-Source Based and ETSI Compliant Cooperative, Connected, and Automated Mini-Cars](https://arxiv.org/abs/2603.06343)
...
TL;DR: Researchers propose using 1:10 scaled mini-cars with ROS2 and OScar as an affordable intermediate platform for testing cooperative autonomous vehicle algorithms before real-world deployment.


<details>
  <summary>Details</summary>
Motivation: The transition from simulation to real-world testing of cooperative autonomous vehicle algorithms requires huge investments that create barriers for researchers. There's a need for an intermediate, cost-effective platform that bridges the gap between simulations and full-scale vehicle testing.

Method: Developed 1:10 scaled mini-cars equipped with Jetson Orin board running ROS2 for autonomous operations, sensors for perception, and Raspberry Pi with OScar (Open Stack for Car) implementing a full ETSI C-ITS compliant stack for connectivity and cooperative functions.

Result: Successfully demonstrated the platform's feasibility by implementing and validating the Day-1 intersection collision warning (ICW) application, showing the mini-car platform can effectively test cooperative intelligent transport systems.

Conclusion: The proposed scaled mini-car platform with ROS2 and OScar provides an affordable, practical intermediate step for testing cooperative autonomous vehicle algorithms, bridging the gap between simulations and expensive real-world vehicle testing while maintaining compliance with industry standards.
...
### [217] [Safe Consensus of Cooperative Manipulation with Hierarchical Event-Triggered Control Barrier Functions](https://arxiv.org/abs/2603.06356)
...
<details>
  <summary>Details</summary>
Motivation: Need for coordinated formation tracking of heavy/bulky payloads by multiple manipulators while enforcing strict safety constraints in varying environments with limited communication and real-time computation budgets.

Method: 1) Consensus-based protocol using local neighbor info for translational/rotational consistency; 2) Three-level hierarchical event-triggered safety architecture with CBFs; 3) Risk-aware leader selection and smooth switching strategy to reduce computation.

Result: Validated through real-world hardware experiments with two Franka manipulators and comprehensive simulations with dynamic obstacles. Achieved higher precision cooperation under strict safety constraints with substantially reduced computational cost and communication frequency compared to baselines.

Conclusion: Proposed distributed control framework successfully enables safe, coordinated multi-manipulator transport with reduced computation and communication requirements through hierarchical event-triggered CBFs and consensus coordination.
...
### [218] [Data Analogies Enable Efficient Cross-Embodiment Transfer](https://arxiv.org/abs/2603.06450)
*Jonathan Yang,Chelsea Finn,Dorsa Sadigh*

Main category: cs.RO

TL;DR: Cross-embodiment robot policy transfer benefits more from structured paired demonstrations (data analogies) than from simply scaling unpaired heterogeneous data.


<details>
  <summary>Details</summary>
Motivation: Generalist robot policies use heterogeneous data from various robots, scenes, and viewpoints, but it's unclear how to best organize this data to improve performance in specific target settings.

Method: Conducted controlled experiments varying end-effector morphology, robot platform appearance, and camera perspective. Compared scaling demonstrations vs. broadening diversity systematically, focusing on data analogies (paired demonstrations aligning scenes, tasks, and trajectories across embodiments).

Result: Simulation experiments showed perceptual shifts (viewpoint) benefit from broad diversity, while morphology shifts benefit more from data analogies. Real-world cross-embodiment transfer improved by 22.5% over large-scale unpaired datasets by changing data composition.

Conclusion: Data composition matters more than scale for cross-embodiment transfer. Structured paired demonstrations (data analogies) are particularly effective for handling morphology differences between robots.
...
### [219] [History-Conditioned Spatio-Temporal Visual Token Pruning for Efficient Vision-Language Navigation](https://arxiv.org/abs/2603.06480)
*Qitong Wang,Yijun Liang,Ming Li,Tianyi Zhou,Christopher Rasmussen*

Main category: cs.RO

TL;DR: Training-free spatio-temporal vision token pruning for VLA-based VLN that reduces computational latency while preserving navigation accuracy through attention-based token selection and query-guided filtering.


<details>
  <summary>Details</summary>
Motivation: VLA models show strong navigation performance but have high computational cost that limits real-time deployment; need to bridge gap between large-scale multimodal modeling and efficient real-time robotic navigation.

Method: Training-free spatio-temporal vision token pruning framework: spatial token selection for current view + spatio-temporal compression for historical memories, using attention-based token importance and query-guided spatio-temporal filtering without retraining pretrained models.

Result: Significantly outperforms existing pruning strategies, preserves superior navigation accuracy under extreme pruning, maintains competitive inference efficiency, and validates reliable low-latency navigation on real quadruped robot.

Conclusion: Proposed plug-and-play pruning framework helps bridge gap between large-scale multimodal modeling and efficient real-time embodied deployment in robotic navigation systems.
...
### [220] [Control Barrier Corridors: From Safety Functions to Safe Sets](https://arxiv.org/abs/2603.06494)
*Ömür Arslan,Nikolay Atanasov*

Main category: cs.RO

TL;DR: The paper introduces Control Barrier Corridors (CBCs) that unify control barrier functions and safe motion corridors by converting barrier functions into local safe goal regions for reference-governor control.


<details>
  <summary>Details</summary>
Motivation: Current safety methods for robotics use either functional approaches (control barrier functions) or geometric approaches (safe motion corridors), but these are technically distinct. There's a need to unify these approaches to create more effective safety frameworks for autonomous systems operating in complex environments.

Method: The paper introduces Control Barrier Corridors (CBCs) that convert control barrier functions into local safe goal regions for reference goal selection in feedback control systems. The approach requires matching control convergence rate with barrier decay rate for convex barrier functions.

Result: The method enables individual state safety to be extended locally over control barrier corridors for convex barrier functions. This allows for safely reachable persistent goal selection over continuously changing barrier corridors during system motion.

Conclusion: Control Barrier Corridors provide a unified framework that bridges functional and geometric safety approaches, enabling verifiably safe and persistent path following in autonomous exploration of unknown environments, with demonstrated applications on various system types.
...
### [221] [A Unified Low-Dimensional Design Embedding for Joint Optimization of Shape, Material, and Actuation in Soft Robots](https://arxiv.org/abs/2603.06497)
*Vittorio Candiello,Manuel Mekkattu,Mike Y. Michelis,Robert K. Katzschmann*

Main category: cs.RO

TL;DR: A unified low-dimensional design embedding for soft robots that jointly optimizes shape, material, and actuation using structured basis functions, outperforming neural network and voxel-based approaches with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Soft robot design requires joint consideration of geometry, material composition, and actuation, but current approaches face computational challenges from nonlinear mechanics, contact handling, and non-smooth state transitions that limit gradient-based optimization.

Method: Introduces a smooth, low-dimensional design embedding using continuous deformation maps for shape variation and spatial fields for material properties, both constructed from shared basis functions to unify shape morphing, multi-material distribution, and actuation.

Result: The structured embedding outperforms neural network and voxel-based baselines across multiple dynamic tasks, shows predictable scaling with basis functions (unlike neural networks), and enables efficient joint co-optimization that beats sequential strategies.

Conclusion: Structuring the design space itself enables efficient co-design of soft robots, with the unified embedding providing expressive design capabilities while reducing search space dimensionality and maintaining compatibility with black-box simulation pipelines.
...
### [222] [CFEAR-Teach-and-Repeat: Fast and Accurate Radar-only Localization](https://arxiv.org/abs/2603.06501)
*Maximilian Hilger,Daniel Adolfsson,Ralf Becker,Henrik Andreasson,Achim J. Lilienthal*

Main category: cs.RO

TL;DR: CFEAR-TR is a teach-and-repeat localization system using a single spinning radar that achieves lidar-level accuracy (0.117m, 0.096°) in adverse weather by jointly aligning live scans to stored maps and recent keyframes.


<details>
  <summary>Details</summary>
Motivation: Reliable localization in prior maps is essential for autonomous navigation under adverse weather conditions where optical sensors (like cameras and lidars) may fail. Radar sensors work well in poor visibility but need robust localization methods.

Method: Uses a teach-and-repeat pipeline with single spinning radar. Represents radar scans as sparse oriented surface points from Doppler-compensated measurements. Localizes by jointly aligning live scans to both stored teach-pass scans and a sliding window of recent live keyframes. Stores map in a pose graph for traversal during localization.

Result: Achieves localization accuracy of 0.117m and 0.096° on Boreas dataset, improving over previous state-of-the-art by up to 63%. Runs efficiently at 29Hz, substantially narrowing gap to lidar-level localization, especially in heading estimation.

Conclusion: CFEAR-TR provides easily deployable, lightweight, and robust radar-based navigation that works across different seasons and weather conditions, with performance approaching lidar systems while being more resilient to adverse weather.
...
### [223] [SG-DOR: Learning Scene Graphs with Direction-Conditioned Occlusion Reasoning for Pepper Plants](https://arxiv.org/abs/2603.06512)
*Rohit Menon,Niklas Mueller-Goldingen,Sicong Pan,Gokul Krishna Chenchani,Maren Bennewitz*

Main category: cs.RO

TL;DR: SG-DOR: Scene Graphs with Direction-Conditioned Occlusion Reasoning for robotic harvesting in dense crops, using graph neural networks to model physical attachments and occlusion relationships between organs.


<details>
  <summary>Details</summary>
Motivation: Robotic harvesting in dense crop canopies requires understanding not just geometry but also explicit direction-conditioned occlusion relationships to identify which organs obstruct target fruits during harvesting interventions.

Method: SG-DOR framework uses instance-segmented organ point clouds to infer scene graphs encoding physical attachments and direction-conditioned occlusion. Introduces occlusion ranking task and direction-aware graph neural architecture with per-fruit leaf-set attention and union-level aggregation.

Result: Experiments on multi-plant synthetic pepper dataset show improved occlusion prediction (F1=0.73, NDCG@3=0.85) and attachment inference (edge F1=0.83) over strong ablations.

Conclusion: SG-DOR provides structured relational signals for downstream intervention planning in robotic harvesting, effectively modeling complex occlusion relationships in dense crop environments.
...
TL;DR: A two-wheel monopedal robot that rolls, jumps, and reorients in low-gravity using only three actuators for multimodal locomotion.


<details>
  <summary>Details</summary>
Motivation: To explore multimodal locomotion capabilities for robots operating on low-gravity celestial bodies, where traditional wheeled or legged robots face challenges.

Method: Uses two reaction wheels for spatial orientation control with two controllers: ground balancing for jump direction and aerial reorientation for landing leg positioning. Only three actuators total enable rolling, jumping, landing, and self-righting.

Result: Successfully demonstrated rolling, targeted jumping and landing, and self-righting capabilities with a compact system (0.33m, 1.25kg).

Conclusion: Simple switching between locomotion modes enables effective navigation of varying landscapes and environmental conditions on low-gravity bodies.
...
TL;DR: Learning symbolic and subsymbolic temporal constraints from human demonstrations for bimanual manipulation planning


<details>
  <summary>Details</summary>
Motivation: Existing approaches address high-level task planning and low-level movement synchronization in isolation, leaving a gap between symbolic temporal reasoning and concrete timing parameters needed for bimanual coordination.

Method: Three-part approach: (1) 3D timing representation using multivariate Gaussian Mixture Models for subsymbolic temporal relationships, (2) DPLL-based algorithm to find contradiction-free Allen relations assignments for symbolic task modes, (3) optimization-based planning combining both constraint types.

Result: Method generates temporally parametrized plans closer to human demonstrations than baseline approaches across several datasets.

Conclusion: The approach successfully bridges the gap between high-level symbolic reasoning and low-level timing parameters for bimanual manipulation, enabling executable, temporally coordinated robot plans.
...
### [226] [Fly360: Omnidirectional Obstacle Avoidance within Drone View](https://arxiv.org/abs/2603.06573)
*Xiangkai Zhang,Dizhe Zhang,WenZhuo Cao,Zhaoliang Wan,Yingjie Niu,Lu Qi,Xu Yang,Zhiyong Liu*

Main category: cs.RO

TL;DR: Fly360 enables UAVs to perform omnidirectional obstacle avoidance using panoramic vision, overcoming limitations of forward-view sensors.


<details>
  <summary>Details</summary>
Motivation: Current UAV obstacle avoidance methods rely on limited field-of-view sensors and fail when movement direction differs from heading, creating a need for full-spatial awareness solutions.

Method: Two-stage perception-decision pipeline: panoramic RGB → depth maps → lightweight policy network → velocity commands, with fixed random-yaw training strategy.

Result: Fly360 achieves stable omnidirectional obstacle avoidance and outperforms forward-view baselines across all tested flight tasks in both simulation and real-world experiments.

Conclusion: The proposed Fly360 system successfully addresses omnidirectional obstacle avoidance for panoramic drones, demonstrating superior performance over traditional forward-view approaches.
...
### [227] [Uncertainty-Aware Adaptive Dynamics For Underwater Vehicle-Manipulator Robots](https://arxiv.org/abs/2603.06548)
*Edward Morgan,Nenyi K Dadson,Corina Barbalata*

Main category: cs.RO

TL;DR: Novel uncertainty-aware adaptive dynamics model for underwater vehicle-manipulator systems that maintains linear parameter structure with physical consistency constraints, enabling online estimation with quantified uncertainty.


<details>
  <summary>Details</summary>
Motivation: Underwater vehicle-manipulator systems require accurate dynamic models to handle time-varying hydrodynamic effects, but existing approaches often lack physical consistency guarantees and uncertainty quantification needed for reliable control and simulation.

Method: Proposes a framework that maintains linearity in lumped parameters while embedding convex physical consistency constraints (inertia, damping, friction, hydrostatics) during online estimation using moving horizon estimation to stack regressors and quantify parameter evolution uncertainty.

Result: Experiments on BlueROV2 Heavy with 4-DOF manipulator show rapid convergence with R² = 0.88-0.98 for manipulator fits, good vehicle motion reproduction under coupling/noise, median solver time ~0.023s per update, consistent MAE/RMSE reductions vs fixed models, and near 100% confidence interval coverage.

Conclusion: The uncertainty-aware adaptive framework enables physically plausible parameter estimation with reliable uncertainty quantification, supporting online feedforward control and simulation for underwater vehicle-manipulator systems with time-varying hydrodynamic effects.
