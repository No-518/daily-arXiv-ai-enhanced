...
TL;DR: UniFit is a universal virtual try-on framework that uses a Multimodal Large Language Model to address semantic gaps between text instructions and reference images, and data scarcity in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations in existing VTON methods: semantic gap between text instructions and reference images, and data scarcity in complex scenarios that prevents building a universal framework.
...
Conclusion: The proposed framework successfully addresses key VTON challenges through MLLM guidance and progressive training, enabling universal virtual try-on capabilities with superior performance.
...
TL;DR: EfficientSAM3 is a family of efficient models for on-device concept segmentation and tracking, developed through Progressive Hierarchical Distillation (PHD) to transfer capabilities from the computationally expensive SAM3 model.


<details>
  <summary>Details</summary>
Motivation: SAM3's unified architecture with shared vision backbone, DETR-style detector, and dense-memory tracker is too computationally intensive for on-device applications, requiring a more efficient alternative.

Method: Progressive Hierarchical Distillation (PHD) in three stages: (1) Encoder Distillation with prompt-in-the-loop training on SA-1B, (2) Temporal Memory Distillation using Perceiver-based module on SA-V, and (3) End-to-End Fine-Tuning on SAM3 PCS data. Uses lightweight backbones like RepViT, TinyViT, and EfficientViT.

Result: Achieves strong performance-efficiency trade-offs on VOS datasets, enabling on-device concept segmentation and tracking while maintaining high fidelity to the teacher model's behavior.

Conclusion: EfficientSAM3 successfully enables on-device visual understanding through progressive distillation, providing a spectrum of efficient models for concept segmentation and tracking applications.
...
TL;DR: The paper proposes four novel extensions to model-based 6D pose estimation methods to improve robustness under occlusion: dynamic non-uniform dense sampling, multi-hypothesis inference, iterative refinement, and occlusion-focused training augmentations, achieving significant accuracy improvements and faster inference.


<details>
  <summary>Details</summary>
Motivation: Accurate 6D object pose estimation is vital for robotics and AR, but existing methods struggle with unseen objects and are prone to error propagation under occlusion in multi-stage pipelines.

Method: Four key extensions: (1) dynamic non-uniform dense sampling focusing on visible regions, (2) multi-hypothesis inference with confidence-ranked candidates, (3) iterative pose refinement, and (4) occlusion-focused training augmentations for robustness.

Result: Achieves >5% accuracy improvement on ICBIN and >2% on BOP benchmarks, with approximately 3x faster inference compared to existing methods.

Conclusion: The proposed approach effectively addresses occlusion challenges in 6D pose estimation through novel sampling, inference, and training strategies, demonstrating significant performance gains across standard benchmarks.
...
TL;DR: The paper addresses the data scarcity problem in historical map analysis by proposing methods to generate synthetic training data that mimics the visual characteristics of specific historical map corpora, enabling effective domain-adaptive semantic segmentation.


<details>
  <summary>Details</summary>
Motivation: Historical document analysis faces challenges due to limited annotated training data, especially for homogeneous map corpora. Manual annotation is time-consuming, and existing synthetic data often lacks realism and diversity needed for effective learning.

Method: Proposes two approaches: (1) automatic deep generative method and (2) manual stochastic degradation technique to transfer cartographic style from original maps onto vector data, emulating visual uncertainty and noise in historical map scans.

Result: Generated effectively unlimited synthetic historical maps suitable for land-cover interpretation tasks. The synthetic datasets were successfully used for domain-adaptive semantic segmentation using a Self-Constructing Graph Convolutional Network.

Conclusion: The proposed data bootstrapping methods effectively address the training data scarcity problem in historical map analysis by creating realistic and diverse synthetic datasets that enable successful domain adaptation for semantic segmentation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing 6D pose estimation methods have limitations: model-based methods require exact CAD models and transfer poorly, model-free methods fail under challenging conditions, and category-level approaches are too general and ignore environmental priors, limiting industrial practicality.

Method: Box6D uses a fast binary search to infer box dimensions from single RGB-D observation, estimates poses using category CAD templates instead of instance-specific models, and employs depth-based plausibility filtering with early-stopping to reject implausible hypotheses.
...
Conclusion: Box6D provides an effective balance of flexibility and accuracy for warehouse box pose estimation, overcoming limitations of existing methods while significantly improving computational efficiency.
...
TL;DR: A two-stage self-improvement method that uses VLM-generated rationales as intermediate supervision to bridge the rationale gap in domain-specific video classification, achieving better performance than direct supervised fine-tuning without new annotations.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with domain-specific video classification due to a 'rationale gap' where sparse domain data can't bridge the semantic distance between complex spatio-temporal content and abstract labels.

Method: Two-stage approach: 1) Generate detailed textual rationales for each video using VLMs, then fine-tune on these self-generated rationales; 2) Perform conventional supervised fine-tuning on task labels after the model has acquired domain reasoning.
...
Conclusion: Self-generated rationales provide an effective, annotation-efficient paradigm for adapting VLMs to domain-specific video analysis by bridging the rationale gap.
...
<details>
  <summary>Details</summary>
Motivation: CLIP's focus on single-label, single-granularity alignment limits effectiveness in complex domains where images have multiple high-level labels across different annotation granularities, particularly in medical imaging.

Method: Proposes Multi-Granular Language Learning (MGLL) framework with structured multi-label supervision, cross-granularity text integration, soft-label supervision with point-wise constraints, and smooth KL divergence for cross-granularity consistency.
...
Conclusion: MGLL effectively addresses multi-label and cross-granularity alignment challenges in vision-language models, demonstrating superior performance as a plug-and-play module for complex domains like medical imaging.
...
TL;DR: Automated method to extract standard 2D echocardiography views from 3D cardiac ultrasound volumes using deep learning and anatomical heuristics, validated with 96% accuracy by cardiologists and maintaining diagnostic quality.


<details>
  <summary>Details</summary>
Motivation: 3D echocardiography offers better image quality and faster acquisition than conventional 2D ultrasound, but physicians are accustomed to interpreting 2D views. This bridges the gap by automatically generating familiar 2D views from 3D data.

Method: Combines deep learning view classifier with anatomical landmark-based heuristics and cardiologist-provided rules to reconstruct standard echocardiography views from 3D ultrasound volumes.

Result: Validated with 96% accuracy on 1,600 videos from 2 hospitals by blinded cardiologist evaluation. Extracted 2D videos preserved spatial calibration and diagnostic features, worked with existing AI models (EchoPrime, PanEcho), and generated clinical-grade measurements (EchoNet-Measurement).

Conclusion: The approach successfully bridges 3D scanning benefits with clinician familiarity by automatically generating accurate 2D views from 3D volumes, enabling real-world clinical interpretation while maintaining diagnostic quality.
...
<details>
  <summary>Details</summary>
Motivation: Current VSGG systems are closed, feed-forward pipelines without human guidance, while promptable segmentation models lack semantic reasoning. There's a need to unify interactive prompting with comprehensive scene understanding.

Method: Uses a Dynamic Interaction Discovery Module for subject-conditioned object prompts and a Semantic Classification Head for joint entity-predicate reasoning. Segments and tracks subjects across time, discovers interacting objects, and predicts triplets automatically.
...
TL;DR: InfoCLIP is a method that uses information theory to transfer alignment knowledge from pretrained CLIP to open-vocabulary semantic segmentation, preventing overfitting and preserving vision-language alignment during fine-tuning.
...
Method: InfoCLIP leverages mutual information objectives to: 1) compress pixel-text modality alignment from pretrained CLIP to reduce noise from coarse-grained local semantics, and 2) maximize mutual information between pretrained CLIP and fine-tuned model to transfer compact local semantic relations suited for segmentation.
...
Conclusion: InfoCLIP demonstrates adaptability and superiority in asymmetric transfer for open-vocabulary semantic segmentation while preserving the original CLIP's vision-language alignment.
...
TL;DR: Proposes a consistency regularization approach using BI-RADS-inspired morphological features to mitigate destructive task interference in multi-task learning for breast ultrasound tumor segmentation, achieving significant improvements in generalization across external datasets.
...
Method: Novel consistency regularization approach composed of differentiable BI-RADS-inspired morphological features to reduce interference between segmentation and classification tasks.

Result: Statistically significant improvements (p<0.001) in segmentation generalization: UDIAT (0.81 vs 0.59 Dice), BUSI (0.66 vs 0.56), BUS-UCLM (0.69 vs 0.49). Achieves state-of-the-art segmentation on UDIAT dataset.

Conclusion: The proposed multi-task learning approach with consistency regularization effectively mitigates destructive task interference and significantly improves generalization performance in breast ultrasound tumor segmentation across multiple external datasets.
...
TL;DR: A detection-guided generative framework that predicts hierarchical category and attribute tokens for unified visual semantic understanding, outperforming similarity-based and multi-stage approaches.


<details>
  <summary>Details</summary>
Motivation: Current approaches relying on global similarity struggle with fine-grained category distinctions and category-specific attribute diversity in large-scale e-commerce scenarios.

Method: Extract refined ROI-level features for detected objects and use a BART-based generator to produce semantic tokens in coarse-to-fine sequence covering category hierarchies and property-value pairs, with property-conditioned attribute recognition.
...
Conclusion: Achieves stronger fine-grained recognition and more coherent unified inference for visual semantic understanding.
...
TL;DR: Proposes FADS, a fairness-aware demonstration selection method for in-context learning that addresses demographic imbalances in medical image reasoning with MLLMs, achieving improved fairness without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: MLLMs show promise for medical image reasoning but suffer from fairness issues across demographic groups. Existing debiasing methods require large labeled datasets or fine-tuning, which are impractical for foundation-scale models.

Method: Proposes Fairness-Aware Demonstration Selection (FADS) that uses clustering-based sampling to create demographically balanced and semantically relevant demonstrations for in-context learning, avoiding the need for fine-tuning.
...
Conclusion: FADS offers an efficient and scalable path toward fair medical image reasoning, highlighting the potential of fairness-aware in-context learning as a data-efficient solution for equitable medical AI.
...
TL;DR: The paper proposes a graph-based method using GCNs and Gaussianization to improve OOD detection in long-tailed vision datasets, addressing high FPR and low tail-class accuracy issues.


<details>
  <summary>Details</summary>
Motivation: OOD detection is challenging in long-tailed ID datasets due to high false positive rates and poor tail-class classification accuracy in existing methods.

Method: Uses graph-based representation with GCNs, initializes graph from pre-trained model features, applies Gaussianization to normalize activations, and refines features for long-tailed OOD detection.
...
Conclusion: Graph-based representation with GCNs and Gaussianization effectively improves OOD detection in long-tailed recognition scenarios.
...
TL;DR: A sequence-level optimization framework generates printable adversarial textures for clothing that maintain concealment across entire walking videos, achieving robust human detection evasion in both digital and physical settings.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks for human detection are vulnerable to adversarial attacks, and existing wearable attack methods fail to maintain concealment across long video sequences with motion, pose changes, and garment deformation.

Method: Product images are mapped to UV space and converted to a compact palette with control-point parameterization. A physically based human-garment pipeline simulates motion, camera viewpoints, cloth dynamics, and illumination. Expectation-over-transformation with temporal weighting optimizes control points to minimize detection confidence across sequences.
...
TL;DR: The paper proposes a Mixture-of-Ranks (MoR) architecture for single-step image super-resolution, integrating sparse Mixture-of-Experts into real-world image super-resolution to address limitations of dense models in capturing heterogeneous degradation characteristics.


<details>
  <summary>Details</summary>
Motivation: Existing dense Real-ISR models using LoRA fine-tuning lack adaptive capability for complex real-world degraded samples and efficient knowledge sharing under equivalent computational budgets.
...
Conclusion: The MoR architecture successfully integrates sparse MoE into Real-ISR, enabling flexible knowledge recombination and adaptive computational resource allocation based on degradation severity.
...
TL;DR: Proposes a deep learning-based material classification method using laser speckle patterns to enable safe and efficient laser cutting, achieving high accuracy even when laser color changes.


<details>
  <summary>Details</summary>
Motivation: Laser cutting generates hazardous dust and smoke, requiring real-time monitoring. Existing speckle sensing methods struggle with laser color changes, limiting practical applications.

Method: Uses convolutional neural network (CNN) trained on laser speckle pattern dataset to classify materials, addressing the laser color change issue that affects previous methods.
...
Conclusion: The method provides a robust and accurate solution for material-aware laser cutting using speckle sensing, enabling safer and more efficient industrial laser processing.
...
TL;DR: CuriGS is a curriculum-guided framework that enhances 3D Gaussian Splatting for sparse-view reconstruction by generating pseudo-views (student views) around ground-truth poses and gradually increasing perturbation levels during training.
...
Method: Generate student views around ground-truth teacher poses with different perturbation levels, follow curriculum schedule to gradually increase perturbation, apply depth-correlation and co-regularization, and use multi-signal evaluation to retain best-performing students.
...
Conclusion: The curriculum-guided framework with student view generation effectively addresses sparse-view synthesis challenges in 3D Gaussian Splatting.
...
TL;DR: A cross-modal learning approach that enhances high-resolution satellite imagery with UAV-level detail for crop canopy trait estimation, bridging the gap between satellite and UAV sensing in agriculture.


<details>
  <summary>Details</summary>
Motivation: Satellite missions have spatial resolution limitations that hinder effectiveness for micro-plot management in modern farming, while UAVs provide high-resolution data but have different operational constraints. There's a need to combine the strengths of both modalities.

Method: Train a model using co-registered satellite-UAV image pairs from 84 hybrid maize varieties across five locations in the U.S. Corn Belt to learn fine-grained spectral-spatial correspondences between sensing modalities.

Result: Generated UAV-like representations from satellite inputs consistently outperform real satellite imagery on multiple downstream tasks including yield and nitrogen prediction.

Conclusion: Cross-modal correspondence learning effectively bridges the gap between satellite and UAV sensing in agricultural monitoring, enabling enhanced crop canopy trait estimation from satellite data.
...
TL;DR: A framework using large language models (LLMs) to improve food recognition by generating food titles and ingredients from images, aligning text and image features in a shared embedding space, and using multimodal features for classification.


<details>
  <summary>Details</summary>
Motivation: Food recognition faces challenges including domain shift between training data (from the Internet) and real-world user images, long-tailed data distribution, and subtle visual differences between similar dishes.
...
Result: The proposed approach outperforms existing methods specifically designed for long-tailed data distribution, domain adaptation, and fine-grained classification on two food datasets.

Conclusion: The simple LLM-powered framework effectively addresses multiple challenges in food recognition simultaneously, demonstrating superior performance compared to specialized approaches.
...
TL;DR: AMS-KV is a scale-adaptive KV caching policy for visual autoregressive models that reduces KV cache usage by up to 84.83% and self-attention latency by 60.48% by prioritizing condensed and local scales while optimizing cache utilization through inter-scale similarity analysis.


<details>
  <summary>Details</summary>
Motivation: KV caching in next-scale prediction based VAR transformers faces excessive KV memory growth with increasing scales, severely limiting scalability. The unique challenges of next-scale prediction compared to LLMs remain unexplored.

Method: Systematic investigation revealed three key insights: local scale attention contributes to quality, condensed scales stabilize generation, and cache-demanding layers show weak inter-scale similarity. Based on these, AMS-KV prioritizes storing KVs from condensed and local scales and optimizes cache utilization through inter-scale similarity analysis.
...
Conclusion: AMS-KV effectively addresses the KV memory bottleneck in VAR models through scale-adaptive caching policies, enabling significant performance improvements and better scalability for next-scale prediction tasks.
...
TL;DR: LiSTAR is a generative world model for synthesizing high-fidelity 4D LiDAR data using a Hybrid-Cylindrical-Spherical representation and Spatio-Temporal Attention with Ray-Centric Transformer to address challenges in sensor geometry, temporal sparsity, and dynamic scene complexity.


<details>
  <summary>Details</summary>
Motivation: The need for scalable autonomous driving simulations requires high-fidelity and controllable 4D LiDAR data synthesis, which is challenging due to spherical sensor geometry, temporal sparsity of point clouds, and complex dynamic scenes.

Method: Proposes LiSTAR with Hybrid-Cylindrical-Spherical (HCS) representation to reduce quantization artifacts, Spatio-Temporal Attention with Ray-Centric Transformer (START) for temporal coherence, and Masked Generative START (MaskSTART) for controllable synthesis using tokenized scene representations.

Result: State-of-the-art performance in 4D LiDAR tasks: 76% reduction in generation MMD, 32% improvement in reconstruction IoU, and 50% reduction in prediction L1 Med error.
...
TL;DR: VideoSeg-R1 is the first reinforcement learning-based framework for video reasoning segmentation that uses a decoupled architecture with hierarchical frame sampling, explicit reasoning chains, and adaptive reasoning length control.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised fine-tuning methods for video reasoning segmentation have limited generalization to out-of-distribution scenarios and lack explicit reasoning capabilities.

Method: A three-stage decoupled architecture: (1) hierarchical text-guided frame sampling to emulate human attention, (2) reasoning model producing spatial cues with explicit reasoning chains, and (3) segmentation-propagation using SAM2 and XMem with adaptive reasoning length control.

Result: Extensive evaluations show VideoSeg-R1 achieves state-of-the-art performance in complex video reasoning and segmentation tasks across multiple benchmarks.

Conclusion: The proposed reinforcement learning approach successfully addresses limitations of traditional methods by enabling explicit reasoning and better generalization in video segmentation tasks.
...
TL;DR: SpectralTrain is a universal training framework for hyperspectral image classification that combines curriculum learning with PCA-based spectral downsampling to achieve 2-7x training speedups while maintaining competitive accuracy across various datasets and models.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral image classification faces challenges with large-scale data and computationally intensive training, limiting practical deployment of deep learning models in real-world remote sensing applications.

Method: Integrates curriculum learning with PCA-based spectral downsampling to gradually introduce spectral complexity while preserving essential information, making it architecture-agnostic and compatible with various models, optimizers, and loss functions.

Result: Achieves 2-7x training speedups with small-to-moderate accuracy deltas across three benchmark datasets (Indian Pines, Salinas-A, CloudPatch-7), demonstrating strong generalization across spatial scales, spectral characteristics, and application domains.

Conclusion: Training strategy optimization serves as an effective complement to architectural design in HSI models, with particular potential in climate-related remote sensing applications like cloud classification.
...
### [25] [LEGO-SLAM: Language-Embedded Gaussian Optimization SLAM](https://arxiv.org/abs/2511.16144)
*Sibaek Lee,Seongbo Ha,Kyeongsu Kang,Joonyeol Choi,Seungjun Tak,Hyeonwoo Yu*

Main category: cs.CV

TL;DR: LEGO-SLAM is the first real-time open-vocabulary SLAM system using 3D Gaussian Splatting, featuring a scene-adaptive encoder-decoder for compact language embeddings, language-guided pruning, and language-based loop detection.


<details>
  <summary>Details</summary>
Motivation: Current 3DGS-based SLAM systems lack semantic understanding for robotic interaction, and existing methods face memory/rendering overhead from high-dimensional features or lack adaptability for novel environments.

Method: Uses a scene-adaptive encoder-decoder to distill language embeddings into compact 16D features, language-guided pruning to reduce Gaussian count by 60%, and language-based loop detection reusing mapping features.
...
Conclusion: LEGO-SLAM successfully integrates language understanding into real-time SLAM with minimal overhead, enabling advanced robotic interaction through compact semantic representations and adaptive feature learning.
...
<details>
  <summary>Details</summary>
Motivation: To develop a robust outdoor mapping system that can handle kilometer-scale environments using 4D mmWave radar, addressing challenges with dynamic objects and rendering artifacts in traditional camera or LiDAR-based methods.

Method: Combines raw radar point clouds with Doppler information and geometrically enhanced point clouds to guide dynamic object masking in synchronized images. Uses unsynchronized image frames for global refinement of 3D Gaussian representation. Implements global octree structure with targeted Gaussian primitive management to reduce noise and memory consumption.

Result: Achieves performance comparable to traditional 3D Gaussian methods based on camera or LiDAR inputs, with successful real-world reconstruction at kilometer scale. Effectively suppresses noise and reduces memory consumption in large-scale environments.

Conclusion: Rad-GS demonstrates the feasibility of robust outdoor mapping using 4D mmWave radar, validating its potential for large-scale scene reconstruction with improved texture consistency and novel view synthesis fidelity.
...
TL;DR: The paper proposes T2T-VICL, a collaborative pipeline that enables cross-task visual in-context learning (VICL) for vision-language models, allowing them to perform tasks when visual prompts and target images come from different vision tasks.


<details>
  <summary>Details</summary>
Motivation: To investigate whether vision-language models can enable visual in-context learning when the visual prompt and target images originate from different visual tasks, unlocking the boundaries of cross-task VICL.

Method: Developed a mechanism to generate and select text prompts that implicitly describe differences between distinct low-level vision tasks, constructed the first cross-task VICL dataset, and proposed an inference framework combining perceptual score-based reasoning with traditional evaluation metrics.
...
Conclusion: Successfully unlocked the boundaries of cross-task visual in-context learning within vision-language models through the proposed T2T-VICL pipeline.
...
<details>
  <summary>Details</summary>
Motivation: Existing 4DGS methods struggle with ambiguous pixel correspondences and inadequate densification in dynamic regions, leading to poor reconstruction of dynamic scenes.
...
Conclusion: The proposed method effectively addresses limitations in dynamic scene reconstruction for 4DGS, providing superior temporal consistency and rendering quality through targeted error correction and grouped splatting approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing models couple scale with content complexity, using more tokens for higher-resolution/frame-rate content, but latent capacity should primarily depend on content complexity rather than scale.

Method: Constructs hierarchical, scale-independent latent space with multi-level tokens that models sample complexity and supports decoding to arbitrary resolutions/frame rates within fixed latent representation.
...
Conclusion: DCS-LDM successfully decouples information complexity from scale, enabling progressive coarse-to-fine generation and flexible computation-quality tradeoffs across various resolutions and frame rates.
...
TL;DR: VTinker is a novel Video Frame Interpolation pipeline that addresses flow upsampling issues and pixel-level artifacts through guided flow upsampling and texture mapping techniques.


<details>
  <summary>Details</summary>
Motivation: Existing VFI methods suffer from blur/mosaic artifacts in flow upsampling and misalignment issues due to low-resolution motion estimation, leading to ghosting and discontinuities in interpolated frames.

Method: Proposes two core components: 1) Guided Flow Upsampling (GFU) that uses input frames as guidance to refine bilinear upsampled flows, and 2) Texture Mapping that creates an intermediate proxy frame for selecting clear texture blocks from input frames, followed by reconstruction.

Result: Extensive experiments show VTinker achieves state-of-the-art performance in Video Frame Interpolation.

Conclusion: VTinker effectively addresses flow upsampling artifacts and pixel-level ghosting through its guided upsampling and texture mapping approach, demonstrating superior VFI performance.
...
TL;DR: PiN-CLIP improves AI-generated image detection by injecting positive-incentive noise during training to suppress spurious shortcuts and amplify stable forensic cues, achieving state-of-the-art performance with 5.4% average accuracy improvement.
...
Method: Jointly trains noise generator and detection network using variational positive-incentive principle, constructing noise via cross-attention fusion of visual and categorical features to fine-tune visual encoder.

Result: Achieves state-of-the-art performance on open-world dataset with 42 generative models, improving average accuracy by 5.4% over existing methods.

Conclusion: PiN-CLIP effectively addresses shortcut dominance in AI-generated image detection by leveraging positive-incentive noise to extract more robust and generalized artifact representations.
...
Method: Uses pretrained DRL module to extract high-dimensional multiscale degradation representations, and introduces hierarchical termination mechanism to dynamically adjust artifact reduction stages based on compression level.
...
Conclusion: The proposed approach effectively addresses limitations of existing blind QECV methods by providing detailed degradation representations and adaptive computational efficiency.
...
Method: Two main components: spatial-prioritized optimal transport assignment that dynamically emphasizes well-located samples, and rank-aware adaptive self-distillation that injects ranking perception through self-distillation.
...
Conclusion: SR3D effectively bridges the training-inference gap in 3D object detection and achieves state-of-the-art performance for indoor point cloud detection.
...
TL;DR: This paper proposes a new high-resolution dataset (HSRW-CD) for water body change detection and introduces a Spatial Semantics and Continuity Perception (SSCP) attention module to better exploit spatial semantic and structural information in deep features.


<details>
  <summary>Details</summary>
Motivation: Current limitations include scarcity of high spatial resolution datasets for water body change detection and inadequate exploitation of spatial semantic and structural information in existing deep learning methods.

Method: Proposes HSRW-CD dataset with >3m spatial resolution and develops SSCP attention module with three components: Multi-Semantic spatial Attention (MSA), Structural Relation-aware Global Attention (SRGA), and Channel-wise Self-Attention (CSA).

Result: Experiments on HSRW-CD and Water-CD datasets validate the effectiveness and generalization of the proposed SSCP module.

Conclusion: The SSCP module significantly improves water body discrimination capability and can be integrated as a plug-and-play module into existing WBCD models.
...
TL;DR: The paper proposes Reasoning Guided Embeddings (RGE), a method that incorporates explicit reasoning from MLLMs into multimodal embedding extraction, improving representation quality through structured rationale generation and contrastive training.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat embedding extraction from MLLMs as direct encoding, overlooking their generative reasoning capabilities that could enhance representation quality.

Method: RGE enables structured rationale generation conditioned on instructions, extracts representations after reasoning unfolds, and couples this with contrastive training to enhance context-conditional inference signals.

Result: Experiments on MMEB benchmark show 4.9% improvement in multimodal retrieval performance over non-reasoning baselines.

Conclusion: Explicit reasoning can effectively enhance multimodal embedding quality when incorporated into the embedding process.
...
TL;DR: PPCL is a flexible structured pruning framework for Diffusion Transformers that achieves 50% parameter reduction with minimal performance degradation, enabling efficient deployment in resource-constrained settings.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) have exceptional image generation performance but high computational costs due to large parameter counts, which limits deployment in resource-constrained environments.

Method: Proposes Pluggable Pruning with Contiguous Layer Distillation (PPCL) - identifies redundant layer intervals through linear probing and similarity trend analysis, then uses plug-and-play teacher-student alternating distillation for depth-wise and width-wise pruning in a single training phase.

Result: Achieves 50% parameter reduction compared to full model with less than 3% degradation in key objective metrics, maintains high-quality image generation capabilities while achieving higher compression ratios.

Conclusion: PPCL provides an effective solution for compressing DiT models while preserving performance, making them suitable for resource-constrained environments through flexible knowledge transfer across pruning ratios without per-configuration retraining.
...
<details>
  <summary>Details</summary>
Motivation: Current grid-based map methods for spatial understanding in MLLMs rely on discretized raster representations, which limit fine-grained spatial reasoning capabilities. The authors aim to overcome this limitation by using continuous coordinates.

Method: The framework uses continuous object boundary coordinates to quantify inter-object distances and object sizes. It involves two stages: supervised fine-tuning with data from AI2THOR simulator to learn mapping from visual inputs to boundary coordinates, followed by reinforcement fine-tuning for real-world generalization.

Result: The model V2LO-7B achieves an average improvement of 4.92% over models trained on grid maps on QVS-Bench and mainstream spatial reasoning benchmarks, validating the superiority of the continuous coordinate approach.

Conclusion: Using continuous object boundary coordinates instead of discretized grid representations significantly enhances spatial reasoning capabilities in MLLMs, enabling more precise quantitative spatial computation and reducing ambiguity in spatial relationship descriptions.
...
TL;DR: Simba reformulates point cloud completion from direct regression to distribution learning using diffusion models with symmetry priors, achieving SOTA performance while improving robustness and avoiding overfitting.
...
Method: Integrates symmetry priors with diffusion models for distribution learning, plus hierarchical Mamba-based architecture for high-fidelity upsampling.

Result: Achieves state-of-the-art performance across PCN, ShapeNet, and KITTI benchmarks with improved robustness and generalization.

Conclusion: Reformulating point-wise transformation as distribution learning with diffusion models effectively addresses overfitting and noise sensitivity in point cloud completion.
...
TL;DR: LNG-SWR is a layer-wise noise-guided selective wavelet reconstruction method that improves segmentation model robustness against distribution shifts and adversarial attacks without sacrificing clean accuracy, working with or without adversarial training.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of adversarial training (AT) in medical imaging, which suffers from clean-robustness trade-offs and high training costs, by developing a more scalable and maintainable robustness solution.

Method: Inject small zero-mean noise at multiple layers during training to learn frequency-bias prior, then apply prior-guided selective wavelet reconstruction to suppress noise-sensitive bands while enhancing directional structures and maintaining spectral consistency.

Result: On CT and ultrasound datasets, LNG-SWR consistently improves clean Dice/IoU and significantly reduces performance drop under strong attacks (PGD-L∞/L2 and SSAH), with additive gains when combined with adversarial training.
...
TL;DR: Proposes VTIA, a two-stage adversarial attack that injects imperceptible perturbations into images to make Vision-Language Models generate verbose outputs, increasing token consumption and deployment costs.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing methods that only delay EOS tokens implicitly, lacking stability and controllability in maximizing output token length as an explicit optimization objective.

Method: Two-stage framework: 1) Adversarial prompt search using reinforcement learning to find prompts that induce verbose outputs, 2) Vision-aligned perturbation optimization to craft adversarial images that maximize similarity with adversarial prompt embeddings.

Result: Comprehensive experiments on four popular VLMs show significant advantages in effectiveness, efficiency, and generalization capability.

Conclusion: VTIA successfully demonstrates the vulnerability of VLMs to verbose-text induction attacks, highlighting security concerns in real-world deployment scenarios.
...
TL;DR: EvoVLA is a self-supervised VLA framework that addresses stage hallucination in long-horizon robotic manipulation through stage-aligned rewards, pose-based exploration, and long-horizon memory, achieving significant improvements in task success and sample efficiency.
...
Method: Uses three components: Stage-Aligned Reward (SAR) with triplet contrastive learning and hard negatives, Pose-Based Object Exploration (POE) that grounds curiosity in relative object-gripper pose, and Long-Horizon Memory with selective context retention and gated fusion.

Result: Improves average task success by 10.2 percentage points over OpenVLA-OFT to 69.2%, achieves 1.5x better sample efficiency, reduces stage hallucination from 38.5% to 14.8%, and reaches 54.6% success rate in real-world deployment.

Conclusion: EvoVLA effectively addresses stage hallucination in VLA models, demonstrating strong performance in long-horizon manipulation tasks with successful sim-to-real transfer and generalization capabilities.
...
TL;DR: RF-CLIP is a training-free approach that addresses CLIP's attention diversion to irrelevant tokens by filtering dimension-specific over-activation, achieving state-of-the-art open-vocabulary semantic segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods leverage CLIP's vision-language alignment but fail to investigate its performance boundaries for dense prediction from an interpretability perspective. CLIP diverts significant attention from target regions to irrelevant tokens, analogous to human distraction.

Method: Propose ReFocusing CLIP (RF-CLIP), a training-free approach that emulates human distraction-refocusing behavior by filtering dimension-specific over-activation tokens to redirect attention back to target regions.

Result: Achieves state-of-the-art performance on eight benchmarks while maintaining high inference efficiency.

Conclusion: RF-CLIP successfully enhances CLIP's multimodal alignment granularity by addressing attention diversion issues, demonstrating improved dense prediction capabilities for open-vocabulary semantic segmentation.
...
TL;DR: Mantis introduces a Disentangled Visual Foresight framework that decouples visual prediction from the VLA backbone using meta queries and diffusion Transformer, achieving state-of-the-art performance on robotics tasks while maintaining language comprehension capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models face issues with high-dimensional visual state prediction distributing model capacity and incurring high training costs, while compressed visual signals create information bottlenecks and neglect language supervision, leading to poor comprehension and reasoning.

Method: Mantis uses a Disentangled Visual Foresight approach with meta queries and a diffusion Transformer head. It provides current visual state to DiT via residual connection, enabling meta queries to capture latent actions through simple next-state prediction, while the VLA backbone maintains language supervision.
...
Conclusion: Mantis effectively addresses VLA limitations by disentangling visual foresight, enabling efficient learning while preserving language comprehension and reasoning capabilities, demonstrating strong performance in robotics manipulation tasks.
...
TL;DR: Proposes DSLGA, a two-stage model for Unsupervised Domain Adaptation in Visible-Infrared person Re-Identification, addressing modality discrepancies between public datasets and real-world data without requiring new annotations.


<details>
  <summary>Details</summary>
Motivation: Existing VI-ReID algorithms struggle in real-world applications due to discrepancies between public datasets and real-world data, creating a need for unsupervised domain adaptation that transfers knowledge without compromising accuracy.

Method: Two-stage approach: 1) Pre-training with Domain-Shared Learning Strategy (DSLS) to mitigate inter-domain modality discrepancies, 2) Fine-tuning with Gradual Alignment Strategy (GAS) using cluster-to-holistic alignment for intra-domain modality discrepancies. Also introduces CMDA-XD testing framework.
...
Conclusion: The proposed DSLGA model effectively addresses both inter-domain and intra-domain modality discrepancies in UDA-VI-ReID, enabling successful knowledge transfer from public datasets to real-world applications without requiring new annotations.
...
TL;DR: PrIntMesh is a template-based framework that reconstructs organs as unified systems by jointly deforming all substructures while preserving internal boundaries and topology, achieving high geometric accuracy and robustness with limited data.


<details>
  <summary>Details</summary>
Motivation: Current deep-learning approaches treat organ substructures independently, producing anatomically implausible reconstructions that don't account for the geometric and spatial relationships between interconnected parts.

Method: Template-based topology-preserving framework that starts from a connected template and jointly deforms all substructures to match patient-specific anatomy while preserving internal boundaries and enforcing smooth surfaces.

Result: Achieved high geometric accuracy, correct topology, and robust performance on heart, hippocampus, and lungs, even with limited or noisy training data. Better reconstructs shared interfaces and maintains structural consistency compared to voxel- and surface-based methods.

Conclusion: PrIntMesh provides a data-efficient solution suitable for clinical use by treating organs as unified systems rather than independent parts, producing anatomically plausible reconstructions.
...
TL;DR: VLA-Fool is a comprehensive study examining the adversarial robustness of Vision-Language-Action models in embodied environments, introducing multimodal attacks including textual, visual, and cross-modal misalignment perturbations.


<details>
  <summary>Details</summary>
Motivation: Despite impressive progress in VLAs for robotics, their adversarial robustness remains unexplored, especially under realistic multimodal and black-box conditions, with existing studies overlooking cross-modal misalignment that affects embodied reasoning.

Method: VLA-Fool unifies three multimodal adversarial attacks: textual perturbations (gradient-based and prompt-based), visual perturbations (patch and noise distortions), and cross-modal misalignment attacks. It also incorporates a VLA-aware semantic space into linguistic prompts.

Result: Experiments on LIBERO benchmark using OpenVLA show that even minor multimodal perturbations cause significant behavioral deviations, revealing the fragility of embodied multimodal alignment.

Conclusion: The study demonstrates that embodied VLA models are vulnerable to multimodal adversarial attacks, highlighting the need for improved robustness in these systems.
...
<details>
  <summary>Details</summary>
Motivation: Current unsupervised image classification methods focus solely on clustering while bypassing representation learning, creating an opportunity to enhance clustering performance through ensemble techniques.

Method: Train multiple clustering heads on a frozen backbone to produce diverse clusterings, then use cluster ensembling to consolidate results into consensus clustering, and finally train a classifier using consensus as pseudo-labels.

Result: Achieved 99.3% accuracy on CIFAR10, 89% on CIFAR100, and 70.4% on ImageNet - the first fully unsupervised method to exceed 70% on ImageNet.

Conclusion: ICCE demonstrates that cluster ensembling with adaptive nearest neighbors effectively improves unsupervised image classification, significantly narrowing the performance gap with supervised methods.
...
TL;DR: MLLMs lack human-like social reasoning to detect deception in complex interactions. The paper introduces MIDA task and dataset, benchmarks 12 MLLMs showing poor performance, and proposes SoCoT reasoning with DSEM module to improve social reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art MLLMs demonstrate advanced reasoning but lack the ability to 'read the room' and assess deception in social interactions, which is a core component of human intelligence.

Method: Introduced Multimodal Interactive Deception Assessment (MIDA) task with synchronized video-text dataset. Proposed Social Chain-of-Thought (SoCoT) reasoning pipeline and Dynamic Social Epistemic Memory (DSEM) module to improve social reasoning.

Result: Benchmark of 12 state-of-the-art MLLMs revealed significant performance gap - even powerful models like GPT-4o struggle to reliably distinguish truth from falsehood. Models fail to ground language in multimodal social cues and lack ability to model others' knowledge, beliefs, or intentions.

Conclusion: The proposed framework with SoCoT and DSEM yields performance improvement on deception assessment, demonstrating a promising path toward building MLLMs capable of genuine human-like social reasoning.
...
TL;DR: SwiTrack is a novel cross-modal object tracking framework that uses three specialized streams (RGB, NIR with gated adapter, and trajectory prediction) to handle modality switching and prevent object drift, achieving state-of-the-art performance on RGB-NIR tracking benchmarks.
...
Method: Proposes a state-switching framework with three streams: RGB frames processed by visual encoder, NIR frames refined via NIR gated adapter with visual encoder, and consistency trajectory prediction module for invalid modalities. Also includes dynamic template reconstruction and similarity alignment loss.

Result: Achieves state-of-the-art performance with 7.2% precision rate and 4.3% success rate gains over existing methods, while maintaining real-time tracking at 65 frames per second.

Conclusion: SwiTrack effectively addresses cross-modal object tracking challenges by combining specialized modality processing, trajectory prediction for invalid modalities, and feature consistency mechanisms, demonstrating superior performance and robustness.
...
Method: Uses ReLU MLP composed with multi-resolution tetrahedral positional encoder, where barycentric interpolation preserves global CPWA structure. Includes fixed analytic input preconditioner to reduce directional bias and stabilize training.
...
Conclusion: TetraSDF provides a precise analytic meshing solution that overcomes limitations of previous methods, enabling exact mesh extraction from encoded neural SDFs while maintaining competitive performance and efficiency.
...
Method: Process image flow with sliding window and aligned submaps, use VGGT tracking head to aggregate 2D semantic instance masks into 3D objects, store timestamps and instance-level identities for temporal consistency.
...
TL;DR: Hybrid deep learning framework combining CNNs, ViTs, and GNNs for robust weed detection, achieving 99.33% accuracy with GAN-based augmentation and self-supervised pre-training.
...
Method: Hybrid framework using CNNs, Vision Transformers, and Graph Neural Networks with GAN-based augmentation and self-supervised contrastive pre-training for feature learning from limited data.
...
Conclusion: Framework enables real-time deployment on edge devices for automated weed detection, reducing herbicide overuse and providing scalable sustainable farming solutions.
...
TL;DR: Texture3dgs is an optimized mobile GPU implementation of 3D Gaussian Splatting that introduces a novel sorting algorithm optimized for 2D texture cache, achieving up to 4.1× sorting speedup and 1.7× overall reconstruction speedup while reducing memory usage by 1.6×.


<details>
  <summary>Details</summary>
Motivation: To enable efficient 3D scene reconstruction on mobile devices for benefits like data privacy, offline operation, and faster responses by optimizing 3D Gaussian Splatting for mobile GPU constraints.
...
Conclusion: Texture3dgs effectively optimizes 3D Gaussian Splatting for mobile deployment through texture cache-aware algorithms and memory optimizations, demonstrating significant performance improvements for mobile 3D reconstruction.
...
TL;DR: Upsample Anything is a test-time optimization framework that upsamples low-resolution vision model features to high-resolution pixel outputs using learned anisotropic Gaussian kernels, achieving SOTA performance without training.


<details>
  <summary>Details</summary>
Motivation: Vision Foundation Models have strong generalization but their representations are heavily downsampled (14x/16x), limiting pixel-level applications. Existing upsampling methods require dataset-specific retraining or heavy optimization.
...
Result: Runs in ≈0.419s per 224x224 image and achieves state-of-the-art performance on semantic segmentation, depth estimation, and both depth/probability map upsampling.

Conclusion: The method provides a lightweight, training-free solution for high-resolution feature reconstruction that transfers across architectures and modalities.
...
TL;DR: SAEs are reframed as topic models, leading to SAE-TM framework that learns reusable topic atoms for thematic analysis across text and image datasets without retraining.


<details>
  <summary>Details</summary>
Motivation: To clarify the role and practical value of sparse autoencoders (SAEs) by showing they can be naturally understood as topic models rather than just steerable directions.
...
Result: The benchmark evaluates models using class-balanced macro-F1 (plus domain-specific metrics for FishNet and FungiCLEF), with ViT-L models evaluating in 6 hours on an A6000 GPU.

Conclusion: BioBench provides new signal for computer vision in ecology and serves as a template for building reliable AI-for-science benchmarks in any domain.
...
TL;DR: NaTex is a native 3D texture generation framework that directly predicts texture colors in 3D space, overcoming limitations of multi-view diffusion methods by treating texture as a dense color point cloud and using a novel latent color diffusion approach.


<details>
  <summary>Details</summary>
Motivation: To address inherent limitations of MVD-based texture generation methods, including difficulties with occluded regions, mesh-texture alignment, and cross-view consistency in content and color intensity.

Method: Proposes latent color diffusion with geometry-aware color point cloud VAE and multi-control diffusion transformer (DiT), trained from scratch using 3D data. Uses native geometry control via positional embeddings and geometry latents extracted through a dedicated geometry branch.

Result: Demonstrates strong performance, significantly outperforming previous methods in texture coherence and alignment. Shows strong generalization capabilities for downstream applications like material generation, texture refinement, and part segmentation.

Conclusion: NaTex presents an effective native 3D texture generation framework that overcomes key limitations of MVD-based approaches through its novel point cloud-based paradigm and tightly coupled geometry-texture architecture.
...
TL;DR: WWE-UIE is an efficient underwater image enhancement network that integrates three interpretable priors: adaptive white balance, wavelet-based enhancement, and gradient-aware edge preservation, achieving competitive performance with significantly reduced computational cost.
...
Method: Three interpretable components: 1) Adaptive white balance for color correction, 2) Wavelet-based enhancement block for multi-band decomposition capturing global structures and fine textures, 3) Gradient-aware module using Sobel operators with learnable gating to preserve edge structures.
...
Conclusion: The proposed WWE-UIE provides an efficient solution for underwater image enhancement that balances performance and computational efficiency, with each component validated through ablation studies.
...
TL;DR: ChangeDINO is a Siamese framework for remote sensing change detection that combines a lightweight backbone with frozen DINOv3 features and uses a spatial-spectral transformer decoder with learnable morphology to achieve state-of-the-art performance on building change detection.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for remote sensing change detection rely too heavily on change-map annotations and underutilize semantic information from non-changing regions, making them less robust to illumination variations, off-nadir views, and limited labeled data.

Method: End-to-end multiscale Siamese framework that fuses lightweight backbone features with frozen DINOv3 features, uses spatial-spectral differential transformer decoder with multi-scale absolute differences as change priors, and incorporates learnable morphology module for boundary refinement.

Result: Outperforms recent state-of-the-art methods on four public benchmarks in terms of IoU and F1 scores, with ablation studies confirming the effectiveness of each component.

Conclusion: ChangeDINO provides an effective solution for robust building change detection by leveraging semantic-rich features and sophisticated change modeling, demonstrating superior performance across multiple benchmarks.
...
TL;DR: ARASFSR enables arbitrary-scale face super-resolution using implicit representation networks with local frequency estimation and global coordinate modulation to handle various input sizes and scales.


<details>
  <summary>Details</summary>
Motivation: Existing face super-resolution methods are limited by fixed up-sampling scales and sensitivity to input size variations, which restricts their practical applications.

Method: Uses implicit representation networks with 2D deep features, local relative coordinates, and scale ratios; incorporates local frequency estimation module to capture high-frequency textures and global coordinate modulation module to leverage facial structure priors.

Result: Demonstrates robustness over state-of-the-art methods in quantitative and qualitative evaluations, effectively super-resolving facial images across various input sizes and up-sampling scales.

Conclusion: ARASFSR successfully addresses limitations of fixed scales and input size sensitivity in face super-resolution, enabling flexible and effective enhancement of low-resolution facial images.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in UAV terrain classification including data annotation complexity, temporal consistency issues, data scarcity, and technology range limitations in aerial positioning tasks.
...
Conclusion: The proposed framework overcomes traditional temporal consistency training deficiencies in aerial tasks and demonstrates that fully labeled data is not optimal for UAV terrain classification.
...
TL;DR: Real-time camera localization using pre-captured LiDAR point clouds with neural rendering to bridge synthetic-real domain gap, achieving drift-free metric-scale tracking.


<details>
  <summary>Details</summary>
Motivation: Existing visual localization methods suffer from drift, scale ambiguity, and dependency on fiducials or loop closure, limiting reliable navigation and XR applications.

Method: Render synthetic views from colored LiDAR point clouds to establish 2D-3D correspondences with live frames, using neural rendering to reduce domain gap between synthetic and real images for improved feature matching.
...
Conclusion: The method enables reliable real-time camera localization without drift or scale ambiguity by leveraging pre-captured LiDAR data and neural rendering techniques.
...
<details>
  <summary>Details</summary>
Motivation: Real-world RGB-D data often suffers from misalignment due to hardware limitations and calibration drift, causing performance degradation in existing guided depth super-resolution methods that assume perfect spatial alignment.

Method: Proposes Multi-Order Matching Network with: 1) Multi-order matching mechanism (zero-order, first-order, second-order) to identify relevant RGB information across feature spaces, and 2) Multi-order aggregation with structure detectors that use multi-order priors as prompts for selective feature transfer.
...
Conclusion: MOMNet provides an effective alignment-free solution for depth super-resolution that overcomes real-world misalignment challenges through comprehensive multi-order feature matching and aggregation.
...
<details>
  <summary>Details</summary>
Motivation: Previous OSV methods rely on holistic features for pair comparisons, but the authors argue that fine-grained differences are more important for robust verification. They also observe that transformer-based backbones can obscure local details without specific modifications.

Method: Proposes DetailSemNet with a Detail Semantics Integrator that uses feature disentanglement and re-entanglement to enhance intricate details and expand discriminative semantics. The model emphasizes local structural matching between signature images.

Result: The model consistently outperforms recent methods on leading OSV benchmarks, achieving state-of-the-art results with clear margins. It also shows remarkable generalization capabilities in cross-dataset testing scenarios.

Conclusion: The focus on local structure matching improves both performance and interpretability. The combination of generalizability and interpretability makes DetailSemNet highly suitable for real-world applications in forensic signature verification.
...
TL;DR: CAMS proposes a novel approach for compositional zero-shot learning using CLIP, focusing on semantic feature extraction and multi-space disentanglement to improve recognition of unseen attribute-object compositions.
...
Method: CAMS uses Gated Cross-Attention to extract fine-grained semantic features from CLIP's image encoder blocks through latent units, suppressing irrelevant information, followed by Multi-Space Disentanglement to separate attribute and object semantics.

Result: Experiments on MIT-States, UT-Zappos, and C-GQA benchmarks show state-of-the-art performance in both closed-world and open-world settings.

Conclusion: The proposed CAMS framework effectively improves compositional zero-shot learning by enabling better semantic disentanglement in multidimensional spaces, achieving superior performance on standard benchmarks.
...
TL;DR: Introduces Rigid Body Marker (RBM) for optical motion capture, providing unambiguous 6-DoF data with simplified setup. Uses deep learning with geodesic loss to estimate SMPL parameters, achieving state-of-the-art accuracy with much less computation than optimization methods.


<details>
  <summary>Details</summary>
Motivation: Traditional marker-based MoCap has practical challenges including time-consuming preparation and marker identification ambiguity due to dense marker configurations, limiting scalability.

Method: Uses Rigid Body Marker (RBM) as fundamental unit for unambiguous 6-DoF data. Develops deep-learning regression model with geodesic loss to directly estimate SMPL parameters from synthesized AMASS dataset data.

Result: End-to-end model matches optimization-based methods' performance while requiring over 10x less computation. Achieves state-of-the-art accuracy in body pose estimation. Real-world Vicon system data validates practical viability.

Conclusion: Combining sparse 6-DoF RBM with manifold-aware geodesic loss provides practical, high-fidelity solution for real-time MoCap in graphics, VR, and biomechanics applications.
...
TL;DR: A geometry-guided method for self-supervised surround-view depth estimation that improves cross-view consistency by projecting 3D points onto a shared cylinder and using spatial attention across images.
...
Method: Predicts initial depth maps per image, projects 3D points onto a shared unit cylinder to establish cross-image neighborhood relations, creates 2D position maps, and applies explicit spatial attention to aggregate features across images based on cylinder distances.
...
Conclusion: The proposed geometry-guided method with cylinder projection and spatial attention effectively addresses cross-view consistency issues in surround-view depth estimation, enabling more reliable 360° 3D perception.
...
<details>
  <summary>Details</summary>
Motivation: Accurate identification of hepatocystic anatomy is critical for preventing surgical complications during laparoscopic cholecystectomy. Deep learning models struggle with occlusions, long-range dependencies, and capturing fine-scale geometry of rare structures.
...
Result: 7-8% improvement in Mean Intersection over Union (mIoU) and 6% improvement in Mean Dice (mDice) scores over state-of-the-art baselines, with anatomically coherent predictions particularly on thin, rare and safety-critical structures.

Conclusion: Graph-based segmentation methods enhance both performance and anatomical consistency in surgical scene segmentation by combining ViT-based global context with graph-based relational reasoning, improving interpretability and reliability for safer laparoscopic and robot-assisted surgery.
...
TL;DR: LDAG introduces a language-driven approach for few-shot segmentation by using LLM-generated attribute descriptions instead of support images to provide unbiased meta guidance, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing FSS methods rely on support images for meta guidance, but intra-class variations make this guidance inaccurate for untrained classes. The key insight is that support should provide unbiased guidance rather than visual references.

Method: Uses LLMs to generate multiple attribute descriptions of target classes, then employs Multi-attribute Enhancement (MaE) module for refined visual-text guidance and Multi-modal Attribute Alignment (MaA) for cross-modal interaction between text and visual features.

Result: Outperforms existing approaches by a clear margin and achieves state-of-the-art performance in few-shot segmentation.

Conclusion: Language-driven attribute generalization provides more robust and unbiased support guidance than visual references from support images, effectively addressing intra-class variation challenges in few-shot segmentation.
...
TL;DR: The paper introduces StreetView-Waste, a comprehensive dataset for urban waste management with three tasks: container detection, tracking, and overflow segmentation, along with baseline models and performance improvements through heuristic tracking and geometry-aware segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing litter detection datasets lack annotations for container tracking and are captured in static environments, limiting their utility for real-world logistics in smart city waste management.

Method: Created StreetView-Waste dataset with urban scenes featuring litter and waste containers, benchmarked state-of-the-art models, and proposed two strategies: heuristic-based container tracking and geometry-aware segmentation using geometric priors.

Result: Fine-tuned object detectors achieved reasonable container detection, baseline tracking struggled but proposed heuristics reduced counting error by 79.6%, and geometry-aware strategy improved segmentation mAP@0.5 by 27% on lightweight models.

Conclusion: StreetView-Waste provides a challenging benchmark to advance real-world perception systems for urban waste management, demonstrating the value of multimodal inputs and specialized strategies for this domain.
...
TL;DR: VLA-Pruner is a token pruning method for Vision-Language-Action models that addresses the dual-system nature of VLA tasks by considering both semantic understanding and action execution importance, achieving efficient real-time deployment while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Existing token pruning methods for VLMs focus only on semantic salience, overlooking VLA's dual-system nature of high-level semantic understanding and low-level action execution, which leads to degraded performance when critical action information is discarded.

Method: VLA-Pruner uses a dual-level importance criterion: vision-language prefill attention for semantic relevance and action decode attention (estimated via temporal smoothing) for action importance. It adaptively preserves tokens for both semantic understanding and action execution under computational constraints.
...
Conclusion: The proposed dual-level token pruning approach effectively aligns with VLA's dual-system nature and exploits temporal continuity, providing a versatile plug-and-play solution for efficient VLA deployment without compromising performance.
...
TL;DR: LLaVA³ improves 3D scene understanding in vision-language models using multi-view 2D images without fine-tuning, inspired by Cubist art to create omnidirectional object representations from 3D reconstructions.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of limited 3D training data for multi-modal language models by leveraging abundant 2D datasets instead, enabling better 3D scene understanding without costly 3D data collection.

Method: Create omnidirectional visual representations of objects from multi-view 2D images using intermediate 3D scene reconstruction, inspired by Cubist painting techniques that show multiple viewpoints simultaneously.

Result: Outperforms previous 2D-based vision-language model solutions on 3D visual question answering and 3D language grounding tasks through extensive experiments.

Conclusion: LLaVA³ demonstrates that effective 3D scene understanding can be achieved using only 2D multi-view images and intermediate 3D reconstruction, bypassing the need for expensive 3D training data while improving performance over existing methods.
...
### [74] [Physics-Informed Machine Learning for Efficient Sim-to-Real Data Augmentation in Micro-Object Pose Estimation](https://arxiv.org/abs/2511.16494)
*Zongcai Tan,Lan Wei,Dandan Zhang*

Main category: cs.CV

TL;DR: A physics-informed GAN framework that integrates wave optics and depth alignment to synthesize high-fidelity microscope images for microrobot pose estimation, achieving 93.9%/91.9% accuracy while maintaining real-time rendering speeds.


<details>
  <summary>Details</summary>
Motivation: Current methods for microrobot pose estimation require large, high-quality microscope image datasets that are difficult and costly to acquire due to complex fabrication and labor-intensive labeling. Existing digital twin systems struggle to replicate complex optical microscopy phenomena like diffraction artifacts.

Method: Proposes a novel physics-informed deep generative learning framework that integrates wave optics-based physical rendering and depth alignment into a GAN to synthesize high-fidelity microscope images efficiently.

Result: Improves structural similarity index (SSIM) by 35.6% compared to purely AI-driven methods while maintaining real-time rendering speeds (0.022 s/frame). Pose estimator trained on synthetic data achieves 93.9%/91.9% (pitch/roll) accuracy, only 5.0%/5.4% below real data training. Framework generalizes to unseen poses without additional training.

Conclusion: The proposed framework enables efficient data augmentation and robust pose estimation for novel microrobot configurations, overcoming the limitations of expensive real data collection while maintaining high accuracy comparable to real data training.
...
<details>
  <summary>Details</summary>
Motivation: The corpus callosum is important for aging and disease research but lacks comprehensive automated analysis tools.

Method: Automated framework that identifies mid-sagittal slices, segments corpus callosum and fornix, localizes commissures, generates thickness profiles and subdivisions, and extracts shape metrics.
...
Conclusion: FastSurfer-CC provides an efficient and fully automated solution for comprehensive corpus callosum analysis that advances neurological research capabilities.
...
### [76] [Supervised Contrastive Learning for Few-Shot AI-Generated Image Detection and Attribution](https://arxiv.org/abs/2511.16541)
*Jaime Álvarez Urueña,David Camacho,Javier Huertas Tato*

Main category: cs.CV

TL;DR: A two-stage detection framework using supervised contrastive learning and k-NN classifier achieves 91.3% accuracy for synthetic image detection with only 150 images per class, outperforming existing methods by 5.2 percentage points.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of detecting synthetic images from rapidly evolving generative AI models, where traditional detection approaches relying on periodic retraining become computationally infeasible.

Method: Two-stage framework: 1) Vision model trained via supervised contrastive learning on strategically partitioned generator subset, 2) k-NN classifier operating on learned embeddings using few-shot learning with limited samples from unseen generators.

Result: Achieves 91.3% average detection accuracy with only 150 images per class, improving over existing approaches by 5.2 percentage points. For source attribution, obtains 14.70% and 4.27% improvements in AUC and OSCR respectively.

Conclusion: The framework represents significant advancement toward robust, scalable forensic attribution systems that can adapt to evolving generative AI landscape without requiring exhaustive retraining.
...
TL;DR: This paper presents a systematic study on occluded video prediction by incorporating point-flow (motion information) and depth-maps (geometric structure) to improve prediction performance in occluded scenarios and background motion.


<details>
  <summary>Details</summary>
Motivation: Occlusion remains a fundamental challenge in video prediction despite general models achieving good performance in standard scenarios. The authors hypothesize that explicit motion and geometric structure information can help models better handle occlusion and background motion.
...
Conclusion: Explicit incorporation of motion (via point-flow) and geometric structure (via depth-maps) significantly improves video prediction performance in challenging occluded scenarios.
...
### [78] [NutriScreener: Retrieval-Augmented Multi-Pose Graph Attention Network for Malnourishment Screening](https://arxiv.org/abs/2511.16566)
*Misaal Khan,Mayank Vatsa,Kuldeep Singh,Richa Singh*

Main category: cs.CV

TL;DR: NutriScreener is a retrieval-augmented graph attention network that uses CLIP visual embeddings and context awareness to detect malnutrition and predict anthropometric measurements from children's images, achieving high accuracy and efficiency in clinical settings.
...
Method: A retrieval-augmented, multi-pose graph attention network that combines CLIP-based visual embeddings, class-boosted knowledge retrieval, and context awareness for malnutrition detection and anthropometric prediction from children's images.
...
Conclusion: NutriScreener offers a scalable and accurate solution for early malnutrition detection in low-resource environments, demonstrating deployment readiness for clinical use.
...
### [79] [TimeViper: A Hybrid Mamba-Transformer Vision-Language Model for Efficient Long Video Understanding](https://arxiv.org/abs/2511.16595)
*Boshen Xu,Zihan Xiao,Jiaze Li,Jianzhong Ju,Zhenbo Luo,Jian Luan,Qin Jin*

Main category: cs.CV

TL;DR: TimeViper is a hybrid Mamba-Transformer model for long video understanding that introduces TransV module to transfer and compress vision tokens into instruction tokens, enabling processing of hour-long videos with over 10,000 frames while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Long video understanding requires both efficient architecture and effective handling of extended temporal contexts. The authors identified vision-to-text information aggregation causing severe vision token redundancy in hybrid models.

Method: Hybrid Mamba-Transformer backbone combining state-space model efficiency with attention expressivity, plus TransV module that transfers and compresses vision tokens into instruction tokens.
...
Conclusion: This work represents an initial step towards developing, interpreting, and compressing hybrid Mamba-Transformer architectures for long video understanding.
...
Conclusion: Incorporating knowledge of phase acquisition time through FiLM layers enhances breast tumor segmentation in DCE-MRI and improves model generalization across different imaging protocols.
...
### [81] [Generative AI for Enhanced Wildfire Detection: Bridging the Synthetic-Real Domain Gap](https://arxiv.org/abs/2511.16617)
*Satyam Gaba*
...
Method: Leverage generative AI to synthesize annotated smoke datasets, apply unsupervised domain adaptation, and use style transfer, GANs, and image matting to enhance synthetic data realism.

Result: Generated comprehensive synthetic smoke dataset and explored domain adaptation methods to bridge synthetic-real data gap for improved detection.

Conclusion: Generative AI and domain adaptation techniques can effectively address data scarcity in wildfire smoke detection, enabling more accurate and scalable models.
...
TL;DR: A novel method for jointly mapping indoor scenes and registering ceiling-mounted cameras using a mobile agent with RGB-D camera and synchronized CMC videos, enabling automatic registration through trajectory correlation and factor graph optimization.


<details>
  <summary>Details</summary>
Motivation: Manual registration of ceiling-mounted cameras is inefficient and costly, while automatic visual localization may fail due to visual ambiguity, creating a need for a more robust solution.

Method: Use a mobile agent with head-mounted RGB-D camera to traverse scene while CMCs capture the agent. Correlate trajectories from egocentric and CMC videos, then optimize using a customized factor graph for ego-camera poses, scene layout, and CMC poses.

Result: The method effectively accomplishes both scene mapping and CMC registration in a unified framework, jointly enhancing performance of both tasks, as validated on a new benchmark dataset.

Conclusion: Provides a reliable tool for automatic CMC registration that facilitates downstream position-aware applications by solving registration challenges through collaborative mapping.
...
### [83] [Improving Long-Tailed Object Detection with Balanced Group Softmax and Metric Learning](https://arxiv.org/abs/2511.16619)
*Satyam Gaba*
...
<details>
  <summary>Details</summary>
Motivation: Real-world object detection faces class imbalance where rare categories have few instances, biasing models toward frequent classes and degrading performance on rare categories.

Method: Two-stage Faster R-CNN with enhanced Balanced Group Softmax framework, plus metric learning for better feature separation and k-NN classification for improved rare class performance.

Result: Achieved new state-of-the-art 24.5% mAP on LVISv1 dataset, surpassing previous benchmark of 24.0%.

Conclusion: The proposed methods effectively advance long-tailed object detection by addressing class imbalance through enhanced softmax and metric learning approaches.
...
TL;DR: A comprehensive video dataset for boxing punch detection and classification with 6,915 manually annotated punch clips across six punch types, extracted from YouTube sparring sessions.


<details>
  <summary>Details</summary>
Motivation: Address the bottleneck in combat sports computer vision research caused by the lack of robust datasets, particularly for dynamic, unstructured actions in varying recording environments.

Method: Created dataset by manually segmenting and labeling punch clips from 20 publicly available YouTube sparring sessions involving 18 different athletes, ensuring precise temporal boundaries and class consistency.
...
Conclusion: This dataset aims to accelerate progress in boxing movement analysis, automated coaching, and performance assessment by providing a rich benchmark for computer vision research in combat sports.
...
### [85] [SAM 3D: 3Dfy Anything in Images](https://arxiv.org/abs/2511.16624)
...
TL;DR: SAM 3D is a generative model that reconstructs 3D objects from single images, handling geometry, texture, and layout while excelling in natural scenes with occlusion and context.


<details>
  <summary>Details</summary>
Motivation: To overcome challenges in 3D reconstruction from natural images where occlusion, scene clutter, and contextual cues are common, and to break the "3D data barrier" by creating large-scale visually grounded 3D data.

Method: Uses a human- and model-in-the-loop pipeline for annotating object shape, texture, and pose, combined with multi-stage training that includes synthetic pretraining and real-world alignment.

Result: Achieves significant improvements over recent work with at least 5:1 win rate in human preference tests on real-world objects and scenes.

Conclusion: SAM 3D successfully breaks the 3D data barrier and provides state-of-the-art performance in visually grounded 3D reconstruction from single images, with plans to release code, models, demo, and a new benchmark.
...
### [86] [Contrastive vision-language learning with paraphrasing and negation](https://arxiv.org/abs/2511.16527)
...
<details>
  <summary>Details</summary>
Motivation: Address CLIP's mixed performance on negated or paraphrased text, where minimal lexical changes (negation) radically change meaning while paraphrasing creates different expressions with same meaning, posing challenges for vision-language model alignment.

Method: Proposes SemCLIP with new contrastive loss function accounting for both paraphrasing and negation, using LLM-generated training triples (original, paraphrased, negated captions) in CLIP-like training models.

Result: SemCLIP moves paraphrased captions closer to original image embeddings while pushing negated captions further away. Improves CC-Neg benchmark accuracy from 68.1% to 78.1%, shows better performance than CLIP on downstream zero-shot classification tasks.

Conclusion: SemCLIP achieves significant robustness to semantic transformations while preserving CLIP's performance, demonstrating improved alignment for vision-language models on negated and paraphrased text.
...
TL;DR: A novel teacher-guided pruning framework that integrates Knowledge Distillation with importance score estimation for one-shot global pruning, achieving high sparsity with minimal performance degradation.
...
Result: Outperforms state-of-the-art baselines (EPG, EPSD) at high sparsity levels and offers more computationally efficient alternative to iterative pruning schemes like COLT.
...
### [88] [Enhancing Multi-Camera Gymnast Tracking Through Domain Knowledge Integration](https://arxiv.org/abs/2511.16532)
...
<details>
  <summary>Details</summary>
Motivation: Tracking gymnasts is challenging due to limited camera installations in stadiums and detection failures from lighting variations, backgrounds, uniforms, and occlusions, making conventional multi-camera triangulation unreliable.
...
Result: The method demonstrates superior robustness over existing methods in challenging scenarios and has been successfully deployed in Gymnastics World Championships, earning recognition from the International Gymnastics Federation.

Conclusion: Incorporating domain knowledge through ray-plane intersection effectively compensates for uncertain trajectories and minimizes tracking failures in gymnast tracking systems.
...
### [89] [Investigating Optical Flow Computation: From Local Methods to a Multiresolution Horn-Schunck Implementation with Bilinear Interpolation](https://arxiv.org/abs/2511.16535)
*Haytham Ziani*

Main category: cs.CV

TL;DR: Analysis of local and global optical flow methods, focusing on Horn-Schunck algorithm with multiresolution implementation using bilinear interpolation and prolongation to improve motion estimation accuracy.


<details>
  <summary>Details</summary>
Motivation: To explore and compare the theoretical and practical aspects of local (Lucas-Kanade) and global (Horn-Schunck) optical flow methods, and enhance motion estimation performance under varying image conditions.

Method: Implemented a multiresolution version of Horn-Schunck algorithm using bilinear interpolation and prolongation techniques to improve convergence and accuracy in optical flow computation.

Result: The study demonstrates the effectiveness of combining multiresolution strategies with Horn-Schunck algorithm for improved motion estimation between frames under different image conditions.

Conclusion: Multiresolution implementation of Horn-Schunck algorithm with bilinear interpolation and prolongation enhances optical flow computation accuracy and convergence compared to standard approaches.
...
### [90] [Thinking-while-Generating: Interleaving Textual Reasoning throughout Visual Generation](https://arxiv.org/abs/2511.16671)
*Ziyu Guo,Renrui Zhang,Hongyu Li,Manyuan Zhang,Xinyan Chen,Sifan Wang,Yan Feng,Peng Pei,Pheng-Ann Heng*

Main category: cs.CV

TL;DR: TwiG is the first framework that interleaves textual reasoning throughout visual generation, enabling co-evolving reasoning to guide and reflect on visual content as it's being generated.


<details>
  <summary>Details</summary>
Motivation: Existing visual generation methods use reasoning only before or after generation, lacking on-the-fly multimodal interaction during the generation process itself.
...
TL;DR: Dataset distillation method for pre-trained vision models that optimizes synthetic images to match linear classifier gradients of real data, enabling efficient training of linear probes on top of large pre-trained models.


<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods focus on training from scratch, but modern vision approaches use pre-trained models. Need distillation methods that work with pre-trained feature extractors rather than training from scratch.

Method: Linear Gradient Matching - optimizes synthetic images such that when passed through a pre-trained feature extractor, they induce gradients in the linear classifier similar to those produced by real data.

Result: Synthetic data outperforms all real-image baselines, generalizes across pre-trained vision models (e.g., CLIP probe trained with DINO-distilled data), effective for fine-grained classification, and useful for model interpretability.

Conclusion: Proposed method enables efficient dataset distillation for pre-trained models, with synthetic data that generalizes across architectures and provides valuable tools for model analysis and interpretability.
...
TL;DR: EOGS++ extends 3D Gaussian Splatting for satellite imagery, operating directly on raw panchromatic data with embedded bundle adjustment and improved reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: To improve upon Earth Observation Gaussian Splatting by eliminating preprocessing requirements and enhancing camera pose estimation for satellite imagery.

Method: Extends EOGS framework with direct operation on raw high-resolution panchromatic data, embeds bundle adjustment using optical flow during training, adds early stopping and TSDF post-processing.

Result: Achieves state-of-the-art performance on IARPA 2016 and DFC2019 datasets, improving building reconstruction MAE from 1.33 to 1.19 compared to original EOGS.

Conclusion: EOGS++ outperforms original EOGS and NeRF-based methods while maintaining computational advantages of Gaussian Splatting for Earth observation applications.
...
TL;DR: VARiant is a variant of Visual Auto-Regressive (VAR) models that reduces memory overhead by using depth-adaptive subnets. It employs weight-sharing subnets ranging from 16 to 2 layers from the original 30-layer network, with early scales processed by the full network and later scales by subnets.


<details>
  <summary>Details</summary>
Motivation: Progressive multi-scale generation in VAR models incurs substantial memory overhead due to cumulative KV caching, limiting practical deployment. The authors observed scale-depth asymmetric dependency where early scales are sensitive to network depth while later scales are robust to depth reduction.

Method: Proposed VARiant with equidistant sampling to select multiple subnets (16 to 2 layers) from original 30-layer VAR network. Uses progressive training strategy to address optimization conflicts from weight sharing between subnets and full network, achieving joint optimality.

Result: On ImageNet, VARiant-d16 and VARiant-d8 achieve nearly equivalent quality (FID 2.05/2.12 vs 1.95) while reducing memory consumption by 40-65%. VARiant-d2 achieves 3.5x speedup and 80% memory reduction at moderate quality cost (FID 2.97).

Conclusion: VARiant's single-model architecture supports zero-cost runtime depth switching and provides flexible deployment options from high quality to extreme efficiency, catering to diverse application scenarios while significantly reducing memory overhead.
...
Method: Design a compact yet expressive backbone, hybrid cost aggregation module, and three-stage training strategy on million-scale data to bridge sim-to-real gap.

Result: Ranks 1st across four widely used real-world benchmarks, achieving accuracy comparable to or exceeding state-of-the-art non-prior-based methods with less than 1% computational cost.
...
TL;DR: POMA-3D is the first self-supervised 3D representation model using point maps, which encode 3D coordinates on 2D grids to enable transfer of 2D foundation model priors to 3D understanding.


<details>
  <summary>Details</summary>
Motivation: Address the scarcity of pretrained priors and limited data in 3D representation learning by leveraging point maps that preserve 3D geometry while being compatible with 2D foundation models.

Method: Uses point maps on structured 2D grids, view-to-scene alignment strategy, and POMA-JEPA architecture for geometrically consistent features across multiple views. Pretrained on ScenePoint dataset from 6.5K room-level RGB-D scenes.

Result: POMA-3D serves as strong backbone for both specialist and generalist 3D understanding, benefiting diverse tasks including 3D question answering, embodied navigation, scene retrieval, and embodied localization using only geometric inputs.

Conclusion: POMA-3D explores point maps as an effective approach to 3D scene understanding, successfully transferring 2D priors to 3D while addressing data scarcity in 3D representation learning.
...
TL;DR: Erase to Retain is a controllable unlearning framework for medical image segmentation that uses teacher-student distillation with LoRA modules to selectively remove knowledge while preserving overall performance.


<details>
  <summary>Details</summary>
Motivation: Selective knowledge removal from medical segmentation networks is important for privacy compliance, ethical deployment, and continual dataset revision without full retraining.
...
Result: On ISIC segmentation: reduced forget-set IoU from 0.875 to 0.509 while maintaining retain/validation performance (0.647 to 0.677 IoU). On ISIC classification: decreased forget accuracy from 87.0% to 64.1% while improving retain accuracy from 83.9% to 90.6%.

Conclusion: LoRA-based subspace unlearning provides a practical pathway toward responsible, controllable, and reversible unlearning in medical image analysis, enabling selective forgetting while preserving essential performance.
...
Method: Proposes SAM2S with three key components: (1) DiveMem for robust long-term tracking, (2) temporal semantic learning for instrument understanding, and (3) ambiguity-resilient learning to mitigate annotation inconsistencies.
...
Conclusion: SAM2S effectively addresses surgical iVOS challenges through specialized architecture and training on the SA-SV benchmark, demonstrating substantial performance improvements while maintaining real-time capabilities.
...
TL;DR: AGU is an efficient method for upscaling low-light images that simultaneously reduces noise and increases sharpness through multi-parameter optimization and guided image techniques.


<details>
  <summary>Details</summary>
Motivation: Existing guided image methods fail with low-light images due to high noise and low brightness, resulting in suboptimal image quality improvements.

Method: Uses multi-parameter optimization to learn associations between low-light and bright image characteristics from few sample image pairs, based on guided image principles.

Result: AGU can render high-quality images in real time from low-quality, low-resolution input and outperforms state-of-the-art methods in low-light scenarios.

Conclusion: The proposed method effectively solves the limitations of guided image techniques for low-light image upscaling, achieving superior results with real-time performance.
...
TL;DR: TRIM is a post-training approach that accelerates 3D Gaussian diffusion models through trajectory reduction and instance mask denoising, improving efficiency without quality loss.
...
Method: Uses temporal trimming via a lightweight selector model to evaluate latent primitives and enable early trajectory reduction, plus spatial trimming through instance mask denoising to prune redundant background regions.

Result: Significantly improves both efficiency and quality of 3D generation while supporting inference-time scaling.

Conclusion: TRIM provides an effective post-training solution to accelerate 3D Gaussian diffusion models without compromising output quality.
...
Conclusion: The proposed framework effectively mitigates multi-hierarchy conflicts and class imbalance, providing robust 3D hierarchical semantic segmentation for embodied intelligence applications.
...
TL;DR: The paper critically analyzes Cambrian-S's spatial supersensing benchmarks and methods, showing they can be nearly solved by simple baselines without spatial cognition and that the proposed inference methods exploit benchmark shortcuts rather than demonstrating robust spatial supersensing.


<details>
  <summary>Details</summary>
Motivation: To critically evaluate whether Cambrian-S's benchmarks and methods truly measure spatial supersensing capabilities, or if they can be solved through simpler approaches and shortcut exploitation.

Method: Introduced NoSense baseline using only bag-of-words SigLIP model without temporal structure; created VSC-Repeat sanity check by concatenating videos with themselves; tested Cambrian-S performance on modified benchmarks.

Result: NoSense achieved 95% accuracy on VSR benchmark without spatial cognition; VSC-Repeat collapsed Cambrian-S accuracy from 42% to 0%, revealing reliance on benchmark shortcuts rather than spatial supersensing.
...
Method: Built on PartField part decomposition, PartUV combines semantic part decomposition with geometric heuristics in a top-down recursive framework, ensuring distortion below user threshold while minimizing chart count, with parallelization and handling of non-manifold meshes.

Result: PartUV outperforms existing tools and neural methods in chart count and seam length, achieves comparable distortion, has high success rates on challenging meshes, and enables part-specific multi-tiles packing across four diverse datasets.

Conclusion: PartUV provides an effective solution for UV unwrapping challenging AI-generated meshes by leveraging part-based decomposition and geometric heuristics to produce fewer, better-aligned charts with controlled distortion.
...
TL;DR: TriDiff-4D is a novel 4D generative pipeline that uses diffusion-based triplane re-posing to create high-quality, temporally coherent 4D avatars from text descriptions with skeleton-driven control.
...
Method: Auto-regressive pipeline with two diffusion models: first generates canonical 3D avatar and motion sequence from text, then animates avatar using triplane re-posing. Learns 3D structure and motion priors from large datasets.
...
Conclusion: TriDiff-4D enables efficient, high-quality 4D avatar generation with temporal consistency, motion accuracy, computational efficiency, and visual fidelity through diffusion-based triplane re-posing.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-object pose control suffer from limited controllability and degraded quality, failing to achieve comprehensive 9D pose manipulation for multiple objects simultaneously.

Method: Uses branched network with pre-trained base model, CNOCS map representation for 9D pose encoding, two-stage training with reinforcement learning on rebalanced data, and Disentangled Object Sampling for inference.

Result: Significantly outperforms existing approaches in both controllability and quality, enabling accurate multi-object 9D pose manipulation with customized control for reference subjects.

Conclusion: SceneDesigner provides an effective solution for comprehensive multi-object 9D pose control in image generation, addressing key limitations of previous methods through novel representations and training strategies.
...
TL;DR: V-ReasonBench is a new benchmark for evaluating video reasoning across four dimensions: structured problem-solving, spatial cognition, pattern-based inference, and physical dynamics, using reproducible and verifiable tasks.


<details>
  <summary>Details</summary>
Motivation: The rapid progress in generative video models like Veo-3 has created a need for systematic and reliable evaluation of their reasoning capabilities, as current benchmarks may not adequately measure these emerging abilities.

Method: The benchmark is built from both synthetic and real-world image sequences and provides diverse answer-verifiable tasks that are reproducible, scalable, and unambiguous. It evaluates models across four reasoning dimensions.

Result: Evaluations of six state-of-the-art video models revealed clear dimension-wise differences in reasoning capabilities, with strong variation across structured, spatial, pattern-based, and physical reasoning. The benchmark also enabled comparison with image models, analysis of hallucination behaviors, and study of video duration effects on reasoning.

Conclusion: V-ReasonBench provides a unified and reproducible framework for measuring video reasoning capabilities, supporting the development of models with more reliable, human-aligned reasoning skills.
...
Method: VANS leverages reinforcement learning with Joint-GRPO to align a Vision-Language Model (VLM) with a Video Diffusion Model (VDM), optimizing the VLM to produce visualization-friendly captions and guiding the VDM to generate faithful videos.

Result: VANS achieves state-of-the-art performance on procedural and predictive benchmarks in both video event prediction and visualization.

Conclusion: The proposed VNEP task and VANS model successfully extend video as a new answer modality for next-event prediction, demonstrating the potential of video generation beyond entertainment applications.
...
TL;DR: DualMindVLM enables visual language models to automatically switch between fast and slow thinking modes based on task difficulty, improving efficiency while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs pursue lengthy reasoning chains for all tasks, leading to excessive computational costs, while humans naturally use fast thinking for simple problems and slow thinking for complex ones.

Method: Two-stage approach: 1) Label data as fast/slow thinking based on model output length, 2) Train model using GRPO with thinking mode labels to develop dual-mode thinking capability.

Result: DualMindVLM significantly outperforms base model and achieves performance comparable to state-of-the-art visual reasoning models while maintaining high token efficiency.

Conclusion: The proposed simple RL approach successfully enables VLMs to automatically adapt thinking modes based on task difficulty, achieving both high performance and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To enhance LMM reasoning capabilities without relying on human-curated data or external reward models, enabling autonomous and scalable improvement.

Method: Uses two cooperative agents from a single backbone model: Proposer generates diverse image-grounded questions, and Solver solves them through internal consistency in a continuous self-rewarding process.

Result: Achieves consistent gains up to ~3% on multimodal math-reasoning benchmarks (ChartQA, MathVista, MathVision) using only raw training images.

Conclusion: EvoLMM provides a simple yet effective baseline for self-improving LMMs in a fully unsupervised fashion, demonstrating promising results without ground-truth or human judgments.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods rely on accurate ground-truth camera and human poses for reconstruction, but performance significantly degrades when pose estimates are noisy, limiting practical applicability.
...
Result: Experiments on THuman2.0, XHuman, and HuGe100K datasets show NoPo-Avatar outperforms baselines in practical settings (without ground-truth poses) and achieves comparable results in lab settings (with ground-truth poses).
...
### [111] [Gimballed Rotor Mechanism for Omnidirectional Quadrotors](https://arxiv.org/abs/2511.15909)
*J. Cristobal,A. Z. Zain Aldeen,M. Izadi,R. Faieghi*

Main category: cs.RO

TL;DR: A gimballed rotor mechanism for omnidirectional quadrotors that achieves full actuation through independent rotor tilting, with minimal structural modifications and a new control allocation scheme in PX4.


<details>
  <summary>Details</summary>
Motivation: To create a modular and efficient omnidirectional quadrotor that achieves full actuation in all six degrees of freedom, overcoming the underactuation limitations of conventional quadrotors while maintaining lightweight design.

Method: Proposed a gimballed rotor system with servo motors integrated into rotor platforms, enabling independent tilting of each rotor without major central structure changes. Developed a new control allocation scheme in PX4 Autopilot.

Result: Successful flight tests validated the effectiveness of the proposed gimballed rotor mechanism and control allocation approach.

Conclusion: The gimballed rotor system provides a practical solution for constructing fully-actuated omnidirectional quadrotors with minimal structural modifications, demonstrating successful implementation through flight testing.
...
### [112] [I've Changed My Mind: Robots Adapting to Changing Human Goals during Collaboration](https://arxiv.org/abs/2511.15914)
*Debasmita Ghose,Oz Gitelson,Ryan Jin,Grace Abawe,Marynel Vazquez,Brian Scassellati*

Main category: cs.RO

TL;DR: A method for detecting human goal changes during collaboration by tracking multiple action sequences and using Receding Horizon Planning to adaptively assist while revealing updated goals.


<details>
  <summary>Details</summary>
Motivation: Human-robot collaboration requires robots to adapt to changing human goals mid-task, but prior approaches assume fixed goals, limiting effectiveness in real-world scenarios where goals frequently shift.

Method: Track multiple candidate action sequences and verify plausibility against a policy bank. Upon goal change detection, refine belief in past actions and construct Receding Horizon Planning trees to select actions that assist while encouraging Differentiating Actions to reveal updated goals.

Result: Outperforms three baseline human goal prediction algorithms in collaborative cooking environment with 30 recipes, quickly converging to correct goals after switches, reducing task completion time, and improving collaboration efficiency.

Conclusion: The proposed method effectively handles dynamic goal changes in human-robot collaboration, enabling robots to adapt to shifting human intentions and improve collaborative task performance.
...
### [113] [The Role of Consequential and Functional Sound in Human-Robot Interaction: Toward Audio Augmented Reality Interfaces](https://arxiv.org/abs/2511.15956)
*Aliyah Smith,Monroe Kennedy*

Main category: cs.RO

TL;DR: Study examines how robot sounds affect human perception and behavior, finding that operational sounds don't negatively impact perceptions, spatial localization works well laterally but declines frontally, and spatial sounds can convey information while increasing warmth and reducing discomfort.


<details>
  <summary>Details</summary>
Motivation: As robots become more integrated into everyday environments, understanding how they communicate with humans through sound is critical for effective human-robot interaction.

Method: Examined effects of consequential and functional sounds on human perception and behavior, including novel exploration of spatial sound through localization and handover tasks using Kinova Gen3 manipulator.

Result: Consequential sounds didn't negatively affect perceptions, spatial localization was highly accurate for lateral cues but declined for frontal cues, and spatial sounds could simultaneously convey task-relevant information while promoting warmth and reducing discomfort.

Conclusion: Functional and transformative auditory design has potential to enhance human-robot collaboration and can inform future sound-based interaction strategies.
...
### [114] [PushingBots: Collaborative Pushing via Neural Accelerated Combinatorial Hybrid Optimization](https://arxiv.org/abs/2511.15995)
*Zili Tang,Ying Zhang,Meng Guo*

Main category: cs.RO

TL;DR: This paper presents a framework for multi-robot collaborative pushing of arbitrary objects in cluttered environments, addressing challenges like online task coordination, hybrid contact modes, and under-actuation through combinatorial hybrid optimization.


<details>
  <summary>Details</summary>
Motivation: Many robots lack manipulators and many objects are unsuitable for prehensile manipulation, making pushing an essential non-prehensile skill. Existing work assumes predefined pushing modes and fixed-shape objects, while this work tackles the general problem of multi-robot collaborative pushing of arbitrary objects in complex environments.

Method: The method uses combinatorial hybrid optimization with three main components: (I) decomposition and rolling assignment of pushing subtasks to robot subgroups, (II) keyframe-guided hybrid search to optimize pushing mode sequences, and (III) hybrid control for mode execution and transitions. A diffusion-based accelerator predicts keyframes and prioritized pushing modes.

Result: The framework is complete under mild assumptions and demonstrates efficiency and effectiveness across different robot numbers and general-shaped objects in simulations and hardware experiments. It also generalizes to heterogeneous robots, planar assembly, and 6D pushing.

Conclusion: The proposed framework successfully addresses the complex problem of multi-robot collaborative pushing in cluttered environments, providing a comprehensive solution that handles task coordination, hybrid contact modes, and under-actuation through optimized hybrid optimization and control strategies.
...
### [115] [Semantic Glitch: Agency and Artistry in an Autonomous Pixel Cloud](https://arxiv.org/abs/2511.16048)
*Qing Zhang,Jing Huang,Mingyang Xu,Jun Rekimoto*

Main category: cs.RO

TL;DR: This paper presents a "lo-fi" robotics approach using a soft flying robot called "Semantic Glitch" that navigates autonomously using only semantic understanding from a Multimodal Large Language Model, rejecting conventional sensors like LiDAR and SLAM.


<details>
  <summary>Details</summary>
Motivation: To explore the creative potential of deliberately "lo-fi" robotics that prioritizes character and narrative over metric precision and flawless performance, creating imperfect companions measured by personality rather than efficiency.

Method: Developed a novel autonomous pipeline using a Multimodal Large Language Model for semantic understanding instead of conventional sensors, with a bio-inspired personality authored through natural language prompts to create a "narrative mind" complementing the robot's "weak" physical form.

Result: Successful 13-minute autonomous flight demonstrated emergent behaviors including landmark-based navigation and a "plan to execution" gap, with follow-up study validating the framework's robustness for creating distinct personas. The robot exhibited unpredictable but plausible behavior due to lack of precise proprioception.

Conclusion: The lo-fi framework successfully creates imperfect robotic companions whose success is measured by character rather than efficiency, demonstrating the creative potential of semantic-based navigation over traditional precision-focused approaches.
...
### [116] [Bi-AQUA: Bilateral Control-Based Imitation Learning for Underwater Robot Arms via Lighting-Aware Action Chunking with Transformers](https://arxiv.org/abs/2511.16050)
*Takeru Tsunoori,Masato Kobayashi,Yuki Uranishi*

Main category: cs.RO

TL;DR: Bi-AQUA is the first underwater bilateral control-based imitation learning framework that addresses lighting challenges through a three-level lighting adaptation mechanism, enabling robust underwater robotic manipulation.


<details>
  <summary>Details</summary>
Motivation: Underwater robotic manipulation faces extreme lighting variations, color distortion, and reduced visibility, which traditional methods struggle to handle effectively.

Method: Uses a hierarchical three-level lighting adaptation: Lighting Encoder for lighting representation extraction, FiLM modulation for adaptive feature extraction, and explicit lighting token for task-aware conditioning in transformer encoder.

Result: Achieves robust performance in real-world underwater pick-and-place tasks under diverse static and dynamic lighting conditions, substantially outperforming baseline without lighting modeling.

Conclusion: Bridges terrestrial bilateral control-based imitation learning with underwater manipulation, enabling force-sensitive autonomous operation in challenging marine environments.
...
### [117] [MagBotSim: Physics-Based Simulation and Reinforcement Learning Environments for Magnetic Robotics](https://arxiv.org/abs/2511.16158)
*Lara Bergmann,Cedric Grothues,Klaus Neumann*

Main category: cs.RO

TL;DR: This paper introduces MagBotSim, a physics-based simulation for magnetic levitation systems that enables coordinated swarm robotics for manufacturing by merging transportation and manipulation capabilities.


<details>
  <summary>Details</summary>
Motivation: Magnetic levitation systems have unexploited potential for manipulation beyond transportation. Merging transportation and manipulation into coordinated magnetic robot swarms can achieve higher efficiency, adaptability, and compactness in manufacturing systems.

Method: The authors developed MagBotSim, a physics-based simulation framework for magnetic levitation systems, framing them as robot swarms to support intelligent algorithm development.

Result: MagBotSim provides a dedicated simulation environment for magnetic levitation systems, enabling research and development of next-generation manufacturing systems powered by magnetic robotics.

Conclusion: This work lays the foundation for next-generation manufacturing systems by introducing a simulation framework that treats magnetic levitation systems as coordinated robot swarms, enabling the development of intelligent algorithms for magnetic robotics.
...
### [118] [PIPHEN: Physical Interaction Prediction with Hamiltonian Energy Networks](https://arxiv.org/abs/2511.16200)
*Kewei Chen,Yayu Long,Mingsheng Shang*

Main category: cs.RO

TL;DR: PIPHEN is a distributed physical cognition-control framework that replaces raw data transmission with semantic communication, compressing data to <5% of original volume and reducing decision latency from 315ms to 76ms.


<details>
  <summary>Details</summary>
Motivation: Address the 'shared brain dilemma' in multi-robot systems where transmitting high-dimensional multimedia data (e.g., video streams at ~30MB/s) creates severe bandwidth bottlenecks and decision-making latency.

Method: Uses semantic distillation at robot edge to reconstruct perceptual data into compact physical representations via Physical Interaction Prediction Network (PIPN) and Hamiltonian Energy Network (HEN) controller for coordinated actions.

Result: Compresses information representation to <5% of original data volume, reduces collaborative decision-making latency from 315ms to 76ms, and significantly improves task success rates compared to baseline methods.

Conclusion: Provides a fundamentally efficient paradigm for resolving the 'shared brain dilemma' in resource-constrained multi-robot systems through semantic communication rather than raw data transmission.
...
### [119] [DynaMimicGen: A Data Generation Framework for Robot Learning of Dynamic Tasks](https://arxiv.org/abs/2511.16223)
*Vincenzo Pomponi,Paolo Franceschi,Stefano Baraldo,Loris Roveda,Oliver Avram,Luca Maria Gambardella,Anna Valente*

Main category: cs.RO

TL;DR: DynaMimicGen (D-MG) is a scalable dataset generation framework that creates diverse manipulation training data from minimal human demonstrations, enabling robust policy learning in dynamic environments through Dynamic Movement Primitives and real-time adaptation.


<details>
  <summary>Details</summary>
Motivation: Traditional methods require large, diverse datasets that are time-consuming and impractical to collect, especially for dynamic environments where conditions change during task execution.

Method: Given few human demonstrations, D-MG segments them into sub-tasks, then uses Dynamic Movement Primitives (DMPs) to adapt and generalize behaviors to novel dynamic environments, producing smooth Cartesian trajectories that adapt to real-time changes in object poses, robot states, or scene geometry.

Result: Agents trained on D-MG-generated data achieve strong performance across long-horizon and contact-rich benchmarks like cube stacking and placing mugs in drawers, even under unpredictable environment changes.

Conclusion: D-MG eliminates the need for extensive human demonstrations and enables generalization in dynamic settings, offering a powerful and efficient alternative to manual data collection for scalable autonomous robot learning.
...
### [120] [FT-NCFM: An Influence-Aware Data Distillation Framework for Efficient VLA Models](https://arxiv.org/abs/2511.16233)
*Kewei Chen,Yayu Long,Shuai Li,Mingsheng Shang*

Main category: cs.RO

TL;DR: FT-NCFM is a data-centric framework that uses causal attribution and contrastive verification to distill high-value datasets, enabling VLA models to achieve 85-90% performance with only 5% of data while reducing training time by 80%.


<details>
  <summary>Details</summary>
Motivation: Current VLA models rely on massive, redundant datasets that hinder widespread application, and existing model-centric approaches like compression or policy distillation fail to address the fundamental data-level challenges.

Method: Uses a Fact-Tracing (FT) engine combining causal attribution with programmatic contrastive verification to assess sample value, followed by adversarial NCFM process to synthesize model-agnostic, information-dense data assets.

Result: Models trained on just 5% of distilled coreset achieve 85-90% success rate compared to full dataset training, while reducing training time by over 80% on mainstream VLA benchmarks.

Conclusion: Intelligent data distillation represents a highly promising new path for building efficient, high-performance VLA models by fundamentally addressing data-level bottlenecks.
...
### [121] [How Robot Dogs See the Unseeable](https://arxiv.org/abs/2511.16262)
...
TL;DR: This paper proposes using robotic peering motions to create synthetic aperture sensing, which blurs out occluding foreground elements while keeping background objects in focus, enabling better scene understanding in cluttered environments.


<details>
  <summary>Details</summary>
Motivation: To overcome the fundamental limitation of partial occlusion in robotic vision, where conventional cameras render both foreground obstacles and background objects in sharp focus, causing occluders to obscure critical scene information.

Method: By having a robot execute peering motions (side-to-side motion), the camera describes a wide synthetic aperture. Computational integration of the captured images synthesizes an image with extremely shallow depth of field, effectively blurring out occluding elements while bringing background into sharp focus.
...
Conclusion: Synthetic aperture sensing via peering is robust to occlusion, computationally efficient, and immediately deployable on any mobile robot, bridging animal behavior and robotics for advanced scene understanding in complex, cluttered environments.
...
### [122] [Funabot-Upper: McKibben Actuated Haptic Suit Inducing Kinesthetic Perceptions in Trunk, Shoulder, Elbow, and Wrist](https://arxiv.org/abs/2511.16265)
*Haru Fukatsu,Ryoji Yasuda,Yuki Funabora,Shinji Doki*

Main category: cs.RO

TL;DR: Funabot-Upper is a wearable haptic suit that induces kinesthetic perception of 14 upper-body motions across trunk, shoulder, elbow, and wrist joints using a new design policy that stimulates joints and muscles independently, reducing perceptual mixing and improving recognition accuracy from 68.8% to 94.6%.


<details>
  <summary>Details</summary>
Motivation: Previous haptic devices were limited to single body parts or caused perceptual mixing when applied to multiple parts. The authors aimed to develop a comprehensive upper-body haptic suit that could accurately induce kinesthetic perception across multiple joints without perceptual interference.

Method: Developed a new simplified design policy using artificial muscle contraction technology to deform clothing in 3D, stimulating joints and muscles independently rather than stimulating multiple human muscles together. This approach prevents perceptual mixing between adjacent joints like shoulder and elbow.

Result: The new Funabot-Upper suit successfully induced kinesthetic perception across multiple joints while significantly reducing perceptual mixing. Recognition accuracy improved dramatically from 68.8% in the previous design to 94.6% in the new design.

Conclusion: The independent stimulation of joints and muscles through the new design policy effectively reduces perceptual mixing and enables accurate kinesthetic perception induction across multiple upper-body joints, demonstrating the suit's superiority and potential for future haptic applications.
...
### [123] [InEKFormer: A Hybrid State Estimator for Humanoid Robots](https://arxiv.org/abs/2511.16306)
*Lasse Hohmeyer,Mihaela Popescu,Ivan Bergonzani,Dennis Mronga,Frank Kirchner*

Main category: cs.RO

TL;DR: Proposes InEKFormer, a hybrid state estimation method combining invariant extended Kalman filter with Transformer network for humanoid robot state estimation, showing Transformer potential but highlighting autoregressive training challenges.


<details>
  <summary>Details</summary>
Motivation: Humanoid robots face challenges in stable bipedal locomotion across different environments, requiring fast and accurate state estimation. Classical methods like Kalman filters need expert tuning, while deep learning offers promising alternatives.

Method: Developed InEKFormer - a hybrid approach integrating invariant extended Kalman filter (InEKF) with Transformer network architecture for state estimation.

Result: Compared with InEKF and KalmanNet on RH5 humanoid robot datasets. Results show Transformers have potential for humanoid state estimation but reveal challenges with robust autoregressive training in high-dimensional problems.

Conclusion: Transformers demonstrate promise for humanoid state estimation tasks, though effective training strategies for high-dimensional autoregressive problems need further development.
...
### [124] [Safe and Optimal Variable Impedance Control via Certified Reinforcement Learning](https://arxiv.org/abs/2511.16330)
*Shreyas Kumar,Ravi Prakash*

Main category: cs.RO

TL;DR: C-GMS is a trajectory-centric RL framework that learns stable DMP and VIC policies by sampling from a mathematically defined manifold, guaranteeing Lyapunov stability and actuator feasibility without reward penalties.


<details>
  <summary>Details</summary>
Motivation: Traditional model-free RL for robot control risks instability and unsafe exploration due to time-varying impedance gains, requiring safer learning approaches.

Method: Reframes policy exploration as sampling from a mathematically defined manifold of stable gain schedules, ensuring every policy rollout is stable and physically realizable by construction.

Result: The approach guarantees bounded tracking error even with model errors and uncertainties, and demonstrates effectiveness in simulation and real robot verification.

Conclusion: C-GMS enables reliable autonomous interaction in complex environments by providing certified stability and feasibility guarantees for learned policies.
...
### [125] [Flow-Aided Flight Through Dynamic Clutters From Point To Motion](https://arxiv.org/abs/2511.16372)
*Bowen Xu,Zexuan Yan,Minghao Lu,Xiyu Fan,Yi Luo,Youshen Lin,Zhiqiang Chen,Yeke Chen,Qiyuan Qiao,Peng Lu*

Main category: cs.RO

TL;DR: This paper presents a reinforcement learning-based autonomous flight system that uses single LiDAR sensing to navigate dynamic environments without explicit object detection, tracking, or prediction. The system integrates depth sensing and point flow features to create a lightweight representation of dynamic environments, enabling safe quadrotor navigation.


<details>
  <summary>Details</summary>
Motivation: Previous approaches for navigating dynamic environments rely on explicit modeling of obstacle motion, which is time-consuming and unreliable in highly dynamic scenarios with occlusions. The authors aim to develop a more efficient and reliable system that doesn't require object detection, tracking, and prediction.

Method: The method uses reinforcement learning with single LiDAR sensing. It creates a depth sensing distance map from raw point clouds and extracts motion features (point flow) from multi-frame observations. These are integrated into a lightweight representation of dynamic environments. The policy optimization is guided by relative motion modulated distance field.

Result: The proposed system demonstrates superior success rate and adaptability compared to alternatives. The policy trained in simulation successfully transfers to real-world quadrotors, enabling safe maneuvers in dynamic environments.

Conclusion: The system provides an effective approach for autonomous flight in dynamic environments by leveraging reinforcement learning with integrated depth and motion sensing, eliminating the need for explicit object modeling while maintaining safety and performance.
...
### [126] [Robot Metacognition: Decision Making with Confidence for Tool Invention](https://arxiv.org/abs/2511.16390)
*Ajith Anil Meera,Poppy Collis,Polina Arbuzova,Abián Torres,Paul F Kinghorn,Ricardo Sanz,Pablo Lanillos*

Main category: cs.RO

TL;DR: The paper proposes a robot metacognition architecture using confidence as a second-order judgment to enable robots to reflect on their own decisions, demonstrated through autonomous tool invention.


<details>
  <summary>Details</summary>
Motivation: Current robots lack the ability to reflect on their cognitive processes, which is crucial for intelligent behavior. Human metacognition enables self-monitoring, confidence evaluation, and behavior regulation, which robots need for better learning and decision-making.

Method: Proposes a robot metacognition architecture centered on confidence as a metacognitive measure within decision making, inspired by neuroscience. Demonstrates the approach on autonomous tool invention use case.

Result: Confidence-informed robots can evaluate decision reliability and improve robustness during real-world physical deployment through embodied action monitoring.

Conclusion: Robot metacognition using confidence enables better informed decisions and opens up new applications and research directions for intelligent robotic systems.
...
### [127] [Homogeneous Proportional-Integral-Derivative Controller in Mobile Robotic Manipulators](https://arxiv.org/abs/2511.16406)
*Luis Luna,Isaac Chairez,Andrey Polyakov*

Main category: cs.RO

TL;DR: A novel homogeneous PID (hPID) control strategy for mobile robotic manipulators that uses homogeneous control theory to enhance stability and convergence, outperforming traditional PID controllers in trajectory tracking.


<details>
  <summary>Details</summary>
Motivation: Mobile robotic manipulators face control challenges due to nonlinear dynamics, underactuation, and coupling between base and manipulator subsystems, requiring robust control methods that handle uncertainties and disturbances.

Method: Developed a homogeneous PID controller using homogeneous control theory framework, replacing traditional PID gains with nonlinear state-dependent functions through graded homogeneity approach, with Lyapunov-based stability analysis.

Result: Experimental validation showed hPID controller achieves high-precision trajectory tracking, outperforming conventional PID in response time, steady-state error, and robustness to model uncertainties, with proven global asymptotic stability and finite-time convergence.

Conclusion: The hPID controller provides a scalable and analytically grounded framework for enhancing autonomy and reliability of mobile manipulation systems in various environments.
...
### [128] [LAOF: Robust Latent Action Learning with Optical Flow Constraints](https://arxiv.org/abs/2511.16407)
*Xizhou Bu,Jiexi Lyu,Fulei Sun,Ruichen Yang,Zhiqiang Ma,Wei Li*

Main category: cs.RO

TL;DR: LAOF is a pseudo-supervised framework that uses optical flow constraints to learn robust latent action representations from videos, outperforming existing methods on imitation and reinforcement learning tasks even with limited action labels.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with action-irrelevant distractors in videos, and action supervision is limited by scarce action labels. Optical flow naturally suppresses background elements and emphasizes moving objects, providing action-driven signals.

Method: Propose LAOF framework that leverages agent's optical flow as action-driven signal to learn latent action representations. Uses optical flow constraints to suppress distractors and improve representation quality.

Result: LAOF outperforms existing methods on downstream imitation learning and reinforcement learning tasks. Optical flow constraints stabilize training and improve latent representation quality under label-scarce conditions (effective even with only 10% action labels).

Conclusion: LAOF demonstrates that optical flow constraints enable robust latent action learning without heavy reliance on action supervision, achieving competitive performance even without action labels compared to supervised methods with 1% labels.
...
### [129] [From Prompts to Printable Models: Support-Effective 3D Generation via Offset Direct Preference Optimization](https://arxiv.org/abs/2511.16434)
*Chenming Wu,Xiaofan Li,Chengkai Dai*

Main category: cs.RO

TL;DR: SEG is a novel framework that integrates Direct Preference Optimization with Offset (ODPO) into 3D generation to create models requiring minimal support structures for 3D printing, reducing material waste and production time.


<details>
  <summary>Details</summary>
Motivation: Current 3D printing support strategies focus on post-processing optimizations rather than addressing support-efficient design during model generation, leading to unnecessary material waste and longer production times.

Method: SEG integrates Direct Preference Optimization with an Offset (ODPO) into the 3D generation pipeline and incorporates support structure simulation during training to encourage geometries that inherently require fewer supports.

Result: SEG significantly outperforms baseline models (TRELLIS, DPO, DRO) on Thingi10k-Val and GPT-3DP-Val datasets in support volume reduction and printability, while maintaining high fidelity to input prompts.

Conclusion: SEG demonstrates the potential to transform 3D printing by directly optimizing models during the generative process, enabling more sustainable and efficient digital fabrication practices.
...
### [130] [MiMo-Embodied: X-Embodied Foundation Model Technical Report](https://arxiv.org/abs/2511.16518)
...
TL;DR: MiMo-Embodied is the first cross-embodied foundation model achieving SOTA performance in both Autonomous Driving and Embodied AI across 29 benchmarks, demonstrating strong positive transfer between these domains.


<details>
  <summary>Details</summary>
Motivation: To create a unified foundation model that can bridge Autonomous Driving and Embodied AI domains, exploring their synergistic relationship and mutual reinforcement through cross-domain learning.

Method: Multi-stage learning, curated data construction, and CoT/RL fine-tuning to integrate knowledge from both Autonomous Driving and Embodied AI domains.

Result: Sets new records across 17 embodied AI benchmarks (Task Planning, Affordance Prediction, Spatial Understanding) and 12 autonomous driving benchmarks (Environmental Perception, Status Prediction, Driving Planning), significantly outperforming all existing baselines.

Conclusion: Autonomous Driving and Embodied AI domains exhibit strong positive transfer and mutually reinforce each other, enabling superior performance when integrated in a unified foundation model.
...
### [131] [InternData-A1: Pioneering High-Fidelity Synthetic Data for Pre-training Generalist Policy](https://arxiv.org/abs/2511.16651)
...
TL;DR: This paper demonstrates that synthetic data alone can match real-robot pre-training performance for Vision-Language-Action models, achieving comparable results across 49 simulation tasks, 5 real-world tasks, and 4 long-horizon dexterous tasks.


<details>
  <summary>Details</summary>
Motivation: To challenge the current paradigm that real-robot data is essential for VLA model pre-training by showing that large-scale synthetic data can achieve comparable performance.

Method: Created InternData-A1 synthetic dataset with 630k+ trajectories across 4 embodiments using a fully autonomous simulation pipeline, then pre-trained a VLA model using the same architecture as π₀ but exclusively on synthetic data.

Result: The synthetic-data-only model matched the official π₀ model's performance across all tested scenarios, demonstrating surprising zero-shot sim-to-real transfer on challenging tasks.

Conclusion: Synthetic data alone can be as effective as real-robot data for VLA model pre-training, enabling scalable data creation and lowering barriers for embodied AI research.
...
### [132] [Dexterity from Smart Lenses: Multi-Fingered Robot Manipulation with In-the-Wild Human Demonstrations](https://arxiv.org/abs/2511.16661)
...
TL;DR: AINA framework enables learning multi-fingered robot policies directly from human videos captured by Aria Gen 2 glasses, eliminating the need for robot data collection and bridging the human-robot embodiment gap.


<details>
  <summary>Details</summary>
Motivation: To overcome the bottleneck of learning robot policies from human videos due to embodiment gap and difficulty extracting relevant cues, enabling generalizable robot manipulation in human environments without labor-intensive robot data collection.

Method: Uses Aria Gen 2 glasses with high-resolution RGB camera, accurate 3D head/hand poses, and stereo view for depth estimation to collect human demonstration data, then learns 3D point-based policies for multi-fingered hands that are robust to background changes.

Result: Successfully demonstrates learning across nine everyday manipulation tasks without requiring robot data, online corrections, reinforcement learning, or simulation, outperforming prior human-to-robot policy learning approaches.

Conclusion: AINA represents a significant step toward learning multi-fingered robot policies from in-the-wild human videos, enabling data collection by anyone, anywhere, and in any environment using portable hardware.
...
TL;DR: An ensemble framework (eLLM) using multiple large language models improves text categorization by addressing individual model weaknesses, achieving up to 65% F1-score improvement over single models and near human-level performance.


<details>
  <summary>Details</summary>
Motivation: Individual LLMs suffer from inconsistency, hallucination, category inflation, and misclassification in text categorization tasks, limiting their reliability for taxonomy-based classification.

Method: Developed an ensemble LLM (eLLM) framework that integrates multiple models through mathematical collective decision-making with principled aggregation criteria, evaluated on 10 state-of-the-art LLMs using IAB taxonomy on 8,660 human-annotated samples under zero-shot conditions.

Result: eLLM achieved up to 65% F1-score improvement over the strongest single model, showing individual models plateau due to text compression into sparse categorical representations, while ensemble approach improves both robustness and accuracy to near human-expert levels.

Conclusion: The eLLM framework provides a scalable and reliable solution for taxonomy-based classification that can significantly reduce dependence on human expert labeling while achieving near human-level performance.
...
Method: Encode past decision graphs and retrieve them through structural and semantic similarity, enabling compositional reuse of subgraphs across new reasoning tasks with an optimization objective balancing efficiency and consistency.

Result: Theoretical framework established for interpretable, cost-efficient reasoning with persistent memory capabilities, though specific empirical results not provided in abstract.

Conclusion: This framework lays groundwork for persistent memory in large-scale agentic systems, enabling self-improving reasoning architectures through reusable computational traces.
...
TL;DR: MACIE is a framework for explaining multi-agent reinforcement learning systems using causal models, counterfactuals, and Shapley values to attribute outcomes, quantify emergent behaviors, and generate natural language explanations.


<details>
  <summary>Details</summary>
Motivation: Understanding decision-making and collective behavior in multi-agent systems is crucial for safety-critical applications, but existing explainable AI methods fail in multi-agent settings by not attributing collective outcomes to individuals or quantifying emergent behaviors.

Method: MACIE combines structural causal models, interventional counterfactuals, and Shapley values to provide three types of explanations: individual agent causal contributions, system-level emergent intelligence through synergy metrics, and actionable natural language narratives.

Result: Evaluation across four MARL scenarios showed accurate outcome attribution (mean φᵢ=5.07, std<0.05), detection of positive emergence in cooperative tasks (synergy index up to 0.461), and efficient computation (0.79 seconds per dataset on CPU).

Conclusion: MACIE uniquely combines causal rigor, emergence quantification, and multi-agent support while remaining practical for real-time use, representing progress toward interpretable, trustworthy, and accountable multi-agent AI.
...
TL;DR: The paper analyzes how different text and image modalities affect perception in ARC-AGI tasks, finding that structured text provides precise coordinates while images capture 2D shapes, with combined modalities improving execution accuracy.


<details>
  <summary>Details</summary>
Motivation: To understand how different encoding modalities (text vs. images) shape model perception in ARC-AGI tasks and separate instruction errors from execution errors, since current systems lack principled accounts of how encodings affect perception.

Method: Used a two-stage reasoning pipeline with nine text and image modalities, employing a weighted set-disagreement metric to isolate perception from reasoning across different encoding formats.
...
TL;DR: ToolMind is a large-scale, high-quality tool-agentic dataset with 160k synthetic and 200k augmented data instances, featuring turn-level validation to prevent error propagation during LLM agent training.


<details>
  <summary>Details</summary>
Motivation: The scarcity of high-quality trajectories hinders LLM agent development, and existing works only validate correctness at trajectory level, overlooking turn-level errors that propagate during training.

Method: Construct function graph based on parameter correlations, use multi-agent framework for realistic user-assistant-tool interactions, and employ fine-grained turn-level filtering to remove erroneous steps while preserving self-corrective reasoning signals.

Result: Models fine-tuned on ToolMind show significant improvements over baselines on several benchmarks.

Conclusion: ToolMind addresses data quality issues in LLM agent training through turn-level validation and filtering, enabling more robust tool-use learning and better model performance.
...
TL;DR: CoS (Chain of Summaries) generates information-dense web content summaries using Hegel's dialectical method, improving LLM performance while reducing token usage.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with web content due to unfriendly formats and context length limitations, needing better digestible summaries.

Method: Iterative refinement using Hegel's dialectical method: initial summary (thesis) → questioning limitations (antithesis) → general-purpose summary (synthesis).

Result: Outperforms zero-shot LLM baselines by up to 66% and specialized summarization methods by up to 27%, with higher Q&A performance using fewer tokens.

Conclusion: CoS provides an effective solution for making web content more accessible to LLMs while allowing human oversight.
...
TL;DR: A multimodal AI framework combining text and image analysis for construction safety hazard identification, using LLMs for OSHA report analysis and VLMs for safety violation detection in site imagery.


<details>
  <summary>Details</summary>
Motivation: Construction sites generate safety data in multiple formats (text reports, images), making traditional hazard synthesis challenging. Need integrated multimodal approach to improve safety monitoring.

Method: Two case studies: 1) Hybrid pipeline using GPT-4o models to analyze 28,000 OSHA accident reports (2000-2025); 2) Evaluation of lightweight VLMs (Molmo 7B, Qwen2 VL 2B) on ConstructionSite10k dataset for rule-level safety violation detection using natural language prompts.

Result: Lightweight VLMs showed competitive performance in certain prompt configurations despite smaller size. GPT-4o models successfully extracted structured insights from large text dataset. Framework demonstrated feasibility of low-resource multimodal systems for safety monitoring.

Conclusion: Multimodal AI framework combining text and visual analysis is viable for construction safety hazard identification. Lightweight open-source models can provide cost-effective alternatives to proprietary models for rule-aware safety monitoring.
...
TL;DR: This survey paper introduces a cognitive-based taxonomy for spatial reasoning in MLLMs, organizing tasks by reasoning complexity and cognitive functions rather than input modality, and analyzes methods for improving spatial abilities.


<details>
  <summary>Details</summary>
Motivation: Existing surveys categorize spatial reasoning progress based on input modality (text, image, video, 3D), but spatial ability is not solely determined by input format. The authors argue for a cognitive perspective to enable more principled cross-task comparisons and reveal gaps between current models and human-like reasoning.

Method: The survey introduces a taxonomy that organizes spatial intelligence from cognitive aspects, divides tasks by reasoning complexity, and links them to cognitive functions. It maps existing benchmarks across text-only, vision-language, and embodied settings onto this taxonomy, and reviews evaluation metrics and methodologies.

Result: The cognitive perspective enables more principled cross-task comparisons and reveals critical gaps between current model capabilities and human-like spatial reasoning. The analysis spans both training-based and reasoning-based approaches for improving spatial ability.

Conclusion: By surveying tasks, benchmarks, and recent advances from a cognitive perspective, this survey provides new researchers with a comprehensive understanding of spatial reasoning in MLLMs and actionable directions for future research, clarifying strengths and complementary mechanisms of different improvement approaches.
...
TL;DR: This thesis proposes a consistency-guided cross-modal transfer framework for uncertainty-resilient multimodal learning that uses cross-modal semantic consistency to improve robustness against noisy data, low-quality labels, and heterogeneous modality characteristics.


<details>
  <summary>Details</summary>
Motivation: Multimodal learning faces substantial uncertainty from noisy data, low-quality labels, and heterogeneous modality characteristics, especially in human-computer interaction settings where data quality and annotation consistency vary across users and recording conditions.

Method: The framework projects heterogeneous modalities into a shared latent space to mitigate modality gaps and uncover structural relations that support uncertainty estimation and stable feature learning, using cross-modal semantic consistency as a basis for robust representation learning.
...
Conclusion: The thesis provides a unified perspective on resilient multimodal learning by integrating uncertainty modeling, semantic alignment, and data-efficient supervision, offering practical insights for developing reliable and adaptive brain-computer interface systems.
...
TL;DR: This paper proposes using LLMs and AI agents to create education assistants for biomechanics courses, employing RAG for conceptual questions and Multi-Agent Systems for calculation problems, showing significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with domain-specific knowledge and complex multi-step reasoning in specialized fields like biomechanics, creating a need for enhanced educational tools.

Method: Developed a dual-module framework: 1) RAG for improving conceptual question responses, 2) Multi-Agent System for solving calculation problems requiring multi-step reasoning and code execution.

Result: RAG significantly enhanced LLM performance on conceptual questions, and the MAS successfully handled complex calculation tasks with explainable solutions across multiple LLM models.

Conclusion: RAG and MAS show strong potential for enhancing LLM performance in specialized engineering education, providing a promising approach for intelligent tutoring systems.
...
TL;DR: Multi-agent orchestration dramatically improves LLM-based incident response quality, achieving 100% actionable recommendations vs 1.7% for single-agent approaches, with zero quality variance across 348 trials.


<details>
  <summary>Details</summary>
Motivation: Single-agent LLM approaches generate vague, unusable recommendations for incident response, limiting their production deployment despite LLMs' potential to accelerate response times.
...
Result: Multi-agent orchestration achieved 80x improvement in action specificity, 140x improvement in solution correctness, and 100% actionable recommendation rate vs 1.7% for single-agent, with zero quality variance across all trials.
...
TL;DR: This paper addresses the gap in systematic assessment of AI supply chain risks and proposes a taxonomy to help stakeholders systematically inventory dependencies across AI systems.


<details>
  <summary>Details</summary>
Motivation: There's a critical gap in assessing supply chain risks associated with AI systems, particularly problematic when AI is used in critical applications like healthcare, utilities, law, and transport.
...
Result: The paper presents a proposed taxonomy that enables stakeholders, especially non-experts, to systematically assess AI supply chain risks and dependencies.

Conclusion: The contribution bridges the gap between current AI governance and the urgent need for actionable risk assessment and management of AI use in critical applications.
...
TL;DR: Comparative analysis of rule-based NLP methods vs LLMs for extracting medical information from Polish EHRs, showing rule-based methods have higher accuracy for demographics while LLMs excel at drug recognition and scalability.


<details>
  <summary>Details</summary>
Motivation: Extracting structured medical insights from unstructured clinical text is challenging in non-English healthcare contexts where NLP resources are scarce, particularly in Polish clinical settings.

Method: Evaluated rule-based NLP methods and Large Language Models for extracting patient demographics, clinical findings, and medications from Polish EHRs, examining effects of text normalization and translation-induced information loss.

Result: Rule-based methods provided higher accuracy for age and sex extraction, while LLMs offered greater adaptability and scalability, excelling in drug name recognition. Translation from Polish to English caused information loss affecting LLM performance.
...
TL;DR: IMACT-CXR is an interactive multi-agent conversational tutor for chest X-ray interpretation that combines spatial annotation, gaze analysis, knowledge retrieval, and vision-language reasoning in an AutoGen-based workflow to provide personalized coaching for trainees.


<details>
  <summary>Details</summary>
Motivation: To create an intelligent tutoring system that helps medical trainees improve their chest X-ray interpretation skills by integrating multiple modalities (spatial annotations, gaze tracking, knowledge retrieval) and providing personalized, Socratic coaching while preventing premature disclosure of answers.

Method: Uses an AutoGen-based multi-agent workflow that processes learner bounding boxes, gaze samples, and free-text observations. Includes specialized agents for localization evaluation, Socratic coaching, PubMed evidence retrieval, similar case suggestion from REFLACX, and NV-Reason-CXR-3B for vision-language reasoning. Implements Bayesian Knowledge Tracing (BKT) for skill mastery tracking and lung-lobe segmentation for anatomically aware gaze feedback.
...
Conclusion: IMACT-CXR successfully integrates multiple AI components into a cohesive tutoring system that provides personalized, interactive training for chest X-ray interpretation, with promising results for improving trainee skills while maintaining appropriate answer disclosure controls.
...
<details>
  <summary>Details</summary>
Motivation: Current AI systems struggle with interconnected real-world decision-making challenges like multi-faceted optimization, active learning from sparse experience, long-horizon planning under uncertainty, and spatial reasoning. Existing benchmarks isolate these capabilities rather than testing them holistically.

Method: Developed MAPs, an amusement-park simulator that requires agents to model environments, anticipate long-term consequences under uncertainty, and strategically operate a complex business. Provided human baselines and evaluated state-of-the-art LLM agents.

Result: Humans significantly outperform current LLM agents - by 6.5x on easy mode and 9.8x on medium mode. Analysis reveals persistent weaknesses in long-horizon optimization, sample-efficient learning, spatial reasoning, and world modeling.

Conclusion: MAPs provides a unified foundation for benchmarking agents capable of adaptable decision making by integrating multiple real-world challenges in a single environment, highlighting the need for improved AI systems that can handle complex, interconnected decision-making tasks.
...
TL;DR: Step-Audio-R1 is the first successful audio reasoning model that demonstrates reasoning capabilities can be effectively transferred to the audio domain through proper grounding in acoustic features.


<details>
  <summary>Details</summary>
Motivation: Address the perplexing phenomenon where audio language models perform better with minimal reasoning, questioning whether audio intelligence can truly benefit from deliberate thinking.
...
Conclusion: Reasoning is a transferable capability across modalities when appropriately anchored, transforming extended deliberation from a liability into a powerful asset for audio intelligence, opening new pathways for multimodal reasoning systems.
...
TL;DR: The study shows that activation steering improves Theory of Mind (ToM) in LLMs by enhancing emotional processing while suppressing analytical reasoning, with accuracy increasing from 32.5% to 46.7% on belief attribution tasks.
...
Method: Used linear probes trained on 45 cognitive actions to compare steered vs baseline LLM activations, applied Contrastive Activation Addition (CAA) steering to Gemma-3-4B, and evaluated on 1,000 BigToM forward belief scenarios.

Result: Improved performance on belief attribution tasks (32.5% to 46.7% accuracy) mediated by increased emotional processing (emotion perception +2.23, emotion valuing +2.20) and suppressed analytical processes (questioning -0.78, convergent thinking -1.59).
...
TL;DR: A self-correcting framework for LLMs that detects hallucinations during multi-step reasoning using fine-grained uncertainty signals (confidence alignment and token-level entropy) and improves reasoning through reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: To address hallucinations in LLMs during multi-step reasoning by detecting unreliable reasoning in real-time rather than just checking final answer correctness.

Method: Uses fine-grained uncertainty signals (self-assessed confidence alignment and token-level entropy spikes) with a composite reward function that penalizes unjustified high confidence and entropy spikes. Implements reinforcement learning to shape generation behavior through confidence-aware feedback.

Result: Improves both final answer accuracy and reasoning calibration, with ablations validating the contribution of each uncertainty signal.
...
TL;DR: JudgeBoard is a novel evaluation pipeline that directly queries models to assess answer correctness without requiring answer comparisons. It introduces MAJ (Multi-Agent Judging), a multi-agent framework that uses multiple interacting SLMs to approximate LLM-level judgment accuracy through collaborative deliberation.


<details>
  <summary>Details</summary>
Motivation: Small language models (SLMs) show promise on reasoning tasks but their ability to judge answer correctness remains unclear compared to LLMs. Existing LLM-as-a-judge frameworks rely on indirect answer comparisons and are difficult to fully automate, offering limited support for fine-grained evaluation.

Method: Proposed JudgeBoard pipeline that directly queries models to assess correctness without extra answer comparisons. Focused on mathematical and science/commonsense reasoning domains. Developed MAJ framework using multiple interacting SLMs with distinct reasoning profiles for collaborative deliberation.

Result: Significant performance gap between SLMs and LLMs in isolated judging tasks. MAJ framework substantially improves reliability and consistency of SLMs. On MATH dataset, MAJ using smaller-sized models performs comparably or better than larger counterparts.
...
TL;DR: KRAL is a novel paradigm that enhances LLMs for clinical antimicrobial therapy by combining knowledge distillation, semi-supervised learning, and agentic reinforcement learning, achieving significant improvements in accuracy and reasoning at reduced costs.


<details>
  <summary>Details</summary>
Motivation: Address limitations of LLMs in clinical decision-making including knowledge gaps, privacy concerns, high costs, and limited reasoning capabilities in complex medical scenarios.
...
Result: Outperforms RAG and SFT methods: Accuracy@1 on MEDQA increased by 1.8% vs. SFT and 3.6% vs. RAG; Pass@1 on PUMCH Antimicrobial increased by 27% vs. SFT and 27.2% vs. RAG, achieved at ~20% of SFT's training costs.
...
TL;DR: A dual-method detection system combining semantic drift analysis and canary baseline comparison achieves 92.5% accuracy in identifying backdoored LLMs (sleeper agents) in real-time without model modification.


<details>
  <summary>Details</summary>
Motivation: LLMs can be backdoored to exhibit malicious behavior while appearing safe during training, and existing safety training fails to remove these backdoors with no practical detection methods available.
...
Result: Achieved 92.5% accuracy with 100% precision (zero false positives) and 85% recall on the Cadenza-Labs dolphin-llama3-8B sleeper agent model, with detection in <1s per query.
...
TL;DR: LLMs struggle with clinical reasoning even when provided correct evidence, requiring rigorous evaluation of reasoning quality alongside retrieval accuracy.
...
Method: Proposed an evaluation framework measuring accuracy, consistency, and fidelity of reasoning, using clinician-vetted questions and authoritative passages to test model responses.

Result: Found that errors persist even with correct evidence, though retrieval-augmented generation can constrain outputs, highlighting both potential and risks.

Conclusion: Safe deployment of LLMs in clinical settings requires assessing reasoning quality as rigorously as retrieval accuracy to ensure proper alignment with structured protocols.
...
TL;DR: Sensorium Arc is a real-time multimodal AI system that personifies the ocean as a poetic speaker, enabling natural conversations that blend scientific data with ecological poetics while triggering dynamic visualizations based on dialogue cues.


<details>
  <summary>Details</summary>
Motivation: To reimagine ocean data not as abstract datasets but as living narratives, making complex marine data more accessible through affective and intuitive interactions inspired by eco-aesthetic philosophy.

Method: Built on a modular multi-agent system and retrieval-augmented LLM framework, using keyword detection and semantic parsing to dynamically trigger data visualizations and audiovisual playback based on time, location, and thematic dialogue cues.

Result: The system successfully enables natural spoken conversations with AI agents embodying the ocean's perspective, generating responses that combine scientific insight with ecological poetics.

Conclusion: Demonstrates the potential of conversational AI agents to mediate affective access to environmental data and proposes a new paradigm for human-machine-ecosystem interaction.
...
TL;DR: SpellForger is a game where players create custom spells using natural language prompts, with AI interpreting descriptions to generate balanced spell prefabs in real-time.


<details>
  <summary>Details</summary>
Motivation: To explore AI as a core gameplay co-creation tool for dynamic content generation and personalization, addressing the underexplored use of AI in direct gameplay mechanics.

Method: Uses a supervised-trained BERT model to interpret natural language prompts, mapping them to spell prefabs with balanced parameters (damage, cost, effects). Developed in Unity Game Engine with Python AI backend.
...
<details>
  <summary>Details</summary>
Motivation: Existing CP models for serial batch scheduling rely on predefined virtual batches that suffer from dimensionality issues and complexity. This is particularly problematic in practical settings like semiconductor manufacturing where minimum batch sizes are required.

Method: Proposes a CP model that uses key alignment parameters to reason directly on sequences of same-family jobs instead of virtual batches. Enhanced with tailored search phases and strengthened constraint propagator inference levels.

Result: Extensive experiments on nearly 5000 instances show superiority on small-to-medium instances (up to 100 jobs) and ability to find solutions up to 25% better than existing methods on large-scale instances (up to 500 jobs, 10 families, 10 machines).

Conclusion: The proposed CP model provides a more compact and effective formulation for serial batch scheduling with minimum batch sizes, significantly outperforming existing MIP, tabu search, and CP approaches across various instance sizes.
...
TL;DR: This paper proposes a framework to classify AI-accounting research along research focus and methodological dimensions, analyzes strategic positioning for accounting scholars, and examines how GenAI transforms research processes while creating both opportunities and competitive threats.
...
Method: The authors develop a classification framework with two dimensions: research focus (accounting-centric vs AI-centric) and methodological approach (AI-based vs traditional methods), then apply this framework to analyze papers from IJAIS special issue and leading accounting journals.
...
Conclusion: These shifts call for reforming doctoral education to cultivate comparative advantages while building AI fluency, as human researchers maintain value in areas requiring judgment, creativity, and theoretical depth despite AI's transformative impact.
...
TL;DR: A proactive cloud-edge resource management framework combining CNN-LSTM forecasting with multi-agent DRL for intelligent workload orchestration that anticipates future needs rather than reacting to problems.


<details>
  <summary>Details</summary>
Motivation: Traditional reactive resource management using static thresholds leads to either overspending on unused resources or performance degradation due to insufficient resources.

Method: Hybrid architecture embedding CNN-LSTM time series forecasts directly into multi-agent Deep Reinforcement Learning state space, enabling the AI manager to make long-term decisions based on future predictions.

Result: The system significantly outperforms traditional methods, effectively solving complex multi-objective optimization problems while balancing cost, performance, and reliability.

Conclusion: By giving the AI manager predictive capabilities to "see the future," the framework enables smooth, proactive resource management that avoids reactive problem-solving and finds optimal long-term paths.
...
TL;DR: SkyRL-Agent is a framework for efficient multi-turn agent training with asynchronous dispatching and tool integration. It trains SA-SWE-32B, a software engineering agent that achieves 39.4% Pass@1 on SWE-Bench with 2x cost reduction, and generalizes to other agentic tasks.


<details>
  <summary>Details</summary>
Motivation: To create an efficient framework for training long-horizon agents with seamless integration to existing RL frameworks, addressing the need for cost-effective and scalable agent training.

Method: Uses optimized asynchronous pipeline dispatcher (1.55x speedup), tool-enhanced training with AST-based search for code navigation, and reinforcement learning from Qwen3-32B base model.

Result: SA-SWE-32B achieves 39.4% Pass@1 on SWE-Bench Verified with >2x cost reduction compared to prior models. The agent also generalizes well to Terminal-Bench, BrowseComp-Plus, and WebArena despite being trained only on SWE tasks.

Conclusion: SkyRL-Agent provides an extensible framework for efficient agent training, demonstrating strong performance in software engineering and generalization to other domains through case studies on deep research, computer use, and memory agents.
...
TL;DR: MR-RML is a novel alignment framework that addresses LLM limitations in medical practice through structured medical standards integration, multi-dimensional reward modeling, and geometric projection constraints, achieving state-of-the-art performance on medical benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current LLMs face critical alignment challenges in medical applications: disconnect between static benchmarks and dynamic clinical needs, difficulty adapting to evolving medical standards, and inability of conventional reward models to capture nuanced medical quality criteria.

Method: Proposes MR-RML via GPRC framework with three innovations: (1) 'Dimensions-Scenarios-Disciplines' medical standard system embedded throughout training, (2) independent multi-dimensional reward model that decomposes evaluation criteria, (3) geometric projection reference constraints that transform medical logic into mathematical regularization.

Result: Achieves substantial performance gains over base Qwen-32B (45% on full subset, 85% on Hard subset), sets SOTA among open-source LLMs with scores of 62.7 (full) and 44.7 (hard) on Healthbench, and outperforms most closed-source models.

Conclusion: MR-RML successfully bridges the alignment gap for LLMs in medical applications through structured integration of medical standards and geometric constraints, demonstrating significant improvements in clinical reasoning and performance on authoritative medical benchmarks.
...
TL;DR: FOOTPASS is the first benchmark dataset for play-by-play action spotting in soccer that combines computer vision outputs with tactical knowledge to generate reliable play-by-play data.


<details>
  <summary>Details</summary>
Motivation: Current action recognition methods are insufficient for reliable play-by-play data extraction in soccer, and there's a need to leverage tactical knowledge as prior to support computer-vision predictions for more automated annotation.

Method: Multi-modal, multi-agent approach that integrates spatiotemporal action detection and multi-object tracking with tactical knowledge of soccer, including tactical regularities over long time horizons.

Result: Created FOOTPASS dataset - the first benchmark for play-by-play action spotting over entire soccer matches in a tactical context.

Conclusion: The dataset enables development of player-centric action spotting methods that can generate reliable play-by-play data streams essential for sports analytics.
...
TL;DR: The paper proposes combining LLM-driven algorithm design with explainable benchmarking to create interpretable, class-specific optimization algorithms rather than just performance-driven automated design.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based algorithm design is performance-driven but opaque - it doesn't reveal why algorithms work, which components matter, or how design choices relate to problem structures. The field needs to shift from blind search to interpretable design.
...
Result: Forms a closed knowledge loop where discovery, explanation, and generalization reinforce each other, enabling interpretable algorithm design.

Conclusion: This integration will shift automated algorithm design from blind search to interpretable, class-specific design, accelerating progress while producing reusable scientific insight into optimization strategy success conditions.
...
TL;DR: CRM replaces single reward models with a team of specialist evaluators for better robustness and interpretability in RLHF, using multi-agent collaboration to handle conflicting preferences and provide transparency.


<details>
  <summary>Details</summary>
Motivation: Conventional reward models struggle with optimizing multiple conflicting preference dimensions (factuality, helpfulness, safety) and lack transparency in scoring decisions, limiting RLHF effectiveness.

Method: Decomposes preference evaluation into domain-specific agents producing partial signals, with global evaluators and a centralized aggregator that fuses signals using factors like step-wise correctness, multi-agent agreement, and repetition penalties.
...
Conclusion: CRM and rewardBench provide a practical, modular approach for more transparent reward modeling and stable optimization in RLHF systems.
...
<details>
  <summary>Details</summary>
Motivation: Chemistry has remained an open challenge for AI reasoning due to its unique multimodal symbolic language, while mathematics and physics have established Olympiad-level benchmarks.

Method: Two key innovations: Assessment-Equivalent Reformulation (AER) converts visual output problems into tractable formats, and Structured Visual Enhancement (SVE) disentangles visual perception from chemical reasoning. Also proposes ChemLabs, a hierarchical multi-agent framework with specialized agents for problem decomposition, perception, reasoning, and auditing.

Result: Combining SVE with the multi-agent system yields dramatic performance gains, achieving a score of 93.6/100, surpassing estimated human gold medal threshold and establishing new state-of-the-art in automated chemical problem-solving.

Conclusion: ChemO successfully addresses the challenge of automated chemical reasoning through innovative assessment methods and multi-agent collaboration, demonstrating that AI can achieve expert-level performance in complex chemical problem-solving.
...
<details>
  <summary>Details</summary>
Motivation: Existing instruction-tuning and RL datasets are costly to curate and often use synthetic samples that introduce hallucination and limited diversity, while abundant high-quality human-authored QA content in textbooks remains underexploited due to PDF transformation difficulties.
...
### [168] [Revisiting Fairness-aware Interactive Recommendation: Item Lifecycle as a Control Knob](https://arxiv.org/abs/2511.16248)
...
<details>
  <summary>Details</summary>
Motivation: To address fairness in interactive recommendation by leveraging the compressed three-phase lifecycle pattern of items in short-video platforms, which differs from classical models.

Method: Proposes LHRL framework with PhaseFormer (STL decomposition + attention for phase detection) and two-level HRL agent (high-level imposes phase-aware fairness constraints, low-level optimizes user engagement).

Result: LHRL significantly improves both fairness and user engagement on multiple real-world datasets, and integrating lifecycle-aware rewards into existing RL models consistently yields performance gains.

Conclusion: The lifecycle-aware approach effectively reconciles long-term equity and short-term utility, demonstrating generalizability and practical value for fair interactive recommendation systems.
...
### [169] [MuISQA: Multi-Intent Retrieval-Augmented Generation for Scientific Question Answering](https://arxiv.org/abs/2511.16283)
*Zhiyuan Li,Haisheng Yu,Guangchuan Guo,Nan Zhou,Jiajun Zhang*

Main category: cs.AI

TL;DR: The paper introduces MuISQA benchmark to evaluate RAG systems on multi-intent scientific questions and proposes an intent-aware retrieval framework using LLMs to decompose questions, retrieve evidence for each intent, and aggregate results via RRF.
...
Result: The method outperforms conventional approaches on both MuISQA benchmark and general RAG datasets, particularly in retrieval accuracy and evidence coverage.

Conclusion: The proposed intent-aware retrieval framework effectively addresses the limitations of single-intent RAG systems by providing better evidence coverage for multi-intent scientific questions.
...
### [170] [Distributed Agent Reasoning Across Independent Systems With Strict Data Locality](https://arxiv.org/abs/2511.16292)
...
Method: Uses Orpius platform for multi-agent orchestration with OperationRelay calls for natural-language communication. Agents (Clinic, Insurer, Specialist Network) operate on local data only, using HMAC-based pseudonymous tokens for identification while preserving patient anonymity.

Result: Successfully demonstrated feasibility of distributed reasoning among specialized agents while keeping data local to each organization. The system enables secure cooperation through pseudonymised case tokens and controlled operational boundaries.

Conclusion: The prototype highlights architectural patterns and privacy considerations for decentralized multi-agent systems, outlining opportunities for more rigorous evaluation and future research in this domain.
...
<details>
  <summary>Details</summary>
Motivation: The lack of transparent and reproducible data curation and training strategies in multimodal reasoning research creates barriers to scalable advancement.
...
Conclusion: The work establishes a solid empirical foundation for large-scale multimodal reasoning research by providing fully transparent training recipes and open-sourcing all components (code, pipeline, data).
...
### [172] [Reducing Instability in Synthetic Data Evaluation with a Super-Metric in MalDataGen](https://arxiv.org/abs/2511.16373)
*Anna Luiza Gomes da Silva,Diego Kreutz,Angelo Diniz,Rodrigo Mansilha,Celso Nobre da Fonseca*

Main category: cs.AI

TL;DR: This paper introduces a Super-Metric for evaluating synthetic Android malware data quality by aggregating eight metrics across four fidelity dimensions into a single weighted score, showing improved stability and correlation with classifier performance compared to traditional metrics.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of synthetic Android malware data faces challenges due to metric instability and lack of standardization, making quality assessment difficult.
...
Conclusion: The proposed Super-Metric provides a more reliable and standardized approach for evaluating synthetic Android malware data quality.
...
### [173] [An Agent-Based Framework for the Automatic Validation of Mathematical Optimization Models](https://arxiv.org/abs/2511.16383)
*Alexander Zadorojniy,Segev Wasserkrug,Eitan Farchi*

Main category: cs.AI

TL;DR: Proposes an agent-based method for automatic validation of optimization models generated by LLMs, extending software testing techniques to optimization modeling with multiple agents handling API generation, test creation, and mutation testing.
...
Method: Uses multiple agents: one generates problem-level testing API, another generates tests using this API, and a third generates optimization model-specific mutations to assess test suite fault detection power.

Result: The framework demonstrates high quality validation as measured by mutation coverage, a well-known software testing metric.

Conclusion: The proposed agent-based validation framework effectively addresses the challenge of validating LLM-generated optimization models by adapting and extending software testing methodologies.
...
### [174] [CorrectHDL: Agentic HDL Design with LLMs Leveraging High-Level Synthesis as Reference](https://arxiv.org/abs/2511.16395)
*Kangwei Xu,Grace Li Zhang,Ulf Schlichtmann,Bing Li*

Main category: cs.AI

TL;DR: CorrectHDL is a framework that uses HLS results as functional references to correct errors in LLM-generated HDL designs, achieving better area/power efficiency than conventional HLS while maintaining functional correctness.


<details>
  <summary>Details</summary>
Motivation: LLMs show promise in hardware design but suffer from hallucination that introduces functional errors in generated HDL code, requiring a method to ensure correctness while leveraging LLM capabilities.

Method: Uses C/C++ program input, generates HDL via LLM, repairs syntax errors with RAG, then iteratively improves functional correctness by comparing simulation behavior with HLS reference design.
...
### [175] [Trustworthy AI in the Agentic Lakehouse: from Concurrency to Governance](https://arxiv.org/abs/2511.16402)
*Jacopo Tagliabue,Federico Bianchi,Ciro Greco*

Main category: cs.AI

TL;DR: The paper proposes Bauplan, an agent-first lakehouse design that solves infrastructure problems to enable trustworthy agentic workflows by implementing data and compute isolation.


<details>
  <summary>Details</summary>
Motivation: Most enterprises don't trust AI agents with production data due to infrastructure limitations - traditional lakehouses aren't suited for agent access patterns.

Method: Proposes Bauplan, an agent-first lakehouse design that reimplements data and compute isolation, drawing operational analogy to MVCC in databases but adapted for decoupled, multi-language settings.

Result: A reference implementation of a self-healing pipeline in Bauplan that seamlessly couples agent reasoning with guarantees for correctness and trust.

Conclusion: The path to trustworthy agentic workflows begins with solving infrastructure problems first - designing lakehouses around transactions enables governance to follow naturally.
...
### [176] [Pharos-ESG: A Framework for Multimodal Parsing, Contextual Narration, and Hierarchical Labeling of ESG Report](https://arxiv.org/abs/2511.16417)
*Yan Chen,Yu Zou,Jialei Zeng,Haoran You,Xiaorui Zhou,Aixi Zhong*

Main category: cs.AI

TL;DR: Pharos-ESG is a unified framework that transforms chaotic ESG reports into structured representations using multimodal parsing, contextual narration, and hierarchical labeling to address challenges in understanding ESG performance data.


<details>
  <summary>Details</summary>
Motivation: ESG reports present significant challenges for large-scale understanding due to chaotic reading order from irregular layouts and implicit hierarchies in lengthy, weakly structured content, making it difficult to assess corporate ESG performance effectively.

Method: The framework integrates reading-order modeling based on layout flow, hierarchy-aware segmentation guided by table-of-contents anchors, and multimodal aggregation that transforms visual elements into coherent natural language, enriched with ESG, GRI, and sentiment labels.
...
Conclusion: Pharos-ESG successfully transforms ESG reports into structured representations that better support ESG integration in financial governance and decision-making, providing a foundation for improved ESG performance assessment.
...
<details>
  <summary>Details</summary>
Motivation: Existing federated VLM adaptation methods incur high communication costs and are vulnerable to attacks due to iterative training. One-shot approaches are attractive but face challenges in exploiting multimodal information, handling data heterogeneity, and requiring additional training resources.

Method: TOFA uses visual and textual pipelines: hierarchical Bayesian model for personalized class prototypes in visual pipeline, and global alignment of local text prompts in textual pipeline. Adaptive weight calibration combines both modalities to balance personalization and robustness.

Result: Extensive experiments across 9 datasets in various federated settings demonstrate the effectiveness of TOFA method.

Conclusion: TOFA provides an efficient training-free solution for federated VLM adaptation that handles data heterogeneity while leveraging multimodal features without requiring additional training resources.
...
### [178] [From generative AI to the brain: five takeaways](https://arxiv.org/abs/2511.16432)
*Claudius Gros*

Main category: cs.AI

TL;DR: The paper argues that generative AI principles and ML research characterizations should be investigated for their relevance to brain function and cognitive neuroscience.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between machine learning advances and neuroscience by exploring how generative principles and ML characterizations might operate in the brain.

Method: The authors discuss five specific examples from ML research: shortcomings of world modelling, generation of thought processes, attention mechanisms, neural scaling laws, and quantization.

Result: The analysis suggests that neuroscience could potentially learn significant insights from ML research about neural information processing systems.

Conclusion: It is imperative to thoroughly investigate which generative principles from AI and ML characterizations are operative in the brain and relevant for cognitive neuroscience.
...
### [179] [PersonaDrift: A Benchmark for Temporal Anomaly Detection in Language-Based Dementia Monitoring](https://arxiv.org/abs/2511.16445)
*Joy Lai,Alex Mihailidis*

Main category: cs.AI

TL;DR: PersonaDrift is a synthetic benchmark for evaluating ML methods to detect progressive communication changes in dementia patients, simulating 60-day interaction logs with caregiver-informed personas and testing detection of flattened sentiment and semantic drift.


<details>
  <summary>Details</summary>
Motivation: Current computational tools don't track gradual communication changes in dementia patients, which caregivers notice informally but lack systematic monitoring tools.

Method: Created synthetic benchmark with 60-day interaction logs for dementia-like personas, injected progressive changes (flattened sentiment and semantic drift), and evaluated anomaly detection methods including statistical models, sequence models, and supervised classifiers.
...
### [180] [Utilizing Large Language Models for Zero-Shot Medical Ontology Extension from Clinical Notes](https://arxiv.org/abs/2511.16548)
...
<details>
  <summary>Details</summary>
Motivation: Clinical notes contain valuable medical insights but are underutilized for ontology extension due to their unstructured nature and privacy concerns. Existing methods don't effectively leverage this rich source of domain-specific knowledge.

Method: Uses pre-trained LLMs to extract disease-related concepts and hierarchical relationships from clinical notes in a zero-shot manner, automatically removes PHI for privacy protection, and integrates new concepts into existing medical ontologies without requiring additional training or labeled data.

Result: Experimental results show CLOZE provides accurate, scalable, and privacy-preserving ontology extension, effectively identifying medical entities and capturing complex hierarchical relationships from clinical notes.

Conclusion: CLOZE offers a cost-efficient solution for enhancing medical ontology coverage using clinical notes, with strong potential to support various downstream applications in biomedical research and clinical informatics.
...
### [181] [Consciousness in Artificial Intelligence? A Framework for Classifying Objections and Constraints](https://arxiv.org/abs/2511.16582)
*Andres Campero,Derek Shiller,Jaan Aru,Jonathan Simon*

Main category: cs.AI

TL;DR: A taxonomical framework for classifying challenges to digital AI consciousness, distinguishing levels of granularity and degrees of force, applied to 14 literature examples.


<details>
  <summary>Details</summary>
Motivation: To provide structure and disambiguation tools for distinguishing between challenges to computational functionalism and challenges to digital consciousness in AI systems.

Method: Developed a framework using Marr's levels of granularity and three degrees of force (challenges to functionalism, practical improbability, strict impossibility), applied to 14 examples from scientific and philosophical literature.

Result: Created a systematic classification system that helps disambiguate different types of challenges to digital consciousness at appropriate levels of analysis.

Conclusion: The framework provides valuable structure for parsing challenges to digital AI consciousness without taking sides in the debate, enabling clearer analysis of different argument types.
...
### [182] [Formal Abductive Latent Explanations for Prototype-Based Networks](https://arxiv.org/abs/2511.16588)
*Jules Soria,Zakaria Chihani,Julien Girard-Satabin,Alban Grastien,Romain Xu-Darme,Daniela Cancila*

Main category: cs.AI

TL;DR: This paper introduces Abductive Latent Explanations (ALEs) to address misleading explanations in case-based reasoning networks by providing formal guarantees about sufficient conditions for predictions.


<details>
  <summary>Details</summary>
Motivation: Case-based reasoning models are interpretable by design but their explanations can be misleading, which limits their usefulness in safety-critical applications where reliable explanations are crucial.

Method: Proposed ALEs formalism that expresses sufficient conditions on latent representations that imply predictions, combining case-based reasoning interpretability with formal XAI guarantees. Developed a solver-free, scalable algorithm using three distinct paradigms to generate these explanations.

Result: Demonstrated feasibility on diverse datasets for both standard and fine-grained image classification, showing that ALEs provide more reliable explanations than traditional case-based reasoning explanations.

Conclusion: ALEs successfully bridge the gap between inherent interpretability of case-based reasoning models and formal guarantees from XAI, providing more trustworthy explanations suitable for safety-critical contexts.
...
### [183] [D-GARA: A Dynamic Benchmarking Framework for GUI Agent Robustness in Real-World Anomalies](https://arxiv.org/abs/2511.16590)
...
Method: Proposes D-GARA framework with diverse real-world anomalies, constructs annotated benchmark with Android apps containing embedded anomalies, and conducts comprehensive experiments.
...
Conclusion: D-GARA provides a modular, extensible framework for evaluating GUI agent robustness and supports integration of new tasks and anomaly types for broader research.
...
Method: YOFO uses a template-conditioned approach where an autoregressive model accepts structured requirement templates and produces binary yes/no decisions for each requirement in a single forward pass by reading logits of the final token associated with each requirement.
...
Conclusion: YOFO successfully addresses the speed-accuracy trade-off in MLLM judging by enabling efficient single-pass requirement verification while preserving interpretability and supporting advanced analysis capabilities.
...
TL;DR: DPPO is a metacognitive training framework that alternates between supervised fine-tuning and reinforcement learning to maximize learning efficiency from sparse data, achieving 20.3% performance improvement over base models.


<details>
  <summary>Details</summary>
Motivation: Address the embodied data bottleneck (scarce real-world data) and algorithmic inefficiency of existing methods that are resource-prohibitive for developing universal embodied intelligence systems.
...
Result: Training Pelican-VL 1.0 with DPPO achieved 20.3% performance improvement over base model and surpassed open-source models at 100B-parameter scale by 10.6%.

Conclusion: DPPO provides the first systematic framework that alleviates data and resource bottlenecks, enabling efficient development of versatile embodied agents. Models and code are open-sourced.
...
TL;DR: MedBayes-Lite is a lightweight Bayesian enhancement for clinical transformers that adds uncertainty quantification without retraining, improving calibration and reducing overconfidence by 32-48% while preventing diagnostic errors through uncertainty flags.
...
Method: Integrates three components: Bayesian Embedding Calibration using Monte Carlo dropout for epistemic uncertainty, Uncertainty-Weighted Attention that marginalizes over token reliability, and Confidence-Guided Decision Shaping inspired by clinical risk minimization - all without retraining or architectural changes.
...
Conclusion: MedBayes-Lite effectively enables reliable uncertainty propagation and improves interpretability in medical AI systems while maintaining minimal computational overhead.
...
Method: Integration of holistic input features including fundamental macroeconomic variables (GDP, Unemployment Rate from Euro Zone and US) and technical variables (indicators, oscillators, Fibonacci levels, price divergences), evaluated using machine learning metrics and backtesting simulations.

Result: Performance evaluation using standard machine learning metrics for predictive accuracy and backtesting simulations across historical data to assess trading profitability and risk.
...
TL;DR: This paper analyzes how large language models (LLMs) reason differently from humans, revealing systematic structural differences and proposing cognitive guidance to improve model performance on complex problems.
...
Method: Developed a taxonomy of 28 cognitive elements from cognitive science, analyzed 170K reasoning traces from 17 models across text/vision/audio modalities and 54 human think-aloud traces, conducted meta-analysis of 1,598 LLM reasoning papers, and created test-time reasoning guidance.

Result: Revealed systematic differences: humans use hierarchical nesting and meta-cognitive monitoring while models rely on shallow forward chaining, especially on ill-structured problems. Research community focuses on easily quantifiable behaviors while neglecting meta-cognitive controls that correlate with success.

Conclusion: By bridging cognitive science and LLM research, this establishes a foundation for developing models that reason through principled cognitive mechanisms, opening new directions for improving model capabilities and testing human cognition theories at scale.
