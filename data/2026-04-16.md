...
Motivation: Need for reliable image quality assessment in automated image acquisition systems where pristine reference images are unavailable, requiring efficient no-reference methods to filter large volumes of images before analysis.

Method: Multi-metric framework combining interpretable distortion cues: blur, edge structure, low resolution artifacts, exposure imbalance, noise, haze, and frequency content. Uses Python/OpenCV implementation with limited intermediate storage (grayscale, filtered, frequency-domain representations).

Result: Achieved SRCC values of 0.647-0.830 on five benchmark datasets (KonIQ-10k, LIVE Challenge, KADID-10k, TID2013, BIQ2021). Consistent performance on synthetic agricultural dataset. Processing time ~1.97s per image with linear memory scaling relative to image size.

Conclusion: MM-IQA enables fast image quality screening with explicit distortion awareness and modest computational requirements, suitable for practical applications with large image volumes.
...
TL;DR: GPPU is a graph-based projection unlearning method that achieves 10-20x speedups for class-level unlearning across vision and audio models while preserving performance on retained classes.


<details>
  <summary>Details</summary>
Motivation: The need for selective and efficient information erasure from deep neural networks is growing due to privacy concerns, regulatory compliance requirements, and the need for adaptive system design.

Method: GPPU uses graph-based propagation to identify class-specific directions in feature space, projects representations onto orthogonal subspaces to remove target class information, and performs targeted fine-tuning to ensure irreversible removal.

Result: GPPU achieves highly efficient unlearning with 10-20x speedups over prior methods while maintaining model utility on retained classes, demonstrated across six vision datasets and two large-scale audio benchmarks with various architectures (CNNs, Vision Transformers, Audio Transformers).
...
TL;DR: PatchPoison is a dataset-poisoning method that prevents unauthorized 3D Gaussian Splatting reconstruction by adding small adversarial checkerboard patches to images, disrupting SfM camera pose estimation while remaining visually unobtrusive.


<details>
  <summary>Details</summary>
Motivation: The accessibility of 3D Gaussian Splatting (3DGS) raises privacy concerns as publicly available images/videos can be exploited to reconstruct detailed 3D models without consent. There's a need to protect multi-view data from unauthorized 3D reconstruction.

Method: PatchPoison injects small high-frequency adversarial patches (structured checkerboard patterns) into the periphery of each image in a multi-view dataset. These patches corrupt the feature-matching stage of Structure-from-Motion pipelines like COLMAP by introducing spurious correspondences that systematically misalign estimated camera poses, causing downstream 3DGS optimization to diverge from correct scene geometry.

Result: On the NeRF-Synthetic benchmark, inserting a 12x12 pixel patch increases reconstruction error by 6.8x in LPIPS metric, while the poisoned images remain unobtrusive to human viewers. The method requires no pipeline modifications and serves as a practical "drop-in" preprocessing step.

Conclusion: PatchPoison provides an effective, lightweight solution for content creators to protect their multi-view data from unauthorized 3D reconstruction, offering privacy protection without significant visual degradation or requiring system modifications.
...
TL;DR: 3DRealHead: Few-shot 3D head avatar reconstruction with novel expression control from monocular video, using Style U-Net and 3D Gaussians for higher expressivity beyond 3DMM limitations.


<details>
  <summary>Details</summary>
Motivation: Current 3D head avatar methods struggle to faithfully reproduce identity and facial expressions, especially for person-specific features like mouth/teeth. Limited training data and reliance on 3DMM-based expression control restrict expressivity.

Method: Few-shot inversion of a 3D human head prior represented as Style U-Net emitting 3D Gaussian primitives. Learned on NeRSemble dataset. Animation uses 3DMM facial expression signals plus mouth region features from driving video for higher expressivity.

Result: Enables reconstruction from few pictures and driving with consumer webcam. Recovers facial expressions not representable by 3DMM, achieving higher expressivity and closer resemblance to physical reality.

Conclusion: 3DRealHead addresses limitations of current avatar methods by combining few-shot reconstruction with enhanced expression control, enabling more faithful digital presence with person-specific details.
...
TL;DR: GeoLink: A 3D-aware semantic-consistent framework for generalizable cross-view geo-localization that uses 3D point clouds as structural priors to improve 2D representation learning and achieve better generalization across unseen domains.
...
Conclusion: GeoLink effectively addresses semantic inconsistency and poor generalization in cross-view geo-localization by leveraging 3D structural priors to enhance 2D representation learning, demonstrating strong performance and generalization capabilities.
...
TL;DR: First patient-specific non-rigid point cloud registration method for surgery using Transformer architecture with overlap estimation and physics-based registration, achieving 45% Matching Score and 92% Inlier Ratio on synthetic data.


<details>
  <summary>Details</summary>
Motivation: Unsafe surgical care due to limitations in surgeon experience and situational awareness; need for patient-specific 3D models to enhance visualization and reduce complications, but current registration methods struggle with organ deformations and noise.

Method: Patient-specific non-rigid point cloud registration combining Transformer encoder-decoder architecture with overlap estimation and matching module for dense correspondences, followed by physics-based registration algorithm.
...
Conclusion: The patient-specific registration method shows strong potential to improve surgical care by enabling reliable integration of patient-specific 3D models into surgical procedures despite organ deformations and noise.
...
TL;DR: Multitask embedding approach using ResNet-18 for automated blastocyst quality assessment from day-5 embryo images, predicting TE, ICM, and expansion grades.


<details>
  <summary>Details</summary>
Motivation: Current embryo grading relies on subjective visual assessment with inter-embryologist variability, making standardization difficult. Need for automated, objective blastocyst quality evaluation for improved IVF success rates.

Method: Multitask embedding-based approach using pretrained ResNet-18 enhanced with embedding layer to learn discriminative representations from limited dataset. Automatically identifies trophectoderm (TE), inner cell mass (ICM), and blastocyst expansion (EXP) regions and grades from day-5 human embryo images.

Result: Experimental results demonstrate promise of the multitask embedding approach for robust and consistent blastocyst quality assessment, effectively handling visually similar and difficult-to-distinguish TE and ICM structures.

Conclusion: The proposed multitask embedding approach shows potential for automated, objective blastocyst quality assessment that could reduce subjectivity and variability in IVF embryo selection, potentially improving treatment outcomes.
...
TL;DR: Neural height field approach for planetary surface reconstruction from descent imagery outperforms traditional multi-view stereo methods.


<details>
  <summary>Details</summary>
Motivation: Digital elevation modeling is crucial for planetary geology, but descent imagery presents challenges: strong radial distortion, limited parallax from nadir-facing cameras, and traditional MVS methods lack domain-specific priors.

Method: Developed a novel neural reconstruction approach with explicit neural height field representation, leveraging domain priors that planetary surfaces are continuous, smooth, solid, and free from floating objects.

Result: Experiments on simulated lunar and Mars descent sequences show increased spatial coverage while maintaining satisfactory estimation accuracy compared to traditional MVS.

Conclusion: Neural approaches offer a strong competitive alternative to traditional MVS for planetary descent imaging, with the proposed neural height field method providing better performance.
...
TL;DR: SemiFA is an AI agent framework that automates semiconductor failure analysis by generating structured reports from inspection images in under one minute using a multi-modal pipeline.


<details>
  <summary>Details</summary>
Motivation: Traditional semiconductor failure analysis is time-consuming, requiring experts to spend hours examining images, correlating equipment data, consulting historical records, and writing reports. This manual process creates bottlenecks in semiconductor manufacturing quality control.

Method: A four-agent LangGraph pipeline: 1) DefectDescriber (DINOv2 + LLaVA-1.6 for defect classification/morphology), 2) RootCauseAnalyzer (fuses SECS/GEM telemetry with historical defects from Qdrant vector DB), 3) SeverityClassifier (assigns severity/yield impact), 4) RecipeAdvisor (proposes process adjustments), plus a fifth node for PDF assembly. Uses SemiFA-930 dataset of 930 annotated defect images.

Result: DINOv2 classifier achieves 92.1% accuracy (macro F1 = 0.917) on 140 validation images. Full pipeline generates complete FA reports in 48 seconds on A100 GPU. Multi-modal fusion improves root cause reasoning by +0.86 points over image-only baseline, with equipment telemetry being the most critical modality.

Conclusion: SemiFA demonstrates the first integration of SECS/GEM equipment telemetry into vision-language models for autonomous semiconductor failure analysis, significantly reducing analysis time from hours to under one minute while maintaining high accuracy.
...
TL;DR: First implementation of concept-based Explainable AI (XAI) for Species Distribution Models using Robust TCAV to quantify landscape concept influence on predictions, demonstrated with aquatic insects using CNNs and Vision Transformers.


<details>
  <summary>Details</summary>
Motivation: Deep learning SDMs have become increasingly complex, making it difficult to extract ecological insights about driving factors of species distribution, which is essential for both robust predictions and ecological understanding.

Method: Proposed concept-based XAI using Robust TCAV methodology; created new open-access landscape concept dataset from high-resolution multispectral and LiDAR drone imagery (653 patches across 15 concepts + 1,450 random reference patches); applied to two aquatic insects using two CNNs and one Vision Transformer.
...
Conclusion: Concept-based XAI successfully reconciles predictive performance with ecological interpretability in SDMs, offering both validation against existing knowledge and discovery of new ecological relationships for conservation applications.
...
TL;DR: Workshop report summarizing the 5 benchmark challenges, evaluation protocols, datasets, results, and technical reports from top teams in the MaCVi 2026 maritime computer vision workshop at CVPR 2026.


<details>
  <summary>Details</summary>
Motivation: To advance maritime computer vision research by providing benchmark challenges that emphasize both predictive accuracy and real-time feasibility for embedded systems, and to analyze emerging trends in the field.

Method: Organized 5 benchmark challenges with specific evaluation protocols and datasets, collected submissions from participating teams, conducted quantitative and qualitative analyses of results, and compiled technical reports from top-performing teams.

Result: The workshop successfully conducted 5 benchmark challenges, produced quantitative results and qualitative comparisons, identified emerging method trends through cross-challenge analysis, and documented practical design choices from top teams.

Conclusion: The MaCVi 2026 workshop provided valuable benchmarks for maritime computer vision, highlighting the importance of balancing accuracy with real-time feasibility, and established resources (datasets, leaderboards) for continued research in the field.
...
TL;DR: Uncertainty estimates in medical image segmentation are rarely used for decisions; optimizing uncertainty alone misses safety gains. Best methods remove 80% of errors at 25% deferral, with calibration not translating to better decisions.


<details>
  <summary>Details</summary>
Motivation: Uncertainty estimates in medical image segmentation are often reported but rarely used to guide actionable decisions like accepting, flagging, or deferring predictions. There's a disconnect between standard uncertainty metrics and real-world utility.

Method: Formulate segmentation as two-stage pipeline (estimation then decision). Evaluate two uncertainty sources (Monte Carlo Dropout and Test-Time Augmentation) combined with three deferral strategies on retinal vessel segmentation benchmarks (DRIVE, STARE, CHASE_DB1). Introduce simple confidence-aware deferral rule prioritizing uncertain and low-confidence predictions.

Result: Best method/policy combination removes up to 80% of segmentation errors at only 25% pixel deferral, achieving strong cross-dataset robustness. Calibration improvements do not translate to better decision quality, highlighting disconnect between uncertainty metrics and utility.

Conclusion: Uncertainty should be evaluated based on the decisions it enables rather than in isolation. Optimizing uncertainty alone fails to capture most achievable safety gains in medical image segmentation.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs have strong cross-modal reasoning but their potential for vision-only tasks like image retrieval is underexplored. The paper investigates using MLLMs as training-free similarity estimators for instance-level image-to-image retrieval.

Method: Prompt MLLMs with paired images and convert next-token probabilities into similarity scores for zero-shot re-ranking. Combine with memory-efficient indexing and top-k candidate re-ranking for scalability. Avoid specialized architectures and fine-tuning.
...
Conclusion: MLLMs are a promising alternative for open-world large-scale image retrieval as training-free similarity estimators, though future work is needed to address limitations with severe appearance changes.
...
TL;DR: DroneScan-YOLO improves tiny object detection in UAV imagery through higher resolution, dynamic filter pruning, stride-4 detection branch, and hybrid loss function, achieving significant performance gains while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard YOLO detectors fail for aerial object detection due to: 8-pixel minimum stride making sub-32px objects nearly undetectable, CIoU loss producing zero gradients for non-overlapping tiny boxes, and architectural filter redundancy under computational constraints.

Method: Four coordinated design choices: (1) 1280x1280 input resolution for spatial detail, (2) RPA-Block dynamic filter pruning with lazy cosine-similarity updates and 10-epoch warm-up, (3) MSFD lightweight P2 detection branch at stride 4 (+114,592 parameters), (4) SAL-NWD hybrid loss combining Normalized Wasserstein Distance with size-adaptive CIoU weighting in YOLOv8's TaskAligned assignment.

Result: On VisDrone2019-DET: 55.3% mAP@50 and 35.6% mAP@50-95, outperforming YOLOv8s by +16.6 and +12.3 points respectively. Recall improved from 0.374 to 0.518, maintaining 96.7 FPS with only +4.1% parameters. Tiny object classes show dramatic improvements: bicycle AP@50 from 0.114 to 0.328 (+187%), awning-tricycle from 0.156 to 0.237 (+52%).

Conclusion: DroneScan-YOLO provides a holistic solution for aerial object detection that simultaneously addresses tiny object detection limitations, computational efficiency, and loss function issues, achieving state-of-the-art performance on UAV datasets while maintaining real-time inference speeds.
...
TL;DR: Lightweight LSTM framework with T-SHAP for skeleton-based fall detection achieves 94.3% accuracy with stable, clinically interpretable explanations by smoothing SHAP attributions over time windows.


<details>
  <summary>Details</summary>
Motivation: Existing post-hoc explainability methods produce temporally unstable attribution maps when applied frame-by-frame to sequential fall detection data, making them unreliable for clinical use where clinicians need consistent explanations they can trust.

Method: Combines efficient LSTM model with T-SHAP (temporally aware post-hoc aggregation strategy) that applies linear smoothing to SHAP-based feature attributions over contiguous time windows, preserving Shapley value guarantees while reducing high-frequency variance.
...
Conclusion: The framework satisfies real-time constraints on mid-range hardware and produces stable, clinically relevant explanations that align with established fall dynamics, supporting deployment as transparent decision aids in long-term care environments.
...
TL;DR: See&Say combines geometric safety analysis with semantic perception using Vision-Language Models for robust drone package delivery drop zone identification in cluttered environments.


<details>
  <summary>Details</summary>
Motivation: Existing drone delivery systems struggle with safe package drop-offs in cluttered urban/suburban environments because they rely on either geometry-based analysis or semantic segmentation alone, lacking integrated semantic reasoning for robust decision-making.

Method: Proposes See&Say framework that fuses monocular depth gradients with open-vocabulary detection masks to create safety maps, using a Vision-Language Model for iterative refinement of object category prompts and hazard detection across time, with capability to identify alternative drop zones when primary zones are unsafe.

Result: Outperforms all baselines with highest accuracy and IoU for safety map prediction, and superior performance in alternative drop zone evaluation across multiple thresholds on a curated dataset of urban delivery scenarios with moving objects and human activities.

Conclusion: Demonstrates the promise of VLM-guided segmentation-depth fusion for advancing safe and practical drone-based package delivery by enabling reliable reasoning under dynamic conditions during final delivery phase.
...
TL;DR: PAT-VCM is a plug-and-play framework for video coding for machines that uses shared baseline compression plus lightweight task-aware auxiliary tokens to support multiple downstream tasks without retraining separate codecs.


<details>
  <summary>Details</summary>
Motivation: Existing video coding for machines is tightly coupled to specific downstream tasks and models, making it difficult to scale across multiple tasks or adapt to model updates.

Method: PAT-VCM maintains a shared baseline compressed stream and augments it with three types of lightweight task-aware auxiliary tokens: visual residual tokens, prompt/control tokens, and semantic tokens, allowing different tasks to recover needed information.

Result: Evaluation on segmentation, depth estimation, and semantic recognition shows: shared detection-oriented auxiliary branch provides reusable first refinement; task-specific visual branches improve segmentation and depth; prompt tokens provide further segmentation gains with negligible bitrate; semantic tokens achieve strong recognition performance with extremely low overhead.

Conclusion: A shared compressed representation combined with lightweight task-aware auxiliary tokens is a practical and scalable alternative to tightly task-coupled VCM design.
...
TL;DR: CLTs (Cross-Layer Transcoders) are introduced as sparse, depth-aware proxy models for ViT MLP blocks that enable interpretable decomposition of final representations into layer-wise contributions.
...
Method: CLTs use encoder-decoder scheme to reconstruct post-MLP activations from sparse embeddings of preceding layers, creating linear decomposition of final representations into layer-resolved contributions.

Result: CLTs achieve high reconstruction fidelity while preserving CLIP zero-shot accuracy; cross-layer contribution scores provide faithful attribution showing final representations concentrate in few dominant layer-wise terms.

Conclusion: CLTs offer a reliable alternative interpretable proxy for ViTs, enabling process-level interpretability and faithful attribution through depth-aware, sparse decomposition of representations.
...
TL;DR: Reward models in text-to-image systems encode demographic biases that cause optimization to sexualize female subjects, reinforce stereotypes, and collapse diversity, challenging their reliability as quality metrics.


<details>
  <summary>Details</summary>
Motivation: While reward models are crucial for text-to-image generation systems (used for filtering, evaluation, optimization, and safety), their robustness and fairness as scoring functions remain largely unknown, particularly regarding demographic biases.
...
Conclusion: Current reward models have significant shortcomings, challenging their reliability as quality metrics, and highlighting the need for improved data collection and training procedures for more robust scoring.
...
TL;DR: Unsupervised HSI clustering framework combining masked autoencoder representation learning with diffusion-based clustering for improved accuracy.
...
Method: Two-stage approach: 1) Unsupervised masked autoencoder (UMAE) with Vision Transformer learns denoised latent representations using only small subset of pixels, 2) Entropy rate superpixel segmentation followed by spatially regularized diffusion graph construction in latent space.

Result: Proposed DS²DL algorithm demonstrates efficacy on Botswana and KSC datasets, showing improved labeling accuracy and clustering quality compared to baseline methods.

Conclusion: The framework successfully integrates deep representation learning with diffusion clustering, leveraging more faithful diffusion distances that better reflect data manifold geometry for superior HSI clustering performance.
...
TL;DR: Spectrascapes: First open-access multi-spectral terrestrial-view dataset for urban climate monitoring, combining RGB, NIR, and thermal imagery from bike-mounted sensors across diverse Dutch urban areas.


<details>
  <summary>Details</summary>
Motivation: Existing urban monitoring datasets have limitations: manual methods lack scalability, embedded sensing has inconsistent resolutions, remote sensing provides only overhead views, and standard street-view imagery lacks spectral information. There's a need for high-resolution spatial-temporal data for climate-resilient cities.

Method: Created a multi-spectral terrestrial-view dataset using bike-mounted sensors capturing RGB, Near-infrared, and Thermal imagery across diverse urban morphologies (village, town, small city, big urban area) in the Netherlands. Emphasized data calibration and quality, with detailed hardware/software methodology documentation.

Result: Produced Spectrascapes dataset with 17,718 street-level multi-spectral images, the first open-access dataset of its kind. Demonstrated two downstream use-cases and provided potential research directions for machine learning, urban planning, and remote sensing.

Conclusion: Spectrascapes addresses limitations of existing urban monitoring methods by providing calibrated, high-quality multi-spectral terrestrial-view data that enables new research in climate resilience, urban planning, and machine learning applications.
...
TL;DR: MLLMs struggle with 2D orientation tasks, but contrary to previous hypotheses, orientation information IS preserved in visual encoder embeddings and can be recovered with linear models, suggesting encoder limitations are not the cause of MLLM failures.


<details>
  <summary>Details</summary>
Motivation: Prior work suggests MLLMs fail at 2D orientation tasks due to visual encoder limitations, since encoders like CLIP/SigLIP are trained for semantic alignment rather than geometric reasoning. This paper aims to empirically test whether orientation information is actually lost in encoder representations.

Method: Designed controlled protocol to test orientation recoverability from encoder embeddings. Examined SigLIP and ViT features from LLaVA OneVision and Qwen2.5-VL models, and CLIP representations from LLaVA 1.5/1.6. Used linear regressors to predict object orientation from encoded features as test of null hypothesis that orientation information is not preserved.

Result: Contrary to hypothesis, orientation information IS recoverable from encoder representations - simple linear models accurately predict object orientations from embeddings. However, orientation information is spread diffusely across tens of thousands of features.

Conclusion: MLLM orientation failures do NOT originate in visual encoder limitations, as orientation information is preserved in embeddings. The real cause of MLLM failures remains unknown, though the diffuse distribution of orientation information across features may contribute to why MLLMs fail to exploit it.
...
TL;DR: Proposes RavelingArena benchmark to evaluate model robustness for asphalt raveling detection, showing data quantity and diversity are critical for real-world deployment.


<details>
  <summary>Details</summary>
Motivation: Current machine learning methods for raveling detection degrade in real-world deployments due to diverse conditions (different runs, sensors, environments), requiring more generalizable solutions.

Method: Created RavelingArena benchmark by augmenting existing dataset with controlled variations to evaluate model robustness; studied impact of training data quantity, illumination differences, and spatial shifts.

Result: Data quantity and diversity achieved at least 9.2% accuracy gain under diverse conditions; case study showed improved year-to-year consistency in Georgia test section.

Conclusion: Provides guidance for reliable model deployment in raveling detection and other real-world tasks requiring adaptability to diverse conditions, with implications for temporal deterioration modeling.
...
TL;DR: The paper introduces a diagnostic called "Flip" to quantify semantic label-flip errors in segmentation models, where models correctly identify foreground objects but assign wrong semantic labels due to spurious correlations.


<details>
  <summary>Details</summary>
Motivation: While robustness testing for spurious correlations is established in classification, segmentation-specific failure modes are not well understood. Models may achieve reasonable overlap metrics while swapping semantic labels between plausible foreground classes.

Method: Proposes a diagnostic (Flip) that counts how often ground truth foreground pixels are assigned wrong foreground identity while remaining predicted as foreground. Also introduces an entropy-based, label-free 'flip-risk' score computed from foreground identity uncertainty.

Result: Increasing correlation between category and scene during training widens the gap between common/rare test conditions and increases within-object label swaps on counterfactual groups. The flip-risk score can flag flip-prone cases at inference time.

Conclusion: Segmentation robustness should be assessed beyond overlap metrics by decomposing foreground errors into correct pixels, flipped-identity pixels, and missed-to-background pixels. The proposed diagnostics help understand and detect semantic label-flip failures.
...
Conclusion: SSD-GS achieves high-quality physically-based relighting with better fidelity and physical interpretability than previous approaches, enabling downstream tasks like controllable light source editing and interactive scene relighting.
...
<details>
  <summary>Details</summary>
Motivation: Prior approaches rely on utterance-level or manually specified emotion labels, which lack temporal granularity and continuous control over emotional expressions during speech-driven facial animation.

Method: Uses frame-level speech emotion diarization to predict dense emotion categories/intensities from speech, encodes these as learned embeddings, and conditions a speech-driven 3D animation model based on a hybrid Transformer-Mamba architecture for disentangling linguistic content and emotional style.

Result: Strong frame-level emotion recognition performance, low geometric/temporal reconstruction errors, smooth emotion transitions, and consistent expression control on multi-corpus datasets and EmoVOCA dataset.
...
### [27] [A 3D SAM-Based Progressive Prompting Framework for Multi-Task Segmentation of Radiotherapy-induced Normal Tissue Injuries in Limited-Data Settings](https://arxiv.org/abs/2604.13367)
*Caiwen Jiang,Lei Zeng,Wei Liu*

Main category: cs.CV

TL;DR: A 3D SAM-based progressive prompting framework for segmenting radiotherapy-induced normal tissue injuries using text, dose-guided box, and click prompts with small-target focus loss.


<details>
  <summary>Details</summary>
Motivation: Radiotherapy-induced normal tissue injury is clinically important but automatic segmentation remains challenging due to limited annotations and heterogeneity across injury types, lesion sizes, and imaging modalities.

Method: Proposed a 3D SAM-based progressive prompting framework with three complementary prompts: text prompts for task-aware adaptation, dose-guided box prompts for coarse localization, and click prompts for iterative refinement, plus a small-target focus loss for small lesions.
...
Conclusion: The proposed progressive prompting framework effectively addresses the challenges of limited data and heterogeneity in radiotherapy-induced normal tissue injury segmentation.
...
TL;DR: Multispectral 3D Gaussian Splatting extends 3DGS with spectral radiance representation and dual-loss optimization for wavelength-aware view synthesis with improved fidelity in challenging scenes.


<details>
  <summary>Details</summary>
Motivation: To enhance 3D Gaussian Splatting with multispectral capabilities for better view synthesis, particularly in scenes with translucent materials and anisotropic reflections where RGB-only approaches are limited.

Method: Augment each Gaussian with spectral radiance using per-band spherical harmonics, optimize with dual-loss supervision (RGB + multispectral signals), and perform spectral-to-RGB conversion at pixel level to retain richer spectral cues.

Result: Consistent improvements over RGB-only 3DGS baseline in image quality and spectral consistency on both public and self-captured datasets, especially excelling in challenging scenes with translucent materials and anisotropic reflections.

Conclusion: The approach maintains 3DGS's compactness and real-time efficiency while providing a foundation for future integration with physically based shading models, demonstrating effective multispectral extension to 3DGS.
...
TL;DR: DF3DV-1K is a large-scale real-world dataset with 1,048 scenes containing both clean and cluttered images for benchmarking distractor-free radiance field methods, addressing a critical gap in the field.


<details>
  <summary>Details</summary>
Motivation: Existing radiance field datasets lack large-scale real-world collections with both clean and cluttered images per scene, limiting development of distractor-free methods that need to handle real-world occlusions and clutter.

Method: Created DF3DV-1K dataset with 1,048 scenes, 89,924 images captured using consumer cameras, spanning 128 distractor types and 161 scene themes. Includes curated DF3DV-41 subset for challenging scenario evaluation. Used dataset to benchmark 9 distractor-free methods and 3D Gaussian Splatting, and fine-tuned diffusion-based 2D enhancer.

Result: Dataset enables comprehensive benchmarking, identifies most robust methods and challenging scenarios. Fine-tuned enhancer achieves average improvements of 0.96 dB PSNR and 0.057 LPIPS on held-out sets. Provides first large-scale benchmark for distractor-free radiance fields.

Conclusion: DF3DV-1K fills a critical gap in radiance field research by providing the first large-scale dataset with clean/cluttered image pairs, enabling systematic benchmarking and development of distractor-free methods beyond scene-specific approaches.
...
TL;DR: Edge-based object detection system using LLM-based natural language interface and multi-agent orchestration on a single Raspberry Pi platform, integrating YOLO vision agent with Slack chatbot and Ollama LLM reporting agents.


<details>
  <summary>Details</summary>
Motivation: To move beyond traditional edge computing approaches by leveraging LLM-based natural language interfaces for system control and demonstrating complete integration of multi-agent AI systems on resource-constrained hardware platforms.

Method: Multi-agent object detection framework with three agents: YOLO-based computer vision agent for real-time detection/tracking, Slack channel chatbot agent for natural language interface, and Ollama LLM reporting agent. All agents run locally on Raspberry Pi with event-based message exchange for orchestration.

Result: Successfully implemented a fully integrated edge-based object detection system on a single Raspberry Pi, demonstrating practical feasibility of centralized multi-agent AI systems on low-cost hardware while providing insights into platform limitations.

Conclusion: The approach demonstrates fast prototyping potential of generative AI systems for edge computing, showing that centralized multi-agent orchestration on resource-constrained hardware is feasible without cloud dependencies, though platform limitations must be considered.
...
### [31] [Event-Adaptive State Transition and Gated Fusion for RGB-Event Object Tracking](https://arxiv.org/abs/2604.13426)
*Jinlin You,Muyu Li,Xudong Zhao*

Main category: cs.CV

TL;DR: MambaTrack: A multimodal RGB-Event tracking framework using dynamic state space models with event-adaptive state transitions and gated projection fusion to handle event sparsity variations.


<details>
  <summary>Details</summary>
Motivation: Existing Vision Mamba-based RGB-Event tracking methods use static state transition matrices that fail to adapt to variations in event sparsity, leading to imbalanced modeling (underfitting sparse events and overfitting dense ones) and degraded cross-modal fusion robustness.

Method: Proposes MambaTrack with two key innovations: 1) Event-adaptive state transition mechanism that dynamically modulates state transition matrix based on event stream density using a learnable scalar, and 2) Gated Projection Fusion module that projects RGB features into event feature space and generates adaptive gates from event density and RGB confidence scores to control fusion intensity.
...
Conclusion: MambaTrack addresses limitations of static state transition matrices in RGB-Event tracking by introducing dynamic adaptation to event sparsity variations and robust cross-modal fusion, demonstrating superior performance and efficiency.
...
TL;DR: MaMe is a GPU-friendly, training-free token merging method using matrix operations to accelerate Vision Transformers, with MaRe for token restoration enabling an image synthesis pipeline.
...
Method: MaMe is a differentiable token merging method based entirely on matrix operations (GPU-friendly), with MaRe as its inverse operation for token restoration, forming a MaMe+MaRe pipeline for image synthesis tasks.

Result: MaMe doubles ViT-B throughput with 2% accuracy drop, boosts accuracy by 1.0% at 1.1x speed when fine-tuning last layer, accelerates SigLIP2-B@512 by 1.3x with negligible degradation, accelerates VideoMAE-L by 48.5% with 0.84% accuracy loss, and MaMe+MaRe reduces Stable Diffusion v2.1 latency by 31% while enhancing quality.

Conclusion: MaMe and MaRe effectively accelerate vision models across various tasks (classification, video, synthesis) through GPU-friendly matrix operations, achieving simultaneous performance and speed improvements in some cases.
...
### [33] [UniBlendNet: Unified Global, Multi-Scale, and Region-Adaptive Modeling for Ambient Lighting Normalization](https://arxiv.org/abs/2604.13383)
*Jiatao Dai,Wei Dong,Han Zhou,Chengzhou Tang,Jun Chen*

Main category: cs.CV

TL;DR: UniBlendNet is a unified framework for ambient lighting normalization that outperforms existing methods by jointly modeling global illumination, multi-scale structures, and region-adaptive refinement.


<details>
  <summary>Details</summary>
Motivation: Existing methods like IFBlend have limitations in global context modeling and spatial adaptivity, leading to suboptimal restoration in challenging lighting conditions. There's a need for better handling of complex, spatially varying illumination.
...
Result: Extensive experiments on NTIRE Ambient Lighting Normalization benchmark show UniBlendNet consistently outperforms baseline IFBlend, achieving improved restoration quality with more natural and stable visual results.

Conclusion: UniBlendNet effectively addresses limitations of existing methods by unifying global illumination modeling, multi-scale processing, and adaptive refinement, leading to superior ambient lighting normalization performance.
...
TL;DR: The paper presents a failure mode analysis study of two-stage HOI detection models, focusing on understanding model limitations in complex scenes rather than just improving benchmark performance.


<details>
  <summary>Details</summary>
Motivation: Current HOI detection evaluations focus mainly on overall accuracy but provide limited insight into model failure causes, especially in complex scenes with multiple people and rare interactions. High benchmark performance doesn't necessarily reflect robust visual reasoning about human-object relationships.

Method: Decomposes HOI detection into multiple interpretable perspectives and analyzes model behavior across these dimensions. Curates a subset of images from existing HOI datasets organized by human-object-interaction configurations (multi-person interactions, object sharing) to examine different failure modes.

Result: The study provides insights into how HOI models behave under different scene compositions and why their predictions fail, revealing limitations that aren't captured by overall benchmark performance metrics.

Conclusion: The analysis offers useful insights into HOI model limitations and provides observations for future research, emphasizing the need for more robust visual reasoning about human-object relationships beyond just benchmark performance improvements.
...
TL;DR: ESCAPE is an embodied AI system that combines episodic spatial memory with adaptive policy execution to improve long-horizon navigation and manipulation tasks in indoor environments, achieving state-of-the-art results on ALFRED benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing methods for embodied AI struggle with long-horizon tasks due to catastrophic forgetting, spatial inconsistency, and rigid execution when coordinating navigation and manipulation in complex indoor environments.

Method: ESCAPE uses a tightly coupled perception-grounding-execution workflow with: 1) Spatio-Temporal Fusion Mapping for depth-free persistent 3D spatial memory, 2) Memory-Driven Target Grounding for precise interaction masks, and 3) Adaptive Execution Policy that dynamically orchestrates proactive navigation and reactive manipulation.

Result: Achieves state-of-the-art performance on ALFRED benchmark with 65.09% success in test seen and 60.79% in unseen environments with step-by-step instructions. Maintains robust performance (61.24%/56.04%) even without detailed guidance for long-horizon tasks, while reducing redundant exploration.

Conclusion: ESCAPE's episodic spatial memory coupled with adaptive policy execution effectively addresses key challenges in long-horizon embodied AI tasks, demonstrating superior performance through its integrated perception-grounding-execution workflow.
...
### [36] [A Multimodal Clinically Informed Coarse-to-Fine Framework for Longitudinal CT Registration in Proton Therapy](https://arxiv.org/abs/2604.13397)
...
TL;DR: A deep learning framework for fast, clinically-informed deformable image registration in proton therapy that integrates multimodal clinical data (contours, dose, text) to handle anatomical changes across longitudinal CT scans.


<details>
  <summary>Details</summary>
Motivation: Proton therapy requires accurate DIR for adaptive workflows but conventional methods are too slow, and existing deep learning approaches don't utilize clinically relevant information beyond images, limiting their clinical applicability.

Method: Coarse-to-fine deformable registration framework with dual CNN encoders for hierarchical feature extraction and transformer decoder. Integrates multimodal clinical priors (target/OAR contours, dose distributions, planning text) via anatomy/risk-guided attention, text-conditioned feature modulation, and foreground-aware optimization.

Result: Evaluated on large-scale proton therapy DIR dataset (1,222 paired planning/repeat CT scans across multiple anatomical regions/disease types). Shows consistent improvements over state-of-the-art methods, enabling fast and robust clinically meaningful registration.

Conclusion: The proposed framework successfully addresses clinical scalability needs in proton therapy by integrating multimodal clinical information, achieving superior performance for adaptive radiotherapy workflows.
...
TL;DR: UniRect-CoT is a training-free framework that uses UMMs' own understanding capability to rectify generation through a chain-of-thought reflection process during diffusion denoising.


<details>
  <summary>Details</summary>
Motivation: Unified Multimodal Models have a capability mismatch where understanding significantly outperforms generation, leaving rich internal knowledge underactivated during generation.

Method: Proposes UniRect-CoT, a training-free chain-of-thought framework that treats diffusion denoising as visual reasoning and aligns intermediate results with target instructions as self-supervisory signals.
...
Conclusion: The approach successfully activates UMMs' internal knowledge for generation through reflection and rectification, bridging the understanding-generation gap without additional training.
...
### [38] [HiVLA: A Visual-Grounded-Centric Hierarchical Embodied Manipulation System](https://arxiv.org/abs/2604.14125)
...
TL;DR: HiVLA is a hierarchical VLA framework that decouples semantic planning (using VLMs) from motor control (using Diffusion Transformers) to preserve reasoning while improving manipulation.


<details>
  <summary>Details</summary>
Motivation: End-to-end VLA models face a fundamental trade-off: fine-tuning on control data compromises the reasoning capabilities inherited from base VLMs. There's a need to preserve VLM's zero-shot reasoning while enabling robust motor control.

Method: Hierarchical framework with two parts: 1) High-level VLM planner performs task decomposition and visual grounding to generate structured plans (subtask instruction + target bounding box). 2) Low-level flow-matching Diffusion Transformer action expert with cascaded cross-attention mechanism that sequentially fuses global context, high-resolution object-centric crops, and skill semantics.
...
Conclusion: The decoupled architecture successfully resolves the fundamental trade-off by preserving VLM's zero-shot reasoning while allowing independent improvement of both components, demonstrating superior performance in complex manipulation tasks.
...
### [39] [Why Multimodal In-Context Learning Lags Behind? Unveiling the Inner Mechanisms and Bottlenecks](https://arxiv.org/abs/2604.13403)
*Yu Wang,Sharon Li*

Main category: cs.CV

TL;DR: Multimodal ICL performs similarly to text-only ICL in zero-shot but degrades with few-shot demonstrations due to poor cross-modal alignment and unreliable task mapping transfer.


<details>
  <summary>Details</summary>
Motivation: Despite ICL's success in LLMs, its extension to multimodal settings remains poorly understood in terms of internal mechanisms and differences from text-only ICL.

Method: Systematic analysis of multimodal ICL using identical task formulations across modalities, decomposing it into task mapping construction and transfer, and analyzing cross-modal alignment across layers.

Result: Current models lack reasoning-level alignment between visual and textual representations and fail to reliably transfer learned task mappings to queries, leading to performance degradation in few-shot settings.

Conclusion: The analysis reveals mechanisms and limitations of multimodal ICL, proposes an inference-stage enhancement method to reinforce task mapping transfer, and suggests directions for more effective multimodal adaptation.
...
TL;DR: Training batch composition affects 3D medical vision-language models - balanced sampling hurts performance compared to random sampling, and performance scales sub-linearly with data size.


<details>
  <summary>Details</summary>
Motivation: To investigate how training batch composition (normal-to-abnormal ratio and data scaling) affects learned representations in 3D medical vision-language models, which remains unexplored despite their strong zero-shot diagnostic capabilities.

Method: Reproduced Merlin dual-encoder model aligning 3D abdominal CT volumes with radiology reports using symmetric InfoNCE loss. Conducted two experiments: 1) Controlled normal-to-abnormal ratio in training batches (25:75, 50:50, 75:25) using section-level balanced sampling; 2) Data scaling ablations on subset (20%, 40%, 100% of data) with and without balanced sampling.

Result: Achieved 74.45% zero-shot macro F1 (original: 73.00%). All balanced configurations underperformed baseline by 2.4-2.8 points (best balanced: 72.02%). Performance scaled sub-linearly from 65.26% to 71.88% with data scaling. Balanced sampling on subset further degraded performance to 68.01%.

Conclusion: Stochastic diversity of random sampling combined with anatomical subsection batching provides more effective regularization than engineered class ratios at small batch sizes required for 3D medical volumes. Explicit class balancing hurts performance regardless of dataset or balancing granularity.
...
### [41] [CausalDisenSeg: A Causality-Guided Disentanglement Framework with Counterfactual Reasoning for Robust Brain Tumor Segmentation Under Missing Modalities](https://arxiv.org/abs/2604.13409)
*Bo Liu,Yulong Zou,Jin Hong*

Main category: cs.CV

TL;DR: CausalDisenSeg is a causal disentanglement framework for robust brain tumor segmentation that addresses modality bias in incomplete MRI data through causal intervention and counterfactual reasoning.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for multimodal brain tumor segmentation suffer from robustness issues with incomplete MRI data due to modality bias, where models exploit spurious correlations rather than learning true anatomical structures.

Method: A three-stage causal intervention framework: 1) Explicit causal disentanglement using CVAE with HSIC constraint to separate anatomical and style features; 2) Causal representation reinforcement via Region Causality Module; 3) Counterfactual reasoning with dual-adversarial strategy to suppress bias effects.
...
Conclusion: CausalDisenSeg effectively addresses modality bias in incomplete MRI segmentation through causality-guided disentanglement and counterfactual reasoning, demonstrating superior robustness and generalizability compared to existing methods.
...
TL;DR: UHR-BAT is a query-guided token compression framework for ultra-high-resolution remote sensing that efficiently selects visual tokens under strict context budgets while preserving query-critical details.


<details>
  <summary>Details</summary>
Motivation: Ultra-high-resolution remote sensing imagery combines kilometer-scale context with tiny query-critical objects, causing quadratic token explosion that hinders small object extraction. Existing methods either lose critical details or have unpredictable computational costs.

Method: Proposes UHR-BAT with text-guided, multi-scale importance estimation for visual tokens, plus region-wise preserve and merge strategies to reduce token redundancy while maintaining query-critical information.

Result: Achieves state-of-the-art performance across various benchmarks while efficiently compressing tokens under strict computational budgets.

Conclusion: UHR-BAT effectively addresses the token explosion problem in UHR remote sensing through query-guided, region-faithful compression, enabling efficient extraction of small objects in vast spatial contexts.
...
### [43] [Physically-Guided Optical Inversion Enable Non-Contact Side-Channel Attack on Isolated Screens](https://arxiv.org/abs/2604.13419)
*Zhiwen Zheng,Yuheng Qiao,Xiaoshuai Zhang,Zhao Huang,Tao Zhang,Huiyu Zhou,Shaowei Jiang,Jin Liu,Wenwen Tang,Xingru Huang*

Main category: cs.CV

TL;DR: IR4Net: A neural network approach for reconstructing screen content from optical projection side-channels using passive speckle patterns, addressing instability and irreversibility challenges in projection mapping.


<details>
  <summary>Details</summary>
Motivation: Noncontact exfiltration of electronic screen content via side-channels is a security threat. Current optical projection side-channels face two core problems: (1) near-singular Jacobian spectrum causing hypersensitive inversion to perturbations, and (2) irreversible compression in light transport destroying global semantic cues and increasing reconstruction ambiguity.

Method: IR4Net uses passive speckle patterns from diffuse reflection. It combines: (1) Physically Regularized Irradiance Approximation (PRIrr-Approximation) that embeds radiative transfer equation in learnable optimizer, (2) contour-to-detail cross-scale reconstruction to prevent noise propagation, and (3) Irreversibility Constrained Semantic Reprojection (ICSR) module to restore lost global structure through context-driven semantic mapping.
...
Conclusion: The proposed IR4Net framework successfully addresses the stability and irreversibility challenges in optical projection side-channel attacks, enabling more robust and accurate screen content reconstruction from passive optical observations.
...
TL;DR: BVE is a 3D editing framework that uses a self-built large-scale dataset and lightweight trainable modules to enable text-guided 3D editing while preserving local invariance through annotation-free masking.


<details>
  <summary>Details</summary>
Motivation: Existing 3D editing methods have limitations: multi-view editing suffers from projection losses, voxel-based editing is constrained in modification regions and scale, and there's a lack of large editing datasets for training and evaluation.

Method: Proposes BVE framework with self-constructed large-scale dataset, enhances foundational image-to-3D architecture with lightweight trainable modules for efficient semantic injection, and introduces annotation-free 3D masking strategy to preserve local invariance.
...
Conclusion: BVE framework effectively addresses key challenges in 3D editing by combining dataset construction, efficient architecture modifications, and local invariance preservation techniques.
...
### [45] [VibeFlow: Versatile Video Chroma-Lux Editing through Self-Supervised Learning](https://arxiv.org/abs/2604.13425)
*Yifan Li,Pei Cheng,Bin Fu,Shuai Yang,Jiaying Liu*

Main category: cs.CV

TL;DR: VibeFlow is a self-supervised framework that leverages pre-trained video generation models to edit video color and illumination without expensive supervised training, enabling diverse applications like relighting and recoloring.


<details>
  <summary>Details</summary>
Motivation: Video chroma-lux editing (modifying illumination and color while preserving structure and temporal fidelity) is challenging. Existing methods require expensive supervised training with synthetic paired data, which is resource-intensive and limits generalization.

Method: VibeFlow uses a self-supervised approach that leverages pre-trained video generation models' intrinsic physical understanding. It introduces: 1) A disentangled data perturbation pipeline that adaptively recombines structure from source videos and color-illumination cues from reference images, 2) Residual Velocity Fields to rectify discretization errors in flow-based models, and 3) Structural Distortion Consistency Regularization for structural preservation and temporal coherence.

Result: The framework eliminates need for costly training resources and generalizes in zero-shot manner to diverse applications including video relighting, recoloring, low-light enhancement, day-night translation, and object-specific color editing. Extensive experiments show impressive visual quality with significantly reduced computational overhead.

Conclusion: VibeFlow provides an effective self-supervised solution for video chroma-lux editing that leverages pre-trained models' capabilities, eliminates expensive supervised training requirements, and enables robust generalization to various color and illumination editing tasks while maintaining structural and temporal fidelity.
...
### [46] [Med-CAM: Minimal Evidence for Explaining Medical Decision Making](https://arxiv.org/abs/2604.13695)
*Pirzada Suhail,Aditya Anand,Amit Sethi*

Main category: cs.CV

TL;DR: Med-CAM generates minimal, sharp evidence maps for medical AI decisions via Classifier Activation Matching, providing faithful, interpretable explanations unlike fuzzy Grad-CAM outputs.


<details>
  <summary>Details</summary>
Motivation: Medical AI systems are opaque black boxes lacking interpretability, which is critical for diagnostic decisions that directly impact patient care. Current explanation methods like Grad-CAM produce only fuzzy regions without conclusive evidence.

Method: Med-CAM trains a segmentation network from scratch to produce minimal evidence masks that highlight critical regions for model decisions. It uses Classifier Activation Matching to ensure explanations are faithful to network behavior, compact, and diagnostically aligned.

Result: Med-CAM produces superior spatial awareness to shapes, textures, and boundaries, delivering conclusive evidence-based explanations that faithfully replicate model predictions for any image, unlike prior methods.

Conclusion: By providing compact, consistent, and diagnostically aligned explanations, Med-CAM advances transparent AI to foster clinician understanding and trust in high-stakes medical applications like pathology and radiology.
...
### [47] [Gaslight, Gatekeep, V1-V3: Early Visual Cortex Alignment Shields Vision-Language Models from Sycophantic Manipulation](https://arxiv.org/abs/2604.13803)
...
<details>
  <summary>Details</summary>
Motivation: Vision-language models are deployed in high-stakes settings but their susceptibility to sycophantic manipulation is poorly understood. There's an open question about whether models with visual representations more aligned with human neural processing are more resistant to adversarial pressure, with implications for both neuroscience and AI safety.

Method: Evaluated 12 open-weight vision-language models across 6 architecture families (256M-10B parameters) on two axes: brain alignment (measured by predicting fMRI responses from Natural Scenes Dataset across 8 human subjects and 6 visual cortex regions) and sycophancy (measured through 76,800 two-turn gaslighting prompts spanning 5 categories and 10 difficulty levels).

Result: Alignment specifically in early visual cortex (V1-V3) is a reliable negative predictor of sycophancy (r = -0.441), with all 12 leave-one-out correlations negative. The strongest effect was for existence denial attacks (r = -0.597, p = 0.040). This anatomically specific relationship was absent in higher-order category-selective regions.

Conclusion: Faithful low-level visual encoding provides a measurable anchor against adversarial linguistic override in vision-language models. Models with better brain alignment in early visual cortex are more resistant to sycophantic manipulation, suggesting that biologically-inspired representations may enhance AI safety.
...
### [48] [Enhanced Text-to-Image Generation by Fine-grained Multimodal Reasoning](https://arxiv.org/abs/2604.13491)
...
<details>
  <summary>Details</summary>
Motivation: Current unified MLLMs have strong reasoning capabilities but underutilize them for text-to-image generation. Existing multimodal reasoning methods rely on holistic alignment judgments without fine-grained reflection on detailed prompt attributes, limiting precise control over generated images.

Method: FiMR decomposes input prompts into minimal semantic units (entities and attributes), verifies each unit through visual question answering (VQA) to generate explicit feedback, then applies targeted, localized refinements based on this fine-grained analysis.

Result: Extensive experiments show FiMR consistently outperforms image generation baselines, including reasoning-based methods, particularly on compositional text-to-image benchmarks, achieving better image-prompt alignment and overall generation quality.

Conclusion: FiMR demonstrates that fine-grained self-reasoning and self-refinement through decomposed VQA enables MLLMs to achieve more precise improvements in text-to-image generation, addressing limitations of holistic alignment approaches.
...
### [49] [Feed-Forward 3D Scene Modeling: A Problem-Driven Perspective](https://arxiv.org/abs/2604.14025)
...
TL;DR: Survey paper proposing a novel taxonomy for feed-forward 3D reconstruction methods based on model design strategies rather than output representations, organizing research into five key problems and reviewing benchmarks, datasets, and applications.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D reconstruction methods suffer from slow per-scene optimization or category-specific training limitations, hindering practical deployment. Feed-forward approaches enable efficient single-pass reconstruction with cross-scene generalization, but existing surveys focus on output representations rather than shared architectural patterns.

Method: Proposes a novel taxonomy centered on model design strategies agnostic to output format, organizing research into five key problems: feature enhancement, geometry awareness, model efficiency, augmentation strategies, and temporal-aware models. Includes comprehensive review of benchmarks/datasets and categorization of real-world applications.

Result: A unified framework for understanding feed-forward 3D reconstruction that abstracts away from representation differences (implicit fields vs explicit primitives) and focuses on shared architectural patterns like image feature extraction, multi-view fusion, and geometry-aware design principles.

Conclusion: The proposed taxonomy provides a more insightful organization of the field by focusing on model design rather than output representations. Future directions include addressing scalability, establishing better evaluation standards, and advancing world modeling capabilities.
...
### [50] [ADP-DiT: Text-Guided Diffusion Transformer for Brain Image Generation in Alzheimer's Disease Progression](https://arxiv.org/abs/2604.13495)
*Juneyong Lee,Geonwoo Baek,Ikbeom Jang*

Main category: cs.CV

TL;DR: ADP-DiT: A diffusion transformer model that generates personalized follow-up MRI scans for Alzheimer's disease patients by incorporating clinical metadata and follow-up intervals as text prompts.


<details>
  <summary>Details</summary>
Motivation: Alzheimer's disease progresses differently in each patient, creating a need for personalized MRI synthesis to track disease progression. Current methods lack fine-grained control over follow-up time and comprehensive clinical metadata integration.

Method: ADP-DiT uses a transformer-based diffusion model with dual text encoders (OpenCLIP and T5) to encode follow-up intervals and clinical metadata as natural language prompts. The model employs cross-attention for fine-grained guidance and adaptive layer normalization for global modulation, with rotary positional embeddings and SDXL-VAE latent space for anatomical fidelity.

Result: On 3,321 longitudinal MRI scans from 712 participants, ADP-DiT achieved SSIM 0.8739 and PSNR 29.32 dB, outperforming baseline DiT by +0.1087 SSIM and +6.08 dB PSNR, successfully capturing progression-related anatomical changes like ventricular enlargement and hippocampal shrinkage.

Conclusion: Integrating comprehensive subject-specific clinical conditions with advanced architectures significantly improves longitudinal Alzheimer's disease MRI synthesis, enabling more personalized and clinically interpretable progression assessment.
...
### [51] [Enhancing Mixture-of-Experts Specialization via Cluster-Aware Upcycling](https://arxiv.org/abs/2604.13508)
*Sanghyeok Chu,Pyunghwan Ahn,Gwangmo Song,SeungHwan Kim,Honglak Lee,Bohyung Han*

Main category: cs.CV

TL;DR: Cluster-aware Upcycling improves MoE initialization by partitioning dense model activations into semantic clusters and initializing each expert with cluster-specific subspace representations, breaking expert symmetry and enabling early specialization.


<details>
  <summary>Details</summary>
Motivation: Sparse Upcycling initializes MoE models from pretrained dense weights but suffers from expert symmetry (all experts start identical) and limited early specialization due to random router initialization, limiting model performance.

Method: 1) Partition dense model's input activations into semantic clusters; 2) Initialize each expert using subspace representations of its corresponding cluster via truncated SVD; 3) Set router's initial weights to cluster centroids; 4) Add expert-ensemble self-distillation loss for stable training with routing guidance.

Result: Outperforms existing methods on CLIP ViT-B/32 and ViT-B/16 across zero-shot and few-shot benchmarks; produces more diverse/disentangled expert representations; reduces inter-expert similarity; leads to more confident routing behavior.

Conclusion: Cluster-aware Upcycling effectively incorporates semantic structure into MoE initialization, breaking expert symmetry and enabling early specialization aligned with data distribution, resulting in improved performance and more meaningful expert representations.
...
### [52] [UI-Zoomer: Uncertainty-Driven Adaptive Zoom-In for GUI Grounding](https://arxiv.org/abs/2604.14113)
...
<details>
  <summary>Details</summary>
Motivation: Current test-time zoom-in methods for GUI grounding apply uniform cropping across all instances with fixed crop sizes, ignoring whether the model is actually uncertain on each case. This fails to address the challenge of localizing small icons and dense layouts effectively.
...
Conclusion: UI-Zoomer provides an effective training-free adaptive zoom-in framework that improves GUI grounding performance by selectively applying zoom-in based on prediction uncertainty, outperforming uniform zoom-in methods while requiring no additional training.
...
### [53] [DiT as Real-Time Rerenderer: Streaming Video Stylization with Autoregressive Diffusion Transformer](https://arxiv.org/abs/2604.13509)
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based video stylization methods struggle with stability and consistency in long videos, have high computational costs, and use multi-step denoising that makes them impractical for real-world applications.

Method: Proposes RTR-DiT framework: 1) Fine-tunes bidirectional teacher model on curated video stylization dataset for text/reference-guided tasks, 2) Distills into few-step autoregressive model using Self Forcing and Distribution Matching Distillation, 3) Implements reference-preserving KV cache update strategy for long video stability and real-time style switching.

Result: Outperforms existing methods in both text-guided and reference-guided video stylization tasks on quantitative metrics and visual quality, demonstrates excellent performance in real-time long video stylization and interactive style-switching applications.

Conclusion: RTR-DiT provides an effective solution for practical video stylization with stable long-video processing, real-time performance, and flexible style switching capabilities.
...
### [54] [Reconstruction of a 3D wireframe from a single line drawing via generative depth estimation](https://arxiv.org/abs/2604.13549)
*Elton Cao,Hod Lipson*

Main category: cs.CV

TL;DR: A generative approach using Latent Diffusion Model with ControlNet-style conditioning converts 2D sketches to 3D models via conditional dense depth estimation, enabling flexible "draw in 3D" workflow without CAD constraints.


<details>
  <summary>Details</summary>
Motivation: Traditional sketch-to-3D methods have limitations: symbolic logic approaches are brittle, while parametric modeling restricts users to predefined CAD primitives. There's a need to bridge human creativity with digital fabrication more flexibly.

Method: Frames reconstruction as conditional dense depth estimation using Latent Diffusion Model (LDM) with ControlNet-style conditioning to handle orthographic projection ambiguities. Introduces graph-based BFS masking for iterative "sketch-reconstruct-sketch" workflow.

Result: Trained and evaluated on over 1 million image-depth pairs from ABC Dataset. Demonstrates robust performance across varying shape complexities, providing scalable pipeline for converting sparse 2D line drawings to dense 3D representations.

Conclusion: The approach effectively allows users to "draw in 3D" without rigid CAD constraints, offering a generative solution to the sketch-to-3D reconstruction challenge that bridges human creativity with digital fabrication.
...
### [55] [AI Powered Image Analysis for Phishing Detection](https://arxiv.org/abs/2604.13555)
*K. Acharya,S. Ale,R. Kadel*

Main category: cs.CV

TL;DR: Deep learning approach using webpage screenshots for image-based phishing detection, comparing ConvNeXt-Tiny and ViT-Base models with threshold-aware evaluation.


<details>
  <summary>Details</summary>
Motivation: Phishing websites increasingly use visual imitation (logos, layouts, colors) to evade text- and URL-based detection systems, creating need for image-based detection methods.

Method: Dataset creation, preprocessing, transfer learning with ImageNet weights, evaluation using different decision thresholds. Two vision models tested: ConvNeXt-Tiny and Vision Transformer (ViT-Base).

Result: ConvNeXt-Tiny performs best overall with highest F1-score at optimized threshold and better computational efficiency than ViT-Base. Threshold tuning crucial for real-world deployment.

Conclusion: Convolutional models (ConvNeXt-Tiny) stronger for visual phishing detection than transformer-based models. Threshold-aware evaluation essential for practical deployment. Dataset will be released for reproducibility.
...
TL;DR: ZoomSpec: Physics-guided coarse-to-fine framework for wideband spectrum sensing that integrates signal processing priors with deep learning to overcome domain mismatch in existing approaches.
...
Method: Proposes ZoomSpec framework with three key components: 1) Log-Space STFT (LS-STFT) to overcome geometric bottleneck of linear spectrograms, 2) Coarse Proposal Net (CPN) for rapid full-band screening, 3) Adaptive Heterodyne Low-Pass (AHLP) module for center-frequency aligning and bandwidth-matched filtering, and 4) Fine Recognition Net (FRN) that fuses purified time-domain I/Q with spectral magnitude via dual-domain attention.

Result: Achieves state-of-the-art 78.1 mAP@0.5:0.95 on SpaceNet real-world dataset, surpassing existing leaderboard systems with superior stability across diverse modulation bandwidths.

Conclusion: ZoomSpec successfully bridges the gap between signal processing priors and deep learning for wideband spectrum sensing, demonstrating that physics-guided approaches can overcome domain mismatch and achieve superior performance in real-world monitoring scenarios.
...
TL;DR: RadarMOT improves 3D multi-object tracking by explicitly using radar point clouds to refine state estimation and recover detector misses, especially at long ranges and in adverse weather.
...
Method: Proposes RadarMOT framework that explicitly uses radar point cloud data as additional observation to refine state estimation and recover detector misses at long ranges.

Result: On MAN-TruckScenes dataset, RadarMOT improves AMOTA by absolute 12.7% at long range and 10.3% in adverse weather.

Conclusion: Explicit radar-informed approach significantly improves 3D MOT robustness in challenging real-world conditions compared to standard fusion methods.
...
TL;DR: SocialMirror is a diffusion-based framework that reconstructs interactive human meshes from monocular videos by integrating semantic and geometric cues to handle severe occlusions and motion ambiguities in close-interaction scenarios.


<details>
  <summary>Details</summary>
Motivation: Accurate human reconstruction in close-interaction scenarios is crucial for AR, sports analysis, and human-robot collaboration, but current methods struggle with severe mutual occlusions, motion ambiguities, and disrupted temporal continuity in monocular videos.

Method: A diffusion-based framework with two key components: 1) semantic-guided motion infiller using vision-language model descriptions to hallucinate occluded bodies and resolve pose ambiguities, and 2) sequence-level temporal refiner that enforces smooth motions with geometric constraints for plausible contact and spatial relationships.

Result: Achieves state-of-the-art performance on multiple interaction benchmarks, demonstrates strong generalization across unseen datasets and in-the-wild scenarios, and effectively handles severe occlusions and motion ambiguities.

Conclusion: SocialMirror successfully addresses the challenges of human reconstruction in close-interaction scenarios by integrating semantic and geometric cues, providing a robust solution for realistic virtual interactions and motion analysis applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing ViT-based multi-view 3D detection methods are computationally expensive, and current SOTA ToC3D has limitations: fixed token selection ratios and requires full backbone retraining.

Method: Introduces image token compensator with dynamic layer-wise token selection, plus parameter-efficient fine-tuning that trains only proposed modules (1.6M params vs 300M+).

Result: Reduces GFLOPs by 48-55%, inference latency by 9-25%, while improving mAP by 1.0-2.8% and NDS by 0.4-1.2% over ToC3D on NuScenes dataset.

Conclusion: Proposed method achieves significant efficiency gains while maintaining/improving accuracy, enabling more practical deployment of ViT-based multi-view 3D object detection.
...
TL;DR: Two-stage pipeline for multi-view smoke removal and novel view synthesis using generative dehazing followed by 3D Gaussian Splatting with physics-informed regularization to address cross-view inconsistencies.
...
Conclusion: MCMC-based densification with early stopping, combined with depth and haze-suppression priors, effectively mitigates artifacts from cross-view inconsistencies in dehaze-then-reconstruct pipelines for multi-view smoke removal and novel view synthesis.
...
TL;DR: VGGT-Segmentor (VGGT-S) is a new framework that combines geometric modeling with semantic segmentation for instance-level object segmentation across egocentric and exocentric views, achieving state-of-the-art results on Ego-Exo4D benchmark without requiring paired annotations.


<details>
  <summary>Details</summary>
Motivation: Instance-level object segmentation across disparate egocentric and exocentric views is critical for embodied AI and remote collaboration, but current geometry-aware models fail at dense prediction tasks due to pixel-level projection drift despite having consistent object-level attention.
...
Result: On the Ego-Exo4D benchmark, VGGT-S achieves 67.7% average IoU for Ego to Exo and 68.0% for Exo to Ego tasks, setting new state-of-the-art results. The correspondence-free pretrained model surpasses most fully-supervised baselines.

Conclusion: VGGT-S effectively bridges the gap between geometric modeling and pixel-accurate segmentation, demonstrating strong generalization capabilities and scalability through its self-supervised training approach.
...
TL;DR: Supervised dataset classification overstates semantic bias by detecting resolution artifacts, not true semantic differences; unsupervised clustering shows real separability is near chance.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the fundamental assumption in computer vision that high dataset classification accuracy indicates meaningful semantic differences. The authors argue that current methods fail to account for low-level resolution artifacts that create dataset-specific signatures, leading to systematic overestimation of semantic bias.

Method: The authors first demonstrate the flaw through controlled experiments showing models achieve strong dataset classification on non-semantic, procedurally generated images. They then introduce an unsupervised approach that measures true semantic separability by clustering semantically-rich features from foundational vision models, deliberately bypassing supervised classification on dataset labels.
...
Conclusion: The fundamental assumption that supervised dataset classification measures semantic differences is flawed due to resolution-based artifacts. The proposed unsupervised clustering approach provides a more accurate assessment of true semantic separability, showing that perceived dataset bias is often exaggerated.
...
TL;DR: VRAG-DFD is a novel MLLM-based deepfake detection framework that combines Retrieval-Augmented Generation (RAG) and Reinforcement Learning (RL) to provide dynamic forgery knowledge retrieval and critical reasoning capabilities, achieving state-of-the-art performance on generalization testing.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based deepfake detection methods suffer from lack of professional forgery knowledge, either relying on complementary combination with small detectors or static knowledge injection. This limitation hinders their performance in DFD tasks.

Method: Proposes VRAG-DFD framework with: 1) Two RAG-based datasets - Forensic Knowledge Database (FKD) for DFD knowledge annotation and Forensic Chain-of-Thought Dataset (F-CoT) for critical reasoning construction; 2) Three-stage training (Alignment->SFT->GRPO) to cultivate critical reasoning ability; 3) Combination of RAG for dynamic knowledge retrieval and RL for reasoning enhancement.

Result: VRAG-DFD achieved state-of-the-art and competitive performance on deepfake detection generalization testing, demonstrating superior capabilities compared to existing MLLM-based methods.

Conclusion: The framework successfully addresses two key challenges in DFD-MLLMs: providing high-quality dynamic forgery knowledge through RAG and endowing MLLMs with critical reasoning abilities through RL, resulting in improved deepfake detection performance.
...
<details>
  <summary>Details</summary>
Motivation: Video storage in DNA remains an open challenge because existing approaches treat compression and molecular encoding as independent stages, leaving biochemical constraints and compression objectives fundamentally misaligned. The two fields have evolved independently, requiring co-design from the ground up.

Method: HELIX introduces TK-SCONE (Token-Kronecker Structured Constraint-Optimized Neural Encoding), which uses token-based representations that naturally align with DNA's quaternary alphabet. It employs Kronecker-structured mixing to break spatial correlations and FSM-based mapping to guarantee biochemical constraints, learning token distributions simultaneously optimized for visual quality, prediction under masking, and DNA synthesis efficiency.

Result: The system achieves 1.91 bits per nucleotide, demonstrating that learned compression and molecular storage converge naturally at token representations.

Conclusion: This work establishes a new paradigm where neural video codecs can be designed for biological substrates from the ground up, showing that joint optimization of compression and DNA encoding is both feasible and effective for video storage.
...
<details>
  <summary>Details</summary>
Motivation: Existing adaptation methods for MLLMs (like full fine-tuning and LoRA) disrupt pre-trained semantic spaces and impair structured knowledge essential for reasoning. The paper argues adaptation should focus on eliciting pre-trained representations rather than overwriting them.

Method: Proposes SLQ framework that adapts frozen MLLMs through a small set of Shared Latent Queries appended to text and image token sequences. These queries leverage the model's native causal attention as global aggregation interfaces, producing compact embeddings in unified space while keeping backbone unchanged.

Result: SLQ outperforms full fine-tuning and LoRA on COCO and Flickr30K, achieves competitive performance on MMEB, and yields substantial gains on KARR-Bench (knowledge-aware reasoning retrieval benchmark).

Conclusion: SLQ provides an effective and efficient framework for adapting MLLMs to retrieval by preserving pre-trained representations, demonstrating that eliciting rather than overwriting representations is key for retrieval adaptation.
...
TL;DR: MGTD dataset and granularity-aware distillation method for synthetic-to-real transfer in forestry perception with coarse real labels and fine-grained synthetic annotations.
...
Method: Introduce MGTD dataset (53k synthetic + 3.6k real images) with four-stage protocol, and granularity-aware distillation that transfers structural priors from fine-grained synthetic teachers to coarse-label student via logit-space merging and mask unification.
...
Conclusion: The proposed approach effectively addresses domain shift and granularity mismatch in forestry perception, providing a framework for synthetic-to-real transfer with mixed-granularity labels.
...
TL;DR: First continual learning framework for text-to-3D generation that prevents catastrophic forgetting through replay memory and introduces a new benchmark for evaluation.


<details>
  <summary>Details</summary>
Motivation: Continual learning for text-to-3D generation is unexplored, and existing models suffer from catastrophic forgetting when incrementally trained on new 3D categories.

Method: ReConText3D uses text-embedding k-Center selection to build compact replay memory for representative rehearsal, enabling incremental learning without architectural changes.

Result: Outperforms all baselines on Toys4K-CL benchmark, maintaining high-quality generation for both old and new classes across different generative backbones.

Conclusion: Establishes first continual learning framework and benchmark for text-to-3D generation, opening new direction for incremental 3D generative modeling.
...
TL;DR: ClipGStream: A hybrid framework for dynamic 3D scene reconstruction that combines clip-level optimization with stream processing for scalable, flicker-free reconstruction of long multi-view videos.


<details>
  <summary>Details</summary>
Motivation: Dynamic 3D scene reconstruction is essential for immersive media (VR/MR/XR) but challenging for long sequences with large-scale motion. Existing approaches either sacrifice temporal stability (Frame-Stream) or have high memory costs and limited sequence length (Clip).

Method: Hybrid Clip-Stream framework: divides sequence into short clips, uses clip-independent spatio-temporal fields with residual anchor compensation for local variations, while inter-clip inherited anchors and decoders maintain structural consistency across clips.
...
Conclusion: ClipGStream successfully bridges the gap between scalability and temporal stability in dynamic 3D scene reconstruction, offering a practical solution for long multi-view sequences with large-scale motion.
...
TL;DR: Investigates patch-wise sparse Mixture-of-Experts layers for semantic segmentation in CNNs, showing architecture-dependent improvements with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Sparse MoE layers are widely used in transformers but their integration into CNNs remains inconsistent, with most prior work focusing on fine-grained MoEs at filter/channel levels rather than coarser patch-level formulations for dense prediction tasks like semantic segmentation.

Method: Proposes a coarser, patch-wise formulation of sparse MoE layers for semantic segmentation where local regions are routed to a small subset of convolutional experts. Conducts design analysis on Cityscapes and BDD100K datasets using encoder-decoder and backbone-based CNNs to assess architectural choices affecting routing dynamics and expert specialization.

Result: Demonstrates consistent, architecture-dependent improvements (up to +3.9 mIoU) with little computational overhead, while revealing strong design sensitivity in how architectural choices affect routing dynamics and expert specialization.

Conclusion: Provides empirical insights into the design and internal dynamics of sparse MoE layers in CNN-based dense prediction, showing that patch-wise MoE formulations can effectively improve semantic segmentation performance with minimal computational cost.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D-SOT methods have limited temporal capacity due to severe temporal feature inconsistency and excessive memory overhead, restricting them to short-term context. There's a need for robust long-term tracking that can maintain feature consistency while efficiently aggregating diverse target features.

Method: Proposes ChronoTrack with compact learnable memory tokens and two complementary objectives: 1) temporal consistency loss for feature alignment across frames to reduce temporal drift, and 2) memory cycle consistency loss that encourages tokens to encode diverse target representations via memory-point-memory cyclic walks.

Result: Achieves state-of-the-art performance on multiple 3D-SOT benchmarks while running at real-time speed (42 FPS on RTX 4090 GPU), demonstrating effective long-term target modeling with compact memory.

Conclusion: ChronoTrack successfully addresses the fundamental limitations of existing 3D-SOT methods by providing robust long-term tracking through temporal feature consistency and efficient memory aggregation, enabling real-time performance with superior accuracy.
...
TL;DR: PBE-UNet: Progressive Boundary Enhanced U-Net for ultrasound lesion segmentation with scale-aware aggregation and boundary-guided feature enhancement modules to handle scale variations and indistinct boundaries.


<details>
  <summary>Details</summary>
Motivation: Accurate lesion segmentation in ultrasound images is challenging due to low contrast, blurry boundaries, and significant scale variations. Existing deep learning methods struggle with scale variations and indistinct tumor boundaries.

Method: Proposed PBE-UNet with two key modules: 1) Scale-Aware Aggregation Module (SAAM) that dynamically adjusts receptive field to capture multi-scale contextual information, and 2) Boundary-Guided Feature Enhancement (BGFE) module that progressively expands narrow boundary predictions into broader spatial attention maps to cover wider segmentation error regions.

Result: Experiments on four benchmark ultrasound datasets (BUSI, Dataset B, TN3K, and BP) show that PBE-UNet outperforms state-of-the-art ultrasound image segmentation methods.

Conclusion: PBE-UNet effectively addresses scale variations and indistinct boundaries in ultrasound lesion segmentation through progressive boundary enhancement and scale-aware feature aggregation, achieving superior performance on multiple datasets.
...
TL;DR: Syn2Seq-Forcing reformulates Exo-to-Ego video generation as sequential signal modeling by interpolating source and target videos into a continuous sequence, enabling diffusion models to better handle spatio-temporal discontinuities.


<details>
  <summary>Details</summary>
Motivation: Synchronized exo-ego video data introduces substantial spatio-temporal and geometric discontinuities that violate smooth-motion assumptions of standard video generation, making conventional condition-output approaches ineffective for this task.
...
Result: Empirical results show that interpolating only the videos (without pose interpolation) produces significant improvements, indicating that spatio-temporal discontinuities are the dominant challenge. The method establishes a unified framework for both Exo2Ego and Ego2Exo generation.

Conclusion: Syn2Seq-Forcing provides a principled foundation for cross-view video synthesis by reframing the problem as sequential signal modeling, enabling better handling of synchronization-induced jumps and offering a flexible framework for future research.
...
<details>
  <summary>Details</summary>
Motivation: Vision transformers show promising feature detection capabilities for medical image analysis, but fully supervised training is impractical due to lack of expert resources for labeling large datasets.

Method: Used weakly supervised training where image patches are automatically labeled at the whole-slide-image level, trained a ViT model on 100,000 image patches for classifying anaplastic large cell lymphoma vs classic Hodgkin lymphoma.
...
Conclusion: Weakly supervised training of vision transformers provides a practical solution for clinical deep learning applications, balancing performance with feasibility of implementation using automated image patch extraction.
...
TL;DR: DRG-Font is a contrastive font generation method that decomposes glyph attributes into style and content embeddings, using dynamic reference selection and multi-scale fusion to generate high-quality fonts from few examples.


<details>
  <summary>Details</summary>
Motivation: Few-shot font generation struggles to capture complex font styles from limited examples and often fails to retain local characteristics in generated glyphs. Existing methods need improvement in style consistency and detail preservation.

Method: Uses contrastive learning with decomposed style/content embeddings. Features: 1) Reference Selection Module for optimal style reference, 2) Multi-scale Style/Content Head Blocks for attribute decomposition, 3) Multi-Fusion Upsampling Block to combine reference style with target content.

Result: Significant improvements over state-of-the-art methods across multiple visual and analytical benchmarks, demonstrating better style consistency and local characteristic preservation.

Conclusion: DRG-Font effectively addresses few-shot font generation challenges by decomposing glyph attributes and using dynamic reference selection, achieving superior performance in generating stylistically consistent glyphs with preserved local characteristics.
...
TL;DR: Lightweight hybrid CNN-LSTM system achieves 94.38% accuracy for bean leaf disease classification with 70% size reduction (1.86MB), outperforming traditional CNNs through spatial-sequential modeling and tailored image augmentation.


<details>
  <summary>Details</summary>
Motivation: Need for accurate yet resource-efficient automated diagnosis in agriculture; CNNs have limitations in capturing long-range spatial dependencies and high memory footprint hinders deployment on portable devices.

Method: Proposes lightweight hybrid CNN-LSTM system integrating LSTM layer to model spatial-sequential relationships within feature maps, with systematic evaluation of tailored image augmentation strategies.

Result: Achieves 94.38% accuracy with 1.86MB footprint (70% reduction), state-of-the-art F1 scores of 99.22% with EfficientNet-B7+LSTM on ibean dataset, demonstrating superiority of tailored augmentation over generic combinations.

Conclusion: The hybrid CNN-LSTM system provides robust, scalable framework for real-time agricultural decision support in resource-constrained environments, with code and datasets publicly available.
...
TL;DR: DiffMagicFace is a video editing framework that uses two fine-tuned diffusion models to edit facial videos while preserving identity and ensuring frame consistency, without needing video datasets.


<details>
  <summary>Details</summary>
Motivation: Extending text-conditioned image editing to facial video editing is challenging due to difficulties in preserving facial identity across frames and ensuring consistency of edited subjects throughout the video.
...
Result: Produces high-quality video edits with excellent consistency and content, even for complex tasks like talking head videos and distinguishing closely related categories. Results are comparable to traditional rendering software and outperform state-of-the-art methods in both visual appeal and quantitative metrics.

Conclusion: DiffMagicFace successfully addresses facial video editing challenges by maintaining identity preservation and frame consistency through concurrent model integration and specialized dataset creation, achieving superior performance without video dataset dependency.
...
Method: Three key components: 1) AnyHead unified data suite with identity diversity, dense multi-view supervision, and realistic accessories; 2) Initialization from Plücker-aware structured 3D Gaussian scaffold with one-step conditional denoising; 3) Auxiliary view-conditioned appearance supervision on latent tokens alongside 3D Gaussian reconstruction.

Result: Outperforms prior single-image full-head reconstruction methods in rendering fidelity while remaining substantially faster, with the fastest setting reconstructing a full head in under one second.

Conclusion: Any3DAvatar bridges the quality-speed gap in single-image 3D head reconstruction by combining comprehensive data, structured initialization, and auxiliary supervision to achieve both high fidelity and fast inference.
...
TL;DR: PostureObjectStitch: A novel image synthesis method for industrial anomaly detection that generates pose-accurate images of assembly components by decoupling multi-view features and using temporal modulation in diffusion models.


<details>
  <summary>Details</summary>
Motivation: Existing image generation techniques for industrial anomaly detection don't properly account for component pose and orientation in assemblies, making generated images difficult to use in downstream applications.

Method: 1) Condition decoupling separates multi-view images into high-frequency, texture, and RGB features; 2) Feature temporal modulation adapts features across diffusion time-steps for progressive generation; 3) Conditional loss enhances critical industrial elements; 4) Geometric prior guides component positioning for correct assembly relationships.

Result: Comprehensive experiments on MureCom dataset, new DreamAssembly dataset, and downstream applications validate outstanding performance in generating pose-accurate industrial assembly images.

Conclusion: PostureObjectStitch effectively solves the pose/orientation problem in industrial image generation, enabling accurate synthetic data for enhancing anomaly detection model performance in assembly applications.
...
TL;DR: Proposes context-sensitive similarity computation from neural embeddings to model human-like context-dependent perception, achieving 15% improvement in odd-one-out tasks.


<details>
  <summary>Details</summary>
Motivation: Current ML models use fixed embeddings while humans process information context-sensitively; need to bridge this gap between machine and human perception.

Method: Context-sensitive similarity computation from neural network embeddings, applied to modeling triplet odd-one-out tasks with anchor images as context.

Result: 15% improvement in odd-one-out accuracy over context-insensitive models, consistent across original and "human-aligned" vision foundation models.

Conclusion: Modeling context enables more human-like perception in ML models, demonstrating significant performance gains in context-dependent tasks.
...
TL;DR: SparseGen is an efficient image-to-3D generation framework using sparse 3D anchor queries and local Gaussian primitives, reducing memory/time while maintaining multi-view fidelity with less input-view bias.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D generation approaches using dense volumetric grids, triplanes, or pixel-aligned primitives are computationally expensive and suffer from input-view bias, where models overfit to conditioning views. There's a need for more efficient 3D representation that allocates capacity only where needed for geometry and appearance.

Method: Models scenes with a compact sparse set of learned 3D anchor queries and a learned expansion operator that decodes each transformed query into a small local set of 3D Gaussian primitives. Trained under a rectified-flow reconstruction objective without 3D supervision, enabling the model to learn where to allocate representation capacity.

Result: Achieves significant reductions in memory and inference time while preserving multi-view fidelity. Introduces quantitative measures showing sparse queries reduce overfitting to conditioning views while being representationally efficient. Demonstrates sparse set-latent expansion as a practical alternative for efficient 3D generative modeling.

Conclusion: Sparse set-latent expansion with learned anchor queries and local Gaussian primitives provides a principled, practical approach for efficient image-to-3D generation that reduces input-view bias while maintaining quality and computational efficiency.
...
TL;DR: M-GDM is a metadata-guided diffusion model for blind video recovery that automatically identifies and repairs corrupted regions without needing predefined masks, using motion vectors and frame types as corruption indicators.


<details>
  <summary>Details</summary>
Motivation: Existing video recovery methods require manually annotated masks of corrupted regions, which is labor-intensive and impractical for real-world applications where corruption patterns are unknown.

Method: Proposes Metadata-Guided Diffusion Model (M-GDM) with: 1) dual-stream metadata encoder for motion vectors and frame types, 2) prior-driven mask predictor for pseudo masks, 3) cross-attention between metadata and corrupted features, and 4) post-refinement module to reduce boundary artifacts.
...
Conclusion: M-GDM successfully addresses blind video recovery by leveraging intrinsic metadata as corruption indicators and diffusion priors, eliminating the need for predefined masks while achieving high-quality restoration.
...
TL;DR: PartNerFace: A part-based neural radiance fields approach for reconstructing animatable facial avatars from monocular RGB videos that better handles unseen expressions and fine-scale motions.


<details>
  <summary>Details</summary>
Motivation: Existing methods either use simple morphable model conditioning or learn imaginary canonical radiance fields, which fail to generalize to unseen facial expressions and capture fine-scale motion details.

Method: Uses inverse skinning based on parametric head model to map points to canonical space, then models fine-scale motions with part-based deformation field using multiple local MLPs that adaptively partition canonical space with soft-weighting aggregation.

Result: Extensive experiments show the method generalizes well to unseen expressions and models fine-scale facial motions, outperforming state-of-the-art methods both quantitatively and qualitatively.

Conclusion: PartNerFace successfully addresses limitations of existing approaches by using part-based deformation modeling, enabling better generalization to unseen expressions and capture of fine-scale facial motions.
...
TL;DR: ASTRA is a novel framework for multi-subject image generation that disentangles appearance from pose structure using retrieval-augmented pose guidance and asymmetric token encoding.


<details>
  <summary>Details</summary>
Motivation: Current subject-driven image generation struggles with multiple subjects having complex, distinct actions, leading to identity fusion and pose distortion due to entangled appearance and structure signals.

Method: ASTRA uses a dual-pronged approach: 1) Retrieval-Augmented Pose (RAG-Pose) pipeline for clean structural priors, 2) Enhanced Universal Rotary Position Embedding (EURoPE) to decouple identity tokens from spatial locations while binding pose tokens to canvas, and 3) Disentangled Semantic Modulation (DSM) adapter for identity preservation in text conditioning.

Result: ASTRA achieves state-of-the-art pose adherence on COCO-based complex pose benchmark while maintaining high identity fidelity and text alignment in DreamBench, demonstrating superior disentanglement.

Conclusion: ASTRA successfully resolves the fundamental conflict in multi-subject generation by architecturally disentangling appearance from pose structure within a unified Diffusion Transformer framework.
...
TL;DR: A computer vision framework combining YOLO and U-Net ensembles with refinement techniques achieved second place in the Riva Cytology Challenge for Bethesda cell detection in Pap smear images.


<details>
  <summary>Details</summary>
Motivation: To advance computer vision applications in medical imaging, specifically for detecting Bethesda cells in Pap smear images as part of the Riva Cytology Challenge (ISBI Track B).

Method: Proposed an ensemble framework combining YOLO and U-Net architectures, followed by refinement using overlap removal techniques and a binary classifier for improved cell detection.

Result: Achieved second place in the competition with a mAP50-95 score of 0.5909, demonstrating effective performance in medical cell detection.

Conclusion: The ensemble approach with refinement techniques provides an effective solution for automated Bethesda cell detection in Pap smear images, contributing to medical computer vision applications.
...
TL;DR: SceneGlue is a scene-aware feature matching framework that combines implicit parallel attention and explicit visibility estimation to overcome limitations of traditional local feature descriptors for cross-view image correspondence.
...
Method: SceneGlue uses a hybridizable matching paradigm integrating: 1) implicit parallel attention that simultaneously exchanges information among local descriptors within and across images, and 2) explicit cross-view visibility estimation via a Visibility Transformer that categorizes features into visible/invisible regions.

Result: Extensive experiments on homography estimation, pose estimation, image matching, and visual localization validate SceneGlue's superior performance in accuracy, robustness, and interpretability compared to traditional methods.

Conclusion: SceneGlue effectively compensates for local descriptor constraints through scene-aware matching, improving cross-view correspondence without requiring scene-level groundtruth annotations during training.
...
TL;DR: Lightweight architectures detect weather conditions from RGB images using style-inspired techniques, achieving high accuracy with real-time performance suitable for embedded systems.


<details>
  <summary>Details</summary>
Motivation: To investigate how weather conditions manifest as visual style variations in images and develop efficient models for weather detection that can run on resource-constrained embedded systems.

Method: Proposes two families of architectures: RTM (ResNet50-Truncated-MultiTasks) and PMG (PatchGAN-MultiTasks-Gram), using style-inspired techniques like Gram matrices, truncated ResNet-50, and PatchGAN within multi-task frameworks with attention mechanisms.

Result: Models achieve F1 scores above 96% on internal test set and above 78% in zero-shot evaluation on external datasets. PMG architecture has <5M parameters, runs in real-time with small memory footprint.

Conclusion: Weather conditions can be effectively detected as visual style variations using lightweight architectures. The modular models generalize well and are suitable for embedded applications, with released dataset of 503,875 annotated images.
...
<details>
  <summary>Details</summary>
Motivation: Standard vision-language models struggle to associate informative text components in diagnostic reports with tiny but clinically important image observations, as reports encode complex imaging data into concise, anatomically contextualized findings.
...
Result: MApLe successfully aligns different image regions with multiple diagnostic findings in free-text reports and improves alignment performance compared to state-of-the-art baselines on several downstream tasks.

Conclusion: The proposed MApLe approach effectively addresses the challenge of associating small image locations with clinically meaningful text components in diagnostic reports through disentangled anatomical-diagnostic concept modeling and patch-wise alignment.
...
TL;DR: HiProto introduces hierarchical prototype learning for interpretable object detection, using contrastive and regularization losses with scale-aware pseudo labels to improve semantic discrimination under degraded imaging conditions.


<details>
  <summary>Details</summary>
Motivation: Existing object detection methods lack interpretability and fail to improve semantic discrimination under low-quality imaging conditions. Prototype learning offers interpretable modeling through class-centered semantics, providing stable representations under degradation.

Method: HiProto uses hierarchical prototype learning across multiple feature levels. It introduces: 1) Region-to-Prototype Contrastive Loss (RPC-Loss) to enhance semantic focus on target regions, 2) Prototype Regularization Loss (PR-Loss) to improve class prototype distinctiveness, and 3) Scale-aware Pseudo Label Generation Strategy (SPLGS) to suppress mismatched supervision for RPC-Loss.

Result: Experiments on ExDark, RTTS, and VOC2012-FOG datasets demonstrate competitive performance while providing clear interpretability through prototype responses, without relying on image enhancement or complex architectures.

Conclusion: HiProto offers a new paradigm for interpretable object detection that effectively models class-specific semantics through hierarchical prototype learning, enhancing both semantic discrimination and interpretability under degraded imaging conditions.
...
TL;DR: TexADiff improves remote sensing image super-resolution by addressing texture imbalance through a Relative Texture Density Map that guides diffusion, modulates loss, and adapts sampling.


<details>
  <summary>Details</summary>
Motivation: Standard generative diffusion priors work well for natural images but fail for remote sensing images due to unique texture distributions where ground objects are globally stochastic but locally clustered, creating highly imbalanced textures that hinder spatial perception.

Method: Proposes TexADiff framework that: 1) estimates a Relative Texture Density Map (RTDM) to represent texture distribution, 2) uses RTDM as explicit spatial conditioning to guide diffusion process, 3) employs RTDM as loss modulation term to prioritize texture-rich regions, and 4) uses RTDM as dynamic adapter for sampling schedule.
...
Conclusion: TexADiff successfully addresses texture imbalance in remote sensing images by providing explicit texture-aware capabilities to diffusion models, leading to improved reconstruction quality and practical benefits for downstream applications.
...
TL;DR: Novel image/video orientation estimation using depth distribution across image quadrants, enhanced with depth gradient consistency and horizontal symmetry analysis for VR/AR applications.


<details>
  <summary>Details</summary>
Motivation: Need robust orientation estimation for immersive applications like VR, AR, autonomous navigation, and surveillance systems where spatial coherence and perceptual stability are critical.

Method: Estimates orientation by analyzing depth distribution across image quadrants, then refines with depth gradient consistency (DGC) and horizontal symmetry analysis (HSA) for precise correction.
...
TL;DR: The paper introduces POINTS-Seeker-8B, a multimodal agentic search model built from scratch that outperforms existing models on visual reasoning benchmarks by addressing long-horizon interaction challenges through adaptive history compression.


<details>
  <summary>Details</summary>
Motivation: Large Multimodal Models (LMMs) are limited by static parametric knowledge and current multimodal search approaches merely retrofit general LMMs with search tools as modular extensions rather than building agentic search capabilities from the ground up.

Method: Three key contributions: (1) Agentic Seeding phase to build foundational precursors for agentic behaviors; (2) V-Fold adaptive history-aware compression scheme to handle long-horizon interactions by preserving recent dialogue turns while folding historical context into visual space; (3) Development of POINTS-Seeker-8B model.
...
Conclusion: Building multimodal agentic search models from scratch with proper agentic seeding and adaptive history compression enables superior performance on complex visual reasoning tasks compared to retrofitting existing LMMs with search tools.
...
TL;DR: DailyClue benchmark evaluates MLLMs' visual clue-driven reasoning in daily scenarios, requiring active exploration of visual evidence rather than simple recognition.


<details>
  <summary>Details</summary>
Motivation: Current MLLM benchmarks focus on pre-existing knowledge or basic perception, neglecting the critical reasoning capability needed for visual-rich daily scenarios where models must filter noise and identify decisive visual clues.

Method: Created DailyClue benchmark with two core principles: strict grounding in authentic daily activities and challenging query design requiring more than surface-level perception. Curated comprehensive dataset spanning 4 major daily domains and 16 distinct subtasks.
...
Conclusion: DailyClue addresses the gap in evaluating MLLMs' reasoning capabilities in daily scenarios, emphasizing the importance of visual clue identification for accurate reasoning beyond simple perception tasks.
...
TL;DR: Delta-LLaVA: A novel MLLM framework for remote sensing change understanding that overcomes "temporal blindness" through specialized attention mechanisms and achieves state-of-the-art performance on the new Delta-QA benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing Multimodal Large Language Models (MLLMs) suffer from "temporal blindness" when applied to remote sensing change understanding - they lack mechanisms for multi-temporal contrastive reasoning and struggle with precise spatial grounding, limiting their effectiveness in analyzing temporal changes in earth observation data.

Method: Proposes Delta-LLaVA with three core innovations: 1) Change-Enhanced Attention module that isolates and amplifies visual differences, 2) Change-SEG module using Change Prior Embedding to extract differentiable difference features for LLM input, and 3) Local Causal Attention to prevent cross-temporal contextual leakage. Also introduces Delta-QA benchmark with 180k samples across four cognitive dimensions.
...
Conclusion: The proposed Delta-LLaVA framework successfully addresses the "temporal blindness" problem in MLLMs for remote sensing, providing a comprehensive solution for multi-temporal change understanding that bridges the gap between general vision-language models and specialized earth observation applications.
...
TL;DR: Free Geometry enables feed-forward 3D reconstruction models to self-evolve at test time through self-supervised learning, improving accuracy without 3D ground truth.
...
Method: Leverages the insight that more views produce more reliable reconstructions. Masks subset of frames to create self-supervised task, enforces cross-view feature consistency between full and partial observations while maintaining pairwise relations from held-out frames. Uses lightweight LoRA updates for fast recalibration.

Result: Consistently improves state-of-the-art foundation models (Depth Anything 3 and VGGT) across 4 benchmark datasets, with average improvements of 3.73% in camera pose accuracy and 2.88% in point map prediction. Takes less than 2 minutes per dataset on single GPU.
...
TL;DR: OneHOI is a unified diffusion transformer framework that consolidates HOI generation and editing using structured interaction representations, achieving SOTA results on both tasks.


<details>
  <summary>Details</summary>
Motivation: Existing HOI approaches are disjoint: HOI generation fails to integrate mixed conditions like HOI and object-only entities, while HOI editing struggles to decouple pose from physical contact and scale to multiple interactions.

Method: Introduces OneHOI with Relational Diffusion Transformer (R-DiT) that models verb-mediated relations through role- and instance-aware HOI tokens, layout-based spatial Action Grounding, Structured HOI Attention to enforce interaction topology, and HOI RoPE to disentangle multi-HOI scenes.
...
Conclusion: OneHOI provides a unified framework that successfully consolidates HOI generation and editing into a single conditional denoising process, overcoming limitations of previous disjoint approaches.
...
TL;DR: The paper introduces Unconstrained HOI (U-HOI), a new Human-Object Interaction detection task that removes predefined interaction vocabularies using Multimodal Large Language Models for flexible in-the-wild recognition.


<details>
  <summary>Details</summary>
Motivation: Current HOI detection models are limited by predefined interaction vocabularies, restricting their applicability to static environments. The advent of Multimodal Large Language Models enables exploration of more flexible paradigms for interaction recognition in real-world scenarios.

Method: The authors define the U-HOI task and evaluate various MLLMs on this setting. They introduce a pipeline with test-time inference and language-to-graph conversion to extract structured interactions from free-form text generated by MLLMs.

Result: The findings highlight the limitations of current HOI detectors and demonstrate the value of MLLMs for U-HOI, showing their capability to handle flexible interaction recognition without predefined vocabularies.

Conclusion: MLLMs offer a promising approach for unconstrained human-object interaction detection, enabling more flexible and applicable systems for real-world scenarios beyond static predefined interaction lists.
...
TL;DR: TF-SMOT is a training-free Semantic Multi-Object Tracking pipeline that composes pretrained components for detection, tracking, and video-language generation, achieving SOTA tracking performance while improving video summaries and captions.


<details>
  <summary>Details</summary>
Motivation: Existing SMOT systems require end-to-end training with expensive supervision, limiting rapid adaptation to new foundation models and interactions. The authors aim to create a more flexible, training-free approach.

Method: TF-SMOT combines D-FINE and SAM2 for detection and mask-based tracking, uses contour grounding with InternVideo2.5 for video summaries and instance captions, and aligns interaction predicates to WordNet synsets via gloss-based semantic retrieval with LLM disambiguation.

Result: On BenSMOT benchmark, TF-SMOT achieves state-of-the-art tracking performance in SMOT setting and improves summary and caption quality compared to prior methods. Interaction recognition remains challenging under strict exact-match evaluation.

Conclusion: TF-SMOT demonstrates that training-free composition of pretrained components can achieve strong SMOT performance, though interaction recognition on fine-grained WordNet labels remains difficult due to semantic overlap and label granularity issues.
...
TL;DR: ACPO is a dual-axis preference optimization framework that reduces audio hallucination in AVLMs by penalizing visual shortcuts and enforcing audio grounding.


<details>
  <summary>Details</summary>
Motivation: Audio-Visual Language Models suffer from cross-modal hallucination where they exploit visual shortcuts to hallucinate expected sounds instead of using true auditory evidence, creating unreliable audio grounding.

Method: Audio-Contrastive Preference Optimization (ACPO) with two objectives: output-contrastive penalizes visual descriptions masquerading as audio facts, and input-contrastive swaps audio tracks to penalize generation invariant to true auditory signals.

Result: Extensive experiments show ACPO establishes highly faithful audio grounding and mitigates audio hallucination without compromising overall multimodal capabilities.

Conclusion: ACPO effectively counteracts visual dominance in AVLMs through dual-axis preference learning, significantly improving audio reliability while maintaining multimodal performance.
...
TL;DR: LingBot-Map is a feed-forward 3D foundation model for streaming scene reconstruction that achieves ~20 FPS performance with geometric accuracy and temporal consistency using a novel geometric context transformer architecture.


<details>
  <summary>Details</summary>
Motivation: Streaming 3D reconstruction requires geometric accuracy, temporal consistency, and computational efficiency, but existing approaches struggle with these simultaneous requirements. The authors aim to create a feed-forward model that can handle long video sequences while maintaining stable performance.

Method: Built on a geometric context transformer (GCT) architecture with a carefully designed attention mechanism that integrates three components: anchor context for coordinate grounding, pose-reference window for dense geometric cues, and trajectory memory for long-range drift correction. This design keeps streaming state compact while retaining rich geometric context.

Result: Achieves stable efficient inference at around 20 FPS on 518 x 378 resolution inputs over long sequences exceeding 10,000 frames. Extensive evaluations show superior performance compared to both existing streaming and iterative optimization-based approaches across various benchmarks.

Conclusion: LingBot-Map successfully addresses the challenges of streaming 3D reconstruction by combining geometric accuracy, temporal consistency, and computational efficiency through its novel transformer architecture, making it a practical solution for real-time scene reconstruction from video streams.
...
TL;DR: SpatialEvo: Self-evolving framework for 3D spatial reasoning using deterministic geometric validation instead of model consensus, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Continuous improvement in 3D spatial reasoning is bottlenecked by expensive geometric annotation costs. Traditional self-evolving methods rely on model consensus which reinforces existing errors rather than correcting them.
...
Result: Achieves highest average score across nine benchmarks at both 3B and 7B scales. Shows consistent gains on spatial reasoning benchmarks with no degradation on general visual understanding.

Conclusion: SpatialEvo demonstrates that leveraging deterministic geometric properties enables effective self-evolving training for 3D spatial reasoning, overcoming limitations of model consensus approaches and achieving state-of-the-art performance.
...
TL;DR: ROSE framework enhances MLLM segmentation models with real-time web retrieval to handle novel and emerging entities, achieving 19.2 gIoU improvement over baseline.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based segmentation models struggle with novel/emerging entities due to outdated knowledge and inability to incorporate real-time information.

Method: Proposes ROSE: Retrieval-Oriented Segmentation Enhancement with Internet Retrieval-Augmented Generation, Textual/Visual Prompt Enhancers, and WebSense module for intelligent retrieval activation.

Result: ROSE significantly boosts performance on NEST benchmark, outperforming Gemini-2.0 Flash-based retrieval baseline by 19.2 gIoU.

Conclusion: ROSE effectively addresses knowledge gaps in MLLM segmentation through real-time web retrieval, enabling better handling of novel and emerging entities.
...
TL;DR: Seedance 2.0 is a native multi-modal audio-video generation model with unified architecture supporting text, image, audio, and video inputs, offering comprehensive content reference/editing capabilities and generating 4-15 second videos at 480p/720p resolution.


<details>
  <summary>Details</summary>
Motivation: To create a unified, efficient multi-modal audio-video generation model that surpasses previous versions (Seedance 1.0 and 1.5 Pro) by supporting multiple input modalities and providing comprehensive content reference and editing capabilities for enhanced creative experiences.

Method: Adopts a unified, highly efficient, large-scale architecture for multi-modal audio-video joint generation. Supports four input modalities (text, image, audio, video) with comprehensive multi-modal content reference and editing capabilities. Includes a Fast version for low-latency scenarios.

Result: Delivers substantial improvements across all key video and audio generation dimensions. Performance matches leading industry levels in expert evaluations and public tests. Supports 4-15 second audio-video content generation at 480p/720p resolution with support for up to 3 video clips, 9 images, and 3 audio clips as reference inputs.

Conclusion: Seedance 2.0 represents a significant advancement in multi-modal audio-video generation with enhanced foundational capabilities, improved multi-modal performance, and better creative experience for end users compared to previous versions.
...
TL;DR: X-Comp is a VLM for long video understanding that achieves extreme compression via learnable token-level compression (LP-Comp) and question-conditioned frame-level compression (QC-Comp), enabling processing of 2x-4x more frames with improved performance.


<details>
  <summary>Details</summary>
Motivation: Long video understanding is challenging for VLMs due to extensive frames and limited LLM context length, forcing sparse frame perception and loss of temporal information. Current heuristic-based compression methods are prone to information loss.

Method: Proposes LP-Comp (learnable progressive token-level compression) supervised at LLM layers for extreme compression to one token per frame. Introduces QC-Comp (question-conditioned frame-level compression) using LLM attention scores to select relevant frames. Mitigates LLM position bias by splitting long videos into segments with local attention.

Result: Achieves 2x-4x more frames processed with improved performance. Boosts accuracy from 42.9% to 46.2% on LVBench with only 2.5% of supervised fine-tuning data. Enhances multiple other long video benchmarks with significantly larger compression ratio and denser frame sampling.

Conclusion: X-Comp demonstrates effective extreme compression for long video understanding through combined token-level and frame-level compression, enabling VLMs to process more frames efficiently while maintaining or improving performance with minimal training data.
...
### [104] [Olfactory pursuit: catching a moving odor source in complex flows](https://arxiv.org/abs/2604.13121)
*Maurizio Carbone,Lorenzo Piro,Robin A. Heinonen,Luca Biferale,Massimo Cencini,Antonio Celani*

Main category: cs.RO

TL;DR: A hybrid policy combining information-gain (Infotaxis) with greedy value function from fully observable control achieves near-optimal olfactory pursuit across target persistence times, outperforming purely exploratory approaches.


<details>
  <summary>Details</summary>
Motivation: Locating and intercepting moving targets from delayed, intermittent sensory signals (like odor plumes) is a fundamental challenge in decision-making under uncertainty, relevant to animal behavior and autonomous robotics.

Method: Formulate olfactory pursuit as partially observable Markov decision process (POMDP) with joint belief over target position/velocity. Compute quasi-optimal policies via Bellman equation, benchmark against Infotaxis, and introduce hybrid policy combining information-gain with greedy value function from associated fully observable control problem.

Result: Purely exploratory policies work well when target frequently reorients but fail with persistent motion. Hybrid policy achieves near-optimal performance across all persistence times, outperforms purely exploratory approaches, and shows robustness in complex scenarios including continuous motion, model mismatch, and accurate plume dynamics.

Conclusion: Predictive inference of target motion is key for effective olfactory pursuit. Hybrid approach provides general framework for search in information-poor, dynamically evolving environments.
...
### [105] [Multi-modal panoramic 3D outdoor datasets for place categorization](https://arxiv.org/abs/2604.13142)
*Hojung Jung,Yuki Oto,Oscar M. Mozos,Yumi Iwashita,Ryo Kurazume*

Main category: cs.RO

TL;DR: Two multi-modal panoramic 3D outdoor datasets for semantic place categorization with six categories, featuring dense static scans and sparse real-time scans, achieving high classification accuracy.


<details>
  <summary>Details</summary>
Motivation: To provide comprehensive datasets for semantic place categorization research, addressing the need for both high-quality static 3D data and real-time sparse data collected from moving vehicles.

Method: Created two datasets: 1) 650 dense static panoramic scans using FARO laser scanner with color/reflectance point clouds and synchronized images; 2) 34,200 sparse real-time scans using Velodyne laser scanner while driving. Both collected in Fukuoka, Japan with six place categories.

Result: Best classification results of 96.42% for dense dataset and 89.67% for sparse dataset, demonstrating effectiveness of the datasets for semantic place categorization tasks.

Conclusion: The presented MPO datasets provide valuable resources for semantic place categorization research, with both dense static and sparse real-time 3D data achieving high classification accuracy across six place categories.
...
### [106] [Weakly-supervised Learning for Physics-informed Neural Motion Planning via Sparse Roadmap](https://arxiv.org/abs/2604.13204)
...
<details>
  <summary>Details</summary>
Motivation: Existing physics-informed motion planning methods (like Neural Time Fields) struggle to scale in complex, multi-room environments where increasing samples cannot resolve local minima or guarantee global consistency.

Method: Hierarchical framework combining weak supervision from sparse roadmaps (providing global topological anchors through travel time bounds) with physics-informed PDE regularization (enforcing local geometric fidelity and obstacle-aware propagation).
...
### [107] [Capability-Aware Heterogeneous Control Barrier Functions for Decentralized Multi-Robot Safe Navigation](https://arxiv.org/abs/2604.13245)
*Joonkyung Kim,Yanze Zhang,Wenhao Luo,Yiwei Lyu*

Main category: cs.RO

TL;DR: CA-HCBF: A decentralized framework for safe navigation in heterogeneous multi-robot teams using capability-aware control barrier functions that account for different robot dynamics and motion capabilities.
...
Method: Proposes Capability-Aware Heterogeneous Control Barrier Function (CA-HCBF) with: 1) Canonical second-order control-affine representation unifying holonomic and nonholonomic robots via canonical transformation and backstepping; 2) Support-function-based directional capability metric quantifying each robot's ability to follow motion intent; 3) Pairwise responsibility allocation distributing safety burden proportionally to motion capability; 4) Feasibility-aware clipping mechanism constraining allocations to physically achievable ranges.
...
Conclusion: CA-HCBF enables consistent safety enforcement and capability-aware coordination in heterogeneous robot teams, addressing the limitations of homogeneous assumptions in decentralized safety methods while maintaining forward invariance of safe sets.
...
### [108] [GeoVision-Enabled Digital Twin for Hybrid Autonomous-Teleoperated Medical Responses](https://arxiv.org/abs/2604.13248)
*Parham Kebria,Soheil Sabri,Laura J Brattain*

Main category: cs.RO

TL;DR: Digital Twin architecture for hybrid autonomous-teleoperated medical response systems using GeoVision capabilities to enhance remote emergency care in disaster areas.


<details>
  <summary>Details</summary>
Motivation: To improve emergency medical response in disaster-affected and infrastructure-limited environments by providing better remote situational awareness and decision-making capabilities for clinical and operational users.

Method: Proposes a Digital Twin framework that integrates perception and adaptive navigation with real-time synchronization, creating a virtual representation mirroring system states, environmental dynamics, patient conditions, and mission objectives.

Result: The Digital Twin provides an intuitive, continuously updated virtual representation that enhances situational awareness and informed decision-making compared to traditional ground control interfaces.

Conclusion: Digital Twin architecture enabled by GeoVision capabilities offers a promising approach for improving remote medical response systems in challenging environments through enhanced virtual representation and real-time synchronization.
...
### [109] [Utilizing Inpainting for Keypoint Detection for Vision-Based Control of Robotic Manipulators](https://arxiv.org/abs/2604.13309)
*Sreejani Chatterjee,Venkatesh Mullur,Abhinav Gandhi,Berk Calli*

Main category: cs.RO

TL;DR: A visual servoing framework using natural robot features without markers, trained via inpainting-based data generation and enhanced with real-time inpainting and UKF for robust keypoint detection under occlusion.


<details>
  <summary>Details</summary>
Motivation: To enable vision-based robotic control using natural visual features without external markers, especially for scenarios where markers are not feasible or preferred during runtime.

Method: 1) Data collection with ArUco markers, labeling centers as keypoints, then inpainting to remove markers and reconstruct occluded regions for training data. 2) Train keypoint detection algorithm on markerless images. 3) Real-time inpainting model for runtime occlusion handling. 4) Unscented Kalman Filter for refining keypoint estimates over time.

Result: Successful control results with model-free, purely vision-based control using natural robot features, working under both full visibility and partial occlusion conditions.

Conclusion: The framework eliminates dependencies on accurate camera calibration and robot models through inpainting-based approaches, enabling robust visual servoing with natural features even in occluded environments.
...
### [110] [Vectorizing Projection in Manifold-Constrained Motion Planning for Real-Time Whole-Body Control](https://arxiv.org/abs/2604.13323)
*Shrutheesh R Iyer,I-Chia Chang,Andrew Z. Liu,Yan Gu,Zachary Kingston*

Main category: cs.RO

TL;DR: CPU SIMD-accelerated manifold-constrained motion planner achieves 100-1000x speed-up over state-of-the-art, enabling real-time constrained planning for humanoid robots.


<details>
  <summary>Details</summary>
Motivation: Current manifold-constrained motion planning methods are too slow (tens of seconds) for complex systems like humanoid robots, making real-world use impractical in dynamic settings.

Method: Revisits projection-based constraint satisfaction through parallelization lens, transforms components into parallelizable structures, and uses CPU SIMD parallelism to plan constraint-satisfying solutions.

Result: Achieves up to 100-1000x speed-ups over state-of-the-art, enabling real-time constrained motion planning for the first time. Demonstrated on real humanoid robot with real-time whole-body quasi-static plan generation.

Conclusion: The SIMD-accelerated approach makes real-time constrained motion planning feasible for complex robotic systems, overcoming previous computational bottlenecks.
...
### [111] [Boundary Sampling to Learn Predictive Safety Filters via Pontryagin's Maximum Principle](https://arxiv.org/abs/2604.13325)
*James Dallas,Thomas Lew,John Talbot,Jonathan DeCastro,Somil Bansal,John Subosits*

Main category: cs.RO

TL;DR: PMP-guided sampling improves learning efficiency for Hamilton-Jacobi reachability safety filters by focusing data collection on safety-critical boundary states.


<details>
  <summary>Details</summary>
Motivation: Learning-based safety filters need informative data near constraint violations, but sampling these critical states is inefficient in complex high-dimensional systems.

Method: Use Pontryagin Maximum Principle to characterize boundary trajectories that barely avoid violations, then guide data collection for learned Hamilton-Jacobi Reachability to concentrate learning near safety-critical states.

Result: PMP sampling improves learning efficiency with faster convergence, reduced failure rates, and better safe set reconstruction, achieving 3ms wall times in automotive racing experiments.

Conclusion: Characterizing boundary trajectories via PMP enables efficient data collection for learned safety filters, improving performance in high-dimensional autonomous systems.
...
### [112] [Singularity Avoidance in Inverse Kinematics: A Unified Treatment of Classical and Learning-based Methods](https://arxiv.org/abs/2604.13405)
*Vishnu Rudrasamudram,Hariharasudan Malaichamee*

Main category: cs.RO

TL;DR: Survey paper that bridges classical singularity-robust inverse kinematics with modern learning-based approaches, providing unified treatment, taxonomy, and experimental benchmarking of 12 IK solvers on Franka Panda robot.


<details>
  <summary>Details</summary>
Motivation: Singular configurations cause serious problems in inverse kinematics (loss of mobility, unbounded joint velocities, solver divergence), but there's no existing survey connecting classical singularity-robust IK with rapidly growing learning-based approaches.
...
Result: Pure learning methods fail even on well-conditioned targets (MLP: 0% success, ~10mm mean error), while hybrid warm-start architectures rescue learned solvers via classical refinement (IKFlow: 59-100%, CycleIK: 0-98.6%, GGIK: 0-100%). DLS converges from initial errors up to 207mm.

Conclusion: Hybrid approaches combining learning with classical refinement are most effective. Deeper evaluation of singularity regimes is identified as immediate future work. The paper provides a needed bridge between classical and learning-based IK methods with systematic evaluation framework.
...
### [113] [Robust Energy-Aware Routing for Air-Ground Cooperative Multi-UAV Delivery in Wind-Uncertain Environments](https://arxiv.org/abs/2604.13441)
*Tianshun Li,Hongliang Lu,Yanggang Sheng,Zhongzhen Wang,Haoang Li,Xinhu Zheng*

Main category: cs.RO

TL;DR: Online risk-sensitive routing framework for UAV delivery that balances energy expenditure and uncertainty-aware risk under dynamic wind conditions, improving mission success rates.


<details>
  <summary>Details</summary>
Motivation: Existing routing approaches assume static or deterministic energy models, making them unreliable for UAV delivery missions under dynamic wind uncertainty, which is critical for safety and reliability.

Method: Battery-Efficient Routing (BER) - online risk-sensitive planning framework that formulates routing on time-dependent energy graphs with evolving edge costs based on wind-induced aerodynamic effects, continuously evaluating return feasibility while balancing energy expenditure and risk.

Result: Extensive simulations show BER significantly improves mission success rates and reduces wind-induced failures compared to static and greedy baselines in synthetic ER graphs and quasi-real wind environments.

Conclusion: Integrating real-time energy budgeting and environmental awareness is crucial for UAV delivery planning under dynamic wind conditions, with BER demonstrating effective risk-sensitive routing for improved reliability.
...
### [114] [RobotPan: A 360$^\circ$ Surround-View Robotic Vision System for Embodied Perception](https://arxiv.org/abs/2604.13476)
...
TL;DR: RobotPan is a surround-view robotic vision system using six cameras + LiDAR for 360° coverage, with a feed-forward framework predicting metric-scaled 3D Gaussians for real-time rendering, reconstruction, and streaming.
...
Method: Combines six cameras with LiDAR for full 360° coverage. Uses RobotPan framework that lifts multi-view features into unified spherical coordinate representation and decodes Gaussians using hierarchical spherical voxel priors (fine resolution near robot, coarser at larger radii). Includes online fusion for long sequences that updates dynamic content while preventing unbounded growth in static regions.

Result: Achieves competitive quality against prior feed-forward reconstruction and view-synthesis methods while producing substantially fewer Gaussians, enabling practical real-time embodied deployment. Releases a multi-sensor dataset for 360° novel view synthesis and metric 3D reconstruction covering navigation, manipulation, and locomotion.

Conclusion: RobotPan provides a surround-view robotic vision system that meets geometric and real-time constraints for embodied deployment, addressing limitations of current robotic visual interfaces and enabling better human-in-the-loop robotic operations.
...
### [115] [RadarSplat-RIO: Indoor Radar-Inertial Odometry with Gaussian Splatting-Based Radar Bundle Adjustment](https://arxiv.org/abs/2604.13492)
...
<details>
  <summary>Details</summary>
Motivation: Radar SLAM suffers from substantial drift due to heavy reliance on frame-to-frame odometry, while visual odometry benefits from bundle adjustment. No equivalent BA formulation exists for radar despite its weather resilience advantages.

Method: Proposes radar bundle adjustment enabled by Gaussian Splatting, a dense differentiable scene representation. Jointly optimizes radar sensor poses and scene geometry using full range-azimuth-Doppler data, integrated with existing radar-inertial odometry frontend.

Result: Significantly reduces pose drift and improves robustness. Across multiple indoor scenes, reduces average absolute translational errors by 90% and rotational errors by 80% compared to prior radar-inertial odometry.

Conclusion: First successful implementation of bundle adjustment for radar SLAM using Gaussian Splatting, bringing multi-frame optimization benefits to radar for the first time and substantially improving localization accuracy.
...
### [116] [A transformable slender microrobot inspired by nematode parasites for interventional endovascular surgery](https://arxiv.org/abs/2604.13513)
*Xin Yang,Dongliang Fan,Yunteng Ma,Yuxuan Liao,Diancheng Li,U Kei Cheang,Bo Peng,Hongqiang Wang*

Main category: cs.RO

TL;DR: Nematode-inspired transformable magnetic microrobot for cardiovascular interventions with high flexibility, maneuverability, and drug delivery capabilities.


<details>
  <summary>Details</summary>
Motivation: Cardiovascular diseases cause 17.9 million annual deaths globally, but treatment is challenging due to confined vascular spaces and complex topologies. Existing robots face trade-offs between versatility and maneuverability.

Method: Inspired by nematodes, developed a transformable slender magnetic microrobot with aspect ratio >100 (diameter <200μm, length >20mm), featuring uniformly distributed magnetic beads on a polymer string and a larger head bead.

Result: Robot shows exceptional flexibility (curvature 0.904 mm⁻¹), high speed (125 mm/s), can navigate sharp turns (radius 0.84mm) and 3D holes. Successfully demonstrated drug delivery (carrying 95x its weight) and embolization in aneurysm phantoms, plus injectability through medical needles.

Conclusion: The nematode-inspired magnetic microrobot offers promising potential for minimally invasive cardiovascular interventions with superior maneuverability, drug delivery capabilities, and compatibility with existing medical instruments.
...
### [117] [Stability Principle Underlying Passive Dynamic Walking of Rimless Wheel](https://arxiv.org/abs/2604.13530)
*Fumihiko Asano*

Main category: cs.RO

TL;DR: The paper re-examines stability analysis of rimless wheel passive dynamic walking, investigating the relationship between stability and energy conservation principles beyond the known asymptotic stability conditions.


<details>
  <summary>Details</summary>
Motivation: While rimless wheels are known to exhibit asymptotically stable 1-period passive gaits due to gravity effects and two necessary conditions (impact posture constraint and restored mechanical energy constraint), there is room for deeper understanding of the inherent stability principles beyond the existing recurrence formula analysis.

Method: The authors reconsider stability of the stance phase through linearization of the equation of motion and investigate the relationship between stability and the energy conservation law, conducting mathematical analysis to provide deeper insights.

Result: Through mathematical analysis, the paper provides greater understanding of the inherent stability principle in rimless wheel passive dynamic walking, revealing deeper connections between stability and energy conservation.

Conclusion: The paper advances theoretical understanding of passive dynamic walking stability by examining the fundamental relationship between stability analysis and energy conservation principles in rimless wheel models.
...
### [118] [Evolvable Embodied Agent for Robotic Manipulation via Long Short-Term Reflection and Optimization](https://arxiv.org/abs/2604.13533)
*Jianzong Wang,Botao Zhao,Yayun He,Junqing Peng,Xulong Zhang*

Main category: cs.RO

TL;DR: EEAgent framework uses large vision-language models and reflective optimization to enable robots to self-evolve through experience without extensive training, achieving state-of-the-art performance on VIMA-Bench tasks.
...
Method: Proposes EEAgent framework leveraging large VLMs for environmental interpretation and policy planning. Introduces LSTRO (long short-term reflective optimization) mechanism that dynamically refines prompts based on past experiences and new lessons, enabling continuous self-evolution.

Result: Evaluation on six VIMA-Bench tasks shows the approach sets new state-of-the-art, notably outperforming baselines in complex scenarios.

Conclusion: The EEAgent framework with LSTRO enables effective self-evolving robotics through experience-based reflection, addressing key limitations of traditional methods and demonstrating superior performance on benchmark tasks.
...
### [119] [Self-adaptive Multi-Access Edge Architectures: A Robotics Case](https://arxiv.org/abs/2604.13542)
*Mahyar T Moghaddam,Joakim Leed,Anders Frandsen*

Main category: cs.RO

TL;DR: A self-adaptation system using MAPE-K-based intelligent agents to optimize neural network processing for human mobility prediction in mixed human-robot environments through distributed edge offloading and Kubernetes orchestration.


<details>
  <summary>Details</summary>
Motivation: To address the growing compute-intensive AI tasks by improving performance, energy efficiency, and reducing processing costs in mixed human-robot environments, particularly for neural network-based human mobility prediction to enhance robot path planning and ensure human safety.

Method: Built a distributed edge offloading system with heterogeneous processing units orchestrated by Kubernetes, using MAPE-K-based adaptation supervisors that monitor response times and power consumption to make informed decisions on scaling and computation offloading.

Result: The system shows notable improvements in service quality over traditional setups, demonstrating effectiveness for AI-driven systems through better performance and energy efficiency.

Conclusion: The proposed self-adaptation approach with intelligent agents as architectural adaptation supervisors effectively optimizes neural network processing in mixed human-robot environments, providing a scalable and efficient solution for AI-driven systems.
...
### [120] [UNRIO: Uncertainty-Aware Velocity Learning for Radar-Inertial Odometry](https://arxiv.org/abs/2604.13584)
*Jui-Te Huang,Tinashu Huang,Anthony Rowe,Michael Kaess*

Main category: cs.RO

TL;DR: UNRIO is an uncertainty-aware radar-inertial odometry system that directly estimates ego-velocity from raw mmWave radar IQ signals using a transformer-based neural network, bypassing traditional point cloud processing.


<details>
  <summary>Details</summary>
Motivation: Existing radar-inertial odometry methods rely on handcrafted signal processing pipelines that discard latent information in raw spectrum data and require careful parameter tuning, limiting their performance especially in challenging motion scenarios.

Method: Proposes a transformer-based neural network (GRT architecture) that processes the full 4-D spectral cube to predict body-frame velocity in two modes: direct linear velocity estimate and per-anglebin Doppler velocity map. Uses three-stage training: geometric pretraining on LiDAR-projected depth, velocity/Doppler fine-tuning, and uncertainty calibration via negative log-likelihood loss. Uncertainty estimates are propagated into a sliding-window pose graph that fuses radar velocity factors with IMU preintegration measurements.

Result: Achieves the lowest relative pose error on majority of sequences in the IQ1M dataset across diverse indoor environments, with particularly strong gains over classical DSP baselines on lateral-motion trajectories where sparse point clouds degrade conventional velocity estimators.

Conclusion: UNRIO demonstrates that direct processing of raw radar IQ signals with uncertainty-aware neural networks outperforms traditional point-cloud-based approaches, especially in challenging motion scenarios, enabling more robust radar-inertial odometry.
...
### [121] [A Mechanistic Analysis of Sim-and-Real Co-Training in Generative Robot Policies](https://arxiv.org/abs/2604.13645)
*Yu Lei,Minghuan Liu,Abhiram Maddukuri,Zhenyu Jiang,Yuke Zhu*

Main category: cs.RO

TL;DR: The paper analyzes the mechanisms behind effective co-training for robot policies, identifying two key effects: structured representation alignment (primary) and importance reweighting (secondary), validated through experiments and leading to improved methods.


<details>
  <summary>Details</summary>
Motivation: Despite widespread empirical success of co-training (combining limited real-world data with abundant surrogate data like simulation), the underlying mechanisms determining when and why it works remain poorly understood, creating a gap between practice and theory.

Method: The authors investigate co-training mechanisms through theoretical analysis and empirical studies, including controlled experiments on toy models and extensive sim-and-sim/sim-and-real robot manipulation experiments to validate identified effects.

Result: Identified two intrinsic effects governing co-training performance: 1) structured representation alignment (balance between cross-domain alignment and domain discernibility, primary role), and 2) importance reweighting effect (domain-dependent action weighting modulation, secondary role). These effects were validated experimentally.

Conclusion: The analysis provides a unified interpretation of recent co-training techniques and motivates a simple method that consistently improves upon prior approaches, aiming to examine the inner workings of co-training and facilitate future research in this direction.
...
### [122] [Vision-and-Language Navigation for UAVs: Progress, Challenges, and a Research Roadmap](https://arxiv.org/abs/2604.13654)
...
<details>
  <summary>Details</summary>
Motivation: To provide a structured overview of the UAV-VLN field, which is crucial for enabling UAVs to understand human commands and perform complex tasks in 3D environments, addressing the need for systematic organization of this emerging research area.

Method: Establishes methodological taxonomy tracking evolution from early modular/deep learning approaches to current agentic systems using foundation models (VLMs, VLAs), generative world models; systematically reviews resources (simulators, datasets, metrics); analyzes deployment challenges.

Result: Provides comprehensive survey organizing UAV-VLN field, identifies key technological evolution patterns, catalogs essential research resources, critically analyzes major real-world deployment challenges (sim-to-real gap, perception robustness, linguistic ambiguity, hardware constraints).

Conclusion: Proposes forward-looking research roadmap focusing on key frontiers like multi-agent swarm coordination and air-ground collaborative robotics, synthesizing current benchmarks and limitations to guide future inquiry in embodied AI for UAV navigation.
...
### [123] [Empirical Prediction of Pedestrian Comfort in Mobile Robot Pedestrian Encounters](https://arxiv.org/abs/2604.13677)
*Alireza Jafari,Hong-Son Nguyen,Yen-Chen Liu*

Main category: cs.RO

TL;DR: This paper develops comfort estimators for mobile robots interacting with pedestrians by analyzing kinematic variables and their correlation with subjective comfort, creating predictors that can be integrated into path planning for more socially compliant robots.


<details>
  <summary>Details</summary>
Motivation: Mobile robots in public spaces need to consider pedestrian comfort, not just objective safety. Current research focuses on collision avoidance but neglects subjective safety/comfort, which is challenging to quantify and prevents robots from responding to human emotions.

Method: Conducted one-on-one experimental trials with mobile robots and volunteers, statistically analyzed reported comfort versus kinematic variables, then designed three comfort estimators: minimum distance-based, minimum projected time-to-collision-based, and a composite estimator using all studied kinematic variables.

Result: Found moderate but significant correlations between kinematic variables and comfort. The composite estimator achieved highest prediction rate with odds ratio of 3.67 (when it identifies pedestrian as comfortable, they're almost 4 times more likely to actually be comfortable).

Conclusion: Provides a quantifiable comfort measure that can be incorporated into robot path planners to create more socially compliant robots that consider pedestrian feelings, addressing the gap between objective safety and subjective comfort in human-robot interaction.
...
### [124] [Failure Identification in Imitation Learning Via Statistical and Semantic Filtering](https://arxiv.org/abs/2604.13788)
*Quentin Rolland,Fabrice Mayran de Chamisso,Jean-Baptiste Mouret*

Main category: cs.RO

TL;DR: FIDeL is a policy-independent failure detection module for robotics that combines anomaly detection with semantic filtering to distinguish genuine failures from benign anomalies, outperforming existing methods on real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Imitation learning policies in robotics perform well in controlled settings but fail in real-world deployments due to rare events outside training distributions. While anomaly detection methods exist, they don't distinguish between failures and benign deviations, leading to false alarms.

Method: FIDeL builds compact representations of nominal demonstrations and aligns incoming observations via optimal transport matching to produce anomaly scores and heatmaps. It uses spatio-temporal thresholds from conformal prediction extension and employs a Vision-Language Model for semantic filtering to discriminate benign anomalies from genuine failures.

Result: FIDeL consistently outperforms state-of-the-art baselines, achieving +5.30% AUROC in anomaly detection and +17.38% failure-detection accuracy on the BotFails dataset compared to existing methods.

Conclusion: FIDeL provides an effective policy-independent failure detection solution that can distinguish genuine failures from benign anomalies, making imitation learning policies more robust for real-world robotic deployments.
...
### [125] [EmbodiedClaw: Conversational Workflow Execution for Embodied AI Development](https://arxiv.org/abs/2604.13800)
*Xueyang Zhou,Yihan Sun,Xijie Gong,Guiyao Tie,Pan Zhou,Lichao Sun,Yongchao Chen*

Main category: cs.RO

TL;DR: EmbodiedClaw is a conversational AI agent that automates embodied AI development workflows, reducing manual engineering effort while improving reproducibility.


<details>
  <summary>Details</summary>
Motivation: Embodied AI research is shifting to multi-task, multi-scene, multi-model settings, which substantially increases engineering overhead and development time for tasks like environment construction, trajectory collection, model training, and evaluation.

Method: Proposes a conversational paradigm where users express goals/constraints through conversation, and the system automatically plans/executes development workflows. EmbodiedClaw implements this with executable skills for environment creation/revision, benchmark transformation, trajectory synthesis, model evaluation, and asset expansion.

Result: Experiments on end-to-end workflow tasks, capability-specific evaluations, human researcher studies, and ablations show EmbodiedClaw reduces manual engineering effort while improving executability, consistency, and reproducibility.

Conclusion: Suggests a shift from manual toolchains to conversationally executable workflows for embodied AI development, enabling more efficient and reproducible research.
...
### [126] [Mosaic: An Extensible Framework for Composing Rule-Based and Learned Motion Planners](https://arxiv.org/abs/2604.13853)
*Nick Le Large,Marlon Steiner,Lingguang Wang,Willi Poh,Jan-Hendrik Pauls,Ömer Şahin Taş,Christoph Stiller*

Main category: cs.RO

TL;DR: Mosaic is a framework that combines rule-based and learned planners through arbitration graphs to achieve safe, explainable motion planning while outperforming individual planners.


<details>
  <summary>Details</summary>
Motivation: Rule-based planners are predictable and explainable but struggle with real-world complexity, while learned planners are adaptive but lack transparency and sometimes violate safety. There's a need to combine both approaches for optimal performance.

Method: Uses extensible arbitration graphs to integrate rule-based and learned planners, decoupling trajectory verification/scoring from generation. Features higher-level verification for redundancy and unified scoring with optimal trajectory selection.

Result: Achieves 95.48 CLS-NR and 93.98 CLS-R on nuPlan Val14 benchmark (state-of-the-art), reduces at-fault collisions by 30% vs individual planners. Scores 54.30 CLS-R on interPlan benchmark, outperforming best constituent planner by 23.3%.

Conclusion: Mosaic successfully combines rule-based and learned planners to achieve superior safety and performance without retraining or additional data, providing transparent and traceable decision-making.
...
### [127] [Beyond Conservative Automated Driving in Multi-Agent Scenarios via Coupled Model Predictive Control and Deep Reinforcement Learning](https://arxiv.org/abs/2604.13891)
*Saeed Rahmani,Gözde Körpe,Zhenlin,Xu,Bruno Brito,Simeon Craig Calvert,Bart van Arem*

Main category: cs.RO

TL;DR: Integrated MPC-RL framework improves autonomous intersection navigation by combining MPC's constraint handling with RL's adaptive learning, achieving better safety and efficiency than standalone methods.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving at unsignalized intersections requires balancing safety and efficiency in complex multi-vehicle interactions. Standalone MPC is too conservative with hand-crafted rules, while pure RL lacks safety assurance and generalization capabilities.

Method: Integrated Model Predictive Control (MPC) and Reinforcement Learning (RL) framework that combines MPC's structured constraint handling with RL's adaptive learning from experience.

Result: MPC-RL outperforms standalone MPC and end-to-end RL across three traffic-density levels, reducing collision rate by 21% and improving success rate by 6.5% compared to pure MPC. Shows better zero-shot transfer to highway merging and faster training convergence.

Conclusion: The integrated MPC-RL approach improves safety-efficiency balance in multi-agent intersection scenarios, with MPC providing robust generalization foundation across driving environments while reducing RL's learning burden.
...
### [128] [Goal2Skill: Long-Horizon Manipulation with Adaptive Planning and Reflection](https://arxiv.org/abs/2604.13942)
*Zhen Liu,Xinyu Ning,Zhe Hu,Xinxin Xie,Weize Li,Zhipeng Tang,Chongyu Wang,Zejun Yang,Hanlin Wang,Yitong Liu,Zhongzhu Pu*

Main category: cs.RO

TL;DR: Dual-system VLA framework separates high-level semantic reasoning from low-level motor execution for robust long-horizon embodied manipulation with memory and recovery capabilities.
...
Method: Dual-system framework with: 1) High-level planner (VLM-based agentic module) for structured task memory, goal decomposition, outcome verification, and error-driven correction; 2) Low-level executor (VLA-based visuomotor controller) with diffusion-based action generation on geometry-preserving filtered observations. Systems form closed-loop planning-execution cycle.

Result: Achieves 32.4% average success rate on RMBench tasks compared to 9.8% for strongest baseline. Ablation studies confirm importance of structured memory and closed-loop recovery for long-horizon manipulation.

Conclusion: The proposed dual-system framework effectively addresses limitations of existing VLA policies by separating reasoning from execution, enabling memory-aware reasoning, adaptive replanning, and robust online recovery for long-horizon embodied manipulation tasks.
...
### [129] [Towards Multi-Object-Tracking with Radar on a Fast Moving Vehicle: On the Potential of Processing Radar in the Frequency Domain](https://arxiv.org/abs/2604.14013)
*Tim Hansen,Arturo Gomez-Chavez,Ilya Shimchik,Andreas Birk*

Main category: cs.RO

TL;DR: Processing radar data in frequency domain improves robustness against noise and structural errors, especially for high-dynamic scenes with multiple moving objects, enabling radar-only odometry without sensor fusion.


<details>
  <summary>Details</summary>
Motivation: To achieve higher robustness against noise and structural errors in radar data processing, especially for automotive applications with high dynamics (ego-motion plus unknown moving objects), and to leverage frequency-domain processing advantages for detecting all moving structures.

Method: Processing radar data in the frequency domain using Fourier SOFT in 2D (FS2D), with correlation-based methods for registration, applied to radar-only odometry using the Boreas dataset.

Result: Initial experiments demonstrate successful radar-only odometry without sensor fusion, showing improved robustness and the ability to detect all moving structures in dynamic scenes.

Conclusion: Frequency-domain processing of radar data offers superior robustness and comprehensive motion detection capabilities compared to feature-based methods, making it promising for autonomous applications like overtaking maneuvers in racing scenarios.
...
### [130] [Neuromorphic Spiking Ring Attractor for Proprioceptive Joint-State Estimation](https://arxiv.org/abs/2604.14021)
...
TL;DR: Spiking ring-attractor network for robot joint angle representation with stable activity bump, velocity modulation, and boundary conditions for mechanical limits.


<details>
  <summary>Details</summary>
Motivation: Need for stable internal representations of continuous variables in robotic control under resource constraints, with biologically inspired continuous attractor networks rarely addressing proprioceptive estimation in neuromorphic implementations.

Method: Spiking ring-attractor network using local excitation and broad inhibition to create stable activity bump, with velocity-modulated asymmetries for translation and boundary conditions to confine motion within mechanical limits.

Result: Network reproduces smooth trajectory tracking, remains stable near joint limits, shows reduced drift and improved accuracy compared to unbounded models, with compact hardware-compatible implementation preserving multi-second stability and near-linear bump velocity-synaptic modulation relationship.

Conclusion: The spiking ring-attractor network provides an effective, resource-efficient solution for proprioceptive estimation in robotics, demonstrating stable continuous variable representation with biologically inspired mechanisms suitable for hardware implementation.
...
### [131] [Scale-Invariant Sampling in Multi-Arm Bandit Motion Planning for Object Extraction](https://arxiv.org/abs/2604.14026)
*Servet B. Bayraktar,Andreas Orthey,Marc Toussaint*

Main category: cs.RO

TL;DR: Scale-invariant sampling strategy improves object extraction in tight spaces by finding optimal sampling scales and directions, boosting success rates significantly compared to existing methods.
...
Method: Proposes scale-invariant sampling using grow-shrink search to find high-entropy sampling scales, then uses PCA to identify optimal extraction directions, embedded in a multi-arm bandit RRT planner.

Result: Tested on 8 challenging 3D object extraction scenarios; improved success rate by one order of magnitude on 7 out of 8 scenarios compared to classical and modern sampling strategies.

Conclusion: Scale-invariant sampling is crucial for general purpose object extraction in disassembly tasks, significantly outperforming existing sampling approaches in tight-space scenarios.
...
### [132] [UMI-3D: Extending Universal Manipulation Interface from Vision-Limited to 3D Spatial Perception](https://arxiv.org/abs/2604.14089)
*Ziming Wang*

Main category: cs.RO

TL;DR: UMI-3D enhances the Universal Manipulation Interface by adding LiDAR for robust 3D data collection, improving reliability and enabling challenging manipulation tasks while maintaining portability.


<details>
  <summary>Details</summary>
Motivation: The original UMI's reliance on monocular visual SLAM makes it vulnerable to occlusions, dynamic scenes, and tracking failures, limiting real-world applicability for embodied manipulation data collection.

Method: Introduces a lightweight LiDAR sensor integrated into the wrist-mounted interface, enabling LiDAR-centric SLAM with accurate metric-scale pose estimation. Develops hardware-synchronized multimodal sensing and unified spatiotemporal calibration to align visual observations with LiDAR point clouds.

Result: Significantly improves data quality and reliability, achieving high success rates on standard tasks and enabling learning of challenging tasks like large deformable object manipulation and articulated object operation that were infeasible with vision-only UMI.

Conclusion: UMI-3D provides a robust, scalable data collection system for embodied manipulation that maintains portability while overcoming limitations of vision-only approaches, with all hardware and software open-sourced to accelerate research.
...
### [133] [Exploration and Exploitation Errors Are Measurable for Language Model Agents](https://arxiv.org/abs/2604.13151)
*Jaden Park,Jungtaek Kim,Jongwon Jeong,Robert D. Nowak,Kangwook Lee,Yong Jae Lee*

Main category: cs.AI

TL;DR: The paper introduces controllable environments to measure exploration vs exploitation in LM agents without access to internal policies, finding current models struggle and showing improvements through minimal engineering.


<details>
  <summary>Details</summary>
Motivation: LM agents need to balance exploration and exploitation in open-ended tasks, but current methods can't systematically quantify these behaviors from observed actions alone without policy access.

Method: Design controllable 2D grid environments with partially observable maps and unknown task DAGs, programmatically adjustable for exploration/exploitation difficulty, plus a policy-agnostic metric to quantify errors.

Result: State-of-the-art LM agents struggle on the task with distinct failure modes; reasoning models perform better, and both exploration/exploitation can be significantly improved through minimal harness engineering.

Conclusion: The framework enables systematic evaluation of exploration-exploitation tradeoffs in LM agents, revealing current limitations and engineering opportunities for improvement.
...
### [134] [SciFi: A Safe, Lightweight, User-Friendly, and Fully Autonomous Agentic AI Workflow for Scientific Applications](https://arxiv.org/abs/2604.13180)
*Qibin Liu,Julia Gonski*

Main category: cs.AI

TL;DR: A safe, lightweight agentic framework for autonomous execution of well-defined scientific tasks using isolated environments, three-layer agent loops, and self-assessing mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing agentic AI systems face challenges in reliable real-world scientific deployment, needing safer, more user-friendly solutions for autonomous scientific workflows.
...
Conclusion: The framework provides a practical solution for autonomous scientific task execution, supporting researchers in focusing on creative activities and open-ended inquiry.
...
TL;DR: LLMs exhibit unpredictable behavior due to floating-point numerical instability, with rounding errors causing chaotic "avalanche effects" in early layers and three distinct regimes of behavior depending on perturbation size.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly used in agentic workflows, their unpredictability from numerical instability has become a critical reliability issue. While studies show downstream effects, the root causes and mechanisms remain poorly understood.

Method: Rigorous analysis of how unpredictability stems from finite numerical precision of floating-point representations, tracking how rounding errors propagate, amplify, or dissipate through Transformer computation layers. Identifies chaotic "avalanche effect" in early layers.

Result: LLMs exhibit universal, scale-dependent chaotic behaviors with three regimes: 1) stable regime (perturbations vanish), 2) chaotic regime (rounding errors dominate), and 3) signal-dominated regime (true input variations override noise). Validated across multiple datasets and model architectures.

Conclusion: Numerical instability in LLMs is systematic and predictable, with chaotic behaviors rooted in floating-point precision limitations. Understanding these regimes is crucial for improving reliability in agentic workflows.
...
### [136] [Optimizing Earth Observation Satellite Schedules under Unknown Operational Constraints: An Active Constraint Acquisition Approach](https://arxiv.org/abs/2604.13283)
*Mohamed-Bachir Belaid*

Main category: cs.AI

TL;DR: Earth Observation satellite scheduling with unknown constraints using interactive learning to discover feasibility constraints from a binary oracle, achieving better performance with fewer queries.


<details>
  <summary>Details</summary>
Motivation: Real-world EO scheduling constraints are often embedded in engineering artefacts or simulators rather than explicit mathematical models, making traditional optimization approaches insufficient.

Method: Conservative Constraint Acquisition (CCA) embedded in Learn&Optimize framework - alternates optimization under learned constraint model with targeted oracle queries to identify justified constraints efficiently.

Result: L&O improves over greedy baseline (65-68% gap reduced to 17.7-35.8% for n≤30) and uses far fewer queries than acquire-then-solve baseline (21.3 vs 100 queries at n=50) with better average performance.

Conclusion: Interactive constraint learning approach effectively handles unknown constraints in EO scheduling, achieving practical efficiency with fewer oracle queries while maintaining solution quality.
...
### [137] [WebXSkill: Skill Learning for Autonomous Web Agents](https://arxiv.org/abs/2604.13318)
...
TL;DR: WebXSkill bridges the grounding gap in web agents by creating executable skills that combine parameterized action programs with natural language guidance, improving task success rates on web benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current web agents struggle with long-horizon workflows due to a grounding gap: textual workflow skills can't be executed directly, while code-based skills are opaque to agents, preventing step-level understanding for error recovery and adaptation.

Method: Three-stage framework: 1) Skill extraction mines reusable action subsequences from synthetic trajectories and abstracts them into parameterized skills, 2) Skill organization indexes skills into URL-based graph for context-aware retrieval, 3) Skill deployment offers grounded mode (fully automated execution) and guided mode (agent follows skills as step-by-step instructions).

Result: Improves task success rate by up to 9.8 points on WebArena and 12.9 points on WebVoyager over baseline, demonstrating effectiveness of executable skills for web agents.
...
### [138] [Listening Alone, Understanding Together: Collaborative Context Recovery for Privacy-Aware AI](https://arxiv.org/abs/2604.13348)
*Tanmay Srivastava,Amartya Basu,Shubham Jain,Vaishnavi Ranganathan*

Main category: cs.AI

TL;DR: CONCORD is a privacy-aware A2A framework that enables proactive speech AI assistants to collaborate safely by enforcing owner-only speech capture and recovering missing context through negotiated exchanges between assistants.


<details>
  <summary>Details</summary>
Motivation: As AI assistants evolve from reactive to always-listening proactive systems, they face significant privacy risks from capturing non-consenting speakers' speech, making social deployment challenging. There's a need for privacy-preserving proactive conversational agents.

Method: CONCORD implements owner-only speech capture via real-time speaker verification, producing privacy-preserving one-sided transcripts. It recovers missing context through: (1) spatio-temporal context resolution, (2) information gap detection, and (3) minimal A2A queries governed by relationship-aware disclosure policies, treating context recovery as negotiated safe exchanges between assistants rather than hallucination-prone inference.

Result: On a multi-domain dialogue dataset, CONCORD achieves 91.4% recall in gap detection, 96% relationship classification accuracy, and 97% true negative rate in privacy-sensitive disclosure decisions, demonstrating effective privacy-preserving context recovery.

Conclusion: By reframing always-listening AI as a coordination problem between privacy-preserving agents, CONCORD offers a practical path toward socially deployable proactive conversational agents that balance functionality with privacy protection.
...
### [139] [ReSS: Learning Reasoning Models for Tabular Data Prediction via Symbolic Scaffold](https://arxiv.org/abs/2604.13392)
*Chenlang Yi,Gang Li,Zizhan Xiong,Tue Minh Cao,Yanmin Gong,My T. Thai,Tianbao Yang*

Main category: cs.AI

TL;DR: ReSS is a framework that bridges symbolic and neural reasoning for tabular data by using decision trees to extract symbolic scaffolds, then fine-tuning LLMs to generate faithful natural-language reasoning that adheres to the underlying decision logic.


<details>
  <summary>Details</summary>
Motivation: Tabular data in high-stakes domains like healthcare and finance requires both high accuracy and human-understandable reasoning. Symbolic models lack expressiveness, while LLMs need specialized fine-tuning for domain-specific tabular reasoning. There's a need to address scalable data curation and reasoning consistency challenges.

Method: ReSS uses decision trees to extract instance-level decision paths as symbolic scaffolds. These scaffolds, along with input features and labels, guide an LLM to generate grounded natural-language reasoning. The resulting dataset fine-tunes a pretrained LLM into a specialized tabular reasoning model, enhanced by scaffold-invariant data augmentation for better generalization.
...
Conclusion: ReSS successfully bridges symbolic and neural reasoning, creating specialized tabular reasoning models that provide both high accuracy and faithful, human-understandable explanations, with quantitative metrics introduced to rigorously assess faithfulness.
...
### [140] [Quantifying and Understanding Uncertainty in Large Reasoning Models](https://arxiv.org/abs/2604.13395)
*Yangyi Li,Chenxu Zhao,Mengdi Huai*

Main category: cs.AI

TL;DR: Novel conformal prediction method for Large Reasoning Models that provides statistical guarantees for reasoning-answer uncertainty and identifies key training examples/steps via Shapley values.


<details>
  <summary>Details</summary>
Motivation: Traditional uncertainty quantification methods lack finite-sample guarantees for reasoning-answer generation in LRMs, ignore logical connections between reasoning traces and final answers, and fail to interpret uncertainty origins while disentangling reasoning quality from answer correctness.
...
Result: Extensive experiments on challenging reasoning datasets verify the effectiveness of the proposed methods in providing statistically rigorous uncertainty quantification and interpretable explanations.

Conclusion: The proposed approach successfully addresses limitations of existing CP methods for LRMs by providing statistical guarantees for reasoning-answer uncertainty while offering interpretable explanations through Shapley-based analysis of training examples and reasoning steps.
...
### [141] [Towards Scalable Lightweight GUI Agents via Multi-role Orchestration](https://arxiv.org/abs/2604.13488)
*Ziwei Wang,Junjie Zheng,Leyang Yang,Sheng Zhou,Xiaoxuan Tang,Zhouhua Fang,Zhiwei Liu,Dajun Chen,Yong Li,Jiajun Bu*

Main category: cs.AI

TL;DR: LAMO framework enables lightweight MLLMs to perform GUI automation through role-oriented data synthesis and two-stage training, achieving task scalability for resource-constrained devices.


<details>
  <summary>Details</summary>
Motivation: Lightweight GUI agents face deployment cost challenges on resource-constrained devices, with limited capacity and poor task scalability under end-to-end episodic learning, hindering adaptation to multi-agent systems and realistic GUI workflows.
...
Result: Developed LAMO-3B, a task-scalable native GUI agent supporting monolithic execution and MAS-style orchestration, which can continuously benefit from planner advances when paired with advanced planners as a plug-and-play policy executor.

Conclusion: LAMO effectively addresses the cost-scalability dilemma for lightweight GUI agents, enabling them to participate in realistic GUI workflows through multi-role orchestration and achieving validated performance through extensive static and online evaluations.
...
### [142] [RiskWebWorld: A Realistic Interactive Benchmark for GUI Agents in E-commerce Risk Management](https://arxiv.org/abs/2604.13531)
*Renqi Chen,Zeyin Tao,Jianming Guo,Jing Wang,Zezhou Xu,Jingzhe Zhu,Qingqing Sun,Tianyi Zhang,Shuai Chen*

Main category: cs.AI

TL;DR: RiskWebWorld is the first realistic interactive benchmark for evaluating GUI agents in e-commerce risk management, featuring 1,513 tasks from production pipelines and showing that foundation model scale matters more than interface grounding for professional tasks.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agent benchmarks focus on benign consumer environments, leaving high-stakes investigative domains like e-commerce risk management underexplored. There's a need for realistic evaluation in authentic risk-control scenarios with uncooperative websites.

Method: Created RiskWebWorld benchmark with 1,513 tasks from production risk-control pipelines across 8 domains, capturing authentic challenges. Built Gymnasium-compliant infrastructure that decouples policy planning from environment mechanics for scalable evaluation and agentic RL.

Result: Top-tier generalist models achieved 49.1% success, while specialized open-weights GUI models failed completely. Agentic RL improved open-source models by 16.2%, showing foundation model scale matters more than zero-shot interface grounding for professional tasks.

Conclusion: RiskWebWorld serves as a practical testbed for developing robust digital workers in e-commerce risk management, highlighting the importance of realistic benchmarks and showing that agentic RL can significantly improve model performance in professional domains.
...
### [143] [Weight Patching: Toward Source-Level Mechanistic Localization in LLMs](https://arxiv.org/abs/2604.13694)
*Chenghao Sun,Chengsheng Zhang,Guanzheng Qin,Rui Dai,Xinmei Tian*

Main category: cs.AI

TL;DR: Weight Patching is a parameter-space intervention method for analyzing model capabilities by transferring weights from specialized to base models, revealing hierarchical component roles in instruction following.


<details>
  <summary>Details</summary>
Motivation: Prior activation-space localization methods can't distinguish whether modules encode capabilities in their own parameters vs. merely aggregating upstream signals. Need parameter-space analysis to identify components that causally realize specific capabilities.

Method: Weight Patching replaces selected module weights from a behavior-specialized model into a base model under fixed inputs. Uses vector-anchor behavioral interface to assess task-relevant control state formation in open-ended generation.

Result: Reveals hierarchy from shallow source-side carriers to aggregation/routing modules to downstream execution circuits. Component scores guide mechanism-aware model merging, improving selective fusion across expert combinations.

Conclusion: Weight Patching enables parameter-space causal analysis of model capabilities, providing insights into component roles and enabling better model merging through mechanism-aware fusion.
...
### [144] [Rethinking AI Hardware: A Three-Layer Cognitive Architecture for Autonomous Agents](https://arxiv.org/abs/2604.13757)
*Li Chen*

Main category: cs.AI

TL;DR: Tri-Spirit Architecture decomposes AI intelligence into three layers (planning, reasoning, execution) mapped to different hardware, reducing latency by 75.6% and energy by 71.1% while cutting LLM calls by 30%.


<details>
  <summary>Details</summary>
Motivation: Current AI paradigms treat planning, reasoning, and execution as monolithic processes, causing unnecessary latency, energy consumption, and fragmented behavioral continuity across heterogeneous hardware.

Method: Three-layer cognitive framework (Super Layer for planning, Agent Layer for reasoning, Reflex Layer for execution) coordinated via asynchronous message bus, with parameterized routing, habit-compilation mechanism, convergent memory model, and explicit safety constraints.

Result: In simulation of 2000 synthetic tasks: 75.6% reduction in mean task latency, 71.1% reduction in energy consumption, 30% reduction in LLM invocations, and 77.6% offline task completion.

Conclusion: Cognitive decomposition, rather than model scaling alone, is a primary driver of system-level efficiency in AI hardware, enabling more efficient autonomous systems.
...
### [145] [The cognitive companion: a lightweight parallel monitoring architecture for detecting and recovering from reasoning degradation in LLM agents](https://arxiv.org/abs/2604.13759)
*Rafflesia Khan,Nafiul Islam Khan*

Main category: cs.AI

TL;DR: LLM agents fail on multi-step tasks up to 30% of the time. Cognitive Companion monitors agents in parallel with two implementations: LLM-based (52-62% less repetition, 11% overhead) and Probe-based (zero overhead, AUROC 0.840). Benefits are task-dependent, best for loop-prone/open-ended tasks.


<details>
  <summary>Details</summary>
Motivation: LLM agents suffer from reasoning degradation, looping, drift, and stuck states at high rates (up to 30%) on hard multi-step tasks. Current solutions have limitations: hard step limits are abrupt, and LLM-as-judge monitoring incurs significant overhead (10-15% per step).

Method: Introduces Cognitive Companion, a parallel monitoring architecture with two implementations: 1) LLM-based Companion that monitors agent behavior, and 2) Probe-based Companion using trained probes on hidden states from layer 28. Conducted three-batch feasibility study with Gemma 4 E4B, plus exploratory analysis on small models (Qwen 2.5 1.5B and Llama 3.2 1B).

Result: LLM-based Companion reduced repetition on loop-prone tasks by 52-62% with ~11% overhead. Probe-based Companion showed mean effect size +0.471 at zero inference overhead, with strongest probe achieving cross-validated AUROC 0.840. Benefits are task-type dependent: most helpful on loop-prone/open-ended tasks, neutral/negative on structured tasks. Small models (1B-1.5B) showed no improvement even with interventions.

Conclusion: This is a feasibility study showing sub-token monitoring may be useful. Task-type sensitivity is a practical design constraint. Selective companion activation is promising for future work. Results are encouraging but not definitive validation.
...
TL;DR: AlphaCNOT uses RL with Monte Carlo Tree Search to minimize CNOT gates in quantum circuits, achieving up to 32% reduction compared to baseline methods.


<details>
  <summary>Details</summary>
Motivation: Quantum circuit optimization is crucial for NISQ devices where error propagation scales with operation count. CNOT gates are particularly important as the only 2-qubit gate in the universal Clifford+T set, and current heuristic algorithms have limitations in minimizing them efficiently.

Method: AlphaCNOT is a reinforcement learning framework based on Monte Carlo Tree Search that models CNOT minimization as a planning problem. It's model-based and uses lookahead search to evaluate future trajectories, enabling more efficient CNOT sequence discovery.

Result: Achieves up to 32% reduction in CNOT gate count compared to PMH baseline for linear reversible synthesis, and consistent gate count reduction on various topologies with up to 8 qubits compared to state-of-the-art RL-based solutions.

Conclusion: The combination of RL with search-based strategies like MCTS is effective for CNOT minimization and can be extended to other circuit optimization tasks, potentially advancing quantum computing toward the "quantum utility" era.
...
### [147] [GeoAgentBench: A Dynamic Execution Benchmark for Tool-Augmented Agents in Spatial Analysis](https://arxiv.org/abs/2604.13888)
*Bo Yu,Cheng Yang,Dongyang Hou,Chengfu Liu,Jiayao Liu,Chi Wang,Zhiming Zhang,Haifeng Li,Wentao Yang*

Main category: cs.AI

TL;DR: GeoAgentBench (GABench) is a dynamic benchmark for evaluating LLM-based GIS agents with 117 tools across 53 tasks, featuring new metrics and a Plan-and-React architecture that outperforms traditional approaches.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based GIS agent evaluation is inadequate because it uses static text/code matching without dynamic runtime feedback and doesn't account for multimodal spatial outputs, making it hard to assess real-world performance.

Method: Created GeoAgentBench with 117 atomic GIS tools across 6 domains; introduced Parameter Execution Accuracy (PEA) metric with "Last-Attempt Alignment"; developed VLM-based verification for spatial accuracy; designed Plan-and-React agent architecture that separates global planning from reactive execution.
...
Conclusion: GABench establishes a robust standard for evaluating autonomous GeoAI, highlights current capability boundaries, and shows that the Plan-and-React paradigm effectively addresses parameter misalignment and runtime anomaly challenges in GIS workflows.
...
### [148] [AI-Assisted Peer Review at Scale: The AAAI-26 AI Review Pilot](https://arxiv.org/abs/2604.13940)
...
TL;DR: First large-scale deployment of AI-assisted peer review at AAAI-26 conference, showing AI reviews were preferred over human reviews on technical accuracy and research suggestions.


<details>
  <summary>Details</summary>
Motivation: Scientific peer review is strained by increasing submission volumes, making it difficult to maintain quality, consistency, and timeliness. The community is exploring AI assistance but needs to determine if AI can generate technically sound reviews at real-world conference scale.

Method: Deployed AI review system for all 22,977 AAAI-26 main-track submissions using frontier models, tool use, and safeguards in a multi-stage process. Conducted large-scale survey of authors and program committee members, and created novel benchmark to evaluate AI review quality.

Result: AI reviews were generated for all papers in less than a day. Survey participants preferred AI reviews over human reviews on key dimensions including technical accuracy and research suggestions. The system substantially outperformed simple LLM-generated review baselines at detecting scientific weaknesses.

Conclusion: State-of-the-art AI methods can already make meaningful contributions to scientific peer review at conference scale, opening a path toward synergistic human-AI teaming for evaluating research.
...
### [149] [[Emerging Ideas] Artificial Tripartite Intelligence: A Bio-Inspired, Sensor-First Architecture for Physical AI](https://arxiv.org/abs/2604.13959)
*You Rim Choi,Subeom Park,Hyung-Sin Kim*

Main category: cs.AI

TL;DR: ATI is a bio-inspired sensor-first architecture for physical AI that co-designs sensing and inference through a tripartite system, improving accuracy by 34.2% while reducing cloud inference by 43.3%.


<details>
  <summary>Details</summary>
Motivation: Physical AI faces tight constraints (latency, energy, privacy, reliability) that require more than just scaling model size. Performance depends on how signals are acquired through controllable sensors in dynamic environments, necessitating co-design of sensing and inference.

Method: Artificial Tripartite Intelligence (ATI) - a bio-inspired, sensor-first architectural contract with three levels: Brainstem (L1) for reflexive safety/signal-integrity control, Cerebellum (L2) for continuous sensor calibration, and Cerebral Inference Subsystem (L3/L4) for skill selection, coordination, and deep reasoning. This modular design keeps time-critical sensing on-device while invoking higher-level inference only when needed.

Result: In mobile camera prototype under dynamic lighting/motion: ATI (L1/L2 adaptive sensing) improved end-to-end accuracy from 53.8% to 88% (+34.2%) compared to default auto-exposure, while reducing remote L4 invocations by 43.3%.

Conclusion: Co-designing sensing and inference is valuable for embodied AI. ATI's tripartite architecture enables sensor control, adaptive sensing, edge-cloud execution, and foundation model reasoning to co-evolve within one closed-loop system, addressing physical AI constraints while maintaining performance.
...
### [150] [Reward Design for Physical Reasoning in Vision-Language Models](https://arxiv.org/abs/2604.13993)
*Derek Lilienthal,Manisha Mukherjee,Sameera Horawalavithana*

Main category: cs.AI

TL;DR: Systematic reward ablation study for GRPO-based VLM training shows reward design induces domain-specific reasoning behaviors rather than uniform improvements, with accuracy-based rewards providing strongest overall gains.


<details>
  <summary>Details</summary>
Motivation: Current VLMs fall short on physical reasoning benchmarks, and while post-training algorithms like SFT and GRPO show promise, how reward design shapes VLM physical reasoning behavior remains poorly understood.

Method: Systematic reward ablation study comparing four reward signals: format compliance, answer accuracy, composite rubric reward (correctness, physics principle identification, unit consistency), and novel internal attention-weight reward derived from model attention over input images. Evaluated on PhyX benchmark using IBM Granite Vision 3.3 (2B).

Result: GRPO with accuracy-based rewards outperforms SFT on most domains, but gains vary by reward type and domain. Accuracy rewards provide strongest overall gains. Rubric rewards improve structured reasoning quality without consistent accuracy improvements. Attention-based rewards enhance spatial reasoning (improving spatial relation accuracy from 0.27 to 0.50) while degrading symbolic domain performance.

Conclusion: Reward design induces domain-specific reasoning behaviors rather than uniform improvements. Supervising where models attend during generation (via attention-weight rewards) is promising for visually grounded physical reasoning, requiring no spatial annotations.
...
### [151] [Memory Transfer Learning: How Memories are Transferred Across Domains in Coding Agents](https://arxiv.org/abs/2604.14004)
*Kangsan Kim,Minki Kang,Taeil Kim,Yanlai Yang,Mengye Ren,Sung Ju Hwang*

Main category: cs.AI

TL;DR: Memory Transfer Learning (MTL) enables coding agents to share memory across heterogeneous domains, improving performance by 3.7% on average through transfer of meta-knowledge like validation routines rather than task-specific code.


<details>
  <summary>Details</summary>
Motivation: Existing memory-based self-evolution approaches for coding agents are limited to homogeneous task domains, failing to leverage shared infrastructural foundations (runtime environments, programming languages) that exist across diverse real-world coding problems.

Method: Investigates Memory Transfer Learning (MTL) using a unified memory pool from heterogeneous domains. Evaluates across 6 coding benchmarks with four memory representations ranging from concrete traces to abstract insights.

Result: Cross-domain memory improves average performance by 3.7%, primarily transferring meta-knowledge (validation routines) rather than task-specific code. Abstraction dictates transferability: high-level insights generalize well, low-level traces cause negative transfer. Transfer effectiveness scales with memory pool size, and memory can be transferred between different models.

Conclusion: Establishes empirical design principles for expanding memory utilization beyond single-domain silos, showing that abstraction level is crucial for effective cross-domain memory transfer in coding agents.
...
### [152] [Hierarchical Reinforcement Learning with Runtime Safety Shielding for Power Grid Operation](https://arxiv.org/abs/2604.14032)
*Gitesh Malik*
...
<details>
  <summary>Details</summary>
Motivation: RL for power-grid operation faces deployment challenges due to strict safety requirements, brittleness under rare disturbances, and poor generalization to unseen grid topologies. Safety-critical infrastructure cannot tolerate catastrophic failures.

Method: Safety-constrained hierarchical control framework decouples long-horizon decision-making from real-time feasibility enforcement. High-level RL policy proposes abstract actions, while deterministic runtime safety shield filters unsafe actions using fast forward simulation.
...
Conclusion: Safety and generalization in power-grid control are best achieved through architectural design rather than complex reward engineering, providing practical path toward deployable learning-based controllers for real-world energy systems.
...
### [153] [TREX: Automating LLM Fine-tuning via Agent-Driven Tree-based Exploration](https://arxiv.org/abs/2604.14116)
*Zerun Ma,Guoqiang Wang,Xinchen Xie,Yicheng Chen,He Du,Bowen Li,Yanan Sun,Wenran Liu,Kai Chen,Yining Li*

Main category: cs.AI

TL;DR: TREX is a multi-agent system that automates the entire LLM training lifecycle, from requirement analysis to model evaluation, using a search tree approach for efficient experimentation.


<details>
  <summary>Details</summary>
Motivation: While LLMs have enabled AI agents to perform isolated scientific tasks, automating complex real-world workflows like LLM training remains challenging. Current systems lack the ability to handle the entire training lifecycle autonomously.

Method: TREX uses a multi-agent system with two core modules: Researcher and Executor. It orchestrates collaboration to perform requirement analysis, literature/data research, training strategy formulation, data recipe preparation, and model training/evaluation. The experimental process is modeled as a search tree for efficient planning, historical result reuse, and insight distillation.

Result: The system was evaluated using FT-Bench, a benchmark with 10 real-world tasks ranging from optimizing fundamental capabilities to domain-specific performance. Experimental results show that TREX consistently optimizes model performance on target tasks.

Conclusion: TREX successfully automates the complete LLM training lifecycle, demonstrating effective optimization of model performance through its multi-agent architecture and search tree approach to experimental planning.
