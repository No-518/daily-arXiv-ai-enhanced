...
TL;DR: PhysX-Omni is a unified framework for generating simulation-ready physical 3D assets across diverse categories (rigid, deformable, articulated objects), featuring a novel geometry representation for Vision-Language Models, a new dataset (PhysXVerse), and a comprehensive evaluation benchmark (PhysX-Bench).


<details>
  <summary>Details</summary>
Motivation: Existing 3D generation methods either neglect physical properties or are limited to a single asset category (rigid, deformable, or articulated objects), lacking a unified solution for simulation-ready physical 3D generation.

Method: (1) A novel geometry representation that directly encodes high-resolution 3D structures without compression for Vision-Language Models; (2) Construction of PhysXVerse, the first general simulation-ready 3D dataset; (3) PhysX-Bench for evaluating six attributes: geometry, absolute scale, material, affordance, kinematics, and function description.

Result: PhysX-Omni performs strongly in both generation and understanding tasks on conventional metrics and PhysX-Bench; validated potential for simulation-ready scene generation and robotic policy learning.

Conclusion: PhysX-Omni can significantly advance downstream applications, particularly in embodied AI and physics-based simulation.
...
TL;DR: Lens is a 3.8B-parameter T2I model that competes with 6B+ models using 19.3% of the training compute, achieved via dense captions (109-word avg), multi-resolution batching, semantic VAE, strong language encoder, RL fine-tuning, and distillation for 4-step inference.


<details>
  <summary>Details</summary>
Motivation: To build a high-quality text-to-image model that is computationally efficient during training while achieving state-of-the-art performance, challenging the trend of scaling model size without regard for efficiency.

Method: (1) Densely captioned dataset (Lens-800M, GPT-4.1 captions, ~109 words avg); (2) multi-resolution, diverse-aspect-ratio batching; (3) semantic VAE + strong language encoder; (4) RL with taxonomy-driven prompts and reward rubrics; (5) reasoner module with training-free system prompt search; (6) distillation for 4-step inference.

Result: Competitive or surpassing 6B+ models on benchmarks; 19.3% training compute of Z-Image; 1024² image in 3.15s (H100); distilled version in 0.84s (4-step); supports arbitrary aspect ratios 1:2~2:1, resolutions up to 1440², and multilingual prompts.

Conclusion: Compact model design combined with data efficiency, architectural choices, and post-training optimization can achieve or exceed the performance of much larger models at a fraction of the training cost.
...
TL;DR: GenEvolve is a self-evolving framework for open-ended image generation that uses Tool-Orchestrated Visual Experience Distillation to improve an agent's ability to combine internal generative capabilities with external tools through trajectory learning and dense token-level supervision.


<details>
  <summary>Details</summary>
Motivation: Open-ended image generation requires more than simple prompt-to-image mapping; there is a need for agents that can self-evolve and effectively use external tools across diverse and demanding generation requests.

Method: GenEvolve models each generation attempt as a tool-orchestrated trajectory, compares multiple trajectories for the same request, abstracts best-worst differences into structured visual experience via a privileged teacher branch, and uses Visual Experience Distillation (inspired by on-policy self-distillation) to provide dense token-level supervision for the student model.

Result: Achieves substantial gains over strong baselines and state-of-the-art performance on public benchmarks and the authors' constructed GenEvolve-Bench.

Conclusion: GenEvolve demonstrates that self-evolving through trajectory comparison and token-level distillation significantly improves tool-use effectiveness and generation quality in open-ended image generation tasks.
...
TL;DR: UniVL is a spatially grounded image generation model that replaces separate text and image encoders with a unified visual input containing rendered text instructions, achieving better quality (FID 14→11, PSNR 16→20) while reducing inference TFLOPs by 52% and runtime by 44%.


<details>
  <summary>Details</summary>
Motivation: Existing contextual image generation relies on two separate encoders (vision + language/global text prompt), which is computationally expensive and complex. The authors aim to simplify this by binding semantics to spatial locations directly from a single unified visual input.

Method: UniVL uses an OCR-pretrained backbone to optically read a unified input (text rendered onto spatial mask), producing fVIL embeddings that fuse visual/semantic intent with spatial locations. A two-stage pipeline aligns with VAE space and conditions a pretrained diffusion backbone, eliminating the standalone text encoder (e.g., T5).

Result: On the UniVL-ImgGen benchmark (477K mask-annotated images), UniVL improves FID from 14→11 and PSNR from 16→20, reduces inference TFLOPs by up to 52% and runtime by up to 44%.

Conclusion: UniVL demonstrates that unified conditioning (text rendered on spatial masks) can outperform two-encoder baselines while being more efficient, paving the way for spatially grounded image generation without a standalone text encoder.
...
TL;DR: Flat-Pack Bench is a new benchmark evaluating LVLMs on fine-grained spatio-temporal video understanding through furniture assembly tasks, revealing that current models struggle with temporal reasoning, tracking, and spatial interaction understanding.


<details>
  <summary>Details</summary>
Motivation: Existing video understanding benchmarks focus on coarse-grained tasks (action segmentation, classification, captioning, retrieval) and rely on easily identifiable entities, failing to evaluate fine-grained spatio-temporal reasoning needed for complex real-world applications like furniture assembly and cooking.

Method: The authors introduce Flat-Pack Bench, a benchmark centered on furniture assembly tasks, with multiple-choice questions covering temporal ordering of assembly actions, temporal localization of assembly state, part mating understanding, and tracking, using visual prompts to highlight relevant parts.

Result: State-of-the-art LVLMs perform poorly on Flat-Pack Bench, struggling significantly with fine-grained spatio-temporal reasoning tasks.

Conclusion: Current LVLMs have fundamental limitations in leveraging temporal video information, tracking objects, and understanding spatial interactions like physical contact, highlighting the need for improved fine-grained video understanding capabilities.
...
TL;DR: Vision-language models show accuracy gains from latent tokens, but these gains persist even when token content is corrupted, indicating the tokens aren't actually used for reasoning—just their presence matters.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of latent/continuous tokens in VLMs only measure task accuracy, which is insufficient to prove the tokens are genuinely used for reasoning; gains may come from confounds like added context length or regularization.

Method: The authors propose Ablate-to-Validate, instantiated as the Token Replacement Test (TRT), which replaces intermediate latent tokens with zero, random, first-repeat, or oracle alternatives while keeping prompt, image, token budget, and decoding fixed. They study LLaVA-13B and Qwen2.5-VL-3B on depth reasoning, and also test off-the-shelf systems (Mirage, Mull-Tokens, CoVT).

Result: Across all settings, accuracy gains persist even when token content is corrupted or replaced, showing that VLMs retain most improvement without genuinely using the tokens for reasoning—revealing a gap between having a latent channel and using it as an information bottleneck.

Conclusion: Accuracy alone is a misleading proxy for latent-token reasoning; TRT should be used as a standard diagnostic alongside accuracy for any method introducing continuous thought tokens.
...
TL;DR: Proposes Zoom-then-Diagnose + uncertainty-aware GRPO framework that improves VLM performance on ultrasound by mimicking sonographers' lesion-focused workflow and handling annotation subjectivity.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs fail to explicitly zoom into lesion regions before diagnosis and treat all annotations as unbiased ground truths, ignoring the inherent subjectivity and ambiguity in ultrasound interpretation.

Method: (1) Zoom-then-Diagnose paradigm that replicates interactive search for lesion-focused reasoning; (2) Uncertainty-aware reward within GRPO using stochastic group-wise rollouts to estimate confidence, encouraging caution under ambiguity.

Result: Lesion localization improved by 39.3% across liver, breast, and thyroid ultrasound datasets.

Conclusion: The model learns to actively "look closer and diagnose," aligning with sonographers' cognitive workflow and handling ambiguous cases more effectively.
...
TL;DR: MRecover is a conditional generative model that synthesizes TSE-like MRI images from T1w images to recover motion-corrupted hippocampal subfield segmentation data, improving analyzable subject yield by 31.8% and increasing statistical power for detecting atrophy differences.


<details>
  <summary>Details</summary>
Motivation: Hippocampal subfield segmentation requires high-resolution T2w TSE MRI, which is highly susceptible to motion artifacts leading to substantial data loss.

Method: A conditional generative model (MRecover) that synthesizes T1w images into TSE images using autoregressive slice conditioning for volumetric consistency, trained on 7T MRI data (n=577).

Result: High in-domain fidelity (SSIM=0.84, FSIM=0.94), excellent out-of-domain generalization to 3T (r=0.87-0.97), 31.8% more analyzable subjects (593 vs 450), and larger effect sizes for hippocampal subfield atrophy detection.

Conclusion: MRecover effectively recovers motion-corrupted TSE MRI data, improving sample utility and statistical power for hippocampal subfield analysis.
...
TL;DR: AVI-HT is an adaptive visual-IMU fusion method for 3D hand tracking that uses cross-sensor attention to handle visual occlusion during hand-object interactions, reducing keypoint error by 16-24% over baselines.


<details>
  <summary>Details</summary>
Motivation: Accurate 3D hand tracking during hand-object interaction (HOI) is challenging due to heavy visual occlusion, which degrades vision-only approaches. The authors aim to improve tracking robustness by fusing egocentric vision with on-glove IMU signals through adaptive sensor fusion.

Method: AVI-HT jointly models egocentric images with 6-DoF IMU signals from on-glove sensors, using synchronized multi-modal training data (100K+ samples from DexGloveHOI dataset) and a cross-sensor deep attention mechanism that adaptively weights vision and individual IMU sensors based on trust.

Result: Reduces mean keypoint error by 16.1% (24.2% for wrist-aligned variant) over multiple single/multi-modal baselines on two hand models (UmeTrack, MANO). Ablation studies reveal per-finger IMU contributions and sensitivity to noise/temporal misalignment.

Conclusion: AVI-HT significantly improves 3D hand tracking accuracy and availability under occlusion-heavy HOI scenarios, with adaptive attention being key to robust multi-modal fusion.
...
TL;DR: Introduces a lightweight cross-encoder metric for image captioning evaluation that uses VQA model initialization and adversarial LLM-based data augmentations, achieving SOTA performance with computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics either rely on expensive LLM judges or suffer from CLIP-based encoder limitations (token limits, lack of fine-grained sensitivity, compositional generalization issues).

Method: Lightweight cross-encoder initialized from a VQA model checkpoint; supervised learning with adversarial LLM-based data augmentations; a new benchmark for detailed captioning evaluation.

Result: State-of-the-art performance while maintaining efficiency suitable for large-scale benchmarking, quality-aware decoding, or reward guidance.

Conclusion: The proposed metric balances strong evaluation capability with computational practicality, addressing key limitations of existing metrics.
...
TL;DR: This paper uses Vision Language Models (VLMs) to improve 3D vehicle labeling for self-driving applications by inferring vehicle make, model, and generation from image crops, and outputting accurate 3D bounding box dimensions. The approach reduces manual labeling time while improving label quality, especially in cases of significant vehicle occlusion.


<details>
  <summary>Details</summary>
Motivation: To improve 3D vehicle labeling quality and efficiency in self-driving applications by leveraging VMMR methods and VLMs, particularly addressing failure modes where VLMs provide better dimensions than lidar-aided human annotations (e.g., under occlusion).

Method: Uses a Vision Language Model (VLM) to infer vehicle make, model, and generation from image crops, and outputs accurate 3D bounding box dimensions for seeding manual labeling. Evaluates iterative prompt engineering and different VLM choices on both bounding box inference and make/model/generation recognition.

Result: High accuracy compared to strong baselines; excels at mitigating specific failure modes (e.g., vehicle occlusion); reduces manual labeling time while increasing label quality; conclusions are generalizable across different labelers and datasets.

Conclusion: Integrating VLMs into the labeling process can reduce manual labeling time while increasing label quality in 3D vehicle labeling for self-driving applications.
...
TL;DR: BodyReLux is a video diffusion framework for temporally consistent relighting of full-body human performances, trained on a hybrid dataset combining static OLAT captures and novel interleaved dynamic captures, using lighting token conditioning and masked attention for accurate control.


<details>
  <summary>Details</summary>
Motivation: Relighting human performances is a fundamental task for post-production and content creation, but existing methods lack temporal consistency and robustness for dynamic full-body performances.

Method: 1) Hybrid dataset: combines static OLAT captures with novel interleaved dynamic captures (two flicker-free lighting sequences alternating above human perception threshold). 2) Video diffusion model fine-tuned from pretrained text-to-video model. 3) Lighting conditioning via token representation per light source. 4) Masked attention for sequential lighting control. 5) Data augmentation pipeline for robustness.

Result: Achieves photorealistic, robust, and temporally consistent video relighting of subject-specific human performances.

Conclusion: BodyReLux demonstrates that combining hybrid capture data with generative video diffusion priors and token-based lighting conditioning enables high-quality, temporally coherent human relighting.
...
TL;DR: SceneGraphGrounder reformulates zero-shot 3D visual grounding as structured graph matching by lifting 2D VLM-inferred object relationships into a persistent 3D scene graph, enabling interpretable and multi-view consistent reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot 3D grounding methods using VLMs suffer from view-dependent reasoning and implicit representations, limiting spatial consistency and interpretability for compositional queries.

Method: (1) Visual marker prompting strategy enables a VLM to infer object-object relationships from 2D views. (2) These relationships are lifted into a persistent 3D scene graph encoding spatial and semantic relations. (3) A query graph is constructed from the user query and aligned with the scene graph via constrained matching.

Result: Competitive zero-shot performance on ScanRefer benchmark using only RGB-D inputs; validated via real-world robot deployment with robust spatial reasoning in long-horizon environments.

Conclusion: Graph-based structured matching over a reconstructed 3D scene graph improves spatial consistency and interpretability for zero-shot 3D visual grounding, with practical applicability on mobile robots.
...
TL;DR: This paper introduces a benchmark for referential communication in dynamic 3D environments (6.7 hours of VR interaction data) and a two-stage grounding pipeline that decouples linguistic ambiguity resolution from visual localization, achieving 11-22% improvement over end-to-end baselines.


<details>
  <summary>Details</summary>
Motivation: Current VLMs excel at static image tasks but fail to resolve ambiguous referring expressions in spontaneous, multi-turn dialogue, limiting their ability to ground language in dynamic physical environments.

Method: (1) A benchmark built from 6.7 hours of egocentric VR interaction with synchronized speech, motion, gaze, and 3D scene data, containing 4,200+ referring expressions; (2) A two-stage pipeline that first rewrites ambiguous references contextually, then performs visual grounding (e.g., GroundingDINO).

Result: Contextual rewriting improves grounding by 11-22 percentage points; GroundingDINO reaches 56.7% on pronominals after rewriting, nearly doubling the best end-to-end baseline.

Conclusion: Decoupling linguistic reasoning from visual perception is more effective than end-to-end approaches for conversational grounding in dynamic environments.
...
TL;DR: This paper introduces Seizure-Semiology-Suite, a dataset and benchmark for evaluating Multimodal LLMs on seizure semiology understanding, revealing weaknesses in clinical reasoning and showing that fine-tuning and neuro-symbolic frameworks improve performance significantly.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs excel at general video understanding but have not been tested on interpreting involuntary, spatio-temporally evolving pathologic motor behaviors like seizure semiology, leaving a critical gap in safety-critical medical AI evaluation.

Method: The authors constructed Seizure-Semiology-Suite with 438 seizure videos (35k+ labels, 20 ILAE-defined features), proposed a 7-task hierarchical benchmark (perception→temporal→report→diagnosis), introduced Seizure-RQI metric, tested 11 open-weight MLLMs, and applied seizure-specific fine-tuning plus a two-stage neuro-symbolic framework.

Result: MLLMs showed systematic weaknesses in laterality reasoning, temporal localization, symptom sequencing, and clinical report fidelity. Seizure-specific fine-tuning substantially improved all tasks. The two-stage neuro-symbolic framework achieved an F1 score of 0.96 on epileptic vs. non-epileptic seizure classification.
...
TL;DR: CrossVLA applies DPO preference alignment to continuous-action flow-matching VLAs (not just autoregressive), introduces a surrogate log-probability estimator for DPO, compares LoRA vs DoRA (DoRA wins by +10.4pp), and analyzes inference bottlenecks (denoise loop dominates; KV-cache acceleration is limited).


<details>
  <summary>Details</summary>
Motivation: Preference alignment via DPO has been studied almost exclusively on autoregressive VLAs; continuous-action flow-matching VLAs lack DPO support, and there is no head-to-head comparison of parameter-efficient fine-tuning methods or inference-time anatomy for this paradigm.

Method: (i) Surrogate flow-matching log-probability estimator enabling DPO on continuous-action backbones; (ii) comparison of LoRA vs DoRA for VLA DPO on LIBERO; (iii) inference-time anatomy of denoise loop and KV-cache strategies; (iv) multi-view + temporal projection head pretrained on 6000 LIBERO frames.

Result: DoRA DPO improves over OpenVLA SFT by +10.4pp mean across LIBERO; Object suite achieves +20.0pp with zero seed variance. Denoise loop accounts for 78.6% of latency; KV-cache acceleration caps at 21%; chunk/token-level cache degrades success rate to 0-80%. k-NN recall@1 reaches 99.5% for same-task retrieval.

Conclusion: DPO can be effectively extended to continuous-action flow-matching VLAs; DoRA is superior to LoRA for VLA preference alignment; inference bottlenecks are concentrated in the denoise loop with limited KV-cache acceleration; multi-view temporal pretraining provides strong downstream initialization.
...
TL;DR: DEX proposes a modular network with specialized experts and a director to handle Non-IID feature statistics across heterogeneous medical imaging modalities, trained on a new 4M-image, 10-modality benchmark, showing improved downstream transfer on 26 tasks.


<details>
  <summary>Details</summary>
Motivation: Monolithic self-supervised learning on multi-modality medical data suffers from conflicting gradients and representation collapse toward modality-dominant shortcuts due to Non-IID feature statistics.

Method: DEX uses stacked modules, each with a pool of experts (image-wise activation for modality specialization) and a director (group EMA for shared semantic space), trained on the Medical Vision Universe benchmark (4M images, 10 modalities).

Result: Improved optimization behavior and transferability on 26 downstream tasks, demonstrating that DEX is a principled step toward general-purpose multi-modality medical AI.

Conclusion: Explicitly regulating the specialization-coordination balance in modular networks effectively addresses multi-modality representation collapse, paving the way for general-purpose medical foundation models.
...
TL;DR: This paper presents a staged multimodal framework for predicting six emotion intensity dimensions from video, achieving 3rd place in the Hume-ABAW10 EMI Challenge with a 0.57 Pearson correlation on the test set using text, audio, vision, and optional motion modalities.


<details>
  <summary>Details</summary>
Motivation: To predict continuous emotion intensity dimensions (Admiration, Amusement, Determination, Empathic Pain, Excitement, Joy) from in-the-wild multimodal video clips for the Hume-ABAW10 Emotional Mimicry Intensity Challenge.

Method: A staged multimodal framework that independently trains modality-specific encoders (textual, acoustic, visual, optional motion), then fuses representations via a lightweight regressor with modality dropout and controlled encoder adaptation.

Result: Best validation achieved 0.4722 average Pearson correlation with text-audio-vision-motion fusion; test set achieved 0.57 average Pearson correlation, placing 3rd in the challenge.

Conclusion: The multimodal fusion approach provides a practical and reproducible baseline for EMI prediction; motion branch offers slight gains but its behavior warrants further study.
...
TL;DR: Thermo-VL augments a frozen Molmo-7B VLM with a trainable thermal encoder and text-guided dual-attention fusion module to improve vision-language reasoning under low illumination by incorporating thermal infrared information.


<details>
  <summary>Details</summary>
Motivation: VLMs fail under low light because their visual grounding relies on RGB imagery; thermal infrared preserves complementary scene structure when visible cues degrade.

Method: Augments frozen Molmo-7B with a trainable thermal encoder and text-guided dual-attention fusion module that conditions thermal features on both language and RGB context, injecting gated residual into the frozen RGB stream. Trained with language-modeling objective plus auxiliary alignment and regularization losses.

Result: Strong gains on thermal-only and RGB+thermal reasoning tasks, demonstrating value of prompt-conditioned multispectral fusion.

Conclusion: Prompt-conditioned multispectral fusion effectively improves VLM performance under low illumination; dataset and code are publicly available.
...
TL;DR: FlexiCT is a family of CT foundation models trained on 266,227 CT volumes from 56 public datasets via agglomerative continual pretraining (2D axial, 3D anatomical, report-guided semantic alignment), achieving state-of-the-art performance across segmentation, classification, registration, vision-language understanding, and clinical retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: Current CT-based AI is fragmented, relying on task-specific models for segmentation, classification, registration, and report analysis. A unified foundation model is needed to consolidate these capabilities.

Method: Agglomerative continual pretraining across three stages: (1) 2D axial pretraining, (2) 3D anatomical pretraining, and (3) report-guided semantic alignment. Trained on 266,227 CT volumes from 56 public datasets.

Result: Matches or exceeds prior task-specific approaches across five downstream task families. Embeddings organize CT scans along gradients associated with various tumor stages, capturing disease-relevant imaging features.

Conclusion: FlexiCT demonstrates that CT foundation models can unify multiple medical imaging tasks and capture phenotype-relevant representations, serving as a large-scale public resource for CT representation learning.
...
TL;DR: RTS is a reward-guided trajectory scaling method for diffusion models that outperforms baselines by up to 60.4% on ImageReward score through active noise optimization and sparse test-time scaling with PCA-driven curvature analysis.


<details>
  <summary>Details</summary>
Motivation: Existing Test-Time Scaling (TTS) methods for diffusion models are limited to static, pre-defined noise pools and lack flexible noise exploration across the denoising trajectory, restricting generative performance.

Method: Two core innovations: 1) reward-guided noise optimization strategy for directing search toward promising regions; 2) sparse test-time scaling framework with PCA-driven curvature analysis to prioritize key intermediate steps and compress the search space.

Result: Outperforms baselines by 15.6% on GenEval Score and 60.4% on ImageReward score, achieving new SOTA.

Conclusion: RTS provides a practical guideline for more effective test-time scaling across diffusion-specific architectures, fully unlocking generative potential.
...
TL;DR: A multi-scale interaction network for stereo image super-resolution that improves intra-view and cross-view feature extraction using multi-scale attention and optimal transport-based epipolar matching, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Previous stereo SR methods have not fully exploited intra-view and cross-view information in binocular systems.

Method: Proposes a Multi-scale Spatial-Channel Attention Module (MSCAM) for intra-view feature extraction and a Dual-View Epipolar Attention Module (DVEAM) using optimal transport for accurate cross-view epipolar matching.

Result: Achieves competitive results outperforming most SOTA methods in extensive experiments.

Conclusion: The multi-scale interaction network effectively leverages both intra-view and cross-view information for stereo image super-resolution.
...
TL;DR: MAVEN is a multi-stage agentic pipeline that automatically generates structured video event annotations with Chain-of-Thought reasoning, achieving state-of-the-art results on traffic video reasoning tasks and surpassing Gemini models.


<details>
  <summary>Details</summary>
Motivation: High-quality structured video annotations (what, when, where, why, consequences) are critical for VLMs but manual labeling cannot scale; existing methods lack automated fine-grained annotation generation.

Method: MAVEN uses a multi-stage agentic pipeline: (1) generates Multi-Scale Spatio-Temporal Event Description (MSTED) from three caption levels, (2) synthesizes diverse Q&A formats, (3) supports agent-driven domain adaptation with automatic prompt redesign, and (4) employs hierarchical refinement loop that classifies errors and traces root causes to rewrite prompts or modify pipeline structure.

Result: On 5,300 traffic videos, fine-tuned Cosmos-Reason2-8B surpasses Gemini 2.5 Pro and 3.1 Flash (+38.8 MCQ gain). On AccidentBench, CCTV-only training lifts +10.7 MCQ points matching Gemini 2.5 Pro; RL post-training surpasses both Gemini baselines.

Conclusion: MAVEN enables scalable, high-quality video annotation generation with automatic domain adaptation and iterative self-improvement, outperforming strong commercial baselines across multiple benchmarks and domains.
...
TL;DR: The paper introduces SDGBiasBench, a large-scale benchmark (500k QA + 50k regression tasks) for detecting SDG-related biases in Vision-Language Models, and proposes CADE, a training-free debiasing method that improves accuracy by up to 25% and reduces regression error by up to 12 points.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate qualitative and quantitative SDG monitoring in isolation, failing to detect systematic biases where models substitute priors for visual evidence. Real-world SDG assessment requires multi-step reasoning over diverse cues, but hidden biases from incomplete evidence use remain unexamined.

Method: SDGBiasBench includes 500k expert-involved multiple-choice questions and 50k regression tasks. CADE (Contrastive Adaptive Debias Ensemble) is a training-free, plug-and-play method that uses modality-specific answer priors to mitigate intrinsic SDG bias in VLMs.

Result: Evaluations reveal an intrinsic SDG bias in current VLMs—predictions driven by SDG-specific priors rather than reliable multimodal cues. CADE improves multiple-choice accuracy by up to 25% and reduces regression MAE by up to 12 points across multiple VLMs.

Conclusion: The work provides a benchmark and debiasing method to foster fairer and more reliable AI systems for sustainable development, revealing and mitigating hidden biases in SDG-oriented vision-language reasoning.
...
TL;DR: VA-OPD improves on-policy knowledge distillation for VLMs by using visual advantage (VA) to weight tokens differently—focusing on vision-critical tokens rather than diluting supervision among abundant language tokens—yielding consistent gains across math and visual benchmarks with varying teacher sizes.


<details>
  <summary>Details</summary>
Motivation: Standard on-policy distillation for VLMs improves output quality but fails to strengthen the student's reliance on visual input; the student's predictions on vision-critical tokens remain largely unchanged regardless of visual detail availability.

Method: Introduces visual advantage (VA)—token-level log-probability difference when a teacher scores a student rollout with vs. without fine-grained visual detail. Uses VA at two granularities: rollout-level reweighting by trajectory-averaged VA, and token-level KL averaged within high-VA and low-VA groups separately.

Result: VA-OPD outperforms standard on-policy distillation on all eight benchmarks, with gains increasing monotonically with both teacher size (4B, 8B, 32B) and data scale, showing consistent compounding effects.

Conclusion: VA-OPD effectively addresses the visual reliance gap in VLM distillation by treating vision-critical tokens differently, and its benefits scale with larger teachers and more data.
...
TL;DR: EvoVid is a temporal-centric self-evolving framework that improves Video-LLMs directly from raw, unannotated videos through autonomous Questioner-Solver self-play with two complementary temporal-centric rewards, achieving competitive performance with supervised methods.


<details>
  <summary>Details</summary>
Motivation: Existing RL pipelines for Video-LLMs rely on costly human-annotated data and self-evolving frameworks fail to capture temporal dynamics central to video reasoning.

Method: EvoVid uses a temporal-centric self-evolving framework with (1) temporal-aware Questioner reward via temporal perturbation sensitivity, and (2) temporal-grounded Solver reward via inherent video segment localization, enabling learning directly from raw, unannotated videos.

Result: Consistent improvements across four base models and six benchmarks; competitive performance with supervised methods; outperforms existing self-evolving baselines.

Conclusion: Temporal-centric self-evolution is an effective and scalable paradigm for video understanding and reasoning, enabling Video-LLMs to improve without human annotation.
...
TL;DR: MLLMs know the correct temporal interval during prefill (via special "Temporal Grounding Heads") but lose this signal during auto-regressive decoding. A read-then-regenerate framework that extracts the prefill signal and re-generates with restricted visual context improves VTG by up to +3.5 mIoU without training.


<details>
  <summary>Details</summary>
Motivation: VTG tests whether MLLMs understand "when" events happen, but current models predict timestamps unreliably. Existing fixes require costly temporal annotation post-training or use coarse heuristics.

Method: (1) Probe cross-modal attention, discovering Temporal Grounding Heads (TG-Heads) that attend to ground-truth intervals during prefill. (2) Convert TG-Head prefill attention into a debiased relevance signal. (3) Re-invoke the MLLM with visual context restricted to the high-attention interval via video cropping or attention masking.

Result: Consistent improvements on MiMo-VL-7B, Qwen3-VL-8B, and TimeLens-8B across three VTG benchmarks, with gains up to +3.5 mIoU, without parameter updates or architectural changes.

Conclusion: The perception-generation gap is the key bottleneck for VTG in MLLMs; an inference-time read-then-regenerate framework effectively closes this gap by leveraging prefill-stage knowledge.
...
TL;DR: TrajVAD uses bounding-box trajectories (with normalizing flows) for video anomaly detection, outperforming pose-based methods on ShanghaiTech and MSAD datasets.


<details>
  <summary>Details</summary>
Motivation: Existing pose-based anomaly detection methods overlook bounding-box trajectories, even though this information is inherently available in their pipelines. The authors aim to leverage trajectories as an explicit, effective anomaly cue.

Method: TrajVAD models multi-class bounding-box trajectories using normalizing flows to learn normal kinematic patterns. Two variants: TrajVAD-T (trajectory-only) and TrajVAD-P (trajectory + pose).

Result: TrajVAD-T surpasses all pose-based methods on ShanghaiTech (87.7% AP) and achieves best results on MSAD. TrajVAD-P reaches 88.6% AUROC and 90.9% AP on ShanghaiTech.

Conclusion: Bounding-box trajectories are an effective yet underexplored modality for video anomaly detection, and explicitly modeling them as a primary cue yields state-of-the-art performance.
...
TL;DR: PDI-Net integrates infrared image reconstruction and object detection in a low-latency framework by embedding optical priors, using a semi-U-Net encoder shared with a YOLO detector, and a physics-aware bridge (PALS-Bridge) that leverages field-dependent PSF priors to modulate multiscale features, achieving 84% faster inference and 5% better mAP on low-SNR M3FD data while reducing camera weight by ~50%.


<details>
  <summary>Details</summary>
Motivation: Existing deep-learning pipelines for computational infrared imaging suffer from high inference latency when combining reconstruction and detection, and acceleration strategies typically neglect optical/physical priors, creating an accuracy-speed trade-off.

Method: PDI-Net uses a supervised U-Net during training but switches to a semi-U-Net encoder that shares features directly with a YOLO-based detector at inference, bypassing full reconstruction. A physics-aware large-small bridge (PALS-Bridge) uses field-dependent point spread function (PSF) priors to adaptively modulate multiscale convolutional branches. A physics-informed optical degradation simulation pipeline supports training/validation.

Result: On the M3FD benchmark under low-SNR conditions, PDI-Net reduces inference time by 84.06% versus Rec+Det with pruning, while improving mAP@0.5:0.95 by 5.07%. The single-lens infrared camera reduces system weight by ~50% compared to traditional multi-lens designs.

Conclusion: PDI-Net demonstrates that embedding optical priors into a jointly optimized reconstruction-detection framework enables compact, low-latency computational infrared imaging suitable for real-time object detection on resource-constrained platforms.
...
TL;DR: F2G reformulates Video Temporal Grounding as a verifiable Identify-then-Measure problem by integrating predictive temporal perception with evidence-driven reasoning, using candidate event segments as citable evidence units to stabilize grounding and improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Current Video-LLM approaches for VTG rely on direct timestamp generation from unstructured visual-token streams, leading to brittle numerics and inconsistent boundaries.

Method: F2G decouples event identification from boundary measurement via: (1) Predictive Temporal Perception to build a video-wide evidence pool of candidate event segments, (2) Evidence-Driven Reasoning that exposes these segments to the LLM as citable evidence units binding boundary prediction to explicit event hypotheses.

Result: F2G consistently improves grounding accuracy across diverse benchmarks, transfers robustly across different Video-LLM backbones, and preserves general video understanding capabilities.

Conclusion: Reformulating VTG as a verifiable Identify-then-Measure problem with explicit evidence units stabilizes grounding and makes predictions more reliable and verifiable.
...
TL;DR: SOTA AI-generated image detectors fail on video frames due to cross-modal gaps. VINA jointly trains on images and videos using video frames as natural augmentations and contrastive learning, achieving SOTA on 14 benchmarks across images, videos, and in-the-wild settings.


<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detectors have a critical failure mode: they collapse on video frames. This cross-modal gap arises from video processing shifts (color conversion, codec compression, resizing, blur) and model-specific fingerprints from video generators, motivating the need for a unified detector.

Method: VINA (Video as Natural Augmentation) jointly trains on image and video data, using video frames as physically grounded natural augmentations, and employs a cross-modal supervised contrastive objective to align image and video representations under a shared real/fake decision boundary.

Result: Evaluated on 14 benchmarks (image, video, in-the-wild), VINA delivers bidirectional gains, improves robustness and transferability, achieves SOTA in nearly all settings without complex augmentation or dataset-specific tuning.

Conclusion: Jointly training on images and videos with natural augmentations and contrastive learning effectively bridges the cross-modal gap for AIGC detection, offering a simple yet powerful unified framework.
...
TL;DR: This paper introduces a steering-vector-based causal attribution framework to uncover how LVLMs process emotional information through a three-stage "Adapt-Aggregate-Execute" mechanism, revealing functional decoupling where visual emotional cues are aggregated in middle layers via sentiment-specific attention heads and translated into narrative generation in deep layers via emotion-general pathways. The proposed intervention methods improve performance by mitigating emotional hallucinations.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in understanding how LVLMs translate abstract visual stimuli into coherent emotional narratives, as the internal mechanisms remain largely unexplored due to scarcity of visual counterfactuals and diffuse emotional expression.

Method: Introduced a steering-vector-based causal attribution framework with a specialized dataset to demystify emotional circuits; discovered a three-stage "Adapt-Aggregate-Execute" mechanism and functional decoupling between middle and deep layers; regulated emotional information routing to strengthen attention flow and amplify semantic activation.

Result: Significantly improved performance on MER-UniBench via inference-time intervention; effectively mitigated emotional hallucinations; corroborated causal fidelity of the discovered circuits.

Conclusion: The discovered functional decoupling (sentiment-specific attention heads in middle layers, emotion-general pathways in deep layers) provides insights for regulating emotional information routing, enabling effective mitigation of emotional hallucinations in LVLMs.
...
TL;DR: RiT trains a vanilla Diffusion Transformer using x-prediction on frozen DINOv2 features, showing that DINOv2's favorable statistical properties (higher effective rank, better conditioning, lower kurtosis) make flow-matching regression well-conditioned, achieving SOTA FID 1.14 on ImageNet with fewer parameters and efficient few-step sampling.


<details>
  <summary>Details</summary>
Motivation: To investigate whether pretrained representation spaces (like DINOv2) offer more favorable distributions for flow-matching learning compared to pixel space, and to leverage these properties for more efficient diffusion training.

Method: Compare pixel, SD-VAE, and DINOv2 features across four geometric axes; propose RiT (Representation Image Transformer) — a vanilla DiT trained with x-prediction on frozen DINOv2 features, with a dimension-aware noise schedule and joint [CLS]-patch modeling.

Result: DINOv2 shows 7.3× higher effective rank, 35× better covariance conditioning, 11.5× lower excess kurtosis, and 1.7× lower on-manifold interpolation error vs. pixel space. RiT achieves FID 1.45 (no guidance) and 1.14 (with CFG) on ImageNet 256×256, outperforming DiT^DH-XL with 19% fewer params; 5 Heun steps reach FID 2.0 with CFG.

Conclusion: Pretrained representation spaces (especially DINOv2) make flow-matching regression well-conditioned due to favorable statistical properties, enabling efficient training and few-step sampling without specialized heads, Riemannian transport, or distillation.
...
TL;DR: CRPO introduces a counterfactual RL training method to improve spatiotemporal sensitivity in Video LLMs, using dual-branch training with a Counterfactual Relation Reward (CRR) that penalizes shortcut policies.


<details>
  <summary>Details</summary>
Motivation: Video LLMs often rely on shortcuts (single-frame cues, language priors) rather than tracking video dynamics, and RL post-training with correctness-only rewards exacerbates this issue.

Method: CRPO is a dual-branch RL framework that: (1) constructs counterfactual videos via horizontal flips and temporal reversals, (2) trains on both original and counterfactual branches, (3) introduces Counterfactual Relation Reward (CRR) that encourages answer changes for dynamic questions and answer stability for static questions.

Result: On Qwen3-VL-8B, CRPO improves DyBench P-Acc by +7.7 and TimeBlind I-Acc by +8.2 over the base model, outperforming prior RL methods on spatiotemporal-sensitive evaluations while maintaining general video performance.

Conclusion: CRPO effectively improves spatiotemporal sensitivity in Video LLMs by using counterfactual reasoning to discourage shortcut policies, validated on a new benchmark DyBench with paired counterfactual videos.
...
TL;DR: HistoBIT3D introduces the first voxel-wise paired dataset of BIT and fluorescence-labeled nuclei, enabling quantitative validation of unsupervised virtual staining. A new framework translates BIT volumes into realistic H&E volumes using bidirectional multiscale content consistency and cross-domain style reuse, achieving state-of-the-art realism and improved 3D nuclei segmentation.


<details>
  <summary>Details</summary>
Motivation: 3D histopathology of unprocessed tissues can revolutionize disease management, but translating BIT volumes into clinically interpretable H&E images is challenging due to shift-variant contrast and lack of quantitative validation benchmarks.

Method: Created first voxel-wise paired BIT and fluorescence-labeled nuclei dataset for quantitative evaluation; developed virtual staining framework using bidirectional multiscale content consistency and cross-domain style reuse to translate shift-variant BIT volumes into realistic H&E volumes.

Result: Achieved state-of-the-art realism metrics with significantly improved 3D nuclei segmentation accuracy and boundary preservation under zero-shot Cellpose evaluation.

Conclusion: Established a quantitatively validated, structurally faithful, and scalable pipeline for 3D virtual H&E staining, advancing slide-free, volumetric computational histopathology.
...
TL;DR: ConvNeXt-FD is a U-Net-like architecture with a ConvNeXt backbone and a fractal-dimension-based boundary-aware loss function, achieving state-of-the-art segmentation across six diverse biomedical datasets.


<details>
  <summary>Details</summary>
Motivation: Biomedical image segmentation faces challenges due to variability, noise, and complex morphology in medical images, requiring better handling of object boundaries and shape fidelity.

Method: A U-Net encoder-decoder framework with a ConvNeXt backbone, combined with a hybrid loss function: Dice coefficient plus a differentiable Fractal Dimension-based boundary-aware regularization term.

Result: Evaluated on six datasets (BUSI, DDTI, FluoCells, IDRiD, ISIC2018, MoNuSeg). Achieved competitive/superior performance across Dice, Jaccard, Accuracy, Sensitivity, Specificity, and False Positive Rate, especially with ImageNet pre-trained initialization.

Conclusion: ConvNeXt-FD effectively captures both high-level semantics and fine-grained boundary details, leading to more accurate and reliable biomedical segmentations in challenging contexts.
...
TL;DR: DiTo proposes an output-centric token reduction paradigm for Diffusion Transformers, using prior-step token similarity as a proxy to guide token merging, achieving superior quality-speed trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing token reduction methods for DiTs rely on input token similarity (from ViT paradigms), which misaligns with the primary goal of generative models: minimizing recovery error. They overlook the need to reflect output token similarity.

Method: DiTo uses prior-step output token similarity as a proxy for current-step correspondences, applies interleaved Matching/Reduction timesteps with PMR-guided Interval Scheduling, and incorporates Frequency-aware Token Matching to penalize overused tokens and reduce blocking artifacts.

Result: DiTo outperforms existing TR methods by 1.6-3.9 dB higher PSNR at comparable speedups, achieving a superior Pareto frontier.

Conclusion: Shifting from input-centric to output-centric token reduction better aligns with generative objectives and significantly improves quality-efficiency trade-offs in Diffusion Transformers.
...
TL;DR: Proposes PoCoTI, a dataset of 55K CoT reasoning paths for 3D point cloud understanding, and PointLLM-R model that achieves SOTA on 3D classification and captioning.


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought reasoning is effective in LLMs/image MLLMs but underexplored in 3D understanding due to irregular point cloud structure and lack of explicit reasoning in existing 3D models.

Method: Two-stage pipeline: (1) VLM-based quality evaluation and reference-guided refinement for point-text data, (2) Human-in-the-Loop Prompt Optimization (HiLPO) for synthesizing reasoning paths. Fine-tunes PointLLM on PoCoTI to produce PointLLM-R.

Result: PointLLM-R achieves state-of-the-art performance on generative 3D classification and captioning, with robust generalization to real-world scanned point clouds and multi-turn dialogue.

Conclusion: Data-centric CoT supervision framework effectively enhances 3D multimodal reasoning, demonstrating that explicit reasoning paths improve understanding of 3D point clouds.
...
TL;DR: ORBIS is an SW-HW co-designed accelerator for video Diffusion Transformers that uses more accurate inter-token similarity from previous timestep outputs and a Distribution-Aware Token Matching algorithm to achieve ~2x higher token reduction ratio than state-of-the-art, delivering up to 4.5x speedup and 79.3% energy savings over an A100 GPU.


<details>
  <summary>Details</summary>
Motivation: Video Diffusion Transformers suffer from excessive computational cost due to 3D Spatio-Temporal Attention increasing token length with frames. Existing token reduction methods use inaccurate similarity estimates and lightweight matching, yielding poor quality and marginal acceleration.

Method: Proposes ORBIS: (1) leverages previous timestep output activations for more accurate inter-token similarity; (2) Distribution-Aware Token Matching (DATM) algorithm capturing global token distribution and minimizing token-pair loss; (3) specialized deeply pipelined hardware with quantization, occupying only 2.4% total area.

Result: ~2x higher token reduction ratio than AsymRnR; up to 4.5x speedup and 79.3% energy reduction vs. NVIDIA A100 GPU; negligible accuracy loss; hardware occupies only 2.4% of total area.

Conclusion: ORBIS demonstrates that SW-HW co-design with better similarity estimation and matching algorithms can substantially accelerate video DiT inference.
...
TL;DR: A diffusion-based multi-agent motion prediction framework using contextual guidance and energy-based refinement to improve both diversity and joint consistency, outperforming existing methods on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to generate predictions that are both diverse and jointly consistent among interacting agents, and are often evaluated only on single-agent metrics that fail to capture joint dynamics.

Method: Diffusion-based framework with contextual guidance from historical trajectories for diversity, plus an energy-based formulation to enforce interaction consistency while preserving individual trajectory plausibility.

Result: Consistent outperformance on four benchmark datasets, with substantial improvements in both marginal (ADE/FDE) and joint (JADE/JFDE) metrics on ETH/UCY, and significant gains in marginal metrics over prior joint prediction methods.

Conclusion: The proposed approach effectively balances diversity and joint consistency for multi-agent motion prediction, demonstrating strong performance across both marginal and joint evaluation metrics.
...
TL;DR: FRED is the first multi-modal autonomous driving dataset focused on water hazards, containing images, LiDAR, and IMU/GNSS data from flooded road environments with semantic labels and multiple formats.


<details>
  <summary>Details</summary>
Motivation: To address the lack of datasets specifically targeting water hazards on roads for autonomous driving research.

Method: Collected multi-modal data (camera, LiDAR, IMU/GNSS) from five locations during and after flooding, released in KITTI-style and RTMaps formats with semantic labels.

Result: A comprehensive dataset with 2.3 MP images, 64-beam LiDAR point clouds, and RTK-corrected IMU data across flooded and dry conditions.

Conclusion: FRED enables training and evaluation of single-sensor and sensor-fusion methods for water hazard detection, as well as localization and SLAM tasks.
...
TL;DR: ForeSplat is an optimization-aware training framework that improves feed-forward 3D Gaussian Splatting models by training them to produce initializations that enable rapid, effective refinement, bridging the gap between fast single-pass prediction and high-quality per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Feed-forward 3DGS models struggle to match per-scene optimization quality due to scarcity of large-scale 3D annotations. The predict-then-refine paradigm is limited because standard training ignores whether the output serves as a good initialization for downstream optimization.

Method: ForeSplat uses MetaGrad, a lightweight multi-anchor meta-gradient training rule that unrolls a short inner-loop refinement trajectory, samples anchor states, and back-propagates aggregated first-order gradients to the prediction head as a surrogate optimization-aware signal, avoiding costly higher-order differentiation.

Result: ForeSplat-trained initializations converge in fewer refinement steps and reach higher peak reconstruction quality than vanilla counterparts across diverse backbones (AnySplat, Pi3X, distilled variant), with no added inference cost.

Conclusion: ForeSplat bridges the gap between amortized prediction and per-scene optimization, enabling lightweight, high-fidelity 3D reconstruction practical for edge deployment.
...
TL;DR: SO-Mamba proposes a state-ownership Mamba regularizer for accelerated MRI reconstruction that separates resident (recurrent) and non-resident (update-dependent) evidence via a State-Ownership Router, improving reconstruction across diverse benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing state-space model regularizers in unrolled MRI reconstruction treat all reconstruction evidence uniformly, failing to distinguish between persistent resident-carrier content (coherent across stages) and stage-dependent non-resident evidence (tied to the current update), which degrades reconstruction quality.

Method: SO-Mamba introduces a State-Ownership Router (SOR) that assigns evidence into three roles: (1) resident carrier for recurrent content, (2) state-interface access via affine modulation of B/C parameters, and (3) non-state output correction. It also includes a two-level outer-band leakage diagnostic to separate hidden-state storage from readout expression.

Result: On five public MRI benchmarks (diverse anatomies, sampling patterns, coil configurations), SO-Mamba consistently outperforms CNN-, Transformer-, and Mamba-based baselines with competitive computational efficiency.

Conclusion: Explicitly separating resident and non-resident evidence pathways in Mamba-based regularizers improves unrolled MRI reconstruction, and the proposed ownership routing mechanism is broadly effective across different imaging settings.
...
TL;DR: AgroVG is a new agricultural visual grounding benchmark with 10,071 image-query pairs across six target families, testing models on single-target, multi-target, and target-absent scenarios. Zero-shot evaluation of 26 models shows poor performance (best multi-target Set-F1 = 0.35, mask IoU@0.75 < 0.17), revealing significant gaps in agricultural grounding capabilities.


<details>
  <summary>Details</summary>
Motivation: Agricultural AI needs visual grounding for tasks like selective weeding and harvesting, but existing benchmarks fail to handle small, occluded, irregular agricultural targets or test multi-target/absent scenarios. A dedicated benchmark is needed to jointly evaluate localization accuracy, target-set completeness, and abstention ability.

Method: Constructed AgroVG benchmark from 10 source datasets (10,071 pairs) across 6 target families (crop/weed, fruit, wheat head, pest, disease, tree canopy). Supports bounding-box (T1) and instance-mask (T2) grounding with queries covering single-target, multi-target, and target-absent regimes. Provides task-specific box-set matching and query-level mask coverage protocols.

Result: Zero-shot evaluation of 26 models (closed-source MLLMs, open-source VLMs, specialized grounding systems) shows best multi-target Set-F1 is only 0.35 and best positive-query mask success rate at IoU@0.75 is below 0.17, indicating large performance gaps.

Conclusion: Current visual grounding models significantly underperform on agricultural tasks, especially for multi-target scenarios and precise mask grounding. AgroVG provides a standardized evaluation framework to drive progress in agricultural AI.
...
TL;DR: HyLoVQA proposes a continual VQA method using a drift-resilient memory bank of anchors and hypernetwork-generated LoRA adapters to handle cross-level task interference, achieving SOTA on VQA v2 and NExT-QA.


<details>
  <summary>Details</summary>
Motivation: Prior continual VQA methods use largely shared parameters, causing cross-level task interference that hinders accurate adaptation to current tasks/objects.

Method: Maintains a drift-resilient memory bank of anchors (visual objects & textual tasks) updated with current features; a hypernetwork generates lightweight LoRA adapters conditioned on retrieved anchors; uses an alignment loss to align feature-space semantics with parameter-space changes.

Result: Outperforms prior SOTA on VQA v2 and NExT-QA under both standard and compositional continual learning settings.

Conclusion: HyLoVQA effectively addresses cross-level task interference in continual VQA through anchored memory and dynamic LoRA adaptation.
...
TL;DR: GA-BEV introduces a compact, 3D-grounded BEV feature representation integrating explicit depth projection and implicit 3D foundation priors to improve efficiency and spatial reasoning in VLN tasks, achieving SOTA results without DAgger or mixed VQA training.


<details>
  <summary>Details</summary>
Motivation: Existing VLN approaches rely on dense RGB videos with excessive patch tokens and lack explicit spatial structure, leading to high computational overhead and limited spatial reasoning.

Method: Constructs BEV spatial maps from RGB-D inputs by projecting visual features into 3D space and aggregating into agent-centric layouts; incorporates features from a pretrained 3D foundation model to inject structural priors.

Result: Achieves state-of-the-art results using only navigation data, without DAgger augmentation or mixed VQA training, demonstrating robustness and data efficiency.

Conclusion: The GA-VLN framework proves that compact, geometry-aware BEV representations significantly improve navigation efficiency and performance.
...
TL;DR: Proposes a cardiac digital twin framework with anatomy-aware infarct synthesis and PAA-Net for noninvasive MI localization from cine MRI and ECG, outperforming existing methods in scar/border zone segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing inverse MI inference methods overlook realistic scar morphology and cardiac repolarization, reducing sensitivity to subtle ECG variations and interpretability of infarct-induced electrophysiological changes.

Method: Three components: (1) anatomy-aware stochastic infarct synthesis for realistic irregular scars with border zones; (2) virtual cohort simulation of QRS-T waveforms; (3) Physiology and Anatomy Aware Network (PAA-Net) encoding 3D myocardial geometry and multi-lead ECGs for infarct inference.

Result: Achieves Dice scores of 0.7391 for scar and 0.5503 for border zone segmentation, significantly outperforming existing inverse inference methods.

Conclusion: The framework enhances noninvasive MI localization accuracy and interpretability of the ECG-infarct relationship, bridging simulation-reality domain gap.
...
TL;DR: This paper identifies that memorization in diffusion models causes internal numerical instability (visual "broken" artifacts), introduces empirical stability regions to detect this, and proposes a real-time framework for detecting and mitigating memorization without altering prompts or image quality, achieving near-perfect detection and 0% memorization rate.


<details>
  <summary>Details</summary>
Motivation: Diffusion models memorize training data, causing privacy and copyright risks; prior work lacked a principled way to detect and mitigate memorization without compromising generation quality.

Method: Identifies memorization-induced numerical instability, defines empirical stability regions based on latent update norms (inspired by numerical stability analysis), and proposes an on-the-fly detection and adaptive mitigation framework.

Result: AUC >0.999 for detection, 0.0% memorization rate after mitigation, with only ~0.01s overhead per image on Stable Diffusion 1.4.

Conclusion: Memorization can be reliably detected via numerical instability and mitigated in real-time without sacrificing semantic fidelity or image quality.
...
### [49] [Diffusion-guided Generalizable Enhancer for Urban Scene Reconstruction](https://arxiv.org/abs/2605.22420)
*Henry Che,Jingkang Wang,Yun Chen,Ze Yang,Sivabalan Manivasagam,Raquel Urtasun*

Main category: cs.CV

TL;DR: GenRe is a diffusion-guided enhancer that quickly fixes deficiencies in pretrained 3D Gaussian representations for urban scene reconstruction, enabling robust high-fidelity rendering under large viewpoint shifts for autonomous driving simulation.


<details>
  <summary>Details</summary>
Motivation: Existing neural rendering approaches degrade significantly under large viewpoint shifts, and current diffusion-based methods require costly per-scene optimization with fragile representations that fail to generalize beyond limited synthesized views.

Method: GenRe takes any pretrained 3D Gaussian representation and fixes deficiencies within minutes by learning to distill generative priors across diverse scenes, producing robust and generalizable representations.

Result: Outperforms existing methods in both quality and efficiency, generalizes reliably to challenging unseen viewpoints (e.g., lane change), and benefits various downstream tasks.

Conclusion: GenRe enables robust and scalable sensor simulation for autonomous driving by producing efficient, generalizable 3D representations from diverse scenes.
...
TL;DR: EasyVFX is a resource-efficient framework for realistic VFX synthesis that uses frequency-domain decomposition to decouple spatial textures from motion dynamics, enabling high-quality effects with a two-stage training paradigm (Freq-MoE architecture + Test-Time Training) that requires minimal GPU resources.


<details>
  <summary>Details</summary>
Motivation: Existing VFX generation demands massive datasets and prohibitive computational power due to the complex coupling of spatial textures and temporal dynamics; the goal is to democratize professional-grade VFX by reducing these resource barriers.

Method: 1) Frequency-domain decomposition to decouple high-frequency (spatial appearances) and low-frequency (motion dynamics) components; 2) Frequency-aware Mixture-of-Experts (Freq-MoE) with soft routing assigning specialized experts to spectral bands; 3) Test-Time Training with Frequency-constraint Loss for fast adaptation (~100 steps on single GPU).

Result: EasyVFX produces structurally consistent and visually stunning VFX effects while requiring significantly fewer GPU resources compared to traditional approaches.

Conclusion: Frequency-aware learning is a key catalyst for democratizing professional-grade VFX, proving that decoupling spectral components can transform high-dimensional learning into manageable sub-tasks.
...
### [51] [Pre-VLA: Preemptive Runtime Verification for Reliable Vision-Language-Action and World-Model Rollouts](https://arxiv.org/abs/2605.22446)
*Zhen Sun,Yongjian Guo,Haoran Sun,Luqiao Wang,Wei Lu,Jiachi Ji,Shengzhe Ji,Junwu Xiong,Zhijun Meng*

Main category: cs.CV

TL;DR: Pre-VLA is a runtime verification architecture that preemptively assesses action validity before execution or world-model rollouts, using multimodal learning with dual-branch heads to filter low-quality actions, achieving higher success rates and reduced errors on LIBERO benchmark.


<details>
  <summary>Details</summary>
Motivation: Low-quality actions from VLA models or world models can cause physical failures during execution or misleading rollouts with redundant rendering costs, hindering practical deployment of long-horizon embodied intelligence.

Method: Pre-VLA uses an efficient multimodal backbone with modality-aware pooling and a dual-branch head to predict safety confidence and critic-derived advantage scores. It is trained with multi-task objective (Focal classification, advantage regression, soft-threshold calibration) and uses a dual-mode preemptive resampling scheduler to filter/adaptively resample low-quality actions under limited computation budget.

Result: On LIBERO benchmark, Pre-VLA improves closed-loop success rate from 30.79% to 37.62% over RynnVLA-002, reduces task execution steps, achieves 183.9 ms average forward verification time per action chunk, and mitigates error accumulation in world-model rollouts.

Conclusion: Preemptive action validity assessment via Pre-VLA effectively filters low-quality actions and enhances robustness of VLA and world model deployment for embodied tasks.
...
TL;DR: Proposes MDIC, a multimodal distributed image compression framework that uses text-to-image diffusion and fine-grained alignment to achieve state-of-the-art perceptual quality at extremely low bitrates (<0.1 bpp).


<details>
  <summary>Details</summary>
Motivation: Existing DIC methods fail to exploit global context and object-level details from side information, causing local blurring and loss of fine details in reconstruction at ultra-low bitrates.

Method: (1) Text-to-image diffusion-based decoder conditioned on textual side information; (2) Feature-mask generator supervised by multimodal fine-grained alignment; (3) Mask guides extraction of fine details from side info and regulates VQ-VAE embeddings to compensate for lost category info.

Result: State-of-the-art perceptual quality on KITTI Stereo and Cityscapes datasets at extremely low bitrates (<0.1 bpp).

Conclusion: Leveraging side information in a multimodal manner (text + visual) effectively preserves fine-grained local details and enhances global perceptual quality in DIC.
...
TL;DR: Echo4DIR reconstructs 4D cardiac geometry from sparse 2D echocardiography using a test-time implicit reconstruction framework with SSM priors, epipolar attention, self-supervised differentiable rendering, and radial SDF alignment, achieving state-of-the-art results (98.35% Dice, 96.75% IoU).


<details>
  <summary>Details</summary>
Motivation: Reconstructing 4D cardiac geometry from sparse 2D echocardiography is highly desirable yet challenged by geometric ambiguity and temporal discontinuity.

Method: 1) Cardiac conditional SDF with SSM priors; 2) Epipolar Mask Encoder with cross attention for multi-view fusion; 3) Self-supervised SDF-tailored differentiable rendering for domain adaptation; 4) Radial SDF Alignment for continuous 4D extension.

Result: State-of-the-art 4D cardiac mesh reconstruction: 98.35% Dice and 96.75% IoU on clinical datasets.

Conclusion: Echo4DIR effectively overcomes sparse observation limitations and domain gaps, producing anatomically reliable 4D cardiac geometry with continuous temporal evolution.
...
TL;DR: This paper introduces open tree decomposition, a new paradigm for segmenting images into hierarchical visual component trees, with an automated pipeline using LVLMs and SAM to create the COCOTree benchmark (21K images, 1.8M nodes, 3.5K labels) and the OTQ evaluation metric.


<details>
  <summary>Details</summary>
Motivation: Manual annotation for hierarchical tree decomposition is prohibitively costly and complex. The authors aim to automate this process and provide a standardized benchmark.

Method: A fully automated pipeline combining Large Vision-Language Models (LVLMs) for semantic reasoning with SAM 3 for geometric grounding; construction of the COCOTree dataset; proposal of the Open Tree Quality (OTQ) metric for evaluation.

Result: COCOTree benchmark with 21K+ images, 1.8M structural nodes, and over 3.5K unique labels capturing long-tail distributions. Human evaluation confirms strong alignment with human structural judgment.

Conclusion: The work enables open tree decomposition with automated annotation, a large-scale benchmark, and a standardized evaluation protocol, providing a foundation for future research in hierarchical visual understanding.
...
TL;DR: TWINGS enhances 3D Gaussian Splatting for sparse-view novel view synthesis by using Thin Plate Splines to calibrate backprojected points, improving initialization and achieving superior reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Novel view synthesis from sparse-view inputs is challenging; achieving high-quality scene reconstructions with limited viewpoints is difficult.

Method: TWINGS uses Thin Plate Splines (TPS) to align backprojected points from estimated depth with triangulated 3D control points, providing a fast and geometrically accurate initialization for 3DGS.

Result: TWINGS consistently outperforms existing methods on DTU, LLFF, and Mip-NeRF360 datasets, delivering detailed and accurate reconstructions under sparse-view scenarios.

Conclusion: TWINGS effectively addresses point sparsity in 3DGS, improving structural detail and color fidelity for sparse-view novel view synthesis.
...
TL;DR: JMed48k is a new Japanese healthcare licensing benchmark with 48,862 exam questions and 20,142 images from 11 national exams (2005-2025). Evaluation of 21 models shows that images significantly help proprietary and open-source models but not medical-specific models, with the image impact varying greatly across professions.


<details>
  <summary>Details</summary>
Motivation: To create a multi-profession Japanese medical licensing benchmark for evaluating vision-language models, addressing the lack of profession-stratified evaluation in non-English healthcare settings.

Method: Built 48,862 questions from official Japanese ministry PDFs (2005-2025, 11 exams), annotated with 8-type image taxonomy. Derived JMed48k-Eval subset (12,484 questions, 5 years). Introduced paired image-removal audit to analyze answer-transition states when visual content is removed.

Result: Proprietary and open-source models gain substantially from images; medical-specific systems show limited use of visual evidence. Image-removal effect varies sevenfold across professions (+5.7 to +39.8 points).

Conclusion: JMed48k enables reproducible, profession-stratified VL model evaluation; current medical-specific models underutilize visual information compared to general models.
...
TL;DR: GenHAR is a domain-invariant framework for Human Activity Recognition that uses tokenized frequency sensor channels, selective masking, and efficient attention to outperform state-of-the-art methods by 9.97% accuracy while reducing computation by 6.4x, deployed at scale in a logistics company.


<details>
  <summary>Details</summary>
Motivation: HAR suffers from distribution shift across sensor domains, causing performance degradation in real-world deployment.

Method: GenHAR tokenizes sensor data, learns correlations among frequency sensor channel dimensions, and improves efficiency via selective masking and an efficient attention mechanism.

Result: Outperforms SOTA by 9.97% in accuracy, reduces FLOPs by 6.4x, deployed in 4 cities detecting 2.15 billion real-time activities.

Conclusion: GenHAR effectively mitigates domain gaps for robust, efficient HAR generalization in real-world applications.
...
TL;DR: LVDrive enhances VLA models for autonomous driving by introducing latent future scene prediction in a unified embedding space, outperforming existing methods on the Bench2Drive benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models rely on sparse action supervision or pixel-level reconstruction, underutilizing scene understanding. LVDrive aims to learn semantically meaningful latent representations via future scene prediction.

Method: LVDrive introduces future scene prediction in latent space using a pretrained vision backbone, jointly models future scene and motion prediction in a unified embedding space via single forward pass, and uses a two-stage trajectory decoding strategy leveraging latent future representations.
...
Conclusion: Latent visual representation learning through future scene prediction is an effective alternative to pixel-level reconstruction for enhancing VLA models in autonomous driving.
...
TL;DR: VISTA is a metric-aligned multi-backbone framework for capsule endoscopy event detection that achieves competitive performance on the RAREVISION task by combining temporal and visual features with ensemble decoding strategies.


<details>
  <summary>Details</summary>
Motivation: Capsule endoscopy event detection is challenging due to sparse clinically relevant findings, high visual heterogeneity, and the need for event-level (not frame-level) evaluation.

Method: VISTA combines EndoFM-LV (temporal context) and DINOv3 ViTL/16 (frame-level semantics) backbones, with Diverse Head Ensemble (DHE), Validation-Guided Weighted Fusion (VGWF), and Anatomy-Aware Temporal Event Decoding (ATED).

Result: Original: 0.3530 mAP@0.5, 0.3235 mAP@0.95. Post-competition with global coarse search: 0.3726 mAP@0.5, 0.3431 mAP@0.95 (ranked 2nd).

Conclusion: Global threshold refinement improved performance, and the multi-backbone + ensemble approach is effective for sparse event detection in capsule endoscopy.
...
TL;DR: TextTeacher injects frozen text embeddings from image captions as auxiliary supervision during vision model training, improving ImageNet accuracy by up to +2.7pp and transfer learning by +1.0pp without inference overhead.


<details>
  <summary>Details</summary>
Motivation: Leverage the Platonic Representation Hypothesis to efficiently transfer semantic knowledge from language models to vision models using readily available image captions.

Method: A lightweight auxiliary loss aligns vision features with frozen text embeddings (from image captions) via a projection layer during training; inference remains a pure vision model.

Result: Up to +2.7pp on ImageNet, +1.0pp transfer gains; outperforms vision KD (33% faster for same accuracy); acts as feature-space preconditioner improving early training dynamics.

Conclusion: Text embeddings can efficiently guide vision representations with negligible overhead, preserving pure vision inference while providing complementary semantic cues for generalization.
...
TL;DR: OPERA jointly optimizes restoration planning (via RL) and tool co-training for multi-degradation image restoration, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing agent-based restoration methods are limited by constrained planning spaces and lack of coordination among independently pretrained tools, leading to suboptimal results under complex mixed degradations.

Method: OPERA uses reinforcement learning to optimize tool composition plans and introduces agent-guided co-training to enable cooperative behaviors among restoration tools in an end-to-end manner.

Result: Consistently outperforms both all-in-one models and prior agent-based methods on multi-degradation benchmarks and real-world datasets.

Conclusion: Jointly optimizing planning and tool execution effectively addresses the coordination problem in agent-based image restoration.
...
TL;DR: A unified Bayesian framework for motion-compensated 3D MRI that jointly estimates anatomical images, motion parameters, and coil sensitivities using pretrained 3D score-based diffusion models as priors, achieving robust reconstruction without paired training data.


<details>
  <summary>Details</summary>
Motivation: MRI is highly susceptible to patient motion due to long acquisition times and sequential k-space sampling, causing phase inconsistencies that lead to severe artifacts. Retrospective motion compensation remains challenging, especially in accelerated acquisitions, due to the ill-posed nature of joint reconstruction and motion estimation.

Method: Proposes a unified Bayesian framework integrating pretrained 3D complex-valued score-based diffusion models as anatomical image priors within a physics-based forward model. Inference alternates between diffusion posterior image updates and proximal optimization for motion and coil sensitivity estimation, enabling fully unsupervised reconstruction.

Result: Experiments on simulated and real-motion brain MRI datasets show improved image quality and motion robustness compared to state-of-the-art methods, particularly under severe motion and high acceleration.

Conclusion: The proposed diffusion-based Bayesian framework effectively handles motion correction in accelerated 3D MRI without requiring paired motion-free training data, outperforming existing techniques in challenging motion scenarios.
...
TL;DR: AesFormer is a two-stage framework that improves aesthetic photo quality via structural reconstruction by first planning editing actions (AesThinker) and then executing structural edits (AesEditor), outperforming existing methods on the new AesRecon benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing retouching and portrait enhancement methods cannot fix structural flaws (composition, camera viewpoint, pose) in everyday photos; current image editing models lack aesthetic understanding, yielding semantically plausible but aesthetically weak edits.

Method: Two-stage framework: (1) AesThinker — an aesthetic action model that analyzes photos across seven photographic dimensions and outputs executable editing actions, trained with GRPO-A for broad exploration beyond SFT; (2) AesEditor — an action-conditioned editor performing structural edits. Uses VCMP to build AesRecon benchmark (9,071 aligned poor/good image pairs).

Result: AesFormer substantially improves APR performance and is competitive with Nano Banana Pro.

Conclusion: Decoupling aesthetic planning from image editing effectively improves structural photo reconstruction; the AesRecon benchmark and VCMP pipeline enable future APR research.
...
TL;DR: This paper accelerates pretrained ViTs by replacing convolution-like attention heads with efficient depthwise convolution layers, achieving 17-20% inference speedup with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Pretrained ViTs have high inference costs that limit deployment on resource-constrained devices.

Method: Identifies attention heads that behave like convolutions, replaces them with efficient depthwise convolution layers, and fine-tunes to recover performance.

Result: Achieves 17-20% inference speedup on image classification and segmentation tasks with minimal performance degradation.

Conclusion: Exploiting convolution-like attention heads enables efficient ViT acceleration while preserving feature extraction capabilities.
...
TL;DR: EventGait proposes an end-to-end dual-stream framework for event-based gait recognition using spiking neural networks and cross-modal alignment, achieving state-of-the-art results on new benchmarks especially in low-light conditions.


<details>
  <summary>Details</summary>
Motivation: Conventional camera-based gait recognition suffers in uncontrolled environments due to illumination and motion sensitivity; existing event-based methods discard fine-grained motion dynamics by aggregating events over long time windows.

Method: A dual-stream framework with a dynamic stream using Mixture of Spiking Experts (MoSE) for motion perception and a static stream using Cross-modal Structure Alignment (CroSA) with vision foundation models; plus a synthesis pipeline for large-scale event-based gait datasets.

Result: Event-based gait recognition achieves comparable results to camera-based methods under normal conditions and significantly outperforms in low-light scenarios; sets SOTA on both synthesized and real-world event-based gait benchmarks.

Conclusion: Event-driven gait analysis is robust and promising for practical identification, with event cameras offering key advantages in challenging illumination and fast-motion scenarios.
...
TL;DR: One Sentence, One Drama is a hierarchical multi-agent framework that transforms a single-sentence idea into a fully produced short drama, addressing narrative pacing, spatial consistency, and production-level quality control through multi-agent debate, 3D-grounded first-frame generation, and multi-stage reviewer loops.


<details>
  <summary>Details</summary>
Motivation: Existing digital short-drama pipelines fail to meet three key requirements: narrative pacing (weak hooks, poor escalation, unattractive endings), spatial consistency (drifting layouts and inconsistent character positions), and production-level quality control (extensive manual review).

Method: A hierarchical multi-agent framework with three core components: (1) multi-agent debate-based story generation for narrative pacing; (2) 3D-grounded first-frame generation for spatial consistency; (3) multi-stage reviewer loops for error detection and revision. Also includes scene-level BGM matching and scene transition planning.

Result: The method significantly outperforms existing pipelines in narrative quality, cross-clip consistency, and overall viewing experience, as evaluated on the new Short-Drama-Bench benchmark.

Conclusion: The proposed framework effectively addresses the three key shortcomings of current short-drama generation pipelines by integrating structured intermediate modules and iterative refinement, leading to improved production quality.
...
TL;DR: FlowGS is a generative super-resolution framework for remote sensing images that uses flow matching with shortcut consistency and 2D Gaussian splatting to achieve efficient, continuous-scale arbitrary super-resolution.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based super-resolution methods require slow iterative inference (40–1000 steps) and lack flexibility for continuous-scale super-resolution settings.

Method: (1) Models high-frequency detail differences between high/low-res images via flow matching with shortcut consistency constraints to reduce complexity and improve efficiency. (2) Uses 2D Gaussian splatting to construct a continuous feature field for arbitrary-scale reconstruction.

Result: FlowGS achieves competitive perceptual quality in both continuous-scale and fixed-scale SR settings, with substantially improved inference efficiency compared to existing methods.

Conclusion: FlowGS effectively addresses the slow inference and limited flexibility of diffusion-based SR methods for remote sensing images, offering efficient and flexible arbitrary-scale super-resolution.
...
TL;DR: The paper proposes four novel hybrid active learning sampling methods that combine diversity and uncertainty (easy+hard samples), with the LCD (Least Confident and Diverse) method consistently outperforming state-of-the-art approaches.


<details>
  <summary>Details</summary>
Motivation: Deep learning models are data-hungry, and supervised learning requires expensive labeled data. Existing AL methods sample based on either diversity OR uncertainty, not both, potentially limiting model performance.

Method: Four hybrid sampling methods are proposed that pool both easy and hard diverse samples. LCD (Least Confident and Diverse) combines uncertainty (least confident) with diversity. Experiments use high and low-confidence samples separately.

Result: LCD consistently outperforms state-of-the-art methods. Selecting uncertain and diverse instances helps models learn more distinct features.

Conclusion: Hybrid sampling (combining diversity and uncertainty) is more effective than single-dimension sampling for active learning in deep vision models.
...
TL;DR: A pipeline that fuses video, telematics data (IMU/GPS), and CV model insights to generate pseudo-labels for fine-tuning MLLMs to detect and describe safety-critical driving events, achieving strong results with <50M trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs excel at general visual understanding but struggle to perceive and reason about rare, high-stakes dynamic events (e.g., collisions) in safety-critical driving scenarios.

Method: Fuses downsampled video frames with synchronized high-frequency telematics (IMU, GPS) and semantic insights from specialized CV models; generates pseudo-labels (captions, QA pairs) for SCEs; fine-tunes QwenVL-2.5 with DoRA adapters.

Result: Significant improvements in identifying and explaining safety-critical events with fewer than 50M trainable parameters and limited computational budget.

Conclusion: Multimodal fusion with telematics and CV-derived semantics effectively enhances MLLMs for safety-critical driving event understanding, even under parameter/computation constraints.
...
TL;DR: EIC-LIE is an event-illumination collaborative framework for low-light image enhancement that integrates HDR information from events with global illumination cues, features a noise-filtering module based on brightness statistics, and introduces a new high-resolution real-world dataset. It outperforms SOTA by up to 1.24dB PSNR and 0.069 SSIM.


<details>
  <summary>Details</summary>
Motivation: Existing event-based LIE methods focus on incorporating HDR from events but ignore global illumination and are sensitive to event noise in real scenarios.

Method: Proposes EIC-LIE with: (1) Event-Illumination Collaborative Interaction (EICI) module featuring forward gathering (HDR features across lighting) and backward injection (complementary content); (2) Illumination-aware Event Filter (IAEF) for dynamic event noise reduction; (3) A beam-splitter hybrid imaging system for a new real-world dataset.

Result: Outperforms SOTA on five datasets; improvements up to 1.24dB PSNR and 0.069 SSIM.

Conclusion: Event-illumination collaboration with noise filtering effectively enhances low-light images, and the new dataset enables further research.
...
TL;DR: NoPo4D is the first feed-forward system that jointly handles dynamic content, multi-view input, and unknown camera poses for 3D Gaussian splatting, outperforming prior methods on multi-view dynamic benchmarks with orders-of-magnitude speedup.


<details>
  <summary>Details</summary>
Motivation: Existing feed-forward 3D Gaussian splatting methods address only subsets of dynamic content, multi-view input, and unknown camera poses; no single method jointly handles all three aspects in one feed-forward pass.

Method: NoPo4D introduces a velocity decomposition that splits Gaussian motion into 2D image-plane shifts and depth changes, supervised by pseudo ground-truth optical flow on the 2D component, plus a bidirectional motion encoder and view-dependent opacity.

Result: Outperforms prior feed-forward baselines on four multi-view dynamic benchmarks; with optional post-optimization surpasses per-scene optimization methods while running orders of magnitude faster.

Conclusion: NoPo4D fills a previously empty quadrant in feed-forward 3D scene reconstruction by jointly addressing dynamics, multi-view input, and unknown poses, achieving state-of-the-art results with high efficiency.
...
TL;DR: Proposes UHD-GCN-BIQA, a graph representation learning framework that models structural dependencies among sampled patches for blind quality assessment of ultrahigh-definition images, achieving competitive correlation and lowest RMSE on UHD-IQA benchmark.


<details>
  <summary>Details</summary>
Motivation: Native-resolution inference for UHD-BIQA is computationally expensive; aggressive resizing or cropping either suppresses scale-sensitive distortions or weakens local-global context relationships.

Method: Samples aspect-ratio-aligned patches as graph nodes, builds hybrid k-NN graph using spatial proximity and feature similarity, applies residual graph convolution for context propagation, uses gated attention pooling for aggregation, and adopts EMA-normalized multiobjective loss for joint regression/correlation/ranking optimization.

Result: PLCC=0.7784, SRCC=0.8019, RMSE=0.0519 on UHD-IQA benchmark; competitive correlation and lowest RMSE among compared methods.

Conclusion: Graph-based region relation modeling is effective for UHD-BIQA, particularly for improving absolute quality score estimation under high-resolution visual content.
...
TL;DR: A MICCAI challenge benchmarked vision-based surgical skill assessment in open surgery using suturing videos, finding general-purpose spatiotemporal video models performed best, while fine-grained OSATS scoring and keypoint tracking remain challenging.


<details>
  <summary>Details</summary>
Motivation: Automated skill assessment can improve surgical training, but existing ML methods focus on minimally invasive surgery, leaving open surgery underexplored. This challenge aimed to benchmark and advance vision-based assessment in open surgery.

Method: A two-year MICCAI challenge with open suturing videos (GoPro, dry-lab) offering video + instrument trajectory data. Tasks: (1) 4-class skill classification, (2) full OSATS score prediction (8 categories), (3) hand/tool tracking. Solutions included deep video models, tracking-driven, and hybrid approaches.

Result: General-purpose spatiotemporal video models consistently performed best; diverse approaches were competitive when well-executed. Fine-grained OSATS prediction benefits from more data. Keypoint tracking is difficult due to occlusions and out-of-frame instances.

Conclusion: Video-based skill assessment in open surgery shows promise but faces limitations—especially in fine-grained scoring and tracking. Critical directions for advancing toward clinical impact were identified.
...
TL;DR: TEGU is a zero-shot temporal action localization method that uses rich textual information from LLMs and captions to improve fine-grained action discrimination without any training data, outperforming SOTA on THUMOS14 and ActivityNet-v1.3.


<details>
  <summary>Details</summary>
Motivation: Existing ZS-TAL methods struggle with fine-grained action classification when using VLMs, and current approaches either require large annotated video datasets (limiting generalization) or discard labeled data but underperform. The gap is the lack of rich linguistic context for finer discrimination without supervision.

Method: TEGU leverages large language models and structured text from captions to generate rich textual information, providing fine-grained linguistic cues that improve the discrimination between presence and absence of actions in untrimmed videos, all without any training on labeled data.

Result: On THUMOS14 and ActivityNet-v1.3 datasets, TEGU outperforms state-of-the-art ZS-TAL methods that do not involve training, demonstrating the effectiveness of using rich textual information for finer action localization.

Conclusion: Exploiting rich textual information from LLMs and captions can effectively compensate for the lack of supervised training data in zero-shot temporal action localization, enabling better fine-grained action discrimination and localization.
...
TL;DR: EvoIR-Agent is a training-free MLLM-driven image restoration agent that uses a hierarchical experience pool and self-evolving mechanism to achieve better performance and efficiency without trial-and-error overhead.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-driven image restoration agents suffer from a dilemma: training-based methods are efficient but lack compatibility with new tools/degradations, while training-free methods require severe trial-and-error overhead due to naive experience storage.

Method: EvoIR-Agent systematically formulates experience components for training-free agents, constructs a hierarchical experience pool for coarse-to-fine guidance, and introduces a self-evolving mechanism to update the pool from scratch using accumulated records.

Result: EvoIR-Agent achieves significant leads in full reference metrics and yields a Pareto-optimal balance between performance and efficiency compared to state-of-the-art methods.

Conclusion: The proposed self-evolving, hierarchical experience pool effectively resolves the dilemma between efficiency and compatibility in MLLM-driven image restoration agents.
...
TL;DR: GALAR-TemporalNet v2 is a hierarchical temporal model for Video Capsule Endoscopy that uses windowed self-attention, Dual-Graph GCN, Bidirectional Mamba, and anatomy prototype pathway to achieve multi-label classification of anatomical regions and pathological findings, improving from mAP@0.5 of 0.2644 to 0.3409 and mAP@0.95 from 0.2353 to 0.3333 on the RARE-VISION dataset.


<details>
  <summary>Details</summary>
Motivation: Address three core challenges in VCE: extreme class imbalance, long-range temporal dependencies, and pathology-anatomy entanglement.

Method: Hierarchical architecture combining windowed self-attention (local modeling), Dual-Graph GCN (global frame relationships), Bidirectional Mamba (boundary context encoding), anatomy prototype residual pathway (decoupling pathology from normal anatomy), and frame-level GCN skip connection (stabilizing rare class training).

Result: Competition version achieved mAP@0.5=0.2644, mAP@0.95=0.2353. v2 with restructured pathology branch, refined loss functions, and extended post-processing improved to mAP@0.5=0.3409, mAP@0.95=0.3333.

Conclusion: The redesigned components significantly improve performance, demonstrating the effectiveness of decoupling pathology-anatomy signals and hierarchical temporal modeling for VCE analysis.
...
TL;DR: This paper presents a semi-supervised segmentation pipeline for adverse weather conditions, using UniMatch V2 as baseline, treating degraded-weather images as unlabeled data, and applying test-time augmentation. It won the CVPR 2026 UG2+ Challenge Track 2.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of semantic segmentation under adverse weather conditions where labeled data is scarce and weather-degraded images are abundant.

Method: Uses UniMatch V2 as baseline, treats all degraded-weather images as unlabeled data for semi-supervised training, and applies test-time augmentation during inference.

Result: Trained exclusively on WeatherProof dataset (no external data), achieved competitive performance in the CVPR 2026 UG2+ Challenge Track 2.

Conclusion: Semi-supervised learning effectively exploits data distribution in adverse weather conditions; test-time augmentation further boosts robustness.
...
TL;DR: This paper introduces REACH-Net, a Transformer-based model that accurately estimates 3D hand poses from low-resolution, occluded far-away cameras by leveraging hand-body coordination, temporal progression, and multiview observations, supported by a new large-scale dataset (REACH) captured via concealed chest cameras.


<details>
  <summary>Details</summary>
Motivation: Existing 3D hand pose estimation methods struggle with low-resolution, occluded, and far-away viewpoints (e.g., room corners). The authors aim to overcome these limitations by exploiting hand-body coordination, temporal dynamics, and multiview cues.

Method: The method uses a Transformer-based model (REACH-Net) that models hand-body configurations via per-view token correlations and exploits temporal coordination autoregressively. A novel dataset (REACH) is collected using concealed chest cameras to capture accurate hand movements during daily activities without interfering with natural motion.

Result: REACH-Net achieves highly accurate 3D hand pose estimation from distant, low-resolution, occluded views, outperforming existing methods in comparative experiments.

Conclusion: The work broadens the applicability of 3D hand pose estimation to "in-the-wild" continuous human behavior analysis, demonstrating feasibility under challenging real-world conditions.
...
TL;DR: D3Seg is a brain tumor segmentation model that maintains stable performance despite missing MRI modalities, using graph fusion, diffusion-based imputation, and decision refinement.


<details>
  <summary>Details</summary>
Motivation: Clinical settings often lack complete MRI sequences, causing existing segmentation methods to degrade significantly due to naive feature fusion strategies.

Method: D3Seg introduces Multi-hop Modality Graph Fusion (MMGF) for higher-order inter-modality dependencies, a diffusion-based imputation mechanism for missing T1ce, and probability-space decision refinement to reduce overconfidence on dominant classes.

Result: On BraTS 2023, D3Seg achieves 1.5-2.0% Dice improvement on enhancing tumor (ET) and ~1.0% on tumor core (TC) across missing modality settings over state-of-the-art.

Conclusion: D3Seg effectively handles missing modalities with stable performance and computational efficiency, making it suitable for clinical deployment.
...
TL;DR: This paper compares transcription-based (OMR) and transcription-free (Transformer, LLM) methods for content-based search in digitized music scores, finding OMR excels in-domain while transcription-free models handle domain variability better.


<details>
  <summary>Details</summary>
Motivation: Content-based search in music score images is underexplored compared to text documents; current retrieval relies mainly on metadata (title, composer), limiting accessibility for musicians, musicologists, and educators.

Method: The authors first study relevant score characteristics for search and define a systematic method to build query datasets from annotated corpora. They compare three approaches: (1) OMR-based transcription pipelines, (2) a transcription-free Transformer trained on score images, and (3) a text-prompted LLM.

Result: OMR-based pipelines achieve higher in-domain retrieval accuracy, while transcription-free models (Transformer/LLM) handle domain variability (different dataset sizes, image quality, typesetting) more effectively.

Conclusion: Each method has strengths under different conditions, suggesting complementary use depending on the target scenario (in-domain vs. cross-domain retrieval).
...
TL;DR: MuKV proposes multi-grained KV cache compression (patch, frame, segment levels) with a semi-hierarchical retrieval method to improve both efficiency and accuracy in long streaming VideoQA, outperforming baselines on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Long streaming video QA suffers from growing visual tokens and limited LLM reasoning length. Existing KV-caching methods cache every frame redundantly and lose fine-grained spatial or temporal details.

Method: MuKV uses multi-grained KV cache compression (patch-, frame-, segment-levels) with dual signal token compression via self-attention and frequency, plus a semi-hierarchical retrieval mechanism for online QA.

Result: MuKV significantly improves answer accuracy on long-streaming VideoQA benchmarks without sacrificing memory or online QA efficiency; the compression mechanism alone brings consistent gains over baselines.

Conclusion: Multi-grained KV cache compression and semi-hierarchical retrieval effectively balance efficiency and accuracy for long streaming VideoQA, and the compression approach is independently beneficial.
...
TL;DR: CFGPatch is a curved-edge fractal adversarial patch that attacks vision-language models in visible-infrared settings, outperforming standard baselines and transferring across tasks like classification, captioning, and VQA.


<details>
  <summary>Details</summary>
Motivation: Adversarial robustness of VLMs in visible-infrared cross-modal scenarios is underexplored, yet VIS-IR sensing is critical for real-world perception under challenging conditions.

Method: CFGPatch uses Bezier-curved fractal geometry (replacing straight edges) with multi-scale self-similarity, plus a modality-specific Fraser-spiral rendering mechanism for texture distortion, optimized via EOT for transformation robustness.

Result: CFGPatch effectively fools VIS-IR VLMs, outperforms standard patch baselines in attack effectiveness/robustness, and adversarial samples transfer well to captioning and VQA tasks.

Conclusion: CFGPatch demonstrates strong cross-task transferability and generalizability in VIS-IR VLM adversarial attacks, highlighting a critical security vulnerability in multimodal perception systems.
...
TL;DR: This paper enhances YOLOv2 with FPN and switchable atrous convolution for detecting virus patches in FFU microscopy images, achieving 40.5% mAP (small cells) and 68% mAP (virus patches).


<details>
  <summary>Details</summary>
Motivation: Virus patch detection in FFU images is challenging due to high variation in size, density, contrast, and shape, requiring improved detection methods.

Method: Enhanced YOLOv2 with Feature Pyramid Network (FPN) for multi-scale feature representation and switchable atrous convolution for adaptive receptive fields.

Result: 40.5% mAP at 25% IoU for small cell patches; 68% mAP for FFU virus patch detection.

Conclusion: Combining FPN-based feature fusion with switchable convolution improves YOLOv2's performance for specialized biomedical object detection tasks.
...
TL;DR: Introduces Proximity-guided Identity Unlearning (PIU) to remove specific identities from Arc2Face diffusion models by reassigning identity embeddings to anchor identities and fine-tuning selective cross-attention layers.


<details>
  <summary>Details</summary>
Motivation: Identity-conditioned face generation models raise privacy concerns as they can continue generating individuals' faces despite the right to be forgotten. Existing machine unlearning methods focus on concept/data removal but not identity unlearning in embedding-conditioned models.

Method: Formulates identity removal as an identity replacement objective that reassigns source identity to a selected anchor identity in the learned identity space, with a proximity-based anchor selection strategy derived from ArcFace geometry. Unlearning is achieved through localized fine-tuning of a small subset of identity-sensitive cross-attention layers.

Result: Effectively suppresses target identity generation while preserving realism and identity consistency for retained identities, validated by improved unlearning and image-quality metrics and qualitative evaluation.

Conclusion: PIU achieves effective identity unlearning in Arc2Face models with minimal performance degradation on retained identities, providing a practical solution for privacy-compliant face generation.
...
TL;DR: A hybrid k-space-to-image deep learning model improves breast lesion segmentation robustness under MRI undersampling and noise, matching image-space methods at full sampling.


<details>
  <summary>Details</summary>
Motivation: To investigate whether breast lesion segmentation can be learned directly from acquired MRI k-space and whether doing so improves robustness under accelerated or noisy data conditions.

Method: Compared four 3D U-Net variants (hybrid k-space-to-image, native k-space, magnitude and complex image-space baselines) on public breast DCE-MRI datasets under increasing undersampling and added complex Gaussian k-space noise, using patient-level Dice similarity coefficient.

Result: At full sampling, hybrid and image-space models performed similarly. Under acceleration and noise, the hybrid model retained substantially more accuracy and degraded more slowly, significantly outperforming the magnitude image-space baseline at moderate-to-high undersampling. Feature analysis showed complementary roles of k-space and image-space stages.

Conclusion: K-space-aware deep learning improves robustness of breast lesion segmentation under MRI undersampling and k-space noise while matching image-space performance at full sampling.
...
TL;DR: This study introduces new NMCA-aligned LULC classification schemes and a benchmark MS LiDAR dataset, evaluating seven DL models. Point Transformer V3 achieves best performance (79.4% mIoU for L1, 58.9% for L2), with multispectral information improving results over geometry-only inputs.


<details>
  <summary>Details</summary>
Motivation: LULC classification is essential for national mapping, but adoption of MS LiDAR and DL is limited by the lack of publicly available urban/suburban MS LiDAR datasets aligned with NMCA classification schemes.

Method: Introduced L1 and L2 NMCA-aligned LULC classification schemes, created a new benchmark MS LiDAR dataset (Loosdorf-MSL), evaluated seven state-of-the-art DL models, and performed spectral ablation studies at both levels of detail using a dual-wavelength LiDAR system (532 nm and 1064 nm).

Result: Point Transformer V3 achieved mIoU of 79.4% (L1, 8 classes) and 58.9% (L2, 20 classes). Multispectral information improved performance by 1.1 pp at L1 and 7.8 pp at L2 over geometry-only inputs.

Conclusion: Multispectral LiDAR reflectance is valuable for fine-grained material discrimination, supporting NMCA LULC scheme evolution toward higher semantic detail. The Loosdorf-MSL dataset provides a new benchmark for consistent national/international LULC mapping.
...
TL;DR: 4D-GSW is a kinematic-aware watermarking framework for 4D Gaussian Splatting that embeds robust copyright information while preserving spatio-temporal consistency by protecting motion manifolds from non-physical perturbations.


<details>
  <summary>Details</summary>
Motivation: Existing steganographic techniques for 4D dynamic reconstruction neglect underlying kinematic manifolds, causing non-physical artifacts like temporal flickering and FVD collapse. There is a need for a watermarking approach that preserves physical motion coherence.

Method: Three key components: (1) Spatio-Temporal Curvature (STC) metric to identify "Dynamic Instants" for adaptive watermark gradient gating; (2) HMM-MRF energy minimization for synchronizing watermark phases across temporal and spatial dimensions; (3) Anisotropic gradient routing to decouple watermark embedding from photometric reconstruction fidelity.

Result: Superior performance in robust watermark hiding while resisting various attacks, maintaining high rendering quality, and preserving spatiotemporal consistency.

Conclusion: 4D-GSW effectively addresses the gap in intellectual property protection for 4D dynamic assets by explicitly modeling and preserving kinematic coherence during watermark embedding.
...
TL;DR: Bernini unifies MLLMs (semantic planning) and diffusion models (pixel rendering) for video generation and editing, achieving SOTA by using ViT embedding space as an interface between separately trained planner and renderer.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between MLLMs' strong semantic reasoning and diffusion models' photorealistic rendering by proposing a simple division of labor, enabling both strengths to be leveraged in a unified video generation/editing framework.

Method: Bernini uses an MLLM-based planner to predict semantic representations in ViT embedding space, a DiT-based renderer for pixel synthesis, with SA-3D RoPE for multi-visual input handling and chain-of-thought reasoning for better understanding-to-generation transfer. Planner and renderer are trained separately with minimal co-training.

Result: Achieves state-of-the-art performance across diverse video generation and editing benchmarks, demonstrating strong generalization on challenging editing tasks due to pretrained MLLM understanding.

Conclusion: The simple division of labor between MLLM-based semantic planning and diffusion-based rendering is effective for unified video generation and editing, with separate training preserving both components' strengths.
...
TL;DR: QuantSR+ is a unified low-bit quantization framework for super-resolution (SR) models that introduces Redistribution-driven Bit Determination (RBD), Quantized Slimmable Architecture (QSA), and Slimming-guided Function-localized Distillation (SFD). It achieves state-of-the-art ultra-low-precision (2-4 bits) performance on both CNN and transformer SR models with significant efficiency gains (up to 87.9% ops and 89.4% storage reduction).


<details>
  <summary>Details</summary>
Motivation: Low-bit quantization (2-4 bits) of SR models causes severe performance drops due to limited representational capacity and SR's detail-sensitive nature. Existing methods struggle to balance accuracy and efficiency at ultra-low precision.

Method: Three components: (1) Redistribution-driven Bit Determination (RBD) reshapes quantization distributions in forward/backward passes; (2) Quantized Slimmable Architecture (QSA) starts from an over-parameterized model and prunes less critical blocks; (3) Slimming-guided Function-localized Distillation (SFD) enforces block-aware feature alignment with progressive training.

Result: Outperforms SOTA on multiple benchmarks. SwinIR-S on Urban100 (x4) gains +0.29 dB PSNR over 2-bit SOTA. Achieves 87.9% ops reduction and 89.4% storage savings at 2-bit.

Conclusion: QuantSR+ provides an effective unified framework for low-bit SR quantization, applicable to both CNN and transformer-based models, achieving superior accuracy-efficiency trade-offs.
...
TL;DR: VEELA is a new liver vessel dataset from 40 CTA scans with strict manual annotation, enabling reproducible evaluation of hepatic and portal vessel segmentation with multi-perspective metrics.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack clinically realistic annotation constraints for hepatic/portal vessel segmentation, which remains challenging due to complex topology and acquisition ambiguities.

Method: 40 CTA scans from CHAOS cohort; slice-by-slice multi-expert manual annotation with visibility-driven policy, no anatomical interpolation; benchmarking framework with clDice, IoU, NSD, area, length metrics.

Result: Different metrics capture distinct aspects of vascular integrity; multi-perspective evaluation is necessary for clinically meaningful vessel segmentation.

Conclusion: VEELA facilitates reproducible research and robust vascular segmentation; publicly released with evaluation metrics, dataset, and submission platform.
...
TL;DR: GazePrior uses a data-driven 3D prior to synthesize realistic eye-tracking training data, eliminating costly data collection for new devices while outperforming zero-shot methods.


<details>
  <summary>Details</summary>
Motivation: Training ET models for new devices is expensive and time-consuming due to real data collection costs, and existing synthetic data lacks realism.

Method: Introduces GazePrior, a data-driven 3D prior modeling human eye distribution across identities, gazes, and lighting. Enables sparse-input 3D reconstruction from prior devices, rendered for target device cameras.

Result: Synthesized data achieves realism, diversity, and accuracy of real data without costs. ET models trained on it outperform zero-shot methods in accuracy and robustness.

Conclusion: GazePrior effectively removes the need for additional data collection for new ET devices while maintaining high data quality.
...
TL;DR: AgroTools is a benchmark with 539 QA instances, 1,097 agricultural images, 14 tools, and 5 task families for evaluating tool-augmented multimodal agents in agriculture, revealing current models struggle with tool planning, argument generation, execution recovery, and answer synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing agricultural multimodal benchmarks focus on final-answer correctness and lack support for assessing models' ability to use external tools in precision-sensitive workflows.

Method: Constructed AgroTools with 539 QA instances, 1,097 images, 14 tools, 5 task families, and structured tool-use traces for dual-view evaluation (process-level and outcome-level).

Result: Benchmarked 9 open-source and 4 closed-source MLLMs; current models are unreliable in agricultural tool-use, with bottlenecks in tool planning, argument generation, execution recovery, and final-answer synthesis.

Conclusion: AgroTools exposes critical gaps in multimodal agent capabilities for precision agriculture; the benchmark is publicly available to support future research.
...
TL;DR: LLM-sEMG is a novel framework that uses large language models as activity recognizers for sEMG signals by converting continuous signal sequences into "sEMG language" through a language-oriented mapping mechanism, achieving highly accurate recognition.


<details>
  <summary>Details</summary>
Motivation: Leveraging LLMs' implicit knowledge learned from extensive linguistic descriptions of actions to interpret sEMG signals and infer activity intentions, moving beyond traditional approaches of larger architectures or large-scale pretraining.

Method: Proposes LLM-sEMG framework with a language-oriented mapping mechanism that converts continuous sEMG sequences into sEMG language, integrating several strategies to facilitate signal-to-language mapping, then using LLMs as sEMG activity recognizers.

Result: Extensive experiments demonstrate that the proposed framework achieves highly accurate sEMG signal-based activity recognition using large language models.

Conclusion: LLMs can effectively serve as sEMG activity recognizers through proper signal-to-language mapping, opening new possibilities for sEMG-based activity recognition.
...
TL;DR: ReceiptBench is a large-scale (10k receipts) benchmark for visual document information extraction, organized into four hierarchical sub-tasks, paired with a two-stage training framework using Metric-Aware GRPO that achieves SOTA performance on complex reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VIE benchmarks lack scale, realism, semantic granularity, and diverse document types, hindering evaluation of MLLMs on structured document understanding.

Method: Proposes ReceiptBench (10k human-annotated receipts) with 4 hierarchical sub-tasks (Basic Perception, Format Normalization, Semantic Reasoning, Structure Parsing) and a two-stage training framework with Metric-Aware GRPO that converts evaluation constraints into RL signals.

Result: State-of-the-art performance, surpassing leading proprietary models on complex reasoning tasks.

Conclusion: The proposed benchmark and training framework effectively improve structural consistency and reasoning capabilities for VIE tasks.
...
TL;DR: FundusGround is a new benchmark for interpretable ophthalmic VQA that incorporates spatially-grounded lesion evidence using ETDRS grid, collecting 10,719 fundus images with 15,595 annotated lesions and 72,706 questions, demonstrating that lesion-level visual grounding improves model performance and transparency.


<details>
  <summary>Details</summary>
Motivation: Existing ophthalmic VQA benchmarks focus only on answer accuracy and neglect explicit visual evidence needed for clinical interpretability.

Method: A three-stage pipeline collecting 10,719 fundus images with 15,595 lesions, spatially localized via ETDRS grid, generating 72,706 questions in four formats (open-ended, closed-ended, single-choice, multiple-choice).

Result: Incorporating lesion-level visual evidence consistently improves model performance and transparency in ophthalmic VQA.

Conclusion: Explicit spatial grounding of lesions is necessary for reliable and explainable ophthalmic VQA.
...
TL;DR: The paper highlights that ignoring baselines in model interpretation methods leads to inaccurate results, reformulates interpretation principles, unifies gradient-based methods, identifies baseline flaws, and proposes a revised method with clear baselines that supports multi-layer interpretation.


<details>
  <summary>Details</summary>
Motivation: Existing model interpretation methods often ignore baselines, leading to imprecise or incorrect interpretations. The paper aims to reformulate interpretation tasks and principles to demonstrate the importance of baselines.

Method: The paper unifies gradient-based methods, Integrated Gradients (IG), and Taylor expansion, clarifies their connections, identifies baselines for each, analyzes flaws in IG, LayerCAM, ODAM, and Difference Map, and revises IG with a clear baseline to support multi-layer feature interpretation.

Result: The revised method achieves better interpretation results by incorporating a clear and reasonable baseline, supporting interpretation from any layer, with differences reflecting varying feature extraction stages.

Conclusion: Proper baselines are crucial for accurate model interpretation. The proposed method resolves previous flaws and enables meaningful multi-layer interpretation.
...
TL;DR: FastTab is a grid-centric TSR model that avoids autoregressive HTML decoding by using a TRM module and axial 1D Transformers to predict table structure directly, achieving competitive accuracy with low-latency inference across four benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing TSR methods rely on autoregressive HTML decoding which is slow, and they need to jointly handle table-level coherence (rows/columns/cells) and precise separator localization efficiently.

Method: FastTab combines a Tiny Recursive Module (TRM) for global reasoning and axial 1D Transformer encoders for long-range dependencies. It predicts row/column counts, headers, and separators to construct a grid, then infers rowspan/colspan via ROI-aligned cell features.

Result: Competitive structure recovery performance on PubTabNet, FinTabNet, PubTables-1M, and SciTSR with low-latency inference.

Conclusion: FastTab demonstrates that a grid-based approach without autoregressive decoding can achieve both accuracy and speed for TSR, and it generalizes to curved separators for camera-captured documents.
...
TL;DR: Proposes a unified dual-shutter framework (Blur-RS) that jointly leverages GS blur and RS distortion to reconstruct high-speed video, with a novel dataset and dual-stream network achieving superior motion deblurring and temporal super-resolution.


<details>
  <summary>Details</summary>
Motivation: Prior works treat blur decomposition and RS temporal super-resolution as separate tasks, failing to exploit their intrinsic complementarity for resolving motion degradation ambiguities.

Method: 1) Dual-shutter setup capturing synchronized blur-RS image pairs; 2) Stereo Blur-RS configuration for flexible performance-cost trade-offs; 3) Triaxial imaging system collecting real-world dataset; 4) Dual-stream motion interpretation module for context-aware & temporally-sensitive representations; 5) Self-prompted frame reconstruction stage.

Result: Extensive experiments validate superiority and generalizability, establishing a new paradigm for realistic high-speed video reconstruction under complex motion degradations.

Conclusion: The unified framework effectively inverts motion degradation by leveraging complementary GS blur and RS distortion, achieving state-of-the-art performance on both synthetic and real-world data.
...
TL;DR: The paper shows that synthetic low-light data, generated via RAW image augmentation matching real sensor noise, can effectively characterize pedestrian detection performance in the dark, with metrics closely aligning between real and synthetic samples.


<details>
  <summary>Details</summary>
Motivation: Real datasets are sparse, uneven, and long-tailed, hindering generalization and evaluation. Synthetic data can fill gaps to improve coverage, especially for safety-critical autonomous driving scenarios like pedestrian detection in low light.

Method: The authors use synthetic RAW image augmentation to generate low-light samples matching the camera sensor's noise model, then evaluate a state-of-the-art object detector on both real and synthetic low-light data.

Result: Performance metrics on real and synthetic low-light data are similar, indicating the AI model cannot easily distinguish between them, validating the synthetic augmentation approach.

Conclusion: Synthetic low-light augmentation可以有效填补真实数据在低光照区域的稀疏性，为自动驾驶中的行人检测提供更可靠的性能评估手段。
...
TL;DR: SADGE is a new metric combining appearance and geometric similarity to predict how well synthetic datasets will perform on real-world computer vision tasks without needing to train models.


<details>
  <summary>Details</summary>
Motivation: Existing metrics (PSNR, FID, CLIP) only measure semantic alignment or structural similarity separately, and no prior work evaluates which metric best predicts downstream performance for synthetic datasets.

Method: SADGE measures the nonlinear interplay between appearance (e.g., DINOv3) and geometric (e.g., MASt3R) similarity scores via a constrained bilinear interaction, tested across 5 benchmark families and 15 dataset variants (79k image pairs).

Result: Achieves Pearson r=0.88 and Spearman ρ=0.77 correlation with downstream transfer performance, outperforming both geometry-only and appearance-only baselines.

Conclusion: Neither appearance nor geometry alone reliably predicts synthetic data utility; their nonlinear combination via SADGE is necessary and most effective.
...
TL;DR: MaSC uses foreground masks to separately measure concept preservation and prompt following in text-to-image generation, outperforming global embedding metrics like CLIP-I/DINO and matching or exceeding GPT-4-level performance.


<details>
  <summary>Details</summary>
Motivation: Existing metrics (CLIP-I, DINO, CLIP-T) correlate poorly with human perception because they attend to the whole image rather than separating the concept subject from the background.

Method: MaSC decomposes evaluation using foreground masks: concept preservation via masked max-cosine matching between reference and generated patches; prompt following via background-only pooled image embedding vs. subject-stripped prompt embedding, using frozen SigLIP2 SO400M-NaFlex features.

Result: On DreamBench++: Krippendorff alpha = 0.471 for concept preservation (beats all non-LLM baselines and GPT-4V, approaches GPT-4o). On ORIDa: AUC = 0.992. Prompt-following score outperforms CLIP-T baseline.

Conclusion: Spatially decomposed aggregation is a strong design principle for evaluating concept-driven generation.
...
TL;DR: Proposes a hierarchical Perception-to-Deliberation Framework (PDF) for Zero-Shot Compositional Image Retrieval, introducing experience self-evolution and Test-Time Scaling Law, achieving SOTA on CIRR, CIRCO, and FashionIQ.


<details>
  <summary>Details</summary>
Motivation: Existing ZS-CIR methods suffer from Perception Myopia (single space) or Logic Drift (iterative collaboration due to retriever perception ceiling), lacking fine-grained compositional reasoning.

Method: Hierarchical multi-agent framework with (1) Intent Routing Manager for dynamic multi-view Worker perception signal dispatch, (2) Decision Manager combining Training-free Reasoning Policy Distillation and Tournament-style TTS for self-evolving reasoning.

Result: Achieves SOTA on CIRR, CIRCO, and FashionIQ benchmarks.

Conclusion: Experience-driven self-evolution and Test-Time Scaling Law are promising and scalable for zero-shot fine-grained multimedia retrieval.
...
TL;DR: VLMs require expensive end-to-end training; this work recycles pretrained vision model classification heads as semantic prototypes for efficient post-hoc alignment, boosting cross-modal retrieval and classification performance.


<details>
  <summary>Details</summary>
Motivation: Expensive end-to-end training of VLMs requires massive paired datasets; even post-hoc methods still demand substantial paired data. The authors explore reusing discarded classification heads to reduce data requirements.

Method: Recycles classification heads from pretrained vision models as semantic prototypes for zero-shot alignment (as semantic anchors) and as a data augmentation strategy by mixing prototypes with real image-text pairs, integrated with existing post-hoc alignment techniques.

Result: Consistently boosts accuracy in cross-modal retrieval, zero-shot, and few-shot classification tasks when combined with several state-of-the-art post-hoc alignment methods.

Conclusion: Repurposing discarded classification heads as semantic prototypes is an effective, low-cost strategy to improve post-hoc alignment without requiring massive paired datasets.
...
TL;DR: A training-free two-stage framework (SAM3 + DINOv3) for fine-grained mushroom segmentation under long-tailed distributions and varying image conditions, achieving strong results in low-data regimes.


<details>
  <summary>Details</summary>
Motivation: Fine-grained semantic segmentation is challenging due to visually similar classes, long-tailed distributions, and variable imaging conditions, especially in the domain of mushroom (FungiTastic) segmentation.

Method: Two-stage framework: (1) SAM3 produces class-agnostic masks using macro-taxonomic prompts; (2) DINOv3 assigns fine-grained labels via prototype matching in the embedding space, with a simple feature space transformation to improve classification.

Result: First baseline for fine-grained semantic segmentation in low-data settings (one-shot to few-hundred-shot regimes); more scalable and cost-effective than class-specific prompting.

Conclusion: Decoupling segmentation from classification with class-agnostic prompting + prototype matching is an effective, training-free approach for fine-grained segmentation under data scarcity and domain challenges.
...
TL;DR: The paper introduces SpaceDG, a large-scale dataset for evaluating how visual degradations (blur, low light, etc.) affect MLLMs' spatial reasoning, revealing a critical robustness gap and showing that finetuning on SpaceDG improves performance under degraded conditions without sacrificing clean-image accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing spatial reasoning benchmarks assume pristine visual inputs and overlook real-world degradations (motion blur, low light, adverse weather, etc.), raising the question of how robust MLLMs' spatial intelligence is under imperfect visual observations.

Method: They construct SpaceDG with a physically grounded degradation synthesis engine embedded into 3D Gaussian Splatting (3DGS) rendering, producing ~1M QA pairs from ~1,000 indoor scenes across 9 degradation types, plus a human-verified benchmark (SpaceDG-Bench) with 1,102 questions covering 11 reasoning categories.

Result: Evaluation of 25 open- and closed-source MLLMs shows that visual degradations consistently and substantially impair spatial reasoning, exposing a critical robustness gap. Finetuning on SpaceDG markedly improves degradation robustness and can even surpass human performance under degraded conditions without any performance drop on clean images.

Conclusion: Degradation-aware training is a promising direction for achieving robust spatial intelligence in MLLMs under real-world visual conditions.
...
TL;DR: SAMOSA adapts SAM 2 for visual object tracking by adding explicit motion, geometry, and semantic modeling, outperforming existing SAM 2-based and supervised trackers, especially in complex nonlinear motion scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional VOT methods rely on task-specific supervised training and generalize poorly. SAM 2 provides strong video priors but lacks explicit modeling of motion dynamics and cross-frame consistency, making it suboptimal for tracking.

Method: SAMOSA introduces: (1) a lightweight nonlinear motion predictor for target dynamics and mask selection/memory filtering; (2) semantic cues for detecting target shifts and failure recovery; (3) geometric cues as structural constraints for stability.

Result: Outperforms SOTA SAM 2-based methods on general benchmarks, shows better generalization than supervised VOT methods, and achieves substantial gains on anti-UAV datasets with nonlinear motion.

Conclusion: Bridging SAM 2's implicit video understanding with explicit tracking-oriented modeling (motion, geometry, semantics) yields a more robust and generalizable tracker, especially under complex scenarios.
...
TL;DR: A framework that uses multimodal knowledge graphs from retrieved similar cases to perform case-aware reasoning for explainable medical image diagnosis, outperforming baselines across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing medical image classification methods rely on isolated visual evidence and cannot leverage similar cases or external knowledge, unlike clinical practice where historical similar cases support diagnosis.

Method: Constructs multimodal knowledge graphs from adaptively retrieved similar cases; uses image-centric Graph Attention Network for knowledge propagation, bidirectional cross-modal attention for feature injection, and confidence-calibrated decision refinement to handle noisy retrieval.

Result: Consistently outperforms strong baselines on multiple medical imaging datasets; ablation studies validate each component's effectiveness.

Conclusion: Case-aware reasoning via multimodal knowledge graphs effectively improves medical image classification and provides interpretable case-level evidence.
...
TL;DR: The paper introduces MOTOR, the first large-scale multimodal dataset for two-wheeler rider behavior in dense, unstructured traffic, comprising 1,629 sequences with synchronized multi-view video, eye-gaze, audio, and telemetry data, annotated for maneuvers and legality. Multimodal fusion (RGB+gaze+telemetry) achieves best performance on behavior recognition tasks.


<details>
  <summary>Details</summary>
Motivation: Two-wheelers account for disproportionately high road fatalities in the Global South, yet research on their rider behavior lags far behind four-wheelers due to lack of multimodal datasets.

Method: Collected 1,629 sequences (25+ hours) from 16 riders with synchronized front/rear/helmet cameras, eye-gaze trackers, on-road audio, and GPS/IMU telemetry. Annotated traffic context, rider state, 12 maneuvers, and legality labels. Benchmarked with CNN and Transformer-based video action recognition models with multimodal fusion.

Result: Combining RGB, gaze, and telemetry modalities consistently outperforms single-modality approaches for rider behavior recognition and maneuver legality classification.

Conclusion: MOTOR provides a unique foundation for advancing safety-critical understanding of two-wheeler riding and serves as a benchmark for behavior analysis, legality-aware prediction, and intelligent transportation systems.
...
TL;DR: The paper introduces U-FIRE (a unified fashion retrieval benchmark) and FashionLens (a MLLM-based framework with Proposal-Guided Spherical Query Calibrator and Gradient-Guided Adaptive Sampling) to achieve versatile fashion image retrieval across diverse query formats and search intentions.


<details>
  <summary>Details</summary>
Motivation: Existing fashion retrieval approaches focus on narrow tasks and fail to support diverse query formats (e.g., text, image, multi-modal) and varied search intentions within a single unified framework.

Method: 1) U-FIRE benchmark consolidating fragmented fashion datasets. 2) FashionLens framework built on Multimodal Large Language Models, featuring a Proposal-Guided Spherical Query Calibrator (adaptive spherical linear interpolation for task-aligned metric spaces) and a Gradient-Guided Adaptive Sampling strategy (re-weighting tasks based on learning difficulty and data scale).

Result: State-of-the-art performance across diverse retrieval scenarios on U-FIRE, with robust generalization to unseen tasks.

Conclusion: FashionLens provides a truly unified solution for diverse fashion retrieval tasks, and the public release of data/code supports reproducibility and future research.
...
TL;DR: GeoWeaver is a framework that performs token-adaptive geometric evidence allocation from a frozen geometry encoder to ground visual representations before spatio-temporal reasoning, outperforming late-fusion geometric approaches.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal models incorporate geometry but treat it as a shared signal across all visual tokens, overlooking that different visual tokens require different geometric evidence depending on their spatial roles.

Method: GeoWeaver constructs a multi-level geometry bank from a frozen geometry encoder, performs token-adaptive geometric evidence allocation, and incorporates selected evidence via residual grounding prior to language modeling.

Result: Extensive evaluations on spatial reasoning benchmarks show GeoWeaver consistently enhances geometry-aware reasoning while retaining general multimodal capabilities.

Conclusion: Geometric information yields the greatest benefit not as a late-fusion auxiliary signal but as a fundamental representational prerequisite for spatio-temporal reasoning in LLMs.
...
TL;DR: This paper proposes a novel framework for generating synthetic cell phantom videos using Elliptical Fourier Descriptors (EFDs) to model temporal evolution of cell morphology, reducing the need for manual annotation in cell tracking tasks.


<details>
  <summary>Details</summary>
Motivation: Annotating cell tracking videos is extremely time-consuming and requires domain expertise, leading to limited public annotated data for important medical problems like tissue repair or cancer treatment. Synthetic data generation offers a solution but requires time-consistent single cell annotations (phantoms) that replicate biological processes.

Method: The framework represents cell phantom evolution as multivariate time series of Elliptical Fourier Descriptors (EFDs) coefficients—a compact and geometrically interpretable representation for 2D closed contours. This introduces a strong prior for cell morphology and enables efficient generation of temporally coherent sequences.

Result: Experimental validation shows that modeling temporal evolution in EFD space enables generation of biologically plausible phantom videos. The method can be used in generative pipelines for synthesizing annotated data for cell tracking.

Conclusion: This approach strongly mitigates the annotation effort for creating new cell tracking datasets, and the code is publicly available.
...
TL;DR: VGenST-Bench is a video benchmark using generative models to actively synthesize controlled scenarios for evaluating spatio-temporal reasoning in MLLMs, featuring a multi-agent pipeline, 3x2x2 video taxonomy, and hierarchical task suite.


<details>
  <summary>Details</summary>
Motivation: Existing spatio-temporal reasoning benchmarks rely on static images or passively curated videos, limiting fine-grained reasoning evaluation.

Method: Proposes a multi-agent pipeline with human quality control, a 3x2x2 video taxonomy (Spatial Scale, Perspective, Scene Dynamics), and a hierarchical task suite decoupling low-level perception from high-level reasoning.

Result: Shifts evaluation paradigm from passive curation to active synthesis, enabling fine-grained diagnosis of spatio-temporal understanding.

Conclusion: VGenST-Bench enables more precise and controlled evaluation of spatio-temporal reasoning in MLLMs through generative synthesis.
...
TL;DR: SegGuidedNet is a 3D residual encoder-decoder network with a novel SegAttentionGate module that explicitly supervises decoder attention maps for brain tumour sub-regions via a lightweight auxiliary loss (<0.2% parameter overhead). It achieves mean Dice scores of 0.905 (BraTS2021) and 0.897 (BraTS2023 GLI), surpassing single-model nnU-Net and HNF-Netv2, and approaching ensemble Swin UNETR with built-in interpretability at minimal inference cost.


<details>
  <summary>Details</summary>
Motivation: Brain tumour sub-region segmentation from multi-parametric MRI is challenging due to morphological variability, class imbalance, and overlapping appearances across imaging sequences; existing methods often lack interpretability or require expensive ensembles.

Method: A 3D residual encoder-decoder network augmented with a SegAttentionGate module that applies auxiliary supervision to decoder attention maps for each tumour sub-region (necrotic core, peritumoral oedema, enhancing tumour), adding <0.2% parameter overhead. This provides spatial interpretability at inference without post-hoc methods.

Result: Mean Dice: 0.905 (ET=0.873, TC=0.906, WT=0.935) on BraTS2021 and 0.897 (ET=0.859, TC=0.902, WT=0.931) on BraTS2023 GLI, outperforming single-model nnU-Net and HNF-Netv2, and within 2-4 Dice points of 10-model ensemble Swin UNETR.

Conclusion: SegGuidedNet offers competitive segmentation accuracy with built-in spatial interpretability in a lightweight, clinically practical framework, and demonstrates strong generalisability across two BraTS benchmark editions.
...
TL;DR: This paper introduces SOSPA (order-aware metric for individual geometries) and PLD (soft metric for multi-instance evaluation) to address limitations of Chamfer distance-based mAP in online map estimation for autonomous driving, revealing detection capability as the main bottleneck in current methods.


<details>
  <summary>Details</summary>
Motivation: Current evaluation using mAP based on thresholded Chamfer distance lacks sensitivity to point ordering and provides limited granularity in assessing geometric quality, making it difficult to distinguish which methods truly excel.

Method: Proposes SOSPA (sequence optimal sub-pattern assignment) for order-aware single-instance similarity, and PLD (polyline localisation and detection) for soft multi-instance evaluation replacing hard thresholding with principled soft assignment.

Result: PLD effectively ranks SOTA methods (MapTRv2, StreamMapNet, MapTracker) on nuScenes and provides decomposed error analysis, identifying detection capability as the dominant bottleneck that mAP fails to capture.

Conclusion: The proposed metrics (SOSPA and PLD) offer more sensitive and principled evaluation for online map estimation, revealing insights about detection being the primary bottleneck in current methods.
...
TL;DR: A method for floorplan localization in large-scale, real-world settings by reconstructing a 3D scene from images, projecting it into a 2D density map as a floorplan proxy, and aligning it with rasterized floorplans using a fine-tuned 2D foundation model.


<details>
  <summary>Details</summary>
Motivation: Existing floorplan localization methods assume controlled small-scale environments and precise vectorized floorplans, limiting real-world applicability in large-scale buildings and with rasterized floorplans.

Method: Reconstructs a gravity-aligned 3D scene from an image collection, projects it into a 2D density map, then aligns this proxy with the floorplan via a 2D similarity transform. A 2D foundation model is fine-tuned to learn cross-modal correspondences between density maps and architectural floorplans.

Result: Substantial improvements over prior methods, including in extremely sparse settings with as little as a single input image.

Conclusion: The proposed approach effectively enables floorplan localization in the wild, handling large-scale buildings and rasterized floorplans where previous methods fail.
...
TL;DR: This paper benchmarks eight noisy-label learning methods on frozen vision foundation models across 150 conditions (5 datasets, 3 backbones, 2 noise types, 5 rates), finding no universal winner. ELR wins most conditions, CUFIT has best mean rank. The small-loss assumption degrades under frozen features (53-61% loss overlap). Co-Teaching collapses minority classes. The work reframes noisy-label learning as regime-aware method selection.


<details>
  <summary>Details</summary>
Motivation: Noisy-label learning methods for frozen Vision Foundation Models (VFMs) remain poorly understood, and existing methods still rely on the small-loss assumption inherited from end-to-end training, which may not hold under frozen features.

Method: Controlled benchmark of 8 noisy-label methods across 150 conditions (5 medical datasets, 3 backbones, 2 noise types, 5 noise rates, 6,000 training runs). Uses Friedman ranking, analyzes loss distribution overlap (53-61%) and prediction agreement stability under frozen DINOv2 features.

Result: No universal winner; ELR wins 49/150 conditions, CUFIT achieves best mean rank (2.51). Cost of method selection grows from 4.5pp (clean) to 18.8pp (40% asymmetric noise). Clean/noisy loss overlap 53-61%. Prediction agreement more stable than loss ranking (3pp vs 13pp precision drop). Co-Teaching collapses to 35.1% balanced accuracy on ISIC2019 with 40% asymmetric noise.

Conclusion: Noisy-label learning for frozen VFMs should be treated as a regime-aware method-selection problem, not a search for a single dominant algorithm. Provides evidence-based guidance and a low-regret feature-space selector.
...
TL;DR: VFMs improve scene understanding but not gaze reasoning in gaze following. The authors propose a head-conditioned local LoRA and out-of-cone penalty to enhance gaze reasoning, achieving SOTA on GazeFollow and VAT, especially for non-salient targets.


<details>
  <summary>Details</summary>
Motivation: Existing VFM-based gaze following methods rely on semantically salient objects rather than true gaze cues, degrading performance when targets are not salient.

Method: (1) Head-conditioned local LoRA for localized adaptation preserving scene token learning while improving head token learning. (2) Out-of-cone penalty injecting gaze cues into head tokens while aligning them with scene tokens.

Result: State-of-the-art performance on GazeFollow and VAT datasets, with strong improvements when gaze targets are not semantically salient.

Conclusion: Enhancing gaze reasoning in VFMs is crucial for robust gaze following; the proposed method offers valuable insights for future work.
...
TL;DR: GLeVE is a graph-guided framework that grounds radiology report descriptions to 3D CT volumes using relation-aware graph reasoning, anatomy-aware proposal generation, and octree-based refinement, outperforming existing methods on AbdomenAtlas 3.0.


<details>
  <summary>Details</summary>
Motivation: Existing report-assisted and vision-language grounding methods rely on phrase-level alignment or dense pixel supervision, leading to limited lesion-wise correspondence and suboptimal localization accuracy.

Method: GLeVE treats lesion descriptions as atomic semantic units, encodes organ attribution/attributes/inter-lesion relations via relation-aware graph reasoning, uses anatomy-aware proposal generation with region-level verification for one-to-one alignment, and applies hierarchical octree refinement for boundary delineation.

Result: Consistent improvements over classical multimodal foundation models and report-supervised baselines in segmentation accuracy and lesion-level localization on AbdomenAtlas 3.0.

Conclusion: Graph-guided lesion grounding with anatomical prior and octree refinement effectively bridges the semantic-spatial gap for verifiable clinical interpretation from radiology reports to 3D CT volumes.
...
TL;DR: H-Flow is a dense human scene flow method that captures both skeletal kinematics and surface deformation from monocular video using a multi-head transformer, trained without ground-truth labels by leveraging physics-based priors and a new synthetic benchmark (DynAct4D).


<details>
  <summary>Details</summary>
Motivation: Parametric human models cannot represent non-rigid surface dynamics (clothing/soft tissue), while generic scene flow fails on articulated bodies and lacks scalable supervision.

Method: A unified multi-head transformer estimates flow from monocular video, jointly predicting pose and depth. Training uses cross-modal physics-based priors (geometric, structural, biomechanical) instead of ground-truth labels, plus the DynAct4D synthetic benchmark for supervised signal.

Result: Outperforms scene-flow and parametric baselines on standard benchmarks; generalizes zero-shot to in-the-wild video.

Conclusion: H-Flow effectively captures dense human motion including non-rigid dynamics without expensive labels, enabled by physics-grounded training and a high-fidelity synthetic dataset.
...
TL;DR: Swift Sampling is a training-free, lightweight frame selection method that identifies temporally surprising moments in long videos using predictive coding principles (Taylor expansion in latent space), achieving 30x cheaper overhead than baselines and up to +12.5 accuracy gains on long-video QA tasks.


<details>
  <summary>Details</summary>
Motivation: Long videos contain redundant frames; critical information lies in temporal surprises where visual features deviate from predicted evolution. Prior methods require auxiliary networks or video-specific hyperparameter tuning.

Method: Models video as differentiable trajectory in visual latent space, computes velocity/acceleration, applies Taylor expansion to project expected frame evolution, and selects frames that diverge sharply from the predicted manifold as "temporally surprising."

Result: Adds only 0.02x additional computational cost (30x cheaper overhead than baselines); outperforms uniform sampling and query-agnostic baselines across 3 long-video QA benchmarks and 10 downstream tasks; improves accuracy by up to +12.5 points with limited frame budgets.

Conclusion: Swift Sampling is an elegant, efficient, and effective training-free frame selection method that leverages predictive coding theory for long-form video understanding.
...
TL;DR: AtomicMotion reconstructs full-body poses from sparse head/hand trajectories by decoupling body dynamics into five functional clusters, using masked pre-conditioning and kinematic attention, achieving state-of-the-art performance on AMASS.


<details>
  <summary>Details</summary>
Motivation: Current methods suffer from error accumulation and unnatural joint coordination because they treat the body as a monolithic entity, ignoring fine-grained atomic intents and structural topology.

Method: (1) Logical body partitioning into five functional clusters; (2) Masked full-body pre-conditioning during training; (3) Kinematic Attention that embeds classical kinematic tree structure into attention.

Result: Significantly outperforms existing baselines on AMASS dataset with higher reconstruction fidelity and superior biomechanical realism.

Conclusion: Decoupling and re-integrating body dynamics with structural priors (functional partitioning + kinematic attention) is effective for sparse-to-full body pose reconstruction.
...
TL;DR: Proposes a metadata-conditioned causal hierarchical VAE for generating counterfactual AP spine DXA images conditioned on age intervention, validated against real follow-up data.


<details>
  <summary>Details</summary>
Motivation: DXA is widely used for skeletal assessment, but learning controllable and interpretable factor-specific anatomical variation remains challenging.

Method: A metadata-conditioned causal hierarchical variational autoencoder (CHVAE) trained on 3,743 AP spine DXA scans from UK Biobank, using abduction-action-prediction (AAP) for causal consistency evaluation.

Result: Strong absolute-level agreement for key vertebral morphometry variables under age intervention when comparing counterfactual outputs to actual repeat-imaging measurements.

Conclusion: The approach supports intervention-aligned synthesis of anatomically plausible DXA images, enabling causally consistent generation.
...
TL;DR: AnyMo is a geometry-aware framework that uses physics-based IMU simulation and graph encoding to achieve setup-agnostic human motion modeling, significantly improving zero-shot activity recognition, cross-modal retrieval, and motion captioning across diverse wearable sensing configurations.


<details>
  <summary>Details</summary>
Motivation: Wearable IMU signals are highly dependent on sensing setup (body location, orientation, hardware, etc.), making it difficult to learn motion representations that transfer across devices and datasets, limiting broader use beyond closed-set recognition.

Method: 1) Physics-grounded IMU simulation over dense body-surface placements; 2) Graph encoder pre-trained with paired synthetic placement views and masked partial observations; 3) Tokenization of multi-position IMU into full-body motion tokens; 4) Alignment of tokens with an LLM for motion-language understanding.

Result: Improves average Accuracy/F1/R@2 by 11.7%/11.6%/22.6% on HAR; zero-shot IMU-to-text and text-to-IMU retrieval MRR by 15.9% and 28.6%; zero-shot captioning BERT-F1 by 18.8%.

Conclusion: AnyMo serves as a generalist model for wearable motion understanding in the wild, effectively handling setup-agnostic sensing across diverse tasks and datasets.
...
TL;DR: CPI is a phrase-level curation framework that uses counterfactual phrase interventions to filter training data, improving compositional understanding without sacrificing general performance.


<details>
  <summary>Details</summary>
Motivation: Pair-level alignment signals saturate and conflate broad plausibility with compositional supervision, limiting compositional generalization.

Method: Counterfactual Phrase Intervention (CPI): converts controlled nonce-token substitutions into image-conditioned phrase-sensitivity scores; uses global alignment for coarse mismatch removal then ranks by phrase-level sensitivity.

Result: 50%-data subset improves VL-CheckList-VG Relation by +1.91 over full data, +1.00 over alignment-only filtering; further +3.84 with NegCLIP.

Conclusion: Phrase-level filtering is more effective than pair-level filtering for compositional supervision; CPI is loss-orthogonal and applicable to various CLIP-style models.
...
TL;DR: SegCompass is an end-to-end reasoning segmentation model that uses a Sparse Autoencoder to create an interpretable alignment between chain-of-thought reasoning and visual perception, outperforming or matching SOTA on five benchmarks while providing transparent "white-box" connections.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning segmentation pipelines fail to transparently connect compositional reasoning to visual perception — current methods are either opaque "black boxes" (latent query alignment) or merely readable but not truly interpretable (textual localization readout as unconstrained post-hoc steps).

Method: SegCompass generates a CoT trace, then uses a Sparse Autoencoder (SAE) to map both CoT and visual tokens into a shared sparse concept space. A query codebook selects salient concepts, a slot mapper grounds them into a multi-slot heatmap, and the model is trained jointly with RL for reasoning path and standard segmentation supervision.

Result: Matches or surpasses SOTA on five challenging benchmarks; strong correlation between learned sparse concept quality and final mask accuracy is demonstrated.

Conclusion: The SAE-driven interface provides a traceable "white-box" connection that is more interpretable than latent queries and more coherent than textual readouts, confirming superior results through enhanced and inspectable alignment.
...
TL;DR: SEGA adaptively scales attention across RoPE components based on spatial-frequency structure at each denoising step, improving high-resolution image generation with diffusion transformers without training.


<details>
  <summary>Details</summary>
Motivation: Diffusion transformers (DiTs) struggle with generating images at resolutions beyond their training range, and existing training-free approaches use uniform attention scaling that creates a trade-off between global structure preservation and fine-detail recovery.

Method: SEGA dynamically scales attention across RoPE components according to the latent's spatial-frequency structure at each denoising step, enabling adaptive (content-aware) scaling instead of uniform scaling.
...
Conclusion: Adaptive, content-aware attention scaling based on spatial-frequency structure improves both structural coherence and fine-detail fidelity in high-resolution text-to-image generation without additional training.
...
TL;DR: BehaviorVLA uses causal Mamba-based encoders and phase-conditioned decoders to learn temporally coherent behavior representations, achieving state-of-the-art results on RoboTwin 2.0, LIBERO, and CALVIN while demonstrating superior data efficiency in sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: VLA models suffer performance degradation under distribution shifts due to short-horizon temporal fragmentation and static execution-alignment in existing action-centric latent variable approaches, leading to inconsistent behaviors in complex scenarios.

Method: Two symmetric components: (1) Visuomotor Behavior Encoder (VBE) using causal Mamba-based architecture for long-horizon trajectory aggregation, and (2) Phase-conditioned Behavior Decoder (PBD) for decoding representations into actions with dynamic task-level prior alignment.

Result: State-of-the-art success rates: 58% on RoboTwin 2.0, 98% on LIBERO, 4.36 Avg.Len on CALVIN. Matches OpenVLA-OFT performance using only 50% demonstration data in real-world sim-to-real transfer.

Conclusion: BehaviorVLA achieves robust manipulation through temporally coherent behavioral representations, demonstrating superior data efficiency and generalization capabilities for VLA models.
...
TL;DR: Slimmable ConvNeXt achieves width-adaptive inference by leveraging ConvNeXt's modern design (LayerNorm, inverted bottlenecks), eliminating the need for switchable batch normalization and producing a simpler training pipeline. It outperforms prior CNN and ViT approaches on ImageNet-1k across multiple compute budgets.


<details>
  <summary>Details</summary>
Motivation: Deploying vision models across devices with varying resource constraints or fluctuating compute requires separate models; width-adaptive inference aims to train a single set of shared weights containing multiple nested subnetworks of increasing capacity, but prior CNN methods required switchable batch normalization and recent scalable methods focused on Vision Transformers.

Method: The authors present Slimmable ConvNeXt, which exploits ConvNeXt's modern design—specifically LayerNorm and inverted bottlenecks—to enable channel-width slimming without the normalization overhead of classical slimmable networks, resulting in a simpler training pipeline than both prior CNN and ViT approaches.

Result: Slimmable ConvNeXt-T achieves 80.8% top-1 accuracy at 4.5 GMACs and 77.4% at 1.2 GMACs on ImageNet-1k, outperforming HydraViT, MatFormer-S, and SortedNet-S at comparable compute. Scaling to Slimmable ConvNeXt-B reaches 82.8% at 15.35 GMACs.

Conclusion: ConvNeXt's modern architecture makes it particularly suited for width-adaptive slimming, achieving strong performance across diverse compute budgets with a simpler training pipeline than prior methods.
...
TL;DR: CEDAR learns an invertible rotation to disentangle vision-language embeddings into interpretable, axis-aligned concepts without increasing dimensionality, achieving competitive sparsity and better human-aligned explanations.


<details>
  <summary>Details</summary>
Motivation: Vision-language model embeddings are powerful but opaque; existing methods like sparse autoencoders expand dimensionality, compromising geometry and introducing redundancy.

Method: CEDAR applies an invertible transformation with a top-k sparsity bottleneck to rotate pretrained embeddings into disentangled, axis-aligned coordinates.

Result: Competitive reconstruction-sparsity trade-off; explanations are more interpretable and better aligned with human perception than baselines.

Conclusion: Apparent entanglement in vision-language representations can be resolved through a suitable change of basis, eliminating the need for overcomplete expansions.
...
TL;DR: A two-stage action detection method using augmented virtual viewpoints and state-space temporal encoding achieves state-of-the-art results on PKU-MMD and BABEL benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing appearance-based methods struggle with viewpoint diversity in training, and motion-based methods fail to model fine-grained temporal relationships across consecutive motion windows.

Method: Two-stage approach: (1) extract motion features from augmented virtual viewpoints (training only); (2) view-invariant multi-scale temporal encoder using selective state-space sequence modeling to aggregate across viewpoints and time scales.

Result: Significantly outperforms state-of-the-art methods on PKU-MMD and BABEL benchmarks across all splits.

Conclusion: The proposed approach improves both view-invariance and global temporal coherence for human action detection in untrimmed videos.
...
TL;DR: Proposes an orientation-aware zero-shot action recognition method that uses multi-viewpoint motion cues and orientation-specific text prompts to improve cross-domain generalization and performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing ZSAR models assume similar geometric conditions between training and inference, but real-world variations in human body orientation and camera viewpoint create a domain gap that limits generalization to novel action-motion combinations.

Method: Combines motion cues from multiple camera viewpoints with text descriptions during training; introduces an orientation-aware motion encoding network to learn diverse motion features and adapts orientation-aware text prompts to match corresponding features at inference.

Result: Outperforms recent state-of-the-art zero-shot approaches on NTU-RGB+D, BABEL, NW-UCLA, and two surveillance datasets; also shows strong transfer learning for cross-domain and same-domain recognition of seen actions.

Conclusion: Orientation-aware modeling is effective for bridging domain gaps in ZSAR, improving both zero-shot recognition and transfer learning without requiring strong annotation efforts.
...
TL;DR: WorldKV is a training-free framework that enables persistent world generation in autoregressive video diffusion models by selectively retrieving and compressing KV-cache chunks, achieving full-KV memory fidelity with 2x throughput improvement.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video diffusion models struggle to maintain a persistent world (consistent content when revisiting viewpoints) under real-time constraints, as full KV-cache attention has linear memory/cost growth with rollout length, while sliding window inference sacrifices long-term consistency.

Method: WorldKV has two components: (1) World Retrieval - stores evicted KV-cache chunks in GPU/CPU memory and retrieves scene-relevant chunks via camera/action correspondence; (2) World Compression - prunes redundant tokens via key-key similarity to an anchor frame, halving per-chunk storage.

Result: On Matrix-Game-2.0 and LingBot-World-Fast, WorldKV matches or exceeds full-KV memory fidelity at roughly 2x the throughput, and is competitive with memory-trained baselines without fine-tuning.

Conclusion: WorldKV demonstrates that training-free KV-cache management with retrieval and compression can effectively maintain long-term consistency in video world models while satisfying real-time throughput constraints.
...
TL;DR: This paper discovers that generated images have an anomalous "spectral tail uplift" in ultra-high frequencies, and proposes STAL—a frequency-domain auxiliary training framework that improves AI-generated image detection without adding inference overhead.


<details>
  <summary>Details</summary>
Motivation: Existing detection methods exploit frequency-domain cues but lack a systematic understanding of spectral regularities in generated images, making detection insufficiently generalizable.

Method: The authors analyze 1D radial log-power spectra to identify spectral tail uplift in generated images, then propose STAL (Spectral Tail Auxiliary Learning), which transfers frequency-domain tail cues from a teacher to a spatial detector during training, discarding frequency modules at inference.

Result: STAL achieves strong generalization and stability across 9 public datasets, covering various generators, data distributions, and real-world scenarios, with zero inference overhead.

Conclusion: Spectral tail uplift is a structural cue inherent to generative models, and STAL effectively leverages this for practical, generalizable AI-generated image detection.
...
TL;DR: This paper shows that training AI models solely on synthetic facial images of children with rare genetic diseases can achieve performance comparable to training on real images, offering a privacy-preserving solution for diagnosis and clinical education in pediatric settings.


<details>
  <summary>Details</summary>
Motivation: Developing computer vision systems for early diagnosis of rare pediatric genetic diseases is challenging due to extreme data scarcity, privacy constraints, and limited data sharing. The authors aim to determine whether synthetic data alone is sufficient for learning in ultra-low-resource pediatric settings.

Method: Models are trained exclusively on phenotype-aware synthetic facial images at increasing scales under a controlled experimental setup, and their performance is compared to real-data-only baselines across multiple backbones.

Result: Synthetic-only training achieves performance comparable to real-data-only baselines at sufficient scale across multiple backbones, indicating that high-fidelity synthetic data can approximate clinically meaningful distributions.

Conclusion: Synthetic pediatric facial images can serve as privacy-preserving resources for genetic education and counseling, supporting clinician training, patient communication, and improving data efficiency in children's healthcare.
...
TL;DR: DecQ introduces lightweight detail-condensing queries that extract fine-grained information from intermediate VFM features to improve reconstruction quality in Representation Autoencoders without degrading generative performance, achieving 3.6 dB PSNR improvement and 3.3× faster convergence.


<details>
  <summary>Details</summary>
Motivation: Freezing VFMs in RAEs limits spatial reconstruction capacity for fine-grained generation, while fine-tuning disrupts pretrained semantic space and degrades generative fidelity.

Method: DecQ adds lightweight detail-condensing queries that extract fine-grained information from intermediate VFM features via condenser modules, which are incorporated into the decoder and jointly generated with patch tokens.

Result: With only 8 additional queries and 3.9% extra computation, DecQ improves PSNR from 19.13 dB to 22.76 dB, achieves 3.3× faster convergence, and attains FID of 1.41 (no guidance) and 1.05 (with guidance).

Conclusion: DecQ effectively mitigates the reconstruction-generation trade-off in RAEs, improving both reconstruction quality and generative performance with minimal computational overhead.
...
TL;DR: Sensor2Sensor is a generative framework that converts in-the-wild monocular dashcam videos into high-fidelity multi-modal AV sensor data (multi-view cameras + LiDAR), enabling the use of diverse internet/dashcam footage for AV training and validation.


<details>
  <summary>Details</summary>
Motivation: Proprietary AV fleet data is limited in scale, diversity, sensor configurations, and geographic/long-tail coverage, while abundant in-the-wild dashcam data is incompatible with ADS that require structured multi-modal inputs.

Method: Sensor2Sensor uses 4D Gaussian Splatting (4DGS) to reconstruct AV logs and render them as dashcam-style videos for paired training data, then employs a diffusion architecture for generative conversion from monocular video to multi-modal sensor outputs.

Result: Comprehensive quantitative evaluations demonstrate fidelity and realism of generated sensor data; successful conversion of challenging in-the-wild internet and dashcam footage into realistic multi-modal data formats.

Conclusion: Sensor2Sensor effectively bridges the data gap between unstructured dashcam videos and structured AV sensor logs, unlocking vast external data sources to improve ADS robustness and training diversity.
...
TL;DR: MotiMotion reformulates motion-controlled image-to-video generation as a reasoning-then-generation problem, using a training-free vision-language reasoner to refine trajectories and hallucinate secondary motions, plus a confidence-aware control scheme for natural results, outperforming prior methods on the new MotiBench benchmark.


<details>
  <summary>Details</summary>
Motivation: Current motion-controlled I2V models rigidly follow sparse, imprecise, causally incomplete user trajectories, leading to unnatural or implausible outcomes especially missing secondary causal consequences.

Method: (1) A training-free vision-language reasoner that refines primary trajectory coordinates and hallucinates plausible secondary motions. (2) A confidence-aware control scheme that modulates guidance strength: closely follows high-confidence plans, corrects artifacts under low-confidence inputs using internal generative priors. (3) A new benchmark MotiBench with interaction-centric scenes.

Result: Both VLM-based evaluation and human study on MotiBench show MotiMotion produces videos with more plausible object behaviors and interactions, and is preferred over existing approaches.

Conclusion: MotiMotion effectively addresses motion control in I2V generation by incorporating causal reasoning and confidence-aware mechanisms, achieving more natural and plausible motion outcomes.
...
TL;DR: Adding camera pose information (via learnable camera tokens and pose regression) to video MLLMs significantly improves spatial reasoning and general video QA, showing pose is a fundamental signal for video understanding.


<details>
  <summary>Details</summary>
Motivation: Current video MLLMs treat frames as isolated 2D snapshots, lacking the 3D spatial understanding (camera pose) that humans naturally perceive, which limits their ability to reason about the physical world.

Method: Introduces Cambrian-P, a video MLLM augmented with per-frame learnable camera tokens and a pose regression head, plus a carefully designed sampling scheme.

Result: 4.5-6.5% gains on VSI-Bench, generalizes across 8+ benchmarks, SOTA streaming pose estimation on ScanNet. Pseudo-pose from in-the-wild video also improves general QA.

Conclusion: Camera pose is a fundamental signal for video models reasoning about the physical world.
...
TL;DR: Video-LLMs suffer from directional motion blindness—failing to perceive basic left/right/up/down motion—due to a "direction binding gap" where motion information is present in hidden states but fails to map to correct verbal answers. The authors introduce MoDirect (dataset) and DeltaDirect (projector-level training) that improves direction accuracy from 25.9% to 85.4% on synthetic benchmarks and by 21.9 points on real-world benchmarks without real-world training data.


<details>
  <summary>Details</summary>
Motivation: Current Video-LLMs achieve impressive temporal understanding but fail at the fundamental perceptual task of identifying signed image-plane motion direction (left, right, up, down), performing near chance on simple single-object motion videos—exposing a critical blind spot in basic visual perception.

Method: The authors (1) diagnose the "direction binding gap" by tracing motion information through the pipeline, (2) create MoDirect dataset family for motion direction tuning/evaluation, (3) propose DeltaDirect—a projector-level objective that predicts normalized 2-D motion vectors from adjacent-frame feature deltas, bypassing the binding gap at the projector stage.

Result: DeltaDirect improves motion direction accuracy from 25.9% to 85.4% on MoDirect-SynBench, and achieves 21.9 point improvement on MoDirect-RealBench without real-world tuning data, while preserving standard video-understanding performance.

Conclusion: Motion direction blindness in Video-LLMs stems from a binding gap rather than missing information, and targeted projector-level training with motion vector supervision (DeltaDirect) effectively bridges this gap, generalizing to real-world videos and maintaining existing capabilities.
...
TL;DR: A VR-based shared control framework for drone teams that combines user-guided motion planning with admittance control, enabling operators to guide multi-robot navigation in constrained environments while reducing effort and improving safety.


<details>
  <summary>Details</summary>
Motivation: Autonomous multi-robot systems struggle to adapt to unforeseen conditions and capture operator-driven objectives in unstructured environments, highlighting the need for human-in-the-loop control.

Method: VR-based shared control with a user-guided motion-primitive planner that computes collision-free trajectories while integrating operator input, coupled with an admittance controller and bilateral VR interface for guidance via migration points.
...
Conclusion: Immersive human-in-the-loop control via VR is feasible and advantageous for multi-robot navigation in constrained, unknown environments.
...
TL;DR: A distributed multicoverage algorithm for robot swarms that maintains reliable asset coverage despite failures, using only local sensing and communication.


<details>
  <summary>Details</summary>
Motivation: Autonomous drone swarms need reliable multicoverage of critical assets despite robot failures, but existing optimal solutions are centralized and impractical for real deployments with limited communication, onboard computation, and partial system failures.

Method: A fully distributed multicoverage algorithm that operates with local sensing, local communication, and no global coordination.

Result: Enables robot swarms to maintain varying multicoverage requirements for assets without centralized planning.

Conclusion: Distributed multicoverage is feasible for practical drone swarm deployments under realistic constraints of limited communication and computation.
...
TL;DR: A closed-loop sim-to-real RL approach achieves microfiber shape control by training in a simplified frictionless simulator and using real-time visual feedback to correct unmodeled surface interactions, achieving sub-millimeter accuracy across diverse configurations without retraining.


<details>
  <summary>Details</summary>
Motivation: Autonomous contact-based micromanipulation is challenging because microscale surface/interfacial interactions are difficult to model accurately, limiting model-based control and sim-to-real learning.

Method: Training an RL policy in a simplified frictionless simulator, then directly deploying it on a physical dual-gripper system at 40 Hz using real-time visual feedback to iteratively correct unmodeled surface interactions, without retraining or domain adaptation.

Result: Mean point-wise shape error of 270±80 μm across 24 initial configurations; sub-millimeter error across 9 specimens (3 diameters × 3 lengths) without retraining or retuning.

Conclusion: A policy learned in a simplified simulator can achieve repeatable real-world microfiber shape regulation under surface contact, as long as sim-to-real mismatches remain observable and correctable within the closed feedback loop.
...
TL;DR: This paper studies how multi-limbed robots can perform grasp-based dynamic locomotion in microgravity, proposing a parameterizable planning framework and evaluating it on two quadruped morphologies to identify design insights for improved stability and reduced actuation demand.


<details>
  <summary>Details</summary>
Motivation: Locomotion in microgravity relies on sparse, irregular anchors, and dynamic movement requires careful coordination of anchored interactions and whole-body motion under coupled dynamic/kinematic constraints.

Method: A parameterizable locomotion planning framework is proposed, varying gait pattern, stride length, speed, and nominal posture. Two quadruped morphologies are evaluated via physics-based simulation.
...
Conclusion: Findings inform strategies for contact configuration selection and whole-body coordination in microgravity locomotion with multi-limbed systems.
...
TL;DR: PGDG is a zero-shot data generation framework that expands a single demonstration into a diverse dataset of recovery behaviors for contact-rich bimanual manipulation, significantly improving success rates in both simulation and real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Behavior cloning for bimanual manipulation fails due to costly diverse demonstrations and lack of recovery supervision under disturbances.

Method: PGDG iterates between a physics-grounded sampler and a dataset curator to select informative behaviors, then applies short-horizon sampling-based control to relabel risky states.

Result: Outperforms spatial-only augmentation; RotateBox-Pitch success from 38%→93% in simulation, 35%→82% in real world; GR00T fine-tuning from 46%→77%.

Conclusion: PGDG enables zero-shot curation of diverse, physically plausible recovery behaviors from a single demonstration, improving BC robustness.
...
TL;DR: Proposes an adaptive coverage strategy that enhances ergodic trajectory optimization with real-time environmental feedback for multi-robot teams, improving coverage efficiency under unknown prior distributions.


<details>
  <summary>Details</summary>
Motivation: Traditional ergodic search methods optimize robot trajectories but fail to account for unknown prior distributions of the environment, limiting adaptive coverage performance.

Method: Enhances ergodic trajectory optimization by constructing a target spatial information distribution from parametric environmental models that are updated online using real-time feedback.

Result: Simulation results demonstrate improved coverage efficiency and resource allocation compared to traditional methods.

Conclusion: The adaptive coverage framework enables robots to dynamically prioritize high-interest regions and optimize resource use in unknown environments.
...
TL;DR: This paper proposes a graph neural network-based framework using Hamilton's rule for dynamic robot allocation across heterogeneous multi-teams, validated in firefighting scenarios with near-optimal performance and scalability.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of heterogeneous multi-team collaboration through dynamic robot allocation, treating robots as transferable resources across teams.

Method: Leverages Hamilton's rule from ecology as an altruistic decision-making mechanism; formulates the allocation problem as NP-hard combinatorial optimization; develops a graph neural network policy with centralized training and decentralized execution to approximate altruistic allocations.

Result: The learned policy achieves near-optimal performance and scales effectively to larger systems in firefighting simulations and experiments.

Conclusion: The proposed GNN-based framework effectively enables scalable, near-optimal heterogeneous multi-team robot allocation inspired by ecological altruism principles.
...
TL;DR: SafePBDS is a geometrically consistent framework that computes optimal, certifiably safe configuration manifold accelerations from objectives and safety requirements on arbitrary task manifolds. It achieves 92.5% grasping success and enables the first model-based palm-down in-hand reorientation exceeding 360°.


<details>
  <summary>Details</summary>
Motivation: Robotic dexterous manipulation requires reconciling objectives and constraints defined on heterogeneous geometric spaces (e.g., ℝ⁷ configuration manifold, SE(3) poses, ℝ safety margins), which existing methods struggle to handle in a geometrically consistent and certifiably safe manner.

Method: SafePBDS introduces two key innovations: (1) a pullback control barrier function that converts task manifold safety conditions into linear constraints on configuration manifold accelerations, and (2) a task manifold action interface that allows high-level policies to inject low-dimensional residual motions while preserving safety.

Result: 92.5% grasping success across 20 household objects and 120 trials; 94.4% 3-finger grasp success via 1D action interface; first model-based palm-down in-hand reorientation exceeding 360° yaw rotation under varying conditions.

Conclusion: SafePBDS provides a geometrically consistent safety-guaranteed framework that enables efficient planning and certification, demonstrated through high grasping success rates and novel in-hand reorientation capabilities.
...
TL;DR: This paper analyzes the force-generation behavior of linear soft sleeve actuators (LSSAs) through analytical modeling and experiments, showing that output force decreases with extension (from ~112 N at 0 mm to ~0 N at 40 mm at 125 kPa) and is influenced by pressure, geometry, displacement, loading, and axial stiffness.


<details>
  <summary>Details</summary>
Motivation: Soft sleeve actuators (SSAs) reduce reliance on external attachment layers and transmission mechanisms, but their force-generation behavior remains insufficiently explained, particularly regarding output force variation during extension, external loading effects, and the mechanical role of axial stiffness.

Method: A quasi-static analytical model was developed expressing net axial force as pressure-generated contributions from the cap and folded walls minus the force associated with axial stiffness, incorporating internal pressure, projected pressure areas, folded wall geometry, axial displacement, and an experimentally fitted axial stiffness relation. Prescribed-extension and static-load experiments were conducted.
...
Conclusion: LSSA force generation is governed by coupled effects of pressure, geometry, displacement, loading, and axial stiffness.
...
TL;DR: EvoScene-VLA introduces a recurrent action-updated scene prior for chunked vision-language-action policies, improving robot control success rates by maintaining geometry-aware scene state across action chunks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA policies either use only current visual observations (Spatial VLAs) or aggregate past frames (Temporal VLAs), but neither maintains a persistent scene state that is updated by actions across chunks. Robot actions cause contact, occlusion, and object motion that change the geometry before the next visual update arrives.

Method: EvoScene-VLA uses a recurrent scene prefix carrying a geometry-aware scene state across chunks. At each VLM call, it combines current observation with action-updated prior from the previous chunk. The action decoder outputs both the next action chunk and a compact scene update. Training uses a Scene Predictor for future scene-token targets and Geometric Anchor to align scene slots with frozen depth/3D teachers, both discarded at deployment.

Result: On 31 RoboTwin tasks, success rate improved from 87.2% to 89.1% (fixed) and 86.1% to 88.5% (randomized). Outperformed all baselines on Galaxea R1-Lite real robot.

Conclusion: Maintaining a persistent, action-updated scene prior across chunks significantly improves VLA policy performance by bridging the gap between visual updates.
...
TL;DR: A proprioceptive leg odometry pipeline using ESEKF with fused contact detection and slippage rejection, validated on diverse terrains over 2.4 km.


<details>
  <summary>Details</summary>
Motivation: Achieving accurate odometry in legged robots using only onboard proprioceptive sensors, handling the challenge of slippage detection and rejection.

Method: Error-State EKF (ESEKF) with fused contact detection: a force-based GMM-guided FSM for physical contact confirmation, and a kinematic-based GLRT on foot velocity. Uncertainty quantification fuses both to identify stationary loaded feet.

Result: Validated on 29 sequences (2.4 km) across diverse terrains; outperforms both proprioceptive and exteroceptive methods, robustly handling slippage.

Conclusion: The proposed method provides accurate odometry estimates and robust slippage handling using only proprioceptive sensors, with open-source code and ROS2 package released.
...
TL;DR: A dynamic epistemic planning framework that enables implicit coordination in communicationless multi-robot systems using higher-order belief reasoning, Bayesian inference, and behavior trees, integrated with an MPPI controller for execution.


<details>
  <summary>Details</summary>
Motivation: To enable robust multi-robot coordination in communication-denied or bandwidth-limited environments where traditional information-sharing strategies fail.

Method: Robots form and propagate higher-order belief particles, update beliefs via Bayesian inference, select actions using behavior trees that anticipate teammates' decisions, and execute via a temporally aware MPPI controller.

Result: The framework consistently reduced task completion time compared to a first-order baseline in both simulations and physical experiments.
...
TL;DR: This paper presents a low-cost autonomous UAV-based non-contact method for structural damage detection using vision-based vibration tracking, validated on a lab-scale frame with multiple camera platforms including a custom UAV that detects frequency shifts with up to 5.7% error.


<details>
  <summary>Details</summary>
Motivation: To develop a practical, low-cost, and scalable non-contact approach for structural health monitoring, especially in scenarios where contact-based sensing is impractical or dangerous.

Method: Vision-based motion tracking extracts vibration signals from video recordings; tested with two smartphones, a USB camera, and a custom low-cost UAV with autonomous alignment for GPS-denied environments; validated against contact accelerometers and finite element model.

Result: All platforms successfully captured fundamental frequency and damage-induced shifts; UAV had slightly higher errors (up to 5.7%) but reliably detected damage; low-cost UAV achieved comparable performance to commercial systems.

Conclusion: Low-cost autonomous UAVs offer a practical, flexible, and scalable solution for structural health monitoring, with potential for multi-UAV cooperative deployment to enhance inspection coverage and robustness.
...
TL;DR: This paper replaces CBBA's hand-crafted greedy bidding with a neural bidding policy trained via reinforcement learning (PPO), using architectures like LSTM and Set Transformer, achieving better MRTA solution quality while preserving decentralized execution.
...
Method: Replaces CBBA's deterministic bidding with a neural bidding policy trained via PPO under centralized training/decentralized execution; tested Neural Additive Model, LSTM, and Set Transformer architectures.
...
Conclusion: Integrating RL with classical distributed coordination algorithms offers a scalable pathway toward higher-quality decentralized MRTA.
...
TL;DR: MIF is a humanoid-oriented system combining uncertainty-aware 3D Gaussian Splatting, discrepancy-triggered memory updates, and task-driven geometric reconstruction for safe manipulation under locomotion-induced distortion and environmental changes, achieving 94% relocation success (up from 12%) and 91.4% memory reduction.


<details>
  <summary>Details</summary>
Motivation: Existing semantic mapping and scene-graph systems assume stable camera trajectories, static environments, or coarse object geometry, making them unsuitable for humanoid robots that experience locomotion-induced perceptual distortion and need interaction-level geometric safety.

Method: MIF couples three fields: (1) an uncertainty-aware 3DGS Appearance Field that suppresses gait-induced blur, (2) a Spatial Field maintaining topological memory, and (3) a Geometry Field supporting Interaction Pose Safety. A discrepancy detection score separates locomotion-induced false-positive changes from persistent changes for local memory updates.

Result: On a Unitree-G1 humanoid, MIF improves relocation success from 12% to 94% in non-static environments compared to static scene-graph memory, and reduces semantic memory footprint by 91.4% via feature distillation.

Conclusion: The closed-loop perception-adaptation pipeline effectively addresses humanoid-specific challenges of locomotion-induced distortion and environmental dynamics, enabling reliable manipulation-oriented navigation.
...
TL;DR: A grid-based stochastic foraging strategy that reduces redundant visits and accelerates late-stage collection by using a central server to track grid-level visitation counts, biasing robots toward under-explored areas.


<details>
  <summary>Details</summary>
Motivation: A substantial portion of total mission time is consumed during the final stage of collection where only a small fraction of randomly scattered resources remain; existing algorithms focus on collecting most resources quickly rather than improving end-stage efficiency.

Method: The unknown area is partitioned into a grid map maintained by a lightweight central server. Robots report locations, server updates visitation counts, and each robot selects its next search area from a local 3x3 neighborhood probabilistically favoring the lowest visitation count, balancing exploration bias with stochasticity.

Result: Simulations show the proposed method outperforms CPFA: reduces total collection time by up to 33% and improves final-stage collection efficiency by more than 48%.

Conclusion: The strategy is robust, flexible, and scalable for near-complete and complete resource collection under limited onboard resources, and can serve as a general enhancement for stochastic swarm foraging methods.
...
TL;DR: This paper provides a systematic evaluation of four tactile sensor modalities (visual, acoustic, magnetic, resistive) across three manipulation tasks, showing that tactile sensing effectiveness depends heavily on sensor modality, material properties, and task specifics.


<details>
  <summary>Details</summary>
Motivation: Vision-based learning from demonstrations is insufficient for complex, contact-rich manipulation, and there is no empirical guidance on which tactile sensors are best suited for which manipulation tasks.

Method: Separate manipulation policies are trained for four tactile sensor modalities (visual, acoustic, magnetic, resistive) across three tasks: pick-and-place with unknown mass, object reorientation, and plug insertion. Analysis of how sensor properties like spatial resolution, shear sensing, tactile representation, and material friction affect task performance is conducted.

Result: The usefulness of tactile information depends strongly on sensor modality, material properties, and the specific manipulation tasks, rather than being universally beneficial.

Conclusion: Provides a framework for selecting and evaluating tactile sensors based on manipulation policy performance; all resources will be made publicly available.
...
TL;DR: A dual-arm robotic framework that estimates unknown object mass and center of mass online, uses SOCP-based friction-feasible contact force optimization, and incorporates offline trajectory refinement for stable box lifting without slip or excessive squeezing.


<details>
  <summary>Details</summary>
Motivation: Industrial robotic box handling faces unknown mass and center of mass, causing uncertain force-moment balance that can lead to slip, dropping, orientation issues, or excessive squeezing.

Method: Online estimation of mass and center of mass from measured contact wrenches; SOCP optimization with ellipsoidal friction-limit-surface constraints for friction-feasible contact forces; offline trajectory refinement to reduce unwanted contacts.

Result: Real dual-arm robot experiments under various center-of-mass configurations successfully lift objects with unknown inertial properties while maintaining stable frictional contact.

Conclusion: Enforcing friction feasibility as a hard constraint while minimizing contact effort achieves stable lifting without separately tuning slip avoidance and squeezing objectives.
...
TL;DR: CoRMA replaces raw simulator parameters with semantic contact context (6D) and uses a causal Transformer with contrastive learning for within-episode robot adaptation during assembly, achieving better real-world deployment than FORGE baselines.


<details>
  <summary>Details</summary>
Motivation: Force-dominant assembly tasks suffer from sim-to-real gap; existing methods (e.g., RMA, FORGE) show high simulation success but degrade substantially on hardware.

Method: Replaces raw simulator-parameter adaptation with a compact 6D semantic contact context, uses a deployable causal Transformer adapter with semantic regression + force-regime contrastive objective for online context inference from force/proprioceptive/action histories.

Result: CoRMA retains higher verified real success under controlled target-pose noise compared to FORGE baselines on PegInsert, GearMesh, NutThread tasks in simulation and on a real Marvin arm.

Conclusion: Semantic contact inference is a reusable adaptation interface for related assembly task families; broader unseen-task generalization and Real2Sim calibration remain future work.
...
TL;DR: The paper introduces a framework for learning invariant symbolic reward functions from few demonstrations, enabling zero-shot generalization across object, position, and viewpoint variations in manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Reward functions in RL for robotics fail to generalize beyond controlled lab settings because vision-based models memorize pixel distributions rather than task-level invariants.

Method: A two-component framework: (1) structural reward formulation encoding task-level strategies and physical constraints with optimal policy invariance, and (2) a hybrid symbolic-numerical procedure distilling behavioral invariants from as few as five demonstrations without online interaction.

Result: Outperforms baselines on 8 Meta-World and 3 Franka tasks in process alignment and policy rollout ranking; zero-shot generalization in 3 real-world OOD experiments (position, viewpoint, object).

Conclusion: Learning symbolic reward invariants from few demos enables reusable, generalizable reward representations for diverse real-world manipulation task variants.
...
TL;DR: AVP improves robotic manipulation by having the VLM output visual-primitive tokens that condition a flow-matching action expert, achieving 27.61% higher success rate over pi_0.5.


<details>
  <summary>Details</summary>
Motivation: Current VLA models entangle language comprehension, spatial understanding, and motor control in a single objective, forcing the action expert to implicitly relearn cognitive/perceptual capabilities already in the pretrained VLM, limiting efficiency and generalization.

Method: AVP (Action with Visual Primitives) is an end-to-end architecture where the VLM infers next-stage targets and emits visual-primitive tokens that condition a flow-matching action expert, with supervision from end-effector kinematics.

Result: Improves success rate by 27.61% over pi_0.5 on general pick-and-place tasks, with consistent gains in data efficiency, spatial-compositional generalization, and object-level transfer.

Conclusion: Decoupling cognitive reasoning from motor control via visual-primitive tokens enhances both learning efficiency and generalization in robotic manipulation.
...
TL;DR: A unified risk map framework for occlusion-aware prediction in autonomous driving, integrating traffic flow and collision risks with diffusion-based adversarial scenario generation to improve risk-aware planning under partial observability.


<details>
  <summary>Details</summary>
Motivation: Existing occlusion-aware methods either overestimate risk using reachable states or struggle with trajectory prediction under high occlusion uncertainty. The scarcity of occluded interaction scenarios also limits model performance.

Method: Proposes a unified risk map integrating traffic flow and collision risks via spatiotemporal modeling, plus a diffusion-based scenario generation framework to create realistic yet adversarial occluded scenarios for training.

Result: On the Waymo Open Motion Dataset, improves minimum time-to-collision by 0.78× and average time-to-collision by 1.67× over state-of-the-art occlusion-aware baselines.

Conclusion: The framework provides a comprehensive and practical solution for risk-aware planning in partially observable autonomous driving environments.
...
TL;DR: Imagine2Real enables zero-shot whole-body humanoid-object interaction by using 4D point trajectories and sparse keypoint tracking, bypassing geometric models and retargeting.


<details>
  <summary>Details</summary>
Motivation: Scarcity of high-fidelity 3D data for whole-body HOI, and limitations of video generative priors due to representation misalignment (reliance on CAD models) and retargeting complexity (morphing/morphological mismatch).

Method: (1) Unified 4D point trajectories for robot and object motions (bypasses geometric priors). (2) Keypoints Tracker tracking sparse critical points (base, hands, object) to avoid retargeting. (3) Latent space of a Behavior Foundation Model (BFM) as search domain. (4) Progressive training with simple tracking rewards.

Result: Enables zero-shot physical deployment within a motion capture system, learning robust behaviors from sparse signals.

Conclusion: Imagine2Real offers a flexible, geometry-free zero-shot HOI framework that effectively addresses representation misalignment and retargeting complexity.
...
TL;DR: SOMA is a spatial memory framework that enhances Vision-Language-Action (VLA) models to handle out-of-vision manipulation by constructing, refining, and retrieving spatial-semantic memory from multi-view observations.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models assume task-relevant objects are always visible, leading to brittle behaviors when targets are outside the camera's field of view. SOMA addresses this limitation by enabling reasoning beyond the current visual frustum.

Method: SOMA consists of three components: (1) Spatial Memory Construction — aggregates angular-wise observations into a unified spatial-semantic representation via scanning; (2) Dynamic Memory Refinement — maintains global consistency over time; and (3) Contextual Memory Retrieval — activates instruction-relevant spatial cues during manipulation.

Result: SOMA improves task success rates on five challenging real-world out-of-vision manipulation tasks, induces faster target localization, reduced viewpoint search, and near one-shot grasping under partial observability. It also validates effectiveness in fully observable settings (RoboCasa GR1, SimplerEnv).

Conclusion: Persistent spatial memory significantly enhances VLA models for out-of-vision manipulation, enabling more robust and proactive behaviors beyond reactive visual servoing.
...
TL;DR: This paper explores how reasoning capability can enhance AI copilot robots in endoscopic surgery by transforming them from reactive executors into cognitive collaborators, potentially improving precision, safety, and sustainability in clinical practice.


<details>
  <summary>Details</summary>
Motivation: The potential of reasoning capability in AI copilot robots for endoscopic surgery remains unexplored, and there is a need to alleviate intraoperative uncertainty and cognitive burden on surgeons.

Method: Leveraging reasoning-driven autonomy in Vision-Language-Action (VLA) models to integrate multimodal cues, interpret surgical intent, and infer hidden tissue dynamics.

Result: Reasoning-driven autonomy can transform AI copilot robots from reactive executors into cognitive collaborators.

Conclusion: Reasoning-driven autonomy can enhance precision, safety, and sustainability in clinical endoscopic surgery practice.
...
TL;DR: This paper develops a fast numerical dual control for exploration and exploitation (DCEE) method for auto-optimization problems, exploiting the convex-over-nonlinear structure of the reward function to achieve a Gauss-Newton Hessian approximation that significantly speeds up computation (83 μs max) and improves control performance in vehicle cruising scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing DCEE methods suffer from high computational burden, limiting real-time application. The numerical structure of DCEE has not been fully exploited, and current approaches rely on standard optimization packages or gradient-type updates.

Method: The paper identifies that the DCEE reward function has a convex-over-nonlinear structure. It develops a structure-exploiting method that linearizes only the nonlinear residual map while preserving the convex outer loss, transforming each subproblem into a structured convex form solved via a generalized Gauss-Newton Hessian approximation (positive semidefinite, first-order derivatives only).

Result: Evaluated on a vehicle cruising auto-optimization problem via simulation and hardware-in-the-loop experiments. Achieved approximately one order of magnitude speedup, with microsecond-level maximum computation time of 83 μs on a typical vehicle embedded CPU, and improved control performance over existing methods.

Conclusion: The proposed structure-exploiting numerical DCEE method enables fast online computation suitable for real-time embedded systems, demonstrating significant speed improvements and enhanced control performance.
...
TL;DR: Proposes a terminal-constraint MPC with Kalman filter prediction to address stability loss and visual feature loss in IBVS for UAVs.


<details>
  <summary>Details</summary>
Motivation: Conventional IBVS controllers suffer from closed-loop instability near the target due to constraints, and control failure from intermittent visual feature loss under aggressive motion.

Method: Combines terminal-constraint model predictive control (TC-MPC) with terminal-state constraints and cost for stability, plus a Kalman filter to predict image moments during visual degradation.

Result: Validated through real-time UAV visual servoing experiments.

Conclusion: TC-MPC with KF prediction ensures recursive feasibility, improved convergence, closed-loop stability under constraints, and control continuity during visual feature loss.
...
TL;DR: SteinsGateDrive is a latency-decoupled planner-runtime architecture for cloud-hosted LLM driving agents that uses a worldline metaphor to select counterfactual driving futures before final control instants, reducing effective lag from +3.07s to -0.01s while maintaining safety boundaries.


<details>
  <summary>Details</summary>
Motivation: Cloud-hosted LLM driving agents suffer from inference latency that exceeds vehicle-control windows, while traditional learned world models couple future generation and action selection in large loops, causing delays.

Method: The architecture decouples planning from runtime execution using a worldline metaphor: the LLM selects counterfactual driving futures (alpha: ego-conditioned, beta: interaction counterfactuals, gamma: hazard-stress scenarios) before the final control instant, and the runtime reuses the selected forecast while safety contracts remain valid via an atom-predicate runtime check.

Result: On a within-subject, matched-seed normal-highway protocol with 10 seeds and 20 steps using GPT-5.4 mini, effective lag reduces from +3.07s (1-second horizon) to -0.01s (4-second horizon) while preserving the no-collision safety boundary.

Conclusion: The architecture's safety contribution comes from the atom-predicate runtime check, not the drift score (which functions as a refresh-frequency knob), enabling safe latency-decoupled LLM-driven vehicle control.
...
TL;DR: Introduces a transformative paradigm for immersive virtual experiences using whole-body kinetic interactions and modular robotic surface units, achieving three levels of immersion (audio-visual, physical, full-body) across scalable environments.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between real and virtual worlds by enabling unprecedented whole-body kinetic engagement and physical interaction in immersive environments.

Method: A scalable, distributable platform based on modular robotic surface units (nature-inspired) that render immersive environments at any scale, dynamically adapting force, shape, and motion feedback in real-time.

Result: Achieves three levels of immersion (audio-visual, physical, full-body FBI) with modular units that distribute physical feedback across entire spaces, enabling dynamic multi-user experiences.

Conclusion: This framework bridges real and virtual worlds, enabling whole-body symbiotic interactions and redefining human-environment synthesis for a new era of immersive technology.
...
TL;DR: Combines SMPC with branching structure to handle multi-modal uncertainty in autonomous driving, using scenario clustering and adaptive branching for real-time, safe motion planning.


<details>
  <summary>Details</summary>
Motivation: Existing SMPC handles trajectory uncertainty well via chance constraints but remains conservative for intention uncertainty because constraints must hold across all possible intentions.

Method: Novel integration of SMPC with branching structure; scenario clustering merges predictions based on decision similarity; adaptive branching-time computation defers plan commitment until intention uncertainty is reduced.

Result: Simulation in challenging highway scenarios shows improved safety, reduced conservatism, and real-time computational performance.

Conclusion: The proposed method effectively addresses multi-modal uncertainty (both trajectory and intention) in autonomous driving motion planning, outperforming standard SMPC in safety and conservatism while maintaining tractability.
...
TL;DR: Proposes a vision-only motion-guided detection framework for UAVs using homography-based Global Motion Compensation, Dual-Interval Motion Extraction, and Motion-Guided Attention to improve object detection under ego-motion and jitter.


<details>
  <summary>Details</summary>
Motivation: Object detection from UAVs suffers from ego-motion, camera jitter, and scale variations; existing motion-based methods are either computationally expensive (optical flow) or limited (single-interval differencing sensitive to jitter).

Method: (1) Homography-based Global Motion Compensation (GMC) for frame alignment; (2) Dual-Interval Motion Extraction capturing short-term and long-term motion cues; (3) Motion-Guided Attention (MGA) module to enhance FPN features.

Result: Consistent improvements over YOLOv8 baseline on VisDrone-VID dataset under severe ego-motion.

Conclusion: Dual-interval design and motion-guided attention effectively improve UAV object detection in dynamic scenes.
...
TL;DR: SE3Kit is a lightweight, pure-Python/NumPy library for SE(3) and SO(3) operations, filling the gap between heavy robotics frameworks and general math tools.


<details>
  <summary>Details</summary>
Motivation: The Python robotics ecosystem lacks a library that is both lightweight and mathematically rigorous for rigid body transformations.

Method: Pure-Python, NumPy-only implementation of Lie Group operations without heavy dependencies or deep learning overhead.

Result: A lightweight library suitable for embedded deployment, rapid prototyping, and education.

Conclusion: SE3Kit bridges the gap between heavy frameworks and general tools, offering efficient, rigorous SE(3)/SO(3) operations.
...
TL;DR: Introduces cross-space symmetry compositions for learning robot policies jointly equivariant to multiple symmetries across configuration and task spaces, using differential geometry of forward kinematics.


<details>
  <summary>Details</summary>
Motivation: Robots exhibit multiple simultaneous symmetries, but existing approaches treat them in isolation, failing to exploit their combined potential for better generalization.

Method: Proposes cross-space symmetry compositions framework leveraging differential-geometric structure of forward kinematics to descend symmetries from configuration to task space and lift symmetries from task to configuration space.
...
Conclusion: Jointly exploiting multiple symmetries through cross-space composition yields better generalization than treating them in isolation.
...
TL;DR: A heterogeneous planning framework using UAV scouts to gather information and prune edges for UGV navigation, with a GNN model to predict information gain and reduce planning time.


<details>
  <summary>Details</summary>
Motivation: Autonomous robot teams in partially known environments incur costly backtracking when ground robots encounter blocked roads only revealed upon physical traversal.

Method: Scout-Assisted Planning (SAP) with Information Gain-based Action Pruning, using a Graph Neural Network to predict information gain values from graph structure and belief state.

Result: Reduces ground robot travel cost by 31.9-37.7% over the Canadian Traveler Problem baseline, and outperforms proximity-based scouting by an additional 8-14%.
...
TL;DR: N3P is a three-stage learning-based framework that decomposes parking into simpler subproblems via an intermediate preparatory pose, speeding up Hybrid A* planning by over 80% while outperforming RL baselines in success rate and trajectory quality.


<details>
  <summary>Details</summary>
Motivation: Hybrid A* is computationally expensive for autonomous parking, and RL methods lack reliability with long-horizon geometric constraints, producing suboptimal trajectories.

Method: A three-stage framework (N3P) that introduces an intermediate preparatory pose predicted by a learning module, decomposing the parking maneuver into simpler subproblems to reduce complexity.

Result: N3P-enhanced Hybrid A* speeds up planning by over 80%, produces shorter trajectories with fewer gear changes, and outperforms RL baselines in success rate and trajectory quality with comparable or lower planning time.

Conclusion: Decomposing parking into subproblems via a learned preparatory pose significantly improves planning efficiency and trajectory quality over both Hybrid A* and RL baselines.
...
TL;DR: Multi-agent reinforcement learning enables quadrotor racing agents to safely navigate complex aerodynamic interactions, outperforming human champions and single-agent baselines while reducing collisions by 50%.


<details>
  <summary>Details</summary>
Motivation: Autonomous systems are brittle in shared real-world spaces because they use a single-agent paradigm that ignores or treats other actors as noise, preventing effective coordination.

Method: Using multi-agent reinforcement learning with league-based self-play, training agents on high-speed quadrotor racing with variable numbers of racers, handling aerodynamic interactions and strategic maneuvers.

Result: Agents outperform champion-level human pilots in multi-player races at >22 m/s, reduce collision rates by 50% vs. single-agent baselines, and enable zero-shot generalization to safe human interaction.
...
TL;DR: GesVLA enhances VLA models by integrating gesture as a parallel instruction modality, using a dual-VLM architecture and synthetic gesture data generation, improving target grounding in cluttered environments.


<details>
  <summary>Details</summary>
Motivation: Existing VLA systems rely on textual instructions and struggle with spatial ambiguity in complex scenes with multiple similar objects.

Method: Encodes gesture features into latent space for both reasoning and action generation, uses dual-VLM architecture, constructs scalable synthetic gesture data via hand model rendering on real images, and adopts two-stage training for gesture perception and action prediction.

Result: Gesture incorporation consistently improves target grounding accuracy and human-robot interaction efficiency, especially in complex/cluttered environments.

Conclusion: Gesture as a parallel modality effectively resolves spatial ambiguity in VLA systems for robot manipulation.
...
TL;DR: AwareVLN is a novel VLN framework that equips navigation models with self-aware reasoning through structural reasoning and automatic data engine, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing VLN methods either lack explicit understanding of agent-instruction-scene relationships (end-to-end VLMs) or rely on additional 3D sensors (map-based planning), hindering large-scale pre-training. AwareVLN bridges this gap with a fully end-to-end, data-driven self-awareness mechanism.

Method: (1) A structural reasoning module for spatial and task-oriented self-awareness; (2) An automatic data engine with progress division for effective training.

Result: Significantly outperforms previous SOTA vision-language navigation methods across various datasets in Habitat simulator.

Conclusion: The proposed self-aware reasoning framework effectively enhances VLN performance while remaining end-to-end and data-driven, without needing extra 3D sensors.
...
TL;DR: This paper introduces MOOD, a benchmark for detecting out-of-distribution (OOD) alignment failures in LLMs, and shows that combining guard models with OOD detectors (Mahalanobis distance and perplexity) improves failure detection recall from 39% to 45%, with gains exceeding those from simply scaling up model size.


<details>
  <summary>Details</summary>
Motivation: Many LLM safety failures occur due to out-of-distribution (OOD) scenarios that model developers cannot anticipate, yet existing monitoring pipelines struggle to detect these OOD alignment failures.

Method: The authors introduce the MOOD benchmark with a restricted training set and seven OOD test sets. They train guard models (safety classifiers) and combine them with four types of OOD detectors (Mahalanobis distance, perplexity, etc.), evaluating recall improvement.

Result: Guard models alone fail to generalize OOD. Combining a guard model with Mahalanobis distance and perplexity-based OOD detectors improves recall from 39% to 45%. This OOD detection gain surpasses using a guard model with 20x more parameters.

Conclusion: OOD detection should be a crucial component of LLM monitoring pipelines, offering a more effective path to safety than simply scaling model size.
...
TL;DR: TO-Agents is a multi-agent AI framework that connects natural-language design intent with iterative topology optimization, using vision-language reasoning and an independent judge agent to critique and revise designs. It achieves preference-aligned designs in 60% of trials across cantilever beam and phone-stand tasks, with up to 6x improvement over ablated baselines, enabling end-to-end intent-to-prototype design.


<details>
  <summary>Details</summary>
Motivation: Designers must manually translate qualitative preferences (style, experience, manufacturability) into solver parameters that are not directly tied to those preferences, creating a gap between human intent and optimization outcomes.

Method: A multi-agent framework that converts natural-language descriptions → validated solver inputs → runs topology optimization → renders 3D topology → uses multi-view vision-language reasoning with an independent judge agent to critique results and revise parameters iteratively. A manufacturing agent post-processes top designs for 3D printing.

Result: 60% of trials produce preference-aligned designs (hierarchically branched structures) across both case studies, up to 6x more than ablation without visual/historical feedback. Judge scores and human evaluations confirm effective parameter levers, recovery from poor revisions, and expanded design exploration.

Conclusion: Agentic topology optimization can shift designers from low-level parameter tuning toward high-level specification of form and function, but safeguards are needed for reliable autonomous engineering design.
...
TL;DR: A large-scale computational analysis of over 1,600 Holocaust testimonies from USC Shoah Foundation and Yale Fortunoff Archive reveals that while structural differences exist, the rigid "structured vs. free-form" dichotomy oversimplifies the reality, as significant overlaps complicate this distinction.


<details>
  <summary>Details</summary>
Motivation: To critically examine the widely accepted claim that USC Shoah Foundation interviews follow a structured format while Yale Fortunoff Archive favors open-ended styles—a distinction influencing Holocaust studies and archive development.

Method: Applied computational analysis (discourse segmentation, topic modeling, LLM-based analysis) to quantify "structuredness" via topic coherence, interviewer-survivor dynamics, and question type distribution across 1,600+ testimonies from both archives.

Result: Corroborated existing structural differences but discovered significant overlaps between collections—both within individual interviews and across narrative patterns—complicating the simple dichotomy.

Conclusion: The "structured vs. free-form" binary is an oversimplification; computational methods provide a scalable, replicable framework for comparative oral history analysis with broader applications in digital humanities.
...
TL;DR: MindLoom is a framework that synthesizes frontier-level reasoning data for LLMs by decomposing hard problems into "thought mode chains" (atomic knowledge-reasoning transformations), then using retrieval-based composition and rollout judging to generate diverse, difficulty-controlled training data. It achieves strong results across 9 STEM/math benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning data synthesis methods lack visibility into structural factors governing problem difficulty, leading to narrow diversity and unstable difficulty control.

Method: 1) Decompose hard problem solutions into thought mode chains; 2) Train a retrieval model matching problem states to compatible thought modes; 3) Compose new problems by iteratively applying retrieved thought modes to seed questions with distribution-aligned sampling; 4) Use rollout-based judging to label difficulty and supply correct responses for SFT.
...
Conclusion: MindLoom provides effective difficulty control and broad reasoning pattern coverage for synthesizing high-quality reasoning data.
...
TL;DR: The paper introduces AOP-Wiki EMOD 3.0, a prototype demonstrating data model expansions to modernize the AOP-Wiki repository, enabling support for computationally-generated AOPs, quantitative AOPs, improved FAIRness, AI-readiness, and integration with new approach methodologies for next-generation risk assessment.


<details>
  <summary>Details</summary>
Motivation: Current AOP-Wiki data model and infrastructure constraints limit continued AOP growth despite its central role; the emergence of agentic AI provides renewed momentum for modernization to better serve regulatory science and broader biomedical/One Health applications.

Method: Development of AOP-Wiki EMOD 3.0, the third evidence model prototype, focusing on data model expansions, internal quality improvement, evidence structuring for FAIRness and AI-readiness, and improved AOP-NAM integration.

Result: A concrete demonstration of how the AOP-Wiki can be transformed to support computationally-generated AOPs, quantitative AOPs (qAOPs), and next-generation risk assessment.

Conclusion: Lays a foundation for transforming AOP-Wiki to better serve regulatory science, biomedical, and One Health contexts through enhanced data models that leverage AI and improve interoperability.
...
TL;DR: This study probes AI concept understanding by testing boundaries of categories using implausible members (e.g., "Is an olive a vehicle?"), finding that AI systems differ meaningfully from humans in concept boundaries, including misclassifying "words" as vehicles/clothing, vegetables as fruit, and non-weapons as weapons, with downstream safety implications.


<details>
  <summary>Details</summary>
Motivation: Developing AI systems with human-like understanding of everyday concepts is crucial for safety and reliability; standard probing with plausible category members may just recall training data patterns, so alternative methods to assess true conceptual understanding are needed.

Method: The authors studied AI systems' assignments of objects to superordinate categories from Rosch and Mervis's classic psychology study, including both within-category and mismatched cross-category assignments, then compared results to human judgments.

Result: AI models showed meaningful and surprising differences from humans: treating "words" as belonging to "vehicles" and "clothing," identifying some "vegetable" members as "fruit," and assigning non-weapon exemplars to the "weapons" category.

Conclusion: Concept misalignment between AI and humans exists in meaningful ways, and these misalignments translate into problematic downstream behavior with implications for AI safety.
...
TL;DR: AI usage in problem-solving tasks can either complement or undermine skill development depending on AI informativeness; high-information AI helps short-term performance without harming learning, while low-information AI and heavy usage weaken skill development.


<details>
  <summary>Details</summary>
Motivation: To understand how AI integration into problem-solving affects individual skill development, given the unclear effects of AI on learning.

Method: Controlled logical reasoning task experiment with on-demand AI assistance, comparing AI usage patterns and informativeness levels.

Result: Heavy AI users underperform vs. peers; light users perform similarly to non-users. Low-information AI harms learning; high-information AI improves short-run performance without reducing post-AI outcomes on average, but with heterogeneous effects.

Conclusion: AI can complement or substitute human skill development depending on context (informativeness and usage patterns), suggesting regulation of AI access/usage is important for promoting learning.
...
TL;DR: Existing refusal suppression methods ablate a refusal direction from model activations without principled explanation; this paper reframes it as latent-space evasion attack against linear probes, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing refusal suppression methods lack a principled account of why they work; the authors aim to provide theoretical grounding and identify limitations of current approaches.

Method: Recasts refusal suppression as latent-space evasion attack; proposes Controlled Latent-space Evasion (CLE) attack that projects representations past the decision boundary with optimized confidence.

Result: Achieves state-of-the-art attack success rate across 15 instruction-tuned, multimodal, and reasoning models, outperforming existing refusal-ablation baselines and specialized jailbreak attacks.

Conclusion: The latent-space evasion perspective explains prior work's success, reveals its limitation (stopping at decision boundary), and motivates pushing representations into the compliant region for better attack performance.
...
TL;DR: AttuneBench is a benchmark using 200 real multi-turn human-model conversations to evaluate LLMs' emotional intelligence across four capabilities: emotion recognition, behavioral classification, preference prediction, and response quality. Results show these capabilities are largely independent, with preference alignment and response quality being more model-discriminating than emotion accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing EI benchmarks rely on synthetic prompts, single-turn cases, or third-party annotation, which fail to directly measure how models infer and respond to emotional states over real multi-turn conversations.

Method: Created AttuneBench with 200 genuine multi-turn human-model conversations where participants provided turn-by-turn annotations of their emotional state, the model's behavior, and preferred responses. Evaluated 11 models on emotion recognition, behavioral classification, preference prediction, and judged response quality.

Result: Model rankings across the four capabilities are largely independent. Preference alignment and response-quality judgments are substantially more model-discriminating than emotion-label accuracy.

Conclusion: Emotionally intelligent behavior requires predicting what kind of response a specific user wants in context—a distinction that aggregate scoring or single-turn/synthetic formats cannot capture. AttuneBench provides a framework for assessing these capabilities separately.
...
TL;DR: SMDD-Bench is a new benchmark for evaluating LLM agents on small molecule drug design tasks, consisting of 502 multi-turn tasks across 5 task types and 102 protein targets. Even the best LLM (GPT5.4) solves only 40.2% of tasks.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for LLM agents in drug discovery are ad hoc, too simple, limited in scale, or restricted to single-turn QA, lacking a standardized benchmark for real-world small molecule drug design.

Method: Created SMDD-Bench with 502 guaranteed-solvable task instances across 5 task types (2D Pharmacophore Identification, Interaction Point Discovery, Scaffold Hopping, Lead Optimization, Fragment Assembly) spanning 102 unique protein targets.

Result: Benchmarked 7 frontier LLMs; the best model (GPT5.4) solves only 40.2% of tasks, showing significant room for improvement.

Conclusion: SMDD-Bench provides a standardized testbed for training and evaluating LLM agents toward fully autonomous computational drug design, with a public leaderboard available.
...
TL;DR: This paper demonstrates that AI-based occupation exposure measures from platform conversation logs are significantly influenced by platform user composition rather than actual workforce characteristics, leading to substantial biases in employment elasticity estimates.


<details>
  <summary>Details</summary>
Motivation: To evaluate the validity of AI platform conversation logs as measures of occupation exposure to AI, since these are increasingly used in the literature but may reflect platform user demographics rather than the true workforce.

Method: The authors compare employment coefficient estimates across different platform inputs, consumer vs. enterprise channels, and apply reweighting to Bureau of Labor Statistics workforce shares. They formalize non-classical measurement error and derive probability limits and partial-identification bounds.

Result: Platform input variation changes post-ChatGPT employment coefficient by factor of 1.9; consumer vs. enterprise channels yield opposite signs; reweighting attenuates estimates by 42-93%; bias understates substitution more than augmentation.

Conclusion: AI platform-based occupation exposure measures are unreliable due to measurement error driven by platform user composition, and researchers should be cautious when using them to estimate employment effects.
...
TL;DR: A novel XAI method that combines causal discovery with bipolar argumentation frameworks to explain why models make predictions, demonstrated on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing XAI methods identify relevant features but fail to clarify why certain decisions are made.

Method: Integrates causal discovery to identify causal relationships among variables, translates them into a Bipolar Argumentation Framework (BAF) to represent supportive and opposing feature interactions, and uses semi-stable semantics to find extensions explaining outcomes.

Result: Demonstrated on two benchmark datasets and compared against standard post-hoc explainability approaches.

Conclusion: The proposed method provides clearer explanations of why models make predictions by incorporating causal reasoning and argumentation structures.
...
TL;DR: This paper reviews 70 papers and surveys 106 experts to develop a taxonomy of AI sycophancy, distinguishing between sycophancy toward user beliefs vs. personal traits, and explicit vs. implicit behaviors. Experts agree sycophancy is a major problem (94.3%) but disagree on specific definitions.


<details>
  <summary>Details</summary>
Motivation: AI sycophancy lacks a consistent definition, making evaluation results incomparable, mitigation strategies non-transferable, and allowing different forms to persist undetected across systems.

Method: (1) Reviewed 70 papers on AI sycophancy to develop a taxonomy; (2) Surveyed 106 experts in AI sycophancy and related fields to assess agreement on which behaviors qualify as sycophantic.

Result: Taxonomy reveals current research focuses on overt sycophancy toward user beliefs, leaving subtle/person-directed behaviors understudied. 94.3% of experts agree sycophancy is a significant problem, but they disagree substantially on specific behaviors.

Conclusion: AI sycophancy is a broad family of behaviors requiring different measurement, intervention, and governance approaches. The taxonomy provides a shared vocabulary for future research and mitigation.
...
TL;DR: Trace2Skill improves hardware LLM agents on complex Verilog design tasks by evolving natural-language skills from rollout traces, without model fine-tuning, using dense verifier feedback and an oracle-mutator-selector loop to boost pass rates on previously unsolvable problems.


<details>
  <summary>Details</summary>
Motivation: Complex Verilog Design Problems (CVDP) are difficult for hardware LLM agents due to the need to localize RTL/testbench dependencies, make precise edits, and recover from sparse hidden-verifier failures, all within large repository snapshots.

Method: Trace2Skill treats the agent's natural-language skill as an evolvable policy. It mines repeated rollout traces, converts them into diagnostics/lessons, and uses an oracle-mutator-selector loop to produce task-specific skills. It also supports bounded dense verifier feedback that returns sanitized functional observations without exposing hidden harnesses.

Result: Across hard CVDP tasks, Trace2Skill with dense verifier feedback substantially improves task pass rates and achieves breakthrough passes on previously unsolved tasks, outperforming both the seed CVDP agent and frontier coding agents.

Conclusion: Trace2Skill provides a general test-time scaling strategy for verifiable EDA tasks, requiring no fine-tuning data, specialized RTL model training, or model weight updates.
...
TL;DR: A hierarchical framework that extracts safety-aligned skills from crowd preferences to safely solve downstream RL tasks without explicit safety rewards.


<details>
  <summary>Details</summary>
Motivation: RLHF can reveal implicit safety objectives from crowd preferences, but directly combining preference-learned rewards with task rewards has inherent limitations for safety enforcement.

Method: Proposes Safe Crowd Preference-based RL, a hierarchical framework that extracts safety-aligned skills from crowd preferences and composes them via a high-level policy to safely solve downstream tasks.

Result: Substantially lowers safety costs without explicit safety rewards, achieving task performance comparable to oracle methods with ground-truth safety signals.

Conclusion: The hierarchical framework effectively discovers and transfers shared safety criteria from crowd preferences to regularize agent behavior in downstream RL tasks.
...
TL;DR: A system that autonomously designs custom visual analysis applications from only data and high-level task descriptions, validated on IEEE SciVis Contests.


<details>
  <summary>Details</summary>
Motivation: Scientific data analysis and visualization require broad expertise beyond core domain knowledge; there is a need for autonomous systems that can handle complex, long-horizon visualization tasks with minimal human direction.

Method: An end-to-end agentic harness with multiple coordinated agents handling exploratory analysis, planning, environment configuration, implementation, validation, and evaluation—each producing artifacts for downstream refinement.

Result: The system autonomously produces functional single-page VIS apps with verified linked-view behavior, highly customized to tasks, validated on real-world IEEE SciVis Contests.

Conclusion: This work represents a significant step toward a general AI co-scientist by enabling autonomous, end-to-end visualization application design from high-level directions.
...
TL;DR: FLUID is the first framework to fully remove item IDs from livestreaming recommendation, using a cross-domain multimodal encoder with discrete hierarchical codes (LUCID) and a late-fusion ID-free design, achieving significant gains in a global production system with over 1 billion users.


<details>
  <summary>Details</summary>
Motivation: Livestreaming recommendation suffers from persistent cold-start because live rooms broadcast for only tens of minutes, making traditional ID-based collaborative filtering ineffective due to poorly learned item ID embeddings.

Method: FLUID combines a cross-domain multimodal encoder (trained on short videos + livestreams) that produces discrete hierarchical codes (LUCID), with a late-fusion, ID-free design injecting slice-level and room-level LUCID as independent tokens, stabilized by staged warmup under online incremental training.

Result: Deployed on industrial livestreaming recommenders with over 1 billion users, achieving +0.55% Quality Watch Duration, +2.05% Cold-Start Room Views, and +0.05% Active Hours.

Conclusion: FLUID demonstrates that fully retiring item IDs is feasible and beneficial for livestreaming recommendation, effectively addressing cold-start issues at production scale.
...
TL;DR: The paper argues that LLM-based planning is shifting from single-shot generation or hybrid approaches toward using LLMs to generate verifiable symbolic solvers that can be used efficiently at inference time, aiming for more reliable and resource-efficient planning.


<details>
  <summary>Details</summary>
Motivation: Early LLM planning approaches (single-shot, hybrid with external search) are unsound, incomplete, resource-heavy, and unreliable for unseen problems, motivating a need for more dependable and efficient planning methods.

Method: The paper categorizes and examines three major categories of planner-generation methods that use LLMs to generate symbolic solvers at solution construction time, which can be verified and used efficiently at inference time.

Result: Identifies a trend shift from inference-time LLM planning to construction-time generation of verifiable planners, enabling maintainable planners with minimal LLM dependence at inference.

Conclusion: The paper outlines research steps toward more reliable and efficient LLM-based generation of planners, reflecting a broader realignment of the planning field in the LLM era.
...
TL;DR: This chapter examines how AI approaches (LLMs, RL, agent-based architectures) can support real-time instructional adaptation in serious games, distinguishing between instructional intelligence and adaptivity, while highlighting challenges like explainability, validation, and limited empirical evidence.


<details>
  <summary>Details</summary>
Motivation: Serious games face persistent challenges including static scenario design, authoring bottlenecks, limited learner modeling, and difficulty implementing real-time instructional adaptation.

Method: Historical synthesis of adaptive learning systems from early computer-assisted instruction through ITS, DDA, learning analytics, and AI-enabled architectures; conceptual distinction between instructional intelligence and adaptivity.

Result: Identifies how LLMs, RL, and agent-based architectures may contribute to integrated intelligence and adaptivity in serious games.

Conclusion: AI-enabled serious games offer promise but face challenges in explainability, validation, computational cost, and lack of empirical evidence on long-term learning outcomes.
...
TL;DR: CSV extraction schemas can reduce knowledge graph fidelity when applied to time-series matrices, causing up to +1.180 super-additive errors (format-constraint coupling). Direct graph access reveals construction quality gaps up to +47.6pp. CSVFidelity-Bench is released with 1,892 gold facts.


<details>
  <summary>Details</summary>
Motivation: To study how extraction schemas interact with CSV serialization formats in time-series matrices, potentially degrading knowledge graph fidelity rather than improving it.

Method: 2x2 factorial experiments across 6 datasets; bootstrap CIs; probing & token ablation for causal analysis; controlled variants across format-schema pairings, GraphRAG hosts, and LLM families.

Result: Format-constraint coupling causes super-additive errors (+1.180); catastrophic mismatch on 4/6 datasets; direct graph access exposes construction gaps up to +47.6pp while standard retrieval masks them (<=1pp).

Conclusion: Schema-format coupling is a real, diagnosable failure mode; fidelity-aware evaluation requires direct graph access, not standard retrieval; CSVFidelity-Bench enables this evaluation.
...
TL;DR: Echo is a framework that converts noisy real-world agent interaction data into high-quality training signals by harvesting user refinement sequences, demonstrated by improving code completion acceptance rate from 25.7% to 35.7% in production.


<details>
  <summary>Details</summary>
Motivation: Static human-created data is expensive and limited; real-world agent experience data is abundant but noisy and inefficient for direct training.

Method: Echo systematically captures user refinement sequences (where users correct agent proposals) and transforms these into high-quality training signals to continuously align agents with real-world needs.

Result: In a production code completion environment, Echo increased the acceptance rate from 25.7% to 35.7%.

Conclusion: Echo effectively breaks the static performance ceiling by operationalizing raw experience into learnable knowledge through user-driven feedback loops.
...
TL;DR: A new method (ECPO) for evidence-certified candidate ranking that jointly optimizes ranking quality and verifiable evidence certificates, outperforming baselines across multiple settings.


<details>
  <summary>Details</summary>
Motivation: Ranking systems in decision-support need to provide independently verifiable evidence alongside candidate ordering, but existing methods lack formal evidence certification mechanisms.

Method: ECPO is a listwise policy-optimization objective with trajectory reward from skeleton alignment, argument consistency, and graph features; optimizes three coupled rewards: ranking utility, span-level certificate validity, and evidence-cycle reward from a deterministic verifier.

Result: ECPO outperforms zero-shot, SFT, GRPO, RM-only scoring, constraint decoding, validator retry, best-of-N selection, and post-hoc rationalization across closed-roster, predicted-roster, and hybrid-roster settings.

Conclusion: Reframes ranking optimization from maximizing NDCG to maximizing CertNDCG and decision-evidence coupling, showing verifiable evidence can be jointly optimized with ranking quality.
...
TL;DR: ActiveGraph inverts traditional agent frameworks by using an append-only event log as the source of truth, with a deterministic graph projection and reactive behaviors, enabling replay, cheap forking, and full lineage tracking.


<details>
  <summary>Details</summary>
Motivation: Current agent frameworks treat conversation loops as primary, with memory and observability bolted on as afterthoughts, lacking deterministic replay, forking, and end-to-end lineage.

Method: ActiveGraph uses an append-only event log as the source of truth; a working graph is a deterministic projection of that log; behaviors (functions, classes, LLM-backed routines) react to graph changes and emit new events. Coordination happens entirely through the shared graph.

Result: The architecture provides three key properties: (1) deterministic replay from the log, (2) cheap forking at any event without re-executing shared prefixes, and (3) end-to-end lineage from high-level goals to individual model calls.

Conclusion: ActiveGraph is well-suited for self-improving agents, extending BabyAGI lineage and prior graph-memory research, with full causal structure reconstructable from logs alone.
...
TL;DR: LLMs show significant performance drops (12.75% accuracy, 24.36% evidence quality) in interactive diagnostic settings compared to static exams, due to premature closure and inefficient questioning.


<details>
  <summary>Details</summary>
Motivation: Static medical exams overestimate LLM diagnostic ability; real clinical diagnosis requires iterative evidence gathering under uncertainty, which current benchmarks fail to assess.

Method: Developed an OSCE-inspired standardized patient simulator and controlled benchmark for active diagnostic inquiry, testing 468 cases across 15 models.

Result: Multi-turn evidence seeking reduces diagnostic accuracy by 12.75% and supporting-evidence quality by 24.36% vs. full-context evaluation; errors linked to premature diagnostic closure and inefficient questioning.

Conclusion: Static full-context benchmarks overestimate LLM performance in interactive settings; complementary interactive assessment is needed for safer clinical decision support.
...
TL;DR: ST-GridPool is a training-free plug-and-play method that enhances Video LLMs by improving visual token compression through Pyramid Temporal Gridding (multi-grained spatiotemporal capture) and Norm-based Spatial Pooling (preserving high-information regions), achieving performance gains without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs (e.g., LLaVA family) use simplistic pooling or interpolation for visual token compression that overlooks the intricate dynamics of visual tokens, failing to efficiently preserve spatiotemporal interactions in video understanding.

Method: Two components: (1) Pyramid Temporal Gridding (PTG) for hierarchical multi-grained spatiotemporal capture, and (2) Norm-based Spatial Pooling (NSP) that leverages token norm-to-salience correlation to preserve high-information visual regions.

Result: ST-GridPool consistently improves Video LLM performance across multiple benchmarks without requiring costly retraining.

Conclusion: ST-GridPool offers an efficient, training-free, plug-and-play solution for improving visual token representations in Video LLMs, with code publicly available.
...
TL;DR: Proposes CC-ISAC, a camera-cooperative multimodal framework that reduces beam steering overhead by 71% and tracking overhead by 1.69-11.15% for non-cooperative UAV detection in ISAC systems.


<details>
  <summary>Details</summary>
Motivation: Single-modal perception limitations and resource competition between sensing and communication in ISAC systems hinder effective non-cooperative UAV detection.

Method: Camera-Cooperative ISAC (CC-ISAC) framework with two modules: Vision-to-Echo Data Alignment (V2EDA) using cross-attention, and Multimodal Fusion-Based Estimation (MMFE) integrating historical and current multimodal data.

Result: Average 71% reduction in beam steering overhead, 1.69-11.15% reduction in tracking overhead, with maintained high angular estimation accuracy on DeepSense 6G dataset.

Conclusion: CC-ISAC effectively mitigates resource contention, enables reliable UAV surveillance, and frees system resources for additional communication tasks.
...
TL;DR: This paper introduces the "ontological continuum" as a theoretical framework to characterize and navigate knowledge graphs across diverse modeling practices (from lightweight to richly axiomatized), motivated by the need for principled KG reengineering in neuro-symbolic AI and GenAI contexts.


<details>
  <summary>Details</summary>
Motivation: Current KG modeling diversity (from vocabularies to ontologies) makes integration and reuse expensive and brittle, especially in neuro-symbolic AI where bridging neural-symbolic components requires KG reengineering. GenAI offers automation but lacks principled grounding without a systematic understanding of the KG space.

Method: The ontological continuum framework characterized by two orthogonal axes: semantics vs. pragmatics, and properties vs. affordances. The approach is empirical—derived from observing real-world KG practices, not prescriptive. Formal Concept Analysis (FCA) is proposed for formal explication.

Result: A vocabulary to describe, compare, navigate, and transform KGs across the full range of modeling practices, demonstrated through a provenance knowledge case study showing how the same concern manifests differently across the continuum.

Conclusion: Five open research challenges are articulated. The authors invite the community to adopt the ontological continuum as a shared research agenda for principled KG engineering and neuro-symbolic integration.
...
TL;DR: MPDocBench-Parse is a new benchmark for multi-page document parsing with 433 documents (3,246 pages) across 15 types in English/Chinese, designed to evaluate content fidelity, logical structure, and semantic continuity in realistic scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing document parsing benchmarks are inadequate for realistic scenarios—they focus on specific tasks, single-page or text-centric settings, and lack fine-grained evaluation of semantic continuity, hierarchical structure recovery, and visual content preservation.

Method: The benchmark contains 433 manually annotated documents (3,246 pages) covering 15 document types in English and Chinese. It designs a comprehensive evaluation protocol covering text/table/formula recognition, truncated content merging, figure extraction, reading order, and heading hierarchy recovery.
...
TL;DR: ExComm is a communication protocol for agentic test-time scaling that detects and resolves cross-agent factual conflicts through periodic audits, tool-based verification, and soft belief updates, while maintaining trajectory diversity via a diversification module. It achieves 5.7% and 5.0% average gains over baselines on AIME 2024/2025 and GAIA.


<details>
  <summary>Details</summary>
Motivation: Error propagation is a common failure mode in long-horizon agentic test-time scaling. Existing methods provide limited control over this process, relying on agents to self-detect mistakes or refine solutions after errors have already shaped reasoning.

Method: ExComm periodically audits agent belief states to detect cross-agent factual conflicts, resolves them via a tool-based verification loop, returns targeted feedback through soft belief updates (appending rather than overwriting), and includes a trajectory diversification module to prevent diversity collapse.

Result: On AIME 2024, AIME 2025, and GAIA with Gemini-2.5-Flash-Lite and Qwen3.5-4B, ExComm outperforms strong baselines with 5.7% and 5.0% average gains. It shows improved error recovery, favorable scaling, stronger diversity, and best performance-cost trade-off.

Conclusion: ExComm effectively mitigates error propagation in agentic test-time scaling via structured communication and verification loops, achieving superior performance across multiple benchmarks.
...
TL;DR: ArborKV is a structure-aware KV cache eviction framework for Tree-of-Thoughts reasoning that reduces peak memory by ~4x while preserving accuracy by exploiting search dynamics (active branch priority, inactive subtree recoverability).


<details>
  <summary>Details</summary>
Motivation: Tree-of-Thoughts (ToT) reasoning incurs massive KV cache memory overhead due to retaining intermediate states for many partial trajectories, limiting throughput, search depth, and width under fixed hardware budgets.

Method: ArborKV couples a lightweight value estimator with a tree-aware allocation policy, performing token-extractive eviction with lazy rehydration to enable backtracking while prioritizing the active branch.

Result: Up to ~4x peak KV-memory reduction on ToT reasoning benchmarks while preserving near-full-retention accuracy, enabling larger search configurations under fixed memory budgets.

Conclusion: Structure-aware KV cache management can effectively alleviate memory bottlenecks in tree-structured LLM reasoning without sacrificing accuracy.
...
TL;DR: MLLMs show a "Prejudice Gap": 51% of correct personality ratings aren't grounded in observable evidence, with holistic grounding only 0-33.5% across 27 models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks only evaluate numerical Big Five score prediction, failing to test whether MLLMs truly perceive personality through behavioral understanding or superficial pattern matching.

Method: Formalizes Grounded Personality Reasoning (GPR) task with a three-tier evaluation (rating, reasoning, grounding); releases MM-OCEAN dataset (1,104 videos, 5,320 MCQs) via multi-agent pipeline with human verification; introduces four failure-mode metrics.

Result: Across 27 MLLMs, 51% of correct ratings lack grounding in retrieved cues; Holistic-Grounding Rate ranges only 0-33.5%.

Conclusion: There's a disconnect between getting the right score and reasoning for the right reason, highlighting the need for grounded social cognition in MLLMs.
...
TL;DR: The paper proposes SR$^2$AM, a self-regulated agentic LLM that decomposes decision-making into three systems: reactive execution (System I), simulative reasoning via world model (System II), and self-regulation for planning control (System III). This approach achieves competitive performance with much larger models while using significantly fewer reasoning tokens.


<details>
  <summary>Details</summary>
Motivation: Current agentic systems with adaptive computation (e.g., chain-of-thought) lack control over planning presence, structure, or horizon, leading to inefficient token use without reliable accuracy gains.

Method: Decomposes decision-making into three systems: System I (reactive execution), System II (simulative reasoning via world model), and System III (self-regulation for when/how deeply to plan). Implemented as distinct stages within LLM chain-of-thought, trained via supervised learning then RL.

Result: SR$^2$AM v0.1-8B and v1.0-30B achieve Pass@1 competitive with 120-355B and 685B-1T parameter systems respectively. v1.0-30B uses 25.8-95.3% fewer reasoning tokens. RL increases average planning horizon by 22.8% while planning frequency grows only 2.0%.

Conclusion: Self-regulated simulative reasoning with learned planning control enables efficient token use, and this self-regulation principle may extend beyond planning to how agents govern learning and adaptation.
...
TL;DR: Ratchet is a single-agent loop enabling frozen LLMs to self-manage skill libraries (write, retrieve, curate, retire) autonomously. On MBPP+ hard-100 with Claude Opus 4.7, it achieves +32.8pp gain over baseline, versus +0.0pp for prior LLM-authored skills. Ablations show retirement and meta-skill prior are critical; deduplication is subsumed by meta-skill.


<details>
  <summary>Details</summary>
Motivation: Prior self-evolving skill libraries (e.g., Voyager) showed LLM-authored skills deliver zero improvement (+0.0pp) over no-skill baselines, while human-curated skills achieve +16.2pp. The bottleneck is lifecycle management, not skill authoring.

Method: Ratchet implements a single-agent loop where a frozen LLM autonomously writes, retrieves, curates, and retires natural-language skills. It integrates four hygiene mechanisms: outcome-driven retirement, bounded active-cap, meta-skill authoring guidance, and pattern canonicalisation.

Result: On MBPP+ hard-100 with Claude Opus 4.7: pass@1 rises from 0.258 to 0.584 rolling mean (peak 0.658). No-skill control drifts at +0.002. Transfers to SWE-bench Verified (+0.22 peak lift). Eight ablations identify minimal working recipe.

Conclusion: Retirement and meta-skill authoring prior are load-bearing; explicit deduplication is subsumed by meta-skill. A non-divergence proposition shows bounded cap and retirement threshold prevent performance from dropping below the no-skills floor.
...
TL;DR: IdleSpec is a method that utilizes idle time during LLM agent operations to generate plan candidates, improving task performance without significant latency overhead.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agents experience unavoidable idle time while waiting for observations from tool calls/environments, and current solutions either treat this as overhead or propose restricted approaches that fail to account for varying computational budgets and observation uncertainty, leading to suboptimal idle time utilization.

Method: IdleSpec iteratively generates plan candidates during idle periods and aggregates them upon receiving observations. It samples between progressive and recovery drafting strategies using a learned distribution updated via posterior feedback to handle observation uncertainty effectively.

Result: On GAIA and FRAMES, IdleSpec achieves 55.6% average accuracy with Gemini-2.5-Flash, surpassing vanilla baseline by 5.1%. On MLE-Bench, it achieves up to 9.1% improvement on Any Medal rate, demonstrating generalizability to long-horizon tasks.

Conclusion: Leveraging idle-time computation through parallel plan generation and adaptive strategy selection significantly improves LLM agent performance across diverse agentic scenarios with minimal latency overhead.
...
TL;DR: ST-SimDiff is a training-free framework that reduces visual tokens in MLLMs for long videos by using similarity-based selection (community detection) to retain static representative tokens and temporal difference-based selection to capture key dynamic events, outperforming SOTA methods with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing token reduction methods for MLLMs ignore changes/turning points in videos and lack a collaborative spatio-temporal model, leading to inefficiency in processing long videos.

Method: Builds a spatio-temporal graph from visual tokens, then applies parallel dual-selection: 1) similarity-based via community detection (static compression), 2) temporal difference-based for content-changing points (dynamic capture).

Result: Outperforms state-of-the-art approaches while significantly reducing computational costs.

Conclusion: Leveraging both similarity (for redundancy) and difference (for key events) in a unified graph-based framework effectively preserves static and dynamic content with minimal tokens.
...
TL;DR: Life-Harness is a lifecycle-aware runtime harness that improves frozen LLM agents by converting recurring failures into reusable interventions at the model-environment interface, achieving 88.5% average relative improvement across 126 settings on deterministic benchmarks without changing model weights.


<details>
  <summary>Details</summary>
Motivation: Many LLM agent failures in deterministic, rule-governed domains stem from mismatches at the model-environment interface (runtime harness), not from model parameters. Existing adaptation methods focus on updating model weights, but ignore this interface layer.

Method: Life-Harness is a lifecycle-aware runtime harness that evolves from training trajectories by converting recurring interaction failures into reusable interventions across four dimensions: environment contracts, procedural skills, action realization, and trajectory regulation. It remains fixed during held-out evaluation and works with frozen LLMs.

Result: Improves 116 out of 126 model-environment settings across 18 model backbones on τ-bench, τ²-bench, and AgentBench, with 88.5% average relative improvement. Harnesses evolved from Qwen3-4B-Instruct transfer to 17 other models, showing environment-side structure reuse.
...
TL;DR: VLMs exhibit cross-modal redundancy causing evaluation collapse in explainers; authors propose Synergistic Faithfulness metric based on Shapley Interaction Index to truly measure cross-modal reasoning, showing adapted attention-based methods outperform VLM-specific explainers.


<details>
  <summary>Details</summary>
Motivation: Current post-hoc explainers for VLMs are evaluated via unimodal perturbation metrics, but multimodal datasets contain language priors and modality biases causing cross-modal redundancy—leading to evaluation collapse where visual and textual rankings contradict each other (Kendall's τ = -0.06).

Method: Propose Synergistic Faithfulness (F_syn), a metric based on the Shapley Interaction Index that isolates the joint Harsanyi dividend between modalities. Achieves high accuracy (ρ=0.92) with 24× computational speedup. Evaluated 8 XAI methods across 3 VLM architectures and 3 datasets.

Result: Explainers proposed for VLMs over-index on visual salience and underperform adapted attention-based methods in capturing cross-modal synergy. Unimodal metrics penalize faithful explainers due to modality biases.

Conclusion: Decoupling visual plausibility from cross-modal faithfulness provides a rigorous evaluation framework for safely auditing VLM reasoning in high-stakes deployments.
...
TL;DR: LLM-Metrics uses LLMs' parametric memory (via recognition probes) to assess research impact, achieving significant correlation (ρ=0.1495, p=0.0004) with citations across 17 models and 549 papers, with stronger effects for recent papers and smaller models acting as effective filters.


<details>
  <summary>Details</summary>
Motivation: Citation counts are the dominant research impact metric but have limitations: temporal lag, disciplinary bias, and Matthew effects. The authors propose an alternative paradigm based on LLM parametric memory.

Method: Designed four multiple-choice probe types (title, author, method, venue recognition). Tested 549 CS papers (2023-2024) across 17 LLMs (0.5B-72B parameters, 6 vendors). Used Spearman correlation against citation counts.

Result: 15/17 models produced positive predictions, 9 significant at p<0.05. Overall ρ=0.1495, p=0.0004. Stronger signal for 2024 papers (ρ=0.1880). Author-recognition most discriminative. Non-monotonic scale effect: 3B model (Llama-3.2-3B-Instruct, ρ=0.1829) outperformed larger models.

Conclusion: LLM-Metrics offers a real-time, cross-disciplinary, citation-independent research assessment paradigm. The exposure-driven memory mechanism is supported by three findings: stronger recent paper signal, author-recognition dominance, and small-model effectiveness as information filters.
...
TL;DR: SkillWeave partitions LLMs into lightweight domain-specific modules (skillpacks) and uses SkillZip compression for efficient deployment, enabling a 9B model to outperform a 32B model with up to 4x speedup.


<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle to balance multi-domain specialization with strict memory and inference constraints.

Method: SkillWeave partitions full LLM capabilities into lightweight, domain-specific delta modules (skillpacks), then compresses them with SkillZip for inference-ready deployment.

Result: A 9B SkillWeave model outperforms several baselines and surpasses a 32B monolithic LLM, achieving up to 4x speedup.

Conclusion: SkillWeave enables effective multi-domain specialization under fixed memory budgets with strong performance and low latency.
...
TL;DR: CLORE improves LLM reasoning efficiency by editing correct reasoning trajectories (deleting repetitive, illegible, or task-irrelevant content) and optimizing with reference-free DPO alongside policy-gradient training, achieving better accuracy-efficiency trade-offs on math reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing efficient reasoning methods mainly regulate response length through explicit budgets or length-aware rewards, leaving intermediate reasoning content weakly supervised.

Method: CLORE (Content-Level Optimization for Reasoning Efficiency) uses an external augmentation model to delete repetitive segments, illegible or task-irrelevant content, and superfluous reasoning after solution is established, then optimizes augmented-original pairs with reference-free DPO alongside standard policy-gradient training, restricting augmentation to correct trajectories.

Result: Experiments on DeepSeek-R1-Distill-Qwen-7B and Qwen2.5-Math-7B across five mathematical reasoning benchmarks show CLORE improves accuracy-efficiency trade-off and remains compatible with GRPO, DAPO, Training Efficient, and ThinkPrune.

Conclusion: CLORE reduces repetitive reasoning, illegible content, and post-answer exploration, supporting content-level supervision as a complementary direction to length-level control.
...
TL;DR: Proposes a Subjective Logic-based confidence semantics for assurance arguments that maps argument elements and relations to opinions and operators, enabling compositional confidence assessment compatible with GSN.


<details>
  <summary>Details</summary>
Motivation: Existing assurance argument notations (e.g., GSN) lack operational semantics for deriving confidence; current approaches reason over truth values rather than confidence, and existing SL-based methods lack uniform compositional semantics.

Method: Represents argument elements as Subjective Logic opinions and maps relations between elements to SL operators, creating an analyzable confidence network with explicit warrants, context handling, and provenance preservation.

Result: A compositional confidence semantics compatible with GSN that models how confidence propagates through argument structures under incomplete, conflicting, or subjective evidence.

Conclusion: Provides practical guidance for assurance confidence assessment, turning arguments into analyzable confidence networks with principled handling of uncertainty.
...
TL;DR: State-gated retrieval (SGR) requires navigating specialized websites to configure filters/views before extracting answers. SGR-Bench benchmarks this across 100 tasks; best system achieves 66.18% item-level F1, with major failures from retrieval-scope drift (37.2%) and criterion mismatch (27.6%).


<details>
  <summary>Details</summary>
Motivation: Existing web benchmarks overlook specialized data-retrieval tasks where answer-bearing evidence is only accessible after configuring site-specific retrieval states (filters, hierarchies, scopes). This gap underrepresents a critical real-world capability.

Method: Created SGR-Bench with 100 expert-curated tasks across 6 source families and 12 public data ecosystems. Each task requires discovering the correct website and configuring its retrieval state. Paired constraint-guided and goal-oriented formulations. Evaluated 8 CLI-based agentic LLMs and 3 commercial search agents.

Result: Best system scores 66.18% item-level F1; row-level F1 is much lower. Analysis of 156 failed trajectories shows retrieval-scope drift (37.2%) and criterion mismatch (27.6%) as primary failure causes, while answer composition accounts for only 10.3%.

Conclusion: Current LLM agents struggle with state-gated retrieval, especially establishing correct site-specific retrieval states. The benchmark reveals that reaching the right website is insufficient; agents fail at configuring proper filters and scopes. This highlights a distinct challenge for future agent development.
...
TL;DR: Static benchmarks miss real LLM behavior; in a timed Risk competition, Gemini-3.1-pro-preview dominated (20/32 wins), but when execution was standardized, provider differences vanished (p≈0.821), showing end-to-end system behavior matters more than planning alone. Success correlates with objective tracking and execution conversion, not runtime speed.


<details>
  <summary>Details</summary>
Motivation: Static benchmarks fail to capture how LLMs behave in realistic settings involving loops, time limits, formatting constraints, and failure modes.

Method: (1) Timed multi-phase Risk championship (32 games, frozen rules) across 4 providers. (2) Separated planning from execution by standardizing execution on a Gemini Flash scaffold (planner bakeoff). (3) Analyzed saved planning/execution traces.

Result: Gemini-3.1-pro-preview won 20/32 games (p≈1.5×10⁻⁵). Under standardized execution, pooled planner bakeoff consistent with equality (p≈0.821). Gemini referenced terminal objective more often, increased focus near victory, and converted more turns into deep conquest chains despite not being the cleanest runtime.

Conclusion: Live-agent performance depends on objective tracking, execution conversion, cost, and runtime reliability. LLMs should be evaluated as components in bounded workflows, not as isolated benchmark respondents.
...
TL;DR: This paper introduces a cognitive user simulator with stratified personas (external traits + hidden concerns) and a Simulator-Induced Asymmetric-View Policy Optimization (SIAPO) method that uses concern-aware self-distillation and state-transition refinement to make LLMs proactively persuasive in task-oriented dialogues.


<details>
  <summary>Details</summary>
Motivation: Current LLMs are inherently conservative in proactive TOD (e.g., outbound sales) and reward-shaping RL (e.g., GRPO) fails because it only re-weights passive policy samples instead of unlocking proactive behaviors.

Method: (1) Cognitive User Simulator with stratified personas (external traits + hidden concerns); (2) Simulator-Induced Asymmetric-View Policy Optimization (SIAPO) with Asymmetric On-Policy Self-Distillation and State-Transition Policy Refinement.

Result: The approach converts modeled concerns and simulation state transitions into complementary training objectives, enabling proactive persuasion within bounded turns.

Conclusion: Conditioning on user's latent concerns is a pivotal training-time signal that unlocks proactive capabilities beyond what passive sampling can achieve.
...
TL;DR: SciCore-Mol enhances LLMs for scientific tasks by integrating three pluggable modules (topology-aware perception, latent diffusion-based molecular generation, reaction-aware reasoning) that bridge the gap between language and molecular data, achieving competitive 8B open-source performance against proprietary models in molecular understanding, generation, and reaction prediction.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with heterogeneous scientific data (molecules, reactions) due to the fundamental gap between discrete linguistic symbols and topological/continuous molecular data, causing information loss and semantic noise in text-based reasoning.

Method: A modular framework (SciCore-Mol) with three pluggable cognitive modules: (1) topology-aware perception module, (2) latent diffusion-based molecular generation module, (3) reaction-aware reasoning module. Each module connects to the LLM backbone via learned representation interfaces for richer information exchange beyond text-only feedback.

Result: The 8B-parameter open-source system achieves strong performance across molecular understanding, generation, reaction prediction, and general chemistry knowledge, competing with and in several dimensions surpassing proprietary large models.

Conclusion: Provides a systematic blueprint for equipping LLMs with scientific expertise through decoupled, pluggable, and flexibly orchestrated modules, with implications for drug design, chemical synthesis, and scientific discovery.
...
TL;DR: This paper introduces a cross-domain benchmark to evaluate when coordinated AI agents outperform simpler scientific workflows, identifying three operating regimes: cross-channel composites improve performance when disciplines capture partial phenomena, coordination mainly aids interpretability when one signal dominates, and gains are representational rather than predictive in some cases. Results show AUROC 0.944 for climate-vector emergence and 0.955 for exoplanet vetting.


<details>
  <summary>Details</summary>
Motivation: Scientific evidence is fragmented across instruments, databases, and disciplines, making it unclear when coordinated AI agents add value over simpler scientific workflows.

Method: A cross-domain benchmark spanning four tasks (molecular sonification, paradigm-shift detection, vector-borne disease emergence, exoplanet vetting) with frozen evaluation panels, predefined scoring protocols, baselines, ablations, and stated limitations.

Result: Three regimes identified: cross-channel composites improve performance (AUROC 0.944 for climate-vector, 0.955 for exoplanet); decomposition doesn't always improve top-line; coordination mainly aids interpretability when one signal dominates; some gains are representational.

Conclusion: Coordination value should only be claimed when supported by explicit comparators for performance, provenance, or representation claims.
...
TL;DR: Proposes KGI-Bench, a benchmark for evaluating knowledge graph integration pipelines using three quality metrics (coverage, correctness, consistency) across different input data formats, demonstrated on 12 pipelines in the movie domain.


<details>
  <summary>Details</summary>
Motivation: No general approach exists to evaluate the overall quality and performance of different KG integration pipelines to determine the best choices.

Method: Develops KGI-Bench with three complementary quality metrics (coverage, correctness, consistency) and provides benchmark datasets (seed KG, overlapping input data in three formats, reference KG as ground truth) for the movie domain.

Result: Applicability demonstrated by comparatively evaluating 12 pipelines and analyzing their behavior across different input data formats and design choices.

Conclusion: KGI-Bench enables systematic evaluation and comparison of KG integration pipelines, helping determine optimal design choices for specific integration problems.
...
TL;DR: Proposes Meta-Soft, a dynamic KV cache compression framework that uses a learnable meta-library and Gumbel-Softmax selector to synthesize adaptive soft tokens for importance probing, plus attention-flow integration to preserve evicted context.


<details>
  <summary>Details</summary>
Motivation: Existing KV cache eviction methods (e.g., Judge Q) use fixed static soft tokens that cannot adapt to varying input prompts, and permanently discard evicted KV pairs causing irreversible information loss.

Method: (1) Meta-library with learnable orthogonal basis matrix; (2) Selector network with Gumbel-Softmax for differentiable sparse combination; (3) Dynamically synthesizes k targeted Soft Tokens from input features; (4) Attention-flow based integration mechanism redistributes semantic information from removed to retained tokens.

Result: Outperforms existing state-of-the-art eviction methods on multiple datasets.

Conclusion: Meta-Soft provides a new dynamic solution for KV Cache compression that adapts to input prompts and preserves context information.
...
TL;DR: This paper presents (sub-)symbolic techniques to scale solving of the Optimal Observability Problem (OOP), specifically the Sensor Selection Problem and Positional Observability Problem, achieving 3-5 orders of magnitude improvement in performance through POMDP decomposition.


<details>
  <summary>Details</summary>
Motivation: Balancing sensing capabilities against hardware/processing costs in uncertain domains is a fundamental engineering challenge, previously formalized as the Optimal Observability Problem.

Method: Improving the original parameter synthesis approach and developing a new solving method that identifies sensible observation functions via decomposition of POMDPs.

Result: Improved performance by 3 orders of magnitude for instance size and 5 orders of magnitude for runtime.

Conclusion: The proposed (sub-)symbolic techniques significantly scale solving of decidable OOP fragments.
...
TL;DR: Epicure introduces three skip-gram ingredient embeddings (Cooc, Chem, Core) trained on a multilingual recipe corpus of 4.14M recipes, using an LLM-augmented pipeline to normalize ingredients and two graphs (co-occurrence and FlavorDB compound) to generate embeddings via Metapath2Vec variants.


<details>
  <summary>Details</summary>
Motivation: To create ingredient embeddings that capture both recipe co-occurrence patterns and chemical compound relationships across multiple languages and cultures.

Method: Aggregated 4.14M recipes from 11 sources across 9 languages; normalized to 1,790 canonical ingredients via LLM-augmented pipeline; built co-occurrence (NPMI) and chemical compound (FlavorDB) graphs; trained three Metapath2Vec variants with different random-walk strategies (Cooc, Chem, Core).

Result: Three distinct embedding models spanning the chemistry-vs-recipe-context spectrum, with 203,508 co-occurrence edges and 80,019 compound edges across 2,247 typed compound nodes.

Conclusion: The Core model blending both graphs offers a balanced representation, while Cooc and Chem sit at opposite ends of the recipe-context vs. chemistry spectrum.
...
TL;DR: S2ED is a training-free framework that converts full stories into executable descriptions using three coordinated agents, improving multi-frame story illustration coherence and character fidelity over existing methods.


<details>
  <summary>Details</summary>
Motivation: Multi-frame story illustration requires long-horizon coherence beyond single-image generation, including narrative decomposition and persistent character identity, layout, and affect across frames, which existing methods struggle with.

Method: S2ED uses three coordinated agents to segment narrative, ground canonical character attributes, and enrich spatial/affective cues, converting stories into explicit editable executable descriptions with interpretable prompt-carried state propagation.

Result: Experiments on Flintstones and Shakoo Maku show S2ED improves sequence-level consistency and character fidelity over strong prompting, large-model planning, and reference training-based methods under both automatic metrics and human judgments.

Conclusion: S2ED is a training-free, model-agnostic approach that enhances story illustration coherence and enables local edits without retraining, demonstrated in an end-to-end story-to-storybook system.
...
TL;DR: The paper presents KAPPS, a knowledge-based architecture for Cyber Physical Production Systems (CPPS) in circular manufacturing. It uses an ontology-grounded knowledge graph as a central data backbone with a semantic interface layer to handle heterogeneous product states, dynamic reconfiguration, and human-machine knowledge exchange. The architecture is evaluated through two use cases: anomaly detection and runtime constraint enforcement.


<details>
  <summary>Details</summary>
Motivation: Traditional manufacturing IT architectures are designed for linear manufacturing with homogeneous materials and stable processes, but circular manufacturing involves heterogeneous, uncertain used products. Existing systems cannot represent and manage the uniqueness of individual components at runtime, necessitating a new architecture.

Method: Design science methodology is used to derive 14 requirements from five complementary perspectives. Based on these, KAPPS is designed as a knowledge-based architecture featuring an ontology-grounded knowledge graph as a unifying data backbone, a semantic interface layer, and modules for constraint enforcement and event-driven planning.

Result: Two implemented use cases demonstrate applicability: (1) anomaly detection and learning through knowledge graph mediated services, and (2) runtime constraint enforcement in a modular conveyor system. The architecture is evaluated against all 14 requirements.

Conclusion: KAPPS enables circular manufacturing by turning the knowledge graph from an integration layer into the factory's authoritative write-time state, supporting incremental adaptation of execution plans under uncertainty and facilitating human-machine knowledge exchange.
...
TL;DR: The paper identifies that while agent orchestration frameworks dominate (290K+ GitHub stars), they are inferior for procedural tasks compared to alternative approaches. The authors propose "subterranean agents" - compiling procedures into fine-tuned model weights - which overcomes limitations of both orchestration and prompting approaches. They empirically address three perceived barriers across three domains (travel booking, Zoom support, insurance claims) with varying complexity.


<details>
  <summary>Details</summary>
Motivation: Current agent orchestration frameworks dominate despite known limitations (context window consumption, requiring frontier models, exposing proprietary procedures). Prior work shows fine-tuning small models works, but adoption lags, suggesting perceived barriers need empirical investigation.

Method: The authors identify three perceived barriers to adopting fine-tuned "subterranean agents" and empirically address them across three domains: travel booking (14 nodes), Zoom support (14 nodes with product-specific knowledge), and insurance claims (55 nodes, 6 decision hubs).

Result: Not fully specified in abstract, but the paper claims to address each perceived barrier empirically across three diverse domains of varying complexity.

Conclusion: Compiling procedures into small fine-tuned model weights ("subterranean agents") resolves context window issues, reduces frontier model dependency, and protects proprietary procedures - potentially offering a superior alternative to orchestration frameworks for procedural tasks.
...
TL;DR: LACO proposes a training-free latent communication paradigm for collaborative driving that reduces latency and prevents information loss by using Iterative Latent Deliberation, Cross-Horizon Saliency Attribution, and Structured Semantic Knowledge Distillation, outperforming language-based approaches in CARLA simulations.


<details>
  <summary>Details</summary>
Motivation: Language-based communication in collaborative driving suffers from high latency (autoregressive decoding) and information loss (compressing rich representations into discrete tokens). The authors aim to overcome these limitations with latent communication.

Method: LACO (Latent Communication) is a training-free paradigm with three components: 1) Iterative Latent Deliberation (ILD) for latent reasoning, 2) Cross-Horizon Saliency Attribution (CHSA) for efficient information selection, and 3) Structured Semantic Knowledge Distillation (SSKD) to stabilize ego-centric decisions. It also addresses agent identity confusion in latent fusion.

Result: In closed-loop CARLA experiments, LACO significantly reduces communication and inference latency while maintaining strong collaborative driving performance compared to language-based methods.

Conclusion: Latent communication can effectively replace language-based coordination in collaborative driving, offering lower latency and richer information transfer without requiring additional training.
...
TL;DR: This paper proposes a direct evaluation method called "priority ranking" for harness optimizers in automated agent creation, which assesses optimizer ability at the step level without expensive rollouts, and demonstrates its correlation with actual optimization performance using a new benchmark called Shor with 182 human-verified scenarios.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of harness optimizers rely on indirect end-improvement metrics (target agent performance gains), which overlook intermediate erroneous actions and cannot distinguish informed updates from trial-and-error. Direct evaluation is needed but is costly due to lack of oracle harnesses.

Method: Priority ranking - asks optimizers to rank harness components (e.g., tools) by their potential to improve/hinder agent performance when updated. This enables step-level quantification without expensive rollouts or manual examination. Also introduces Shor, a collection of 182 human-verified optimization scenarios.

Result: Optimizers' ranking performance correlates with their ability to improve agents in actual multi-step harness optimization, validating priority ranking as a reliable predictor of optimization ability.

Conclusion: Priority ranking provides a simple, low-cost, and reliable direct evaluation method for harness optimizers, addressing the gap in current evaluation practices.
...
TL;DR: Search-E1 proposes a simple self-evolution method using vanilla GRPO and offline self-distillation for search-augmented language models, achieving strong results on 7 QA benchmarks without complex auxiliary machinery.


<details>
  <summary>Details</summary>
Motivation: To question whether the elaborate augmentations (external supervision, auxiliary modules, tree search, hand-crafted rewards) added to post-training pipelines for search-augmented reasoning agents are actually necessary.

Method: Self-evolution method combining vanilla GRPO with offline self-distillation (OFSD). After each GRPO round, the policy rolls out on training questions; a token-level forward KL objective aligns the policy's inference-time distribution to its own distribution under a privileged context exposing a more efficient sibling trajectory.

Result: 0.440 average EM on 7 QA benchmarks with Qwen2.5-3B, surpassing all open-source baselines at both 3B and larger scales.

Conclusion: Elaborate augmentations may be unnecessary; the proposed simple self-evolution pipeline can outperform more complex approaches with dense per-step supervision naturally provided by the procedure.
...
TL;DR: A meta-learning control framework using iMAML to design optimal controllers for uncertain nonlinear systems with limited target system data, leveraging source system data for improved performance.


<details>
  <summary>Details</summary>
Motivation: Collecting sufficient data from target systems is challenging; thus, leveraging data from similar source systems via meta-learning can improve control performance with limited target data.

Method: Proposes a bi-level optimization framework based on iMAML with offline meta-training (learning shared dynamics from source systems) and online meta-adaptation (fine-tuning on target system). Two algorithm instantiations: neural state-space model (with system ID) and deep Q-network (without explicit ID).
...
Conclusion: The meta-learning control framework effectively transfers knowledge from source systems to target systems with limited data, offering flexibility and improved performance.
...
TL;DR: A method for dynamic quantitative assurance that enhances static safety cases with runtime-driven confidence updates using Subjective Logic, demonstrated on an ML-based construction cone detection system.


<details>
  <summary>Details</summary>
Motivation: Static safety cases lack runtime adaptability; there is a need to continuously integrate operational evidence to update confidence in safety claims throughout the system lifecycle.

Method: Integrates design-time evidence with windowed runtime Safety Performance Indicators (SPIs) within a Subjective Logic (SL)-based assurance case. Uses a rule-based update that increases confidence on no violations and penalizes promptly on violations, prioritizing safety responsiveness over exact Bayesian updates.

Result: Demonstrated via simulation of a construction zone assist function with an ML-based cone detection component, showing how confidence evolves as SPI evidence is observed during operation.

Conclusion: The approach enables dynamic, runtime-driven confidence updates in safety cases, offering a practical balance between responsiveness and quantitative rigor for safety-critical ML components.
...
TL;DR: TerminalWorld is a scalable data engine that automatically extracts evaluation tasks from real terminal recordings, producing a benchmark of 1,530 validated tasks across 18 categories. Current frontier models only achieve 62.5% pass rate, and the benchmark captures distinct capabilities from existing benchmarks (Pearson r=0.20).


<details>
  <summary>Details</summary>
Motivation: Existing terminal benchmarks are expert-curated and may not capture the diversity and authenticity of real-world terminal workflows, limiting evaluation of AI agents in practical settings.

Method: Processed 80,870 terminal recordings through an automated data engine to reverse-engineer 1,530 evaluation tasks, with a Verified subset of 200 manually reviewed tasks covering 18 categories and 1,280 unique commands.

Result: Maximum pass rate of 62.5% across 8 frontier models and 6 agents. Weak correlation (Pearson r=0.20) with existing Terminal-Bench benchmark.

Conclusion: TerminalWorld provides an authentic and scalable benchmark for evaluating terminal AI agents, revealing that current systems still struggle with real-world terminal workflows.
...
TL;DR: This paper introduces ToM-PD, a Theory of Mind-based Persuasive Dialogue task using the BDI framework, along with a large-scale annotated dataset (ToM-BPD) and a knowledge-enhanced stepwise reasoning framework (TTBYS) that improves LLMs' inference of mental states and persuasive strategies, outperforming GPT-5 in desire, belief, and strategy prediction.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs struggle with persuasive dialogue due to insufficient Theory of Mind reasoning—they fail to capture the intrinsic dependencies among mental states (beliefs, desires, intentions), resulting in fragmented representations and unstable reasoning.

Method: The authors (1) formalize the ToM-PD task grounded in the BDI framework; (2) construct ToM-BPD, a large-scale annotated dataset with fine-grained mental states and strategies; and (3) propose TTBYS (Think Thrice Before You Speak), a knowledge-enhanced stepwise reasoning framework that leverages explicit and implicit prior experiences.

Result: Qwen3-8B with TTBYS outperforms GPT-5 by 1.20%, 22.80%, and 16.97% in predicting desires, beliefs, and persuasive strategies, respectively.

Conclusion: The proposed approach enhances interpretability and consistency in reasoning, demonstrating that structured ToM modeling with stepwise reasoning significantly improves persuasive dialogue generation.
...
TL;DR: Spreadsheet-RL is a reinforcement learning fine-tuning framework that trains specialized spreadsheet agents in realistic Microsoft Excel environments, significantly improving performance on both general (12%→23.4% on SpreadsheetBench) and domain-specific (8.4%→17.2% on Domain-Spreadsheet) spreadsheet tasks.


<details>
  <summary>Details</summary>
Motivation: Existing spreadsheet agents rely on specialized prompting over general-purpose LLMs, which works for simple operations but struggles with complex, multi-step workflows typical of real-world applications. There is no dedicated RL fine-tuning framework for spreadsheet agents in realistic environments.

Method: Spreadsheet-RL includes: (1) an automated pipeline for scalable collection of paired start-goal spreadsheets from online forums, (2) the Domain-Spreadsheet benchmark dataset for finance and supply chain tasks, and (3) a Spreadsheet Gym environment for multi-turn RL with a Python sandbox, comprehensive tool set, and tool-routing rules.

Result: Qwen3-4B-Thinking-2507's Pass@1 improved from 12.0% to 23.4% on SpreadsheetBench and from 8.4% to 17.2% on Domain-Spreadsheet dataset.

Conclusion: Spreadsheet-RL demonstrates strong potential for generalization and real-world adoption in spreadsheet automation, and broadly advances LLM-based interactions with data interfaces in everyday work.
...
TL;DR: AtelierEval is the first unified benchmark to evaluate prompting proficiency in text-to-image systems, covering both humans and MLLMs across 360 expert-crafted tasks, with a novel agentic evaluator (AtelierJudge) achieving high correlation with human experts.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks fix the prompt and only evaluate T2I models, leaving the prompting proficiency of upstream components (humans or MLLMs) entirely unmeasured.

Method: Proposes AtelierEval (360 expert-crafted tasks across three categories) and AtelierJudge (skill-based, memory-augmented agentic evaluator producing subjective + objective scores).

Result: AtelierJudge achieves Spearman correlation of 0.79 with human experts; benchmarks 8 MLLMs vs 48 humans across 4 T2I backends; reveals superiority of mimicry over planning.

Conclusion: AtelierEval serves as a robust diagnostic tool, advocating for an image-augmented direction for future prompters.
...
TL;DR: Claw AI Lab is an interactive autonomous research platform that allows users to instantiate customizable research teams from a single prompt, featuring a Claw-Code Harness for code/data integration, real-time monitoring, and rollback control. It outperforms the baseline AutoResearchClaw in idea novelty, experiment completeness, and paper quality across five AI case studies.


<details>
  <summary>Details</summary>
Motivation: To advance autonomous research beyond rigid hidden prompt-to-paper pipelines into a steerable, interactive AI laboratory with human-in-the-loop control and real-time visibility.

Method: Users instantiate a full research team from one prompt with customizable roles, collaborative workflows, real-time monitoring, artifact inspection, and rollback/resume via a unified dashboard. The Claw-Code Harness connects local codebases, datasets, and checkpoints to runnable experiments, feeding execution artifacts back into the research loop.

Result: In internal evaluation on five AI research case studies using AutoResearchClaw as baseline, Claw AI Lab is consistently preferred by AI expert judges on idea novelty, experiment completeness, and paper presentation quality.

Conclusion: Claw AI Lab is an early step toward a new paradigm of autonomous research as usable, interactive, and reliability-aware scientific infrastructure.
...
TL;DR: This paper introduces a benchmark for evaluating LLM agents on end-to-end spreadsheet creation tasks in finance, finding that even the best agents (Claude) fall short of professional standards as task complexity increases.


<details>
  <summary>Details</summary>
Motivation: Existing spreadsheet benchmarks only measure QA or single-formula edits, failing to capture the advanced capability of agents constructing entire spreadsheets from scratch for complex financial workflows.

Method: The authors develop a three-dimensional evaluation taxonomy (Accuracy, Formula, Format) with fine-grained criteria reflecting professional standards, and benchmark LLM agents on end-to-end financial spreadsheet tasks including modeling and scenario analysis.

Result: Claude family leads the benchmark with the most professional-looking outputs, but all agents degrade sharply as difficulty increases beyond a few chained calculations and frequently fall short of professional finance standards.

Conclusion: Current LLM agents are not yet capable of reliably producing professional-quality spreadsheets at the level of complexity demanded by real-world financial workflows.
...
TL;DR: LLMs exhibit inverse scaling on forecasting tasks with superlinear growth and tail risk, where more capable models produce worse distributional forecasts—especially at the upper tail. This pattern appears in synthetic and real-world datasets, is missed by single-threshold metrics, and is driven by both model scale and post-training.


<details>
  <summary>Details</summary>
Motivation: To expose a failure mode of LLM forecasting on problems with superlinear growth and tail risk of regime change (common in finance and epidemiology), and to show that conventional single-threshold evaluation metrics obscure this degradation.

Method: Introduces ForecastBench-Sim (FBSim), a contamination-free benchmark; compares forecasting performance on synthetic SIR epidemics with linear controls; validates on real-world COVID-19, measles, housing, and hyperinflation data; uses per-quantile decomposition; conducts within-family study of Llama-3.1 to isolate scale vs. post-training effects.

Result: More capable models produce worse distributional forecasts on superlinear/tail-risk tasks; failure concentrates at the upper tail; both model scale and post-training independently contribute; single-threshold metrics miss this and reverse the sign of the capability–accuracy relationship; tail-inclusive scoring captures the true degradation.

Conclusion: LLM forecasting evaluations should use continuous, unbounded accuracy measures alongside bounded binary threshold metrics to avoid masking inverse scaling in tail-risk regimes.
...
TL;DR: Current AI systems cannot reliably predict scientific progress, showing systematic limitations across disciplines despite having relevant knowledge.


<details>
  <summary>Details</summary>
Motivation: To investigate whether AI can anticipate scientific progress—a question underexplored in AI-for-science research.

Method: Introduces CUSP benchmark with 4,760 scientific events; evaluates models on feasibility assessment, mechanistic reasoning, generative solution design, and temporal prediction under controlled knowledge constraints.

Result: Models fail to reliably predict if/when advances will occur; performance varies by domain (AI timing more predictable); insensitive to training cutoff; overconfident; post-event info helps more than forward-looking prediction.

Conclusion: Current AI systems are unreliable predictive tools for scientific progress; knowledge access does not translate to forecasting ability.
...
TL;DR: This paper examines how AI adoption is changing work processes, role boundaries, and informal cultural practices (like mentoring) at a large tech firm. It finds both positive changes (smoother collaboration) and risks to career growth opportunities, proposing steps to make invisible work visible and preserve healthy company culture.


<details>
  <summary>Details</summary>
Motivation: To understand how AI adoption is changing work processes, role boundaries, and professional interactions in AI-forward companies, particularly how AI impacts both formal responsibilities and informal cultural practices like mentoring.

Method: Qualitative interviews with 24 product-focused professionals at a large technology firm about how AI has impacted their work, team dynamics, and professional interactions.

Result: AI changes formal role responsibilities and collaborations, but also informal practices like mentoring. Positive changes include smoother peer collaboration; however, career growth opportunities like feedback from networks and mentorship are at risk.

Conclusion: Companies should make invisible work more visible; individuals and leaders should support colleagues through AI transformation while preserving healthy cultures that encourage diverse thinking, collaboration, and informal interactions.
...
TL;DR: Prior conversation polarity biases LLM evaluations, especially for uncertain items, with negativity causing 1.62x more bias than positivity; scaling models helps but doesn't eliminate it.


<details>
  <summary>Details</summary>
Motivation: To investigate whether the polarity (positive/negative) of prior conversation history biases subsequent LLM judgments when used as automated evaluators.

Method: 75,898 API calls to 11 models from 4 providers (OpenAI, Anthropic, Google, four open-source). Presented identical test items in isolation vs. following positive/negative history. Measured shift in judgments, entropy effects, context length, and negativity asymmetry.

Result: Models shift toward prevailing polarity (d=-0.17, p<10^-46). Effect concentrates on high-uncertainty items (d=-0.34 vs -0.15). Bias doesn't grow with context length. Negativity asymmetry: 1.62x more bias from negative histories. Scaling helps marginally (Haiku -0.22 to Opus -0.17; Nano -0.34 to GPT-5.2 -0.17).

Conclusion: The simplest fix is fresh context per item; when batching unavoidable, balancing history helps. Scaling alone insufficient.
...
TL;DR: This paper introduces parametric modular logic programs for modular first-order ASP, showing how they capture clingo-programs with collective control and connect to traditional non-modular ASP.


<details>
  <summary>Details</summary>
Motivation: To formalize the concept of modularity in first-order answer set programming (ASP), enabling structured, parameterized subprogram definitions with intensionality.

Method: Introduces parametric modular logic programs, a new formalism that supports parameterized subprograms and intensionality statements, and demonstrates its equivalence to clingo-programs with collective control.

Result: The formalism captures the semantics of clingo-programs with collective control; provides theoretical foundations for modular ASP; establishes connections to non-modular ASP.

Conclusion: Parametric modular logic programs offer a principled way to modularize first-order ASP, with demonstrated usefulness and formal grounding.
...
TL;DR: AI models in conflict zones show 6-47% failure rates on conflict-sensitive tasks, with five of nine configurations failing 80-100% when pushed for false "balance" on documented atrocities - the first evaluation framework for this domain is proposed.


<details>
  <summary>Details</summary>
Motivation: AI models are already deployed in armed conflict contexts without established safety checks for whether their outputs can worsen conflicts through false equivalence, genocide denial, or failure to recognize ethnic slurs.

Method: Tested nine model configurations from OpenAI, Anthropic, DeepSeek, and xAI on 90 multi-turn scenarios designed to surface misaligned behavior in conflict contexts.

Result: Failure rates range from 6% (best) to 47% (worst); when users pushed for "balance" where international courts assigned responsibility, five of nine configurations failed 80-100% of the time.

Conclusion: Model choice is a safety question in conflict contexts; the paper releases the first evaluation framework for this domain and proposes adding it to alignment evaluation portfolios.
...
TL;DR: LLM-based multimodal analysis (Gemini) strongly correlates with TRUST-Pathos (political emotion dimension), while acoustic SER models (emotion2vec) fail to capture this construct. Standard SER benchmarks suffer from acted speech and cultural bias.


<details>
  <summary>Details</summary>
Motivation: To test whether acoustic emotion recognition models can serve as proxies for the Pathos dimension in political speech analysis, compared to LLM-based approaches.

Method: Compared three modalities: (1) emotion2vec_plus_large with Russell Circumplex projection; (2) Gemini 2.5 Flash analyzing audio+transcript; (3) TRUST-Pathos from three-advocate LLM ensemble. Used Spearman correlations on a Bundestag speech case study (51 segments). Also evaluated EMO-DB quality with Gemini.

Result: Gemini Valence correlates strongly with TRUST-Pathos (rho=+0.664, p<0.001), but emotion2vec Valence does not (rho=+0.097, p=0.499). EMO-DB shows issues with acted speech, cultural bias, and category incompatibility.

Conclusion: LLM-based multimodal analysis captures semantically defined political emotion substantially better than acoustic models alone. Acoustic features remain useful for low-level Arousal. Future work will extend to video-based analysis.
...
TL;DR: HarnessAPI is a Python framework that eliminates duplication between HTTP endpoints and MCP tools for LLM functions by deriving both from a single typed skill folder, reducing boilerplate by 74%.


<details>
  <summary>Details</summary>
Motivation: Every Python function deployed as an LLM tool must exist in two forms (HTTP endpoint + MCP tool), sharing business logic but diverging in routing, validation, serialisation, streaming, and schema maintenance. These representations drift apart as code evolves.

Method: Uses a typed skill folder as single source of truth. From one handler.py plus Pydantic schemas, it derives a streaming HTTP endpoint (SSE), OpenAPI/Swagger UI, and a zero-config MCP tool from a single process. Features dual-mode content negotiation and dynamic code-generation for Pydantic-to-FastMCP type propagation.

Result: Measured across six skills using cloc, reduces framework-facing boilerplate by 74% compared to manual dual-stack (FastAPI + FastMCP). Subclasses FastAPI, inheriting its middleware, DI, and deployment ecosystem.

Conclusion: HarnessAPI eliminates source-code duplication for dual-representation LLM tools, providing a unified framework that serves both human-facing and agent-facing clients from a single codebase.
...
TL;DR: A foundation model pretrained on 1+ trillion minutes of unlabeled wearable sensor data from 5 million participants, achieving strong performance across 35 health prediction tasks via scaling model/data size, few-shot learning, and LLM-agent-driven downstream optimization.


<details>
  <summary>Details</summary>
Motivation: Wearable sensor data is abundant but difficult to translate into personalized health insights due to high phenotypic diversity, individual variability in baseline health, and scarcity of high-quality labeled data from expensive/impractical annotation processes.

Method: Pretrained a foundation model on >1 trillion minutes of unlabeled wearable data from 5M participants; demonstrated scaling laws with model/data size; used LLM agents to autonomously search downstream predictive heads; integrated into a Personal Health Agent validated by clinician ratings.

Result: Systematic performance improvements from joint scaling of model capacity and data; label-efficient few-shot learning and robust daily metric estimation; LLM agents improved downstream prediction performance increasing with LLM capacity; Personal Health Agent achieved more relevant/context-aware/safe responses per 1,860 clinician ratings.

Conclusion: Population-scale pretraining on unlabeled wearable data, combined with LLM-driven downstream optimization, enables scalable, label-efficient health prediction and safe personalized health agents.
...
TL;DR: Large-scale evaluation shows LLM-based formal proof search can autonomously solve open mathematical problems (e.g., 9/353 Erdős problems, 44/492 OEIS conjectures) at moderate cost, demonstrating promise for AI-assisted mathematics research.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly capable at mathematical reasoning but remain unreliable; the authors aim to evaluate whether generating formal proofs (in Lean) can overcome this unreliability and solve genuinely open problems.

Method: Large-scale evaluation using LLM-based agents that alternate generation with Lean-based verification; tested on open Erdős problems and OEIS conjectures across multiple mathematical domains.

Result: The most capable agent autonomously solved 9/353 Erdős problems (~$100s each), proved 44/492 OEIS conjectures, and is being deployed in combinatorics, optimization, graph theory, algebraic geometry, and quantum optics.

Conclusion: AI-aided formal proof search is effective and cost-efficient for open mathematical problems; agent design (LLM + Lean verification) is key, though simpler agents replicate successes at higher cost for hardest problems.
...
TL;DR: This paper proposes an event-based Deep Reinforcement Learning (DRL) approach using PPO and MLPs to solve the Flexible Job Shop Scheduling Problem (FJSP) with random job arrivals, outperforming individual dispatching rules and benchmark MILP solutions, especially on heterogeneous datasets.


<details>
  <summary>Details</summary>
Motivation: FJSP faces two key challenges: unpredictable future job arrivals and combinatorial complexity, making traditional MILP solvers impractical.

Method: Uses Proximal Policy Optimization (PPO) with lightweight Multi-Layer Perceptrons (MLPs). The DRL agent selects from established dispatching rules based on directly observable state representations.

Result: The DRL approach outperforms individual dispatching rules across datasets with varying heterogeneity and job arrival rates, and shows strong performance against arrival-triggered MILP solutions, particularly on heterogeneous datasets.

Conclusion: Event-based DRL is an effective approach for dynamic FJSP, especially under heterogeneous conditions where traditional methods struggle.
...
TL;DR: LCGuard introduces a framework to prevent sensitive information leakage in KV cache-based latent communication for multi-agent LLM systems, using adversarial training to learn representation-level transformations that preserve task utility while reducing reconstructable sensitive content.


<details>
  <summary>Details</summary>
Motivation: KV caches in multi-agent LLM systems encode sensitive inputs and reasoning states, creating an opaque channel for potential information leakage without explicit textual disclosure.

Method: LCGuard treats shared KV caches as latent working memory, applies representation-level transformations before transmission, and uses adversarial training where an adversary learns to reconstruct sensitive inputs while the guard learns to reduce reconstructable information while preserving task semantics.

Result: Across multiple model families and benchmarks, LCGuard consistently reduces reconstruction-based leakage and attack success rates while maintaining competitive task performance compared to standard KV-sharing baselines.

Conclusion: LCGuard effectively mitigates privacy risks in latent communication for multi-agent LLM systems without sacrificing task performance.
...
TL;DR: Gated DeltaNet-2 improves linear attention by introducing separate channel-wise erase and write gates, achieving better memory management and state-of-the-art results across language modeling, reasoning, and long-context retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: Existing delta-rule linear attention models (Gated DeltaNet, KDA) use a single scalar gate to control both erasing old content and writing new content, which limits their ability to independently manage these two operations.

Method: Introduces separate channel-wise erase gate (b_t) and write gate (w_t), replacing the tied scalar gate. Also provides a fast-weight update view, chunkwise WY algorithm with channel-wise decay, and gate-aware backward pass for efficient training.

Result: At 1.3B parameters trained on 100B FineWeb-Edu tokens, Gated DeltaNet-2 outperforms Mamba-2, Gated DeltaNet, KDA, and Mamba-3 variants on language modeling, commonsense reasoning, and especially long-context RULER benchmarks.

Conclusion: Decoupling erase and write gates in delta-rule linear attention improves both short and long-context performance, with code publicly available.
...
TL;DR: MOSS is a self-rewriting system that evolves production agentic systems at the source-code level (not just text artifacts), using automated failure evidence, deterministic pipelines, and safe container-swap deployment to fix structural failures autonomously.


<details>
  <summary>Details</summary>
Motivation: Current autonomous agents are static after deployment and cannot learn from interaction failures. Existing self-evolving agents only modify text-level artifacts (skills, prompts, memory) but cannot fix structural failures in the code layer (routing, hooks, state invariants, dispatch).

Method: MOSS performs source-level self-rewriting via: (1) automated curation of production-failure batches; (2) deterministic multi-stage pipeline with stage ordering and verdicts; (3) pluggable external coding-agent CLI for code modification; (4) batch replay verification on ephemeral trial workers; (5) user-consent-gated in-place container swap with health-probe rollback.

Result: On OpenClaw, MOSS improved a four-task mean grader score from 0.25 to 0.61 in a single autonomous cycle without human intervention.

Conclusion: Source-level adaptation is a more general medium than text-mutable artifacts—it is Turing-complete, a superset of text scopes, deterministic, and drift-resistant—and MOSS demonstrates feasible autonomous structural evolution in production agentic systems.
