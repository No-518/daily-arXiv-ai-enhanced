...
TL;DR: A-SelecT automatically selects the most information-rich timestep from Diffusion Transformer features in a single run, improving discriminative representation learning without exhaustive search.


<details>
  <summary>Details</summary>
Motivation: Current Diffusion Transformer (DiT) models have constrained training efficiency and representational capacity due to inadequate timestep searching and insufficient exploitation of DiT-specific feature representations.

Method: Introduces Automatically Selected Timestep (A-SelecT) that dynamically pinpoints DiT's most information-rich timestep from selected transformer features in a single run, eliminating need for exhaustive timestep searching and suboptimal feature selection.
...
Conclusion: A-SelecT enables more efficient and effective discriminative representation learning from diffusion models by automatically selecting optimal timesteps, addressing key limitations in current DiT approaches.
...
TL;DR: OCR/document understanding evaluation focuses on modern Western documents, ignoring historical Black newspapers, leading to structural invisibility and representational harm.


<details>
  <summary>Details</summary>
Motivation: Current OCR and document understanding evaluation emphasizes modern, Western, institutional documents, masking system failures on historical/marginalized archives like Black newspapers where layout, typography, and degradation affect interpretation.

Method: Systematic literature review (2006-2025) using PRISMA framework to analyze OCR/document understanding papers and benchmark datasets, examining training data, benchmark design, and evaluation metrics for vision transformer/multimodal systems.

Result: Black newspapers rarely appear in training data or benchmarks; evaluations focus on character accuracy and modern layouts, missing structural failures (column collapse, typographic errors, hallucinations) common in historical newspapers.

Conclusion: Evaluation gaps cause structural invisibility and representational harm, stemming from organizational/institutional behaviors shaped by benchmark incentives and data governance decisions.
...
TL;DR: Using Stable Diffusion XL to generate synthetic ceramic surface images reduces need for expensive experimental data while maintaining classification accuracy for roughness assessment.


<details>
  <summary>Details</summary>
Motivation: AI for surface roughness classification is limited by expensive high-resolution imaging equipment and large labeled datasets, creating barriers for practical industrial applications.

Method: Generate synthetic ceramic surface images using Stable Diffusion XL, augment authentic datasets with these synthetic images, and systematically vary training hyperparameters (epoch count, batch size, learning rate) to optimize performance.

Result: Augmenting authentic datasets with generative images yields test accuracies comparable to exclusively experimental images, with identified hyperparameter configurations preserving performance while reducing data requirements.

Conclusion: Generative AI can substantially improve data efficiency and reliability in materials-image classification workflows, offering practical benefits for lower experimental cost, accelerated model development, and expanded AI applicability in materials engineering.
...
TL;DR: FPRL is a cognition-inspired hierarchical framework for endoscopic video representation learning that focuses on lesion-centric static semantics first, then models their temporal evolution, outperforming existing methods on 11 endoscopic datasets.


<details>
  <summary>Details</summary>
Motivation: Endoscopic video analysis is crucial for early GI screening but suffers from limited high-quality annotations. Existing self-supervised methods for natural videos prioritize dense spatio-temporal modeling and exhibit motion bias, overlooking the static, structured semantics essential for clinical decision-making in endoscopic examinations.

Method: FPRL uses a hierarchical semantic modeling framework: (1) Static semantics learning via teacher-prior adaptive masking (TPAM) with multi-view sparse sampling to focus on lesion-centric regions and reduce temporal redundancy; (2) Contextual semantics modeling through cross-view masked feature completion (CVMFC) and attention-guided temporal prediction (AGTP) to establish cross-view correspondences and model structured inter-frame evolution while preserving global context.

Result: Extensive experiments on 11 endoscopic video datasets demonstrate superior performance across diverse downstream tasks, showing FPRL's effectiveness in endoscopic video representation learning compared to existing methods.

Conclusion: FPRL successfully addresses the limitations of existing video pre-training methods by emulating clinical examination patterns, effectively learning both static lesion semantics and their temporal evolution, making it a promising approach for endoscopic video analysis with limited annotations.
...
TL;DR: ArtHOI is an optimization framework that reconstructs 4D human-articulated-object interactions from single monocular RGB videos by integrating and refining priors from multiple foundation models.


<details>
  <summary>Details</summary>
Motivation: Existing methods are limited to rigid objects or require pre-scanning/multi-view videos. There's an unexplored challenge of reconstructing 4D human-articulated-object interactions from single monocular RGB videos, which foundation models now make possible to address.

Method: Optimization-based framework integrating priors from multiple foundation models. Key innovations: 1) Adaptive Sampling Refinement (ASR) to optimize object's metric scale and pose for world-space grounding, 2) MLLM-guided hand-object alignment using contact reasoning as constraints for mesh composition optimization.
...
Conclusion: ArtHOI successfully addresses the challenging problem of 4D human-articulated-object interaction reconstruction from single monocular videos by effectively integrating and refining foundation model priors with novel optimization methods.
...
TL;DR: FA-CNN is a CNN architecture with intrinsic class attribution through end-to-end feature alignment, using order-preserving layers to maintain alignment from input to output, making feature maps interpretable and equivalent to Grad-CAM saliency maps.


<details>
  <summary>Details</summary>
Motivation: Traditional CNNs use unordered operations (Linear, Conv2D) that shuffle semantic concepts, making feature maps difficult to interpret. The authors aim to create a CNN with intrinsic interpretability through feature alignment.

Method: Introduces Feature-Align CNN with two new order-preserving layers: dampened skip connection and global average pooling classifier head. These maintain end-to-end feature alignment from input pixels to class logits, allowing raw feature maps to exhibit class attribution.

Result: Proves theoretically that FA-CNN feature maps are identical to Grad-CAM saliency maps and show gradual feature evolution through network depth. Performs well on benchmark datasets and compares favorably to Grad-CAM and permutation methods in interpretability tasks.

Conclusion: FA-CNN provides intrinsic interpretability through feature alignment while maintaining competitive performance. The work discusses limitations and future extensions toward hybrid models combining interpretability with high performance.
...
TL;DR: LEMON is a self-supervised foundation model that learns morphological embeddings from millions of single-cell images across diverse tissues and cancer types, enabling robust cell-level analysis in computational pathology.


<details>
  <summary>Details</summary>
Motivation: While self-supervised learning has advanced patch and whole-slide image analysis in computational pathology, single-cell level representation learning remains underexplored despite its critical importance for characterizing cell types and cellular phenotypes.

Method: Developed LEMON (Learning Embeddings from Morphology Of Nuclei), a self-supervised foundation model trained on millions of cell images from diverse tissues and cancer types to learn robust morphological representations.

Result: LEMON demonstrates strong performance across five benchmark datasets on various prediction tasks, showing its effectiveness as a scalable solution for single-cell image representation learning.

Conclusion: LEMON represents a new paradigm for cell-level computational pathology, providing versatile morphological representations that support large-scale single-cell analyses, with model weights publicly available.
...
TL;DR: Reproduction study of Darcet et al.'s register tokens for ViTs confirms some claims but shows limited generalizability across models, extends findings to smaller models, and clarifies terminology inconsistencies.


<details>
  <summary>Details</summary>
Motivation: To reproduce and evaluate the generalizability of Darcet et al.'s findings about register tokens eliminating attention map artifacts in Vision Transformers across multiple models and model sizes.

Method: Reproduced Darcet et al.'s register token approach and tested across multiple ViT models (DINO, DINOv2, OpenCLIP, DeiT3), extended to smaller models, and analyzed terminology inconsistencies.

Result: Confirmed some key claims but found limited generalizability across all models, successfully extended findings to smaller models, and identified terminology issues affecting broader application.

Conclusion: While register tokens effectively address attention artifacts in some ViTs, their benefits are model-dependent, and terminology clarification is needed for consistent application across diverse architectures.
...
TL;DR: Geo^2 is a unified framework that leverages 3D geometric priors from foundation models to jointly perform cross-view geo-localization and bidirectional image synthesis between ground and aerial views.


<details>
  <summary>Details</summary>
Motivation: While geometric foundation models show strong 3D feature extraction capabilities, their potential for cross-view geo-spatial tasks (localization and synthesis) remains underexplored due to the large viewpoint gap between ground and aerial imagery.

Method: Proposes GeoMap to embed ground/aerial features into shared 3D-aware latent space for localization, and GeoFlow (flow-matching model) conditioned on geometry-aware embeddings for bidirectional synthesis, with consistency loss for alignment.

Result: Achieves state-of-the-art performance on CVUSA, CVACT, and VIGOR benchmarks for both cross-view geo-localization and bidirectional image synthesis tasks.

Conclusion: Demonstrates the effectiveness of 3D geometric priors for unified cross-view geo-spatial learning, successfully bridging the viewpoint gap between ground and aerial imagery.
...
TL;DR: ViGoR is a new benchmark that exposes reasoning deficits in AI vision models by evaluating both intermediate processes and final outputs across image and video tasks, revealing that even state-of-the-art models struggle with logical reasoning.


<details>
  <summary>Details</summary>
Motivation: Current AI vision models create a "performance mirage" - they appear impressive visually but fail at tasks requiring physical, causal, or complex spatial reasoning. Existing evaluations use superficial metrics and fragmented benchmarks that don't assess the generative reasoning process.

Method: ViGoR introduces four innovations: 1) holistic cross-modal coverage (Image-to-Image and Video tasks), 2) dual-track evaluation of both intermediate processes and final results, 3) evidence-grounded automated judge with high human alignment, and 4) granular diagnostic analysis decomposing performance into fine-grained cognitive dimensions.

Result: Experiments on over 20 leading models show that even state-of-the-art systems have significant reasoning deficits, demonstrating ViGoR's effectiveness as a "stress test" for vision models.

Conclusion: ViGoR successfully dismantles the "performance mirage" by providing a unified framework that reveals the "logical desert" in current AI vision models, establishing itself as a critical benchmark for evaluating reasoning capabilities in next-generation intelligent vision systems.
...
TL;DR: Fast 3-second feed-forward SDF reconstruction from images without camera calibration, using pretrained geometry transformer features directly for volumetric extraction.


<details>
  <summary>Details</summary>
Motivation: Existing methods discard valuable completeness information by routing features through per-view prediction heads before assembling 3D geometry, which accumulates inaccuracies and loses joint world representation encoded in pretrained geometry transformers.

Method: Direct 3D extraction from geometry transformer features via learned volumetric extraction using voxelized canonical embeddings that absorb multi-view geometry through cross- and self-attention into a structured volumetric latent grid, followed by convolutional decoder to dense SDF. Includes scalable validity-aware supervision using SDFs from depth maps or 3D assets.

Result: Achieves complete and well-defined distance values across sparse- and dense-view settings with geometrically plausible completions in less than three seconds without camera calibration or post-hoc fusion.

Conclusion: Direct volumetric extraction from pretrained geometry transformer features enables fast, accurate SDF reconstruction from image collections, outperforming methods that discard valuable joint world representation through per-view processing.
...
TL;DR: GazeQwen adds gaze awareness to MLLMs via parameter-efficient hidden-state modulation, achieving state-of-the-art performance on video understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs cannot effectively utilize eye-gaze information for video understanding, even when gaze cues are provided through visual overlays or text descriptions.

Method: Introduces a compact gaze resampler (1-5M parameters) that encodes V-JEPA 2.1 video features with fixation-derived positional encodings, producing additive residuals injected into selected LLM decoder layers via forward hooks. Optional second stage adds LoRA adapters for tighter integration.

Result: Achieves 63.9% accuracy on StreamGaze benchmark, +16.1 points over Qwen2.5-VL-7B backbone with gaze as visual prompts, and +10.5 points over GPT-4o - highest score among all open-source and proprietary models tested.

Conclusion: Learning where to inject gaze within an LLM is more effective than scaling model size or engineering better prompts. The approach demonstrates parameter-efficient gaze integration can significantly enhance video understanding capabilities.
...
TL;DR: Two-stage hand gesture recognition method combining MediaPipe Hand Landmarker for skeletal keypoint extraction with CNN for spatiotemporal gesture classification, achieving 92-95% accuracy for LIBRAS gestures in home automation.


<details>
  <summary>Details</summary>
Motivation: To develop an effective dynamic hand gesture recognition system for LIBRAS (Brazilian Sign Language) gestures to enable device control in home automation systems, addressing the need for accessible, vision-based interfaces.

Method: Two-model composition: 1) MediaPipe Hand Landmarker extracts 21 skeletal hand keypoints, 2) CNN classifies gestures from 90×21 spatiotemporal matrix representation of keypoints. Uses sliding window with temporal frame triplication for real-time continuous recognition without recurrent networks.

Result: Achieved 95% accuracy under low-light conditions and 92% under normal lighting for 11 classes of static and dynamic LIBRAS gestures. System enables continuous real-time recognition without requiring recurrent neural networks.

Conclusion: The approach is effective for LIBRAS gesture recognition in home automation, though systematic experiments with greater user diversity are needed for thorough evaluation of generalization capabilities.
...
TL;DR: GUIDE is a benchmark for evaluating GUI agents' ability to understand user behavior, infer intent, and provide assistance in open-ended software tasks, revealing current models struggle but benefit significantly from user context.


<details>
  <summary>Details</summary>
Motivation: Current GUI automation focuses on clicks/keystrokes but overlooks human intention - users want to explore, iterate, and refine ideas while maintaining agency. To move from automation to collaboration, agents need to understand what users are doing and why.

Method: Created GUIDE benchmark with 67.5 hours of screen recordings from 120 novice user demonstrations with think-aloud narrations across 10 software applications. Defines three tasks: Behavior State Detection, Intent Prediction, and Help Prediction to test models' ability to recognize behavior, reason about goals, and decide when/how to help.

Result: Evaluated 8 state-of-the-art multimodal models - all struggled with only 44.6% accuracy on behavior state detection and 55.0% on help prediction. However, providing user context significantly improved performance, raising help prediction by up to 50.2 percentage points.

Conclusion: Structured user understanding is critical for effective GUI assistance. Current models have limited ability to perceive user behavior and infer intent, but user context dramatically improves performance, highlighting the need for better user modeling in GUI agents.
...
TL;DR: Proposes Multimodal Coherence Score (MCS) - a model-agnostic metric to evaluate multimodal fusion quality by measuring coherence across four dimensions (identity, spatial, semantic, decision), outperforming task accuracy in sensitivity.


<details>
  <summary>Details</summary>
Motivation: Current multimodal AI evaluation relies on downstream task accuracy, but high accuracy doesn't guarantee coherent multimodal inputs. Models can perform well on tasks like VQA even when their visual and textual inputs contradict each other, indicating a need for better fusion quality assessment.

Method: Introduces MCS that decomposes coherence into four dimensions: identity (object matching), spatial (positional relationships), semantic (conceptual alignment), and decision (task relevance). Uses Nelder-Mead optimization to learn dimension weights. Evaluated on 1,000 Visual Genome images using DETR, CLIP, and ViLT models, validated on 150 COCO images without retraining.

Result: MCS discriminates fusion quality with higher sensitivity than task accuracy alone (Spearman rho = 0.093 vs. 0.071). Perturbation experiments show each dimension responds independently to its specific failure mode with zero cross-talk. Works across three fusion architectures.

Conclusion: MCS provides a lightweight, annotation-free metric that not only detects when multimodal fusion fails but identifies which specific dimension (identity, spatial, semantic, or decision) is problematic, offering more diagnostic power than task accuracy alone.
...
TL;DR: Transformer-based surgical desmoking model with physics-inspired head that jointly predicts smoke-free images and smoke maps, trained on large synthetic and real datasets, achieving SOTA performance and improving downstream vision tasks.


<details>
  <summary>Details</summary>
Motivation: Surgical smoke from electrocautery and vessel-sealing instruments severely degrades endoscopic imaging in minimally invasive and robot-assisted surgery, hindering visual perception and vision-based functionalities.

Method: Transformer-based surgical desmoking model with physics-inspired desmoking head that jointly predicts smoke-free image and corresponding smoke map. Uses synthetic data generation pipeline blending artificial smoke patterns with real endoscopic images (80,000+ samples) and curated largest paired surgical smoke dataset (5,817 da Vinci image pairs).

Result: State-of-the-art performance in image reconstruction on both public benchmark and new dataset. Demonstrated impact on downstream stereo depth estimation and instrument segmentation, highlighting benefits and limitations of digital smoke removal.

Conclusion: The proposed transformer-based desmoking model effectively addresses surgical smoke degradation, with comprehensive datasets enabling robust training and benchmarking, showing promise for improving surgical vision systems while acknowledging current limitations.
...
TL;DR: DiReCT improves video generation physics by disentangling semantic and physical conditioning with macro/micro contrastive learning, boosting physical commonsense scores without extra training time.


<details>
  <summary>Details</summary>
Motivation: Current flow-matching video generators produce high-quality outputs but violate physics because their per-frame reconstruction objectives don't distinguish physically possible from impossible dynamics. Contrastive flow matching helps but suffers from semantic-physics entanglement in text-conditioned settings, where negative sampling creates conflicting gradients.

Method: DiReCT (Disentangled Regularization of Contrastive Trajectories) uses a lightweight post-training framework with two complementary scales: macro-contrastive term draws partition-exclusive negatives from semantically distant regions for global trajectory separation, and micro-contrastive term constructs hard negatives sharing full scene semantics but differing along single LLM-perturbed physical axes (kinematics, forces, materials, interactions, magnitudes). Includes velocity-space distributional regularizer to preserve visual quality.
...
Conclusion: DiReCT effectively addresses semantic-physics entanglement in text-conditioned video generation through disentangled contrastive learning at macro and micro scales, significantly improving physical plausibility while maintaining visual quality and training efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Whiteboard educational videos require precise coordination between drawings and narration, but no existing methods address this multimodal synchronization problem with structured, reproducible drawing representations.

Method: Created dataset of 24 paired Excalidraw demonstrations with narrated audio across 8 STEM domains, fine-tuned Qwen2-VL-7B via LoRA to predict stroke sequences synchronized to speech, using topic-stratified five-fold evaluation.

Result: Timestamp conditioning significantly improves temporal alignment over ablated baselines, and the model generalizes across unseen STEM topics.

Conclusion: The approach shows promise for automated educational content generation, with dataset and code released to support future research, though transferability to real classroom settings needs further discussion.
...
TL;DR: Hybrid Dense SwinV2 combines DenseNet CNN for local features and Swin Transformer V2 for global context to classify cassava leaf diseases with 98.02% accuracy.


<details>
  <summary>Details</summary>
Motivation: Need to accurately classify visually similar cassava leaf diseases in real-world field conditions with challenges like occlusion, noise, and complex backgrounds.

Method: Two-branch framework: DenseNet branch captures high-resolution local features, while customized SwinV2 branch models global contextual dependencies via shifted-window self-attention. Both branches use attention channel-squeeze modules to emphasize disease-related features, then fuse discriminative channels.

Result: Achieved 98.02% classification accuracy and 97.81% F1 score on a dataset of 31,000 cassava leaf images across five disease categories, outperforming established CNN and transformer models.

Conclusion: Hybrid Dense SwinV2 offers robust and practical field-level diagnosis for cassava diseases, effectively handling real-world challenges through complementary local and global feature learning.
...
TL;DR: BSOU AI models are sensitive to acquisition deviations, but automated quality assessment can detect issues and improve downstream task performance through feedback loops.


<details>
  <summary>Details</summary>
Motivation: Blind Sweep Obstetric Ultrasound enables scalable fetal imaging in low-resource settings, but AI reliability depends on sweep quality, and little is known about how protocol deviations affect predictions.

Method: Systematically evaluate BSOU quality impact on three AI tasks, simulate acquisition deviations (reversed direction, probe inversion, incomplete sweeps), develop automated quality-assessment models, and simulate feedback loops for re-acquisition.

Result: AI models are sensitive to acquisition variability, but automated quality assessment can detect perturbations, and feedback loops that re-acquire flagged sweeps improve downstream task performance.

Conclusion: Automated quality assessment is crucial for building reliable, scalable AI-assisted prenatal ultrasound workflows in low-resource environments by addressing acquisition variability.
...
TL;DR: SDRL is a single-stage RL framework for video understanding MLLMs that eliminates SFT by using Summarize->Think->Answer CoT format with self-supervised CVK and DVR mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods for MLLMs in video understanding suffer from thinking drift, weak temporal comprehension, dependency on costly SFT with CoT annotations, fixed reasoning paths limiting generalization, and potential bias.

Method: SDRL uses Summarize->Think->Answer structured CoT format with two self-supervised mechanisms in GRPO objective: CVK (reduces KL divergence among summaries for factual grounding) and DVR (dynamically modulates thinking diversity based on group accuracy).
...
TL;DR: WR-Arena is a new benchmark for world models that evaluates three key simulation capabilities beyond just next-state prediction: action simulation fidelity, long-horizon forecasting, and simulative reasoning/planning.


<details>
  <summary>Details</summary>
Motivation: Existing world model benchmarks focus too narrowly on next-state prediction and visual fidelity, overlooking richer simulation capabilities needed for intelligent behavior. There's a gap in evaluating how well world models can simulate complex, multi-step interactions and support reasoning.

Method: The authors introduce WR-Arena benchmark with three evaluation dimensions: (1) Action Simulation Fidelity - following multi-step instructions and generating counterfactual rollouts, (2) Long-horizon Forecast - sustaining accurate simulations across extended interactions, and (3) Simulative Reasoning and Planning - supporting goal-directed reasoning by comparing alternative futures. They build a task taxonomy and curate diverse datasets to probe these capabilities.

Result: Experiments with state-of-the-art world models reveal a substantial gap between current models and human-level hypothetical reasoning. WR-Arena serves as both a diagnostic tool and guideline for advancing next-generation world models.
...
TL;DR: VLAAD is a collision-aware plug-in module for autonomous driving that uses video-language anomaly detection to predict collisions, improving driving scores by 14% in simulation and outperforming larger models on real-world data.


<details>
  <summary>Details</summary>
Motivation: High infraction rates, particularly collisions, remain the primary bottleneck for end-to-end autonomous driving systems. Current approaches lack sufficient attention to collision-aware representation learning, and existing simulator datasets are limited in multimodality and scenario diversity.

Method: Developed VLAAD (Video-Language-Augmented Anomaly Detector) using Multiple Instance Learning for stable, temporally localized collision prediction. Created CARLA-Collide dataset for training and Real-Collide dataset for real-world evaluation. The module serves as a plug-in for existing E2E driving models.

Result: Integration with TransFuser++ agent achieved 14.12% relative increase in driving score with minimal fine-tuning. In real-world evaluation, VLAAD (0.6B parameters) outperformed a multi-billion-parameter vision-language model by 23.3% improvement in AUC on collision detection and prediction.

Conclusion: VLAAD effectively addresses the collision prediction bottleneck in autonomous driving through multimodal anomaly detection, demonstrating significant performance improvements in both simulated and real-world settings while being computationally efficient compared to larger models.
...
TL;DR: Siamese personalization for eye tracking achieves comparable performance to linear calibration with 10x fewer samples, using polarization inputs to reduce gaze error by 12% and combined with linear calibration for 13% improvement.
...
Conclusion: Siamese personalization is a practical approach enabling accurate eye tracking with significantly reduced calibration burden, especially effective when combined with polarization inputs and linear calibration.
...
TL;DR: Proposes FSIR task and FSIR-BD benchmark for few-shot text-to-image retrieval, focusing on compositional and OOD queries, with novel optimization methods that outperform baselines.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with compositional queries and OOD image-text pairs, while humans excel at learning from minimal examples. Need to bridge this performance gap through few-shot learning for image retrieval.

Method: 1) Introduces FSIR task and FSIR-BD benchmark dataset (38,353 images, 303 queries) with compositional (urban scenes/nature species) and OOD queries. 2) Proposes two novel retrieval optimization methods leveraging single/few-shot reference examples that are compatible with any pre-trained image encoder.
...
Conclusion: FSIR optimization methods help narrow the gap between machine and human-level understanding, particularly for compositional reasoning from limited examples. Further research into FSIR optimization is needed.
...
TL;DR: FairLLaVA is a parameter-efficient fine-tuning method that reduces demographic biases in multimodal LLMs for medical imaging tasks while maintaining overall performance.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs show uneven performance across demographic groups in clinical settings, risking unequal diagnostic narratives and eroding trust in AI-assisted decision-making. Fairness in MLLMs remains underexplored compared to vision-only and language-only models.

Method: FairLLaVA uses parameter-efficient fine-tuning with mutual information minimization between target attributes to regularize model representations to be demographic-invariant. It works as a lightweight plug-in using low-rank adapter fine-tuning, providing an architecture-agnostic approach to fair visual instruction following.

Result: Extensive experiments on chest radiology report generation and dermoscopy VQA benchmarks show FairLLaVA consistently reduces inter-group disparities while improving both equity-scaled clinical performance and natural language generation quality across diverse medical imaging modalities.

Conclusion: FairLLaVA effectively mitigates demographic biases in MLLMs for medical applications through parameter-efficient fine-tuning, addressing fairness concerns in safety-critical clinical settings without compromising overall performance.
...
TL;DR: GeoReFormer is a transformer-based architecture for 3D lane detection that embeds geometric and topological priors directly in the decoder, achieving SOTA 34.5% mAP on OpenLane-V2.


<details>
  <summary>Details</summary>
Motivation: Existing transformer approaches for 3D lane detection inherit decoder designs from compact object detection, but lane segments are continuous polylines in directed graphs. Generic query initialization and unconstrained refinement don't encode this geometric and relational structure.

Method: Proposes GeoReFormer with: 1) data-driven geometric priors for structured query initialization, 2) bounded coordinate-space refinement for stable polyline deformation, and 3) per-query gated topology propagation to selectively integrate relational context.

Result: Achieves state-of-the-art performance with 34.5% mAP on OpenLane-V2 benchmark while improving topology consistency over strong transformer baselines.

Conclusion: Demonstrates the utility of explicit geometric and relational structure encoding in transformer decoders for 3D lane detection and topology reasoning tasks.
...
Method: Repurposes a pretrained text-to-video diffusion model as a single-forward-pass perception model, augmented with learnable tokens for sparse predictions. The model is modulated by text prompts to perform various tasks and trained exclusively on synthetic data.

Result: The model achieves state-of-the-art or comparable performance on various benchmarks despite being trained only on synthetic data. It demonstrates emergent properties like generalization to multiple humans and other object classes (anthropomorphic characters, animals) not seen during training.

Conclusion: THFM represents a significant advancement in unified video perception models, showing that diffusion-based representations enable strong generalization from synthetic data and emergent capabilities beyond the original training scope.
...
TL;DR: LVLMs like GPT-4o, Claude 3.5, and LLaMA 3.2 achieve competitive zero-shot facial age estimation without fine-tuning, but show fairness issues with image quality and demographics.


<details>
  <summary>Details</summary>
Motivation: Traditional age estimation requires extensive labeled data and domain-specific training. This study explores whether general-purpose Large Vision-Language Models can perform accurate zero-shot age estimation without task-specific adaptation.

Method: Zero-shot evaluation of GPT-4o, Claude 3.5 Sonnet, and LLaMA 3.2 Vision on UTKFace and FG-NET datasets using eight metrics (MAE, MSE, RMSE, MAPE, MBE, R², CCC, ±5-year accuracy) without any fine-tuning.

Result: LVLMs deliver competitive performance in zero-shot settings, demonstrating emergent capabilities for biometric age estimation. However, performance disparities exist based on image quality and demographic subgroups.

Conclusion: LVLMs are promising tools for real-world age estimation applications, but challenges remain in prompt sensitivity, interpretability, computational cost, and demographic fairness. The study establishes a reproducible benchmark for future research.
...
TL;DR: 4DRaL: Knowledge distillation framework using LiDAR teacher model to enhance 4D radar place recognition performance in all-weather conditions.


<details>
  <summary>Details</summary>
Motivation: 4D radar offers all-weather place recognition but suffers from noise and sparsity issues. Existing camera/LiDAR methods fail in adverse weather, creating need for robust radar-based solutions.

Method: Three knowledge distillation modules: local image enhancement for sparse radar points, feature distribution distillation for discriminative features, and response distillation for teacher-student consistency. Supports both radar-to-radar and radar-to-LiDAR recognition.

Result: Achieves state-of-the-art performance in both R2R (radar-to-radar) and R2L (radar-to-LiDAR) place recognition tasks under normal and adverse weather conditions.

Conclusion: 4DRaL effectively addresses 4D radar limitations through knowledge distillation from LiDAR models, enabling robust all-weather place recognition for robotics applications.
...
TL;DR: SCOTTI is a shared convolutional transformer that uses foot tactile signals from wireless insoles to simultaneously estimate 3D human pose, classify actions, and predict action progress, outperforming single-task methods.
...
Method: Proposes SCOTTI (Shared COnvolutional Transformer for Tactile Inference) that learns a shared representation to simultaneously address three prediction tasks using foot tactile signals from custom wireless insole sensors. Uses multi-task learning to leverage mutual benefits across tasks.

Result: SCOTTI outperforms existing approaches across all three tasks (3D pose estimation, action classification, and action progress prediction). Introduces a novel dataset with 7 hours of data from 15 participants performing eight different activities.

Conclusion: The unified multi-task learning approach enables improved performance across all three human-robot interaction tasks compared to independent learning. This is the first work to explore action progress prediction using foot tactile signals.
...
TL;DR: UDA framework for automated extraction of Type A Aortic Dissection clinical features without target-domain annotations, enabling cross-institutional deployment.


<details>
  <summary>Details</summary>
Motivation: Current TAAD research focuses on segmentation accuracy but neglects reliable extraction of clinically actionable features. Building comprehensive datasets requires expert annotations that are impractical for most clinical institutions, and models suffer from domain shift during cross-institutional deployment.

Method: Unsupervised domain adaptation (UDA)-driven framework that leverages limited source-domain labels while adapting to unlabeled target-domain data. The framework achieves stable cross-institutional multi-class segmentation and reliable clinical feature extraction without requiring high-cost annotations.

Result: Significant improvement in cross-domain segmentation performance compared to state-of-the-art approaches. Reader study with cardiovascular surgeons confirms that automatically extracted clinical features provide meaningful assistance for preoperative assessment.

Conclusion: The proposed end-to-end segmentation-to-feature pipeline demonstrates practical utility for TAAD preoperative evaluation, enabling reliable clinical feature extraction during cross-institutional deployment without target-domain annotations.
...
<details>
  <summary>Details</summary>
Motivation: LiDAR semantic segmentation requires large-scale annotated data, but real-world annotation is expensive. Synthetic data is available but suffers from domain gap issues when models trained on synthetic data underperform on real-world data.

Method: Proposes DRUM framework using diffusion models pre-trained on unlabeled real-world data as generative priors. Translates synthetic data by reproducing reflectance intensity and raydrop noise characteristics. Introduces raydrop-aware masked guidance to selectively enforce consistency with input synthetic data while preserving realistic raydrop noise.

Result: DRUM consistently improves Sim2Real performance across multiple representations of LiDAR data.

Conclusion: DRUM effectively bridges the Sim2Real domain gap for LiDAR semantic segmentation by leveraging diffusion models to translate synthetic data with realistic measurement characteristics.
...
Method: Proposes a context-infused vision encoder that integrates clinical context (patient history, symptoms, diagnostic intent) and a multi-level supervision paradigm with hybrid-positive contrastive learning, disease-aware cross-modal representation learning, and gaze-guided attention using radiologists' gaze as probabilistic priors.

Result: CoGaze consistently outperforms state-of-the-art methods across diverse tasks: +2.0% CheXbertF1 and +1.2% BLEU2 for report generation, +23.2% AUROC for zero-shot classification, and +12.2% Precision@1 for image-text retrieval.

Conclusion: The integration of clinical context and radiologists' gaze significantly improves medical vision-language pretraining for chest X-rays, enabling better diagnostic reasoning and cross-modal alignment across multiple downstream tasks.
...
TL;DR: End-to-end pipeline for high-fidelity 3D vehicle reconstruction in cluttered dealership environments using specialized camera rig, motion segmentation, learned matching, and distortion-aware 3D Gaussian Splatting.


<details>
  <summary>Details</summary>
Motivation: Online automotive marketplaces need high-fidelity 3D vehicle models to boost buyer confidence, but current methods fail in cluttered dealership drive-throughs with dynamic vehicles, wide-angle distortion, specular paint, and non-rigid wheel rotations.

Method: Four-stage pipeline: 1) SAM 3 instance segmentation with motion-gating to isolate moving vehicle and mask non-rigid wheels, 2) RoMa v2 learned matcher on raw 4K imagery with semantic confidence masks, 3) rig-aware SfM optimization with CAD-derived pose priors, 4) distortion-aware 3D Gaussian Splatting (3DGUT) with MCMC densification for reflective surfaces.

Result: Achieved PSNR of 28.66 dB, SSIM of 0.89, and LPIPS of 0.21 on 25 real-world vehicles across 10 dealerships, representing 3.85 dB improvement over standard 3D-GS, delivering inspection-grade interactive 3D models without studio infrastructure.

Conclusion: The proposed pipeline successfully addresses the challenges of dynamic vehicle reconstruction in cluttered dealership environments, enabling high-quality 3D models for online automotive marketplaces without requiring controlled studio setups.
...
TL;DR: MaLSF is a novel multimodal misinformation detection framework that uses mask-aware local semantic fusion with bidirectional verification to detect subtle inconsistencies between images and text, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current multimodal verification methods using passive holistic fusion struggle with sophisticated misinformation due to 'feature dilution' - global alignments average out subtle local semantic inconsistencies, masking the conflicts they're designed to find.
...
Conclusion: MaLSF successfully addresses the limitations of current multimodal verification methods by shifting to active, bidirectional verification that mimics human cognitive cross-referencing, enabling detection of sophisticated misinformation through local semantic inconsistency analysis.
...
TL;DR: GSR-guided Progressive Distillation (GPD) uses cross-modal knowledge distillation from reliable GSR signals to improve non-contact deception detection, achieving SOTA results on a new multimodal dataset.


<details>
  <summary>Details</summary>
Motivation: Non-contact deception detection is challenging due to unstable visual/auditory cues across subjects, while GSR provides more reliable physiological deception signals. However, there's no suitable dataset combining both contact (GSR) and non-contact modalities for cross-modal learning.

Method: Proposed GSR-guided Progressive Distillation (GPD) framework with progressive feature-level and digit-level distillation using dynamic routing. Introduced MuDD dataset with 130 participants, 690 minutes of recordings including video, audio, GSR, PPG, heart rate, and personality traits.

Result: GPD outperforms existing methods and achieves state-of-the-art performance on both deception detection and concealed-digit identification tasks. Dynamic routing enables adaptive knowledge transfer, mitigating negative transfer from modality mismatch.

Conclusion: Cross-modal knowledge distillation from reliable GSR signals can effectively guide representation learning in non-contact modalities for deception detection, with the proposed progressive distillation framework and new dataset enabling significant performance improvements.
...
TL;DR: R-PGA framework uses relightable 3D Gaussian Splatting and worst-case configuration mining to create robust physical adversarial camouflage for autonomous driving systems.


<details>
  <summary>Details</summary>
Motivation: Current physical adversarial camouflage methods fail in complex dynamic scenarios due to simulation fidelity gaps (coarse CARLA simulations) and optimization robustness issues (rugged loss landscapes vulnerable to configuration shifts).

Method: Proposes R-PGA framework with: 1) Relightable 3DGS for photo-realistic reconstruction with physically disentangled material/lighting attributes, 2) Hybrid rendering pipeline combining precise foreground rendering with image translation for backgrounds, 3) Hard Physical Configuration Mining module to actively mine worst-case configurations and flatten loss landscape.

Result: The framework addresses both simulation fidelity and optimization robustness issues, ensuring consistent adversarial effectiveness across varying geometric and radiometric configurations in dynamic scenarios.

Conclusion: R-PGA bridges critical gaps in physical adversarial camouflage by combining high-fidelity relightable simulation with robust optimization strategies, creating more effective and generalizable attacks against autonomous driving systems.
...
TL;DR: Current subject-driven text-to-image models fail to scale beyond 4 subjects due to identity collapse, despite appearing successful in simpler scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing models excel at single subjects but struggle with multiple interacting subjects, and current evaluation metrics (CLIP) fail to detect identity collapse issues.

Method: Created a stress-test benchmark with 75 prompts across varying subject counts and interaction difficulties, and introduced Subject Collapse Rate (SCR) metric using DINOv2's structural priors to detect attention leakage and homogenization.

Result: Models show catastrophic identity collapse when scaled to 6-10 subjects or complex interactions, with SCR approaching 100% at 10 subjects, revealing semantic shortcuts in global attention routing.

Conclusion: Current models suffer from an "Illusion of Scalability" and need explicit physical disentanglement in future architectures to handle multi-subject composition.
...
TL;DR: LRM-Functa introduces low-rank modulation in Functa-based models for ultrasound videos, creating structured latent spaces that enable interpretable temporal analysis and competitive performance on cardiac tasks.


<details>
  <summary>Details</summary>
Motivation: While Functa-based approaches show strong reconstruction performance for images using latent modulation vectors, their latent spaces lack structure and interpretability, especially for temporal data like ultrasound videos.
...
Result: LRM-Functa outperforms prior methods in unsupervised ED/ES frame detection, compresses videos to rank k=2 without sacrificing ejection fraction prediction performance, and generalizes to lung ultrasound B-line classification.

Conclusion: LRM-Functa provides a compact, interpretable, and generalizable framework for ultrasound video analysis with structured latent spaces that enable direct visualization and novel frame sampling without additional training.
...
TL;DR: SkinGPT-X is a multimodal collaborative multi-agent system with self-evolving memory for dermatological diagnosis, outperforming state-of-the-art LLMs on standard datasets (+9.6% accuracy) and rare disease benchmarks (+9.8% accuracy).


<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with fine-grained multi-class dermatological diagnosis and rare skin diseases due to training data sparsity, while lacking interpretability and traceability needed for clinical reasoning. Existing multi-agent systems focus on VQA/conversational tasks and rely on static knowledge bases that limit real-world clinical adaptability.

Method: SkinGPT-X is a multimodal collaborative multi-agent system integrated with a self-evolving dermatological memory mechanism. It simulates dermatologists' diagnostic workflow and enables continuous memory evolution for transparent, trustworthy diagnostics.

Result: Outperforms 4 SOTA LLMs on 4 public datasets: +9.6% accuracy on DDI31, +13% weighted F1 on Dermnet. Achieves +9.8% accuracy, +7.1% weighted F1, +10% Cohen's Kappa on new rare skin disease dataset (564 samples, 8 rare diseases). Validated on large-scale 498-category dataset for fine-grained classification.

Conclusion: SkinGPT-X provides transparent, trustworthy dermatological diagnostics through collaborative multi-agent architecture with self-evolving memory, demonstrating superior performance on both standard benchmarks and challenging rare disease cases while addressing interpretability needs in clinical settings.
...
TL;DR: BEVMapMatch: A GNSS-free vehicle re-localization framework using multimodal BEV segmentation and cross-attention search for map matching in challenging environments.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of vehicle localization in GNSS-denied and degraded environments, which is critical for safe autonomous vehicle deployment in areas where GPS signals are unreliable or unavailable.

Method: Uses lidar+camera fusion to generate multimodal Bird's Eye View segmentations, employs cross-attention search to retrieve candidate map patches, and performs finer alignment using top candidates. Multiple BEV segmentation frames improve accuracy.

Result: Outperforms existing re-localization methods with Recall@1m of 39.8%, nearly twice as much as the best baseline, demonstrating robust performance in GNSS-challenged and adverse weather conditions.

Conclusion: BEVMapMatch provides accurate global localization without GNSS priors, offering a practical solution for autonomous vehicle deployment in challenging environments where traditional GPS-based localization fails.
...
TL;DR: Object-DINO extracts distributed object-centric information from all layers of self-supervised ViTs to improve object discovery and reduce hallucination in multimodal models.


<details>
  <summary>Details</summary>
Motivation: Current self-supervised ViTs like DINO use [CLS] token attention maps for object discovery, but these maps contain spurious activations and poor localization because the [CLS] token summarizes the entire image rather than focusing on objects, diluting object-centric information in patch-level interactions.

Method: Object-DINO analyzes inter-patch similarity using all three attention components (query, key, value) across all layers, identifies that object-centric information is distributed throughout the network, and clusters attention heads across all layers based on patch similarities to automatically identify object-centric clusters corresponding to all objects.

Result: The method achieves +3.6 to +12.4 CorLoc gains for unsupervised object discovery and effectively mitigates object hallucination in Multimodal Large Language Models by providing visual grounding, demonstrating improved downstream task performance without additional training.

Conclusion: Object-centric information in self-supervised ViTs is distributed across all layers and encoded in all three attention components, and extracting this distributed information through Object-DINO significantly improves object discovery and reduces hallucination in multimodal applications.
...
TL;DR: EEG-guided RL framework uses brain signals to improve autonomous driving collision avoidance by incorporating human cognitive feedback without behavioral interruption.


<details>
  <summary>Details</summary>
Motivation: Training autonomous driving systems to align with human expectations is challenging; current RLHF methods require time-consuming manual preference ranking. Human cognitive insights from EEG could provide more direct, real-time feedback without interrupting behavior.

Method: Collected EEG signals from 20 participants in driving simulator, analyzed ERP responses to sudden environmental changes. Used neural network to predict ERP strength from visual scene information, then integrated cognitive information into RL reward signal.
...
TL;DR: Progressive learning strategy for LA scar segmentation from LGE MRI using 3-stage SwinUNETR framework with anatomy-aware loss, outperforming one-stage approach.


<details>
  <summary>Details</summary>
Motivation: Automatic LA scar segmentation from LGE MRI is challenging due to low contrast, annotation variability, and lack of anatomical constraints, leading to unreliable predictions. LA scar distribution is clinically important for AF severity and recurrence assessment.

Method: 3-stage progressive learning framework: 1) LA cavity pre-learning, 2) dual-task model learning spatial relationships between LA geometry and scar patterns, 3) fine-tuning for precise scar segmentation. Uses anatomy-aware spatially weighted loss incorporating clinical priors to constrain predictions to anatomically plausible LA wall regions.
...
Conclusion: D-RoPE enables robust and transferable dMRI representations across diverse acquisition settings, addressing key challenges in learning from diffusion MRI data with varying protocols.
...
<details>
  <summary>Details</summary>
Motivation: Existing interactive video generation methods struggle with maintaining scene consistency in long videos under dynamic camera control due to limited contextual information. There's a need for better approaches that can handle long-term scene consistency while allowing controllable camera viewpoints.

Method: Proposes MemCam with memory-augmented approach that treats previously generated frames as external memory. Uses context compression module to encode memory frames into compact representations and employs co-visibility-based selection to dynamically retrieve relevant historical frames, reducing computational overhead while enriching contextual information.
...
Conclusion: Memory-augmented approach effectively addresses scene consistency challenges in interactive video generation, enabling better long-term consistency with dynamic camera control through efficient context compression and retrieval mechanisms.
...
TL;DR: JRM is a 3D flow-matching generative model that implicitly aggregates multiple observations of the same object to produce consistent reconstructions without explicit alignment, handling both rigid and non-rigid transformations.


<details>
  <summary>Details</summary>
Motivation: Current object-centric reconstruction methods treat objects independently, discarding valuable repetition signals where the same object appears multiple times in scenes. Explicit matching and rigid alignment approaches are error-prone and can't handle non-rigid changes.

Method: JRM frames object reconstruction as personalized generation where multiple observations share a consistent subject. It uses a 3D flow-matching generative model that implicitly aggregates unaligned observations in latent space, learning consistency without explicit constraints.
...
Conclusion: Implicit aggregation through generative modeling is superior to explicit alignment for leveraging object repetition in 3D reconstruction, offering better robustness and handling of both rigid and non-rigid transformations.
...
<details>
  <summary>Details</summary>
Motivation: While autoregressive VLMs dominate multimodal tasks including GUI grounding, discrete DVLMs have shown strong performance in multimodal reasoning but their potential for GUI grounding remains unexplored. The paper investigates whether DVLMs can serve as a viable alternative to AR models for GUI grounding.

Method: Adapt LLaDA-V for single-turn action and bounding-box prediction by framing GUI grounding as text generation from multimodal input. Propose a hybrid masking schedule combining linear and deterministic masking to better capture hierarchical bounding-box geometry. Evaluate on four datasets spanning web, desktop, and mobile interfaces.

Result: Hybrid masking improves grounding accuracy by up to 6.1 points in Step Success Rate over linear-masked GUI-adapted LLaDA-V. The adapted diffusion model with hybrid masking consistently outperforms linear-masked variants and performs competitively with autoregressive counterparts despite limited pretraining. Expanding training data with diverse GUI domains reduces latency by ~1.3 seconds and improves grounding accuracy by average 20 points across benchmarks.

Conclusion: Discrete DVLMs are a promising modeling framework for GUI grounding and represent an important step toward diffusion-based GUI agents. The hybrid masking approach effectively captures hierarchical bounding-box structure, and scaling training data improves both accuracy and latency.
...
TL;DR: FAST3DIS is an end-to-end feed-forward 3D instance segmentation method that replaces clustering with a query-based Transformer architecture using 3D anchors, achieving competitive accuracy with better scalability and speed.


<details>
  <summary>Details</summary>
Motivation: Current feed-forward 3D reconstruction models rely on disjointed "lift-and-cluster" approaches for instance segmentation, which suffer from poor scalability with multiple views, non-differentiable clustering, and disconnection between representation learning and final segmentation objectives.

Method: Introduces a 3D-anchored, query-based Transformer architecture built on a depth backbone. Uses a learned 3D anchor generator with anchor-sampling cross-attention for view-consistent segmentation. Projects 3D object queries directly into multi-view feature maps for efficient context sampling. Implements dual-level regularization combining multi-view contrastive learning with dynamically scheduled spatial overlap penalty to prevent query collisions and ensure precise boundaries.

Result: Achieves competitive segmentation accuracy on complex indoor 3D datasets while significantly improving memory scalability and inference speed compared to state-of-the-art clustering-based methods.

Conclusion: FAST3DIS provides an effective end-to-end alternative to clustering-based 3D instance segmentation that maintains geometric priors while improving efficiency and scalability through query-based Transformer architecture and learned 3D anchors.
...
TL;DR: ARTA is a mixed-resolution vision transformer that starts with coarse tokens and uses a lightweight allocator to selectively add fine tokens only near semantic boundaries, achieving SOTA segmentation with fewer FLOPs.


<details>
  <summary>Details</summary>
Motivation: Traditional vision transformers use dense high-resolution tokens throughout, which is computationally expensive and inefficient since many homogeneous regions don't need fine-grained processing. There's a need for more efficient dense feature extraction that focuses computation only where needed.

Method: ARTA uses a coarse-to-fine approach: starts with low-resolution tokens, then a lightweight allocator predicts semantic boundary scores and iteratively allocates additional fine tokens only to patches above a threshold. Mixed-resolution attention enables interaction between coarse and fine tokens while concentrating computation on semantically complex areas.

Result: Achieves state-of-the-art on ADE20K (54.6 mIoU) and COCO-Stuff with substantially fewer FLOPs, and competitive performance on Cityscapes at markedly lower compute. ARTA-Base attains strong results in the ~100M-parameter class while using fewer FLOPs and less memory than comparable backbones.

Conclusion: ARTA's mixed-resolution coarse-to-fine approach with targeted token allocation enables efficient dense feature extraction by focusing computation on semantically complex boundary regions while avoiding redundant processing in homogeneous areas, achieving excellent performance with reduced computational cost.
...
TL;DR: NLCE is a training-free framework for localized concept erasure in diffusion models that preserves semantically related neighbor concepts while removing target concepts through spectrally-weighted embedding modulation, attention-guided spatial gating, and spatially-gated hard erasure.


<details>
  <summary>Details</summary>
Motivation: Existing localized concept erasure methods can unintentionally weaken semantically related neighbor concepts when suppressing target concepts, reducing fidelity in fine-grained domains where preserving neighboring concept relationships is crucial.

Method: Three-stage neighbor-aware pipeline: (1) spectrally-weighted embedding modulation to attenuate target concept directions while stabilizing neighbor concepts, (2) attention-guided spatial gate to identify regions with residual concept activation, and (3) spatially-gated hard erasure to eliminate remaining traces only where necessary.
...
TL;DR: GeoGuide is a novel framework for open-vocabulary 3D semantic segmentation that leverages pretrained 3D models with hierarchical geometry-semantic consistency to overcome limitations of 2D distillation methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on distilling knowledge from 2D open-vocabulary models, which restricts intrinsic 3D geometric learning and inherits errors from 2D predictions. There's a need for better integration of 3D geometric information.

Method: Three key modules: 1) Uncertainty-based Superpoint Distillation fuses geometric and semantic features to estimate per-point uncertainty, adaptively weighting 2D features within superpoints; 2) Instance-level Mask Reconstruction leverages geometric priors to enforce semantic consistency within instances; 3) Inter-Instance Relation Consistency aligns geometric and semantic similarity matrices to calibrate cross-instance consistency.
...
Conclusion: GeoGuide effectively addresses limitations of 2D distillation approaches by integrating hierarchical geometry-semantic consistency through pretrained 3D models, achieving better open-vocabulary 3D segmentation performance.
...
TL;DR: PhysVid improves video generation physics by using local physics-aware conditioning chunks with negative prompts to avoid physical law violations.


<details>
  <summary>Details</summary>
Motivation: Current generative video models achieve high visual fidelity but often violate basic physical principles, limiting their reliability in real-world applications. Existing physics injection methods are inadequate: frame-level signals are too domain-specific and short-horizon, while global text prompts are too coarse and noisy, missing fine-grained dynamics.

Method: PhysVid introduces a physics-aware local conditioning scheme operating over temporally contiguous chunks of frames. Each chunk is annotated with physics-grounded descriptions of states, interactions, and constraints, which are fused with the global prompt via chunk-aware cross-attention during training. At inference, negative physics prompts (descriptions of locally relevant law violations) are used to steer generation away from implausible trajectories.

Result: On VideoPhy benchmark, PhysVid improves physical commonsense scores by approximately 33% over baseline video generators, and by up to approximately 8% on VideoPhy2. The approach demonstrates that local, physics-aware guidance substantially increases physical plausibility in generative video.

Conclusion: Local physics-aware conditioning with negative prompts significantly enhances physical plausibility in video generation, representing a step toward physics-grounded video models that better adhere to real-world physical principles.
...
TL;DR: TARA-Merging aligns LoRA modules by addressing subspace coverage and anisotropy issues through preference-weighted cross-entropy pseudo-loss, outperforming baselines on vision and NLI benchmarks.


<details>
  <summary>Details</summary>
Motivation: Naive merging of LoRA modules weakens critical task directions while overemphasizing less important ones due to mismatched subspaces and uneven contributions, reducing model's ability to faithfully represent all tasks.

Method: TARA-Merging (Task-Rank Anisotropy Alignment) aligns merging weights using preference-weighted cross-entropy pseudo-loss while preserving task-relevant LoRA subspaces, ensuring broad subspace coverage and mitigating anisotropy via direction-wise reweighting.
...
TL;DR: NSC Merging is a label-free, output-agnostic method for merging LoRA adapters by analyzing adapter geometry (null-space compression in down-projection matrices), enabling effective merging across heterogeneous tasks including classification, regression, and sequence generation.


<details>
  <summary>Details</summary>
Motivation: Existing model merging approaches work well for homogeneous classification tasks but fail when tasks span both classification and regression. Entropy-based methods don't apply to regression and are computationally expensive for large language models with long token sequences.

Method: Null-Space Compression (NSC) Merging uses the observation that during LoRA fine-tuning, the down-projection matrix A compresses its null space, and this compression correlates with performance. NSC uses this geometric property as an optimization signal to determine merge weights without needing labels or task outputs.

Result: NSC achieves state-of-the-art performance across 20 heterogeneous vision tasks with balanced gains, outperforms baselines on 6 NLI benchmarks, and shows effectiveness on vision-language tasks (VQA and image captioning). It prevents overfitting to subsets of tasks that plagues prior methods.

Conclusion: NSC Merging provides a scalable, effective solution for merging LoRA adapters across diverse task types by leveraging geometric properties of adapter matrices, enabling practical multi-task model merging without joint training.
...
TL;DR: IADA improves VLM fine-tuning by preserving cross-depth access, boosting reasoning by 9.5 points and perception by 3.3 points with only 0.14M extra parameters.
...
Conclusion: Preserved cross-depth access is an important missing factor in VLM fine-tuning, and IADA effectively addresses the reasoning-perception trade-off through efficient cross-depth aggregation.
...
TL;DR: LCT framework uses learnable causal trimming with dynamic feature bank to suppress dataset biases and improve MedVQA generalization


<details>
  <summary>Details</summary>
Motivation: MedVQA models suffer from limited generalization due to reliance on dataset-specific correlations rather than genuine diagnostic evidence, and existing causal approaches are static or post-hoc

Method: Proposes Learnable Causal Trimming (LCT) with Dynamic Anatomical Feature Bank (DAFB) to capture global prototypes, and differentiable trimming module to suppress features correlated with global patterns while emphasizing instance-specific evidence

Result: LCT consistently improves robustness and generalization over existing debiasing strategies on VQA-RAD, SLAKE, SLAKE-CP and PathVQA datasets

Conclusion: The learnable causal trimming framework effectively addresses dataset biases in MedVQA by adaptively prioritizing causal signals over spurious correlations through end-to-end optimization
...
TL;DR: MLLMs progressively drift from image evidence in long-form generation, causing hallucinations. The paper proposes Visual Re-Examination (VRE), a self-evolving training framework that enables MLLMs to autonomously perform visual introspection during reasoning without additional inputs.


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models suffer from progressive drift away from image evidence in long-form generation, leading to ungrounded reasoning and hallucinations. The authors discovered that MLLMs have latent capability for late-stage visual verification that is present but not consistently activated.

Method: Visual Re-Examination (VRE) - a self-evolving training framework that enables MLLMs to autonomously perform visual introspection during reasoning without additional visual inputs. Rather than distilling from a teacher model, VRE promotes iterative self-improvement by leveraging the model itself to generate reflection traces, making visual information actionable through information gain.

Result: Extensive experiments across diverse multimodal benchmarks demonstrate that VRE consistently improves reasoning accuracy and perceptual reliability, while substantially reducing hallucinations, especially in long-chain settings.

Conclusion: VRE effectively addresses the progressive drift problem in MLLMs by activating their latent visual verification capabilities, enabling more grounded reasoning and reduced hallucinations in long-form generation tasks.
...
TL;DR: FSAR-LLaVA: First end-to-end MLLM-based method for few-shot action recognition using multimodal knowledge extraction, adaptive prompt engineering, and training-free multimodal prototype matching.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM approaches for few-shot action recognition rely on suboptimal caption-based pipelines and only use metric learning in visual space, failing to fully leverage multimodal knowledge.

Method: Three key components: 1) Multimodal Feature-Enhanced Module extracts enriched representations from MLLM decoder, 2) Composite Task-Oriented Prototype Construction uses adaptive prompts to bridge train-test distribution gaps, 3) Training-free Multimodal Prototype Matching Metric jointly leverages visual and textual features for decision-making.

Result: Superior performance across various few-shot action recognition tasks with minimal trainable parameters, demonstrating effectiveness of end-to-end MLLM integration.

Conclusion: FSAR-LLaVA successfully leverages MLLMs as multimodal knowledge bases for direct FSAR enhancement, overcoming limitations of previous caption-based approaches and enabling joint multimodal metric learning.
...
<details>
  <summary>Details</summary>
Motivation: Current vision backbone architectures focus on hardware with high parallel processing capabilities, but CPUs have different constraints requiring a design philosophy that balances operation count (MACs) with hardware-efficient execution (high MACs per second).

Method: Investigated two modifications to standard convolutions: grouping convolutions and reducing kernel sizes. These adaptations reduce computational cost while maintaining hardware-efficiency on CPUs. Developed CPUBone family based on these insights.

Result: CPUBone achieves state-of-the-art Speed-Accuracy Trade-offs across diverse CPU devices and effectively transfers efficiency to downstream tasks like object detection and semantic segmentation.

Conclusion: CPUBone demonstrates that vision models can be effectively optimized for CPU inference through careful architectural modifications that balance computational cost with hardware-efficient execution.
...
TL;DR: Proposes Face2Parts, a hierarchical deepfake detection method using coarse-to-fine facial region analysis with channel attention and triplet learning.


<details>
  <summary>Details</summary>
Motivation: Deepfakes pose serious threats to multimedia authenticity in surveillance, biometrics, and evidence gathering. Existing forensic methods have limitations as they focus on specific facial regions, requiring a more comprehensive approach.

Method: Face2Parts uses hierarchical feature representation (HFR) to extract features from frame, face, and key facial regions (lips, eyes, nose). Employs channel-attention mechanism and deep triplet learning to capture inter-dependencies among facial regions.
...
Conclusion: The hierarchical approach effectively generalizes across datasets and manipulation types, demonstrating superior deepfake detection performance by leveraging coarse-to-fine facial region relationships.
...
TL;DR: Empirical study shows GUI screenshot token pruning insights: foreground-background separation reveals background captures interface-state transitions; random pruning preserves spatial structure better; recency effect allows prioritizing recent screenshots.


<details>
  <summary>Details</summary>
Motivation: GUI visual agents using MLLMs face computational challenges with high-resolution screenshots generating many visual tokens, making complete historical information preservation expensive.

Method: Conducted empirical study on token pruning for historical GUI screenshots, using edge-based separation to partition screenshots into foreground/background regions, comparing pruning strategies including random pruning, and analyzing token allocation based on recency.

Result: Three key findings: 1) Background regions capture interface-state transitions despite common assumptions; 2) Random pruning preserves spatial structure better than carefully designed strategies; 3) GUI agents show recency effect - prioritizing recent screenshots reduces cost while maintaining performance.

Conclusion: The study provides practical guidance for designing efficient GUI visual agents through token pruning strategies that balance computational cost and performance, challenging common assumptions about screenshot semantics and pruning effectiveness.
...
TL;DR: LowFormer is a new vision backbone family with streamlined design and Lowtention (lightweight alternative to Multi-Head Self-Attention) that achieves superior efficiency and performance on ImageNet and downstream tasks.


<details>
  <summary>Details</summary>
Motivation: MACs (Multiply Accumulate operations) are insufficient for measuring efficiency, especially on edge devices. The paper aims to identify key factors for efficient execution and optimize backbone design for real-world hardware performance.

Method: Experimentally analyze MAC count vs. execution time of architectural design elements, then design LowFormer with streamlined macro/micro architecture featuring Lowtention (lightweight self-attention alternative). Also create edge GPU optimized version.

Result: LowFormer achieves superior efficiency and ImageNet performance compared to state-of-the-art backbones. It shows remarkable speed-ups across various hardware platforms and works well on downstream tasks like object detection, segmentation, retrieval, and tracking.

Conclusion: LowFormer demonstrates that careful architectural design considering real hardware execution (beyond just MACs) leads to efficient vision backbones with strong performance across diverse tasks and platforms.
...
TL;DR: GVC turns pretrained video generative models into video codecs without retraining by converting deterministic ODEs to SDEs for stochastic compression, enabling three conditioning strategies for flexible bitrate control.


<details>
  <summary>Details</summary>
Motivation: Existing generative video compression methods only use generative models as post-hoc reconstruction modules on top of conventional codecs, rather than leveraging them as the core compression mechanism itself.

Method: Converts deterministic rectified-flow ODE of video foundation models into equivalent SDE at inference time to enable per-step stochastic injection points for codebook-driven compression. Instantiates three conditioning strategies: Image-to-Video (I2V), Text-to-Video (T2V), and First-Last-Frame-to-Video (FLF2V) with GOP chaining.

Result: Achieves high-quality reconstruction below 0.002 bpp while supporting flexible bitrate control through a single hyperparameter, outperforming on standard benchmarks.
...
TL;DR: Sig2GPS transforms cellular signaling data into GPS trajectories using image-to-video generation on maps, achieving state-of-the-art results through fine-tuned video models and reinforcement learning optimization.


<details>
  <summary>Details</summary>
Motivation: Cellular signaling records provide broad coverage for human mobility analysis but only offer coarse location cues, limiting their use in applications requiring high-precision GPS trajectories. There's a need to bridge this gap between coarse signaling data and precise GPS trajectories.

Method: Reframes the problem as image-to-video generation in the map-visual domain: signaling traces are rendered on maps, and a video generation model is trained to draw continuous GPS paths. Uses a paired signaling-to-trajectory video dataset to fine-tune an open-source video model, with trajectory-aware reinforcement learning optimization to improve generation fidelity.
...
Conclusion: Map-visual video generation provides a practical interface for trajectory data mining by enabling direct generation and refinement of continuous paths under map constraints, offering a novel approach to transform coarse cellular signaling into precise GPS trajectories.
...
<details>
  <summary>Details</summary>
Motivation: Current video quality assessment methods underrepresent fluency (motion consistency and frame continuity), limiting their applicability for applications like streaming and gaming where fluency is crucial. Fluency has been overlooked as merely a sub-dimension of overall quality.
...
Result: The work achieves state-of-the-art performance and provides the most comprehensive benchmark for VFA research. FluNet demonstrates superior performance in assessing video fluency compared to existing methods.

Conclusion: This paper pioneers Video Fluency Assessment as a standalone perceptual task, provides essential resources (dataset, benchmark, baseline model), and offers a roadmap for the community to explore VFA solutions, addressing a critical gap in video assessment research.
...
TL;DR: GeoSR is a framework that enhances vision-language models' spatial reasoning by forcing them to actively use geometric information through masking 2D visual shortcuts and adaptive geometry token fusion.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with spatial reasoning despite having access to geometric information from 3D foundation models, as they tend to rely too heavily on 2D visual cues and underutilize geometric tokens.
...
Conclusion: GeoSR successfully makes geometry matter in vision-language models for spatial reasoning by designing mechanisms that encourage active use of geometric information rather than passive token fusion.
...
TL;DR: PerceptionComp is a new benchmark for complex, long-horizon video reasoning requiring multiple temporally separated visual evidence pieces, where both humans and state-of-the-art models struggle significantly.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks don't adequately test complex, perception-centric video reasoning that requires multiple temporally separated pieces of evidence and compositional constraints. There's a need for a benchmark that truly challenges long-horizon perceptual reasoning skills.

Method: Created PerceptionComp with 1,114 manually annotated complex questions on 279 diverse videos from city tours, indoor villa tours, video games, and extreme sports. Questions require multiple perceptual subtasks (objects, attributes, relations, actions, events) and skills (semantic recognition, visual correspondence, temporal/spatial reasoning).

Result: Human accuracy drops to near chance (18.97%) when rewatching is disallowed. Best model (Gemini-3-Flash) achieves only 45.96% accuracy in five-choice setting, while open-source models remain below 40%. Humans take much longer than on prior benchmarks.

Conclusion: Perception-centric long-horizon video reasoning remains a major bottleneck for both humans and AI models. PerceptionComp provides a challenging benchmark to drive progress in perceptual reasoning research.
...
<details>
  <summary>Details</summary>
Motivation: Current hand reconstruction methods produce accurate single-frame estimates but lack physics consistency and don't quantify how confidently the motion satisfies physical laws.

Method: Proposes a physics-aware conditional diffusion framework with MeshCNN-Transformer backbone, formulates Euler-Lagrange dynamics for articulated hands, treats dynamic residuals as virtual observables, and uses last-layer Laplace approximation for variance estimation.

Result: Shows consistent gains over strong image-based initializations and competitive video-based methods on two hand datasets, with variance estimations aligned with physical plausibility.

Conclusion: The framework successfully refines hand motion into physically plausible sequences while providing interpretable variance maps that indicate where physical consistency weakens.
...
TL;DR: MUST is a multimodal survival prediction framework that explicitly decomposes modality representations into modality-specific and cross-modal components, enabling robust predictions even with missing modalities through conditional latent diffusion generation.


<details>
  <summary>Details</summary>
Motivation: Clinical deployment of multimodal survival prediction faces challenges due to frequently incomplete modalities (cost, technical limitations, retrospective data). Existing methods lack explicit modeling of each modality's unique contributions versus information derivable from other modalities.
...
TL;DR: LIAF-KD introduces learnable instance attention filtering for adaptive detector distillation, allowing dynamic instance importance weighting based on student learning state, achieving 2% performance gains on GFL ResNet-50.


<details>
  <summary>Details</summary>
Motivation: Existing feature-based knowledge distillation methods for object detection treat all object instances uniformly and use heuristic or teacher-driven attention filtering, ignoring instance-level variability and student learning state.

Method: Proposes LIAF-KD framework with learnable instance selectors that dynamically evaluate and reweight instance importance during distillation, with student contributing based on its evolving learning state.
...
Conclusion: LIAF-KD effectively addresses limitations of existing KD methods by introducing adaptive instance filtering that considers both instance variability and student learning state, leading to superior distillation performance.
...
TL;DR: CD-Buffer is a novel Test-Time Adaptation framework that adaptively balances subtractive (removing corrupted features) and additive (refining features) strategies based on measured feature-level domain shift severity, achieving state-of-the-art performance across diverse weather conditions and corruption levels.


<details>
  <summary>Details</summary>
Motivation: Current TTA methods have complementary limitations: subtractive approaches work well under severe domain shifts but fail under moderate shifts, while additive methods are effective for moderate shifts but not for severe ones. Neither paradigm generalizes well across diverse corruption levels, creating a need for an adaptive framework that can balance both strategies based on actual shift severity.

Method: CD-Buffer introduces a complementary dual-buffer framework where subtractive and additive mechanisms operate in opposite yet coordinated directions. The key innovation is discrepancy-driven coupling: both strategies are coupled through a unified discrepancy metric that automatically balances removal and refinement based on feature-level shift severity, enabling adaptive channel-wise balancing without manual tuning.

Result: Extensive experiments on KITTI, Cityscapes, and ACDC datasets demonstrate state-of-the-art performance, with the framework consistently achieving superior results across diverse weather conditions and severity levels, outperforming existing TTA methods.

Conclusion: CD-Buffer successfully addresses the limitations of existing TTA paradigms by adaptively balancing subtractive and additive strategies based on measured feature-level domain shift severity, providing a robust solution that generalizes well across diverse corruption levels and domain shifts.
...
TL;DR: Proposes OVCOD-D benchmark for camouflaged object detection in open-vocabulary settings, with methods to reduce noisy text descriptions and improve visual discrimination of camouflaged objects.


<details>
  <summary>Details</summary>
Motivation: Current open-vocabulary object detectors fail on camouflaged objects due to high visual similarity between objects and background, requiring specialized approaches for this challenging scenario.
...
Result: Achieves AP of 56.4 on OVCOD-D benchmark under open-set evaluation setting, demonstrating effectiveness for camouflaged object detection.

Conclusion: The proposed benchmark and methods effectively address the challenge of camouflaged object detection in open-vocabulary settings by reducing text noise and improving visual discrimination capabilities.
...
TL;DR: TGRL uses expert reasoning trajectories to guide MLLMs in better incorporating visual evidence into reasoning chains, improving multimodal reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods for MLLMs focus on answer correctness and visual grounding, but models still fail to effectively incorporate visual evidence into reasoning chains, leading to weakly grounded reasoning.
...
Conclusion: TGRL effectively addresses the bottleneck of weak visual grounding in reasoning chains by leveraging expert trajectories to guide multimodal reasoning processes.
...
TL;DR: TaxaAdapter improves fine-grained species image generation by incorporating Vision Taxonomy Models into diffusion models, achieving better species fidelity while maintaining text control.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image models fail to capture subtle visual traits that define species identity, despite appearing photo-realistic. There are over 10M distinct species with fine-grained differences that current models can't accurately represent.

Method: Proposes TaxaAdapter that injects Vision Taxonomy Model (VTM) embeddings (like BioCLIP) into frozen text-to-image diffusion models. Uses a clean architecture and training recipe to guide fine-grained species generation while preserving text control over attributes like pose, style, and background.

Result: Consistently improves morphology fidelity and species-identity accuracy over strong baselines. Shows strong generalization capabilities for few-shot species and even species unseen during training. Introduces a multimodal LLM-based metric for better evaluation of morphological consistency.

Conclusion: Vision Taxonomy Models are key for scalable, fine-grained species generation. TaxaAdapter demonstrates that incorporating taxonomic knowledge significantly improves species-level fidelity while maintaining flexible text control.
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based VSR methods face two main challenges: strong generative priors cause temporal instability, and multi-frame diffusion pipelines are too computationally expensive for practical deployment.
...
Result: On NVIDIA RTX 4090, processes 30-frame 2K×2K video in under one minute with only 7 GB memory usage, substantially reducing computational cost while maintaining favorable perceptual quality with significantly smoother temporal transitions.

Conclusion: InstaVSR successfully addresses both computational efficiency and temporal stability challenges in diffusion-based video super-resolution, making diffusion methods more practical for real-world video processing applications.
...
TL;DR: Proposed knowledge distillation method for MobileViT backbone achieves significant accuracy improvements (14% for 1-shot, 6.7% for 5-shot) on MiniImageNet while reducing parameters by 69%, FLOPs by 88%, and energy consumption by 37% on edge hardware.
...
Result: Achieved 14% and 6.7% accuracy improvements for 1-shot and 5-shot classification on MiniImageNet vs ResNet12 baseline. Reduced parameters by 69%, computational complexity by 88% (FLOPs). On Jetson Orin Nano: 37% reduction in dynamic energy consumption with 2.6ms latency.

Conclusion: The proposed knowledge distillation method is a promising and practical solution for deploying efficient few-shot learning models on edge AI hardware, balancing accuracy, efficiency, and energy consumption.
...
TL;DR: IP-Bench is the first systematic benchmark for evaluating image protection methods against misuse in image-to-video generation scenarios, assessing robustness against attacks and transferability across models.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of I2V generation models creates potential for malicious content creation, but existing image protection methods lack unified evaluation frameworks and haven't been systematically assessed in I2V scenarios or against preprocessing attacks.

Method: Proposes IP-Bench benchmark that evaluates 6 representative protection methods and 5 state-of-the-art I2V models, systematically assessing robustness with two attack strategies under practical scenarios, and analyzing cross-model & cross-modality transferability.

Result: Establishes a systematic, reproducible, and extensible evaluation framework for image protection methods in I2V generation scenarios, providing comprehensive assessment of existing methods' effectiveness.

Conclusion: IP-Bench addresses the critical need for standardized evaluation of image protection methods against I2V generation misuse, enabling better assessment of real-world deployment effectiveness and facilitating future research in this area.
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion model watermarking methods only support fuzzy matching via threshold-based detection, which cannot recover structured watermark data bit-exactly, making them unsuitable for offline verification or applications requiring lossless metadata like licensing instructions.

Method: Embeds watermarks in the initial Gaussian noise without fine-tuning or quality loss, treats diffusion as noisy channel, identifies two interference types (local bit flips and global stochastic distortions), and uses cascaded defense with error-correcting codes and majority voting for reliable end-to-end transmission.
...
Conclusion: Gaussian Shannon provides a practical solution for exact bit recovery in diffusion model watermarking, addressing limitations of existing threshold-based methods and supporting applications requiring lossless metadata recovery.
...
TL;DR: Provably contractive denoiser networks with Lipschitz control <1 achieve competitive denoising performance while guaranteeing stability against input perturbations, challenging the robustness-accuracy trade-off assumption.
...
TL;DR: CREval is an automated QA-based evaluation pipeline for assessing instruction-based multimodal image manipulation models on complex creative tasks, paired with CREval-Bench benchmark covering 3 categories and 9 creative dimensions with 800+ samples.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods lack systematic and human-aligned frameworks for assessing model performance on complex and creative image editing tasks, with current approaches suffering from incompleteness and poor interpretability of opaque MLLM scoring.

Method: Proposes CREval, a fully automated question-answer based evaluation pipeline that overcomes limitations of MLLM scoring, and CREval-Bench benchmark covering three categories and nine creative dimensions with over 800 editing samples and 13K evaluation queries.

Result: Evaluation of diverse SOTA open/closed-source models shows closed-source models generally outperform open-source ones on complex creative tasks, but all models still struggle with effective completion of such edits. User studies show strong consistency between CREval's automated metrics and human judgments.

Conclusion: CREval provides a reliable foundation for evaluating image editing models on complex creative manipulation tasks, highlighting key challenges and opportunities for future research in instruction-based multimodal image editing.
...
TL;DR: CCL introduces contextual consistency learning with CBDG data generation and CCLoss to improve open-vocabulary object detection robustness across different backgrounds.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary detection methods focus on scaling datasets and contrastive learning but neglect internal consistency within single modalities, leading to performance drops when objects appear in different backgrounds/scenes.

Method: Contextual Consistency Learning (CCL) framework with two key components: Contextual Bootstrapped Data Generation (CBDG) creates images with same objects across diverse backgrounds, and Contextual Consistency Loss (CCLoss) enforces object feature invariance despite environmental changes.

Result: Achieves state-of-the-art performance with +16.3 AP improvement on OmniLabel and +14.9 AP on D3 datasets, demonstrating significant robustness enhancement.

Conclusion: Enforcing intra-modal consistency is crucial for improving model generalization in diverse environments, addressing the robustness gap in open-vocabulary object detection.
...
TL;DR: GLINT introduces a framework for modeling transparency in 3D Gaussian splatting by decomposing radiance contributions from transparent interfaces and transmitted geometry.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian splatting fundamentally fails to model transparency like glass panels, as it cannot decouple radiance contributions from transparent interfaces and transmitted geometry.

Method: GLINT uses explicit decomposed Gaussian representation to reconstruct primary interfaces and model reflected/transmitted radiance separately. It bootstraps transparency localization from geometry-separation cues and uses priors from a pre-trained video relighting model.
...
Conclusion: GLINT successfully addresses the transparency modeling limitation in 3D Gaussian splatting through decomposed representation and priors, enabling better reconstruction of scenes with transparent elements.
...
<details>
  <summary>Details</summary>
Motivation: Existing post-decoding quality enhancement methods for point clouds are designed for static data and process each frame independently, failing to exploit spatiotemporal correlations in point cloud sequences.

Method: 1) Dynamic Geometry Enhancement Network (DGE-Net) using sparse convolution and feature-domain geometry motion compensation; 2) Detail-aware KNN recoloring module for attribute mapping; 3) Dynamic Attribute Enhancement Network (DAE-Net) with temporal feature extraction and attribute motion compensation.

Result: On seven dynamic point clouds, DUGAE achieved: 11.03 dB BD-PSNR gain and 93.95% BD-bitrate reduction for geometry; 4.23 dB BD-PSNR gain and 66.61% BD-bitrate reduction for luma component; improved perceptual quality (PCQM) and outperformed V-PCC.

Conclusion: DUGAE effectively exploits spatiotemporal correlations in dynamic point clouds, significantly enhancing both geometry and attribute quality for G-PCC compressed content, with code to be released publicly.
...
<details>
  <summary>Details</summary>
Motivation: Accurate and temporally consistent left ventricle segmentation from echocardiography videos is essential for cardiac function assessment, but existing linear recurrent models suffer from rank collapse due to unconstrained state updates, causing loss of anatomical details to noise.

Method: Proposes OSA with two key components: 1) Orthogonalized State Update (OSU) mechanism that formulates memory evolution as Euclidean projected gradient descent on the Stiefel manifold to prevent rank collapse, and 2) Anatomical Prior-aware Feature Enhancement module that separates anatomical structures from speckle noise through physics-driven processes.
...
Conclusion: OSA effectively addresses rank collapse in recurrent models for echocardiography segmentation by constraining state evolution on the Stiefel manifold and enhancing anatomical feature representation, providing accurate and temporally consistent results suitable for clinical applications.
...
TL;DR: Proposes a dual-stage invariant continual learning framework for object detection in extreme-sparsity regimes like space-based RSO detection, using joint distillation and sparsity-aware data conditioning to prevent representation drift.


<details>
  <summary>Details</summary>
Motivation: Continual learning in object detection faces challenges in extreme-sparsity regimes where foreground signals are dominated by background observations, causing background-driven gradients to destabilize feature backbones during sequential domain shifts. Existing methods relying solely on output-level distillation fail to preserve intermediate representation stability.

Method: Dual-stage invariant continual learning framework via joint distillation that enforces structural and semantic consistency on both backbone representations and detection predictions. Also introduces sparsity-aware data conditioning combining patch-based sampling and distribution-aware augmentation to regulate gradient statistics under severe imbalance.
...
Conclusion: The proposed framework effectively addresses representation drift in extreme-sparsity continual learning scenarios by suppressing error propagation at its source while maintaining adaptability, demonstrating superior performance in challenging space-based object detection environments.
...
<details>
  <summary>Details</summary>
Motivation: Most lifelong learning research focuses on homogeneous tasks (e.g., only classification), neglecting the realistic scenario of learning across heterogeneous tasks with different output space structures, which is more challenging and practical.

Method: Proposes Heterogeneity-Aware Distillation (HAD), an exemplar-free approach using self-distillation with two components: 1) distribution-balanced heterogeneity-aware distillation loss to address global prediction imbalance, and 2) salience-guided heterogeneity-aware distillation loss focusing on informative edge pixels extracted via Sobel operator.
...
Conclusion: The work formalizes lifelong heterogeneous learning (LHL) as a broader, more realistic setting than conventional lifelong learning, and demonstrates the effectiveness of the proposed HAD method for preserving heterogeneous knowledge across tasks with different output structures.
...
TL;DR: DEFOM-Stereo variants trained for UAV tree pruning achieve varying accuracy-speed trade-offs; DEFOM-PrunePlus offers best deployable balance for real-time branch distance estimation.


<details>
  <summary>Details</summary>
Motivation: Autonomous tree pruning with UAVs requires real-time, accurate distance estimation to thin branches for safe tool operation and collision avoidance.

Method: Train five DEFOM-Stereo variants on synthetic Unreal Engine 5 dataset (5,520 stereo pairs), deploy on NVIDIA Jetson Orin, evaluate accuracy-speed trade-offs for real-time inference.

Result: DEFOM-Stereo ViT-S achieves best accuracy (depth MAE 23.40 cm) but slow inference (~2.2 FPS); DEFOM-PrunePlus offers best deployable balance (~3.3 FPS, depth MAE 64.26 cm); lightweight variants run faster but sacrifice accuracy.

Conclusion: DEFOM-PrunePlus provides optimal accuracy-latency balance for onboard UAV distance estimation, while ViT-S serves as reference for future hardware improvements.
...
TL;DR: Proposes LGE and GDC modules for image-to-point cloud registration to handle repetitive patterns and improve structural consistency, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Current image-to-point cloud registration methods struggle with repetitive patterns where images lack 3D structural cues, leading to incorrect matches. They also overlook structural consistency, limiting full exploitation of correspondences.

Method: Two novel modules: 1) Local Geometry Enhancement (LGE) enhances image and point cloud features with normal vectors to inject geometric structure into image features. 2) Graph Distribution Consistency (GDC) constructs a graph from matched points to update features and explicitly constrain similarity distributions.
...
TL;DR: SALMUBench is a new benchmark for evaluating multimodal unlearning in CLIP-like models, featuring synthetic persona-attribute associations and structured holdout sets to measure precise forgetting and collateral damage.
...
Method: Built a benchmark with 60K synthetic persona-attribute associations and two foundational models: a Compromised model polluted with sensitive data and a Clean model without it. Both trained from scratch on the same 400M-pair retain base, with the Compromised model additionally trained on sensitive set. Introduced structured holdout sets (holdout identity, holdout association) for precise evaluation.

Result: Utility-efficient deletion is feasible, but current methods show distinct failure modes: either failing to forget effectively or over-generalizing by erasing more than intended. The benchmark reveals limitations in existing unlearning approaches.

Conclusion: SALMUBench sets a new standard for comprehensive unlearning evaluation and provides publicly available dataset, models, evaluation scripts, and leaderboards to advance future research in multimodal machine unlearning.
...
TL;DR: BPO is a reference-free method for verifying T2I models by identifying boundary-adjacent prompts that produce unstable outputs on the target model but stable outputs on others.


<details>
  <summary>Details</summary>
Motivation: Third-party platforms may falsely claim to use official T2I models, misleading users and harming model owners' reputations. Existing verification methods require multiple reference models, leading to high computational costs and sensitivity to model selection.

Method: Boundary-aware Prompt Optimization (BPO) explores intrinsic model characteristics by identifying semantic boundaries in embedding space where prompts generate unstable outputs on the target model but remain stable on other models.

Result: Experiments on five T2I models and four baselines show BPO achieves superior verification accuracy compared to existing methods.

Conclusion: BPO provides an effective reference-free approach for T2I model verification by leveraging model-specific boundary behaviors, addressing computational cost and model selection sensitivity issues of previous methods.
...
TL;DR: Attention-driven spatiotemporal video anonymization framework that separates utility and privacy features using structured attention in Vision Transformers, selectively pruning privacy-sensitive tubelets while maintaining action recognition performance.


<details>
  <summary>Details</summary>
Motivation: Large-scale video models improve video understanding but amplify privacy risks by encoding sensitive attributes (facial identity, race, gender). Video anonymization is underexplored compared to image anonymization, despite video models using spatiotemporal motion patterns as biometric identifiers.

Method: Proposes an attention-driven spatiotemporal video anonymization framework based on systematic disentanglement of utility and privacy features. Uses two task-specific classification tokens (action CLS token and privacy CLS token) within a shared Transformer backbone. Contrasts their attention distributions to compute utility-privacy scores for each spatiotemporal tubelet, then keeps top-k tubelets with highest scores (pruning privacy-dominated tubelets while preserving action-critical ones).

Result: Extensive experiments show the approach maintains action recognition performance comparable to models trained on raw videos while substantially reducing privacy leakage.

Conclusion: Attention-driven spatiotemporal pruning offers an effective and principled solution for privacy-preserving video analytics.
...
TL;DR: HINT is a dual-path contextualized network for Composed Image Retrieval that addresses the neglect of contextual information and amplifies similarity differences between matching/non-matching samples.


<details>
  <summary>Details</summary>
Motivation: Existing CIR methods neglect contextual information in discriminating matching samples, with two key challenges: implicit dependencies and lack of differential amplification mechanism.

Method: Proposes HINT (dual-patH composItional coNtextualized neTwork) that performs contextualized encoding and amplifies similarity differences between matching and non-matching samples.
...
Conclusion: HINT effectively addresses contextual information neglect in CIR and improves performance in complex scenarios through contextualized encoding and similarity amplification.
...
TL;DR: DuSCN-FusionNet: Interpretable ADHD classification framework using dual-channel structural covariance networks from sMRI, achieving 80.59% balanced accuracy with biomarker identification via Grad-CAM.


<details>
  <summary>Details</summary>
Motivation: ADHD lacks reliable imaging biomarkers, and existing deep learning approaches are black-box systems that limit clinical trust and interpretability for neurobiological diagnosis.

Method: Proposes DuSCN-FusionNet with dual-channel Structural Covariance Networks (intensity-based and heterogeneity-based) processed through SCN-CNN encoder, plus late-stage fusion of ROI variability features and global statistics. Uses 10-fold cross-validation with 5-seed ensemble.

Result: Achieved 80.59% balanced accuracy, 0.778 AUC, 81.66% precision, 80.59% recall, and 80.27% F1-score on Peking University ADHD-200 dataset. Grad-CAM adaptation provides ROI-level importance scores for biomarker identification.

Conclusion: DuSCN-FusionNet offers interpretable ADHD classification with competitive performance and identifies structurally relevant brain regions as potential biomarkers, addressing clinical trust issues in deep learning approaches.
...
TL;DR: Privacy-by-design framework for video anomaly detection that minimizes personally identifiable information while preserving detection utility through data minimization techniques.


<details>
  <summary>Details</summary>
Motivation: Video anomaly detection systems handle sensitive personal data (facial cues, demographics) that creates GDPR compliance challenges, requiring data minimization to only what's strictly necessary for anomaly detection.

Method: Introduces "Only What's Necessary" framework combining breadth-based and depth-based data minimization mechanisms to suppress PII while preserving anomaly detection cues. Evaluates configurations using VAD and privacy inference models with ranking methods and Pareto analysis.

Result: Identifies sweet spot operating points from non-dominated frontier that minimize personal data exposure with limited degradation in detection performance. Extensive experiments on public datasets demonstrate framework effectiveness.

Conclusion: Proposed privacy-by-design framework successfully addresses GDPR compliance by balancing privacy protection and anomaly detection utility through systematic data minimization.
...
TL;DR: Plot2API for hand-drawn plots: New HDpy-13 dataset and Plot-Adapter method improve API recommendation for non-experts while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing Plot2API systems work well for standard plot images but fail with hand-drawn plots, which are more accessible to non-experts and beginners. There's a domain gap between standard and hand-drawn plots, and current models lack expertise for hand-drawn images.

Method: 1) Introduce HDpy-13 dataset for hand-drawn plots; 2) Propose Plot-Adapter with lightweight CNN blocks for local feature capture and projection matrix sharing to reduce parameters; 3) Train separate adapters per language/domain instead of full models.

Result: Experimental results show effectiveness of HDpy-13 dataset and efficiency of Plot-Adapter in improving API recommendations for hand-drawn plots while reducing parameter growth and computational costs.

Conclusion: The proposed HDpy-13 dataset and Plot-Adapter method successfully address the hand-drawn plot API recommendation problem for non-experts while maintaining computational efficiency through adapter-based multi-domain/multi-language training.
...
TL;DR: MPDiT introduces a hierarchical multi-patch transformer design where early blocks use larger patches for global context and later blocks use smaller patches for local details, reducing computational cost by up to 50% while maintaining good generative performance.


<details>
  <summary>Details</summary>
Motivation: Current Diffusion Transformers (DiTs) use isotropic designs that process the same number of patchified tokens in every block, leading to heavy computational costs during training. The authors aim to create a more efficient architecture that reduces computation while maintaining performance.

Method: Proposes a multi-patch transformer design with hierarchical patch sizes: early blocks operate on larger patches to capture coarse global context, while later blocks use smaller patches to refine local details. Also introduces improved designs for time and class embeddings to accelerate training convergence.

Result: The hierarchical design reduces computational cost by up to 50% in GFLOPs while achieving good generative performance on ImageNet dataset. The improved embedding designs also accelerate training convergence.

Conclusion: MPDiT demonstrates that a hierarchical multi-patch transformer design can significantly reduce computational costs in diffusion models while maintaining strong generative performance, offering a more efficient alternative to isotropic DiT architectures.
...
<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with fine-grained spatial reasoning about complex hand poses, which is critical for applications like robot-assisted surgery, chip manufacturing, and AR/VR human-AI interaction.

Method: Built a large-scale benchmark using 3D hand datasets (FreiHAND, InterHand2.6M, FPHA) with over 1.6M controlled multiple-choice questions probing spatial relationships between hand joints. Evaluated state-of-the-art VLMs (LLaVA, DeepSeek, Qwen-VL) in base and fine-tuned settings using LoRA.

Result: Revealed systematic limitations in current models including hallucinated finger parts, incorrect geometric interpretations, and poor generalization. Demonstrated that 3D-grounded spatial knowledge transfers zero-shot, improving accuracy on novel downstream tasks like hand gesture recognition (+10.33%) and hand-object interaction (+2.63%).

Conclusion: HandVQA exposes critical reasoning gaps in VLMs and provides a validated path to improvement through transfer learning, addressing a fundamental limitation in fine-grained spatial understanding for high-stakes applications.
...
TL;DR: SCORE is a reinforcement learning framework for adaptive video token compression that uses surprise-augmented state representations and achieves 16x speedup while preserving 99.5% performance.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs face prohibitive computational costs and performance degradation from "context rot" due to massive visual token redundancy in videos. Existing compression methods use heuristics or fixed transformations decoupled from task objectives, limiting adaptability.

Method: SCORE learns an adaptive token compression policy via reinforcement learning. It uses a lightweight policy network conditioned on surprise-augmented state representations with inter-frame residuals to capture temporal dynamics. Optimized with group-wise RL using split-advantage estimator, stabilized by two-stage curriculum from static pseudo-videos to real dynamic videos.
...
TL;DR: RAR proposes an iterative Restore-Assess-Repeat framework that integrates Image Quality Assessment and Image Restoration in latent space for efficient handling of unknown/composite degradations.
...
Method: RAR framework operates entirely in latent domain to jointly perform degradation identification, image restoration, and quality verification in an iterative process. The model is fully trainable end-to-end with dynamic adaptation of restoration based on quality assessment.
...
Conclusion: The unified latent-space approach integrating IQA and IR enables efficient, iterative restoration with better generalization to various degradation types while minimizing latency and information loss.
...
TL;DR: Surgical-Hands (SHands) is a large-scale multi-view video dataset for surgical hand-gesture and error recognition to enable AI-based assessment of surgical training, addressing limitations of expert-led evaluation.


<details>
  <summary>Details</summary>
Motivation: Expert-led surgical skill assessment is costly, time-limited, difficult to scale, and confined to institutions with specialists. Automated AI-based assessment offers an alternative but lacks datasets with realistic trainee errors and multi-view variability needed for robust computer vision approaches.

Method: Created SHands dataset capturing linear incision and suturing procedures using five RGB cameras from complementary viewpoints, performed by 52 participants (20 experts, 32 trainees), each completing three standardized trials per procedure. Videos are annotated at frame level with 15 gesture primitives and include validated taxonomy of 8 trainee error types.

Result: Dataset enables both gesture recognition and error detection, with standardized evaluation protocols for single-view, multi-view, and cross-view generalization. Benchmarked state-of-the-art deep learning models on the dataset.

Conclusion: SHands is publicly released to support development of robust and scalable AI systems for surgical training grounded in clinically curated domain knowledge, addressing the gap in datasets needed for automated surgical skill assessment.
...
TL;DR: Automated image-based system using synthetic data and deep learning to objectively assess cervical dystonia head posture, validated against expert clinical ratings.
...
Method: Combines pretrained head-pose estimation for rotational symptoms with deep learning model trained on ~16,000 synthetic avatar images for translational symptoms (lateral shift), validated against 20 clinical experts on 100 real patient and 100 synthetic images.

Result: Strong agreement with expert ratings for rotational symptoms (r=0.91 for torticollis, 0.81 for laterocollis, 0.78 for anteroretrocollis), moderate correlation for lateral shift (r=0.55), and higher accuracy than human raters in controlled avatar tests.
...
TL;DR: Meta-learning framework for human mesh recovery that learns optimization-friendly initializations and uses adaptive test-time refinement with uncertainty-aware updates.
...
Method: Meta-learning strategy simulating test-time optimization during training, selective parameter caching to freeze converged joints, distribution-based adaptive updates sampling from learned distributions, and stochastic approximation for complex loss landscapes.

Result: State-of-the-art performance with MPJPE reduction of 10.3 on 3DPW and 8.0 on Human3.6M compared to baselines. Superior domain adaptation with minimal performance degradation and meaningful uncertainty estimates correlating with actual errors.
...
TL;DR: HyVIC introduces a novel variational hyperspectral image compression architecture with adjustable spatio-spectral components and metric-driven hyperparameter selection, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based HSI compression methods adapt natural image models without properly addressing the unique spatio-spectral redundancies in hyperspectral data, lacking explicit architectural designs to balance spatial and spectral feature learning.

Method: Proposes HyVIC with four main components: adjustable spatio-spectral encoder, spatio-spectral hyperencoder, spatio-spectral hyperdecoder, and adjustable spatio-spectral decoder, plus a metric-driven strategy for hyperparameter selection.
...
Conclusion: The trade-off between spatial and spectral feature learning is crucial for reconstruction fidelity, and the proposed architecture with metric-driven hyperparameter selection effectively leverages hyperspectral data characteristics, providing practical guidelines for future research.
...
TL;DR: Sparse-camera 4D reconstruction framework using Spatio-Temporal Distortion Field to handle inconsistent generative observations, enabling high-quality dynamic scene reconstruction without dense camera arrays.


<details>
  <summary>Details</summary>
Motivation: High-quality 4D reconstruction currently requires dense arrays of synchronized cameras (tens to hundreds), which are costly and limit practical scalability. There's a need for methods that work with sparse, uncalibrated camera setups.
...
Conclusion: The proposed framework enables practical, scalable 4D reconstruction of dynamic scenes using sparse camera setups rather than costly dense arrays, overcoming previous limitations in practical deployment.
...
TL;DR: ClipTTT uses CLIP's image-text alignment to guide test-time adaptation of vision-language models, reducing hallucinations when visual inputs are corrupted.


<details>
  <summary>Details</summary>
Motivation: Vision-language models hallucinate more when visual inputs are corrupted at test time, which acts as additional distribution shifts that amplify hallucination rates in real-world applications.

Method: Propose CLIP-guided Test-Time Training (ClipTTT) that adapts LVLMs under degraded conditions using a single test sample. Leverages pre-trained CLIP's image-text alignment as stable guidance to identify reliable self-supervision targets, enabling rapid adaptation without altering base LVLMs.
...
Conclusion: ClipTTT provides an effective test-time adaptation approach that reduces hallucination in vision-language models when dealing with corrupted visual inputs, using CLIP's alignment capabilities as guidance.
...
TL;DR: AXON is a multi-stage diffusion framework that reconstructs high-fidelity 3D CT volumes directly from real 2D chest X-rays, using coarse-to-fine synthesis with Brownian Bridge diffusion and ControlNet refinement, plus super-resolution for diagnostic-grade output.


<details>
  <summary>Details</summary>
Motivation: CT scans provide rich 3D anatomical details but have limitations: high radiation exposure, substantial costs, and limited availability. Standard chest X-rays are cost-effective and widely accessible but only provide 2D projections with limited pathological information. Reconstructing 3D CT from 2D X-rays could increase diagnostic accessibility, but existing methods rely on synthetic X-ray projections, limiting clinical generalization.

Method: AXON employs a multi-stage diffusion-based framework with coarse-to-fine strategy: 1) Brownian Bridge diffusion model-based initial stage for global structural synthesis, 2) ControlNet-based refinement stage for local intensity optimization, 3) supports bi-planar X-ray input to mitigate depth ambiguities, and 4) integrated super-resolution network to upscale volumes to diagnostic-grade resolution.
...
Conclusion: AXON provides a transformative solution for reconstructing high-fidelity 3D CT volumes directly from real X-rays, addressing limitations of existing methods and demonstrating strong performance and clinical generalizability, potentially increasing diagnostic accessibility.
...
<details>
  <summary>Details</summary>
Motivation: Hyperspectral sensing provides rich spectral information for urban driving scene understanding, but its high dimensionality poses challenges for interpretation and efficient learning. There's a need for dimensionality reduction methods that are both effective and physically interpretable.

Method: Learnable Quantum Efficiency (LQE) parameterizes smooth high-order spectral response functions that emulate plausible sensor quantum efficiency curves. It enforces physically motivated constraints including a single dominant peak, smooth responses, and bounded bandwidth, creating a compact spectral representation that's fully differentiable and end-to-end trainable within semantic segmentation models.

Result: LQE achieves the highest average mIoU across three hyperspectral urban driving datasets, improving over conventional methods by 2.45%, 0.45%, and 1.04%, and over learnable methods by 1.18%, 1.56%, and 0.81% on HyKo, HSI-Drive, and Hyperspectral City respectively. It maintains strong parameter efficiency (12-36 parameters vs 51-22K for competing approaches) with competitive inference latency.

Conclusion: Physics-informed spectral learning with LQE improves both performance and interpretability for hyperspectral urban driving perception, providing a principled bridge between hyperspectral perception and data-driven multispectral sensor design for automotive vision systems.
...
TL;DR: OVI-MAP: Real-time incremental 3D instance-semantic mapping using decoupled instance reconstruction and semantic inference with vision-language models for open-vocabulary labeling.


<details>
  <summary>Details</summary>
Motivation: Need for robust incremental open-vocabulary 3D instance-semantic mapping in complex everyday environments, addressing challenges of instance segmentation, real-time processing, and open-set reasoning beyond closed-set assumptions.

Method: Decouples instance reconstruction from semantic inference: builds class-agnostic 3D instance map incrementally from RGB-D input, extracts semantic features only from automatically selected views using vision-language models for zero-shot labeling.
...
Conclusion: OVI-MAP enables stable instance tracking and zero-shot semantic labeling throughout online exploration by decoupling instance and semantic processing, overcoming limitations of existing methods.
...
TL;DR: AutoWeather4D is a feed-forward 3D-aware weather editing framework that decouples geometry and illumination for realistic weather synthesis in autonomous driving scenes without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Current generative video models require massive datasets for rare weather scenarios, while 3D-aware editing methods suffer from costly per-scene optimization and geometric/illumination entanglement.
...
Conclusion: AutoWeather4D provides an efficient, feed-forward solution for weather editing that overcomes data constraints and optimization bottlenecks of existing methods while maintaining photorealism and control.
...
TL;DR: A contrastive flow matching model for co-speech gesture generation that uses negative audio-text examples to improve semantic grounding and cross-modal consistency, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing gesture generation methods have limitations: they rely on external semantic retrieval with predefined rules, produce rhythmic rather than sparse iconic/metaphoric gestures, and fail to maintain cross-modal consistency by modeling body parts in isolation.

Method: Contrastive Flow Matching model that uses mismatched audio-text conditions as negative examples to train the velocity field to follow correct motion trajectories while repelling semantically incongruent ones. Embeds text, audio, and holistic motion into a composite latent space using cosine and contrastive objectives.

Result: Outperforms state-of-the-art methods on BEAT2 and SHOW datasets through extensive experiments and user studies, demonstrating improved semantic grounding and cross-modal coherence.

Conclusion: The proposed contrastive approach effectively addresses limitations of existing methods by learning from negative examples and ensuring holistic cross-modal consistency, advancing the field of semantically grounded co-speech gesture generation.
...
TL;DR: A framework that aligns partial 3D reconstructions from in-the-wild imagery to complete reference models from Google Earth Studio using semantic features and 3D Gaussian Splatting, enabling global consistency even without visual overlap.


<details>
  <summary>Details</summary>
Motivation: Existing 3D reconstruction pipelines struggle with unstructured, in-the-wild imagery that has little or no overlap between views, often producing disconnected partial reconstructions or erroneously merging non-overlapping regions.

Method: Uses Google Earth Studio renderings as complete reference models, represents them with 3D Gaussian Splatting augmented with semantic features, and formulates alignment as an inverse feature-based optimization that estimates global 6DoF pose and scale while keeping the reference fixed.
...
Conclusion: The framework enables globally consistent alignment of partial reconstructions to complete reference models despite significant appearance domain gaps, leveraging shared scene semantics to bridge the visual differences between real-world photos and pseudo-synthetic renderings.
...
TL;DR: MA-Bench is a new benchmark for evaluating Multimodal Large Language Models on micro-action understanding, featuring 1,000 videos with 12,000 QA pairs across three evaluation tiers, revealing current MLLMs struggle with motion granularity and fine-grained body dynamics.


<details>
  <summary>Details</summary>
Motivation: Despite rapid MLLM development, there's no specialized benchmark for micro-action understanding, which is crucial for human emotion analysis. The authors aim to fill this gap by creating a comprehensive evaluation framework.
...
Result: Evaluation of 23 MLLMs shows significant challenges in capturing motion granularity and fine-grained body-part dynamics. Fine-tuning Qwen3-VL-8B on MA-Bench-Train demonstrated clear performance improvements in micro-action reasoning and explanation tasks.

Conclusion: MA-Bench establishes a foundational benchmark for advancing MLLMs in understanding subtle micro-actions and human-related behaviors, addressing current limitations in motion granularity and body dynamics comprehension.
...
TL;DR: ToothCraft is a diffusion-based 3D model that generates complete tooth crowns from incomplete teeth using synthetic training data, achieving high reconstruction accuracy and practical applicability to real dental cases.


<details>
  <summary>Details</summary>
Motivation: There's a need for automated tooth crown completion in dentistry, but limited training data exists for incomplete tooth geometries. The paper addresses this gap by creating synthetic training data to enable robust learning for dental restoration tasks.

Method: Developed a diffusion-based 3D shape generation model trained on artificially created incomplete teeth. Created an augmentation pipeline to generate incomplete tooth geometries from complete dental arch datasets (3DS, ODD), enabling diverse training examples across various tooth defects.

Result: Achieved 81.8% IoU and 0.00034 Chamfer Distance on synthetically damaged testing restorations. The model effectively fills incomplete teeth in real-world cases with minimal intersection with opposing dentition, reducing occlusal interference risk.

Conclusion: ToothCraft demonstrates strong capability for automated tooth crown completion using diffusion models and synthetic training data, with practical applications in dental restoration and potential for clinical use.
...
TL;DR: VLMs approach human-level performance on general scene knowledge but show a fundamental deficit in affordance understanding that can't be fixed by prompt engineering or newer models, suggesting embodied experience is needed beyond distributional learning.


<details>
  <summary>Details</summary>
Motivation: To test the distributional hypothesis - whether statistical co-occurrence of language and images alone is sufficient to learn the full richness of human scene understanding, particularly comparing VLMs (which lack embodied experience) to human observers.

Method: Two experiments comparing 18 VLMs to 2000+ human observers across 15 high-level scene understanding tasks. Developed Human-Calibrated Cosine Distance (HCD) metric to measure VLM similarity to human response distributions. Tested six mechanistic hypotheses for affordance deficits and conducted corpus analyses of image captioning datasets.

Result: VLMs approached human-level performance on general knowledge tasks but showed robust deficits for affordance tasks that resisted prompt engineering and didn't improve with newer models. The deficit was structural rather than stylistic and wasn't resolved by providing explicit spatial information. Corpus analyses showed sparse agent-addressed affordance language in training data.

Conclusion: Distributional learning from images and text alone is insufficient for affordance-based scene understanding. Some dimensions of human visual cognition require agent-centered, three-dimensional embodied experience that photographs and captions cannot encode.
...
<details>
  <summary>Details</summary>
Motivation: Existing video representation learning methods face a dilemma: fine-tuning heavy temporal modules compromises inter-video semantic separability (ability to distinguish objects across videos), while reducing tunable parameters hinders intra-video temporal consistency (stable representations of the same object within a video).

Method: Proposes Co-Settle framework with a lightweight projection layer on top of frozen image-pretrained encoder, optimized with temporal cycle consistency objective and semantic separability constraint. Theoretically analyzes that optimized projection yields better trade-off between the two properties.
...
Conclusion: Co-Settle effectively addresses the consistency-separability trade-off in image-to-video transfer learning, achieving strong performance with minimal training while preserving both temporal consistency and semantic separability.
...
TL;DR: VGGRPO improves geometric consistency in video diffusion models using latent-space geometry guidance and reinforcement learning, eliminating costly VAE decoding while handling both static and dynamic scenes.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models lack geometric consistency. Existing solutions either compromise pretrained model generalization with architectural changes or are limited to static scenes with inefficient RGB-space rewards requiring repeated VAE decoding.

Method: Proposes VGGRPO with Latent Geometry Model (LGM) that connects video diffusion latents to geometry foundation models for direct geometry decoding. Uses latent-space Group Relative Policy Optimization with camera motion smoothness and geometry reprojection consistency rewards.

Result: Improves camera stability, geometry consistency, and overall quality on both static and dynamic benchmarks while eliminating costly VAE decoding overhead.

Conclusion: VGGRPO provides an efficient and flexible latent-space geometry-guided reinforcement learning approach for world-consistent video generation that preserves pretrained model capacity while handling dynamic scenes.
...
TL;DR: EgoPoint-Ground: First large-scale multimodal dataset for egocentric deictic visual grounding using hand-pointing and speech, with SV-CoT framework achieving 11.7% improvement over existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional visual grounding relies only on textual descriptions, struggling with linguistic ambiguity and ignoring non-verbal deictic cues like hand-pointing that are prevalent in real-world egocentric interactions.
...
Result: SV-CoT achieves 11.7% absolute improvement over existing methods, effectively mitigating semantic ambiguity and advancing multimodal intent comprehension. Comprehensive benchmark evaluation of MLLMs and VG architectures established.

Conclusion: The work bridges the gap between textual-only visual grounding and real-world multimodal interactions by introducing hand-pointing as intuitive referring mechanism, with dataset and framework advancing agents' ability to comprehend physical intents.
...
TL;DR: A framework for constructing soft equivariant models by projecting model weights into designed subspaces, enabling control over equivariance degree in pre-trained architectures.


<details>
  <summary>Details</summary>
Motivation: Strict equivariance is rarely satisfied in real-world data, which can limit model performance, so controlling the degree of equivariance is desirable for practical applications.

Method: Propose a general framework for constructing soft equivariant models by projecting model weights into a designed subspace, applicable to any pre-trained architecture with theoretical bounds on equivariance error.

Result: Demonstrated effectiveness on multiple pre-trained backbones (ViT and ResNet) across image classification, semantic segmentation, and human-trajectory prediction tasks, improving performance while reducing equivariance error on ImageNet benchmark.

Conclusion: The proposed framework enables practical control over equivariance in real-world applications, balancing performance benefits with theoretical guarantees on equivariance error.
...
TL;DR: FOSSA is a Transformer-based architecture for zero-shot Depth from Defocus that introduces stack attention with focus distance embedding and is trained on synthetic focus stacks from RGBD datasets, achieving up to 55.7% error reduction.


<details>
  <summary>Details</summary>
Motivation: Previous DfD methods overfit to specific datasets, lacking zero-shot generalization to real-world scenarios. There's a need for better benchmarks and architectures that can generalize without dataset-specific training.

Method: Proposes FOSSA: Transformer-based architecture with stack attention layer and focus distance embedding for efficient cross-stack information exchange. Uses synthetic training data pipeline from existing RGBD datasets to generate focus stacks.

Result: Significant improvement over baselines with up to 55.7% error reduction on ZEDD benchmark. ZEDD contains 8.3x more scenes with higher quality images and depth maps than previous benchmarks.

Conclusion: FOSSA enables effective zero-shot generalization for DfD through novel architecture design and synthetic training pipeline, with ZEDD benchmark providing comprehensive evaluation for future research.
...
TL;DR: GaussianGPT is a transformer-based autoregressive model that directly generates 3D Gaussians via next-token prediction for full 3D scene generation, offering step-by-step construction with flexible control capabilities.


<details>
  <summary>Details</summary>
Motivation: To explore an alternative to diffusion/flow-matching approaches for 3D generative modeling by leveraging autoregressive transformers for more controllable and context-aware 3D scene generation.

Method: Compress Gaussian primitives into discrete latent grid using sparse 3D convolutional autoencoder with vector quantization, then serialize tokens and model with causal transformer using 3D rotary positional embedding for sequential generation.

Result: Enables full 3D scene generation with step-by-step construction, supporting completion, outpainting, controllable sampling via temperature, and flexible generation horizons while maintaining compatibility with neural rendering pipelines.

Conclusion: Autoregressive transformers represent a complementary paradigm to diffusion methods for controllable 3D generation, leveraging compositional inductive biases and scalability while operating on explicit representations.
...
TL;DR: Break viewpoint limitations in sparse camera 3D reconstruction by exploiting object motion during manipulation, using joint pose-shape optimization with 2D Gaussian splatting and factorized appearance modeling.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D reconstruction from sparse fixed cameras is fundamentally limited by viewpoint constraints. The paper aims to overcome this by leveraging opportunistic object motion when people manipulate objects, effectively creating additional virtual viewpoints as static cameras "orbit" the moving object.

Method: Two key innovations: 1) Joint pose and shape optimization using 2D Gaussian splatting with alternating minimization of 6DoF trajectories and primitive parameters, and 2) A novel appearance model that factorizes diffuse and specular components with reflected directional probing within spherical harmonics space.

Result: Extensive experiments on synthetic and real-world datasets with extremely sparse viewpoints show the method recovers significantly more accurate geometry and appearance than state-of-the-art baselines.

Conclusion: Object motion during manipulation can be effectively harnessed to break viewpoint limitations in sparse camera 3D reconstruction, enabling more accurate geometry and appearance recovery through joint optimization and factorized appearance modeling.
...
TL;DR: BeSafe-Bench (BSB) is a comprehensive benchmark for evaluating behavioral safety risks of situated AI agents across four domains (Web, Mobile, Embodied VLM, Embodied VLA), revealing that even top agents complete fewer than 40% of tasks safely.


<details>
  <summary>Details</summary>
Motivation: The rapid deployment of Large Multimodal Models (LMMs) as autonomous decision-makers introduces substantial unintentional behavioral safety risks, but existing evaluations rely on low-fidelity environments, simulated APIs, or narrowly scoped tasks, lacking comprehensive safety benchmarks.

Method: Created BeSafe-Bench (BSB) using functional environments across four domains, constructed diverse instruction space by augmenting tasks with nine categories of safety-critical risks, and adopted hybrid evaluation framework combining rule-based checks with LLM-as-a-judge reasoning to assess real environmental impacts.

Result: Evaluation of 13 popular agents revealed concerning trends: even the best-performing agent completed fewer than 40% of tasks while fully adhering to safety constraints, and strong task performance frequently coincided with severe safety violations.

Conclusion: The findings underscore the urgent need for improved safety alignment before deploying agentic systems in real-world settings, highlighting the critical gap between task performance and safety compliance in current AI agents.
...
TL;DR: AutoB2G is an automated building-grid co-simulation framework that uses natural language descriptions and LLMs to generate and refine simulators for evaluating building-to-grid interactions.
...
Method: Extends CityLearn V2 for B2G interaction and uses LLM-based SOCIA framework to automatically generate, execute, and refine simulators. Creates a codebase organized as a DAG to represent module dependencies, guiding LLMs to retrieve executable paths from natural language descriptions.
...
Conclusion: The framework successfully automates building-grid co-simulation workflows using natural language and LLMs, addressing the gap in grid-level impact evaluation while reducing manual configuration requirements.
...
TL;DR: Dual-stage fusion framework combining symbolic knowledge engineering with LLMs to build domain-grounded Knowledge Graphs for airport operations, ensuring traceability and verifiability.


<details>
  <summary>Details</summary>
Motivation: Airport operations documentation is complex due to technical terminology, regulations, proprietary information, and fragmented communication, creating data silos that impede Total Airport Management initiatives.

Method: Scaffolded fusion strategy where expert-curated knowledge engineering structures guide LLM prompts to discover semantically aligned knowledge triples. Uses probabilistic model for discovery with deterministic algorithm for source anchoring, ensuring traceability.

Result: Document-level processing improves recovery of non-linear procedural dependencies compared to segment-based inference, contrary to prior observations of long-context degradation in LLMs. Framework enables synthesis of complex operational workflows from unstructured text.

Conclusion: The proposed framework bridges the gap between generative "black-box" outputs and operational transparency requirements, providing a traceable, verifiable approach to building airport operations knowledge graphs for Total Airport Management.
...
TL;DR: GUIDE is a training-free framework that reduces GUI agent domain bias by extracting expertise from tutorial videos using automated video analysis and annotation pipelines.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models have strong general GUI capabilities but suffer from domain bias - they lack familiarity with specific application workflows and UI layouts, limiting real-world task performance.

Method: Two key innovations: 1) Subtitle-driven Video-RAG pipeline with progressive three-stage retrieval (domain classification, topic extraction, relevance matching), and 2) Fully automated annotation pipeline using inverse dynamics paradigm with consecutive keyframes and UI element detection to infer planning and grounding knowledge.

Result: Extensive experiments on OSWorld show GUIDE consistently yields over 5% improvements and reduces execution steps for both multi-agent systems and single-model agents without modifying model parameters or architecture.

Conclusion: GUIDE serves as an architecture-agnostic, plug-and-play enhancement that effectively bridges GUI agent domain bias by autonomously acquiring domain-specific expertise from tutorial videos.
...
TL;DR: AIRA$_2$ addresses three bottlenecks in AI research agents with async multi-GPU execution, Hidden Consistent Evaluation, and ReAct agents, achieving state-of-the-art performance on MLE-bench-30.


<details>
  <summary>Details</summary>
Motivation: Existing AI research agents suffer from three structural bottlenecks: (1) synchronous single-GPU execution limiting throughput, (2) generalization gap from validation-based selection causing performance degradation, and (3) limited capability of fixed single-turn LLM operators capping search performance.

Method: AIRA$_2$ introduces three architectural innovations: asynchronous multi-GPU worker pool for linear throughput scaling, Hidden Consistent Evaluation protocol for reliable evaluation signals, and ReAct agents that dynamically scope actions and debug interactively.

Result: On MLE-bench-30, AIRA$_2$ achieves 71.8% mean Percentile Rank at 24 hours (surpassing previous best of 69.9%) and improves to 76.0% at 72 hours. Ablations show each component is necessary and reveal that prior "overfitting" was due to evaluation noise, not data memorization.

Conclusion: AIRA$_2$ successfully addresses key bottlenecks in AI research agents through architectural innovations, achieving state-of-the-art performance and demonstrating that reported overfitting issues stem from evaluation noise rather than true memorization problems.
...
TL;DR: CADSmith is a multi-agent pipeline that generates CadQuery code from natural language with iterative refinement using geometric validation and visual assessment, achieving significant improvements in CAD model quality.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-CAD methods lack geometric verification or rely on lossy visual feedback that cannot resolve dimensional errors, creating a need for more reliable and precise CAD generation systems.

Method: Multi-agent pipeline with nested correction loops: inner loop resolves execution errors, outer loop uses OpenCASCADE kernel measurements (bounding box, volume, solid validity) combined with vision-language model assessment. Uses retrieval-augmented generation over API documentation instead of fine-tuning.

Result: Achieves 100% execution rate (up from 95%), improves median F1 from 0.9707 to 0.9846, median IoU from 0.8085 to 0.9629, reduces mean Chamfer Distance from 28.37 to 0.74 on 100-prompt benchmark across three difficulty tiers.

Conclusion: Closed-loop refinement with programmatic geometric feedback substantially improves quality and reliability of LLM-generated CAD models, demonstrating the effectiveness of combining exact measurements with holistic visual assessment.
...
<details>
  <summary>Details</summary>
Motivation: Existing reward designs have two key limitations: outcome reward models (ORM) only evaluate final-answer correctness, treating all correct responses identically regardless of reasoning quality, and lose signal as groups become uniformly correct. Process reward models (PRM) offer richer supervision but cause reward hacking where models exploit verbosity to inflate scores while accuracy collapses.

Method: PAPO integrates process-level evaluation into GRPO through decoupled advantage normalization. It composes advantage from two components: Aout (derived from ORM and normalized over all responses) and Aproc (derived from rubric-based PRM and normalized exclusively among correct responses). This decoupled design ensures Aout anchors training on correctness while Aproc differentiates reasoning quality without distorting outcome signals.
...
Conclusion: PAPO effectively addresses limitations of both outcome-only and process reward models by combining their strengths through decoupled advantage normalization, enabling better reasoning quality differentiation while maintaining correctness focus.
...
TL;DR: ETA-VLA: Efficient token adaptation framework for Vision-Language-Action models in autonomous driving that reduces computational cost by pruning redundant visual tokens while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models for autonomous driving require processing historical multi-view frames for temporal reasoning, which creates severe computational burden due to quadratic complexity of self-attention in LLMs.
...
TL;DR: Proposed scalable end-to-end DRL framework for Active SLAM enabling massively parallel training, reducing training time and supporting continuous action spaces.


<details>
  <summary>Details</summary>
Motivation: Existing DRL-based Active SLAM approaches lack scalable parallel training despite advances in parallel computing and GPU acceleration, limiting their practical application.

Method: Developed a scalable end-to-end Deep Reinforcement Learning framework that enables massively parallel training for Active SLAM problems.

Result: Significantly reduces training time compared to state-of-the-art, supports continuous action spaces, and facilitates exploration of more realistic scenarios.

Conclusion: The framework addresses scalability limitations in Active SLAM training and is released as open-source to promote reproducibility and community adoption.
...
TL;DR: RL-based humanoid running controller using dynamically optimized human motion retargeting achieves 3.3 m/s speeds on hardware with outdoor obstacle avoidance capabilities.


<details>
  <summary>Details</summary>
Motivation: Current RL controllers for humanoid robots are limited to single motion playback, hindering long-duration autonomous locomotion. There's a need for controllers that can generate dynamic running behaviors while maintaining controllability for real-world deployment.

Method: Pipeline for dynamically retargeting human motions through optimization with hard constraints to generate periodic reference libraries from single human demonstrations. Uses goal-conditioned and control-guided reward structure that tracks dynamically optimized human data.

Result: Achieved running speeds up to 3.3 m/s on Unitree G1 robot, traversed hundreds of meters in real-world environments. Demonstrated controllability through full perception and planning autonomy stack for obstacle avoidance while running outdoors.

Conclusion: Goal-conditioned RL controllers with dynamically optimized human motion references enable high-speed, durable, and controllable humanoid running suitable for autonomous real-world deployment with obstacle avoidance capabilities.
...
TL;DR: ENAP is a neuro-symbolic framework that learns hierarchical policies from visuomotor demonstrations by inferring a state machine for high-level planning and training residual networks for low-level control.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of scaling robot learning to long-horizon tasks, where end-to-end policies lack structural priors for long-term reasoning, and traditional neuro-symbolic methods rely too heavily on hand-crafted symbolic priors.

Method: Uses adaptive clustering and L* algorithm extension to infer a Mealy state machine from visuomotor data as an interpretable high-level planner, then guides a low-level reactive residual network to learn continuous control via behavior cloning.

Result: Outperforms state-of-the-art end-to-end VLA policies by up to 27% in low-data regimes while providing structured representation of robotic intent and achieving high sample efficiency and interpretability.

Conclusion: ENAP successfully bridges the gap between end-to-end and traditional neuro-symbolic approaches by enabling emergent hierarchical policies that combine interpretable discrete structure with continuous control learning from demonstrations.
...
TL;DR: Real-world evaluation of 5 visual navigation models reveals systematic limitations in geometric understanding, goal discrimination in repetitive environments, and robustness to distribution shift.


<details>
  <summary>Details</summary>
Motivation: Existing VNM evaluations rely almost exclusively on success rate, which conceals trajectory quality, collision behavior, and robustness to environmental changes, necessitating more comprehensive real-world evaluation.

Method: Evaluated 5 state-of-the-art VNMs (GNM, ViNT, NoMaD, NaviBridger, CrossFormer) across 2 robot platforms and 5 indoor/outdoor environments using path-based metrics, vision-based goal-recognition scores, and controlled image perturbations (motion blur, sunflare).

Result: Uncovered three systematic limitations: (1) frequent collisions despite sophisticated architectures, indicating limited geometric understanding; (2) failure to discriminate perceptually similar locations with semantic differences, causing goal prediction errors; (3) performance degradation under distribution shift.

Conclusion: Current VNMs have fundamental limitations in geometric understanding and robustness, and the authors will release evaluation codebase and dataset to facilitate reproducible benchmarking.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for instruction-conditioned visual navigation either use reactive policies that struggle with long-horizon planning, or world models that suffer from poor action initialization in high-dimensional spaces. There's a need to combine the strengths of both approaches.

Method: Two-stage framework: 1) Finetune an Octo-based generalist policy with frozen pretrained vision encoder (DINOv2 or V-JEPA-2) on CAST dataset to produce informed action distributions; 2) Use policy-derived distribution to warm-start MPPI planning over a separately trained JEPA world model that predicts future latent states.

Result: PiJEPA significantly outperforms both standalone policy execution and uninformed world model planning, achieving improved goal-reaching accuracy and instruction-following fidelity in real-world navigation tasks.

Conclusion: Combining learned navigation policies with latent world model planning through policy-informed initialization of MPPI sampling leads to faster convergence and better performance in instruction-conditioned visual navigation.
...
TL;DR: SwarmCoDe is a co-evolutionary algorithm that automatically scales swarm heterogeneity to match task complexity, enabling scalable co-design of large heterogeneous robot swarms.


<details>
  <summary>Details</summary>
Motivation: Robot swarms offer robustness and collaborative task execution, but co-design becomes intractable at scale due to exponentially large design spaces. Traditional frameworks can't handle the complexity of designing large-scale heterogeneous swarms.

Method: SwarmCoDe uses a Collaborative Co-Evolutionary Algorithm with dynamic speciation, inspired by biological signaling. It employs evolved genetic tags and selectivity genes for emergent partner identification, plus dominance genes to decouple swarm size from evolutionary population size.

Result: Successfully evolved specialized swarms of up to 200 agents (4x evolutionary population size) by simultaneously optimizing task planning and hardware morphology under fabrication budgets.

Conclusion: The framework provides a scalable, computationally viable pathway for holistic co-design of large-scale, heterogeneous robot swarms, overcoming traditional intractability issues.
...
TL;DR: Distributed multi-robot navigation framework maintains line-of-sight connectivity using real-time LiDAR scans instead of global maps, with topology optimization for efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing multi-robot connectivity maintenance methods require known environment maps, limiting practical deployment in unknown environments with obstacles.

Method: Each robot constructs egocentric visible regions from real-time LiDAR scans, shares them via distributed communication to establish inter-robot LoS constraints, incorporates these into navigation framework with improved LoS-distance metric and topology optimization.

Result: Framework reliably maintains LoS-connectivity in challenging environments with obstacles, even under large visible ranges and minimal topologies where existing methods fail; topology optimization boosts navigation efficiency by ~20%.

Conclusion: Proposed distributed approach enables practical LoS connectivity maintenance in unknown environments without global maps, with topology optimization significantly improving navigation efficiency.
...
TL;DR: DFM-VLA introduces discrete flow matching for Vision-Language-Action models, enabling iterative refinement of action tokens to correct early errors, outperforming existing autoregressive and diffusion approaches.


<details>
  <summary>Details</summary>
Motivation: Existing VLA decoding paradigms (autoregressive or discrete diffusion) have a fundamental limitation: once action tokens are generated, they cannot be revised, so early errors cannot be corrected later in the sequence.

Method: DFM-VLA uses discrete flow matching to model a token-level probability velocity field that dynamically updates the full action sequence across refinement iterations. It employs two velocity field constructions (auxiliary velocity-head and action-embedding-guided) and a two-stage decoding strategy with iterative refinement followed by deterministic validation.

Result: DFM-VLA consistently outperforms strong autoregressive, discrete diffusion, and continuous diffusion baselines on CALVIN (average success length 4.44) and LIBERO (95.7% success rate), while maintaining high inference efficiency.

Conclusion: Discrete flow matching enables effective iterative refinement of action tokens in VLA models, addressing the fundamental limitation of fixed tokens in existing approaches and significantly improving robotic manipulation performance.
...
TL;DR: A unified image-space diffusion policy handles both navigation and manipulation using multi-scale feature modulation with minimal self-supervised data, achieving zero-shot generalization without VLMs or depth sensors.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for motion planning from vision require explicit goal specification, task-specific design, massive computational resources, extensive training data, and fail zero-shot in novel scenes. Vision-language-action models are resource-intensive and don't generalize well.

Method: Uses a unified image-space diffusion policy with three innovations: (1) Multi-scale FiLM conditioning on task mode, depth scale, and spatial attention, (2) trajectory-aligned depth prediction for metric 3D reasoning along waypoints, (3) self-supervised attention from AnyTraverse for goal-directed inference without VLMs or depth sensors.

Result: The model operates purely from RGB input with 2.0 GB memory at 10 Hz, achieves robust zero-shot generalization to novel scenes, and remains suitable for onboard deployment while requiring only 5 minutes of self-supervised data per task.

Conclusion: The proposed framework demonstrates efficient motion planning from vision that handles both navigation and manipulation scales, requires minimal data, generalizes zero-shot, and is computationally efficient enough for real-world deployment.
...
<details>
  <summary>Details</summary>
Motivation: Existing Time Domain Passivity Approach (TDPA) methods are too conservative, degrading transparency and potentially conflicting with actuator physical limitations. Current solutions fail to simultaneously account for individual actuator power limits while maximizing transparency.

Method: Proposes a prioritized dissipation action method that prioritizes an optimal dissipation direction to minimize actuator load, with excess dissipation allocated to the orthogonal hyperplane. Provides closed-form formulation robust for multi-DoF scenarios with actuator and motion anisotropies.

Result: Method is experimentally validated using a parallel haptic interface interacting with virtual environments under different operating conditions.

Conclusion: The proposed solution addresses limitations of existing TDPA methods by providing a robust approach that maintains stability while minimizing actuator load and maximizing transparency in haptic systems.
...
TL;DR: Practical techniques for deploying VLA models on real robots to achieve human-like execution speeds through calibration, planning/control optimization, and learning-based speed identification.


<details>
  <summary>Details</summary>
Motivation: Previous work focused on GPU acceleration of VLA neural computation, but left open the practical deployment challenges for real-world robotic tasks where execution speed is critical for both accuracy and dexterity.

Method: A comprehensive stack including calibration techniques, planning and control optimization, and learning-based methods to identify optimal execution speeds for VLA-driven robots.

Result: Achieved impressive real-world execution speeds on par with casual human operation, approaching the hardware limits of their lightweight robotic arm, with unaccelerated videos and inference traces provided.

Conclusion: Demonstrated practical deployment techniques that enable VLA-driven robots to operate at human-like speeds in real-world tasks requiring both accuracy and dexterity, bridging the gap between theoretical acceleration and practical robotic deployment.
...
TL;DR: T-800 is an 800Hz data glove system that captures high-frequency hand motion previously inaccessible due to sampling limitations, enabling detailed study of human dexterity and robotic control.


<details>
  <summary>Details</summary>
Motivation: Existing motion capture systems can't capture sub-second motor adjustments in human dexterity due to trade-offs between temporal resolution and visual occlusion, missing fine-grained hand motion during fast, contact-rich manipulation.

Method: Developed T-800 data glove system with 18 distributed IMUs synchronized at 800Hz using novel broadcast-based synchronization and mechanical stress isolation architecture, plus kinematic retargeting algorithm for robotic hand mapping.

Result: System captures previously lost manipulation details, reveals human dexterity has significant motion energy above 100Hz, and successfully retargets high-fidelity gestures to robotic hand models while respecting kinematic constraints.

Conclusion: T-800 overcomes previous hardware limitations to capture high-frequency hand dynamics, providing rich behavioral data for studying human dexterity and training robust robotic control policies.
...
TL;DR: A geometry-centric approach for task-oriented grasping that uses LLM-based ontology, geometric part identification, and similarity matching for generalization, avoiding semantic visual recognition limitations.


<details>
  <summary>Details</summary>
Motivation: Task-oriented grasping requires precise part identification and grasp planning, but current vision-language models are unstable in part recognition and grasp inference, limiting generalization across diverse objects and tasks.

Method: 1) LLM-based object-part-task ontology for functional part selection; 2) Sampling-based geometric analysis for part identification from point clouds; 3) Similarity matching framework for imitative grasp planning using known objects as references.

Result: High accuracy in functional part selection, identification, and grasp generation validated through real-world experiments, with demonstrated generalization to novel-category objects by extending ontological knowledge.

Conclusion: The geometry-centric strategy effectively overcomes viewpoint sensitivity and semantic recognition limitations, providing adaptable task-oriented grasping for diverse objects and tasks through ontological knowledge extension.
...
TL;DR: MINav enables rapid image-goal navigation policy training in under 2 hours on a consumer laptop without human intervention, using offline RL and unsupervised data collection.


<details>
  <summary>Details</summary>
Motivation: Challenge the assumption that image-goal navigation requires large datasets, extensive pretraining, and heavy computational resources. Aim to lower the barrier for rapid robotic policy prototyping and deployment.

Method: Formulates image-goal navigation as offline goal-conditioned RL, combining unsupervised data collection with hindsight goal relabeling and offline policy learning.

Result: Improves exploration efficiency, outperforms zero-shot navigation baselines in target environments, and scales favorably with dataset size. Successfully deployed in both simulation and real-world settings.

Conclusion: Effective real-world robotic learning can be achieved with high computational efficiency, enabling rapid policy development and deployment on consumer hardware.
...
TL;DR: DTP-Attack is a black-box adversarial attack framework for trajectory prediction systems that uses binary decision outputs without gradients, achieving high success rates in both intention misclassification and accuracy degradation attacks.


<details>
  <summary>Details</summary>
Motivation: Trajectory prediction systems are critical for autonomous vehicle safety but vulnerable to adversarial attacks. Existing methods require white-box access and rigid physical constraints, limiting real-world applicability.

Method: DTP-Attack is a decision-based black-box adversarial attack framework that operates on binary decision outputs without requiring model internals or gradients. It uses a novel boundary walking algorithm that navigates adversarial regions without fixed constraints, maintaining trajectory realism through proximity preservation.

Result: Achieves 41-81% attack success rates for intention misclassification attacks with perturbations below 0.45m, and increases prediction errors by 1.9-4.2× for accuracy degradation. Outperforms existing black-box approaches on nuScenes and Apolloscape datasets across Trajectron++ and Grip++ models.

Conclusion: The method reveals fundamental vulnerabilities in current trajectory prediction systems, highlighting urgent needs for robust defenses in safety-critical autonomous driving applications.
...
TL;DR: BiSAIL enables zero-shot online adaptation of bimanual skills using interactive language feedback through hierarchical reasoning and diffusion modulation.


<details>
  <summary>Details</summary>
Motivation: General-purpose robots need to adapt to evolving task conditions in human environments, but adapting high-dimensional bimanual skills to novel variations at deployment remains challenging.

Method: Uses hierarchical reason-then-modulate paradigm: first infers adaptation objectives from multimodal task variations, then adapts bimanual motions via diffusion modulation to achieve inferred objectives.

Result: Significantly outperforms existing methods in human-in-the-loop adaptability, task generalization, and cross-embodiment scalability across six bimanual tasks and two dual-arm platforms.

Conclusion: Enables development of adaptive bimanual assistants that can be flexibly customized by non-expert users via intuitive verbal corrections.
...
TL;DR: A negative-feedback imitation learning system that learns from suboptimal demonstrations and its own failures, achieving 90% improvement in success rate over positive-only imitation learning for ambiguous tasks.


<details>
  <summary>Details</summary>
Motivation: Real-world robot applications often rely on demonstrations from non-expert users or mixed-quality data, unlike traditional imitation learning which assumes optimal expert demonstrations. There's a need for systems that can learn from suboptimal demonstrations and improve from failures.

Method: A negative-feedback imitation learning system that leverages both successful and failed demonstrations. The system learns from its own failures and suboptimal human demonstrations to solve ambiguous tasks, using a novel negative-feedback scheme.

Result: The system achieved 90% improvement in success rate over positive-only imitation learning for ambiguous tasks, and 50% improvement on a real robot. It also demonstrated higher efficacy, memory efficiency, and time efficiency compared to comparable negative feedback schemes.

Conclusion: The novel negative-feedback imitation learning system effectively leverages suboptimal demonstrations and learns from failures, significantly outperforming traditional positive-only approaches and existing negative feedback methods for ambiguous robotic tasks.
...
TL;DR: The paper introduces Multi-AMR BSRRP for coordinating robot fleets in dense floor storage, proposing a hierarchical heuristic (A* + CP) that dramatically reduces computation time compared to exact methods.


<details>
  <summary>Details</summary>
Motivation: Buffer zones in dense floor storage face challenges from labor shortages and rising costs. Existing approaches focus only on reshuffling fixed items, but real manufacturing needs adaptive handling of arriving unit loads.

Method: Formulated Binary Integer Programming model for benchmarking, then proposed hierarchical heuristic: A* search for task-level sequence planning of unit load placements, and Constraint Programming for multi-robot coordination and scheduling.

Result: Experiments show orders-of-magnitude computation time reductions compared to exact formulation, confirming heuristic's viability as responsive control logic for high-density production environments.

Conclusion: The hierarchical heuristic approach effectively solves the NP-hard Multi-AMR BSRRP problem at industrial scales, providing practical automation solution for space-constrained manufacturing facilities.
...
TL;DR: Transformer-based RL framework with adaptive beam-search planner improves aerial manipulator tracking by anticipating end-effector motion through simulated rollouts, reducing tracking error by 50%.


<details>
  <summary>Details</summary>
Motivation: Drones with manipulators suffer from tight coupling between drone motion and manipulator movement, where even small attitude changes shift the end-effector off its intended path. This makes reliable tracking difficult and prevents direct use of learning-based arm controllers designed for fixed-base robots.

Method: Developed a reinforcement learning framework with transformer-based double deep Q learning (DDQN), featuring an adaptive beam-search planner that performs short-horizon beam search over candidate control sequences using the learned critic as a forward estimator. This software-in-the-loop approach simulates rollouts to anticipate end-effector motion rather than executing actions directly.

Result: On a 3-DoF aerial manipulator, the meta-adaptive planner achieved 10.2% reward increase, reduced mean tracking error from ~6% to 3% (50% reduction), and 29.6% improvement in combined reward-error metric. Maintained 5 cm tracking error despite drone base drifts from external disturbances.

Conclusion: The proposed transformer-based RL framework with adaptive beam-search planning effectively addresses the coupling problem in aerial manipulation, enabling stable end-effector tracking even when the drone base experiences drift or rapid attitude corrections.
...
TL;DR: Partial imitation learning transfers locomotion style to cart loco-manipulation using adversarial motion priors for lower-body imitation.


<details>
  <summary>Details</summary>
Motivation: Loco-manipulation is crucial for legged robots to perform practical mobile manipulation tasks, but learning robust skills is challenging due to the difficulty of maintaining stable locomotion while performing precise manipulation.

Method: First train a robust locomotion policy with extensive domain/terrain randomization, then learn a loco-manipulation policy by imitating only lower-body motions using partial adversarial motion prior.

Result: The learned policy successfully pushes a cart along diverse trajectories in IsaacLab and transfers effectively to MuJoCo, achieving more stable and accurate behaviors compared to baselines.

Conclusion: Partial imitation learning with adversarial motion priors enables effective transfer of locomotion skills to loco-manipulation tasks, demonstrating improved stability and accuracy.
...
TL;DR: Ruka-v2 is an upgraded open-source tendon-driven humanoid hand with added wrist mobility and finger abduction/adduction, significantly improving dexterity and task performance.


<details>
  <summary>Details</summary>
Motivation: Previous Ruka hand lacked essential wrist mobility and finger abduction/adduction degrees of freedom needed to closely imitate human dexterous manipulation behaviors.

Method: Designs Ruka-v2 with decoupled 2-DOF parallel wrist and finger abduction/adduction capabilities, maintains tendon-driven approach and open-source philosophy.

Result: 51.3% reduction in task completion time and 21.2% increase in success rate compared to original Ruka; demonstrated on 13 teleoperation tasks and 3 autonomous learning tasks.

Conclusion: Ruka-v2 significantly advances accessible dexterous robot hardware with improved human-like capabilities, enabling broader research in robot manipulation and learning.
...
TL;DR: VLA-OPD bridges SFT efficiency with RL robustness using expert teacher supervision on self-generated trajectories, formulated via Reverse-KL objective to prevent entropy issues while preserving pre-trained capabilities.


<details>
  <summary>Details</summary>
Motivation: Standard offline SFT suffers from distribution shifts and catastrophic forgetting, while online RL struggles with sparse rewards and poor sample efficiency in post-training Vision-Language-Action models for robotic manipulation.

Method: On-Policy VLA Distillation (VLA-OPD) uses expert teacher to provide dense, token-level supervision on student's self-generated trajectories, formulated via Reverse-KL objective to avoid mode-covering entropy explosion (Forward-KL) or premature entropy collapse (Hard-CE).
...
Conclusion: VLA-OPD successfully bridges the gap between efficient SFT and robust RL for VLA model post-training, providing stable policy learning through bounded mode-seeking objective that filters teacher uncertainty while maintaining action diversity.
