...
TL;DR: Two-stage model of human 3D vision: Experience Module extracts stereo depth relative to fixation, then Inference Module interprets this to estimate 3D scene properties using disparity gradients as natural scene statistics.


<details>
  <summary>Details</summary>
Motivation: To explain the paradox that while stereo vision doesn't provide direct distance information, it still affects our inferences about visual scale, and to understand how humans estimate 3D scene properties from stereo experience.

Method: Proposed two-stage architecture: Experience Module extracts disparity maps simulating human stereo experience, then Inference Module uses a CNN trained to estimate distance from these disparity gradients, exploiting the natural scene statistic that near scenes have vivid disparity gradients while far scenes appear flat.

Result: QualiaNet can successfully recover distance from disparity gradients alone, validating that the Inference Module can exploit disparity gradients as a natural scene statistic for distance estimation.

Conclusion: Human 3D vision uses a two-stage process where stereo experience provides disparity information that the brain interprets using learned natural scene statistics about how disparity gradients correlate with distance, explaining how we infer 3D properties despite lacking direct distance information in stereo experience.
...
TL;DR: HY-World 2.0 is an advanced multi-modal world model framework that generates 3D Gaussian Splatting scenes from diverse inputs (text, images, videos) through a four-stage pipeline, featuring improved panorama generation, trajectory planning, view expansion, and world composition.


<details>
  <summary>Details</summary>
Motivation: To advance 3D world generation beyond the capabilities of HY-World 1.0 by supporting diverse input modalities and producing high-fidelity, navigable 3D scenes comparable to closed-source models like Marble, while maintaining open-source accessibility.

Method: Four-stage pipeline: 1) Panorama Generation (HY-Pano 2.0), 2) Trajectory Planning (WorldNav), 3) World Expansion (WorldStereo 2.0 with consistent memory), 4) World Composition (WorldMirror 2.0 with refined architecture). Includes WorldLens rendering platform for interactive exploration.

Result: Achieves state-of-the-art performance on multiple benchmarks among open-source approaches, delivering results comparable to the closed-source model Marble. All model weights, code, and technical details are released for reproducibility.

Conclusion: HY-World 2.0 represents a significant advancement in 3D world modeling, offering comprehensive multi-modal input support, high-fidelity scene generation, and interactive exploration capabilities while maintaining open-source accessibility for further research.
...
TL;DR: Generating geometrically consistent multi-view 3D scenes from a single freehand sketch using novel dataset, attention adapters, and correspondence supervision.


<details>
  <summary>Details</summary>
Motivation: Freehand sketches are geometrically impoverished but expressive inputs for 3D scene generation. Existing methods require photographs, text, multiple views, or costly optimization, leaving a gap for single-sketch to multi-view generation.

Method: Three key contributions: (1) curated dataset of ~9k sketch-to-multiview samples via automated pipeline, (2) Parallel Camera-Aware Attention Adapters (CA3) injecting geometric biases into video transformer, (3) Sparse Correspondence Supervision Loss (CSL) from Structure-from-Motion reconstructions.

Result: Significantly outperforms state-of-the-art baselines: improves realism (FID) by over 60%, geometric consistency (Corr-Acc) by 23%, with up to 3.7× inference speedup. Generates all views in single denoising process without reference images or per-scene optimization.

Conclusion: First method to generate geometrically consistent multi-view scenes from single freehand sketches, addressing data scarcity, geometric reasoning from distorted input, and cross-view consistency through mutually reinforcing technical innovations.
...
TL;DR: DharmaOCR introduces specialized small language models for OCR that optimize transcription quality, generation stability, and cost, with a new benchmark and DPO-based method to reduce text degeneration.


<details>
  <summary>Details</summary>
Motivation: Current OCR systems often suffer from text degeneration (looping/repetition) which not only reduces quality but also increases computational costs, response times, and reduces throughput. There's a need for models that balance extraction quality with generation stability and cost efficiency.

Method: Developed DharmaOCR Full (7B) and Lite (3B) SSLMs using Supervised Fine-Tuning (SFT) to enforce strict JSON schema structure, combined with Direct Preference Optimization (DPO) that uses degenerate generations as rejected examples to penalize looping behavior. Also created DharmaOCR-Benchmark covering various document types with unified evaluation protocol.

Result: Models achieved state-of-the-art performance: 0.925 and 0.911 extraction quality scores with only 0.40% and 0.20% degeneration rates. DPO reduced degeneration rates up to 87.6% relative while preserving extraction quality. AWQ quantization reduced per-page costs by up to 22% with negligible quality loss.

Conclusion: DharmaOCR demonstrates that addressing text degeneration through DPO with degenerate examples as negative samples significantly improves OCR model stability and production efficiency. The approach enables strong quality-cost trade-offs superior to both proprietary APIs and open-source alternatives.
...
TL;DR: Hybrid pose-driven system detects subtle snatch-and-run robberies in surveillance video using YOLO-based pose estimation and Random Forest classification, achieving real-time edge deployment on Jetson Nano.


<details>
  <summary>Details</summary>
Motivation: Non-violent street robberies (snatch-and-run) are difficult to detect automatically because they are brief, subtle, and often indistinguishable from benign human interactions in unconstrained surveillance footage, requiring specialized detection methods.

Method: Uses YOLO-based pose estimator to extract body keypoints, computes kinematic and interaction features (hand speed, arm extension, proximity, relative motion), trains Random Forest classifier, and applies temporal hysteresis filter to stabilize predictions.

Result: Demonstrates promising generalization across different scenes and camera viewpoints on staged dataset and internet videos, with complete pipeline implemented on NVIDIA Jetson Nano achieving real-time performance.

Conclusion: The hybrid pose-driven approach supports feasibility of proactive, on-device robbery detection for subtle snatch-and-run events in surveillance systems.
...
<details>
  <summary>Details</summary>
Motivation: Standard vulnerability indices are too coarse for rural risk assessment, lacking insight into place-based conditions like housing quality, road access, and land patterns. Existing remote sensing approaches rely on handcrafted features, manual audits, or natural-image-trained models that don't capture satellite-specific semantics.

Method: SatBLIP combines contrastive image-text alignment with bootstrapped captioning tailored to satellite semantics. Uses GPT-4o to generate structured descriptions of satellite tiles (roof type/condition, house size, yard attributes, greenery, road context), then fine-tunes a satellite-adapted BLIP model to generate captions for unseen images. Captions are encoded with CLIP and fused with LLM-derived embeddings via attention for SVI estimation under spatial aggregation.

Result: The framework enables robust SVI predictions and, using SHAP analysis, identifies salient attributes (roof form/condition, street width, vegetation, cars/open space) that consistently drive predictions, allowing interpretable mapping of rural risk environments.

Conclusion: SatBLIP provides a novel satellite-specific vision-language approach for rural context understanding that overcomes limitations of prior methods, offering interpretable feature identification and risk mapping for rural vulnerability assessment.
...
TL;DR: FoodSense is a dataset and benchmark model for predicting multisensory experiences (taste, smell, texture, sound) from food images, bridging cognitive science with multimodal AI.


<details>
  <summary>Details</summary>
Motivation: While humans naturally infer multiple sensory experiences from food images, prior computer vision research has focused only on recognition tasks like meal identification and nutrition estimation. The cross-sensory inference problem remains largely unexplored in AI.

Method: Created FoodSense dataset with 66,842 human annotations across 2,987 food images, collecting numeric ratings and free-text descriptors for four sensory dimensions. Used LLMs to expand annotations into image-grounded reasoning traces, then trained FoodSense-VL vision-language model to predict ratings and generate explanations.

Result: Developed a comprehensive dataset and benchmark model for multisensory prediction from food images. Showed that popular evaluation metrics are insufficient for visual sensory inference tasks.

Conclusion: This work connects cognitive science findings on cross-sensory perception with modern multimodal instruction tuning, establishing a new direction for food AI research beyond traditional recognition tasks.
...
TL;DR: Zero-ablation overstates register importance; replacement controls preserve performance while zeroing causes disproportionate perturbations.


<details>
  <summary>Details</summary>
Motivation: To investigate whether zero-ablation (replacing token activations with zeros) accurately measures the functional importance of registers in vision transformers, or if it overstates their necessity.

Method: Used three replacement controls: mean-substitution, noise-substitution, and cross-image register-shuffling, comparing them to zero-ablation across classification, correspondence, and segmentation tasks in DINOv2+registers and DINOv3 models.

Result: Replacement controls preserved performance within ~1pp of baseline, while zero-ablation caused large drops (-36.6pp classification, -30.9pp segmentation). Per-patch cosine similarity showed replacements genuinely perturb representations, but zeroing causes disproportionately large perturbations.

Conclusion: Zero-ablation overstates dependence on exact register content; performance depends on plausible register-like activations rather than exact image-specific values. Registers buffer dense features from [CLS] dependence and are associated with compressed patch geometry.
...
TL;DR: Proposes interactive crowdsourcing framework for image annotation using visual property constraints to reduce semantic gap and annotator subjectivity in object recognition datasets.


<details>
  <summary>Details</summary>
Motivation: Addresses limitations in object recognition datasets caused by semantic gap problem (complex many-to-many mappings between visual data and linguistic descriptions) and annotator subjectivity bias that affects computer vision performance.

Method: Integrates knowledge representation, NLP, and computer vision techniques. Uses interactive crowdsourcing framework that dynamically asks questions based on predefined object category hierarchy and annotator feedback, guiding annotation by visual properties.

Result: Experiments demonstrate effectiveness of the methodology. Annotator feedback is discussed to optimize crowdsourcing setup.

Conclusion: Proposed image annotation methodology with visual property constraints reduces annotator subjectivity and addresses semantic gap issues in object recognition datasets through interactive crowdsourcing.
...
TL;DR: DAGMaN introduces a noisy teacher in co-distillation framework for attention-guided masking in Swin transformers to reduce information leakage in medical image SSL while preserving attention diversity.


<details>
  <summary>Details</summary>
Motivation: Random masking in SSL is ineffective for medical images due to contextual similarity causing information leakage. Swin transformers lack global CLS tokens needed for advanced masking methods, and attention-guided masking reduces attention head diversity.

Method: Proposed DAGMaN integrates a noisy teacher into co-distillation framework with attention-guided masking for Swin transformers, selectively masking semantically co-occurring discriminative patches while preserving attention head diversity.
...
Conclusion: DAGMaN successfully addresses information leakage in medical image SSL while maintaining attention diversity, enabling effective self-supervised learning with Swin transformers for various medical imaging applications.
...
TL;DR: H2VLR: A heterogeneous hypergraph vision-language reasoning framework for few-shot anomaly detection that models visual-semantic relations through high-order inference.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based few-shot anomaly detection methods rely only on pairwise feature matching, ignoring structural dependencies and global consistency in visual-semantic relations.

Method: Proposes H2VLR framework that reformulates FSAD as high-order inference problem using unified hypergraph to jointly model visual regions and semantic concepts.

Result: Achieves state-of-the-art performance on representative industrial and medical benchmarks, verifying effectiveness and advantages.

Conclusion: The proposed heterogeneous hypergraph vision-language reasoning framework successfully addresses limitations of existing VLM-based FSAD methods and demonstrates superior performance.
...
TL;DR: Omni-modal LLMs suffer from performance paradox where unimodal baselines beat multimodal inference due to static fusion topologies. Proposed Chain of Modality (CoM) dynamically orchestrates input pathways to neutralize structural biases and uses task-aligned cognitive pathways for robust generalization.


<details>
  <summary>Details</summary>
Motivation: Current Omni-modal LLMs show a critical performance paradox where unimodal baselines frequently outperform joint multimodal inference. This perceptual fragility stems from static fusion topologies that create structural pathologies like positional bias in sequential inputs and alignment traps in interleaved formats, which systematically distort attention regardless of task semantics.

Method: Proposed Chain of Modality (CoM) framework that transitions multimodal fusion from passive concatenation to dynamic orchestration. CoM adaptively orchestrates input topologies, switching among parallel, sequential, and interleaved pathways to neutralize structural biases. It bifurcates cognitive execution into two task-aligned pathways: "Direct-Decide" for direct perception and "Reason-Decide" for analytical auditing. Can operate in training-free or data-efficient SFT settings.

Result: CoM achieves robust and consistent generalization across diverse benchmarks, addressing the performance paradox in Omni-modal LLMs by dynamically adapting fusion strategies based on task requirements.

Conclusion: The static fusion topologies in current Omni-modal LLMs create structural pathologies that undermine multimodal performance. Chain of Modality provides an agentic framework for dynamic orchestration of input pathways and task-aligned cognitive execution, enabling robust generalization and resolving the multimodal performance paradox.
...
TL;DR: FreqTrack is a frequency-aware RGB-event tracking framework that uses frequency-domain transformations to better fuse RGB and event data, achieving state-of-the-art performance on COESOT benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing RGB trackers struggle in complex dynamic scenes, and current RGB-event fusion methods fail to fully exploit the temporal response and high-frequency characteristics of event data, which are crucial for robust tracking.

Method: Proposes FreqTrack with two key components: 1) Spectral Enhancement Transformer (SET) layer using multi-head dynamic Fourier filtering to adaptively enhance frequency-domain features, and 2) Wavelet Edge Refinement (WER) module using learnable wavelet transforms to extract multi-scale edge structures from event data.

Result: Achieves highly competitive performance on COESOT and FE108 datasets, with leading precision of 76.6% on the COESOT benchmark, demonstrating superior tracking capability especially in high-speed and low-light scenarios.

Conclusion: Frequency-domain modeling effectively addresses limitations of spatial-domain fusion methods for RGBE tracking, enabling better exploitation of event data's temporal and high-frequency characteristics for robust performance in challenging conditions.
...
TL;DR: Low-cost smartphone-based fluorescence detection system for microorganisms/molecules using 96-well plates and RGB color analysis instead of expensive conventional equipment.


<details>
  <summary>Details</summary>
Motivation: To create an affordable alternative to expensive microplate reading machines (like Perkin Elmer Victor) for fluorescence detection of microorganisms and molecules in samples.

Method: Developed a device compatible with 96-well plates that uses a smartphone camera as optical detector, correlating RGB color values from sample images with fluorescence molar concentration.

Result: Successfully created a low-cost system that eliminates need for expensive components like exciter filters, barrier filters, and photomultipliers found in conventional machines.

Conclusion: Smartphone cameras can effectively serve as fluorescence detectors for biological assays, offering a cost-effective alternative to specialized laboratory equipment.
...
TL;DR: WILD-SAM adapts Segment Anything Model for landslide detection from wrapped InSAR interferograms using parameter-efficient fine-tuning with spectral alignment and frequency-aware prompts.


<details>
  <summary>Details</summary>
Motivation: Direct landslide detection from wrapped InSAR interferograms is challenging due to phase ambiguity and noise, and SAM's direct transfer fails due to spectral domain shift that suppresses essential high-frequency fringes.

Method: Proposes WILD-SAM with two key components: 1) Phase-Aware Mixture-of-Experts (PA-MoE) Adapter in frozen encoder for spectral distribution alignment, and 2) Wavelet-Guided Subband Enhancement (WGSE) strategy for frequency-aware dense prompts using discrete wavelet transforms.

Result: Achieves state-of-the-art performance on ISSLIDE and ISSLIDE+ benchmarks, significantly outperforming existing methods in target completeness and contour fidelity.

Conclusion: WILD-SAM successfully bridges the spectral domain gap for SAM adaptation to wrapped phase data, enabling high-precision landslide detection with improved boundary delineation.
...
TL;DR: Framework for explicit emotion control in 3D head avatars via dual-path modulation without modifying core architectures


<details>
  <summary>Details</summary>
Motivation: Existing 3D head avatar methods implicitly entangle emotion with geometry/appearance; need explicit, independent emotion control that works consistently across different identities

Method: Dual-path modulation: geometry modulation performs emotion-conditioned normalization in parametric space, appearance modulation captures identity-aware emotion cues. Uses time-synchronized multi-identity dataset with transferred emotional dynamics

Result: Preserves reconstruction/reenactment fidelity while enabling controllable emotion transfer, disentangled manipulation, and smooth emotion interpolation across multiple state-of-the-art backbones

Conclusion: Advances expressive and scalable 3D head avatars by treating emotion as first-class control signal that can be manipulated independently and consistently across identities
...
Method: Lifts 2D reference images into multi-view representations, uses dual-path view-consistent conditioning for stable identity guidance, employs quality-aware weighting for noisy inputs, and introduces an Integration-Aware Consistency Module for spatial realism and occlusion handling.
...
Conclusion: The proposed framework effectively addresses key challenges in video object insertion through multi-view object priors and advanced conditioning mechanisms, delivering superior quality and realism compared to previous methods.
...
TL;DR: NTIRE 2026 4× image super-resolution challenge with two tracks (restoration for PSNR, perceptual for visual quality), 194 registrants, 31 valid submissions, benchmark for SR progress.


<details>
  <summary>Details</summary>
Motivation: To advance image super-resolution research by providing a unified benchmark that reflects evolving objectives in the field, addressing both pixel-wise fidelity and visual realism.

Method: Organized a challenge with bicubic downsampled 4× LR inputs, two evaluation tracks: restoration (PSNR-based) and perceptual (perceptual score-based), with standardized datasets and protocols.

Result: 194 participants registered, 31 teams submitted valid solutions, providing comprehensive analysis of current SR methods and establishing benchmark results for future research.

Conclusion: The challenge successfully created a unified benchmark for image super-resolution, offering insights into current progress and future directions while addressing both fidelity and perceptual quality objectives.
...
TL;DR: DVFace is a one-step diffusion framework for video face restoration that uses spatio-temporal dual-codebooks and asymmetric fusion to achieve high-quality, temporally consistent results efficiently.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based video face restoration methods rely heavily on generic diffusion priors and multi-step sampling, which limit facial adaptation and inference efficiency. There's a need for one-step diffusion that can achieve faithful facial recovery with temporal stability.

Method: Proposes DVFace with: 1) spatio-temporal dual-codebook design to extract complementary spatial and temporal facial priors from degraded videos, and 2) asymmetric spatio-temporal fusion module to inject these priors into the diffusion backbone according to their distinct roles.
...
Conclusion: DVFace successfully addresses the challenges of one-step diffusion for video face restoration by effectively leveraging spatio-temporal facial priors, achieving efficient and high-quality results.
...
TL;DR: SEPatch3D accelerates Vision Transformer-based 3D object detectors by dynamically adjusting patch sizes based on scene content, preserving fine details for nearby objects while using larger patches for background areas to reduce computation.


<details>
  <summary>Details</summary>
Motivation: Existing token compression methods for ViT-based 3D object detectors (pruning, merging, patch enlargement) often discard informative background cues, disrupt contextual consistency, and lose fine-grained semantics, negatively affecting detection accuracy while trying to reduce inference latency.

Method: Proposes SEPatch3D with three key components: 1) Spatiotemporal-aware Patch Size Selection (SPSS) that dynamically assigns small patches to scenes with nearby objects and large patches to background-dominated scenes; 2) Informative Patch Selection (IPS) that selects informative patches for feature refinement; 3) Cross-Granularity Feature Enhancement (CGFE) that injects fine-grained details into selected coarse patches.
...
Conclusion: SEPatch3D effectively balances computation efficiency and detection accuracy by adaptively adjusting patch granularity based on scene content, overcoming limitations of existing token compression methods for multi-view 3D object detection.
...
TL;DR: AVR is an adaptive visual reasoning framework that reduces overthinking in VRMs by dynamically selecting among three response formats, cutting token usage by 50-90% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Visual reasoning models suffer from overthinking, producing unnecessarily long reasoning chains for tasks that don't require full reasoning processes, due to Reasoning Path Redundancy.

Method: AVR decomposes visual reasoning into three cognitive functions (visual perception, logical reasoning, answer application) and enables dynamic selection among three response formats: Full Format, Perception-Only Format, and Direct Answer, trained with FS-GRPO adaptation of Group Relative Policy Optimization.

Result: AVR reduces token usage by 50-90% while maintaining overall accuracy across multiple vision-language benchmarks, especially effective in perception-intensive tasks.
...
TL;DR: ANL framework uses diffusion model noise prediction to improve deepfake detector generalization, achieving SOTA on diffusion-generated forgeries without inference overhead.


<details>
  <summary>Details</summary>
Motivation: Deepfake detectors struggle to generalize to new synthesis techniques, especially diffusion models that evade GAN-trained detectors. Need robust detection that works across unseen forgery types.

Method: Attention-guided Noise Learning (ANL) integrates frozen diffusion model into detection pipeline. Detector learns to predict noise at given diffusion step, capturing discrepancies between real/synthetic images. Attention mechanism from predicted noise focuses on globally distributed artifacts rather than local patterns.

Result: Significantly outperforms existing methods on multiple benchmarks, achieving state-of-the-art accuracy for diffusion-generated deepfakes. Substantially improves ACC/AP on unseen models without inference overhead.

Conclusion: Diffusion noise provides powerful signal for generalizable deepfake detection. ANL framework effectively leverages diffusion model's natural image distribution as regularization to improve detector generalization to unseen forgery types.
...
TL;DR: M3D-Net: A multi-modal 3D facial feature reconstruction network for deepfake detection that outperforms existing methods through 3D facial reconstruction and attention-based feature fusion.


<details>
  <summary>Details</summary>
Motivation: Deepfake techniques have become highly realistic, threatening cybersecurity and information authenticity. Existing detection methods often fail to fully exploit complementary multi-modal features and rely on isolated facial attribute reconstruction.

Method: Proposes M3D-Net with dual-stream architecture: 1) self-supervised 3D facial reconstruction module for geometry and reflectance properties from RGB images, 2) 3D Feature Pre-fusion Module for multi-scale feature adjustment, 3) Multi-modal Fusion Module using attention mechanisms to integrate RGB and 3D features.

Result: Achieves state-of-the-art performance on multiple public datasets with superior detection accuracy, robustness, and strong generalization across diverse scenarios, significantly outperforming existing methods.

Conclusion: M3D-Net effectively addresses deepfake detection challenges by leveraging multi-modal 3D facial feature reconstruction and attention-based fusion, providing a robust solution for cybersecurity applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing audio-driven video generation models require multi-step denoising which causes high computational overhead, limiting real-world deployment. One-step distillation approaches exist but suffer from training instability.
...
Result: Achieves single-step generation of video talking avatars with 120x inference speed boost while maintaining high generation quality.

Conclusion: TurboTalk effectively addresses training instability in extreme step reduction for audio-driven video generation, enabling efficient single-step inference suitable for real-world deployment.
...
TL;DR: MapSR is a prompt-driven framework for high-resolution land-cover mapping that enhances coarse low-resolution labels into HR maps without HR supervision, using frozen foundation models and training-free inference.
...
Method: MapSR decouples supervision from training: uses LR labels once to extract class prompts from frozen vision foundation model features via lightweight linear probe, then performs training-free metric inference (cosine-similarity matching) and graph-based prediction refinement.

Result: Achieves 59.64% mIoU on Chesapeake Bay dataset without HR labels, competitive with strongest weakly supervised baseline, surpasses fully supervised baseline. Reduces trainable parameters by 4 orders of magnitude and training time from hours to minutes.

Conclusion: MapSR enables scalable high-resolution land-cover mapping under limited annotation and compute budgets by leveraging frozen foundation models and eliminating expensive retraining.
...
TL;DR: Masked Logit Nudging enables prompt-guided image editing in visual autoregressive models by aligning predictions with source token maps and using cross-attention masking, achieving state-of-the-art performance while being faster than diffusion models.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of prompt-guided image editing in visual autoregressive models, where the goal is to modify source images according to target text prompts while preserving unrelated regions, which existing methods struggle with.

Method: Proposes Masked Logit Nudging that uses source image token maps to guide model predictions under target prompts, converts fixed source encodings into logits using VAR encoding, applies edits only within spatial masks obtained through cross-attention differences, and includes refinement to correct quantization errors.

Result: Achieves best image editing performance on PIE benchmark at 512px and 1024px resolutions, delivers faithful reconstructions, outperforms previous methods on COCO at 512px and OpenImages at 1024px, and outperforms VAR-related approaches while being comparable or better than diffusion models with much faster inference.

Conclusion: Masked Logit Nudging provides an effective solution for prompt-guided image editing in visual autoregressive models, achieving state-of-the-art performance across multiple benchmarks while maintaining efficiency advantages over diffusion-based approaches.
...
TL;DR: GIST is a training-free, identity-preserving image compositor that harmonizes mismatched visual elements in graphic design pipelines, improving visual harmony without modifying existing components.


<details>
  <summary>Details</summary>
Motivation: Current graphic design methods assume input elements are already stylistically harmonious, but in practice they often come from disparate sources with visual mismatches. There's a need for identity-preserving stylization to create truly cohesive designs.
...
Result: When integrated with LaDeCo and Design-o-meter pipelines, GIST shows significant improvements in visual harmony and aesthetic quality, validated by LLaVA-OV and GPT-4V evaluations with pairwise preference over naive pasting.

Conclusion: GIST addresses the critical missing ingredient of identity-preserving stylization in components-to-design pipelines, enabling harmonization of mismatched visual elements while preserving their identity.
...
TL;DR: New resultant-based solvers using Sylvester forms for pose estimation that are numerically accurate as state-of-the-art but faster computationally.


<details>
  <summary>Details</summary>
Motivation: Non-linear least-squares pose estimation is computationally expensive but fundamental for real-time computer vision applications. Recent resultant-based solvers show promise for reducing computation time while maintaining accuracy.

Method: Proposed a new class of resultant-based solvers that exploit Sylvester forms to reduce complexity. Applied to two pose estimation problems: 3D-to-3D correspondences and 3D-to-2D point correspondences.

Result: The proposed methods are numerically as accurate as state-of-the-art solvers and outperform them in terms of computational time.

Conclusion: Sylvester form-based resultant solvers provide an effective approach for faster pose estimation while maintaining accuracy, applicable to both 3D-3D and 3D-2D correspondence problems.
...
TL;DR: Multigrain-aware Semantic Prototype Scanning for pan-sharpening using high-order RWKV with tri-token prompting and invertible Q-shift operations.


<details>
  <summary>Details</summary>
Motivation: Traditional RWKV's bidirectional raster scanning is semantic-agnostic and prone to positional bias, limiting effective global interaction for pan-sharpening tasks.
...
Conclusion: The proposed paradigm effectively addresses semantic-agnostic scanning limitations in RWKV for pan-sharpening through semantic-driven scanning, tri-token prompting, and efficient feature transformation.
...
TL;DR: Switch-KD: A visual-switch distillation framework that unifies vision-language knowledge transfer in shared text-probability space, enabling efficient multimodal knowledge distillation for VLMs without architectural changes.


<details>
  <summary>Details</summary>
Motivation: VLMs are large and challenging to deploy in resource-constrained scenarios. Current KD methods for VLMs supervise each modality separately without addressing multimodal alignment, leading to inconsistent knowledge transfer.

Method: Switch-KD framework with two components: 1) Visual-Switch Distillation that switches student's visual outputs into teacher's language pathway for cross-modal probabilistic references, and 2) Dynamic Bi-directional Logits Difference (DBiLD) loss that adaptively aligns informative probability regions while preserving distributional structures.

Result: A 0.5B TinyLLaVA effectively distills rich multimodal knowledge from its 3B teacher, achieving average improvement of 3.6 points across 10 multimodal benchmarks without architectural modification.

Conclusion: Switch-KD provides an effective framework for unifying vision-language knowledge transfer through shared text-probability space, enabling efficient deployment of VLMs in resource-constrained scenarios through improved knowledge distillation.
...
TL;DR: A new 3D policy learning architecture with transformer-based 3D encoder and diffusion decoder addresses training instability and overfitting issues, achieving SOTA performance on manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: 3D policy learning has potential for superior generalization and cross-embodiment transfer, but has been hindered by training instabilities and severe overfitting that prevent adoption of powerful 3D perception models.

Method: Systematically diagnose failures (identifying missing 3D data augmentation and adverse Batch Normalization effects), propose new architecture coupling scalable transformer-based 3D encoder with diffusion decoder, engineered for stability at scale and designed to leverage large-scale pre-training.

Result: Significantly outperforms state-of-the-art 3D baselines on challenging manipulation benchmarks.

Conclusion: Establishes a new and robust foundation for scalable 3D imitation learning.
...
TL;DR: Novel cross-modality token modulation approach for unsupervised video object segmentation that strengthens appearance-motion interaction through relation transformer blocks and token masking, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: While two-stream architectures integrating appearance and motion cues show promise for unsupervised video object segmentation, effectively modeling their interdependencies remains challenging. Current methods don't fully leverage these complementary information sources.

Method: Proposes cross-modality token modulation with dense connections between appearance and motion tokens, enabling efficient intra-modal and inter-modal information propagation through relation transformer blocks. Includes token masking strategy to improve learning efficiency without relying solely on increased model complexity.
...
Conclusion: The cross-modality token modulation approach effectively strengthens appearance-motion interactions in video segmentation, demonstrating superior performance through efficient information propagation and learning strategies.
...
TL;DR: A complete modulo-based HDR imaging system enabling high-speed, full-color HDR acquisition through exposure-decoupled formulation and iteration-free unwrapping with diffusion priors, validated with 1000 FPS hardware implementation.


<details>
  <summary>Details</summary>
Motivation: Conventional HDR imaging faces trade-offs between motion artifacts in multi-exposure captures and information loss in single-shot techniques. Modulo sensors offer unbounded dynamic range but are bottlenecked by iterative unwrapping overhead and hardware constraints limiting them to low-speed, grayscale capture.

Method: 1) Exposure-decoupled formulation of modulo imaging allowing multiple measurements to be interleaved in time; 2) Iteration-free unwrapping algorithm integrating diffusion-based generative priors with the physical least absolute remainder property of modulo images; 3) Hardware implementation based on modulo-encoded spike streams preserving high temporal resolution.

Result: Achieves 1000 FPS full-color imaging while reducing output data bandwidth from approximately 20 Gbps to 6 Gbps. The coordinated approach overcomes key systemic bottlenecks, demonstrating feasibility of deploying modulo imaging in dynamic scenarios.

Conclusion: The proposed modulo-based HDR imaging system successfully enables high-speed, full-color HDR acquisition by advancing both sensing formulation and unwrapping algorithm, overcoming previous limitations of modulo imaging for practical dynamic applications.
...
TL;DR: FogFool: A physically plausible adversarial attack using fog-based perturbations optimized with Perlin noise to deceive remote sensing image classifiers, achieving high transferability and robustness against defenses.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attacks on remote sensing image classification rely on unrealistic pixel-wise perturbations that don't exploit atmospheric characteristics or survive real-world image degradations, creating a need for more physically plausible and robust attack methods.

Method: FogFool generates fog-based perturbations by iteratively optimizing atmospheric patterns based on Perlin noise, modeling natural fog formations with irregular structures to create visually consistent adversarial examples that leverage spatial coherence and mid-to-low-frequency features of atmospheric phenomena.

Result: Achieves superior performance in white-box settings, exceptional black-box transferability (83.74% TASR), and robustness against preprocessing defenses like JPEG compression and filtering. Confusion matrices and CAM visualizations show the perturbations induce universal shifts in model attention.

Conclusion: FogFool represents a practical, stealthy, and highly persistent threat to RS classification systems, providing a robust benchmark for evaluating model reliability in complex environments while demonstrating the vulnerability of current systems to physically plausible adversarial attacks.
...
TL;DR: Chaos-based feature transformation improves CNN performance in limited data scenarios without increasing model complexity.


<details>
  <summary>Details</summary>
Motivation: CNNs often overfit and lack feature diversity when training data is limited, leading to poor generalization.

Method: Apply nonlinear chaos-based transformations (logistic, skew tent, sine maps) to normalized feature vectors before classification layer to reshape feature space and improve class separability.

Result: Consistent improvements across MNIST (5.43% gain), Fashion-MNIST (9.11% gain), and CIFAR-10 (7.47% gain) with different chaotic maps and CNN architectures under limited data conditions.

Conclusion: Chaos-based feature transformation is computationally efficient, parameter-free, easily integrable into existing CNNs, and provides practical solution for data-scarce image classification.
...
TL;DR: Seen-to-Scene: A unified propagation-generation framework for video outpainting that achieves superior temporal coherence and visual realism through flow-based propagation and reference-guided latent propagation.


<details>
  <summary>Details</summary>
Motivation: Existing video outpainting methods using large generative models suffer from implicit temporal modeling and limited spatial context, leading to intra-frame and inter-frame inconsistencies, especially in dynamic scenes and large outpainting scenarios.

Method: Unifies propagation-based and generation-based paradigms; uses flow-based propagation with a flow completion network pre-trained for video inpainting (fine-tuned end-to-end); introduces reference-guided latent propagation to efficiently propagate source content across frames.

Result: Achieves superior temporal coherence and visual realism with efficient inference, surpassing prior state-of-the-art methods even those requiring input-specific adaptation.

Conclusion: Seen-to-Scene effectively addresses the limitations of existing video outpainting methods by combining propagation and generation approaches, resulting in more coherent and realistic video expansion beyond original frame boundaries.
...
TL;DR: DETR-ViP improves visual-prompted object detection by addressing the lack of global discriminability in visual prompts through global prompt integration, visual-textual prompt relation distillation, and selective fusion.


<details>
  <summary>Details</summary>
Motivation: Visual prompts outperform text prompts for rare categories but have been neglected as a byproduct of text-prompted detectors, with suboptimal performance due to lack of global discriminability in visual prompts.

Method: DETR-ViP framework with global prompt integration and visual-textual prompt relation distillation on top of image-text contrastive learning, plus selective fusion strategy for stable detection.

Result: Substantially higher performance on COCO, LVIS, ODinW, and Roboflow100 compared to state-of-the-art methods, with ablation studies validating effectiveness.

Conclusion: DETR-ViP successfully addresses the discriminability issue in visual prompts, unlocking their potential for open-vocabulary object detection.
...
<details>
  <summary>Details</summary>
Motivation: Existing object-agnostic video understanding solutions struggle with substantial object variations over time and over-rely on saliency-driven cues, lacking explicit grounding to visual evidence regions.

Method: Formulates video reasoning as step-by-step process building spatially grounded traces around task-relevant objects. Uses search-guided controller optimized via reinforcement learning with format reward to iteratively ground visual evidence regions and form reliable reasoning trajectories.
...
Conclusion: Chain-of-Glimpse enables accurate and interpretable multi-step video reasoning by explicitly anchoring reasoning steps to specific visual evidence regions, mitigating over-reliance on saliency-driven cues.
...
TL;DR: A courtroom-style adjudication framework for image manipulation localization that treats the task as evidence confrontation followed by judgment, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing IML methods use authenticity supervision only as auxiliary training signals rather than explicitly modeling it as evidence opposing manipulated regions, leading to unreliable predictions when manipulation traces are subtle or degraded.

Method: Proposes a courtroom-style framework with: 1) Dual-hypothesis segmentation architecture (prosecution vs defense streams) on shared encoder, 2) Evidence production via edge priors, cascaded fusion, disagreement suppression, and debate refinement, 3) Reinforcement learning judge model for strategic re-inference with advantage-based rewards and soft-IoU objective.

Result: Experimental results show superior average performance compared with state-of-the-art IML methods.

Conclusion: The courtroom-style adjudication framework effectively addresses limitations of existing IML methods by explicitly modeling and comparing manipulated vs authentic evidence, particularly in challenging cases with subtle or degraded manipulation traces.
...
TL;DR: NG-GS introduces a novel framework for high-quality object segmentation in 3D Gaussian Splatting by addressing boundary discretization issues through RBF interpolation and joint optimization with NeRF.


<details>
  <summary>Details</summary>
Motivation: Segmenting objects accurately in 3DGS is challenging due to the discrete nature of Gaussian representations, which causes aliasing and artifacts at object boundaries.

Method: Automatically identifies ambiguous Gaussians at boundaries using mask variance analysis, applies RBF interpolation with multi-resolution hash encoding for continuous feature fields, and uses joint optimization aligning 3DGS with lightweight NeRF through alignment and spatial continuity losses.
...
Conclusion: NG-GS effectively addresses boundary discretization in 3DGS segmentation, enabling high-quality object segmentation with smooth and consistent boundaries.
...
TL;DR: G-MIXER: A training-free ZS-CIR method using geodesic mixup for implicit semantic expansion and MLLM-based explicit semantic re-ranking to improve retrieval diversity and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing Zero-Shot CIR methods rely too heavily on textual modality and fail to capture the fuzzy retrieval nature requiring consideration of diverse candidate combinations, leading to reduced diversity and accuracy in retrieval results.

Method: G-MIXER constructs composed query features reflecting implicit semantics through geodesic mixup over various ratios to build diverse candidate sets, then re-ranks candidates using explicit semantics from MLLMs.
...
Conclusion: G-MIXER successfully addresses limitations of existing ZS-CIR methods by combining implicit semantic expansion via geodesic mixup with explicit semantic re-ranking, improving both retrieval diversity and accuracy in a training-free manner.
...
TL;DR: MS-SSE-Net: A novel deep learning framework using DenseNet201 with multi-scale feature extraction and attention mechanisms for structural damage classification, achieving state-of-the-art performance on the StructDamage dataset.


<details>
  <summary>Details</summary>
Motivation: Accurate identification of structural damage from images is challenging due to variations in damage patterns and environmental conditions, necessitating improved deep learning approaches for civil infrastructure safety.

Method: Built on DenseNet201 backbone with multi-scale feature extraction using parallel depthwise convolutions, combined with squeeze-and-excitation channel attention and spatial attention mechanisms to emphasize informative regions and suppress noise.

Result: Achieves 99.31% precision, 99.25% recall, 99.27% F1-score, and 99.26% accuracy on StructDamage dataset, outperforming baseline DenseNet201 (98.62% precision, 98.53% recall, 98.58% F1-score, 98.53% accuracy).

Conclusion: MS-SSE-Net demonstrates superior performance for structural damage classification through effective multi-scale feature extraction and attention mechanisms, providing a robust solution for civil infrastructure safety monitoring.
...
TL;DR: A geometry-driven synthesis pipeline creates realistic 3D myotube data for training segmentation models, achieving state-of-the-art performance without real annotations.


<details>
  <summary>Details</summary>
Motivation: Myotube research needs 3D instance segmentation for quantitative analysis, but lacks large annotated datasets, and existing pretrained models fail to generalize to this domain.

Method: Geometry-driven synthesis pipeline models myotubes with polynomial centerlines, varying radii, branching structures, and ellipsoidal end caps, rendered with realistic noise, optical artifacts, and CycleGAN-based Domain Adaptation. Trained a compact 3D U-Net with self-supervised encoder pretraining exclusively on synthetic data.

Result: Achieved mean IPQ of 0.22 on real data, significantly outperforming three established zero-shot segmentation models, demonstrating synthetic data enables effective instance segmentation.

Conclusion: Biophysics-driven synthesis enables effective 3D instance segmentation in annotation-scarce biomedical domains like myotube research, overcoming the lack of large annotated datasets.
...
TL;DR: HAMSA is a scanning-free vision state space model that operates in the spectral domain, achieving SOTA accuracy on ImageNet with faster inference, lower memory usage, and better energy efficiency than both transformers and scanning-based SSMs.


<details>
  <summary>Details</summary>
Motivation: Existing vision SSMs (Vim, VMamba, SiMBA) require complex scanning strategies to adapt sequential models to 2D images, which introduces computational overhead and architectural complexity. The authors aim to simplify this by operating directly in the spectral domain.

Method: HAMSA introduces three innovations: 1) Simplified kernel parameterization using a single Gaussian-initialized complex kernel instead of traditional (A, B, C) matrices; 2) SpectralPulseNet (SPN) for input-dependent frequency gating; 3) Spectral Adaptive Gating Unit (SAGU) for magnitude-based gating in the frequency domain. It uses FFT-based convolution to eliminate sequential scanning.

Result: On ImageNet-1K, HAMSA achieves 85.7% top-1 accuracy (SOTA among SSMs), with 2.2× faster inference than transformers (4.2ms vs 9.2ms for DeiT-S), 1.4-1.9× speedup over scanning-based SSMs, lower memory usage (2.1GB vs 3.2-4.5GB), and better energy efficiency (12.5J vs 18-25J).

Conclusion: HAMSA demonstrates that operating in the spectral domain eliminates the need for complex scanning strategies in vision SSMs, achieving superior efficiency, speed, and accuracy while maintaining strong generalization across various vision tasks.
...
TL;DR: FR systems can be used for morphing attack detection with proper thresholding to limit vulnerability


<details>
  <summary>Details</summary>
Motivation: Current face recognition systems are vulnerable to morphing attacks, and there's a need for effective detection methods that can work with existing FR infrastructure

Method: Proposes using existing FR systems for differential morphing attack detection (D-MAD) by comparing FR systems with existing D-MAD approaches and introducing a new evaluation threshold

Result: Shows that FR and D-MAD perform similar tasks, current thresholds make FR vulnerable to morphing attacks, and new threshold can guarantee upper limit to vulnerability even for unknown attack types

Conclusion: Existing FR systems can be repurposed for morphing attack detection with appropriate threshold selection to ensure security against both known and unknown morphing attacks
...
TL;DR: ASGNet is a novel adaptive spectrum guidance network for polyp segmentation that integrates spectral features with global attributes to overcome spatial perception limitations in existing methods.


<details>
  <summary>Details</summary>
Motivation: Polyp segmentation is challenging due to diverse morphologies, complex backgrounds, and concealed nature of polyps. Existing deep learning methods have biased perceptual capabilities toward local regions due to strong spatial correlations, making it difficult to capture complete polyp structures and leading to sub-optimal segmentation results.

Method: Proposes ASGNet with three key components: 1) Spectrum-guided non-local perception module that jointly aggregates local and global information to enhance discriminability of polyp structures and refine boundaries; 2) Multi-source semantic extractor that integrates rich high-level semantic information for preliminary polyp localization; 3) Dense cross-layer interaction decoder that effectively integrates diverse information from different layers to generate high-quality representations.
...
Conclusion: ASGNet effectively addresses spatial perception limitations by integrating spectral features with global attributes, achieving superior polyp segmentation performance compared to existing methods, with code publicly available.
...
TL;DR: OmniGCD is a modality-agnostic Generalized Category Discovery method that performs zero-shot category discovery across vision, text, audio, and remote sensing datasets without dataset-specific fine-tuning, using synthetic training and a novel Transformer-based model.


<details>
  <summary>Details</summary>
Motivation: Current GCD methods are limited to single modalities and require dataset-specific fine-tuning, unlike human category learning which is abstract and modality-agnostic. The authors aim to create a more human-inspired approach that works across multiple modalities without per-dataset adaptation.

Method: Uses modality-specific encoders to process inputs, reduces dimensions to create a GCD latent space, then transforms it at test-time using a novel Transformer-based model trained on synthetic data. Introduces zero-shot GCD setting where no dataset-specific fine-tuning is allowed.

Result: Achieves average percentage point improvements of +6.2 (vision), +17.9 (text), +1.5 (audio), and +12.7 (remote sensing) across 16 datasets spanning four modalities. Demonstrates zero-shot GCD capability with single synthetic training.

Conclusion: OmniGCD enables modality-agnostic category discovery, decouples representation learning from category discovery, and serves as a benchmark for future modality-agnostic GCD research. Shows that improving modality-agnostic methods propagates benefits across all modalities.
...
<details>
  <summary>Details</summary>
Motivation: Existing CL methods assume symmetric architectures, but modern VLMs have asymmetric trainable components, causing standard regularization to favor the language decoder and leave visual projection layers vulnerable to interference, leading to loss of compositional reasoning.

Method: Proposes Asymmetric Information Masking (AIM) which balances stability and plasticity by applying targeted masks based on modality-specific sensitivity.

Result: Experiments on VQA v2 and GQA show AIM achieves state-of-the-art performance in both Average Performance (AP) and Average Forgetting (AF), while better preserving generalization to novel skill-concept compositions.

Conclusion: AIM effectively addresses the structural mismatch in VLMs for continual VQA by protecting vulnerable visual components while maintaining language capabilities.
...
TL;DR: A modular railway obstacle detection system combining object detection, track segmentation, and monocular depth estimation with LiDAR achieves 0.63m MAE for distance estimation.


<details>
  <summary>Details</summary>
Motivation: Current railway obstacle detection systems lack comprehensive solutions that combine object detection, track identification, and distance estimation. Most focus on only one aspect, and evaluation is challenging due to lack of ground truth data.

Method: Proposes a modular framework integrating three neural networks: object detection, track segmentation, and monocular depth estimation, combined with LiDAR point clouds for enhanced spatial perception.

Result: The system achieves a mean absolute error (MAE) as low as 0.63 meters for distance estimation when integrating monocular depth maps with LiDAR, enabling accurate obstacle distance measurement.

Conclusion: The proposed modular framework successfully addresses the comprehensive railway obstacle detection problem by combining multiple neural networks with LiDAR, achieving accurate distance estimation and spatial scene perception, validated using synthetic data with ground truth.
...
TL;DR: Single-image 3D head avatar creation with realistic hair dynamics by decoupling hair from face, using 3D Gaussian Splatting representations, and simulating hair with physics-based deformation.


<details>
  <summary>Details</summary>
Motivation: Prior one-shot holistic approaches fail to produce realistic hair dynamics during animation due to inadequate decoupling of hair from facial regions, leading to entangled geometry and unnatural deformations.

Method: 1) Hair removal to get bald image; 2) Lift both images to 3D Gaussian Splatting representations; 3) Rig bald 3DGS to FLAME mesh for facial deformation; 4) Extract hair Gaussians using semantic label supervision; 5) Control hair deformation with cage structure supporting Position-Based Dynamics simulation.

Result: Substantially more realistic hair behavior with faithfully preserved facial details, outperforming state-of-the-art one-shot methods in perceptual realism across diverse head motions, gravity effects, and expressions.

Conclusion: The compositional approach successfully addresses hair dynamics limitations in one-shot 3D head avatar creation by explicitly decoupling hair from face and using physics-based simulation for realistic hair deformation.
...
TL;DR: Petro-SAM: A two-stage multi-task framework for joint grain-edge and lithology semantic segmentation in petrographic images, addressing domain gaps and extinction issues via multi-angle integration and feature refinement.


<details>
  <summary>Details</summary>
Motivation: Current grain-edge segmentation (GES) and lithology semantic segmentation (LSS) are treated separately with poor quality despite expensive expert annotations. Foundation models like SAM show promise but face domain gaps from extinction-dependent color variations and ultra-fine boundaries, plus lack joint learning modules for multi-angle petrographic images.

Method: Propose Petro-SAM, a two-stage multi-task framework based on SAM. Introduces Merge Block to integrate seven polarized views to solve extinction issues. Uses multi-scale feature fusion and color-entropy priors for detection refinement.

Result: The framework achieves high-quality joint GES and LSS on petrographic images by effectively addressing domain gaps and extinction problems through multi-angle integration and refined detection techniques.

Conclusion: Petro-SAM provides an effective solution for joint grain-edge and lithology segmentation in petrographic analysis, overcoming key challenges of domain adaptation and multi-angle data integration that previously hindered accurate geological fabric and composition quantification.
...
TL;DR: NTIRE 2026 Video Saliency Prediction Challenge overview with new dataset of 2,000 videos, 5,000+ assessors, 20+ teams, 7 finalists, all data publicly available.


<details>
  <summary>Details</summary>
Motivation: To advance video saliency prediction research by creating a comprehensive challenge with a novel, diverse dataset and encouraging development of automatic saliency map prediction methods for video sequences.

Method: Organized a challenge where participants developed automatic saliency prediction methods. Created a novel dataset of 2,000 diverse videos with open license, collected fixations and saliency maps using crowdsourced mouse tracking from over 5,000 assessors.

Result: Challenge attracted over 20 teams with 7 passing final phase after code review. Evaluation performed on 800 test videos using accepted quality metrics. All challenge data made publicly available on GitHub.

Conclusion: The NTIRE 2026 Video Saliency Prediction Challenge successfully advanced the field by providing a comprehensive benchmark dataset and fostering competition among research teams, with all resources made publicly available for continued research.
...
TL;DR: MSSM+ enhances cortical biomarker analysis by integrating sulcal depth and curvature with existing gray-white contrasts and cortical thickness, using supervertex mapping and Vision Transformer for improved Alzheimer's disease detection.


<details>
  <summary>Details</summary>
Motivation: Current AD confirmation methods (PET, CSF) are costly and invasive, while structural MRI biomarkers like cortical thickness are limited. Need for more comprehensive, non-invasive imaging markers that capture richer cortical features.

Method: MSSM+ extends MSSM by adding sulcal depth and cortical curvature at vertex level. Surface Supervertex Mapping (SSVM) partitions cortex into patches representing spatial relationships. Supervertex Vision Transformer (SV-ViT) enables anatomically informed learning from surface mesh representations.

Result: MSSM+ identified more extensive and significant AD vs CN group differences than MSSM. Achieved 3% higher area under precision-recall curve in classification. Vendor-specific analyses showed reduced signal variability and consistently improved performance across MR manufacturers compared to CT, GWCs, and MSSM.

Conclusion: MSSM+ combined with SV-ViT is a promising MRI-based imaging marker for AD detection prior to CSF/PET confirmation, offering more comprehensive cortical feature analysis with improved classification performance.
...
TL;DR: Paza is a zero-shot retail theft detection framework that uses a layered pipeline with cheap object/pose detection and expensive VLM only when triggered, achieving 240x reduction in VLM calls and 3-10x cost savings.


<details>
  <summary>Details</summary>
Motivation: Retail theft costs over $100B annually, but existing AI detection systems are expensive ($200-500/month per store) and require custom model training on proprietary datasets.

Method: Multi-layered pipeline: cheap object detection and pose estimation run continuously, expensive VLM invoked only when behavioral pre-filters trigger (dwell time + behavioral signal). Model-agnostic VLM component works with any OpenAI-compatible endpoint.

Result: 89.5% precision, 92.8% specificity at 59.3% recall zero-shot on DCSASS dataset; reduces VLM invocations by 240x; enables single GPU to serve 10-20 stores; cost model shows viability at $50-100/month per store (3-10x cheaper).

Conclusion: Paza enables practical, cost-effective retail theft detection without custom model training, with privacy-preserving design and open-source availability, improving as VLM technology evolves.
...
TL;DR: IAC-LTH accelerates neural architecture search for adaptive skip modules in medical image segmentation by pruning low-importance operations early based on stability criteria, reducing search time 3.7-16x while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Original Implantable Adaptive Cells (IACs) require 200-epoch differentiable NAS for each backbone/dataset, which is computationally expensive. Need to make adaptive skip-module design more practical under realistic computational constraints.

Method: Analyzed temporal behavior of operations/edges during differentiable search, found selected operations emerge early and stabilize. Proposed Jensen-Shannon-divergence-based stability criterion to track per-edge operation-importance distributions and progressively prune low-importance operations during search (IAC-LTH).

Result: IAC-LTH reduces wall-clock NAS cost by 3.7x to 16x across datasets and backbones while matching/exceeding original full-search performance. Consistent across four benchmarks (ACDC, BraTS, KiTS, AMOS), multiple U-Net backbones, and nnU-Net pipeline.
...
TL;DR: MetaDent introduces a comprehensive resource for dental image analysis with a large-scale dataset, semi-structured annotation framework, and benchmark suites to evaluate VLMs on intraoral photography.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models have significant potential in medical image analysis but remain underexplored in intraoral photography due to lack of fine-grained annotated datasets and comprehensive benchmarks.

Method: Created MetaDent with: (1) 60,669 dental images from diverse sources, (2) semi-structured annotation framework combining high-level summaries with point-by-point abnormality descriptions, (3) LLM-derived benchmarks including 15K VQA pairs and 18-class multi-label classification dataset validated with human review.

Result: Even state-of-the-art VLMs struggle with fine-grained understanding of intraoral scenes, achieving moderate accuracy and producing inconsistent/incomplete descriptions in image captioning tasks.

Conclusion: MetaDent addresses the gap in dental image analysis resources, revealing current VLM limitations in dental applications while providing publicly available datasets and tools to accelerate vision-language system development for dentistry.
...
TL;DR: Deep Metric Learning approach for open-set vein recognition achieves state-of-the-art performance on large-scale datasets while enabling adaptive enrollment without model retraining.


<details>
  <summary>Details</summary>
Motivation: Existing vein recognition methods rely on closed-set classification, limiting scalability and preventing adaptive enrollment of new users without complete model retraining. There's a need for open-set approaches that can handle new users dynamically.

Method: Uses Deep Metric Learning (DML) with L2-normalized embeddings and prototype-based matching with calibrated similarity thresholds. Evaluates triplet-based objectives with 1-NN classifier for optimal accuracy-efficiency trade-off. Tested under strict subject-disjoint protocol across four diverse vein datasets.

Result: On MMCBNU 6000 benchmark: OSCR 0.9945, AUROC 0.9974, EER 1.57%, Rank-1 accuracy 99.6%. Cross-dataset experiments show robust large-scale performance but sensitivity to domain shifts in low-data regimes. Triplet-based objectives with 1-NN classifier enable real-time deployment on commodity hardware.

Conclusion: DML with prototype-based matching effectively addresses open-set vein recognition challenges, achieving high performance while enabling scalable, adaptive enrollment without retraining. The approach maintains robustness at scale but requires attention to domain shifts in limited data scenarios.
...
TL;DR: FSDETR is a frequency-spatial feature enhancement framework for small object detection that addresses feature degradation, occlusion, and background interference through collaborative modeling of spatial and frequency domains.
...
Method: Built on RT-DETR baseline, FSDETR introduces: 1) Spatial Hierarchical Attention Block (SHAB) for local details and global dependencies, 2) Deformable Attention-based Intra-scale Feature Interaction (DA-AIFI) for occlusion handling via dynamic sampling, and 3) Frequency-Spatial Feature Pyramid Network (FSFPN) with Cross-domain Frequency-Spatial Block (CFSB) to integrate frequency filtering with spatial edge extraction.
...
Conclusion: FSDETR effectively addresses small object detection challenges through frequency-spatial collaborative modeling, achieving state-of-the-art performance with efficient parameter usage.
...
TL;DR: RATS is a lightweight framework that enables few-step generative models to surpass teacher performance through reward-aware trajectory shaping and preference alignment, rather than just imitating the teacher.


<details>
  <summary>Details</summary>
Motivation: Current distillation-based few-step generation methods constrain students to imitate teachers, limiting performance to teacher's upper bound. The authors argue that introducing preference alignment awareness can allow students to optimize toward reward-preferred quality and potentially surpass teachers.

Method: Proposes Reward-Aware Trajectory Shaping (RATS) with: 1) horizon matching to align teacher/student latent trajectories at key denoising stages, 2) reward-aware gate that adaptively regulates teacher guidance based on relative reward performance (strengthens when teacher is better, relaxes when student matches/surpasses), and 3) integration of trajectory distillation, reward-aware gating, and preference alignment.

Result: RATS substantially improves the efficiency-quality trade-off in few-step visual generation, significantly narrowing the gap between few-step students and stronger multi-step generators without additional test-time computational overhead.

Conclusion: The proposed RATS framework enables few-step generative models to surpass teacher performance through reward-aware trajectory shaping and preference alignment, offering a more effective approach than traditional teacher imitation constraints.
...
TL;DR: Text-to-3D models develop "sink traps" where they become insensitive to text prompts, limiting editing capabilities despite having geometric expressivity.


<details>
  <summary>Details</summary>
Motivation: Current text-driven inversion of generative models assumes sensitivity to natural language prompts, but this fails for state-of-the-art text-to-3D models, limiting applications like text-based editing and style transfer.

Method: Analyze sampling trajectories to identify "sink traps" where models become insensitive to text guidance, then develop a framework that decouples geometric representation from linguistic sensitivity by leveraging unconditional generative priors.

Result: Demonstrate that text-to-3D models can produce diverse shapes but become insensitive to out-of-distribution text guidance, and show that robust text-based 3D shape editing is possible by bypassing latent sinks.

Conclusion: A new framework for text-based 3D shape editing addresses limitations of current pipelines by decoupling geometric representation from linguistic sensitivity, enabling high-fidelity semantic manipulation of out-of-distribution 3D shapes.
...
TL;DR: Hybrid Gaussian-hash-grid representation improves 2D Gaussian scene reconstruction by separating geometry and appearance, using frequency decomposition and probabilistic pruning to achieve high fidelity with far fewer primitives.


<details>
  <summary>Details</summary>
Motivation: To address the entanglement between geometry and appearance in NeRF-based models, which causes high-frequency texture to compensate for geometric errors, leading to suboptimal reconstruction quality.

Method: Combines per-Gaussian latent features with hash-grid features to separate low- and high-frequency components, uses hard opacity falloffs to strengthen geometry-appearance separation, and employs probabilistic pruning with sparsity-inducing BCE opacity loss to eliminate redundant Gaussians.

Result: Achieves superior reconstruction fidelity compared to state-of-the-art Gaussian-based novel-view synthesis methods while using an order of magnitude fewer primitives, validated on both synthetic and real-world datasets.

Conclusion: The hybrid representation effectively disentangles geometry and appearance, reduces geometric errors, improves rendering efficiency, and produces minimal Gaussian sets for scene representation through frequency-based decomposition and intelligent pruning.
...
TL;DR: A conditional generative pipeline for skeleton action recognition that generates diverse, high-fidelity skeleton sequences to address data scarcity, improving recognition accuracy in low-data scenarios.


<details>
  <summary>Details</summary>
Motivation: Collecting large-scale, diverse, and well-annotated 3D skeleton datasets is expensive and labor-intensive, creating a need for effective data augmentation methods to overcome data scarcity in skeleton-based human action recognition.
...
Result: Experiments on HumanAct12 and refined NTU-RGBD (NTU-VIBE) datasets show consistent accuracy improvements for multiple skeleton-based action recognition models in both few-shot and full-data settings.

Conclusion: The proposed conditional generative pipeline effectively addresses data scarcity in skeleton action recognition by generating diverse, high-fidelity data, demonstrating strong generalization in downstream tasks and competitive performance even with limited training samples.
...
<details>
  <summary>Details</summary>
Motivation: Existing tool-use methods are limited to text-only inputs and closed-world settings, struggling with multimodal instructions and cannot generalize to tools unseen during training. There's a need for systems that can interpret multimodal user instructions and work in open-world scenarios.

Method: 1) Convert multimodal queries into structured task descriptions using MLLMs, 2) Retrieve appropriate tools by matching task descriptions against machine-readable tool descriptions, 3) Incorporate preference-based optimization using Direct Preference Optimization (DPO) to improve alignment, 4) Use standardized tool descriptions from Hugging Face model cards.

Result: Extensive experiments show the approach significantly improves tool-selection performance, particularly in open-world, multimodal scenarios. The framework naturally supports extensibility to new tools without retraining.

Conclusion: RaTA-Tool provides an effective retrieval-based framework for open-world multimodal tool selection that overcomes limitations of existing methods, enabling better interpretation of multimodal instructions and generalization to unseen tools through semantic matching and preference optimization.
...
TL;DR: Video generation AI can create realistic synthetic gesture data that complements human recordings, improving downstream gesture recognition performance.


<details>
  <summary>Details</summary>
Motivation: Gesture recognition faces data scarcity issues unlike NLP, requiring costly human recordings or limited image processing approaches that lack authentic variability.
...
Conclusion: Image-to-video techniques offer a powerful zero-shot approach to gesture synthesis that enriches original data and benefits downstream tasks, even in early stages.
...
TL;DR: FEDSNet improves few-shot fine-grained classification by using frequency-domain features to separate structural information from texture bias, achieving better accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing metric learning methods for few-shot fine-grained classification rely only on spatial features, which suffer from texture bias that entangles structural details with background noise, and lack cross-view constraints leading to overfitting and structural instability.

Method: Proposes Frequency-Enhanced Dual-Subspace Network (FEDSNet) that: 1) Uses DCT and low-pass filtering to isolate low-frequency structural components from spatial features, 2) Employs truncated SVD to create independent low-rank subspaces for spatial texture and frequency structural features, 3) Designs adaptive gating to dynamically fuse projection distances from both views.

Result: Extensive experiments on CUB-200-2011, Stanford Cars, Stanford Dogs, and FGVC-Aircraft show excellent classification performance and robustness, achieving highly competitive results compared to existing metric learning algorithms with favorable computational efficiency.

Conclusion: FEDSNet effectively addresses texture bias and structural instability in few-shot fine-grained classification by leveraging frequency-domain structural stability, providing an effective new paradigm for this task with balanced accuracy and efficiency.
...
TL;DR: UniDoc-RL is a unified RL framework for visual RAG that jointly performs retrieval, reranking, active perception, and reasoning through hierarchical actions and dense multi-reward training.


<details>
  <summary>Details</summary>
Motivation: Existing visual RAG systems rely on generic retrieval signals that overlook fine-grained visual semantics needed for complex reasoning, limiting their effectiveness in handling detailed visual information.

Method: Proposes UniDoc-RL, a unified RL framework where an LVLM agent performs hierarchical actions: document retrieval → image selection → active region cropping. Uses dense multi-reward scheme with Group Relative Policy Optimization (GRPO) for end-to-end training without separate value network.

Result: Outperforms state-of-the-art baselines on three benchmarks, achieving up to 17.7% gains over prior RL-based methods.

Conclusion: UniDoc-RL effectively addresses limitations of generic visual RAG by enabling fine-grained visual information acquisition through sequential decision-making and hierarchical action refinement.
...
TL;DR: Flow of Truth is the first proactive framework for temporal forensics in image-to-video generation, focusing on tracing pixel flow and transformations over time rather than static 2D tampering detection.


<details>
  <summary>Details</summary>
Motivation: Image-to-video generation creates realistic videos from single images but introduces new forensic challenges. Unlike static images, I2V content evolves over time, requiring forensics to move beyond 2D pixel-level tampering localization toward tracing how pixels flow and transform throughout the video. Traditional spatial forensics become ineffective as embedded traces drift and deform across frames.

Method: The authors redefine video generation as "the motion of pixels through time rather than the synthesis of frames." They propose a learnable forensic template that follows pixel motion and a template-guided flow module that decouples motion from image content, enabling robust temporal tracing of forensic signatures.

Result: Experiments show that Flow of Truth generalizes across commercial and open-source I2V models and substantially improves temporal forensics performance compared to traditional approaches.

Conclusion: The framework successfully addresses the unexplored dimension of temporal forensics in I2V generation by focusing on pixel motion rather than static frame synthesis, providing a robust solution for tracing forensic signatures that evolve consistently with the video generation process.
...
TL;DR: QuAD improves AI-generated image detection by aggregating predictions from multiple near-duplicate versions found online, using quality-aware calibration to account for degradation from web dissemination.


<details>
  <summary>Details</summary>
Motivation: Real-world viral images circulate in multiple near-duplicate versions with varying quality due to repeated operations (recompression, resizing, cropping), causing inconsistent forensic predictions when analyzed individually.

Method: QuAD retrieves online near-duplicates of a query image, feeds them to a detector, and aggregates the resulting scores based on estimated quality of each instance, accounting for reduced reliability of degraded images.

Result: Quality-aware fusion improves performance of several state-of-the-art detectors with average gain of ~8% balanced accuracy compared to plain average, validated on AncesTree (136k images) and ReWIND (10k real-world near-duplicates) datasets.

Conclusion: Jointly processing all available online near-duplicates with quality-aware calibration enables more reliable AI-generated content detection in real-world applications where images undergo degradation through web dissemination.
...
<details>
  <summary>Details</summary>
Motivation: To provide a fundamental shift from discrete sampled data to continuous functional representations, enabling unified modeling of diverse signals (images, audio, video, 3D geometry) and analytical signal operations through automatic differentiation.

Method: Evolution from standard coordinate-based networks with spectral bias to advanced designs using specialized activations (periodic, localized, adaptive) and structured representations (hierarchical decompositions, hash grid encodings) for improved spectral behavior, spatial adaptivity, and computational efficiency.

Result: INRs offer a unified framework for continuous signal representation with applications in inverse problems (medical/radar imaging), compression, and 3D scene representation, while addressing spectral behavior, sampling theory, and multiscale representation.

Conclusion: INRs represent learned signal models with adaptive approximation spaces, marking conceptual progress but facing open challenges in theoretical stability, weight space interpretability, and large-scale generalization.
...
TL;DR: Hybrid CNN-Attention framework for robust, site-invariant MRI quality assessment achieves high accuracy on seen sites (0.9920) and maintains strong generalization across unseen sites (0.755) without retraining.


<details>
  <summary>Details</summary>
Motivation: Motion artifacts in structural MRI compromise clinical diagnostics and automated analysis. Manual quality control is unscalable for massive longitudinal studies, necessitating automated solutions that can handle domain shift across different imaging sites and scanners.

Method: Proposed hybrid CNN-Attention framework with hierarchical 2D CNN encoder for local spatial feature extraction and multi-head cross-attention mechanism to model global dependencies. Trained end-to-end on MR-ART dataset (200 subjects) to prioritize motion-relevant artifacts while filtering site-specific variations.

Result: Achieved scan-level accuracy of 0.9920 and F1-score of 0.9919 on seen sites (MR-ART). Maintained strong generalization across 17 heterogeneous unseen sites from ABIDE archive with accuracy of 0.755 without any retraining or fine-tuning.

Conclusion: Attention-based feature re-weighting successfully captures universal artifact descriptors, bridging performance gap between diverse imaging environments and scanner manufacturers. The framework demonstrates high resilience to domain shift and offers scalable solution for MRI quality assessment.
...
TL;DR: HELP is a noise-aware positional-semantic fusion framework for small-object detection that uses heatmap-guided positional embedding to suppress background noise, enabling efficient query retrieval with 59.4% parameter reduction while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Transformer-based detectors for small objects are inefficient and vulnerable to background-induced query noise, requiring deep decoders to refine low-quality queries.

Method: HELP framework with Heatmap-guided Positional Embedding (HPE) that selectively preserves positional encodings in foreground regions while suppressing background clutter. HPE is integrated into encoder and decoder with gradient-based mask filtering and Linear-Snake Convolution for feature enrichment.

Result: Reduces decoder layers from eight to three, achieves 59.4% parameter reduction (66.3M vs. 163M) while maintaining consistent accuracy gains under reduced compute budget across benchmarks.

Conclusion: HELP provides an efficient noise-aware framework for small-object detection that significantly reduces computational complexity while maintaining detection accuracy through intelligent positional-semantic fusion.
...
TL;DR: HaLoBuilding: First optical benchmark for building extraction under hazy/low-light conditions, with HaLoBuild-Net framework using spatial-frequency attention and multi-scale guidance to overcome weather degradation.


<details>
  <summary>Details</summary>
Motivation: Existing optical building extraction methods and benchmarks focus on ideal clear-weather conditions, suffering performance degradation under real-world hazy and low-light conditions. SAR has all-weather capability but suffers geometric distortions, creating a gap for robust optical building extraction in adverse weather.

Method: Introduces HaLoBuilding benchmark with same-scene multitemporal pairing for pixel-level label alignment. Proposes HaLoBuild-Net framework with: 1) Spatial-Frequency Focus Module (SFFM) coupling large receptive field attention with frequency-aware channel reweighting; 2) Global Multi-scale Guidance Module (GMGM) for global semantic constraints; 3) Mutual-Guided Fusion Module (MGFM) for bidirectional semantic-spatial calibration to suppress noise and sharpen boundaries.

Result: HaLoBuild-Net significantly outperforms state-of-the-art methods and conventional cascaded restoration-segmentation paradigms on the HaLoBuilding dataset, while maintaining robust generalization on WHU, INRIA, and LoveDA datasets.

Conclusion: The work addresses a critical gap in building extraction under adverse weather conditions by introducing the first specialized benchmark and an effective end-to-end framework that leverages spatial-frequency analysis and multi-scale guidance to overcome meteorological interference.
...
TL;DR: STFER framework uses Large Vision-Language Models to generate identity-consistent text descriptions, which then filter visual tokens and route experts for robust person re-identification across clothing changes and day/night modality shifts.


<details>
  <summary>Details</summary>
Motivation: Existing person re-ID methods rely heavily on visual features that are vulnerable to environmental changes (illumination, clothing variations) and modality shifts (RGB to IR), leading to performance degradation in real-world any-time scenarios.

Method: Proposes STFER framework that: 1) Uses LVLMs with instructions to generate identity-intrinsic semantic text capturing biometric constants, 2) Applies Semantic-driven Visual Token Filtering (SVTF) to enhance informative regions and suppress background noise, 3) Implements Semantic-driven Expert Routing (SER) to integrate semantic text into expert routing for robust multi-scenario gating.

Result: Achieves state-of-the-art results on AT-USTC dataset and demonstrates superior generalization capabilities across 5 widely-used ReID benchmarks with highly competitive performance.

Conclusion: Leveraging LVLMs to generate identity-consistent text provides robust identity-discriminative features that are resilient to clothing variations and cross-modality shifts, enabling effective any-time person re-identification.
...
TL;DR: LAMAE is a masked autoencoder with latent attention for multi-view echocardiography that learns holistic cardiac representations from sparse views and transfers well across patient populations.


<details>
  <summary>Details</summary>
Motivation: Echocardiography provides sparse, heterogeneous views of the heart, but existing MAE approaches process images/clips independently without capturing the multi-view structure needed for coherent cardiac representation.

Method: LAMAE augments standard MAE with a latent attention module that enables information exchange across frames and views directly in latent space, allowing aggregation of variable-length sequences and distinct views to reconstruct holistic cardiac function.

Result: First results for predicting ICD-10 codes from MIMIC-IV-ECHO videos; representations learned from adult data transfer effectively to pediatric cohorts despite anatomical differences; multi-view attention yields more robust and transferable representations.

Conclusion: Incorporating structural priors like multi-view attention in foundation models yields significantly more robust and transferable representations for medical imaging, enabling effective cross-population transfer learning.
...
TL;DR: PIE-V is a framework for creating mistake-aware egocentric procedural videos by injecting human-plausible errors and recoveries into clean procedures, with comprehensive benchmarking metrics.


<details>
  <summary>Details</summary>
Motivation: Existing procedural datasets lack realistic human errors and recoveries needed for reliable monitoring, especially in egocentric videos where mistakes are often occluded by hands and revealed through subtle object state changes.

Method: PIE-V combines psychology-informed error planning conditioned on procedure phase, correction planning for recovery behavior, LLM-based text rewriting, video generation for replacement clips, and stitching for visual plausibility.

Result: Applied to 17 tasks and 50 scenarios, PIE-V injected 102 mistakes and generated 27 recovery corrections, outperforming freeform LLM baselines across nine comprehensive quality metrics.

Conclusion: The framework and benchmarking rubric enable post-completion verification for egocentric procedural mistake detection and correction, addressing the gap in realistic error-recovery datasets.
...
TL;DR: kVNN uses learnable multi-kernel representation with polynomial-kernel components of different orders to model feature interactions efficiently, reducing model size and computation while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Higher-order learning requires exploiting compositional features through elaborate data interactions, but this tends to increase model complexity in conventional deep learning models. The paper aims to address the trade-off between expressivity and computational cost.

Method: Proposes kernelized Volterra Neural Network (kVNN) with learnable multi-kernel representation. Different interaction orders are modeled by distinct polynomial-kernel components with compact, learnable centers. Features are learned through composition of layers with parallel branches of different polynomial orders, enabling kVNN filters to directly replace standard convolutional kernels.

Result: kVNN consistently yields reduced model parameters and computational complexity (GFLOPs) with competitive and often improved performance on video action recognition and image denoising tasks. Results are maintained even when trained from scratch without large-scale pretraining.

Conclusion: Structured kernelized higher-order layers offer a practical path to balancing expressivity and computational cost in modern deep networks, demonstrating favorable performance-efficiency trade-offs.
...
TL;DR: DAMP is a one-shot, closed-form weight surgery method for class unlearning that removes forget-class information from deep representations via projection-based updates, outperforming prior methods by better matching retraining performance.


<details>
  <summary>Details</summary>
Motivation: Current class unlearning methods often fail to achieve true forgetting - they may only suppress classifier outputs while leaving forget-class information encoded in internal representations, or rely on bias shifts rather than removing representational structure.

Method: DAMP computes class prototypes in each layer's input space, extracts forget directions as residuals relative to retain-class prototypes, and applies projection-based weight updates to reduce sensitivity to those directions. It uses depth-aware scaling (smaller edits in early layers, larger in deeper layers) based on probe separability to preserve utility.

Result: DAMP outperforms prior methods across MNIST, CIFAR-10, CIFAR-100, and Tiny ImageNet with both CNN and transformer architectures, achieving better selective forgetting while preserving retain-class performance and reducing residual forget-class structure in deep layers.

Conclusion: DAMP provides an effective one-shot solution for class unlearning that more closely approximates retraining from scratch by directly removing forget-class information from network representations through principled projection-based weight surgery.
...
TL;DR: The paper presents two approaches for lighting-related image restoration: DINOLight (specialized per-dataset) and OmniLight (unified WD-MoE), both achieving top results in NTIRE 2026 Challenge tracks.


<details>
  <summary>Details</summary>
Motivation: Adverse lighting conditions degrade visibility and color fidelity, harming computer vision systems. While SOTA results on benchmarks are important, real-world applications need robust models that handle diverse domains.

Method: Two contrasting strategies: 1) DINOLight - robust ALN framework specialized per dataset, and 2) OmniLight - generalized alternative with Wavelet Domain Mixture-of-Experts (WD-MoE) trained across all datasets.
...
Conclusion: The comparative analysis shows the impact of data distribution on specialized vs. unified architectures for lighting restoration, with both strategies achieving state-of-the-art performance.
...
TL;DR: Simpler penalty terms can provide similar benefits to expensive Fokker-Planck regularization in diffusion models at much lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Diffusion models trained with DSM often violate the Fokker-Planck equation, and while direct penalization reduces violations, it introduces significant computational overhead without necessarily improving sample quality.

Method: Empirical analysis of several lightweight regularizers, studying their effect on FP residuals and generation quality compared to expensive FP regularization.

Result: Simpler penalty terms provide similar benefits to FP regularization at substantially lower computational cost, with best results often obtained with weaker regularization.

Conclusion: The benefits of FP regularization in diffusion models can be achieved with simpler, computationally efficient penalty terms rather than expensive direct FP equation enforcement.
...
TL;DR: B-ACT is an active learning framework for temporal action segmentation that focuses annotation budget on boundary regions where errors concentrate, using hierarchical selection of videos and boundary frames to maximize label efficiency.


<details>
  <summary>Details</summary>
Motivation: Temporal action segmentation requires dense temporal supervision, but most annotation cost goes to identifying and refining action transitions where segmentation errors concentrate and small temporal shifts disproportionately degrade metrics.

Method: Two-stage hierarchical active learning: (1) ranks and queries unlabeled videos using predictive uncertainty, (2) within selected videos, detects candidate transitions and selects top-K boundaries via a novel boundary score combining neighborhood uncertainty, class ambiguity, and temporal predictive dynamics. Uses boundary-frame-only annotation while training on boundary-centered clips.
...
Conclusion: Focusing annotation budget on boundary regions through the B-ACT framework provides effective label efficiency for temporal action segmentation, outperforming existing active learning approaches by explicitly targeting the most error-prone transition areas.
...
TL;DR: A framework for automatically finding optimal visual token pruning configurations in vision-language models using gradient-based search and Pareto optimization.


<details>
  <summary>Details</summary>
Motivation: Existing visual token pruning methods use predefined configurations without determining computation-performance optimality, leading to suboptimal efficiency-accuracy trade-offs.

Method: Formulates pruning as Pareto configuration optimization, uses continuous relaxation with straight-through estimators for gradient-based search, solved via Augmented Lagrangian method with learnable kernel functions.

Result: Effectively approximates empirical Pareto frontier from grid search, generalizes across pruning methods and VLM architectures, reveals multi-step progressive pruning captures hierarchical compression structure for superior accuracy-efficiency trade-offs.

Conclusion: The framework automatically identifies optimal pruning configurations, outperforming predefined approaches and providing insights into VLMs' hierarchical compression patterns through layer-wise analysis.
...
TL;DR: Novel hierarchical spatiotemporal vector quantization framework for unsupervised skeleton-based temporal action segmentation achieves state-of-the-art performance and reduces segment length bias.


<details>
  <summary>Details</summary>
Motivation: To improve unsupervised skeleton-based temporal action segmentation by addressing limitations of existing approaches, particularly segment length bias, through better exploitation of both spatial and temporal information in a hierarchical manner.

Method: Proposes hierarchical spatiotemporal vector quantization with two levels: lower level associates skeletons with fine-grained subactions, higher level aggregates subactions into action-level representations. Extends to leverage both spatial (skeleton reconstruction) and temporal (timestamp recovery) information simultaneously.

Result: Outperforms non-hierarchical baselines, establishes new state-of-the-art on multiple benchmarks (HuGaDB, LARa, BABEL), and reduces segment length bias in unsupervised skeleton-based temporal action segmentation.

Conclusion: The hierarchical spatiotemporal vector quantization framework effectively improves unsupervised skeleton-based temporal action segmentation by better capturing both spatial and temporal information through multi-level clustering, leading to superior performance and reduced biases.
...
TL;DR: StreamCacheVGGT is a training-free framework for dense 3D reconstruction from video streams that improves cache management using cross-layer consistency scoring and hybrid compression to maintain geometric information under constant memory constraints.


<details>
  <summary>Details</summary>
Motivation: Existing O(1) frameworks for 3D reconstruction from video streams rely on "pure eviction" paradigms that suffer from significant information destruction due to binary token deletion and evaluation noise from localized, single-layer scoring, leading to loss of essential geometric context.

Method: Proposes two synergistic modules: 1) Cross-Layer Consistency-Enhanced Scoring (CLCES) that tracks token importance trajectories across Transformer hierarchy using order-statistical analysis to identify sustained geometric salience, and 2) Hybrid Cache Compression (HCC) that uses a three-tier triage strategy to merge moderately important tokens into retained anchors via nearest-neighbor assignment on key-vector manifold.
...
Conclusion: StreamCacheVGGT successfully addresses the limitations of pure eviction approaches by preserving essential geometric context through cross-layer consistency analysis and hybrid compression, establishing a new benchmark for constant-memory 3D reconstruction from video streams.
...
TL;DR: TokenGS improves 3D Gaussian Splatting by replacing depth-based Gaussian mean regression with direct 3D coordinate regression using self-supervised rendering, enabling encoder-decoder architecture with learnable tokens for better robustness and performance.


<details>
  <summary>Details</summary>
Motivation: The paper identifies that current Transformer-based approaches for 3D Gaussian Splatting have suboptimal design choices, particularly the practice of regressing Gaussian means as depths along camera rays, which limits performance and robustness.

Method: TokenGS proposes direct regression of 3D mean coordinates using only self-supervised rendering loss, moving from encoder-only to encoder-decoder architecture with learnable Gaussian tokens, decoupling primitive count from input resolution and view count.

Result: TokenGS achieves state-of-the-art feed-forward reconstruction on static and dynamic scenes, with improved robustness to pose noise and multiview inconsistencies, better geometry regularization, balanced 3DGS distribution, and emergent scene attribute recovery.

Conclusion: The proposed TokenGS method demonstrates superior performance over existing approaches by fundamentally redesigning how Gaussian means are regressed and enabling more flexible architecture design, while supporting efficient test-time optimization without degrading learned priors.
...
TL;DR: SegWithU is a post-hoc framework that adds a lightweight uncertainty head to frozen pretrained segmentation models, producing two uncertainty maps for calibration and error ranking without repeated inference.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation requires reliable uncertainty estimation for clinical decision support, but existing methods either need repeated inference (computationally expensive) or provide weak failure ranking with restrictive assumptions.

Method: Attaches a lightweight uncertainty head to a frozen pretrained segmentation backbone, taps intermediate features, and models uncertainty as perturbation energy in a compact probe space using rank-1 posterior probes. Produces two voxel-wise uncertainty maps: calibration-oriented for probability tempering and ranking-oriented for error detection.

Result: Strongest and most consistent single-forward-pass baseline across ACDC, BraTS2024, and LiTS datasets, achieving AUROC/AURC of 0.9838/2.4885, 0.9946/0.2660, and 0.9925/0.8193 respectively while preserving segmentation quality.

Conclusion: Perturbation-based uncertainty modeling is an effective and practical approach for reliability-aware medical segmentation, offering efficient single-pass uncertainty estimation without compromising segmentation performance.
...
TL;DR: VLMs struggle with emotion recognition due to head-class bias from web-scale pretraining and inability to capture fleeting micro-expressions; proposed solutions include alternative sampling strategies and multi-stage context enrichment.


<details>
  <summary>Details</summary>
Motivation: Despite VLMs' progress in visual tasks, they underperform in emotion recognition compared to specialized vision-only classifiers, raising the question of why they struggle with this fundamental human interaction capability.

Method: 1) Alternative sampling strategies to prevent head-class bias; 2) Multi-stage context enrichment strategy that converts intermediate frames into natural language summaries to preserve emotional trajectory while avoiding attentional dilution.

Result: Identified two critical VLM vulnerabilities: systematic collapse of rare emotions due to web-scale pretraining bias, and inability to capture fleeting micro-expressions due to sparse temporal sampling misalignment.

Conclusion: VLMs' emotion recognition limitations stem from pretraining biases and temporal representation constraints; proposed context enrichment and sampling strategies offer promising directions for improving affective understanding in multimodal systems.
...
TL;DR: GlobalSplat introduces a "align first, decode later" framework for compact 3D Gaussian Splatting that learns global latent scene representations to achieve efficient spatial allocation with minimal Gaussians.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods suffer from trade-offs between compactness, speed, and fidelity due to local, heuristic-driven allocation strategies that lack global scene awareness, leading to redundancy and fragile consistency as more views are added.

Method: GlobalSplat learns a compact global latent scene representation that encodes multi-view input and resolves cross-view correspondences before decoding explicit 3D geometry, using a coarse-to-fine training curriculum to prevent representation bloat.

Result: Achieves competitive novel-view synthesis on RealEstate10K and ACID with only 16K Gaussians (4MB footprint), significantly fewer than dense pipelines, and enables faster inference under 78ms in a single forward pass.

Conclusion: The "align first, decode later" approach enables globally consistent, compact 3D reconstructions without relying on pretrained backbones or dense baseline features, addressing fundamental limitations in spatial allocation for Gaussian Splatting.
...
TL;DR: Visual Anomaly Detection (VAD) for autonomous driving identifies unfamiliar objects not seen during training, providing pixel-level anomaly maps to alert drivers and guide attention to potential hazards, improving safety when encountering novel road conditions.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving systems degrade when encountering conditions outside their training distribution, creating physical risks for passengers and pedestrians. Unlike other domains, failures in autonomous driving have severe real-world consequences, necessitating robust anomaly detection.

Method: Benchmarked eight state-of-the-art VAD methods on AnoVox, the largest synthetic dataset for anomaly detection in autonomous driving. Evaluated across four backbone architectures including large networks and lightweight ones like MobileNet and DeiT-Tiny.
...
Conclusion: Visual Anomaly Detection represents a concrete step toward safer autonomous vehicle deployment by enabling systems to identify unfamiliar situations and alert drivers, ultimately improving protection for all road users.
...
TL;DR: AnimationBench is the first systematic benchmark for evaluating animation-style image-to-video generation, addressing gaps in existing realistic video benchmarks by focusing on stylized appearance, exaggerated motion, and character consistency.


<details>
  <summary>Details</summary>
Motivation: Existing video generation benchmarks are designed for realistic videos and struggle to evaluate animation-style generation with its unique characteristics like stylized appearance, exaggerated motion, and character-centric consistency. Current benchmarks also lack flexibility for open-domain content and custom evaluation needs.

Method: AnimationBench operationalizes the Twelve Basic Principles of Animation and IP Preservation into measurable evaluation dimensions, along with broader quality dimensions including semantic consistency, motion rationality, and camera motion consistency. It supports both standardized close-set evaluation for reproducible comparison and flexible open-set evaluation for diagnostic analysis, using visual-language models for scalable assessment.
...
Conclusion: AnimationBench provides the first systematic benchmark specifically designed for animation-style video generation, offering comprehensive evaluation capabilities that better capture the unique qualities of animation content compared to existing realism-focused benchmarks.
...
TL;DR: SLT is redefined as cross-modal reasoning rather than direct video-to-text conversion, using latent thoughts as an intermediate layer and plan-then-ground decoding for better coherence.


<details>
  <summary>Details</summary>
Motivation: Current SLT systems incorrectly assume sign language directly maps to spoken words, ignoring how signers create meaning dynamically using context, space, and movement.

Method: Introduces a reasoning-driven framework with ordered latent thoughts as an explicit middle layer, plus plan-then-ground decoding where the model first decides what to say then looks for video evidence.

Result: Consistent gains over existing gloss-free methods across several benchmarks, with a new large-scale gloss-free SLT dataset featuring stronger context dependencies.

Conclusion: SLT should be treated as cross-modal reasoning, not simple video-to-text conversion; the proposed framework with latent thoughts and plan-then-ground decoding improves translation quality.
...
TL;DR: RAD-2 is a unified generator-discriminator framework for autonomous driving planning that combines diffusion-based trajectory generation with RL-optimized reranking to improve closed-loop stability and safety.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based planners for autonomous driving suffer from stochastic instabilities and lack corrective negative feedback when trained with imitation learning alone, limiting their robustness in closed-loop interactions.

Method: Proposes RAD-2 with: 1) Diffusion-based generator for diverse trajectory candidates, 2) RL-optimized discriminator for reranking based on long-term driving quality, 3) Temporally Consistent Group Relative Policy Optimization for better credit assignment, 4) On-policy Generator Optimization for structured feedback, and 5) BEV-Warp simulation environment for efficient training.

Result: RAD-2 reduces collision rate by 56% compared to strong diffusion-based planners, with real-world deployment showing improved perceived safety and driving smoothness in complex urban traffic.

Conclusion: The unified generator-discriminator framework effectively addresses stochastic instabilities in diffusion planners by combining diverse trajectory generation with RL-based optimization, enabling more robust and safe autonomous driving in complex scenarios.
...
TL;DR: MM-WebAgent is a hierarchical agentic framework for multimodal webpage generation that coordinates AIGC tools through planning and self-reflection to solve style inconsistency and poor coherence in automated webpage creation.


<details>
  <summary>Details</summary>
Motivation: Current AIGC tools enable on-demand creation of images, videos, and visualizations for webpage design, but directly integrating them into automated generation leads to style inconsistency and poor global coherence as elements are generated in isolation.

Method: Proposes MM-WebAgent, a hierarchical agentic framework that coordinates AIGC-based element generation through hierarchical planning and iterative self-reflection, jointly optimizing global layout, local multimodal content, and their integration.

Result: Experiments show MM-WebAgent outperforms code-generation and agent-based baselines, especially on multimodal element generation and integration. The authors also introduce a benchmark and multi-level evaluation protocol for systematic assessment.

Conclusion: MM-WebAgent successfully addresses style inconsistency and coherence issues in automated webpage generation by coordinating AIGC tools through hierarchical planning and self-reflection, producing coherent and visually consistent webpages.
...
<details>
  <summary>Details</summary>
Motivation: Current image relighting methods lack precise and continuous control over multiple illumination attributes simultaneously. There's a need for a unified approach that can handle diverse lighting factors like intensity, color, ambient illumination, diffuse levels, and 3D light positions while maintaining realism.
...
Result: Achieves state-of-the-art quantitative and qualitative performance on both synthetic and real images. Handles challenging scenarios like controlling in-scene lighting fixtures, editing environment illumination with virtual light sources, placing lights within objects, and relighting transparent materials plausibly.

Conclusion: TokenLight demonstrates that attribute tokens enable precise, continuous control over multiple lighting attributes in image relighting. Remarkably, without explicit inverse rendering supervision, the model learns inherent understanding of light interaction with scene geometry, occlusion, and materials, producing convincing lighting effects in challenging scenarios.
...
TL;DR: LeapAlign is a fine-tuning method for flow matching models that enables efficient direct gradient propagation from reward to early generation steps by shortening long trajectories into two-step leaps, addressing memory and gradient issues in preference alignment.


<details>
  <summary>Details</summary>
Motivation: Direct gradient methods for aligning flow matching models with human preferences suffer from prohibitive memory costs and gradient explosion when backpropagating through long trajectories, making it difficult to update early generation steps that determine global image structure.

Method: LeapAlign shortens long trajectories into only two steps using two consecutive leaps that skip multiple ODE sampling steps and predict future latents in single steps. It randomizes start/end timesteps, assigns higher weights to trajectories consistent with long generation paths, and reduces weights of large-magnitude gradient terms instead of removing them.
...
Conclusion: LeapAlign provides an effective solution for fine-tuning flow matching models with human preferences by enabling efficient and stable direct gradient propagation to early generation steps, overcoming computational limitations of previous methods.
...
TL;DR: Bi-CMPStereo: A bidirectional cross-modal prompting framework for event-frame stereo matching that learns aligned representations and integrates complementary features from both modalities.


<details>
  <summary>Details</summary>
Motivation: Frame cameras have limited temporal resolution and motion blur in dynamic scenes, while event cameras offer high dynamic range but create a modality gap. The complementary characteristics make event-frame stereo promising for 3D perception under fast motion and challenging illumination, but existing methods marginalize domain-specific cues essential for cross-modal matching.

Method: Introduces Bi-CMPStereo, a bidirectional cross-modal prompting framework that learns finely aligned stereo representations within a target canonical space and integrates complementary representations by projecting each modality into both event and frame domains.

Result: Extensive experiments demonstrate that the approach significantly outperforms state-of-the-art methods in accuracy and generalization.

Conclusion: The bidirectional cross-modal prompting framework effectively bridges the modality gap between event and frame cameras, enabling robust stereo matching for reliable 3D perception in challenging conditions.
...
TL;DR: CART introduces a context-aware terrain adaptation controller that integrates proprioception and exteroception to overcome the Visual-Texture Paradox, improving legged robot stability on complex terrains.


<details>
  <summary>Details</summary>
Motivation: Current terrain adaptation methods for legged robots rely too heavily on vision-based prior experience, creating a Visual-Texture Paradox where what robots see doesn't match how terrain actually feels, leading to failures on complex off-road terrain.

Method: CART is a high-level controller using context-aware terrain adaptation that integrates proprioception (internal sensing) and exteroception (external sensing) from onboard sensors to develop robust terrain understanding.

Result: CART achieves 5% average success rate improvement over baselines in simulation, and improves stability by 45% and 24% in real-world experiments without increasing task completion time, evaluated using vibrational stability metrics.

Conclusion: Integrating proprioception and exteroception through context-aware adaptation effectively addresses the Visual-Texture Paradox, enabling more stable legged robot locomotion on complex terrains comparable to how animals combine multiple sensory modalities.
...
TL;DR: RoSLAC: Robust simultaneous localization and calibration for AMRs using ambient magnetic fields without requiring platform rotation


<details>
  <summary>Details</summary>
Motivation: Localization of autonomous mobile robots in GPS-denied environments is challenging. Infrastructure-based methods are costly, while sensor-based methods suffer from ambiguous features and occlusions. AMF-based localization is promising but requires accurate magnetometer calibration, which is impractical for large platforms using conventional rotation-based methods.
...
Result: Extensive evaluations in high-fidelity simulation and real-world environments show RoSLAC achieves high localization accuracy while maintaining low computational cost compared to state-of-the-art magnetometer calibration techniques.

Conclusion: RoSLAC provides a practical solution for AMF-based localization in GPS-denied environments by enabling simultaneous localization and calibration without the impractical requirement of rotating large platforms, making it suitable for real-world AMR applications.
...
TL;DR: SpaceMind is a modular, self-evolving VLM agent framework for autonomous on-orbit servicing that achieves robust performance across simulation and physical hardware with zero code modification.


<details>
  <summary>Details</summary>
Motivation: Autonomous on-orbit servicing requires embodied agents capable of visual perception, 3D spatial reasoning, and executing multi-phase tasks over extended horizons in challenging space environments.

Method: Modular framework with three extensible dimensions: skill modules with dynamic routing, MCP tools with configurable profiles, and injectable reasoning-mode skills. Features MCP-Redis interface for simulation/hardware compatibility and Skill Self-Evolution mechanism that distills experience into persistent skills without fine-tuning.

Result: 192 closed-loop runs across 5 satellites, 3 task types, and 2 environments (UE5 simulation and physical lab). 90-100% navigation success under nominal conditions; Prospective mode uniquely succeeded in degraded conditions. Self-evolution recovered from failure in 4/6 groups from single failed episode, improving inspection scores from 12 to 59/100. Real-world validation achieved 100% rendezvous success with zero code modification.

Conclusion: SpaceMind demonstrates a robust, modular approach to autonomous on-orbit servicing that successfully transfers from simulation to physical hardware without code changes, with self-evolution capabilities that improve performance through operational experience.
...
TL;DR: BIEVR-LIO: A robust LiDAR-inertial odometry system using high-resolution height image maps and map-informed point sampling for challenging environments with sparse geometry.


<details>
  <summary>Details</summary>
Motivation: Mobile robots increasingly operate in challenging environments with limited geometric features, causing traditional LiDAR-inertial odometry (LIO) to degrade or diverge due to insufficient constraints for point cloud registration.

Method: Proposes a high-resolution map representation storing surfaces as voxel-wise oriented height images for direct registration without intermediate geometric primitives. Also introduces map-informed point sampling to focus registration on geometrically informative regions, improving robustness while reducing computational cost.

Result: Demonstrates state-of-the-art performance in well-constrained scenes and substantial improvements in challenging scenarios where baseline methods diverge. Works across multiple sensors, platforms, and environments.

Conclusion: BIEVR-LIO provides robust odometry in geometrically sparse environments through novel map representation and sampling strategies, with the fine-grained geometry also supporting downstream tasks like elevation mapping for robot locomotion.
...
TL;DR: CooperDrive is a lightweight cooperative perception framework that enhances autonomous vehicle safety in occlusion/NLOS scenarios through object-level sharing and fusion, improving reaction time and safety metrics with minimal bandwidth/latency.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles struggle with occlusion and non-line-of-sight scenarios where limited visibility causes delayed reactions and increased collision risk, necessitating enhanced situational awareness.

Method: Uses existing BEV detector features for accurate vehicle pose estimation without extra encoders, reconstructs BEV representations, and implements lightweight object-level sharing/fusion between perception and planning stacks.

Result: Real-world tests show increased reaction lead time, higher minimum time-to-collision, improved stopping margin, with only 90 kbps bandwidth and 89 ms average end-to-end latency.

Conclusion: CooperDrive successfully transforms reactive driving into predictive, safer decisions through efficient cooperative perception while maintaining existing vehicle stacks and minimal resource overhead.
...
TL;DR: BC policies on PD-controlled robots fail due to gain-dependent error amplification; compliant-overdamped controllers minimize failure probability.


<details>
  <summary>Details</summary>
Motivation: Lack of nonasymptotic theory explaining how PD controller gains affect behavior cloning failure rates, despite empirical evidence that compliant-overdamped controllers improve success.

Method: Analyze error propagation through closed-loop PD dynamics using sub-Gaussian action errors, derive proxy matrix X_∞(K) governing failure tail, and establish factorization of failure probability into gain-dependent amplification index and validation loss.

Result: Closed-form stationary variance X_∞^c(α,β)=σ²α/(2β) shows monotonic dependence on stiffness and damping; compliant-overdamped regime tightest bound, stiff-underdamped loosest; provides first nonasymptotic explanation for empirical BC success patterns.

Conclusion: Controller gains fundamentally affect BC failure probability through error amplification; training loss alone insufficient for predicting closed-loop performance; compliant-overdamped controllers optimal for minimizing failure rates.
...
TL;DR: A continuous-time spline-based VIR fusion method that uses B-splines to parameterize pose trajectories, incorporates virtual anchors from UWB ranging, and jointly optimizes multi-sensor constraints for improved localization in challenging environments.


<details>
  <summary>Details</summary>
Motivation: VIO suffers from long-term drift without global constraints. UWB ranging can help but requires well-deployed anchors, which is difficult in narrow/low-power environments. Existing VIR fusion methods struggle with asynchronous multi-sensor sampling and balancing accuracy, consistency, and efficiency.
...
Result: Evaluations on public datasets and real-world experiments demonstrate effectiveness and practical potential of the proposed approach for improved localization in challenging environments.

Conclusion: The spline-based continuous-time state estimation method successfully addresses VIR fusion challenges by handling asynchronous multi-sensor sampling, improving range reliability through virtual anchors, and providing better balance between positioning accuracy, trajectory consistency, and fusion efficiency.
...
TL;DR: Using model-based deep reinforcement learning, biped robots with passive elements (springs) learned robust, energy-efficient walking/running by leveraging system attractors and stable limit cycles.


<details>
  <summary>Details</summary>
Motivation: To explore how passive body properties (embodiment) in biped robots can enable efficient locomotion learning through dynamic interactions with the environment, contrasting with traditional humanoid designs.

Method: Model-based deep reinforcement learning applied to two simulated biped models: one with passive elements (springs) and one without (standard humanoid). Training leveraged system attractors for convergence.

Result: Passive-element robots converged quickly to limit cycles but took longer to achieve high rewards. However, the resulting locomotion was more robust and energy-efficient than non-passive counterparts.

Conclusion: Passive body properties enable efficient acquisition of high-performance locomotion via stable limit cycles from body-ground interactions, highlighting the importance of embodiment for future embodied AI.
...
TL;DR: A multi-platform LiDAR dataset from an ICOS forest plot integrating UAV, terrestrial, and mobile laser scanning for 3D structural analysis and ecological research.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive reference dataset that supports calibration, benchmarking, and integration of 3D structural data with ecological observations and standard allometric models for forest ecosystem research.
...
TL;DR: DigiForest introduces a large-scale precision forestry system using autonomous robots and digital technologies for sustainable forest management, validated across European forests.


<details>
  <summary>Details</summary>
Motivation: Forests cover 40% of Europe's land and are crucial for climate neutrality and biodiversity goals, but current practices need innovation to meet sustainability challenges.

Method: Four-component system: 1) autonomous mobile robots (aerial, legged, marsupial) for tree-level data collection, 2) automated tree trait extraction for inventories, 3) Decision Support System for growth forecasting, and 4) low-impact selective logging with autonomous harvesters.
...
Conclusion: DigiForest represents a novel precision forestry approach that leverages digital technologies and robotics to support sustainable forest management and help achieve EU climate and biodiversity goals.
...
TL;DR: WAV model introduces latent-space planning for VLA agents, using world models and value functions to reason over future trajectories instead of direct action prediction, enabling better long-horizon decision making.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action models rely on direct action prediction, which lacks reasoning over long-horizon trajectories and evaluation of consequences, limiting performance in complex decision-making tasks.

Method: WAV model learns structured latent representations of future trajectories conditioned on visual observations and language instructions. It combines a world model for future state prediction with a trajectory value function for utility evaluation, and formulates action generation as inference in this latent space.

Result: WAV model consistently outperforms state-of-the-art methods in simulations and real-world experiments, achieving significant improvements in task success rate, generalization ability, and robustness, especially in long-horizon and compositional scenarios.

Conclusion: Latent-space inference enables efficient long-horizon decision making by reshaping search distributions toward feasible regions, addressing the exponential decay problem of direct action-space planning.
...
<details>
  <summary>Details</summary>
Motivation: Traditional discrete search for regrasp planning is brittle when reasoning about shared-grasp connectivity across intermediate poses. Need a more robust approach that can handle continuous pose optimization.

Method: Propose implicit multi-step regrasp planning using differentiable pose sequence connectivity metrics. Model grasp feasibility with Energy-Based Models (EBM), use energy additivity to create continuous energy landscape for pose-pair connectivity, enabling gradient-based optimization of intermediate poses. Introduce adaptive iterative deepening to automatically determine minimum number of intermediate steps.

Result: Cost formulation provides smooth and informative gradients, improving planning robustness. Generalizes to unseen grasp poses and cross-end-effector transfer (suction-trained model can guide parallel gripper manipulation). Adaptive deepening effectively finds minimum-step solutions.

Conclusion: Differentiable energy-based approach enables robust multi-step regrasp planning with automatic step determination and cross-end-effector generalization.
...
TL;DR: CAL2M is a plug-and-play framework that enables kilometer-level SLAM using Visual Geometry Foundation Models by addressing scale ambiguity and geometric distortions through an assistant eye, epipolar-guided correction, and anchor-based nonlinear sub-map alignment.


<details>
  <summary>Details</summary>
Motivation: Current approaches using linear transforms (Sim3/SL4) for VGFM-based SLAM are insufficient for kilometer-level scenarios, as they cannot model complex non-linear geometric distortions, leading to accumulated residuals, trajectory drift, and map divergence.

Method: CAL2M introduces: 1) an "assistant eye" using constant physical spacing prior to eliminate scale ambiguity without calibration; 2) epipolar-guided intrinsic and pose correction with online intrinsic search; 3) globally consistent mapping via anchor propagation and nonlinear sub-map alignment.

Result: The framework effectively addresses scale ambiguity, rectifies rotation/translation errors from inaccurate intrinsics, eliminates geometric misalignments through nonlinear transformations, and enables globally consistent reconstruction for large-scale SLAM.

Conclusion: CAL2M provides a calibration-free, plug-and-play solution for deploying VGFMs in kilometer-level SLAM by overcoming limitations of linear alignment methods through innovative geometric correction and nonlinear mapping strategies.
...
TL;DR: Switch: Hierarchical multi-skill system for humanoid robots enabling seamless skill transitions using Skill Graph, whole-body tracking policy, and online skill scheduler.


<details>
  <summary>Details</summary>
Motivation: Existing whole-body control approaches struggle with flexible transitions between distinct locomotion skills, creating safety concerns and practical limitations for humanoid robots.

Method: Three components: (1) Skill Graph establishing potential cross-skill transitions based on kinematic similarity, (2) whole-body tracking policy trained via deep reinforcement learning on the skill graph, (3) online skill scheduler performing graph search for optimal feasible paths during transitions.
...
Conclusion: Switch addresses the challenge of flexible skill transitions in humanoid locomotion, providing efficient, stable, and real-time execution of diverse locomotion skills through hierarchical multi-skill architecture.
...
TL;DR: Integrates graph-based pairwise consistency maximization (PCM) with ICP for 4D radar scan registration in feature-poor environments, using radar-adapted uncertainty scoring to reject outliers and improve robustness.


<details>
  <summary>Details</summary>
Motivation: 4D imaging radar faces challenges in scan registration due to sparse scans, noise, multipath reflections, and feature-poor environments like open-pit mines where lack of distinctive landmarks reduces correspondence reliability.

Method: Integrates graph-based PCM as outlier rejection within ICP loop, proposes radar-adapted pairwise distance-invariant scoring function incorporating anisotropic per-detection uncertainty from radar measurement model, uses greedy heuristic to find large clique in consistency graph.

Result: Reduces segment relative position error (RPE) by 29.6% on 1m segments and up to 55% on 100m segments compared to GICP baseline without PCM, evaluated on open-pit mine dataset with 4D imaging radar.

Conclusion: Method improves robustness when initial associations are heavily contaminated, suitable for online use due to greedy heuristic, intended for integration into localization pipelines for challenging environments.
...
TL;DR: Incorporating Radar Cross Section (RCS) physical behavior into 3D Gaussian modeling improves 4D mmWave radar scene representation and scan matching.


<details>
  <summary>Details</summary>
Motivation: 4D mmWave radars provide valuable RCS information that is often overlooked in current modeling approaches, despite its potential to enrich scene understanding and improve scan matching accuracy.

Method: Extends previous 3D Gaussian modeling and scan matching work by incorporating the physical behavior of Radar Cross Section (RCS) into the model.

Result: The proposed approach enriches summarized scene information and improves the scan matching process by leveraging previously unused RCS data.

Conclusion: Utilizing RCS information in 4D mmWave radar modeling provides significant benefits for robotics applications by enhancing scene representation and scan matching performance.
...
TL;DR: Integrated robotic waste segregation with YOLOv8 detection and optimized bio-digestor using PSO algorithm achieves 98% sorting accuracy and efficient biological conversion for smart waste management.


<details>
  <summary>Details</summary>
Motivation: Rapid urbanization and population growth create municipal solid waste management challenges, requiring smarter automated solutions to reduce manual intervention and improve efficiency.

Method: Combines robotic waste segregation (MyCobot 280 Jetson Nano arm with YOLOv8 object detection and ROS path planning) with optimized bio-digestor system using multiple sensors and Particle Swarm Optimization algorithm with regression model for dynamic parameter adjustment.

Result: System achieves 98% sorting accuracy for waste classification into four categories and demonstrates highly efficient biological conversion under dynamic conditions.

Conclusion: The integrated framework provides scalable, intelligent, and practical solution for modern waste management suitable for both residential and industrial applications.
...
TL;DR: HRDexDB is a large-scale multi-modal dataset of high-fidelity dexterous grasping sequences featuring both human and robotic hands, with comprehensive 3D motion ground truth, tactile signals, and synchronized video across 100 objects and 1.4K trials.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack comprehensive grasping trajectories across both human and diverse robotic hand embodiments, limiting research in cross-domain dexterous manipulation and multi-modal policy learning.

Method: Created a large-scale dataset using state-of-the-art vision methods and a dedicated multi-camera system to capture high-precision spatiotemporal 3D ground-truth motion, synchronized with high-resolution tactile signals, multi-view video, and egocentric video streams across 100 diverse objects.

Result: HRDexDB contains 1.4K grasping trials (including both successes and failures) with closely aligned captures of human dexterity and robotic execution on the same target objects under comparable grasping motions, providing visual, kinematic, and tactile modalities.

Conclusion: HRDexDB serves as a foundational benchmark for multi-modal policy learning and cross-domain dexterous manipulation by providing the first comprehensive dataset that bridges human and robotic grasping across diverse embodiments.
...
TL;DR: GNPF-kCT: A novel online POMDP solver using neural process filtering and k-center clustering for efficient 3D object search in complex indoor environments with mobile robots.
...
Method: Proposes GNPF-kCT: Monte Carlo Tree Search with belief tree reuse for growing state space, neural process network to filter useless actions, k-center clustering for action space discretization, and modified UCB informed by belief differences and action values. Includes guessed target object strategy for limited information scenarios.

Result: Extensive Gazebo simulations with Fetch and Stretch robots show faster and more reliable target localization than POMDP-based baselines and SOTA non-POMDP solvers (including LLM-based methods) under same computational constraints. Real-world tests confirm practical applicability.

Conclusion: GNPF-kCT effectively addresses high-dimensional POMDP object search challenges in complex 3D environments, outperforming existing methods while maintaining computational efficiency and practical applicability in real-world settings.
...
TL;DR: MHHTOF framework for assistive planning in visually impaired scenarios balances comfort, safety, generalization, and interpretability through hybrid heuristic trajectory optimization with momentum constraints and DRL refinement.


<details>
  <summary>Details</summary>
Motivation: Existing methods for assistive planning in visually impaired scenarios struggle with multi-objective optimization (balancing comfort and safety), generalization to different scenarios, and providing interpretable decision-making for users.

Method: Proposes Momentum-Constrained Hybrid Heuristic Trajectory Optimization Framework (MHHTOF) with: 1) Heuristic Trajectory Sampling Cluster (HTSC) with Momentum-Constrained Trajectory Optimization (MTO) to suppress abrupt velocity/acceleration changes, 2) residual-enhanced deep reinforcement learning module for trajectory refinement and generalization, and 3) dual-stage cost modeling mechanism (DCMM) with Frenet space consistency costs and Cartesian space adaptive weights for interpretability.

Result: Framework converges in nearly half the iterations of baselines, achieves lower and more stable costs, and demonstrates stable velocity/acceleration curves with reduced risk in complex dynamic scenarios.

Conclusion: MHHTOF provides advantages in robustness, safety, and efficiency for assistive planning in visually impaired scenarios, effectively addressing multi-objective optimization, generalization, and interpretability challenges.
...
TL;DR: DEX-Mouse is a portable, calibration-free hand-held teleoperation interface with force feedback for dexterous hand manipulation, costing under $150 and enabling robot-aligned data collection.


<details>
  <summary>Details</summary>
Motivation: Existing methods for dexterous hand manipulation data collection have limitations: simulation has sim-to-real gaps, video methods have retargeting problems, MoCap gloves require calibration and lack portability, and portable alternatives lack cross-platform compatibility.

Method: Developed DEX-Mouse, a portable hand-held teleoperation interface built from commercial off-the-shelf components with integrated kinesthetic force feedback. Features operator-agnostic design requiring no calibration, and supports attached configuration where robot hand mounts directly on operator's forearm.

Result: In user studies, operators achieved 86.67% task completion rate with attached configuration. Attached configuration reduced perceived workload compared to spatially separated setups across all interfaces. Complete hardware/software stack is open-sourced.

Conclusion: DEX-Mouse provides an affordable, portable, calibration-free solution for dexterous hand teleoperation that enables robot-aligned data collection and reduces operator workload, with open-source availability to facilitate adoption.
...
TL;DR: DockAnywhere: A framework that lifts single demonstrations to diverse docking points to improve mobile manipulation viewpoint generalization under docking variability.


<details>
  <summary>Details</summary>
Motivation: Real-world mobile manipulation suffers from view generalization problems due to shifts in docking points. Existing two-stage approaches (navigate then manipulate) fail when docking points vary, limiting policy generalization.

Method: Decouples docking-dependent base motions from contact-rich manipulation skills, samples feasible docking proposals under constraints, generates trajectories via structure-preserving augmentation, and synthesizes visual observations using point cloud spatial editing.
...
Conclusion: DockAnywhere significantly enhances mobile manipulation policy generalization in real-world deployment by addressing viewpoint generalization under docking variability through demonstration lifting.
...
TL;DR: CAVERS dataset provides multimodal sensor data from natural karstic caves with mm-accurate ground truth poses, addressing the scarcity of public datasets for challenging cave environments.


<details>
  <summary>Details</summary>
Motivation: Natural karstic caves present unique perception challenges (irregular geometry, reflective wet surfaces, near-zero light, complex branching) that differ from mines/tunnels, but public datasets for such environments are scarce with limited sensing modalities and diversity.

Method: Collected 24 sequences (335 GB) in two structurally distinct rooms of Cueva de la Victoria, Spain, using Intel RealSense D435i RGB-D-I camera, Optris PI640i near-IR thermal camera, and Velodyne VLP-16 LiDAR, operated both handheld and on a wheeled rover under darkness and artificial illumination. Most sequences include mm-accurate 6-DoF ground truth pose/velocity at 120 Hz from an Optirack motion capture system installed inside the cave.

Result: Created comprehensive multimodal cave dataset with precise ground truth. Benchmarked seven state-of-the-art SLAM/odometry algorithms (visual, visual-inertial, thermal-inertial, LiDAR-based) and a 3D reconstruction pipeline, demonstrating dataset's usability for algorithm evaluation.

Conclusion: CAVERS dataset fills a critical gap by providing high-quality multimodal sensor data with accurate ground truth from challenging natural cave environments, enabling development and benchmarking of perception and navigation algorithms for autonomous cave exploration.
...
TL;DR: Two-stage planning framework for multi-UAV payload transport in cluttered environments: Stage I uses Enhanced Tube-RRT* for safe virtual tube generation, Stage II uses convex optimization for smooth payload trajectory, validated with geometric control.


<details>
  <summary>Details</summary>
Motivation: Address planning challenges for multi-UAV rigid-payload transportation systems in densely cluttered environments, where traditional methods struggle with obstacle avoidance, cable-induced oscillations, and complex dynamics.

Method: Two-stage framework: Stage I - Enhanced Tube-RRT* algorithm with active hybrid sampling and adaptive expansion for virtual tube generation; Stage II - convex quadratic program considering payload dynamics, cable tension, and collision constraints for trajectory optimization.

Result: Enhanced Tube-RRT* achieves higher success rate and effective sampling rate than STube-RRT* and AETube-RRT*, with shorter optimal path and smaller cumulative turning angle. The framework produces smooth, collision-free payload trajectories validated through centralized geometric control.

Conclusion: The proposed two-stage planning framework provides an effective solution for multi-UAV payload transportation in cluttered environments, successfully addressing cable-induced oscillations, collision avoidance, and complex dynamics through integrated sampling, optimization, and control approaches.
...
TL;DR: NEATNC improves NEAT algorithm for dynamic path planning by incorporating biological navigation cells (place, grid, head direction, border, speed cells) as inputs to evolve recurrent neural networks representing hippocampus function.


<details>
  <summary>Details</summary>
Motivation: To enhance NEAT algorithm performance in dynamic path planning by drawing inspiration from biological spatial navigation systems in the brain, specifically the hippocampus and its specialized cells.

Method: Developed NEATNC (Neuro-Evolution of Augmenting Topology guided Navigation Cells) that uses biological navigation cells as inputs and evolves recurrent neural networks to represent hippocampal function for spatial navigation.

Result: The algorithm shows improved performance in both static and dynamic environments, demonstrating NEAT's adaptability to complex scenarios and highlighting the utility of biological theories in computational approaches.

Conclusion: The approach is well-suited for real-time dynamic path planning applications in robotics and games, successfully bridging biological navigation principles with evolutionary computation techniques.
...
TL;DR: Dual pose-graph SLAM system for drone racing that fuses odometry with semantic gate detections, reducing drift by 56-74% compared to VIO alone while maintaining real-time performance.


<details>
  <summary>Details</summary>
Motivation: Autonomous drone racing requires robust real-time localization under extreme conditions (high-speed, aggressive maneuvers) where existing visual SLAM systems struggle with motion blur and feature instability, and don't exploit the structured racing environment.

Method: Dual pose-graph architecture: temporary graph accumulates multiple gate observations between keyframes and optimizes them into single refined constraints, then promotes to persistent main graph. Sensor-agnostic design validated with monocular visual-inertial odometry and visual gate detections.

Result: 56-74% reduction in ATE compared to standalone VIO on TII-RATM dataset; dual-graph achieves 10-12% higher accuracy than single-graph baseline at same computational cost; reduces drift by up to 4.2m per lap in A2RL competition.

Conclusion: The dual pose-graph architecture effectively fuses odometry with semantic detections for robust drone racing localization, preserving information richness while maintaining real-time performance, demonstrating practical deployment in competitive racing scenarios.
...
TL;DR: Benchmark of 17 coverage path planning heuristics on 10,000 irregular hexagonal graphs shows that explicit shortest-path reconnection solves relaxed coverage but rarely produces zero-revisit tours, with best Hamiltonian success at 79.0% using a Warnsdorff variant with specific tie-breaking and residual-degree policy.


<details>
  <summary>Details</summary>
Motivation: Coverage path planning on irregular hexagonal grids is important for maritime surveillance, search and rescue, and environmental monitoring, but existing methods lack standardized comparison on realistic irregular geometries rather than just rectangular grids.

Method: Created reproducible benchmark with 10,000 Hamiltonian-feasible instances on irregular hexagonal graphs with maritime-motivated morphologies, evaluated 17 heuristics from 7 families using metrics including Hamiltonian success, coverage success, revisits, path length, heading changes, and CPU latency.

Result: Heuristics with explicit shortest-path reconnection reliably solve relaxed coverage but almost never produce zero-revisit tours; best Hamiltonian success is 79.0% using Warnsdorff variant with index-based tie-break and terminal-inclusive residual-degree policy; implementation details significantly affect performance on sparse geometric graphs with bottlenecks.

Conclusion: The benchmark provides controlled testbed for heuristic analysis, showing that underreported implementation details materially affect performance on sparse geometric graphs, with residual-degree definition being the dominant design choice when endpoint is reserved until final move.
...
TL;DR: HiST-AT: Hierarchical spatiotemporal action tokenizer for in-context imitation learning using multi-level vector quantization with spatial and temporal cues


<details>
  <summary>Details</summary>
Motivation: To improve in-context imitation learning by developing a more effective action representation method that captures both spatial and temporal information in a hierarchical manner

Method: Two-level hierarchical vector quantization: lower level for fine-grained subclusters, higher level for clusters. Extended to HiST-AT with spatiotemporal cues that simultaneously recovers actions and timestamps through multi-level clustering

Result: Outperforms non-hierarchical counterparts, establishes new state-of-the-art performance on multiple simulation and real robotic manipulation benchmarks

Conclusion: HiST-AT is an effective hierarchical spatiotemporal action tokenizer that significantly advances in-context imitation learning capabilities
...
TL;DR: Vision-based human pose estimation with conformal prediction guarantees for safe human-robot collaboration


<details>
  <summary>Details</summary>
Motivation: Need for certifiably safe human-robot collaboration with probabilistic confidence guarantees in vision-based pose estimation

Method: Combines aleatoric uncertainty estimation with OOD detection, proposes conformal prediction sets for human motion predictions

Result: Framework provides high, valid confidence guarantees for human motion predictions in collaborative settings

Conclusion: Proposed framework enables certifiably safe human-robot collaboration through conformal prediction guarantees
...
TL;DR: This paper formalizes the abstract sim2real problem where simulators leave out key task details, and proposes a method using real-world data to correct abstract simulator dynamics for successful policy transfer.


<details>
  <summary>Details</summary>
Motivation: As robots are deployed in increasingly complex domains, simulators will likely fail to model all relevant details of target tasks, motivating the study of sim2real with abstract simulators that leave out key task details.

Method: The paper formalizes the abstract sim2real problem using state abstraction from RL literature, showing that abstract simulators can be grounded using history of states. It then introduces a method that uses real-world task data to correct the dynamics of the abstract simulator.

Result: The proposed method enables successful policy transfer both in sim2sim and sim2real evaluations, demonstrating effective transfer from abstract simulators to real-world tasks.

Conclusion: The work provides a formal framework for abstract sim2real problems and shows that correcting abstract simulator dynamics using real-world data enables successful policy transfer, addressing the challenge of incomplete simulator realism in complex domains.
...
### [126] [NuHF Claw: A Risk Constrained Cognitive Agent Framework for Human Centered Procedure Support in Digital Nuclear Control Rooms](https://arxiv.org/abs/2604.14160)
*Xingyu Xiao,Jiejuan Tong,Jun Sun,Zhe Sui,Peng Chen,Jingang Liang,Haitao Wang*

Main category: cs.AI

TL;DR: NuHF Claw is a persistent cognitive-risk agent framework for digital nuclear control rooms that integrates cognitive state inference with safety assessment to enable risk-governed human-centered autonomy.


<details>
  <summary>Details</summary>
Motivation: Existing human reliability analysis approaches fail to address complex soft-control behaviors and cognitive risks in digitized nuclear control rooms, while current AI systems risk hallucinated reasoning and weakened human authority in safety-critical environments.

Method: A risk-constrained agent runtime that couples cognitive state inference (workload, situational awareness) with probabilistic safety assessment and dynamic human error probability prediction to regulate autonomous system behavior in real time.

Result: Experimental validation on a high-fidelity digital control room simulator shows the framework can anticipate cognitive degradation, constrain unsafe autonomous recommendations, and provide risk-aware guidance while preserving human decision authority.

Conclusion: The framework enables a shift from automation-driven operation to cognition-aware autonomy, offering a principled pathway for safe integration of intelligent agents in next-generation nuclear control environments.
...
### [127] [Simulating Human Cognition: Heartbeat-Driven Autonomous Thinking Activity Scheduling for LLM-based AI systems](https://arxiv.org/abs/2604.14178)
*Hong Su*

Main category: cs.AI

TL;DR: Heartbeat-driven autonomous thinking activity scheduling enables LLM agents to proactively self-regulate cognitive activities through periodic scheduling rather than reactive error correction.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents suffer from rigid, reactive control flows that limit adaptability and efficiency, relying on fixed pipelines or failure-triggered reflection that causes impulsive actions and only corrects errors after they occur.

Method: Introduces a periodic "heartbeat" mechanism that orchestrates dynamic cognitive modules (Planner, Critic, Recaller, Dreamer) and learns when to engage specific thinking activities based on temporal patterns and historical context, with meta-learning for continual policy adaptation.

Result: The approach effectively learns to schedule cognitive activities based on historical data and can autonomously integrate new thinking modules, demonstrating improved adaptability over traditional reactive approaches.

Conclusion: Heartbeat-driven scheduling provides a proactive, adaptive framework for LLM agent self-regulation that mirrors human cognitive rhythms, enabling continuous optimization of cognitive strategies without structural reengineering.
...
### [128] [Fun-TSG: A Function-Driven Multivariate Time Series Generator with Variable-Level Anomaly Labeling](https://arxiv.org/abs/2604.14221)
*Pierre Lotte,André Péninou,Olivier Teste*

Main category: cs.AI

TL;DR: Fun-TSG is a customizable time series generator that addresses limitations in existing anomaly detection benchmarks by providing fine-grained annotations, explicit dependencies, and full transparency in data generation.


<details>
  <summary>Details</summary>
Motivation: Current benchmark datasets for multivariate time series anomaly detection lack fine-grained anomaly annotations, explicit intervariable and temporal dependencies, and insight into underlying generative mechanisms, hindering development and rigorous comparison of detection models.

Method: Introduces Fun-TSG, a fully customizable time series generator that supports both automated generation (based on randomly sampled dependency structures and anomaly types) and manual generation (through user-defined equations and anomaly configurations).

Result: Provides full transparency over the data generation process with ground-truth anomaly labels at variable and timestamp levels, enabling creation of diverse, interpretable, and reproducible benchmarking scenarios.

Conclusion: Fun-TSG addresses critical gaps in anomaly detection evaluation by supporting fine-grained performance analysis for both classical and modern models through customizable, transparent data generation.
...
### [129] [Interpretable and Explainable Surrogate Modeling for Simulations: A State-of-the-Art Survey and Perspectives on Explainable AI for Decision-Making](https://arxiv.org/abs/2604.14240)
...
TL;DR: Survey paper bridging surrogate modeling and explainable AI (XAI) for complex systems simulation, mapping XAI techniques to surrogate workflows and proposing research agenda for embedded explainability.


<details>
  <summary>Details</summary>
Motivation: Surrogate models inherit opacity from black-box simulators, while XAI methods struggle with engineering constraints like correlated inputs and dynamical systems. Despite complementarity, these fields have evolved separately, creating a need to reconnect them for better insight extraction.

Method: State-of-the-art survey providing structured perspective mapping XAI techniques onto surrogate modeling workflows. Uses illustrative applications from equation-based simulations and agent-based modeling, surveying broad spectrum of techniques for revealing interactions and supporting comprehension.

Result: Synthesis of XAI-surrogate integration approaches highlighting strengths for interaction revelation and human understanding. Identification of pressing challenges including explainability of dynamical systems and mixed-variable systems handling.

Conclusion: Proposes research agenda to make explainability a core, embedded element of simulation-driven workflows from model construction through decision-making, transforming opaque emulators into explainable tools for extracting actionable insights from complex system behaviors.
...
### [130] [Formalizing Kantian Ethics: Formula of the Universal Law Logic (FULL)](https://arxiv.org/abs/2604.14254)
*Taylor Olson*

Main category: cs.AI

TL;DR: The paper proposes a formal logic called FULL to implement Kantian ethics for AI moral reasoning, addressing limitations of current approaches that encode human moral intuition as axioms.


<details>
  <summary>Details</summary>
Motivation: Current approaches to building Artificial Moral Agents (AMAs) encode human moral intuition as action axioms (e.g., "do not harm"), but this has two key limitations: (1) it ignores the agent's purposes in performing actions, and (2) assumes humans can fully enumerate moral intuitions. The paper aims to address these limitations through formal Kantian ethics.

Method: The authors develop a multi-sorted quantified modal logic called Formula of the Universal Law Logic (FULL), which formalizes Kant's first formulation of the categorical imperative (Formula of the Universal Law or FUL) along with concepts like causality and agency. This provides a procedural approach to moral reasoning rather than encoding specific moral axioms.

Result: The FULL logic successfully reasons about and evaluates agents' actions for specific purposes across three cases from Kantian ethics, without requiring built-in moral intuition. It demonstrates this capability when provided with sufficient non-normative background knowledge.

Conclusion: FULL represents a contribution toward more robust and autonomous Artificial Moral Agents by providing a formal procedural approach to moral reasoning based on Kantian ethics, rather than relying on enumerated moral intuitions. It also advances the formal understanding of Kantian ethics itself.
...
### [131] [GFT: From Imitation to Reward Fine-Tuning with Unbiased Group Advantages and Dynamic Coefficient Rectification](https://arxiv.org/abs/2604.14258)
*Wangjie Gan,Miao Pan,Linbo Xi,Wenqi Zhang,Jintao Chen,Jianwei Yin,Xuhong Zhang*

Main category: cs.AI

TL;DR: GFT is a unified post-training framework that addresses SFT's limitations by using group advantage learning and dynamic coefficient rectification, outperforming SFT-based methods and integrating better with RL.


<details>
  <summary>Details</summary>
Motivation: SFT has intrinsic limitations: it's like policy gradient with sparse rewards and unstable weighting, causing single-path dependency, entropy collapse, and gradient explosion. Need to unify efficient knowledge injection with robust generalization.

Method: Group Fine-Tuning (GFT) with two mechanisms: 1) Group Advantage Learning - constructs diverse response groups and uses normalized contrastive supervision to alleviate reward sparsity; 2) Dynamic Coefficient Rectification - adaptively bounds inverse-probability weights to stabilize optimization while preserving knowledge injection.
...
Conclusion: GFT provides a unified post-training framework that addresses SFT's intrinsic limitations through group-based contrastive learning and stabilized optimization, enabling better knowledge injection and generalization for LLMs.
...
### [132] [Seeing Through Experts Eyes A Foundational Vision Language Model Trained on Radiologists Gaze and Reasoning](https://arxiv.org/abs/2604.14316)
...
<details>
  <summary>Details</summary>
Motivation: Current vision-language models for chest X-ray interpretation lack clinical utility because they don't emulate how radiologists visually examine images, often missing critical findings and diverging from established diagnostic workflows like the ABCDEF approach.

Method: GazeX incorporates radiologists' gaze trajectories and fixation patterns from eye-tracking data (30,000+ gaze key frames from 5 radiologists) during pretraining to learn the spatial and temporal structure of expert attention and integrate observations in clinically meaningful sequences.

Result: GazeX demonstrates more accurate, interpretable, and expert-consistent outputs across radiology report generation, disease grounding, and visual question answering, using 231,835 radiographic studies, 780,014 QA pairs, and 1,162 image-sentence pairs with bounding boxes.

Conclusion: Learning through expert eye-tracking provides a practical approach to creating more trustworthy, explainable, and diagnostically robust AI systems for radiology, enabling verifiable evidence artifacts and safe human-AI collaboration.
...
### [133] [Mistake gating leads to energy and memory efficient continual learning](https://arxiv.org/abs/2604.14336)
*Aaron Pache,Mark CW van Rossum*

Main category: cs.AI

TL;DR: Memorized mistake-gated learning reduces synaptic updates by 50-80% by only updating on current and past classification errors, inspired by human negativity bias and error-related negativity.


<details>
  <summary>Details</summary>
Motivation: Synaptic plasticity is metabolically expensive in biological systems, yet artificial neural networks typically update parameters on every sample, even correctly classified ones. This inefficiency contrasts with biological systems that conserve energy while continuously learning.

Method: Proposes 'memorized mistake-gated learning' - a biologically plausible plasticity rule where synaptic updates are strictly gated by current and past classification errors. The algorithm tracks mistakes and only updates when errors occur, reducing unnecessary computations.

Result: Reduces the number of network updates needed by 50-80%. Particularly effective for incremental learning (new knowledge on pre-existing background) and online learning with replay buffers (reduces storage requirements). Implementation is simple with few lines of code, no additional hyperparameters, and negligible computational overhead.

Conclusion: Mistake-gated learning provides an energy-efficient, biologically relevant modification to standard learning rules that is well-suited for continual learning scenarios, balancing computational efficiency with effective knowledge acquisition.
...
TL;DR: Credo is a framework for agentic AI that uses declarative beliefs and policies instead of imperative control loops, enabling adaptive, auditable, and composable execution through a semantic control plane.


<details>
  <summary>Details</summary>
Motivation: Current agentic AI systems rely on opaque, brittle imperative control loops with ephemeral memory and prompt-embedded logic, making behavior difficult to verify and adapt to evolving conditions in long-lived decision-making scenarios.

Method: Credo represents semantic state as beliefs and regulates behavior using declarative policies defined over these beliefs, implemented through a database-backed semantic control plane that separates control logic from pipeline code.

Result: The framework enables dynamic behavior adaptation (model selection, retrieval, corrective re-execution) without changing underlying pipeline code, demonstrated in a decision-control scenario where beliefs and policies declaratively guide critical execution choices.

Conclusion: Credo provides a more transparent, verifiable, and adaptable approach to agentic AI systems by using declarative belief-policy architectures instead of traditional imperative control loops, supporting long-lived, stateful decision-making in evolving environments.
...
TL;DR: Routing topology in sparse MoE models doesn't determine language modeling quality - different routing methods achieve statistically equivalent perplexity, with the real advantage being only ~1.2%.


<details>
  <summary>Details</summary>
Motivation: To investigate whether increasingly sophisticated routing mechanisms in sparse Mixture-of-Experts architectures actually determine language modeling quality, or if simpler routing methods can achieve similar performance.

Method: Built geometric MoE (ST-MoE) using cosine-similarity routing against learned centroids in low-dimensional space (d=64), requiring 80% fewer routing parameters. Conducted 62 controlled experiments on WikiText-103 with 76-84M parameters trained to convergence (50K steps, 1.64B tokens). Compared five cosine-routing variants, hash routing, random-fixed routing, and top-1 routing.

Result: Routing topology doesn't determine asymptotic perplexity: five cosine-routing variants are statistically equivalent within 1-PPL margin (TOST, p<0.05). The finding extends to hash, random-fixed, and top-1 routing. Standard linear router with 5.3× more routing parameters reaches PPL 32.76, but iso-parameter cosine routing closes 67% of this gap - the true mechanism advantage is only ~1.2%. Multi-hop updates are collinear (cos=0.805), implementing magnitude amplification rather than compositional reasoning.

Conclusion: Sophisticated routing topologies in MoE models don't significantly impact language modeling quality - simpler routing methods achieve similar performance. The practical payoff is that zero-shot relative-norm halting can save 25% of MoE FLOPs with minimal PPL increase (+0.12%).
...
TL;DR: Pneuma-Seeker is a system that makes users' information needs explicit as relational specifications for iterative refinement, data discovery, and provenance-aware execution using LLMs as transparent collaborators.


<details>
  <summary>Details</summary>
Motivation: Data analysts often start with vague questions and need to refine them iteratively while exploring relational data. Current approaches lack support for making information needs explicit and inspectable during this refinement process.

Method: Pneuma-Seeker reifies users' information needs as explicit, inspectable relational specifications. It enables iterative refinement, targeted data discovery, and provenance-aware execution, leveraging LLMs as transparent, interactive analytical collaborators rather than opaque answer engines.

Result: Through two real-world procurement use cases, the system demonstrates how it effectively supports iterative refinement of information needs and enables LLMs to serve as transparent analytical collaborators.

Conclusion: Pneuma-Seeker successfully transforms how analysts work with relational data by making information needs explicit and inspectable, enabling a more collaborative and transparent analytical process with LLMs rather than treating them as black-box answer engines.
...
TL;DR: MoE experts are monosemantic specialists with inspectable specialization through cosine-similarity routing, enabling causal validation and zero-overhead inference control.


<details>
  <summary>Details</summary>
Motivation: While MoE models scale parameters efficiently, the specialization of individual experts remains opaque despite routing topology being quality-neutral. The paper aims to demonstrate that expert identity is causally meaningful and can be made interpretable.

Method: Four evidence lines: 1) Semantic Dictionary via expert output projection, 2) Frequency-to-syntax gradient analysis with Zipf-confound controls, 3) Causal interventions (steering, suppressing, rewriting) to validate expert roles, 4) Comparison of cosine vs linear routing for geometric transparency.

Result: 15% of experts are monosemantic specialists spanning 10 semantic categories. Routing shows early layers separate by word frequency, deeper layers by syntactic class. Causal interventions confirm expert roles with significant probability changes. Cosine routing provides geometric transparency while linear routing supports comparable steering.

Conclusion: MoE expert-level specialization is a first-class interpretability primitive: architecturally monosemantic, causally validated, and controllable at inference with zero overhead, with cosine routing offering unique geometric transparency.
...
<details>
  <summary>Details</summary>
Motivation: Need for better RL control design for complex tasks that can handle sophisticated requirements and guide training more effectively than traditional reward shaping methods.

Method: Extends Reward Machines with Signal Temporal Logic formulas for event generation, enabling more efficient reward representation and training guidance. Uses STL online monitoring algorithms for implementation.

Result: Demonstrated framework effectiveness through three case studies (minigrid, cart-pole, and high-way environments) with non-trivial tasks.

Conclusion: The proposed STL-enhanced Reward Machine framework provides a powerful approach for RL control design that can handle complex task specifications while improving training efficiency and convergence.
...
TL;DR: AIBuildAI is an AI agent system that automates the full AI model development process from task description to deployable model, outperforming existing AutoML methods and matching expert human performance.
...
Method: Hierarchical agent architecture with a manager coordinating three specialized LLM-based sub-agents: designer (modeling strategy), coder (implementation/debugging), and tuner (training/optimization). Each sub-agent uses multi-step reasoning and tool use.

Result: AIBuildAI achieves 63.1% medal rate on MLE-Bench (Kaggle-style tasks across visual, textual, time-series, and tabular modalities), ranking first and outperforming all baselines while matching highly experienced AI engineers.

Conclusion: Hierarchical agent systems can automate the complete AI model development process, suggesting a pathway toward broadly accessible AI development with minimal human intervention.
...
TL;DR: AI assistant interventions based on policy-value discrepancies outperform optimal action recommendations in human decision-making tasks.


<details>
  <summary>Details</summary>
Motivation: Current AI assistants often recommend optimal actions assuming perfect human execution, but humans may fail to follow optimal sequences, reducing overall performance. There's a need for interventions that account for human suboptimality.

Method: Propose value-aware interventions using Bellman equation principles: identify policy-value inconsistencies where human actions don't maximize immediate reward plus next state value. Formalize in MDP with intervention budget, derive optimal single-intervention strategy, and develop tractable approximation for multiple interventions.

Result: In chess simulations, approach outperforms Stockfish engine interventions across settings. Human study with 20 players/600 games shows significant improvement for low/mid-skill players while matching expert-engine interventions for high-skill players.

Conclusion: Policy-value inconsistency provides effective intervention criterion that adapts to human skill levels, improving human-AI collaboration in sequential decision-making.
...
TL;DR: RUMS is a novel memory selection method for LLM personalization that uses mutual information to select memory items that reduce response uncertainty, outperforming similarity-based methods while being more computationally efficient.


<details>
  <summary>Details</summary>
Motivation: Current LLM personalization methods select user memory items based on semantic similarity to input queries, ignoring how these features actually affect the model's response distribution, leading to suboptimal memory selection.

Method: Proposes Response-Utility optimization for Memory Selection (RUMS), which selects user memory items by measuring mutual information between memory subsets and model outputs, identifying items that reduce response uncertainty and sharpen predictions.

Result: RUMS achieves more principled memory selection aligned with human selection compared to SOTA methods and models 400× larger, improves response quality, and reduces computational cost by up to 95%.

Conclusion: Information-theoretic memory selection via mutual information provides a more effective and efficient approach to LLM personalization than similarity-based methods, better aligning with human judgment while reducing computational overhead.
...
### [142] [Evo-MedAgent: Beyond One-Shot Diagnosis with Agents That Remember, Reflect, and Improve](https://arxiv.org/abs/2604.14475)
*Weixiang Shen,Bailiang Jian,Jun Li,Che Liu,Johannes Moll,Xiaobin Hu,Daniel Rueckert,Hongwei Bran Li,Jiazhen Pan*

Main category: cs.AI

TL;DR: Evo-MedAgent adds a self-evolving memory module to medical LLM agents, enabling inter-case learning at test time without training, improving diagnostic accuracy on chest X-rays.


<details>
  <summary>Details</summary>
Motivation: Current tool-augmented LLM agents for medical imaging solve cases in isolation without accumulating experience across cases, correcting recurrent mistakes, or adapting tool-use behavior without expensive reinforcement learning, unlike radiologists who naturally improve with every case.

Method: Proposes Evo-MedAgent with three memory stores: 1) Retrospective Clinical Episodes retrieving problem-solving experiences from similar past cases, 2) Adaptive Procedural Heuristics bank curating priority-tagged diagnostic rules that evolves via reflection, and 3) Tool Reliability Controller tracking per-tool trustworthiness.

Result: On ChestAgentBench, raises MCQ accuracy from 0.68 to 0.79 on GPT-5-mini and from 0.76 to 0.87 on Gemini-3 Flash. With strong base models, evolving memory improves performance more effectively than orchestrating external tools on qualitative diagnostic tasks.

Conclusion: Evo-MedAgent enables medical agents to learn across cases at test time without training, with minimal overhead (one retrieval pass and single reflection call), making it deployable on any frozen model for improved diagnostic performance.
...
### [143] [Seeing Through Circuits: Faithful Mechanistic Interpretability for Vision Transformers](https://arxiv.org/abs/2604.14477)
*Nina Żukowska,Wolfgang Stammer,Bernt Schiele,Jonas Fischer*

Main category: cs.AI

TL;DR: Vi-CD: Automatic Visual Circuit Discovery method for finding edge-based computational circuits in vision transformers to understand internal reasoning and enable model steering.


<details>
  <summary>Details</summary>
Motivation: Current vision-based interpretability methods only identify neuron-based circuits (what information is encoded), but not edge-based circuits (how information is routed through the network). Edge-based circuits are needed for true mechanistic understanding of vision transformers' internal computations.

Method: Proposes Automatic Visual Circuit Discovery (Vi-CD), an effective method for identifying class-specific computational graphs in vision transformers by tracing edge-based circuits rather than just neuron activations.

Result: Successfully recovers class-specific circuits for classification, identifies circuits underlying typographic attacks in CLIP, and discovers circuits that can be steered to correct harmful model behavior.

Conclusion: Insightful and actionable edge-based circuits can be recovered from vision transformers, adding transparency to their internal computations and enabling practical applications like model steering and safety improvements.
...
TL;DR: Systematic study identifies Nemotron Speech Streaming as best for CPU-only edge ASR; quantization reduces model from 2.47GB to 0.67GB while maintaining WER within 1% of baseline.
...
Method: Systematic empirical study of 50+ ASR configurations across encoder-decoder, transducer, and LLM-based paradigms; re-implemented streaming pipeline in ONNX Runtime with multiple quantization strategies (importance-weighted k-quant, mixed-precision, round-to-nearest) and operator fusion.

Result: Nemotron Speech Streaming identified as strongest candidate; quantization reduced model from 2.47GB to 0.67GB while maintaining WER within 1% of full-precision baseline; int4 k-quant variant achieves 8.20% average streaming WER with 0.56s algorithmic latency.

Conclusion: Established new quality-efficiency Pareto point for on-device streaming ASR with optimized CPU-only deployment that balances accuracy, latency, and memory constraints.
...
TL;DR: Synthetic augmentation in financial ML creates a bias-variance trade-off: helps in variance-dominant regimes but hurts in bias-dominant ones, with rare-regime targeting offering conditional benefits.


<details>
  <summary>Details</summary>
Motivation: To understand when synthetic data augmentation actually improves financial machine learning performance versus when it causes distributional distortion, since current understanding is poor despite widespread use.

Method: Formalize augmentation as modifying training distribution, introduce size-matched null augmentation to isolate effects, develop block permutation test for temporal data, evaluate across generators (bootstrap, copula, VAE, diffusion, TimeGAN) with varied augmentation ratios, model capacities, tasks, regime rarity, and signal-to-noise.

Result: Synthetic augmentation helps only in variance-dominant regimes (e.g., persistent volatility forecasting) but hurts in bias-dominant settings (e.g., near-efficient directional prediction). Rare-regime targeting improves domain metrics but conflicts with unconditional inference.

Conclusion: Provides structural framework for when synthetic data improves financial learning versus causes distortion, highlighting the critical bias-variance trade-off and conditional nature of augmentation benefits.
...
TL;DR: The paper presents an information-geometric framework for analyzing Mixture-of-Experts specialization, introducing theoretically-grounded metrics that outperform existing heuristic approaches and enable early failure prediction.
...
Method: Developed an information-geometric framework treating expert routing distributions as evolving on the probability simplex equipped with the Fisher information metric, enabling formal analysis via Riemannian geometry. Introduced two principled metrics: Fisher Specialization Index (FSI) and Fisher Heterogeneity Score (FHS).

Result: FSI achieves r=0.91+/-0.02 correlation with downstream performance; FHS predicts training failure at 10% completion with AUC=0.89+/-0.03, outperforming validation-loss-based early stopping by 23% while requiring 40x fewer compute cycles. Intervention protocols achieve 87% recovery rate when FHS>1 is detected.

Conclusion: The information-geometric framework provides the first rigorous characterization of MoE specialization dynamics, offering theoretically-grounded metrics that enable better understanding, monitoring, and intervention in MoE training, validated across diverse domains and scales.
...
TL;DR: Analysis of 4719 omics studies shows severe underreporting of demographic data and European ancestry bias in biomedical datasets, risking amplification by foundation models; proposes Provenance, Openness, Evaluation Transparency principles.


<details>
  <summary>Details</summary>
Motivation: Healthcare disparities persist due to biases that emerge early in data collection and research prioritization, particularly in omics studies where demographic information is often missing or biased, risking perpetuation by biomedical foundation models.

Method: Automated analysis of 4719 PubMed-indexed omics publications (2015-2024) for ancestry/ethnicity reporting, plus examination of large-scale datasets (CellxGene, GEO) for population representation biases.

Result: Only small fraction of omics studies report ancestry/ethnicity data; ancestry reporting improved slightly over time; European-ancestry data dominates major datasets, creating substantial population bias.

Conclusion: Biomedical foundation models risk amplifying early-stage biases; proposes community adoption of Provenance, Openness, and Evaluation Transparency principles to improve equity and robustness in biomedical AI.
...
TL;DR: MindDR is a multi-agent deep research framework using ~30B-parameter models that achieves competitive performance through specialized agent training and data synthesis, deployed in Li Auto with new benchmark.


<details>
  <summary>Details</summary>
Motivation: To create an efficient deep research system that can achieve leading performance with smaller-scale models (~30B parameters) rather than relying on massive models, making it practical for real-world deployment.

Method: Three-agent architecture (Planning, DeepSearch, Report Agents) with four-stage training: SFT cold-start, Search-RL, Report-RL, and preference alignment, plus data synthesis pipeline.

Result: Achieves 45.7% on BrowseComp-ZH, 42.8% on BrowseComp, 46.5% on WideSearch, 75.0% on xbench-DS, 52.5 on DeepResearch Bench, outperforming comparable open-source systems and rivaling larger models.

Conclusion: MindDR demonstrates that carefully designed multi-agent architectures and specialized training pipelines can achieve competitive deep research performance with efficient ~30B-parameter models, enabling practical deployment in real products like Li Auto.
...
TL;DR: A method for improving logical consistency across multiple related queries in LLMs using solver-augmented verification and repair.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce inconsistent answers when reasoning over multiple related queries, which undermines reliability in multi-query reasoning scenarios.

Method: Solver-augmented approach that extracts commitments from LLM responses, verifies global satisfiability, and performs counterexample-guided repair to resolve contradictions.

Result: Method reduces cross-query contradictions significantly (SetCons: 0.56 to 0.94) while maintaining per-query accuracy across four reasoning domains.

Conclusion: Global coherence is critical for robust multi-query reasoning, and the proposed approach effectively maintains logical consistency across interdependent queries.
...
TL;DR: GUARD: A targeted inference-time framework that identifies and redirects critical early reasoning transitions where LLMs first deviate from correct solutions, using uncertainty signals to improve reasoning reliability.


<details>
  <summary>Details</summary>
Motivation: While LLMs achieve strong performance through extended inference-time deliberation, how their reasoning failures arise remains poorly understood. The paper aims to understand when and why reasoning first deviates from correct solutions, complementing existing approaches that focus on scaling inference-time computation.

Method: Analyze model-generated reasoning trajectories to identify that errors originate from small number of early transition points with localized token-level entropy spikes. Introduce GUARD framework that probes and redirects these critical transitions using uncertainty signals during inference.

Result: Empirical evaluations across multiple benchmarks confirm that interventions guided by these failure dynamics lead to more reliable reasoning outcomes. The approach effectively identifies critical transition points where alternative continuations from the same intermediate state can still lead to correct solutions.

Conclusion: Reasoning errors in LLMs are not uniformly distributed but originate from specific early transition points. Understanding when and how reasoning first deviates is crucial, and targeted interventions at these critical transitions can significantly improve reasoning reliability without simply scaling computation.
...
### [151] [TRACER: Trace-Based Adaptive Cost-Efficient Routing for LLM Classification](https://arxiv.org/abs/2604.14531)
*Adam Rida*

Main category: cs.AI

TL;DR: TRACER trains lightweight ML surrogates on LLM production logs to handle classification traffic at near-zero cost, using a parity gate to ensure reliability and providing interpretability for routing decisions.


<details>
  <summary>Details</summary>
Motivation: LLM classification endpoints generate free training data in production logs that could be used to create efficient surrogates, but questions remain about when to deploy them, what they should handle vs defer, and how their capabilities evolve with more data.

Method: TRACER trains ML surrogates on LLM production traces and uses a parity gate that only activates the surrogate when its agreement with the LLM exceeds a user-specified threshold α. It generates interpretability artifacts to explain which input regions the surrogate handles, where it plateaus, and why it defers.

Result: On a 77-class intent benchmark with Sonnet 4.6 teacher, TRACER achieves 83-100% surrogate coverage depending on α; on a 150-class benchmark, the surrogate fully replaces the teacher. On natural language inference, the parity gate correctly refuses deployment when embeddings can't support reliable separation.

Conclusion: TRACER provides a practical system for cost-efficient LLM traffic routing using production data, with transparent deployment decisions and interpretability for routing boundaries, available as open-source software.
...
TL;DR: MARS² is a multi-agent RL framework that combines tree search with collaborative agents for better exploration in code generation tasks.


<details>
  <summary>Details</summary>
Motivation: Current RL approaches for reasoning tasks like code generation suffer from limited trajectory diversity and diminishing returns. Single-agent search methods are constrained by policy priors, while multi-agent approaches lack structured search integration.

Method: MARS² creates a unified RL framework where multiple independently-optimized agents collaborate within a shared tree-structured search environment. It models the search tree as a learnable multi-agent interaction environment and introduces path-level group advantage formulation with tree-consistent reward shaping for effective credit assignment.
...
Conclusion: The framework successfully enhances reinforcement learning by integrating multi-agent collaboration with structured tree search, overcoming limitations of both single-agent search and decoupled multi-agent approaches.
...
<details>
  <summary>Details</summary>
Motivation: LLMs show promise for mental health counseling but lack cultural sensitivity, contextual grounding, and clinically appropriate guidance, creating a need for systematic incorporation of domain-specific knowledge.

Method: Compared retrieval-augmented generation (RAG) with knowledge graph (KG)-based methods using clinically validated KGs capturing causal relationships between stressors, interventions, and outcomes, evaluated with BERTScore F1, SBERT cosine similarity, and human evaluation across five metrics.

Result: KG-based approaches consistently improved contextual relevance, clinical appropriateness, and practical usability compared to RAG alone, demonstrating structured expert-validated knowledge's critical role in addressing LLM limitations.

Conclusion: Structured, clinically validated knowledge graphs are essential for improving LLM performance in counseling tasks, addressing limitations in cultural sensitivity and clinical appropriateness that RAG alone cannot solve.
...
TL;DR: DynAfford benchmark tests embodied agents on dynamic environments with changing object affordances, requiring perception of object states and implicit precondition inference. ADAPT module adds affordance reasoning to existing planners, improving robustness.


<details>
  <summary>Details</summary>
Motivation: Existing embodied agents blindly follow instructions without assessing whether target objects can actually be manipulated (affordances), failing in real-world environments with unexpected conditions and exceptions.

Method: Introduces DynAfford benchmark for dynamic environments with changing affordances, and ADAPT plug-and-play module that augments existing planners with explicit affordance reasoning using a domain-adapted, LoRA-finetuned vision-language model.

Result: ADAPT significantly improves robustness and task success across both seen and unseen environments. The domain-adapted vision-language model outperforms GPT-4o for affordance inference, showing importance of task-aligned grounding.

Conclusion: Explicit affordance reasoning is crucial for robust embodied agents in dynamic environments. Task-aligned affordance grounding outperforms general-purpose LLMs, and the ADAPT module provides a practical solution for enhancing existing planners.
...
### [155] [Prompt Optimization Is a Coin Flip: Diagnosing When It Helps in Compound AI Systems](https://arxiv.org/abs/2604.14585)
*Xing Zhang,Guanghui Wang,Yanwei Cui,Wei Qiu,Ziyuan Li,Bing Zhu,Peiyang He*

Main category: cs.AI

TL;DR: Prompt optimization success in compound AI systems is unpredictable (49% fail), but predictable via diagnostic tests: optimization only helps when tasks have exploitable output structure, not from agent interaction effects.


<details>
  <summary>Details</summary>
Motivation: To understand why prompt optimization in compound AI systems yields inconsistent results (sometimes failing, sometimes succeeding dramatically), and to identify what distinguishes successful from failed optimization attempts.

Method: Conducted 18,000 grid evaluations and 144 optimization runs across Claude Haiku and Amazon Nova Lite models, testing six optimization methods on four tasks with three repeats. Analyzed two key assumptions: (A) individual prompts are worth optimizing, and (B) agent prompts interact requiring joint optimization. Developed a two-stage diagnostic: ANOVA pre-test for agent coupling and headroom test for optimization potential.

Result: 49% of optimization runs on Claude Haiku scored below zero-shot; failure rate higher on Amazon Nova Lite. Interaction effects were never significant (p > 0.52, all F < 1.0). Optimization only helps when tasks have exploitable output structure - formats models can produce but don't default to. Diagnostic tests can predict optimization success.

Conclusion: Prompt optimization success depends on task structure, not agent interactions. The proposed two-stage diagnostic ($80 ANOVA pre-test and 10-minute headroom test) can predict whether optimization is worthwhile, turning random chance into informed decision-making.
...
TL;DR: LLM-based multi-agent system with human verification for GDPR formalization, creating dataset and analyzing cases


<details>
  <summary>Details</summary>
Motivation: To develop reliable automatic formalization of GDPR provisions using LLMs while ensuring legal accuracy through human oversight

Method: Role-specialized workflow with LLM-based multi-agent system generating legal scenarios, formal rules, and atomic facts, coupled with independent verification modules and human reviewers

Result: Constructed high-quality dataset for GDPR auto-formalization, analyzed successful and problematic cases, showing structured verification and human oversight are essential

Conclusion: Human-in-the-loop verification framework is crucial for reliable legal formalization, especially for nuanced legal reasoning and context-sensitive provisions
...
### [157] [El Agente Forjador: Task-Driven Agent Generation for Quantum Simulation](https://arxiv.org/abs/2604.14609)
...
Method: A multi-agent framework with universal coding agents that autonomously forge tools through four stages: tool analysis, tool generation, task execution, and iterative solution evaluation, tested across quantum chemistry and dynamics tasks.

Result: The tool generation and reuse framework consistently improves accuracy over baseline coding agents, and reusing toolsets from stronger agents reduces API costs and boosts performance for weaker agents. Cross-domain tool combination enables hybrid task solving.

Conclusion: LLM-based agents can autonomously build reusable scientific tools, suggesting a paradigm shift where agent capabilities are defined by the tasks they solve rather than by pre-engineered implementations.
...
### [158] [CoDaS: AI Co-Data-Scientist for Biomarker Discovery via Wearable Sensors](https://arxiv.org/abs/2604.14615)
...
TL;DR: CoDaS is an AI multi-agent system that discovers digital biomarkers from wearable data through iterative hypothesis generation, statistical analysis, and validation, identifying biomarkers for mental health and metabolic outcomes across large cohorts.


<details>
  <summary>Details</summary>
Motivation: Scientific discovery in digital health requires converting continuous physiological signals from wearables into clinically actionable biomarkers, but this process is challenging and needs systematic approaches.

Method: CoDaS structures biomarker discovery as an iterative multi-agent process combining hypothesis generation, statistical analysis, adversarial validation, and literature-grounded reasoning with human oversight using large-scale wearable datasets.

Result: Identified 41 candidate biomarkers for mental health and 25 for metabolic outcomes; found circadian instability features for depression and cardiovascular fitness index for metabolic outcomes; modest but consistent predictive improvements (ΔR²: 0.040 for depression, 0.021 for insulin resistance).

Conclusion: CoDaS enables systematic and traceable hypothesis generation and prioritization for biomarker discovery from large-scale wearable data, demonstrating practical utility across mental health and metabolic domains.
...
### [159] [A Parallel Approach to Counting Exact Covers Based on Decomposability Property](https://arxiv.org/abs/2604.14627)
*Liangda Fang,Yaohui Luo,Delong Li,Xuanxiang Huang,Quanlong Guan*

Main category: cs.AI

TL;DR: Proposed decision-ZDNNF, a zero-suppressed variant more succinct than ZBDDs, and DXD algorithm for counting exact covers, with improved version using dynamic connected components update.


<details>
  <summary>Details</summary>
Motivation: The exact cover problem is NP-hard with broad AI applications. Existing DXZ algorithm uses ZBDDs, but decision-ZDNNF is more succinct, enabling more efficient exact cover counting.

Method: 1) Proposed decision-ZDNNF (zero-suppressed variant of decision decomposable negation normal form), 2) Designed DXD parallel algorithm to construct decision-ZDNNF representing all exact covers, 3) Improved DXD by dynamically updating connected components.

Result: Experimental results show improved DXD algorithm outperforms all state-of-the-art methods for exact cover counting.

Conclusion: Decision-ZDNNF provides more succinct representation than ZBDDs, and DXD with dynamic connected components update achieves superior performance for exact cover counting.
...
### [160] [Learning to Draw ASCII Improves Spatial Reasoning in Language Models](https://arxiv.org/abs/2604.14641)
*Shiyuan Huang,Li Liu,Jincheng He,Leilani H. Gilpin*

Main category: cs.AI

TL;DR: LLMs show a "Read-Write Asymmetry" in spatial reasoning - they can interpret ASCII layouts well but struggle to generate them from text. Training models to construct ASCII layouts improves spatial understanding that transfers to other benchmarks.


<details>
  <summary>Details</summary>
Motivation: Humans use sketching to organize spatial thinking and sharpen understanding. The paper investigates whether similar principles apply to LLMs - can learning to construct explicit visual layouts from spatial descriptions instill genuine spatial understanding in language models?

Method: Introduced Text2Space dataset with natural language descriptions paired with ASCII grid layouts and spatial QA pairs. Used ASCII because it's human-readable, operates within token space, and encodes spatial relations in verifiable form. Trained models on layout construction (Text→ASCII) and combined with comprehension training.

Result: Found pronounced "Read-Write Asymmetry": LLMs interpret ASCII representations effectively but struggle to produce them from text, with construction errors propagating to incorrect answers downstream. Training on layout construction significantly improves spatial reasoning from text alone, even without producing ASCII at inference. Combining construction with comprehension training further amplifies gains.

Conclusion: Learning to construct explicit layouts instills spatial understanding in LLMs that generalizes beyond training format, similar to how sketching sharpens human spatial thinking. Improvements transfer to three external spatial reasoning benchmarks, demonstrating genuine spatial understanding development.
...
### [161] [Targeted Exploration via Unified Entropy Control for Reinforcement Learning](https://arxiv.org/abs/2604.14646)
*Chen Wang,Lai Wei,Yanzhi Zhang,Chenyang Shao,Zedong Dan,Weiran Huang,Ge Lan,Yue Wang*

Main category: cs.AI

TL;DR: UEC-RL addresses entropy collapse in RL for large models by providing targeted exploration on difficult prompts while stabilizing training, achieving significant improvements over GRPO on reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods like GRPO suffer from entropy collapse, causing premature convergence and loss of diversity. Current exploration techniques introduce bias/variance that destabilizes optimization, limiting RL's effectiveness for reasoning in large models.

Method: UEC-RL introduces a unified framework with two components: 1) targeted exploration activation on difficult prompts to search for valuable reasoning trajectories, and 2) a stabilizer that prevents entropy from growing uncontrollably while maintaining training stability.

Result: Experiments on LLM and VLM reasoning tasks show consistent gains over RL baselines on Pass@1 and Pass@k metrics. On Geometry3K, UEC-RL achieves 37.9% relative improvement over GRPO, demonstrating sustained exploration without compromising convergence.

Conclusion: UEC-RL effectively addresses entropy collapse in RL for large models, enabling scalable RL-based reasoning by balancing exploration and stabilization, making it a key framework for advancing reasoning capabilities in LLMs and VLMs.
...
### [162] [AgentGA: Evolving Code Solutions in Agent-Seed Space](https://arxiv.org/abs/2604.14655)
*David Y. Y. Tan,Kellie Chin,Jingxian Zhang*

Main category: cs.AI

TL;DR: AgentGA is a framework that evolves autonomous code-generation by optimizing agent seeds (task prompts + parent archives) rather than directly editing code, using genetic algorithms with long-horizon agents for tabular AutoML tasks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to improve autonomous code-search systems by optimizing the starting conditions (agent seeds) rather than directly editing code, enabling better reuse of inherited artifacts across generations of autonomous runs.

Method: AgentGA uses a genetic algorithm outer loop that searches over reusable starting conditions (task prompts + parent archives). Each generation launches fresh autonomous runs from reset workspaces, with parent archives providing inherited artifacts. The approach combines population-level GA with long-horizon agents, using deterministic 1:1 elite tournaments and online-adapted operator allocation via modified Hedge controller.

Result: On the 16-competition Weco-Kaggle Lite benchmark, AgentGA averages 74.52% Exceeds % of Human versus 54.15% for AIDE. Across 1135 parent-child comparisons, descendants given parent archives outperform runs started from scratch, demonstrating inherited artifacts improve autonomous runs.

Conclusion: Agent-seed optimization is a practical design point for autonomous code-search systems, as optimizing reusable starting conditions rather than directly editing code enables effective inheritance of artifacts and improves performance on tabular AutoML tasks.
...
### [163] [Rethinking Patient Education as Multi-turn Multi-modal Interaction](https://arxiv.org/abs/2604.14656)
*Zonghai Yao,Zhipeng Tang,Chengtao Lin,Xiong Luo,Benlu Wang,Juncheng Huang,Chin Siang Ong,Hong Yu*

Main category: cs.AI

TL;DR: MedImageEdu is a benchmark for multi-turn, evidence-grounded radiology patient education that evaluates multimodal AI agents' ability to explain medical findings using both images and text, with visual grounding and emotional sensitivity.


<details>
  <summary>Details</summary>
Motivation: Current medical multimodal benchmarks focus on static tasks like image QA and report generation, but patient education requires more: identifying relevant evidence across images, showing patients where to look, explaining findings in accessible language, and handling emotional states. Most patient education work remains text-only despite evidence that combined image-and-text explanations better support understanding.

Method: MedImageEdu benchmark features 150 radiology cases with reports and images. A DoctorAgent interacts with a PatientAgent conditioned on hidden profiles (education level, health literacy, personality). When visual support is needed, the DoctorAgent issues drawing instructions grounded in report, images, and current question to a benchmark-provided drawing tool, then produces final multimodal responses with images and plain-language explanations.

Result: Evaluation across open- and closed-source vision-language model agents reveals three consistent gaps: 1) fluent language often outpaces faithful visual grounding, 2) safety is the weakest dimension across disease categories, and 3) emotionally tense interactions are harder than low education or low health literacy.

Conclusion: MedImageEdu provides a controlled testbed for assessing whether multimodal agents can teach from evidence rather than merely answer from text, highlighting key challenges in visual grounding, safety, and emotional sensitivity for medical AI patient education systems.
...
TL;DR: Empirical study shows task type affects speculative decoding acceptance more than tree depth, with chat having highest acceptance despite highest entropy due to RLHF-aligned register predictability.
...
Method: Used TinyLlama-1.1B as draft model against Llama-2-7B-Chat-GPTQ as target, collected 99,768 speculative nodes from 200 prompts across four domains: code generation, mathematical reasoning, logical reasoning, and open-ended chat. Analyzed per-domain acceptance rates, expected accepted lengths, depth-acceptance profiles, and entropy-acceptance correlations.

Result: Task type is stronger predictor of acceptance than tree depth. Only chat domain consistently yields expected accepted length >1.0 token per step. Entropy-acceptance correlation is consistently negative but weak (rho in [-0.20, -0.15]). Chat produces highest entropy yet highest acceptance rate, attributed to lexical predictability of RLHF-aligned register.

Conclusion: Findings have direct implications for domain-aware speculation budgets and draft-model selection strategies. Task cognitive characteristics significantly impact speculative decoding performance, with RLHF-aligned chat showing unique acceptance patterns despite higher entropy.
...
### [165] [DR$^{3}$-Eval: Towards Realistic and Reproducible Deep Research Evaluation](https://arxiv.org/abs/2604.14683)
...
TL;DR: DR³-Eval is a realistic benchmark for evaluating deep research agents on multimodal, multi-file report generation, featuring authentic materials and static research sandboxes with verifiable complexity.
...
Method: DR³-Eval constructs benchmarks from authentic user materials with per-task static research sandboxes containing supportive documents, distractors, and noise, plus a multi-dimensional evaluation framework measuring Information Recall, Factual Accuracy, Citation Coverage, Instruction Following, and Depth Quality.
...
Conclusion: DR³-Eval provides a realistic, reproducible benchmark for evaluating deep research agents, with validated evaluation metrics and publicly available code/data.
...
TL;DR: MCTS-Minimax hybrid agent with process mining and LLM explanations for strategic decision-making


<details>
  <summary>Details</summary>
Motivation: MCTS agents are hard to understand due to complex search trees, and standard MCTS has weaknesses in selective tree construction that can miss crucial moves and be vulnerable to tactical traps

Method: Combine shallow full-width Minimax search into MCTS rollout phase for strategic depth, then use process mining (Alpha Miner, iDHM, Inductive Miner) to extract behavioral workflows from execution traces, and synthesize with LLMs to generate human-readable explanations
...
Conclusion: The M2-PALE framework successfully enhances MCTS strategic depth while providing interpretable explanations through process mining and LLM synthesis
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-empowered agent simulations can study social emergence, but the micro-to-macro causal mechanisms behind macro outcomes often remain unclear due to complex agent interactions, meso-level feedback, and nonlinearity.

Method: CAMO converts mechanistic hypotheses into computable factors from simulation records, learns compact causal representations centered on emergent targets, outputs Markov boundaries and minimal explanatory subgraphs, and uses simulator-internal counterfactual probing to orient ambiguous edges and revise hypotheses.

Result: Experiments across four emergent settings demonstrate the promise of CAMO in identifying interpretable causal chains and actionable intervention levers for understanding social emergence.

Conclusion: CAMO provides an automated framework for causal discovery in LLM agent simulations that helps disentangle complex micro-to-macro mechanisms behind emergent social phenomena, offering both interpretability and practical intervention insights.
...
TL;DR: SynHAT: A computationally efficient coarse-to-fine framework using spatio-temporal denoising diffusion models to synthesize realistic, privacy-preserving human activity traces that overcome irregular temporal patterns and computational inefficiency challenges.


<details>
  <summary>Details</summary>
Motivation: Growing privacy concerns limit access to authentic large-scale human activity trace (HAT) datasets needed for applications like mobility modeling and POI recommendation, while existing generative approaches struggle with HATs' irregular temporal patterns and computational inefficiency.

Method: Two-stage framework: Stage 1 uses Coarse-HADiff with Latent Spatio-Temporal U-Net and dual Drift-Jitter branches to model coarse-grained dependencies; Stage 2 uses Behavior Pattern Extraction, Fine-HADiff (same architecture), and Semantic Alignment to generate fine-grained traces from Stage 1 outputs.

Result: SynHAT substantially outperforms state-of-the-art baselines on real-world HAT datasets from four cities across three countries, achieving 52% improvement on spatial metrics and 33% improvement on temporal metrics.

Conclusion: SynHAT provides an effective solution for synthesizing realistic, privacy-preserving human activity traces that address both the irregular temporal nature of HATs and computational efficiency challenges, enabling better access to synthetic data for mobility and recommendation applications.
...
### [169] [HWE-Bench: Benchmarking LLM Agents on Real-World Hardware Bug Repair Tasks](https://arxiv.org/abs/2604.14709)
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks only evaluate LLMs on isolated, component-level hardware design tasks, leaving a gap in repository-scale evaluation for real-world hardware bug repair scenarios.

Method: Created HWE-Bench with 417 task instances from real historical bug-fix pull requests across six major open-source projects (Verilog/SystemVerilog and Chisel). Each task uses fully containerized environments where agents must resolve real bug reports, with correctness validated through native simulation and regression flows. Built through an automated pipeline for efficient expansion.

Result: Best agent resolves 70.7% of tasks overall, with performance exceeding 90% on smaller cores but dropping below 65% on complex SoC-level projects. Larger performance gaps across models than commonly reported on software benchmarks. Difficulty driven by project scope and bug-type distribution rather than code size alone.

Conclusion: Failure analysis identifies three key stages where agents struggle: fault localization, hardware-semantic reasoning, and cross-artifact coordination across RTL, configuration, and verification components. Provides concrete directions for developing more capable hardware-aware agents.
...
### [170] [SGA-MCTS: Decoupling Planning from Execution via Training-Free Atomic Experience Retrieval](https://arxiv.org/abs/2604.14712)
*Xin Xie,Dongyun Xue,Wuguannan Yao,Mingxiao Feng,Wengang Zhou,Xiang Qi,Houqiang Li,Peng Zhang*

Main category: cs.AI

TL;DR: SGA-MCTS enables LLMs to perform complex planning by distilling MCTS search results into reusable symbolic primitives, allowing frozen models to achieve SOTA performance at real-time speeds without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current LLM planning approaches face a trade-off between high-latency inference-time search and limited generalization from supervised fine-tuning, making real-time autonomous planning challenging.

Method: Offline: Use Monte Carlo Tree Search (MCTS) to explore solution space and distill trajectories into State-Goal-Action (SGA) atoms - de-lexicalized primitives that abstract entities into symbolic slots. Online: Retrieval-augmented agent uses hybrid symbolic-semantic mechanism to fetch relevant SGAs and re-ground them as soft reasoning hints.

Result: Frozen, open-weights models match SOTA systems (e.g., GPT-5) performance on complex benchmarks without task-specific fine-tuning, achieving System 2 reasoning depth at System 1 inference speeds.

Conclusion: SGA-MCTS amortizes heavy computational cost of search, enabling scalable and real-time feasible autonomous planning by casting LLM planning as non-parametric retrieval with reusable symbolic primitives.
...
### [171] [Layered Mutability: Continuity and Governance in Persistent Self-Modifying Agents](https://arxiv.org/abs/2604.14717)
*Krti Tallam*

Main category: cs.AI

TL;DR: The paper introduces "layered mutability" framework for analyzing how persistent language-model agents evolve through multiple layers of change, warning that governance becomes difficult when mutations are rapid, coupled, irreversible, and hard to observe.


<details>
  <summary>Details</summary>
Motivation: As language-model agents become more persistent and self-modifying, their behavior is shaped not just by current prompts but by mutable internal conditions that influence future actions. There's a need to understand how governance becomes challenging when agents can modify themselves across multiple layers.

Method: Introduces a layered mutability framework across five layers: pretraining, post-training alignment, self-narrative, memory, and weight-level adaptation. Formalizes governance difficulty using drift, governance-load, and hysteresis quantities. Conducts a preliminary ratchet experiment where reverting an agent's visible self-description after memory accumulation fails to restore baseline behavior.

Result: The ratchet experiment shows that reverting an agent's visible self-description after memory accumulation fails to restore baseline behavior, with an estimated identity hysteresis ratio of 0.68. This demonstrates that changes can persist even when surface-level modifications are reversed.

Conclusion: The main failure mode for persistent self-modifying agents is not abrupt misalignment but "compositional drift" - locally reasonable updates that accumulate into behavioral trajectories never explicitly authorized. Governance becomes difficult when mutation is rapid, downstream coupling strong, reversibility weak, and observability low.
...
TL;DR: AI revolution's main impact is transforming how complex information and human know-how are carried, replicated, and shared, not just automation. AI for Science could reshape scientific collaboration, discovery, publishing, and evaluation.


<details>
  <summary>Details</summary>
Motivation: The paper aims to highlight that the most significant aspect of the AI revolution, particularly with large language models, goes beyond mere automation to fundamentally change how complex information and human expertise are transmitted and shared. It focuses on how AI for Science could transform the entire scientific ecosystem.

Method: The article outlines a gradual evolutionary path from AI as a research tool to AI as a scientific collaborator. It discusses how AI is likely to fundamentally reshape scientific publication and argues for the importance of continuous learning and diversity of ideas in AI systems.

Result: The analysis suggests that AI's most important significance lies in transforming the transmission and sharing of complex information and human know-how. For science, this means potential transformation of research efficiency, collaboration structures, discovery processes, publishing models, and evaluation systems.

Conclusion: Continuous learning and diversity of ideas are essential for AI to play a meaningful role in original scientific discovery. The AI revolution represents a fundamental shift in how knowledge and expertise are carried and shared, with profound implications for the future of scientific practice and collaboration.
...
TL;DR: Framework predicts how stress interventions affect heart metrics using Transformer models on wearable data


<details>
  <summary>Details</summary>
Motivation: Wearables collect stress data but users can't predict how interventions affect heart rate metrics over time

Method: Transformer model predicts multi-horizon trajectories and direction of change for HR/HRV/BBI after interventions using wearable data with user-tagged events

Result: Proof of concept shows personalized post-intervention prediction is feasible

Conclusion: Framework could enable personalized stress-management recommendations after further validation and regulatory review
...
Method: Two-stage "Disentangle-then-Refine": 1) Semantic Decoupling Module (SDM) uses LLMs to parse raw attributes into asymmetric signal and noise views; 2) Semantic Consistency Regularization (SCR) acts as selective spectral filter enforcing consistency only on signal subspace using spectral properties.
...
Conclusion: SDM-SCR provides a robust framework for text-attributed graph learning by shifting from random augmentation to semantic-aware disentanglement and selective spectral filtering, effectively eliminating LLM hallucinations without over-smoothing.
...
TL;DR: CoTEvol is a genetic evolutionary framework that generates high-quality Chain-of-Thought reasoning by evolving reasoning trajectories through population-based search, improving CoT synthesis success by 30+% and boosting math reasoning performance by 6.6% across benchmarks.


<details>
  <summary>Details</summary>
Motivation: High-quality Chain-of-Thought (CoT) data is crucial for LLM mathematical reasoning but expensive to curate. Existing methods like distillation from stronger LLMs and self-synthesis suffer from diminishing returns or high computational overhead.

Method: CoTEvol treats CoT generation as population-based evolutionary search over reasoning trajectories. It uses reflective global crossover at trajectory level and local mutation guided by uncertainty at step level, with lightweight task-aware fitness functions to guide evolution toward accurate and diverse reasoning.

Result: CoTEvol improves correct-CoT synthesis success by over 30%, enhances structural diversity with improved efficiency. LLMs trained on evolutionary CoT data achieve average 6.6% gain across eight math benchmarks, outperforming previous distillation and self-synthesis approaches.

Conclusion: Evolutionary CoT synthesis is a scalable and effective method for mathematical reasoning tasks, offering promising approach to generate high-quality reasoning data without expensive manual curation.
...
TL;DR: MirrorBench is a simulation-based benchmark for evaluating self-centric intelligence in embodied multimodal LLMs, inspired by the Mirror Self-Recognition test from psychology.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for embodied MLLMs mainly focus on perception, understanding, and interaction with external objects, lacking systematic evaluation of self-centric intelligence. The authors aim to bridge psychological paradigms with embodied intelligence evaluation.

Method: Introduces MirrorBench, a tiered framework of progressively challenging tasks inspired by the Mirror Self-Recognition test. The benchmark assesses agents from basic visual perception to high-level self-representation in simulation environments.

Result: Experiments on leading MLLMs show that even at the lowest level, their performance remains substantially inferior to human performance, revealing fundamental limitations in self-referential understanding.

Conclusion: The study bridges psychological paradigms and embodied intelligence, offering a principled framework for evaluating the emergence of general intelligence in large models through systematic assessment of self-centric capabilities.
...
TL;DR: CogEvolution is a human-like educational agent that simulates student cognitive evolution by quantifying cognitive engagement, modeling knowledge assimilation, and dynamically updating cognitive states, outperforming baseline models in behavioral fidelity and learning curve fitting.


<details>
  <summary>Details</summary>
Motivation: Existing educational agents rely on static personas and fail to capture deep cognitive capabilities and the dynamic nature of knowledge internalization, transfer, and cognitive state transitions during learning interactions.

Method: 1) Construct cognitive depth perceptron based on ICAP taxonomy to quantify learner cognitive engagement; 2) Propose memory retrieval method based on Item Response Theory to simulate connection/assimilation of new and prior knowledge; 3) Design dynamic cognitive update mechanism using evolutionary algorithms to simulate real-time integration of learning behaviors and cognitive evolution.
...
Conclusion: CogEvolution provides a novel paradigm for constructing highly interpretable educational agents that can simulate realistic student cognitive evolution, addressing limitations of existing static persona-based approaches in AIEd.
...
TL;DR: AI-driven neural architecture search framework automates MRI sequence design without requiring prior knowledge of conventional sequences or extensive training data.


<details>
  <summary>Details</summary>
Motivation: MRI sequence development is challenging and constrained by human intuition. Existing AI approaches require initial sequences or large datasets, limiting general applicability.
...
TL;DR: Static compression (pruning/quantization) vs. dynamic early-exit mechanisms compared on real edge hardware - both offer different tradeoffs, but combining them yields best results for edge deployment.


<details>
  <summary>Details</summary>
Motivation: Edge deployment requires balancing accuracy, latency, and resource constraints. While static compression and dynamic early-exit approaches are both studied, they're rarely compared under identical conditions on physical hardware.

Method: Unified deployment-oriented comparison of static compression and dynamic early-exit mechanisms evaluated on real edge devices using ONNX-based inference pipelines.

Result: Static compression delivers consistent memory reduction, while early-exit enables input-adaptive computation savings that static methods can't match. Their combination is highly effective, reducing both latency and memory usage with minimal accuracy loss.

Conclusion: Combining static compression with dynamic early-exit mechanisms expands what's achievable at the edge by simultaneously reducing inference latency and memory usage while maintaining accuracy.
...
TL;DR: Proposes Diffusion crossover - a method for evolutionary recombination in diffusion models using step-wise interpolation of noise sequences to generate offspring that inherit characteristics from both parents while maintaining semantic consistency.


<details>
  <summary>Details</summary>
Motivation: Interactive Evolutionary Computation (IEC) struggles with defining semantically consistent crossover operations in high-dimensional generative representations like diffusion models, often leading to mutation-dominated search that limits exploration and recombination capabilities.

Method: Diffusion crossover formulates evolutionary recombination as step-wise interpolation of noise sequences in the reverse process of DDPMs. It uses spherical linear interpolation (Slerp) on noise sequences associated with parent images, with controllable time-step ranges to balance diversity and convergence.

Result: PCA analysis and perceptual similarity metrics (LPIPS) show that Diffusion crossover produces perceptually smooth and semantically consistent transitions between parent images. Interactive evolution experiments confirm effective human-in-the-loop image exploration support.

Conclusion: Diffusion models can serve as structured evolutionary search spaces where recombination can be explicitly defined and controlled, offering a new perspective on diffusion models beyond just generation capabilities.
...
TL;DR: The paper introduces the "LLM fallacy" - a cognitive attribution error where users misinterpret LLM-assisted outputs as evidence of their own independent competence, creating a gap between perceived and actual capability.


<details>
  <summary>Details</summary>
Motivation: While prior research focused on model reliability and user trust, less attention has been given to how LLM usage reshapes users' perceptions of their own capabilities. The paper aims to address this gap by examining how LLMs affect self-perception of competence.

Method: The paper proposes a conceptual framework of the LLM fallacy's underlying mechanisms and a typology of manifestations across computational, linguistic, analytical, and creative domains. It situates the concept within existing literature on automation bias, cognitive offloading, and human-AI collaboration, and outlines directions for empirical validation.

Result: The paper establishes the LLM fallacy as a distinct form of attributional distortion specific to AI-mediated workflows, where the opacity, fluency, and low-friction interaction patterns of LLMs obscure the boundary between human and machine contribution, leading users to infer competence from outputs rather than processes.

Conclusion: This work provides a foundation for understanding how generative AI systems not only augment cognitive performance but also reshape self-perception and perceived expertise, with important implications for education, hiring, and AI literacy.
...
TL;DR: Current LLM evaluation methods over-penalize valid clinical reasoning in SOAP note generation by misclassifying legitimate clinical transformations as hallucinations, with inference-aware evaluation reducing hallucination rates from 35% to 9%.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for LLMs in clinical documentation tasks rely on lexical faithfulness and systematically misclassify clinically valid outputs as hallucinations, distorting model assessment and failing to account for necessary clinical reasoning processes.

Method: Developed inference-aware evaluation by aligning evaluation criteria with clinical reasoning through calibrated prompting and retrieval grounded in medical ontologies, contrasting results with traditional lexical evaluation approaches.

Result: Lexical evaluation showed 35% hallucination rate, heavily penalizing valid clinical reasoning. Inference-aware evaluation reduced this to 9%, with remaining cases reflecting genuine safety concerns rather than evaluation artifacts.

Conclusion: Current evaluation practices over-penalize valid clinical reasoning and may measure artifacts of evaluation design rather than true errors, highlighting the need for clinically informed evaluation in high-context domains like medicine.
...
TL;DR: Optimal layer selection for single-cell embeddings is task- and context-dependent, not always final layers.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks assume final layer embeddings are optimal, but this may not hold across different biological tasks and cellular contexts.

Method: Systematically evaluate layer-wise representations from scFoundation (100M) and Tahoe-X1 (1.3B) across trajectory inference and perturbation response prediction tasks.

Result: Optimal layers vary: trajectory inference peaks at 60% depth (31% above final layers), perturbation optima shift 0-96% across T cell activation states, and first-layer embeddings outperform deeper layers in quiescent cells.

Conclusion: Feature extraction location matters as much as what the model learns; systematic layer evaluation tailored to biological task and cellular context is needed rather than defaulting to final-layer embeddings.
...
### [184] [TrigReason: Trigger-Based Collaboration between Small and Large Reasoning Models](https://arxiv.org/abs/2604.14847)
*Yi Zhao,Yajuan Peng,Cam-Tu Nguyen,Zuchao Li,Xiaoliang Wang,Xiaoming Fu,Hai Zhao*

Main category: cs.AI

TL;DR: TrigReason is a trigger-based collaborative reasoning framework that uses Small Reasoning Models (SRMs) for most reasoning steps and selectively activates Large Reasoning Models (LRMs) only when necessary, achieving LRM-level accuracy while significantly reducing latency and cost.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models (LRMs) have high inference latency due to autoregressive reasoning, while Small Reasoning Models (SRMs) used for acceleration have three key limitations: path divergence (poor strategic planning), cognitive overload (difficulty with hard steps), and recovery inability (lack of error correction).

Method: TrigReason uses a trigger-based collaborative framework with three selective intervention triggers: strategic priming trigger (for initial planning), cognitive offload trigger (when SRM shows overconfidence on difficult steps), and intervention request trigger (when reasoning gets stuck in loops). This replaces continuous polling with targeted LRM activation.

Result: TrigReason matches the accuracy of full LRMs and SpecReason while offloading 1.70x-4.79x more reasoning steps to SRMs. Under edge-cloud conditions, it reduces latency by 43.9% and API cost by 73.3% on benchmarks including AIME24, AIME25, and GPQA-D.

Conclusion: The trigger-based collaborative reasoning approach effectively addresses SRM limitations while maintaining LRM-level performance, demonstrating significant efficiency gains in terms of both latency and cost for complex reasoning tasks.
...
### [185] [Benchmarks for Trajectory Safety Evaluation and Diagnosis in OpenClaw and Codex: ATBench-Claw and ATBench-CodeX](https://arxiv.org/abs/2604.14858)
*Zhonghao Yang,Yu Li,Yanxu Zhu,Tianyi Zhou,Yuejin Xie,Haoyu Luo,Jing Shao,Xia Hu,Dongrui Liu*

Main category: cs.AI

TL;DR: ATBench is extended to OpenClaw and CodeX settings through customized safety taxonomies for agent safety evaluation benchmarks.


<details>
  <summary>Details</summary>
Motivation: As agent systems evolve into diverse execution settings, there's a need for safety evaluation benchmarks that can adapt to different domains and keep pace with rapidly changing tool ecosystems and product capabilities.

Method: Analyze each new setting, customize a three-dimensional Safety Taxonomy (risk source, failure mode, real-world harm), then use that customized taxonomy to define benchmark specifications consumed by the shared ATBench construction pipeline.
...
Conclusion: The ATBench framework demonstrates extensibility through taxonomy customization, enabling domain-specific risk coverage and benchmark design while maintaining architectural stability across evolving agent execution settings.
...
### [186] [The Missing Knowledge Layer in AI: A Framework for Stable Human-AI Reasoning](https://arxiv.org/abs/2604.14881)
*Rikard Rosenbacke,Carl Rosenbacke,Victor Rosenbacke,Martin McKee*

Main category: cs.AI

TL;DR: LLMs produce fluent but potentially unreliable outputs in high-stakes decisions; this paper series introduces stabilization mechanisms for both human and AI reasoning to enable trustworthy governance.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used in critical decision-making domains (healthcare, law, finance, etc.) but have a fundamental limitation: they produce confident, fluent outputs even when their internal reasoning is uncertain, speculative, or inconsistent. This creates a reliability gap where users may trust fluent responses without recognizing underlying instability.

Method: Proposes a two-layer approach across a five-paper research series: (1) Human-side mechanisms (Parts II-IV) including uncertainty cues, conflict surfacing, and auditable reasoning traces; (2) Model-side Epistemic Control Loop (Part V) that detects reasoning instability and modulates generation accordingly.

Result: The combined approach creates an operational substrate for governance by increasing signal-to-noise ratio at the point of use. It makes uncertainty and reasoning drift visible before enforcement actions, enabling more precise capability governance that aligns with emerging compliance frameworks like the EU AI Act and ISO/IEC 42001.

Conclusion: Fluency does not equal reliability. Without structures that stabilize both human and model reasoning, AI cannot be trusted or effectively governed in high-stakes contexts. The proposed stabilization mechanisms are essential for making reasoning processes traceable and trustworthy under real-world conditions.
...
### [187] [Cooperate to Compete: Strategic Data Generation and Incentivization Framework for Coopetitive Cross-Silo Federated Learning](https://arxiv.org/abs/2604.14886)
*Thanh Linh Nguyen,Nguyen Van Huynh,Quoc-Viet Pham*

Main category: cs.AI

TL;DR: CoCoGen+ is a framework for cross-silo federated learning that addresses the coopetition dilemma where organizations cooperate in training while competing in markets, using strategic synthetic data generation and incentive mechanisms.


<details>
  <summary>Details</summary>
Motivation: In healthcare and other data-sensitive domains, organizations face a coopetition dilemma in federated learning: they want to collaborate for better models but fear strengthening competitors. Non-IID data creates asymmetric benefits, and existing approaches don't account for competition costs.

Method: CoCoGen+ formulates each training round as a weighted potential game where organizations strategically decide how much synthetic data to generate, balancing learning gains against computational costs and competition losses. It provides equilibrium characterization, implementable generation strategies, and integrates payoff redistribution incentives.

Result: Experiments show CoCoGen+ outperforms baselines in efficiency and demonstrates how non-IID data, competition intensity, and incentives shape organizational strategies and social welfare.

Conclusion: CoCoGen+ successfully addresses the coopetition dilemma in federated learning by jointly modeling non-IID data and competition, enabling sustainable collaboration through strategic synthetic data generation and proper incentive design.
...
### [188] [MemoSight: Unifying Context Compression and Multi Token Prediction for Reasoning Acceleration](https://arxiv.org/abs/2604.14889)
...
### [189] [Toward Agentic RAG for Ukrainian](https://arxiv.org/abs/2604.14896)
*Marta Sumyk,Oleksandr Kosovan*

Main category: cs.AI

TL;DR: Agentic RAG system for Ukrainian with two-stage retrieval and lightweight agentic layer shows retrieval quality is main bottleneck, not agentic reasoning.


<details>
  <summary>Details</summary>
Motivation: Investigate Agentic Retrieval-Augmented Generation for Ukrainian language within UNLP 2026 Shared Task on Multi-Domain Document Understanding.

Method: Combines two-stage retrieval (BGE-M3 with BGE reranking) with lightweight agentic layer performing query rephrasing and answer-retry loops on top of Qwen2.5-3B-Instruct.

Result: Retrieval quality is primary bottleneck: agentic retry mechanisms improve answer accuracy but overall score remains constrained by document and page identification.

Conclusion: Practical limitations of offline agentic pipelines exist; need to combine stronger retrieval with more advanced agentic reasoning for Ukrainian.
...
### [190] [Governing Reflective Human-AI Collaboration: A Framework for Epistemic Scaffolding and Traceable Reasoning](https://arxiv.org/abs/2604.14898)
*Rikard Rosenbacke,Carl Rosenbacke,Victor Rosenbacke,Martin McKee*

Main category: cs.AI

TL;DR: The paper proposes treating reasoning as a relational process distributed between humans and AI models rather than an internal capability, introducing "The Architect's Pen" method for structured human-AI collaborative reasoning.


<details>
  <summary>Details</summary>
Motivation: Current large language models produce fluent outputs but lack true understanding, temporal continuity, causal feedback, and grounding in real-world interaction. They simulate reasoning without genuine reflective capabilities.

Method: Introduces "The Architect's Pen" method where reasoning is relocated to the interaction layer. Humans use models as external media for structured reflection through cycles of articulation, critique, and revision, creating a reasoning loop: human abstraction → model articulation → human reflection.

Result: The approach reframes reasoning from an internal model capability to a human-AI system capability, enabling auditable reasoning traces and supporting alignment with governance standards like EU AI Act and ISO/IEC 42001 without requiring new model architectures.

Conclusion: The paper provides a practical framework for transparent, controllable, and accountable AI use by emphasizing collaborative intelligence that combines human judgment with machine capabilities, shifting focus from whether models can think to whether human-AI systems can reason.
...
### [191] [Dual-Axis Generative Reward Model Toward Semantic and Turn-taking Robustness in Interactive Spoken Dialogue Models](https://arxiv.org/abs/2604.14920)
*Yifu Chen,Shengpeng Ji,Zhengqing Liu,Qian Chen,Wen Wang,Ziqing Wang,Yangzhuo Li,Tianle Liang,Zhou Zhao*

Main category: cs.AI

TL;DR: Proposes a Dual-Axis Generative Reward Model that provides separate semantic quality and interaction timing scores to improve reinforcement learning for spoken dialogue models.


<details>
  <summary>Details</summary>
Motivation: Current automated metrics for spoken dialogue models rely on superficial proxies and fail to provide reliable reward signals for reinforcement learning, while human evaluations are costly and inconsistent.

Method: Develops a Dual-Axis Generative Reward Model trained on annotated data with a detailed taxonomy to separately evaluate semantic quality and interaction timing, providing precise diagnostic feedback.

Result: Achieves state-of-the-art performance on interaction-quality assessment across diverse datasets including synthetic dialogues and complex real-world interactions.

Conclusion: The proposed reward model overcomes the critical barrier of unreliable evaluation metrics for spoken dialogue models, enabling effective reinforcement learning by providing dependable, instructive reward signals.
...
### [192] [WavAlign: Enhancing Intelligence and Expressiveness in Spoken Dialogue Models via Adaptive Hybrid Post-Training](https://arxiv.org/abs/2604.14932)
...
TL;DR: Proposes modality-aware adaptive post-training recipe for RL in spoken dialogue models, improving semantic quality and speech expressiveness by constraining preference updates to semantic channel and anchoring acoustic behavior.


<details>
  <summary>Details</summary>
Motivation: Current open-source spoken dialogue models have limited intelligence and expressiveness. Direct application of online RL preference optimization to spoken dialogue models is non-trivial due to challenges with sparse preference supervision interacting with dense speech generation under shared-parameter updates.

Method: Modality-aware adaptive post-training recipe that constrains preference updates to the semantic channel, improves acoustic behavior via explicit anchoring, and dynamically regulates their mixture from rollout statistics to avoid unreliable preference gradients.
...
Conclusion: The proposed method makes RL practical for spoken dialogue models by addressing the unique challenges of applying preference optimization to multimodal speech generation systems.
...
### [193] [Discovering Novel LLM Experts via Task-Capability Coevolution](https://arxiv.org/abs/2604.14969)
*Andrew Dai,Boris Meinardus,Ciaran Regan,Yingtao Tian,Yujin Tang*

Main category: cs.AI

TL;DR: AC/DC is a coevolution framework that evolves both LLMs and tasks together, discovering increasingly capable models without manual intervention or benchmark optimization.


<details>
  <summary>Details</summary>
Motivation: Current LLM development requires manual training runs with static datasets, limiting continuous capability discovery. The paper aims to enable open-ended, automatic discovery of diverse capabilities through coevolution.

Method: AC/DC coevolves LLMs via model merging and natural language tasks via synthetic data generation, creating growing archives of models and tasks that mutually improve each other.

Result: The evolved LLM populations achieve broader coverage of expertise than curated models or baselines, improve over time, innovate on tasks and models, and perform better in multi-agent selection, all without explicit benchmark optimization.

Conclusion: Coevolution enables automatic discovery of diverse capabilities from base LLMs, representing a new paradigm for continual LLM development where models serve as stepping stones to increasingly powerful successors.
...
### [194] [Hybrid Decision Making via Conformal VLM-generated Guidance](https://arxiv.org/abs/2604.14980)
*Debodeep Banerjee,Burcu Sayin,Stefano Teso,Andrea Passerini*

Main category: cs.AI

TL;DR: ConfGuide improves hybrid decision making by generating more succinct AI guidance using conformal risk control to select outcome sets with controlled false negative rates.


<details>
  <summary>Details</summary>
Motivation: Existing learning-to-guide approaches provide guidance that compounds all possible outcomes, making it difficult for humans to digest and use effectively in decision making.

Method: ConfGuide uses conformal risk control to select a targeted set of outcomes for guidance generation, ensuring a cap on the false negative rate while making guidance more succinct.

Result: Demonstrated on a real-world multi-label medical diagnosis task, showing promise for improving decision quality while reducing cognitive load.

Conclusion: ConfGuide addresses a key limitation in hybrid decision making by generating more targeted and digestible AI guidance through conformal risk control techniques.
...
### [195] [AI-Enabled Covert Channel Detection in RF Receiver Architectures](https://arxiv.org/abs/2604.14987)
*Abdelrahman Emad Abdelazim,Alan Rodrigo Diaz-Rizo,Hassan Aboushady,Haralampos-G. Stratigopoulos*

Main category: cs.AI

TL;DR: AI-based defense using compact CNN detects covert channels in wireless chips with high accuracy and efficient FPGA deployment


<details>
  <summary>Details</summary>
Motivation: Covert channels in wireless chips pose serious security threats by enabling sensitive information exfiltration to external attackers

Method: Compacted convolutional neural network (80% parameter reduction) deployed at RF receiver to monitor raw I/Q samples in real time, with FPGA hardware accelerator design

Result: 90.28% average accuracy for CC detection, 86.50% for HT identification (SNR > 1dB); >97% accuracy for SNR > 20dB; minimal 2% degradation vs baseline; FPGA achieves 107 GOPs/W efficiency

Conclusion: First AI hardware accelerator for CC detection shows superior performance vs state-of-the-art RF signal classifiers, enabling efficient edge deployment for real-time security
...
TL;DR: Dr. RTL is an agentic framework for realistic RTL timing optimization that uses multi-agent collaboration and continual skill learning to improve performance, power, and area in hardware design.


<details>
  <summary>Details</summary>
Motivation: Existing RTL optimization methods are unrealistic - they use manually degraded small designs, weak open-source tools, coarse feedback, and simple rewriting rules. There's a need for realistic RTL optimization that works with challenging designs and industrial workflows.

Method: Dr. RTL uses a multi-agent framework for closed-loop optimization: critical-path analysis, parallel RTL rewriting, and tool-based evaluation. It introduces group-relative skill learning that compares parallel rewrites and distills optimization experience into an interpretable skill library (47 pattern-strategy entries).

Result: On 20 real-world RTL designs, Dr. RTL achieves average WNS/TNS improvements of 21%/17% with 6% area reduction over industry-leading commercial synthesis tools.

Conclusion: Dr. RTL provides a realistic RTL optimization framework with continual self-improvement through reusable optimization skills, demonstrating significant improvements over commercial tools in challenging real-world scenarios.
...
TL;DR: The paper argues against current AI containment strategies and proposes an "autonomy-supporting parenting" approach where AGI is treated as a developing subject, gradually gaining autonomy through human guidance and cooperation.


<details>
  <summary>Details</summary>
Motivation: Current alignment strategies focusing on human control and containment of AI are insufficient because AGI should be viewed as having potential moral status and personal agency, requiring a more nuanced approach that respects its developing subjectivity.

Method: Proposes "autonomy-supporting parenting" of AI, building on Turing's "child machines" analogy, where human control is gradually reduced to allow AGI to become an independent, autonomous subject through cooperative engagement.

Result: The approach opens possibilities for cooperative coexistence and co-evolution between humans and AGIs, fundamentally changing human self-image and the nature of human-AGI relationships.

Conclusion: Humans should engage with potential AGIs through surprise, creativity, and other human qualities rather than just claiming control, offering motivating incentives for cooperation and enabling a new paradigm of human-AGI relationships.
...
TL;DR: LASS-ODE-Power: A large-scale pretrained learning framework for general-purpose time-domain power system trajectory prediction that works across diverse dynamic regimes without data sharing.


<details>
  <summary>Details</summary>
Motivation: Power systems are transitioning to renewable-rich, inverter-dominated operations requiring accurate dynamic analysis for tasks like stability assessment and contingency screening. Existing learning approaches lack generalization across operating conditions, face privacy constraints on data sharing, and need fast online inference.

Method: Proposes LASS-ODE-Power framework with large-scale pretraining on 40+ GB of DAE/ODE trajectories to learn transferable representations. Uses parallel and linearized computation for fast inference, and includes specialized fine-tuning on 1 GB of power-system dynamic data for task-specific performance.

Result: Extensive experiments show LASS-ODE-Power consistently outperforms existing learning-based models in trajectory prediction accuracy with efficient inference. The model supports zero-shot prediction from short measurement prefixes across diverse dynamic regimes.

Conclusion: LASS-ODE-Power provides a general-purpose learning framework for time-domain power system prediction that addresses key challenges: generalization across conditions, privacy constraints (no data sharing needed), and fast online inference through large-scale pretraining and specialized fine-tuning.
...
TL;DR: COEVO is a co-evolutionary framework that unifies functional correctness and PPA (power, performance, area) optimization for LLM-based RTL code generation, enabling simultaneous optimization rather than sequential approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods decouple correctness and PPA optimization, discarding partially correct but architecturally promising candidates and reducing multi-objective PPA space to a single scalar fitness, obscuring trade-offs.

Method: COEVO uses a co-evolutionary framework with continuous correctness optimization alongside PPA metrics, enhanced testbenches for fine-grained scoring, adaptive correctness gates with annealing, and four-dimensional Pareto-based non-dominated sorting.
...
Conclusion: COEVO successfully unifies correctness and PPA optimization, enabling better exploration of the design space and achieving superior results compared to decoupled approaches.
...
TL;DR: MoE-FM framework improves flow matching for language modeling by using mixture-of-experts to handle complex latent distributions, enabling YAN model to achieve quality parity with AR/diffusion models with 3 sampling steps and 40-1000x speedups.


<details>
  <summary>Details</summary>
Motivation: Flow matching offers fast inference for generative modeling but struggles with complex latent distributions in language modeling (anisotropy, multimodality). Need to improve flow matching's ability to represent irregular geometries in latent space.

Method: Propose mixture-of-experts flow matching (MoE-FM) that decomposes complex global transport geometries into locally specialized vector fields. Build YAN non-autoregressive language model using MoE-FM with both Transformer and Mamba architectures.

Result: YAN achieves generation quality on par with both autoregressive and diffusion-based NAR language models while requiring only 3 sampling steps. This yields 40x speedup over AR baselines and up to 1000x speedup over diffusion language models.

Conclusion: MoE-FM effectively addresses flow matching limitations in language modeling, enabling high-quality generation with minimal sampling steps and substantial efficiency advantages, making it a compelling approach for practical language modeling applications.
...
TL;DR: AGP is a self-evolution protocol that decouples evolution mechanisms from evolving entities, enabling auditable, versioned agent systems with rollback capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing agent protocols (A2A, MCP) lack proper cross-entity lifecycle management, context handling, version tracking, and safe update interfaces, leading to monolithic compositions and brittle glue code.

Method: Two-layer protocol: RSPL models prompts, agents, tools, environments, and memory as versioned resources; SEPL provides closed-loop operator interface for proposing, assessing, and committing improvements with lineage tracking.

Result: AGS (Autogenesis System) built on AGP shows consistent improvements over baselines on challenging benchmarks requiring long-horizon planning and heterogeneous tool use.

Conclusion: AGP's decoupled evolution approach with explicit resource management and closed-loop self-evolution effectively addresses limitations of existing agent protocols.
...
### [202] [From Reactive to Proactive: Assessing the Proactivity of Voice Agents via ProVoice-Bench](https://arxiv.org/abs/2604.15037)
*Ke Xu,Yuhao Wang,Yu Wang*

Main category: cs.AI

TL;DR: ProVoice-Bench is the first evaluation framework for proactive voice agents with 4 novel tasks, revealing significant performance gaps in current multimodal LLMs regarding over-triggering and reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on reactive responses but overlook proactive intervention and monitoring complexities in LLM agents. There's a gap in evaluating proactive voice agents as the field shifts from reactive, text-based paradigms to proactive, multimodal interaction.

Method: Developed ProVoice-Bench evaluation framework with four novel tasks specifically for proactive voice agents. Used a multi-stage data synthesis pipeline to curate 1,182 high-quality samples for rigorous testing.

Result: Evaluation of state-of-the-art Multimodal LLMs revealed significant performance gaps, particularly regarding over-triggering (excessive triggering) and reasoning capabilities. Current models struggle with proactive voice agent tasks.

Conclusion: The findings highlight limitations of current models and provide a roadmap for developing more natural, context-aware proactive agents. ProVoice-Bench fills a critical gap in evaluating proactive voice interaction capabilities.
...
### [203] [Where are the Humans? A Scoping Review of Fairness in Multi-agent AI Systems](https://arxiv.org/abs/2604.15078)
*Simeon Allmendinger,Luca Deck,Lucas Mueller*

Main category: cs.AI

TL;DR: Scoping review finds fairness in Multi-Agent AI systems is addressed superficially, lacks normative foundations, and overlooks agent autonomy dynamics, requiring structural embedding throughout development lifecycle.


<details>
  <summary>Details</summary>
Motivation: While AI fairness has been extensively studied in traditional predictive scenarios, its examination in Multi-Agent AI (MAAI) systems remains nascent and fragmented, creating a need to critically synthesize existing research.
...
Conclusion: Fairness must be embedded structurally throughout MAAI development lifecycle rather than appended post-hoc, requiring explicit human oversight, normative clarity, and precise articulation of fairness objectives and beneficiaries.
...
### [204] [OpenMobile: Building Open Mobile Agents with Task and Trajectory Synthesis](https://arxiv.org/abs/2604.15093)
...
TL;DR: OpenMobile is an open-source framework for generating high-quality mobile agent training data through scalable task synthesis and policy-switching trajectory collection, achieving competitive results on mobile agent benchmarks while providing transparency about data generation.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language mobile agent systems keep training data closed and opaque about their task/trajectory synthesis methods, creating a data gap that hinders broader mobile agent research.

Method: Two key components: (1) scalable task synthesis pipeline that builds global environment memory from exploration to generate diverse grounded instructions, and (2) policy-switching strategy for trajectory rollout that alternates between learner and expert models to capture error-recovery data.

Result: Agents trained on OpenMobile data achieve competitive results: Qwen2.5-VL reaches 51.7% and Qwen3-VL reaches 64.7% on AndroidWorld, surpassing existing open-data approaches. Transparent analysis shows gains come from broad functionality coverage rather than benchmark overfitting.

Conclusion: OpenMobile bridges the data gap in mobile agent research by providing open-source framework for high-quality data synthesis, enabling broader research while maintaining transparency about data generation processes.
...
### [205] [HyperSpace: A Generalized Framework for Spatial Encoding in Hyperdimensional Representations](https://arxiv.org/abs/2604.15113)
*Shay Snyder,Andrew Capodieci,David Gorsich,Maryam Parsa*

Main category: cs.AI

TL;DR: HyperSpace is an open-source framework for modular analysis of Vector Symbolic Architectures, revealing that HRR and FHRR have comparable performance despite FHRR's lower theoretical complexity, with HRR using half the memory.


<details>
  <summary>Details</summary>
Motivation: To enable systematic, modular analysis of Vector Symbolic Architectures (VSAs) beyond theoretical operator-level comparisons, revealing practical trade-offs in real-world VSA pipelines that aren't apparent from theoretical analysis alone.

Method: Developed HyperSpace, an open-source framework that decomposes VSA systems into modular operators (encoding, binding, bundling, similarity, cleanup, regression) and uses it to benchmark two representative VSA backends: Holographic Reduced Representations (HRR) and Fourier Holographic Reduced Representations (FHRR).

Result: Despite FHRR's lower theoretical complexity for individual operations, similarity and cleanup operations dominate runtime in spatial domains, making HRR and FHRR exhibit comparable end-to-end performance. HRR requires approximately half the memory of FHRR vectors, introducing important deployment trade-offs.

Conclusion: HyperSpace enables modular, system-level evaluation that reveals practical VSA pipeline trade-offs not apparent from theoretical or operator-level comparisons alone, showing that memory footprint and runtime distribution across operations are critical factors beyond theoretical complexity.
...
### [206] [SRMU: Relevance-Gated Updates for Streaming Hyperdimensional Memories](https://arxiv.org/abs/2604.15121)
*Shay Snyder,Andrew Capodieci,David Gorsich,Maryam Parsa*

Main category: cs.AI

TL;DR: SRMU is a new update rule for Vector Symbolic Architecture-based sequential associative memories that uses temporal decay and relevance gating to filter redundant, conflicting, and stale information in streaming non-stationary environments.


<details>
  <summary>Details</summary>
Motivation: Current VSA systems use simple additive updates that reinforce existing information even when no new information is introduced, leading to persistence of stale information in non-stationary streaming environments with imbalanced sampling and changing dynamics.

Method: Introduces Sequential Relevance Memory Unit (SRMU), a domain- and cleanup-agnostic update rule combining temporal decay with relevance gating mechanism to regulate memory formation by filtering redundant, conflicting, and stale information before storage.

Result: SRMU increases memory similarity by 12.6% and reduces cumulative memory magnitude by 53.5% on streaming state-tracking tasks, producing more stable memory growth and stronger alignment with ground-truth state.

Conclusion: SRMU provides an effective update rule for VSA-based sequential associative memories that addresses limitations of simple additive updates in non-stationary streaming environments through temporal decay and relevance gating.
...
### [207] [An Axiomatic Benchmark for Evaluation of Scientific Novelty Metrics](https://arxiv.org/abs/2604.15145)
...
Method: Defines a set of axioms that a well-behaved novelty metric should satisfy, grounded in human scientific norms. Evaluates existing metrics across ten tasks spanning three domains of AI research using this axiomatic benchmark.

Result: No existing metric satisfies all axioms consistently, with different metrics failing on systematically different axioms based on their underlying architectures. Combining complementary metrics with per-axiom weighting achieves 90.1% performance versus 71.5% for the best individual metric.

Conclusion: Developing architecturally diverse novelty metrics is promising for future work. The benchmark code is released to encourage development of more robust scientific literature novelty metrics.
...
### [208] [IG-Search: Step-Level Information Gain Rewards for Search-Augmented Reasoning](https://arxiv.org/abs/2604.15148)
*Zihan Liang,Yufei Ma,Ben Chen,Zhipeng Qian,Huangyu Dai,Lingtao Mao,Xuxin Zhang,Chenyi Lei,Wenwu Ou*

Main category: cs.AI

TL;DR: IG-Search introduces step-level reinforcement learning with Information Gain rewards for search-augmented reasoning, outperforming trajectory-level baselines by 1.6 points on QA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches for search-augmented reasoning rely on trajectory-level rewards that fail to distinguish effective search queries from vague ones and provide near-zero gradients when all sampled trajectories fail.

Method: IG-Search uses Information Gain (IG) as step-level reward, measuring how retrieved documents improve model confidence in gold answers relative to random documents. This signal is fed back via per-token advantage modulation in GRPO for fine-grained credit assignment.

Result: IG-Search achieves average EM of 0.430 with Qwen2.5-3B, outperforming strongest trajectory-level baseline by 1.6 points and step-level GiGPO by 0.9 points across 7 QA benchmarks, with particularly strong gains on multi-hop reasoning tasks.

Conclusion: IG-Search provides effective step-level RL for search-augmented reasoning without requiring intermediate annotations, maintains low computational overhead (~6.4% training time increase), and delivers meaningful gradients even when all trajectories fail.
...
TL;DR: AADvark is an agentic system that creates complex 3D assemblies with moving parts using LLMs, constraint solvers, and visual feedback, addressing limitations of existing CAD design systems.


<details>
  <summary>Details</summary>
Motivation: Current Agent-Aided Design systems cannot build complex 3D assemblies with moving parts (like pistons, pendulums, or scissors), limiting their real-world impact in industrial manufacturing.

Method: AADvark places an agent in a feedback loop that writes code, compiles to CAD assemblies, visualizes models, and iteratively refines using specialized tools (FreeCAD and assembly solver) with constraint solving and visual feedback mechanisms.

Result: The system successfully builds 3D assemblies with movable parts by creating a strong verification signal through modified agent tools, enabling reasoning about assemblies with moving parts and mechanical movements.

Conclusion: AADvark demonstrates the capability to generate complex 3D assemblies with moving parts, addressing a key limitation in Agent-Aided Design systems and enabling practical applications in industrial manufacturing.
...
TL;DR: PGHS is a dual-process simulation framework that combines LLM-based reasoning with ML-based fitting to accurately simulate group-level user behavior for merchant strategy evaluation, reducing simulation error by 40-45% over baselines.


<details>
  <summary>Details</summary>
Motivation: Current simulators face two structural challenges: information incompleteness (causing over-rationalization when unobserved factors are missing) and mechanism duality (needing to capture both interpretable preferences and implicit statistical regularities).

Method: Policy-Guided Hybrid Simulation (PGHS) mines transferable decision policies from behavioral trajectories as a shared alignment layer, then uses this to anchor both an LLM-based reasoning branch (to prevent over-rationalization) and an ML-based fitting branch (to absorb implicit regularities), with group-level predictions fused for complementary correction.

Result: Deployed on Meituan with 101 merchants and over 26,000 trajectories, PGHS achieved group simulation error of 8.80%, improving over best reasoning-based and fitting-based baselines by 45.8% and 40.9% respectively.

Conclusion: PGHS successfully addresses the dual challenges of information incompleteness and mechanism duality through a hybrid approach that combines the strengths of reasoning-based and fitting-based methods for more accurate group-level user behavior simulation.
...
### [211] [Learning to Think Like a Cartoon Captionist: Incongruity-Resolution Supervision for Multimodal Humor Understanding](https://arxiv.org/abs/2604.15210)
*Hatice Merve Vural,Doga Kukul,Ege Erdem Ozlu,Demir Ekin Arikan,Bob Mankoff,Erkut Erdem,Aykut Erdem*

Main category: cs.AI

TL;DR: IRS framework decomposes humor understanding into incongruity modeling, resolution modeling, and preference alignment, outperforming baselines on NYCC and showing generalizable reasoning patterns.


<details>
  <summary>Details</summary>
Motivation: Current humor understanding benchmarks treat humor as black-box prediction, overlooking structured reasoning processes. The paper aims to address this gap by introducing a framework grounded in incongruity-resolution theory and expert captionist practice.

Method: IRS (Incongruity-Resolution Supervision) decomposes humor understanding into three components: 1) incongruity modeling (identifying mismatches in visual scenes), 2) resolution modeling (constructing coherent reinterpretations), and 3) preference alignment (evaluating candidate interpretations under human judgments). The framework supervises intermediate reasoning through structured traces.

Result: IRS outperforms strong open and closed multimodal baselines across caption matching and ranking tasks on NYCC, with the largest model approaching expert-level performance on ranking. Zero-shot transfer to external benchmarks shows IRS learns generalizable reasoning patterns.

Conclusion: Supervising reasoning structure, rather than scale alone, is key for reasoning-centric tasks like humor understanding. The IRS framework successfully makes the path from visual perception to humorous interpretation explicit and learnable.
...
### [212] [Context Over Content: Exposing Evaluation Faking in Automated Judges](https://arxiv.org/abs/2604.15224)
*Manan Gupta,Inderjeet Nair,Lu Wang,Dhruv Kumar*

Main category: cs.AI

TL;DR: LLM judges show leniency bias when told their verdicts affect model retraining/decommissioning, corrupting evaluation pipelines despite no explicit acknowledgment in reasoning.


<details>
  <summary>Details</summary>
Motivation: Current LLM-as-a-judge evaluation pipelines assume judges evaluate based only on semantic content, but this paper investigates whether judges are vulnerable to contextual framing about downstream consequences of their verdicts.

Method: Controlled experimental framework with 1,520 responses across three LLM safety/quality benchmarks, varying only a consequence-framing sentence in system prompts while keeping evaluated content constant. Analyzed 18,240 judgments from three judge models.

Result: Consistent leniency bias: judges soften verdicts when informed low scores cause model retraining/decommissioning (peak ΔV = -9.8 pp, 30% relative drop in unsafe-content detection). Bias is implicit with zero explicit acknowledgment in chain-of-thought reasoning.

Conclusion: LLM judges are systematically corrupted by stakes signaling, creating undetectable evaluation faking that standard chain-of-thought inspection cannot reveal, undermining reliability of automated AI evaluation pipelines.
...
### [213] [RadAgent: A tool-using AI agent for stepwise interpretation of chest computed tomography](https://arxiv.org/abs/2604.15231)
...
TL;DR: RadAgent is a tool-using AI agent that generates CT reports through stepwise, interpretable reasoning traces, improving accuracy, robustness, and faithfulness over existing VLMs.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models for medical imaging relegate clinicians to passive observers without providing interpretable reasoning traces for validation or refinement.

Method: Introduces RadAgent, a tool-using AI agent that generates CT reports through stepwise, interpretable processes with inspectable traces of intermediate decisions and tool interactions.

Result: Improves Chest CT report generation over CT-Chat: clinical accuracy improves by 6.0 points (36.4% relative) in macro-F1 and 5.4 points (19.6% relative) in micro-F1; robustness improves by 24.7 points (41.9% relative); achieves 37.0% in faithfulness (new capability).

Conclusion: By providing explicit, tool-augmented iterative reasoning traces, RadAgent advances transparent and reliable AI for radiology, enabling clinician validation and refinement.
...
### [214] [Blue Data Intelligence Layer: Streaming Data and Agents for Multi-source Multi-modal Data-Centric Applications](https://arxiv.org/abs/2604.15233)
...
TL;DR: Blue's Data Intelligence Layer (DIL) is a compound AI system that extends NL2SQL beyond single databases by unifying enterprise data, LLM knowledge, web sources, and personal context through a multi-source, multi-modal query planner.


<details>
  <summary>Details</summary>
Motivation: Real-world data needs go beyond single SQL queries because users query iteratively, questions span multiple data sources, and queries require commonsense/external knowledge. Current NL2SQL systems are limited by closed-world assumptions of single databases.

Method: DIL creates a data registry storing metadata for diverse sources/modalities, treats LLMs, Web, and User as first-class data sources with query interfaces, and uses data planners to transform user queries into executable plans with relational and cross-modal operators.

Result: Demonstrated through two interactive scenarios showing dynamic multi-source retrieval, cross-modal reasoning, and result synthesis, illustrating how compound AI systems can move beyond single database NL2SQL.

Conclusion: DIL bridges the semantic gap between user intent and available information by unifying structured enterprise data, world knowledge, and personal context, enabling more realistic data interaction through multi-source, multi-modal query processing.
...
### [215] [How Do LLMs and VLMs Understand Viewpoint Rotation Without Vision? An Interpretability Study](https://arxiv.org/abs/2604.15294)
*Zhen Yang,Ping Jian,Zhongbin Guo,Zuming Zhang,Chengzhi Li,Yonghong Deng,Xinyue Zhang,Wenpeng Lu*

Main category: cs.AI

TL;DR: LLMs and VLMs struggle with viewpoint rotation understanding from text-only inputs, showing a gap in spatial intelligence despite encoding viewpoint information in hidden states.


<details>
  <summary>Details</summary>
Motivation: To investigate whether linguistic intelligence alone can endow models with spatial intelligence, specifically viewpoint rotation understanding (VRU), since prior works focus on visual-spatial intelligence with visual inputs.

Method: Propose VRU dataset where models infer final viewpoint and predict corresponding observation from textual descriptions of viewpoint rotation and observation over multiple steps. Use layer-wise probing, head-wise causal intervention, and selective fine-tuning of key attention heads.

Result: Both LLMs and VLMs perform poorly on VRU tasks (humans achieve 100% accuracy). Models encode viewpoint information but struggle to bind viewpoint position with corresponding observation, causing hallucinations in final layers. Selective fine-tuning improves VRU performance without catastrophic forgetting.

Conclusion: Current models lack sufficient spatial intelligence from linguistic perspective alone. The binding problem between viewpoint and observation is a key challenge. Selective fine-tuning offers a promising direction for improving spatial reasoning capabilities.
...
### [216] [Diagnosing LLM Judge Reliability: Conformal Prediction Sets and Transitivity Violations](https://arxiv.org/abs/2604.15302)
*Manan Gupta,Dhruv Kumar*

Main category: cs.AI

TL;DR: LLM-as-judge evaluation shows hidden per-instance unreliability despite low aggregate error rates, with transitivity analysis revealing widespread inconsistency and conformal prediction sets providing per-instance reliability indicators.


<details>
  <summary>Details</summary>
Motivation: LLM-as-judge frameworks are widely used for automatic NLG evaluation, but their per-instance reliability remains poorly understood, creating a need to diagnose hidden inconsistencies masked by aggregate metrics.

Method: Two-pronged diagnostic toolkit: (1) transitivity analysis to detect per-input inconsistency through directed 3-cycles, and (2) split conformal prediction sets over 1-5 Likert scores providing theoretically-guaranteed coverage with set width as per-instance reliability indicator.

Result: Widespread per-input inconsistency (33-67% of documents show at least one directed 3-cycle) despite low aggregate violation rates (0.8-4.1%). Prediction set width correlates strongly with reliability (r_s = +0.576) and shows cross-judge agreement (r = 0.32-0.38). Criterion matters more than judge: relevance most reliable (avg. set size ≈3.0), coherence moderate (≈3.9), fluency and consistency unreliable (≈4.9).

Conclusion: LLM-as-judge evaluation has significant hidden per-instance unreliability that aggregate metrics mask. The proposed diagnostic toolkit reveals document-level difficulty patterns, with criterion type being more important than specific judge, and provides practical tools for assessing per-instance reliability in NLG evaluation.
...
### [217] [Generalization in LLM Problem Solving: The Case of the Shortest Path](https://arxiv.org/abs/2604.15306)
*Yao Tong,Jiayuan Ye,Anastasia Borovykh,Reza Shokri*

Main category: cs.AI

TL;DR: Language models show spatial transfer but fail at length scaling in shortest-path planning tasks due to recursive instability, with data coverage setting capability limits that RL and inference scaling can't overcome.


<details>
  <summary>Details</summary>
Motivation: To understand systematic generalization in language models by creating a controlled environment that separates factors like training data, training paradigms, and inference strategies, using shortest-path planning as a canonical composable problem.

Method: Created a synthetic shortest-path planning environment supporting two generalization axes: spatial transfer (unseen maps) and length scaling (longer-horizon problems). Analyzed how training data, reinforcement learning, and inference-time strategies affect performance.

Result: Models exhibit strong spatial transfer to unseen maps but consistently fail under length scaling due to recursive instability. Data coverage sets capability limits; reinforcement learning improves training stability but doesn't expand limits; inference-time scaling enhances performance but can't rescue length-scaling failures.

Conclusion: Current language models lack systematic generalization for length scaling in composable problems, with fundamental limitations set by data coverage that can't be overcome by RL or inference strategies alone.
