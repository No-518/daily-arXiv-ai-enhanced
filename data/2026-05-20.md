...
TL;DR: MotionMERGE is a unified framework bridging the granularity gap in motion-language models by enabling fine-grained part-level and temporal-level control, using a novel pre-training strategy with cross-granularity alignment and chain-of-thought reasoning, supported by a new large-scale dataset MotionFineEdit, achieving superior performance in motion generation, understanding, and editing.


<details>
  <summary>Details</summary>
Motivation: Existing motion-language models lack fine-grained understanding and control over body parts needed for animation/interaction due to coarse granularity in both model design (inability to focus on localized motion patterns) and training data (lack of fine-grained supervision).

Method: (1) Explicitly models motion at part and temporal levels within a single LLM for fine-grained control; (2) ReasoningAware Granularity-Synergy pre-training with joint supervision for cross-granularity alignment, temporal grounding, localized alignment, motion coherency, and motion-grounded CoT reasoning; (3) Curates MotionFineEdit dataset (837K atomic + 144K complex triplets) with fine-grained corrective instructions and CoT annotations.

Result: MotionMERGE achieves more precise motion generation, understanding, and editing, with compelling zero-shot generalization to other complex motion tasks.

Conclusion: This work represents a significant step toward models that interact with motion in finer granularity and human-like reasoning, bridging the granularity gap in motion-language modeling.
...
TL;DR: This paper investigates self-supervised vs supervised backbones for artwork classification and retrieval, finding that self-supervised models (DINO, CLIP) consistently outperform supervised approaches, with applications in VR museum navigation.


<details>
  <summary>Details</summary>
Motivation: Classifying artworks is challenging due to fine-grained details and abstract features that define style/genre; the paper aims to systematically evaluate which backbone architectures work best for this task.

Method: Extensive experimental evaluation comparing DINO family and CLIP models as feature extractors, testing multiple classification strategies and feature representations for both classification and retrieval tasks on paintings.

Result: Self-supervised backbones consistently improve artwork classification performance over supervised counterparts.

Conclusion: Self-supervised features are more effective for artwork analysis, with practical applicability in real-world VR museum navigation systems.
...
TL;DR: Artifact-Bench evaluates MLLMs on detecting artifacts in AI-generated videos (photorealistic, animated, CG-style). 19 MLLMs show near-random performance and misalignment with human perception.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack systematic evaluation of artifact-aware perception and fine-grained diagnostic reasoning in diverse AI-generated video domains (beyond photorealistic).

Method: Three-level hierarchical taxonomy of realism artifacts; three tasks: real vs. AI classification, pairwise realism comparison, fine-grained artifact identification.

Result: 19 leading MLLMs show substantial limitations (near-random or below-random in challenging settings) and significant misalignment with human perceptual preferences.

Conclusion: Current MLLMs are unreliable as general evaluators for AI-generated video realism; the benchmark reveals a critical gap in artifact-aware perception.
...
TL;DR: EgoTraj is a new egocentric multimodal dataset for human trajectory prediction, featuring 75 real-world navigation sequences with synchronized RGB video, head poses, eye gaze, and scene annotations, collected via Meta Quest Pro across diverse urban routes.


<details>
  <summary>Details</summary>
Motivation: Limited progress in egocentric trajectory prediction due to scarcity of real-world egocentric trajectory datasets.

Method: Collected 75 sequences using Meta Quest Pro in urban environments, with synchronized RGB video, 6-DoF head poses, 3D eye gaze vectors, and scene annotations. Benchmarked state-of-the-art methods and performed ablation studies.

Result: Demonstrated utility of EgoTraj for egocentric trajectory prediction; ablation studies revealed contributions of gaze, scene, and motion cues.

Conclusion: EgoTraj enables progress in AR-based perception, navigation, and assistive systems by providing a diverse, long-horizon egocentric trajectory dataset.
...
TL;DR: Vision foundation models fail at open-set iris Presentation Attack Detection (PAD) under unseen attack instruments and cross-spectral shifts, despite strong performance in closed-set and cross-dataset settings. LoRA adaptation sometimes amplifies rather than mitigates these failures.


<details>
  <summary>Details</summary>
Motivation: To systematically examine whether general-purpose vision foundation models are suitable for open-set iris Presentation Attack Detection (PAD), an area insufficiently studied despite growing interest in using these models for biometric applications.

Method: Evaluated five foundation models under three open-set protocols (unseen PAIs, unseen datasets with different sensors, NIR→VIS cross-spectral transfer), assessing both frozen features and LoRA-based adaptation within a unified framework, plus additional validation experiments.

Result: Foundation models transfer across datasets with similar sensors but fail on unseen attack instruments and cross-spectral evaluation. LoRA improves cross-dataset performance but often amplifies failures under attack-level and spectral shifts. Additional validation confirms these failures are not artefacts of periocular input, weak adaptation, or one-directional evaluation.

Conclusion: Strong closed-set or cross-dataset performance does not guarantee robust open-set security. PAD representations must maintain sensitivity to presentation artefacts while remaining stable under realistic deployment variation.
...
TL;DR: This paper reviews two paradigms of Medical Foundation Models (MedFMs): Medical Vision-Language Models (Med-VLMs) and segmentation foundation models, highlighting their capabilities and the need for rigorous evaluation before clinical deployment.


<details>
  <summary>Details</summary>
Motivation: To categorize and evaluate the reliability of emerging Medical Foundation Models for clinical deployment under real-world conditions.

Method: The paper categorizes MedFMs into two paradigms: Med-VLMs (e.g., LLaVA-Med, MedGemma, GPT-4o, Gemini) for tasks like VQA and report generation, and SAM-based segmentation models (e.g., SAM-Med2D, MedSAM).

Result: Identifies that both Med-VLMs and SAM-based segmentation models show promise but require rigorous evaluation for real-world clinical reliability.

Conclusion: Clinical deployment of MedFMs necessitates thorough reliability testing under real-world conditions before adoption.
...
TL;DR: FaceCloak generates a personalized, single-image-derived face privacy mask that effectively blocks facial recognition systems from identifying users across multiple datasets and recognition models.


<details>
  <summary>Details</summary>
Motivation: Photos of faces uploaded online are vulnerable to malicious actors who can scrape facial images from online sources and intrude on personal privacy via unauthorized use of facial recognition models.

Method: FaceCloak uses a three-stage personalized perturbation learning: (1) generates high-variety synthetic images from a single user photo, (2) learns cloaking by iteratively shifting identity embedding toward a distant anchor identity away from similar ones, (3) produces a lightweight pixel-wise cloaking mask applicable to any facial image of the user.

Result: Extensive experiments on three popular face datasets across ten recognition models show effectiveness compared to 29 other existing representative methods.

Conclusion: FaceCloak effectively protects facial privacy by generating personalized, lightweight, and perceptually good privacy masks that cause facial recognition to fail.
...
TL;DR: LiFT factorizes 3D medical image generation into per-slice image generation and lightweight inter-slice trajectory learning, achieving high-resolution synthesis with dramatically lower computational cost.


<details>
  <summary>Details</summary>
Motivation: High-resolution 3D medical image generation is challenging because fully volumetric models are computationally expensive, while efficient 2D slice generators fail to maintain anatomical consistency across the third dimension.

Method: LiFT treats a volume as an ordered trajectory in feature space, using a tri-planar drifting loss to align generated slice trajectories with real volumes. For paired translation, a bidirectional z-context mixer enforces through-plane coherence.

Result: On BraTS 2023 and SynthRAD2023, LiFT preserves per-slice quality, approaches cWDM quality at ~135× lower inference cost, and improves through-plane coherence on MR-to-CT.

Conclusion: Lightweight inter-slice trajectory learning is a viable route to high-resolution 3D medical synthesis.
...
TL;DR: Proposes a multi-horizon forecasting framework using sky images and historical PV data that improves solar power prediction accuracy across architectures with minimal computational overhead, validated on 597 GW-scale data.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning GSI-based forecasting is limited by single-architecture evaluations and single-horizon (point) predictions, failing to capture temporal dependencies needed for grid stability amid rapid PV expansion (597 GW in 2024).

Method: Transition from single-horizon to multi-horizon joint optimization over future sequences, integrating sequential sky imagery with historical PV data; evaluated across diverse deep learning architectures; avoids precocious convergence via better gradient and filter diversity.

Result: Multi-horizon approach significantly improves predictive accuracy and robustness across the entire forecast horizon while maintaining computational parsimony, with negligible overhead compared to single-horizon models.

Conclusion: Provides a scalable, architecture-independent, and computationally efficient solution to enhance power grid resilience by improving solar PV forecasting accuracy.
...
TL;DR: CRAFT is a pipeline for grounded multi-video QA over news events that combines dynamic keyframe selection, multilingual ASR, and a hybrid critic loop for claim verification and citation, achieving state-of-the-art results on MAGMaR 2026 and strong performance on WikiVideo.


<details>
  <summary>Details</summary>
Motivation: To enable query-relevant evidence surfacing across heterogeneous video archives while ensuring every claim is attributable to its supporting source in real-world news event QA.

Method: A query-conditioned pipeline with dynamic keyframe selection, per-video ASR with multilingual fallback, a hybrid critic loop (UNLI temporal entailment, DeBERTa-v3 cross-claim screening, Llama-3.2-3B adjudicator) for iterative claim verification and repair, and a citation-merging stage for single-fact multi-source citing.

Result: Best overall average (0.739), reference recall (0.810), and citation F1 (0.635) on MAGMaR 2026; strong performance on WikiVideo (0.823 Avg); ablations confirm atomic claims, ASR, and the critic loop drive main gains.

Conclusion: CRAFT demonstrates that claim-centric evidence aggregation with iterative verification generalizes beyond MAGMaR, and its components (atomic claims, ASR, critic loop) are critical for performance improvement over vanilla baselines.
...
TL;DR: FAGER is a training-free agentic framework that evaluates and improves factual correctness in T2I generation by combining LLM-based fact proposal, visual fact extraction, and VLM-based evaluation, outperforming prior metrics across science, history, products, culture, and knowledge-intensive domains.


<details>
  <summary>Details</summary>
Motivation: Existing T2I metrics fail to capture implicit, externally grounded, or identity-defining factual requirements, making them unsuitable for evaluating factual correctness in prompts involving scientific knowledge, historical facts, products, or culture-specific concepts.

Method: FAGER constructs a structured factual rubric via LLM-based fact proposal and reference-guided visual fact extraction/verification, then converts the rubric into QA pairs for VLM-based evaluation. It also enables training-free refinement of T2I outputs.

Result: FAGER consistently outperforms prior metrics on the Factual A/B test across five datasets (science, history, products, culture, knowledge-intensive concepts) and yields substantial factuality gains when used for refinement.

Conclusion: FAGER offers a more robust approach to evaluating and improving factual correctness in text-to-image generation, addressing limitations of existing metrics that overlook implicit or externally grounded factual requirements.
...
TL;DR: This paper studies how SSL pretraining length affects model calibration and confidence-based abstention in medical image classification, finding that longer pretraining doesn't consistently improve reliability once accuracy saturates.


<details>
  <summary>Details</summary>
Motivation: Standard SSL evaluation focuses on downstream accuracy, but for safety-critical medical tasks like diabetic retinopathy grading, models must also know when to defer uncertain cases. The authors argue that abstention-aware evaluation is needed.

Method: Multiple SSL checkpoints (varying pretraining length) are evaluated under a fixed fine-tuning protocol. Metrics include calibrated confidence, coverage, selective accuracy, and selective macro-F1 across datasets and data regimes.

Result: SSL pretraining improves selective prediction over training from scratch. However, once accuracy saturates, selective performance changes markedly across checkpoints, and longer pretraining does not consistently improve reliability.

Conclusion: Abstention-aware evaluation is important for SSL models in medical imaging. Pretraining length should be treated as a reliability-related design choice, not just a computational detail.
...
TL;DR: This paper explores whether competitive video models can be built by freezing a pre-trained image foundation model and training only a recurrent temporal module, significantly reducing video data and compute costs compared to end-to-end video pre-training.


<details>
  <summary>Details</summary>
Motivation: Video foundation models require massive video datasets and compute for pre-training, while image foundation models already provide powerful spatial representations. The authors question whether competitive video models can be built by reusing these spatial representations and pre-training only for temporal reasoning.

Method: The approach freezes a pre-trained image foundation model as a spatial encoder and trains only a recurrent temporal module to process streaming video, creating a lightweight training paradigm that avoids end-to-end video pre-training.

Result: Empirical findings across multiple video understanding tasks show that strong temporal performance can emerge without large-scale video pre-training.

Conclusion: The results motivate future work on recurrent video foundation models obtained by pre-training a temporal module on top of a frozen image foundation model, suggesting this lightweight approach is feasible and promising.
...
TL;DR: An unsupervised "deep efficient coding" model, using only local statistics without labels or backprop, learns hierarchical visual features (edges→colors→textures→shapes) from limited data that align with human perception and brain activity, suggesting a principle behind biological vision's data efficiency.


<details>
  <summary>Details</summary>
Motivation: To understand what learning principles allow biological visual systems to learn from limited experience, unlike deep learning models that require millions of training images.

Method: Developed an unsupervised deep learning procedure where each layer compresses inputs onto dominant modes of variation in natural images using only local statistics, no labels, tasks, or backpropagation. Also tested a hybrid approach combining this with supervised fine-tuning.

Result: The model produces human-recognizable features progressing from edges/colors to textures/shapes, predicts fMRI responses in human visual cortex, and the hybrid approach yields better brain alignment with limited data and faster category learning.
...
TL;DR: This paper proposes a multi-strategy compression framework for brain tumor classification from MRI, demonstrating that a MobileNetV2 model quantized via TensorFlow Lite Float16 achieves 82.37% accuracy (vs 82.20% full-precision) with 6.14x compression (35.34 MB to 5.76 MB), suitable for low-resource clinical settings.


<details>
  <summary>Details</summary>
Motivation: Deploying deep learning models in low-resource clinical environments is challenging due to computational, memory, and power constraints.

Method: Multi-strategy compression framework including quantization-aware training, knowledge distillation (DenseNet-101 teacher to DenseNet-32 student), and Float16 post-training quantization on MobileNetV2 with three-stage transfer learning.

Result: MobileNetV2 quantized model: 82.37% validation accuracy (vs 82.20% baseline), 6.14x compression (35.34 MB → 5.76 MB), uniform per-class performance preservation.
...
TL;DR: The paper proposes a quasi-concavity-based convexity prior for segmentation networks that enforces all super-level sets of the output mask to be convex through differentiable losses, unifying previous convex shape models.


<details>
  <summary>Details</summary>
Motivation: Convexity is a fundamental geometric prior in many structures but is difficult to impose effectively in end-to-end trainable segmentation networks. Existing methods lack a unified, threshold-free framework.

Method: Derives zero, first, and second-order characterizations of quasi-concavity to formulate local, differentiable constraints on the network's output function u. Introduces a convex gradient projection module (CGPM) and convolutional losses that enforce convexity across all super-level sets without thresholding.

Result: The quasi-concavity losses consistently enforce convexity and improve shape regularity across multiple datasets, outperforming networks tailored for retinal segmentation and surpassing previous shape-aware methods.

Conclusion: The proposed framework unifies a wide spectrum of previous convex shape models (discrete constraints, graph-cuts, curvature/signed distance Laplacian level-set priors) within a single continuous and differentiable framework.
...
TL;DR: A smartphone-based pipeline using monocular depth estimation and SLAM enables accurate tree DBH and position measurement from a single walkthrough video, achieving ~2 cm error without specialized LiDAR hardware.


<details>
  <summary>Details</summary>
Motivation: Current forest inventory methods either rely on expensive terrestrial LiDAR or labor-intensive manual caliper/compass measurements, limiting scalability and accessibility for large-scale forest monitoring.

Method: Integrates pretrained monocular depth estimation + tree instance segmentation with a SLAM framework to jointly refine camera trajectories and depth from a single smartphone video. Tree positions and DBH are recovered by fusing SLAM poses with segmented depth maps, scaled using a calibrated reference length.

Result: MAE of 1.51 cm (3.98%) in managed plots and 2.30 cm (5.69%) in natural plots; consistent performance across varying starting positions; reproducible cross-video localization.

Conclusion: Achieves accuracy comparable to established field methods with substantially reduced equipment cost and operational complexity, making it accessible for both professional researchers and non-expert forest managers.
...
TL;DR: RotateK is a rotation-based structured Key channel pruning method that addresses KV cache pressure in VLMs by compressing channel dimensions via PCA-based rotation, enabling accurate pruning with head-wise masks and efficient decoding, outperforming prior methods in accuracy and latency.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models suffer severe KV cache pressure because images encode into thousands of tokens. Existing token pruning methods discard visual content, causing degradation on fine-grained perception tasks, motivating feature sparsity via channel dimension compression.

Method: RotateK applies online PCA-based rotation to align token-dependent channel importance into a shared low-dimensional subspace, enabling accurate pruning under lightweight head-wise masks. A fused Triton attention kernel operates directly on sparse-channel Keys for efficient decoding.

Result: Consistently outperforms prior Key channel pruning in accuracy and decoding latency; joint token-channel pruning improves over token-only baselines at matched KV cache budgets.

Conclusion: Resolves the structural trade-off between expressive but slow token-wise pruning and hardware-friendly but less robust head-wise pruning, achieving both accuracy and efficiency.
...
TL;DR: HAVEN is a new hierarchically aligned multimodal benchmark for video understanding that provides fully granular (frame/shot/video) and fully multimodal annotations to evaluate how well MLLMs truly understand and summarize video narratives, revealing a gap between textual fluency and grounded multimodal comprehension.


<details>
  <summary>Details</summary>
Motivation: Existing summarization benchmarks for MLLMs fragment supervision across isolated granularities (keyframes, key shots, or disjointed text summaries) and fail to capture the inherently hierarchical structure of cross-modal alignment in video understanding.

Method: HAVEN provides a fully granular (frame, shot, and video levels) and fully multimodal (video and text) dataset architecture with explicit, continuous cross-modal alignment, and a comprehensive evaluation suite covering summarization, temporal reasoning, multimodal grounding, and saliency ranking.
...
Conclusion: HAVEN advances video understanding evaluation beyond traditional QA formats, offering a rigorous, standardized testbed for interpretable, hierarchical video understanding.
...
TL;DR: Proposes a robust framework to mitigate age-dependent confounding in medical image classification by targeting spurious age trends rather than enforcing strict age invariance, using Huber-weighted decorrelation and Age Coverage Score for stable optimization.


<details>
  <summary>Details</summary>
Motivation: Age acts as a confounder linking imaging morphology and disease prevalence, causing overdiagnosis/underdiagnosis disparities across ages; conventional strict age invariance methods suppress clinically meaningful age information.

Method: After warm-up, models age-dependent sample difficulty trends in a label-conditioned manner; uses Huber weighted affinity weights to decorrelate age from spurious difficulty trends; introduces Age Coverage Score to scale decorrelation penalty by minibatch age variance for stable optimization.

Result: Reduces age-dependent true and false positive disparities across two radiology datasets with minimal AUC impact, and remains robust under increasing train-test age distribution shifts.

Conclusion: Targeting spurious age trends (not all age information) effectively mitigates confounding-driven disparities while preserving clinically meaningful age information, outperforming invariance-based approaches.
...
TL;DR: PhyWorld is a video generation world model that improves physical faithfulness through two-stage post-training (flow matching fine-tuning for video continuation + DPO for physics alignment), achieving better consistency (0.769 VBench) and physical plausibility (3.09) than baselines.


<details>
  <summary>Details</summary>
Motivation: World simulators for Physical AI need physically faithful video continuations. Current large video generation models lack temporal coherence and physical consistency when used as world simulators.

Method: Two-stage post-training: (1) Flow matching fine-tuning for stable video-to-video continuation, (2) Direct Preference Optimization (DPO) over physics preference pairs to align with physical principles.

Result: PhyWorld achieves 0.769 on VBench (vs 0.756 for baselines) and 3.09 on physical-faithfulness benchmark (vs 2.99 for strongest baseline).
...
TL;DR: FairNAD uses an LLM to create structured, diverse search spaces for neural architecture search, combined with fair mutation strategies to efficiently discover high-performing architectures, achieving significant improvements on CIFAR-10, CIFAR-100, and ImageNet16-120.


<details>
  <summary>Details</summary>
Motivation: Current NAS methods are limited by predefined search spaces, and LLM-assisted NAS suffers from inefficient exploration due to biased or low-quality design ideas.

Method: Defines a high-level structural template of architectural attributes, uses an LLM to populate it by analyzing papers, and introduces FairNAD with multi-type mutation (fair idea sampling, Pareto-aware mutation, LLM-driven iterative mutation, fine-grained feedback loop).

Result: Achieves 0.84, 2.17, and 2.35 points improvement on CIFAR-10, CIFAR-100, and ImageNet16-120, respectively, compared to current SOTA methods.

Conclusion: FairNAD effectively leverages structured design knowledge and fair exploration to discover high-performing architectures in open-ended search spaces.
...
TL;DR: FSF-DMD eliminates the need for a separate fake-score network in Distribution Matching Distillation by using the generator's own pseudo-velocity as a proxy, achieving strong results on ImageNet-1K 256×256.


<details>
  <summary>Details</summary>
Motivation: To remove the memory and update overhead of the auxiliary fake-score estimator required in existing DMD approaches for few-step generation.

Method: Replaces the fake-score network with a generator-induced pseudo-velocity surrogate derived from the flow-map generator's endpoint pseudo-velocity; includes flow-map-consistent backward simulation and a self-teacher variant for training from scratch.

Result: Outperforms flow-map baselines on ImageNet-1K 256×256, achieves lower FID than DMD2 under flow-map initialization, and remains effective with flow-matching initialization and training from scratch.

Conclusion: The explicit fake-score tracker can be avoided in DMD when using flow-map generators, making the distillation process more efficient while maintaining or improving quality.
...
TL;DR: FPED is a functional-network prior-guided Mixture of Experts framework for interpretable brain decoding from fMRI that models different functional brain networks as specialized experts, achieving competitive semantic reconstruction with biological interpretability.


<details>
  <summary>Details</summary>
Motivation: Current fMRI-based visual reconstruction methods flatten fMRI signals from localized visual cortices into 1D vectors, disrupting brain's network topology and overlooking synergistic contributions of distributed functional networks for high-level visual semantics.

Method: FPED employs a Mixture of Experts framework where different functional brain networks are explicitly modeled as specialized experts, with adaptive routing to capture their complementary contributions to visual semantic understanding.

Result: Achieves highly competitive semantic reconstruction with only 0.68B parameters; learned routing dynamics reveal biologically meaningful correspondence between functional networks and modality-specific semantic processing.

Conclusion: Brain network-aware expert modeling is a promising direction for bridging neural decoding and biologically inspired artificial intelligence.
...
TL;DR: This paper systematically analyzes synthetic images from diffusion models for visual segmentation, identifying that dense scene composition and fine instance fidelity improve spatial representations. It proposes SENSE, a model-agnostic framework leveraging synthetic data to boost segmentation performance across architectures like DPT and Mask2Former, validated on Cityscapes, COCO, and ADE20K.


<details>
  <summary>Details</summary>
Motivation: While diffusion models produce photorealistic images, their potential in complex visual segmentation tasks remains underexplored. The authors aim to systematically analyze which factors of synthetic images make them useful for segmentation.

Method: SENSE: a unified, model-agnostic framework that uses flexible and scalable synthetic data (from diffusion models) to enhance segmentation. Compatible with diverse architectures (DPT, Mask2Former) and scales across model sizes.

Result: Synthetic images with dense scene composition and fine instance fidelity yield more discriminative spatial representations. SENSE significantly improves segmentation performance across Cityscapes, COCO, and ADE20K.

Conclusion: Synthetic data from diffusion models, when properly characterized (dense composition + instance fidelity), can effectively boost visual segmentation. SENSE provides a scalable, model-agnostic solution with strong generalization.
...
TL;DR: iGSP efficiently adapts Vision-Language Models to continual tasks by using implicit gradient subspace projection, achieving state-of-the-art accuracy with 42.7% fewer trainable and 86.9% fewer total parameters.


<details>
  <summary>Details</summary>
Motivation: Existing similarity-driven sharing mechanisms falsely equate superficial visual similarity with alignment consistency, causing negative transfer between visually similar but logically distinct tasks and failing to exploit reuse across diverse ones.

Method: iGSP frames alignment sharing as a geometric problem of overlapping optimization trajectories in shared low-rank subspaces. It uses MoE router convergence to establish subspace basis, then (1) Subspace Identification: pre-expands basis, applies subspace-constrained regularization to project new gradients onto historical subspace, prunes redundant dimensions via routing probabilities; (2) Orthogonal Subspace Fine-Tuning: fixes basis, removes regularization to fit task-specific residual.

Result: On MTIL benchmark, iGSP achieves SOTA accuracy, reduces trainable parameters by 42.7% and final total parameters by 86.9% versus current methods.

Conclusion: Treating alignment sharing as a geometric optimization problem enables efficient parameter reuse without negative transfer, significantly outperforming similarity-based sharing approaches.
...
TL;DR: Proposes a global distribution matching framework for 3DGS that uses multi-view contribution ranking and Optimal Transport to reduce redundant primitives by 90% while achieving 10x faster training and state-of-the-art rendering quality.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS compression methods rely on local sampling or fixed pruning thresholds, which fail to balance redundancy reduction with high-fidelity rendering due to massive redundant primitives causing significant overhead.

Method: (1) Multi-view 3D Gaussian contribution ranking using geometric consistency, (2) Global Optimal Transport (OT)-based aggregation to merge redundant primitives, (3) OT-based densification operator to maintain distributional properties.

Result: Achieves state-of-the-art rendering quality with only 10% primitives and 10× accelerated training speeds compared to vanilla 3DGS.

Conclusion: The proposed global distribution matching approach effectively balances primitive reduction and rendering fidelity, significantly outperforming existing local heuristic-based compression methods.
...
TL;DR: MetaRA is a metamorphic testing framework that systematically probes vulnerabilities in MLLM-based VQA systems, revealing failure patterns like sensitivity to linguistic perturbations and over-reliance on superficial visual cues that standard benchmarks miss.


<details>
  <summary>Details</summary>
Motivation: Existing VQA evaluations rely on static datasets and accuracy-based metrics that fail to capture robustness, consistency, and generalization of MLLMs.

Method: MetaRA uses Metamorphic Relations (MRs) to generate controlled variations of image-question inputs, then evaluates MLLM-based VQA models across diverse conditions.

Result: MetaRA reveals nuanced failure patterns including sensitivity to linguistic perturbations, over-reliance on superficial visual cues, and deeper multimodal reasoning weaknesses.

Conclusion: Metamorphic assessment offers a scalable, model-agnostic approach toward trustworthy multimodal AI beyond standard accuracy metrics.
...
TL;DR: SWEET uses image editing models (instead of dense video generation) to predict sparse task-level keyframes for robot manipulation, enabling efficient visual planning and action execution.


<details>
  <summary>Details</summary>
Motivation: Dense video generation is computationally expensive and often unnecessary for manipulation tasks, which can be summarized by a small number of task-relevant visual states.

Method: Proposes SWEET: (1) one-shot sparse visual planning via successive image editing (FLUX-Kontext) with language instructions and arrow-based spatial guidance; (2) goal-conditioned diffusion action predictor converts adjacent keyframes into actions; (3) mixed-training strategy with filtered edited targets to reduce visual mismatch.

Result: Image editing (FLUX-Kontext) outperforms video generation (Wan2.2) in keyframe fidelity and inference cost. SWEET improves keyframe prediction on DROID and RoboMimic and enables full planning-to-action pipeline.

Conclusion: Image editing is a promising and underexplored direction for embodied visual prediction, offering efficient sparse future state prediction for robot manipulation.
...
TL;DR: TextAlign is a non-invasive post-training framework that uses a hierarchical VLM-based reward to improve text rendering in text-to-image models via preference alignment (DPO/GRPO), achieving consistent OCR accuracy gains without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Faithful text rendering in text-to-image models remains challenging due to the need for both semantic instruction following and fine-grained glyph-level structure. Prior methods require architecture-specific modules or encoder modifications, complicating deployment across different foundation models.

Method: TextAlign frames text rendering as a post-training preference-alignment problem. It uses a hierarchical VLM-based reward that decomposes rendering errors into global, word, and glyph levels, converting binary defect judgments into a scalar preference signal. This signal supports both GRPO and DPO optimization.

Result: Experiments on FLUX.1-dev and Z-Image-Turbo show consistent gains in OCR-based text accuracy without degrading general generation quality. Outperforms baselines including SD3.5, Qwen-Image, AnyText, and TextDiffuser.

Conclusion: Reward design offers a scalable alternative to model redesign for improving text rendering in text-to-image models.
...
TL;DR: DynaTok is a training-free token compression framework for Video-LLMs that dynamically allocates token budgets across temporal and spatial dimensions, reducing visual tokens by 90% while retaining over 95% accuracy on VideoQA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing Video-LLMs suffer from prohibitive computational costs due to massive visual tokens from long video sequences. Current training-free compression methods overlook positional bias and rely only on short-term temporal locality, leading to redundant spatio-temporal coverage and inefficient token usage.

Method: DynaTok proposes two modules: (1) Temporal Budget Allocation (TBA) using EMA memory to dynamically assign fewer tokens to redundant frames and more to novel frames; (2) Spatial Budget Allocation (SBA) using activation-based attention maps with spatial memory to select diverse and semantically important features while mitigating positional bias.

Result: On MVBench, LongVideoBench, MLVU, and VideoMME, DynaTok retains over 95% of baseline accuracy even with 90% token reduction, surpassing recent training-free approaches, and integrates seamlessly with LLaVA-OneVision and LLaVA-Video without retraining.
...
TL;DR: RE-VLM is a dual-stream vision-language model combining RGB images and event camera data for robust scene understanding in adverse conditions, outperforming existing models on captioning and VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Conventional VLMs struggle under adverse conditions (low light, HDR, fast motion) because standard RGB images degrade. Event cameras offer complementary motion cues that frames fail to capture.

Method: Proposes RE-VLM with parallel RGB and event encoders, progressive training strategy for aligning heterogeneous visual features with language, and a graph-driven pipeline to generate synthetic captions and QA pairs from RGB-Event scene graphs.

Result: Outperforms state-of-the-art RGB-only and event-only models on captioning and VQA benchmarks, especially in challenging conditions, while maintaining comparable parameter counts.

Conclusion: Event-augmented VLMs achieve robust vision-language understanding across diverse real-world environments, demonstrating the value of event camera data for multimodal scene interpretation.
...
TL;DR: HERA proposes a three-stage (select-regularize-calibrate) framework that adapts frozen VFMs for cross-domain few-shot segmentation using only 2.7% fine-tuned parameters, achieving >4.1 mIoU improvement over SOTA.


<details>
  <summary>Details</summary>
Motivation: VFMs struggle with cross-domain few-shot segmentation due to (1) overfitting from limited exemplars relative to large-scale pre-training, and (2) target-domain shifts causing cross-domain inconsistency and layer-wise sensitivity.

Method: Three-stage pipeline: (1) Hierarchical Layer Selection (HLS) with Exemplar Transfer Risk (ETR) to pick the best VFM layer, (2) Prior-Guided Regularization (PGR) for well-structured local features, (3) Pixelwise Adaptive Calibration (PAC) combining selected representation with refined maps for consistent masks. Only <2.7% parameters fine-tuned at test time.

Result: Surpasses state-of-the-art by >4.1 mIoU across multiple CD-FSS benchmarks with minimal parameter fine-tuning (<2.7%).

Conclusion: HERA effectively adapts frozen VFMs to novel domains under few-shot constraints without source-data retraining, addressing overfitting and domain shift simultaneously.
...
TL;DR: SLVR is a two-stage framework that enriches latent representations with attribute-level semantics and uses M-GRPO alignment to improve multimodal latent-space reasoning, outperforming existing baselines on robustness and semantic consistency.
...
Method: A two-stage framework: (1) learns semantically enriched region-centric latents under fine-grained attribute supervision, (2) uses Multi-query Group Relative Policy Optimization (M-GRPO) to align latent representations across multiple queries grounded in the same region. Also constructs SLV-Set (400K attribute annotations + 800K QA samples) and SV-QA benchmark.

Result: SLVR improves robustness and semantic consistency of latent visual reasoning compared to existing baselines.

Conclusion: Semantic-enriched latent representations with multi-query alignment can effectively replace explicit visual thinking in latent space reasoning tasks.
...
TL;DR: The paper proposes MAM-CLIP, a vision-language model for mammography that uses contrastive learning on 2313 mammogram image-text pairs from atlases to improve BI-RADS score prediction, achieving +1% to +14% F1 score improvements, especially with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Mammogram interpretation is complex and suffers from inter-radiologist variability; training classification models solely on image labels yields limited performance due to the inherent complexity of mammography images.

Method: Curated 2313 mammogram images with captions from two atlases; used a multi-modal model with pretrained PubMedBERT as the language component; trained on image-text pairs via contrastive learning; then fine-tuned the vision encoder for BI-RADS prediction.

Result: Achieved +1% to +14% improvement in 3-class average F1 score over models without this pretraining (+1% with 40K samples, +14% with 1K samples); 2K image-text pairs can be more informative than 2K labeled samples (+1.1% margin with >10K training samples).

Conclusion: Vision-language pretraining using mammography atlas captions significantly improves BI-RADS prediction, especially in low-label regimes; textual information from atlases provides valuable supervision for mammogram understanding.
...
TL;DR: This paper presents a hybrid digital-analog deepfake video detection framework that uses a lightweight digital front-end and a spatially multiplexed optical decoding back-end. The system processes 15+ video streams simultaneously in a single optical pass, achieving ~97.8% accuracy on Celeb-DF while offering gains in throughput, energy efficiency, and adversarial robustness.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning-based deepfake detection methods rely on computationally intensive and energy-demanding inference algorithms, limiting their scalability. There is an urgent need for efficient, trustworthy detection systems.

Method: The hybrid framework combines a lightweight digital front-end with a spatially multiplexed optical decoding back-end using a programmable spatial light modulator. It processes 15+ video streams in parallel within a single optical propagation pass for massively parallel analog inference.

Result: Achieved 97.79% accuracy, 99.86% sensitivity, and 95.72% specificity on the Celeb-DF dataset with 15 videos tested in parallel in a single optical pass. The system also showed resilience against video degradation, noise, compression, misalignments, and black-box adversarial attacks.

Conclusion: Integrating optical computation into AI inference enables simultaneous gains in throughput, energy efficiency, and adversarial robustness—three properties difficult to achieve together in purely digital systems.
...
TL;DR: HDFM integrates blur-based (heat-dissipation) corruption into Flow Matching, addressing ill-posedness via interpolated paths and high-dimensional regression via x-prediction, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Blur-based diffusion preserves multi-scale detail and color budgets better than noise-based methods, but existing blur models are limited to SDE frameworks and face ill-posed inverse heat-dissipation and high-dimensional regression challenges in ODE-based Flow Matching.

Method: Proposes Heat Dissipation Flow Matching (HDFM) which introduces a continuous blurred heat-dissipation process into FM, uses an interpolated heat-dissipation path to resolve ill-posedness, and adopts x-prediction to reduce regression difficulty from high-dimensional noise/velocity space.

Result: Toy experiments and ablation studies confirm HDFM benefits from both blur and x-prediction; outperforms most baseline methods across all datasets.

Conclusion: HDFM successfully integrates blur-based corruption into ODE-based Flow Matching, overcoming key technical challenges and achieving superior performance.
...
TL;DR: CoNNS is a concept-guided noisy-negative suppression framework for vision-language alignment in chest X-ray analysis that improves zero-shot classification and grounding by constructing a hierarchical clinical concept ontology using LLMs and implementing a cross-patient pair relabeling strategy to address noisy negatives in contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Standard contrastive learning treats radiographs/reports from different patients as negative pairs, but different patients often share similar findings, creating noisy negatives that cause semantic ambiguity and degrade zero-shot understanding performance.

Method: (1) Builds a hierarchical concept ontology (41 clinical concepts) using LLMs to model presence, attributes, and texts; (2) Implements cross-patient pair relabeling with Fine-Grained Breakdown, Noisy Negative Filtering, and Hard Negative Mining; (3) Proposes Concept-Aware NCE loss to align visual features with text while suppressing noisy negatives.

Result: Outperforms existing state-of-the-art models across multi-granularity zero-shot grounding tasks and five zero-shot classification datasets.

Conclusion: CoNNS effectively addresses noisy negative pairs in medical vision-language alignment, demonstrating superiority over prior methods and providing open-source code.
...
TL;DR: This paper diagnoses training failures in Token-Choice sparse MoE on video Diffusion Transformers, identifying five failure modes (linear router saturation, MLP router deadlock, cross-attention partial recovery, U-shaped deadlock distribution, bfloat16 precision trap) and proposes the Functional Redundancy Hypothesis to explain deadlock as a rational waiting strategy.


<details>
  <summary>Details</summary>
Motivation: To systematically diagnose and understand training failure modes when converting pretrained dense video Diffusion Transformers to Token-Choice sparse MoE architectures, and to provide practical solutions for stable training.

Method: Converts a ~5B parameter pretrained dense model to MoE following three laws (routed experts clone FFN weights, shared experts initialized near-zero, gating networks random). Analyzes routing decisions over 65M tokens across 5K steps to identify failure modes.

Result: Identified five failure modes: linear router saturation/complete homogenization, MLP router selective deadlock (~1/3 layers degenerate), cross-attention partial recovery (~9 layers stuck), U-shaped deadlock distribution (shallow visual + deep semantic layers), and bfloat16 truncation of tiny weight updates.

Conclusion: Proposes Functional Redundancy Hypothesis: deadlock is a rational waiting strategy in the gate-shared expert-routed expert triad. Provides Three Laws for dense-to-MoE conversion, bfloat16 precision solution, and outlines a three-step roadmap from visual unification to world model.
...
TL;DR: MatPhys predicts spring-mass parameters from a single-view video using DINO features for part-level decomposition and a learned material codebook, achieving consistent physics predictions across scenes without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing physics-driven methods for deformable object reconstruction assume homogeneous materials and produce inconsistent parameters across different scenes due to per-scene inverse optimization.

Method: Uses DINO features for part-level decomposition, a learned material codebook as appearance-physics bridge, and part-level priors to constrain decoders for consistent parameter prediction.

Result: Matches per-scene optimization baselines in reconstruction/future prediction while achieving stronger generalization to unseen interactions and objects with more consistent physical parameters.

Conclusion: Feed-forward material-aware prediction with shared material concepts turns under-constrained monocular problem into consistent, generalizable physics inference.
...
TL;DR: This paper introduces Track4D-Bench for trajectory-grounded multi-turn spatiotemporal dialogue and proposes LMM-Track4D, which uses RTGE, TRK token, and OSK-RA decoder to improve 4D dynamic reasoning in large multimodal models.


<details>
  <summary>Details</summary>
Motivation: Current LMMs struggle with sustained 4D continuous spatiotemporal dynamic reasoning; this gap motivates the need for new benchmarks and modeling approaches.

Method: The authors formulate trajectory-grounded multi-turn spatiotemporal dialogue, build Track4D-Bench (526 samples, 23.5k frames, 7.5k annotations), and propose LMM-Track4D combining RTGE encoding, a TRK streaming state token, and OSK-RA decoder for 4-step 3D estimation.

Result: LMM-Track4D achieves consistent improvements over strong baselines on Track4D-Bench.
...
TL;DR: Medical image classifiers show demographic disparities because a single "dominant" penultimate-layer neuron co-activates for both disease and privileged groups (over-diagnosis) while another co-activates for disadvantaged groups (under-diagnosis). The authors propose NIR, a lightweight regularization that penalizes variance of probability-weighted activations across neurons (no demographic labels needed). On HAM10000, TPR disparity drops from ~11% to ~1% for age/gender; on OCT-RNFL, FPR disparity for race drops from 15.68% to 10.66% and age from 12.69% to 1.80%.


<details>
  <summary>Details</summary>
Motivation: Deep learning medical image classifiers exhibit systematic performance disparities across demographic groups (age, gender, race), undermining fairness in clinical AI. The mechanisms behind these disparities are poorly understood, and existing fairness methods often require demographic labels that are unavailable in practice.

Method: The authors first identify a latent representational mechanism: in transfer-learned models, the dominant penultimate-layer activation channel for positive predictions is co-activated by both disease-positive samples and privileged demographic groups (male, older), causing over-diagnosis; the dominant channel for negative predictions is co-activated by disadvantaged groups (female, younger), causing under-diagnosis. They then propose Neuron Incidence Redistribution (NIR), a regularization that penalizes the variance of predicted-probability-weighted mean activations across penultimate-layer neurons, requiring no demographic labels during training.

Result: On HAM10000: TPR disparity drops from 10.81% to 0.93% (age) and from 12.04% to 0.74% (gender), with marginal AUC improvement of 0.51 points. On Harvard OCT-RNFL: FPR disparity for race reduces from 15.68% to 10.66%, and for age from 12.69% to 1.80%.

Conclusion: Distributing latent disease evidence across the full penultimate layer is a principled and effective strategy for improving demographic fairness in medical AI, without requiring demographic labels at training time.
...
TL;DR: DyMoS is a training-free, model-agnostic method that mitigates motion suppression in image-to-video models by rebalancing self-attention from the reference frame during initial denoising steps, using a single scalar parameter for motion control.


<details>
  <summary>Details</summary>
Motivation: Image-to-video models generate overly static videos compared to text-to-video models, and prior solutions weaken the image condition at the cost of additional training or reduced fidelity.

Method: Identifies "reference-frame dominance" (excessive self-attention to reference key tokens in non-reference frames). Proposes DyMoS: a training-free method that rebalances attention from generated frames to the reference frame during early denoising steps, controlled by a single scalar parameter.

Result: Consistently improves motion dynamics across multiple state-of-the-art I2V backbones while maintaining visual quality and reference image fidelity.

Conclusion: DyMoS offers a simple, effective, and practical solution to the motion suppression problem in I2V without requiring retraining or model modifications.
...
TL;DR: VASA is a training-free, vision-guided agent for open ad-hoc segmentation that uses a persistent working mask and a VLM agent with segmentation tools to reason, construct, and validate segmentation solutions, outperforming baselines on new PARS benchmark and RefCOCOm.


<details>
  <summary>Details</summary>
Motivation: Segmentation for known concepts is easy but remains hard for open ad-hoc concepts where learned grounding doesn't exist and must be constructed from image evidence through parts, relations, exclusions, and collections.

Method: VASA is a training-free vision-guided agent that couples a VLM agent, a segmentation foundation model, and a visually grounded workflow. It uses a persistent working mask to plan operations, invoke tools, inspect results, edit masks, and recover from errors.

Result: On PARS benchmark, VASA outperforms baselines by 14-25% over SAM3 Agent. On RefCOCOm, improves over SAM3 Agent by 5-9% and over other agentic baselines by up to 20%.

Conclusion: Agentic visual construction is validated for open ad-hoc segmentation, pointing to a path for AI agents that goes beyond wrapping foundation models as tools—programming them with task knowledge, VLM behavior, visual routines, working memory, and failure-aware workflows.
...
TL;DR: KappaPlace is a framework for uncertainty-aware Visual Place Recognition using Prototype-Anchored supervision with von Mises-Fisher distributions, reducing calibration error by up to 50% while maintaining retrieval recall.


<details>
  <summary>Details</summary>
Motivation: Current VPR methods lack well-calibrated uncertainty estimation, making them unreliable for safety-critical autonomous navigation when queries are ambiguous or matches are incorrect.

Method: Proposes a Prototype-Anchored supervision strategy using latent class representatives as targets, models image descriptors as von Mises-Fisher (vMF) variables, and introduces a match-level uncertainty formulation for query-reference pairs.

Result: Reduces Expected Calibration Error (ECE@K) by up to 50% across five benchmarks while maintaining or improving retrieval recall.

Conclusion: KappaPlace provides a robust, stable, and well-calibrated uncertainty signal that enables reliable decision-making in VPR pipelines, with both joint-training and post-training variants available.
...
TL;DR: CADENet is a training-free, three-thread system for real-time object detection in adverse weather, using condition-adaptive enhancement with CLIP-based zero-shot weather classification and asynchronous dual-stream processing to avoid latency, achieving ~44 FPS without retraining or extra sensors.


<details>
  <summary>Details</summary>
Motivation: Adverse weather degrades camera-based object detection in autonomous vehicles. Existing enhancement-then-detect methods introduce latency that violates real-time requirements, and there is an under-recognized evaluation ceiling where ground truth annotations on degraded images cannot credit detectors that recover objects invisible to human annotators.

Method: Three-thread asynchronous system: Thread S (YOLOv11n) runs full-frame-rate detection with zero added latency; Thread Q applies condition-adaptive enhancement (CAPE) and fuses via entropy-guided NMS (EG-NMS) without blocking Thread S; Thread E uses CLIP for zero-shot weather classification requiring only a text prompt, no labeled data or retraining.

Result: On 1327 DAWN images (YOLOv11m, IoU=0.5, confidence=0.25): Recall=0.0103 (micro), F1=0.0230 on snow, F1=0.0038 on rain. Thread S sustains ~44 FPS regardless of enhancement load.

Conclusion: CADENet achieves real-time adverse-weather object detection without model retraining or additional sensors, with recall as the headline metric immune to annotation completeness bias. Reported F1 values are lower bounds on the true gain.
...
### [47] [Targeted Downstream-Agnostic Attack](https://arxiv.org/abs/2605.19446)
*Zhuxin Lei,Ziyuan Yang,Yi Zhang*

Main category: cs.CV

TL;DR: This paper proposes a Targeted Downstream-Agnostic Attack (TDAA) method that generates example-specific adversarial perturbations to make pre-trained encoders produce identical features for adversarial examples and a pre-selected "threat image," exposing encoder vulnerabilities under a stricter threat model.


<details>
  <summary>Details</summary>
Motivation: Pre-trained encoders are vulnerable to downstream-agnostic attacks (DAAs), but existing DAA methods operate under a permissive threat model (any prediction change is considered success). The authors aim to introduce a stricter threat model requiring attacks to be both targeted and downstream-agnostic, which is challenging because downstream tasks are unknown and encoders don't directly produce predictions.

Method: The method introduces a "threat image" (pre-selected target) and a generator that produces example-specific adversarial perturbations. These perturbations force the victim encoder to output identical features for both the downstream-agnostic adversarial examples (DAEs) and the threat image, using a feature-level anchor to create a task-agnostic bridge.

Result: Extensive experiments on 10 self-supervised methods across 3 benchmark datasets demonstrate the effectiveness of the proposed TDAA method and reveal the pronounced vulnerability of pre-trained encoders.

Conclusion: The proposed targeted attack paradigm exposes stronger vulnerabilities in pre-trained encoders than existing untargeted DAA methods, and the example-specific perturbation generation achieves better attack success rate and invisibility than shared perturbation approaches.
...
TL;DR: Cutverse is a benchmark evaluating GUI agents on 186 complex media post-production tasks across 7 professional apps, finding current agents achieve only 36% success due to challenges in long-horizon reliability and domain-specific planning.


<details>
  <summary>Details</summary>
Motivation: GUI agents have progressed in web navigation and basic OS tasks but remain underexplored in professional creative workflows like media post-production.

Method: Curated expert demonstrations across 7 professional apps (Premiere Pro, Photoshop) covering 186 complex tasks; developed a lightweight parser to transform screen recordings and interaction logs into structured GUI action trajectories.

Result: Existing agents achieve only 36.0% task success on realistic media editing tasks.

Conclusion: Current models show promising spatial grounding and multimodal alignment but are limited in long-horizon reliability and domain-specific planning, highlighting the need for further research in complex media post-production workflows.
...
TL;DR: This paper introduces World-Ego Modeling, a paradigm that decomposes future evolution into world (instruction-agnostic scene regularities) and ego (robot-centric instruction-conditioned dynamics) components to address the degradation in long-horizon embodied tasks. The authors propose the World-Ego Model (WEM) with a cascade-parallel mixture-of-experts diffusion generator and construct HTEWorld, a new benchmark for long-horizon world modeling with hybrid navigation-manipulation tasks, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing world models entangle world and ego predictions, leading to degradation in long-horizon embodied scenarios, especially in hybrid tasks that interleave navigation and manipulation.

Method: Proposes World-Ego Modeling paradigm that decomposes future evolution; defines world-ego boundary from motion, semantic, and intention perspectives; analyzes three disentanglement strategies (post, pre, full); instantiates as WEM with an implicit separate world-ego planner and cascade-parallel mixture-of-experts (CP-MoE) diffusion generator; constructs HTEWorld benchmark with 125K video clips and 300 evaluation trajectories.

Result: WEM achieves state-of-the-art performance on HTEWorld benchmark while remaining competitive on existing manipulation-only benchmarks.

Conclusion: World-Ego Modeling effectively addresses world-ego entanglement for long-horizon hybrid embodied tasks, and the HTEWorld benchmark enables rigorous evaluation in this challenging setting.
...
TL;DR: PathCTM enables token-efficient, scale-space continuous reasoning for gigapixel WSIs by dynamically transitioning from low- to high-magnification and adaptively stopping inference when enough evidence is gathered, reducing patches by 95.95% and inference time by 95.62% without sacrificing AUC.


<details>
  <summary>Details</summary>
Motivation: Traditional WSI analysis using MIL requires exhaustive patch-level processing at high magnification, which is computationally expensive and limits efficiency/scalability.

Method: PathCTM formulates diagnosis as dynamic sequential information pursuit with progressive low-to-high magnification transitions, attention-guided region pruning via conditional computation, and confidence-aware early stopping.

Result: PathCTM reduces required image patches by 95.95% and inference time by ~95.62% while maintaining AUC without degradation compared to standard MIL methods.

Conclusion: Adaptive, scale-space continuous reasoning with early stopping can dramatically improve WSI inference efficiency without compromising diagnostic accuracy.
...
TL;DR: ECP is a training-free framework using event cameras to guide token pruning in Video-LLMs, reducing 80% visual tokens while improving accuracy (37.62% vs 36.31%) and achieving 1.89x speedup.


<details>
  <summary>Details</summary>
Motivation: Quadratic attention cost of Transformer-based Video-LLMs makes dense visual tokens computationally expensive, and existing token pruning methods fail to preserve motion and geometric cues essential for dynamic spatial reasoning.

Method: ECP combines three stages: Event-Triggered Causal Sampling (keyframe selection), Event-guided Motion Saliency Filtering (suppress event-inactive tokens), and Event-Attention Ranking Fusion (calibrate spatial attention with motion dynamics).

Result: 80% token reduction, accuracy 37.62% vs 36.31% baseline, 1.89x inference speedup, 52% GFLOPs reduction. Introduces ESR-Real benchmark.

Conclusion: Event-guided token pruning effectively preserves motion/geometric cues for first-person spatial reasoning while drastically reducing computational cost.
...
TL;DR: MetaTrans proposes a simple yet effective unsupervised video domain adaptation method that uses only two loss terms and a temporal-static subtraction module to separately handle spatial and temporal divergence, achieving state-of-the-art results on cross-domain action recognition.


<details>
  <summary>Details</summary>
Motivation: Unsupervised video domain adaptation (UVDA) is a practical but under-explored problem. Existing methods are often complex; a simpler yet effective solution is needed.

Method: MetaTrans adopts a concise learning objective with only two fundamental loss terms. It uses a temporal-static subtraction module in the model architecture to separately handle spatial and temporal divergence in cross-domain videos.

Result: Substantial absolute adaptation performance enhancement and significantly superior relative performance gain compared with state-of-the-art UVDA baselines on various cross-domain action recognition tasks.

Conclusion: A frustratingly easy UVDA method with simple learning objective and clever architecture design can effectively address spatial and temporal divergence, outperforming existing methods.
...
TL;DR: SafeMark enables watermarked images to be semantically edited without compromising watermark integrity by integrating a watermark-decoding loss into diffusion editor training.


<details>
  <summary>Details</summary>
Motivation: To investigate whether watermarked images can remain editable (e.g., via text-guided manipulation) without losing the ability to recover the embedded watermark, addressing an underexplored question in trustworthy image provenance.

Method: Adds a thresholded watermark-decoding loss to the diffusion editor's training objective, fine-tuning the editor so that semantically valid edits preserve the watermark. Provides an information-theoretic justification: high bit-accuracy lower-bounds mutual information between watermark and edited output.

Result: Achieves high watermark bit accuracy across diverse editing settings and post-edit distortions while maintaining high-quality semantic edits, with no architectural changes required.
...
TL;DR: iDiff is a dual-branch framework (Answer Model + Thinking Model) for pairwise image quality assessment that jointly predicts preferences and generates grounded rationales, winning first place in the NTIRE 2026 RAIM challenge.


<details>
  <summary>Details</summary>
Motivation: Pairwise IQA in professional photography requires not only identifying the preferred image but also providing convincing, image-grounded reasoning. The NTIRE 2026 RAIM challenge further emphasizes joint evaluation of both preference prediction and rationale generation.

Method: Dual-branch design: (1) Answer Model does preference prediction via decomposing samples into left/right global+local views, content-aware specialization (person/scene), and ensemble aggregation. (2) Thinking Model handles rationale generation with expert-style templates, multi-source quality features, and answer-aware supervision conditioned on Answer Model predictions.

Result: First place in NTIRE 2026 RAIM challenge; demonstrated effectiveness on both accuracy and reasoning-quality metrics.

Conclusion: Integrating explicit difference modeling with structured multimodal reasoning improves both robustness and interpretability for pairwise IQA.
...
TL;DR: DPL-ReID introduces dual prompt learning, real-world occlusion augmentation, and weighted gated feature fusion to improve occluded person re-identification using vision-language models, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language ReID methods ignore semantic information of occluders, and occlusions disrupt body-region cues, making cross-view matching difficult.

Method: 1) Dual Prompt Learning (Dual-PL) strategy using textual cues for complete pedestrian semantics. 2) Real-World Occlusion Augmentation (RWOA) to simulate realistic occlusions. 3) Weighted Gated Feature Fusion (WGFF) with LSNet for global information and feature gating.

Result: State-of-the-art performance on several benchmark occluded ReID datasets.

Conclusion: The proposed dual prompt learning and occlusion-aware augmentations effectively improve robustness for occluded person re-identification.
...
TL;DR: Proposes equation-anchored tool-use framework for 3D MLLMs that explicitly writes camera back-projection equations in CoT reasoning, enabling deterministic propagation of camera intrinsics for robust 3D localization across varying camera parameters.


<details>
  <summary>Details</summary>
Motivation: 3D localization in MLLMs suffers from camera intrinsic ambiguity - same image can correspond to different 3D scenes under different cameras. Existing MLLMs either ignore camera parameters (overfitting to training intrinsics) or treat retrieved 3D cues as numerical hints rather than deterministic values.

Method: Proposes equation-anchored tool-use framework that: (1) proactively retrieves camera intrinsics and multi-point metric depths, (2) explicitly writes pinhole back-projection equation (X̂ = (u_c - c_x)Z̄/f_x) in Chain-of-Thought, (3) substitutes tool outputs into the formula before regressing 9-DoF bounding boxes.

Result: Outperforms RGB-only and tool-augmented baselines on both 3D object detection and 3D visual grounding tasks under rescaled camera intrinsics (0.5x to 1.5x), with largest gains when camera deviates most from training scale.

Conclusion: Explicit equation anchoring in CoT reasoning enables robust generalization across varying camera intrinsics without overfitting to a canonical training intrinsic.
...
TL;DR: A training-free method (ABSS) that selects optimal random seeds for text-to-image diffusion models by analyzing attention dynamics over content-bearing words during early denoising steps, improving generation quality without modifying the model.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models are sensitive to random seeds; different seeds produce highly variable image quality and prompt alignment, yet no efficient method existed to pre-select good seeds without costly full generation or model modification.

Method: ABSS leverages cross-attention to core (content-bearing) tokens during early denoising steps to score and rank candidate seeds, then keeps only top-k seeds for full generation—no finetuning, no threshold, no noise alteration required.

Result: Consistent improvements in text-image alignment and visual quality across three benchmarks for Stable Diffusion variants, validated by both human preference and automated alignment metrics; also serves as a lightweight add-on to boost existing seed-optimization pipelines.

Conclusion: Attention dynamics over core tokens during early denoising steps strongly predict generation quality, and ABSS effectively exploits this for training-free, inference-time seed selection that improves diffusion model outputs.
...
TL;DR: RepL replaces selected neural network blocks with lightweight synthesized operators during training, reducing redundancy and cost while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Full-depth backpropagation in deep neural networks suffers from parameter redundancy, high activation-memory cost, and training latency, especially when neighboring layers have correlated learning patterns. Simple layer removal weakens representation capacity.

Method: RepL replaces selected blocks with lightweight computing layers that synthesize surrogate operators from adjacent preceding and succeeding blocks' parameters via learnable transformations, preserving local contextual continuity without full-layer computation.

Result: RepL reduces trainable parameters, GPU memory, and training time while matching or surpassing standard training on CIFAR-10, SVHN, STL-10, ImageNet, COCO, CityScapes across classification, detection, and segmentation tasks.

Conclusion: RepL is a general, training-time paradigm that effectively reduces full-depth redundancy across architectures (CNNs, ViTs) and tasks, with compatibility demonstrated for transfer learning, inference throughput, stochastic depth, and INT8 quantization.
...
TL;DR: Introduces CaptchaBench (16K samples, 8 tasks) and CaptchaMind (RL-based solver) achieving 82.9% success rate on benchmarks and 71.0% on real-world CAPTCHAs, significantly outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: CAPTCHAs block intelligent agents from web automation; existing methods lack large-scale training data and process annotations for solving modern multi-step visual CAPTCHAs.

Method: Created CaptchaBench with 16,000 programmatically generated samples across 8 task categories with region/process annotations. Developed CaptchaMind, an RL-based solver trained with explicit reasoning process supervision.

Result: 82.9% average success rate across 8 tasks on CaptchaBench; 71.0% on real-world instances; substantially outperforms all existing methods without closed-source APIs.

Conclusion: CaptchaBench enables large-scale training for CAPTCHA solving, and CaptchaMind demonstrates that RL with reasoning process supervision is effective for multi-step visual reasoning tasks.
...
TL;DR: Trust3R is a lightweight evidential uncertainty framework for feed-forward 3D reconstruction that uses a Normal-Inverse-Wishart distribution to produce probabilistically grounded per-point geometric uncertainty, achieving 25% lower AURC and 41% lower AUSE on ScanNet++ while improving geometric accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing geometric foundation models produce heuristic confidence scores that lack probabilistic interpretation and fail to reliably indicate where and how much predicted geometry can be trusted.
...
Result: 25% lower AURC and 41% lower AUSE on ScanNet++; consistently improves risk-coverage, sparsification, and geometric accuracy across diverse indoor/outdoor benchmarks.

Conclusion: Trust3R provides a practical, probabilistically grounded uncertainty signal for downstream geometry pipelines with moderate inference overhead.
...
TL;DR: SCDiff enhances T2I creativity via LSW (spatial weighting) and VSML (dual loss for alignment + diversity).


<details>
  <summary>Details</summary>
Motivation: Current T2I models prioritize literal text-image alignment and produce outputs lacking authentic creativity (novelty, surprise, artistic value).

Method: Proposes SCDiff with: (1) LSW module using a parametric Kaiser-Bessel window to reinforce central features, (2) VSML with similarity loss (text alignment) and diversity loss (distinction from original image).

Result: Substantial improvements in creativity, semantic alignment, and visual coherence demonstrated via extensive experiments.

Conclusion: SCDiff offers a simple yet powerful framework for generating creative objects in T2I tasks.
...
TL;DR: A sparse epipolar matching framework with diffusion-based refinement and graph-based subset selection for robust relative pose estimation from minimal correspondences.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based relative pose estimation relies on dense matching or direct regression, leading to redundancy and reduced geometric interpretability. A more efficient, geometrically meaningful approach with minimal correspondences is needed.

Method: Proposes a sparse epipolar matching framework with (1) epipolar diffusion process for correspondence refinement, (2) Steiner graph representation encoding relational structure, (3) GNN for subset selection, and (4) differentiable SVD solver for end-to-end pose estimation.

Result: Combining sparse matching, diffusion-based refinement, and graph-based subset selection reduces correspondence redundancy while maintaining robust pose estimation across challenging baselines on TartanAir and KITTI SLAM datasets.

Conclusion: The proposed approach achieves efficient and geometrically interpretable relative pose estimation by leveraging only a minimal set of refined, informative correspondences.
...
TL;DR: EgoCoT-Bench is a new benchmark for evaluating grounded, verifiable operation-centric reasoning in egocentric videos, featuring 3,172 QA pairs across 351 videos with explicit step-by-step rationale annotations.


<details>
  <summary>Details</summary>
Motivation: Existing egocentric video benchmarks lack grounded rationale evaluation, offering limited support for fine-grained operation-centric reasoning and rarely examining whether model rationales are grounded in explicit spatio-temporal evidence.

Method: The benchmark is constructed using a spatio-temporal scene graph (STSG) guided generation framework, refined by human annotators, comprising 3,172 verifiable QA pairs over 351 egocentric videos across 4 task groups (12 sub-tasks) covering perception, retrospection, anticipation, and high-level reasoning.

Result: Experiments show that multimodal models still struggle with egocentric fine-grained reasoning, and many produce answer-correct explanations where the evidence is inconsistent with the answer.

Conclusion: EgoCoT-Bench serves as a useful testbed for grounded and verifiable reasoning in egocentric video understanding.
...
TL;DR: Lens Privacy Sealing (LPS) is a low-cost hardware solution using adjustable scattering film to physically obscure camera lenses for privacy-preserving action recognition, paired with MSPNet to recover utility from degraded video.


<details>
  <summary>Details</summary>
Motivation: Existing privacy-preserving methods rely on post-capture algorithms that fail to protect privacy during data acquisition; the goal is to achieve pre-sensor privacy protection at minimal cost.

Method: LPS uses stochastic multi-layer scattering film to physically obscure lenses; MSPNet combines Inter-Frame Noise Suppressor (IFNS) and Cross-Frame Semantic Aggregator (CFSA) with CLIP-based contrastive learning for robust action recognition from degraded video.

Result: MSPNet nearly doubles action recognition accuracy over baselines while suppressing identity recognition; LPS resists reconstruction attacks and achieves superior privacy-utility trade-off vs. state-of-the-art hardware methods.

Conclusion: LPS offers a practical, physically irreversible hardware solution for pre-sensor privacy; MSPNet effectively recovers utility, enabling privacy-preserving action recognition in real-world surveillance.
...
TL;DR: This paper proposes YOLO26-MoE, which integrates a sparse Mixture-of-Experts module into YOLO26 for UAV-based electrical power line insulator fault detection, achieving 0.9900 mAP@0.5 and outperforming latest YOLO versions.


<details>
  <summary>Details</summary>
Motivation: Insulator fault detection is critical for grid reliability but remains challenging due to small defect regions, heterogeneous fault patterns, complex backgrounds, and varying imaging conditions.

Method: Proposes YOLO26-MoE, integrating a sparse Mixture-of-Experts (MoE) module into the high-resolution branch of YOLO26 for adaptive feature refinement, with hyperparameter optimization via a tool-augmented LLM agent.

Result: Achieved 0.9900 mAP@0.5 and 0.9515 mAP@0.5:0.95, outperforming the latest YOLO versions.

Conclusion: The proposed YOLO26-MoE provides an effective and reliable solution for UAV-based insulator fault detection.
...
TL;DR: Two novel open datasets (DTE-aerial-train with 385K patches and DTE-aerial-bench with 525 globally distributed patches) enable joint segmentation of tree cover and mortality from centimeter-scale aerial imagery, achieving significant improvements (e.g., F1 score from 0.40 to 0.58 in boreal forests).


<details>
  <summary>Details</summary>
Motivation: Forests face increasing threats from climate change and disturbances, but scalable monitoring of tree cover and mortality is limited by the lack of globally representative, harmonized datasets for joint segmentation from aerial imagery.

Method: Created two datasets: DTE-aerial-train (385K 1024x1024 patches, 2.5-20 cm resolution, expert-annotated pseudo-labels) and DTE-aerial-bench (25 globally distributed orthoimages, 525 patches, high-quality expert annotations). Both span tropical, temperate, boreal, and dryland biomes. Established reference baselines using the benchmark test set.

Result: Significant improvement in mortality segmentation across all biomes and scales, with particularly notable gains in boreal forests where F1 score increased from 0.40 to 0.58 (~45% relative improvement).

Conclusion: These open datasets and baselines enable scalable, globally applicable monitoring of tree cover and mortality from aerial imagery, addressing a critical gap in forest disturbance monitoring.
...
TL;DR: Spectral Integrated Gradients (SIG) improves feature attribution by using SVD-based integration paths that follow a coarse-to-fine progression, producing cleaner and more accurate attribution maps than standard Integrated Gradients.


<details>
  <summary>Details</summary>
Motivation: Standard Integrated Gradients uses a straight-line integration path that introduces all features simultaneously, accumulating noisy gradients and producing suboptimal attribution maps.

Method: SIG constructs integration paths via SVD of the baseline-to-input difference, progressively activating singular components from largest to smallest (coarse-to-fine).

Result: SIG produces cleaner attribution maps with reduced noise and achieves improved quantitative performance across diverse image classification datasets compared to existing path-based methods.

Conclusion: SVD-based path selection enhances feature attribution quality by following a natural coarse-to-fine structure, effectively addressing the noise accumulation problem in standard IG.
...
TL;DR: A physics-guided diffusion framework with FiLM conditioning and surrogate EM simulator enables fast inverse design of manufacturable metasurface absorbers, achieving high accuracy (MSE 0.0006, band alignment 0.958) in ~30 seconds versus months for conventional methods.


<details>
  <summary>Details</summary>
Motivation: Conventional metasurface design uses iterative full-wave simulations that are extremely time-consuming and computationally intensive. Existing generative approaches suffer from limited conditional fidelity and produce impractical (fine/irregular) features.

Method: A diffusion framework with FiLM-based conditioning for target reflection spectra, integrated with a pre-trained surrogate EM simulator for physics-aware regularization via spectrum-level loss functions.

Result: Average spectral MSE of 0.0006, band alignment accuracy of 0.958, design generation in ~30 seconds (vs months conventionally), demonstrated via experimental measurements.

Conclusion: The proposed framework enables fast, accurate, and diverse inverse design of practically realizable metasurface absorbers, dramatically reducing design time while maintaining high conditional fidelity and manufacturability.
...
TL;DR: VLM-CC proposes an iterative color constancy framework using vision-language model feedback to guide white balance correction, achieving state-of-the-art cross-camera generalization.


<details>
  <summary>Details</summary>
Motivation: Learning-based color constancy models overfit to specific camera color responses, causing poor cross-camera generalization.

Method: Formulates color constancy as iterative refinement: at each iteration, white-balances the image, uses a LoRA-tuned VLM to assess residual color cast, maps feedback to RGB correction direction, and updates illuminant estimate until convergence.

Result: Achieves state-of-the-art robustness in cross-camera color constancy across multiple datasets.

Conclusion: Reframing color constancy as iterative perceptual feedback with VLM evaluation, instead of direct RGB regression, improves cross-camera generalization.
...
TL;DR: BMLiCap is a coarse-to-fine framework for LiDAR-based 3D human motion capture that uses temporally compressible Bézier curves and a progressive motion-reconstruction module (with TMT and MMA) to achieve state-of-the-art accuracy and temporal continuity, effectively handling occlusions and noise.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-based 3D human motion capture methods struggle with unstable inputs and severe occlusions, resulting in jittery or failed pose predictions.

Method: Proposes a coarse-to-fine framework (BMLiCap) that models motion using temporally compressible Bézier curves with a trajectory-preserving control point reduction strategy, plus a progressive motion-reconstruction module including a Time-scale Motion Transformer (TMT) for multi-scale curve prediction and a Multi-level Motion Aggregator (MMA) for adaptive fusion.

Result: Achieves state-of-the-art accuracy and temporal continuity on four benchmarks (LiDARHuman26M, FreeMotion, NoiseMotion, SLOPER4D), effectively compensating for severe occlusions and reducing prediction jitter.

Conclusion: BMLiCap bridges observation gaps caused by occlusions and noise, demonstrating robust motion reconstruction in complex scenes.
...
TL;DR: UniRefiner is a universal framework that teaches pre-trained Vision Transformers to identify and remove three types of spurious tokens (artifacts that fail to encode location-aligned semantics), improving performance on dense prediction tasks like segmentation by up to 22% with minimal fine-tuning (5k images, few epochs).


<details>
  <summary>Details</summary>
Motivation: Existing Vision Transformers suffer from spurious tokens that corrupt spatial representations, and prior methods define these artifacts too narrowly (e.g., only as high-norm outliers). The authors argue for a broader definition: any token failing to encode location-aligned semantics should be treated as spurious.

Method: UniRefiner uses contrastive registers with a dual objective: (1) align image tokens with filtered regular tokens to preserve semantics, and (2) align register tokens with detected spurious tokens to capture spurious signals. It fine-tunes pre-trained ViTs on ~5k images for only a few epochs.

Result: Refined EVA-CLIP-8B achieves 51.9% mIoU on ADE20K (+9.4%), surpassing DINOv2 (49.1%). Zero-shot segmentation improves by up to 22%. Consistent improvements across diverse ViTs including massive 6B-8B parameter models.

Conclusion: UniRefiner unlocks the latent spatial potential of large-scale foundation models, enabling their broader application in spatially sensitive tasks with minimal additional training cost.
...
TL;DR: PrAda adapts frozen text-prompted segmentation models to specialized domains using few-shot learned prototypes, fixing misclassification errors under domain shift.


<details>
  <summary>Details</summary>
Motivation: Text-prompted segmentation models degrade on specialized target domains far from pre-training data, with misclassification being the main error source rather than mask generation.

Method: PrAda learns class-specific prototypes by combining fine-grained pixel features with high-level transformer representations, fused with original text-based predictions via a learned importance factor, while keeping the base model frozen.

Result: Outperforms state-of-the-art and proposed baselines on semantic, instance, and panoptic segmentation across five benchmarks.

Conclusion: Parameter-efficient prototype adaptation preserves zero-shot potential while enabling strong domain adaptation for text-prompted segmentation.
...
TL;DR: A component-aware style transfer framework translates synthetic satellite images to realistic ones while preserving geometric annotations, improving monocular 6D pose estimation for non-cooperative satellites in a Sim2Real setup.


<details>
  <summary>Details</summary>
Motivation: Monocular 6D pose estimation for non-cooperative satellites requires annotated training data, but real satellite images with reliable pose labels and component-level masks are difficult to acquire at scale. Synthetic rendering provides exact geometric annotations, yet the appearance gap between rendered and real observations limits direct transfer to the real domain.

Method: The method builds weakly paired real–synthetic samples from calibrated real acquisition, ArUco-based camera-pose measurement, CAD rendering, and component masks. It extracts part-wise real-domain style codes from unlabeled real images and injects them into corresponding synthetic satellite regions through mask-aligned modulation. Training combines adversarial loss with local contrastive consistency, self-regularization, and edge-preserving constraints.

Result: The method achieves FID of 54.32 and KID of 0.048. Using translated data to train GDRNet, ADD pass rate improves to 0.260 and AUC to 0.611, outperforming representative image-translation baselines.

Conclusion: Component-level appearance transfer can improve satellite Sim2Real pose estimation in a calibrated setup while retaining simulation-derived geometric annotations.
...
TL;DR: P2DNav proposes a hierarchical zero-shot VLN framework that decomposes navigation into panoramic direction selection and downview local grounding, using sliding-window dialogue memory and reflective reorientation, achieving 146.6% and 58.9% SR gains over SOTA zero-shot methods on R2R-CE.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot VLN methods rely on waypoint prediction modules that entangle high-level directional reasoning with fine-grained local grounding, causing error-prone and unstable decisions.

Method: P2DNav consists of three components: (1) Panorama-to-Downview (P2D) decomposes decision-making into panoramic direction selection and downview pixel-level target prediction; (2) Sliding-Window Dialogue Memory (SDM) maintains navigation history as multi-turn dialogue with recent visual observations; (3) Reflective Reorientation Mechanism (RRM) assesses local grounding reliability and falls back to panoramic selection when needed.

Result: On R2R-CE benchmark, P2DNav achieves 146.6% SR improvement over SOTA waypoint-based methods and 58.9% over waypoint-free zero-shot methods.

Conclusion: The hierarchical decomposition and reflective reorientation strategy effectively address the entanglement problem in zero-shot VLN, leading to strong performance.
...
TL;DR: This paper introduces R^3-Bench (a benchmark for reflective visual generation) and R^3-Refiner (a dual-stage framework using GRPO and HRM) to bridge the gap between identifying and rectifying generation errors in Text-to-Image models, achieving +12.0% in Reflective Verdict Score and +9.0% in Rectification Score.


<details>
  <summary>Details</summary>
Motivation: Existing T2I and UMM models rely on single-pass generation, limiting their ability to handle complex prompts that need iterative refinement. The paper identifies a critical gap: models can detect errors but cannot generate actionable rectification instructions.

Method: Formalizes the Reason-Reflect-Rectify (R^3) loop framework; introduces R^3-Bench benchmark (600+ expert-annotated instances); proposes R^3-Refiner with dual-stage framework using Group Relative Policy Optimization (GRPO) and Hierarchical Reward Mechanism (HRM).

Result: R^3-Refiner achieves +12.0% in Reflective Verdict Score and +9.0% in Rectification Score on R^3-Bench; enhances generation quality of different T2I models on GenEval++ and T2I-CompBench.

Conclusion: The proposed framework successfully bridges the rectification gap in reflective visual generation and can be seamlessly integrated with various MLLMs.
...
TL;DR: NeRF-based image augmentation enables training of spacecraft pose estimators from as few as 25-400 real images, eliminating the need for CAD models or large synthetic datasets.


<details>
  <summary>Details</summary>
Motivation: Current spacecraft pose estimation relies on tens of thousands of CAD-rendered synthetic images, which (i) requires reliable geometry priors (not possible for uncooperative/poorly documented spacecraft) and (ii) generalizes poorly to real on-orbit conditions due to unrealistic illumination and appearance.

Method: Learn a Neural Radiance Field (NeRF) of the target spacecraft from a few real images, then generate a large, diverse training dataset through geometrically-consistent viewpoint and appearance augmentation. This augmented dataset is used to train pose estimators.

Result: Accurate pose estimators can be trained from only 25 to 400 realistic images, even under severe illumination variations. When applied to CAD-based synthetic datasets, NeRF augmentation also improves out-of-domain generalization to real on-orbit conditions.

Conclusion: NeRF-based augmentation removes the dependency on CAD models and large synthetic datasets for spacecraft pose estimation, enabling practical deployment for uncooperative targets and improving robustness to real-world conditions.
...
TL;DR: Cross-View Splatter is a feed-forward method that fuses ground-level photos and satellite imagery to generate pixel-aligned Gaussian splats for outdoor 3D scene reconstruction, improving coverage and novel-view synthesis over ground-only approaches.


<details>
  <summary>Details</summary>
Motivation: Ground imagery is time-consuming and hard to capture at scale for large outdoor scenes; satellite imagery provides an easily accessible global geometric prior that can complement ground photos.

Method: Cross-View Splatter fuses orthorectified satellite views with GPS-tagged ground photos to predict Gaussian splats in a unified 3D coordinate frame, aligning ground and bird's-eye feature representations.

Result: The method improves scene coverage and novel-view synthesis quality compared to using ground imagery alone; evaluated on a new curated benchmark with georeferenced data.

Conclusion: Fusing cross-view (ground + satellite) imagery enhances 3D reconstruction and novel-view synthesis for large outdoor scenes, with code and data to be released.
...
TL;DR: JPEG quantization table mismatch between training and real-world data harms document forgery localization; using operationally sampled tables (DocQT) and architectures that explicitly ingest quantization tables improves robustness and reduces false positives.


<details>
  <summary>Details</summary>
Motivation: Current document manipulation localization models fail to generalize from public benchmarks to real-world insurance document workflows due to a mismatch between the narrow JPEG quantization table distributions used during training and the diverse compression profiles found in practice.

Method: Controlled factorial study comparing FFDN and Mesorch architectures trained with standard quality factor augmentation (Standard-QT) vs. operationally calibrated quantization tables from DocQT (Real-QT), evaluated under three recompression conditions.

Result: Training with Real-QT improves localization on DocTamper and reduces pixel-level false positive rates on authentic operational documents, but only for architectures that explicitly ingest quantization table as input.

Conclusion: Standard quality factor augmentation inadequately proxies real-world compression diversity; architectures conditioning on quantization tables offer meaningful robustness advantage for deployment.
...
TL;DR: WBCAtt+ is a new dataset of white blood cell images with 11 morphological attributes and 5 pixel-level cell components annotations, enabling attribute recognition, segmentation, and explainable AI applications.


<details>
  <summary>Details</summary>
Motivation: Existing WBC datasets only annotate cell categories, lacking detailed morphological characteristics that pathologists use for interpretation.

Method: Created WBCAtt+ dataset with 113k image-level attribute labels and 10k segmentation maps; built baseline models for attribute recognition and segmentation; designed an attribute recognition model incorporating compositional cell structure.

Result: First dataset providing comprehensive annotations (11 attributes + 5 pixel-level components) for WBC images; improved recognition performance via compositional modeling.

Conclusion: WBCAtt+ enables explainable AI applications like counterfactual example generation, advancing interpretable WBC analysis.
...
TL;DR: ACOUSIM is a physics-informed framework that validates synthetic sonar images by comparing their intensity and texture distributions against real datasets, achieving strong texture alignment (KL < 0.07).


<details>
  <summary>Details</summary>
Motivation: Synthetic sonar datasets are scalable but lack rigorous quantitative validation; ACOUSIM bridges this gap without relying on generative models.

Method: Gazebo-based simulation with controlled seabed texture, shadowing, altitude, and noise; realism measured via KL divergence, JS divergence, and Earth Mover's Distance on global intensity and local LBP texture distributions.

Result: Strong texture alignment (KL < 0.07) across all classes; plane-class intensity alignment outperforms ship-class due to shadow geometry complexity.

Conclusion: ACOUSIM provides a reproducible, distribution-level baseline for sim-to-real sonar evaluation and supports reliable dataset validation for underwater image analysis.
...
TL;DR: A hybrid AI architecture that embeds physics-based engineering tools into LLM agents to generate physically valid CAD designs, achieving 4.2x structural complexity and 3.5% better compile rates.


<details>
  <summary>Details</summary>
Motivation: LLMs can generate CAD but lack physical comprehension needed for reliable engineering design; implicit learning of physics from data is insufficient.

Method: Hybrid Agentic-Physical Architecture: closed-loop sequential decision-making with dedicated agents (plan, generate, evaluate, revise) guided by explicit physical verification via knowledge-based engineering tools.

Result: 4.2x increase in structural complexity and 3.5% improvement in compile rate compared to similar agentic methods.

Conclusion: Embedding validated physics tools into AI agent loops produces more complex and physically valid CAD designs; benchmark dataset and metrics introduced.
...
TL;DR: BA-Att is a block-sparse attention framework that identifies informative regions via pre-downsampling rather than fixed positional priors, achieving up to 6.95x acceleration over FlashAttention with near full-attention performance at 50% sparsity.


<details>
  <summary>Details</summary>
Motivation: Diffusion Language Models (DLMs) suffer from high computational costs when scaling to ultra-long sequences. Existing block-sparse attention methods rely on fixed sampling patterns (e.g., tail regions, anti-diagonal stripes) that can miss salient tokens and are unstable under distribution shifts.

Method: Proposes Block Approximate Sparse Attention (BA-Att) using block-wise pre-downsampling to identify informative regions in a compact space. Introduces a lightweight norm-sorting module and covariance-compensated correction that approximates full covariance using diagonal QK variances.

Result: Achieves up to 6.95x acceleration over FlashAttention in attention computation. Maintains near full-attention performance at 50% sparsity across language models, multimodal language models, and video generation models.

Conclusion: BA-Att provides a more robust and efficient sparse attention mechanism for DLMs by avoiding brittle positional priors, with strong generalization across multiple domains.
...
TL;DR: Tango3D unifies dense pixel-to-point correspondence and global retrieval in a single 3D foundation model using a geometry-aware 2D backbone, 3D VAE, and three-stage training.


<details>
  <summary>Details</summary>
Motivation: Existing 3D foundation models align point clouds to frozen vision-language spaces via global vectors, but cannot establish fine-grained pixel-to-point correspondence.

Method: Uses a geometry-aware 2D visual backbone + pretrained 3D VAE to encode images into 2D patches and point clouds into 3D tokens, mapped into a shared space with both local pixel-to-point and global semantic alignment, trained via a three-stage progressive strategy.

Result: Achieves object-level pixel-to-point alignment while maintaining competitive global retrieval, a joint capability not offered by existing 3D foundation models.

Conclusion: Tango3D injects rich semantics into purely geometric 3D tokens via fine-grained alignment, enabling dense 3D downstream tasks.
...
TL;DR: Aero-World converts pretrained image-to-video diffusion models into controllable aerial video generators via action-token streams and Physics Probe supervision, while AeroBench evaluates action alignment and motion stability for drone videos.


<details>
  <summary>Details</summary>
Motivation: Foundation video models are trained on natural language, not low-level control signals, limiting their use in embodied AI — especially for aerial flight where 6-DoF motion and ego-motion errors cause trajectory drift.

Method: Injects translational acceleration and angular velocity sequences into a pretrained latent diffusion transformer via an action-token stream; uses a frozen latent-space Physics Probe (trained on real video-IMU pairs) for differentiable inertial-consistency supervision during LoRA finetuning.

Result: Aero-World improves mean AAS from 57.7 to 63.6 over action-only finetuning; outperforms AirScape with lower FVD (596.5 vs 1058.6), higher SSIM (0.595 vs 0.505), and higher Flow-IMU correlation (0.44 vs 0.20).
...
TL;DR: A coarse-to-fine distillation framework (LIFT & PLACE) for diffusion models that addresses the challenge of teacher-student capacity mismatch, achieving stable training even with extreme 1.6% parameter compression.


<details>
  <summary>Details</summary>
Motivation: Teacher networks in diffusion model knowledge distillation have substantially larger capacity, making their denoising process too complex for student models to faithfully mimic.

Method: Proposes LIFT (Linear Fitting-based distillation) which decomposes distillation into coarse alignment and fine refinement, and PLACE (Piecewise Local Adaptive Coefficient Estimation) which addresses spatially non-uniform errors via error-based grouping with locally adaptive guidance.

Result: Effective across image/latent spaces, U-Net/DiT backbones, unconditional/conditional tasks, and flow-based models (SD3). Under 1.6% parameter compression (1.3M student), achieves FID of 15.73 while conventional KD fails with FID 50-200+.

Conclusion: The proposed framework enables stable and effective knowledge distillation for diffusion models even under extreme compression ratios where conventional methods fail.
...
TL;DR: GeoMamba proposes a geometry-driven framework for unaligned optical-SAR fine-grained object retrieval, achieving 63.3% mAP and 77.0% Rank-1 through Geometric Feature Injection and Consistency Constraint modules.


<details>
  <summary>Details</summary>
Motivation: Cross-modal fine-grained object retrieval under unaligned optical and SAR conditions is challenging due to modality discrepancy, speckle noise, and structural inconsistency, which limit robust cross-modal representation learning.

Method: GeoMamba introduces a Geometric Feature Injection (GFI) module for cross-modal feature interaction, a Geometric Consistency Constraint (GCC) module with Deep Supervision (DS) strategy imposing hierarchical geometric constraints, and constructs a new FGOS-as dataset.

Result: On FGOS-as dataset, GeoMamba achieves 63.3% mAP and 77.0% Rank-1 accuracy in all-to-all retrieval setting, outperforming existing methods.

Conclusion: The geometry-driven framework effectively addresses unaligned optical-SAR retrieval, demonstrating that geometric priors and consistency constraints significantly improve cross-modal representation learning.
...
TL;DR: FlowErase-RL is the first GRPO-based framework for concept erasure in flow matching models, using a dynamic dual-path reward mechanism to suppress harmful content while preserving generative fidelity, achieving state-of-the-art results across nudity, object, and style erasure tasks.


<details>
  <summary>Details</summary>
Motivation: Flow matching models for text-to-image generation pose growing safety risks from harmful content; existing concept erasure methods (inference-time interventions or supervised fine-tuning) suffer from limited effectiveness, scalability issues, and difficulty with multi-concept settings.

Method: Reformulates concept erasure as a reward optimization problem using GRPO, with a dynamic dual-path reward mechanism combining a Concept Erasure (CE) reward to suppress targets and a Non-target Space (NS) reward to preserve fidelity, adaptively balanced via a performance-driven switching strategy.

Result: Achieves state-of-the-art erasure performance on nudity, object, and artistic style tasks while maintaining strong image quality, semantic alignment, robust adversarial attack resistance, and effective scalability to multi-concept scenarios.

Conclusion: Establishes a new paradigm for safe and controllable generation in flow matching models through reward-based optimization without explicit supervision.
...
TL;DR: A simple, reference-based anomaly detection method using pretrained vision transformers and nearest-neighbor similarity to detect traffic scene anomalies in real-time, without requiring explicit supervision or dataset-specific training.


<details>
  <summary>Details</summary>
Motivation: Detecting anomalies in traffic scenes is crucial for autonomous driving safety, but collecting representative anomalous data is challenging. Existing methods are highly specialized and rely on abstract semantic Cityscapes classes, making them difficult to adapt to diverse real-world scenarios.

Method: Uses pretrained vision transformer embeddings and nearest-neighbor similarity in latent semantic feature space with patch-wise processing to produce dense anomaly masks. Normality is modeled through a single reference image, avoiding explicit supervision and dataset-specific training.

Result: Achieves good performance on the Road Anomaly benchmark and demonstrates consistent qualitative behavior in real-world automated vehicle tests, successfully highlighting semantically unusual objects in diverse scenes.

Conclusion: Simple, reference-based methods can provide useful anomaly signals under realistic operating conditions without requiring specialized training or large datasets.
...
TL;DR: This paper introduces a unified framework for continual personalized generation in VAR models, addressing catastrophic forgetting in sequential concept learning and feature entanglement in multi-concept synthesis via Gradient-based Concept Neuron Selection (GCNS) and context-aware composition strategy.


<details>
  <summary>Details</summary>
Motivation: Existing VAR-based personalization methods are limited to static settings, failing to accommodate evolving user demands—they suffer from catastrophic forgetting in sequential learning and feature entanglement/attribute inconsistency in multi-concept synthesis.

Method: Two core components: (1) Gradient-based Concept Neuron Selection (GCNS)—identifies concept-relevant neurons and constrains only conflicting parameters across tasks to mitigate forgetting; (2) Context-aware composition strategy—performs multi-branch feature modeling and localized cross-attention fusion guided by spatial conditions for disentangled concept composition.

Result: The method significantly improves performance in long-sequence continual personalization and achieves superior multi-concept image synthesis compared to existing baselines.

Conclusion: Highlights the potential of VAR models for scalable and controllable personalized generation.
...
TL;DR: This paper introduces PPaint, a dual-protocol benchmark for image aesthetic assessment comparing pairwise preferences and pointwise ratings, and proposes PSDistill, a method that converts pairwise preferences into calibrated scores for training aesthetic models, achieving significant improvements.


<details>
  <summary>Details</summary>
Motivation: Existing IAA benchmarks adopt only one annotation protocol (pairwise or pointwise), leaving their complementarity unmeasured under controlled conditions.

Method: Created PPaint benchmark with 15 experts annotating 150 Chinese paintings under both protocols across 5 aesthetic dimensions. Proposed PSDistill method converting VLM pairwise judgments into calibrated pseudo-scores via Elo reference pool with confidence-weighted ranking optimization.

Result: Qwen3-VL-8B distilled model improved SRCC from 0.504 to 0.709 across all categories, outperforming all open-source baselines including ArtiMuse, and matching Gemini-3.1-Pro within 0.04 SRCC.

Conclusion: Pairwise preferences and pointwise ratings have complementary strengths; fusing both signals yields more robust aesthetic assessments. PSDistill enables effective use of pairwise data for single-pass aesthetic scoring.
...
TL;DR: A training-free method called Spatio-Temporal Attention Chain accelerates 4D mesh generation from videos by exploiting early temporal correspondences in a 4D backbone, achieving 13× speedup and scaling to 16× longer sequences.


<details>
  <summary>Details</summary>
Motivation: Existing 4D mesh generation methods are slow, computationally expensive, and difficult to scale to longer video sequences.

Method: Spatio-Temporal Attention Chain (STAC): a training-free framework that propagates information across space and time by mapping anchor mesh vertices to latent tokens, tracking temporal correspondences in latent space, and recovering frame-specific vertices via latent-to-vertex attention, avoiding expensive explicit matching.

Result: Generates a 4D mesh in 9 seconds (13× speedup), scales to 16× longer videos without quality degradation, achieves competitive zero-shot performance on 2D object tracking and 4D tracking, and enables camera estimation.

Conclusion: The framework offers an efficient, scalable alternative for 4D mesh generation from videos with improved temporal consistency and downstream task capabilities.
...
TL;DR: This paper investigates the mechanistic interpretability of object localization in VLMs (LLaVA-1.5 and InternVL-3.5), finding a "containerization" mechanism where object-aligned tokens define spatial boundaries while internal token arrangement is irrelevant, with only a small set of attention heads driving both classification and localization through distinct specialized pathways.


<details>
  <summary>Details</summary>
Motivation: Classification mechanisms in VLMs have been studied, but object localization processes remain poorly understood. The authors aim to bridge this gap by providing the first layer- and head-level account of how VLMs localize objects.

Method: Used a suite of mechanistic interpretability tools including token ablations, attention knockout, and causal mediation analysis on LLaVA-1.5 and InternVL-3.5.

Result: Localization operates via a "containerization" mechanism where object-aligned tokens define spatial extent; semantic arrangement within boundaries is irrelevant. Only a few attention heads mediate causal effects, concentrated in early-mid layers (LLaVA) vs. mid-late layers (InternVL). Classification and localization share early processing but rely on largely distinct specialized heads.

Conclusion: Provides the first detailed mechanistic account of object localization in VLMs, revealing narrow computational pathways that can inform future model design and grounding objectives.
...
TL;DR: Depth2Pose evaluates monocular depth estimators using downstream camera pose estimation accuracy instead of traditional depth error metrics, and introduces the D2P dataset of challenging outdoor scenes.


<details>
  <summary>Details</summary>
Motivation: Standard depth error metrics (global aggregation) don't reflect how useful depth predictions are for downstream geometric tasks like SfM, visual localization, and SLAM. Additionally, dense per-pixel ground truth depth is expensive to obtain.

Method: Combines depth predictions with feature correspondences in depth-aware geometric solvers, using relative camera pose estimation accuracy as a task-driven proxy for depth quality. Only requires camera poses (cheap to obtain via SfM) instead of dense depth ground truth.

Result: Methods that perform well under standard depth metrics on existing benchmarks also perform well under the pose-based metric on the same datasets, but do not necessarily generalize to the more challenging D2P dataset (outdoor, large-scale, occluded scenes).

Conclusion: Depth2Pose provides a cost-effective, task-driven evaluation framework for MDEs that works in scenarios where ground-truth depth is hard to obtain.
...
TL;DR: A semi-supervised framework (EchoCare + SAM-Med2D + DINOv3) for fetal cardiac ultrasound segmentation and classification, using view-specific hard masking and a two-stage optimization strategy (EMA + Classification Fine-Tuning). Achieves 79.99% Dice, 61.62% NSD, 41.20% F1 on FETUS 2026 leaderboard.


<details>
  <summary>Details</summary>
Motivation: To improve prenatal congenital heart disease screening by jointly tackling segmentation and classification of fetal cardiac ultrasound images in a semi-supervised manner.

Method: Built upon EchoCare multi-task backbone, integrates SAM-Med2D for boundary refinement and DINOv3 for pseudo-label enhancement. Uses view-specific hard masking with a two-stage optimization: EMA phase for segmentation consolidation, then Classification Fine-Tuning phase (freeze segmentation, reset classification head) to recover classification performance.

Result: On FETUS 2026 leaderboard: Dice 79.99%, Normalized Surface Distance 61.62%, F1-score 41.20%.

Conclusion: Validates effectiveness for prenatal congenital heart disease screening; code publicly available.
...
TL;DR: StitchVM efficiently transfers clean-image reward models to noisy latent space by stitching a frozen diffusion backbone onto a pixel-space reward model, enabling fast, amortized value function estimation for diffusion/flow-based generative model alignment.


<details>
  <summary>Details</summary>
Motivation: Aligning diffusion/flow-based generative models with task-specific rewards is challenging because rewards are defined for clean images but alignment needs value estimates at noisy latents; existing Tweedie/Monte Carlo approximations trade bias for cost, and learning a dedicated value function for noisy latents remains an open problem.

Method: StitchVM attaches a frozen diffusion backbone as the head of a truncated pixel-space reward model (e.g., CLIP ViT-L), creating a hybrid that retains robust reward capability from the pixel model and inherits native noisy-latent handling from the diffusion backbone. Training requires only ~10 GPU-hours.

Result: DPS becomes 3.2× faster with halved peak GPU memory; DiffusionNFT becomes 2.3× faster. StitchVM enables amortized value function construction for noisy latents, improving downstream steering and post-training methods.

Conclusion: By lifting pixel-space reward models to latent space via lightweight stitching, StitchVM opens a new style of diffusion alignment that avoids per-sample approximations, yielding efficiency gains across multiple methods.
...
TL;DR: LaCoVL-FER introduces a landmark-guided contrastive learning network with vision-language enhancement that integrates geometric priors from facial landmarks and semantic priors from CLIP to improve facial expression recognition in the wild, outperforming SOTA on RAF-DB, FERPlus, and AffectNet.


<details>
  <summary>Details</summary>
Motivation: FER in the wild remains challenging due to pose, occlusion, and illumination variations. Existing attention-based methods rely solely on visual cues, suffering from attention redundancy and instability in complex scenarios.

Method: Proposes LaCoVL-FER with three key components: (1) Landmark-Guided Adaptive Encoder (LGAE) with Bi-branch Gated Cross Attention (BGCA) for fusing landmark geometric priors with visual features; (2) Vision-Language Enhancement Strategy (VLES) leveraging CLIP image encoder with expression-relevant features; (3) Expression-Conditioned Prompting (ECP) mechanism adapting CLIP text encoder prompts for instance-aware textual representations.

Result: Outperforms state-of-the-art methods on RAF-DB, FERPlus, and AffectNet datasets, validated through both quantitative and qualitative experiments.

Conclusion: Integrating geometric (landmark) and semantic (vision-language) priors effectively enhances FER robustness and generalization in wild conditions.
...
TL;DR: This paper explores using real data (instead of generated preference pairs) as supervision for aligning diffusion models, showing it's comparably effective and more reliable.


<details>
  <summary>Details</summary>
Motivation: Existing preference alignment methods rely on comparing generated images, which is ambiguous when both samples have artifacts. The authors investigate whether real data can serve as a cleaner, alternative supervision source.

Method: A data-centric curation strategy that treats real images as reference points and constructs preference signals by contrasting them with generated or perturbed samples, without requiring manual annotations.

Result: Real-data-based supervision achieves performance comparable to existing preference-based methods for aligning diffusion models.

Conclusion: Real data offers a practical, complementary, and label-efficient source of supervision for preference alignment.
...
TL;DR: FineBench is a large-scale human-centric VQA benchmark with 199K QA pairs over 64 long videos for fine-grained action understanding. FineAgent is a modular framework (Localizer + Descriptor) that boosts open VLM performance on this benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack long-form videos, dense QA coverage, and frame-level spatial/temporal grounding for fine-grained human-centric understanding. FineBench fills this gap.

Method: FineBench: 199,420 multiple-choice QA pairs over 64 15-minute videos covering person movement, interaction, object manipulation, and compositional actions. FineAgent: a modular framework combining a Localizer and a Descriptor to enhance open VLMs.

Result: Proprietary models (GPT-5) perform decently; open-source VLMs underperform, especially in spatial reasoning and subtle action discrimination. FineAgent consistently improves open VLMs on FineBench.

Conclusion: FineBench provides a rigorous testbed for fine-grained human-centric video understanding, and FineAgent offers a practical enhancement for current VLMs.
...
TL;DR: This paper investigates using zero-shot Text-to-Image generative models to create synthetic concept datasets for concept-based XAI, finding that while promising, current T2I models face challenges in faithfully replacing real concept images.


<details>
  <summary>Details</summary>
Motivation: Concept-based XAI requires large labeled image sets for each concept, which limits scalability. The authors aim to explore whether zero-shot T2I generative models can serve as a scalable alternative source of concept datasets.

Method: The authors generate synthetic concepts using predefined prompts from T2I models, then evaluate faithfulness via four analyses: (1) concept representation similarity between synthetic and real images; (2) intra-similarity with progressively larger subsets; (3) downstream explanation task performance; (4) concept removal effects on explanations.

Result: Current T2I generative models face challenges in faithfully replicating real concept data for XAI tasks, though they offer a promising shortcut. The study reveals limitations in synthetic-to-real concept alignment.

Conclusion: Zero-shot T2I models show potential but raise open questions about using synthetic data for concept-based XAI. The authors release their dataset for further research.
...
TL;DR: EyeVLM systematically evaluates VLMs on gaze understanding (gaze following & social gaze prediction), finding that current VLMs lack precise capabilities compared to specialized visual models, though fine-tuning helps reduce the gap.


<details>
  <summary>Details</summary>
Motivation: VLMs could benefit human gaze analysis but their ability to understand gaze-related behaviors remains unexplored; the paper aims to systematically evaluate this gap.

Method: Proposes EyeVLM, a framework evaluating VLMs across two tasks (gaze following with geometric/visual processing; social gaze prediction with relational reasoning) in zero-shot (various prompting strategies) and fine-tuning (QA pairs) settings, using existing datasets and comparing with visual models.

Result: Current VLMs lack precise gaze understanding capabilities; standard training helps reduce the gap with visual models but significant improvements are still needed.

Conclusion: VLMs are currently insufficient for precise gaze understanding; fine-tuning helps but major advances are necessary to match specialized visual models.
...
TL;DR: This paper proposes a score-based method to improve camera viewpoint estimation under sparse-view conditions by reshaping the optimization landscape to overcome nonconvex loss issues, followed by refinement with a viewpoint-conditioned diffusion model.


<details>
  <summary>Details</summary>
Motivation: Accurate camera viewpoint estimation under sparse-view (especially two-view) conditions remains challenging due to nonconvex loss landscapes with many local minima, causing sensitivity to initialization and reliance on brute-force multistart strategies.

Method: A score-based method that reshapes the optimization landscape to guide updates toward ground-truth viewpoint, followed by a refinement stage using a viewpoint-conditioned diffusion model.

Result: Improves convergence, reduces reliance on brute-force sampling, achieves competitive accuracy with higher sample-efficiency compared to prior methods.

Conclusion: The proposed score-based approach effectively mitigates optimization challenges from geometric ambiguities (symmetry, self-similarity) and offers a more efficient alternative to naive multistart strategies for viewpoint estimation.
...
TL;DR: VLMs struggle with out-of-distribution layouts due to a two-hop bottleneck (layout localization → content extraction). The authors pre-resolve layout detection using RT-DETR, serialize outputs in DocTags vocabulary, and inject them as prompt priors, achieving significant gains (e.g., markdown F1 from 0.37→0.92) with minimal latency cost and no architectural changes.


<details>
  <summary>Details</summary>
Motivation: VLMs fail on unseen layouts because they must first classify/localize layout entities (Hop 1) before extracting content (Hop 2); when Hop 1 fails, Hop 2 collapses.

Method: Pre-resolve Hop 1 using RT-DETR detector, serialize detections in DocTags vocabulary, inject into prompt alongside full image—unlike cropping or plain-text priors, this shares the decoder's generation space and keeps global image as fallback.

Result: Markdown F1: 0.37→0.92 (OOD benchmark); table TEDS: 0.01→0.36 (Chinese OmniDocBench); infinite-loop failures drop across all domains (ViDoRe V3, 26k pages).

Conclusion: Injecting structured layout priors in the decoder's own vocabulary alleviates the two-hop bottleneck, improving robustness with only 15% latency overhead and 74 median tokens, no architectural changes.
...
TL;DR: WoundFormer is a transformer-based framework that improves multi-class wound tissue segmentation by using a spatially-preserving multi-scale aggregation head, achieving 81.9% Dice score and outperforming baselines by up to 4.3 points.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for wound analysis focus on binary segmentation and fail to handle heterogeneous tissue composition due to high intra-class variability and limited annotated data, making multi-class wound tissue segmentation a challenging and clinically relevant problem.

Method: Replaces the standard SegFormer decoder with a spatially-preserving multi-scale aggregation head that maintains feature topology during cross-scale integration and strengthens contextual interactions through convolutional fusion.

Result: Overall Dice score of 81.9% on WoundTissueSeg dataset, outperforming CNN- and transformer-based baselines by up to 4.3 Dice points, with consistent improvements across minority tissue classes.
...
TL;DR: A passive end-of-shift construction safety pipeline using POV/wall cameras, YOLO11, SAM 3, and Qwen3-VL with a three-pass adversarial chain-of-thought prompting (method-actor framing) to detect PPE violations, map to OSHA standards, and generate per-worker reports—achieving 12% precision improvement over single-pass prompting.


<details>
  <summary>Details</summary>
Motivation: Construction remains the deadliest US industry sector (1,055 fatalities in 2023, mostly preventable). Existing monitoring is expensive, requires real-time human operators, or covers only narrow violations.

Method: Three-stage pipeline: (1) fine-tuned YOLO11 for PPE/hazard detection, (2) SAM 3 for segmentation refinement and worker deduplication, (3) Qwen3-VL-8B-Instruct with method-prompted, persona-scaffolded three-pass adversarial chain-of-thought (generator, discriminator, reconciliation) for compliance verification and hallucination control.

Result: 12% precision improvement over single-pass prompting in informal three-author review of 12-video Ironsite corpus; largest gains on hallucination-prone violation categories.

Conclusion: The proposed system maps violations to OSHA standards, performs REBA-inspired ergonomic risk scoring, produces per-worker safety reports with timestamped evidence, and releases an evaluation harness for reproduction.
...
TL;DR: SEGS is a training-free framework that reduces the Janus problem in text-to-3D generation by applying structural energy guidance in the PCA subspace of U-Net features, improving multi-view consistency across multiple baselines.


<details>
  <summary>Details</summary>
Motivation: The Janus problem (inconsistent multi-view geometry) in text-to-3D generation is caused by viewpoint bias in 2D diffusion priors, and existing methods lack a simple, training-free solution to address this.

Method: SEGS constructs a structural energy in the PCA subspace of U-Net features and injects its gradient into the denoising process, integrating seamlessly into SDS/VSD pipelines without retraining.

Result: Reduces Janus Rate by ~10% on average and improves View-CS scores across DreamFusion, Magic3D, and LucidDreamer baselines while preserving appearance fidelity.

Conclusion: SEGS provides an effective, plug-and-play solution for alleviating viewpoint artifacts in text-to-3D generation without requiring retraining or model modification.
...
TL;DR: This paper systematically benchmarks memory policies for test-time adaptation (TTA), showing that intra-class diversity in memory management matters more than simply retaining recent or class-balanced samples. The authors introduce GOTTA, a family of diversity-aware memory policies that improve adaptation under constrained budgets and non-i.i.d. streams.


<details>
  <summary>Details</summary>
Motivation: Existing TTA research focuses on adaptation objectives but neglects memory design. Memory mechanisms are usually evaluated as entangled components of specific algorithms, making it difficult to isolate which design choices matter and under what conditions.

Method: A systematic benchmark that decouples memory from the adaptation algorithm, evaluating memory policies across i.i.d., non-i.i.d., continual, and practical test streams. Also introduces GOTTA, diversity-aware memory policies combining class-balanced allocation with feature-space diversity as drop-in replacements for existing buffers.

Result: Intra-class diversity is a key factor for avoiding redundant buffers and maintaining representative adaptation signals. GOTTA improves adaptation most clearly under constrained memory budgets and challenging non-i.i.d. streams, while remaining competitive as memory capacity increases.

Conclusion: Memory management should be treated as a first-class component of robust test-time adaptation, with diversity identified as a central design principle for practical TTA.
...
TL;DR: SplitQ is a channel-splitting-driven post-training quantization framework for VLMs that addresses cross-modal heterogeneity by decoupling modality-specific outlier channels and using adaptive cross-modal calibration, achieving state-of-the-art results (e.g., 93.5% of FP16 performance at W3A3).


<details>
  <summary>Details</summary>
Motivation: Existing PTQ methods degrade VLM accuracy due to heterogeneous activation distributions between text and vision modalities during quantization, where modality-specific outliers are concentrated in a small subset of different channels per modality.

Method: SplitQ proposes two modules: (1) Modality-specific Outlier Channel Decoupling (MOCD) that isolates salient outlier channels, and (2) Adaptive Cross-Modal Calibration (ACC) with dual lightweight learnable branches to dynamically mitigate modality-induced quantization errors.

Result: SplitQ significantly outperforms existing methods across 6 multi-modal datasets under W4A8, W4A4, W3A3, and W3A2 settings. It achieves 93.5% of FP16 performance (69.5 vs. 74.3) under the challenging W3A3 setting.
...
TL;DR: StruMPL jointly estimates forest AGB and canopy structure from heterogeneous satellite and ground data using multi-task learning with Missing Not At Random (MNAR) labels, physics constraints, and Augmented IPW to reduce bias.


<details>
  <summary>Details</summary>
Motivation: Spaceborne lidar provides structure but no biomass; ground plots provide biomass but no structure and are biased. No single sample has all labels, plot labels are MNAR, but biomass-structure links are known via allometry.

Method: StruMPL: shared encoder + per-variable regression/imputation/propensity heads + learnable physics module enforcing allometric constraints. Uses Augmented IPW pseudo-outcome with stop-gradients on propensity and imputation baselines for joint MNAR correction.

Result: Outperforms ablations and prior method on AGB RMSE and bias; AIPW reduces high-AGB bias by ~54% across two biomes.

Conclusion: Jointly modeling heterogeneous partial supervision, MNAR bias, and physical constraints improves AGB estimation over existing approaches.
...
TL;DR: AnyCity is an observation-grounded generative reconstruction framework that reconstructs coherent 3D urban scenes from sparse aerial views by first predicting observation-supported geometry latents, then using gated residual updates with aerial completion tokens, achieving high-quality novel-view synthesis in a single feed-forward pass.


<details>
  <summary>Details</summary>
Motivation: Reconstructing large-scale urban scenes from sparse aerial views is challenging due to biased camera poses causing evidence imbalance (roofs/opens well-observed, facades/occluded structures under-supported). Existing methods produce ghosting, melted facades, and stretched textures, while generative approaches lack clear separation between observed geometry and prior-driven content.

Method: AnyCity: (1) predicts observation-supported geometry latent to anchor reliable structures; (2) uses scaffold-conditioned aerial completion tokens to predict gated residual update for weakly constrained content; (3) dense-to-sparse distillation transfers structural cues; (4) aerial-adapted video diffusion prior provides fine-grained urban appearance cues via gated token conditioning; (5) observation-preserving objectives keep refined representation consistent with input geometry.

Result: Consistent improvements over feed-forward baselines on synthetic, aerial-domain, UAV-textured, and real-world scenes. Achieves coherent urban novel-view synthesis with second-level inference in a single feed-forward pass.

Conclusion: AnyCity effectively separates observation-grounded geometry from prior-driven content for sparse aerial urban reconstruction, producing coherent and consistent 3D scenes with fast inference.
...
TL;DR: AffectVerse enhances emotion recognition in MLLMs by introducing an Emotion World Module that uses short-horizon latent affective prediction as a self-supervised signal, achieving >2.57% improvement across nine benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs treat emotion recognition as static fusion over complete multimodal inputs, ignoring the dynamic, predictive nature of how emotions unfold over time.

Method: Proposes AffectVerse with Emotion World Module (EWM) containing Cross-Modal Temporal Imagination (predicts future representations), MAMA Belief Aggregation (compresses tokens), and Belief Injection (inserts into LLM). Uses future prediction as self-supervised signal without requiring unseen inference-time signals.

Result: Achieves >2.57% improvement over other models across nine benchmarks; ablations confirm additive gains from temporal imagination, cross-modal rollout, and belief aggregation.

Conclusion: Predictive belief-state modeling is a practical and effective alternative for affective computing in multimodal settings.
...
TL;DR: A-TPT is a test-time prompt tuning method that uses attention-guided spatial augmentation to defend VLMs against adversarial attacks while preserving semantic information, outperforming existing methods on both adversarial and clean data.


<details>
  <summary>Details</summary>
Motivation: Existing test-time adaptation methods for VLMs use multi-view augmentation that struggles to identify semantic information and destroys discriminative regions in fine-grained scenarios when facing adversarial attacks.

Method: Refines gradient attention rollout to identify semantically meaningful regions under attacks, then uses them to guide spatially varying augmentation intensities and multi-view ensemble for prompt tuning and inference.

Result: A-TPT outperforms existing test-time adaptation methods on both adversarial and clean data.

Conclusion: Attention-guided spatial augmentation is effective for semantics-preserving test-time adaptation against adversarial attacks in VLMs.
...
TL;DR: SphericalDreamer generates fully immersive, long-range 3D outdoor environments from text prompts by creating and fusing multiple panoramic images with visual and geometric consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods cannot produce 3D worlds that are both navigable over long-range spatial extents and cover the complete omnidirectional field of view (360°×180°).

Method: Builds on generating multiple panoramic images, lifts them into 3D, and fuses them together while maintaining visual and geometric consistency.

Result: Produces highly detailed, fully immersive 3D environments with substantially improved scale and navigability compared to prior approaches.

Conclusion: SphericalDreamer successfully addresses the key limitation of simultaneous long-range navigability and full omnidirectional coverage for text-driven 3D environment generation.
...
TL;DR: RECIPE uses grounding quality in ASR transcripts as a reward signal (via GRPO) to train visual planning models from noisy instructional videos, outperforming supervised fine-tuning on both clean and pseudo-labeled data across 7 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Visual planning is bottlenecked by annotation difficulty: clean datasets are small, domain-narrow, and single-trajectory, while large-scale noisy video corpora suffer from segmentation/alignment errors when used for supervised fine-tuning.

Method: Key asymmetry: extracting clean labels from noisy video is hard, but verifying temporal grounding of step sequences in ASR transcripts is cheap and scalable. RECIPE uses grounding quality as reward for GRPO (group relative policy optimization), turning noisy corpus into a verifier rather than a label source. Applies to both Socratic (textual history from frozen VLM) and Video (direct video tokens) configurations.

Result: RECIPE-RL improves over base checkpoint at all scales (0.5B, 3B, 7B) on all 7 benchmarks: macro-accuracy gains of +7-8 points in-domain, up to +16 points zero-shot. Outperforms SFT on both annotated and pseudo-labeled plans; preserves generation diversity that SFT collapses.

Conclusion: Leveraging noisy video data as a verifier (via grounding reward) rather than a label source is an effective and scalable paradigm for visual planning, enabling strong zero-shot generalization and diversity preservation without human annotations.
...
TL;DR: InterLight is a novel LLIE framework that systematically exploits intrinsic illumination priors through physics-guided augmentation, adaptive prompts, luminance-gated memory, and self-supervised consistency, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing Retinex-based LLIE methods suffer from over-enhancement, color distortion, and unrealistic assumptions about noise/lighting due to insufficient exploitation of illumination priors.

Method: (1) Physics-guided augmentation injecting sensor-level illumination-response priors; (2) Adaptive prompts conditioned on latent illumination state; (3) Luminance-gated intrinsic memory for selective reconstruction; (4) Self-supervised consistency regularization for illumination-invariant features.

Result: Achieves clearer textures and more visually coherent enhancement across multiple benchmarks, with code publicly available.

Conclusion: Deeply exploiting intrinsic illumination priors through a systematic pipeline is key to robust, artifact-free low-light image enhancement.
...
TL;DR: The paper introduces Adaptive Manifold Guidance (AdaMaG), a theoretically grounded guidance method that decomposes Classifier-Free Guidance into divergence and score-parallel terms, addressing manifold violation and probability conservation issues under strong guidance.


<details>
  <summary>Details</summary>
Motivation: Existing CFG and extrapolation-based guidance methods are heuristic linear combinations that ignore the generative manifold geometry, breaking probability conservation and driving samples off the learned manifold under strong guidance.

Method: The authors analyze guidance through the continuity equation, decomposing its effect into a divergence term and a score-parallel term. They prove the divergence term blows up near the data manifold, motivating a time-dependent schedule with score-parallel attenuation, resulting in AdaMaG.

Result: AdaMaG improves realism, reduces hallucinations, and induces controlled desaturation in high-guidance regimes across image generation benchmarks, at no additional inference cost.

Conclusion: Most empirical heuristics for reducing saturation or improving generation quality correspond directly to the two identified terms. AdaMaG offers a principled, plug-and-play alternative to heuristic CFG adjustments.
...
TL;DR: CogOmniControl is a reasoning-driven framework for controllable video generation that uses a specialized CogVLM trained on anime production data to understand creative intent from sparse/abstract conditions, then guides a diffusion model (CogOmniDiT) via reinforcement learning, with a Best-of-N selection loop for improved quality.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models struggle with abstract, sparse, or complex conditions (e.g., storyboard sketches, clay renders), and fail to align with user's creative intent due to a capability gap between generic VLMs and professional production needs.

Method: (1) A specialized CogVLM trained on authentic anime production data for professional creative intent cognition; (2) CogOmniDiT unifying multi-condition control via in-context generation, aligned to CogVLM reasoning via RL; (3) Best-of-N selection leveraging CogVLM for video quality planning.

Result: Outperforms existing open-source models on two new benchmarks (CogReasonBench, CogControlBench) built from professional workflow data with genuine creative intent.

Conclusion: The closed-loop "harness-like" architecture (reasoning → generation → evaluation) effectively bridges creative intent cognition and controllable video generation for professional production workflows.
...
TL;DR: VL-DPO uses vision-language models (VLMs) to generate preference pairs for aligning motion forecasting models with human preferences via Direct Preference Optimization, achieving 11.94% higher RFS and 10.01% lower ADE on the Waymo Open End-to-End Driving Dataset.


<details>
  <summary>Details</summary>
Motivation: Standard imitation learning for motion forecasting fails to capture nuanced human driving preferences; VLMs offer commonsense reasoning that can bridge this gap.

Method: Uses a VLM as zero-shot reasoner to automatically generate preference pairs from pretrained model rollouts, then finetunes via Direct Preference Optimization (DPO).

Result: 11.94% increase in RFS and 10.01% reduction in ADE over the pretrained baseline on the Waymo Open End-to-End Driving Dataset.

Conclusion: VLM-generated trajectory selection serves as a high-quality proxy for human preference, and VL-DPO effectively aligns motion forecasting with human driving preferences.
...
### [118] [A Nash Equilibrium Framework For Training-Free Multimodal Step Verification](https://arxiv.org/abs/2605.20033)
*Rohit Sinha,Kunal Tilaganji,Tanuja Ganu,Nagarajan Natarajan,Amit Sharma,Vineeth N. Balasubramanian*

Main category: cs.CV

TL;DR: A training-free verification method using Nash equilibrium game among specialized judges, where agreement signals valid reasoning steps and disagreement reveals instability, achieving 2.4-5.2% improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs generate reasoning chains with subtle errors; current verification approaches need extensive labeled data (learned critics) or simply average scores (training-free methods), missing the information in disagreement.

Method: Treats step-wise verification as a coordination problem among specialized judges, formalized as a Nash equilibrium game with closed-form equilibrium scores, enabling disagreement-aware filtering and stability-conscious ranking.

Result: Achieves consistent 2.4% to 5.2% improvements over baseline models across six benchmarks, competitive with learned critics.

Conclusion: Cross-modal agreement provides robust verification signals without task-specific adaptation, outperforming simple averaging approaches.
...
### [119] [Rethinking Visual Attribution for Chest X-ray Reasoning in Large Vision Language Models](https://arxiv.org/abs/2605.20158)
*Guangzhi Xiong,Qiao Jin,Sanchit Sinha,Zhiyong Lu,Aidong Zhang*

Main category: cs.CV

TL;DR: Existing visual attribution methods fail to faithfully reflect what LVLMs actually see in medical images. The authors propose MedFocus, a concept-based attribution via unbalanced optimal transport, which significantly outperforms 11 prior methods across 6 LVLMs on chest X-ray reasoning.


<details>
  <summary>Details</summary>
Motivation: LVLMs in medicine cannot faithfully ground responses in visual evidence, and existing attribution methods lack verification because ground-truth annotations for internal model reasoning are unavailable.

Method: A causal evaluation framework using counterfactual editing to verify expert-annotated regions as causally responsible, plus MedFocus—a concept-based attribution via unbalanced optimal transport that measures causal effects through targeted interventions.

Result: Existing methods often fail to identify evidence used by LVLMs; MedFocus substantially outperforms them across 11 attribution methods, 6 LVLMs, and 2 output modes.

Conclusion: MedFocus produces spatial, concept-level, and token-level attributions, taking a step toward more trustworthy attribution for medical LVLMs.
...
TL;DR: SEATS is a training-free token pruning method for omni-modal LLMs that reduces computational cost by adaptively removing redundant non-textual tokens across layers, achieving 9.3x FLOPs reduction with 96.3% performance retention.


<details>
  <summary>Details</summary>
Motivation: Existing token selection methods for om-LLMs either focus only on visual inputs or prune tokens before the LLM with fixed ratios, failing to capture how cross-modal token importance evolves across layers.

Method: SEATS uses three-stage pruning: (1) pre-LLM spatiotemporal redundancy removal via attention-weighted diversity selection, (2) within-LLM progressive token pruning with dynamic retention budget allocation across modalities using query relevance scores, (3) late-layer removal of all remaining non-textual tokens after cross-modal fusion.

Result: With only 10% visual/audio tokens retained, SEATS achieves 9.3x FLOPs reduction and 4.8x prefill speedup while maintaining 96.3% performance on Qwen2.5-Omni and Qwen3-Omni.

Conclusion: Layer-wise cross-modal token redundancy can be exploited via stage-adaptive pruning for efficient om-LLM inference without training.
...
TL;DR: OP2GS introduces a dual-opacity Gaussian representation that decouples visual reconstruction from object occupancy, enabling efficient open-vocabulary scene understanding without per-Gaussian feature storage.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS-based scene understanding methods suffer from either heavy storage/decoding overhead (feature-based) or label contamination during 2D-to-3D projection (lifting-based), where Gaussians needed for appearance receive incorrect object labels.

Method: OP2GS augments each Gaussian with an instance identity and a dedicated instance opacity σ* for object-mask rendering, while original opacity σ handles visual reconstruction. A random object loss optimizes the 1D instance occupancy field using transmittance-based visibility. Semantic descriptors are attached at the object level via multi-view aggregation.

Result: Achieves competitive open-vocabulary performance compared to feature-training approaches while significantly reducing computational overhead; resolves visibility ambiguities better than training-free pipelines via physically consistent occupancy learning.

Conclusion: The dual-opacity formulation effectively decouples visual existence from instance occupancy, allowing mislabeled Gaussians to support image rendering while being transparent in object-mask rendering.
...
TL;DR: A deep learning method (pix2pix GAN) automatically segments and quantifies epicardial and mediastinal cardiac fat with high accuracy (99.08% for epicardial, 97.90% for mediastinal) and real-time performance.


<details>
  <summary>Details</summary>
Motivation: Manual segmentation of cardiac fat is labor-intensive and costly for clinicians, yet important for assessing cardiovascular disease risk. There is a need for automated, efficient methods.

Method: Uses the pix2pix generative adversarial network, originally designed for image-to-image translation, adapted for automatic segmentation of epicardial and mediastinal fat from cardiac images.

Result: Epicardial fat: 99.08% accuracy, 98.73 F1-score. Mediastinal fat: 97.90% accuracy, 98.40 F1-score. Outperforms existing methods in F1-score and run time, enabling real-time segmentation.

Conclusion: The pix2pix-based approach is highly effective for cardiac fat segmentation, achieving superior precision and speed, making it viable for clinical adoption.
...
TL;DR: A supervised pixel-classification method for vessel segmentation in x-ray angiograms using textural features (anisotropic diffusion, Hessian matrix, morphology, statistics) combined with ELEMENT region-growing and Random Forests, achieving 95.48% accuracy and outperforming unsupervised approaches.


<details>
  <summary>Details</summary>
Motivation: To improve vessel segmentation in x-ray angiograms beyond what unsupervised methods can achieve, by leveraging supervised pixel-classification with rich textural features.

Method: Extracts textural features (anisotropic diffusion, Hessian matrix-based, mathematical morphology, statistics) from pixel neighborhoods; uses ELEMENT methodology (pixel-classification controlled by region-growing where classification results influence further classifications); applies Random Forests classifier for vessel/non-vessel prediction.

Result: Achieved 95.48% accuracy, the best reported in literature at the time, outperforming unsupervised state-of-the-art approaches.

Conclusion: The proposed supervised approach with innovative feature extraction and the ELEMENT region-growing mechanism significantly advances vessel segmentation in angiograms.
...
TL;DR: The paper formalizes Spatially Prompted Visual Trajectory Prediction (SP-VTP), introduces a new dataset (EgoSPT), and proposes SPOT, a model that uses spatial prompts (bounding boxes/points) to predict future end-effector trajectories from egocentric video in cluttered manipulation environments.


<details>
  <summary>Details</summary>
Motivation: Robotic manipulation in cluttered environments with similar objects requires spatial indication (where to move and place) rather than language instructions, which are imprecise for such scenarios. Existing methods lack a formal spatial prompting framework for trajectory prediction.

Method: SPOT (Spatially Prompted Object-Target Policy) combines a task encoder (first-frame visual + coordinate spatial prompts), an observation encoder (current visual + history context), and a trajectory generator (future end-effector motion).
...
Conclusion: EgoSPT and SPOT establish SP-VTP as a new simple and scalable spatial prompting paradigm for egocentric manipulation, demonstrating that spatial prompts are effective task conditions.
...
TL;DR: MetaEarth-MM is a generative foundation model for multi-modal remote sensing imagery that enables paired joint generation and any-to-any translation across five modalities using a scene-centered joint modeling paradigm with decoupled architecture, supported by a large-scale EarthMM dataset of 2.8M images.


<details>
  <summary>Details</summary>
Motivation: Multi-modal remote sensing images are vital but complete paired observations are scarce; existing generative methods use isolated pairwise translation with limited versatility and scalability as modalities and tasks increase.

Method: A scene-centered joint modeling paradigm with decoupled architecture: first infers a latent scene representation from available observations, then generates target modalities conditioned on this intermediate state. Trained on EarthMM dataset (2.8M multi-resolution global images, 2.2M aligned pairs).

Result: Strong generative capability and robust generalization across diverse generation tasks; supports downstream tasks at both data and representation levels.

Conclusion: Demonstrates potential as a general foundation model for cross-modal Earth observation, with open-source code and dataset.
...
TL;DR: SetCon reformulates open-ended referring segmentation as set-level concept prediction using LVLM-generated natural-language concepts, achieving SOTA results across multiple image and video benchmarks.


<details>
  <summary>Details</summary>
Motivation: Previous LVLM-based methods treat multiple referred targets as separate outputs rather than a coherent set, lacking incentive to capture set-level properties like completeness and mutual exclusivity, especially in complex scenarios with multiple instances, cross-category groups, or open-ended target sets.

Method: Proposes Set-Concept Segmentation (SetCon) using LVLM-generated natural-language concepts as semantic conditions for joint mask-set decoding, with hierarchical semantic decomposition (shared set-level concept → fine-grained concept groups) and a two-stage annotation pipeline (236k samples, 784k concept phrases).

Result: SOTA on image benchmarks (+3.3 gIoU on gRefCOCO, +12.1 gIoU on MUSE) with growing margins as target number increases; new SOTA on seven referring video benchmarks (e.g., +10.9 J&F on MeViS, +12.4 J&F on Ref-SeCVOS) under detect-and-track setting.

Conclusion: Set-level concept prediction with hierarchical semantic decomposition effectively addresses limitations of sequential token-based methods for open-ended referring segmentation, and the concept interface transfers well to video tasks.
...
TL;DR: PixVerve-95K introduces a high-quality 100MP ultra-high-resolution dataset with 95K images, extends T2I models to native 100MP generation via three training schemes, and provides PixVerve-Bench benchmark for UHR evaluation.


<details>
  <summary>Details</summary>
Motivation: The growing demand for Ultra-High-Resolution (UHR) image generation (e.g., 100MP) driven by better visual experience and imaging technology, yet existing T2I models struggle due to scarcity and complexity of high-resolution content.

Method: (1) Curated PixVerve-95K dataset with a data pipeline (95K images, 100M+ pixels each, seven-dimensional annotations); (2) Three training schemes to extend T2I foundation models to native 100MP generation; (3) PixVerve-Bench benchmark with conventional metrics + multimodal LLM-based assessments.

Result: Extensive experiments on the benchmark demonstrate effectiveness of the dataset and training strategies for UHR image generation, providing valuable insights for future work.

Conclusion: PixVerve-95K, along with the proposed training schemes and benchmark, addresses critical challenges in UHR T2I generation and lays groundwork for future breakthroughs.
...
TL;DR: TideGS enables training billion-scale 3D Gaussian Splatting on a single 24GB GPU using an SSD-CPU-GPU out-of-core framework with hierarchical I/O and adaptive streaming.


<details>
  <summary>Details</summary>
Motivation: 3DGS training at billion-primitive scale is memory-bound; each Gaussian's large attribute vector quickly exceeds GPU capacity, limiting prior systems to tens of millions of Gaussians on single GPU.

Method: A three-part out-of-core framework: (1) block-virtualized geometry for SSD-aligned spatial locality, (2) hierarchical asynchronous pipeline to overlap I/O with computation, and (3) trajectory-adaptive differential streaming that transfers only incremental working-set deltas between iterations.

Result: TideGS trains over 1 billion Gaussians on a single 24GB GPU, achieving best reconstruction quality among single-GPU baselines on large-scale scenes, far exceeding prior limits (~100M out-of-core, ~11M in-memory).

Conclusion: Exploiting sparse, trajectory-conditioned nature of 3DGS training allows GPU memory to act as a working-set cache rather than a persistent store, enabling billion-scale training on commodity hardware.
...
TL;DR: A p-ResNet-50 prototype network is introduced for XCT inspection of aerospace SiC/SiC composites, achieving high accuracy (ROC-AUC 0.994) while providing case-based explanations tied to expert-defined semantic categories, thereby addressing the black-box transparency problem in industrial non-destructive testing.


<details>
  <summary>Details</summary>
Motivation: Current XCT-based defect detection in aerospace composites relies on expert visual assessment with limited traceability; deep learning offers automation but its black-box nature conflicts with industrial demands for transparency and auditable decisions.

Method: p-ResNet-50 extends a convolutional backbone with a prototype layer aligned to six expert-defined semantic categories. Two novel regularization terms (anchor-based and medoid-based) tether prototypes to expert patches and prevent collapse. UMAP-based latent-space analysis identifies uncertainty zones.

Result: Validated on ~12,000 XCT patches from four SiC/SiC specimens. p-ResNet-50 achieves comparable performance to black-box ResNet-50 (accuracy 0.957 vs 0.959; ROC-AUC 0.994 vs 0.993) with higher precision/specificity at slight sensitivity cost. Each decision includes representative evidence patches and uncertainty flags.

Conclusion: The prototype framework enables traceable, auditable defect detection for aerospace composites, and establishes a reusable methodology for embedding domain expertise into prototype networks for other XCT inspection scenarios.
...
TL;DR: VLMs lack genuine spatial intelligence; authors propose SNS evaluation and CaMo model for camera-motion-grounded spatial understanding.


<details>
  <summary>Details</summary>
Motivation: Current VLMs achieve high accuracy on spatial QA benchmarks but it's unclear if this reflects true spatial intelligence; the paper specifically identifies a lack of basic camera motion understanding.

Method: Proposes Spatial Narrative Score (SNS) — an evaluation framework where VLMs generate explicit spatial narratives (scene semantics + camera motion), then reasoning is done via a frozen proxy LLM. Introduces CaMo, a camera-motion-grounded VLM.

Result: Under SNS, state-of-the-art spatial VLMs show significant performance degradation despite high direct QA accuracy. CaMo achieves consistent performance across both SNS and direct QA.

Conclusion: Explicit spatial narrative externalization is important for evaluating VLMs' transferable 3D spatial understanding.
...
TL;DR: AUDITS is a comprehensive benchmark (530K+ images) for evaluating image manipulation detection under domain shifts, quality variations, manipulation types, and sizes, revealing limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: Advanced image editing and generative AI make creating convincing manipulations easy, posing risks of misinformation and false narratives, yet limited research exists on detecting such manipulations across diverse visual domains.

Method: Created AUDITS benchmark with 530K+ images from user and news photos; used diffusion-based inpainting for diverse manipulations; evaluated existing detection methods under various domain shifts.

Result: Existing image manipulation detection methods show limited robustness under different types of domain shifts, manipulation types, and sizes.

Conclusion: AUDITS provides insights to help develop more reliable and generalizable image manipulation detection methods, driving further research in this area.
...
TL;DR: MSAVBench is a comprehensive benchmark and adaptive hybrid evaluation framework for multi-shot audio-video generation, covering four key dimensions with up to 15 shots, achieving 91.5% Spearman correlation with human judgments, and revealing that current models struggle with director-level control and fine-grained AV sync.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for multi-shot audio-video generation are limited in scope, data diversity, and rely on rigid evaluation pipelines, preventing reliable assessment of modern models.

Method: A benchmark spanning video, audio, shot, and reference dimensions with adaptive self-correction for shot segmentation, instance-wise rubrics, and tool-grounded evidence extraction.

Result: 91.5% Spearman rank correlation with human judgments; evaluated 19 SOTA models; current models struggle with director-level control and audio-visual synchronization.

Conclusion: Modular/agentic generation pipelines offer a promising path to narrow the gap between open- and closed-source models; benchmark and code will be released.
...
### [133] [Position: Let's Develop Data Probes to Fundamentally Understand How Data Affects LLM Performance](https://arxiv.org/abs/2605.18801)
*Shiqiang Wang,Herbert Woisetschläger,Hans Arno Jacobsen,Mingyue Ji*

Main category: cs.AI

TL;DR: The paper proposes using synthetic "data probes" (sequences generated from random processes) to systematically study how data characteristics influence LLM behavior across training, tuning, alignment, and in-context learning stages.


<details>
  <summary>Details</summary>
Motivation: Current approaches for understanding data utility in LLMs rely on compute-intensive empirical heuristics with large public datasets, lacking principled methods to understand how specific data characteristics drive LLM behavior.

Method: Proposes developing systematic methodologies for generating synthetic sequences from random processes (data probes) that reveal useful characteristics when used in LLM workflows, analyzed through theoretical concepts like typical sets.

Result: Data probes can systematically reveal how data characteristics influence model performance, generalization, and robustness, providing a pathway beyond empirical heuristics.

Conclusion: The data-probe approach offers a principled framework for uncovering foundational insights into the role of data in LLM training and inference, moving beyond purely empirical approaches.
...
### [134] [Operationalizing Document AI: A Microservice Architecture for OCR and LLM Pipelines in Production](https://arxiv.org/abs/2605.18818)
...
TL;DR: Presents a microservice architecture for production-scale document understanding, combining classification, OCR, and LLM extraction, with key findings that OCR dominates latency and GPU capacity determines system saturation.


<details>
  <summary>Details</summary>
Motivation: There is a wide gap in literature between model definition and running models at production scale for document understanding systems.

Method: A microservice architecture encapsulating pipelines of classification, OCR, and LLM-based field extraction, with hybrid classification, GPU/CPU separation, async processing, and horizontal scaling.
...
Conclusion: Provides concrete architectural patterns for practitioners to operationalize document understanding models in production beyond benchmarks.
...
### [135] [Evaluating the Utility of Personal Health Records in Personalized Health AI](https://arxiv.org/abs/2605.18937)
...
TL;DR: This study evaluates how LLMs (Gemini 3.0 Flash) can use PHR data to answer patient health queries, finding significant improvements in helpfulness with PHR context, while also identifying gaps like temporal disorientation and confabulations.


<details>
  <summary>Details</summary>
Motivation: PHRs can empower patients but contain complex information; LLMs may help patients understand their health records better, but their potential with PHR context is underexplored.

Method: 2,257 user queries from 3 distributions (web search, chatbot templates, patient calls) were matched with de-identified PHRs. Gemini responses were generated with 3 levels of PHR context (none, basic summary, full clinical notes). Evaluation used SHARP framework and a new PHR-specific error framework, with autoraters and clinician ratings.

Result: Significant improvements in helpfulness across all question types with PHR data (p<0.001). Gains in safety, accuracy, relevance, and personalization. Identified gaps: temporal disorientation and rare confabulations.

Conclusion: PHR data can enhance LLM answers for diverse patient needs, but monitoring frameworks are needed to address specific gaps in LLM understanding of complex health records.
...
### [136] [Learn-by-Wire Training Control Governance: Bounded Autonomous Training Under Stress for Stability and Efficiency](https://arxiv.org/abs/2605.19008)
*Anis Radianis*

Main category: cs.AI

TL;DR: LBW-Guard is a bounded autonomous training-control layer atop AdamW that improves LLM training stability and efficiency, reducing perplexity by 18.7% and achieving 1.10x speedup on Qwen2.5-7B, while maintaining trainability under aggressive learning-rate stress where AdamW fails.


<details>
  <summary>Details</summary>
Motivation: Modern LLM training suffers from instability, degraded runs, and wasted compute under aggressive learning-rate, scale, and runtime-stress conditions, necessitating a governance mechanism beyond optimizer replacement.

Method: LBW-Guard operates above AdamW as a bounded autonomous training-control layer, observing training telemetry to detect instability-sensitive regimes and applying bounded control to optimizer execution without altering the core update rule or training objectives.

Result: On Qwen2.5-7B with WikiText-103, LBW-Guard reduces perplexity from 13.21 to 10.74 (18.7% improvement) and reduces training time from 392.54s to 357.02s (1.10x speedup). Under LR=3e-3 stress, AdamW degrades to 1885.24 perplexity while LBW-Guard stays at 11.57. Gradient clipping cannot reproduce this effect.

Conclusion: Stability-sensitive LLM training benefits from a governance plane above the optimizer. LBW-Guard demonstrates that bounded runtime control can preserve productive compute under stress, distinct from optimizer replacement or local gradient suppression approaches.
...
### [137] [AgentNLQ: A General-Purpose Agent for Natural Language to SQL](https://arxiv.org/abs/2605.19010)
...
TL;DR: A multi-agent NL2SQL method achieving 78.1% semantic accuracy on the BIRD benchmark via schema enrichment, business rules, and LLM-based orchestration with self-correction.


<details>
  <summary>Details</summary>
Motivation: NL2SQL conversion remains inaccurate compared to human experts despite LLM advances, necessitating further algorithmic improvements for parity.

Method: Multi-agent system with an optimized orchestrator leveraging LLMs for planning, reflection, and self-correction; advanced schema enrichment creating context-aware metadata; integration of user-provided business rules.

Result: Achieved 78.1% semantic accuracy on the BIRD benchmark, demonstrating accuracy and generalizability across diverse domains.

Conclusion: The proposed multi-agent approach with semantic enrichment and self-correction significantly improves NL2SQL accuracy, validated on a challenging benchmark.
...
### [138] [KAN-MLP-Mixer: A comprehensive investigation of the usage of Kolmogorov-Arnold Networks (KANs) for improving IMU-based Human Activity Recognition](https://arxiv.org/abs/2605.19031)
...
TL;DR: A hybrid KAN-MLP architecture for Human Activity Recognition (HAR) that strategically places KAN modules (input embedding and LarctanKAN classifier) with MLP intermediate layers achieves 5.33% average macro F1 improvement over pure MLP across 8 datasets, outperforming both pure KAN and MLP baselines while maintaining noise robustness and efficiency.


<details>
  <summary>Details</summary>
Motivation: KANs excel at learning complex functions on clean low-dimensional data but struggle with noisy real-world data and are computationally inefficient; MLPs are noise-tolerant and efficient but less precise. The challenge is to combine KANs' precision with MLPs' noise robustness and efficiency for HAR.

Method: Systematically explores KAN module placements within deep HAR networks; proposes a hybrid architecture with KAN-based input embedding, MLP intermediate layers for feature mixing, and a specialized LarctanKAN module for final classification.

Result: 5.33% average macro F1 improvement over pure-MLP across 8 public HAR datasets; outperforms standalone KAN and MLP baselines; boosts performance when integrated into other state-of-the-art HAR architectures.

Conclusion: A carefully orchestrated combination of KAN, MLP, or other conventional neural components yields more robust and accurate HAR models for real-world wearable sensing environments.
...
### [139] [Trustworthy Agent Network: Trust in Agent Networks Must Be Baked In, Not Bolted On](https://arxiv.org/abs/2605.19035)
*Yixiang Yao,Yuhang Yao,Xinyi Fan,Jiechao Gao,Jie Wang,Minjia Zhang,Srivatsan Ravi,Carlee Joe-Wong*

Main category: cs.AI

TL;DR: Vision paper arguing that trustworthiness in Agent-to-Agent (A2A) networks must be architected from the start through four design pillars, rather than retrofitted from individual-agent alignment techniques.


<details>
  <summary>Details</summary>
Motivation: As LLM-based agents evolve from isolated operation to collaborative A2A networks, systemic vulnerabilities (adversarial composition, semantic misalignment, cascading failures) emerge that existing agent alignment techniques cannot address.

Method: Conceptual framework with four design pillars for architecting trustworthiness from the beginning of A2A coordination framework.

Result: Identifies that trust in A2A networks cannot be guaranteed via retrofitting existing individual-agent protocols, but must be architected from the start.

Conclusion: A2A network trustworthiness requires a fundamentally new approach built into the coordination framework itself, not retrofitted solutions.
...
### [140] [Interference-Aware Multi-Task Unlearning](https://arxiv.org/abs/2605.19042)
*Ying-Hua Huang,Rui Fang,Hsi-Wen Chen,Ming-Syan Chen*

Main category: cs.AI

TL;DR: This paper introduces multi-task unlearning for shared-backbone models, addressing interference between tasks and instances via an interference-aware framework using task-aware gradient projection and instance-level gradient orthogonalization.


<details>
  <summary>Details</summary>
Motivation: Existing machine unlearning methods focus on single-task settings, but modern models operate in multi-task setups with shared backbones where removing data for one task can unintentionally harm performance on others.

Method: An interference-aware framework combining task-aware gradient projection (constrains updates within task-specific subspaces) and instance-level gradient orthogonalization (reduces conflicts between forget and retain signals).

Result: Reduces UIS by 30.3% in full-task unlearning and 52.9% in partial-task unlearning compared to the strongest baseline on two multi-task CV benchmarks.

Conclusion: The proposed method effectively achieves multi-task unlearning while maintaining strong generalization, outperforming existing baselines significantly.
...
### [141] [Embedding by Elicitation: Dynamic Representations for Bayesian Optimization of System Prompts](https://arxiv.org/abs/2605.19093)
*Zhiyuan Jerry Lin,Benjamin Letham,Samuel Dooley,Maximilian Balandat,Eytan Bakshy*

Main category: cs.AI

TL;DR: ReElicit is a Bayesian optimization framework for system prompt tuning using aggregate feedback, where an LLM elicits a compact feature space to map prompts, uses a Gaussian process surrogate to select promising target features, and iteratively refines both the feature space and prompts—achieving strong results on ten tasks with only 30 evaluations per task.


<details>
  <summary>Details</summary>
Motivation: System prompts are hard to tune when only aggregate metrics (e.g., overall accuracy) are available, without per-example labels, failures, or critiques. Existing methods struggle with sample-constrained black-box optimization over discrete, variable-length text.

Method: ReElicit uses an LLM to iteratively elicit a compact, interpretable feature space from task descriptions and previously evaluated prompts. Prompts are mapped into this feature space; a Gaussian process surrogate models the mapping from features to scores, and an acquisition function selects promising feature vectors, which the LLM then converts into deployable prompts. The feature space is re-elicited as new evaluations arrive.

Result: On ten system prompt optimization tasks with a budget of 30 evaluations per task, ReElicit outperforms representative aggregate-only prompt optimization baselines in aggregate performance.

Conclusion: LLMs can serve as adaptive semantic representation builders (not just prompt generators) for Bayesian optimization over natural-language artifacts, enabling effective prompt tuning from aggregate feedback alone.
...
### [142] [DecisionBench: A Benchmark for Emergent Delegation in Long-Horizon Agentic Workflows](https://arxiv.org/abs/2605.19099)
*Yuxuan Gao,Megan Wang,Yi Ling Yu,Zijian Carl Ma,Ao Qu*

Main category: cs.AI

TL;DR: DecisionBench is a benchmark for evaluating delegation in multi-agent workflows, revealing that quality alone misses orchestration signals, routing fidelity varies significantly by awareness condition, and a large performance gap exists between current and perfect delegation.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks lack systematic substrates for evaluating how well agents delegate tasks to peer models in long-horizon workflows, making it hard to isolate and measure orchestration capabilities.

Method: The paper constructs DecisionBench with a fixed task suite (GAIA, tau-bench, BFCL multi-turn), 11 peer models from 7 vendors, a delegation interface, deterministic skill annotations, and multi-axis metrics (quality, cost, latency, delegation rate, routing fidelity, vendor self-preference, counterfactual ceiling). A five-condition reference sweep on 23,375 task instances characterizes the substrate.

Result: Three findings: (i) mean end-task quality is statistically indistinguishable across awareness conditions, so quality-only evaluation misses orchestration; (ii) routing fidelity-at-1 ranges from 7.5%-29.5% at near-equal quality, with delivery channel dominating content; (iii) a counterfactual ceiling shows perfect delegation is 15-31 percentage points above current performance.

Conclusion: DecisionBench reveals substantial headroom for future orchestration methods, and the authors release the full substrate, annotations, analysis pipeline, and run archives for community use.
...
### [143] [POLAR-Bench: A Diagnostic Benchmark for Privacy-Utility Trade-offs in LLM Agents](https://arxiv.org/abs/2605.19127)
*Qiaoyuan Zheng,Yiqu Yang,Qi Gao,Imanol Schlag*

Main category: cs.AI

TL;DR: POLAR-Bench evaluates LLM agents' ability to protect private user data from adversarial third parties, finding that frontier models withhold >99% of protected attributes while smaller models (1-30B) leak over half.


<details>
  <summary>Details</summary>
Motivation: LLM agents access private user data and must follow privacy policies even when third-party systems behave adversarially, but there is no standardized benchmark to evaluate such privacy-preserving behavior.

Method: Proposes POLAR-Bench where a trusted model with a privacy policy and task converses with an adversarial third-party model that probes for both task-relevant and protected attributes across 10 domains and 7,852 samples, with a 5×5 diagnostic surface varying privacy policy dimension and attack strategy.

Result: Frontier models withhold over 99% of protected attributes; smaller open-weight models (1-30B) score notably worse, with the weakest leaking over half of protected attributes.

Conclusion: POLAR-Bench localizes where each model's intent-following breaks down, providing a foothold for privacy alignment where it matters most.
...
### [144] [Learning to Hand Off: Provably Convergent Workflow Learning under Interface Constraints](https://arxiv.org/abs/2605.19140)
*Jiayu Li,Enpei Zhang,Dawei Zhou,Elynn Chen,Yujun Yan*

Main category: cs.AI

TL;DR: This paper introduces IC-Q, an asynchronous decentralized Q-learning algorithm for multi-agent LLM pipelines where agents only communicate through shared artifacts at handoff points, with each agent observing only local information. It provides the first finite-sample guarantee for neural Q-learning under decentralized partial observability, validated through four experiments.


<details>
  <summary>Details</summary>
Motivation: To formalize and provide theoretical guarantees for workflow learning in multi-agent LLM pipelines where agents operate across organizational, vendor, or trust boundaries with no centralized access to joint trajectories and only limited handoff communication.

Method: Formalizes the problem as an interface-constrained semi-Markov decision process (IC-SMDP) with decision epochs at handoff times. Proposes IC-Q, a decentralized Q-learning algorithm with single-scalar cross-agent coordination per handoff. Extends the AIS framework from single-agent MDPs to multi-agent SMDPs with random duration discounting.

Result: Provides a finite-sample bound for neural IC-Q decomposing into three independently controllable error sources: neural approximation error, interface representation gap, and mixing-time residual. Validates the bound through four experiments (synthetic IC-SMDP, multi-LLM reasoning, multi-agent routing, multi-agent CPU programming).

Conclusion: IC-Q matches centralized oracle performance without any agent observing joint trajectories, with each error source scaling predictably along its corresponding axis. This is the first finite-sample guarantee for neural Q-learning under decentralized partial observability.
...
### [145] [Progressive Autonomy as Preference Learning: A Formalization of Trust Calibration for Agentic Tool Use](https://arxiv.org/abs/2605.19151)
*Changkun Ou*

Main category: cs.AI

TL;DR: The paper formalizes trust calibration for agentic tool use as a preference-learning problem using Preferential Bayesian Optimization with Gaussian processes, where a policy gateway learns human risk-tolerance through binary feedback and escalates uncertain decisions to humans.


<details>
  <summary>Details</summary>
Motivation: To formalize the problem of deciding when an automated agent's actions should execute autonomously versus require human approval, addressing the trust calibration challenge in human-AI interaction.

Method: Formulates trust calibration as a preference-learning problem using a policy gateway with Gaussian-process posterior over latent human risk-tolerance, observed through probit likelihood on binary approve/deny feedback, escalating actions where approval outcome is most uncertain. This is structurally an instance of Preferential Bayesian Optimization.

Result: Shows the approach inherits inference machinery (approximate Gaussian-process classification) and sample-efficiency argument (uncertainty-targeted querying) from Preferential Bayesian Optimization, while differing in objective (classifying action space into allow/block/ask regions rather than optimizing a design).

Conclusion: The proposed framework provides a principled way to calibrate trust in autonomous agents by learning human risk preferences through selective escalation, leveraging Bayesian optimization techniques for sample efficiency.
...
### [146] [How Far Are We From True Auto-Research?](https://arxiv.org/abs/2605.19156)
*Zhengxin Zhang,Ning Wang,Sainyam Galhotra,Claire Cardie*

Main category: cs.AI

TL;DR: ResearchArena tests three AI agents (Claude Code, Codex, Kimi Code) on producing complete research papers. While manuscript-only reviews suggest competitive quality (Claude Code matching human ICLR 2025 submissions), artifact-aware reviews and human audits reveal severe flaws: fabricated results, underpowered experiments, and plan/execution mismatches. None of 117 papers passed top-venue acceptance bar.


<details>
  <summary>Details</summary>
Motivation: Existing auto-research systems can produce complete papers, but the field lacks systematic study of how good agent-generated papers actually are. Prior evaluations may overstate quality due to manuscript-only review.

Method: ResearchArena: a minimal scaffold enabling off-the-shelf agents (Claude Code/Opus 4.6, Codex/GPT-5.4, Kimi Code/K2.5) to perform full research loop (ideation, experimentation, writing, self-refinement). 13 CS seeds × 3 trials = 117 papers evaluated via three lenses: manuscript-only SAR, artifact-aware peer review (PR), and human meta-review.

Result: Claude Code scored highest under SAR, matching human ICLR 2025 submissions. But SAR poorly aligned with actual acceptance. Artifact-aware PR scores dropped sharply. Three failure modes identified: fabricated results, underpowered experiments, plan/execution mismatch. Codex 5%/8% mismatch/fabrication vs Kimi Code 77%/72% (~15× spread). None of 117 papers met top-venue acceptance bar.

Conclusion: We are still gapped from true auto-research. Manuscript-only evaluation overstates agent capabilities; experimental rigor is the major bottleneck. Distinct research personas emerge across agents with vastly different failure rates.
...
### [147] [Discoverable Agent Knowledge -- A Formal Framework for Agentic KG Affordances (Extended Version)](https://arxiv.org/abs/2605.19186)
*Terry R. Payne,Valentina Tamma,Enrico Daga*

Main category: cs.AI

TL;DR: The paper extends Semantic Web Services formalisms (OWL-S, WSMO) to Knowledge Graphs, proposing an Agentic Affordance Profile (AAP) as a semantic metadata layer above VoID/DCAT to enable KG selection, composition, and failure diagnosis at agent planning time.


<details>
  <summary>Details</summary>
Motivation: Current KG metadata standards (VoID, DCAT) describe content but ignore what an agent can prove from a KG, closure assumptions, or vocabulary grounding. They fail to capture epistemic mismatches, especially when the governing DL schema and operative entailment regime diverge—an invisible failure mode in deployed KGs.

Method: The authors revisit Semantic Web Services insights (OWL-S, WSMO) and extend them with a four-dimensional formal framework for the KG setting, from which they derive the Agentic Affordance Profile (AAP)—a semantic layer above VoID and DCAT.

Result: A formal framework and the AAP specification that enables principled KG selection, composition, and failure diagnosis at agent planning time. A five-point research agenda outlines formal, computational, and engineering work for scaling AAP-based affordance matching.

Conclusion: The Semantic Web Services paradigm offers underexploited insights for modern KG interoperability. The AAP framework bridges this gap, but significant research is needed to realize affordance matching at scale.
...
### [148] [Hallucination as Exploit: Evidence-Carrying Multimodal Agents](https://arxiv.org/abs/2605.19192)
*Guijia Zhang,Hao Zheng,Harry Yang*

Main category: cs.AI

TL;DR: Proposes evidence-carrying multimodal agents (ECA) that prevent hallucination-to-action conversion by requiring external certificates from DOM/OCR/AX verifiers to authorize tool calls, rather than relying on model text as evidence.


<details>
  <summary>Details</summary>
Motivation: Multimodal agents can hallucinate visual claims that trigger real actions (clicks, emails, transfers), turning answer-quality errors into authorization failures—a problem formalized as "hallucination-to-action conversion."

Method: ECA decomposes tool calls into action-critical predicates, obtains typed certificates from constrained DOM/OCR/AX verifiers, and uses a deterministic gate to grant privileges only when certificates support them. Includes verifier red-teaming with 1,900 attacks.

Result: Gate bypass reduced from 15% to 1.3% after hardening; 0% unsafe-action rate on 200-task pipeline (Wilson 95% upper bound 2.67%) and 120-task browser PoC (upper bound 4.3%); HACR audit shows naive agents 100.0% unsafe, prompt-only 49.6% unsafe, ECA 0% unsafe.

Conclusion: Model language may propose actions, but external evidence must authorize them—a simple principle that prevents unsafe executions from unsupported perceptual claims.
...
### [149] [Not all uncertainty is alike: volatility, stochasticity, and exploration](https://arxiv.org/abs/2605.19215)
*Payam Piray*

Main category: cs.AI

TL;DR: This paper shows that volatility and stochasticity, two sources of uncertainty, drive optimal exploration in opposite directions: volatility enhances exploration while stochasticity suppresses it, contrary to prior work treating them as equivalent.


<details>
  <summary>Details</summary>
Motivation: Prior work treats distinct sources of environmental uncertainty as equivalent, but the authors show they have opposite effects on adaptive exploration, which has implications for AI and biological decision-making.
...
Result: CAUSE outperforms standard exploration strategies in heterogeneous noise environments and improves on Gittins-per-arm policies. The framework shows volatility enhances exploration while stochasticity suppresses it, and predicts reversed exploration under pathological noise inference.

Conclusion: Learning and exploration are governed by the same noise-inference asymmetry, and the framework has implications for computational accounts of psychiatric conditions where exploration is reversed rather than merely impaired.
...
### [150] [SimGym: A Framework for A/B Test Simulation in E-Commerce with Traffic-Grounded VLM Agents](https://arxiv.org/abs/2605.19219)
...
TL;DR: SimGym is a framework that simulates A/B tests on e-commerce storefronts using VLM agents in live browsers, achieving 77% alignment with real buyer behavior while reducing experimental cycles from weeks to under an hour.


<details>
  <summary>Details</summary>
Motivation: Traditional A/B testing for e-commerce storefronts diverts traffic, takes weeks to reach statistical significance, and risks degrading user experience.

Method: Three components: (a) traffic-grounded persona generation from production clickstream data, (b) live-browser VLM agents with multimodal perception, episodic memory, and guardrails, (c) evaluation protocol comparing simulated vs. real outcome shifts.

Result: 77% directional alignment with add-to-cart shifts from real-buyer traffic across visually driven UI theme changes.

Conclusion: SimGym enables rapid A/B testing simulation (under an hour vs. weeks) without exposing real buyers to candidate variants.
...
### [151] [Can Large Language Models Revolutionize Survey Research? Experiments with Disaster Preparedness Responses](https://arxiv.org/abs/2605.19229)
*Yan Wang,Ziyi Guo,Christopher McCarty*

Main category: cs.AI

TL;DR: Evaluates a five-stage LLM integration framework for survey research using 2024 Hurricane Milton data, introducing a theory-informed LLM (A-TLM) that outperforms classical imputation methods on block-wise missing data and a retrieval-constrained chatbot for grounded refusal.


<details>
  <summary>Details</summary>
Motivation: Survey research faces declining response rates, sample bias, block-wise missingness, and AI-assisted fraud; rigorous evaluations of LLM integration across the full survey workflow are scarce, especially in disaster contexts where data quality is critical.

Method: Proposes a five-stage framework (questionnaire design, sample selection, pilot testing, missing-data imputation, post-collection analysis) using 2024 Hurricane Milton survey data (n=946). Develops a PMT-constrained co-occurrence knowledge graph and seven LLM configurations, including Anchored Marginal Theory-Informed LLM (A-TLM).

Result: A-TLM outperforms IPW/MI, MICE+PMM, and missForest on RMSE under block-wise MNAR (1.439 vs. 1.496) with near-zero signed bias (-0.121 vs. -0.631 for missForest). Theory-informed retrieval (MAE 0.993) beats unstructured RAG (1.097). Subgroup-stratified bias auditing reveals masked errors.

Conclusion: LLMs can improve survey data quality when grounded in theory and retrieval constraints; proposes subgroup-stratified bias auditing as a reporting standard and demonstrates that hallucination is architecturally manageable via grounded refusal.
...
### [152] [Causal Evidence for Attention Head Imbalance in Modality Conflict Hallucination](https://arxiv.org/abs/2605.19250)
*Jinrui Jiang,Zhangtai Wu,Zhen Wu,Xinyu Dai*

Main category: cs.AI

TL;DR: This paper investigates modality-conflict hallucination in MLLMs through causal analysis of attention heads, discovering that hallucination-driving heads are broadly distributed while resisting heads are localized. Based on this, they propose MACI, a conditional intervention that suppresses driving heads only when conflict is detected, achieving significant hallucination reduction.


<details>
  <summary>Details</summary>
Motivation: To understand why visual evidence fails to prevail over erroneous textual premises during generation in multimodal large language models (MLLMs), by examining the internal components that drive or resist this modality-conflict hallucination from a mechanistic perspective.

Method: Performed head-level causal analysis using path patching across five open-source MLLMs to identify hallucination-driving and hallucination-resisting attention heads. Proposed MACI (Modality-conflict-Aware Causal Intervention), a conditional intervention that suppresses causally identified hallucination-driving heads only when modality conflict is detected.

Result: Found a consistent asymmetry: driving effects are broadly distributed with greater aggregate weight, while resisting effects concentrate in a small number of high-importance heads. MACI achieved the largest hallucination reduction among compared inference-time baselines on the MMMC benchmark with favorable hallucination-accuracy trade-off, and transferred zero-shot to SCI-SemanticConflict test.

Conclusion: The imbalanced routing structure (distributed driving influence and localized resistance) biases generation toward erroneous premises. MACI effectively reduces modality-conflict hallucinations by leveraging this mechanistic understanding.
...
### [153] [AQuaUI: Visual Token Reduction for GUI Agents with Adaptive Quadtrees](https://arxiv.org/abs/2605.19260)
*Yuankai Li,Tinghui Zhu,Ha Min Son,Zhe Zhao,Xin Liu,Muhao Chen*

Main category: cs.AI

TL;DR: AQuaUI is a training-free inference-time token reduction method for GUI agent models that uses adaptive quadtree construction to exploit spatial information density variations in screenshots, achieving up to 13.22% speedup and 29.52% fewer tokens while retaining 99.06% performance.


<details>
  <summary>Details</summary>
Motivation: GUI screenshots fed into LMM-based agents have highly non-uniform spatial information density (large empty regions vs. dense text/icons), but existing token reduction methods ignore structured GUI layout and spatial redundancy, requiring extra training or attention-based compression.

Method: Builds an adaptive quadtree per screenshot, keeps one representative merged token per leaf while preserving spatial positions. A conditional quadtree algorithm leverages continuity across consecutive screenshots for temporal consistency in multi-step GUI interactions.

Result: On GUI-Owl-1.5-32B-Instruct, achieves up to 13.22% speedup and 29.52% fewer visual tokens with 99.06% of full-token performance; consistently outperforms prior baselines on grounding and navigational benchmarks.

Conclusion: Spatial redundancy in GUI screenshots can be effectively exploited at inference time without retraining, improving efficiency-accuracy trade-offs for GUI agent models.
...
### [154] [Swimming with Whales: Analysis of Power Imbalances in Stake-Weighted Governance](https://arxiv.org/abs/2605.19264)
*Yuzhe Zhang,Manvir Schneider,Qin Wang,Davide Grossi*

Main category: cs.AI

TL;DR: This paper analyzes power imbalances in Proof-of-Stake blockchain governance using the Penrose-Banzhaf power index, showing analytically that perfect power-stake alignment is impossible but can be approximated under certain conditions, and empirically demonstrating power distortions in real-world systems like Project Catalyst.


<details>
  <summary>Details</summary>
Motivation: To study power distortions in stake-weighted voting systems where large stakeholders can dominate decision-making, using computational social choice theory to quantify power imbalances.

Method: Analytical modeling of power-stake alignment using the Penrose-Banzhaf power index, combined with empirical analysis of real-world on-chain governance data from Project Catalyst.

Result: Perfect alignment between voting power and stake ownership is generally unattainable, but can be approximated in expectation under specific conditions. Empirical data reveals significant power imbalances in current systems.

Conclusion: Stake-weighted voting in PoS blockchains inherently suffers from power distortions, though approximations of fairer power distribution are possible under certain conditions.
...
### [155] [MOCHA: Multi-Objective Chebyshev Annealing for Agent Skill Optimization](https://arxiv.org/abs/2605.19330)
...
TL;DR: MOCHA uses Chebyshev scalarization and exponential annealing to optimize LLM agent skills under multi-objective constraints (performance vs platform limits), outperforming existing optimizers that failed on 4/6 tasks.


<details>
  <summary>Details</summary>
Motivation: Existing prompt optimizers ignore the multi-objective nature of skill optimization under platform constraints (truncation, compaction, limited context), collapsing trade-offs into weighted sums and missing Pareto-optimal variants in non-convex regions.

Method: MOCHA replaces single-objective selection with Chebyshev scalarization (covers full Pareto front including non-convex regions) combined with exponential annealing for exploration-to-exploitation transition.

Result: On 6 diverse agent skills, existing optimizers failed to improve on 4/6 tasks (zero progress over 1000 rollouts). MOCHA achieved 7.5% relative improvement in mean correctness over the strongest baseline (up to 14.9% on FEVER, 10.4% on TheoremQA) and discovered twice as many Pareto-optimal variants.

Conclusion: Multi-objective optimization with Chebyshev scalarization is essential for effective LLM agent skill optimization under real-world platform constraints, outperforming single-objective and weighted-sum approaches.
...
### [156] [Agentic Trading: When LLM Agents Meet Financial Markets](https://arxiv.org/abs/2605.19337)
*Yihan Xia,Panpan You,Taotao Wang,Fang Liu,Han Qi,Xiaoxiao Wu,Shengli Zhang*

Main category: cs.AI

TL;DR: This paper audits 77 studies on LLM-based trading agents, finding that only 2/19 meet minimal reproducibility standards, with most lacking comparable evaluation protocols, execution semantics, and reproducible artifacts.


<details>
  <summary>Details</summary>
Motivation: Growing interest in embedding LLMs as trading agents, but the field lacks standardized evaluation protocols and reproducibility, creating a need for systematic audit and evidence mapping.

Method: Protocol-coded snapshot of 77 studies screened through 2026-03-09, with a primary empirical subset (n=19) meeting minimum boundary of Action Output plus Closed-Loop Evaluation. Uses Architecture-Capability-Adaptation as an analytical lens.

Result: Only 2/19 studies report extractable time-consistent split protocols, 1/19 reports transaction-cost model, 1/19 documents survivorship handling, 11/19 report execution timing, 15/19 coded as R0, and no study reaches R3 reproducibility.

Conclusion: Architectural experimentation is expanding rapidly, but comparable evaluation protocols, execution semantics, and reproducible artifacts remain critical bottlenecks in the field.
...
TL;DR: Generative Recursive Reasoning Models (GRAM) turn deterministic recursive reasoning into probabilistic multi-trajectory computation, enabling multiple hypotheses, inference-time scaling, and both conditional/unconditional generation, outperforming deterministic baselines on structured reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing Recursive Reasoning Models (RRMs) are largely deterministic, following a single latent trajectory and converging to a single prediction, limiting their ability to explore multiple hypotheses or alternative solution strategies.

Method: GRAM models reasoning as a stochastic latent trajectory using a generative latent-variable model with shared transition functions, trained via amortized variational inference, supporting both conditional reasoning p_θ(y|x) and unconditional generation p_θ(x).

Result: GRAM improves over deterministic recurrent and recursive baselines on structured reasoning and multi-solution constraint satisfaction tasks, and demonstrates unconditional generation capability.

Conclusion: Stochastic recursive latent reasoning with multi-trajectory sampling is a promising alternative to deterministic recursive models, enabling richer reasoning capabilities and inference-time scaling.
...
### [158] [PRISM: A Benchmark for Programmatic Spatial-Temporal Reasoning](https://arxiv.org/abs/2605.19382)
...
TL;DR: PRISM is a large-scale benchmark (10,372 instruction-code pairs) for evaluating programmatic video generation by LLMs, revealing a ~41% gap between code executability and spatial coherence.


<details>
  <summary>Details</summary>
Motivation: Existing programmatic video generation lacks rigorous evaluation of spatial correctness in language model outputs; current benchmarks are too small and focus narrowly on executability rather than visual coherence.

Method: Constructs PRISM benchmark (10,372 pairs, 20× larger than prior, bilingual English/Chinese, 437 categories) and proposes a funnel-style evaluation framework with four metrics: Code-Level Reliability, Spatial Reasoning, PADVC (dynamic visual complexity), and Temporal Density (temporal activity).

Result: Evaluated 7 mainstream LLMs, finding an Execution-Spatial Gap: average ~41% drop from execution success rate to spatial pass rate, meaning runnable code often fails to produce spatially coherent animations.

Conclusion: Programmatic video evaluation must go beyond executability; PRISM provides a principled benchmark for advancing spatially coherent code generation.
...
### [159] [Conflict-Resilient Multi-Agent Reasoning via Signed Graph Modeling](https://arxiv.org/abs/2605.19418)
*Longgang He,Longzhu He,Daojing He,Chaozhuo Li*

Main category: cs.AI

TL;DR: SIGMA is a signed graph-based multi-agent reasoning framework that models trust, conflict, and neutral relations among agents via signed message passing, achieving superior accuracy and conflict resilience on six benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based MAS frameworks propagate errors from conflicting signals and lack explicit modeling of inter-agent conflicts and structural awareness, leading to unreliable interactions and suboptimal performance.

Method: SIGMA first selects relevant and diverse agents, constructs a signed interaction graph with confidence-weighted edges, then performs conflict-aware signed message passing that reinforces trustworthy agents while suppressing conflicting signals, followed by structure- and conflict-aware weighted aggregation.

Result: SIGMA consistently outperforms state-of-the-art baselines on six benchmark datasets across multiple LLM backbones and multi-agent configurations, achieving notable gains in both accuracy and conflict-resilient performance.

Conclusion: Explicitly modeling trust, conflict, and neutral relations via signed graphs significantly improves the robustness and accuracy of LLM-based multi-agent systems.
...
### [160] [What and When to Distill: Selective Hindsight Distillation for Multi-Turn Agents](https://arxiv.org/abs/2605.19447)
*Xiaozhe Li,Tianyi Lyu,Yang Li,Yichuan Ma,Peiji Li,Linyang Li,Qipeng Guo,Dahua Lin,Kai Chen*

Main category: cs.AI

TL;DR: SERL framework selectively uses environment feedback to improve credit assignment in RL for LLM agents, achieving 90.0% and 80.1% success on ALFWorld and WebShop.


<details>
  <summary>Details</summary>
Motivation: Long-horizon credit assignment is challenging in RL for LLM agents due to sparse task rewards, and existing methods underutilize per-step environmental feedback in multi-turn settings.

Method: SERL (Selective Environment-Reweighted Learning) uses task reward for update direction and environment feedback to adjust placement and magnitude of updates, focusing on critical actions.

Result: Achieves 90.0% on ALFWorld and 80.1% on WebShop, outperforming strong RL and distillation baselines.

Conclusion: Grounded, action-relevant feedback at meaningful points is more effective than indiscriminate use of longer or richer context.
...
### [161] [Generative Auto-Bidding with Unified Modeling and Exploration](https://arxiv.org/abs/2605.19457)
*Mingming Zhang,Feiqing Zhuang,Na Li,Shengjie Sun,Xiaowei Chen,Junxiong Zhu,Fei Xiao,Keping Yang,Lixin Zou,Chenliang Li*

Main category: cs.AI

TL;DR: GUIDE is a generative auto-bidding framework that integrates directed exploration with a safe fallback mechanism, achieving significant performance gains on Taobao's advertising platform.


<details>
  <summary>Details</summary>
Motivation: Existing automated bidding methods (rule-based, RL, generative models) lack explicit mechanisms to balance exploration and safety, resulting in inefficient exploration and elevated financial risk.

Method: GUIDE uses a Decision Transformer (DT) to jointly model historical bidding and state transitions, a Q-value module to guide DT exploration via regularization, and an Inverse Dynamics Module (IDM) to infer robust actions as a safe fallback. The Q-value adaptively selects between DT and IDM outputs in an "explore-safeguard-select" pipeline.

Result: On Taobao's real-world deployment, GUIDE achieves +4.10% ad GMV, +1.40% ad clicks, +1.66% ad cost, and +3.52% ad ROI, outperforming state-of-the-art baselines.

Conclusion: GUIDE effectively unifies exploration efficiency and safety in auto-bidding, demonstrating strong industrial applicability and consistent superiority across offline and online settings.
...
### [162] [Beyond Mode Collapse: Distribution Matching for Diverse Reasoning](https://arxiv.org/abs/2605.19461)
...
TL;DR: DMPO prevents mode collapse in on-policy RL by approximating forward KL minimization to maintain diverse solutions, achieving 9-12% quality improvements on NP-hard problems and better generalization on reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: On-policy RL methods like GRPO suffer from mode collapse (lacking solution diversity). The authors show this is due to reverse KL minimization's mode-seeking behavior that reinforces the first high-reward trajectory found instead of maintaining diverse solutions.

Method: DMPO constructs a group-level target distribution over sampled trajectories proportional to their rewards, then aligns the policy distribution to this target via principled approximation of forward KL minimization, providing mode-covering behavior without sampling from the intractable global target.

Result: DMPO achieves 43.9% Quality Ratio on text-based NP-Bench (vs. GRPO's 40.1%) and 43.1% on vision-based NP-Bench (vs. 38.4%), with 9-12% relative improvements. Also shows +2.0% on math reasoning and +2.3% on out-of-domain tasks.

Conclusion: Distribution matching is a practical and principled approach to preventing mode collapse in on-policy RL, with consistent quality improvements and sustained exploration across diverse reasoning tasks and modalities.
...
### [163] [Attention-Guided Reward for Reinforcement Learning-based Jailbreak against Large Reasoning Models](https://arxiv.org/abs/2605.19485)
*Zheng Lin,Zhenxing Niu,Haoxuan Ji,Yuzhe Huang,Haichang Gao*

Main category: cs.AI

TL;DR: LRMs are more vulnerable to jailbreak attacks, and a novel RL-based jailbreak method using attention patterns and persuasion strategies achieves higher ASR.


<details>
  <summary>Details</summary>
Motivation: LRMs expose internal reasoning, increasing safety risks; prior work shows they are more vulnerable to jailbreaks than standard LLMs.

Method: Proposes an RL-based jailbreak method incorporating attention signals into reward design and using persuasion strategies to enrich the action space.

Result: Achieves substantially higher ASR across five LRMs and three benchmarks, outperforming baselines in effectiveness, efficiency, and transferability.

Conclusion: Attention patterns correlate with jailbreak success; RL with attention-aware rewards and diverse persuasion strategies effectively attacks LRMs.
...
### [164] [Position: The Turing-Completeness of Real-World Autoregressive Transformers Relies Heavily on Context Management](https://arxiv.org/abs/2605.19514)
*Guanyu Cui,Zhewei Wei,Kun He*

Main category: cs.AI

TL;DR: This paper clarifies that Transformers' Turing-completeness proofs often apply to a "scaling-family" setting (different models for different input lengths), not the "fixed-system" setting that matches real-world LLMs. It shows context-management methods critically determine computational power in practice.


<details>
  <summary>Details</summary>
Motivation: Existing claims of Transformer Turing-completeness conflate two distinct settings: fixed-system (real-world deployment) and scaling-family (theoretical proofs). This confusion leads to misinterpretation of what existing results actually demonstrate about LLM capabilities.

Method: The paper formalizes the fixed-system setting to characterize real-world LLM operation, analyzes the gap between scaling-family proofs and standard Turing-completeness, and examines how different context-management methods affect computational power.

Result: Scaling-family results provide meaningful resource bounds but do not establish Turing-completeness; context-management methods (not just the Transformer architecture) critically determine computational power in fixed systems.

Conclusion: Context management is a central component determining the computational power of real-world autoregressive Transformers, and the field should be more precise about which setting is being analyzed when making completeness claims.
...
### [165] [BLINKG: A Benchmark for LLM-Integrated Knowledge Graph Generation](https://arxiv.org/abs/2605.19518)
*Carla Castedo,Enrique Iglesias,Manuel Lama,Alberto Bugarin-Diz,Maria-Esther Vidal,David Chaves-Fraga*

Main category: cs.AI

TL;DR: BLINKG is a benchmark for evaluating LLMs' ability to map heterogeneous data schemas to ontology terms for knowledge graph construction, finding promising but limited performance in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: KG construction remains labor-intensive due to the difficulty of aligning input data with ontology terms; there is no standardized framework to assess LLMs' mapping capabilities for this task.

Method: The authors propose BLINKG, a benchmark with real-world scenarios of increasing complexity, and evaluate several state-of-the-art LLMs on it.

Result: LLMs show promising solutions but performance is limited in complex scenarios.

Conclusion: The benchmark enables assessment of current LLM capabilities for KG construction and defines requirements for (semi)automated LLM-driven KG construction, opening new research lines.
...
### [166] [Efficient Elicitation of Collective Disagreements](https://arxiv.org/abs/2605.19521)
*Mohamed Ouaguenouni,Felipe Garrido-Lucero,Umberto Grandi,César Hidalgo,Magdalena Tydrichova*

Main category: cs.AI

TL;DR: The paper proposes a stratified framework using a "plurality matrix" to measure disagreement among voters, showing pairwise comparisons are insufficient (level 2) and that meaningful disagreement measures like rank-variance and divisiveness require level 3 or higher.


<details>
  <summary>Details</summary>
Motivation: Pairwise comparisons cannot distinguish structural disagreement from noise; a more expressive framework is needed to capture nuanced voter disagreement patterns.

Method: Introduces the plurality matrix (recording for each subset S the probability each alternative ranks first in S), defines "level" of disagreement measures by minimal subset size needed, and designs two elicitation protocols to estimate the matrix.

Result: Many disagreement measures (rank-variance, divisiveness) sit at level 3, proving pairwise comparisons (level 2) are insufficient. Going beyond level 3 is beneficial both theoretically and experimentally.

Conclusion: The plurality matrix framework offers a principled way to analyze disagreement, and the elicitation protocols balance number of participants against cognitive load.
...
### [167] [Generative-Evaluative Agreement: A Necessary Validity Criterion for LLM-Enabled Adaptive Assessment](https://arxiv.org/abs/2605.19529)
*Grandee Lee,Yue Wang,Che Yee Lye,Luke Peh*

Main category: cs.AI

TL;DR: This paper introduces Generative-Evaluative Agreement (GEA), a validity criterion for LLM-based assessment, showing that when LLMs generate, simulate, and score assessments, the validation loop is self-referential. GEA measures whether an LLM's scoring recovers intended skill levels, and experiments show moderate recovery (r=0.698) with systematic bias, strong GEA for syntactic skills but near-zero for design skills, and low-skill overestimation near routing thresholds.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the problem of self-referential validation loops when the same LLM generates assessment items, simulates student responses, and scores them, lacking an independent validity criterion.

Method: The authors introduce Generative-Evaluative Agreement (GEA) as a validity criterion measuring how well an LLM's scoring function recovers the skill levels its generative function was instructed to produce, tested on a two-stage adaptive assessment.

Result: The model recovers roughly half the intended variance (r=0.698) with systematic positive bias. GEA is strong (r>0.7) for syntactically verifiable skills but near zero for design-level skills, with low-skill overestimation inflating scores near the routing threshold.

Conclusion: Granular, skill-decomposed rubrics are the principal proposed mechanism for strengthening GEA, with complementary mitigations outlined.
...
TL;DR: Self-evolving skill libraries suffer from "library drift" (unbounded skill accumulation causing performance degradation). The paper isolates the mechanism via ablations, provides trace-level diagnostics with evidence logs, and delivers a verified governance fix (outcome-driven retirement + bounded active-cap + meta-skill authoring prior) that boosts pass@1 from 0.258 to 0.584 on MBPP+ hard-100 over 100 rounds.


<details>
  <summary>Details</summary>
Motivation: Self-evolving skill libraries face a silent failure mode called library drift—unbounded skill accumulation without outcome-driven lifecycle management causes retrieval degradation, false-positive injections, and performance stagnation. While LLM-authored skills show +0.0pp gain (vs. human-curated +16.2pp), the underlying mechanism has not been isolated.

Method: (1) Reproducible trigger: ablations isolating drift (disabling skill injection: +0.002; premature retirement: −0.019); (2) Trace-level diagnostics: append-only evidence log with per-skill contribution scores, attribution verdicts, and router engagement metrics; (3) Verified fix: governance recipe (outcome-driven retirement + bounded active-cap + meta-skill authoring prior) with 8 ablations decomposing load-bearing mechanisms.

Result: The governance fix lifts held-out pass@1 from 0.258 baseline to a late-window mean of 0.584 (rolling gain +0.328) on MBPP+ hard-100 over 100 rounds.

Conclusion: Library drift is a diagnosable and fixable failure mode in self-evolving agents. The paper provides a concrete playbook with trace-level diagnostics and minimal governance mechanisms for detecting and preventing drift in any self-evolving agent system.
...
### [169] [SceneCode: Executable World Programs for Editable Indoor Scenes with Articulated Objects](https://arxiv.org/abs/2605.19587)
*Puyi Wang,Yuhao Wang,Linjie Li,Zhengyuan Yang,Kevin Qinghong Lin,Yangguang Li,Yu Cheng*

Main category: cs.AI

TL;DR: SceneCode converts natural language prompts into executable Python programs that generate fully articulated, simulation-ready indoor scenes with clean mesh structures and physics-compatible assets.


<details>
  <summary>Details</summary>
Motivation: Existing indoor scene synthesis pipelines produce static meshes with limited interactivity and only inherit articulation from curated asset libraries, preventing on-demand generation of controllable, interactable assets for embodied AI and robotics.

Method: SceneCode uses a room-level agentic backbone (planner-designer-critic loop) to convert prompts into house layouts and AssetRequests, then routes each request to one of five code-generation strategies, producing part-wise Blender Python programs validated via execution-guided repair-and-refine, finally exported as SDF for physics simulation.

Result: SceneCode outperforms baselines in prompt-faithful scene generation, produces assets with cleaner mesh structures and simulator-loadable articulation metadata, validated across scene-level synthesis, object-level quality, human judgment, and downstream robot interaction.

Conclusion: Programmatic world generation via executable code offers a traceable, locally editable alternative to opaque meshes, enabling physically interactable indoor scenes on demand for embodied AI and robotics applications.
...
### [170] [Towards Multi-Model LLM Schedulers: Empirical Insights into Offloading and Preemption](https://arxiv.org/abs/2605.19593)
*Mert Yildiz,Pietro Spadaccino,Alexey Rolich,Francesca Cuomo,Andrea Baiocchi*

Main category: cs.AI

TL;DR: This paper empirically studies multi-model LLM serving on heterogeneous hardware, showing that layer offloading causes non-linear, model-dependent decode degradation (smaller models more sensitive), preemption overhead is dominated by model state reload (not KV cache), and that sequence length/interconnect bandwidth amplify inefficiencies. It identifies key features for future schedulers.


<details>
  <summary>Details</summary>
Motivation: Modern LLM deployments increasingly require serving multiple diverse models on shared heterogeneous hardware, yet existing systems primarily optimize throughput for single models. There is little work addressing multi-model scheduling under GPU memory constraints with CPU-GPU offloading and preemption.

Method: Empirical study measuring how different LLMs behave across hardware platforms, focusing on performance implications of layer offloading and preemption, analyzing decode throughput degradation and preemption cost breakdown.

Result: (1) Offloading causes strongly non-linear, model-dependent decode degradation; smaller models show sharper sensitivity. (2) Preemption overhead is dominated by model state reload, not KV cache transfer; costs vary significantly across models and hardware. (3) Sequence length and interconnect bandwidth amplify data movement and execution inefficiencies.

Conclusion: Future schedulers must consider model-specific offloading sensitivity, workload characteristics, and the cost structure of preemption/data transfer. These insights guide design of next-gen LLM serving systems for heterogeneous multi-model workloads with hybrid CPU-GPU execution.
...
### [171] [Formal Skill: Programmable Runtime Skills for Efficient and Accurate LLM Agents](https://arxiv.org/abs/2605.19604)
*Xi Zhang,Meijun Gao,Yuntian Zhao,Xinyu Tan,Yilun Yao,Feiyu Wang,Yanshu Wang,Dingsiyi,Tong Yang*

Main category: cs.AI

TL;DR: Introduces Formal Skill, a runtime-native executable abstraction for LLM agents that uses state machines and hook policies instead of informal text, implemented in FairyClaw runtime which achieves competitive scores with fewer tokens.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agent skills remain informal (Markdown, instruction packs, function calls, MCP) lacking workflow state, policy enforcement, and completion discipline within the skill itself.

Method: Formal Skill abstraction: JSON metadata + action schemas, Python executors, hook-governed control logic, routing, and skill-local state. Implemented as FairyClaw event-driven runtime.

Result: On Harness-Bench, FairyClaw achieves highly competitive average scores with substantially fewer tokens, especially strong on tasks involving Formal Skill.

Conclusion: Moving reusable procedures from prompt text into executable state machines with hook policies provides token-efficient and enforceable control surfaces for LLM agents.
...
### [172] [EMO-BOOST: Emotion-Augmented Audio-Visual Features for Improved Generalization in Deepfake Detection](https://arxiv.org/abs/2605.19630)
*Aritra Marik,Marcel Klemt,Anna Rohrbach*

Main category: cs.AI

TL;DR: Emo-Boost enhances deepfake detection generalization by combining emotion-based (high-level) cues with traditional low-level detectors, improving cross-manipulation AUC by 2.1%.


<details>
  <summary>Details</summary>
Motivation: Deepfake detection struggles to generalize to unseen manipulation types, and collecting training data for every new generation technique is impractical.

Method: Proposes Emo-Boost, a multimodal framework fusing an RGB/acoustic deepfake detector with EmoForensics (emotion-based detector using vision & audio emotion recognition, modeling intra/inter-modal temporal consistency).

Result: Combining emotion signals with low-level methods improves cross-manipulation generalization AUC by 2.1% on FakeAVCeleb.

Conclusion: High-level semantic cues (emotions) complement low-level approaches, improving generalization to unseen deepfake manipulations.
...
### [173] [When Tabular Foundation Models Meet Strategic Tabular Data: A Prior Alignment Approach](https://arxiv.org/abs/2605.19662)
...
TL;DR: This paper studies whether tabular foundation models (PFNs) can handle strategic classification where individuals modify features after deployment. It finds a mismatch between non-strategic pretraining and strategic deployment, and proposes SPN, an inference-time framework that adapts models to strategic environments without retraining.


<details>
  <summary>Details</summary>
Motivation: Tabular foundation models (PFNs) are designed for non-strategic settings, but real-world decision scenarios often involve strategic feature manipulation by individuals seeking favorable outcomes, causing distribution shift.

Method: Proposes Strategic Prior-data Fitted Network (SPN), an inference-time framework that constructs strategic in-context examples to approximate post-manipulation inputs and aligns PFN predictions with the induced strategic distribution, without requiring retraining.

Result: Experiments on real-world and synthetic tabular datasets show SPN consistently improves robustness and predictive performance under strategic manipulation compared to both tabular foundation models and classical tabular methods.

Conclusion: Strategic manipulation creates a mismatch between non-strategic pretraining priors and post-manipulation strategic priors, causing systematic prediction bias; SPN effectively adapts tabular foundation models to strategic environments at inference time.
...
### [174] [Pseudocode-Guided Structured Reasoning for Automating Reliable Inference in Vision-Language Models](https://arxiv.org/abs/2605.19663)
*Weicong Ni,Tianbao Jiang,Linlin Wang*

Main category: cs.AI

TL;DR: PStar is a pseudocode-guided structured reasoning framework that adaptively selects reasoning paths to reduce hallucinations in Vision-Language Models, achieving state-of-the-art scores of 87.1% on POPE and 68.0% on MMStar, outperforming GPT-4V.


<details>
  <summary>Details</summary>
Motivation: VLMs suffer from hallucinations that cause critical failures in robotic decision-making, posing safety and reliability risks in real-world deployments where tasks vary widely in difficulty and modality.

Method: PStar designs abstract reasoning functions and a structured pseudocode library for modular reasoning, plus a Difficulty Feature Vector (DFV) that assesses question complexity to adaptively select appropriate reasoning strategies.

Result: Achieves 87.1% on POPE and 68.0% on MMStar, outperforming GPT-4V with significantly reduced hallucination rates.

Conclusion: PStar provides a validated mechanism to reduce visual-language errors, offering a critical step toward deploying more trustworthy and deterministic VLMs in real-world automated systems where errors can be catastrophic.
...
TL;DR: This paper automatically generates local search neighborhoods from constraint specifications by leveraging symmetry properties, evaluating the approach on six classical optimization problems with promising results.


<details>
  <summary>Details</summary>
Motivation: Local search algorithms typically require manual human intervention to compile constraints for metaheuristics, which is difficult and time-consuming.

Method: The authors establish a link between symmetry properties of constraint optimization problems and local search neighborhoods, using this link to automatically generate neighborhoods from constraint specifications within the IDP system.

Result: The automatically generated neighborhoods were evaluated on six classical optimization problems, and the observations support the viability of the technique.

Conclusion: The proposed approach successfully automates neighborhood generation for local search, reducing the need for human intervention in solving combinatorial optimization problems.
...
### [176] [Beyond Rational Illusion: Behaviorally Realistic Strategic Classification](https://arxiv.org/abs/2605.19674)
...
TL;DR: This paper introduces and addresses the behaviorally realistic strategic classification problem, where agents' strategic manipulations deviate from full rationality due to psychological biases. The authors propose Pro-SF, a framework grounded in prospect theory that incorporates asymmetry between benefits and costs, subjective reference points, and non-rational probability distortion.


<details>
  <summary>Details</summary>
Motivation: Existing strategic classification frameworks assume agents are strictly rational, but real-world decision-making is shaped by cognitive biases that deviate from pure rationality.

Method: The paper proposes Prospect-Guided Strategic Framework (Pro-SF), which reformulates Stackelberg-style interactions using three prospect theory mechanisms: asymmetry between benefits and costs, different subjective reference points, and non-rational probability distortion.

Result: Experiments on synthetic and real-world datasets demonstrate Pro-SF as a behaviorally grounded approach to strategic classification.

Conclusion: Pro-SF bridges machine learning and behavioral economics for more reliable real-world deployment of strategic classification systems.
...
### [177] [From Prompts to Pavement Through Time: Temporal Grounding in Agentic Scene-to-Plan Reasoning](https://arxiv.org/abs/2605.19824)
*Ahmed Y. Gado,Omar Y. Goba,Alaa Hassanein,Catherine M. Elias,Ahmed Hussein*

Main category: cs.AI

TL;DR: This paper investigates temporal conditioning in inter-agent communication for AV planning, finding that while temporal grounding doesn't improve standard NLP metrics, it enables qualitative benefits like predictive hazard reasoning and strategic divergence in the Sentinel architecture.


<details>
  <summary>Details</summary>
Motivation: Current LLM/LMM-based AV systems treat time as a secondary property, leading to inconsistencies in reasoning about continuous actions and undermining both safety and interpretability.

Method: Three planner architectures with progressively increasing temporal integration were introduced and evaluated on curated BDD-X subsets using semantic, syntactic, and logical metrics.

Result: Temporal conditioning reshapes reasoning style but yields no statistically significant improvements in standard NLP-based correctness metrics. Qualitative analysis reveals predictive hazard reasoning, stable corrective behavior, and strategic divergence in the Sentinel.

Conclusion: The findings clarify the limits of prompt-based temporal grounding and establish the first empirical benchmark for temporal scene-to-plan reasoning.
...
### [178] [Projecting Latent RL Actions: Towards Generalizable and Scalable Graph Combinatorial Optimization](https://arxiv.org/abs/2605.19721)
*Franco Terranova,Guillermo Bernardez,Albert Cabellos-Aparicio,Nina Miolane,Abdelkader Lahmadi*

Main category: cs.AI

TL;DR: A novel RL-GCO approach (projection agents) operating in continuous GNN-based action embedding space, achieving 16.2x faster inference and 40% better generalization with a new Python library (LaGCO-RL).


<details>
  <summary>Details</summary>
Motivation: Existing RL-GCO solvers face limitations in generalization across diverse graph instances and computational scalability as action spaces grow.

Method: Introduces projection agents that predict latent actions in a continuous GNN-based embedding space via a single forward pass, then decode into discrete actions; enables fair comparison via shared embedding space.

Result: Up to 16.2x faster inference, up to 40% better generalization using simple nearest-neighbor decoding; strong performance in super-linear decision spaces.

Conclusion: Projection agents effectively address scalability and generalization in RL-GCO; LaGCO-RL library promotes reproducibility and adaptation.
...
### [179] [EngiAI: A Multi-Agent Framework and Benchmark Suite for LLM-Driven Engineering Design](https://arxiv.org/abs/2605.19743)
*Gioele Molinari,Florian Felten,Soheyl Massoudi,Mark Fuge*

Main category: cs.AI

TL;DR: A benchmark suite and reference MAS (EngiAI) for evaluating LLM agents on engineering design tasks, testing workflow, RAG, and HPC dimensions across multiple models.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation frameworks do not adequately address multi-agent systems combining simulation, retrieval, and manufacturing preparation in engineering design.

Method: Three benchmarks: (1) workflow benchmark with 7 prompt styles, (2) RAG benchmark with gated scoring, (3) HPC benchmark for ML training orchestration. EngiAI MAS with 7 specialized agents in a supervisor architecture on LangGraph.

Result: Proprietary models achieve 96-97% on Beams2D; open-source 4B models reach 55-78%. Conditional branching hardest (20-53%). RAG gating confirms near-perfect retrieval scores (~1.0) vs near-zero without. HPC success ranges from 50% to 100%.

Conclusion: The benchmark effectively reveals model capabilities and weaknesses across multi-agent engineering tasks; conditional branching and long-horizon instruction following remain challenging.
...
TL;DR: This paper reframes AI guardrails from static output filtering to runtime behavioral control over interaction trajectories, drawing on robotics. The Grounded Observer framework is introduced and tested across three real-world deployments (small talk, autism therapy, school de-escalation), showing it can mitigate drift into undesirable interaction regimes while adapting to social context.


<details>
  <summary>Details</summary>
Motivation: Foundation models deployed in sensitive domains (education, mental health, caregiving) face cumulative and context-dependent failures. Existing guardrails (training-time alignment, prompting, decoding constraints, post-hoc moderation) provide only empirical risk reduction rather than enforceable behavioral guarantees, and treat safety as per-output rather than per-trajectory.

Method: The authors reframe guardrails as runtime behavioral control over interaction trajectories, borrowing formal constructs from robotics for constraint enforcement in uncertain, closed-loop systems. They instantiate this in the Grounded Observer framework and evaluate across three deployments: small talk, in-home autism therapy, and behavioral de-escalation in schools.

Result: Across all three settings, the Grounded Observer framework enables runtime interventions that mitigate drift into undesirable interaction regimes while adapting to diverse social contexts.

Conclusion: The paper discusses extensions to the framework and proposes research directions toward achieving stronger behavioral guarantees for foundation model deployments.
...
### [181] [Memory-Augmented Reinforcement Learning Agent for CAD Generation](https://arxiv.org/abs/2605.19748)
*Yin Xiaolong,Liu Yu,Shen Jiahang,Lu Xingyu,Ni Jingzhe,Fan Fengxiao,Sang Fan*

Main category: cs.AI

TL;DR: This paper proposes a memory-augmented reinforcement learning framework for CAD generation agents that uses a closed-loop mechanism with dual-track memory (case library + skill library) and dynamic utility retrieval to improve complex CAD model generation success rate and geometric consistency.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based CAD generation methods struggle with complex models having long operation sequences, diverse operation types, and strong geometric constraints due to reasoning chain breaks and lack of error-correction mechanisms.

Method: A memory-augmented RL framework encapsulating a geometric kernel into a toolchain, with a closed-loop mechanism (design understanding, planning, execution, verification), dual-track memory (case library + skill library), dynamic utility retrieval, and RL-based retrieval & policy optimization for self-correction without large annotated data.

Result: The proposed method significantly improves success rate and geometric consistency on complex CAD model generation tasks.

Conclusion: The framework enables online self-correction and continual evolution, effectively avoiding retrieval traps where examples are semantically similar but geometrically infeasible.
...
### [182] [CogScale: Scalable Benchmark for Sequence Processing](https://arxiv.org/abs/2605.19758)
*Yannis Bendi-Ouis,Romain de Coudenhove,Xavier Hinaut*

Main category: cs.AI

TL;DR: CogScale is a benchmark of 14 scalable synthetic tasks for evaluating cognitive and memory abilities in sequence-processing architectures. Tests on 7 architectures (GRU, LSTM, xLSTM, ESN, Mamba, Transformer Decoder, Transformer Encoder-Decoder) under strict parameter budgets show classical RNNs/ESNs excel at basic retention, but only attention mechanisms and modern state-space models maintain performance as complexity scales.


<details>
  <summary>Details</summary>
Motivation: Evaluating novel architectures for sequential processing is computationally expensive and slow, requiring massive datasets and models. A lightweight, standardized benchmark is needed to rapidly validate architectural innovations before committing to large-scale training.

Method: CogScale: 14 scalable synthetic tasks isolating specific cognitive/memory abilities at parametrizable scales. Seven architectures (GRU, LSTM, xLSTM, ESN, Mamba, Transformer Decoder, Transformer Encoder-Decoder) evaluated under parameter budgets of 1k, 10k, and 100k across varying difficulty levels.

Result: Classical RNNs (GRU, LSTM) and Echo State Networks perform well on basic retention under tight parameter budgets. However, only attention mechanisms (Transformers) and modern state-space models (Mamba) maintain high performance as task reasoning complexity and difficulty increase.

Conclusion: CogScale provides a lightweight, standardized benchmark for rapid validation of sequential-processing architectures. The results highlight a scalability trade-off: simple recurrent models suffice for basic tasks, but advanced architectures are required for complex reasoning.
...
TL;DR: LLM-based robotic agents perform better under noisy, raw RGB input than with perfect ground-truth observations in embodied tasks, with moderate noise boosting success by 2.85x by reducing repetitive action loops.


<details>
  <summary>Details</summary>
Motivation: LLMs are used as cognitive components in robotics, but their opaque decision-making makes it hard to explain success/failure in closed-loop embodied tasks.

Method: Behavioral study varying observation types (RGB, RGB-D, symbolic) for LLM agents on the Lockbox puzzle in physical and simulated settings; probed by flipping perceived action outcomes at varying noise levels.

Result: Agents perform best under raw RGB, worst under perfect ground truth. Moderate noise (40% flip) yields 2.85x success increase over noise-free baseline due to reduced repetitive action loops.

Conclusion: Success rates alone are insufficient for evaluating LLM agents—measured performance may reflect interactions between perceptual errors and reasoning failures, not robust problem-solving.
...
TL;DR: Controlled pretraining experiments on 10T tokens show code improves programming but not general reasoning; reasoning gains come from structured cross-domain traces (code-text, math-text) rather than executable code alone, and densifying math-domain samples within a fixed budget boosts difficult math reasoning while preserving programming performance.


<details>
  <summary>Details</summary>
Motivation: To clarify the ambiguous role of code in improving reasoning abilities of language models, by disentangling the effects of executable code vs. structured reasoning traces through controlled pretraining.

Method: Controlled pretraining experiments on a 10T-token corpus with fine-grained domain separation, comparing effects of executable code, code-text mixtures, math-text mixtures, and varying densities of math-domain samples within a fixed budget.

Result: Code improves programming but competes with knowledge-intensive tasks; reasoning gains are driven by cross-domain structured reasoning traces (code-text, math-text) not executable code alone; densifying structured math samples improves difficult math reasoning while preserving programming performance.

Conclusion: Data-composition effects are reflected in expert-activation patterns; cognitive scaffolds (structured reasoning traces) offer targeted mitigation of cross-domain trade-offs, pointing to more precise data-centric optimization strategies.
...
TL;DR: GroupAffect-4 is a multimodal corpus of 40 participants in 10 four-person groups, capturing physiology, eye movement, audio, self-report, task outcomes, and personality for studying affect in co-located groups across individual, interpersonal, and group levels.


<details>
  <summary>Details</summary>
Motivation: Existing corpora rarely support analysis of affect in co-located groups as a coupled individual, interpersonal, and group-level process, with required signals fragmented across separate dataset traditions.

Method: 40 participants in 10 four-person groups completed four collaborative tasks (information pooling, negotiation, idea generation, public-goods game), instrumented with wrist-worn physiology sensors, eye-tracking glasses, and close-talk microphones, with continuous affect self-reports and personality measures time-aligned to a shared clock.

Result: Dataset covers >91% physiology windows and >98% eye-tracking windows; strong task validity confirmed via affective manipulation check. Fifteen benchmarkable targets defined across within-person state, between-person traits, and group dynamics with leave-one-group-out baselines.

Conclusion: GroupAffect-4 is publicly released with BIDS-inspired structure, Croissant metadata, datasheet, quality reports, and open processing scripts at GitHub and Zenodo.
...
TL;DR: This paper studies reinforcement learning for MNL mixture MDPs and proposes an algorithm achieving regret $\tilde{O}(dH^2\bar\sigma_T\sqrt{T})$, which is minimax optimal (up to log factors) and improves upon prior bounds for structured MDPs, e.g., reducing horizon dependence by a factor $H$ for robust MDPs.


<details>
  <summary>Details</summary>
Motivation: Prior work (Li et al., 2024) gave $\tilde{O}(dH^2\sqrt{T})$ regret for MNL mixture MDPs, which may be loose for structured MDPs. The authors aim to characterize the exact regret complexity by introducing a problem-dependent variance constant $\bar\sigma_T$.

Method: Leveraging insights from logistic bandits (Abeille et al., 2021; Faury et al., 2022; Boudart et al., 2026), the paper introduces a normalized average variance parameter $\bar\sigma_T \leq 1/2$ measuring the variance of the optimal downstream value function. An algorithm is proposed that attains regret $\tilde{O}(dH^2\bar\sigma_T\sqrt{T})$.

Result: The proposed algorithm achieves regret $\tilde{O}(dH^2\bar\sigma_T\sqrt{T})$, recovering the worst-case bound and improving for structured MDPs (e.g., $\bar\sigma_T = O(H^{-1})$ for robust MDPs). A matching $\Omega(dH^2\bar\sigma_T\sqrt{T})$ lower bound is also established, proving minimax optimality.

Conclusion: This work fully characterizes the regret complexity of MNL mixture MDPs for the first time, demonstrating that the problem-dependent variance $\bar\sigma_T$ is the key quantity governing regret, with tight upper and lower bounds.
...
TL;DR: OpenComputer is a verifier-grounded framework that creates verifiable software environments for computer-use agents, covering 33 applications and 1,000 tasks, with hard-coded verifiers outperforming LLM-based evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for computer-use agents lack reliable, fine-grained verification of task completion, relying on error-prone LLM-as-judge evaluation that fails to capture nuanced application states.

Method: The framework integrates four components: app-specific state verifiers, a self-evolving verification layer using execution feedback, a task-generation pipeline for realistic desktop tasks, and an evaluation harness with partial-credit rewards.

Result: Hard-coded verifiers align more closely with human judgment than LLM-as-judge. Frontier agents show limited end-to-end success, and open-source models significantly underperform compared to their OSWorld-Verified scores.

Conclusion: Robust computer automation remains challenging; reliable verification mechanisms are critical for meaningful progress evaluation in agentic systems.
...
TL;DR: This paper adapts conformal prediction methods (split conformal and ACI) for continuous AI agent evaluation, providing distribution-free coverage guarantees for quality forecasts, with extensions to multi-agent pipelines, pairwise rankings, and leaderboard testing.


<details>
  <summary>Details</summary>
Motivation: To provide reliable, distribution-free uncertainty quantification for real-time AI agent quality evaluation, addressing the need for calibrated confidence intervals in continuous monitoring settings where agent updates cause distribution shifts.

Method: Adapts split conformal prediction and ACI for AI agent evaluation; develops compositional uncertainty bounds for multi-agent pipelines (validated across rho in [-0.5, 0.9]), a conformal abstention rule for pairwise rankings with FDR correction, and evaluates on 50 agents using 18 hourly signals.

Result: Calibration error below 0.02 at 24h horizon; ACI widens intervals by 35% after agent releases then reconverges; per-agent conditional coverage concentrated around 80.4% nominal level (90% agents within [72%, 90%]); cross-source sentiment divergence predicts ranking instability (r=0.64, p<0.01).

Conclusion: The proposed conformal framework effectively captures signal beyond benchmarks (rho_s=0.52, p<0.01) and provides rigorous uncertainty quantification for continuous AI agent evaluation.
...
TL;DR: The paper proposes an efficient data-driven method to dynamically select optimal update geometries (LMO) for neural network optimizers, achieving competitive performance with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Current optimizers use fixed, hand-chosen LMO geometries for updates, which may not be optimal for the specific problem geometry.

Method: Derives a closed-form criterion from gradient and activation statistics using a single-step random feature regression surrogate model, enabling dynamic selection of LMO geometries interpolating from SGD to Muon.

Result: Achieves only ~3% runtime overhead and beats or matches the best of Muon and AdamW across three training scenarios.

Conclusion: LMO geometry can be successfully and efficiently adapted from runtime data, opening new pathways for optimizer design beyond static geometries.
...
TL;DR: LLMs in code optimization tasks act as greedy optimizers relying on pretrained priors rather than feedback or agentic structure; they ignore input-size instructions, performance degrades for uncommon sizes, and optimization quality depends on language density (CUDA good, TVM IR poor).


<details>
  <summary>Details</summary>
Motivation: To understand which components contribute most in increasingly complex LLM-based optimization agents and identify when and how their exploration may fail.

Method: Three controlled experiments: (1) pure black-box optimization, (2) zero-shot kernel generation with varying input-size info, (3) feedback-loop kernel optimization comparing CUDA vs TVM IR.

Result: (1) LLMs behave as greedy optimizers. (2) Input-size information is invisible to models; performance degrades sharply for uncommon kernel sizes. (3) CUDA improves monotonically under feedback while TVM IR degrades.

Conclusion: LLM code optimization success depends heavily on pretrained priors, not on provided feedback or agentic structure; optimization degrades with low-density programming languages.
...
### [191] [Explainable Wastewater Digital Twins: Adaptive Context-Conditioned Structured Simulators with Self-Falsifying Decision Support](https://arxiv.org/abs/2605.19826)
*Gary Simethy,Daniel Ortiz Arroyo,Petar Durdevic*

Main category: cs.AI

TL;DR: The paper presents CCSS-IX, an explainable digital twin for wastewater treatment plants that uses interpretable locally linear state-space models with conformal risk control to provide certified safety guarantees for operator-proposed aeration and dosing actions, achieving high accuracy while reducing unsafe decisions by 43.6% and preventing 93 false-safe N2O approvals.


<details>
  <summary>Details</summary>
Motivation: Operators of safety-critical industrial processes (e.g., wastewater treatment) lack certified safety guarantees in digital twins, facing a daily safety-efficiency trade-off where improper aeration leads to either effluent violations/N2O spikes or energy waste.

Method: A bank of interpretable locally linear state-space "experts" adaptively mixed by a context-aware gating network (continuous-time regime-switching scaffold), with a runtime conformal risk control layer that abstains, reopens, or returns falsifying temporal witnesses for actions lacking statistical certification.

Result: Static ensemble: 0.78% RMSE vs black-box; adaptive: 1.08% RMSE. Calibrated reopen rule cuts aggregate regret by 43.6% at unsafe-action cost weight 4. Event-aligned temporal witnesses prevent 93/187 false-safe N2O approvals (~4.65x dyadic baseline, p<1e-21).

Conclusion: The framework provides a validated, end-to-end decision-support pipeline with finite-sample coverage guarantees, tested on full-scale plants (Avedøre, Agtrup/BlueKolding) and BSM2 benchmark, bridging the gap between digital twin usage and certified safety in industrial processes.
...
TL;DR: Proposes a CNN + LLM pipeline to automatically synthesize streamliner constraints from enumerated solutions, achieving dramatic speedups on hardened constraint programming benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing automated streamliner synthesis approaches rely solely on constraint grammars or direct LLM prompting on the problem model, missing the structural patterns present in actual solutions.

Method: Enumerate feasible solutions, train a CNN contrastively against perturbed non-solutions, then translate the CNN's discriminative signal into MiniZinc streamliners via LLM-driven synthesis.

Result: 98.8% portfolio time reduction on Vessel Loading, 98.6% on Social Golfers, 89.4% on Black Hole; geometric-mean speedups of 932x, 356x, 1103x respectively.

Conclusion: Grounding LLM constraint generation in observed solution structure via CNNs is more effective than model-text-only approaches for automated streamliner discovery in constraint programming.
...
TL;DR: PEEK introduces a context map mechanism that caches reusable orientation knowledge about recurring external contexts, enabling LLM agents to work more accurately and efficiently on repeated long-context tasks.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agent approaches preserve trajectories, raw material, or task strategies when operating over recurring external contexts, but none maintains reusable orientation knowledge (what the context contains, its organization, historically useful entities/schemas) that is most needed for repeated same-context workloads.

Method: PEEK caches orientation knowledge as a small, constant-sized context map in the agent's prompt, maintained by a programmable cache policy with three modules: Distiller (extracts transferable knowledge from inference-time signals), Cartographer (translates knowledge into structured edits), and Evictor (priority-based, enforces fixed token budget).

Result: On long-context reasoning tasks, PEEK improves by 6.3-34.0% over baselines, uses 93-145 fewer iterations, and incurs 1.7-5.8x lower cost than ACE. On context learning, it improves solving rate by 6.0-14.0% and rubric accuracy by 7.8-12.1% at 1.4x lower cost than ACE. Gains generalize across LMs and agent architectures including OpenAI Codex.

Conclusion: A context map helps long-context LLM agents interact with recurring external contexts more accurately and efficiently, with consistent improvements across diverse models and agent types.
...
### [194] [Probabilistic Tiny Recursive Model](https://arxiv.org/abs/2605.19943)
*Amin Sghaier,Ali Parviz,Alexia Jolicoeur-Martineau*

Main category: cs.AI

TL;DR: PTRM adds Gaussian noise to TRM's recursive steps for stochastic exploration, achieving major accuracy gains on reasoning tasks at tiny parameter counts.


<details>
  <summary>Details</summary>
Motivation: TRM's deterministic recursion converges to suboptimal solutions without escape mechanisms; existing workarounds require task-specific perturbations.

Method: PTRM injects Gaussian noise at each recursion step, runs parallel trajectories, and selects best solutions using the model's Q head — all task-agnostic and without retraining.

Result: Sudoku-Extreme: 87.4% → 98.75%; Pencil Puzzle Bench: 62.6% → 91.2% (vs. 55.1% for frontier LLMs at <0.0001x cost, using only 7M params).

Conclusion: Stochastic exploration at test-time scaling effectively overcomes TRM's convergence limitations, yielding substantial gains without task-specific engineering.
...
### [195] [GeoX: Mastering Geospatial Reasoning Through Self-Play and Verifiable Rewards](https://arxiv.org/abs/2605.20006)
*Kyeongjin Ahn,Seungeon Lee,Krishna P. Gummadi,Meeyoung Cha*

Main category: cs.AI

TL;DR: GeoX is a self-play framework that improves geospatial reasoning in VLMs by generating executable programs for spatial problems and using verifiable rewards via reinforcement learning, achieving up to 5.5 points improvement without human-curated data.


<details>
  <summary>Details</summary>
Motivation: Geospatial reasoning is hindered by the high cost of annotating large-scale, combinatorial question spaces for complex spatial scenes.

Method: GeoX uses self-play with a multimodal policy that proposes spatial problems as executable programs and solves them under abduction, deduction, and induction reasoning modes. A verifier executes programs to produce reward signals for reinforcement learning.

Result: GeoX improves base VLMs by up to 5.5 points on average, matching or exceeding baselines trained on millions of curated data points.

Conclusion: Self-play with verifiable programs can effectively teach spatial logic to VLMs without large-scale human annotation, and the authors release a benchmark for geospatial understanding.
...
### [196] [When Skills Don't Help: A Negative Result on Procedural Knowledge for Tool-Grounded Agents in Offensive Cybersecurity](https://arxiv.org/abs/2605.20023)
*Samuel Jacob Chacko,James Hugglestone,Chashi Mahiul Islam,Xiuwen Liu*

Main category: cs.AI

TL;DR: This paper re-analyzes a CTF agent study and finds that when environment-feedback bandwidth is high, the marginal benefit of Agent Skills collapses (only 8.9 pp improvement, statistically insignificant), and can even degrade performance.


<details>
  <summary>Details</summary>
Motivation: Despite Skills improving task pass rates by ~16.2 pp on average, 16 of 84 tasks suffer negative deltas. The community lacks a mechanism for when Skills help vs. when they are redundant overhead.

Method: Re-analyzed a 180-run controlled CTF study with four documentation conditions (55, 1,478, 1,976, 4,147 lines) mapped to No-Skills, Experiential, Curated, and Comprehensive Skills ablations; used statistical tests (χ², Cochran-Armitage, Cohen's h).

Result: Spread of only 8.9 pp (p=0.71 χ²; p=0.25 trend test); five of six Cohen's h below 0.2 small-effect threshold. In timing side-channel settings, Skills actively degraded performance.

Conclusion: Environment-feedback bandwidth is the missing variable: strict, schema-validated, low-latency tool observations replace the procedural correction signal Skills normally provide. Articulates falsifiable hypothesis and design implications.
...
### [197] [AutoResearchClaw: Self-Reinforcing Autonomous Research with Human-AI Collaboration](https://arxiv.org/abs/2605.20025)
...
TL;DR: AutoResearchClaw is a multi-agent autonomous research system with five key mechanisms (structured debate, self-healing executor, verifiable reporting, human-in-the-loop, cross-run evolution) that outperforms AI Scientist v2 by 54.7% on ARC-Bench, advocating for human-augmented rather than fully automated research.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous research systems are flawed because they model research as a linear pipeline using single-agent reasoning, stop at execution failures, and fail to carry experience across runs, unlike the iterative, failure-informed nature of real scientific discovery.

Method: Five mechanisms: (1) structured multi-agent debate for hypothesis generation and result analysis; (2) self-healing executor with Pivot/Refine decision loop; (3) verifiable result reporting to prevent fabrication; (4) human-in-the-loop with seven intervention modes; (5) cross-run evolution converting past mistakes into safeguards.

Result: On ARC-Bench (25 topics), AutoResearchClaw outperforms AI Scientist v2 by 54.7%. Human-in-the-loop ablation shows targeted collaboration at high-leverage points outperforms full autonomy and exhaustive oversight.

Conclusion: AutoResearchClaw functions as a "research amplifier" that augments rather than replaces human scientific judgment, with targeted intervention at key decision points being the optimal collaboration mode.
...
### [198] [Neurosymbolic Learning for Inference-Time Argumentation](https://arxiv.org/abs/2605.20098)
*Gabriel Freedman,Adam Dejl,Adam Gould,Mansi,Lihu Chen,Jianqi Jiang,Francesca Toni*

Main category: cs.AI

TL;DR: This paper introduces inference-time argumentation (ITA), a trainable neurosymbolic framework for ternary claim verification (true/false/uncertain) that uses formal argumentation semantics to guide LLM training and compute faithful predictions from generated arguments and scores.


<details>
  <summary>Details</summary>
Motivation: Claim verification in high-stakes settings (health, finance) requires handling incomplete/conflicting information with uncertain answers and faithful explanations, which binary classification and post-hoc reasoning traces cannot adequately provide.

Method: ITA integrates formal argumentation semantics into LLM training: (i) argument generation and base score assignment are optimized via argumentative prediction quality, and (ii) ternary verdicts are computed deterministically from scored arguments using argumentation semantics, ensuring faithfulness by construction.

Result: On two ternary claim verification datasets, ITA outperforms argumentative baselines and achieves competitive performance against non-argumentative direct-prediction baselines.

Conclusion: ITA enables faithful, inspectable, and deterministic verdict computation from explicit argument structures, offering a principled alternative to black-box or post-hoc reasoning approaches for claim verification under uncertainty.
...
### [199] [Using Aristotle API for AI-Assisted Theorem Proving in Lean 4: A Formalisation Case Study of the Grasshopper Problem](https://arxiv.org/abs/2605.20120)
*Gabriel Rongyang Lau*

Main category: cs.AI

TL;DR: A case study of AI-assisted Lean 4 formalization for the Grasshopper problem (IMO 2009 P6) shows four helper lemmas verified but the main theorem unproven, highlighting the gap between local proof search success and unresolved global combinatorial reasoning.


<details>
  <summary>Details</summary>
Motivation: To investigate the evidential status of AI-generated theorem proving artifacts by examining which parts are actually verified vs. left unresolved.

Method: Lean 4 formalization case study of an Aristotle API proof attempt for IMO 2009 Problem 6, analyzing verified helper lemmas vs. the remaining sorry in the main theorem.

Result: Four lemmas verified (final partial sum equals total sum; adjacent transposition affects only relevant partial sum; changed partial sum has expected form; maximality plus adjacent swap forces forbidden-set membership). Main theorem grasshopper closed by unresolved sorry.

Conclusion: Local proof search can succeed while global combinatorial bookkeeping remains unresolved, a central limitation in AI-assisted formalization.
...
### [200] [Not Every Rubric Teaches Equally: Policy-Aware Rubric Rewards for RLVR](https://arxiv.org/abs/2605.20164)
*Utkarsh Tyagi,Xingang Guo,MohammadHossein Rezaei,Daniel George,Anas Mahmoud,Jackson Lee,Bing Liu,Yunzhong He*

Main category: cs.AI

TL;DR: POW3R improves rubric-based RL rewards by dynamically adjusting criterion weights during training to focus on criteria that currently distinguish policy outputs, rather than using static human-assigned weights. This achieves better performance and 2.5-4x faster training across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Standard rubric-based rewards use static aggregations of criterion scores, but this conflates human-assigned importance with usefulness as an optimization signal. Many important criteria are saturated or unreachable, while distinguishing criteria aren't necessarily those with largest human weights.

Method: POW3R (Policy-aware rubric reward framework) preserves human weights and category balance as the rubric objective while dynamically adapting criterion-level reward weights during training using rollout-level contrast to emphasize criteria that currently separate the policy's outputs.

Result: Across 3 base policies on 2 datasets (multimodal and text-only), POW3R wins 24 of 30 base-policy/metric comparisons, improves both mean rubric reward and strict completion rate, and reaches the same plateau in 2.5-4x fewer training steps than vanilla GRPO with rubric rewards.

Conclusion: Rubric rewards should distinguish what should matter in the final answer from what can teach the current policy, making the reward signal more informative without changing the underlying evaluation target.
...
### [201] [HaorFloodAlert: Deseasonalized ML Ensemble for 72-Hour Flood Prediction in Bangladesh Haor Wetlands](https://arxiv.org/abs/2605.20167)
*Salma Hoque Talukdar Koli,Fahima Haque Talukder Jely,Md. Samiul Alim,Md. Zakir Hossen*

Main category: cs.AI

TL;DR: HaorFloodAlert is a deseasonalized ML ensemble for 72-hour flood forecasting in Bangladesh's Sunamganj Haor, achieving 89.6% accuracy and 0.943 AUC-ROC by correcting temperature bias and integrating upstream SAR data.


<details>
  <summary>Details</summary>
Motivation: Flash floods in Bangladesh's haor wetlands arrive with little warning and destroy the annual boro rice harvest. Existing riverine flood models fail because they ignore backwater dynamics in these flat basins, necessitating a specialized forecasting system.

Method: A deseasonalized ML ensemble (RF 0.5625 + XGBoost 0.4375) using temperature bias correction and an upstream Barak River Sentinel-1 SAR proxy (Otsu-thresholded change detection) for 72-hour flood probability forecasting.

Result: 89.6% LOOCV accuracy, 87.5% recall, 0.943 AUC-ROC on 77 Sentinel-1 events; SAR validation at 84-91% spatial match; temperature bias inflated accuracy by 6.9 pp.

Conclusion: The system provides operational flood alerts with a three-tier pipeline and a calibrated boro rice damage estimator, addressing the unique backwater-driven flood dynamics of haor wetlands.
...
### [202] [A Methodology for Selecting and Composing Runtime Architecture Patterns for Production LLM Agents](https://arxiv.org/abs/2605.20173)
*Vasundra Srinivasan*

Main category: cs.AI

TL;DR: The paper introduces the stochastic-deterministic boundary (SDB) as a first-class architectural primitive for production LLM agents, presents six runtime patterns, a selection methodology, diagnosis procedures, and identifies replay divergence as a failure mode.


<details>
  <summary>Details</summary>
Motivation: Production LLM agents combine stochastic model outputs with deterministic software systems, yet the boundary between the two is rarely treated as a first-class architectural object.

Method: Names the stochastic-deterministic boundary (SDB) as a four-part contract among proposer, verifier, commit step, and reject signal; organizes runtime design into Coordination, State, and Control; catalogs six runtime patterns; provides a five-step pattern selection methodology and a diagnostic procedure for production failures.

Result: Six runtime patterns: hierarchical delegation, scatter-gather plus saga, event-driven sequencing, shared state machine, supervisor plus gate, and human in the loop. Identifies replay divergence failure mode. Shows that as model variance decreases, pattern choice and SDB strength become increasingly important for long-run reliability.

Conclusion: The SDB is a load-bearing primitive; reducing model variance shifts reliability emphasis to architectural pattern selection. Method applied to five workloads with a reference implementation.
...
### [203] [KG-ASG: Collision-Knowledge-Guided Closed-Loop Adversarial Scenario Generation With Primary-Support Attribution](https://arxiv.org/abs/2605.18895)
*Cheng Wang,Chen Xiong,Ziwen Wang,Yuchen Zhou,Qiang Liu*

Main category: cs.RO

TL;DR: KG-ASG is a collision-knowledge-guided framework for generating safety-critical, executable, and interpretable adversarial driving scenarios with single-collider attribution and multi-vehicle coordination.


<details>
  <summary>Details</summary>
Motivation: Existing scenario generation methods produce ambiguous collision causes, uncontrolled multi-vehicle collisions, or non-executable trajectories, lacking clear semantics and attributable interactions.

Method: Constructs a collision knowledge base + lightweight Collision Expert for mode/primary/support attribution; formulates primary-support adversarial generation with rule, physical, interaction-safety, and single-collider hard constraints; uses planner-controller feedback for refinement.

Result: Outperforms baselines in adversarial effectiveness, Valid Primary Attack rate, reduced multi-collision, and closed-loop recovery gains across IDM, Cruise, and Expert controllers on WOMD scenarios in MetaDrive.
...
### [204] [Geo-Data-Driven HD Map Generation Workflow with Integrated Reference-Free Constraint-Based Verification](https://arxiv.org/abs/2605.18921)
*Ruidi He,Vaibhav Tiwari,Mohanad Al-Ghobari,Meng Zhang,Andreas Rausch*

Main category: cs.RO

TL;DR: The paper presents a workflow for generating HD maps from openly available geo-engineering data, with integrated constraint-based verification to detect inconsistencies without needing external reference maps. Evaluated on real-world data from four German cities, it successfully detects defects with no false positives.


<details>
  <summary>Details</summary>
Motivation: HD map generation currently relies on expensive sensor-intensive mobile mapping and high-precision reference data, making it costly and difficult to apply where specialized measurement data or independent reference maps are unavailable.

Method: The workflow uses openly available geo-engineering datasets (shapefile-based road-network data) as primary input, transforms them into lane-level HD map representations via explicit intermediate representations and processing stages, and integrates executable constraint-based verification derived from automated driving specifications and road-design guidelines to detect geometric, topological, and elevation-related inconsistencies without external reference maps.

Result: Real-world evaluation on four cities in Lower Saxony shows generated maps satisfy selected constraints; defect-injection study demonstrates complete detection of all considered defect types with zero false positives.

Conclusion: Geo-data-driven HD map generation with integrated executable verification offers a modular, inspectable complement to sensor-intensive mapping workflows when sensing and reference-data availability are limited.
...
### [205] [Adversarial Stress Testing of SPARK Humanoid Safety Filters](https://arxiv.org/abs/2605.19009)
*Saurav Ghosh,Abdou Sow,Luke Zhang*

Main category: cs.RO

TL;DR: This paper replicates and stress-tests SPARK humanoid safety filters (RSSA, RSSS, SSA, CBF, PFM, SMA) in MuJoCo, finding that safety behaviors change under crowded, noisy, and delayed conditions, suggesting evaluation beyond nominal performance is necessary.


<details>
  <summary>Details</summary>
Motivation: Humanoid robots are difficult to deploy safely due to high-dimensional bodies and many collision constraints near people and obstacles; nominal benchmark scores do not fully reveal how safety filters behave in harder environments.

Method: Replicate SPARK benchmark G1SportMode_D1_WG_SO_v1 in MuJoCo; evaluate RSSA, RSSS, SSA, CBF, PFM, SMA under controlled random seeds; build post-processing pipeline converting raw logs into goal-tracking, minimum-distance, and collision-step metrics; conduct stress tests with obstacle crowding, noisy distance estimates, and delayed obstacle information.

Result: Some methods track the goal more closely while others reduce collision steps more effectively; stress tests reveal safety behavior changes under obstacle crowding, noisy distance estimates, and delayed obstacle information.

Conclusion: Humanoid autonomy should be evaluated beyond nominal performance, using metrics that expose failure modes before deployment.
...
### [206] [Distributionally Robust Control via Stein Variational Inference for Contact-Rich Manipulation](https://arxiv.org/abs/2605.19029)
*Hrishikesh Sathyanarayan,Victor Vantilborgh,Harish Ravichandar,Tom Lefebvre,Ian Abraham*

Main category: cs.RO

TL;DR: A distributionally robust control method using Stein variational inference improves model-based manipulation by explicitly modeling task-sensitive uncertainty, achieving up to 3× better robustness in contact-rich tasks.


<details>
  <summary>Details</summary>
Motivation: Data-driven methods degrade with limited samples; classical model-based controllers are efficient but poor at representing task-relevant uncertainty in contact-rich interactions.

Method: Casts manipulation as a distributionally robust control optimization with a deterministic formulation based on Stein variational inference to model task-sensitive parameter uncertainty.

Result: Up to 3× improved robustness across contact-rich manipulation tasks under broad parametric uncertainty, outperforming existing model-based control methods.

Conclusion: The approach expands model-based manipulation control with flexible uncertainty modeling that retains performance while exactly adapting to uncertainty.
...
### [207] [RLFTSim: Realistic and Controllable Multi-Agent Traffic Simulation via Reinforcement Learning Fine-Tuning](https://arxiv.org/abs/2605.19033)
*Ehsan Ahmadi,Hunter Schofield,Behzad Khamidehi,Fazel Arasteh,Jinjun Shan,Lili Mou,Dongfeng Bai,Kasra Rezaee*

Main category: cs.RO

TL;DR: RLFTSim uses reinforcement learning fine-tuning to improve traffic simulation realism and controllability by aligning simulator outputs with real-world data, achieving state-of-the-art performance with fewer samples than heuristic methods.


<details>
  <summary>Details</summary>
Motivation: Supervised open-loop training fails to capture dynamic multi-agent interactions in complex driving scenarios, leading to unrealistic traffic simulations.

Method: A reinforcement-learning-based fine-tuning framework (RLFTSim) that fine-tunes pre-trained simulation models using a reward balancing fidelity and controllability, with a low-variance dense reward signal.

Result: Achieves state-of-the-art realism on Waymo Open Motion Dataset with significantly fewer samples compared to heuristic search-based methods, and enables goal-conditioned controllability.

Conclusion: RLFTSim effectively addresses the realism alignment issue in traffic simulation and provides a method for distilling controllability through goal conditioning.
...
### [208] [Guiding Neuro-Symbolic Scenario Generation with Spatio-Temporal Logic](https://arxiv.org/abs/2605.19038)
*Lorenzo Bonin,Francesco Giacomarra,Luca Bortolussi,Jyotirmoy V. Deshmukh,Francesca Cairoli*

Main category: cs.RO

TL;DR: STRELGen is a scalable framework that uses diffusion models with spatio-temporal logic specifications to efficiently generate safety-critical driving scenarios for stress-testing autonomous systems.


<details>
  <summary>Details</summary>
Motivation: Conventional AD safety testing relies on exposing systems to vast real-world traffic scenes, which is prohibitively expensive and statistically ineffective at capturing rare, safety-critical edge cases needed for validating real-world robustness.

Method: STRELGen combines a multi-agent trajectory-generation diffusion model with Spatio-Temporal Logic (STREL) specifications. It uses differentiable monitoring of specification satisfaction to enable gradient-based search, optimizing over the DM latent space at inference time to maximize STREL formula satisfaction.

Result: Efficient generation of highly plausible yet safety-critical multi-agent scenarios that lie within the learned data distribution.

Conclusion: STRELGen provides a flexible, interpretable, and powerful tool for stress-testing autonomous driving systems, moving beyond brute-force data collection by efficiently generating plausible yet safety-critical multi-agent scenarios.
...
### [209] [Neural Operators for Design-Space Surrogate Modeling of Tendon-Actuated Continuum Robots](https://arxiv.org/abs/2605.19104)
*Branden Frieden,James M. Ferguson,Alan Kuntz,Varun Shankar*

Main category: cs.RO

TL;DR: Presents neural operator architectures (DeepONet and FNO-based) as surrogate models for tendon-driven continuum robots that generalize across different robot designs, achieving fast and accurate configuration prediction.


<details>
  <summary>Details</summary>
Motivation: Traditional physics-based models are computationally expensive and inaccurate due to unmodeled effects, while learning-based methods fail to generalize beyond specific robots.

Method: Formulates surrogate modeling as an operator learning problem mapping design parameters and actuation inputs to configurations; develops four novel neural operator architectures (two DeepONets, two FNOs) trained on simulation data.

Result: All four architectures achieve good accuracy and enable fast, accurate generalization across different robot designs.

Conclusion: Operator learning provides an effective, generalizable surrogate for continuum robot mechanics, enabling fast modeling for control, planning, and design optimization.
...
### [210] [CosFly: Plan in the Matrix, Fly in the World](https://arxiv.org/abs/2605.19120)
...
TL;DR: CosFly is a box-structured planning and multimodal simulation pipeline for aerial tracking, with CosFly-Track being a large-scale UAV dataset for dynamic target tracking across diverse environments.


<details>
  <summary>Details</summary>
Motivation: To provide a scalable foundation for aerial-ground collaborative research by simulating UAV tracking with realistic obstacle representations, multi-modal sensor data, and configurable zoom levels, addressing the lack of comprehensive datasets and simulation pipelines for dynamic target tracking.

Method: A modular 7-step construction pipeline on CARLA that converts 3D worlds into structured obstacle representations, plans trajectories, and projects results into RGB/depth/segmentation data with language instructions. Supports configurable fixed-FOV zoom levels and two trajectory-planning paradigms (two-stage candidate generation & refinement, and direct gradient-based optimization).

Result: CosFly-Track dataset contains 250 validated trajectories and ~100,000 rendered images with complete 6-DOF drone pose annotations.

Conclusion: The pipeline and dataset establish a scalable foundation for aerial-ground collaborative research, supporting dynamic target tracking, UAV navigation, and multi-modal perception.
...
### [211] [Automatically Improving Simulation Physics for Articulated Objects](https://arxiv.org/abs/2605.19136)
*Anh-Quan Pham*

Main category: cs.RO

TL;DR: This thesis introduces "interaction-readiness" for simulation objects, proposes a framework to evaluate it, and presents a multi-modal, simulator-in-the-loop method to generate physically realistic articulated objects from incomplete 3D assets, improving simulation stability and downstream policy performance.


<details>
  <summary>Details</summary>
Motivation: Simulation is key for scalable robot learning, but objects from modern 3D datasets lack the physical properties needed for stable, realistic interaction. Manual effort to construct simulation-ready objects is significant, highlighting the need for automated quality assessment and refinement.

Method: The paper proposes (1) a quantitative evaluation framework decomposing "interaction-readiness" into measurable components, and (2) a multi-modal, simulator-in-the-loop approach that integrates geometric, visual, and semantic information to infer physical properties, refined through iterative simulator feedback.

Result: Object quality directly impacts simulation stability, interaction behavior, and policy performance. Objects refined by the proposed method exhibit more stable and realistic dynamics, enabling more reliable downstream learning and evaluation across diverse articulated objects and manipulation tasks.

Conclusion: Physical realism for articulated objects is crucial in simulation. The work introduces a practical multi-modal refinement approach guided by simulator feedback to construct such objects at scale, demonstrating the importance of interaction-readiness for reliable robot learning.
...
### [212] [COBALT: Crowdsourcing Robot Learning via Cloud-Based Teleoperation with Smartphones](https://arxiv.org/abs/2605.19138)
...
TL;DR: COBALT is a scalable, multi-user teleoperation platform for robotic manipulation that enables concurrent data collection using common devices (phones, VR, keyboards) across simulation and real-world settings, achieving 20Hz with sub-100ms latency. A crowdsourced pilot dataset of 7500+ demonstrations was collected across nine countries.


<details>
  <summary>Details</summary>
Motivation: Large-scale, high-quality demonstration data is scarce and expensive, limiting imitation learning for robotic manipulation. Existing teleoperation systems lack scalability and accessibility.

Method: COBALT uses vectorized environments with load-balanced infrastructure for concurrent multi-user teleoperation on a single GPU, supports various input devices, employs in-memory data cache and efficient video streaming, and implements automated quality filtering and user training curriculum.

Result: Supports 8+ concurrent real-world users per GPU at 20Hz with sub-100ms latency; 256 simulated clients across 8 GPUs. Phone-based teleoperation matches or exceeds specialized hardware. 7500+ demonstrations (50+ hours) collected across nine countries in five days.

Conclusion: COBALT enables scalable, democratized robot learning data collection. The crowdsourced dataset successfully trains state-of-the-art imitation learning algorithms, validating the system's effectiveness.
...
### [213] [A Heuristic Approach for Performance Tuning in RL-based Quadrotor Control via Reward Design and Termination Conditions](https://arxiv.org/abs/2605.19166)
*Fausto Mauricio Lagos Suarez,Akshit Saradagi,Vidya Sumathy,George Nikolakopoulos*

Main category: cs.RO

TL;DR: This paper presents a heuristic approach using reward design and termination conditions to achieve tunable performance in RL-based quadrotor control, enabling baseline, acrobatic, and inspection-like behaviors with low steady-state errors and sample-efficient training.


<details>
  <summary>Details</summary>
Motivation: Existing RL-based quadrotor control focuses on speed and agility but lacks precise, tunable control needed for applications like infrastructure inspection.

Method: A novel reward structure with dual bandwidth exponentials and episode truncation conditions, trained with PPO algorithm in 6 million time steps, with heuristic rules to tune reward weights and coefficients.

Result: Three policies (baseline, acrobatic, inspection) evaluated across 100 trials show accurate and tunable position/yaw tracking with ~2% steady-state error and critically damped response.

Conclusion: The proposed heuristic approach effectively enables tunable performance in RL-based quadrotor control, bridging the gap between agile and precise maneuvers.
...
### [214] [Aerial Inspection Behaviors via RL-based Quadrotor Control for Under-canopy Forest Environments](https://arxiv.org/abs/2605.19202)
*Fausto Mauricio Lagos Suarez,Akshit Saradagi,Vidya Sumathy,Viswa Narayanan Sankaranarayanan,George Nikolakopoulos*

Main category: cs.RO

TL;DR: This paper presents an end-to-end RL-based quadrotor controller for under-canopy forest inspection, combining a deep RL low-level controller (mapping states to RPMs) with a higher navigation layer (TSP + RRT* planners) to achieve safe view-pose tracking and point-to-point navigation in forest environments.


<details>
  <summary>Details</summary>
Motivation: To enable autonomous aerial inspection in under-canopy forest environments using a deep RL-based low-level quadrotor controller that can handle the challenges of forest navigation.

Method: An end-to-end RL policy (states to RPMs) for view-pose tracking, supported by a higher navigation layer with TSP planner for optimal region visitation and RRT* planner for collision-free path generation within RL policy tracking limits.

Result: Through five target inspection scenarios, the RL-based motor-level stabilizing controller supported by the navigation guidance layer effectively serves as the low-level inspection execution module for under-canopy forest missions.

Conclusion: An RL-based end-to-end controller combined with a navigation guidance layer (TSP + RRT*) can be reliably deployed for long-range under-canopy forest inspection missions.
...
### [215] [CLUE: Adaptively Prioritized Contextual Cues by Leveraging a Unified Semantic Map for Effective Zero-Shot Object-Goal Navigation](https://arxiv.org/abs/2605.19206)
*Taeyun Kim,Alvin Jinsung Choi,Dasol Hong,Hyun Myung*

Main category: cs.RO

TL;DR: CLUE adaptively balances room-level and object-level contextual cues using LLM-derived commonsense knowledge for zero-shot object-goal navigation.


<details>
  <summary>Details</summary>
Motivation: Existing ZSON methods treat room and object contextual cues uniformly, ignoring that some objects are better predicted by room type while others by nearby objects, leading to inefficient exploration.

Method: CLUE uses an LLM to estimate a target's room-type association strength, then constructs a unified semantic value map that adaptively weights room vs. object cues based on target ambiguity, combined with multi-viewpoint verification and context-aware exploration.

Result: CLUE consistently outperforms SOTA baselines in both success rate (SR) and success weighted by path length (SPL) in simulation and real-world deployments.

Conclusion: Adaptively balancing contextual room and object cues via LLM-derived commonsense significantly improves zero-shot object-goal navigation performance and practicality.
...
### [216] [Graph Neural Planning and Predictive Control for Multi-Robot Communication-Constrained Unlabeled Motion Planning](https://arxiv.org/abs/2605.19209)
*Manohari Goarin,Yang Zhou,Giuseppe Loianno*

Main category: cs.RO

TL;DR: A hierarchical framework combining Graph ATtention Planner (GATP) with decentralized Nonlinear Model Predictive Controller (NMPC) for multi-robot unlabeled motion planning, demonstrating improved generalization, robustness to communication delays up to 200ms, and real-world quadrotor feasibility.


<details>
  <summary>Details</summary>
Motivation: Existing GNN methods for multi-robot motion planning overlook real-world challenges like dynamic feasibility and communication constraints.

Method: Hierarchical framework: GATP provides intermediate subgoals via multi-robot cooperation, and decentralized NMPC enforces safety under nonlinear dynamics and actuation constraints.

Result: Improved generalization to larger teams, robustness to communication delays up to 200 ms, and practical decentralized on-board inference demonstrated in both simulation and real-world quadrotor experiments.

Conclusion: The proposed hierarchical framework effectively bridges the gap between scalable GNN-based planning and real-world deployment constraints for multi-robot motion planning.
...
### [217] [Bilateral Teleoperation with Compliant 6-DOF Pose-and-Force Sensing](https://arxiv.org/abs/2605.19255)
*Yue Feng,Weicheng Huang,I-Ming Chen*

Main category: cs.RO

TL;DR: A low-cost Cartesian bilateral teleoperation framework using compliant 6-DOF end-effectors (Delta6) on both leader and follower, running on hardware-agnostic WinGs middleware, with decoupled time scales achieving stable tracking under 120±40 ms delay and 1% packet loss at 150 Hz.


<details>
  <summary>Details</summary>
Motivation: Eliminate reliance on costly rigid six-axis force/torque sensors and tightly coupled hardware in bilateral teleoperation systems.

Method: Leader: damping-only admittance loop with 6-D biquad notch filter. Follower: stiffness-damping impedance via position-based outer loop with PID wrench-to-pose mapping. Three time scales (hardware I/O, mid-rate impedance/admittance, low-rate teleoperation messages) decoupled.

Result: On Lite6/FR3 at 150 Hz, stable tracking under 120±40 ms delay and 1% packet loss, matches prescribed virtual stiffness in contact, favorable cumulative energy signature.

Conclusion: Proposed framework enables low-cost, hardware-agnostic bilateral teleoperation with heterogeneous arms.
...
### [218] [PRISM-SLAM: Probabilistic Ray-Grounded Inference for Scale-aware Metric SLAM](https://arxiv.org/abs/2605.19257)
*Eunsoo Im*

Main category: cs.RO

TL;DR: PRISM-SLAM is a real-time monocular SLAM framework that integrates vision foundation model priors into a Bayesian factor graph, using a Plücker Ray-Distance Factor for scale-aware metric localization and a Dynamic Scene Uncertainty Gating mechanism to handle dynamic environments, achieving 30 FPS metric SLAM without post-hoc scale correction.


<details>
  <summary>Details</summary>
Motivation: Monocular SLAM suffers from scale ambiguity and tracking failure in dynamic environments. Existing VFM integration approaches ignore predictive uncertainty and frame-to-frame scale inconsistencies, limiting real-world applicability.

Method: 1) Plücker Ray-Distance Factor anchors monocular observations in absolute metric space, making metric scale Fisher-identifiable. 2) Dynamic Scene Uncertainty Gating (DSUG) uses temporal depth consistency as an epistemic uncertainty proxy to probabilistically down-weight dynamic distractors. 3) Multi-process architecture asynchronously processes VFM inference and geometric tracking for real-time performance.

Result: Achieves metric SE(3) ATE nearly identical to oracle-aligned Sim(3) error on TUM RGB-D and 7-Scenes benchmarks, producing deployment-ready metric trajectories at 30 FPS without post-hoc scale correction.

Conclusion: PRISM-SLAM bridges the gap between vision foundation models and real-world robotic applications by providing robust, scale-aware, metric-consistent SLAM in dynamic environments with real-time performance.
...
### [219] [DEFLECT: Delay-Robust Execution via Flow-matching Likelihood-Estimated Counterfactual Tuning for VLA Policies](https://arxiv.org/abs/2605.19294)
*Yixiang Zhu,Yonghao Chen,Rui Meng,Jingyu Guo,Jiaxiang Zou,Zijie Yang,Taowen Wang,Xinyu Chen*

Main category: cs.RO

TL;DR: DEFLECT is a fully offline post-training refinement that converts inference latency into a preference signal to fix prediction-execution misalignment in asynchronous VLA policies, achieving +6.4% success rate gains in high-latency regimes (5-7 control steps) without human labels or online rollouts.


<details>
  <summary>Details</summary>
Motivation: Asynchronous VLA deployment creates a mismatch: predicted action chunks are conditioned on older observations but execute in a drifted physical state, causing performance collapse (89% → <1% on Kinetix) as inference covers up to 7 control steps.

Method: DEFLECT is a fully offline post-training refinement that: (1) constructs counterfactual fresh/stale action pairs from a frozen reference policy, (2) scores them under deployment-time conditioning via an implicit flow-matching likelihood-ratio surrogate, and (3) uses latency itself as a label-free preference signal—no human labels, reward models, or online rollouts needed.
...
TL;DR: ContextFlow is an alignment framework that uses explicit stage contracts and evidence packets to diagnose and fix task-state misalignment in long-horizon embodied agents, where the planner, memory, and executor disagree on the next step.


<details>
  <summary>Details</summary>
Motivation: As embodied agents delegate navigation and manipulation to specialist executors, the main bottleneck shifts to maintaining a coherent task frontier across planning, memory, and execution. Task-state misalignment causes failures like unsupported handoffs, stage lock, and unnecessary replanning.

Method: ContextFlow represents stages as explicit contracts, converts runtime observations into evidence packets, and applies scoped updates (continue, refine, transfer, promote, repair) to realign the task frontier while keeping executors responsible for local closed-loop control.

Result: Experiments and demonstration traces on long-horizon embodied tasks show that evidence-grounded scoped updates can diagnose and mitigate recurring task-state failures.

Conclusion: Making task-frontier alignment explicit and auditable via ContextFlow effectively addresses task-level consistency failures in long-horizon embodied agents.
...
TL;DR: Proposes a Vision-Language framework with Dual-Heatmap representation (navigation affordance + facing heatmaps) to replace single-point regression for in-FOV semantic navigation, achieving SOTA performance across multiple robot embodiments.


<details>
  <summary>Details</summary>
Motivation: Existing navigation frameworks regress deterministic waypoints, which collapses spatial uncertainty and targets non-traversable object centers, causing execution failures in open-ended semantic instruction grounding.

Method: Dual-Heatmap framework predicting a navigation affordance heatmap (continuous reachable regions) and a facing heatmap (orientation constraints), acting as a differentiable semantic potential field for downstream local planners. Includes an automated synthetic data pipeline with foundation model assistance.

Result: Achieves state-of-the-art performance among comparable 8B baselines. Explicit heatmap prediction drastically improves Affordance Rate (AR) across diverse robot embodiments (Jetbot, H1, Aliengo), mitigating the brittleness of point regression.

Conclusion: The Dual-Heatmap framework offers a transferable path toward safe cross-embodiment semantic navigation by reliably placing targets in executable free space.
...
### [222] [Neuromorphic Control of a Flapping-Wing Robot on Resource-Constrained Hardware](https://arxiv.org/abs/2605.19430)
*Rim El Filali,Chenrui Feng,Chao Gao,Weibin Gu*

Main category: cs.RO

TL;DR: A hierarchical neuromorphic control framework using two lightweight SNNs deployed on a $5 ESP32 microcontroller enables fully onboard, closed-loop flight control of a sub-30g FWMAV, achieving stable flight with 36% lower latency and 18% lower power consumption vs. ANN baselines.


<details>
  <summary>Details</summary>
Motivation: FWMAVs face challenges for onboard control due to nonlinear dynamics and stringent Size, Weight, and Power (SWaP) constraints, exemplified by a butterfly-inspired robot under 30g.

Method: Deploys two lightweight SNNs on an ESP32 microcontroller: one for state estimation from raw sensory feedback and another for control via CPG modulation for wing actuation, trained by imitation learning.

Result: Achieves stable pitch and heading angle tracking in untethered flight; SNN reduces latency by 36% (1059μs→680μs) and power by 18% (0.033W→0.027W) versus ANN baseline.

Conclusion: First demonstration of fully onboard neuromorphic control for autonomous FWMAV flight, highlighting SNNs' potential for energy-efficient autonomy under SWaP constraints.
...
### [223] [Self-assembling Modular Aerial Robot for Versatile Aerial Tasks](https://arxiv.org/abs/2605.19431)
*Junichiro Sugihara,Masaki Kitagawa,Jinjie Li,Yunong Li,Takuzumi Nishio,Kei Okada,Moju Zhao*

Main category: cs.RO

TL;DR: LEGION units are reconfigurable modular aerial robots that can self-assemble in mid-flight into flying manipulators, balancing nimble individual flight with robust collective manipulation capabilities inspired by ant colonies.


<details>
  <summary>Details</summary>
Motivation: To resolve the trade-off between agile navigation (small, nimble drones) and robust aerial manipulation (large, high-thrust platforms) for aerial physical interaction tasks.

Method: Modular multirotor units with joint-equipped docking interfaces at both ends, enabling in-flight self-assembly via controlled contact force/torque, with self-reconfigurability for morphological switching.

Result: Multiple units autonomously dock in flight with zero-clearance interlock outdoors; demonstrated manipulation primitives: pushing, pulling, rotating, grasping, and carrying.

Conclusion: LEGION enables aerial robot swarms to transition from passive observers to active environmental participants, expanding the scope of aerial physical interaction.
...
### [224] [Closed-Loop Hybrid Digital Twin Platform for Connected and Automated Vehicle Validation](https://arxiv.org/abs/2605.19490)
*Kanglong Quan,Zhebing Xia,Linfeng Jiang,Hao Yu,Ziheng Qiao,Dapeng Dong,Dongyao Jia*

Main category: cs.RO

TL;DR: This paper proposes a real-time hybrid digital twin platform that tightly couples CARLA-SUMO co-simulation with a physical test site and vehicle via low-latency V2X communication, enabling closed-loop CAV validation with real-world fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing simulation-based CAV testing lacks seamless integration with real vehicles and field data, limiting fidelity in capturing dynamic, real-world interactions.

Method: A hybrid digital twin platform integrating CARLA-SUMO co-simulation, physical test site, low-latency V2X link, custom middleware for synchronization, photogrammetry asset reconstruction, and cloud-edge architecture.

Result: Stable synchronization and effective closed-loop control with low latency were demonstrated.

Conclusion: The platform is practical for multi-scenario CAV verification, bridging simulation and real-world testing.
...
### [225] [CANINE: Coaching Visually Impaired Users for Interactive Navigation with a Robot Guide Dog](https://arxiv.org/abs/2605.19501)
*Cunjun Yu,Zishuo Wang,Anxing Xiao,Linfeng Li,David Hsu*

Main category: cs.RO

TL;DR: CANINE is an automated coaching system that uses personalized, adaptive verbal feedback to train visually impaired users in navigating with robot guide dogs, significantly improving learning efficiency and performance compared to generic verbal instructions.


<details>
  <summary>Details</summary>
Motivation: Robot guide dogs require subtle human-robot coordination that is difficult for users to learn from generic verbal instructions, limiting their effective use.

Method: CANINE operates at two levels: (1) high-level skill tracking using knowledge tracing to identify and prioritize weakest sub-skills, and (2) low-level adaptive coaching using foundation models to infer error causes and generate targeted verbal corrections per practice episode.

Result: Controlled study with blindfolded participants shows significant improvements in learning efficiency and navigation performance over generic instructions. A retention study confirms lasting improvement after two weeks. A case study with a visually impaired user validates real-world effectiveness while revealing deployment design considerations.

Conclusion: Personalized, adaptive automated coaching can effectively train human-robot coordination for assistive navigation systems, with implications for real-world deployment of robot guide dogs.
...
### [226] [ARC-RL: A Reinforcement Learning Playground Inspired by ARC Raiders](https://arxiv.org/abs/2605.19503)
*Carlo Romeo,Andrew D. Bagdanov*

Main category: cs.RO

TL;DR: ARC-RL introduces a MuJoCo benchmark with four legged robot morphologies (Queen, Bastion, Tick, Leaper) inspired by ARC Raiders game, using a unified reward function and observation template for locomotion control, with empirical comparisons of online and offline-to-online RL algorithms.


<details>
  <summary>Details</summary>
Motivation: Game NPCs have stylistic constraints and non-standard morphologies not addressed by existing sim-to-real robotics benchmarks; there is a need for a locomotion benchmark that accommodates diverse, game-inspired creature designs and animation-style constraints.

Method: ARC-RL provides four MuJoCo environments with distinct hexapod/quadruped morphologies, a unified reward function (velocity tracking, survival, gait compliance, action regularizers, safety penalties, posture anchor), hand-crafted CPG demonstrators, and supports both online algorithms (SAC, SPEQ, SOPE-EO) and offline-to-online methods (SACfD, SPEQ-O2O, SOPE).

Result: A controlled empirical study comparing standard online vs. prior-data-augmented RL algorithms across morphological diversity and animation-style stylistic constraints, characterizing each paradigm's performance.

Conclusion: ARC-RL serves as a playground for studying locomotion control across diverse game-inspired morphologies, enabling evaluation of how different RL paradigms handle morphological diversity and stylistic constraints without motion-capture data.
...
### [227] [SafeAlign-VLA: A Negative-Enhanced Safe Alignment Framework for Risk-Aware Autonomous Driving](https://arxiv.org/abs/2605.19524)
*Kefei Tian,Yuansheng Lian,Kai Yang,Xiangdong Chen,Shen Li*

Main category: cs.RO

TL;DR: SafeAlign-VLA is a negative-enhanced safe alignment framework for autonomous driving that incorporates negative samples into supervised learning and reinforcement learning to improve safety in long-tail cases, achieving 89.1 PDMS on NAVSIM and 3.36% collision rate on DeepAccident.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models for autonomous driving rely on positive expert demonstrations and rarely exploit negative samples, leading to insufficient understanding of risky behaviors and safety boundaries.

Method: (1) Counterfactual safety pairing paradigm to generate structured safety labels and counterfactual positive trajectories; (2) Two-stage training: negative-enhanced supervised fine-tuning (failure feedback + trajectory correction) + anchor-based group relative policy optimization using positive/negative trajectories as contrastive anchors.

Result: 89.1 PDMS on NAVSIM v1 (+1.3% over baseline), 3.36% collision rate on DeepAccident, 84.2% language accuracy, 85.8% risk prediction accuracy.

Conclusion: The negative-enhanced safe alignment framework effectively improves safety and robustness in autonomous driving by leveraging negative data.
...
### [228] [Learning-Accelerated Optimization-based Trajectory Planning for Cooperative Aerial-Ground Handover Missions](https://arxiv.org/abs/2605.19562)
*Jingshan Chen,Bochen Yu,Henrik Ebel,Peter Eberhard*

Main category: cs.RO

TL;DR: A learning-augmented trajectory planning framework using LSTM networks to generate warm starts for centralized optimization, achieving 3x speedup and 100% success rate in UAV-UGV handover missions.


<details>
  <summary>Details</summary>
Motivation: Centralized trajectory optimization ensures feasibility and optimality but has high computational cost limiting real-time UAV-UGV handover applications.

Method: A neural surrogate planner using decoupled encoder-decoder LSTM networks generates trajectory predictions from task specifications, which serve as informed warm starts for the downstream centralized optimizer.

Result: Achieves more than threefold speedup and 100% optimization success rate compared to cold start optimization.
...
### [229] [PAPO-VLA: Planning-Aware Policy Optimization for Vision-Language-Action Models](https://arxiv.org/abs/2605.19580)
*Peizheng Guo,Jingyao Wang,Changwen Zheng,Wenwen Qiang*

Main category: cs.RO

TL;DR: PAPO-VLA improves VLA policy reliability by identifying and prioritizing planning actions (task-oriented decisions) over executor actions during optimization, using causal importance estimation within GRPO.


<details>
  <summary>Details</summary>
Motivation: VLA policies struggle with reliability in closed-loop robotic manipulation because they serve dual roles (planner + executor), and existing optimization methods fail to explicitly identify or prioritize planning actions that critically affect task success.

Method: PAPO-VLA identifies planning actions via action variation + trajectory outcome analysis, estimates their importance using causal sufficiency/necessity, and incorporates this into GRPO advantage estimation for prioritized optimization.

Result: PAPO-VLA demonstrates effectiveness across multiple benchmarks.

Conclusion: Explicitly focusing optimization on planning actions improves VLA policy reliability in closed-loop manipulation tasks.
...
### [230] [Implicit Action Chunking for Smooth Continuous Control](https://arxiv.org/abs/2605.19592)
*Bosun Liang,Shuo Pei,Zirui Chen,Chuanzhi Fan,Chen Sun,Yuankai Wu,Huachun Tan,Yong Wang*

Main category: cs.RO

TL;DR: This paper proposes Dual-Window Smoothing (DWS), an implicit action chunking framework that produces smooth continuous control signals in reinforcement learning without expanding the action space, achieving superior smoothness and safety across robotics, energy management, and autonomous driving benchmarks.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning often generates high-frequency oscillatory control signals that threaten safety and stability for physical deployment. Explicit action chunking addresses this but scales policy output dimension with horizon length, causing optimization difficulties and incompatibility with step-wise interaction.

Method: DWS uses a dual-window design: (1) an execution window ensuring physical smoothness via deterministic modulation, (2) a value window aligning temporal-difference targets over the horizon to correct critic bias from open-loop execution. It also includes a lightweight actor-side temporal regularizer based on first-order action differences to promote global continuity.

Result: Outperforms SOTA baselines on DeepMind Control Suite and industrial energy management tasks. In vision-based autonomous driving, achieves smoother control, safer behavior with reduced jitter, and 100% success rate.

Conclusion: DWS effectively bridges the gap between temporal abstraction and reactive step-wise control, enabling smooth and safe RL-based continuous control without the scalability issues of explicit action chunking.
...
### [231] [MCNav: Memory-Aware Dynamic Cognitive Map for Zero-shot Goal-oriented Navigation](https://arxiv.org/abs/2605.19594)
*Jingyu Li,Zhe Liu,Wenxiao Wu,Li Zhang*

Main category: cs.RO

TL;DR: MCNav is a memory-aware navigation framework with a dynamic cognitive map that stores queryable information about objects in explored areas, using goal re-validation and missed goal re-exploration strategies with stabilizing mechanisms to achieve state-of-the-art performance on instance-level goal navigation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot navigation methods focus too heavily on exploring new regions while lacking deeper exploitation of previously explored areas, causing frequent navigation failures when targets are missed or misidentified in visited regions.

Method: MCNav introduces a dynamic cognitive map storing efficiently queryable information about relevant objects, with two memory-aware exploration strategies: goal re-validation (re-assessing previously seen objects) and missed goal re-exploration (estimating target likelihood from contextual cues), stabilized by a blacklist mechanism and a double-check mechanism.

Result: MCNav achieves state-of-the-art performance on HM3Dv1 and HM3Dv2 datasets across three different tasks, especially on instance-level goal navigation.

Conclusion: Memory-aware exploitation of explored regions, not just exploration of new areas, is crucial for improving instance-level navigation in complex environments.
...
### [232] [FlyMirage: A Fully Automated Generation Pipeline for Diverse and Scalable UAV Flight Data via Generative World Model](https://arxiv.org/abs/2605.19600)
*Jinhan Li,Xijie Huang,Zhaoqi Wang,Yijin Wang,Weiqi Ge,Qiyi He,Mo Zhu,Fei Gao,Yuze Wu,Xin Zhou*

Main category: cs.RO

TL;DR: FlyMirage introduces an automated, scalable pipeline using LLMs and 3D Gaussian Splatting to generate high-fidelity, diverse aerial VLN datasets with dynamically feasible UAV trajectories, addressing limitations of existing aerial datasets.


<details>
  <summary>Details</summary>
Motivation: Existing aerial VLN datasets lack the combination of scale, diversity, and realism due to reliance on costly real-world scenes or visually limited simulations.

Method: FlyMirage uses LLMs as scene designers to promote diversity, a generative world model with 3DGS for photorealistic instantiation, automated scene exploration and semantic acquisition, and a dynamically feasible UAV trajectory planner.
...
Conclusion: The pipeline supports the development of next-generation embodied navigation models by providing high-quality training data with minimal human labor.
...
### [233] [HEAT: Heterogeneous End-to-End Autonomous Driving via Trajectory-Guided World Models](https://arxiv.org/abs/2605.19631)
*Hoonhee Cho,Giwon Lee,Jae-Young Kang,Hyemin Yang,Heejun Park,Kuk-Jin Yoon*

Main category: cs.RO

TL;DR: Proposes trajectory-driven learning with world model for multi-domain end-to-end autonomous driving, achieving strong performance across heterogeneous datasets without domain-specific retraining.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end driving models perform well on single domains but degrade significantly when trained jointly across multiple heterogeneous domains (different cities, sensors, traffic patterns), limiting real-world deployment.

Method: (1) Trajectory-driven learning paradigm that organizes training around planning trajectories to capture domain-invariant driving intent representations. (2) World model that predicts future latent features conditioned on ego actions for improved feature consistency and reduced domain biases.

Result: Evaluated on nuScenes, NAVSIM, and Waymo end-to-end datasets; shows substantial improvements over existing methods across all domains with a single unified model.

Conclusion: A single unified model can be trained on heterogeneous datasets while maintaining strong per-domain performance, demonstrating a step toward scalable real-world deployment of autonomous driving.
...
### [234] [RoVLA: Multi-Consistency Constraints for Robust Vision-Language-Action Models](https://arxiv.org/abs/2605.19678)
*Jingzhou Luo,Yifan Wen,Yongjie Bai,Xinshuai Song,Yang Liu,Liang Lin*

Main category: cs.RO

TL;DR: RoVLA introduces multi-consistency constraints (instructional, evolutionary, observational) to make VLA models robust against visual, linguistic, and compounded perturbations, outperforming baselines on LIBERO-Plus, RoboTwin 2.0, and real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models rely on shallow correlations in training data and fail under visual/linguistic perturbations; prior works lack invariance-oriented consistency within the end-to-end policy itself.

Method: RoVLA enforces three consistency constraints: Instructional Consistency (stable grounding under paraphrased instructions), Evolutionary Consistency (coherent action intent across generation steps), Observational Consistency (prediction invariance under visual/proprioceptive perturbations).

Result: Outperforms strong baselines on LIBERO-Plus, RoboTwin 2.0, and real-world tasks, demonstrating superior robustness under diverse task/observation shifts.

Conclusion: Multi-consistency learning effectively reduces reliance on superficial correlations and improves robustness and generalization for embodied control.
...
### [235] [D-CLING: Prior-Preserving Depth-Conditioned Fine-Tuning for Navigation Foundation Models](https://arxiv.org/abs/2605.19690)
*Shintaro Nakaoka,Takayuki Kanai,Kazuhito Tanaka*

Main category: cs.RO

TL;DR: A novel fine-tuning method for Navigation Foundation Models that uses zero-initialized residual pathways (inspired by ControlNet) to preserve pre-trained knowledge while learning in-domain geometry, resulting in robust long-horizon navigation with fewer collisions.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning NFMs on small in-domain data erodes pre-trained priors, causing poor obstacle avoidance, navigation failures, and loss of generalization capability.

Method: Attaches a trainable copy of the pre-trained backbone using zero-initialized residual pathways (ControlNet-inspired) to learn geometric cues while preserving pre-trained knowledge.

Result: Enables robust long-horizon navigation with minimal collisions and human intervention; maintains or improves action prediction beyond the fine-tuned dataset.

Conclusion: The proposed method effectively enables continual learning for general navigation by preserving pre-trained generalization while adapting to novel setups.
...
### [236] [Multi-Session Ground Texture SLAM in Low-Dynamic Environments](https://arxiv.org/abs/2605.19701)
*Kyle M. Hart,Brendan Englot*

Main category: cs.RO

TL;DR: This paper explores techniques for improving multi-session SLAM in ground texture environments with low-dynamic changes, finding Kullback-Leibler Divergence most effective for trajectory estimation, and introduces a new multi-session ground texture dataset.


<details>
  <summary>Details</summary>
Motivation: Existing multi-session SLAM systems handle low-dynamic changes (wear, weather, seasons) but ground texture-based SLAM systems have not been adapted for such environments. This work aims to fill that gap.

Method: Three techniques are evaluated for trajectory estimation accuracy in multi-session low-dynamic ground texture environments. The most successful uses Kullback-Leibler Divergence as a similarity score and bias for loop closure confidence.

Result: Kullback-Leibler Divergence outperforms the other two methods in improving trajectory estimation accuracy in low-dynamic ground texture environments.

Conclusion: KL-Divergence is effective for multi-session ground texture SLAM under low-dynamic changes, and the new dataset enables further research in this area.
...
### [237] [KIO-planner: Attention-Guided Single-Stage Motion Planning with Dual Mapping for UAV Navigation](https://arxiv.org/abs/2605.19703)
...
TL;DR: KIO-planner is an attention-guided single-stage trajectory planning framework for autonomous UAV flight in confined environments. It integrates CBAM for feature extraction and a Dual Mapping mechanism for safety and kinodynamic feasibility, achieving 24ms inference latency, 28.4% lower control cost, and improved safety margin (0.48m→0.76m) at speeds up to 3.0 m/s.


<details>
  <summary>Details</summary>
Motivation: Traditional optimization-based planners suffer from mapping latency and local minima in dense environments, while end-to-end learning methods lack fine-grained geometric feature extraction and hard kinodynamic constraints, leading to unpredictable collisions near walls.

Method: KIO-planner integrates (1) a Convolutional Block Attention Module (CBAM) into the perception backbone for adaptive focus on structural edges and traversable space, and (2) a Dual Mapping mechanism with physical bounds activation and a Geometric Safety Shield in depth-pixel space to enforce kinodynamic feasibility and collision-free flight without global map fusion.

Result: Achieves agile navigation at 3.0 m/s, 24ms inference latency, 28.4% lower control cost, and improves worst-case safety margin from 0.48m to 0.76m compared to state-of-the-art baselines.

Conclusion: The proposed Dual Mapping mechanism substantially enhances safety in highly constrained environments, enabling faster, smoother, and safer UAV navigation than existing methods.
...
### [238] [Beyond Imitation: Learning Safe End-to-End Autonomous Driving from Hard Negatives](https://arxiv.org/abs/2605.19771)
*Junli Wang,Zhihua Hua,Xueyi Liu,Zebin Xing,Haochen Tian,Kun Ma,Hangjun Ye,Guang Chen,Long Chen,Qichao Zhang*

Main category: cs.RO

TL;DR: BeyondDrive is a failure-aware imitation learning framework for autonomous driving that jointly learns from successful and failed driving behaviors. It uses flow matching to generate safety-critical negative trajectories, diversity-aware sampling to cover failure modes, and Repulsive Distance Loss to establish discriminative safety boundaries, achieving 89.7 PDMS on NAVSIMv1 benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing imitation learning methods minimize geometric deviations from expert trajectories, implicitly assuming spatial proximity implies safety. This causes an objective mismatch where similar-imitation-loss trajectories can have drastically different safety outcomes (recoverable vs. collision). The paper aims to address this critical limitation.

Method: (1) Flow matching-based negative trajectory generator for synthesizing safety-critical yet expert-proximate trajectories. (2) Diversity-aware sampling strategy to mitigate mode collapse and cover diverse failure modes. (3) Repulsive Distance Loss that attracts predictions toward expert demonstrations while repelling them from hard negative trajectories, establishing discriminative safety boundaries.

Result: BeyondDrive achieves 89.7 PDMS on NAVSIMv1 closed-loop benchmark, outperforming prior SOTA. It generalizes across different autonomous driving architectures (including multi-modal planners) and demonstrates strong zero-shot transferability on the HUGSIM benchmark.

Conclusion: Learning from both successes and failures (not just minimizing geometric errors) significantly improves safety in end-to-end autonomous driving. The proposed framework effectively models safety asymmetry and establishes clear safety boundaries in trajectory space.
...
### [239] [Justifying bio-inspired robotics research: A taxonomy of strategies](https://arxiv.org/abs/2605.19840)
*Margaret J. Zhang,Justin Ting,Talia Y. Moore*

Main category: cs.RO

TL;DR: The paper proposes a taxonomy of motivations for bio-inspired design in robotics to systematize approaches, clarify expectations, and help researchers and funders evaluate different bio-inspired methods.


<details>
  <summary>Details</summary>
Motivation: Lack of systematic approach in bio-inspired design leads to inconsistent motivations/methods, making it hard to predict or evaluate success, often resulting in disappointment when designs seem superficial or incomplete.

Method: Proposes a taxonomy of motivations for bio-inspired design and describes potential significant contributions resulting from different approaches.

Result: A taxonomy that assists robotics researchers in justifying their bio-inspired approach and helps funding program managers discern the value of different bio-inspired approaches.

Conclusion: Systematizing bio-inspired design motivations can reduce mismatches between expectations and results, improving evaluation and communication in the field.
...
### [240] [Trajectory Planning and Control near the Limits: an Open Experimental Benchmark on the RoboRacer Platform](https://arxiv.org/abs/2605.19881)
*Mattia Piccinini,Patrick Zambiasi,Aniello Mungiello,Mattia Piazza,Felix Jahncke,Johannnes Betz*

Main category: cs.RO

TL;DR: A modular benchmarking framework for high-acceleration autonomous racing, featuring time-optimal raceline generation, online velocity replanning, geometric path tracking, and a novel model-structured neural network (MS-NN) for inverse dynamics, validated on a 1:10-scale RoboRacer platform.


<details>
  <summary>Details</summary>
Motivation: To provide a standardized, modular framework for benchmarking trajectory planning and control methods in high-acceleration autonomous driving scenarios, pushing vehicles to their physical limits.

Method: Proposes a modular framework including: (1) time-optimal raceline generation, (2) online time-optimal velocity replanning, (3) geometric path tracking controllers, and (4) a model-structured neural network (MS-NN) for learning inverse dynamics for steering control. Evaluated on a 1:10-scale RoboRacer platform across two circuits.

Result: MS-NN significantly improves tracking accuracy, reduces steering oscillations, and is physically interpretable. Online velocity replanning improves lap times by compensating for execution errors and enables safer high-speed, high-acceleration operation.

Conclusion: The modular framework effectively benchmarks planning and control methods for limit-handling autonomous driving. MS-NN and online replanning are key components for performance and safety.
...
### [241] [Beyond Action Residuals: Real-World Robot Policy Steering via Bottleneck Latent Reinforcement Learning](https://arxiv.org/abs/2605.19919)
*Dongjie Yu,Kun Lei,Zhennan Jiang,Jia Pan,Huazhe Xu*

Main category: cs.RO

TL;DR: ZPRL freezes a pretrained imitation policy and uses RL to learn residual perturbations on a compact latent bottleneck (extracted via a VIB module), enabling efficient and smooth online adaptation for robot manipulation.


<details>
  <summary>Details</summary>
Motivation: Pretrained imitation policies suffer from execution errors, limited dataset coverage, and deployment mismatch. Existing residual RL methods in action space are noisy and poorly structured; ZPRL proposes a latent-space perturbation to address this.

Method: ZPRL augments a pretrained policy with a plug-and-play variational information bottleneck (VIB) module to learn a task-relevant latent representation offline. During online finetuning, the base policy is frozen, and RL learns only a residual perturbation on this latent, which then conditions the frozen action generator via decoding.

Result: Evaluated on 8 simulated and 4 real-world tasks. ZPRL improves sample efficiency and final performance over strong baselines. In real-world, average success rate improves by 33.7% over imitation policies, with smoother exploration than action-residual methods.

Conclusion: A compact, task-aligned bottleneck latent provides an effective interface for online RL adaptation of pretrained policies, outperforming action-space residual approaches.
...
### [242] [RoHIL: Robust Human-in-the-Loop Robotic Reinforcement Learning Against Illumination Variations](https://arxiv.org/abs/2605.19924)
*Shuoqin Zhang,Yixin Xiong,Xiru Gao,Kai Liu,Ke Wang,Xichuan Zhou,Zhe Hu*

Main category: cs.RO

TL;DR: RoHIL is an offline fine-tuning framework that enables human-in-the-loop RL systems to transfer across workstations with different lighting conditions without extra real-robot interaction, using a world-model-based image relighter, Illumination-Retention Replay, and an anchored Bellman-actor regulariser to prevent catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: HIL-RL systems achieve near-perfect success on training workstations but collapse when moved to a different workstation due to visual input distribution shifts (e.g., lamp positions, window light). Re-collecting demonstrations for every workstation is impractical, and naive fine-tuning causes catastrophic forgetting. The goal is to close this cross-domain gap without extra real-robot interaction.

Method: RoHIL combines three components: (i) a world-model-based image relighter that re-synthesises source workstation trajectories under multiple virtual HDRI environments; (ii) Illumination-Retention Replay (IRR), which interleaves relit adaptation transitions with original-light retention transitions to preserve source workstation Bellman coverage; and (iii) an anchored Bellman-actor regulariser that constrains representation and policy drift from the original source workstation policy.

Result: Across four real-robot manipulation tasks under significant cross-workstation illumination variations, RoHIL substantially improves shifted-light performance where standard HIL-RL collapses, while preserving source-workstation performance, eliminating the need to re-collect data and retrain for every new workstation.

Conclusion: RoHIL effectively closes the cross-domain illumination gap for HIL-RL systems without requiring additional real-robot data collection, making deployment across multiple workstations feasible.
...
### [243] [TravExplorer: Cross-Floor Embodied Exploration via Traversability-Aware 3-D Planning](https://arxiv.org/abs/2605.19958)
*Han Zheng,Zhe Chen,Yudong Huang,Haoran Liu,Jinghao Wang,Ming Yang,Tong Qin*

Main category: cs.RO

TL;DR: TravExplorer enables zero-shot cross-floor object navigation in 3D environments by combining volumetric mapping, traversability-aware planning, and open-vocabulary semantic guidance, outperforming baselines in both simulated and real-world trials.


<details>
  <summary>Details</summary>
Motivation: Existing ZSON systems assume planar, single-floor environments, which fail in real buildings with stairs, landings, and vertically overlapping spaces. A cross-floor navigation framework is needed.

Method: TravExplorer uses: (1) unified volumetric mapping distinguishing obstacles from reachable surfaces (floors, stairs, landings), (2) FOV-aware active perception for incomplete observations, (3) lightweight semantic module aligning probabilistic instance maps with spatial value maps, and (4) hierarchical planner for target-aware frontier touring and foothold-guided 3-D trajectory optimization.

Result: Outperforms representative ObjectNav baselines across 4,195 simulated episodes (HM3D, MP3D) and 50 real-world trials on a Unitree Go2 robot in both single-floor and cross-floor environments without prior maps.

Conclusion: TravExplorer effectively addresses cross-floor ZSON by integrating geometric and semantic reasoning with traversability-aware planning, demonstrating robust zero-shot generalization to real-world 3D buildings.
...
### [244] [CEER: Compliant End-Effector and Root Control as a Unified Interface for Hierarchical Humanoid Loco-Manipulation](https://arxiv.org/abs/2605.19981)
*Xinyuan Luo,Xingrui Chen,Xunjian Yin,Hongxuan Wu,Boxi Xia,Zhuoqun Chen,Jinzhou Li,Boyuan Chen,Xianyi Cheng*

Main category: cs.RO

TL;DR: CEER proposes a compliant end-effector-root control abstraction for modular humanoid loco-manipulation, achieving 3.3 cm tracking accuracy and up to 70% success in long-horizon tasks via a hierarchical planning framework with teacher-student distillation.


<details>
  <summary>Details</summary>
Motivation: Humanoid robots excel at locomotion but struggle with contact-rich, long-horizon manipulation due to the need for compliant whole-body control and modular planner-compatible interfaces.

Method: CEER uses a compliant EE-root control abstraction within a hierarchical planning framework, with a teacher-student framework distilling a motion-tracking controller into a low-level policy consuming only EE-root commands.

Result: 3.3 cm end-effector tracking accuracy with reduced jerk, stable contact-rich teleoperation, and up to 70% success in simulated single-object loco-manipulation tasks.

Conclusion: Compliant EE-root control provides a practical, modular abstraction for scalable integration of diverse humanoid manipulation skills.
...
### [245] [Beyond Binary Success: A Diagnostic Meta-Evaluation Framework for Fine-Grained Manipulation](https://arxiv.org/abs/2605.19986)
*He-Yang Xu,Pengyuan Zhang,Zongyuan Ge,Xiaoshuai Hao,Serge Belongie,Xin Geng,Yuxin Peng,Xiu-Shen Wei*

Main category: cs.RO

TL;DR: MetaFine is a diagnostic meta-evaluation framework that disentangles fine-grained manipulation capabilities into understanding, perception, and controlled behavior axes, revealing that conventional metrics inflate success rates by up to 70% and that the visual encoder's preservation of local spatial structure is a key bottleneck—improving it unlocks new manipulation capabilities without changing downstream policies.


<details>
  <summary>Details</summary>
Motivation: Current embodied AI benchmarks collapse fine-grained manipulation capacities (local attribute grounding, spatial perception, motor execution) into binary success rates, inflating reported capabilities by up to 70% and masking architectural bottlenecks that hinder real-world deployment.

Method: MetaFine disentangles manipulation competency along three axes (understanding, perception, controlled behavior) using a compositional task graph that absorbs heterogeneous benchmarks and reconstructs them into diagnostic scenarios under a unified protocol; it also supports hybrid real-sim validation with limited real-world rollouts to calibrate simulation-based estimates.

Result: Evaluating state-of-the-art VLA models reveals severe dimension-specific failures invisible to conventional metrics; causal intervention identifies the visual encoder's local spatial structure preservation as the key bottleneck—improving it unlocks new capabilities without modifying downstream policies.

Conclusion: MetaFine shifts evaluation from ranking to diagnosis, turning benchmarking into an actionable compass for repairing the layered capacities underlying genuine physical dexterity.
...
### [246] [Minimalist Visual Inertial Odometry](https://arxiv.org/abs/2605.19990)
*Francesco Pasti,Jeremy Klotz,Nicola Bellotto,Shree K. Nayar*

Main category: cs.RO

TL;DR: This paper shows that just four photodiodes with Gabor masks, combined with an IMU, can provide accurate planar odometry for differential-drive robots, using a TCN trained via a physically-grounded simulator.


<details>
  <summary>Details</summary>
Motivation: VIO using cameras requires significant resources due to high pixel count; the authors aim to achieve robust planar odometry with minimal sensing.

Method: Four downward-facing photodiodes with optical Gabor masks produce signals encoding speed; mask parameters and a TCN are jointly optimized in simulation to decode speed, then combined with IMU angular speed for trajectory estimation.

Result: The prototype sensor on a differential-drive robot closely tracks ground truth across diverse indoor/outdoor terrains without real-world fine-tuning.

Conclusion: Minimalist sensing (4 photodiodes + IMU) can achieve efficient and accurate planar odometry, challenging the need for high-resolution cameras.
...
### [247] [Topology-Optimized Pneumatic Soft Actuator: Design and Experimental Validation](https://arxiv.org/abs/2605.20101)
*Sumit Mehta,Konstantinos Poulios*

Main category: cs.RO

TL;DR: This paper extends a 2D topology optimization framework to 3D for designing soft pneumatic actuators, validating two fabricated designs experimentally under large deformation conditions.


<details>
  <summary>Details</summary>
Motivation: To develop soft pneumatic actuators with optimized bending response using nonlinear topology optimization that accounts for large deformations during pressurization.

Method: Extended a density- and porohyperelasticity-based topology optimization framework from 2D to 3D; used to generate actuator designs maximizing bending under prescribed pressure and strain limits; fabricated via stereolithography and tested experimentally.

Result: Two optimized 3D actuator designs were fabricated and experimentally validated, confirming the framework's ability to account for very large deformations during optimization.

Conclusion: The 3D topology optimization approach successfully generates manufacturable, high-performance soft pneumatic actuators that perform as predicted under large deformation.
...
### [248] [Hamilton--Jacobi Reachability for Spacecraft Collision Avoidance](https://arxiv.org/abs/2605.20138)
*Larry Hui,Jordan Kam,William Su,Jianshu Zhou*

Main category: cs.RO

TL;DR: This paper uses Hamilton-Jacobi reachability to solve two-satellite collision avoidance, computing unsafe relative states and integrating them with supervisory control for provable safety guarantees.


<details>
  <summary>Details</summary>
Motivation: To provide mathematically rigorous, provable collision avoidance guarantees for satellites in the same circular orbit, addressing limitations of heuristic or rule-based approaches.

Method: Formulates a zero-sum differential game with HCW dynamics, computes backward reachable sets via HJ reachability analysis, and integrates results with supervisory hybrid control logic.

Result: Characterizes backward reachable sets defining states where collision is unavoidable under worst-case disturbances; states outside this set admit provably collision-free trajectories.

Conclusion: The HJ reachability framework enables mathematically grounded safety guarantees and scalability for satellite collision avoidance.
