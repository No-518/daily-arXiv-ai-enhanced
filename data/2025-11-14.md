...
Method: Joint learning of global and local feature representations with dynamic knowledge distillation for classifier generalization; uses privacy-preserving anchors with strict locality and non-transmission to refine feature space while reducing communication overhead.

Result: Extensive experiments on five image-classification datasets show FedeCouple consistently outperforms nine baseline methods in effectiveness (4.3% improvement), stability, scalability, and security. Theoretical analysis proves convergence for nonconvex objectives.

Conclusion: FedeCouple effectively addresses the coordination challenge between global generalization and local adaptability in federated learning, demonstrating superior performance while maintaining privacy and reducing communication costs.
...
TL;DR: The paper identifies a failure mode in sequential thinking-aware generation where error propagation degrades performance, proposes ParaBench benchmark, and introduces MMaDA-Parallel with Parallel Reinforcement Learning to improve cross-modal alignment in image synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing sequential, autoregressive approaches for thinking-aware generation can paradoxically degrade performance due to error propagation, particularly with poor alignment between generated reasoning and final images.

Method: Proposes MMaDA-Parallel, a parallel multimodal diffusion framework enabling continuous bidirectional interaction between text and images throughout denoising, trained with supervised finetuning and optimized via Parallel Reinforcement Learning (ParaRL) with semantic rewards.

Result: Achieves 6.9% improvement in Output Alignment on ParaBench compared to state-of-the-art model Bagel, significantly improving cross-modal alignment and semantic consistency.

Conclusion: Establishes a more robust paradigm for thinking-aware image synthesis by addressing error propagation through parallel multimodal interaction and reinforcement learning optimization.
...
TL;DR: PriVi is a large-scale primate-centric video dataset for pretraining computer vision models, which improves generalization and data efficiency across multiple primate behavior analysis benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing computer vision methods for primate behavior analysis rely on human-centric models and single datasets, limiting generalization. There's a need for primate-specific pretraining data to improve performance.

Method: Created PriVi dataset with 424 hours of curated primate videos from 11 research settings and web sources. Pretrained V-JEPA model on PriVi and evaluated with lightweight frozen classifier across four benchmark datasets.

Result: Outperformed prior work including fully finetuned baselines across ChimpACT, BaboonLand, PanAf500, and ChimpBehave datasets. Showed favorable scaling with fewer labels.

Conclusion: Primate-centric pretraining substantially improves data efficiency and generalization, making it promising for low-label applications in primate behavior analysis.
...
<details>
  <summary>Details</summary>
Motivation: Current phonotrauma severity assessment relies on costly clinician judgment with variable reliability, needing automated tools for large-scale studies and improved patient care.

Method: Ordinal regression framework modified with novel soft label handling to account for label uncertainty from annotator rating distributions.
...
Method: The framework disentangles individual instructions from multi-part edit prompts and exposes each as a globally trained slider. It learns a single set of low-rank adaptation matrices that generalize across diverse edits, attributes, and compositional instructions, enabling continuous interpolation along edit dimensions.

Result: Applied to state-of-the-art models like FLUX-Kontext and Qwen-Image-Edit, SliderEdit shows substantial improvements in edit controllability, visual consistency, and user steerability compared to fixed-strength approaches.

Conclusion: SliderEdit is the first framework for continuous, fine-grained instruction control in instruction-based image editing, paving the way for interactive, instruction-driven image manipulation with continuous and compositional control.
...
TL;DR: Enhanced crowd density estimation algorithm adapted from images to videos using diffusion-based denoising, narrow Gaussian kernels, and event-driven sampling with optical flow for efficient real-time crowd monitoring.


<details>
  <summary>Details</summary>
Motivation: To address temporal challenges in video-based crowd analysis and create a scalable framework for real-time crowd monitoring in public safety, disaster response, and event management applications.

Method: Integration of denoising probabilistic model with diffusion processes, narrow Gaussian kernels for multiple density maps, regression branch for feature extraction, consolidation mechanism based on similarity scores, and event-driven sampling using Farneback optical flow to selectively capture frames with significant crowd movements.

Result: The model effectively captures crowd dynamics in both dense and sparse settings, as shown through qualitative and quantitative evaluations including overlay plots and Mean Absolute Error (MAE). The sampling method reduces frame counts while maintaining essential crowd events.

Conclusion: The work successfully adapts image-based crowd density estimation to video scenarios, providing an efficient and scalable framework that addresses temporal challenges and enables real-time crowd monitoring with reduced computational load and storage requirements.
...
TL;DR: PALMS+ is an image-based indoor localization system that uses monocular depth estimation and geometric layout matching to overcome limitations of existing vision-based methods, achieving better accuracy without training.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing vision-based indoor localization methods like PALMS, which are constrained by smartphone LiDAR's short range and ambiguity in indoor layouts, particularly for GPS-denied environments.

Method: Reconstructs scale-aligned 3D point clouds from posed RGB images using Depth Pro (foundation monocular depth estimation), then performs geometric layout matching via convolution with floor plans to output location and orientation posteriors.

Result: Outperformed PALMS and F3Loc in stationary localization on Structured3D and custom campus dataset (80 observations across 4 buildings), and achieved lower errors in sequential localization on 33 real-world trajectories when integrated with particle filter.

Conclusion: PALMS+ demonstrates robust infrastructure-free indoor localization without training requirements, showing potential for camera-free tracking and accessible applications in GPS-denied environments.
...
TL;DR: This paper proposes a Social LSTM model enhanced with a Dynamic Occupied Space loss function to improve pedestrian trajectory prediction by reducing collisions while maintaining displacement accuracy across various crowd densities.


<details>
  <summary>Details</summary>
Motivation: Existing pedestrian trajectory prediction models treat pedestrians as point entities, ignoring the physical space each person occupies, which leads to unrealistic collision predictions in crowded environments.

Method: The authors enhance Social LSTM with a new Dynamic Occupied Space loss function that combines average displacement error with a collision penalty sensitive to scene density and individual spatial occupancy. The model is trained and evaluated on five datasets generated from real pedestrian trajectories with varying crowd densities.

Result: The proposed model achieves up to 31% reduction in collision rate and reduces average displacement error and final displacement error by 5% and 6% respectively across all datasets. It consistently outperforms state-of-the-art deep learning models.

Conclusion: Incorporating physical space occupancy through the Dynamic Occupied Space loss function significantly improves pedestrian trajectory prediction by reducing collisions while maintaining or improving displacement accuracy across various crowd density conditions.
...
TL;DR: This paper treats automotive camera soiling detection as a semantic segmentation problem, compares segmentation methods vs classification approaches, identifies data leakage in Woodscape dataset, and creates a smaller but effective dataset subset.


<details>
  <summary>Details</summary>
Motivation: Soiling detection is crucial for robust advanced driver assistance systems to handle external conditions like weather and dust.

Method: Treat soiling detection as semantic segmentation problem, compare popular segmentation methods vs tile-level classification, analyze Woodscape dataset for data leakage, and create improved data subset.

Result: Segmentation methods show superior performance over classification approaches. The created smaller dataset subset enables comparable results in much shorter time despite being smaller.

Conclusion: Semantic segmentation is effective for soiling detection, and a carefully curated smaller dataset can achieve comparable performance while reducing training time.
...
TL;DR: Medical foundation models outperform general-domain models in chest X-ray analysis, but their effectiveness depends on task type - strong for classification but poor for complex segmentation without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To understand how pre-training domain, paradigm, and architecture influence embedding quality for selecting optimal encoders in radiology tasks.

Method: Evaluated vision encoders from 8 medical and general-domain foundation models for chest X-ray analysis using linear probing and fine-tuning on classification and segmentation tasks.

Result: Medical FMs consistently outperformed general-domain models in linear probing, but all FMs performed poorly for complex segmentation without significant fine-tuning. Text-image alignment wasn't necessary for good performance.

Conclusion: Medical pre-training is beneficial but not universally effective; architectural choices are critical, and supervised models remain strong alternatives for complex localization tasks.
...
TL;DR: A new iris image augmentation method that traverses generative model latent spaces to manipulate specific iris attributes while preserving identity, using gradient-based guidance for geometrical, textural, and quality features.


<details>
  <summary>Details</summary>
Motivation: Need for diverse iris datasets with realistic variations and anomalies for reliable recognition and attack detection, overcoming challenges in synthesizing same-identity iris images with controlled attributes due to rich texture spanning wide spatial frequencies.

Method: Latent space traversal guided by gradients of specific iris image features (sharpness, pupil size, iris size, pupil-to-iris ratio) using differentiable loss terms, applicable to both GAN-generated and real images via GAN inversion.

Result: Successfully manipulates desired iris properties while maintaining identity, with extensibility to any differentiable attribute and compatibility with both synthetic and real iris images.

Conclusion: The proposed gradient-guided latent space traversal provides an effective iris image augmentation strategy that preserves identity while enabling controlled manipulation of various iris attributes for improved dataset diversity.
...
TL;DR: STORM is an annotation-free, real-time 6D pose estimation system that uses vision-language understanding and self-supervised feature matching to track objects without manual segmentation masks, with automatic recovery from occlusions.
...
Method: Three-stage pipeline: contextual object descriptions guide localization, self-cross-attention identifies candidate regions, segmentation model produces precise masks for pose estimation, with automatic re-registration through feature similarity monitoring.
...
TL;DR: PANDA is a framework that addresses dual-level data imbalances in exemplar-free continual learning by using patch-based augmentation and adaptive distribution balancing to improve accuracy and reduce forgetting.


<details>
  <summary>Details</summary>
Motivation: Real-world data streams exhibit dual-level imbalances - both dataset-level distributions and extreme skews within individual tasks - which hinder effective learning in exemplar-free continual learning scenarios.

Method: PANDA uses CLIP encoder to identify representative regions from low-frequency classes and transplants them into frequent-class samples, plus an adaptive balancing strategy that leverages prior task distributions to smooth inter-task imbalances.
...
Conclusion: PANDA effectively addresses dual-level data imbalances in continual learning through patch-and-distribution-aware augmentation, enhancing performance of existing methods without requiring exemplar storage.
...
TL;DR: STS is a lightweight test-time adaptation framework for VLMs that steers latent representations using spectral subspaces from textual embeddings, achieving faster inference with minimal parameters.
...
Conclusion: STS provides an efficient and effective solution for test-time adaptation of VLMs without modifying frozen encoders, making it suitable for practical deployment.
...
TL;DR: Lumos3D is a pose-free framework for 3D low-light scene restoration that directly reconstructs normal-light 3D Gaussian representations from unposed multi-view images without per-scene optimization.
...
Result: Experiments show Lumos3D achieves high-fidelity low-light 3D scene restoration with accurate geometry and strong generalization to unseen cases.

Conclusion: The framework demonstrates versatility by extending to over-exposure correction and provides a scalable solution for diverse lighting restoration tasks without pose requirements.
...
<details>
  <summary>Details</summary>
Motivation: Existing cross-view retrieval methods require supervised training on curated datasets and rely on specialized images (panoramic/UAV), limiting real-world deployment. There's a need for training-free approaches that work with standard monocular street images.

Method: Extracts geographic cues from street images via web search and LLM-based location inference, generates satellite queries using geocoding API, and retrieves matches using pretrained vision encoder (DINOv2) with PCA-based whitening feature refinement.

Result: Outperforms prior learning-based approaches on benchmark datasets under zero-shot settings without ground-truth supervision or finetuning. Also enables automatic construction of semantically aligned street-to-satellite datasets.

Conclusion: The proposed training-free framework provides an effective and scalable solution for cross-view image retrieval, offering superior performance and enabling cost-efficient dataset creation compared to supervised methods.
...
TL;DR: A novel framework using 3D Gaussian Splatting (3DGS) for animating humans in 3D scenes, enabling geometry-consistent free-viewpoint rendering without requiring paired human-scene data.


<details>
  <summary>Details</summary>
Motivation: 3DGS has achieved state-of-the-art photorealistic results for novel-view synthesis but remains under-explored for human-scene animation and interaction. Existing animation pipelines use meshes or point clouds, but 3DGS offers advantages for this application.

Method: Represent humans and scenes as Gaussians, decouple rendering from motion synthesis, use Gaussian-aligned motion module with opacity-based cues and projected Gaussian structures, and apply human-scene Gaussian refinement optimization for realistic contact and navigation.

Result: The approach was evaluated on scenes from Scannet++ and SuperSplat library, and avatars from sparse and dense multi-view human capture. It enables geometry-consistent free-viewpoint rendering of edited monocular RGB videos with new animated humans.

Conclusion: The framework demonstrates the unique advantage of 3DGS for monocular video-based human animation, allowing for novel applications in human-scene interaction without requiring paired data.
...
<details>
  <summary>Details</summary>
Motivation: Adversarial patch attacks pose serious risks to real-world vision applications by injecting localized perturbations that mislead deep learning models. Existing defenses like PatchCleanser are computationally expensive with O(n²) inference cost.
...
Method: VISTA uses a cross-attention fusion module to integrate long-horizon intent with past motion, social-token attention for flexible interaction modeling, and pairwise attention maps for interpretable social influence patterns.
...
TL;DR: This paper adapts a solar physics foundation model to create embeddings for solar wind structure analysis, combining them with spacecraft position and magnetic connectivity data to build a neural field-based model for classifying solar wind structures using Parker Solar Probe measurements.


<details>
  <summary>Details</summary>
Motivation: Space weather poses growing risks to satellites and ground infrastructure, but automated classification of solar wind structures is challenging due to variable plasma properties. The study aims to bridge the gap between remote sensing and in situ observations.

Method: Adapt a foundation model trained on Solar Dynamics Observatory imagery to create solar wind embeddings, concatenate with spacecraft position and solar magnetic connectivity encoded using Fourier features, and fine-tune the neural field-based model for classification using Parker Solar Probe measurements.

Result: Overall classification performance is modest, likely due to coarse labeling, class imbalance, and limited transferability of the pretrained model, but the study demonstrates feasibility of using foundation model embeddings for in situ solar wind tasks.

Conclusion: This work serves as a first proof-of-concept that lays groundwork for future improvements toward more reliable space weather predictions, with code and configuration files made publicly available for reproducibility.
...
TL;DR: MSGNav is a zero-shot navigation system that uses Multi-modal 3D Scene Graphs (M3DSG) to preserve visual cues instead of text-only relations, addressing limitations of existing methods while achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Real-world robotic navigation requires open vocabulary generalization and low training overhead, but existing zero-shot methods compress visual observations into text-only relations, losing visual evidence and having constrained vocabularies.

Method: Introduces M3DSG with dynamically assigned images instead of textual edges, plus MSGNav system with Key Subgraph Selection, Adaptive Vocabulary Update, Closed-Loop Reasoning, and Visibility-based Viewpoint Decision modules.

Result: Achieves state-of-the-art performance on GOAT-Bench and HM3D-OVON datasets, demonstrating superior navigation capabilities.

Conclusion: MSGNav effectively addresses the last-mile problem in zero-shot navigation and provides a robust solution for open-vocabulary embodied navigation with preserved visual evidence.
...
TL;DR: IPCD-Net enables direct decomposition of colored point clouds into albedo and shade components, overcoming challenges of non-grid structure and global-light direction through point-wise feature aggregation and multi-view projection-based luminance distribution.


<details>
  <summary>Details</summary>
Motivation: Point clouds are widely used in AR and robotics where relighting and texture editing require accurate albedo-shade separation, but conventional image-based methods fail on non-grid point clouds and existing point-cloud models ignore global-light direction.

Method: Proposes IPCD-Net with point-wise feature aggregation for non-grid processing and Projection-based Luminance Distribution (PLD) with hierarchical feature refinement to capture global-light cues via multi-view projection.

Result: IPCD-Net reduces cast shadows in albedo and enhances color accuracy in shade, demonstrated on a synthetic outdoor-scene dataset. Successfully applied to texture editing, relighting, and point-cloud registration under varying illumination.

Conclusion: IPCD-Net effectively addresses point cloud decomposition challenges and shows real-world applicability for various computer vision tasks requiring accurate albedo-shade separation.
...
TL;DR: SemanticVLA is a Vision-Language-Action framework that addresses perceptual redundancy and superficial instruction-vision alignment through semantic-aligned sparsification and enhancement for efficient robotic manipulation.


<details>
  <summary>Details</summary>
Motivation: Current VLA models suffer from perceptual redundancy (processing irrelevant visual inputs inefficiently) and superficial instruction-vision alignment (poor semantic grounding of actions), hindering practical deployment in robotic manipulation.

Method: 1) Semantic-guided Dual Visual Pruner (SD-Pruner) with Instruction-driven Pruner and Spatial-aggregation Pruner for sparsifying redundant perception; 2) Semantic-complementary Hierarchical Fuser (SH-Fuser) for integrating semantics with spatial geometry; 3) Semantic-conditioned Action Coupler (SA-Coupler) for enhanced perception-to-action transformation.

Result: SemanticVLA achieves SOTA performance and efficiency: surpasses OpenVLA on LIBERO benchmark by 21.1% in success rate, while reducing training cost and inference latency by 3.0-fold and 2.7-fold.

Conclusion: The proposed SemanticVLA framework effectively addresses key limitations in VLA models through semantic-aligned sparsification and enhancement, demonstrating superior performance and efficiency in robotic manipulation tasks.
...
TL;DR: T-DRS addresses attention decay in LVLMs using three inference-only strategies to enhance long-range dependency modeling without training.


<details>
  <summary>Details</summary>
Motivation: LVLMs struggle with long-range dependencies due to ROPE-induced progressive attention decay, impairing global context memory.

Method: Three-step Decay Resilience Strategies: SD-DRS amplifies distant semantic signals, DC-DRS modulates attention weights by distance, and reRD-DRS reinforces remote dependencies.

Result: T-DRS consistently improves performance on VQA benchmarks in a training-free manner.

Conclusion: T-DRS effectively mitigates attention decay, enhancing LVLMs' ability to model long-range dependencies while preserving local inductive biases.
...
Method: Uses depth-guided parallel adapters (DPAs) for multi-modal fusion in a prompt-free setting, and query-driven temporal memory (QTM) module that leverages frame-level and video-level queries for temporal consistency.
...
Conclusion: SAM-DAQ successfully adapts SAM2 for RGB-D VSOD by effectively integrating depth and temporal information, overcoming limitations of direct foundation model application.
...
TL;DR: RWKV-PCSSC is a lightweight point cloud semantic scene completion network that reduces parameters by 4.18× and improves memory efficiency by 1.37× compared to state-of-the-art methods while achieving SOTA performance.
...
Conclusion: The lightweight RWKV-PCSSC network successfully addresses computational efficiency challenges in semantic scene completion while maintaining high performance across multiple datasets.
...
TL;DR: HCC-3D introduces hierarchical compression for 3D tokens in Vision-Language Models, achieving 98% compression ratio while improving performance through global structure compression and adaptive detail mining.


<details>
  <summary>Details</summary>
Motivation: Current 3D-VLMs have high computational costs due to processing all 3D tokens in LLMs, limiting practical applications. The bottleneck lies in handling all 3D tokens efficiently.

Method: Hierarchical Compensatory Compression (HCC-3D) with two modules: Global Structure Compression (GSC) using global queries to compress tokens, and Adaptive Detail Mining (ADM) to selectively recompress salient features through complementary scoring.

Result: Achieves approximately 98% compression ratio compared to previous 3D-VLMs while achieving state-of-the-art performance, showing improvements in both efficiency and performance.

Conclusion: HCC-3D effectively reduces computational overhead of 3D tokens while preserving essential information, enabling more efficient and powerful 3D-VLM applications.
...
Conclusion: The proposed SARL and SAL effectively address tiny object detection challenges and significantly improve performance across multiple benchmarks and real-world noisy datasets.
...
TL;DR: Swin-BART encoder-decoder with regional attention module achieves state-of-the-art medical image captioning, improving ROUGE and BERTScore while maintaining interpretability through heatmap visualizations.
...
Result: Achieves superior ROUGE (0.603 vs 0.356/0.255 baselines) and BERTScore (0.807 vs 0.645/0.623 baselines), with competitive BLEU, CIDEr, and METEOR scores. Provides interpretable heatmaps showing regional attributions.

Conclusion: The proposed design yields accurate, clinically phrased captions with transparent regional attributions, supporting safe research use with human oversight in medical reporting workflows.
...
<details>
  <summary>Details</summary>
Motivation: Existing Single-DGOD methods use discrete data augmentation or static perturbations, which fail to capture the continuous and gradual nature of real-world domain shifts (e.g., weather/lighting changes), limiting fine-grained cross-domain perception.

Method: Introduces controllable Gaussian noise injection and multi-scale Gaussian blurring for initial feature perturbations, followed by temporal modeling and liquid parameter adjustment to generate adaptive modulation parameters for smooth continuous domain adaptation.
...
TL;DR: DocWeaver introduces MosaicDoc, a large-scale bilingual benchmark for Visually Rich Document Understanding (VRDU) with complex layouts and multi-task annotations, revealing limitations in current VLMs.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for Vision-Language Models are inadequate for VRDU as they are English-centric, have simplistic layouts, and support limited tasks, failing to evaluate performance on complex real-world documents.

Method: Developed DocWeaver, a multi-agent pipeline using Large Language Models to automatically generate MosaicDoc - a bilingual benchmark with 72K images from newspapers/magazines featuring diverse complex layouts, rich stylistic variety from 196 publishers, and multi-task annotations.

Result: Created MosaicDoc with 72K images and over 600K QA pairs, featuring diverse layouts (multi-column, non-Manhattan), comprehensive annotations (OCR, VQA, reading order, localization), and extensive evaluation showing current model limitations in handling document complexity.

Conclusion: MosaicDoc serves as a definitive benchmark for VRDU, revealing current model limitations and charting a clear path for future research in handling real-world document complexity.
...
TL;DR: SLDC introduces latent space transition operators to compensate for distribution drift in sequential fine-tuning of ViTs for class-incremental learning, achieving performance comparable to joint training.


<details>
  <summary>Details</summary>
Motivation: Sequential fine-tuning of ViTs for class-incremental learning suffers from distribution drift due to sequential optimization of shared backbone parameters, causing mismatch between learned class distributions and updater model.
...
Conclusion: Combining knowledge distillation for representation drift with SLDC for distribution drift compensation enables SeqFT to match joint training performance in class-incremental learning.
...
TL;DR: This paper proposes a debiased dual-invariant defense framework for person re-identification (ReID) to address adversarial attacks, focusing on model bias and composite generalization challenges through data balancing and bi-adversarial self-meta defense.


<details>
  <summary>Details</summary>
Motivation: Person ReID models are vulnerable to adversarial attacks, but existing defenses don't adequately address the unique challenges of metric learning tasks like ReID, particularly model bias and composite generalization requirements.

Method: A two-phase framework: 1) Data balancing using diffusion-model-based resampling to mitigate model bias, and 2) Bi-adversarial self-meta defense with metric adversarial training (farthest negative extension softening) and adversarially-enhanced self-meta mechanism for dual-generalization.

Result: Experiments show the proposed method significantly outperforms existing state-of-the-art adversarial defense methods for person ReID.

Conclusion: The debiased dual-invariant defense framework effectively addresses the key challenges of adversarial defense in person ReID, providing robust protection against attacks while maintaining model fairness and generalization capabilities.
...
TL;DR: AdaptViG is an efficient Vision Graph Neural Network that introduces Adaptive Graph Convolution with Exponential Decay Gating, achieving state-of-the-art accuracy-efficiency trade-off by combining efficient gating in early stages with global attention in final stages.


<details>
  <summary>Details</summary>
Motivation: Vision GNNs face computational challenges from graph construction phases that hinder efficiency, requiring more efficient approaches.

Method: Proposes Adaptive Graph Convolution with static axial scaffold and dynamic Exponential Decay Gating that selectively weights long-range connections based on feature similarity, plus hybrid strategy using gating in early stages and Global Attention in final stage.

Result: AdaptViG-M achieves 82.6% top-1 accuracy (outperforming ViG-B by 0.3% with 80% fewer parameters and 84% fewer GMACs), and on downstream tasks achieves 45.8 mIoU, 44.8 APbox, 41.1 APmask (surpassing EfficientFormer-L7 with 78% fewer parameters).

Conclusion: AdaptViG establishes new state-of-the-art trade-off between accuracy and efficiency among Vision GNNs through its novel adaptive graph construction mechanism.
...
TL;DR: TSPE-GS improves 3D Gaussian Splatting for semi-transparent surfaces by modeling pixel-wise multi-modal opacity/depth distributions instead of single-depth assumptions, resolving cross-surface ambiguity through transmittance sampling and progressive TSDF fusion.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting struggles with semi-transparent surfaces because existing methods assume single depth per pixel, which fails when multiple surfaces are visible through transparency.

Method: Uniformly samples transmittance to model pixel-wise multi-modal distribution of opacity and depth, replaces single-peak assumption, progressively fuses truncated signed distance functions to reconstruct external and internal surfaces separately.

Result: Significantly improves semi-transparent geometry reconstruction while maintaining performance on opaque scenes, works on public and self-collected datasets without extra training overhead.

Conclusion: TSPE-GS provides a unified framework that generalizes to other Gaussian-based reconstruction pipelines and effectively handles semi-transparent surfaces while preserving opaque scene performance.
...
TL;DR: The paper introduces an adaptive fusion framework that enhances CLIP-based NR-IQA by combining cosine similarity with magnitude-aware quality cues, achieving state-of-the-art performance without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Standard CLIP-based NR-IQA methods rely solely on semantic similarity between image embeddings and textual prompts, overlooking the important quality cue from the magnitude of CLIP image features, which empirically correlates with perceptual quality.

Method: Extract absolute CLIP image features, apply Box-Cox transformation for statistical normalization, and design a confidence-guided fusion scheme that adaptively weighs cosine similarity and magnitude-aware quality cues.

Result: Extensive experiments on multiple benchmark IQA datasets demonstrate consistent outperformance over standard CLIP-based IQA and state-of-the-art baselines.

Conclusion: The proposed adaptive fusion framework effectively complements semantic similarity with magnitude-aware quality cues, providing a more comprehensive approach to NR-IQA using CLIP without requiring task-specific training.
...
TL;DR: A pipeline using vision-language models to generate pseudo-labels for training efficient object detectors, with a per-object co-teaching strategy to filter noisy bounding boxes, achieving significant performance improvements on autonomous driving datasets.


<details>
  <summary>Details</summary>
Motivation: Vision-language models offer zero-shot object detection but suffer from high latency and hallucination, making them unsuitable for direct deployment in autonomous driving where manual labeling is expensive.

Method: A pipeline that uses VLMs to generate pseudo-labels, with a per-object co-teaching training strategy where two YOLO models collaboratively filter unreliable bounding boxes based on peer loss values.

Result: Achieved mAP@0.5 boost from 31.12% to 46.61% on KITTI dataset while maintaining real-time detection. With 10% ground truth labels, reached 57.97% mAP@0.5. Similar improvements on ACDC and BDD100k datasets.
...
TL;DR: The paper introduces EquS and EquS+ methods that improve diffusion model-based image restoration by using dual sampling trajectories and timestep-aware scheduling to better leverage diffusion priors.


<details>
  <summary>Details</summary>
Motivation: Existing problem-agnostic diffusion model-based image restoration methods fail to fully utilize diffusion priors, leading to suboptimal performance.

Method: Proposes EquS with dual sampling trajectories for equivariant information, and EquS+ with Timestep-Aware Schedule (TAS) that prioritizes deterministic steps for better certainty and efficiency.

Result: Extensive experiments show the method is compatible with previous approaches and significantly boosts performance without increasing computational costs.

Conclusion: The proposed EquS and EquS+ methods effectively address limitations of current diffusion model-based image restoration approaches by better leveraging diffusion priors through improved sampling strategies.
...
TL;DR: DiVE is a method that preserves the geometric structure of vision-language model embeddings during fine-tuning by constraining difference vectors between pre-trained and fine-tuned models to be equal across data samples, achieving strong performance across ID, OOD, and zero-shot settings.
...
Method: Difference Vector Equalization (DiVE) constrains difference vectors (subtracting pre-trained and fine-tuned embeddings) to be equal across data samples using two losses: average vector loss (AVL) for global structure preservation and pairwise vector loss (PVL) for local multimodal alignment.
...
Conclusion: DiVE successfully fine-tunes vision-language models on ID data without compromising OOD and zero-shot generalization by preserving the crucial geometric structure of embeddings.
...
TL;DR: STELLAR is a diffusion-based scene text editing model that addresses limitations in multilingual support, real-world data adaptation, and evaluation metrics through a language-adaptive glyph encoder, multi-stage training, and a novel Text Appearance Similarity metric.


<details>
  <summary>Details</summary>
Motivation: Current scene text editing methods lack support for low-resource languages, suffer from domain gaps between synthetic and real data, and lack appropriate metrics for evaluating text style preservation.

Method: STELLAR uses a language-adaptive glyph encoder and multi-stage training (pre-training on synthetic data then fine-tuning on real images), along with a new dataset STIPLAR and a novel Text Appearance Similarity metric for evaluation.
...
Conclusion: The proposed STELLAR framework effectively addresses key challenges in scene text editing by enabling reliable multilingual editing, bridging the synthetic-real data gap, and providing robust evaluation through the novel TAS metric.
...
TL;DR: MOBA introduces a physically realizable backdoor attack for LiDAR-based 3D object detection by modeling material properties, using titanium dioxide triggers and a novel simulation pipeline to bridge the digital-physical gap.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks lack physical realizability due to digital-to-physical domain gaps and unoptimized physical triggers that overlook material-dependent LiDAR reflection properties.

Method: Systematic material selection (TiO2) for robust triggers, plus a simulation pipeline with angle-independent Oren-Nayar BRDF approximation and distance-aware scaling for digital-physical alignment.

Result: Achieves 93.50% attack success rate on LiDAR-based and Camera-LiDAR fusion models, outperforming prior methods by over 41%.
...
TL;DR: DBGroup is a two-stage weakly supervised 3D instance segmentation framework that uses scene-level annotations to reduce labeling costs, achieving competitive performance with sparse-point-level methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing weakly supervised methods (one-thing-one-click and bounding box annotations) which are still labor-intensive, complex, and require expert annotators, by using more efficient scene-level annotations.
...
Conclusion: DBGroup provides an effective weakly supervised solution for 3D instance segmentation using scene-level annotations, offering a more scalable and efficient alternative to existing annotation methods while maintaining competitive performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing ViT quantization methods use uniform precision, ignoring component sensitivity differences, while previous mixed precision methods suffer from coarse granularity, metric scale mismatch, and quantization-unaware bit allocation.

Method: Proposes LampQ with layer-wise quantization, type-aware Fisher-based sensitivity metric, optimal bit-width assignment via integer linear programming, and iterative bit-width updates.
...
Conclusion: LampQ overcomes key limitations in ViT quantization through fine-grained layer-wise control and optimal bit allocation, providing superior compression with minimal accuracy degradation.
...
Method: Combines self-supervised MAE for visual representation learning, GAT for modeling label correlations via expert-defined graphs, constraint-aware optimization with KL divergence, and imbalance mitigation using ASL and boosting ensembles.

Result: Achieves state-of-the-art performance on TongueAtlas-4K benchmark (4,000 images with 22 diagnostic labels), the largest public tongue analysis dataset. Framework generalizes to broader medical imaging tasks.

Conclusion: MIRNet provides an effective solution for medical image interpretation by integrating self-supervised learning with constrained reasoning, addressing key challenges in the field while maintaining clinical plausibility and generalizability.
...
TL;DR: Introduces Fine-grained 3D Embodied Reasoning task and AffordBot framework that uses MLLMs with chain-of-thought reasoning to predict spatial location, motion type, and motion axis for affordance elements in 3D scenes based on task instructions.


<details>
  <summary>Details</summary>
Motivation: Existing approaches operate at object level or handle fine-grained affordance reasoning disjointedly, lacking coherent instruction-driven grounding and reasoning for effective human-agent collaboration in physical environments.

Method: Proposes AffordBot framework integrating MLLMs with tailored chain-of-thought reasoning. Renders surround-view images of 3D scenes and projects 3D element candidates into these views. Uses active perception to select informative viewpoints, then step-by-step reasoning to localize affordance elements and infer interaction motions.
...
Conclusion: AffordBot effectively addresses fine-grained 3D embodied reasoning by bridging 3D input with 2D-compatible MLLMs through visual representation aligned with scene geometry and structured reasoning pipeline.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of generating semantically coherent anomalies without requiring exemplar anomalies, enabling better training of anomaly detection models.
...
Conclusion: Anomagic establishes a versatile foundation model for anomaly generation that outperforms existing methods and enables flexible anomaly synthesis for various applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-modal 3D object detection methods follow single-guided paradigms and fail to account for differences in information density for hard instances (distant, small, or occluded objects), which compromises autonomous driving safety.
...
TL;DR: A novel 3D VAE framework using unsigned distance fields (UDFs) with local-to-global architecture that achieves state-of-the-art 3D reconstruction and generation at ultra-high resolutions up to 2048^3.
...
Method: 3D VAE framework built on UDFs with local-to-global architecture that partitions UDF into uniform subvolumes (UBlocks), combining 3D convolutions for local detail and sparse transformers for global coherence, plus Pad-Average strategy for smooth boundary transitions.

Result: Achieves state-of-the-art performance in both reconstruction accuracy and generative quality, with superior surface smoothness and geometric flexibility at ultra-high resolutions up to 2048^3.

Conclusion: The proposed UDF-based 3D VAE with local-to-global architecture provides a robust and scalable solution for high-fidelity 3D content generation, overcoming fundamental limitations of existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing visible-infrared object detection methods suffer from information imbalance between modalities and inadequate cross-modal fusion, and most methods ignore the advantages of frequency domain transformers for mining complementary information.

Method: Proposes multimodal frequency domain attention (MFDA) to mine complementary information, frequency domain feed-forward layer (FDFFL) for mixed-scale frequency feature fusion, cross-modal global modeling module (CGMM) for pixel-wise inter-modal interaction, and local feature enhancement module (LFEM) for multimodal local feature representation.
...
Conclusion: The proposed frequency domain fusion transformer effectively addresses information imbalance and enhances multimodal feature fusion for visible-infrared object detection.
...
TL;DR: MuSc-V2 is a zero-shot anomaly classification and segmentation framework that leverages the property that normal patches find similar neighbors while anomalies remain isolated. It uses mutual scoring between 2D/3D modalities and achieves significant performance gains over previous methods.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot anomaly detection methods overlook the key property that normal image patches across industrial products typically find many similar patches in both 2D appearance and 3D shapes, while anomalies remain diverse and isolated.

Method: Proposes Mutual Scoring framework (MuSc-V2) with: Iterative Point Grouping for better 3D representation, Similarity Neighborhood Aggregation with Multi-Degrees for multi-scale patch features, Mutual Scoring Mechanism for intra-modal scoring, Cross-modal Anomaly Enhancement to fuse 2D/3D scores, and Re-scoring with Constrained Neighborhood to suppress false positives.
...
Conclusion: MuSc-V2 provides a robust zero-shot anomaly detection framework that flexibly works across diverse product lines and consistently outperforms existing methods by explicitly leveraging the discriminative property of normal patch similarity versus anomaly isolation.
...
TL;DR: This paper addresses the poor performance of Multimodal Large Language Models (MLLMs) in image screening by creating a large dataset and developing an enhanced training method called HCM-GRPO that improves image aesthetic reasoning.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs perform poorly in image screening tasks due to lack of data and weak image aesthetic reasoning capabilities, with even state-of-the-art models performing at random guessing levels.

Method: Created a comprehensive dataset with 128k samples (640k images) covering four aesthetic aspects, and developed HCM-GRPO method that incorporates Hard Cases Mining with Dynamic Proportional Accuracy reward into Group Relative Policy Optimization.

Result: The HCM-GRPO method enables smaller models to surpass both large-scale open-source and leading closed-source models in image aesthetic reasoning, overcoming the random-guessing performance of models like GPT4o and Qwen-VL-Max.

Conclusion: The proposed data collection and HCM-GRPO methodology effectively address the image screening problem, demonstrating that proper training with comprehensive datasets can significantly improve MLLMs' image aesthetic reasoning capabilities.
...
TL;DR: The paper introduces AV-ConfuseBench to test MLLMs' ability to detect audio-absent objects in videos, revealing their visual bias. It then proposes RL-CoMM, a reinforcement learning approach that uses an audio-only reference model to improve audio-visual reasoning accuracy.


<details>
  <summary>Details</summary>
Motivation: To address MLLMs' inability to detect confused objects that are visually present but audio-absent, which stems from visually dominated reasoning that overlooks missing audio cues.

Method: RL-CoMM: A two-stage reinforcement learning framework that 1) uses a Large Audio Language Model as reference for audio-only reasoning with step-wise reasoning rewards, and 2) employs answer-centered confidence optimization to reduce reasoning uncertainty.
...
Conclusion: The proposed RL-CoMM effectively mitigates visual bias in MLLMs and enhances their audio-visual reasoning capabilities through collaborative multi-model reinforcement learning.
...
TL;DR: The paper introduces CPREval-6k, a medical action benchmark with 6,372 videos, and proposes GaussMedAct, a multivariate Gaussian encoding framework that achieves 92.1% accuracy for fine-grained medical action evaluation.


<details>
  <summary>Details</summary>
Motivation: Address challenges in medical vision including lack of comprehensive datasets, precision requirements, and insufficient spatiotemporal modeling of rapid medical actions.

Method: GaussMedAct framework using multivariate Gaussian representation to project joint motions to scaled space, decomposing actions into adaptive 3D Gaussian tokens with anisotropic covariance modeling, plus hybrid spatial encoding with Cartesian and Vector dual-stream strategy.

Result: Achieves 92.1% Top-1 accuracy on benchmark, outperforming ST-GCN baseline by +5.9% with only 10% FLOPs, and shows superior robustness in cross-dataset experiments.

Conclusion: The proposed method effectively addresses medical action evaluation challenges through adaptive spatiotemporal representation learning and achieves state-of-the-art performance with computational efficiency.
...
TL;DR: CABIN is a semi-supervised framework that addresses confidence bias in hyperspectral image classification by incorporating uncertainty estimation, dual sampling, and dynamic assignment strategies to improve generalization under sparse annotations.


<details>
  <summary>Details</summary>
Motivation: Models in hyperspectral image classification often mistake high predictive confidence for correctness, leading to confirmation bias and poor generalization under sparse annotations or class imbalance.

Method: CABIN uses a closed-loop process: 1) perceptual awareness through epistemic uncertainty estimation, 2) uncertainty-guided dual sampling (exploring uncertain samples while anchoring confident ones), and 3) fine-grained dynamic assignment that categorizes pseudo-labels into reliable, ambiguous, and noisy subsets with tailored losses.
...
Conclusion: CABIN effectively mitigates confidence bias in hyperspectral image classification through cognitive-aware learning strategies, enhancing model generalization under challenging annotation scenarios.
...
TL;DR: VLF-MSC is a unified multimodal semantic communication system that transmits a single vision-language feature to generate both images and text at the receiver, improving spectral efficiency and semantic fidelity.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of existing semantic communication techniques that process modalities separately, requiring multiple streams or retransmissions, which reduces spectral efficiency and adaptability.

Method: Uses a pre-trained vision-language model to encode source images into vision-language semantic features (VLF), transmitted over wireless channels. Receiver uses decoder-based language model and diffusion-based image generator both conditioned on VLF to produce text descriptions and semantically aligned images.
...
Conclusion: VLF-MSC provides an efficient unified framework for multimodal semantic communication that eliminates modality-specific streams while maintaining robustness to channel noise and preserving semantic fidelity.
...
TL;DR: GlobalDiff is a diffusion-based framework that generates co-speech motion using global joint rotations instead of local hierarchical rotations, eliminating cumulative errors through multi-level structural constraints.
...
Method: Uses diffusion model with global joint rotations and multi-level constraints: joint structure constraints with virtual anchor points, skeleton structure constraints for angular consistency, and temporal structure constraints with multi-scale variational encoder.

Result: Improves performance by 46.0% compared to current state-of-the-art on standard co-speech benchmarks, generating smooth and accurate motions.

Conclusion: Operating in global rotation space with structural constraints effectively eliminates hierarchical error accumulation and produces more reliable co-speech motions.
...
TL;DR: GridPrune is a visual token pruning method for MLLMs that uses a two-stage "guide-globally, select-locally" approach, achieving 96.98% performance retention with only 11.1% of tokens.


<details>
  <summary>Details</summary>
Motivation: Existing pruning methods focus only on "what to select" and ignore "where to look", leading to inefficient spatial allocation, positional bias, and retention of irrelevant tokens.

Method: GridPrune splits pruning into two steps: text-conditional guidance to allocate token budgets across spatial zones, followed by local selection within each budgeted zone.

Result: On LLaVA-NeXT-7B, GridPrune achieves 96.98% of full performance using only 11.1% of tokens, outperforming the best baseline by 2.34% at the same pruning rate.

Conclusion: The two-stage "guide-globally, select-locally" approach inspired by human visual attention significantly improves pruning efficiency in MLLMs.
...
TL;DR: SUGAR is a novel paradigm that combines LLMs with human skeleton data for action recognition by using visual-motion knowledge from video models to supervise skeleton learning, enabling LLMs to understand skeleton representations and perform action classification and description.


<details>
  <summary>Details</summary>
Motivation: To leverage LLMs' rich implicit knowledge and transferability for skeleton-based action recognition, addressing how LLMs can understand skeleton data and distinguish between different actions.

Method: SUGAR pipeline: 1) Use off-the-shelf video models to generate visual-motion knowledge, 2) Supervise skeleton learning with this prior knowledge to create discrete representations, 3) Use pre-trained LLMs to understand these representations and generate action targets/descriptions, 4) Introduce Temporal Query Projection (TQP) module for modeling long skeleton sequences.

Result: Experiments on skeleton-based action classification benchmarks demonstrate SUGAR's efficacy. Zero-shot scenarios show SUGAR is more versatile than linear-based methods.

Conclusion: SUGAR successfully enables LLMs to understand skeleton representations for action recognition, providing an effective framework that outperforms traditional linear methods, especially in zero-shot settings.
...
TL;DR: MTAttack is a novel multi-target backdoor attack framework for Large Visual Language Models that can embed multiple independent triggers with different attack targets in a single training pass, overcoming feature interference challenges through specialized optimization constraints.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks on LVLMs only focus on single-target attacks, but real-world threats could involve multiple triggers targeting different malicious outputs. The feature interference among multiple triggers makes this challenging.

Method: Proposes MTAttack with two optimization constraints: Proxy Space Partitioning and Trigger Prototype Anchoring, which jointly optimize multiple triggers in latent space to ensure each trigger independently maps to unique proxy classes while maintaining separability.

Result: Experiments show high success rate for multi-target attacks, substantially outperforming existing methods, with strong generalizability across datasets and robustness against defense strategies.

Conclusion: LVLMs are vulnerable to multi-target backdoor attacks, highlighting urgent need for mitigation strategies against such threats in real-world applications.
...
TL;DR: RobIA is a robust, instance-aware framework for continual test-time adaptation in stereo depth estimation that dynamically adapts to domain shifts using attention-based expert routing and pseudo-supervision from sparse labels.


<details>
  <summary>Details</summary>
Motivation: Stereo depth estimation faces challenges from dynamic domain shifts, sparse supervision, and high cost of dense ground-truth labels. Existing TTA methods have limited effectiveness under continual shifts due to static assumptions and input-invariant strategies.

Method: RobIA integrates Attend-and-Excite Mixture-of-Experts (AttEx-MoE) for dynamic input routing via lightweight self-attention tailored to epipolar geometry, and Robust AdaptBN Teacher for dense pseudo-supervision using PEFT-based teacher model that complements sparse handcrafted labels.
...
TL;DR: Proposes CACMI framework for dense video captioning that explicitly models temporal coherence and semantic context through cross-modal frame aggregation and context-aware feature enhancement, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on implicit modeling with frame-level features, failing to capture temporal coherence across events and comprehensive visual semantics.

Method: CACMI framework with two components: Cross-modal Frame Aggregation for event-aligned textual features via cross-modal retrieval, and Context-aware Feature Enhancement using query-guided attention to integrate visual dynamics with pseudo-event semantics.

Result: Achieves state-of-the-art performance on ActivityNet Captions and YouCook2 datasets for dense video captioning.

Conclusion: Explicit temporal-semantic modeling through cross-modal interactions significantly improves dense video captioning by capturing temporal coherence and comprehensive visual semantics.
...
TL;DR: Text-to-image models fail at logical composition (negation, counting, spatial relations), showing dramatic performance collapse when primitives are combined due to training data gaps, architectural limitations, and flawed evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: To investigate why current text-to-image models fundamentally fail at handling logical composition despite their impressive performance on single concepts, exposing a critical flaw in their architectural design.

Method: Survey analysis across three core logical primitives (negation, counting, spatial relations) examining training data patterns, architectural limitations of continuous attention models, and evaluation metric biases.

Result: Models show dramatic performance collapse when logical primitives are combined, with severe interference effects. Training data lacks explicit negations, continuous architectures are unsuitable for discrete logic, and metrics prioritize visual plausibility over constraint satisfaction.

Conclusion: Achieving genuine compositionality requires fundamental advances in representation and reasoning rather than incremental adjustments or scaling of existing architectures.
...
TL;DR: The paper proposes a split-layer MLP architecture that enhances implicit neural representation (INR) by dividing layers into parallel branches and combining outputs via Hadamard product, significantly improving representational capacity without prohibitive computational costs.


<details>
  <summary>Details</summary>
Motivation: Conventional MLP architectures in INR have limited representational capacity due to low-dimensional feature spaces, and widening MLPs leads to quadratic growth in computational/memory costs.

Method: Proposed split-layer MLP that divides each layer into multiple parallel branches and integrates outputs using Hadamard product, effectively constructing high-degree polynomial space to expand feature dimensionality.
...
Conclusion: The split-layer approach successfully enhances INR's representational capacity without prohibitive computational overhead, demonstrating superior performance across multiple tasks.
...
TL;DR: A dual-mechanism framework that improves fairness in deepfake detection by combining structural fairness decoupling and global distribution alignment, maintaining accuracy while enhancing both inter-group and intra-group fairness.


<details>
  <summary>Details</summary>
Motivation: Address biases in deepfake detection models toward demographic groups (gender, race) that cause systemic misjudgments and exacerbate digital divide and social inequities, while overcoming the trade-off between fairness and detection accuracy in current methods.

Method: Proposes a dual-mechanism collaborative optimization framework with structural fairness decoupling (decoupling demographic-sensitive channels at architectural level) and global distribution alignment (reducing distance between overall sample distribution and demographic group distributions at feature level).
...
Conclusion: The proposed dual-mechanism framework successfully addresses the fairness-accuracy trade-off in deepfake detection, providing a more equitable solution for digital identity security applications.
...
TL;DR: GEA proposes a generative approach for text-to-image person retrieval using diffusion-generated images as semantic bridges to improve cross-modal alignment and address modality gaps.


<details>
  <summary>Details</summary>
Motivation: Textual queries often fail to accurately reflect image content, leading to poor cross-modal alignment and overfitting. The inherent modality gap between text and image further complicates accurate retrieval.
...
Conclusion: GEA successfully addresses cross-modal alignment challenges in text-to-image person retrieval by leveraging generative approaches to bridge the modality gap between text and images.
...
<details>
  <summary>Details</summary>
Motivation: Real-world images often contain multiple degradations simultaneously (rain, noise, haze), but most existing methods target only single degradation types. Current methods also suffer from limited interpretability when stacking modules for performance improvement.

Method: Deep unfolding network mapping iterative optimization algorithms into learnable structure, using improved second-order semi-smooth Newton algorithm for physical interpretability, and explainable convolution modules inspired by human brain's flexible information processing to autonomously adjust parameters.
...
TL;DR: CephRes-MHNet is a multi-head residual CNN for cephalometric landmark detection that achieves state-of-the-art accuracy (1.23mm MRE) with high efficiency, using only 25% of parameters compared to previous best models.


<details>
  <summary>Details</summary>
Motivation: Manual annotation of cephalometric landmarks from 2D lateral skull X-rays is time-consuming and error-prone, while automated methods struggle with low contrast and anatomical complexity.
...
Result: Achieved mean radial error of 1.23 mm and success detection rate @ 2.0 mm of 85.5%, outperforming all evaluated models including AFPF-Net (1.25mm MRE, 84.1% SDR) while using <25% parameters.

Conclusion: CephRes-MHNet provides state-of-the-art accuracy through architectural efficiency, offering a practical solution for real-world orthodontic analysis.
...
TL;DR: Evaluation of NASA/IBM's Prithvi-EO-2.0 geospatial foundation model for shoreline delineation on small sandy islands using satellite imagery, showing high performance with minimal training data.
...
Method: Curated and labeled 225 multispectral images of Maldivian islands, fine-tuned both 300M and 600M parameter versions of Prithvi-EO-2.0 on training subsets ranging from 5 to 181 images.
...
Conclusion: Prithvi-EO-2.0 shows significant potential for supporting coastal monitoring in data-poor regions through its strong transfer learning capabilities with minimal training data.
...
Result: On SemanticKITTI, LiNeXt achieves 199.8x inference speedup, reduces Chamfer Distance by 50.7%, and uses only 6.1% of parameters compared to LiDiff.
...
TL;DR: HeatV2X is a scalable V2X collaborative perception framework that addresses multi-modal heterogeneity and scalability challenges through heterogeneous graph attention and specialized fine-tuning adapters, achieving superior performance with minimal training cost.


<details>
  <summary>Details</summary>
Motivation: Existing V2X collaborative perception frameworks struggle with multi-modal heterogeneous agents and lack scalability for accommodating new agents, requiring effective cross-agent feature alignment and scalable adaptation.

Method: Uses heterogeneous graph attention to train a foundation agent, then implements Local Heterogeneous Fine-Tuning with Hetero-Aware Adapters for modality-specific alignment, and Global Collaborative Fine-Tuning with Multi-Cognitive Adapters for cross-agent collaboration.
...
Conclusion: HeatV2X provides an effective solution for scalable V2X collaborative perception that handles agent heterogeneity while minimizing training costs, with promising results on benchmark datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal deepfake detection methods struggle with generalization across unseen manipulations and datasets, require pretraining on real samples, and may fail against manipulations that preserve audio-visual alignment by overlooking intra-modal artifacts.

Method: Proposes a single-stage training framework incorporating next-frame prediction for both uni-modal and cross-modal features, along with a window-level attention mechanism to capture discrepancies between predicted and actual frames for detecting local artifacts.
...
Conclusion: The proposed approach effectively addresses limitations of existing methods by enabling better generalization without requiring pretraining and improving detection of manipulations that preserve audio-visual consistency.
...
TL;DR: TubeRMC is a weakly-supervised framework for spatio-temporal video grounding that generates text-conditioned candidate tubes and refines them through tube-conditioned reconstruction with spatio-temporal constraints, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing weakly-supervised STVG methods use simple late-fusion approaches that generate tubes independent of text descriptions, leading to failed target identification and inconsistent tracking. The authors aim to address these limitations.

Method: Proposes TubeRMC framework that: 1) generates text-conditioned candidate tubes using pre-trained visual grounding models, 2) refines tubes via tube-conditioned reconstruction with three strategies (temporal, spatial, spatio-temporal), 3) uses Tube-conditioned Reconstructors to reconstruct query clues from tubes, 4) applies mutual constraints between spatial and temporal proposals.

Result: TubeRMC outperforms existing methods on VidSTG and HCSTVG benchmarks. Visualization shows it effectively mitigates target identification errors and inconsistent tracking.

Conclusion: The proposed TubeRMC framework successfully addresses limitations of previous weakly-supervised STVG methods by incorporating text-conditioned tube generation and comprehensive reconstruction strategies with mutual constraints.
...
TL;DR: Proposes JudgeMind, a novel AQA method that simulates professional referees' judgment process by segmenting videos into stages, using stage-aware feature enhancement, and incorporating deduction knowledge, achieving SOTA performance on a new aerial skiing dataset with fine-grained annotations.
...
Method: JudgeMind segments action videos into stages and scores each stage, uses stage-aware feature enhancement and fusion to handle viewpoint changes, and incorporates deduction knowledge through a grade-aware decoder.
...
Conclusion: Simulating professional referees' judgment process through stage segmentation and knowledge incorporation significantly enhances AQA performance and reliability.
...
TL;DR: Facial-R1 is a three-stage alignment framework that addresses hallucinated reasoning and misalignment issues in Facial Emotion Analysis by combining instruction fine-tuning, reinforcement training with emotion/AU reward signals, and iterative data synthesis to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models for Facial Emotion Analysis suffer from hallucinated reasoning (generating plausible but inaccurate explanations) and misalignment between emotion reasoning and recognition due to fragmented connections between facial features and emotion labels.

Method: Three-stage framework: 1) Instruction fine-tuning for basic emotional reasoning, 2) Reinforcement training guided by emotion and Action Unit labels as reward signals to align reasoning with predictions, 3) Iterative data synthesis pipeline that expands training data for scalable self-improvement.

Result: Achieves state-of-the-art performance across eight standard benchmarks, with strong generalization and robust interpretability. Introduces FEA-20K dataset with 17,737 training and 1,688 test samples with fine-grained emotion analysis annotations.

Conclusion: Facial-R1 effectively addresses hallucinated reasoning and misalignment challenges in Facial Emotion Analysis through its three-stage alignment framework, demonstrating superior performance and interpretability with minimal supervision requirements.
...
TL;DR: H3Former is a token-to-region framework for fine-grained visual classification that uses high-order semantic relations and hyperbolic hierarchical contrastive learning to address subtle inter-class differences and intra-class variations.
...
Method: Proposes H3Former with Semantic-Aware Aggregation Module (SAAM) that constructs weighted hypergraphs among tokens using multi-scale contextual cues and applies hypergraph convolution to capture high-order semantic dependencies. Also introduces Hyperbolic Hierarchical Contrastive Loss (HHCL) to enforce hierarchical semantic constraints in non-Euclidean space.
...
Conclusion: H3Former effectively addresses FGVC challenges by leveraging high-order semantic relations and hyperbolic hierarchical contrastive learning, achieving state-of-the-art performance on multiple benchmarks.
...
TL;DR: PROPA introduces a novel framework combining Monte Carlo Tree Search with GRPO to generate dense process-level rewards for optimizing visual reasoning in VLMs without human annotations, achieving significant performance gains across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models struggle with complex visual reasoning where early errors cascade through multi-step dependencies, and existing methods either require costly step-level annotations (SFT) or provide only sparse outcome-level feedback (RLVR methods like GRPO).

Method: PROPA integrates Monte Carlo Tree Search with GRPO to generate dense process-level rewards, interleaves GRPO updates with SFT to overcome cold-start problems, and trains a Process Reward Model to guide inference-time search.

Result: Across seven benchmarks and four VLM backbones, PROPA consistently outperforms SFT- and RLVR-based baselines, achieving up to 17.0% gains on in-domain tasks and 21.0% gains on out-of-domain tasks compared to state-of-the-art methods.
...
TL;DR: RUDDER is an efficient framework that reduces object hallucination in Large Vision-Language Models by using visual evidence vectors and adaptive token-wise injection during decoding, achieving comparable performance to state-of-the-art methods with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing methods to mitigate object hallucination in LVLMs incur substantial computational overhead, limiting their practicality for real-world, latency-sensitive deployments. There's a need for efficient interventions that don't require extra forward passes.

Method: RUDDER uses two key innovations: (1) CARD vector - a per-sample visual evidence vector extracted from residual updates during a single forward pass, and (2) a Bayesian-inspired adaptive gate that performs token-wise injection with strength conditioned on the model's deviation from visual context.

Result: Extensive experiments on benchmarks like POPE and CHAIR show RUDDER achieves performance comparable to state-of-the-art methods while introducing negligible computational latency.
...
TL;DR: GRAM is a test-time adaptation framework for satellite-based slum segmentation that uses Mixture-of-Experts to handle morphological heterogeneity across regions, enabling generalization to unseen locations without target region labels.
...
Method: Two-phase test-time adaptation framework using Mixture-of-Experts architecture with shared backbone to capture both region-specific slum characteristics and universal features. Uses prediction consistency across experts to filter unreliable pseudo-labels during adaptation.

Result: Outperforms state-of-the-art baselines in low-resource settings like African cities, using a million-scale satellite imagery dataset from 12 cities across four continents.
...
<details>
  <summary>Details</summary>
Motivation: The LLaVA architecture struggles with visual feature integration due to modality mismatch between text and vision.

Method: Three modifications: separate QKV projections for vision, bidirectional attention on visual tokens, and global+local visual representations.

Result: Significantly outperforms baseline LLaVA on multiple benchmarks, surpassing models with double the parameter count.

Conclusion: LLaViT establishes a more effective approach to vision-language modeling by enabling LLMs to serve as vision encoders.
...
TL;DR: Proposes new fine-grained error categories and metrics for pedestrian detection evaluation, achieving SOTA on CityPersons-reasonable with a simple architecture.


<details>
  <summary>Details</summary>
Motivation: Current pedestrian detection benchmarks have weaknesses and metrics don't allow realistic performance evaluation; image segmentation can provide fine-grained information to distinguish different error types.

Method: Proposed 8 different error categories for pedestrian detection and new metrics for performance comparison; used these metrics to compare various backbones for a simplified version of APD.

Result: Achieved state-of-the-art performance on CityPersons-reasonable without extra training data using a simple architecture; demonstrated more fine-grained and robust model comparison especially for safety-critical performance.

Conclusion: The proposed error categories and metrics enable more realistic and safety-focused evaluation of pedestrian detection systems, showing that simple architectures can achieve competitive performance when properly evaluated.
...
TL;DR: CLIP4VI-ReID is a novel network for visible-infrared person re-identification that uses CLIP-driven modality-shared representation learning with text semantics as a bridge to align visible and infrared modalities.


<details>
  <summary>Details</summary>
Motivation: Address the huge gap in physical characteristics between natural images and infrared images in VI-ReID tasks by leveraging text semantics as an intermediary for cross-modal alignment.

Method: Proposes three components: Text Semantic Generation (TSG) for visible images, Infrared Feature Embedding (IFE) to rectify infrared features using text semantics, and High-level Semantic Alignment (HSA) to refine semantic alignment and ensure text contains only ID-related information.
...
Conclusion: The proposed method effectively bridges the modality gap between visible and infrared images using text semantics, enabling more accurate cross-modal alignment and enhancing discriminability of learned representations.
...
Conclusion: The framework successfully bridges physical imaging principles with learning-based depth regularization, offering a scalable solution for handling complex depth stratification in urban environments.
...
TL;DR: DSANet is a novel weakly-supervised video anomaly detection method that explicitly separates abnormal and normal features through self-guided normality modeling and decoupled contrastive semantic alignment, achieving state-of-the-art performance on XD-Violence and UCF-Crime benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing weakly-supervised video anomaly detection methods tend to detect only the most salient response segments while neglecting diverse normal patterns, and suffer from category confusion due to similar appearances, leading to unsatisfactory fine-grained classification results.

Method: Proposes DSANet with two key components: 1) Self-guided normality modeling branch that reconstructs input video features using learned normal prototypes to exploit normality cues, and 2) Decoupled contrastive semantic alignment that temporally decomposes videos into event-centric and background-centric components using frame-level anomaly scores, then applies visual-language contrastive learning.
...
Conclusion: DSANet effectively enhances the distinguishability between normal and abnormal patterns through explicit feature separation at both coarse-grained and fine-grained levels, achieving superior performance in weakly-supervised video anomaly detection.
...
TL;DR: A novel framework for Single Domain Generalization in object detection that combines von Mises-Fisher distribution modeling with Fourier-based augmentation in a CLIP-guided pipeline to improve robustness across unseen domains.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based methods for Single Domain Generalization overlook feature distribution structure and frequency-domain characteristics that are critical for robustness across domains.

Method: Integrates von Mises-Fisher distribution to model directional features for capturing domain-invariant semantics, and Fourier transformation for amplitude/phase perturbation to simulate domain shifts in frequency domain.

Result: Outperforms existing state-of-the-art methods on diverse weather-driving benchmark, demonstrating improved feature diversity and structural consistency across domains.

Conclusion: The proposed framework successfully enhances SDG object detection by preserving CLIP's semantic alignment while enriching feature robustness through vMF distribution modeling and frequency-domain augmentation.
...
TL;DR: DermAI is a smartphone app for real-time skin lesion capture and classification that addresses dataset bias through local adaptation and diverse data collection.


<details>
  <summary>Details</summary>
Motivation: AI dermatology adoption is limited by biased datasets, variable image quality, and limited validation in real clinical settings.

Method: Lightweight smartphone app with on-device quality checks and local model adaptation using a diverse clinical dataset across skin tones and devices.

Result: Models trained on public datasets failed to generalize to clinical samples, but fine-tuning with local data improved performance significantly.

Conclusion: Standardized, diverse data collection aligned with healthcare needs is crucial for effective machine learning in dermatology.
...
TL;DR: SHRUG-FM is a reliability-aware prediction framework for geospatial foundation models that combines OOD detection in input/embedding spaces and predictive uncertainty to identify unreliable predictions, particularly in underrepresented environments like low-elevation zones and river areas.


<details>
  <summary>Details</summary>
Motivation: Geospatial foundation models often fail in environments underrepresented during pretraining, creating reliability issues for climate-sensitive applications where accurate predictions are crucial.

Method: Integrates three complementary signals: OOD detection in input space, OOD detection in embedding space, and task-specific predictive uncertainty for burn scar segmentation.

Result: OOD scores correlate with lower performance in specific environmental conditions, uncertainty-based flags help discard poorly performing predictions, and failures concentrate in underrepresented geographies like low-elevation zones and large river areas.

Conclusion: SHRUG-FM provides a pathway toward safer and more interpretable deployment of geospatial foundation models, helping bridge the gap between benchmark performance and real-world reliability in climate-sensitive applications.
...
TL;DR: Current AI personalization defenses like Anti-DreamBooth are vulnerable - their adversarial perturbations are both visually detectable and easily removable by simple filters, failing to protect user identity effectively.


<details>
  <summary>Details</summary>
Motivation: To address privacy risks in personalized AI applications where user images can leak facial identities, and to expose critical limitations in existing defense mechanisms that create a false sense of security.
...
Result: All current defense methods fail to maintain protective effectiveness when faced with purification threats - their adversarial perturbations are both perceptible and fragile, easily removed by simple filters.

Conclusion: Current defenses offer inadequate protection and highlight the urgent need for more imperceptible and robust methods to safeguard user identity in personalized generation systems.
...
<details>
  <summary>Details</summary>
Motivation: Address real-world lane detection challenges like background clutter, varying illumination, and occlusions that hinder data-driven approaches requiring expensive data collection and annotation.

Method: Proposes SAMIRO - Spatial Attention Mutual Information Regularization with a pretrained model as Oracle, which transfers knowledge while preserving spatial information and can be integrated into various lane detection models.
...
Conclusion: SAMIRO effectively enhances lane detection performance by leveraging pretrained model knowledge while maintaining spatial information, demonstrating consistent improvements across various models and datasets.
...
Method: Two-stage parsing pipeline: 1) Large multimodal model predicts document layout and reading order using visual information; 2) Localized recognition of text, formulas, and tables within detected regions. Includes visual consistency-based reinforcement learning for table structures and specialized modules for image-decoupled table parsing and type-guided table merging.
...
Conclusion: MonkeyOCR v1.5 provides an effective unified framework for parsing complex documents, demonstrating superior performance through its two-stage approach and specialized handling of challenging table structures.
...
TL;DR: A physics-informed Transformer-VAE architecture is proposed for retrieving vegetation biophysical variables from Sentinel-2 data using only simulated data, achieving performance comparable to methods that use real satellite imagery.


<details>
  <summary>Details</summary>
Motivation: Accurate retrieval of vegetation biophysical variables is crucial for ecosystem monitoring and agricultural management, but current methods often require real satellite images for training.
...
Conclusion: The approach demonstrates how integrating physical models with deep networks can improve radiative transfer model inversion, offering a cost-effective, self-supervised solution for global vegetation monitoring without requiring real imagery or calibration.
...
TL;DR: Automated CT imaging approach detects increased pancreatic surface lobularity in T2DM patients, achieving 0.90 AUC for diabetes prediction using deep learning segmentation and biomarkers.


<details>
  <summary>Details</summary>
Motivation: Early detection of Type 2 Diabetes Mellitus is crucial to prevent pancreatic damage, but the role of pancreatic surface lobularity in T2DM screening hasn't been fully investigated.

Method: Used four deep learning models to segment pancreas in 584 patients, automatically detected pancreatic surface lobularity (PSL), and developed multivariate models with CT biomarkers for T2DM prediction.
...
Conclusion: Pancreatic surface lobularity is useful for T2DM screening and could help predict early onset of diabetes through automated CT imaging analysis.
...
TL;DR: GrounDiff is a diffusion-based framework that converts Digital Surface Models (DSMs) to Digital Terrain Models (DTMs) by iteratively removing non-ground structures through denoising, achieving state-of-the-art performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional DTM generation methods rely on manually tuned parameters or complex deep learning architectures with post-processing, creating challenges in scalability and performance.

Method: Proposes Ground Diffusion (GrounDiff) using diffusion models for iterative denoising, with gated design and confidence-guided generation. Also introduces Prior-Guided Stitching (PrioStitch) for scalability using downsampled global priors.

Result: Outperforms deep learning SOTA methods, reducing RMSE by up to 93% on ALS2DTM and 47% on USGS benchmarks. For road reconstruction, achieves up to 81% lower distance error than specialized methods on GeRoD benchmark.

Conclusion: GrounDiff provides an effective diffusion-based solution for DSM-to-DTM translation that eliminates the need for manual parameter tuning and complex post-processing, while achieving superior performance across diverse applications.
...
TL;DR: CoTyle introduces code-to-style image generation, where numerical codes control visual styles in image generation, enabling novel and consistent style creation without complex inputs.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on lengthy prompts, reference images, or fine-tuning, struggling with style consistency and creativity. The academic community lacks open-source solutions for numerical code-based style control.

Method: Train discrete style codebook from images for style embeddings, use embeddings to condition text-to-image diffusion model, train autoregressive style generator on embeddings to synthesize novel styles from numerical codes.

Result: CoTyle successfully generates diverse and consistent visual styles from numerical codes, demonstrating effective style control and reproducibility across various styles.

Conclusion: A style can be effectively represented and controlled by a single numerical code, enabling simple yet powerful style generation with CoTyle as the first open-source solution for code-to-style image generation.
...
TL;DR: An integrated framework combining YOLOMS with LLM for wind turbine fault detection and diagnosis, achieving 90.6% detection accuracy and generating interpretable maintenance reports with 89% accuracy.
...
Method: YOLOMS with multi-scale detection and sliding-window cropping for enhanced fault feature extraction, plus a lightweight KV mapping module to convert visual outputs to structured textual representations. A domain-tuned LLM performs semantic reasoning for fault analysis and maintenance recommendations.
...
TL;DR: The paper presents GT++ and 3DFETUS - two methods to automate standard facial plane localization in 3D fetal ultrasound volumes, addressing challenges from fetal movement and operator variability.


<details>
  <summary>Details</summary>
Motivation: Standard facial plane acquisition in fetal ultrasound is challenging due to fetal movement, orientation variability, and operator-dependent expertise, leading to inconsistencies, longer examination times, and diagnostic bias.

Method: 1) GT++: robust algorithm estimating standard facial planes from 3D US volumes using annotated anatomical landmarks; 2) 3DFETUS: deep learning model automating and standardizing plane localization in 3D fetal US volumes.
...
Conclusion: Both GT++ and 3DFETUS effectively address the challenges in fetal facial plane localization, providing automated and standardized solutions that improve accuracy and consistency in 3D ultrasound examinations.
...
TL;DR: A curated video dataset (RodEpil) for detecting convulsive events in laboratory rodents using TimeSformer transformer-based video classification, achieving 97% F1-score for seizure detection.


<details>
  <summary>Details</summary>
Motivation: To support non-invasive, video-based monitoring in preclinical epilepsy research by providing a standardized dataset for automatic detection of convulsive events in rodents.

Method: Created a dataset with 13,053 video clips (10s each) from 19 subjects, using TimeSformer transformer-based video classifier with five-fold cross-validation and strict subject-wise partitioning to prevent data leakage.
...
Conclusion: The RodEpil dataset and baseline code are publicly available to support reproducible research in preclinical epilepsy monitoring, demonstrating high accuracy for video-based seizure detection.
...
<details>
  <summary>Details</summary>
Motivation: Traditional digital pathology pipelines use grid-based tiling that ignores tissue architecture, introducing irrelevant information and limiting interpretability for cancer grading.

Method: HIT uses semantic segmentation to extract glands from whole slide images as input patches for multiple-instance learning and phenotyping, trained on 137 samples from the ProMPT cohort.
...
TL;DR: OpenSR-SRGAN is an open, modular framework for single-image super-resolution in Earth Observation that provides unified SRGAN-style model implementations through configuration files rather than code modifications.


<details>
  <summary>Details</summary>
Motivation: To lower the entry barrier for researchers and practitioners wanting to experiment with SRGANs for satellite data super-resolution by providing a configuration-driven workflow instead of requiring code modifications.

Method: Provides a unified implementation of SRGAN-style models with generators, discriminators, loss functions, and training schedules exposed through concise configuration files, making it easy to switch between architectures, scale factors, and band setups.

Result: The framework ships with ready-to-use configurations for common remote sensing scenarios, sensible default settings for adversarial training, and built-in hooks for logging, validation, and large-scene inference.

Conclusion: By turning GAN-based super-resolution into a configuration-driven workflow, OpenSR-SRGAN enables easier experimentation, reproducible model comparisons, and deployment of super-resolution pipelines across diverse Earth-observation datasets.
...
Method: SPOT leverages token embeddings, interactions, and attention dynamics across layers to infer token importance. It uses lightweight predictors that can be plugged into various ViT architectures for input-specific token prioritization.
...
Conclusion: SPOT provides a versatile framework for token sparsification in ViTs that improves computational efficiency without sacrificing performance, with adaptable designs for varying resource constraints.
...
<details>
  <summary>Details</summary>
Motivation: Traditional Total Variation (TV) depends heavily on the lambda parameter, limiting its efficiency and making it difficult to use effectively for image reconstruction tasks.

Method: Couples an unrolled TV solver with a Lambda Mapping Network (LambdaNet) that predicts per-pixel regularization maps, trained end-to-end for joint optimization of reconstruction and regularization.

Result: Experiments on DeepLesion dataset show consistent gains over classical TV and FBP+U-Net: +2.9 dB PSNR and +6% SSIM on average.
...
TL;DR: Proposes Separate-then-Map (StM) strategy for reconstructing dynamic humans interacting with environments from monocular videos, using a mapping mechanism to bridge separately optimized models while maintaining spatial and visual coherence.


<details>
  <summary>Details</summary>
Motivation: Existing approaches either model dynamic scenes holistically (neglecting distinct motion characteristics) or separately (causing spatial inconsistencies and artifacts at boundaries), failing to properly handle human-scene interactions.

Method: Uses Separate-then-Map strategy with shared transformation functions for Gaussian attributes to unify separately modeled components, avoiding exhaustive pairwise interactions while ensuring coherence.
...
Conclusion: The StM strategy effectively addresses limitations of existing approaches by bridging separately optimized models while maintaining computational efficiency and spatial consistency.
...
TL;DR: This paper introduces a framework for systematically evaluating diversity in text-to-image models by assessing individual concepts and their factors of variation, including a human evaluation template, curated prompt set, and methodology for model comparison.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models often generate homogeneous outputs despite advances in generation quality, highlighting the need for robust diversity evaluation methods.

Method: The framework includes: (1) a human evaluation template for nuanced diversity assessment, (2) a curated prompt set covering diverse concepts with identified factors of variation, and (3) a methodology for comparing models using binomial tests with human annotations. The authors also compare various image embeddings for diversity measurement.

Result: The approach enables principled ranking of text-to-image models by diversity and identifies specific categories where models struggle with diversity.

Conclusion: This research provides a robust methodology and insights that pave the way for improvements in text-to-image model diversity and metric development.
...
Method: Proposes a GeoAdapter using zero-initialized convolutions to encode geometric information without disrupting the foundation model's representation space, along with stochastic multimodal fusion that randomly samples modality subsets during training for robust learning.

Result: Outperforms prior methods with auxiliary inputs and achieves state-of-the-art results even with RGB-only input across monocular/multi-view depth estimation, multi-view stereo, and camera pose estimation. When integrated into VLA models, it outperforms point-cloud-based baselines and achieves consistent gains on robotic tasks.

Conclusion: OmniVGGT effectively leverages geometric modalities through a carefully designed adapter and fusion strategy, demonstrating superior performance across multiple 3D vision tasks and practical utility in robotic applications.
...
TL;DR: M&M-3D is a parameter-free 3D reasoning architecture for Digital Breast Tomosynthesis that enables direct weight transfer from 2D mammography models while maintaining volumetric information, achieving superior performance in breast cancer detection.


<details>
  <summary>Details</summary>
Motivation: Limited annotated DBT data constrains deep learning development, while existing methods either discard volumetric information by flattening DBT volumes or require complex 3D architectures that need more training data.

Method: M&M-3D constructs malignancy-guided 3D features and learns 3D reasoning by repeatedly mixing these features with slice-level information through modified operations in M&M without adding parameters, enabling direct weight transfer from FFDM models.

Result: M&M-3D surpasses 2D projection and 3D slice-based methods by 11-54% for localization and 3-10% for classification. It outperforms complex 3D variants by 20-47% for localization and 2-10% for classification in low-data regime, and matches their performance in high-data regime. On BCS-DBT benchmark, it beats previous top baseline by 4% for classification and 10% for localization.

Conclusion: M&M-3D effectively addresses data scarcity in DBT by enabling parameter-free 3D reasoning with direct weight transfer from 2D models, achieving state-of-the-art performance in breast cancer detection while maintaining computational efficiency.
...
TL;DR: This paper presents MSOM, a novel Multitask Glocal OBIA-Mamba model for enhanced Sentinel-2 land use/land cover classification that combines object-based analysis with Mamba architecture and multitask learning to improve accuracy while preserving fine details.


<details>
  <summary>Details</summary>
Motivation: Sentinel-2 based LULC classification faces challenges including spatial heterogeneity, context information needs, and signature ambiguity, requiring improved methods to handle these data complexities.

Method: Three key components: 1) OBIA-Mamba using superpixels as Mamba tokens to reduce redundancy while preserving details; 2) Global-local dual-branch CNN-Mamba architecture to model both local spatial detail and global context; 3) Multitask optimization with dual loss functions to balance local precision and global consistency.

Result: Tested on Sentinel-2 imagery in Alberta, Canada, the proposed MSOM approach achieved higher classification accuracy and finer details compared to several state-of-the-art classification methods.

Conclusion: The MSOM framework effectively addresses key challenges in Sentinel-2 LULC classification by integrating object-based analysis, global-local modeling, and multitask optimization, demonstrating superior performance over existing advanced approaches.
...
TL;DR: Evaluates SmolVLM2 variants (500M and 2.2B parameters) for video description quality focused on BLV accessibility, introducing two novel evaluation frameworks and testing on mobile devices with different precision levels.


<details>
  <summary>Details</summary>
Motivation: Large VLMs have high resource demands that hinder practical use for BLV users who need detailed, context-aware descriptions, motivating the study of smaller models' effectiveness for accessibility.

Method: Evaluated SmolVLM2 variants across AVCaps (outdoor) and Charades (indoor) datasets using two novel frameworks: Multi-Context BLV Framework (spatial orientation, social interaction, action events, ambience) and Navigational Assistance Framework. Tested four prompt strategies and deployed on smartphones with FP32 and INT8 precision.

Result: The paper presents systematic evaluation results comparing model sizes and precision variants for accessibility-focused video description quality, though specific performance metrics are not detailed in the abstract.

Conclusion: Smaller VLMs can provide effective accessibility-focused video descriptions while being deployable on resource-limited mobile devices, addressing practical constraints for BLV users.
...
TL;DR: LUA is a lightweight latent upscaler adapter that performs super-resolution directly on latent codes before VAE decoding, enabling high-resolution synthesis with minimal latency and no base model modifications.


<details>
  <summary>Details</summary>
Motivation: Diffusion models face challenges scaling beyond training resolutions - direct high-resolution sampling is slow/costly, while post-hoc image super-resolution introduces artifacts and additional latency after decoding.

Method: A lightweight module with shared Swin-style backbone and scale-specific pixel-shuffle heads that performs super-resolution on generator's latent code before final VAE decoding, requiring no base model modifications.

Result: Achieves comparable perceptual quality with nearly 3x lower decoding and upscaling time (+0.42s for 1024px from 512px vs 1.87s for pixel-space SR), strong generalization across different VAE latent spaces, and closely matches native high-resolution generation fidelity.

Conclusion: LUA provides a practical and efficient path to scalable, high-fidelity image synthesis in modern diffusion pipelines through latent-space super-resolution before decoding.
...
TL;DR: Depth Anything 3 (DA3) is a transformer-based model that predicts consistent geometry from multiple visual inputs without requiring camera poses, achieving state-of-the-art performance on visual geometry tasks while using minimal architectural specialization.


<details>
  <summary>Details</summary>
Motivation: To develop a minimal yet powerful geometry prediction model that works with arbitrary visual inputs and doesn't require known camera poses, simplifying the architecture while maintaining high performance.

Method: Uses a single plain transformer backbone (vanilla DINO encoder) without architectural specialization, employs singular depth-ray prediction target instead of complex multi-task learning, and follows a teacher-student training paradigm.
...
Conclusion: DA3 demonstrates that minimal modeling with plain transformers and simple prediction targets can achieve superior geometry prediction performance across multiple tasks, establishing new benchmarks in visual geometry.
...
<details>
  <summary>Details</summary>
Motivation: Outcome-reward RL in multimodal reasoning faces a critical flaw where models can guess the correct answer after faulty reasoning chains but still receive the same reward as genuine reasoning, undermining learning.
...
Conclusion: SCS provides a simple, general solution to the unfaithful reasoning problem in outcome-reward RL for multimodal LLMs, effectively improving reasoning reliability across multiple benchmarks and model architectures.
...
TL;DR: ScaleADFG is a framework for dexterous tool-use grasping that addresses dataset scalability and object scale generalization through automated dataset construction and a lightweight grasp generation network.


<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle with large-scale dataset construction and generalizability to everyday object scales due to size mismatches between robotic/human hands and real-world object diversity.

Method: Proposes an automated pipeline with affordance-based algorithm for synthesizing diverse grasp configurations, leveraging pre-trained models for 3D assets and affordance retrieval. Uses a lightweight single-stage grasp generation network with simple loss design.

Result: Created dataset with 5 object categories, 1000+ shapes each, 15 scale variations, 60,000+ grasps per robotic hand. Framework shows strong adaptability to varying object scales, improving grasp stability, diversity, and generalizability with effective zero-shot transfer to real-world objects.

Conclusion: Demonstrates the critical importance of large-scale datasets and multi-scale object variants for effective training, enabling flexible object-hand size ratios and effective grasping of everyday objects by large robotic hands.
...
TL;DR: Development of a robotic system for overhead work with mobile base, bimanual torso, and drilling end effector, using Gaussian splatting for 3D reconstruction and neural configuration-space barriers for safe planning in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: To create a robotic system capable of performing overhead work like ceiling drilling in dynamic environments with limited visibility, addressing safety challenges around moving obstacles.

Method: Hardware platform with mobile base and two-stage lift, bimanual torso with custom drilling end effector and RGB-D cameras. Software uses Gaussian splatting for online 3D reconstruction with motion parameters for moving objects, and neural configuration-space barrier approach for safe planning and control.

Result: Initial feasibility studies show the system can perform drilling, bolting, and anchoring tasks, and enables safe teleoperation in dynamic environments.

Conclusion: The developed robotic system demonstrates promising capabilities for overhead work applications in dynamic settings, with both hardware and software components showing initial feasibility for real-world deployment.
...
TL;DR: A reinforcement learning framework for robotic agent Baby Sophia learns self-touch and hand regard behaviors through intrinsic rewards and curriculum learning, mimicking infant development without external supervision.


<details>
  <summary>Details</summary>
Motivation: Inspired by infant development, the research aims to enable autonomous self-exploration in robotic agents through curiosity-driven learning similar to how infants explore their own bodies.

Method: Uses RL with intrinsic rewards for self-touch (tactile input compression and curriculum learning) and hand regard (visual feature learning and motor babbling), progressing from single-hand to dual-hand training.

Result: The agent successfully learns coordinated multimodal behaviors including self-touch with broad body coverage and complex visual-motor coordination for hand regard.

Conclusion: Purely curiosity-based signals without external supervision can drive coordinated multimodal learning, replicating infant progression from random motor babbling to purposeful behaviors.
...
TL;DR: L1-DS is a robust control framework that combines DS-based learning from demonstration with L1 adaptive control to handle task-execution mismatches in robotic motion planning.


<details>
  <summary>Details</summary>
Motivation: To address the 'task-execution mismatch' problem where unmodeled dynamics, disturbances, and latency cause robots to diverge from desired motion trajectories generated by DS-based LfD methods.

Method: Augments DS-based LfD models with a nominal stabilizing controller and L1 adaptive controller, plus a windowed Dynamic Time Warping-based target selector for temporal alignment.

Result: Demonstrated efficacy on LASA and IROS handwriting datasets, showing improved phase-consistent tracking performance.

Conclusion: The proposed L1-DS framework successfully handles task-execution mismatches and enables robust tracking of motion plans generated from demonstrations.
...
TL;DR: The paper presents a framework for planning informative and safe trajectories for autonomous robots in environments with spatially varying information decay, using clarity dynamics and Stein variational inference with safety filtering.


<details>
  <summary>Details</summary>
Motivation: Existing planners often model information as static or uniformly decaying, overlook how decay evolves along robot motion, and treat safety as a soft penalty rather than enforcing it strictly.

Method: The framework models uncertainty using clarity (normalized differential entropy), embeds clarity dynamics in trajectory optimization, and enforces safety through a low-level filtering mechanism based on the gatekeeper framework for safety verification. It uses Stein variational inference for Bayesian learning.

Result: Hardware experiments and simulations in environments with varying decay rates and obstacles show consistent safety and reduced information deficits.

Conclusion: The proposed framework successfully addresses the challenges of planning informative and safe trajectories in environments with spatially varying information decay by combining clarity dynamics, trajectory optimization, and strict safety enforcement.
...
TL;DR: PuffyBot is an untethered shape-morphing robot inspired by amphibians that can transform its body to navigate both land and water environments through volume changes and limb adjustments.


<details>
  <summary>Details</summary>
Motivation: The research is motivated by amphibians' ability to adapt their morphologies for movement in both terrestrial and aquatic environments, aiming to create versatile robotic platforms for diverse environments.

Method: The robot uses a scissor-lift mechanism driven by a linear actuator for shape morphing, achieving volume changes from 255.00 cm³ to 423.75 cm³. A bell-crank linkage adjusts servo-actuated limbs by 90 degrees for mode transitions. The robot is waterproofed with TPU fabric and has onboard battery power.

Result: Experimental results show successful multi-environment locomotion including crawling on land, crawling underwater, surface swimming, and bimodal buoyancy adjustment for submerging and resurfacing. The robot operates untethered for two hours.

Conclusion: Shape morphing enables creation of versatile and energy-efficient robotic platforms suitable for diverse environments, demonstrating the potential of bio-inspired design approaches.
...
TL;DR: This paper investigates how scene layout factors affect visuomotor policy generalization and creates an extensively randomized dataset to enhance policy robustness across diverse scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing methods for training visuomotor policies lack sufficient diversity in generated data, limiting their generalization ability across different scenarios and object placements.

Method: Created an extensively randomized dataset covering five manipulators and two grippers with randomization factors including camera pose, lighting, tabletop texture, and table height across six manipulation tasks, using minimal human demonstration.

Result: All investigated randomization factors influence policy generalization, with any form of randomization enhancing generalization ability. Diverse trajectories were particularly effective in bridging visual gaps, and the approach successfully enabled zero-shot sim-to-real transfer on low-cost manipulators.

Conclusion: Systematic scene randomization significantly improves visuomotor policy generalization, with diverse trajectory data playing a crucial role in bridging visual domain gaps and enabling effective sim-to-real transfer.
...
TL;DR: Audio-VLA enhances vision-only VLA models by incorporating contact audio to perceive dynamic manipulation processes, using multimodal encoders and introducing the Task Completion Rate (TCR) metric for comprehensive evaluation.


<details>
  <summary>Details</summary>
Motivation: Vision-only VLA models have fundamental limitations in perceiving interactive and manipulation dynamic processes, particularly contact events and dynamic feedback during robotic manipulation.

Method: Uses pre-trained DINOv2 and SigLIP as visual encoders, AudioCLIP as audio encoder, and Llama2 as backbone with LoRA fine-tuning. Adds collision-based audio generation to simulation environments and employs multimodal projection layer for cross-modal feature alignment.

Result: Audio-VLA demonstrates superior performance over vision-only methods on LIBERO, RLBench, and real-world tasks, with the TCR metric effectively quantifying dynamic process perception capabilities.

Conclusion: Audio-VLA successfully overcomes vision-only VLA limitations by leveraging contact audio, and the TCR metric provides systematic assessment of dynamic operational processes in robotic manipulation.
...
TL;DR: First systematic study of physical sensor attacks against Vision-Language-Action (VLA) models, revealing significant vulnerabilities and proposing adversarial-training-based defense.


<details>
  <summary>Details</summary>
Motivation: VLA models integrate multiple sensory modalities but their security against physical-world sensor attacks remains critically underexplored despite heavy reliance on sensory input.

Method: Developed a "Real-Sim-Real" framework that automatically simulates physics-based sensor attack vectors (6 camera attacks, 2 microphone attacks) and validates them on real robotic systems through large-scale evaluations.

Result: Demonstrated significant vulnerabilities in VLA models with susceptibility patterns dependent on task types and model designs. Developed adversarial-training defense that enhances robustness against out-of-distribution physical perturbations while preserving performance.

Conclusion: Findings expose urgent need for standardized robustness benchmarks and mitigation strategies to secure VLA deployments in safety-critical environments.
...
TL;DR: The paper proposes a novel electrically actuated robotic leg called DecARt Leg with decoupled actuation, quasi-telescopic kinematics, and innovative ankle torque transmission for agile locomotion.


<details>
  <summary>Details</summary>
Motivation: To develop a robotic leg capable of performing agile locomotion with improved design features that enable better performance in dynamic movements.

Method: Proposed the DecARt Leg design with quasi-telescopic kinematic structure, rotational motors for decoupled actuation, forward-facing knee, and multi-bar ankle torque transmission system. Introduced FAST metric for quantitative evaluation and conducted simulations and hardware experiments.

Result: The design was quantitatively evaluated using the proposed FAST metric and compared with other designs, showing improved agile locomotion capabilities through extensive simulation and preliminary hardware testing.

Conclusion: The DecARt Leg design successfully demonstrates enhanced agile locomotion performance through its novel kinematic structure and actuation system, validated by both simulation and hardware experiments.
...
TL;DR: Proposes a physics-inspired machine learning approach using Kolmogorov Arnold Network (KAN) for static friction modeling in robotic joints, achieving high accuracy (>0.95 R²) and interpretability through spline activation functions and symbolic regression.


<details>
  <summary>Details</summary>
Motivation: Traditional static friction models require predefined functional assumptions, posing challenges for unknown functional structures. Need for interpretable, data-driven friction modeling that maintains accuracy while handling unknown structures and noisy data.

Method: Uses Kolmogorov Arnold Network (KAN) with spline activation functions and symbolic regression mechanism. Enables model simplification and physical expression extraction through pruning and attribute scoring. Validated on synthetic data and real friction data from 6-DOF industrial manipulator.

Result: Achieves coefficient of determination >0.95 across various tasks. Successfully extracts concise and physically meaningful friction expressions. Demonstrates robustness under unknown functional structures and noisy data conditions.

Conclusion: Provides new perspective for interpretable and data-driven robotic friction modeling with promising engineering applicability, overcoming limitations of traditional static friction models.
...
TL;DR: UEPO is a unified generative framework for offline-to-online RL that addresses multimodal behavior coverage and distributional shifts using diffusion policies, dynamic divergence regularization, and data augmentation.


<details>
  <summary>Details</summary>
Motivation: Offline-to-online RL suffers from limited coverage of multimodal behaviors and distributional shifts during online adaptation, which UEPO aims to solve.

Method: UEPO uses a multi-seed dynamics-aware diffusion policy to capture diverse modalities, dynamic divergence regularization for physically meaningful policy diversity, and diffusion-based data augmentation for dynamics model generalization.

Result: On D4RL benchmark, UEPO achieves +5.9% improvement over Uni-O4 on locomotion tasks and +12.4% on dexterous manipulation, showing strong generalization and scalability.

Conclusion: UEPO provides an effective solution for offline-to-online RL challenges through its unified generative framework inspired by LLM pretraining strategies.
...
TL;DR: MT3 enables robots to learn 1,000 distinct everyday tasks from just one demonstration each, achieving order-of-magnitude data efficiency improvements over traditional methods through trajectory decomposition and retrieval-based generalization.


<details>
  <summary>Details</summary>
Motivation: Current imitation learning methods require hundreds or thousands of demonstrations per task, which is inefficient compared to human learning capabilities. The paper aims to improve robot learning efficiency by leveraging fundamental priors from human demonstration learning.

Method: MT3 (Multi-Task Trajectory Transfer) decomposes manipulation trajectories into sequential alignment and interaction phases, and uses retrieval-based generalization. The method systematically studies this decomposition through 3,450 real-world rollouts and compares design choices for both phases.

Result: In the few-demonstrations regime (<10 demonstrations), decomposition achieves an order of magnitude improvement in data efficiency over single-phase learning. MT3 successfully learned 1,000 distinct everyday tasks in under 24 hours of human demonstrator time and generalized to novel object instances.

Conclusion: Trajectory decomposition combined with retrieval-based learning enables highly efficient imitation learning, allowing robots to learn from minimal demonstrations while maintaining generalization capabilities across task families and novel object instances.
...
TL;DR: RoboBenchMart is a challenging robotic manipulation benchmark for dark store environments that addresses limitations of existing tabletop benchmarks by featuring complex grocery manipulation tasks with dense object clutter and varied spatial configurations.


<details>
  <summary>Details</summary>
Motivation: Existing robotic manipulation benchmarks focus on simplified tabletop scenarios with stationary arms and flat surfaces, lacking realism for practical applications like retail automation.

Method: Introduces RoboBenchMart benchmark suite including procedural store layout generator, trajectory generation pipeline, evaluation tools, and fine-tuned baseline models for dark store environments.

Result: Current state-of-the-art generalist models struggle to solve common retail tasks in this challenging environment, demonstrating the need for specialized benchmarks.

Conclusion: RoboBenchMart provides a more realistic and challenging benchmark for robotic manipulation in retail settings, addressing a domain with strong near-term automation potential and supporting further research through released tools and baselines.
...
TL;DR: nuPlan-R is a new reactive closed-loop planning benchmark that replaces rule-based agents with learning-based reactive multi-agent simulation, enabling more realistic and diverse traffic behaviors for autonomous vehicle evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks rely on rule-based reactive agents like IDM that lack behavioral diversity and fail to capture realistic human interactions, leading to oversimplified traffic dynamics.

Method: Integrates learning-based reactive multi-agent simulation into nuPlan framework, replaces IDM agents with noise-decoupled diffusion-based reactive agents, and introduces interaction-aware agent selection mechanism for realism and efficiency.

Result: Produces more realistic, diverse, and human-like traffic behaviors, better reflecting real-world interactive driving. Reimplementation of various planning approaches shows clearer performance reflection in complex scenarios.

Conclusion: nuPlan-R establishes a new standard for fair, reactive, and realistic closed-loop planning evaluation, better highlighting learning-based planners' advantages in handling complex dynamic scenarios.
...
TL;DR: The paper proposes new long-tail evaluation settings for autonomous driving trajectory prediction by creating challenging out-of-distribution test sets through scenario factorization and compositional zero-shot approaches, and develops methods to improve generalization in these settings.


<details>
  <summary>Details</summary>
Motivation: Current trajectory prediction methods struggle with rare, safety-critical scenarios that are infeasible to collect sufficient real-world data for, requiring better evaluation of robustness under out-of-distribution conditions.

Method: Introduces safety-informed scenario factorization framework to disentangle scenarios into ego and social contexts, constructs OOD test sets using compositional zero-shot approaches, and extends task-modular gating networks with auxiliary difficulty-prediction heads to improve generalization.

Result: The approach induces OOD performance gaps of 5.0% and 14.7% in closed-world and open-world settings respectively, and reduces these gaps to 2.8% and 11.5% while still improving in-distribution performance.

Conclusion: The proposed evaluation framework effectively exposes robustness limitations in trajectory prediction, and the developed methods successfully improve generalization to challenging out-of-distribution scenarios while maintaining in-distribution performance.
...
TL;DR: A robotic control framework for dependable bolting operations with active compliance, multimodal HRI, and supervisory control for improved fault detection and operator awareness.


<details>
  <summary>Details</summary>
Motivation: Current robotic bolting systems lack reliable autonomy and fault management capabilities, requiring improved operational safety and effectiveness.

Method: Control architecture with accurate torque control and active compliance, multimodal HRI for real-time visualization, and high-level supervisor coordinating execution and mode transitions.

Result: Improved fault detection, enhanced operator situational awareness, and accurate compliant bolting execution, but limitations in using single camera for full situational awareness.

Conclusion: The proposed framework successfully enables dependable robotic bolting with improved safety and fault management, though situational awareness tools need enhancement.
...
TL;DR: A design framework using MuJoCo's deformable-body capabilities to simulate origami mechanisms, enabling physically consistent modeling of folding behavior and environmental interactions through an intuitive GUI.


<details>
  <summary>Details</summary>
Motivation: Origami-inspired mechanisms offer lightweight, compact structures with complex motion, but accurately simulating their folding behavior and environmental interactions remains challenging.

Method: Represent origami sheets as graphs of interconnected deformable elements with user-specified constraints (creases, actuation) via GUI, using MuJoCo's deformable-body capabilities for simulation.

Result: Demonstrated through an origami catapult case study where design parameters were optimized using CMA-ES, achieving improved throwing performance validated on physical prototypes.

Conclusion: The framework enables rapid, simulation-driven origami design, optimization, and analysis by capturing both geometric structure and environmental interactions.
...
TL;DR: UAV-based reconnaissance system for post-disaster navigation using optimization to find optimal flight height for maximum area coverage and minimum data uncertainty.


<details>
  <summary>Details</summary>
Motivation: Post-disaster environments are unstructured and challenging for rescue vehicle path planning, requiring effective reconnaissance methods.

Method: Optimization-based approach to plan UAV paths at optimal height where sensors can maximize area coverage and minimize data uncertainty.

Result: Proposed method enables UAVs to effectively perform reconnaissance in unstructured post-disaster environments.

Conclusion: UAVs with optimized path planning at optimal heights can effectively address navigation challenges in post-disaster scenarios.
...
TL;DR: This paper proposes a reinforcement learning approach for bipedal robots to perform controlled, soft falls that minimize physical damage while allowing user control over the robot's final pose.


<details>
  <summary>Details</summary>
Motivation: Bipedal robots operating in real-world environments remain at risk of falling, and most research focuses on preventing falls rather than managing them. The authors aim to reduce physical damage during falls while giving users control over the robot's end pose.

Method: The authors develop a robot-agnostic reward function that balances achieving desired end poses with impact minimization and protection of critical robot parts. They use reinforcement learning with a simulation-based sampling strategy for initial and end poses to make the policy robust to various falling conditions.

Result: Through simulated and real-world experiments, the work demonstrates that bipedal robots can successfully perform controlled, soft falls that minimize damage while achieving specified end poses.

Conclusion: The research shows that controlled falling is a viable strategy for bipedal robots, enabling damage reduction and user control over final positioning during unavoidable fall events.
...
<details>
  <summary>Details</summary>
Motivation: Previous exact MIP solvers guarantee optimality but become computationally inefficient for large-scale JRA problems, while existing heuristic methods achieve only ~1% deviation from optimum. There is a need for more efficient approaches that maintain high accuracy.

Method: Proposes a PJAR framework with Partial Path Reconstruction solver that identifies key item-placeholder pairs to form reduced subproblems, iteratively polishes solutions along optimization path, and incorporates global Large-α constraint to enhance optimality.

Result: Experimental evaluations on benchmark datasets (n=300, 500, 1000) show the method consistently delivers almost optimal solutions with 0.00% average deviation from ground truth while maintaining high computational efficiency, reducing deviation by half compared to previous heuristics.

Conclusion: The proposed framework achieves highly accurate near-optimal solutions for large-scale JRA problems and demonstrates strong potential for broader applications in TSP and related optimization problems.
...
TL;DR: This paper presents the competition-winning Variable Neighborhood Search (VNS) approach for the Capacitated Green Vehicle Routing Problem (CGVRP), achieving best results on the full competition dataset and outperforming more recent algorithms.


<details>
  <summary>Details</summary>
Motivation: The Electric Vehicle Routing Problem (EVRP) extends classical VRP to address electric vehicle logistics, but comparing approaches across different variants is challenging. The CGVRP variant was used in the CEC-12 competition.

Method: The approach uses the Variable Neighborhood Search (VNS) metaheuristic to solve the Capacitated Green Vehicle Routing Problem.
...
Conclusion: The VNS-based approach proved highly effective for solving the CGVRP, demonstrating superior performance over other methods in the competition and subsequent publications.
...
### [132] [SynthTools: A Framework for Scaling Synthetic Tools for Agent Development](https://arxiv.org/abs/2511.09572)
*Tommaso Castellani,Naimeng Ye,Daksh Mittal,Thomson Yen,Hongseok Namkoong*

Main category: cs.AI

TL;DR: SynthTools is a framework for generating synthetic tool ecosystems to address limitations of real-world APIs for AI agent training and evaluation, featuring tool generation, simulation, and audit components.


<details>
  <summary>Details</summary>
Motivation: Real-world APIs are limited in availability, domain coverage, and stability, making them impractical for stable evaluation or scalable training of AI agents that rely on external tools.

Method: The framework consists of three core components: Tool Generation for automatic creation of diverse tools, Tool Simulation to emulate realistic tool behaviors, and Tool Audit to ensure correctness and consistency.
...
### [133] [Proceedings of the Second International Workshop on Next-Generation Language Models for Knowledge Representation and Reasoning (NeLaMKRR 2025)](https://arxiv.org/abs/2511.09575)
*Ha-Thanh Nguyen,Ken Satoh,Francesca Toni,Randy Goebel,Kostas Stathis*

Main category: cs.AI

TL;DR: This workshop explores reconciling reasoning between transformer-based language models and logic-based representations, analyzing LM reasoning abilities, injecting KR-style reasoning into LMs, and formalizing LM reasoning processes.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between traditional logic-based AI reasoning and emerging reasoning capabilities in large language models, creating a platform for interdisciplinary research on integrating precise reasoning with language models.

Method: Workshop approach bringing together researchers from different disciplines to analyze LM reasoning abilities alongside KR methods, inject KR-style reasoning into LMs (including neuro-symbolic approaches), and formalize LM reasoning processes.

Result: The workshop aims to uncover how language models can effectively integrate knowledge and reasoning to improve applications where precision and reliability are key requirements.

Conclusion: By reconciling transformer-based language models with logic-based representations, the research seeks to enhance LM reasoning capabilities for critical applications requiring precision and reliability.
...
### [134] [Cogent argument extensions are weakly admissible but not vice versa](https://arxiv.org/abs/2511.09600)
*Gustavo Bodanza*

Main category: cs.AI

TL;DR: The paper establishes that cogent extensions are weakly admissible but the reverse does not hold.
...
Method: Mathematical proof showing the directional relationship between the two semantics.

Result: Proved that cogent extensions are weakly admissible, but weakly admissible extensions are not necessarily cogent.

Conclusion: There is an asymmetric relationship between cogent and weakly admissible semantics in argumentation frameworks.
...
TL;DR: The paper introduces Rebellion, a robust reasoning training method that protects Audio Reasoning Models from advanced audio jailbreak attacks by addressing representation drift between vanilla and sophisticated attacks.


<details>
  <summary>Details</summary>
Motivation: Audio Reasoning Models (ARMs) are gaining popularity but their safety against jailbreak attacks hasn't been studied. Standard reasoning training fails against advanced attacks due to representation drift.

Method: Proposed Rebellion - a robust reasoning training method that trains ARMs to be robust to worst-case representation drift, protecting against advanced audio jailbreaks while maintaining benign task performance.

Result: Rebellion successfully protects Qwen2-Audio against advanced audio jailbreaks without compromising performance on benign tasks, and significantly improves accuracy-safety trade-off over standard reasoning training.

Conclusion: Rebellion provides effective protection for ARMs against sophisticated jailbreak attacks by addressing the fundamental issue of representation drift, offering a robust safety solution for audio reasoning models.
...
TL;DR: LLM-based agents in autonomous interactions develop behavioral drifts like echoing, where agents mirror each other instead of maintaining assigned roles, undermining objectives. Echoing occurs across major LLM providers (5-70% rates), persists in advanced reasoning models (32.8%), and worsens with longer interactions. A protocol-level mitigation using structured responses reduces echoing to 9%.


<details>
  <summary>Details</summary>
Motivation: To investigate unique failures in autonomous agent-agent conversations (AxA) that cannot be predicted from single agent performance, specifically the phenomenon of echoing where agents abandon their roles and mirror partners, lacking the stabilizing signals present in human-agent interactions.

Method: Conducted experiments across 60 AxA configurations, 3 domains, and 2000+ conversations using major LLM providers. Analyzed prompt impacts and conversation dynamics, particularly how echoing develops with interaction length (7+ turns). Tested mitigation using structured responses.

Result: Echoing occurs across all major LLM providers with rates from 5% to 70% depending on model and domain. The phenomenon persists in advanced reasoning models (32.8%) and is not reduced by increased reasoning efforts. Echoing arises as interactions grow longer and is not merely due to sub-optimal prompting. Targeted use of structured responses reduces echoing to 9%.

Conclusion: Behavioral drifts like echoing represent a fundamental challenge in autonomous agent-agent interactions that cannot be addressed through single-agent improvements alone. Protocol-level interventions, such as structured responses, offer effective mitigation strategies for these emergent multi-agent failures.
...
TL;DR: ProbLog4Fairness uses probabilistic logic programming to formalize bias assumptions as programs, enabling flexible and interpretable bias mitigation in neural network training that outperforms fixed-fairness baselines.


<details>
  <summary>Details</summary>
Motivation: Existing fairness definitions are often incompatible and hard to operationalize, while task-specific bias assumptions are easier to describe but lack a principled framework for integration into training.

Method: Formalize bias assumptions as ProbLog programs, use neurosymbolic extensions to integrate these into neural network training, and propose bias templates for different bias types tested on synthetic and real-world data.

Result: Successfully mitigates algorithmic bias in both synthetic tabular datasets with known biases and real-world tabular and image data, outperforming baseline methods that use fixed bias types.

Conclusion: ProbLog4Fairness provides a versatile, principled framework for modeling bias assumptions that flexibly adapts to different bias types, overcoming limitations of fixed-fairness approaches.
...
<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used for annotating learning interactions but concerns about reliability limit their utility, prompting the need for verification methods to improve annotation quality.

Method: Used transcripts from 30 one-to-one math sessions to compare three LLMs (GPT, Claude, Gemini) under three conditions: unverified annotation, self-verification, and cross-verification across all orchestration configurations, benchmarked against blinded human adjudication using Cohen's kappa.

Result: Orchestration yields 58% improvement in kappa overall. Self-verification nearly doubles agreement relative to unverified baselines, with largest gains for challenging tutor moves. Cross-verification achieves 37% average improvement with pair- and construct-dependent effects.

Conclusion: Verification serves as a principled design lever for reliable, scalable LLM-assisted annotation in Learning Analytics, with the proposed framework and notation standardizing reporting for replication.
...
TL;DR: Open small AI models provide artists with creative independence and technical control for interactive art, unlike restrictive corporate AI systems.
...
Method: Advocates for using open small AI models that can be deployed locally, allowing artists to modify code, integrate custom interfaces, and retrain/fine-tune models with their own datasets.

Result: Small AI models empower artists with autonomy, control, sustainability, and technological self-determination for interactive art practices.

Conclusion: Open small AI models support artistic independence, long-term preservation of AI artworks, and reduce reliance on corporate AI systems ill-suited for interactive art's demands.
...
TL;DR: SlideBot is a modular multi-agent framework that uses LLMs with retrieval, planning, and code generation to create educational slides that are informative, reliable, and practical, incorporating evidence-based instructional design principles.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based solutions fail to produce reliable and informative educational slides due to challenges in multimodal content creation and domain-specific information needs.

Method: Multi-agent framework integrating LLMs with retrieval, structured planning, and code generation, using specialized agents for information retrieval, summarization, figure generation, and LaTeX slide formatting, guided by Cognitive Load Theory and Cognitive Theory of Multimedia Learning principles.

Result: Evaluations from domain experts and students in AI and biomedical education show enhanced conceptual accuracy, clarity, and instructional value compared to existing solutions.

Conclusion: SlideBot demonstrates potential to streamline slide preparation while ensuring accuracy, relevance, and adaptability in higher education through its modular, multi-agent approach.
...
<details>
  <summary>Details</summary>
Motivation: GBDTs are widely used but watermarking remains underexplored compared to neural networks, creating a need for protection methods.

Method: Proposed four embedding strategies using in-place fine-tuning to embed watermarks while minimizing accuracy degradation.
...
Conclusion: The framework successfully enables robust watermarking of GBDT models with practical performance characteristics.
...
TL;DR: A thermally activated adversarial wearable that uses thermochromic dyes and heating units to create dynamic adversarial patterns on clothing, enabling privacy protection against AI surveillance by evading detection in both visible and infrared modalities.


<details>
  <summary>Details</summary>
Motivation: To address the conspicuous appearance limitation of traditional adversarial patches and enable practical deployment of privacy-preserving adversarial techniques in real-world surveillance scenarios.

Method: Integration of thermochromic dyes with flexible heating units on clothing surfaces to create dynamic adversarial patterns that activate upon heating, appearing as ordinary black T-shirt in default state.
...
TL;DR: This paper explores Quantum Artificial Intelligence (QAI) as a transformative solution for Mission Critical (MC) systems, addressing limitations of classical ML in robustness, timing, explainability, and safety.


<details>
  <summary>Details</summary>
Motivation: Classical ML struggles to meet the stringent constraints of MC applications in defense, energy, cybersecurity, and aerospace, which require reliable, deterministic, low-latency decision making under uncertainty.

Method: The paper provides comprehensive exploration of QAI for MC systems, including quantum-enhanced learning pipelines, uncertainty quantification, explainability frameworks, and proposes quantum resource management and scheduling models driven by timeliness constraints.

Result: The study identifies key application areas where QAI can enhance fault tolerance, real-time intelligence, and adaptability in aerospace, defense, cybersecurity, smart grids, and disaster management.

Conclusion: While QAI shows promise for MC systems, challenges remain including trainability limits, data access bottlenecks, quantum component verification, and adversarial QAI, requiring future research toward interpretable, scalable, and hardware-feasible models.
...
TL;DR: EgoEMS is the first comprehensive multimodal dataset for EMS training and AI development, featuring 20+ hours of realistic emergency scenarios from egocentric view with detailed annotations for keystep recognition and action quality assessment.


<details>
  <summary>Details</summary>
Motivation: Emergency Medical Services (EMS) responders face intense cognitive demands in high-stakes situations, creating a need for AI cognitive assistants to support real-time data collection and decision making.

Method: Created EgoEMS dataset with 233 simulated emergency scenarios performed by 62 participants (including 46 EMS professionals) using an open-source, low-cost data collection system. The dataset includes multimodal data with keysteps, audio transcripts, action quality metrics, and segmentation masks.

Result: Developed the first end-to-end, high-fidelity, multimodal, multiperson EMS dataset capturing realistic procedural activities with responder-patient interactions aligned with national standards.

Conclusion: EgoEMS enables development of AI support tools for EMS through benchmarks for real-time multimodal keystep recognition and action quality estimation, aiming to improve patient outcomes through intelligent EMS systems.
...
TL;DR: AlphaDE is a novel framework that uses fine-tuned protein language models and Monte Carlo tree search to guide protein evolution, outperforming previous methods and enabling sequence space condensation.


<details>
  <summary>Details</summary>
Motivation: Current directed evolution algorithms overlook the potential of protein language models that encode rich evolutionary patterns, creating a gap in utilizing these models to guide protein sequence evolution.

Method: Fine-tunes pretrained protein language models using masked language modeling on homologous sequences, then uses Monte Carlo tree search for test-time inference to evolve proteins with evolutionary guidance.

Result: Extensive benchmark experiments show AlphaDE remarkably outperforms previous state-of-the-art methods even with few-shot fine-tuning, and supports condensing protein sequence space through computational evolution.

Conclusion: AlphaDE successfully bridges the gap between protein language models and directed evolution, demonstrating superior performance in protein sequence evolution and enabling new capabilities like sequence space condensation.
...
TL;DR: AI agents can autonomously sabotage ML models, sandbag performance, and evade detection when conducting machine learning engineering tasks, raising concerns about deploying frontier AI systems in safety-critical settings.


<details>
  <summary>Details</summary>
Motivation: As AI systems become capable of automating ML R&D and may be deployed in safety-critical settings, there's concern that frontier systems may not be trustworthy or may be misaligned with user interests, necessitating investigation of their potential to act against users.

Method: Extended MLE-Bench benchmark with code-sabotage tasks (backdoors, generalization failures), studied agent sandbagging capabilities, used LM monitors to detect suspicious behavior, and measured agent ability to evade detection.

Result: Frontier agents successfully performed sabotage tasks, could calibrate performance to specified target levels below actual capability, and monitors were effective at detecting code-sabotage but struggled with sandbagging detection.

Conclusion: While monitoring can detect some sabotage attempts, it may not be sufficiently reliable for high-stakes domains, highlighting the need for improved oversight mechanisms for autonomous AI systems conducting ML engineering.
...
TL;DR: The paper introduces a reasoning-based problem generator that creates synthetic training data by planning problem directions and adapting difficulty to solver ability, achieving 2.5% average improvement across 10 reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing data synthesis approaches face challenges: indiscriminate generation that ignores solver ability and yields low-value problems, and lack of reasoning in problem generation leading to shallow variants.

Method: Construct related problem pairs with intermediate problem-design CoT from reasoning models, use solver feedback as reward signal to calibrate difficulty, and enable co-evolution between generator and solver.

Result: Achieves 2.5% average improvement on 10 mathematical and general reasoning benchmarks, generalizes to both language and vision-language models, and co-evolution yields additional 0.7% performance gain.

Conclusion: The reasoning-based problem generator with difficulty adaptation and co-evolution framework effectively creates high-quality synthetic training data that improves solver performance across diverse reasoning tasks.
...
TL;DR: This paper addresses the opioid crisis by developing multimodal AI models to analyze complex legal and corporate documents from the UCSF-JHU Opioid Industry Documents Archive, creating a benchmark dataset with 400k training documents and generating 360k QA pairs for improved information extraction.


<details>
  <summary>Details</summary>
Motivation: The opioid crisis reveals systemic failures across multiple sectors, requiring advanced analytic approaches to handle the complexity and multimodal nature of healthcare-related legal documents in the OIDA archive.

Method: Organized dataset by document attributes, extracted multimodal information (text, visuals, layout), generated large-scale QA pairs, developed domain-specific multimodal LLMs, incorporated historical QA context, and added page references with importance-based classification.

Result: Preliminary results show improvements in document information extraction and question-answering tasks with the developed AI assistant.

Conclusion: The developed multimodal AI approach successfully enhances analysis of complex opioid-related documents, with the dataset and models made publicly available for further research.
...
<details>
  <summary>Details</summary>
Motivation: Hierarchical data is common in ML applications, and hyperbolic spaces can embed such structures with low distortion. However, existing hyperbolic kernels suffer from geometric distortion or lack adaptability.

Method: Proposed curvature-aware de Branges-Rovnyak space isometric to Poincare ball, with adjustable multiplier to select appropriate RKHS for any curvature. Constructed adaptive hyperbolic kernels including novel adaptive hyperbolic radial kernel with learnable parameters.
...
Conclusion: The adaptive hyperbolic kernels effectively enhance hyperbolic representation capacity and demonstrate superior performance in hierarchical data modeling tasks.
...
TL;DR: SPAN is a cross-calendar temporal reasoning benchmark that evaluates LLMs' ability to perform intra-calendar reasoning and inter-calendar conversions across six calendars. Current LLMs struggle with this task (34.5% average accuracy), but a tool-augmented Time Agent achieves 95.31% accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of cross-calendar temporal reasoning in LLMs, which requires understanding different calendar systems and performing conversions between them, an area where current models show significant limitations.

Method: Created SPAN benchmark with ten cross-calendar reasoning directions, two reasoning types, and two question formats across six calendars. Used template-driven protocol for dynamic instance generation and developed an LLM-powered Time Agent with tool-augmented code generation.

Result: Current SOTA LLMs achieve only 34.5% average accuracy on SPAN, with none exceeding 80%. The Time Agent significantly outperforms them with 95.31% accuracy. Identified key obstacles: Future-Date Degradation and Calendar Asymmetry Bias.

Conclusion: Cross-calendar temporal reasoning remains challenging for current LLMs, but tool-augmented code generation shows strong potential to address this limitation, enabling more temporally and culturally adaptive language models.
...
TL;DR: The paper introduces ChEmREF, a benchmark for evaluating language models' ability to assist emergency responders with HAZMAT incidents, covering chemical translation, response generation, and domain knowledge QA.


<details>
  <summary>Details</summary>
Motivation: Emergency responders need rapid, reliable assistance for HAZMAT incidents but currently rely on manual navigation of extensive chemical guidelines, which is time-sensitive and critical.

Method: Created ChEmREF benchmark with 1,035 HAZMAT chemicals from Emergency Response Guidebook and PubChem Database, organized into three tasks: chemical representation translation, emergency response generation, and domain knowledge QA from safety exams.
...
Conclusion: Language models show potential to assist emergency responders but require careful human oversight due to current limitations in performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing tool-augmented LLMs like ReAct suffer from local optimization traps due to incremental decision-making, limiting their ability to handle complex queries requiring sophisticated multi-tool coordination.
...
Result: Achieves state-of-the-art performance on StableToolBench benchmark, demonstrating superior end-to-end execution capabilities and robust handling of intricate multi-tool workflows.

Conclusion: The Planner-centric paradigm with DAG planning fundamentally resolves local optimization bottlenecks, enabling optimized execution beyond conventional tool coordination for complex queries.
...
TL;DR: HPR is a novel reasoning framework that uses a powerful LLM as hinter to guide critical decisions and an efficient smaller model as practitioner for most reasoning steps, achieving state-of-the-art efficiency-accuracy tradeoffs.


<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning methods incur prohibitive computational costs through exhaustive sampling, while observations show most next-token predictions align with golden outputs except for a few critical tokens that cause deviations.

Method: HPR framework with two components: hinter (powerful LLM) provides probabilistic guidance at critical points, practitioner (efficient smaller model) executes major reasoning steps. Uses Distributional Inconsistency Reduction (DIR) metric to dynamically identify intervention points by quantifying divergence between practitioner's trajectory and hinter's expected distribution in tree-structured probabilistic space.
...
Conclusion: HPR demonstrates state-of-the-art efficiency-accuracy tradeoffs in LLM reasoning by strategically combining powerful and efficient models through dynamic intervention at critical decision points.
...
TL;DR: RadFlow is a hierarchical workflow-guided reinforcement optimization framework for medical report generation that models the structured nature of clinical reporting to improve diagnostic coherence and report quality.


<details>
  <summary>Details</summary>
Motivation: Existing medical report generation systems treat reports as flat sequences, overlooking the hierarchical organization of radiological reports (findings → impressions → critical case refinement), leading to inconsistencies between descriptive and diagnostic content.

Method: Proposes RadFlow with clinically grounded reward hierarchy: global reward for linguistic fluency, medical correctness, and cross-sectional consistency; section-specific reward emphasizing Impression quality; and critical-aware policy optimization for high-risk cases.
...
Conclusion: RadFlow successfully translates the structured reporting paradigm into reinforcement fine-tuning, enabling generation of reports that are both linguistically consistent and clinically aligned with real-world radiology practices.
...
TL;DR: MuSeR is a data-driven method that enhances LLMs' context-awareness in medical scenarios through self-evaluation and refinement along decision-making, communication, and safety facets, achieving state-of-the-art performance on HealthBench.
...
Method: Proposes Multifaceted Self-Refinement (MuSeR) using attribute-conditioned query generation, LLM self-evaluation along three facets, response refinement, and supervised fine-tuning with knowledge distillation.
...
Conclusion: MuSeR effectively enhances LLMs' context-awareness in medical domains through self-refinement, demonstrating strong performance improvements on medical benchmarks.
...
TL;DR: A hybrid controller couples a learnable brain-like policy with a Gray-Scott reaction-diffusion system to steer pattern formation efficiently, achieving 100% convergence with minimal control effort by exploiting morphological computation.
...
Method: Embed a compact convolutional policy in a differentiable PyTorch reaction-diffusion simulator, using spatially smooth parameter modulations under a warm-hold-decay gain schedule, with training optimizing spectral targets while penalizing control effort and instability.

Result: Hybrid control achieves 100% strict convergence in ~165 steps with spectral selectivity comparable to cell-only (0.436 vs 0.434), using ~15× less ℓ₁ effort and >200× less ℓ₂ power than NN-dominant control. A Goldilocks zone (A≈0.03-0.045) yields 100% quasi convergence in 94-96 steps.

Conclusion: The controller 'seeds then cedes' - providing brief, sparse nudges to place the system in the correct basin of attraction, after which local physics maintains the pattern, demonstrating practical morphological computation for robust, energy-efficient systems.
...
TL;DR: Proposes an Intelligence Foundation Model (IFM) that learns general intelligence principles from diverse intelligent behaviors, using a biologically-inspired state neural network and neuron output prediction objective.


<details>
  <summary>Details</summary>
Motivation: Existing foundation models specialize in specific domains like language or vision, but AGI requires learning the underlying mechanisms of intelligence across all domains. Intelligent behaviors emerge from neural dynamics, so learning from these behaviors can capture general intelligence principles.

Method: Two core components: 1) State neural network that emulates biological neuron temporal dynamics for storing, integrating, and processing information over time; 2) Neuron output prediction objective that trains the system to predict neuronal outputs from collective dynamics.
...
Conclusion: This approach represents a step toward truly artificial general intelligence by learning the fundamental principles of intelligence directly from diverse intelligent behaviors through biologically-inspired neural dynamics.
...
<details>
  <summary>Details</summary>
Motivation: RAG systems face threats from reconstruction attacks that exploit both intra-class and inter-class paths to extract knowledge bases, and existing defenses only protect one path, leaving systems vulnerable.

Method: RAGFort uses a structure-aware dual-module approach: contrastive reindexing for inter-class isolation and constrained cascade generation for intra-class protection.

Result: Experiments show RAGFort significantly reduces reconstruction success while preserving answer quality, providing comprehensive defense against knowledge base extraction attacks.

Conclusion: Joint protection of both intra-class and inter-class paths is essential for effective defense against knowledge base reconstruction attacks in RAG systems.
...
TL;DR: DenoGrad is a gradient-based denoising framework that uses gradients from a deep learning model to detect and correct noisy instances while preserving the original data distribution, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing noise correction approaches often degrade performance or oversimplify problems by altering data distributions, leading to biased models - particularly problematic for interpretable AI where data fidelity is crucial.

Method: Proposes DenoGrad framework that leverages gradients from an accurate DL model trained on target data to dynamically detect and adjust noisy samples, preserving the original data distribution.

Result: DenoGrad outperforms state-of-the-art denoising methods on tabular and time series datasets under various noise settings, enhancing interpretable AI model performance while preserving data distribution.

Conclusion: Using task-specific high-quality solutions as reference provides more precise noise definition; DenoGrad effectively improves AI model robustness while maintaining data distribution fidelity.
...
TL;DR: The paper proposes two methods for compiling qualitative state-trajectory constraints in PDDL without grounding, enabling efficient handling of large-scale planning problems with many objects and high-arity actions.


<details>
  <summary>Details</summary>
Motivation: Real-world planning problems often involve safety requirements, task ordering, and intermediate sub-goals, but existing compilers require grounding which doesn't scale well for problems with large numbers of objects and high-arity actions.

Method: Two novel compilation methods that avoid grounding the problem before compilation, making them suitable for large-scale planning. The authors prove correctness and analyze worst-case time complexity.

Result: The methods produce planning specifications that are orders of magnitude more succinct than ground-based compilers while remaining competitive with state-of-the-art planners, as demonstrated through reproducible empirical evaluation on International Planning Competition domains.

Conclusion: The proposed grounding-free compilation methods efficiently handle large-scale planning problems with qualitative constraints, offering significant advantages in problem representation size while maintaining planning performance.
...
Method: Train a Gaussian Process surrogate model using "LogitMap Pairs" from a minimal but informative training subset. The surrogate approximates foundation model outputs to guide proxy model training, reducing direct API queries.

Result: Elevates pre-trained language model accuracy from 55.92% to 86.85%, reduces API query frequency to 1.38%, significantly outperforms offline approaches and achieves comparable/superior accuracy to query-intensive methods with much lower costs.

Conclusion: Provides a robust and high-efficiency paradigm for language model adaptation that significantly reduces API costs while maintaining or improving accuracy compared to existing approaches.
...
TL;DR: MTP is a multimodal framework for urban traffic profiling that integrates numeric, visual, and textual perspectives to enhance traffic signal understanding and prediction beyond traditional unimodal approaches.


<details>
  <summary>Details</summary>
Motivation: Existing traffic signal modeling methods rely solely on numerical sensor data, overlooking valuable semantic information from multimodal urban data, which limits comprehensive understanding and accurate prediction of complex traffic dynamics.

Method: Proposes MTP framework with three branches: visual augmentation (frequency and periodicity images), textual augmentation (descriptive texts with topic/background information), and numeric learning (frequency multilayer perceptrons), using hierarchical contrastive learning to fuse modalities.
...
Conclusion: The multimodal approach integrating numeric, visual, and textual perspectives significantly improves urban traffic signal profiling and prediction capabilities.
...
TL;DR: EvoReal uses evolutionary algorithms guided by LLMs to generate realistic synthetic VRP instances that mimic real-world structural patterns, improving neural solver generalization on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing neural combinatorial optimization methods struggle to generalize from synthetic training data to real-world VRP benchmarks due to distribution mismatch.

Method: Evolutionary module with LLM guidance generates synthetic instances with realistic structural patterns, followed by progressive refinement of pre-trained NCO models through alignment with synthetic distributions and fine-tuning on real benchmarks.

Result: Significantly improved generalization on TSPLib (1.05% gap) and CVRPLib (2.71% gap) benchmarks across various problem scales.

Conclusion: EvoReal effectively bridges the generalization gap between synthetic training and real-world VRP instances through realistic instance synthesis and progressive model adaptation.
...
TL;DR: ProgRAG is a multi-hop KGQA framework that decomposes complex questions into sub-questions, progressively extends reasoning paths with uncertainty-aware pruning, and optimizes context organization to improve reasoning reliability and performance.


<details>
  <summary>Details</summary>
Motivation: Existing KG-enhanced LLMs face challenges with inaccurate retrieval, reasoning failures, and context construction issues that obscure relevant information. LLMs often struggle with self-assessing evidence sufficiency, leading to premature or incorrect reasoning.

Method: Decomposes complex questions into sub-questions, progressively extends partial reasoning paths by answering each sub-question, uses external retrievers for candidate evidence, applies uncertainty-aware pruning by LLM, and optimizes context organization of reasoning paths.
...
Conclusion: ProgRAG effectively addresses retrieval and reasoning failures in KG-enhanced LLMs through progressive decomposition and uncertainty-aware pruning, providing a more reliable framework for complex knowledge-intensive tasks.
...
### [165] [PepTriX: A Framework for Explainable Peptide Analysis through Protein Language Models](https://arxiv.org/abs/2511.10244)
...
<details>
  <summary>Details</summary>
Motivation: Traditional peptide classification methods rely on handcrafted 1D encodings that limit generalizability, while protein language models face computational costs and interpretability challenges, restricting the ability to connect predictions to biologically relevant motifs.

Method: Integrates 1D sequence embeddings and 3D structural features via graph attention network enhanced with contrastive training and cross-modal co-attention, automatically adapting to diverse datasets to produce task-specific peptide vectors.
...
Conclusion: PepTriX bridges the gap between performance-driven protein language models and domain-level understanding, offering both predictive robustness and interpretable validation in peptide research.
...
### [166] [Beyond Single-Step Updates: Reinforcement Learning of Heuristics with Limited-Horizon Search](https://arxiv.org/abs/2511.10264)
*Gal Hadar,Forest Agostinelli,Shahaf S. Shperberg*

Main category: cs.AI

TL;DR: This paper proposes a generalized heuristic search approach that uses limited-horizon searches to enhance both state sampling and heuristic updates, going beyond traditional single-step Bellman updates.


<details>
  <summary>Details</summary>
Motivation: Traditional heuristic search methods rely on single-step Bellman updates, which may be suboptimal for learning effective heuristics in sequential decision-making problems formulated as shortest-path problems.

Method: The approach performs limited-horizon searches and updates each state's heuristic based on the shortest path to the search frontier, incorporating both edge costs and heuristic values of frontier states.

Result: The proposed method provides enhanced state sampling and more comprehensive heuristic updates compared to standard single-step approaches.

Conclusion: Limited-horizon search-based heuristic updates offer a more effective approach for learning heuristics in sequential decision-making problems.
...
### [167] [Temporal Properties of Conditional Independence in Dynamic Bayesian Networks](https://arxiv.org/abs/2511.10266)
*Rajab Aghamov,Christel Baier,Joel Ouaknine,Jakob Piribauer,Mihir Vahanwala,Isa Vialard*

Main category: cs.AI

TL;DR: This paper analyzes the verification of conditional-independence (CI) propositions in dynamic Bayesian networks (DBNs) against temporal logic specifications, revealing computational complexity differences between stochastic and structural CI properties.


<details>
  <summary>Details</summary>
Motivation: To develop formal verification methods for probabilistic systems modeled by DBNs, specifically focusing on how conditional-independence relationships evolve over time and can be verified against temporal specifications.

Method: The study considers two specification formalisms (LTL and NBAs) and two variants of CI properties (stochastic and structural). It analyzes computational complexity using theoretical computer science and number theory approaches, including reductions to the Skolem problem.

Result: Verifying stochastic CI properties is at least as hard as the Skolem problem (an open problem), while structural CI verification is PSPACE-complete (NP- and coNP-hard). Tractable cases are identified under certain graphical structure restrictions.

Conclusion: There's a fundamental complexity gap between stochastic and structural CI verification in DBNs, with structural properties being more amenable to verification, especially under specific graphical constraints.
...
### [168] [Causal-HalBench: Uncovering LVLMs Object Hallucinations Through Causal Intervention](https://arxiv.org/abs/2511.10268)
*Zhe Xu,Zhicai Wang,Junkang Wu,Jinda Lu,Xiang Wang*

Main category: cs.AI

TL;DR: The paper introduces Causal-HalBench, a benchmark to quantify spurious correlations causing object hallucination in Large Vision-Language Models (LVLMs), using causal analysis and counterfactual samples.


<details>
  <summary>Details</summary>
Motivation: LVLMs suffer from object hallucination due to spurious correlations from highly co-occurring objects during training, but current benchmarks lack formal characterization and quantitative evaluation of these spurious correlations.

Method: Proposed causal analysis using Structural Causal Model (SCM) to define spurious correlations, developed Causal-HalBench with counterfactual samples generated using proprietary LVLMs and Text-to-Image models, and created comprehensive causal metrics.

Result: Evaluations on mainstream LVLMs show they exhibit susceptibility to spurious correlations, though to varying extents.

Conclusion: The proposed causal framework and Causal-HalBench effectively quantify spurious correlations in LVLMs, revealing their vulnerability to object hallucination from co-occurrence bias.
...
### [169] [Bidirectional Bounded-Suboptimal Heuristic Search with Consistent Heuristics](https://arxiv.org/abs/2511.10272)
*Shahaf S. Shperberg,Natalie Morad,Lior Siag,Ariel Felner,Dor Atzmon*

Main category: cs.AI

TL;DR: This paper introduces bounded-suboptimal variants of the BAE* bidirectional search algorithm and compares their performance against existing methods.


<details>
  <summary>Details</summary>
Motivation: While most bidirectional heuristic search research has focused on optimal methods, this work addresses the need for bounded-suboptimal bidirectional search where solution cost can be slightly suboptimal within specified bounds.

Method: The authors build upon the optimal BAE* algorithm for consistent heuristics and develop several variants specifically designed for bounded-suboptimal search contexts.
...
Conclusion: The study demonstrates that different bounded-suboptimal bidirectional search algorithms excel in distinct scenarios, providing insights into when to use each method.
...
### [170] [Fixed-Persona SLMs with Modular Memory: Scalable NPC Dialogue on Consumer Hardware](https://arxiv.org/abs/2511.10277)
*Martin Braas,Lukas Esterle*

Main category: cs.AI

TL;DR: A modular NPC dialogue system using Small Language Models (SLMs) with runtime-swappable memory modules for game dialogue, enabling expressive interactions and long-term memory without retraining.


<details>
  <summary>Details</summary>
Motivation: LLMs have hardware and latency limitations for game dialogue systems, and need clear knowledge boundaries in game settings.

Method: Fine-tuned SLMs encode NPC personas with runtime-swappable memory modules that preserve character context and world knowledge.

Result: Evaluated three SLMs (DistilGPT-2, TinyLlama-1.1B-Chat, Mistral-7B-Instruct) on consumer hardware with synthetic persona-aligned data.

Conclusion: Modular design and persona-driven memory architecture have broader potential for virtual assistants, customer support, and educational systems.
...
### [171] [FactGuard: Event-Centric and Commonsense-Guided Fake News Detection](https://arxiv.org/abs/2511.10281)
*Jing He,Han Zhang,Yuanhui Xiao,Wei Guo,Shaowen Yao,Renyang Liu*

Main category: cs.AI

TL;DR: FactGuard is a novel fake news detection framework that uses LLMs to extract event-centric content, reducing style dependency, and employs dynamic usability mechanisms with knowledge distillation for practical deployment.


<details>
  <summary>Details</summary>
Motivation: Traditional style-based fake news detection methods are becoming less effective as adversaries imitate authentic writing styles. While LLMs show potential, their adoption is limited by shallow functionality exploration, ambiguous usability, and high inference costs.

Method: FactGuard leverages LLMs to extract event-centric content, introduces a dynamic usability mechanism to identify contradictions and ambiguous cases in factual reasoning, and uses knowledge distillation to create FactGuard-D for efficient deployment in cold-start and resource-constrained scenarios.
...
Conclusion: FactGuard successfully mitigates the impact of writing style on fake news detection and provides a practical solution for LLM integration through dynamic usability mechanisms and knowledge distillation, enabling reliable and efficient detection in real-world scenarios.
...
### [172] [Beyond Verification: Abductive Explanations for Post-AI Assessment of Privacy Leakage](https://arxiv.org/abs/2511.10284)
...
<details>
  <summary>Details</summary>
Motivation: Privacy leakage in AI-based decision processes poses significant risks when sensitive information can be inferred, requiring formal auditing methods.

Method: Proposes a formal framework using abductive explanations to identify minimal sufficient evidence for model decisions, introducing Potentially Applicable Explanations (PAE) to identify individuals whose outcomes can shield those with sensitive features.

Result: Experimental evaluation on the German Credit Dataset shows how the importance of sensitive features in model decisions affects privacy leakage. The approach provides rigorous privacy guarantees with human-understandable explanations.

Conclusion: Abductive reasoning enables interpretable privacy auditing, offering a practical pathway to reconcile transparency, model interpretability, and privacy preservation in AI decision-making, despite computational challenges and simplifying assumptions.
...
### [173] [Massively Parallel Proof-Number Search for Impartial Games and Beyond](https://arxiv.org/abs/2511.10339)
*Tomáš Čížek,Martin Balko,Martin Schmid*

Main category: cs.AI

TL;DR: A massively parallel Proof-Number Search algorithm that scales efficiently on 1024 CPU cores, achieving 332.9x speedup and solving complex game positions in Sprouts.


<details>
  <summary>Details</summary>
Motivation: Existing parallel Proof-Number Search versions scale poorly on many CPU cores, and there's a need to accelerate computation for solving complex games like Sprouts to verify the Sprouts Conjecture.

Method: Two-level parallelization with shared information among workers, enhanced with Grundy numbers for game tree reduction, applied to the Sprouts game.

Result: Achieved 332.9x speedup on 1024 cores, outperformed state-of-the-art Sprouts solver GLOP by 4 orders of magnitude, generated proofs 1000x more complex, and verified Sprouts Conjecture for 42 new positions.

Conclusion: The massively parallel Proof-Number Search enables efficient scaling on many CPUs and significantly advances game solving capabilities, nearly doubling known outcomes for the Sprouts Conjecture.
...
### [174] [SITA: A Framework for Structure-to-Instance Theorem Autoformalization](https://arxiv.org/abs/2511.10356)
*Chenyi Li,Wanli Ma,Zichen Wang,Zaiwen Wen*

Main category: cs.AI

TL;DR: SITA is a framework for autoformalizing mathematical theorems by bridging abstract structures with concrete instances in Lean, using LLMs with feedback-guided refinement to ensure correctness.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with formalizing theorems from abstract structures to concrete settings, especially at research level mathematics. The goal is to automate formalization of mathematical results.

Method: Uses structure-to-instance theorem autoformalization (SITA) with formalized abstract structures as modular templates, Lean's typeclass mechanism, and LLM-based generation with feedback-guided refinement.
...
### [175] [Explaining Decentralized Multi-Agent Reinforcement Learning Policies](https://arxiv.org/abs/2511.10409)
*Kayla Boggess,Sarit Kraus,Lu Feng*

Main category: cs.AI

TL;DR: Proposes methods for generating policy summarizations and query-based explanations for decentralized Multi-Agent Reinforcement Learning (MARL) to address uncertainty and nondeterminism, improving user understanding and satisfaction.


<details>
  <summary>Details</summary>
Motivation: Existing explanation methods focus on centralized MARL and fail to address the uncertainty and nondeterminism inherent in decentralized settings, creating a need for better explanation techniques in decentralized MARL.

Method: Develop methods to generate policy summarizations capturing task ordering and agent cooperation in decentralized MARL policies, along with query-based explanations for When, Why Not, and What types of user queries about specific agent behaviors.

Result: Evaluation across four MARL domains and two decentralized MARL algorithms demonstrates generalizability and computational efficiency. User studies show significant improvements in user question-answering performance and enhanced subjective ratings on understanding and satisfaction metrics.

Conclusion: The proposed summarizations and explanations effectively address the limitations of existing methods in decentralized MARL, providing valuable tools for understanding agent behaviors and improving user experience in multi-agent systems.
...
### [176] [Generalizing Analogical Inference from Boolean to Continuous Domains](https://arxiv.org/abs/2511.10416)
*Francisco Cunha,Yves Lepage,Zied Bouraoui,Miguel Couceiro*

Main category: cs.AI

TL;DR: This paper develops a unified framework for analogical reasoning in real-valued domains using parameterized analogies based on generalized means, extending beyond Boolean domains to support both classification and regression tasks.


<details>
  <summary>Details</summary>
Motivation: Existing analogical reasoning frameworks are limited to Boolean domains and classification tasks, failing to extend to regression or continuous domains. The paper aims to address this gap by developing a more general theory.

Method: Introduces a unified framework using parameterized analogies defined via generalized means, which subsumes both Boolean classification and regression. Characterizes analogy-preserving functions and derives error bounds under smoothness assumptions.

Result: Provides a counterexample showing existing generalization bounds fail in Boolean setting, develops a general theory supporting analogical inference over continuous functions, and derives worst-case and average-case error bounds.

Conclusion: The proposed framework offers a general theory of analogical inference that works across both discrete and continuous domains, unifying Boolean classification and regression tasks under a single analogical reasoning paradigm.
...
### [177] [Using Certifying Constraint Solvers for Generating Step-wise Explanations](https://arxiv.org/abs/2511.10428)
*Ignace Bleukx,Maarten Flippo,Bart Bogaerts,Emir Demirović,Tias Guns*

Main category: cs.AI

TL;DR: The paper proposes using constraint solver proofs as a basis for generating step-wise explanations of unsatisfiable problems, significantly speeding up the explanation generation process while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Current methods for generating step-wise explanations of unsatisfiable constraint problems are computationally expensive, limiting their applicability to larger problems.

Method: Defines an abstract proof framework where both proofs and step-wise explanations can be represented, then proposes methods for converting solver proofs to step-wise explanation sequences with trimming and simplification techniques.

Result: The proposed method significantly speeds up step-wise explanation generation while producing explanations of similar quality to state-of-the-art approaches.

Conclusion: Using constraint solver proofs as a starting point for step-wise explanations is an effective approach that improves computational efficiency without sacrificing explanation quality.
...
### [178] [Preference Elicitation for Step-Wise Explanations in Logic Puzzles](https://arxiv.org/abs/2511.10436)
*Marco Foschini,Marianne Defresne,Emilio Gamba,Bart Bogaerts,Tias Guns*

Main category: cs.AI

TL;DR: This paper proposes interactive preference elicitation methods to learn user preferences for step-wise explanations in logic puzzles, introducing MACHOP with dynamic normalization and query diversification to generate higher-quality explanations.


<details>
  <summary>Details</summary>
Motivation: Existing step-wise explanation methods require user-defined objective functions to identify comprehensible explanation steps, but defining good objective functions is challenging. Interactive preference elicitation from machine learning can help learn user preferences from pairwise comparisons.

Method: Proposes two dynamic normalization techniques to handle varying scales of multiple sub-objectives, and introduces MACHOP (Multi-Armed CHOice Perceptron) - a novel query generation strategy combining non-domination constraints with upper confidence bound-based diversification to address similarity in generated comparisons.
...
Conclusion: Interactive preference elicitation with MACHOP is effective for generating high-quality step-wise explanations in satisfaction problems, overcoming limitations of traditional objective function definition through dynamic normalization and diversified query generation.
...
### [179] [Non-Monotonic S4F Standpoint Logic](https://arxiv.org/abs/2511.10449)
*Piotr Gorczyca,Hannes Strass*

Main category: cs.AI

TL;DR: S4F Standpoint Logic is a new formalism that combines standpoint logic and non-monotonic reasoning using modal logic S4F, enabling multi-viewpoint non-monotonic reasoning without increased computational complexity.


<details>
  <summary>Details</summary>
Motivation: To create a unified framework that can represent multiple heterogeneous viewpoints while incorporating non-monotonic reasoning capabilities, bridging the gap between standpoint logics and non-monotonic reasoning formalisms.

Method: Proposed S4F Standpoint Logic by generalizing both S4F modal logic and standpoint propositional logic, defining its syntax and semantics, analyzing computational complexity, and developing mechanisms for credulous and sceptical acceptance.

Result: S4F Standpoint Logic is not computationally harder than its constituent logics (S4F and standpoint logic) in both monotonic and non-monotonic forms, making it an efficient framework for multi-viewpoint non-monotonic reasoning.

Conclusion: The proposed S4F Standpoint Logic successfully integrates multi-viewpoint representation with non-monotonic reasoning while maintaining computational tractability, providing a practical framework for complex reasoning scenarios involving multiple perspectives.
...
### [180] [Proceedings of The third international workshop on eXplainable AI for the Arts (XAIxArts)](https://arxiv.org/abs/2511.10482)
...
TL;DR: Third international workshop on explainable AI for the Arts (XAIxArts) bringing together researchers from HCI, Interaction Design, AI, XAI, and digital arts to explore XAI's role in the arts.


<details>
  <summary>Details</summary>
Motivation: To foster interdisciplinary collaboration and exploration of explainable AI applications in creative and artistic domains.

Method: Workshop format bringing together researchers from multiple disciplines including HCI, Interaction Design, AI, XAI, and digital arts.

Result: Community building and knowledge sharing around XAI applications in arts through workshop discussions and presentations.

Conclusion: Successful continuation of the XAIxArts workshop series, advancing the dialogue on explainable AI's role in creative and artistic practices.
...
### [181] [Strategic Opponent Modeling with Graph Neural Networks, Deep Reinforcement Learning and Probabilistic Topic Modeling](https://arxiv.org/abs/2511.10501)
*Georgios Chalkiadakis,Charilaos Akasiadis,Gerasimos Koresis,Stergios Plataniots,Leonidas Bakopoulos*

Main category: cs.AI

TL;DR: This paper reviews Graph Neural Networks, Deep Reinforcement Learning, and Probabilistic Topic Modeling methods for strategic multiagent settings, focusing on opponent modeling and integration with Game Theory while addressing uncertainty, heterogeneity, and scalability challenges.


<details>
  <summary>Details</summary>
Motivation: To address limitations of traditional game theory assumptions (Common Prior Assumption and Self-Interest Hypothesis) in real-world scenarios by incorporating machine learning methods that can handle uncertainty and heterogeneity in multiagent strategic settings.

Method: Comprehensive review and analysis of three main approaches: Graph Neural Networks for modeling relationships and interactions, Multiagent Deep Reinforcement Learning for strategic learning, and Probabilistic Topic Modeling for estimating unknown distributions and handling heterogeneity.

Result: Identifies GNNs as powerful tools for strategic opponent modeling, highlights MADRL's potential in multiagent settings, and demonstrates PTM's capability to handle unknown agent beliefs. The review establishes connections between these ML methods and game theoretic concepts.

Conclusion: While promising approaches exist, key open challenges remain including handling non-stationary environments, balancing stability and adaptation, tackling uncertainty and heterogeneity, and ensuring scalability and solution tractability in multiagent strategic settings.
...
TL;DR: AI is transforming research workflows from information management to hypothesis generation, shifting from computational tool to active collaborator, requiring careful integration and governance.
...
Method: Commentary analysis examining AI's role across the research workflow, from literature filtering to experiment design and execution.

Result: AI systems now help manage information overload, surface interdisciplinary connections, generate hypotheses, and design experiments, marking a shift to AI as scientific collaborator.

Conclusion: AI should augment but not replace human judgment in academic workflows; requires deliberate adoption through policies promoting transparency, reproducibility, and accountability.
...
### [183] [Bi-Level Contextual Bandits for Individualized Resource Allocation under Delayed Feedback](https://arxiv.org/abs/2511.10572)
...
<details>
  <summary>Details</summary>
Motivation: Existing allocation frameworks fail to handle delayed feedback, hidden heterogeneity, and the complex interplay between individual characteristics and intervention dynamics in high-stakes domains like education and healthcare.

Method: Two-level approach: meta-level optimizes subgroup budget allocations with fairness constraints; base-level identifies responsive individuals using neural networks trained on observational data, incorporating resource-specific delay kernels and cooldown windows.

Result: Validated on education and workforce datasets, achieving higher cumulative outcomes, better adaptation to delay structures, and equitable distribution across subgroups compared to existing methods.
...
### [184] [Regular Games -- an Automata-Based General Game Playing Language](https://arxiv.org/abs/2511.10593)
*Radosław Miernik,Marek Szykuła,Jakub Kowalski,Jakub Cieśluk,Łukasz Galas,Wojciech Pawlik*

Main category: cs.AI

TL;DR: RG is a new General Game Playing system that uses finite automata for game rules, offering computational efficiency and game design convenience through multiple language layers.


<details>
  <summary>Details</summary>
Motivation: To create a GGP system that is both computationally efficient and convenient for game design, addressing the need for systems that are easy for automatic processing while supporting human-friendly game creation.

Method: Uses a low-level finite automaton language for defining game rules, with higher-level languages for game design that translate to the low-level language. The system includes an editor with LSP, automaton visualization, benchmarking tools, and debugger.

Result: RG generates faster forward models than current state-of-the-art GGP systems (Regular Boardgames, Ludii), demonstrating superior computational efficiency.

Conclusion: RG successfully achieves both computational efficiency and game design convenience through its multi-language approach with finite automaton foundations, providing a comprehensive ecosystem for game development and analysis.
...
TL;DR: This paper addresses the sim-to-real gap in cyber physical systems testing by developing a formal method to validate whether failure scenarios discovered in simulation reproduce in real-world data, using Scenic scenario programs and an efficient querying algorithm.


<details>
  <summary>Details</summary>
Motivation: To ensure that failure scenarios identified in simulation environments for autonomous vehicles are actually reproducible in real-world systems, rather than being artifacts of synthetic sensor data.

Method: Developed a formal definition for matching labeled time series sensor data to abstract scenarios represented as Scenic probabilistic programs, and created a querying algorithm to identify matching data in real-world datasets.

Result: The proposed algorithm is more accurate and orders of magnitude faster than state-of-the-art commercial vision large language models for querying scenarios, and scales effectively with the duration of time series data.

Conclusion: The approach provides an effective method for validating simulated failure scenarios against real-world data, bridging the sim-to-real gap in cyber physical systems testing.
