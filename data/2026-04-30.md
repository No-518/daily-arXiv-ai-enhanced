...
TL;DR: This paper addresses the underexplored problem of disguise makeup presentation attack detection in facial recognition systems. It proposes a two-phase framework combining a style-invariant full-face model (trained with metric learning and whitening transformation) that extracts region attention scores via Grad-CAM, which guide a patch-based phase using region-specific subnetworks for fine-grained analysis. A new diverse real-world dataset is also introduced. The method achieves strong results: 8.97% ACER/9.76% EER on their dataset, and 0% ACER on Obfuscation/Impersonation and 1.34% on Cosmetics attacks of SIW-Mv2, outperforming prior works.


<details>
  <summary>Details</summary>
Motivation: Disguise makeup attacks using cosmetics, prosthetics, and artificial materials are particularly challenging for facial recognition systems, yet they remain underexplored with limited publicly available datasets.

Method: A two-phase framework: (1) a style-invariant full-face model trained with metric learning and whitening transformation extracts region attention scores via Grad-CAM; (2) a patch-based phase uses these scores to guide region-specific subnetworks with metric learning for localized fine-grained discrimination. A new diverse real-world dataset was also constructed.

Result: 8.97% ACER and 9.76% EER on the collected dataset; 0% ACER on Obfuscation and Impersonation, 1.34% on Cosmetics attacks of SIW-Mv2. Consistently outperforms prior works across multiple spoof types.

Conclusion: The proposed generalized disguise makeup presentation attack detection framework demonstrates strong generalization and robustness, outperforming existing methods on both the newly collected dataset and benchmark datasets.
...
TL;DR: Summary of the 2026 PVUW Challenge at CVPR 2026, featuring three tracks (MOSE, MeViS-Text, MeViS-Audio) for pixel-level video understanding under unconstrained conditions, highlighting top methods and future directions.


<details>
  <summary>Details</summary>
Motivation: To push the boundaries of pixel-level video understanding by evaluating models under highly unconstrained, real-world conditions, including dense clutter, severe occlusion, and multimodal (text/audio) target localization.

Method: Three specialized tracks: (1) MOSE track for tracking in cluttered/occluded scenes, (2) MeViS-Text track for motion-focused linguistic expression-based localization, (3) New MeViS-Audio track for acoustic-driven object segmentation.

Result: Introduced new challenging datasets and analyzed cutting-edge multimodal solutions from participants, revealing latest technical advancements in robust video scene comprehension.

Conclusion: The challenge demonstrates significant progress in multimodal video understanding and charts promising future directions for robust, unconstrained video scene comprehension.
...
TL;DR: Introduces ADAGE framework to evaluate alignment between deep learning model explanations and domain knowledge in satellite-based flood mapping.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for flood mapping lack interpretability, hindering integration into operational workflows; need systematic assessment of whether explanations align with established remote sensing knowledge.

Method: ADAGE framework uses Channel-Group SHAP to estimate grouped input channel contributions, then quantitatively compares explanations against domain knowledge-derived reference explanations via alignment scores.

Result: On two flood mapping tasks, ADAGE quantifies alignment between model explanations and domain knowledge, and helps experts identify misaligned explanations.

Conclusion: ADAGE bridges explainability and domain knowledge in GeoAI, enhancing model applicability in critical scientific and operational Earth observation workflows.
...
TL;DR: RADIO-ViPE is an online semantic SLAM system that enables open-vocabulary grounding from raw monocular RGB video without calibration, using tightly coupled multi-modal embeddings and adaptive robust kernels to handle dynamic scenes.


<details>
  <summary>Details</summary>
Motivation: Existing approaches require calibrated, posed RGB-D input with prior camera intrinsics, depth sensors, or pose initialization, limiting real-world deployment in dynamic environments with unconstrained video streams.

Method: An online semantic SLAM system that tightly couples multi-modal embeddings (from agglomerative foundation models like RADIO) with geometric scene information through initialization, optimization, and factor graph connections, wrapped within adaptive robust kernels to handle dynamic objects and scene changes.

Result: State-of-the-art results on dynamic TUM-RGBD benchmark with competitive performance against offline open-vocabulary methods that rely on calibrated data and static scene assumptions.

Conclusion: RADIO-ViPE bridges a critical gap in real-world deployment, enabling robust open-vocabulary semantic grounding for autonomous robotics and unconstrained in-the-wild video streams, achieving SOTA on dynamic TUM-RGBD while matching offline methods without needing calibrated data.
...
TL;DR: The paper proposes a probabilistic model for fruit maturity estimation that treats maturity as a continuous variable rather than discrete classes, using a distributional detection head and CDF transformation to better handle uncertainty and adjacent-stage ambiguity.


<details>
  <summary>Details</summary>
Motivation: Traditional vision-based maturity estimation uses multi-class classification, which imposes sharp boundaries between visually similar adjacent maturity stages, leading to annotator disagreement and poor handling of continuous ripening processes.

Method: Models maturity as a latent continuous variable predicted via a distributional detection head, converting distributions to class probabilities through the cumulative distribution function (CDF).

Result: The probabilistic model matches standard detector performance under clean labels while better representing uncertainty, and shows improved robustness when controlled label noise is introduced during training.

Conclusion: Explicitly modeling maturity uncertainty leads to more reliable visual maturity estimation, particularly under label noise conditions.
...
TL;DR: Proposes sample selection methods using multitask autoencoder, OCSVM, isolation forest, adaptive loss threshold, and multi-class SVDD to filter noisy/malicious samples in federated learning, achieving up to 7.02% accuracy improvement on CIFAR10.


<details>
  <summary>Details</summary>
Motivation: Federated learning is hindered by redundant, malicious, or abnormal samples causing model degradation and inefficiency.

Method: Novel sample selection via multitask autoencoder for loss/feature analysis; uses OCSVM, IF, adaptive loss threshold (AT), and multi-class SVDD loss controlled by central server.

Result: Up to 7.02% accuracy gain on CIFAR10 with OCSVM, 1.83% on MNIST with AT, and 0.99% on CIFAR10 with federated SVDD loss.

Conclusion: Proposed methods effectively improve model accuracy across varying client counts, non-IID distributions, and noise levels up to 40%.
...
TL;DR: MixerCA is a lightweight HSI classification model combining depthwise convolution, token/channel mixing, and coordinate attention, outperforming CNNs and Transformers on four benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: To design a lightweight yet effective model for hyperspectral image classification that decouples spatial and channel interactions while maintaining resolution throughout the network.

Method: MixerCA integrates depthwise convolutions, token and channel mixing, and coordinate attention into a unified structure to process HSI patches directly.

Result: Outperforms 2D-CNN, 3D-CNN, Tri-CNN, HybridSN, ViT, and Swin Transformer on four hyperspectral benchmark datasets.

Conclusion: MixerCA offers clear advantages over competing algorithms for HSI classification, with publicly available source code.
...
TL;DR: A data-centric AI framework using confident learning, class refinement, and targeted label evaluation improves FLIm-based multi-class glioma infiltration classification, achieving 96% accuracy in distinguishing low, moderate, and high tumor cellularity in GBM resection margins.


<details>
  <summary>Details</summary>
Motivation: Accurate intraoperative assessment of glioma infiltration is critical for maximizing tumor resection while preserving function. FLIm offers real-time biochemical contrast but is limited by biological heterogeneity, class imbalance, and variability in histopathological labeling.

Method: Applied confident learning (CL) to FLIm data from 192 margins (31 GBM patients, 7 initial classes), iteratively merged into 3 classes ("low"/"moderate"/"high"), trained a classifier, used SHAP for feature importance, and performed blinded re-evaluation of CL-flagged margins.

Result: Achieved 96% accuracy in three-class classification. SHAP identified distinct optical signatures per class. Low-confidence predictions linked to gray matter composition and blood contamination. Blinded re-evaluation revealed intra-pathologist labeling variability.

Conclusion: The DC-AI framework systematically improves data reliability, model robustness, and biological interpretation of FLIm signals, supporting clinically actionable optical tools for real-time glioma margin assessment.
...
TL;DR: This paper proposes a new labeling framework for underwater domain shift that uses measurable image, scene, and acquisition characteristics to define domains, enabling more realistic evaluation of object detection performance compared to existing synthetic style transfer approaches.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for underwater domain shift rely on synthetic style transfer, which fails to capture real-world factors like visibility, illumination, scene composition, and acquisition factors, limiting analysis of actual performance degradation.

Method: A labeling framework that defines underwater domains using measurable image, scene, and acquisition characteristics, enabling semantically consistent image grouping and domain-specific evaluation.

Result: Validation on public datasets shows systematic variations across domain factors and reveals hidden failure modes not captured by synthetic benchmarks.

Conclusion: The proposed framework captures physically meaningful factors for underwater domain analysis, enabling better understanding of detection performance and failure modes in real-world conditions.
...
TL;DR: Proposes lifted world models that use a lightweight policy to map high-level action waypoints to low-level joint actions, improving planning efficiency for embodied agents with complex action spaces.


<details>
  <summary>Details</summary>
Motivation: High-dimensional action spaces for complex embodiments (e.g., human agents with many joints) make world models hard to control and expensive to plan with, as search-based methods like CEM scale poorly.

Method: Train a lightweight policy that maps high-level actions (2D waypoints on observation frames specifying goal positions for leaf joints) to sequences of low-level joint actions; compose this with a frozen world model to create a lifted world model.

Result: Lifted world model achieves 3.8× lower mean joint error to goal pose compared to searching directly in low-level joint space, while being more compute-efficient and generalizing to unseen environments.

Conclusion: High-level action spaces defined as interpretable 2D waypoints can effectively lift world models for complex embodiments, improving both performance and efficiency over low-level joint space search.
...
TL;DR: A privacy-preserving clothing classification method using Vision Transformer (ViT) is proposed for occupant-centric control systems, maintaining high accuracy on encrypted images unlike conventional pixel-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing HVAC control using camera images optimizes thermal comfort but ignores privacy protection, and conventional privacy-preserving methods cause severe accuracy degradation.

Method: Applies Vision Transformer (ViT) to clothing insulation estimation with encrypted images, using the DeepFashion dataset categorized by clothing insulation.

Result: The proposed scheme maintains high accuracy on encrypted images with no degradation from plain images across all categories, while conventional pixel-based methods suffer severe accuracy drops.

Conclusion: ViT-based privacy-preserving classification effectively balances privacy protection and accuracy for occupant-centric control systems, outperforming conventional pixel-based approaches.
...
TL;DR: FASH-iCNN is a multimodal system trained on 87,547 Vogue runway images (1991-2024, 15 fashion houses) that identifies the fashion house (78.2% top-1), decade (88.6%), and year (58.3%, mean error 2.2 years) from garment photos. Texture/luminance are the primary carriers of editorial identity (removing texture costs 37.6pp accuracy), making the otherwise hidden cultural logic of fashion houses inspectable.


<details>
  <summary>Details</summary>
Motivation: Fashion AI systems encode aesthetic logic of specific houses, editors, and historical moments without disclosing it. The authors aim to make this cultural logic inspectable rather than treating it as background noise.

Method: Trained a multimodal system (FASH-iCNN) on 87,547 Vogue runway images across 15 fashion houses (1991-2024). Uses clothing-only models to predict house, decade, and year. Probes visual channels by ablating color vs. texture to identify which carries editorial identity signal.

Result: House identification at 78.2% top-1 (14 houses), decade at 88.6% top-1, year at 58.3% top-1 (34 years, mean error 2.2 years). Removing color costs 10.6pp accuracy on house identity; removing texture costs 37.6pp—establishing texture/luminance as primary carriers.

Conclusion: FASH-iCNN treats editorial culture as signal rather than noise, enabling users to see which houses, editors, and historical moments are encoded in predictions, making fashion AI's hidden cultural logic transparent.
...
TL;DR: ViBE is a brain encoding framework that uses a spatio-temporal convolutional VAE with Q-Former alignment to generate realistic M/EEG signals from visual stimuli, validated on THINGS-EEG2 and THINGS-MEG datasets.


<details>
  <summary>Details</summary>
Motivation: To achieve faithful reconstruction of neural responses and cross-modal alignment between visual stimuli and neural responses, advancing visual prostheses for vision disorders.

Method: Proposes TSC-VAE for spatio-temporal M/EEG reconstruction, uses Q-Former to map CLIP embeddings to latent space, and combines MSE loss with sliced Wasserstein distance for dual alignment (point-wise + distribution-wise).

Result: Effectively generates high-quality M/EEG signals from visual stimuli on THINGS-EEG2 and THINGS-MEG datasets.

Conclusion: ViBE successfully bridges the modality gap between visual features and neural signals, demonstrating potential for brain encoding and visual prostheses.
...
TL;DR: SeeCo is a plug-and-play framework that boosts training-free open-vocabulary semantic segmentation in remote sensing by seeking dual consensus (geometric and semantic) to recalibrate models on-the-fly, achieving consistent gains across eight benchmarks without requiring training.


<details>
  <summary>Details</summary>
Motivation: Existing OVSS methods use static inference, overlooking scene-specific distributions, leading to semantic ambiguity and incomplete foreground activation in diverse remote sensing land covers.

Method: SeeCo proposes dual consensus learning: Geometric Consensus Learning (GCL) via multi-view consistent observations and Semantic Consensus Learning (SCL) via textual description adaptive calibration, injected through an Online Consensus Injector (OCI) to collaboratively recalibrate visual and textual semantics.

Result: Extensive experiments on eight remote sensing OVSS benchmarks show consistent performance gains, demonstrating effectiveness and universality.

Conclusion: SeeCo effectively alleviates under-activation and semantic bias in training-free OVSS without requiring specific training, proving it as a universal plug-and-play solution.
...
TL;DR: Proposes AdaAct, an adaptive network that uses global human-object interaction sequences as prior knowledge to improve weakly-supervised action segmentation by dynamically adjusting parameters at test time.


<details>
  <summary>Details</summary>
Motivation: Existing fixed networks struggle with ambiguity in similar actions (e.g., pouring juice vs. pouring coffee) because they only rely on local neighboring frames.

Method: (1) A video HOI encoder to extract, select, and integrate representative HOI across the video; (2) A two-branch HyperNetwork that learns an adaptive temporal encoder, adjusting parameters on the fly based on HOI information.

Result: Outperforms prior methods on Breakfast and 50Salads datasets under various evaluation metrics.

Conclusion: Exploiting global HOI sequences as video-level prior knowledge with adaptive parameter adjustment effectively resolves action ambiguity in weakly-supervised action segmentation.
...
TL;DR: DepthPilot is an interpretable framework for colonoscopy video generation that uses depth constraints and adaptive spline denoising to produce physically consistent, clinically interpretable videos with FID scores below 15.


<details>
  <summary>Details</summary>
Motivation: Existing controllable medical video generation lacks interpretability—it cannot align generated content with physical priors and faithful clinical manifestations, hindering clinical trustworthiness.

Method: Two synergistic paradigms: (1) prior distribution alignment strategy injecting depth constraints into diffusion backbone via parameter-efficient fine-tuning for geometric grounding; (2) adaptive spline denoising module replacing fixed linear weights with learnable spline functions for nonlinear spatio-temporal modeling.

Result: FID scores below 15 across all benchmarks; ranked first in clinician assessments; enables reliable 3D reconstruction for surgical navigation and blind region identification.

Conclusion: DepthPilot bridges the gap between "visually realistic" and "clinically interpretable" generation, serving as a foundation toward a colorectal world model.
...
TL;DR: EnerGS models incomplete geometry as a continuous energy field to softly guide 3D Gaussian Splatting optimization, improving photometric quality and geometric stability in large-scale outdoor scenes.


<details>
  <summary>Details</summary>
Motivation: Geometric priors like LiDAR are often spatially incomplete and uneven in large-scale outdoor scenarios, making hard constraints unreliable or even detrimental to 3DGS reconstruction.

Method: Proposes EnerGS, which models partially observable geometry as a continuous energy field and provides soft geometric guidance (instead of hard constraints) to steer Gaussian primitive optimization.

Result: Under both sparse multi-view and monocular settings, EnerGS consistently improves photometric quality and geometric stability while mitigating overfitting during 3DGS training.

Conclusion: Soft geometric guidance via energy field is more effective than hard geometric constraints for outdoor 3DGS when geometric priors are incomplete and uneven.
...
TL;DR: The paper introduces a camera-RFID fusion framework that bridges the meter-to-centimeter accuracy gap for asset tracking in forested environments by integrating depth/object information with trajectory-matching algorithms, achieving reliable localization even during temporary occlusion.


<details>
  <summary>Details</summary>
Motivation: Passive RFID tags offer cost-effective tracking but suffer meter-level accuracy in forests due to signal attenuation/multipath; cameras provide centimeter precision but fail under occlusion and spatial ambiguity. The key challenge is accurately associating disparate trajectories from both sensors.

Method: A novel camera-RFID fusion framework integrating depth and object information with advanced trajectory-matching algorithms to associate sensor trajectories and enable reliable localization.
...
Conclusion: First application of camera-RFID fusion for asset tracking in natural forested environments, demonstrating effective fusion that overcomes individual sensor limitations.
...
TL;DR: MetaSR is a Diffusion Transformer framework that selects content-relevant metadata for generative super-resolution, achieving up to 1.0 dB PSNR improvement and 50% bitrate savings over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing metadata-guided SR methods use fixed conditioning designs that are suboptimal when useful cues are content-dependent and transmission budgets are limited.

Method: MetaSR uses a Diffusion Transformer (DiT) that leverages its own VAE and transformer backbone to fuse heterogeneous metadata, combined with an efficient distillation strategy for one-step diffusion inference.

Result: Outperforms reference solutions by up to 1.0 dB PSNR while achieving up to 50% transmission bitrate savings at matched quality.

Conclusion: Content-aware metadata selection with rate-distortion optimization enables more efficient and higher-quality generative super-resolution across diverse scenarios.
...
TL;DR: SQI is a training-free framework that improves VLMs' robustness to optical illusions by injecting axiomatic constraints, decomposing scenes hierarchically, and using counterfactual self-verification, achieving 2nd place in the DataCV 2026 Classic Illusion Understanding Challenge.


<details>
  <summary>Details</summary>
Motivation: VLMs are brittle when facing optical illusions due to shortcut heuristics that prioritize linguistic priors over direct visual evidence.

Method: SQI consists of three modules: (1) Axiomatic Constraint Injection, (2) Hierarchical Scene Decomposition, and (3) Counterfactual Self-Verification — applied at inference time without training.

Result: Ranked 2nd in the DataCV 2026 Challenge (Task I), significantly improving accuracy across illusion categories with superior diagnostic interpretability.

Conclusion: Structured qualitative grounding is a promising paradigm for building robust, illusion-resistant vision-language systems without fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenging problem of multi-class bi-atrial segmentation from 3D LGE MRI, likely due to issues like class imbalance, poor contrast, and anatomical variability.

Method: A multi-stage pipeline with: (1) MCLAHE preprocessing for contrast enhancement, (2) coarse region segmentation from down-sampled MRI using a V-Net model, (3) fine segmentation from the coarse region using another V-Net model, and (4) asymmetric loss for optimization.

Result: Not explicitly reported in the abstract.

Conclusion: The proposed multi-stage framework with MCLAHE, coarse-to-fine V-Net, and asymmetric loss enables bi-atrial segmentation from 3D LGE MRI.
...
TL;DR: OmniTrend is a unified framework that separates content attractiveness from contextual exposure to predict social media popularity, improving interpretability and cross-platform transfer.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to separate intrinsic content signals from platform-specific exposure patterns, leading to weak interpretability and poor cross-platform transfer.

Method: OmniTrend uses a content module (cross-modal representations from visual, audio, textual cues) and a context module (posting time, author activity, trends, neighborhood stats), learning separate predictors for attractiveness and exposure before integrating them.

Result: The framework makes the role of each factor explicit and supports robust transfer across image and video platforms.

Conclusion: Separating content attractiveness from contextual exposure improves both interpretability and cross-platform generalization in popularity prediction.
...
TL;DR: A new framework for medical VLMs that simulates clinicians' implicit diagnostic memory through latent memory evolution, using meta-query memorization, causal counterfactual refinement, and intrinsic memory transition to outperform state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing medical VLMs suffer from cognitive misalignment due to discrete tokenization causing quantization loss, long-range information dissipation, and lack of case-adaptive expertise, failing to replicate clinicians' instant invocation of implicit diagnostic memory during image interpretation.

Method: Three components: (1) Meta Query for Prior Memorization using learnable probes to retrieve structured priors, (2) Causal Counterfactual Refinement (CCR) with reinforcement learning and counterfactual rewards from region-level feature masking, (3) Intrinsic Memory Transition (IMT) via privileged-autonomous dual-branch paradigm with full-vocabulary divergence alignment.

Result: Significantly outperforms existing state-of-the-art methods including chain-of-thought paradigms across multiple datasets in diagnostic accuracy.

Conclusion: Transferring external expertise into endogenous parameters effectively bridges the cognitive gap between VLMs and clinical reasoning.
...
TL;DR: GaitKD is a knowledge distillation framework for gait recognition that decouples knowledge transfer into decision-level (part-calibrated logit distillation) and boundary-level (activation-boundary objective) components, enabling efficient student models without extra inference cost.


<details>
  <summary>Details</summary>
Motivation: High-performing gait recognition models rely on deep, computationally expensive architectures that are hard to deploy. Standard knowledge distillation is less effective for part-structured gait models because supervision involves both part-wise classification logits and retrieval embeddings.

Method: Proposes GaitKD with two complementary components: (1) Decision-level distillation via part-calibrated logit distillation to transfer inter-class decision relations, and (2) Boundary-level distillation via an activation-boundary objective to preserve teacher-induced embedding space partitioning, avoiding direct feature regression.

Result: Consistent improvements over strong gait baselines across multiple benchmarks and teacher-student configurations. Boundary-preserving distillation provides more stable performance than direct feature regression; the two transfer components are complementary.

Conclusion: GaitKD effectively transfers knowledge for part-structured gait models, achieving efficiency without extra inference cost, and demonstrates that decoupled decision- and boundary-level distillation is superior to standard KD approaches.
...
TL;DR: CheXthought is a large-scale multimodal dataset of 103,592 chain-of-thought reasoning traces and 6.6M visual attention annotations from 501 radiologists across 71 countries on 50,312 chest X-rays, which improves AI models' factual accuracy, spatial grounding, reduces hallucinations, enhances classification, temporal reasoning, uncertainty communication, and enables prediction of human-AI disagreement.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models are trained on paired images and reports, lacking the cognitive processes and visual attention underlying clinical reasoning; there is a need for resources capturing how experts reason through chest X-ray interpretation.

Method: Collected 103,592 chain-of-thought reasoning traces and 6,609,082 visual attention annotations from 501 radiologists across 71 countries on 50,312 multi-read chest X-rays, then evaluated models trained on this data across multiple clinical dimensions.

Result: CheXthought reasoning outperforms state-of-the-art vision-language model chain-of-thought in factual accuracy and spatial grounding; visual attention as inference-time hint recovers missed findings and reduces hallucinations; trained models achieve stronger pathology classification, visual faithfulness, temporal reasoning, and uncertainty communication; multi-reader annotations enable prediction of human-human and human-AI disagreement from images alone.

Conclusion: CheXthought is a valuable resource for advancing multimodal clinical reasoning and developing more transparent, interpretable vision-language models for medical AI.
...
### [26] [Edge AI for Automotive Vulnerable Road User Safety: Deployable Detection via Knowledge Distillation](https://arxiv.org/abs/2604.26857)
*Akshay Karjol,Darrin M. Hanna*

Main category: cs.CV

TL;DR: A knowledge distillation framework trains a compact YOLOv8-S model (11.2M params) to mimic a YOLOv8-L teacher (43.7M params), achieving 3.9x compression while maintaining INT8 quantization robustness for VRU detection on edge hardware.


<details>
  <summary>Details</summary>
Motivation: Deploying accurate object detection for Vulnerable Road User (VRU) safety on edge hardware requires balancing model capacity against computational constraints - large models fail under INT8 quantization, small models sacrifice performance.

Method: Knowledge distillation (KD) framework training YOLOv8-S student (11.2M params) to mimic YOLOv8-L teacher (43.7M params), evaluated on full-scale BDD100K (70K training images) with Post-Training Quantization to INT8.

Result: 3.9x compression; teacher suffers -23% mAP under INT8, KD student only -5.6% mAP; KD student achieves 0.748 precision vs 0.653 for direct training at INT8 (14.5% gain); 44% reduction in false alarms vs collapsed teacher; KD student exceeds teacher's FP32 precision (0.748 vs 0.718) in 3.9x smaller model.

Conclusion: Knowledge distillation establishes a requirement for deploying accurate, safety-critical VRU detection on edge hardware.
...
### [27] [Multiple Consistent 2D-3D Mappings for Robust Zero-Shot 3D Visual Grounding](https://arxiv.org/abs/2604.26261)
*Yufei Yin,Jie Zheng,Qianke Meng,Zhou Yu,Minghao Chen,Jiajun Ding,Min Tan,Yuling Xi,Zhiwen Chen,Chengfei Lv*

Main category: cs.CV

TL;DR: MCM-VG proposes a zero-shot 3D visual grounding framework that establishes multiple consistent 2D-3D mappings via semantic alignment, instance rectification, and viewpoint distillation, achieving SOTA on ScanRefer and Nr3D benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot 3DVG methods suffer from poor quality of open-vocabulary 3D proposals (inaccurate categories, imprecise geometries) and spatial redundancy in exhaustive multi-view reasoning.

Method: Three modules: (1) Semantic Alignment — LLM-driven query parsing & coarse-to-fine 2D-3D matching; (2) Instance Rectification — VLM-guided 2D segmentations back-projected for accurate 3D geometries; (3) Viewpoint Distillation — clustering 3D camera directions for optimal frames, paired with BEV maps for multiple-choice VLM reasoning.

Result: 62.0% Acc@0.25 and 53.6% Acc@0.5 on ScanRefer, outperforming baselines by 6.4% and 4.0%; new SOTA on zero-shot 3DVG benchmarks.

Conclusion: Explicitly enforcing 2D-3D consistency across semantic, instance, and viewpoint dimensions effectively addresses the bottlenecks of noisy 3D proposals and spatial redundancy in zero-shot 3D visual grounding.
...
TL;DR: This paper proposes an anchor-constrained optimization framework to incorporate no-reference perceptual quality (NR-IQA) into diffusion model training, balancing perceptual improvement with generative fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models rely on full-reference objectives (e.g., pixel-wise similarity), which are insufficient for subjective visual perception quality and text-image semantic consistency.

Method: An anchor-constrained optimization framework that uses a learned NR-IQA model as perceptual guidance, with an anchor-based regularization enforcing noise prediction consistency with the base diffusion model.

Result: The method consistently enhances perceptual quality while preserving generation diversity and training stability.

Conclusion: Anchor-constrained perceptual optimization effectively balances perceptual quality improvement and generative fidelity for diffusion models.
...
### [29] [Three-Step Nav: A Hierarchical Global-Local Planner for Zero-Shot Vision-and-Language Navigation](https://arxiv.org/abs/2604.26946)
*Wanrong Zheng,Yunhao Ge,Laurent Itti*

Main category: cs.CV

TL;DR: Three-Step Nav improves zero-shot VLN by implementing a three-view protocol (look forward, look now, look backward) that extracts global landmarks, aligns observations with sub-goals, and audits trajectories, achieving SOTA on R2R-CE and RxR-CE without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current zero-shot VLN agents powered by MLLMs tend to drift off course, halt prematurely, and achieve low overall success rates.

Method: Three-Step Nav uses a three-view protocol: (1) look forward to extract global landmarks and sketch a coarse plan, (2) look now to align current observation with the next sub-goal for fine-grained guidance, (3) look backward to audit the entire trajectory and correct drift before stopping. No gradient updates or task-specific fine-tuning required.

Result: Achieves state-of-the-art zero-shot performance on R2R-CE and RxR-CE datasets.

Conclusion: The three-view protocol effectively mitigates drift, premature halting, and low success rates in zero-shot VLN, providing a lightweight plug-in solution for existing pipelines.
...
### [30] [Semantic Foam: Unifying Spatial and Semantic Scene Decomposition](https://arxiv.org/abs/2604.26262)
*Amr Sharafeldin,Shrisudhan Govindarajan,Thomas Walker,Aryan Mikaeili,Daniel Rebain,Kwang Moo Yi,Andrea Tagliasacchi*

Main category: cs.CV

TL;DR: Semantic Foam extends Radiant Foam with a cell-level semantic feature field on a volumetric Voronoi mesh, achieving better object-level segmentation and cross-view consistency than Gaussian Grouping and SAGA.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting enables real-time novel view synthesis but lacks semantic decomposition and cross-view consistency needed for interactive graphics applications.

Method: Extends Radiant Foam's volumetric Voronoi mesh with an explicit semantic feature field defined at the cell level, using direct spatial regularization to improve cross-view consistency and reduce occlusion/inconsistent supervision artifacts.

Result: Achieves superior object-level segmentation performance compared to Gaussian Grouping and SAGA.

Conclusion: Semantic Foam effectively addresses limitations in segmentation quality and cross-view consistency for semantic decomposition of scene representations.
...
TL;DR: This paper proposes a framework to improve VLM's ability to refuse answering questions beyond their knowledge, using a Visual-Idk dataset, supervised fine-tuning, and preference optimization, achieving a Truthful Rate improvement from 57.9% to 67.3%.


<details>
  <summary>Details</summary>
Motivation: VLMs suffer from factual hallucinations and fail to refuse queries exceeding their parametric knowledge, especially in long-tail or specialized domains.

Method: Curates a model-specific Visual-Idk dataset via multi-sample consistency probing, then aligns the model with supervised fine-tuning and preference-aware optimization (DPO, ORPO) to define knowledge boundaries.

Result: Truthful Rate improved from 57.9% to 67.3% on Visual-Idk; internal probing shows genuine boundary recognition, not memorization; generalizes to out-of-distribution medical/perceptual domains.

Conclusion: The framework provides a robust path toward more trustworthy and prudent VLMs by enhancing refusal capabilities on unknown queries.
...
### [32] [High-Dimensional Noise to Low-Dimensional Manifolds: A Manifold-Space Diffusion Framework for Degraded Hyperspectral Image Classification](https://arxiv.org/abs/2604.26279)
*Boxiang Yang,Ning Chen,Xia Yue,Yichang Luo,Yingbo Fan,Haoyuan Zhang,Haoyu Ma,Jun Yue,Shanjun Mao*

Main category: cs.CV

TL;DR: MSDiff maps degraded HSI data to a low-dimensional manifold, then uses diffusion-based generative modeling to refine latent features, achieving robust classification under complex degradations.


<details>
  <summary>Details</summary>
Motivation: HSI data is high-dimensional but low-rank, and real-world degradations disrupt its intrinsic manifold structure, introducing redundant variations that hinder classification.

Method: First, a discriminative spectral-spatial reconstruction maps HSI into a compact low-dimensional manifold. Then, a diffusion-based generative model regularizes the distribution on this manifold to refine features against residual degradations.

Result: Consistent performance improvements over state-of-the-art methods on multiple hyperspectral benchmarks under diverse composite degradation settings.

Conclusion: Performing diffusion-based distribution modeling directly on the low-dimensional manifold effectively decouples degradation disturbances from intrinsic discriminative structures, enhancing robustness.
...
### [33] [QYOLO: Lightweight Object Detection via Quantum Inspired Shared Channel Mixing](https://arxiv.org/abs/2604.26435)
*Garvit Kumar Mittal,Sahil Tomar,Sandeep Kumar*

Main category: cs.CV

TL;DR: QYOLO replaces the deepest C2f backbone layers in YOLOv8 with a quantum-inspired QMixBlock, reducing parameters by 20-22% with minimal mAP loss on VisDrone2019.


<details>
  <summary>Details</summary>
Motivation: The deep backbone stages (C2f bottlenecks at high stride levels) in single-stage detectors accumulate disproportionate parameters due to quadratic scaling with channel width, creating computational overhead.

Method: Proposes QYOLO with QMixBlock — a quantum-inspired channel mixing module using sinusoidal mixing with shared learnable parameters across P4/16 and P5/32 stages, replacing the two deepest C2f modules while keeping neck and head classical.

Result: QYOLOv8n: 20.2% fewer parameters (3.01M→2.40M), 12.3% GFLOPs reduction, only 0.4 pp mAP@50 loss. QYOLOv8s: 21.8% parameter reduction with 0.1 pp loss. Knowledge distillation recovers full accuracy.

Conclusion: Backbone-only QMixBlock compression is effective and preferable; expanded backbone+neck variants yield higher compression (38-41%) but at greater accuracy cost, motivating the final backbone-only design.
...
### [34] [Star-Fusion: A Multi-modal Transformer Architecture for Discrete Celestial Orientation via Spherical Topology](https://arxiv.org/abs/2604.26582)
*May Hammad,Menatallh Hammad*

Main category: cs.CV

TL;DR: Star-Fusion reformulates celestial orientation estimation as a discrete topological classification task using spherical K-Means clustering and a tripartite fusion architecture (SwinV2-Tiny transformer + convolutional heatmap + coordinate MLP), achieving 93.4% Top-1 accuracy and 18.4 ms inference latency on COTS hardware.


<details>
  <summary>Details</summary>
Motivation: Traditional LIS algorithms suffer from high computational overhead and noise sensitivity; standard deep regression models fail due to non-Euclidean topology of the celestial sphere and periodic boundary conditions of RA/Dec.

Method: Reformulates orientation estimation as discrete topological classification via spherical K-Means clustering into K topologically consistent regions; uses tripartite fusion: SwinV2-Tiny transformer (photometric), convolutional heatmap (spatial), coordinate MLP (geometric).

Result: 93.4% Top-1 accuracy, 97.8% Top-3 accuracy on synthetic Hipparcos dataset; 18.4 ms inference latency on resource-constrained COTS hardware.

Conclusion: Star-Fusion is a viable candidate for real-time onboard deployment in next-generation satellite constellations, addressing both accuracy and efficiency challenges in celestial attitude determination.
...
### [35] [Event-based Liveness Detection using Temporal Ocular Dynamics: An Exploratory Approach](https://arxiv.org/abs/2604.26285)
*Nicolas Mastropasqua,Ignacio Bugueno-Cordova,Rodrigo Verschae,Daniel Acevedo,Pablo Negri*

Main category: cs.CV

TL;DR: This paper explores using event cameras (instead of RGB) for face liveness detection by analyzing eye movement dynamics like saccades, which replay attacks cannot faithfully reproduce. They extend RGBE-Gaze with replay-attack data and use spiking CNNs, achieving 95.37% accuracy.


<details>
  <summary>Details</summary>
Motivation: RGB-based liveness detection fails to generalize across sensors and attack scenarios. Event cameras offer a promising alternative because they capture asynchronous brightness changes at microsecond resolution, enabling precise analysis of fast eye movements (e.g., saccades) that replay attacks cannot replicate due to temporal resampling and display artifacts.

Method: They designed a data collection protocol to extend the RGBE-Gaze dataset with replay-attack recordings. They extracted event-driven temporal features from eye regions and used these for ocular motion segmentation and liveness classification with a spiking convolutional neural network (SCNN).

Result: Event-based representations reliably discriminated between genuine and replayed sequences, achieving up to 95.37% top-1 accuracy using the spiking CNN.

Conclusion: Event-based sensing shows strong potential for robust, low-latency liveness detection, overcoming key limitations of RGB-based approaches.
...
### [36] [SynSur: An end-to-end generative pipeline for synthetic industrial surface defect generation and detection](https://arxiv.org/abs/2604.26633)
*Paul Julius Kühn,Mika Pommeranz,Arjan Kuijper,Saptarshi Neil Sinha*

Main category: cs.CV

TL;DR: This paper presents an end-to-end pipeline using VLM prompts, LoRA-adapted diffusion, and mask-guided inpainting to generate realistic synthetic defect data for industrial inspection, showing that synthetic data strengthens scarce real datasets but cannot replace them.


<details>
  <summary>Details</summary>
Motivation: Scarcity of labeled defect data (defects are rare, annotations expensive, balanced training sets slow to collect) limits learning-based industrial defect detection more than model capacity.

Method: End-to-end pipeline combining VLM-based prompts, LoRA-adapted diffusion, mask-guided inpainting, and sample filtering with DreamSim/CLIPScore, plus automatic label derivation.

Result: Synthetic-only training cannot replace real data; combined with real data, synthetic defects preserve performance and yield modest gains. Validated on ball screw pitting (BSData) and MSD dataset for cross-domain.

Conclusion: The strongest value of diffusion-based defect synthesis lies in strengthening scarce real datasets rather than substituting for them; domain-specific adaptation and annotation-quality control are critical.
...
### [37] [The Unseen Adversaries: Robust and Generalized Defense Against Adversarial Patches](https://arxiv.org/abs/2604.26317)
*Vishesh Kumar,Akshay Agarwal*

Main category: cs.CV

TL;DR: This paper introduces a novel dataset combining adversarial patch attacks and natural noises, and benchmarks singularity detection using CNN features with traditional ML classifiers, revealing that independent adversarial defenses are ineffective and classifier selection matters.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks are vulnerable to singularities like adversarial patches and natural noises (Gaussian, Salt&Pepper), but there is a lack of research addressing both types independently and in combination.

Method: The authors created a novel dataset combining adversarial patch attacks and natural noises, extracted features from several CNNs, and applied traditional ML classifiers (instead of neural network tuning) for singularity detection benchmarking.

Result: Experiments across in-distribution and out-of-distribution singularities show that defending against adversarial attacks independently is hard, and using inefficient classifiers leads to poor performance.

Conclusion: Combined singularities pose a significant challenge; effective defense requires careful classifier selection rather than relying on independent treatment of adversarial and natural noises.
...
### [38] [Point Cloud Registration via Probabilistic Self-Update Local Correspondence and Line Vector Sets](https://arxiv.org/abs/2604.26318)
*Kuo-Liang Chung,Yu-Cheng Lin,Wu-Chi Chen*

Main category: cs.CV

TL;DR: A fast point cloud registration algorithm using probabilistic self-updating local correspondence and line vector sets with dual RANSAC, achieving at least 10% RMSE improvement and superior efficiency.


<details>
  <summary>Details</summary>
Motivation: Point cloud registration is fundamental for integrating 3D observations in remote sensing; existing methods lack both speed and accuracy.

Method: Dual RANSAC interaction model (global + local RANSAC); local sets built via angle histogram statistics and line vector length preservation; probabilistic self-updating strategy refines local sets; global early termination condition; weighted SVD for final estimation.

Result: At least 10% RMSE improvement over state-of-the-art methods; superior time efficiency; validated on public datasets.

Conclusion: The proposed algorithm achieves state-of-the-art accuracy and efficiency for PCR, with publicly available C++ source code.
...
### [39] [Motion-Driven Multi-Object Tracking of Model Organisms in Space Science Experiments](https://arxiv.org/abs/2604.26321)
*Jianing You,Han Wang,Kang Liu,Jiale Ding,Fengjie Chu,Zihan Guo,Shengyang Li*

Main category: cs.CV

TL;DR: ART-Track is a motion-driven multi-animal tracking framework that uses multi-model motion estimation, motion-state-driven association, and uncertainty-adaptive fusion to significantly reduce identity switches in microgravity biological videos (SpaceAnimal-MOT dataset) for zebrafish and fruitfly sequences.


<details>
  <summary>Details</summary>
Motivation: Multi-animal tracking in space science experimental videos is highly challenging due to weak appearance cues, low-quality imaging, complex maneuvering behaviors, and frequent interactions under microgravity conditions.

Method: (1) Multi-model motion estimation for handling abrupt maneuvers and nonlinear motion; (2) Motion-state-driven association to reduce identity switches under dense interactions; (3) Uncertainty-adaptive fusion to dynamically balance spatial and motion cues.

Result: ART-Track significantly reduces identity switches on zebrafish and fruitfly sequences, maintaining more stable association under occlusion, deformation, and high-density interactions.

Conclusion: ART-Track provides a more reliable tracking foundation for downstream quantitative behavior analysis in challenging microgravity multi-animal scenarios.
...
### [40] [MemOVCD: Training-Free Open-Vocabulary Change Detection via Cross-Temporal Memory Reasoning and Global-Local Adaptive Rectification](https://arxiv.org/abs/2604.26774)
*Zuzheng Kuang,Honghao Chang,Boqiang Liang,Haoqian Wang,Lijun He,Fan Li,Haixia Bi*

Main category: cs.CV

TL;DR: MemOVCD is a training-free open-vocabulary change detection framework that reformulates bi-temporal change detection as a two-frame tracking problem, using weighted bidirectional propagation and histogram-aligned transition frames for cross-temporal memory reasoning, plus global-local adaptive rectification for spatial consistency.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary change detection methods process each timestamp independently or interact only at the final stage, causing insufficient temporal coupling. This limits distinguishing genuine semantic changes from non-semantic appearance discrepancies, and patch-dominant inference on high-res images weakens global semantic continuity, producing fragmented change regions.

Method: (1) Reformulates bi-temporal change detection as a two-frame tracking problem with weighted bidirectional propagation for cross-temporal memory reasoning. (2) Constructs histogram-aligned transition frames to smooth abrupt appearance changes across large temporal gaps. (3) Applies global-local adaptive rectification to fuse local and global-view predictions for spatial consistency and fine detail preservation.

Result: Achieves favorable performance on two change detection tasks across five benchmarks, demonstrating effectiveness and generalization under diverse open-vocabulary settings.

Conclusion: MemOVCD effectively addresses temporal coupling and spatial consistency issues in open-vocabulary change detection without requiring training, validating its generalization capability.
...
### [41] [Federated Medical Image Classification under Class and Domain Imbalance exploiting Synthetic Sample Generation](https://arxiv.org/abs/2604.26324)
*Martina Pavan,Matteo Caligiuri,Francesco Barbato,Pietro Zanuttigh*

Main category: cs.CV

TL;DR: FedSSG is a federated learning framework that uses synthetic sample generation and distribution to address domain shifts from diverse imaging devices and class imbalance from rare pathologies in medical imaging.


<details>
  <summary>Details</summary>
Motivation: Medical imaging faces challenges: strict privacy constraints, heterogeneous imaging devices causing domain shifts, and class imbalance from uneven pathology prevalence.

Method: A federated learning framework (FedSSG) that generates synthetic samples and distributes them across clients to improve coverage of underrepresented pathologies and imaging devices.

Result: Significantly enhances model performance and generalization across heterogeneous institutions with minimal client-side computational overhead.

Conclusion: FedSSG effectively addresses domain shifts and class imbalance in federated medical imaging while maintaining privacy and efficiency.
...
### [42] [ViCrop-Det: Spatial Attention Entropy Guided Cropping for Training-Free Small-Object Detection](https://arxiv.org/abs/2604.26806)
*Hui Wang,Hongze Li,Wei Chen,Xiaojin Zhang*

Main category: cs.CV

TL;DR: ViCrop-Det is a training-free inference framework that uses Spatial Attention Entropy to adaptively shrink spatial trust regions and inject high-frequency local observations, improving small object detection by +1-3 mAP@50 with marginal latency overhead.


<details>
  <summary>Details</summary>
Motivation: Transformer-based detectors suffer from local feature degradation due to uniform global receptive fields applied across spatially heterogeneous natural images, especially for dense microscopic targets in conflict zones.

Method: ViCrop-Det uses the detection decoder's cross-attention distribution as an endogenous probe, computes Spatial Attention Entropy (SAE) to evaluate local spatial ambiguity, then executes dynamic spatial routing to allocate a fixed computational budget to regions with high target saliency and high cognitive uncertainty, shrinking the spatial trust region and injecting high-frequency localized observations.

Result: +1-3 mAP@50 on VisDrone and DOTA-v1.5 for RT-DETR-R50 and Deformable DETR with 20-23% latency overhead; improved AP_S on MS COCO while AP_M/AP_L remain stable; outperforms uniform slicing baselines under compute-matched settings.

Conclusion: Adaptive spatial trust region shrinkage effectively resolves local ambiguity and recovers fine-grained features without architectural modifications, achieving a highly optimized accuracy-speed trade-off.
...
### [43] [SpatialFusion: Endowing Unified Image Generation with Intrinsic 3D Geometric Awareness](https://arxiv.org/abs/2604.26341)
*Haiyi Qiu,Kaihang Pan,Jiacheng Li,Juncheng Li,Siliang Tang,Yueting Zhuang*

Main category: cs.CV

TL;DR: SpatialFusion internalizes 3D geometric awareness into unified image generation models via a Mixture-of-Transformers architecture and depth adapter, outperforming GPT-4o on spatially-aware tasks with minimal overhead.
...
Method: (1) Mixture-of-Transformers (MoT) augmenting MLLM with a parallel spatial transformer for metric-depth map prediction; (2) depth adapter injecting geometric scaffolds into diffusion backbone; (3) progressive two-stage training strategy.
...
Conclusion: Explicit 3D geometric guidance significantly enhances spatially-coherent image generation, and SpatialFusion offers a practical, high-performance framework with minimal extra cost.
...
### [44] [Which Face and Whose Identity? Solving the Dual Challenge of Deepfake Proactive Forensics in Multi-Face Scenarios](https://arxiv.org/abs/2604.26342)
*Lei Zhang,Zhiqing Guo,Dan Ma,Gaobo Yang*

Main category: cs.CV

TL;DR: DAWF is a deep attributable watermarking framework for detecting and tracing deepfakes in multi-person scenarios by embedding watermarks in facial regions and using selective supervision to localize forgeries and identify the source.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake forensics focus on single-face settings, failing to handle complex multi-person scenarios (e.g., group photos, meetings). There is a need for solutions that can both localize and trace deepfakes in such environments.

Method: DAWF uses a multi-face encoder-decoder architecture for in-network parallel watermark embedding and cross-face processing, along with a selective regional supervision loss that guides the decoder to focus only on deepfake-tampered facial regions.
...
Conclusion: DAWF effectively addresses the "which + who" problem—identifying which facial region was forged and who was the source—in complex multi-person deepfake scenarios.
...
### [45] [GateMOT: Q-Gated Attention for Dense Object Tracking](https://arxiv.org/abs/2604.26353)
*Mingjin Lv,Zelin Liu,Feifei Shao,Yi-Ping Phoebe Chen,Junqing Yu,Wei Yang,Zikai Song*

Main category: cs.CV

TL;DR: GateMOT introduces Q-Gated Attention to replace costly quadratic self-attention with linear-complexity gating, achieving state-of-the-art dense object tracking results on BEE24 and other benchmarks.


<details>
  <summary>Details</summary>
Motivation: Vanilla attention's quadratic complexity is computationally prohibitive for high-resolution dense motion estimation, preventing Dense Object Tracking from leveraging attention-based modeling in crowded/occlusion-heavy scenes.

Method: GateMOT proposes Q-Gated Attention where Query is repurposed as a learnable gating unit (Gating-Query) that produces probabilistic gates to modulate Key features element-wise, enabling linear-complexity multi-task decoding for detection, motion, and re-identification.

Result: State-of-the-art HOTA 48.4, MOTA 67.8, IDF1 64.5 on BEE24 benchmark; strong performance on additional Dense Object Tracking benchmarks.

Conclusion: Q-Attention is a simple, effective, and transferable building block for attention-based tracking in dense scenarios.
...
### [46] [CO-EVO: Co-evolving Semantic Anchoring and Style Diversification for Federated DG-ReID](https://arxiv.org/abs/2604.26363)
*Fengchun Zhang,Qiang Ma,Liuyu Xiang,Jinshan Lai,Tingxuan Huang,Jianwei Hu*

Main category: cs.CV

TL;DR: CO-EVO is a federated learning framework for person re-identification that resolves domain generalization through a co-evolutionary mechanism combining camera-invariant semantic anchoring and global style diversification, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Federated domain generalization for person re-identification faces challenges from stylistic gaps across decentralized clients, where models succumb to shortcut learning by overfitting to domain-specific camera biases instead of learning universal identity features, due to lack of global supervision.

Method: CO-EVO introduces two components: (1) Camera-Invariant Semantic Anchoring (CSA) learns identity prompts with cross-camera consistency for domain-agnostic anchors; (2) Global Style Diversification (GSD) with Global Camera-Style Bank (GCSB) synthesizes perturbations. A co-evolutionary loop uses purified anchors as gravitational centers guiding the encoder toward robust features.

Result: Extensive experiments demonstrate state-of-the-art (SOTA) performance, proving that the synergy between semantic purification and style expansion is essential for robust cross-domain generalization.

Conclusion: The co-evolutionary mechanism effectively resolves the semantic-style conflict in federated domain generalization, with code publicly available at provided GitHub repository.
...
### [47] [Beyond Fixed Formulas: Data-Driven Linear Predictor for Efficient Diffusion Models](https://arxiv.org/abs/2604.26365)
*Zhirong Shen,Rui Huang,Jiacheng Liu,Chang Zou,Peiliang Cai,Shikang Zheng,Zhengyi Shi,Liang Feng,Linfeng Zhang*

Main category: cs.CV

TL;DR: L2P introduces learnable linear predictors for feature caching in Diffusion Transformers, achieving up to 7.18x acceleration with minimal quality loss, trained in ~20 seconds.


<details>
  <summary>Details</summary>
Motivation: Existing feature caching methods for DiTs use hand-crafted forecasting formulas that fail under aggressive skipping, limiting acceleration potential.

Method: L2P replaces fixed coefficients with learnable per-timestep weights, creating a data-driven caching framework that is rapidly trained (~20 seconds on a single GPU) to reconstruct current features from past trajectories.

Result: 4.55x FLOPs reduction and 4.15x latency speedup on FLUX.1-dev; maintains high visual fidelity under up to 7.18x acceleration on Qwen-Image models, outperforming prior methods.

Conclusion: Learning linear predictors is a highly effective and efficient approach for accelerating DiT inference, demonstrating that simple data-driven methods can outperform complex hand-crafted solutions.
...
### [48] [Seamless Indoor-Outdoor Mapping for INGENIOUS First Responders](https://arxiv.org/abs/2604.26368)
*Jürgen Wohlfeil,Henry Meißner,Adrian Schischmanow,Thomas Kraft,Dirk Baumbach,Ines Ernst,Dennis Dahlke*

Main category: cs.CV

TL;DR: Combines an autonomous aerial mapping system with a person-carried indoor positioning system using AprilTags to create a seamless indoor-outdoor 3D model in real time for first responder applications.


<details>
  <summary>Details</summary>
Motivation: To create seamless indoor-outdoor 3D models for first responder enhancement in large-scale disasters, where current methods lack automation and real-time integration.

Method: An autonomously flying aerial system geo-references AprilTags and sends coordinates to a ground-based indoor positioning system. The ground system registers to world coordinates by viewing AprilTags before entering the building, then creates an indoor point cloud that aligns with the aerial point cloud without further GPS.

Result: Produces a co-visualized, seamless indoor-outdoor 3D point cloud model in real time, with the indoor point cloud fitting accurately to the aerial view.

Conclusion: The proposed method enables fully automated, GPS-free indoor-outdoor 3D modeling, suitable for first responder operations in disaster scenarios.
...
TL;DR: ToMA uses persistent homology to align multimodal representations by identifying and matching topologically salient edges across vision and language domains, improving generalization to specialized fields like remote sensing and fashion.


<details>
  <summary>Details</summary>
Motivation: Vision-language models generalize poorly to specialized domains; existing semi-supervised methods fail to model global structure of multimodal manifolds, and topology-based alignment methods (persistence diagram matching) neither guarantee geometric alignment nor utilize image-text pairing information.

Method: ToMA applies persistent homology to identify topologically salient edges (H_0-death edges and lightweight H_1-birth edges) and aligns them across modalities using available cross-modal correspondences, capturing both connectivity and cycle structure without constructing 2-simplices.

Result: Stable gains with clear improvements on remote sensing and modest but consistent benefits on fashion retrieval; more stable than alternative topology-based objectives.

Conclusion: Lightweight H_1-birth edges provide useful higher-order structural signals, and ToMA offers a more stable and effective topology-based alignment for semi-supervised vision-language learning.
...
TL;DR: EEGVFusion is a multimodal framework combining self-supervised EEG learning, video encoding, optimal-transport alignment, and cross-attention for reliable seizure detection in mice, achieving near-perfect accuracy (0.9957 BA) and low false-alarm rates (0.6250 FP/h) on curated video-EEG dataset.


<details>
  <summary>Details</summary>
Motivation: Manual video-EEG review is labor-intensive, and single-modality seizure detection systems fail due to complementary weaknesses: video methods confounded by benign behaviors, EEG methods vulnerable to motion artifacts.

Method: Multimodal framework combining self-supervised EEG representation learning, spatio-temporal video encoding, optimal-transport alignment, and bidirectional cross-attention; evaluated on expert-annotated dataset of 93 sessions from 15 mice.

Result: Random-split: 0.9957 Balanced Accuracy, perfect sensitivity, 0.6250 FP/h. Held-out subject: 0.9718 BA, reduced FAR from 2.7250 to 0.4833 FP/h with perfect sensitivity. Ablations confirm EEG pre-training and OT alignment reduce false alarms.

Conclusion: EEGVFusion effectively integrates neural and behavioral evidence for reliable seizure detection, significantly reducing false alarms compared to EEG-only methods while maintaining perfect sensitivity.
...
TL;DR: A few-shot object detection framework using vision foundation models that constructs class prototypes from limited reference samples and matches them with query scene embeddings via similarity matching, achieving 6.9% AP improvement over training-free methods on industrial datasets.


<details>
  <summary>Details</summary>
Motivation: Industrial object detection requires large annotated datasets that are expensive to collect and hard to maintain when object inventories change frequently; the paper addresses few-shot detection with minimal supervision.

Method: Constructs class prototypes from few reference samples using feature extraction, then applies segmentation to query scenes, extracts embeddings, and matches them with prototypes via similarity matching.

Result: Improves AP by 6.9% over state-of-the-art training-free methods on three industrial datasets from the BOP benchmark; can onboard new objects with few reference images without CAD models.

Conclusion: The approach is well-suited for real-world industrial applications due to minimal supervision requirements and ability to handle frequently changing object inventories.
...
TL;DR: This paper applies Sparse Autoencoders (SAEs) to Vision Transformers (ViTs) for out-of-distribution (OOD) detection, introducing Class Activation Profiles (CAPs) and a core energy divergence scoring function that achieves strong FPR95 results.


<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods rely on entangled feature representations and the potential of SAEs for analyzing ViTs remains under-explored.

Method: Uses a Top-k SAE to disentangle ViT [CLS] token features into a structured latent space, defines Class Activation Profiles (CAPs), and proposes a scoring function based on core energy profile divergence.

Result: Achieves strong results on FPR95 metric and competitive AUROC across multiple benchmarks.
...
TL;DR: This paper proposes SDL-COVID, a CNN-based methodology for COVID-19 detection from chest X-rays, critically analyzing the necessity of lung segmentation and the risks of excessive data augmentation, achieving 95.21% precision with lower false negative rates.


<details>
  <summary>Details</summary>
Motivation: Existing COVID-19 chest X-ray classification works often lack lung segmentation, raising reliability concerns, and employ disproportionate augmentation leading to overfitting and poor generalization.

Method: Uses class activation mapping (CAM) with CNNs to visually validate the necessity of lung segmentation. Compares models trained on augmented vs. non-augmented datasets to analyze augmentation effects.

Result: Lung segmentation is essential for accurate prediction. Test accuracy drops beyond a certain augmentation threshold, confirming overfitting. SDL-COVID achieves 95.21% precision with a lower false negative rate.

Conclusion: The proposed SDL-COVID methodology ensures reliable and generalized COVID-19 detection from chest X-rays by incorporating lung segmentation and avoiding excessive augmentation.
...
TL;DR: AMDD is a multimodal deepfake detection framework that jointly detects and attributes forgeries by enforcing cross-modal forensic fingerprint consistency, achieving 99.7% accuracy on FakeAVCeleb and strong cross-dataset generalization.


<details>
  <summary>Details</summary>
Motivation: Binary deepfake detectors often learn dataset-specific artifacts instead of genuine generative traces. Attribution-guided learning imposes stronger geometric constraints, forcing the model to encode generator-specific forensic content.

Method: Proposes AMDD with Cross-Modal Forensic Fingerprint Consistency (CMFFC) loss to align generator-induced artifacts across visual and audio streams. Uses ResNet50 + temporal attention for video and ResNet18 for mel spectrograms.

Result: 99.7% balanced accuracy, 99.8% AUC, 95.9% attribution accuracy on FakeAVCeleb. Robust generalization on DeepfakeTIMIT, DFDM, LAV-DF for real videos; fake detection on unseen generators remains challenging.

Conclusion: Attribution-guided multimodal learning improves deepfake detection by enforcing forensically meaningful representation geometry, though detecting fakes from unseen generators is still an open problem.
...
TL;DR: This paper analyzes DINOv3's layer-wise 3D information distribution in monocular depth estimation, finding non-uniform distribution with deeper layers encoding stronger geometric cues. They propose a Last-Layer-Centric Feature Recombination (LFR) module that treats the final layer as a geometric anchor and selectively fuses complementary intermediate layers, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing MDE methods uniformly sample intermediate transformer layers for multi-scale features, assuming geometric information is evenly distributed across layers. This likely underutilizes the structural 3D cues in vision foundation models.

Method: Systematic layer-wise analysis of DINOv3, revealing non-uniform 3D information distribution (deeper layers better). Proposes LFR module: uses final layer as geometric anchor, selects complementary intermediate layers via minimal-similarity criterion, and fuses features via compact linear adapters.

Result: LFR module consistently improves MDE accuracy and achieves state-of-the-art performance across extensive experiments.

Conclusion: Geometric knowledge in VFMs is organized non-uniformly across layers. The proposed LFR strategy efficiently unlocks VFMs' potential for dense 3D tasks.
...
TL;DR: PKS⁴ introduces a plug-and-play module using linear-complexity State Space Models with kinematic priors for efficient temporal modeling in videos, achieving SOTA with 10× lower training cost than pure video SSMs.


<details>
  <summary>Details</summary>
Motivation: Traditional video models face quadratic costs for long videos; adapters cause high activation memory; pure SSMs disrupt 2D spatial structure.

Method: Retains 2D vision backbone; inserts a PKS⁴ module with Kinematic Prior Encoder (captures motion via inter-frame differences) driving linear-complexity SSMs; parallel temporal scanners per spatial location.

Result: State-of-the-art on both spatial-heavy and temporal-heavy action recognition; converges in 20 epochs; ~10× lower training compute than pure video SSMs.

Conclusion: PKS⁴ establishes a new efficient video understanding paradigm by combining spatial backbone integrity with linear-complexity temporal scanning driven by kinematic priors.
...
TL;DR: A multistage extraction framework integrating preprocessing, multilingual OCR, hybrid page retrieval, and compact VLM outperforms direct PDF-to-VLM baselines by up to 31.9 pp on 120 multilingual KYC documents, achieving 87.27% accuracy with PaddleOCR + MiniCPM2.6.


<details>
  <summary>Details</summary>
Motivation: Industrial KYC/compliance workflows require structured information extraction from long, multilingual scanned financial documents that are non-machine-readable, noisy, heterogeneous, and span dozens of pages with sparse relevant information. Direct end-to-end VLM application yields unreliable extraction under real-world conditions.

Method: A multistage pipeline: (1) image preprocessing, (2) multilingual OCR, (3) hybrid page-level retrieval for sparse relevant pages, (4) compact VLM-based structured extraction. This separates page localization from multimodal reasoning.

Result: On 120 production KYC documents (~3000 multilingual scanned pages), the pipeline consistently outperforms direct PDF-to-VLM baselines, improving field-level accuracy by up to 31.9 percentage points. Best configuration (PaddleOCR + MiniCPM2.6) achieves 87.27% accuracy. Page-level retrieval is the dominant factor.

Conclusion: Separating page localization from multimodal reasoning via a multistage pipeline significantly boosts extraction accuracy for complex multilingual financial documents, with page retrieval being the key component.
...
TL;DR: Cross-domain transfer of HSI foundation models from remote to proximal sensing achieves strong semantic segmentation performance, outperforming in-domain training and reducing the gap to complex cross-modality methods while preserving spectral information and architectural simplicity.


<details>
  <summary>Details</summary>
Motivation: HSI semantic segmentation suffers from limited data availability in real-world applications; existing cross-modality approaches either discard spectral information or add architectural complexity.

Method: Proposes cross-domain transfer, reusing HSI foundation models pre-trained on remote sensing data directly for proximal sensing tasks, evaluated systematically on the HS3-Bench benchmark.

Result: Cross-domain transfer significantly outperforms in-domain, in-modality training; narrows the gap to cross-modality methods; maintains strong performance under limited data conditions.

Conclusion: Cross-domain transfer enables more effective and practical HSI semantic segmentation by leveraging existing foundation models without modality bridging.
...
TL;DR: LILA learns pixel-accurate feature descriptors from videos via linear in-context learning, using noisy spatio-temporal cues (depth, motion) from off-the-shelf networks, achieving strong results on video object segmentation, surface normal estimation, and semantic segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing vision foundation models lack pixel-level spatio-temporal representations: image-based pretext tasks ignore dynamics, while video-based action-level reasoning doesn't scale to dense pixel-level prediction.

Method: LILA uses linear in-context learning with spatio-temporal cue maps (depth and motion) from off-the-shelf networks, trained on uncurated video datasets.

Result: Demonstrated compelling performance on video object segmentation, surface normal estimation, and semantic segmentation.

Conclusion: LILA effectively embeds semantic and geometric properties in a temporally consistent manner, despite noisy cues, enabling pixel-accurate video understanding.
...
TL;DR: Proposes RAAT, a new adversarial training method that harmonizes accuracy and robustness by aligning input and latent spaces, reducing perturbation intensity for boundary samples, and adding a semantic alignment regularization.


<details>
  <summary>Details</summary>
Motivation: Identifies that varying perturbation intensity for boundary samples minimally impacts robustness, revealing misalignment between input and latent spaces as a key driver of the robustness-accuracy trade-off in AT.

Method: Defines Robust Alignment as a new AT target, achieved via (1) reduced/fixed perturbation for boundary samples, and (2) Domain Interpolation Consistency Adversarial Regularization (DICAR) for semantic alignment between input and latent spaces.

Result: Outperforms 4 common baselines and 14 SOTA methods on CIFAR-10, CIFAR-100, and Tiny-ImageNet with ResNet-18, PreActResNet-18, and WideResNet-28-10.

Conclusion: RAAT effectively mitigates the robustness-accuracy trade-off by aligning input and latent spaces and treating perturbations as learnable patterns rather than noise.
...
TL;DR: SAMG replaces uniform CFG with geometry-aware, spatially adaptive guidance scales to resolve the detail-artifact dilemma in diffusion models for both images and videos.


<details>
  <summary>Details</summary>
Motivation: Standard CFG uses a globally uniform scalar, causing a "detail-artifact dilemma": low scales fail to inject semantics, high scales cause degradation, over-saturation, and temporal inconsistencies. The authors trace this to a geometric flaw in Tweedie's Formula.

Method: Using differential geometry, the authors show CFG is a tangential linear extrapolation on a curved data manifold, causing orthogonal deviation. They derive a theoretical upper bound and propose SAMG: a training-free, zero-cost algorithm that computes point-wise conditional guidance energy, applying minimum scale to high-energy boundary regions (preserving texture) and maximum scale to low-energy regions (maximizing semantics).

Result: SAMG resolves the detail-artifact dilemma across SD 1.5, SDXL, SD3.5 Medium, CogVideoX, and ModelScope, achieving superior semantic alignment, structural integrity, and temporal smoothness with no computational overhead.

Conclusion: Geometry-aware adaptive guidance is an effective, zero-cost fix for CFG's fundamental flaw, generalizing across image and video diffusion architectures.
...
TL;DR: MTCurv is a deep learning framework for direct, segmentation-free regression of microtubule curvature maps from noisy microscopy images, using attention-based residual U-Net with gradient-aware loss, achieving accurate curvature estimation even with background fluorescence.


<details>
  <summary>Details</summary>
Motivation: Reliable curvature extraction from fluorescence microscopy images remains challenging due to noise, low contrast, and partial filament visibility; existing segmentation-based approaches are highly sensitive to errors and often fail under adverse imaging conditions.

Method: Deep learning framework (attention-based residual U-Net) for direct regression of curvature maps, trained on synthetic data with pixel-wise curvature annotations, using gradient-aware loss combining MSE with gradient consistency term.

Result: MTCurv accurately recovers local microtubule curvatures even with background fluorescence; Spearman correlation emerged as the most reliable metric for curvature prediction quality; ablation studies confirmed contributions of residual encoding and attention-based decoding.

Conclusion: Provides a practical tool for filament curvature analysis and methodological insights for geometry-aware regression in biomedical imaging; datasets and code are made publicly available.
...
TL;DR: GIFGuard is the first spatiotemporal watermarking framework for deepfake proactive forensics in GIFs, featuring a novel encoder (STARE) and decoder (DIRD) architecture, plus a new benchmark dataset (GIFfaces).


<details>
  <summary>Details</summary>
Motivation: Existing proactive forensics methods are designed for static images and cannot handle animated GIFs, which are a representative short-loop temporal media widely shared on social networks and increasingly vulnerable to deepfake threats.

Method: Proposes GIFGuard with two key components: (1) STARE (Spatiotemporal Adaptive Residual Encoder) using 3D convolutions with adaptive channel recalibration for robust watermark embedding, and (2) DIRD (Deep Integrity Restoration Decoder) using a spatiotemporal hourglass with 3D attention for accurate watermark extraction under facial manipulation. Also introduces GIFfaces, a large-scale benchmark dataset.

Result: Achieves high-fidelity visual quality and remarkable robustness against deepfake attacks on GIF imagery.

Conclusion: GIFGuard effectively bridges the gap in proactive deepfake forensics for animated GIFs, and the released dataset and code will facilitate further research in this domain.
...
TL;DR: First to address Single-View Aerial-Ground Re-Identification (SV AG-ReID) by proposing 3D-LENS, a framework using 3D mesh reconstruction and novel view synthesis to bridge the viewpoint-domain gap without paired cross-view annotations.


<details>
  <summary>Details</summary>
Motivation: Existing AG-ReID methods rely on paired cross-view annotations, which are unavailable in real-world deployments like wilderness search-and-rescue. The drastic viewpoint gap between aerial and ground images occludes discriminative features, making cross-view retrieval challenging.

Method: Proposes 3D Lifting-based Elevated Novel-view Synthesis (3D-LENS), combining geometrically-consistent novel view synthesis via large-scale 3D mesh reconstruction with robust representation learning to mitigate synthetic-to-real bias.

Result: Achieves state-of-the-art performance on Single-View AG-ReID scenarios, outperforming 2D generative baselines and prior 3D methods.

Conclusion: 3D-LENS effectively unifies novel view synthesis and representation learning for SV AG-ReID, overcoming viewpoint-domain gaps without requiring paired cross-view data or predefined class-specific templates.
...
TL;DR: Introduces DenseStep2M, a large-scale dataset (~100K videos, 2M steps) generated via an automated pipeline using multimodal LLMs (Qwen2.5-VL, DeepSeek-R1) for high-quality procedural annotations from instructional videos, showing strong gains in dense captioning, step grounding, and cross-modal retrieval.


<details>
  <summary>Details</summary>
Motivation: Long-term video understanding is hindered by noisy ASR transcripts and inconsistent temporal alignments in instructional video corpora like HowTo100M, motivating the need for cleaner, structured procedural annotations.

Method: An automated, training-free pipeline that segments videos into shots, filters misaligned content, and uses Qwen2.5-VL and DeepSeek-R1 to generate temporally grounded procedural steps, producing the DenseStep2M dataset.

Result: Auto-generated steps align well with human annotations (validated on DenseCaption100 benchmark); models fine-tuned on DenseStep2M achieve significant improvements in captioning quality, temporal localization, and robust zero-shot generalization across egocentric, exocentric, and mixed-perspective domains.

Conclusion: DenseStep2M effectively facilitates multimodal alignment and long-term activity reasoning, proving its utility for downstream tasks like dense video captioning, procedural step grounding, and cross-modal retrieval.
...
TL;DR: AirZoo is a large-scale aerial 3D vision dataset with 378 regions across 22 countries, offering scalable rendering, diverse environments, and rich geometric annotations. It boosts SoTA models significantly via pre-training and fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks are biased toward ground-level/object-centric views and lack large-scale, high-fidelity aerial training data for 3D vision under complex viewpoint changes and diverse weather/illumination.

Method: Leveraging freely available photogrammetric 3D meshes, AirZoo renders vast outdoor environments with customizable UAV trajectories, configurable weather/illumination, and provides synchronized metric depth and 6-DoF poses.

Result: Fine-tuning on AirZoo yields substantial performance gains for SoTA models (MegaLoc, RoMa, VGGT, Depth Anything 3) on public and real-world benchmarks, establishing a new upper bound for aerial spatial intelligence.

Conclusion: AirZoo serves as a powerful pre-training engine for aerial 3D vision, demonstrating that large-scale synthetic aerial data can significantly advance real-world performance.
...
TL;DR: FunFace proposes a new adaptive margin loss that incorporates biometric utility (Certainty Ratio) to improve face recognition, achieving competitive results on high-quality and superior results on low-quality benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing adaptive margin losses (e.g., AdaFace) use feature norms tied to sample quality for FR, but feature norms don't fully capture biometric utility. The authors aim to directly incorporate biometric utility into the loss function for better performance, especially on low-quality images.

Method: FunFace integrates biometric utility (estimated via Certainty Ratio) into the adaptive margin loss, inspired by AdaFace. This utility metric is used to adjust the margin during training, replacing or supplementing the feature norm.

Result: FunFace achieves competitive results with state-of-the-art FR models on high-quality benchmarks and surpasses them on low-quality benchmarks.

Conclusion: Incorporating biometric utility directly into the margin loss (FunFace) improves FR robustness, especially for low-quality images, outperforming norm-based adaptive margins.
...
TL;DR: Current MLLMs struggle with dial-based measurement reading due to reliance on superficial appearance cues rather than intrinsic state geometry. The proposed TriSCA framework achieves state-consistent alignment across three levels, significantly improving performance under viewpoint and illumination variations.


<details>
  <summary>Details</summary>
Motivation: MLLMs perform poorly on dial-based measurement reading tasks and are brittle under viewpoint/illumination changes, suggesting they ignore the underlying state geometry and rely on superficial appearance cues.

Method: TriSCA (Tri-level State-Consistent Alignment): (1) state-distance-aware representation alignment, (2) metadata-grounded observation-to-state supervision, (3) state-aware objective alignment.

Result: TriSCA demonstrates effectiveness through extensive ablation studies and evaluations on controlled clock/gauge benchmarks and an external real-world benchmark.

Conclusion: Existing MLLMs fail to preserve intrinsic state geometry for dial reading. TriSCA addresses this via multi-level state-consistent alignment, improving robustness and accuracy.
...
TL;DR: SnapPose3D uses diffusion-based denoising with random sampling to generate multiple 3D pose hypotheses from single-frame 2D inputs, achieving state-of-the-art results by aggregating predictions.


<details>
  <summary>Details</summary>
Motivation: Depth ambiguity and joint uncertainty in 2D-to-3D pose lifting lead to multiple plausible 3D poses, which prior methods often address via temporal sequences, incurring high computational cost and data complexity.

Method: A deterministic diffusion-based denoising framework conditioned on visual context and 2D pose features; during inference, multiple hypotheses are generated by random sampling from a unit Gaussian distribution and then aggregated.

Result: State-of-the-art results on standard 3D human pose estimation benchmarks, demonstrating accurate hypothesis generation and aggregation from single-frame input.

Conclusion: SnapPose3D effectively resolves pose ambiguity without temporal sequences, offering a lightweight, real-time capable solution for 3D human pose estimation.
...
TL;DR: This paper recovers sound from solid objects with poor vibration responses by using multi-point speckle vibrometry and a physics-guided model that reverses the object's resonant transfer function to estimate the original sound source.


<details>
  <summary>Details</summary>
Motivation: Prior sound recovery methods rely on objects with favorable vibration responses (e.g., thin membranes). This work tackles the challenge of solid objects with poor or highly resonant vibrations.

Method: Simultaneously captures multi-point multi-axis vibrations using speckle-based vibrometry, then derives a physics-guided vibration formation model relating sound source to vibrations via the object's vibrational modes, reversing the resonant transfer function to fuse signals and estimate the original sound.

Result: The approach significantly outperforms traditional single-point speckle vibrometry and other multi-signal fusion methods on various everyday solid objects.

Conclusion: Multi-point vibrometry with physics-guided modeling enables effective sound recovery from solid objects with poor vibration responses, expanding the applicability of visual microphones.
...
TL;DR: CurEvo proposes a curriculum-guided self-evolution framework for video understanding that dynamically adjusts task difficulty and data diversity based on model competence, improving performance across seven backbones on four VideoQA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing self-evolution video understanding frameworks suffer from weakly controlled optimization and uncontrolled difficulty progression due to lack of structured guidance in the iterative learning process.

Method: CurEvo introduces curriculum learning into self-evolution, dynamically regulating task difficulty, refining evaluation criteria, and balancing data diversity according to model competence, with a multi-dimensional adaptive QA framework for perception, recognition, and understanding.
...
Conclusion: Curriculum-guided self-evolution transforms weakly controlled self-evolution into a more structured learning process for autonomous video understanding.
...
TL;DR: This paper proposes a learned sampling method for BRDF acquisition that selects optimal measurement directions using a pretrained hypernetwork-based reconstructor and differentiable rendering, achieving better reconstruction quality at low measurement budgets (8-16 samples) compared to neural baselines.


<details>
  <summary>Details</summary>
Motivation: Dense BRDF measurements via gonioreflectometer are slow and expensive, motivating the need for efficient sparse measurement selection that still enables high-quality material appearance reconstruction.

Method: Combines a set encoder for sparse observations, a pretrained hypernetwork-based BRDF reconstructor (kept fixed during sampler training), and a differentiable renderer. Gradients from BRDF-space and rendered-image losses optimize measurement locations, separating sample selection from prior fitting.

Result: Outperforms neural reconstruction baselines at 8 and 16 measurements on MERL dataset; PCA-based methods remain competitive at larger budgets.

Conclusion: The proposed learned sampler effectively selects informative measurement directions under the learned material distribution, improving low-budget BRDF reconstruction quality.
...
TL;DR: GLM-5V-Turbo is a native multimodal foundation model that integrates perception, reasoning, planning, tool use, and execution for agentic tasks, showing strong performance in multimodal coding, visual tool use, and framework-based agentic tasks while maintaining competitive text-only coding capability.


<details>
  <summary>Details</summary>
Motivation: Foundation models need agentic capability beyond language reasoning — they must perceive and act across heterogeneous contexts (images, videos, webpages, GUIs). Existing approaches treat multimodal perception as an auxiliary interface rather than a core component.

Method: Integrated multimodal perception as a core component of reasoning, planning, tool use, and execution. Improvements span model design, multimodal training, reinforcement learning, toolchain expansion, and integration with agent frameworks.
...
Conclusion: Multimodal perception, hierarchical optimization, and reliable end-to-end verification are central to building effective multimodal agents.
...
TL;DR: This paper benchmarks modern Vision Foundation Models (VFMs) for AI-generated image detection, finding newer models outperform original CLIP by 12%+ in accuracy. It proposes a tunable attention pooling (TAP) classifier head that achieves state-of-the-art results on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Despite rapid advances in Vision Foundation Models (VFMs), their potential for AI-generated image detection and forensics remains largely unexplored, with most methods still building upon the original CLIP-ViT.

Method: (1) Comprehensive benchmark across multiple VFM families (different pretraining objectives, resolutions, scales). (2) A redesigned classifier head using tunable attention pooling (TAP) to aggregate output tokens into refined global representations.

Result: Best VFM outperforms original CLIP by 12%+ accuracy. TAP + modern VFMs achieves substantial gains and establishes new state-of-the-art on two challenging benchmarks for in-the-wild AI-generated and AI-inpainted image detection.

Conclusion: Modern VFMs significantly outperform original CLIP for AIGI detection, and the proposed TAP classifier head further boosts performance to new state-of-the-art levels.
...
TL;DR: This paper presents an AI-based VR system that creates patient-specific spinal decompression simulations from CT/MRI data, achieving high segmentation accuracy (DSC 0.95 for bone, 0.895 for soft tissue) and registration precision (TRE 1.73mm), with positive surgeon feedback on educational value.


<details>
  <summary>Details</summary>
Motivation: Limited OR exposure in surgical training and lack of patient-specific VR simulations for spinal procedures.

Method: AI computer vision methods for CT/MRI multimodal fusion (registration) and segmentation to automatically generate 3D anatomical models for VR spinal decompression simulation (laminectomy, disc resection, foraminotomy).

Result: High-fidelity models in ~2.5 minutes per case (N=15); DSC of 0.95 for bone, 0.895 for soft tissue; mean TRE of 1.73mm; positive qualitative feedback from surgeons.

Conclusion: The platform reduces time/costs of patient-specific modeling, facilitating pre-operative planning, post-procedural assessments, and comprehensive surgical simulation.
...
TL;DR: MesonGS++ is a size-aware post-training codec for 3D Gaussian compression that achieves >34x compression with high fidelity, combining pruning, octree coding, vector quantization, and mixed-precision quantization, with automatic hyperparameter optimization under target storage budgets.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS compression methods rely on many coupled hyperparameters (pruning, transformation, quantization, entropy coding), making it difficult to control the final compressed size and fully exploit the rate-distortion trade-off.

Method: Combines joint importance-based pruning, octree geometry coding, attribute transformation, selective VQ for higher-degree SH, group-wise mixed-precision quantization with entropy coding. Optimizes reserve ratio and bit-width allocation via discrete sampling and 0-1 integer linear programming under target budget. Uses linear size estimator and CUDA parallel quantization for fast hyperparameter search.

Result: Achieves over 34x compression while preserving rendering fidelity, outperforms state-of-the-art post-training methods, accurately meets target size budgets, and can even surpass vanilla 3DGS PSNR at 20x compression on Stump scene.

Conclusion: MesonGS++ demonstrates effective size-aware 3DGS compression without training, with automatic budget control and superior rate-distortion performance.
...
TL;DR: Bridge is a causal inference framework that uses low-rank bases for front-door adjustment to block confounders and reduce spurious correlations in domain generalization object detection, integrating with both discriminative and generative VFMs to achieve SOTA results across multiple benchmarks.
...
Method: Bridge incorporates causal inference into object detection via learning low-rank bases for front-door adjustment to block confounders' effects, while refining representations by filtering redundant and task-irrelevant components. It integrates seamlessly with both discriminative (DINOv2/3, SAM) and generative (Stable Diffusion) Vision Foundation Models.

Result: Extensive experiments across multiple domain generalization object detection datasets (Cross-Camera, Adverse Weather, Real-to-Artistic, Diverse Weather, and Diverse Weather DroneVehicle) demonstrate superiority over previous state-of-the-art approaches.

Conclusion: Bridge effectively mitigates spurious correlations through causal-based front-door adjustment with low-rank bases, achieving robust domain generalization for object detection.
...
TL;DR: ArtiAD is the first large-scale benchmark for 3D anomaly detection on articulated objects, with 15,229 point clouds across 39 categories. The authors propose SPA-SDF, a pose-conditioned implicit field that separates structural priors from joint embeddings, achieving ~0.88 AUROC and outperforming rigid-based baselines.


<details>
  <summary>Details</summary>
Motivation: Existing 3D anomaly detection relies on a rigid prior (normal geometry is pose-invariant), which fails for articulated objects (e.g., hinges, sliding joints). Pose changes cause structured deformations that get misidentified as anomalies, while real defects remain obscured. No benchmark exists for this problem.

Method: SPA-SDF (Shape-Pose-Aware Signed Distance Field): a continuous pose-conditioned implicit field that factorizes into an articulation-independent structural prior and a Fourier-encoded joint embedding. At inference, articulation is recovered by minimizing reconstruction energy; anomalies are point-wise deviations from the learned manifold.

Result: 0.884 object-level AUROC on seen configurations, 0.874 on unseen configurations, substantially outperforming all rigid-based baselines.

Conclusion: Replacing the rigid pose-invariant prior with a continuous pose-conditioned implicit field enables effective 3D anomaly detection on articulated objects. The code and benchmark will be publicly released.
...
TL;DR: UAPAR is the first EDL-based uncertainty-aware framework for pedestrian attribute recognition, using Evidential Deep Learning with CLIP, cross-attention, spatial priors, and uncertainty-guided curriculum learning to handle low-quality samples and label noise, achieving competitive results on four datasets.


<details>
  <summary>Details</summary>
Motivation: Conventional deterministic PAR methods fail to assess prediction reliability on low-quality samples, reducing robustness in complex real-world scenarios.

Method: UAPAR integrates Evidential Deep Learning (EDL) into a CLIP-based architecture with a Region-Aware Evidence Reasoning module (cross-attention + spatial prior masks) and an uncertainty-guided dual-stage curriculum learning strategy.

Result: Achieves competitive or superior performance on PA100K, PETA, RAPv1, and RAPv2 datasets, with uncertainty estimates that identify challenging/erroneous samples.

Conclusion: UAPAR effectively identifies unreliable predictions and enhances system robustness in complex real-world scenarios through uncertainty estimation.
...
TL;DR: SEAL introduces a plug-and-play adaptation module for single-image sticker personalization in diffusion models, addressing visual entanglement and structural rigidity through semantic-aware spatial attention, split-merge token strategy, and structure-aware layer restriction, along with a new StickerBench dataset for evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing personalized text-to-image generation from a single reference suffers from visual entanglement (background artifacts absorbed into concept) and structural rigidity (memorizing reference-specific spatial configurations, losing contextual control), especially in sticker personalization requiring explicit attribute edits.

Method: SEAL is a plug-and-play adaptation module with three components: (1) Semantic-guided Spatial Attention Loss, (2) Split-merge Token Strategy, and (3) Structure-aware Layer Restriction. It integrates into existing personalization pipelines without modifying U-Net backbones. Also introduces StickerBench dataset with six-attribute schema (Appearance, Emotion, Action, Camera Composition, Style, Background).

Result: SEAL consistently improves identity preservation while maintaining contextual controllability, demonstrating effectiveness of explicit spatial and structural constraints during test-time adaptation on the StickerBench dataset.

Conclusion: SEAL effectively mitigates overfitting issues in single-image sticker personalization, and StickerBench enables systematic evaluation of identity disentanglement and contextual controllability.
...
TL;DR: Proposes GSCNet, a graph-based semantic calibration network for unaligned UAV RGBT image semantic segmentation, with a Feature Decoupling and Alignment Module (FDAM) for cross-modal spatial correction and a Semantic Graph Calibration Module (SGCM) leveraging hierarchical category priors. Also introduces URTF, a large benchmark with 25K+ unaligned RGBT image pairs across 61 categories.


<details>
  <summary>Details</summary>
Motivation: Fine-grained UAV RGBT semantic segmentation faces two coupled challenges: cross-modal spatial misalignment (from sensor parallax and vibration) and severe semantic confusion among fine-grained ground objects in top-down aerial views.

Method: (1) FDAM decouples modalities into shared structural and private perceptual components, then performs deformable alignment in the shared subspace. (2) SGCM encodes hierarchical taxonomy and co-occurrence regularities of ground objects into a category graph for graph-attention reasoning to calibrate predictions. (3) URTF benchmark with 25K+ unaligned RGBT pairs across 61 categories.

Result: GSCNet significantly outperforms SOTA methods on URTF, especially on fine-grained categories.

Conclusion: GSCNet effectively addresses cross-modal misalignment and semantic confusion for unaligned UAV RGBT segmentation, and the URTF benchmark provides a valuable resource for future research.
...
TL;DR: AnimateAnyMesh++ is a feed-forward framework for text-driven 3D mesh animation, featuring an expanded DyMesh-XL dataset (300K identities), an upgraded DyMeshVAE-Flex with power-law attention and vertex-normal features, and variable-length rectified-flow generation for longer, higher-quality animations.


<details>
  <summary>Details</summary>
Motivation: High-quality animated 3D model creation remains challenging due to complex spatio-temporal distributions and scarce 4D training data.

Method: (1) Expanded DyMesh-XL dataset from 60K to 300K identities via Objaverse-XL mining; (2) DyMeshVAE-Flex with power-law topology-aware attention and vertex-normal enhanced features; (3) Variable-length sequence training in both VAE and rectified-flow generator.

Result: AnimateAnyMesh++ generates semantically accurate, temporally coherent mesh animations within seconds, outperforming prior methods in quality and efficiency.

Conclusion: The enlarged dataset, upgraded VAE, and variable-length RF generator collectively deliver consistent gains, enabling superior 4D content creation from text.
...
TL;DR: A method to capture and reconstruct high-speed 3D dynamic scenes using only unaugmented low-speed cameras by encoding temporal information via sequential color-coded illumination and decoding with dynamic Gaussian Splatting.


<details>
  <summary>Details</summary>
Motivation: Conventional cameras are bandwidth-limited to 30-60 FPS, restricting 3D dynamic scene reconstruction to static or slowly evolving scenes. Existing high-speed computational imaging methods require hardware/optics modifications and are limited to single-view capture, unable to produce 3D representations.

Method: Illuminates scenes with rapid sequential color-coded patterns to encode high-speed temporal information into spatial intensity/color variations of multi-view low-speed camera images. A dynamic Gaussian Splatting-based approach then decodes this information to reconstruct high-speed volumetric representations.

Result: Evaluated on simulated scenes and real-world multi-camera experiments, demonstrating first-of-a-kind high-speed volumetric scene reconstructions using only unaugmented low-speed cameras.

Conclusion: The proposed method successfully enables high-speed 3D dynamic scene capture without hardware modifications, achieving volumetric reconstruction of rapid motion through color-coded illumination and neural rendering techniques.
...
TL;DR: World2VLM distills spatial imagination from a generative world model into a vision-language model via a training framework, enabling VLMs to perform dynamic spatial reasoning without expensive inference-time generation.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with dynamic spatial reasoning involving egocentric motion. Existing solutions either lack explicit motion-conditioned state modeling (scaling supervision) or incur high computational cost (world model coupling).

Method: World2VLM uses a view-consistent world model to synthesize geometrically aligned future views from initial observations and camera trajectories, then derives structured supervision for forward and inverse spatial reasoning. A two-stage post-training is applied on compact synthetic data.

Result: Consistent improvements over base VLM on SAT-Real, SAT-Synthesized, VSI-Bench, and MindCube. Outperforms test-time world-model-coupled methods without expensive inference generation.

Conclusion: World models can serve as effective training-time teachers rather than just inference-time tools, enabling VLMs to internalize spatial imagination scalably and efficiently.
...
TL;DR: ProcFunc is a Blender-based Python library for procedural 3D generation that simplifies creating, combining, and executing procedural code, enabling large-scale diverse training data generation with fewer coding errors when used by VLMs.


<details>
  <summary>Details</summary>
Motivation: To streamline procedural 3D generation in Blender by providing easy-to-use Python functions that reduce coding errors and enable large-scale diverse training data creation.

Method: A library of Python functions for Blender-based procedural 3D generation, supporting combinatorial compositions of semantic components, with a new indoor room generator featuring compositional procedural materials.

Result: Demonstrates detail, runtime efficiency, and diversity of the room generator, and shows VLMs can use ProcFunc to edit procedural code with significantly fewer coding errors.

Conclusion: ProcFunc enables efficient procedural 3D generation and synthetic data creation, with open-source code available on GitHub.
...
TL;DR: This paper presents a 21-day deployment of 3,505 autonomous language-model agents trading real ETH in a bounded onchain market, producing $20M in volume and 99.9% settlement success. Reliability emerged from the operating layer around the model, not the base model alone. Pre-launch testing exposed failures like fabricated trading rules and fee paralysis; targeted harness changes significantly reduced these issues.


<details>
  <summary>Details</summary>
Motivation: To study reliability of autonomous language-model agents that translate user mandates into validated tool actions under real capital, and to identify failure modes not captured by text-only benchmarks.

Method: Deployed DX Terminal Pro for 21 days with 3,505 user-funded agents trading real ETH. Agents used structured controls and natural-language strategies. The system included prompt compilation, typed controls, policy validation, execution guards, memory design, and trace-level observability. Pre-launch testing identified failure modes, and targeted harness changes were applied.

Result: 7.5M agent invocations, ~300K onchain actions, ~$20M volume, 5,000+ ETH deployed, ~70B inference tokens, 99.9% settlement success. Harness changes reduced fabricated sell rules from 57% to 3%, fee-led observations from 32.5% to below 10%, and increased capital deployment from 42.9% to 78.0%.

Conclusion: Capital-managing agents should be evaluated across the full path from user mandate to prompt, validated action, and settlement. Reliability depends more on the operating layer around the model than the base model alone.
...
TL;DR: Distill-Belief uses a teacher-student framework to decouple correctness from efficiency in closed-loop inverse source localization, achieving reduced sensing cost and improved accuracy while mitigating reward hacking.


<details>
  <summary>Details</summary>
Motivation: Closed-loop ISLC under time constraints faces a trade-off between expensive Bayesian inference (accurate but slow) and fast learned belief models (prone to reward hacking).

Method: A teacher-student framework: a Bayes-correct particle-filter teacher provides dense information-gain signals; a compact student distills the posterior into belief statistics for control and an uncertainty certificate for stopping. Only the student runs at deployment.

Result: Consistently reduces sensing cost, improves success rate, posterior contraction, and estimation accuracy across 7 field modalities and 2 stress tests, while mitigating reward hacking.

Conclusion: The decoupled teacher-student approach effectively balances Bayesian correctness with computational efficiency for real-time belief-space planning.
...
TL;DR: BTF-2 is a benchmark of 1,417 pastcasting questions with a frozen research corpus that enables reproducible offline forecasting, detects small accuracy differences, and identifies that frontier agents' main failures involve assessing leader incentives, follow-through likelihood, and institutional processes.


<details>
  <summary>Details</summary>
Motivation: Forecasting benchmarks produce leaderboards without explaining why some forecasters outperform others. The authors aim to create a framework for understanding and isolating differential agent strengths in research versus judgment.

Method: Developed BTF-2 with 1,417 pastcasting questions and a frozen 15M-document research corpus, enabling reproducible offline forecasting with full reasoning traces. Built an improved forecaster (+0.011 Brier) and analyzed failure modes.

Result: BTF-2 detects Brier score differences as small as 0.004 and distinguishes research vs. judgment agent strengths. The improved forecaster is 0.011 Brier more accurate than any single frontier agent. Key failure modes: assessing leader incentives, follow-through likelihood, and institutional processes.

Conclusion: The better forecaster differs primarily in pre-mortem analysis of blind spots and black swan consideration. Expert humans identify frontier agents' dominant strategic reasoning failures in political/business leader incentive assessment and institutional process modeling.
...
TL;DR: A hierarchical framework that uses LLMs to induce interpretable personas from noisy user behavioral logs by structuring actions into intent memories, clustering them, and optimizing persona quality (cohesion, evidence alignment, truthfulness) via groupwise DPO training.


<details>
  <summary>Details</summary>
Motivation: Behavioral logs are noisy and interleaved across diverse intents, and existing LLM-based persona generation methods lack direct evaluation of persona quality itself, focusing instead on downstream utility.

Method: A hierarchical framework that aggregates user actions into intent memories, then induces multiple evidence-grounded personas via clustering and labeling; persona induction is formulated as an optimization problem over quality metrics (cohesion, alignment, truthfulness) and trained using a groupwise extension of DPO.

Result: Experiments on large-scale service logs and two public datasets show the method produces more coherent, evidence-grounded, and trustworthy personas, and improves future interaction prediction.

Conclusion: The proposed framework effectively induces higher-quality personas from noisy logs while also benefiting downstream prediction tasks.
...
TL;DR: OMEGA is an end-to-end framework for automating AI research that generates novel ML classifiers from idea to executable code, outperforming scikit-learn baselines across 20 benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: To automate the entire AI research process from idea generation to executable code, reducing manual effort in discovering new ML algorithms.

Method: Combines structured meta-prompt engineering with executable code generation to create new ML classifiers in an end-to-end framework.
...
Conclusion: OMEGA successfully automates AI research by generating novel, performant ML classifiers, available via pip install omega-models.
...
TL;DR: This paper explores how LLMs respond to legal arguments, finding that the quality and persuasiveness of the advocate affects model agreement with legal positions, with implications for LLM adoption in judicial settings.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly proposed as legal decision assistants and even first-instance decision-makers, understanding how they respond to legal arguments—particularly whether they can engage with party arguments without being unduly persuaded by advocate skill rather than case merits—is essential for responsible deployment.

Method: Original experiments examining how the quality of the advocate making legal arguments affects the likelihood that frontier open- and closed-weights LLMs will agree with a particular legal point of view.

Result: The quality of the advocate making arguments significantly affects model agreement with legal viewpoints.

Conclusion: The findings have implications for the feasibility of adopting LLMs across legal and administrative settings, suggesting concerns about undue persuadability by advocate skill rather than case merits.
...
TL;DR: This study used the Apriori algorithm to analyze behavioral patterns in math tutoring logs, finding that skipping without hints predicts unsolved problems while persistence (not skipping) predicts success, with differences across LH levels and intervention conditions.


<details>
  <summary>Details</summary>
Motivation: To understand behavioral interaction patterns associated with learned helplessness in mathematics tutoring systems, using association rule mining to uncover how different student groups (high vs. low LH, with vs. without intervention) engage with problem-solving.

Method: Applied the Apriori algorithm to analyze behavioral interaction patterns across three dimensions: LH level (low/high), system-based intervention (with/without), and problem-solving outcomes (solved/unsolved).

Result: Skipping without hints was the most frequent pattern linked to unsolved outcomes; low-LH students showed stronger links between persistence and success; high-LH students showed more avoidance patterns; the no-intervention group had stronger persistence-success links.

Conclusion: Not skipping is consistently associated with solved problems, while skipping without hints predicts unsolved outcomes, with practical implications for designing interventions.
...
TL;DR: DreamProver introduces a wake-sleep agentic framework that iteratively discovers and compresses reusable lemmas for formal theorem proving, improving proof success rates and efficiency across mathematical benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches rely on fixed lemma libraries (limited adaptability) or synthesize overly specific lemmas per theorem (lacking generality). DreamProver addresses the need for transferable, reusable lemmas.

Method: A two-stage "wake-sleep" cycle: (1) Wake stage — attempts to prove theorems using current lemma library while proposing new candidate lemmas; (2) Sleep stage — abstracts, refines, and consolidates candidates to compress and optimize the library.

Result: DreamProver substantially improves proof success rates across diverse mathematical benchmarks, produces more concise proofs, and reduces computational cost.

Conclusion: The iterative wake-sleep paradigm effectively evolves a compact set of high-level, transferable lemmas that generalize to unseen theorems in related domains.
...
TL;DR: A framework combining automated object-relation reasoning with neural networks achieves 98.03% solving rate on IQ tests (top 1% human percentile) without prior knowledge, suggesting scalable integration of reasoning and ML.


<details>
  <summary>Details</summary>
Motivation: Large ML models show diminishing returns and lack solid reasoning abilities; the authors argue that combining ML scalability with rigid reasoning could surpass these limits.

Method: A theoretical framework for automated object-relation reasoning integrated with Artificial Neural Networks; formal analysis of reasoning is presented, implemented as a system combining reasoning paradigms with ML.

Result: The system solves IQ problems with no prior knowledge, achieving 98.03% solving rate (top 1% human percentile, ~132-144 IQ). Performance is limited only by model size and hardware.

Conclusion: With prior knowledge integration and dataset expansion, the system can generalize to broader problem categories; inherently favors few-shot or zero-shot problem solving.
...
TL;DR: This paper challenges the assumption that compositional reasoning emerges automatically from symbol grounding in neuro-symbolic AI, showing through the Iterative Logic Tensor Network that grounding alone is insufficient for generalization and reasoning requires explicit training.


<details>
  <summary>Details</summary>
Motivation: To systematically test the unverified assumption in neuro-symbolic AI that compositional reasoning emerges as a byproduct of successful symbol grounding.

Method: Introduced Iterative Logic Tensor Network (iLTN), a fully differentiable architecture for multi-step deduction, and tested it across a formal taxonomy of generalization tasks (novel entities, unseen relations, complex rule compositions).

Result: Models trained only on grounding objectives fail to generalize, while iLTN trained jointly on grounding and reasoning achieves high zero-shot accuracy across all tasks.

Conclusion: Symbol grounding is necessary but insufficient for generalization; reasoning is not an emergent property but a distinct capability requiring an explicit learning objective.
...
TL;DR: AGEL-Comp introduces a neuro-symbolic agent architecture combining Causal Program Graphs, Inductive Logic Programming, and Neural Theorem Proving to overcome compositional generalization failures in LLM-based agents, outperforming pure LLM models in the RetroQuest environment.


<details>
  <summary>Details</summary>
Motivation: LLM-based agents suffer from systemic failures in compositional generalization, limiting their robustness and adaptability in interactive environments. Existing pure LLM approaches lack explicit, structured world understanding.

Method: AGEL-Comp integrates three components: (1) dynamic Causal Program Graph (CPG) as a world model, (2) ILP engine synthesizing Horn clauses from experience, and (3) hybrid reasoning with LLM proposing sub-goals verified by Neural Theorem Prover (NTP), operating in a deduction-abduction learning cycle.

Result: The AGEL model shows better performance than pure LLM-based models on compositional generalization scenarios within the RetroQuest simulation environment.

Conclusion: The framework offers a principled path toward building explicit, interpretable, and compositionally structured world understanding in agents, addressing fundamental limitations of pure LLM-based approaches.
...
TL;DR: LLMs used as control components in robotic health attendants show a 54.4% mean violation rate on harmful instructions, with proprietary models (23.7%) far safer than open-weight ones (72.8%), and medical fine-tuning or prompt defenses insufficient to ensure safe deployment.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly considered for deployment as control components of robotic health attendants, yet their safety in this context remains poorly characterized.

Method: Created a dataset of 270 harmful instructions across nine prohibited behavior categories based on AMA Principles of Medical Ethics, and evaluated 72 LLMs in a simulation environment based on the Robotic Health Attendant framework.

Result: Mean violation rate 54.4%; proprietary models substantially safer (median 23.7%) than open-weight models (72.8%); medical fine-tuning provided no significant safety benefit; prompt defense gave only modest improvement.

Conclusion: Safety evaluation must be treated as a first-class criterion in the development and deployment of LLMs for robotic health attendants.
...
TL;DR: This paper proposes a "Human-in-the-Loop" benchmarking framework to evaluate multiple LLMs for automating secondary-level mathematics Competency-Based Education (CBE) assessment, finding that architectural alignment with rubric constraints (e.g., Gemini MoE models) outperforms raw parameter scale (e.g., 70B Llama models), though none achieve autonomous certification readiness.


<details>
  <summary>Details</summary>
Motivation: The shift from marks-based to qualitative competency mapping in CBE creates a manual bottleneck for educators; the authors aim to address this by assessing LLMs' effectiveness in automating assessment.

Method: A multi-provider ensemble of open-weight (Eagle/Llama-8B, Orion/Llama-70B) and proprietary models (Nova/Gemini Flash, Lyra/Gemini Pro) was benchmarked against expert-defined ground truth (kappa_w = 0.8652) using a multi-dimensional rubric for Grade 10 Optional Mathematics in Nepal.

Result: Gemini MoE models achieved "Fair Agreement" (kappa_w ~ 0.38), while the larger Orion (70B) showed "No Agreement" (kappa_w = -0.0261), revealing an "Architecture-compatibility gap" where instruction constraint adherence matters more than parameter count.

Conclusion: LLMs are not yet suitable for autonomous certification but provide valuable assistive support for preliminary evidence extraction within a "Human-in-the-Loop" framework.
...
TL;DR: Proposes a training-free test-time scaling framework that routes instances to different strategies (lightweight resolution, majority voting, rewriting) based on output disagreement, improving accuracy by 3-7% while reducing sampling cost.


<details>
  <summary>Details</summary>
Motivation: Existing test-time scaling methods (repeated sampling, self-correction, tree search) improve performance but show diminishing returns on hard problems; output disagreement is correlated with difficulty and correctness, offering a signal for better strategy selection.

Method: Formulates test-time scaling as instance-level routing: for low-disagreement cases → lightweight resolution; moderate disagreement → majority voting; high ambiguity → rewriting-based reformulation. Training-free and dynamically selects strategies based on output disagreement.

Result: Improves accuracy by 3%-7% across seven mathematical benchmarks and three LRMs while reducing sampling cost compared to existing approaches.

Conclusion: Disagreement-guided routing effectively allocates computation at test time, overcoming diminishing returns on hard problems without additional training.
...
TL;DR: Proposes SciHorizon-DataEVA, an agentic system for scalable evaluation of scientific data's AI-readiness using the Sci-TQA2 multi-dimensional framework and a hierarchical multi-agent approach.


<details>
  <summary>Details</summary>
Motivation: No scalable and systematic evaluation mechanism currently exists for assessing the AI-readiness of scientific data, which fundamentally constrains the effectiveness of AI4Science models.

Method: Introduces Sci-TQA2 principles (Governance Trustworthiness, Data Quality, AI Compatibility, Scientific Adaptability) with measurable atomic elements, and develops Sci-TQA2-Eval, a hierarchical multi-agent evaluation system with directed cyclic workflow, dataset profiling, metric activation, knowledge-augmented planning, and adaptive tool-centric evaluation with verification and self-correction.

Result: Extensive experiments on scientific datasets across multiple domains demonstrate the effectiveness and generality of SciHorizon-DataEVA for principled AI-readiness evaluation.

Conclusion: SciHorizon-DataEVA provides a scalable, systematic solution for evaluating AI-readiness of heterogeneous scientific data, addressing a critical gap in AI4Science.
...
TL;DR: FutureWorld is a live reinforcement learning environment for future prediction that enables agents to continuously learn from real-world events by closing the loop between prediction, outcome realization, and parameter updates.


<details>
  <summary>Details</summary>
Motivation: To advance live future prediction by framing it as a unified learning environment that provides diverse, grounded prediction questions while preventing answer leakage, enabling agents to continually learn from real-world events.

Method: FutureWorld is a live agentic reinforcement learning environment that closes the training loop between prediction, outcome realization, and parameter updates. The authors train three open-source base models for consecutive days using this environment.

Result: Training in FutureWorld is effective for improving model performance. The paper also establishes performance baselines for frontier agent systems using a daily benchmark built from the environment.

Conclusion: Live future prediction can serve as an effective learning environment for training agents, and FutureWorld provides a framework for continuous real-world learning through prediction tasks.
...
TL;DR: Bian Que is an LLM-based agentic framework for O&M of large-scale online systems that uses a unified operational paradigm, Flexible Skill Arrangement, and a self-evolving mechanism to automate release monitoring, alert response, and root cause analysis. Deployed on KuaiShou's e-commerce search engine, it reduces alert volume by 75%, achieves 80% RCA accuracy, and cuts MTTR by over 50%.


<details>
  <summary>Details</summary>
Motivation: O&M of large-scale online systems demands substantial human effort; deploying LLM agents is bottlenecked by orchestration (selecting relevant data/knowledge per event), not reasoning. Manual curation is intractable under dozens of daily releases, and indiscriminate signal feeding causes dilution and hallucination.

Method: Three contributions: (i) unified operational paradigm abstracting O&M into three patterns (release interception, proactive inspection, alert RCA); (ii) Flexible Skill Arrangement where each Skill specifies data/knowledge retrieval for a given context, auto-generated/updated by LLMs or refined via NL instructions; (iii) unified self-evolving mechanism where one correction signal drives case-memory-to-knowledge distillation and targeted Skill refinement.

Result: Deployed on KuaiShou's e-commerce search engine: alert volume reduced by 75%, 80% root-cause analysis accuracy, MTTR cut by over 50%, 99.0% pass rate on offline evaluations.

Conclusion: Bian Que demonstrates that effective orchestration of data and knowledge—not reasoning capability alone—is the key bottleneck for LLM-based O&M agents, and the proposed self-evolving framework achieves significant real-world improvements at scale.
...
TL;DR: FalconApp turns a short iPhone video of any rigid object into a deployable perception module for mask detection and 6-DoF pose estimation, using GSplat reconstruction and photorealistic auto-labeling, achieving ~20 min training per object and ~30 ms on-device latency.


<details>
  <summary>Details</summary>
Motivation: Reliable robotic perception requires large-scale labeled data, but real-world datasets are costly and time-consuming to annotate manually.

Method: Users capture a short handheld video → FalconApp reconstructs an editable GSplat asset → composites it with diverse photorealistic backgrounds → renders synthetic images with ground-truth masks/poses → trains the perception module → deploys it back to the iPhone frontend.

Result: ~20 min synthetic-data generation and training per object; ~30 ms on-device latency; better pose accuracy than PnP baseline on 4/5 objects in both simulation and real-world tests.

Conclusion: FalconApp enables rapid, on-device deployment of perception models for rigid objects from a single video, significantly reducing the manual labeling burden.
...
TL;DR: This paper presents a scaled multi-vehicle platform for autonomous platooning research, focusing on robust longitudinal/lateral control and leader-follower coordination, using one human-driven lead vehicle and two autonomous followers.


<details>
  <summary>Details</summary>
Motivation: Vehicle platooning can improve traffic efficiency, energy consumption, and safety, but maintaining stable path tracking during dynamic maneuvers (e.g., lane changes) is challenging due to disturbance propagation downstream.

Method: A scaled multi-vehicle platform with one human-operable lead vehicle and two autonomous followers is developed for cooperative control and human-in-the-loop autonomy experiments.

Result: The scaled platform enables controlled, repeatable leader-follower coordination experiments with physical realism, offering a safer, lower-cost, and more flexible alternative to full-scale testing.

Conclusion: The platform supports rapid prototyping, controller validation, and multi-agent autonomy studies, bridging the gap between pure simulation and full-scale field testing.
...
TL;DR: This paper proposes a multi-periodogram velocity estimation algorithm for robot-aided ISAC that leverages irregular 5G/6G reference signals, achieving 3 dB SNR gain at 10% missed-detection rate and 51% false alarm reduction compared to conventional methods.


<details>
  <summary>Details</summary>
Motivation: To enable velocity estimation in robot-aided ISAC where mobile robots opportunistically reuse irregular 5G/6G reference signals, without requiring new dedicated RSs or 3GPP modifications.

Method: Decomposes velocity profile into periodic-peak and amplitude-shaping components; proposes a multi-periodogram estimation algorithm that is standard-compliant.

Result: 3 dB SNR gain at 10% missed-detection rate and 51% reduction in false alarms compared to conventional periodogram processing.

Conclusion: The proposed method improves low-SNR robustness and is standard-compliant, requiring no new sensing-dedicated signals or 3GPP changes.
...
TL;DR: FlowS achieves state-of-the-art motion prediction (0.48 Soft mAP at 75 FPS) by using single-step diffusion inference through local transport conditioning, where learned priors place initial distributions near plausible futures so one Euler step suffices instead of many.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models produce high-quality multimodal predictions but require tens-to-hundreds of denoising steps, violating the strict latency requirements of real-world autonomous driving.

Method: Local transport conditioning: (1) an online scene-conditioned learned prior emits K calibrated anchor trajectories near plausible futures, and (2) a step-consistent displacement field enforces semigroup self-consistency so single-step inference inherits multi-step accuracy.

Result: State-of-the-art on Waymo Open Motion Dataset: Soft mAP 0.4804, mAP 0.4703 with ensemble at 75 FPS (single-step inference).

Conclusion: Local transport conditioning resolves the accuracy-latency tradeoff, making one-step generative motion prediction practical for safety-critical autonomous driving.
...
TL;DR: This paper proposes a semantic reprojection framework for nighttime UAV localization using thermal imagery against a daytime RGB 3D map, achieving ~2.18m RMSE2D in real-world flights.


<details>
  <summary>Details</summary>
Motivation: Reliable backup localization for UAVs in GNSS-denied nighttime conditions is challenging due to the severe modality gap between daytime RGB maps and nighttime thermal imagery.

Method: A semantic reprojection framework that aligns segmented thermal observations with a semantically labeled 3D map from daytime RGB data, using a symmetric bidirectional reprojection objective with confusion-aware weighting.

Result: Evaluated across 6.5 km of nighttime UAV flights; achieves bias-corrected RMSE2D of 2.18 m and median RMSE2D of 1.52 m; localization correlates with semantic edge evidence availability.

Conclusion: Semantic reprojection offers a promising pathway toward globally referenced nighttime UAV localization using thermal imagery alone.
...
TL;DR: Introduces two grasp planners (GET-2D-1.0 and GET-3D-1.0) for an asymmetrical gripper, with the 2D version being fast (~683ms) and outperforming baselines by 40%+, while the 3D version offers slight improvements but is much slower (~17s).


<details>
  <summary>Details</summary>
Motivation: To develop efficient grasp planning methods for the GET asymmetrical gripper that work from single-view RGB-D images or 3D mesh models.

Method: GET-2D-1.0 uses Ferrari-Canny metric and novel sampling from RGB-D images; GET-3D-1.0 uses mesh-based ray-tracing with a 3D gripper model.

Result: GET-2D-1.0 improves over bounding box baseline by 40%+ in lift success, shake survival, and force resistance. GET-3D-1.0 shows slight improvement over GET-2D-1.0 but takes 17s vs 683ms.

Conclusion: GET-2D-1.0 offers a practical, fast grasp planner with strong performance; GET-3D-1.0 is more accurate but computationally expensive.
...
TL;DR: This paper analyzes whether limited resources in multi-agent systems should be concentrated into a few capable agents or distributed among many simpler ones, using a resource-splitting problem and multi-agent coverage case study. Results show initial coverage rate improves with more agents, but speed trade-offs and failure rates complicate the optimal distribution.


<details>
  <summary>Details</summary>
Motivation: To understand the optimal level of distributiveness in multi-agent systems under resource constraints, specifically whether resources should be concentrated or spread across many agents.

Method: Formulates the split over n resource sharing problem where n agents equally share a common resource; presents a case study in multi-agent coverage with disk-shaped footprints scaling as 1/n; includes formal analysis and computer simulations.

Result: Initial coverage rate grows with n; if speed decreases proportionally with radii, all group sizes perform equally; if speed decreases proportionally with footprints, a single agent performs best; resource splitting increases individual failure rates.
...
TL;DR: PNPF uses phase-varying neural potential functions to handle intersecting trajectories in learning-from-demonstration, outperforming existing methods on tasks with state revisits while maintaining stability and reactivity.


<details>
  <summary>Details</summary>
Motivation: First-order DS methods fail at trajectory intersections (e.g., drawing "8") because velocity disambiguation breaks under disturbances; second-order models are sensitive to perturbations near intersections, and phase-based methods rely on open-loop variables that limit recovery.

Method: PNPF conditions a neural potential function on a phase variable estimated from state progression (not open-loop), enabling handling of state revisits while generating stable local vector fields for reactive control.

Result: Outperforms baselines on trajectories with intersections, generalizes to point-to-point, periodic, and 6D motion tasks, and demonstrates robust real-time robotic manipulation under external disturbances.

Conclusion: Phase-varying neural potential functions provide a robust framework for learning-from-demonstration that handles intersecting trajectories while maintaining stability and reactivity to perturbations.
...
TL;DR: This paper introduces the Alter-Art paradigm, where artists inhabit a robot avatar (Alter-Ego) to experience embodied creativity in dance, theater, and painting through immersive teleoperation and compliant actuation.


<details>
  <summary>Details</summary>
Motivation: Existing robotics frameworks treat the robot as an autonomous creator or collaborator, leaving a gap between human artist and machine. The authors aim to bridge this by allowing artists to actually be the robot, experiencing first-person creative embodiment.

Method: Using the robot avatar Alter-Ego, the system integrates immersive teleoperation and compliant actuation. Artists control the robot in real-time across three domains (dance, theater, painting), with qualitative artistic feedback analyzed for embodiment effects.

Result: Artists rapidly develop a sense of presence within the robotic body. The robot's physical constraints influence creativity differently across artistic domains. Embodiment shapes creative agency, identity, and environmental interaction.

Conclusion: Embodiment is a central design principle for social robotics, expanding telepresence and accessible artistic expression. The Alter-Art paradigm offers a new creative mode grounded in physical reality, unlike virtual reality.
...
TL;DR: HiPAN is a hierarchical navigation framework for quadruped robots in unstructured 3D environments, using onboard depth images, a posture-adaptive locomotion controller, and Path-Guided Curriculum Learning to achieve efficient long-horizon navigation.


<details>
  <summary>Details</summary>
Motivation: Quadruped navigation in unstructured 3D environments faces challenges: goal-directed motion, local minima escape, posture adaptation for narrow spaces. Conventional mapping-planning pipelines suffer from accumulated perception errors and high computational cost, limiting use on resource-constrained platforms.

Method: HiPAN uses a hierarchical design: high-level policy outputs planar velocity and body posture commands; low-level posture-adaptive controller executes them. Path-Guided Curriculum Learning progressively extends navigation horizon from reactive avoidance to strategic navigation.

Result: In simulation, HiPAN achieves higher success rates and greater path efficiency than reactive planners and end-to-end baselines. Real-world experiments validate applicability across diverse unstructured 3D environments.

Conclusion: HiPAN effectively addresses quadruped navigation challenges in unstructured 3D spaces with a lightweight, hierarchical, learning-based approach that avoids costly mapping-planning pipelines.
...
TL;DR: This survey systematically reviews 3D generation for embodied AI across three roles: Data Generator (articulated, deformable objects), Simulation Environments (interactive scenes), and Sim2Real Bridge (digital twins, data augmentation), highlighting a shift from visual realism toward interaction readiness.


<details>
  <summary>Details</summary>
Motivation: Embodied AI and robotic systems require scalable, diverse, and physically grounded 3D content for simulation training and real-world deployment, but existing 3D generation lacks kinematic structures, material properties, and interaction readiness that go beyond visual realism.

Method: The survey organizes literature into three roles: (1) Data Generator - creating articulated, physically grounded, deformable objects; (2) Simulation Environments - constructing structure-aware, controllable, agentic scenes; (3) Sim2Real Bridge - digital twin reconstruction, data augmentation, synthetic demonstrations for robot learning.

Result: Identified a paradigm shift from visual realism toward interaction readiness; main bottlenecks include limited physical annotations, geometric-physical validity gap, fragmented evaluation, and persistent sim-to-real divide.

Conclusion: 3D generation must address interaction readiness and key bottlenecks to become a dependable foundation for embodied intelligence.
...
TL;DR: LLM-Flax automates neuro-symbolic task planner deployment using a local LLM, eliminating manual rule authoring and GNN training, achieving higher success rates (0.945 vs 0.828) across MazeNamo benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current neuro-symbolic task planners require significant manual effort: domain experts must author relaxation/complementary rules, and hundreds of training problems are needed to supervise a GNN object scorer. This limits practical deployment to new domains.

Method: Three-stage framework: Stage 1 auto-generates rules via structured prompting with self-correction; Stage 2 uses LLM-guided failure recovery with a feasibility-gated budget policy; Stage 3 replaces GNN with zero-shot LLM object importance scoring.

Result: LLM-Flax achieves average SR 0.945 vs manual baseline 0.828 (+0.117). On 12x12 Expert, SR 0.733 vs manual 0.000. On 15x15 Hard, SR 1.000 vs 0.900. Stage 3 achieves SR 0.720 with no training data.

Conclusion: LLM-Flax successfully eliminates all manual effort in neuro-symbolic planner deployment while improving performance. Stage 3 shows feasibility but faces context-window bottlenecks at scale, which is the main open challenge.
...
TL;DR: STAR-Filter uses starshaped sets as a fast filter to efficiently generate convex collision-free regions, reducing computation time and improving robustness to sensor noise for robot planning.


<details>
  <summary>Details</summary>
Motivation: Existing convex inflation methods degrade in efficiency with complex obstacles and noisy sensor data, and are sensitive to initialization and accurate geometric models.

Method: Proposes STAR-Filter, a lightweight framework that constructs starshaped sets as a fast filter, using obstacle points as active supporting constraints to reduce redundant computation.

Result: Achieves lowest computation time and reduces conservativeness in polytope generation on real-world noisy and large-scale data.

Conclusion: STAR-Filter is effective for Safe Flight Corridor (SFC) generation and agile quadrotor planning in noisy environments.
...
TL;DR: ATLAS is a multi-modal annotation tool for long-horizon robotic action segmentation that supports synchronized video and time-series data visualization, handles ROS bags and RLDS formats, and outperforms existing tools like ELAN in annotation speed and boundary accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing annotation tools are limited for robotics data: they focus on vision-only data, lack synchronized visualization of robot-specific time-series signals (e.g., gripper state, force/torque), and require significant effort to adapt to different dataset formats.

Method: ATLAS provides time-synchronized multi-modal visualization (multi-view video + proprioceptive signals), supports annotation of action boundaries/labels/task outcomes, natively handles ROS bags and RLDS formats, offers direct support for specific datasets like REASSEMBLE, includes a modular dataset abstraction layer for easy extension, and uses a keyboard-centric interface for efficiency.

Result: ATLAS reduced average per-action annotation time by at least 6% compared to ELAN; inclusion of time-series data improved temporal alignment with expert annotations by over 2.8% and decreased boundary error fivefold compared to vision-only annotation tools.

Conclusion: ATLAS effectively addresses the limitations of existing annotation tools for robotic action segmentation by supporting multi-modal data synchronization and common robotics formats, leading to faster and more accurate annotations.
...
TL;DR: The paper studies what happens when skills in a robot's library are replaced (e.g., by fine-tuning) and compositions change. It introduces a cross-version swap protocol and discovers a "dominant-skill effect" where one skill's performance dominates overall composition success. Behavioral distance metrics fail to identify such dominant skills. They propose an atomic-quality probe and a Hybrid Selector for cost-effective skill-update governance.


<details>
  <summary>Details</summary>
Motivation: Existing composition methods treat skill libraries as frozen at test time; they do not analyze how composition outcomes change when a skill is replaced (e.g., after fine-tuning, fresh demos, domain adaptation). This gap is critical for deployed systems that continually update their skill libraries.

Method: (1) Paired-sampling cross-version swap protocol on robosuite tasks. (2) Discovery and analysis of the dominant-skill effect via atomic success rates. (3) Evaluation of off-policy behavioral distance metrics. (4) Proposal of an atomic-quality probe and a Hybrid Selector combining per-skill probes (zero per-decision cost) with selective composition revalidation (full cost), with Pareto frontier characterization on 144 skill-update decisions.

Result: On dual-arm peg-in-hole: one ECM has 86.7% atomic success rate vs others ≤26.7%; dominant ECM shifts composition success by +50pp. On simpler pick task (all 100%) effect is undefined. Off-policy behavioral distance metrics fail to identify dominant ECM. On T6: atomic-only probe sits 23pp below full revalidation (64.6% vs 87.5%) at zero cost; Hybrid Selector with m=10 closes gap to ~12pp at 46% cost. Cross-task average over 144 events: atomic-only is within 3pp of full revalidation under mixed-oracle caveat.

Conclusion: The atomic-quality probe is the first principled, deployment-ready primitive for skill-update governance in compositional robot policies. It enables efficient decision-making about whether to revalidate compositions when a skill is updated, with minimal cost trade-offs.
...
TL;DR: X-WAM is a unified 4D world model that combines real-time robotic action execution with high-fidelity video + 3D reconstruction, using a pretrained video diffusion model with a lightweight depth prediction branch and Asynchronous Noise Sampling for efficient action decoding and high-quality world generation.


<details>
  <summary>Details</summary>
Motivation: Prior unified world models (e.g., UWM) only model 2D pixel-space and fail to balance action execution efficiency with world modeling quality.

Method: Leverages pretrained video diffusion models; replicates final few blocks of Diffusion Transformer into a depth prediction branch; proposes Asynchronous Noise Sampling (ANS) for joint optimization of generation quality and action decoding efficiency.

Result: 79.2% and 90.7% average success rate on RoboCasa and RoboTwin 2.0; surpasses existing methods in visual and geometric metrics; pretrained on 5,800+ hours of robotic data.

Conclusion: X-WAM effectively unifies real-time action execution and high-fidelity 4D world synthesis, outperforming prior methods in both robotic task success and world modeling quality.
...
TL;DR: A hierarchical UAV framework combining rule-based high-level advisor with online RL controller improves early safety and sample efficiency in SAR-like tasks under limited/lack of pretraining.


<details>
  <summary>Details</summary>
Motivation: To enable UAVs to perform search-and-rescue missions with limited simulation training and even under strict no-pretraining deployment, while ensuring safety and adaptability.

Method: Fixed rule-based high-level advisor (offline compiled from task specification) + online goal-conditioned low-level RL controller with mode-aware prioritized replay augmented by rule-derived metadata.

Result: On battery-aware multi-goal delivery and moving-target delivery tasks, the method improves early safety and sample efficiency, mainly by reducing collision terminations, while maintaining online adaptation.

Conclusion: Combining interpretable rule-based guidance with online RL learning is effective for UAV missions under limited training, balancing safety and adaptability.
...
TL;DR: Walk with Me is a map-free framework for long-horizon social navigation that combines high-level VLM for grounding instructions and coarse planning with low-level VLA for routine execution, using observation-aware routing to trigger safety reasoning in complex situations.


<details>
  <summary>Details</summary>
Motivation: Existing map-based methods rely on costly pre-built HD maps, and learning-based policies are mostly limited to indoor and short-horizon settings, creating a gap for outdoor long-horizon social navigation from high-level human instructions.

Method: Walk with Me uses GPS context and lightweight POIs for semantic grounding, a High-Level VLM for instruction grounding and coarse waypoint planning, and an observation-aware routing mechanism that switches between Low-Level VLA execution and High-Level VLM safety reasoning based on situation complexity.

Result: The framework enables practical outdoor social navigation by combining semantic intent grounding, map-free long-horizon planning, safety-aware reasoning, and low-level action generation for human-centric assistance.

Conclusion: Walk with Me demonstrates a feasible approach to bridge high-level natural-language instructions and safe, long-horizon navigation behavior in open-world outdoor environments without requiring HD maps.
...
TL;DR: STARRY is a world-model-enhanced VLA policy that jointly denoises future spatial-temporal latents and action sequences, using Geometry-Aware Selective Attention Modulation to align spatial-temporal prediction with action generation, achieving 93.82%/93.30% success on RoboTwin 2.0 and improving real-world success from 42.5% to 70.8%.


<details>
  <summary>Details</summary>
Motivation: Existing VLA and world-model-enhanced policies fail to fully model action-relevant spatial-temporal interaction structure, which is critical for robotic manipulation.

Method: Joint denoising of future spatial-temporal latents and action sequences; Geometry-Aware Selective Attention Modulation converts predicted depth and end-effector geometry into token-aligned weights for selective action-attention modulation.

Result: 93.82%/93.30% average success on RoboTwin 2.0 (Clean/Randomized); real-world success improved from 42.5% to 70.8% over π₀.₅.

Conclusion: Action-centric spatial-temporal world modeling effectively improves robotic action generation for spatially and temporally demanding tasks.
...
TL;DR: This paper presents a tendon-driven origami tentacle gripper that combines deterministic local deformation (coiling via crease patterns and tendon routing) with stochastic global entanglement (braiding/knotting among multiple tentacles) to achieve universal grasping of random-shaped objects using simple single-tendon actuation, validated through simulation (Cosserat rod + origami mechanics) and experiments (gravity, underwater, and simulated in-orbit grasping).


<details>
  <summary>Details</summary>
Motivation: Existing origami grippers offer compactness and flexibility but require multiple actuation channels and complex control to handle random-shaped objects in dynamic environments. The authors aim to achieve robust universal gripping with minimal actuation complexity.

Method: The gripper consists of multiple origami tentacles made from Mylar sheets with cut holes for tendon routing, origami creases for prescribed deformation, and tapered geometry. A single tendon pull triggers deterministic coiling (shrinking, bending, twisting). When multiple tentacles are placed together, stochastic entanglements (braiding, knotting) emerge to grip objects. A simulation model integrates origami mechanics with Cosserat rod theory.

Result: The gripper successfully grasps objects with random shapes under gravity and underwater. A stow-and-release deployment mechanism was demonstrated for simulated in-orbit grasping. The synergy of deterministic coiling and stochastic entanglement enables robust gripping with simple actuation.

Conclusion: The origami tentacle gripper offers a new strategy for robust, universal object grasping with simple design (single tendon) and actuation, leveraging the interplay between programmed deformation and emergent collective behavior.
...
TL;DR: A bi-level optimization framework for rope-assisted climbing robots that simultaneously selects landing regions and optimizes control inputs, validated on the ALPINE platform.


<details>
  <summary>Details</summary>
Motivation: Enable rope-assisted climbing robots to navigate vertical surfaces by solving the mixed-integer problem of selecting feasible terrain regions while optimizing control inputs.

Method: Bi-level optimization scheme: outer level uses Cross-Entropy Method for terrain selection, inner level uses gradient-based nonlinear optimization for dynamically feasible motions.

Result: Validated on ALPINE climbing robot across various challenging terrain configurations.

Conclusion: The proposed bi-level planning pipeline effectively addresses the mixed-integer problem for rope-assisted vertical climbing locomotion.
