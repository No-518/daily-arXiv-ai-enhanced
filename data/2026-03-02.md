...
TL;DR: DesignSense-10k dataset and reward model for layout preference evaluation, outperforming existing models and improving layout generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing layout generation models often fail to align with human aesthetic judgment, and current preference datasets/reward models from text-to-image generation don't generalize to layout evaluation where spatial arrangement is key.

Method: Created DesignSense-10k dataset with 10,235 human-annotated preference pairs using 5-stage curation pipeline (semantic grouping, layout prediction, filtering, clustering, VLM refinement). Trained DesignSense vision-language classifier on this dataset.

Result: DesignSense outperforms existing models by 54.6% in Macro F1 over strongest proprietary baseline. Improves layout generation: RL training boosts win rate by ~3%, inference-time scaling provides 3.6% improvement.

Conclusion: Specialized, preference-aware models are needed for layout evaluation as frontier VLMs fail catastrophically on layout preference tasks. DesignSense demonstrates practical impact on real-world layout generation quality.
...
TL;DR: ANTShapes is a Unity-based simulation framework for generating customizable neuromorphic vision datasets to address DVS hardware scarcity, enabling creation of synthetic event-based data for anomaly detection and object recognition tasks.


<details>
  <summary>Details</summary>
Motivation: Limited availability of Dynamic Vision Sensors (DVS) restricts neuromorphic computer vision research, and existing datasets are often small or limited in scenarios, creating a need for comprehensive simulation tools.

Method: Built in Unity engine, ANTShapes simulates configurable 3D scenes with objects displaying randomly-generated behaviors (motion, rotation). Anomaly labeling follows statistical processes based on central limit theorem principles, allowing parameter-adjustable dataset generation with labels and frame data.

Result: A flexible framework that can generate arbitrary-sized datasets with accompanying labels, addressing data scarcity by enabling simulation of bespoke event-based vision datasets for various research purposes.

Conclusion: ANTShapes provides a solution to DVS data availability limitations by offering a customizable simulation framework for generating neuromorphic vision datasets, supporting research in object recognition, localization, and anomaly detection.
...
TL;DR: LIDMark: A unified framework for deepfake forensics that jointly handles detection, localization, and source tracing using a 152D landmark-identity watermark and specialized decoder heads.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake forensics methods treat detection, tampering localization, and source tracing as separate tasks, lacking a unified framework. Malicious face manipulations threaten privacy and security, requiring comprehensive forensic solutions.

Method: Proposes LIDMark - a 152-dimensional landmark-identity watermark that structurally combines facial landmarks with source identifiers. Uses Factorized-Head Decoder (FHD) with specialized regression and classification heads to robustly extract landmarks and identifiers even under distortion.

Result: Extensive experiments demonstrate that LIDMark provides a unified, robust, and imperceptible solution for deepfake detection, localization, and tracing. The framework achieves "all-in-one" trifunctional forensic capabilities.

Conclusion: LIDMark successfully bridges the gap in deepfake forensics by providing a unified framework that jointly addresses detection, localization, and source tracing through innovative watermark design and specialized decoder architecture.
...
TL;DR: SVG2 is a massive panoptic video scene graph dataset with 636K videos and 6.6M objects, and TRaSER is a new model that generates spatio-temporal scene graphs with significant improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Prior spatio-temporal scene graph datasets lack scale and diversity, limiting progress in video understanding. There's a need for larger datasets and better models to generate explicit spatio-temporal scene graphs as intermediate representations for video reasoning.

Method: Created SVG2 using automated pipeline with panoptic segmentation, trajectory tracking, semantic parsing, and GPT-5 relation inference. Developed TRaSER model with trajectory-aligned token arrangement, object-trajectory resampler (aggregates entire trajectories), and temporal-window resampler (binds tokens to short segments).

Result: TRaSER improves relation detection by +15-20%, object prediction by +30-40% over open-source baselines (+13% over GPT-5), and attribute prediction by +15%. When used for VQA, provides +1.5-4.6% accuracy gain over video-only or Qwen2.5-VL scene graphs.

Conclusion: SVG2 enables large-scale video scene graph research, and TRaSER demonstrates that explicit spatio-temporal scene graphs are effective intermediate representations for video understanding, significantly improving downstream tasks like VQA.
...
TL;DR: LE-NeuS is a latency-efficient neuro-symbolic framework for long-form video QA that reduces inference latency from 90x to 10x slower than base VLM prompting while maintaining accuracy gains through adaptive sampling and parallelized proposition detection.


<details>
  <summary>Details</summary>
Motivation: Existing neuro-symbolic approaches to long-form video QA achieve significant accuracy improvements through temporal logic grounding but suffer from prohibitive latency overheads (up to 90x slower than base VLM prompting), making them impractical for latency-sensitive edge deployments.

Method: Two key optimizations: (1) CLIP-guided two-stage adaptive sampling that exploits visual redundancy to skip semantically similar frames while preserving temporal boundaries, and (2) batched proposition detection that parallelizes VLM inference across temporal windows. Theoretically analyzes latency bounds based on video length, proposition complexity, and sampling density.
...
Conclusion: LE-NeuS demonstrates that neuro-symbolic approaches can be made latency-efficient for practical deployment while preserving their accuracy advantages for temporally complex video understanding tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing RGB-X work assumes aligned data pairs exist and focuses on modality fusion, but calibration requires huge engineering effort. The paper aims to remove cumbersome calibration for various RGB-X sensors and advance cross-sensor learning.

Method: Match-densify-consolidate method: 1) RGB-X image matching with guided point densification, 2) confidence-aware densification and self-matching filtering for better view synthesis, 3) consolidation in 3D Gaussian Splatting (3DGS). Uses no 3D priors for X-sensor and only assumes COLMAP for RGB.

Result: Not specified in abstract, but the method aims to achieve better view synthesis and provide a scalable solution for large-scale real-world RGB-X data collection.

Conclusion: Proposes a practical solution to break through the bottleneck in large-scale RGB-X data collection by eliminating the need for cumbersome calibration, advancing the popularity of cross-sensor learning.
...
TL;DR: Evidential Neural Radiance Fields (ENeRF) is a probabilistic NeRF approach that quantifies both aleatoric and epistemic uncertainty in a single forward pass without compromising rendering quality or computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Current NeRF methods lack uncertainty estimation, limiting deployment in safety-critical applications. Existing uncertainty quantification methods either fail to capture both types of uncertainty, compromise rendering quality, or require significant computational overhead.

Method: Introduces Evidential Neural Radiance Fields, a probabilistic approach that integrates with NeRF rendering and enables direct quantification of both aleatoric and epistemic uncertainty from a single forward pass.

Result: State-of-the-art performance on three standardized benchmarks for both scene reconstruction fidelity and uncertainty estimation quality.

Conclusion: The proposed approach successfully addresses the limitations of existing NeRF uncertainty quantification methods by providing efficient, high-quality uncertainty estimation without sacrificing reconstruction accuracy.
...
TL;DR: CycleBEV introduces a regularization framework using cycle consistency to enhance view transformation models for BEV semantic segmentation without increasing inference complexity.


<details>
  <summary>Details</summary>
Motivation: Transforming image features from perspective view to bird's-eye-view in autonomous driving remains challenging due to depth ambiguity and occlusion, despite existing view transformation paradigms.

Method: Proposes CycleBEV with an inverse view transformation network that maps BEV segmentation back to PV segmentation, using cycle consistency losses to regularize VT networks during training. Extends cycle consistency to geometric and representation spaces.

Result: Consistent improvements across four representative VT models on nuScenes dataset, with gains up to 0.74, 4.86, and 3.74 mIoU for drivable area, vehicle, and pedestrian classes respectively.

Conclusion: CycleBEV effectively enhances existing view transformation models for BEV semantic segmentation through cycle consistency regularization, achieving significant performance gains without increasing inference complexity.
...
TL;DR: HDFLIM enables cross-modal alignment between frozen vision and language models using hyperdimensional computing, achieving captioning performance comparable to fine-tuned methods without parameter updates.


<details>
  <summary>Details</summary>
Motivation: Traditional multimodal alignment requires computationally intensive fine-tuning that modifies pretrained representations. The authors hypothesize that independently trained foundation models may already have latent semantic compatibility, raising the question of whether alignment can be achieved without modifying the models themselves.

Method: Projects unimodal embeddings into a shared hyperdimensional space and uses lightweight symbolic operations (binding, bundling, similarity-based retrieval) to construct associative cross-modal representations in a single pass. Caption generation emerges from high-dimensional memory retrieval rather than iterative gradient optimization.
...
TL;DR: Proposes pseudo contrastive training with synthetic diagrams to improve vision-language models' sensitivity to fine-grained structural differences in diagram understanding.


<details>
  <summary>Details</summary>
Motivation: Current multimodal models like CLIP struggle with domains where small visual differences carry large semantic significance, such as diagram understanding, due to limited sensitivity to fine-grained structural variations.

Method: Introduces pseudo contrastive samples generated by a diagram renderer that creates synthetic diagrams using randomly picked text elements, highlighting structural differences without modifying original data. Incorporates these samples into training objective.

Result: Substantial improvements over standard CLIP and hard-negative CLIP training on flowchart benchmark dataset for both image-text matching and visual question answering tasks.

Conclusion: Demonstrates value of domain-specific training strategies for advancing diagrammatic understanding within vision-language learning, showing that pseudo contrastive samples effectively enhance model sensitivity to structural variations.
...
TL;DR: Incremental dimension reduction algorithm for visual anomaly detection that processes features in batches to handle large datasets with thousands of images while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks extract high-dimensional features for anomaly detection, but this dimensionality makes it difficult to apply algorithms to large datasets with thousands of images due to memory constraints.

Method: Incremental dimension reduction algorithm that groups feature vectors into batches, computes truncated singular value decomposition per batch, updates global singular values/vectors incrementally, and re-transforms batch-wise singular vectors to the full feature space.

Result: The algorithm can accelerate training of state-of-the-art anomaly detection algorithms while maintaining close accuracy compared to processing all features at once.

Conclusion: Proposed incremental dimension reduction enables efficient processing of large-scale visual anomaly detection datasets by overcoming memory limitations while preserving detection accuracy.
...
TL;DR: HART enables Large Multimodal Models to focus on key regions in high-resolution images without needing costly human annotations, using a self-verification framework with policy optimization.


<details>
  <summary>Details</summary>
Motivation: Current LMMs struggle with high-resolution visual inputs due to quadratic token growth and irrelevant information. Existing methods need expensive human annotations for visual grounding, creating a need for annotation-free approaches.

Method: Proposes HART framework with Advantage Preference Group Relative Policy Optimization (AP-GRPO) for post-training. Enables models to self-verify key regions in high-resolution images through closed-loop localization and reasoning.

Result: HART improves performance across high-resolution visual tasks, outperforming strong baselines. When applied to Qwen2.5-VL-7B, it surpasses larger models like Qwen2.5-VL-72B and LLaVA-OneVision-72B on vision-centric benchmarks.

Conclusion: HART provides an effective annotation-free approach for enhancing LMMs' high-resolution reasoning capabilities through self-verification and policy optimization, enabling smaller models to outperform much larger ones.
...
TL;DR: Proposed Eva-3M dataset with 3M+ frames (435K visibility labels) and EvaPose method for egocentric human pose estimation with explicit visibility handling.


<details>
  <summary>Details</summary>
Motivation: Existing egocentric HPE datasets lack keypoint visibility annotations, and current methods treat visible/invisible keypoints indiscriminately, compromising accuracy for visible keypoints.

Method: Created Eva-3M dataset with visibility labels, augmented EMHI dataset similarly, and proposed EvaPose method that explicitly incorporates visibility information to enhance pose estimation.

Result: Ground-truth visibility labels significantly improve egocentric HPE, and EvaPose achieves state-of-the-art performance on both Eva-3M and EMHI datasets.

Conclusion: Visibility-aware approach is crucial for accurate egocentric human pose estimation, and the proposed dataset and method effectively address the keypoint invisibility challenge.
...
TL;DR: DeepLookEditBench (DLEBench) is the first benchmark for evaluating Instruction-based Image Editing Models (IIEMs) on small-scale object editing, addressing a critical gap in current evaluation practices.


<details>
  <summary>Details</summary>
Motivation: Current IIEMs show strong reasoning on existing benchmarks but their ability to edit small objects (occupying only 1%-10% of image area) remains underexplored, despite being crucial for precise local editing and detail refinement in both real and generated images.

Method: The authors construct DLEBench with 1889 samples across seven instruction types featuring small objects in complex scenarios (partial occlusion, multi-object editing). They propose an evaluation protocol with refined score rubrics and a dual-mode framework (Tool-driven and Oracle-guided Modes) to address misalignment between LMM-as-a-Judge and human judgments.

Result: Empirical evaluation of 10 IIEMs reveals significant performance gaps in small-scale object editing, demonstrating that current models struggle with this challenging task despite their strong performance on existing benchmarks.

Conclusion: Specialized benchmarks like DLEBench are needed to advance IIEMs' small-scale object editing capabilities, as current evaluation practices fail to adequately assess this important aspect of image editing.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of reconstructing structured 3D buildings from point clouds with diverse distributions (airborne LiDAR, Structure-from-Motion) in a highly underconstrained setting, aiming to recover artist-created building abstractions.

Method: Proposes a Loosely Cascaded Diffusion Transformer (Loca-DiT) that first recovers underlying distributions from noisy/sparse points using conditional latent diffusion models, then uses a decoder-only transformer for autoregressive mesh generation based on recovered point clouds.

Result: The method achieves substantial qualitative and quantitative improvements over prior building abstraction methods, and its recovered point clouds show strong performance on building point cloud completion benchmarks with improved surface accuracy and distribution uniformity.

Conclusion: BuildAnyPoint effectively leverages explicit 3D generative priors in a cascaded diffusion-transformer framework to reconstruct structured 3D buildings from diverse point cloud distributions, demonstrating superior performance in both building abstraction and point cloud completion tasks.
...
TL;DR: MedMAP is a modality-aware pretraining framework for 3D MRI that improves vision-language alignment and outperforms existing VLMs in multi-organ abnormality detection.


<details>
  <summary>Details</summary>
Motivation: Vision-language models show promise for medical imaging diagnostics, but face challenges with modality-specific vision-language alignment and cross-modal feature fusion when applied to multi-organ medical imaging.

Method: Proposes MedMAP with two stages: (1) modality-aware vision-language alignment pretraining where modality-aware encoders capture joint modality distribution, and (2) fine-tuning stage for multi-organ abnormality detection using pre-trained vision encoders with frozen text encoder. Also curated MedMoM-MRI3D dataset with 7,392 3D MRI volume-report pairs across 12 MRI modalities and 9 abnormalities.
...
Conclusion: MedMAP effectively addresses modality-specific alignment challenges in medical VLMs and shows superior performance for 3D MRI multi-organ abnormality detection, with code publicly available.
...
TL;DR: ProtoDCS: A robust open-set test-time adaptation framework for VLMs that uses probabilistic GMM verification for csID/csOOD separation and evidence-driven prototype updates for efficient adaptation.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based TTA methods fail in open-set scenarios where test streams contain both covariate-shifted in-distribution (csID) and out-of-distribution (csOOD) data, leading to interference and overconfident predictions.

Method: ProtoDCS uses: (1) double-check separation with probabilistic GMM verification instead of brittle thresholding, and (2) evidence-driven adaptation with uncertainty-aware loss and efficient prototype-level updates.

Result: Achieves SOTA performance on CIFAR-10/100-C and Tiny-ImageNet-C, significantly boosting both known-class accuracy and OOD detection metrics.

Conclusion: ProtoDCS provides a robust framework for open-set TTA that effectively separates csID/csOOD samples and enables safe, efficient adaptation of VLMs to covariate-shifted data.
...
TL;DR: SDLS is a training-free inference framework that reduces prior-comparison hallucinations in radiology report generation by using LLM-driven semantic decomposition and QR orthogonalization to create semantic-free steering vectors.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models for radiology report generation suffer from prior-comparison hallucination, where models generate historical findings not supported by current imaging studies, compromising clinical accuracy.
...
Result: On MIMIC-CXR, SDLS reduces historical hallucinations (FilBERT score from 0.2373 to 0.1889) and improves clinical label fidelity (CheXpert macro-F1 from 0.2242 to 0.3208), maintaining structural integrity while enabling zero-shot transfer to CheXpert Plus and IU-Xray.

Conclusion: SDLS effectively addresses prior-comparison hallucination in radiology report generation without compromising clinical accuracy, offering a robust training-free solution that generalizes across datasets.
...
TL;DR: VL-WS uses vision-language alignment with CLIP embeddings and FiLM layers to achieve robust crop-weed segmentation across diverse agricultural environments, outperforming CNN baselines by 4.98% in mean Dice score.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models for crop-weed segmentation struggle to generalize across heterogeneous agricultural environments due to reliance on dataset-specific visual features, limiting their practical deployment in precision agriculture.

Method: Proposes VL-WS framework with dual-encoder design: frozen CLIP embeddings and task-specific spatial features fused via FiLM layers conditioned on natural language captions. Trained on unified corpus including ground and UAV imagery across diverse conditions.

Result: Achieves 91.64% mean Dice score, outperforming CNN baseline by 4.98%. Largest gains on challenging weed class: 80.45% vs 65.03% baseline (15.42% improvement). Maintains stable performance under limited target-domain supervision.

Conclusion: Vision-language alignment enables scalable, label-efficient segmentation models deployable across diverse real-world agricultural domains, with improved generalization and data efficiency for precision agriculture applications.
...
TL;DR: rs-embed is a Python library that provides a unified interface for accessing embeddings from various remote sensing foundation models, enabling easy retrieval for any location and time range with minimal code.


<details>
  <summary>Details</summary>
Motivation: The remote sensing community faces challenges in adopting and fairly comparing foundation models due to heterogeneity in model release formats, platforms, interfaces, and input data specifications, which increases costs for obtaining, using, and benchmarking embeddings across different models.

Method: The authors propose rs-embed, a Python library with a unified, region-of-interest (ROI) centric interface that allows users to retrieve embeddings from any supported model for any location and time range with a single line of code, plus efficient batch processing for large-scale operations.

Result: The library provides a practical solution to the interoperability problem, enabling easier adoption and fair comparison of remote sensing foundation models through standardized access to embeddings across different models.

Conclusion: rs-embed addresses the fragmentation in remote sensing foundation model ecosystems by offering a unified interface that reduces barriers to adoption and enables more accessible benchmarking and large-scale embedding generation.
...
TL;DR: SourceSwap is a self-supervised framework for object swapping that learns cross-object alignment without per-object finetuning or paired data, using frequency-separated noise perturbations to create pseudo pairs and enabling zero-shot inference.


<details>
  <summary>Details</summary>
Motivation: Existing object swapping methods either require slow per-object finetuning or rely on paired data showing the same object in different contexts, which forces models to use background cues rather than learning true cross-object alignment.

Method: Uses frequency-separated perturbation in initial-noise space to synthesize pseudo pairs that alter appearance while preserving pose, shape, and scene layout. Trains a dual U-Net with full-source conditioning and noise-free reference encoder for direct inter-object alignment.

Result: Achieves superior fidelity, stronger scene preservation, and more natural harmony compared to existing methods. Transfers well to other edits like subject-driven refinement and face swapping. Enables zero-shot inference without per-object finetuning.

Conclusion: SourceSwap provides an effective self-supervised framework for object swapping that learns cross-object alignment through synthetic pseudo pairs, eliminating the need for paired data or per-object finetuning while achieving state-of-the-art performance.
...
TL;DR: HiDrop: A hierarchical token pruning framework for MLLMs that reduces 90% visual tokens while maintaining performance and accelerating training 1.72×


<details>
  <summary>Details</summary>
Motivation: Quadratic computational cost of processing vision tokens in MLLMs hinders widespread adoption; current progressive pruning methods misinterpret shallow layer functions and use rigid schedules, failing to unlock full efficiency potential.

Method: HiDrop aligns token pruning with true hierarchical function of MLLM layers through: (1) Late Injection - bypasses passive shallow layers to introduce visual tokens where active fusion begins; (2) Concave Pyramid Pruning with Early Exit - dynamically adjusts pruning rates across middle/deep layers using inter-layer similarity and differentiable top-k; plus persistent positional encoding, FlashAttention-compatible token selection, and parallel decoupling of vision computation.

Result: Compresses ~90% visual tokens while matching original performance, accelerates training by 1.72 times, sets new SOTA for efficient MLLM training/inference.

Conclusion: HiDrop not only achieves state-of-the-art efficiency but also provides valuable insights into hierarchical nature of multimodal fusion, enabling practical adoption of MLLMs through significant computational savings.
...
TL;DR: EgoGraph is a training-free framework that builds dynamic knowledge graphs from ultra-long egocentric videos to capture long-term cross-entity dependencies, enabling better temporal reasoning over multi-day sequences.


<details>
  <summary>Details</summary>
Motivation: Existing video understanding approaches struggle with ultra-long egocentric videos spanning multiple days due to fragmented local processing and limited temporal modeling, which restricts their ability to reason over extended sequences.

Method: EgoGraph uses an egocentric schema to unify extraction of core entities (people, objects, locations, events) and their attributes/interactions, with temporal relational modeling that captures dependencies across entities and accumulates stable long-term memory over multiple days.
...
Conclusion: EgoGraph provides an effective new paradigm for ultra-long egocentric video understanding through dynamic knowledge graph construction that captures rich semantic representations and enables complex temporal reasoning.
...
TL;DR: U-MLLMs fail to maintain semantic equivalence across modalities - they reason well in text but collapse when generating visual answers, despite good rendering quality, revealing a cross-modal alignment breakdown.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of Unified Multimodal LLMs assess understanding and generation separately, overlooking semantic equivalence - whether models produce consistent reasoning results regardless of output modality. The paper investigates if current U-MLLMs satisfy this premise.

Method: Introduces VGUBench, a diagnostic framework with three tasks: (1) Textual Generative Understanding (text reasoning baseline), (2) Visual Generative Understanding (generating visual answers), and (3) Visual Rendering control (rendering explicit descriptions without reasoning). This decouples reasoning logic from generation fidelity.

Result: Significant disparity: U-MLLMs show strong textual understanding and visual rendering, but collapse when generating visual answers to questions. Negligible correlation between visual answering performance and basic rendering quality, suggesting failure stems from cross-modal semantic alignment breakdown, not generation fidelity.

Conclusion: Current U-MLLMs fail to maintain semantic equivalence across modalities. The performance collapse in visual answering reveals a fundamental breakdown in cross-modal semantic alignment, not insufficient generation quality. Provides diagnostic insights for future unified generation and understanding models.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models create AI-generated images nearly indistinguishable from real ones, raising misuse concerns. Existing detectors using reconstruction error become less effective as AI images become more realistic.

Method: Instead of using direct reconstruction error (first-order difference), computes difference in reconstruction error (second-order difference) for variance reduction and improved detection accuracy.
...
Conclusion: The proposed difference-in-difference method effectively addresses the challenge of detecting increasingly realistic AI-generated images by leveraging second-order statistical differences.
...
<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based trackers suffer from computational overhead that hinders real-time deployment. Current token pruning methods are fragmented, pruning components in isolation and overlooking inter-component dependencies, leading to suboptimal pruning and degraded accuracy.

Method: UTPTrack employs an attention-guided, token type-aware strategy to holistically model redundancy across all three components (search region, dynamic template, static template). This unified framework supports both RGB-based and multimodal/language-guided tracking within a single model.

Result: UTPTrack prunes 65.4% of vision tokens in RGB-based tracking and 67.5% in unified tracking while preserving 99.7% and 100.5% of baseline performance respectively. It achieves state-of-the-art accuracy-efficiency trade-off across 10 benchmarks.

Conclusion: UTPTrack demonstrates strong performance across both RGB and multimodal scenarios, establishing itself as a robust foundation for future research in efficient visual tracking with its unified approach to token pruning.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal interaction systems are limited to unimodal generation or suffer from degraded reasoning and poor cross-modal alignment, preventing coherent and perceptually grounded interactions needed for intelligent embodied agents.

Method: U-Mind implements a Unified Alignment and Reasoning Framework with segment-wise alignment for cross-modal synchronization and Rehearsal-Driven Learning to preserve reasoning abilities. It uses text-first decoding with internal chain-of-thought planning followed by temporally synchronized generation across modalities, plus a real-time video rendering framework conditioned on pose and speech.

Result: Extensive experiments show U-Mind achieves state-of-the-art performance on multimodal interaction tasks including question answering, instruction following, and motion generation.

Conclusion: U-Mind paves the way toward intelligent, immersive conversational agents by enabling high-intelligence multimodal dialogue with real-time generation and unified modeling of language, speech, motion, and video synthesis.
...
TL;DR: Selfment is a fully self-supervised framework that segments foreground objects from raw images without any manual annotations, pretrained models, or post-processing, achieving state-of-the-art results across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: The core challenge in computer vision is accurately segmenting objects without manual annotations. Current methods often rely on human labels, pretrained models, or post-processing, limiting their applicability in fully unsupervised scenarios.
...
Result: Selfment sets new state-of-the-art results: +4.0% F_max on ECSSD, +4.6% on HKUIS, +5.7% on PASCAL-S. It also demonstrates remarkable zero-shot generalization to camouflaged object detection (0.910 S_m on CHAMELEON, 0.792 F_β^ω on CAMO), outperforming unsupervised approaches and rivaling supervised methods.

Conclusion: Selfment presents a simple yet effective fully self-supervised framework that achieves superior object segmentation without any manual supervision, demonstrating strong performance and remarkable generalization capabilities across diverse segmentation tasks.
...
TL;DR: Diffusion Probe uses early cross-attention patterns to predict final image quality in text-to-image diffusion models, enabling early termination of low-quality generations and reducing computational waste.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models lack efficient early quality assessment mechanisms, leading to costly trial-and-error in multi-generation scenarios like prompt iteration, agent-based generation, and flow-grpo workflows.

Method: Leverages statistical properties of early-stage cross-attention maps from initial denoising steps as predictive signals. Uses a lightweight predictor to map these attention distributions to final image quality across diverse metrics.

Result: Achieves strong correlation (PCC > 0.7) and high classification performance (AUC-ROC > 0.9) across multiple T2I models, early denoising windows, resolutions, and quality metrics. Enables practical gains in prompt optimization, seed selection, and RL training.

Conclusion: Diffusion Probe is a model-agnostic, efficient framework that improves T2I generation efficiency through early quality prediction, reducing computational overhead while improving final output quality in practical workflows.
...
TL;DR: Fourier Angle Alignment (FAA) addresses directional incoherence and task conflict in rotated object detection using Fourier rotation equivariance, with FAAFusion for neck alignment and FAA Head for RoI feature pre-alignment, achieving SOTA results on remote sensing datasets.


<details>
  <summary>Details</summary>
Motivation: Mainstream rotated object detection methods suffer from two key bottlenecks: directional incoherence at the detector neck (feature misalignment between different levels) and task conflict at the detection head (conflicting requirements for classification and regression).

Method: Proposes Fourier Angle Alignment (FAA) that uses Fourier rotation equivariance to analyze angle information through frequency spectrum and align main directions. Introduces two plug-and-play modules: FAAFusion (aligns higher-level features to lower-level features at detector neck) and FAA Head (pre-aligns RoI features to canonical angle before classification/regression).

Result: Achieves state-of-the-art results of 78.72% mAP on DOTA-v1.0 and 72.28% mAP on DOTA-v1.5 with single scale training/testing. Also shows significant improvements on HRSC2016, validating efficacy in remote sensing object detection.

Conclusion: Fourier Angle Alignment effectively addresses directional incoherence and task conflict in rotated object detection through frequency-based angle analysis and alignment, providing a generalizable approach that can be integrated into existing detectors as plug-and-play modules.
...
TL;DR: Sea² adapts perception models to novel environments by using an intelligent pose-control agent rather than fine-tuning models, achieving significant performance gains on visual tasks without downstream labels.


<details>
  <summary>Details</summary>
Motivation: Pre-trained perception models degrade in novel environments like indoor scenes, and conventional fine-tuning causes catastrophic forgetting and requires expensive scene-specific annotations.

Method: Sea² keeps perception modules frozen and uses a pose-control agent that navigates to informative viewpoints. It transforms a VLM into a low-level pose controller through two-stage training: fine-tuning on rule-based exploration trajectories, then refining via unsupervised RL using perception module outputs as rewards.

Result: Achieved performance improvements of 13.54% on visual grounding, 15.92% on segmentation, and 27.68% on 3D box estimation on the ReplicaCAD dataset.

Conclusion: Sea² enables direct use of off-the-shelf perception models for various tasks without retraining, providing a paradigm shift from adapting perception modules to adapting how they are deployed through intelligent viewpoint selection.
...
Result: The approach outperforms 2D-based and point-cloud-based baselines in both simulation (RoboTwin benchmark) and real-world robot executions, achieving state-of-the-art performance in manipulation success, inter-arm coordination, and 3D spatial prediction accuracy.

Conclusion: By explicitly predicting how 3D scenes evolve together with action sequences, the policy gains strong spatial understanding and predictive capability using only RGB observations, demonstrating the effectiveness of leveraging 3D geometric foundation models for bimanual manipulation.
...
TL;DR: A continual learning framework for pathology report generation from whole-slide images that avoids storing past data by using compact domain footprints for generative replay and style descriptors for shifting reporting conventions.


<details>
  <summary>Details</summary>
Motivation: Clinical deployment of pathology report generation systems faces challenges with evolving data (new organs, institutions, reporting conventions) over time, and sequential fine-tuning causes catastrophic forgetting. Existing approaches assume static training with all data available simultaneously.

Method: Introduces an exemplar-free continual learning framework using compact domain footprints: codebooks of representative morphology tokens, slide-level co-occurrence summaries, and patch-count priors. These footprints enable generative replay by synthesizing pseudo-WSI representations. Also distills domain-specific linguistic characteristics into style descriptors to handle shifting reporting conventions, with inference identifying compatible descriptors from slide signals.

Result: Outperforms exemplar-free and limited-buffer rehearsal baselines across multiple public continual learning benchmarks, demonstrating practical effectiveness for evolving clinical settings.

Conclusion: Footprint-based generative replay provides a practical solution for continual learning in pathology report generation, enabling adaptation to new domains without storing raw data and handling evolving reporting conventions through style descriptors.
...
TL;DR: CPN-YOLO improves ship detection in SAR imagery using denoising, attention mechanisms, and Gaussian similarity loss to handle clutter, noise, and small targets.


<details>
  <summary>Details</summary>
Motivation: SAR imagery is crucial for ship detection but faces challenges with clutter, speckle noise causing false alarms, and small targets being missed in complex scenes.

Method: Three improvements to YOLOv8: 1) learnable large-kernel denoising module for input pre-processing, 2) PPA attention mechanism for multi-scale feature enhancement, 3) Gaussian similarity loss based on normalized Wasserstein distance.

Result: On SSDD dataset: 97.0% precision, 95.1% recall, 98.9% mAP, outperforming YOLOv8 baseline and other deep learning detectors. Also validated on HRSID dataset.

Conclusion: CPN-YOLO effectively addresses SAR ship detection challenges through targeted architectural improvements, achieving state-of-the-art performance in complex maritime environments.
...
TL;DR: APPO improves video reasoning by enhancing perception through attention-guided token optimization, outperforming baselines by 0.5-4% without expensive annotations.


<details>
  <summary>Details</summary>
Motivation: Current video reasoning relies too much on fine-grained perception rather than expert-level reasoning. When perception is fixed, improving reasoning yields minimal gains (0.7%), but enhancing perception significantly boosts performance (1.4%). Need to improve perception through reasoning without costly annotations.

Method: APPO (Attention-guided Perception Policy Optimization) uses token-level dense rewards to optimize intra-group perception tokens - tokens from different responses that focus on the same crucial video frame.

Result: APPO consistently outperforms GRPO and DAPO by 0.5% to 4% across diverse video benchmarks and model scales (3B/7B).

Conclusion: APPO provides a low-cost approach to enhance model perception through reasoning, serving diverse scenarios without expensive fine-grained annotations.
...
TL;DR: Multi-modal multi-task model for detecting AI-generated images and identifying their source models, achieving 5th place in competition with F1 scores of 83.16% and 48.88%.


<details>
  <summary>Details</summary>
Motivation: To detect AI-generated images and identify the specific generative models that created them, addressing the growing challenge of AI-generated content detection in real-world scenarios.

Method: Multi-modal multi-task model using pre-trained BERT and CLIP Vision encoders for text and image features, cross-modal feature fusion with tailored multi-task loss, and pseudo-labeling-based data augmentation to expand training data with high-confidence samples.

Result: Achieved 5th place in both Tasks A and B of the CT2: AI-Generated Image Detection competition, with F1 scores of 83.16% (detection) and 48.88% (model identification).

Conclusion: The proposed architecture is effective for AI-generated content detection and has potential for real-world applications, with source code publicly available.
...
TL;DR: ReSeg-CLIP: Training-free Open-Vocabulary Semantic Segmentation for remote sensing using SAM masks to constrain CLIP attention and model composition with weighted averaging.


<details>
  <summary>Details</summary>
Motivation: Address limitations of vision-language models like CLIP in semantic segmentation for remote sensing, particularly inappropriate interactions within self-attention layers that hinder segmentation performance.

Method: 1) Hierarchical scheme using SAM-generated masks to constrain self-attention interactions at multiple scales; 2) Model composition approach averaging parameters of multiple RS-specific CLIP variants with novel weighting scheme based on representational quality using varying text prompts.

Result: Achieves state-of-the-art results across three remote sensing benchmarks without requiring additional training.

Conclusion: ReSeg-CLIP effectively addresses CLIP's segmentation limitations for remote sensing through mask-constrained attention and model composition, achieving superior performance in open-vocabulary semantic segmentation without training overhead.
...
TL;DR: Hybrid V2X offloading system for autonomous driving that splits BEV 3D object detection between vehicle and cloud, reducing latency by 72% and improving accuracy by 20% with adaptive optimization.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving requires real-time situational awareness but faces latency issues due to high processing demands and limited onboard compute, especially in complex urban environments.

Method: Uses V2X communication to offload processing to cloud, transformer-based models for multi-camera BEV fusion, dynamic computation splitting based on layers and quantization, plus feature compression for network efficiency.

Result: 72% reduction in end-to-end latency vs onboard-only solution; adaptive optimization improves accuracy by up to 20% over static approaches under realistic bandwidth variability.

Conclusion: Hybrid V2X offloading with adaptive optimization effectively addresses latency constraints in autonomous driving while maintaining high detection accuracy under varying network conditions.
...
TL;DR: Altitude-adaptive visual place recognition using ground feature density analysis for aerial platforms under significant altitude variations without additional hardware.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of aerial visual place recognition (VPR) under significant altitude variations, especially for small- and medium-sized airborne platforms operating in diverse environments with limited sensor availability.

Method: Integrates ground feature density analysis with image classification: estimates relative altitude by analyzing ground feature density in images, applies altitude-based cropping to generate canonical query images, then uses classification-based VPR strategy for localization.

Result: Achieves high accuracy and robustness in both altitude estimation and VPR; boosts average R@1 by 29.85% and R@5 by 60.20% compared to VPR alone; reduces mean error by 202.1m compared to MMDE methods with additional improvements of 31.4% in R@1 and 44% in R@5.

Conclusion: Establishes a robust, vision-only framework for 3D visual place recognition that offers a practical, scalable, plug-and-play solution for accurate airborne platform localization under large altitude variations without requiring additional hardware.
...
TL;DR: RAM-based Real Embedding Extractor (REE) with degradation selection strategy improves degraded image recognition, combined with Mamba-based network using Conditional Feature Modulator (CFM) for better real-world image super-resolution balancing fidelity and perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Multimodal large models have shown good image super-resolution abilities using language conditions, but their performance on degraded images remains limited. The paper aims to address this limitation by improving recognition of degraded image content.

Method: 1) Analyze RAM's capabilities on degraded images via text similarity; 2) Propose degradation selection strategy to create Real Embedding Extractor (REE) using contrastive learning; 3) Use Conditional Feature Modulator (CFM) to incorporate REE's high-level information into a Mamba-based network for image restoration.

Result: REE achieves significant recognition performance gain on degraded images. The Mamba-based network with CFM effectively restores image textures and produces visually pleasing results, balancing fidelity and perceptual quality better than existing approaches.

Conclusion: The proposed REE with degradation selection strategy and Mamba-based network with CFM demonstrates strong potential for real-world image super-resolution applications, effectively addressing degraded image restoration while balancing fidelity and perceptual quality.
...
TL;DR: Proposes AoE system for low-cost, scalable egocentric data collection using smartphones and human agents to address embodied AI data scarcity.


<details>
  <summary>Details</summary>
Motivation: Existing embodied AI data collection methods have high infrastructure costs, complex hardware dependencies, and limited scope, making scalable expansion difficult. Humans are ideal embodied agents, and using their smartphones offers low-cost, sustainable data collection.

Method: 1) Neck-mounted smartphone holder for ergonomic egocentric capture; 2) Cloud-edge architecture with mobile app for real-time processing; 3) Automated cloud pipelines for labeling/filtering raw videos into training data; 4) Distributed collection system accessible to anyone, anytime, anywhere.

Result: Evaluated on data preprocessing quality and downstream tasks, showing that high-quality egocentric data significantly improves real-world generalization capabilities.

Conclusion: AoE system enables scalable, low-cost, scene-agnostic real-world interaction data collection by leveraging humans as embodied agents with smartphones, addressing data scarcity for embodied foundation models.
...
TL;DR: Self-supervised 3D occupancy flow estimation without human annotations or external flow supervision, using disentangled static/dynamic fields and temporal aggregation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D occupancy and motion estimation require expensive 3D occupancy/flow annotations, velocity labels from bounding boxes, or pretrained optical flow models, which are costly and limit scalability.

Method: Proposes self-supervised method that disentangles scene into separate static and dynamic signed distance fields, learns motion implicitly through temporal aggregation, and introduces strong self-supervised flow cue from features' cosine similarities.
...
Conclusion: The proposed self-supervised approach eliminates dependency on expensive annotations while effectively estimating 3D occupancy flow, making it more scalable for autonomous driving applications.
...
TL;DR: Ref-Adv is a new challenging REC benchmark designed to suppress shortcuts by requiring genuine visual reasoning with linguistically complex expressions and hard distractors, revealing significant performance gaps in current MLLMs.


<details>
  <summary>Details</summary>
Motivation: Standard REC benchmarks (RefCOCO series) have limitations: expressions are too short, images have few distractors, and redundant descriptors enable shortcut solutions that bypass genuine text understanding and visual reasoning.

Method: Created Ref-Adv benchmark with linguistically nontrivial expressions paired with only necessary information for unique target identification, using real images with hard distractors and annotated reasoning facets including negation.

Result: Despite strong performance on standard benchmarks, contemporary multimodal LLMs show marked performance drops on Ref-Adv, revealing reliance on shortcuts and gaps in visual reasoning and grounding.

Conclusion: Ref-Adv exposes limitations in current MLLMs' visual reasoning capabilities and aims to guide future work on improving visual reasoning and grounding in multimodal systems.
...
TL;DR: BUSD-Agent is a two-stage AI framework for breast ultrasound screening that uses experience-guided decision-making to reduce unnecessary diagnostic escalations and biopsies by 26-22% while improving specificity.
...
Method: A cascaded multi-agent framework with two stages: (1) lightweight screening clinic agent filters low-risk cases using classification models, (2) diagnostic clinic agent handles escalated cases with richer perception tools. Uses memory bank of past decision trajectories for retrieval-conditioned in-context adaptation without parameter updates.

Result: Reduced diagnostic escalation from 84.95% to 58.72% and biopsy referrals from 59.50% to 37.08% across 10 datasets. Improved screening specificity by 68.48% and diagnostic specificity by 6.33% compared to same architecture without trajectory conditioning.

Conclusion: Experience-guided decision-making through retrieval-conditioned adaptation effectively reduces unnecessary interventions in breast ultrasound screening while maintaining diagnostic accuracy, demonstrating the value of leveraging historical decision trajectories for clinical AI systems.
...
TL;DR: SegMate is an efficient 2.5D medical image segmentation framework that achieves state-of-the-art accuracy while reducing computational requirements by up to 2.5x and memory by up to 2.1x, with performance gains around 1%.


<details>
  <summary>Details</summary>
Motivation: Current medical image segmentation models achieve excellent accuracy but require substantial computational resources, limiting deployment in resource-constrained clinical settings.
...
Result: Achieves 93.51% Dice score on TotalSegmentator with only 295MB GPU memory, reduces computation by up to 2.5x and memory by 2.1x, with strong generalization on SegTHOR (86.85%) and AMOS22 (89.35%) in zero-shot evaluations.
...
TL;DR: CLIP models often incorrectly increase similarity scores when wrong details are added to correct descriptions (half-truths). CS-CLIP addresses this by fine-tuning with decomposed caption units and foils, improving compositional understanding.


<details>
  <summary>Details</summary>
Motivation: CLIP-style models fail to properly handle half-truths - when plausible but incorrect details are appended to correct descriptions, similarity scores paradoxically increase instead of decrease. This reveals a fundamental weakness in compositional understanding due to weak supervision on individual caption components.

Method: Proposes CS-CLIP (Component-Supervised CLIP) which: 1) decomposes captions into entity and relation units, 2) constructs minimally edited foils for each unit, 3) fine-tunes the model to score correct units above their foils while preserving standard dual-encoder inference architecture.

Result: On COCO, CLIP prefers correct shorter descriptions only 40.6% of the time (dropping to 32.9% for relations). CS-CLIP raises half-truth accuracy to 69.3% and improves average performance on established compositional benchmarks by 5.7 points.

Conclusion: Weak supervision on caption parts causes half-truth vulnerabilities in CLIP models. Explicit component-level supervision via CS-CLIP reduces these errors and improves compositional understanding, suggesting that addressing half-truths aligns with broader compositional reasoning gains.
...
TL;DR: A novel topology-driven transferability estimation framework for selecting optimal medical foundation models for segmentation tasks, outperforming existing methods by 31% without requiring fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current transferability estimation metrics for medical foundation models are designed for classification tasks and fail to capture the topological complexity needed for dense prediction tasks like segmentation, creating a computational bottleneck in model selection.

Method: Proposes a topology-driven framework with three components: (1) Global Representation Topology Divergence using Minimum Spanning Trees to quantify feature-label structural isomorphism, (2) Local Boundary-Aware Topological Consistency assessing manifold separability at anatomical boundaries, and (3) Task-Adaptive Fusion that dynamically integrates global and local metrics based on task semantic cardinality.

Result: Validated on the large-scale OpenMind benchmark across diverse anatomical targets and SSL foundation models, achieving approximately 31% relative improvement in weighted Kendall correlation compared to state-of-the-art baselines.

Conclusion: The proposed framework provides a robust, training-free proxy for efficient medical foundation model selection for segmentation tasks, significantly outperforming existing transferability estimation methods without the computational cost of fine-tuning.
...
TL;DR: GPU-SDF improves neural implicit surface reconstruction by explicitly estimating geometric prior uncertainty and using it to modulate prior influence rather than discarding uncertain regions, with additional edge distance field and multi-view consistency constraints.


<details>
  <summary>Details</summary>
Motivation: Existing neural implicit surface reconstruction methods struggle with fine details like thin structures due to unreliable/noisy geometric priors. Current approaches use implicit uncertainty filtering which is indirect and inefficient, and masking supervision in high-uncertainty regions leads to under-constrained optimization.
...
Conclusion: GPU-SDF addresses limitations of existing neural implicit surface reconstruction methods by leveraging geometric prior uncertainty and complementary constraints to better recover fine details in indoor scenes.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with 3D point cloud understanding, treating geometric reasoning as implicit mapping and suffering from geometric hallucinations where they generate plausible but structurally inaccurate responses.

Method: Proposes PointCoT framework with explicit CoT reasoning for 3D data using "Look, Think, then Answer" paradigm. Creates Point-Reason-Instruct benchmark (~86k samples) with hierarchical CoT annotations. Uses dual-stream multi-modal architecture to combine semantic appearance with geometric truth.

Result: PointCoT achieves state-of-the-art performance on complex reasoning tasks for 3D point cloud understanding.

Conclusion: Explicit Chain-of-Thought reasoning effectively addresses geometric hallucinations in 3D MLLMs, enabling more accurate and grounded understanding of 3D point cloud data through structured reasoning steps.
...
TL;DR: Dual-branch micro-expression network with parallel attention achieves 74.67% accuracy on CASME II, outperforming existing methods.
...
Method: Proposes a dual-branch micro-expression feature extraction network with parallel attention, including: 1) residual network to alleviate gradient vanishing and network degradation, 2) Inception network to enhance representation and suppress irrelevant regions, 3) adaptive feature fusion module to integrate dual-branch features.

Result: Achieves 74.67% accuracy on CASME II dataset, outperforming LBP-TOP by 11.26% and MSMMT by 3.36%, and other comparative methods.
...
TL;DR: AHAP reconstructs 3D humans from arbitrary camera perspectives without calibration, using multi-view geometry fusion for identity association, SMPL prediction, and precise localization, achieving competitive performance with 180× speedup.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view 3D human reconstruction methods require pre-calibration (checkerboards or MVS algorithms), limiting scalability and applicability in diverse real-world scenarios where calibration is impractical.

Method: Feed-forward framework with Cross-View Identity Association using learnable person queries and contrastive learning; Human Head module fuses cross-view features for SMPL prediction with reprojection losses; multi-view triangulation for precise 3D localization without camera calibration.
...
Conclusion: AHAP enables scalable 3D human reconstruction from arbitrary perspectives without calibration requirements, making it practical for diverse real-world applications through efficient multi-view geometry fusion.
...
<details>
  <summary>Details</summary>
Motivation: Current KB-VQA methods suffer from conflicts between static parametric knowledge in VLMs and dynamically retrieved information, leading to outputs that either ignore retrieved contexts or inconsistently integrate knowledge. Existing mitigation methods adapted from language approaches neglect visual information and suffer from redundant retrieved contexts.
...
Conclusion: CC-VQA effectively addresses knowledge conflicts in KB-VQA by incorporating visual information and correlation-aware mechanisms, demonstrating superior performance across multiple benchmarks without requiring training.
...
TL;DR: GDA-YOLO11 improves robotic fruit harvesting by using amodal segmentation to detect occluded fruits, achieving better performance than YOLO11n and higher harvesting success rates under occlusion.


<details>
  <summary>Details</summary>
Motivation: Occlusion in fruit harvesting leads to undetected fruits and crop losses, requiring better perception methods to handle occluded fruits in agricultural robotics.

Method: Proposed GDA-YOLO11 model with architectural improvements and asymmetric mask loss for amodal segmentation, trained on modified citrus dataset, with picking points estimated via Euclidean distance transform and projected to 3D for robotic execution.

Result: GDA-YOLO11 achieves precision 0.844, recall 0.846, mAP@50 0.914, mAP@50:95 0.636, outperforming YOLO11n. Harvesting success rates: 92.59% (zero occlusion), 85.18% (low), 48.14% (medium), 22.22% (high), with 3.5% improvement under medium/high occlusion.

Conclusion: GDA-YOLO11 enhances occlusion-robust segmentation and perception-to-action integration, demonstrating first practical amodal segmentation in fruit harvesting and enabling more reliable autonomous agricultural systems.
...
TL;DR: DeBiasLens is an interpretable, model-agnostic framework that uses sparse autoencoders to locate social attribute neurons in Vision-Language models and selectively deactivates them to mitigate bias while preserving semantic knowledge.


<details>
  <summary>Details</summary>
Motivation: Current debiasing methods only address surface-level bias signals through post-hoc learning or test-time algorithms, leaving the internal model dynamics unexplored. There's a need for interpretable approaches that understand and mitigate bias at the neural level in VLMs.

Method: Uses sparse autoencoders (SAEs) applied to multimodal encoders to localize social attribute neurons. SAEs are trained on facial image or caption datasets without social attribute labels to uncover neurons responsive to specific demographics. Then selectively deactivates the social neurons most strongly tied to bias for each group.

Result: Effectively mitigates socially biased behaviors of VLMs without degrading their semantic knowledge. The framework successfully identifies and neutralizes bias-causing neurons while maintaining model performance.

Conclusion: DeBiasLens provides an interpretable approach to bias mitigation that operates at the neural level, laying groundwork for future auditing tools that prioritize social fairness in real-world AI systems.
...
TL;DR: SwitchCraft is a training-free framework for multi-event video generation that uses Event-Aligned Query Steering and Auto-Balance Strength Solver to improve prompt alignment and temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video diffusion models are optimized for single-event generation and struggle with multi-event prompts, often producing blended or collapsed scenes that break the intended narrative.

Method: SwitchCraft introduces two key components: 1) Event-Aligned Query Steering (EAQS) that steers frame-level attention to align with relevant event prompts, and 2) Auto-Balance Strength Solver (ABSS) that adaptively balances steering strength to preserve temporal consistency and visual fidelity.
...
Conclusion: SwitchCraft offers a simple yet effective training-free solution for multi-event video generation, addressing the limitations of current text-to-video diffusion models.
...
TL;DR: QE proposes token-aware adaptive error compensation with mixture-of-experts for Vision-Language Models quantization, addressing distribution differences of important channels across inputs.


<details>
  <summary>Details</summary>
Motivation: Existing PTQ methods for VLMs rely on static identification and global compensation of sensitive channels, but overlook distributional differences of these important channels across inputs (across modalities and among tokens), leading to unsatisfactory quantization.

Method: Quant Experts (QE) divides important channels into token-independent and token-dependent groups. For token-independent channels, uses shared expert with low-rank adapter for global error compensation. For token-dependent channels, uses routed experts with multiple routed low-rank adapters for local error compensation specific to tokens.
...
Conclusion: The proposed token-aware adaptive error compensation with mixture-of-experts effectively addresses the distributional differences of important channels in VLMs, achieving superior quantization performance across different model scales.
...
TL;DR: NV-CoT enables MLLMs to perform visual reasoning using continuous numerical coordinates instead of textified coordinates or fixed patches, improving localization precision and answer accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM approaches for region-grounded reasoning have limitations: textified coordinates cause modality mismatch and semantic fragmentation, while fixed-granularity patches limit precise region selection and require architectural changes.

Method: Proposes Numerical Visual Chain-of-Thought (NV-CoT) that expands MLLM action space to continuous Euclidean space, allowing direct generation of bounding-box coordinates. Uses Gaussian/Laplace policies with reparameterized sampling, compatible with GRPO-style policy optimization via supervised fine-tuning and reinforcement learning.
...
Conclusion: NV-CoT validates the effectiveness of continuous-action visual reasoning in MLLMs, offering improved precision and accuracy with minimal architectural modifications.
...
<details>
  <summary>Details</summary>
Motivation: Current neural reconstruction methods (NeRF, 3D Gaussian Splatting) produce visually compelling results but suffer from artifacts in novel views and fail to realistically integrate dynamic objects from different scenes, limiting simulation fidelity for autonomous robot development.

Method: Introduces DiffusionHarmonizer, an online generative enhancement framework with a single-step temporally-conditioned enhancer converted from a pretrained multi-step image diffusion model. Uses a custom data curation pipeline to construct synthetic-real pairs emphasizing appearance harmonization, artifact correction, and lighting realism.

Result: Creates a scalable system that significantly elevates simulation fidelity, capable of running in online simulators on a single GPU while producing temporally consistent outputs.

Conclusion: DiffusionHarmonizer overcomes limitations of current neural reconstruction methods by providing a practical solution for realistic simulation of autonomous robots, improving both research and production environments.
...
TL;DR: SpikeTrack is a novel spike-driven framework for energy-efficient RGB object tracking that uses asymmetric design and memory-retrieval modules to achieve state-of-the-art performance with dramatically lower energy consumption than ANN trackers.


<details>
  <summary>Details</summary>
Motivation: Existing SNN tracking frameworks either don't fully align with spike-driven computation or don't fully leverage neurons' spatiotemporal dynamics, creating a trade-off between efficiency and accuracy in RGB visual tracking applications.
...
Conclusion: SpikeTrack is the first spike-driven framework to make RGB tracking both accurate and energy efficient, demonstrating the potential of SNNs for practical vision applications with dramatically reduced energy consumption.
...
TL;DR: Neuro-symbolic verification framework that audits internal consistency of VLM-generated radiology reports using SMT solver to detect logical inconsistencies between findings and diagnoses.


<details>
  <summary>Details</summary>
Motivation: Vision-language models for radiology report generation suffer from logical inconsistencies where diagnostic impressions aren't supported by perceptual findings or miss logically entailed conclusions. Standard metrics fail to capture these deductive failures.

Method: Neurosymbolic verification framework that autoformalizes free-text radiographic findings into structured propositional evidence, then uses SMT solver (Z3) and clinical knowledge base to verify whether each diagnostic claim is mathematically entailed, hallucinated, or omitted.

Result: Evaluation of seven VLMs across five chest X-ray benchmarks reveals distinct reasoning failure modes (conservative observation, stochastic hallucination) invisible to traditional metrics. Solver-backed entailment systematically eliminates unsupported hallucinations, increasing diagnostic soundness and precision.

Conclusion: The neurosymbolic verification framework provides rigorous post-hoc guarantees for clinical reasoning in VLM-generated reports, exposing reasoning failures and improving diagnostic reliability in generative clinical assistants.
...
TL;DR: AesGuide introduces the first large-scale aesthetic guidance dataset and Venus framework that empowers MLLMs to provide actionable shooting guidance and achieves SOTA in aesthetic cropping.


<details>
  <summary>Details</summary>
Motivation: There's a gap between ordinary users and professional photographers who can identify aesthetic issues and provide actionable guidance during capture. Existing MLLMs only offer positive feedback without identifying problems or providing practical shooting guidance.

Method: Introduces AesGuide dataset (10,748 photos with aesthetic scores, analyses, guidance) and Venus framework - a two-stage approach: 1) empowers MLLMs with aesthetic guidance capability through progressively complex questions, 2) activates aesthetic cropping via CoT-based rationales.
...
Conclusion: The work addresses the underexplored domain of aesthetic guidance, providing both dataset and framework that bridge the gap between ordinary users and professional photography expertise through actionable, interpretable guidance.
...
TL;DR: TASOT is an unsupervised method for surgical phase/step recognition that uses multimodal optimal transport with visual and text cues, eliminating need for heavy surgical pre-training.


<details>
  <summary>Details</summary>
Motivation: Current surgical video recognition methods rely on expensive large-scale pre-training on thousands of labeled videos, which incurs substantial computational and data collection costs. The authors question whether such heavy pre-training is truly necessary.
...
TL;DR: MIGM-Shortcut accelerates masked image generation models by learning a lightweight model to predict feature evolution, achieving 4x speedup while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Masked Image Generation Models (MIGMs) suffer from computational inefficiency due to multiple bi-directional attention steps, with existing acceleration methods having high approximation errors when features are aggressively cached.

Method: Proposes MIGM-Shortcut, a lightweight model that incorporates both previous features and sampled tokens to regress the average velocity field of feature evolution, balancing expressivity with computational efficiency.

Result: Achieves over 4x acceleration of text-to-image generation on Lumina-DiMOO while maintaining quality, significantly improving the Pareto frontier of masked image generation.

Conclusion: MIGM-Shortcut effectively addresses computational redundancy in MIGMs by learning feature evolution dynamics, enabling substantial speedups without quality degradation.
...
TL;DR: The paper proposes Omnivorous Vision Encoder, a framework that learns modality-agnostic feature representations by aligning different modalities of the same scene while distilling knowledge from pre-trained vision encoders like DINOv2.


<details>
  <summary>Details</summary>
Motivation: Pre-trained vision encoders like DINOv2 perform well on unimodal tasks but have poor feature alignment across different modalities (e.g., RGB images and corresponding depth maps show similar cosine similarity to unrelated random images). This limits cross-modal understanding capabilities.

Method: Train an encoder with dual objectives: 1) maximize feature alignment between different modalities of the same scene, and 2) use distillation to anchor learned representations to outputs of a frozen teacher model (e.g., DINOv2). This creates a modality-agnostic feature space.

Result: The resulting "omnivorous" encoder produces consistent, powerful embeddings for any given scene regardless of input modality (RGB, Depth, Segmentation, etc.), enabling robust cross-modal understanding while retaining discriminative semantics from the foundation model.

Conclusion: The proposed framework successfully addresses the modality alignment problem in pre-trained vision encoders, creating a versatile encoder that maintains strong semantic understanding across different input modalities while preserving the performance benefits of foundation models.
...
TL;DR: Proposed an ordinal latent diffusion model for generating diabetic retinopathy fundus images that incorporates ordered disease severity structure, improving realism and clinical consistency over standard categorical conditioning.


<details>
  <summary>Details</summary>
Motivation: Current conditional diffusion models treat disease stages as independent classes, ignoring the continuous nature of disease progression. This is problematic in medical imaging where continuous pathological processes are only observed through coarse, discrete but ordered labels like diabetic retinopathy severity stages.

Method: Developed an ordinal latent diffusion model using scalar disease representation instead of categorical conditioning, enabling smooth transitions between adjacent DR stages. Evaluated on EyePACS dataset using visual realism metrics and classification-based clinical consistency analysis.

Result: Reduced Fréchet inception distance for 4 of 5 DR stages compared to standard conditional diffusion model, increased quadratic weighted κ from 0.79 to 0.87. Interpolation experiments showed the model captured continuous spectrum of disease progression from ordered coarse labels.

Conclusion: The ordinal latent diffusion model successfully incorporates ordered disease structure into medical image generation, improving both visual realism and clinical consistency while capturing continuous disease progression from discrete labels.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D super-resolution methods rely on dense inputs and per-scene optimization, limiting reconstruction fidelity, generalization, and real-time usability. They're constrained by 2D super-resolution priors rather than learning 3D-specific high-frequency knowledge.

Method: SR3R reformulates 3DSR as direct feed-forward mapping from sparse LR views to HR 3DGS representations. It uses Gaussian offset learning and feature refinement to stabilize reconstruction and sharpen details. The framework is plug-and-play with any feed-forward 3DGS reconstruction backbone.
...
Conclusion: SR3R fundamentally changes how 3D super-resolution acquires high-frequency knowledge by learning 3D-specific priors from large-scale multi-scene data, enabling robust generalization and real-time performance without per-scene optimization.
...
TL;DR: SteerVAD: A novel intervention framework that actively steers and rectifies MLLM representations for video anomaly detection, achieving SOTA with only 1% training data.


<details>
  <summary>Details</summary>
Motivation: Traditional VAD methods require expensive labeled data and full training, while existing frozen MLLM approaches inherit pre-training biases and can't adapt to specific video contexts, struggling with subtle or ambiguous anomalies.

Method: Uses gradient-free representational separability analysis (RSA) to identify top attention heads as latent anomaly experts (LAEs), then applies hierarchical meta-controller (HMC) to generate dynamic rectification signals that anisotropically scale LAE representation manifolds.
...
Conclusion: SteerVAD establishes a powerful new direction for video anomaly detection by shifting from passive reading to active steering of MLLM representations.
...
TL;DR: GuardAlign is a training-free defense framework that improves safety in large vision-language models by combining optimal transport-based safety detection with cross-modal attentive calibration to reduce unsafe responses while maintaining utility.


<details>
  <summary>Details</summary>
Motivation: Current input-side defenses for large vision-language models suffer from inaccurate detection in complex scenes and unstable safety signals during decoding, creating critical safety challenges despite remarkable progress in vision-language reasoning tasks.
...
Conclusion: The proposed training-free defense framework effectively addresses limitations of existing safety approaches by combining accurate detection through optimal transport with stable safety signal maintenance via attention calibration, offering a practical solution for enhancing LVLM safety without compromising performance.
...
TL;DR: AIR is a training-free framework that reduces hallucination in MLLMs by adaptively reinforcing salient visual tokens while suppressing background noise.
...
Result: Extensive experiments across representative MLLMs show AIR substantially reduces hallucination while preserving general capabilities.

Conclusion: AIR establishes an effective training-free solution for building reliable MLLMs by enhancing reliance on salient visual information and mitigating hallucination.
...
TL;DR: A unified framework for high-fidelity 3D garment reconstruction from monocular images/videos using Implicit Sewing Patterns with diffusion models in UV space, achieving accurate reconstruction of both tight and loose clothing with temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Reconstructing 3D clothed humans from monocular images/videos is fundamental for virtual try-on, avatar creation, and mixed reality, but accurately reconstructing garment geometry, especially for loose-fitting clothing, remains challenging.

Method: Combines Implicit Sewing Patterns (ISP) with generative diffusion models to learn garment shape priors in 2D UV space. Introduces mapping model connecting image pixels, UV pattern coordinates, and 3D geometry. Extends to dynamic reconstruction using spatio-temporal diffusion with test-time guidance and analytic projection-based constraints for temporal consistency.

Result: Method generalizes well to real-world imagery despite training only on synthetic cloth data, outperforms existing approaches on both tight- and loose-fitting garments, preserves fine geometric detail with realistic dynamic motion.

Conclusion: Proposed framework enables high-fidelity 3D garment reconstruction from single images and videos, supporting downstream applications like texture editing, garment retargeting, and animation.
...
TL;DR: EvalMVX: A real-world dataset for benchmarking multiview 3D reconstruction techniques (MVS, MVPS, MVSfP) with 25 objects, 8,500 images, and ground-truth meshes.


<details>
  <summary>Details</summary>
Motivation: Current datasets focus on RGB-based multiview stereo (MVS), but multiview photometric stereo (MVPS) and multiview shape from polarization (MVSfP) lack quantitative assessment despite being crucial for high-fidelity reconstruction and sparse inputs.

Method: Created EvalMVX dataset with 25 objects captured using polarized camera under 20 views and 17 lighting conditions (OLAT and natural illumination), totaling 8,500 images with aligned ground-truth 3D meshes.
...
TL;DR: FoV-Net is a rotation-invariant B-rep learning framework that uses Local Reference Frame UV-grids and Field-of-View grids to capture both local geometry and global context, achieving SOTA performance on CAD analysis tasks.


<details>
  <summary>Details</summary>
Motivation: Existing B-rep learning methods rely on absolute coordinates and normals, making them highly sensitive to rotations (accuracy drops from 95% to 10% under arbitrary SO(3) rotations). There's a need for rotation-invariant CAD analysis.

Method: FoV-Net represents each face with: 1) LRF UV-grid for local surface geometry, 2) FoV grids that capture surrounding 3D context by casting rays and recording intersections. Lightweight CNNs extract per-face features, which are propagated via graph attention network over the B-rep graph.
...
Conclusion: FoV-Net is the first rotation-invariant B-rep learning framework that effectively captures both local and global geometric context, addressing the critical rotation sensitivity problem in CAD analysis while maintaining high performance.
...
TL;DR: FocusTrack is a one-stage 3D point cloud tracking framework that unifies motion-semantics co-modeling through inter-frame motion modeling and focus-and-suppress attention, achieving state-of-the-art performance at 105 FPS.
...
Method: Proposes FocusTrack with two core innovations: (1) Inter-frame Motion Modeling (IMM) using temporal-difference siamese encoder to capture global motion patterns, and (2) Focus-and-Suppress Attention that enhances foreground semantics via motion-salient feature gating and suppresses background noise without explicit segmentation.

Result: Achieves state-of-the-art performance on major 3D tracking benchmarks (KITTI, nuScenes, Waymo) while running at 105 FPS, demonstrating both accuracy and efficiency.

Conclusion: FocusTrack successfully addresses limitations of two-stage approaches through unified motion-semantics co-modeling in a one-stage pipeline, enabling end-to-end training with superior performance and high-speed inference.
...
TL;DR: AgenticOCR introduces query-driven OCR that selectively extracts relevant document regions instead of processing entire pages, improving visual RAG efficiency and accuracy for complex documents.


<details>
  <summary>Details</summary>
Motivation: Current page-level chunking in multimodal RAG systems introduces excessive extraneous context that overloads generators' attention mechanisms, dilutes salient evidence, and increases hallucination risks when compressing information-rich pages into limited visual token budgets.

Method: AgenticOCR transforms OCR from static full-text processing into a dynamic, query-driven extraction system that autonomously analyzes document layout in a "thinking with images" manner, identifying and selectively recognizing regions of interest to perform on-demand decompression of visual tokens.

Result: Experimental results show AgenticOCR improves both efficiency and accuracy of visual RAG systems, achieving expert-level performance in long document understanding while effectively decoupling retrieval granularity from rigid page-level chunking.

Conclusion: AgenticOCR has potential to serve as the "third building block" of visual document RAG stack alongside standard Embedding and Reranking modules, offering a paradigm shift from static OCR to dynamic, query-driven extraction for complex visual documents.
...
TL;DR: A method that improves 3D Gaussian Splatting efficiency through reconstruction-aware pruning and novel 3D Difference-of-Gaussians primitives, achieving 90% Gaussian reduction while maintaining or improving visual quality.


<details>
  <summary>Details</summary>
Motivation: 3DGS requires many primitives for high fidelity, leading to redundancy and high resource consumption, limiting scalability for complex/large scenes. Need effective pruning and more expressive primitives for practical deployment.

Method: 1) Efficient integrated reconstruction-aware pruning strategy that adaptively determines pruning timing and refining intervals based on reconstruction quality. 2) 3D Difference-of-Gaussians primitive that jointly models both positive and negative densities in a single primitive.
...
Conclusion: The proposed approach enables more efficient 3D scene representation with reduced redundancy and resource consumption while preserving or enhancing rendering quality, making 3DGS more practical for complex/large-scale scenes.
...
TL;DR: RETA improves dataset distillation by dynamically retrieving real patches and using topological alignment to enhance synthetic image diversity and generalization.


<details>
  <summary>Details</summary>
Motivation: Current decoupled dataset distillation methods suffer from fit-complexity gap and pull-to-anchor effect due to static real patches, reducing intra-class diversity and hurting generalization performance.

Method: Two components: 1) Dynamic Retrieval Connection (DRC) selects optimal real patches from a pool using fit-complexity scoring, 2) Persistent Topology Alignment (PTA) uses persistent homology to regularize synthetic data topology via mutual k-NN graphs and persistence images.

Result: RETA consistently outperforms baselines across multiple datasets (CIFAR-100, Tiny-ImageNet, ImageNet-1K), achieving 64.3% top-1 accuracy on ImageNet-1K with ResNet-18 at 50 images per class, +3.1% over prior best.

Conclusion: RETA effectively addresses limitations of current decoupled dataset distillation by combining dynamic patch retrieval with topological regularization, improving both diversity and generalization of synthetic datasets.
...
TL;DR: HumanOrbit generates 360° orbit videos from a single image using video diffusion models, producing consistent multi-view human images and reconstructing high-quality 3D meshes.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view synthesis methods from single images produce inconsistent results across views and fail to preserve original identity. Video diffusion models show promise for generating photorealistic, temporally consistent content.

Method: HumanOrbit uses a video diffusion model specifically designed for multi-view human image generation, enabling continuous camera rotations around subjects while maintaining geometric consistency and identity preservation. Includes a reconstruction pipeline to convert generated multi-view frames into textured 3D meshes.

Result: Experimental results show HumanOrbit effectively generates multi-view images and produces 3D models with superior completeness and fidelity compared to state-of-the-art baselines.

Conclusion: Video diffusion models are effective for generating consistent multi-view human images from single inputs, enabling high-quality 3D reconstruction with better geometric and identity preservation than existing methods.
...
TL;DR: RAViT: Multi-branch vision transformer with early exits that processes images at different resolutions to reduce FLOPs by ~30% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Vision transformers achieve excellent accuracy but have very high computational costs compared to CNNs. Need to reduce FLOPs while preserving performance.

Method: Multi-branch network processing same image at different resolutions. Early exit mechanism allows adaptive trade-off between accuracy and computation. Two-branch example: low-res image processed first, prediction reused with original image for final prediction with less computation.

Result: Equivalent accuracy to classical vision transformer with only ~70% of FLOPs on CIFAR-10, Tiny ImageNet, and ImageNet.

Conclusion: RAViT framework successfully reduces computational cost of vision transformers while maintaining accuracy through multi-resolution processing and early exits.
...
TL;DR: Proposes a superpixel hierarchy for high-dimensional images that considers both attribute space and spatial layout, enabling consistent exploration in both image and attribute spaces.


<details>
  <summary>Details</summary>
Motivation: Current hierarchical embedding methods for high-dimensional images ignore spatial layout, creating disconnect between image regions and attribute abstractions, hindering exploration of regions of interest.

Method: Develops a superpixel hierarchy construction method that incorporates both high-dimensional attribute manifold information and spatial pixel layout, creating image-guided hierarchical representation.

Result: The method enables consistent exploration of high-dimensional images in both image and attribute spaces, showing effectiveness compared to classical hierarchical embedding approaches in two use cases.

Conclusion: The proposed image-guided superpixel hierarchy addresses limitations of attribute-only hierarchical methods by integrating spatial information, improving exploration capabilities for large high-dimensional image datasets.
...
TL;DR: A novel framework using geometry-aware diffusion to reconstruct photorealistic, animatable 4D head avatars from single portrait images with accurate 3D geometry.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 4D head avatar reconstruction from single images rely on 2D priors and struggle with consistent 3D geometry, creating a need for better geometry-aware approaches.

Method: Uses geometry-aware diffusion to jointly synthesize portrait images and surface normals, with pose-free expression encoding, then incorporates both into 3D Gaussian-based avatars for photorealistic rendering.
...
Conclusion: The proposed framework successfully addresses the challenge of reconstructing photorealistic and animatable 4D head avatars with accurate geometry from single portrait images.
...
TL;DR: First automated multimodal auditing framework for medical image classifiers that extends slice discovery methods to detect hidden systematic failures more effectively than unimodal approaches.


<details>
  <summary>Details</summary>
Motivation: Existing auditing approaches for medical image classifiers rely on unimodal features or metadata-based subgroup analyses, which have limited interpretability and often fail to capture hidden systematic failures, raising safety and reliability concerns in practical clinical settings.

Method: Introduces an automated auditing framework that extends slice discovery methods to multimodal representations specifically for medical applications, using the MIMIC-CXR-JPG dataset for comprehensive experiments under common failure scenarios.

Result: The framework demonstrates strong capability in both failure discovery and explanation generation. Multimodal information allows more comprehensive and effective auditing of classifiers, while unimodal variants beyond image-only inputs show strong potential in resource-constrained scenarios.

Conclusion: Multimodal auditing frameworks provide superior failure detection and explanation capabilities for medical image classifiers compared to traditional unimodal approaches, with practical applications across different resource constraints.
...
TL;DR: SenCache: A sensitivity-aware caching framework that dynamically selects caching timesteps for diffusion video generation, reducing computation while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion models for video generation require expensive sequential denoising steps. Existing caching methods use heuristic criteria that need extensive tuning and lack theoretical foundation.

Method: Formalizes caching error through model output sensitivity analysis to perturbations in denoising inputs (noisy latent and timestep). Proposes SenCache, a dynamic caching policy that adaptively selects caching timesteps per sample based on sensitivity analysis.

Result: SenCache achieves better visual quality than existing caching methods under similar computational budgets on Wan 2.1, CogVideoX, and LTX-Video models.

Conclusion: Provides theoretical basis for adaptive caching, explains prior heuristic effectiveness, and extends to dynamic sample-specific approach that improves video generation efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Modern microscopy produces gigapixel images with structures across multiple spatial scales, but most vision models operate at single resolution or derive multi-scale features from one view, limiting their ability to exploit the inherently multi-resolution nature of microscopy data.

Method: MuViT embeds all patches into a shared world-coordinate system and extends rotary positional embeddings to these coordinates, enabling attention to integrate wide-field context with high-resolution detail within a single encoder.
...
TL;DR: A new method improves spatial understanding in text-to-image generation by creating a specialized reward model (SpatialScore) trained on 80k+ preference pairs, enabling better evaluation and reinforcement learning for spatial relationships.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with complex spatial relationships, requiring multiple sampling attempts to achieve satisfactory results. There's a need to enhance spatial understanding in image generation models.

Method: 1) Construct SpatialReward-Dataset with over 80k preference pairs for spatial relationships. 2) Build SpatialScore, a reward model that evaluates spatial accuracy in text-to-image generation. 3) Use this reward model to enable online reinforcement learning for complex spatial generation.

Result: SpatialScore outperforms leading proprietary models on spatial evaluation. Extensive experiments across multiple benchmarks show significant and consistent gains in spatial understanding for image generation.

Conclusion: The specialized reward model approach effectively strengthens spatial understanding in text-to-image generation models, addressing the challenge of encoding intricate spatial relationships through reinforcement learning.
...
TL;DR: GTASR proposes a consistency training paradigm for real-world image super-resolution that addresses consistency drift and geometric decoupling issues through trajectory alignment and dual-reference structural rectification.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based Real-ISR has high computational costs from iterative sampling. Existing distillation approaches using T2I priors have large parameter counts and are limited by teacher model capabilities. Consistency models offer lightweight alternatives but suffer from consistency drift accumulation and "Geometric Decoupling" where pixel alignment doesn't preserve structural coherence.

Method: GTASR introduces Trajectory Alignment (TA) strategy to rectify tangent vector field via full-path projection, and Dual-Reference Structural Rectification (DRSR) mechanism to enforce strict structural constraints in a consistency training paradigm for Real-ISR.
...
Conclusion: GTASR provides an effective solution to consistency drift and geometric decoupling in consistency models for real-world image super-resolution, achieving high-quality results with efficient inference.
...
TL;DR: The paper shows that compositional generalization requires linear, orthogonal representations, providing theoretical grounding for the Linear Representation Hypothesis and empirical validation across vision models.
...
Method: Formalize three desiderata for compositional generalization (divisibility, transferability, stability), derive necessary geometric constraints, and empirically evaluate predictions across vision models (CLIP, SigLIP, DINO).
...
Conclusion: The linear structure widely observed in neural representations is a necessary consequence of compositional generalization, and these geometric conditions predict the representational geometry models may converge to as they scale.
...
TL;DR: HAL model uses hierarchical causal modeling with varying timescales to improve weakly-supervised action segmentation by identifying high-level latent action variables that evolve slower than low-level visual features.


<details>
  <summary>Details</summary>
Motivation: Humans perceive actions through key transitions at multiple abstraction levels, while machines relying on visual features tend to over-segment. The challenge is enabling hierarchical reasoning in video understanding, especially given that low-level visual and high-level action latent variables evolve at different rates.

Method: Proposes Hierarchical Action Learning (HAL) model with hierarchical causal data generation process where high-level latent action governs low-level visual features. Uses deterministic processes to align latent variables over time, hierarchical pyramid transformer to capture both visual features and latent variables, and sparse transition constraint to enforce slower dynamics of high-level action variables.
...
Conclusion: The HAL model effectively addresses hierarchical reasoning in video understanding by modeling varying timescales between visual and action variables, achieving superior performance in weakly-supervised action segmentation with practical effectiveness in real-world applications.
...
TL;DR: Proposes MMM (Mode Seeking meets Mean Seeking) training paradigm using Decoupled Diffusion Transformer to generate minute-scale videos by combining global flow matching for long-term coherence with local distribution matching for short-term fidelity.


<details>
  <summary>Details</summary>
Motivation: Scaling video generation from seconds to minutes faces a bottleneck: short-video data is abundant and high-fidelity, but coherent long-form data is scarce and limited to narrow domains, creating a fidelity-horizon gap.

Method: Uses Decoupled Diffusion Transformer with two heads: global Flow Matching head trained via supervised learning on long videos for narrative structure, and local Distribution Matching head that aligns sliding windows to frozen short-video teacher via mode-seeking reverse-KL divergence.

Result: Enables synthesis of minute-scale videos that learn long-range coherence from limited long videos while inheriting local realism from short-video teacher, resulting in few-step fast long video generation that closes fidelity-horizon gap.

Conclusion: The MMM approach effectively addresses the long-video generation challenge by decoupling local fidelity from long-term coherence, jointly improving local sharpness, motion, and long-range consistency in minute-scale video synthesis.
...
TL;DR: UFO-4D is a unified feedforward framework that reconstructs dense 4D representations from just two unposed images by estimating dynamic 3D Gaussian Splats, enabling joint geometry, motion, and camera pose estimation.
...
Method: UFO-4D uses a unified feedforward framework that directly estimates dynamic 3D Gaussian Splats from image pairs. The core insight is that differentiably rendering multiple signals (appearance, depth, motion) from a single Dynamic 3D Gaussian representation enables self-supervised training with tight coupling between modalities.

Result: UFO-4D outperforms prior work by up to 3 times in joint geometry, motion, and camera pose estimation. It enables high-fidelity 4D interpolation across novel views and time, overcoming data scarcity through synergistic supervision.

Conclusion: UFO-4D demonstrates that a unified feedforward approach with dynamic 3D Gaussian representation can effectively solve dense 4D reconstruction from unposed images, achieving superior performance through shared geometric primitives and multi-modal supervision.
...
TL;DR: First large-scale MCP dataset with diverse user queries matching 2800 tools across 308 servers, addressing the gap in realistic user interaction evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing MCP datasets lack realistic, human-like user queries, failing to represent how different users portray requests, leading to poor generalization and inflated benchmark reliability.

Method: Developed on MCP Zero dataset, pairing each of 2800 tools across 308 MCP servers with multiple unique user personas to capture varying levels of user intent from precise tasks to ambiguous exploratory commands.

Result: Created first large-scale MCP dataset featuring diverse, high-quality user queries specifically generated to match real-world interaction patterns.

Conclusion: This dataset addresses critical gaps in evaluating MCP tool usage and ecosystems by providing realistic user queries that reflect the complexity of real-world interactions.
...
TL;DR: LLM agent with RAG automates adverse media screening, reducing false positives and manual review in AML/KYC compliance.


<details>
  <summary>Details</summary>
Motivation: Traditional keyword-based adverse media screening generates high false-positive rates and requires extensive manual review, creating inefficiencies in AML/KYC compliance processes.

Method: Multi-step agentic system using LLMs with Retrieval-Augmented Generation (RAG) that searches the web, retrieves/processes documents, and computes an Adverse Media Index (AMI) score for each subject.

Result: System evaluated with multiple LLM backends on dataset including PEPs, regulatory watchlist persons, sanctioned individuals from OpenSanctions, and clean names, demonstrating ability to distinguish high-risk from low-risk individuals.

Conclusion: Agentic LLM-RAG system provides automated, accurate adverse media screening that improves efficiency over traditional keyword-based approaches in financial compliance.
...
<details>
  <summary>Details</summary>
Motivation: Recent work showed some counterfactual distributions can be directly estimated experimentally (counterfactual realizability), but it was unclear what additional counterfactual quantities become identifiable with this new access to Layer 3 data.

Method: Developed CTFIDU+ algorithm for identifying counterfactual queries from arbitrary sets of Layer 3 distributions, proved its completeness, and derived analytic bounds for non-identifiable quantities using realizable counterfactual data.

Result: CTFIDU+ is complete for identifying counterfactual queries from Layer 3 distributions, establishing the theoretical limit of what counterfactuals can be identified from physically realizable distributions (fundamental limit to exact causal inference). Simulations show counterfactual data helps tighten bounds for non-identifiable quantities.

Conclusion: The paper provides a complete solution for counterfactual identification from Layer 3 data, establishes fundamental limits of causal inference, and offers practical bounds for quantities that cannot be exactly identified, demonstrating the value of counterfactual data in tightening inference bounds.
...
TL;DR: A causal POMDP framework for planning under distribution shifts using interventions to model environmental changes while preserving tractability via PWLC value functions.


<details>
  <summary>Details</summary>
Motivation: Real-world planning faces challenges from distribution shifts where environment models become invalid as conditions change, causing learned strategies to fail. There's a need for planning methods that can handle partial observability while adapting to changing environmental dynamics.

Method: Proposes a theoretical framework using Partially Observable Markov Decision Processes (POMDPs) formulated with causal knowledge. Represents distribution shifts as interventions on causal POMDPs, enabling evaluation of plans under hypothesized changes and identification of altered environmental components. Maintains and updates beliefs over both latent state and underlying domain.

Result: Proves that the value function remains piecewise linear and convex (PWLC) in the augmented belief space. This preservation maintains the tractability of planning via α-vector-based POMDP methods even under distribution shifts.

Conclusion: The framework provides a principled approach to planning under partial observability and distribution shifts by leveraging causal knowledge and intervention modeling, while preserving computational tractability through PWLC value functions.
...
TL;DR: Hybrid RL-CMSA method combines reinforcement learning with exact optimization to solve min-max mTSP, outperforming state-of-the-art genetic algorithms.


<details>
  <summary>Details</summary>
Motivation: The min-max mTSP aims to balance workload among multiple salesmen by minimizing the longest tour, which is important for practical applications like vehicle routing and task scheduling. Existing methods need improvement in solution quality, especially for larger instances.

Method: RL-CMSA iteratively: 1) constructs diverse solutions using probabilistic clustering guided by learned q-values, 2) merges routes into a compact pool, 3) solves restricted set-covering MILP, 4) refines solutions via inter-route moves, and 5) updates q-values based on high-quality solutions while adapting the pool through ageing and pruning.

Result: Computational results on random and TSPLIB instances show RL-CMSA consistently finds (near-)best solutions and outperforms a state-of-the-art hybrid genetic algorithm under comparable time limits, especially as instance size and number of salesmen increase.

Conclusion: The hybrid approach combining exact optimization with reinforcement-guided construction effectively balances exploration and exploitation, demonstrating superior performance for the symmetric single-depot min-max mTSP compared to existing methods.
...
TL;DR: SleepLM is a sleep-language foundation model that bridges natural language with multimodal sleep data, enabling language-based sleep analysis, querying, and generalization beyond predefined labels.


<details>
  <summary>Details</summary>
Motivation: Current sleep analysis systems are limited to closed label spaces (predefined stages/events) and cannot describe, query, or generalize to novel sleep phenomena, creating a gap between sleep physiology and natural language understanding.

Method: Developed a multilevel sleep caption generation pipeline to create the first large-scale sleep-text dataset (100K+ hours from 10K+ individuals), and used a unified pretraining objective combining contrastive alignment, caption generation, and signal reconstruction.

Result: SleepLM outperforms state-of-the-art in zero-shot/few-shot learning, cross-modal retrieval, and sleep captioning, while demonstrating novel capabilities like language-guided event localization and zero-shot generalization to unseen tasks.

Conclusion: SleepLM successfully bridges natural language with multimodal sleep data, enabling more flexible and comprehensive sleep analysis, with all code and data to be open-sourced for community use.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for synthesizing training data have limitations in long-tail knowledge coverage, effectiveness verification, and interpretability. Knowledge-graph-based approaches lack functionality, granularity, customizability, and proper evaluation.
...
Result: Fine-tuning Qwen3 models (0.6B/8B/32B) on synthesized samples improves reasoning accuracy by 9.2%. Framework generates distinct data challenging existing models on table/formula tasks, useful for complex benchmark construction.

Conclusion: MMKG-RDS effectively addresses limitations in reasoning data synthesis, demonstrating improved model performance and generating challenging data for complex benchmarks. Dataset and code are publicly available.
...
TL;DR: The paper critiques the concept of Artificial General Intelligence (AGI) as flawed and proposes Superhuman Adaptable Intelligence (SAI) as a better framework - AI that specializes in exceeding human capabilities and filling human skill gaps.


<details>
  <summary>Details</summary>
Motivation: The paper identifies confusion and disagreement around AGI definitions, questioning whether humans are truly "general" and whether current AGI concepts are plausible, useful, or truly general for describing AI's future.

Method: The authors analyze existing AGI definitions, critique their coherence and usefulness, then propose an alternative framework called Superhuman Adaptable Intelligence (SAI) that emphasizes specialization and superhuman performance rather than generality.

Result: The paper argues that AGI is a flawed concept and introduces SAI as a more useful framework - intelligence that can learn to exceed humans at important tasks and fill human skill gaps through specialization.

Conclusion: AI should embrace specialization rather than strive for generality, focusing on superhuman performance in specific domains (SAI), which provides clearer guidance for AI development than the overloaded AGI concept.
...
TL;DR: PseudoAct introduces a pseudocode planning framework for LLM agents that replaces reactive decision-making with structured pseudocode synthesis for more efficient long-horizon tasks.
...
Method: PseudoAct synthesizes structured pseudocode plans that decompose tasks into subtasks with explicit control flow (sequencing, conditionals, loops, parallel composition), then executes actions by following this global plan.
...
TL;DR: ODAR-Expert is an adaptive routing framework that optimizes accuracy-efficiency trade-offs by dynamically routing queries between fast and slow LLM agents based on difficulty estimation, using free-energy-based answer fusion instead of uniform sampling.


<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning approaches rely on costly uniform brute-force sampling (like best-of-N or self-consistency) that leads to diminishing returns, overthinking, and inefficient resource allocation. There's a need for adaptive resource allocation that optimizes the accuracy-efficiency trade-off.

Method: ODAR-Expert uses a difficulty estimator based on amortized active inference to dynamically route queries between a heuristic Fast Agent and a deliberative Slow Agent. It employs a free-energy-principled, risk-sensitive fusion mechanism that selects answers by minimizing a variational free energy objective, balancing log-likelihood with epistemic uncertainty (varentropy).

Result: Achieves 98.2% accuracy on MATH and 54.8% on Humanity's Last Exam (HLE), improves compute-accuracy frontier under compute-matched settings, and reduces computational costs by 82% compared to homogeneous sampling strategies on open-source models (Llama 4 + DeepSeek).

Conclusion: Thinking-optimal scaling requires adaptive resource allocation with free-energy-based decision-making rather than simply increasing test-time compute, demonstrating that principled resource allocation outperforms uniform sampling approaches.
...
TL;DR: CHIEF is a framework that transforms chaotic multi-agent system trajectories into hierarchical causal graphs and uses oracle-guided backtracking with counterfactual attribution to identify root causes of failures.


<details>
  <summary>Details</summary>
Motivation: Current LLM-powered Multi-Agent Systems suffer from fragility and opaque failure mechanisms. Existing failure attribution methods treat execution logs as flat sequences, failing to capture the intricate causal links in MAS, leading to weak observability and ambiguous responsibility boundaries.

Method: CHIEF transforms chaotic trajectories into structured hierarchical causal graphs, employs hierarchical oracle-guided backtracking to prune search space via synthesized virtual oracles, and implements counterfactual attribution through progressive causal screening to distinguish true root causes from propagated symptoms.

Result: Experiments on Who&When benchmark show CHIEF outperforms eight strong state-of-the-art baselines on both agent- and step-level accuracy. Ablation studies confirm the critical role of each proposed module.

Conclusion: CHIEF provides a novel framework for effective failure attribution in multi-agent systems by moving beyond flat sequence analysis to hierarchical causal modeling, enabling better observability and clearer responsibility assignment.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based e-commerce agents lack interaction depth and contextual breadth for complex product research, while existing Deep Research paradigms suffer from domain gaps when applied to e-commerce shopping scenarios.

Method: A multi-agent framework with User Agent (infers shopping intents from behavioral histories), Supervisor Agent (orchestrates iterative collaboration), and Research Agent generates synthetic tool-use trajectories. These are filtered and distilled through reflective internalization to create coherent single-role training examples for LLM fine-tuning.

Result: A compact MoE model fine-tuned on the synthetic data shows substantial improvements over its base model in response comprehensiveness, research depth, and user-perceived utility, approaching performance of frontier proprietary deep research systems.

Conclusion: Multi-agent synthetic trajectory training is an effective and scalable paradigm for enhancing LLM-based shopping assistance, enabling robust e-commerce conversational agents capable of complex product research.
...
TL;DR: Auton Agentic AI Framework bridges the gap between stochastic LLM outputs and deterministic backend systems through a principled architecture with cognitive blueprints, runtime engines, and formal safety/optimization mechanisms.


<details>
  <summary>Details</summary>
Motivation: Transition from Generative AI to Agentic AI reveals fundamental architectural mismatch: LLMs produce stochastic, unstructured outputs while backend infrastructure requires deterministic, schema-conformant inputs.

Method: Framework separates Cognitive Blueprint (declarative, language-agnostic specification) from Runtime Engine (platform-specific execution). Introduces augmented POMDP with latent reasoning, hierarchical memory consolidation, constraint manifold for safety, three-level self-evolution, and runtime optimizations like parallel graph execution.

Result: Enables cross-language portability, formal auditability, modular tool integration via Model Context Protocol (MCP), reduces end-to-end latency for multi-step agent workflows through various optimizations.

Conclusion: Auton Agentic AI Framework provides a principled architecture standardizing creation, execution, and governance of autonomous agent systems, addressing the fundamental mismatch between LLM outputs and backend requirements.
...
TL;DR: MERaLiON2-Omni (Alpha) is a 10B-parameter multilingual MLLM for Southeast Asia that decouples perception and reasoning, achieving strong performance but revealing an efficiency-stability paradox where reasoning boosts abstract tasks but destabilizes sensory processing.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack robust integration of sensory grounding with complex reasoning, especially for underrepresented regions like Southeast Asia with unique linguistic and cultural characteristics.

Method: Progressive training pipeline: 1) Perception Backbone via orthogonal modality adaptation of region-specific audio-visual cues; 2) Generate-Judge-Refine pipeline using Super-LLM to synthesize silver data for transferring textual Chain-of-Thought to multimodal reasoning.

Result: Identified Efficiency-Stability Paradox: reasoning amplifies abstract tasks (math, instruction-following) but introduces instability in sensory processing (Temporal Drift in audio, Visual Over-interpretation). Comprehensive evaluation on SEA-Omni Benchmark Suite.

Conclusion: The report details architecture, data-efficient training, and diagnostic analysis of trade-offs between robust perception and structured reasoning, highlighting challenges in integrating System 1 (perception) and System 2 (reasoning) capabilities.
...
TL;DR: RD-MLDG uses multimodal LLMs with reasoning chains for domain generalization, addressing challenges in optimizing complex reasoning sequences through multi-task training and self-aligned regularization.


<details>
  <summary>Details</summary>
Motivation: Most DG methods focus on visual feature invariance, but this paper explores using multimodal LLMs' reasoning capabilities to construct reasoning chains for more robust predictions under domain shift.

Method: Proposes RD-MLDG with two components: MTCT (Multi-Task Cross-Training) adds direct classification to guide reasoning supervision, and SARR (Self-Aligned Reasoning Regularization) preserves semantic richness while mitigating reasoning-pattern mismatches via iterative self-labeling.
...
Conclusion: Reasoning chains from multimodal LLMs provide a valuable complementary approach to traditional visual feature invariance methods for domain generalization, with RD-MLDG effectively addressing optimization challenges through multi-task training and self-alignment.
...
TL;DR: EMO-R3 framework enhances MLLMs' emotional reasoning using structured thinking and reflective reinforcement learning, improving interpretability and performance on emotional understanding benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with complex human emotions - supervised fine-tuning has limited generalization, while reinforcement learning methods like GRPO fail to align with emotional cognition characteristics.

Method: Proposes EMO-R3 framework with two key components: Structured Emotional Thinking for step-by-step interpretable reasoning, and Reflective Emotional Reward for re-evaluating reasoning based on visual-text consistency and emotional coherence.
...
Conclusion: EMO-R3 effectively addresses limitations of existing approaches by combining structured reasoning with reflective reinforcement learning, enhancing MLLMs' ability to handle complex emotional understanding tasks.
...
TL;DR: RUMAD is a reinforcement learning framework for dynamic communication topology control in multi-agent debate systems that optimizes accuracy, consensus, and efficiency simultaneously.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent debate approaches struggle to balance accuracy, consensus formation, and computational efficiency. Static topologies lack adaptability, while external LLM coordination risks introducing privileged knowledge that compromises debate neutrality.

Method: RUMAD formulates dynamic communication topology control as a reinforcement learning problem with content-agnostic observations, multi-objective rewards (quality, cohesion, efficiency), PPO-trained controller for edge weight adjustment, and dual-threshold mechanism for agent activation and information visibility control.

Result: Achieves over 80% token cost reduction while improving reasoning accuracy compared to single LLM and multiple MAD baselines across MMLU, GSM8K, and GPQA benchmarks. Shows robust zero-shot generalization to out-of-domain tasks when trained only on MMLU.

Conclusion: RUMAD establishes an efficient and robust approach for deploying multi-agent reasoning applications with practical resource constraints, with learned communication strategies capturing task-independent principles of effective coordination.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based reward function design methods suffer from poor utilization of historical feedback and inefficient search, limiting improvements in complex control tasks.

Method: Treats LLMs as language agents and frames reward design as sequential decision-making, integrating Monte Carlo Tree Search to manage the optimization process and leverage LLMs' multi-stage contextual reasoning.

Result: Outstanding experimental results in 17 diverse low-level control tasks demonstrate the effectiveness of the proposed method.

Conclusion: RF-Agent provides an effective framework for designing reward functions by enhancing optimization through better contextual reasoning and improved search efficiency.
...
TL;DR: PAPER proposes using a pessimistic auxiliary policy to sample reliable actions in offline RL, reducing approximation errors and overestimation by maximizing Q-function's lower confidence bound.


<details>
  <summary>Details</summary>
Motivation: Offline RL faces issues with out-of-distribution actions during learning, causing approximation errors that accumulate and lead to overestimation. Need to sample more reliable actions to reduce these errors.

Method: Construct a pessimistic auxiliary policy that maximizes the lower confidence bound of the Q-function. This policy samples actions with high value but low uncertainty near the learned policy, avoiding high-error actions.

Result: Extensive experiments on offline RL benchmarks show the pessimistic auxiliary strategy effectively improves the performance of other offline RL approaches by reducing error accumulation.

Conclusion: Using a pessimistic auxiliary policy to sample reliable actions alleviates approximation error accumulation in offline RL and enhances existing methods' effectiveness.
...
TL;DR: Proposes macro-conditioned scenario-context rollout (SCR) to generate plausible next-day return scenarios during market stress, with a novel counterfactual state construction method to stabilize RL critic training in portfolio rebalancing.


<details>
  <summary>Details</summary>
Motivation: Market regime shifts cause distribution shifts that degrade portfolio rebalancing policy performance, and historical data alone cannot capture what would have happened differently during stress events.

Method: Uses macro-conditioned scenario-context rollout (SCR) to generate plausible next-day multivariate return scenarios under stress, constructs counterfactual next states using rollout-implied continuations, and augments critic agent's bootstrap target to address reward-transition mismatch.

Result: In out-of-sample evaluations across 31 distinct universes of U.S. equity and ETF portfolios, improves Sharpe ratio by up to 76% and reduces maximum drawdown by up to 53% compared to classic and RL-based baselines.

Conclusion: The proposed method effectively addresses distribution shifts during market stress by generating plausible scenarios and stabilizing RL training, leading to significantly improved portfolio performance metrics.
...
TL;DR: CIRCLE is a six-stage lifecycle framework that bridges the gap between AI model metrics and real-world outcomes by operationalizing validation to connect stakeholder concerns with measurable signals.


<details>
  <summary>Details</summary>
Motivation: Decision-makers lack systematic evidence about AI behavior under real-world conditions, as current frameworks focus on system stability (MLOps) or abstract capabilities (benchmarks) without addressing materialized outcomes in deployment.

Method: CIRCLE operationalizes the Validation phase of TEVV by formalizing translation of stakeholder concerns into measurable signals, integrating field testing, red teaming, and longitudinal studies into a six-stage coordinated pipeline.

Result: Produces systematic knowledge that is comparable across sites yet sensitive to local context, enabling evidence-based governance of AI's downstream effects rather than theoretical capabilities.

Conclusion: CIRCLE provides a structured, prospective protocol for linking qualitative insights to quantitative metrics, addressing limitations of participatory design and algorithmic audits to enable governance based on materialized outcomes.
...
TL;DR: First Turing test for speech-to-speech systems reveals none pass human-likeness test; bottleneck is paralinguistic features, emotional expressivity, and conversational persona rather than semantic understanding.


<details>
  <summary>Details</summary>
Motivation: To determine whether modern speech-to-speech systems can converse like humans by conducting the first Turing test specifically for S2S conversational systems.

Method: Collected 2,968 human judgments on dialogues between 9 state-of-the-art S2S systems and 28 human participants, developed a fine-grained taxonomy of 18 human-likeness dimensions, and crowd-annotated dialogues accordingly.

Result: No existing evaluated S2S system passes the Turing test, revealing significant gap in human-likeness; off-the-shelf AI models perform unreliably as judges; proposed interpretable model delivers accurate human-vs-machine discrimination.

Conclusion: Establishes first human-likeness evaluation for S2S systems, moves beyond binary outcomes to enable diagnostic insights, and provides tools for improving conversational AI systems toward human-like performance.
...
TL;DR: Novel bi-level optimization framework using RL for strategic network partitioning and multi-objective VRP for winter road maintenance, validated on UK road networks with improved workload balance, travel times, emissions, and costs.


<details>
  <summary>Details</summary>
Motivation: Winter road maintenance is critical for public safety and environmental impact, but existing methods struggle with large-scale routing problems and rely heavily on human decision-making, lacking scalability and efficiency.

Method: Bi-level optimization framework: upper level uses reinforcement learning to partition road networks into clusters and allocate resources from multiple depots; lower level solves multi-objective vehicle routing problem within each cluster minimizing maximum travel time and total carbon emissions.
...
Conclusion: Advanced AI-driven bi-level optimization can directly enhance operational decision-making in real-world transportation and logistics by efficiently handling large-scale networks with vehicle-specific constraints, depot capacities, and road segment requirements.
...
<details>
  <summary>Details</summary>
Motivation: To develop AI systems that are reality-embedded, resource-bounded agents that function as part of extended human-tool systems, increasing sensing, understanding, and actuation capabilities while reducing friction between people, tools, and environments.

Method: Unifies predictive compression, intrinsic motivation, empowerment/control, interface quality, and language/self-communication as selective information bottlenecks. Formulated as falsifiable program with explicit costs, staged experiments, and concrete multimodal tokenized testbed where agents allocate limited budget among observation, action, and deliberation.
...
Conclusion: AAP provides a comprehensive position and research agenda for developing AI as reality-embedded, resource-bounded agents driven by curiosity-as-learning-progress, offering a unified framework that bridges multiple theoretical domains with practical experimental testbeds.
...
TL;DR: SCOPE framework uses Process Reward Models to identify first errors in suboptimal rollouts and applies step-wise correction to salvage partially correct trajectories, improving exploration diversity by 13.5% and achieving SOTA results in math reasoning.


<details>
  <summary>Details</summary>
Motivation: Standard outcome-based RLVR penalizes largely correct trajectories as heavily as completely wrong ones, causing models to discard valuable partially correct rollouts. This reduces rollout diversity and prematurely narrows the exploration space, limiting learning effectiveness.

Method: SCOPE (Step-wise Correction for On-Policy Exploration) uses Process Reward Models to pinpoint the first erroneous step in suboptimal rollouts, then applies fine-grained, step-wise off-policy rectification to salvage partially correct trajectories rather than discarding them.

Result: Increases diversity score by 13.5%, achieves average accuracy of 46.6% on math reasoning tasks, and shows robust generalization with 53.4% accuracy on out-of-distribution reasoning tasks, establishing new state-of-the-art results.

Conclusion: SCOPE effectively addresses the exploration space narrowing problem in RLVR by salvaging partially correct trajectories through step-wise correction, leading to improved diversity and better performance on complex reasoning tasks.
...
TL;DR: New benchmark for evaluating LLMs on research-level mathematics using automatically extracted lemmas from arXiv, enabling regular updates without compromising evaluation integrity.
...
Method: Automatic pipeline that extracts lemmas from arXiv papers and rewrites them into self-contained statements by making all assumptions and definitions explicit, creating an updatable benchmark where previous instances can be used for training without affecting future evaluations.

Result: Current state-of-the-art LLMs achieve only 10-15% accuracy (pass@1) in theorem proving on this benchmark, showing significant gap between current LLM capabilities and human-level proving in research contexts.

Conclusion: There is a large margin of progression needed for LLMs to reach human-level theorem proving capabilities in mathematical research, and this updatable benchmark provides a better evaluation framework than static benchmarks.
...
TL;DR: Proposes a deep reinforcement learning method with heterogeneous graph networks for Flexible Job Shop Scheduling with Limited Buffers and Material Kitting, outperforming traditional methods on makespan and pallet changes.


<details>
  <summary>Details</summary>
Motivation: Current FJSP studies often ignore practical constraints like limited buffers, which significantly impact production efficiency. The paper addresses the gap between idealized academic models and real production scenarios.

Method: Uses deep reinforcement learning with heterogeneous graph networks to model global state, constructing efficient message passing among machines, operations, and buffers to avoid decisions causing frequent pallet changes.

Result: Outperforms traditional heuristics and advanced DRL methods on both synthetic and real production line datasets in terms of makespan and pallet changes, achieving good balance between solution quality and computational cost.

Conclusion: The proposed DRL approach with heterogeneous graph networks effectively handles complex dependencies and long-term constraints in practical FJSP with limited buffers, providing better buffer utilization and decision quality.
...
TL;DR: UMPIRE is a training-free uncertainty quantification framework for multimodal LLMs that computes incoherence-adjusted semantic volume of responses using internal model features, outperforming baselines across modalities.


<details>
  <summary>Details</summary>
Motivation: MLLMs produce plausible but erroneous outputs, hindering reliable deployment. Existing uncertainty metrics have practical constraints: modality-specific, reliant on external tools, or computationally expensive.

Method: UMPIRE computes incoherence-adjusted semantic volume of sampled MLLM responses using only internal modality features. It captures global semantic diversity and local incoherence based on internal model confidence.

Result: UMPIRE consistently outperforms baseline metrics in error detection and uncertainty calibration across image, audio, and video-text benchmarks, including adversarial and out-of-distribution settings. Also generalizes to non-text output tasks.

Conclusion: UMPIRE provides an effective, training-free uncertainty quantification framework for MLLMs that works efficiently across modalities without external tools, enabling more reliable deployment through better uncertainty estimation.
...
TL;DR: A minimal agentic baseline for AI theorem proving that enables systematic comparison of different architectures, featuring iterative proof refinement, library search, and context management.


<details>
  <summary>Details</summary>
Motivation: To create a standardized baseline for comparing different AI-based theorem prover architectures, as current state-of-the-art systems share core features but lack a common framework for systematic evaluation.

Method: Designs a minimal agentic baseline implementing core features shared by state-of-the-art systems: iterative proof refinement, library search, and context management. Evaluates using qualitatively different benchmarks and compares various popular models and design choices.
...
Conclusion: The minimal baseline provides a valuable reference for future research and an accessible prover for the community, showing that simpler iterative architectures can match complex state-of-the-art systems while being more efficient.
...
TL;DR: DARE-bench is a new benchmark for evaluating LLMs on data science tasks with verifiable ground truth, addressing gaps in existing benchmarks through standardized process-aware evaluation and providing large-scale training data.


<details>
  <summary>Details</summary>
Motivation: There are two major gaps in existing benchmarks for LLMs on data science tasks: (1) lack of standardized, process-aware evaluation that captures instruction adherence and process fidelity, and (2) scarcity of accurately labeled training data.

Method: Introduced DARE-bench with 6,300 Kaggle-derived tasks that have verifiable ground truth, providing both large-scale training data and evaluation sets to cover a broad range of tasks and support agentic tools.

Result: Even highly capable models like gpt-o4-mini struggle, especially in machine learning modeling tasks. Fine-tuning with DARE-bench training tasks substantially improves performance: supervised fine-tuning boosts Qwen3-32B accuracy by 1.83x, and reinforcement learning boosts Qwen3-4B accuracy by more than 8x.
...
TL;DR: Survey analyzes cybersecurity threats to teleoperated quadruped robots, proposing attack taxonomy, consequence mapping, TRL analysis of defenses, platform comparisons, and research priorities.


<details>
  <summary>Details</summary>
Motivation: Quadruped robots are increasingly used in safety-critical missions but their security infrastructure is insufficiently characterized, with unique challenges from dynamic stability, gait vulnerabilities, kinetic energy, and operator cognitive load.

Method: Synthesizes peer-reviewed literature and vulnerability disclosures (2019-2025) to create comprehensive analysis including: six-layer attack taxonomy, attack-to-consequence mapping with timing, TRL classification of defenses, comparative security analysis of six commercial platforms, deployment guidance, and research gap identification.

Result: Identifies critical maturity gaps between field-deployed communication protections (TRL 7-9) and experimental perception/operator-layer defenses (TRL 3-5), provides platform-specific security assessments, and offers pragmatic deployment guidance with implementation timelines.

Conclusion: Teleoperated quadruped systems face significant cybersecurity challenges requiring prioritized research and development, particularly in perception manipulation and operator-layer defenses where current protections lag behind communication security measures.
...
TL;DR: Action space design significantly impacts robotic policy learning; delta actions consistently improve performance, while joint-space vs task-space representations offer complementary stability vs generalization benefits.


<details>
  <summary>Details</summary>
Motivation: Action space specification is crucial for imitation-based robotic manipulation policy learning but remains guided by ad-hoc heuristics, creating ambiguity in robotic policy design philosophies that needs systematic investigation.

Method: Large-scale empirical study dissecting action design space along temporal (absolute vs delta) and spatial (joint-space vs task-space) axes, evaluating 500+ trained models across four scenarios with 13,000+ real-world rollouts on a bimanual robot.

Result: Delta action representations consistently improve performance; joint-space parameterizations favor control stability while task-space parameterizations favor generalization, revealing complementary strengths.

Conclusion: Proper action space design is critical for robotic policy learning, with delta actions being consistently beneficial and spatial representations offering trade-offs between stability and generalization that should be considered based on application requirements.
...
TL;DR: Embedding air channels into helicoid-based soft continuum robots enables distributed deformation sensing through integrated pressure sensors and IMUs, creating scalable sensorized architected materials for large-scale soft robotic systems.
...
Method: Developed a fabrication method for embedding air channels into helicoid-based soft continuum robots using multi-material segments fabricated via vision-controlled jetting, interfacing with PCBs housing miniature pressure sensors and IMUs for distributed deformation sensing.

Result: Characterized mechanical properties of four helicoid designs, validated sensor response to fundamental deformation modes, and demonstrated scalability with a meter-scale, 14-DoF cable-driven soft arm capable of open-loop trajectory tracking, object grasping, and tactile-based stiffness detection.

Conclusion: Establishes a scalable fabrication strategy for sensorized architected materials in large-scale soft robotic systems, enabling better sensing and control of highly deformable soft robots.
...
TL;DR: HJ-Patch enables real-time adaptation of Control Barrier Functions via localized Hamilton-Jacobi reachability updates for robotic safety in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Existing CBF design is challenging for complex systems, and Hamilton-Jacobi reachability methods are computationally expensive and typically offline, limiting applicability in dynamic environments.

Method: Introduce refineCBF for refining approximate CBFs via warm-started HJ reachability, then present HJ-Patch which accelerates this through localized updates with guaranteed safety recovery and monotonic improvements.

Result: Experiments validate real-time adaptation in simulation and on physical hardware (ground vehicles and quadcopters), successfully adapting to sudden environmental changes like new obstacles and unmodeled wind disturbances.

Conclusion: Provides a practical path toward deploying formally guaranteed safety in real-world settings by bridging offline synthesis and online adaptation of safety functions.
...
TL;DR: A new comprehensive autonomous driving dataset collected from CARLA Leaderboard 2.0 with 2.85M frames, supporting multiple tasks including planning, perception, prediction, and VLAMs, with rarity scoring for scenario analysis.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous driving datasets are incomplete - perception datasets lack planning data, planning datasets have limited behavioral diversity, and real datasets often lack proper closed-loop evaluation setups. CARLA Leaderboard 2.0 addresses long-tail problems but existing datasets have limited sensor configurations.

Method: Collected over 2.85 million frames using CARLA simulation environment for diverse Leaderboard 2.0 challenge scenarios, designed to support multiple tasks including planning, dynamic object detection, lane/centerline detection, traffic light recognition, prediction, and visual language action models.

Result: Created a comprehensive dataset that addresses limitations of existing datasets, demonstrated versatility by training various models, and provided numerical rarity scores to quantify how rarely specific states occur in the dataset.

Conclusion: The new dataset provides a valuable resource for end-to-end autonomous driving research, supporting multiple tasks simultaneously and addressing the limitations of existing datasets through comprehensive data collection from diverse CARLA Leaderboard 2.0 scenarios.
...
TL;DR: V-MORALS extends MORALS to estimate Regions of Attraction from image-based trajectories without requiring full state information, using learned latent spaces and Morse Graphs.


<details>
  <summary>Details</summary>
Motivation: Existing reachability analysis methods require known system dynamics, large datasets, full state information, and are computationally expensive. MORALS addressed some issues but still needed full state knowledge. V-MORALS aims to perform reachability analysis using only sensor measurements (images) without state information.

Method: V-MORALS takes image-based trajectories of a system under a controller, learns a latent space for reachability analysis, generates well-defined Morse Graphs from this latent space, and computes Regions of Attraction (ROAs) for various systems and controllers.

Result: V-MORALS provides capabilities similar to the original MORALS architecture without relying on state knowledge, using only high-level sensor data (images).

Conclusion: V-MORALS successfully extends reachability analysis to vision-based systems, enabling ROA estimation from sensor measurements without requiring full state information or known dynamics.
...
<details>
  <summary>Details</summary>
Motivation: Existing aerial manipulators are limited to manipulation only below UAVs, restricting deployability in multiple directions and through clutter, and are sensitive to propeller downwash.
...
Conclusion: Tilt-X addresses limitations of existing aerial manipulators by enabling multi-directional deployment and reducing downwash interference, providing critical evidence for designing reliable aerial manipulators.
...
TL;DR: Vision-Click-Action (VCA) replaces language commands with click-based visual interaction for robotic manipulation, reducing ambiguity and cognitive load.


<details>
  <summary>Details</summary>
Motivation: Language-based VLA models suffer from ambiguity, cognitive overhead, and difficulties in precise object identification, especially with visually similar objects.

Method: VCA framework replaces textual commands with direct click-based visual interaction using pretrained segmentation models, allowing operators to specify targets through visual selection in 2D camera view.

Result: Experimental results validate effective instance-level manipulation of specified target objects, with reduced interpretation errors and lower cognitive load.

Conclusion: VCA provides a practical and scalable alternative to language-driven interfaces for real-world robotic manipulation by leveraging direct visual interaction.
...
TL;DR: KEEP is a KV-cache-centric memory management system that accelerates embodied planning in LLMs by optimizing memory storage and access, achieving 2.68x speedup with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Existing memory-augmented LLMs for embodied planning store memory as raw text, causing long prompts and high prefill latency. While KV cache storage is possible, frequent updates undermine efficiency benefits.

Method: KEEP introduces three innovations: (1) Static-Dynamic Memory Construction using mixed-granularity memory groups, (2) Multi-hop Memory Re-computation for dynamic identification of important cross-attention, and (3) Layer-balanced Memory Loading to eliminate unbalanced KV cache loading.

Result: KEEP achieves 2.68x speedup with negligible accuracy loss compared to text-based memory methods on ALFRED dataset. Compared to CacheBlend, it shows 4.13% success rate improvement and 1.90x TTFT reduction.

Conclusion: KEEP provides an efficient KV-cache-centric memory management system for embodied planning that significantly improves performance while maintaining accuracy, addressing key limitations of existing approaches.
...
TL;DR: MicroPush is an open-source simulator and benchmark suite for magnetic rolling microrobots that enables reproducible evaluation of contact-rich micromanipulation tasks like cell pushing and multi-target assembly.


<details>
  <summary>Details</summary>
Motivation: Autonomy for contact-rich behaviors in magnetic rolling microrobots (such as cell pushing and multi-target assembly) is difficult to develop and evaluate reproducibly in confined microfluidic environments.

Method: Develops MicroPush simulator with overdamped interaction model, contact-aware stick-slip effects, near-field damping, optional Poiseuille flow, and calibrated actuation mapping. Provides modular planning-control stack with two-phase strategy for contact establishment and goal-directed pushing, plus deterministic benchmark protocol with fixed tasks and unified CSV logging.
...
Conclusion: MicroPush enables reproducible comparison and ablation of planning, control, and learning methods for microscale contact-rich micromanipulation, addressing the challenge of developing and evaluating autonomy for magnetic rolling microrobots.
...
TL;DR: FAVLA is a force-adaptive VLA model that decouples slow perception planning from fast contact-aware control using separate slow VLM and fast AE components with adaptive frequency scheduling based on predicted force variations.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models fuse all modalities at a single frequency, ignoring mismatched sensor sampling rates and forcing downsampling of high-frequency contact cues. This leads to delayed responses to contact events like impacts, stick-slip, and force spikes in contact-rich manipulation tasks.

Method: FAVLA uses a slow VLM at fixed low frequency for perception planning and force variation prediction, and a fast AE at variable high frequency for reactive control. It includes a force adapter to inject high-frequency force features into AE layers and adaptively schedules AE execution frequency based on VLM's predicted force variation.

Result: Extensive experiments on contact-rich tasks show FAVLA significantly outperforms baselines, achieving superior reactivity and success rates, especially with smaller contact forces during manipulation.

Conclusion: Decoupling slow perception planning from fast contact-aware control with adaptive frequency scheduling enables more reactive and successful contact-rich manipulation, addressing limitations of unified-frequency fusion approaches.
...
TL;DR: SpikingTac is a low-cost ($150) miniaturized neuromorphic tactile sensor with custom event camera, achieving 1000Hz perception, 350Hz tracking, exceptional stability, and sub-millimeter accuracy for dynamic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: High-speed event-driven tactile sensors are needed for human-like dynamic manipulation, but current event cameras are too bulky and limit integration into tactile sensing systems.

Method: Developed SpikingTac with custom standalone event camera module; used global dynamic state map with unsupervised denoising network for 1000Hz perception; proposed hysteresis-aware incremental update law with spatial gain damping to address silicone viscoelastic hysteresis.

Result: Achieved 100% return-to-origin success rate with 0.8039 pixel mean bias under extreme torsion; limited obstacle-avoidance overshoot to 6.2mm (5x better than frame-based sensors); achieved sub-mm accuracy with 0.0952mm localization RMSE and 0.0452mm radius RMSE.

Conclusion: SpikingTac demonstrates that miniaturized, low-cost neuromorphic tactile sensors can achieve high-speed, precise tracking with exceptional stability and accuracy, enabling advanced dynamic manipulation capabilities.
...
TL;DR: Hybrid Neural ODE framework embeds physical structure into learned model of antagonistic pneumatic artificial muscle dynamics for improved stiffness control and impedance behavior.


<details>
  <summary>Details</summary>
Motivation: Antagonistic pneumatic artificial muscles (PAMs) provide variable impedance through co-contraction but have coupled, nonlinear, and hysteretic dynamics that are challenging to model and control.

Method: Developed a hybrid neural ordinary differential equation (Neural ODE) framework that combines parametric joint mechanics and pneumatic state dynamics with a neural network force component to capture antagonistic coupling and rate-dependent hysteresis.

Result: Forward model predicts joint motion and chamber pressures with mean R² of 0.88 across 225 co-contraction conditions. Inverse formulation computes pressure commands for desired motion/stiffness profiles. Experimental validation shows reliable stiffness control (126-176 N/mm) and consistent impedance across velocities.

Conclusion: The hybrid Neural ODE framework successfully models antagonistic PAM dynamics and enables effective stiffness control, outperforming static models that degrade at higher velocities.
...
TL;DR: Multimodal gesture recognition framework using Apple Watch IMU data and capacitive gloves outperforms vision-based methods for hands-free robot teleoperation in hazardous environments.


<details>
  <summary>Details</summary>
Motivation: Human operators in hazardous environments need intuitive, hands-free teleoperation for mobile robots and UAVs. Vision-based gesture recognition suffers from occlusion, lighting variations, and cluttered backgrounds, limiting real-world applicability.

Method: Proposed multimodal framework integrating inertial data (accelerometer, gyroscope, orientation) from Apple Watches on both wrists with capacitive sensing from custom gloves. Uses late fusion strategy based on log-likelihood ratio (LLR) for enhanced performance and interpretability.

Result: Achieves performance comparable to state-of-the-art vision-based baseline while significantly reducing computational cost, model size, and training time. Framework is suitable for real-time robot control.

Conclusion: Sensor-based multimodal fusion offers robust, interpretable solution for gesture-driven mobile robot and drone teleoperation in hazardous environments.
...
<details>
  <summary>Details</summary>
Motivation: Indoor navigation systems are unreliable in campuses and small areas, forcing users to rely on confusing static signage or floor maps that are time-consuming and inefficient.

Method: Uses Vuforia Area Target for environment modeling, AI navigation's NavMesh component for navigation, and A* algorithm within NavMesh for shortest path calculation. Combines real-time processing with AR overlays for intuitive directions.

Result: A* algorithm reaches solutions 2-3 times faster than Dijkstra's for smaller search spaces. Experimental results show significantly improved navigation accuracy, better user experience, and greater efficiency compared to traditional methods like GPS and static signage.

Conclusion: AR technology integrated with existing pathfinding algorithms is feasible and scalable for indoor navigation, creating user-friendly solutions. However, NavMesh requires further optimization for large or highly dynamic environments despite being highly effective in limited, defined indoor spaces.
...
TL;DR: Train-free two-layer LLM architecture for UAV decision-making with safety verification and scene adaptation, validated in pursuit-evasion scenarios.
...
Method: Two-layer decision architecture: high-level safety planning with fuzzy Control Barrier Function verification for LLM outputs, plus low-level precise control. Uses star-hierarchical graph-based retrieval-augmented generation for scene adaptation.

Result: Maintains performance while significantly enhancing safety and generalization in pursuit-evasion scenarios with unknown obstacles and emergent threats, without requiring online training.

Conclusion: Framework demonstrates strong extensibility and potential for generalization to broader embodied intelligence systems and safety-critical control domains.
...
TL;DR: StemVLA enhances vision-language-action models by incorporating 3D future spatial forecasting and 4D historical spatiotemporal representations for better robot manipulation in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Current VLA models rely on direct 2D-to-action mappings without explicit 3D spatial structure or temporal dynamics modeling, limiting spatial reasoning and long-horizon decision-making in dynamic environments.

Method: StemVLA incorporates: 1) 3D future spatial-geometric forecasting to anticipate scene geometry, and 2) 4D historical spatiotemporal representations using a pretrained video-geometry transformer with temporal attention (VideoFormer) to capture motion dynamics.
...
Conclusion: Explicitly modeling both future-oriented 3D spatial knowledge and historical 4D spatiotemporal representations enables more comprehensive world understanding and better robot manipulation performance compared to traditional 2D-based VLA approaches.
...
TL;DR: Paper presents an acceleration-level outer-loop control framework that converts commanded tangential/normal accelerations into body-rate and normalized thrust commands for fixed-wing UAVs, enabling practical implementation of acceleration-commanded guidance laws like proportional navigation.
...
Method: Develops acceleration-level outer-loop control framework with two channels: 1) Normal channel: engineering mapping from desired normal acceleration to roll- and pitch-rate commands that regulate lift vector direction/magnitude under small-angle assumptions. 2) Tangential channel: energy-based formulation inspired by total energy control with empirical thrust-energy acceleration relationship derived from flight data, avoiding explicit propulsion modeling.
...
TL;DR: OmniTrack is a two-stage framework that decouples physical feasibility from motion tracking to enable humanoid robots to perform diverse, stable motions by first generating physically plausible trajectories then training policies to track them.


<details>
  <summary>Details</summary>
Motivation: Human-robot morphology/dynamics discrepancies and data noise create physically infeasible artifacts in reference motions (floating, penetration), causing conflicts between tracking accuracy and robot stability during training/execution.

Method: Two-stage approach: 1) Privileged generalist policy generates physically plausible motions via simulation trajectory rollout, 2) General control policy is trained to track these physically feasible motions for stable real-world transfer.

Result: Improved tracking accuracy and strong generalization to unseen motions; achieved hour-long consistent stable tracking in real-world tests including complex acrobatics (flips, cartwheels); supports human-style stable dynamic online teleoperation.

Conclusion: OmniTrack's explicit decoupling of physical feasibility from motion tracking enables robust, generalizable humanoid control that maintains stability while accurately tracking diverse motions, including complex acrobatics and teleoperation.
...
TL;DR: OmniXtreme breaks the fidelity-scalability trade-off in humanoid motion tracking by decoupling general motor skill learning from physical skill refinement.


<details>
  <summary>Details</summary>
Motivation: Current motion tracking policies face a "generality barrier" where tracking fidelity collapses as motion libraries scale in diversity, especially for real-world deployment of high-dynamic motions.

Method: Uses a flow-matching policy with high-capacity architectures to scale representation capacity without interference-intensive multi-motion RL optimization, followed by an actuation-aware refinement phase for robust physical hardware performance.

Result: Maintains high-fidelity tracking across diverse, high-difficulty datasets and successfully executes multiple extreme motions on real robots.

Conclusion: Effectively breaks the long-standing fidelity-scalability trade-off in high-dynamic humanoid control through scalable framework design.
...
Method: Combines physics-consistent modeling and hybrid reinforcement learning: (1) develops first-principles digital twin of da Vinci Xi grasping mechanism, (2) three-stage pipeline: receding-horizon CMA-ES oracle for expert trajectories, fully offline policy learning via Implicit Q-Learning, and online refinement using TD3 for adaptation.

Result: In simulation: maintains grasp force within 1% of desired reference during multi-harmonic jaw motion. Hardware experiments: average force errors below 4% across diverse trajectories. Learned policy has ~71k parameters and executes at kHz rates for real-time deployment.
...
TL;DR: TSC is a decentralized multi-agent RL framework for autonomous driving that uses braid-inspired priority graphs to define local leader-follower dependencies, improving coordination stability and safety in dense traffic without communication.


<details>
  <summary>Details</summary>
Motivation: Existing MARL approaches for autonomous driving either use synchronous decision-making (which worsens non-stationarity) or centralized sequencing (which doesn't scale with traffic density), leading to unstable behaviors like oscillatory yielding or unsafe commitments in dense traffic scenarios.

Method: TSC extracts time-varying directed priority graphs from braid-inspired weaving relations between trajectories, then factorizes dense interactions into graph-local Stackelberg subgames. Under CTDE, it learns sequential coordination policies where leaders are anticipated via action prediction and followers are trained through action-conditioned value learning to approximate local best responses.
...
Conclusion: TSC provides a scalable, communication-free solution for decentralized interactive driving in dense traffic by leveraging topology-conditioned Stackelberg coordination to improve training stability and safety without requiring global ordering.
...
TL;DR: ABPolicy: An asynchronous flow-matching policy using B-spline control points to achieve smooth robotic manipulation by eliminating jitter and discontinuities while maintaining real-time responsiveness.
...
Method: ABPolicy uses asynchronous flow-matching in B-spline control-point action space: 1) B-spline ensures intra-chunk smoothness, 2) bidirectional action prediction with refitting optimization enforces inter-chunk continuity, 3) asynchronous inference enables real-time continuous updates.
...
Conclusion: ABPolicy effectively addresses smoothness and responsiveness challenges in robotic manipulation through asynchronous B-spline-based flow-matching with bidirectional prediction and refitting optimization.
...
TL;DR: Researchers developed an omni-directional dual-arm mobile robot with VR tele-operation for retail automation, enabling shared human-robot control to handle diverse retail items.


<details>
  <summary>Details</summary>
Motivation: Autonomous retail robots struggle to adapt to dynamic retail environments and novel situations, creating a need for more flexible robotic systems that can handle diverse products and scenarios.

Method: Developed an omni-directional dual-arm mobile robot with heterogeneous grippers, plus a VR-based tele-operation system for shared control where human operator commands are captured via motion capture and transmitted to the remote robot.

Result: The system successfully manipulated various common retail items using both single and dual-arm coordinated manipulation techniques in a mockup retail environment.

Conclusion: The proposed tele-operated dual-arm robot system effectively addresses the adaptability challenges of autonomous retail robots by combining human intelligence with robotic capabilities for handling diverse retail items.
...
TL;DR: Autonomous robotic assembly framework uses RL with successor features to build stable structures without predefined blueprints, adapting to environmental uncertainty and construction noise.


<details>
  <summary>Details</summary>
Motivation: Traditional robotic construction relies on fixed architectural blueprints, limiting adaptability to environmental uncertainty and variations during building. The paper aims to develop a more flexible approach that can handle real-world construction challenges.

Method: Uses reinforcement learning (deep Q-learning with successor features) as decision-making component. Construction tasks are defined through targets and obstacles rather than fixed plans. Framework tested on 15 2D robotic assembly tasks with discrete block construction using real-world closed-loop robotic setup.

Result: Experiments demonstrate feasibility of the method and its ability to handle construction noise. The framework successfully adapts to environmental uncertainty and variations during building process.

Conclusion: The framework offers a promising direction for more adaptable and robust robotic construction in real-world environments, moving beyond rigid blueprint-based approaches to flexible, target-oriented assembly.
...
TL;DR: Proposes YE-KG, a large-scale multimodal spatiotemporal knowledge graph from real-world indoor videos, and STE-VLN framework that integrates event-centric knowledge via hierarchical retrieval to enhance VLN agents' long-horizon reasoning with coarse-grained instructions.


<details>
  <summary>Details</summary>
Motivation: VLN agents struggle with long-horizon reasoning in unseen environments, especially with ambiguous, coarse-grained instructions. Existing knowledge graph approaches don't fully leverage multimodal event knowledge inspired by human episodic memory.

Method: 1) Construct YE-KG - first large-scale multimodal spatiotemporal KG with 86k nodes/83k edges from indoor videos using MLLMs (LLaVa, GPT4) to extract semantic-action-effect events. 2) STE-VLN framework with Coarse-to-Fine Hierarchical Retrieval mechanism to integrate KG into VLN models, retrieving causal event sequences and fusing with visual observations.
...
Conclusion: Event-centric knowledge enhancement strategy effectively addresses coarse-grained instruction and long-horizon reasoning challenges in VLN by leveraging multimodal event knowledge through structured knowledge graphs and hierarchical retrieval mechanisms.
...
TL;DR: A novel framework for learning robust control policies to enable miniature blimp robots to achieve and maintain inverted poses through simulation training and sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: Inverted poses are essential for maximizing miniature blimp robot agility, but developing reliable control methods is challenging due to their complex, underactuated dynamics.

Method: Three-stage framework: 1) Build calibrated high-fidelity 3D simulation, 2) Train robust policy using domain randomization and modified TD3 algorithm, 3) Design mapping layer for sim-to-real deployment.

Result: Learned policy outperforms energy-shaping controller in simulation with higher success rate, and enables real-world MBRs to achieve and maintain fully upside-down poses with mapping layer.

Conclusion: The proposed framework successfully enables robust inverted pose control for miniature blimp robots through simulation-based learning and effective sim-to-real transfer.
...
TL;DR: Autonomous UAV inspects transmission towers without prior maps using camera-LiDAR fusion for insulator detection and localization, achieving 24% time savings in simulation and sub-meter accuracy in real-world tests.


<details>
  <summary>Details</summary>
Motivation: Transmission tower inspection typically requires prior maps or multiple flights, which is time-consuming and inefficient. The paper aims to enable autonomous UAV inspection in a single flight without needing pre-existing tower maps.

Method: Uses camera-LiDAR sensor fusion with CNN-based insulator detection, projects LiDAR points onto images, filters using bounding boxes, and employs DBSCAN, RANSAC, and PCA algorithms for insulator localization in an online inspection pipeline.

Result: Simulation shows 24% time savings compared to two-flight strategies. Real-world experiments achieve mean localization errors of 0.16±0.08m horizontally and 0.16±0.11m vertically, with significantly lower variance than existing approaches.

Conclusion: The proposed online inspection algorithm with camera-LiDAR fusion enables efficient autonomous transmission tower inspection without prior maps, demonstrating practical accuracy and time savings for real-world applications.
...
TL;DR: Vision-based curriculum RL framework for drone racing with obstacles achieves faster lap times and higher success rates than existing methods.
...
Method: Combines multi-stage curriculum learning, domain randomization, and multi-scene updating strategy in a vision-based reinforcement learning framework with end-to-end single network control policy.
...
Conclusion: The proposed framework effectively advances drone racing in obstacle-rich environments by addressing the conflicting challenges of obstacle avoidance and gate traversal.
...
TL;DR: Geometry-based pneumatic actuators (GPAs) use configurable CNC heat-sealed chambers to achieve predictable deformation, near-zero bending radii, and multi-state actuation for soft robotics applications.


<details>
  <summary>Details</summary>
Motivation: Soft pneumatic actuators have limitations in complex actuation patterns including minimum bending radii, multi-state capabilities, and structural stability, despite their advantages in safe human-machine interaction.

Method: Introduces geometry-based pneumatic actuators (GPAs) with constraint layers featuring configurable CNC heat-sealed chambers, enabling predictable deformation and complex actuated geometries through mathematical modeling of linear angle transformations and nonlinear torque-angle relationships.

Result: Demonstrated three applications: 49g wrist exoskeleton reducing muscle activity by 51%, 30.8g haptic interface delivering 8N force feedback with fast response, and 208g bipedal robot achieving multi-gait locomotion.
...
TL;DR: A planning-based IRL algorithm enables robots to learn manipulation tasks from visual observations alone in under an hour, without rewards, demonstrations, or prior data.


<details>
  <summary>Details</summary>
Motivation: Observational learning in robotics typically requires hand-designed rewards or demonstrator actions, which are impractical. The paper aims to enable robots to learn tasks from scratch using only observations and interaction, without these restrictive assumptions.

Method: Proposes a planning-based Inverse Reinforcement Learning (IRL) algorithm that learns world models from observation and interaction alone. The approach doesn't require rewards, demonstrator actions, prior knowledge, or pre-training data.

Result: The method successfully learns image-based manipulation tasks from scratch in under an hour in real-world experiments. The learned world model enables online transfer learning and achieves significantly higher sample efficiency and success rates compared to IRL, RL, and Behavior Cloning approaches.

Conclusion: The proposed approach provides a practical path for online world modeling and planning from observation and interaction, overcoming limitations of existing methods that require more restrictive assumptions about rewards, demonstrations, or prior data.
...
TL;DR: VLA policies show strong benchmark performance but may rely on object-location correlations rather than true language grounding; controlled tests reveal manipulation skill is decoupled from instruction following.
...
Method: Conducted controlled multi-object picking study with progressive workspace randomization, evaluating held-out object-location pairings to break familiar associations without increasing spatial difficulty. Tested representative VLA policies (SmolVLA and π₀.₅) across stress tests and data scaling.

Result: Manipulation primitive execution remained substantially more reliable than instruction-conditioned task success in harder regimes, showing that manipulation skill acquisition is decoupled from instruction following.
...
TL;DR: First demonstration of humanoid robot assisting in surgical procedure - teleoperated Unitree G1 provided endoscopic visualization for cadaveric sphenoidectomy.


<details>
  <summary>Details</summary>
Motivation: Current claims about humanoid surgical capabilities lack empirical evidence; no humanoid has actually assisted in surgery. Need to establish whether humanoid form factor can meet physical demands of surgical assistance.

Method: Teleoperated Unitree G1 humanoid robot provided endoscopic visualization while attending otolaryngologist performed cadaveric sphenoidectomy. Cognitive demands handled by human operator.

Result: Procedure completed successfully with stable visualization maintained throughout. Demonstrated form-factor feasibility for surgical assistance while identifying engineering targets for clinical translation.

Conclusion: Establishes proof-of-concept for humanoid surgical assistance, identifies challenges for development, and suggests near-term opportunities like autonomous diagnostic scoping.
...
TL;DR: Two-stage MRIO framework for robust radar-inertial odometry in GPS-denied subterranean environments, addressing IMU bias drift and sparse radar data while maintaining resilience to smoke/dust.


<details>
  <summary>Details</summary>
Motivation: Reliable odometry in subterranean environments is challenging due to IMU bias drift from extreme temperatures/gravity, sparse noisy radar returns, and LiDAR failure under smoke/dust. FMCW radars offer compact, cost-effective, robust alternatives but need better fusion with IMUs.

Method: Two-stage MRIO framework: 1) IMU bias estimator using radar ego-velocity via least-squares in EKF for online bias correction, 2) Fuse corrected IMU accelerations with heterogeneous radar measurements for odometry refinement. Supports radar-only mapping using estimated displacements.

Result: Outperforms EKF-RIO in subterranean field trials, maintains accuracy across cost-efficient FMCW radar setups and different IMUs (Pixhawk to VectorNav), provides robust localization and mapping in smoke-affected GPS-denied environments.

Conclusion: MRIO enables reliable radar-inertial odometry in challenging subterranean conditions, addressing IMU bias drift and sparse radar data while being resilient to smoke/dust. Framework supports multiple radar/IMU configurations and will be open-sourced.
...
TL;DR: Distributed IMUs enable vine robot shape sensing with ~8-16% tip position error, but drift and active steering reduce accuracy.


<details>
  <summary>Details</summary>
Motivation: Vine robots need full-body shape sensing for sensor localization and environmental mapping in search/rescue, but existing methods lack systematic evaluation under realistic conditions.

Method: Experimental evaluation of distributed IMU-based shape sensing on vine robots, quantifying drift rates, tip position errors for passive/active steering, length variations, and sensor spacing effects.

Result: Average orientation drift: 1.33°/min; tip error: 11% (passive), 16% (active), 8% across lengths; intermediate sensor spacing minimizes error for single-curvature shapes.

Conclusion: Distributed IMU shape sensing is feasible for vine robots but requires improved modeling to address drift and active steering limitations for field deployment.
...
TL;DR: SafeGen-LLM is a safety-generalizable LLM for robotic task planning that ensures safety guarantees and generalizes to novel safety properties across domains through two-stage post-training with formal verification guidance.


<details>
  <summary>Details</summary>
Motivation: Classical planners have poor scalability, RL methods generalize poorly, and base LLMs cannot guarantee safety for safety-critical robotic task planning. There's a need for models that can both ensure safety and generalize to novel safety constraints.

Method: Two-stage post-training: 1) Supervised Fine-Tuning on constraint-compliant planning dataset to learn planning syntax/semantics, 2) Group Relative Policy Optimization guided by fine-grained reward machines from formal verification for safety alignment, plus curriculum learning for complex tasks.
...
Conclusion: SafeGen-LLM addresses the safety generalization gap in robotic task planning by combining LLM capabilities with formal verification guidance, providing a promising approach for safety-critical applications.
