...
TL;DR: Proposes a dual-path framework combining video synthesis (for diverse training data) and semantic graph neural networks (for accident anticipation), achieving improved accuracy and lead time in traffic accident prediction.


<details>
  <summary>Details</summary>
Motivation: Traffic accident anticipation is critical but challenging due to complex road-user interactions and limited diverse datasets.

Method: (1) Video synthesis pipeline using structured prompts to generate high-fidelity driving scenes matching real data distributions. (2) Graph neural network with semantic cues for spatial-semantic reasoning. (3) New benchmark dataset with diverse regions, weather, and traffic conditions.

Result: Significant gains in both prediction accuracy and anticipation lead time across existing datasets and the new benchmark.

Conclusion: The framework effectively mitigates data bottlenecks and enhances autonomous driving reliability for accident anticipation.
...
TL;DR: Simply rendering two views per optimizer step (instead of one) closes the performance gap in hybrid-capture 3D Gaussian Splatting, outperforming more complex methods like GradNorm, gradient surgery, and active pairing—a finding explained by a variance-decomposition framework showing between-regime variance is negligible relative to within-regime variance.


<details>
  <summary>Details</summary>
Motivation: Standard 3DGS under-fits the minority camera regime (e.g., ground-level vs. aerial) by 1-3 dB in hybrid-capture settings; the paper aims to isolate the simplest training-side lever that fixes this imbalance.

Method: Systematic comparison of compute-matched alternatives (60K iterations, GradNorm, gradient surgery, projective preconditioning, confidence gating, random two-view vs. structured pairing) on five hybrid-capture benchmarks; proposes a variance-decomposition framework to explain why two-view accumulation dominates.

Result: Rendering two views per optimizer step is the key lever. Pairing rule (near/far, random, active) does not matter beyond seed variance. Two-view accumulation halving variance dominates; direction-aware, magnitude, and gating methods all fall within seed variance of random two-view pairing. Transfers to Scaffold-GS and Pixel-GS backbones.

Conclusion: An honest characterization that the simplest structural change—two views per step—is the only training-side axis that moves PSNR for hybrid-capture 3DGS, supported by a variance-decomposition explanation.
...
TL;DR: AIDA (SF-MIDA) adaptively generates intermediate domains for person re-identification under multi-source and source-free conditions, using uncertainty-guided mixing and pseudo-mirror regularization to improve domain generalization.


<details>
  <summary>Details</summary>
Motivation: Existing intermediate domain methods (IDM, IDM++) rely on fixed mixing and joint source-target access, limiting their use in multi-source and source-free settings.

Method: Proposes AIDA (SF-MIDA) with: (1) a multi-source intermediate domain generator that dynamically synthesizes intermediate representations, (2) adaptive control of feature mixing and regularization via uncertainty and training stability signals, and (3) pseudo-mirror regularization to preserve identity consistency under domain perturbations.

Result: Extensive experiments across domain generalization and source-free settings demonstrate effectiveness.

Conclusion: AIDA effectively overcomes the limitations of fixed mixing strategies, enabling robust person Re-ID in unseen environments under multi-source and source-free constraints.
...
TL;DR: GAFSV-Net converts online signature temporal sequences into 2D Gramian Angular Field images, enabling pretrained 2D vision backbones (ConvNeXt-Tiny) with cross-attention fusion and triplet loss for state-of-the-art online signature verification.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning OSV methods operate directly on raw 1D temporal sequences, restricting them to 1D architectures and preventing the use of powerful pretrained 2D vision backbones.

Method: GAFSV-Net encodes three kinematic channels (pen speed, pressure derivative, direction angle) into six-channel asymmetric GASF+GADF images, processed by dual-branch ConvNeXt-Tiny with bidirectional cross-attention, trained with semi-hard triplet loss and skilled-forgery hard-negative injection.

Result: Outperforms all sequence-based baselines on DeepSignDB and BiosecurID under identical training objectives; representational gain is consistent and independent of training procedure.

Conclusion: 2D temporal encoding via GAF provides consistent OSV performance gains over 1D sequence methods, and the advantage is independent of the training procedure used.
...
TL;DR: This paper presents a methodology for designing SNN-based object detection on Intel Loihi 2, achieving real-time detection with the lowest per-inference dynamic energy and power consumption compared to Jetson platforms and Apple M2 CPU. Their ANN-to-SNN distillation training recovers 87-100% of ANN accuracy while maintaining low latency.


<details>
  <summary>Details</summary>
Motivation: Real-time object detection on energy-constrained platforms (UAVs, mobile robots) needs energy-efficient solutions. SNNs on neuromorphic hardware are promising but lack comprehensive design methodologies for deployment on modern neuromorphic processors like Intel Loihi 2.

Method: The authors propose a general SNN detection architecture methodology with engineering adaptations for Intel Loihi 2. They use ANN-to-SNN distillation-aware training to recover accuracy. Experiments use both frame-based and event-based datasets, benchmarking against ANN-based detection on NVIDIA Jetson Orin Nano, Jetson Nano B01, and Apple M2 CPU.

Result: SNNs on Loihi 2 achieve the lowest per-inference dynamic energy and power consumption among all platforms. ANNs on Jetson Orin Nano achieve higher inference rates. Distillation-aware training recovers 87-100% of ANN accuracy; without distillation, accuracy drops 11-27%.

Conclusion: Neuromorphic systems with SNNs on Loihi 2 are highly promising for energy-efficient, real-time object detection at the edge, offering major power advantages over conventional GPU/CPU platforms.
...
TL;DR: A pipeline for neural implicit surface reconstruction of non-cooperative space objects from monocular inspection imagery, demonstrated on real ISS and H-IIA rocket footage, with key techniques including segmentation-based background removal and photometric correction.


<details>
  <summary>Details</summary>
Motivation: Existing neural implicit surface reconstruction methods are limited to synthetic or lab data with known camera poses and controlled lighting; real on-orbit inspection of non-cooperative space objects remains unexplored.

Method: Proposes a pipeline using segmentation-based background removal for camera pose estimation and photometric correction of per-frame exposure variations, demonstrated on real ISS STS-119 and H-IIA rocket upper stage inspection footage.

Result: Background removal was essential for pose estimation from real footage. Photometric correction behavior varied across datasets, with performance in shadowed regions depending on illumination characteristics.

Conclusion: Neural implicit reconstruction is feasible for real on-orbit inspection data, but requires robust background handling and exposure correction tailored to lighting conditions.
...
TL;DR: VkSplat is the first fully Vulkan-based 3D Gaussian Splatting training pipeline, achieving 3.3× speedup and 33% VRAM reduction over CUDA+PyTorch baselines while maintaining quality and cross-vendor GPU compatibility.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS training pipelines are limited by CUDA+PyTorch's performance bottlenecks and poor cross-vendor GPU compatibility, motivating the need for a more efficient and vendor-agnostic implementation.

Method: Fully implemented in Vulkan compute with various optimizations for the 3DGS training pipeline.

Result: 3.3× speed improvement and 33% VRAM reduction compared to CUDA+PyTorch baseline, with maintained quality and demonstrated cross-vendor compatibility.

Conclusion: VkSplat achieves state-of-the-art performance as the first fully Vulkan-based 3DGS training pipeline, proving Vulkan's viability for high-performance ML training workloads.
...
TL;DR: Standard conformal prediction for egocentric pose estimation has a ~30% conditional coverage gap for hard frames; DINOv2-Bridge adaptive CP closes this gap using a participant-transferable difficulty estimator, improving hardest-quartile coverage from ~0.75 to ~0.93.


<details>
  <summary>Details</summary>
Motivation: Egocentric pose estimation for AR/assistive devices needs guaranteed uncertainty regions, but standard conformal prediction fails to cover hard frames equitably, creating large conditional coverage gaps.

Method: Proposes DINOv2-Bridge adaptive CP: a two-stage difficulty estimator trained on one source participant that transfers cross-participant without test-time images, using geodesic SE(3) nonconformity scores.

Result: Standard CP covers only ~60% of hardest 25% of frames (vs 90% overall); geodesic scoring better identifies hard frames than Euclidean; DINOv2-Bridge adaptive CP raises Q4 coverage from ~0.75 to ~0.93.

Conclusion: Adaptive conformal prediction can close the conditional coverage gap in egocentric pose estimation without retraining or test-time images, enabling reliable uncertainty guarantees for AR/assistive applications.
...
TL;DR: MAEPose uses masked autoencoding directly on mmWave spectrogram videos for privacy-preserving human pose estimation, outperforming baselines by up to 22.1% in MPJPE.


<details>
  <summary>Details</summary>
Motivation: Existing mmWave radar pose estimation methods discard rich spatiotemporal information by using pre-extracted intermediate representations (e.g., point clouds, spectrograms) and rely on fully supervised learning without leveraging unlabeled data.

Method: MAEPose applies masked autoencoding directly on raw mmWave spectrogram videos to learn spatiotemporal motion-aware representations from unlabeled data, then uses a heatmap decoder for multi-frame pose estimation.

Result: Outperforms SOTA baselines by up to 22.1% in MPJPE (p<0.05), maintains robust accuracy under zero-shot bystander interference (only 6.5% error increase), and Range-Doppler video input works best.

Conclusion: Masked autoencoding on raw radar video is effective for learning generalized representations, with both pre-training and heatmap decoder being crucial contributors to performance.
...
TL;DR: Remote SAMsing pipeline boosts SAM2's coverage on remote sensing images from 30-68% to 91-98% via multi-pass painting and tile-boundary merging, without retraining.


<details>
  <summary>Details</summary>
Motivation: SAM2 struggles with remote sensing: (1) quality-coverage trade-off in mask generation, and (2) object fragmentation when tiling large images.

Method: Multi-pass algorithm: repeat SAM2 on each tile, paint accepted masks black between passes, relax thresholds only when gains stagnate. Contextual padding + best-match merge to handle tile boundaries.

Result: Coverage 91-98% vs 30-68%; buildings 95% Det@0.5, cars 82-93%; 3-8x more precise boundaries than SLIC/Felzenszwalb; 1.94B pixel Potsdam at 97% coverage.

Conclusion: An open-source, training-free pipeline that makes SAM2 viable for large-scale remote sensing segmentation with high coverage and quality.
...
TL;DR: REALM is a cross-modal framework that aligns event camera data with RGB foundation models using LoRA, enabling zero-shot transfer of image-trained decoders (like MASt3R) to event streams and achieving SOTA in tasks like depth estimation, semantic segmentation, and feature matching.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based event processing approaches are confined to narrow, task-specific silos and lack the ability to generalize across modalities, unlike RGB foundation models.

Method: REALM projects event representations into the pretrained latent space of RGB foundation models (ViT-based) using low-rank adaptation (LoRA) to bridge the modality gap while keeping RGB backbones frozen.

Result: SOTA performance in wide-baseline feature matching, outperforming specialized architectures; enables zero-shot application of image-trained decoders like MASt3R to raw event data.

Conclusion: REALM effectively maps events into ViT-based foundation latent space, allowing downstream tasks (depth estimation, semantic segmentation) via simple linear heads transferred from RGB teacher, with zero-shot capability for complex decoders.
...
TL;DR: This paper investigates why text-to-image diffusion models fail in multi-object generation, finding that scene complexity (not concept imbalance) is the dominant factor, with counting being particularly difficult in low-data regimes, and compositional generalization degrading when concept combinations are held out during training.


<details>
  <summary>Details</summary>
Motivation: To understand the root causes of multi-object generation failures in text-to-image diffusion models by disentangling data-related factors (concept imbalance vs. compositional generalization).

Method: Introduced MOSAIC (Multi-Object Spatial relations, AttrIbution, Counting), a controlled dataset generation framework. Trained diffusion models on MOSAIC across two regimes: concept generalization (imbalanced data) and compositional generalization (held-out concept combinations).

Result: Scene complexity dominates over concept imbalance; counting is uniquely hard in low-data regimes; compositional generalization collapses when concept combinations are held out.

Conclusion: Highlights fundamental limitations of diffusion models, motivating stronger inductive biases and improved data design for robust multi-object compositional generation.
...
TL;DR: Proposes a multi-scale attention-based model with joint reasoning for explainable driving decisions in autonomous vehicles, evaluated on BDD-OIA and nu-AR datasets using F1-score and a new Joint F1 metric.


<details>
  <summary>Details</summary>
Motivation: Deep learning models in autonomous driving are black-box and lack explainability, making it hard to recognize efficiency, predict failures, or deploy reliably. Existing XAI methods suffer from flawed reasoning and unreliable metrics.

Method: A multi-scale attention-based model where driving decisions are fed into a reasoning component to generate case-specific explanations for each decision. A new Joint F1 score metric is proposed for XAI evaluation.

Result: The proposed reasoning network outperforms classic and state-of-the-art models on both BDD-OIA and nu-AR datasets, demonstrating superior generalization and robustness.

Conclusion: The multi-scale attention model with joint reasoning provides more accurate and reliable explanations for autonomous driving decisions, advancing XAI in this domain.
...
TL;DR: This paper demonstrates that Vision Transformers (ViTs) outperform Multi-Temporal CNNs for vegetation classification in phenological monitoring, achieving an order-of-magnitude reduction in FLOPs with constant parameter complexity regardless of time series length, while maintaining competitive accuracy on Brazilian Cerrado biome datasets.


<details>
  <summary>Details</summary>
Motivation: Plant phenology monitoring via UAVs and near-surface cameras is crucial for understanding ecosystem responses to climate change, but existing CNN-based approaches have poor scalability: they rely on rigid multi-branch architectures that scale linearly with time series length and require large spatial context windows, making them computationally inefficient for real-world deployment.

Method: The authors conduct a comprehensive ablation study on Vision Transformers (ViTs) for spatio-temporal vegetation pixel classification, analyzing seven design dimensions: data normalization, spectral arrangement, boundary handling, spatial context window shape/size, tokenization strategies, positional encoding, and feature aggregation. Evaluated on two Brazilian Cerrado datasets (Serra do Cipó aerial, Itirapina near-surface).

Result: ViT reduces FLOPs by an order of magnitude compared to CNN baselines, maintains constant parameter complexity regardless of time series length (vs. CNN's linear scaling), while achieving competitive classification accuracy on both datasets.

Conclusion: Vision Transformers are a robust, scalable solution for resource-constrained phenological monitoring systems, offering substantially better computational efficiency than CNNs without sacrificing classification performance.
...
TL;DR: GeoSR-Bench is a new benchmark dataset that evaluates super-resolution models for satellite imagery based on their performance on downstream Earth monitoring tasks (like land cover classification) rather than traditional image quality metrics like PSNR/SSIM, revealing that improvements in standard SR metrics often don't correlate—and can even negatively correlate—with actual task performance.


<details>
  <summary>Details</summary>
Motivation: Existing super-resolution benchmarks evaluate models using fidelity metrics (PSNR, SSIM) that don't capture the true utility of SR images for real-world applications like land cover classification, biomass estimation, and change detection in Earth observation.

Method: Created GeoSR-Bench with ~36,000 spatially co-located, temporally aligned, quality-controlled image pairs from 500m to 0.6m resolution across diverse land covers. Benchmarked GAN, transformer, neural operator, and diffusion-based SR models across 270 settings (2 cross-platform SR tasks, 9 SR models, 3 downstream task models, 5 downstream tasks).

Result: Improvements in traditional SR metrics (PSNR/SSIM) often do not correlate with gains in downstream task performance, and correlations can be negative, indicating these metrics provide limited guidance for selecting models for real applications.

Conclusion: There is a need to integrate downstream tasks into SR model development and evaluation, as fidelity metrics alone are insufficient for assessing practical utility in Earth monitoring applications.
...
TL;DR: OSCAR uses online self-calibration via Monte Carlo Tree Search and dual-granularity rewards to reduce hallucinations in LVLMs without relying on stronger teacher models.


<details>
  <summary>Details</summary>
Motivation: Existing preference alignment methods suffer from Supervision-Perception Mismatch where student models are forced to align with details beyond their perceptual capacity, leading to guessing rather than seeing.

Method: OSCAR leverages the Generative-Discriminative Gap within LVLMs, integrating Monte Carlo Tree Search with a Dual-Granularity Reward Mechanism to construct preference data, then iteratively refines via Direct Preference Optimization.

Result: Achieves state-of-the-art performance on hallucination benchmarks while improving general multimodal capabilities.

Conclusion: Online self-supervision is more reliable than offline distillation from stronger models for reducing hallucinations in LVLMs.
...
TL;DR: PAD is an end-to-end diffusion framework that generates pose-aligned 3D objects directly in observation space by unprojecting monocular depth into partial point clouds, avoiding canonical-then-rotate paradigms and achieving superior geometric alignment.
...
Method: PAD uses an end-to-end diffusion framework that unprojects monocular depth into a partial point cloud as a 3D geometric anchor, generating 3D geometry directly in observation space without canonical assumptions.

Result: PAD achieves superior geometric alignment and image-to-3D correspondence compared to SOTA methods, and extends naturally to compositional 3D scene reconstruction while preserving precise spatial layouts.

Conclusion: By enforcing rigorous spatial supervision through native generation in observation space, PAD resolves pose ambiguity and produces high-fidelity pose-aligned 3D assets.
...
TL;DR: CURE-OOD is the first benchmark for evaluating out-of-distribution detection in cancer survival prediction using CT images, showing that covariate shifts reduce performance and standard OOD detectors may fail in this context.


<details>
  <summary>Details</summary>
Motivation: Patients need accurate survival predictions after cancer diagnosis, but variations in CT imaging acquisition cause distribution shifts that undermine model reliability. No existing benchmark systematically studies OOD detection in cancer survival prediction.

Method: The authors introduce CURE-OOD, a benchmark with scanner-parameter-based training, in-distribution (ID), and OOD test splits across four survival prediction tasks. They also include HazardDev as a survival-aware reference baseline for OOD detection.

Result: Covariate shifts notably reduce survival prediction performance. Mainstream classification-oriented OOD detectors can fail in survival prediction tasks.

Conclusion: CURE-OOD enables systematic analysis of how distribution shifts affect both downstream survival performance and OOD detectability, highlighting the need for survival-specific OOD detection methods.
...
TL;DR: The paper introduces TCMP, a lightweight Temporal Convolutional Network for motion prediction in multi-object tracking that outperforms complex generative models while using only 1.4% of the parameters and 5% of the computational cost of prior SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Current MOT methods struggle with real-world non-linear motion and increasingly rely on complex, computationally expensive generative models that limit practical utility.

Method: TCMP uses a modified Temporal Convolutional Network (TCN) with dilated convolutions and a regression head for efficient motion prediction across arbitrary temporal context lengths.

Result: Achieves SOTA results: HOTA 63.4% (+1.1%), IDF1 65.0% (+2.0%), AssA 49.1% (+1.9%). Only 0.014x parameters and 0.05x FLOPs compared to prior SOTA.

Conclusion: A simple, efficient TCN-based approach can outperform complex generative models for MOT motion prediction, demonstrating that complexity is unnecessary for state-of-the-art performance.
...
TL;DR: A flow matching model achieves 4x super-resolution of Sentinel-2 imagery (10m→2.5m) across the CONUS, outperforming diffusion and GAN methods in pixel accuracy, and produced a 1.58 trillion pixel CONUS product and a 2.5m land cover product with 89.11% accuracy.


<details>
  <summary>Details</summary>
Motivation: To overcome the perception-distortion trade-off in satellite imagery super-resolution, enabling high-resolution geospatial products without relying on diffusion or GAN-based methods.

Method: Flow matching model trained on 120,851 same-day Sentinel-2 (10m) and resampled NAIP (2.5m) pairs; evaluated with Euler (1-step) and Midpoint (20-step) solvers; applied to 2025 Sentinel-2 composites for CONUS-wide super-resolution and land cover classification.

Result: Outperformed diffusion and Real-ESRGAN in pixel-wise accuracy; achieved perceptual realism in 20 steps with Midpoint solver; produced a 1.58 trillion pixel CONUS product; land cover accuracy 89.11% over Chesapeake Bay watershed (2020-2025).

Conclusion: Flow matching is an effective generative approach for Sentinel-2 super-resolution, offering improved spectral fidelity and perceptual quality trade-off at inference time, with strong potential for expanding access to high-resolution geospatial data.
...
TL;DR: RTPrune proposes a two-stage token pruning method for DeepSeek-OCR that leverages a two-stage reading trajectory (prioritizing high-norm tokens then merging remaining tokens via optimal transport), achieving 99.47% accuracy and 1.23× faster prefill with 84.25% token retention.


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR's visual tokens contain redundant textual/structural information, and existing token pruning methods for VLMs fail to preserve textual fidelity due to improper compression mechanisms.

Method: A two-stage token pruning method: (1) prioritize high-norm visual tokens capturing salient textual/structural information; (2) pair and merge remaining tokens using optimal transport theory. Also introduces a dynamic pruning ratio adapting to token similarity and textual density.

Result: 99.47% accuracy and 1.23× faster prefill on OmniDocBench with 84.25% token retention on DeepSeek-OCR-Large, achieving state-of-the-art performance.

Conclusion: RTPrune effectively reduces visual token redundancy in DeepSeek-OCR while preserving textual fidelity, achieving better efficiency-accuracy trade-offs through insight-driven two-stage pruning with dynamic ratios.
...
TL;DR: SIMON proposes a saliency-aware multi-view framework for zero-shot EEG-to-image retrieval that uses foreground segmentation and saliency prediction to generate foveated views, achieving state-of-the-art performance on THINGS-EEG (Top-1 accuracy 69.7% intra-subject, 19.6% inter-subject).


<details>
  <summary>Details</summary>
Motivation: Existing EEG-to-image retrieval methods assume a fixed center-focused view, creating a geometric-semantic dissociation between visual features and EEG responses due to center bias conflicting with content-driven human attention.
...
Result: Achieves state-of-the-art Top-1 accuracy of 69.7% (intra-subject) and 19.6% (inter-subject) on THINGS-EEG, consistently outperforming recent baselines. Robustness confirmed across sampling granularity, EEG channel topology, and backbone variations.

Conclusion: Saliency-aware multi-view integration effectively resolves the center bias issue in EEG-to-image retrieval, and the method is robust across various settings.
...
TL;DR: BOLT is a lightweight plug-and-play module that enables preparation-free heterogeneous cooperative perception by using ego-as-teacher distillation to adapt neighboring features online, achieving significant performance gains without any pre-deployment coordination.


<details>
  <summary>Details</summary>
Motivation: Existing heterogeneous cooperative perception methods require impractical prior preparation (offline joint training or model adaptation), which is unrealistic for agents independently trained by different developers that meet online.

Method: BOLT is a lightweight (0.9M parameters) plug-and-play module that performs online cross-agent feature alignment via ego-as-teacher distillation, using high-confidence ego predictions (no ground-truth needed) to guide adaptation while letting neighbors contribute in low-confidence ego regions.

Result: BOLT improves AP@50 by up to 32.3 points over vanilla unadapted fusion in preparation-free setting, consistently outperforms ego-only results on DAIR-V2X and OPV2V across different encoder pairs and fusion strategies.

Conclusion: Preparation-free heterogeneous cooperative perception is feasible with lightweight online adaptation, eliminating the need for pre-deployment coordination while significantly boosting performance over ego-only perception.
...
TL;DR: This paper investigates how temporal resolution (120 Hz, 60 Hz, 30 Hz) affects zero-shot semantic understanding of high-speed human actions using kendo as a case study, finding that higher frame rates significantly improve semantic separability in a training-free pipeline combining video-language models with LLM reasoning.


<details>
  <summary>Details</summary>
Motivation: Understanding human actions from visual observations is essential for human-robot interaction, especially for rapid and uncommon actions where collecting labeled data is challenging, making zero-shot approaches necessary.

Method: Training-free pipeline combining a pre-trained video-language model for semantic representation with LLM-based reasoning for pairwise action comparison, tested across multiple frame rates (120/60/30 Hz).

Result: Higher temporal resolution significantly improves semantic separability in zero-shot settings; high-speed video provides more stable and interpretable semantic representations for fast actions.

Conclusion: Temporal resolution is critical for training-free action recognition, and high-speed perception can enhance semantic understanding capabilities.
...
TL;DR: LeGS replaces heuristic density control in 3DGS with a learnable policy network optimized via RL, using a closed-form reward function that reduces complexity from O(N²) to O(N), achieving SOTA results on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods rely on heuristic density control rules that lack flexibility and fail to adapt to diverse scenes with complex geometries.

Method: Reformulates density control as a parameterized policy network optimized via Reinforcement Learning, with a sensitivity-analysis-based reward function reduced to O(N) complexity via closed-form solution.

Result: Outperforms SOTA methods on Mip-NeRF 360, Tanks & Temples, and Deep Blending datasets, achieving better reconstruction quality and efficiency.

Conclusion: Learning-based density control via RL is a viable and superior alternative to heuristic rules in 3DGS.
...
TL;DR: This paper explores integrating V2X communication (object-level information) into 3D object detection for automated driving, using nuScenes dataset with simulated noise and dropout. A BEVFusion-style detector fuses V2X messages as BEV input, showing significant performance gains (NDS 0.80) but fragility under realistic imperfections. Noise-aware training and confidence encoding improve robustness against V2X degradation.


<details>
  <summary>Details</summary>
Motivation: Onboard perception sensors (cameras, radar) are limited by line-of-sight and field-of-view constraints, causing failures under occlusions or poor visibility. The authors aim to study how V2X cooperative awareness can complement onboard perception and how robust such integration is to realistic V2X imperfections.

Method: Using nuScenes dataset, emulated object-level V2X messages from ground truth with controlled noise and dropout (simulating latency, localization errors, low penetration). Converted messages to dedicated BEV input and fused into a BEVFusion-style detector. Proposed noise-aware training with explicit confidence encoding.

Result: Object-level V2X substantially improves detection (NDS 0.80 under favorable conditions). Models trained on idealized data become fragile and over-reliant on V2X. Noise-aware training with confidence encoding maintains performance gains even under severe noise and reduced V2X penetration.

Conclusion: V2X cooperative information can significantly enhance 3D detection, but robustness requires accounting for realistic imperfections. Noise-aware training strategy is effective against V2X degradation.
...
TL;DR: LIMSSR is a framework that uses LLMs for incomplete multimodal learning without requiring complete training data, outperforming SOTA on action quality assessment.


<details>
  <summary>Details</summary>
Motivation: Existing IML methods unrealistically assume full-modal availability during training. This paper tackles the more realistic setting where modalities are missing even during training.

Method: Reformulates IML as conditional sequence reasoning via LLMs, using Prompt-Guided Context-Aware Modality Imputation, Multidimensional Representation Fusion, and Mask-Aware Dual-Path Aggregation to mitigate hallucinations.

Result: Significantly outperforms state-of-the-art baselines on three Action Quality Assessment datasets without relying on complete training data.

Conclusion: Establishes a new paradigm for data-efficient multimodal learning by eliminating the need for complete modality training data.
...
TL;DR: Proposes MACF, a multi-agent framework for long-video understanding that decouples per-agent perception budgets from global video complexity, using latent communication and curriculum training to outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: MLLMs have limited perception budgets for long-video tasks, and existing agentic methods suffer from information loss, high cost, and reliance on text intermediates.

Method: MACF partitions videos into segments for locally budgeted agents with a latent communication protocol; agents encode observations into compact tokens in a shared embedding space for efficient collaboration via a central coordinator, trained with a curriculum strategy.

Result: Consistently outperforms state-of-the-art MLLMs and multi-agent systems under identical budget constraints across diverse video understanding benchmarks.

Conclusion: Latent collaboration (MACF) is effective for scalable, end-to-end video understanding while preserving visual fidelity and reducing information loss.
...
TL;DR: CT-Lite uses JPEG-compressed CT volumes for thoracic abnormality detection, achieving near-baseline performance (5-7% AUROC gap) with significantly fewer parameters via FAST distillation and SFP projection.


<details>
  <summary>Details</summary>
Motivation: High computational cost of processing uncompressed volumetric CT data limits AI deployment in medical imaging; need for low-resource, efficient solutions.

Method: Proposes FAST (Feature Attention Style Transfer) distillation framework with Gram-matrix attention style preservation and dual-attention feature alignment, plus SFP (Structured Factorized Projection) using Block Tensor Train decomposition for parameter efficiency.

Result: CT-Lite achieves AUROC within 5-7% of uncompressed-input baseline on CT-RATE, NIDCH, and Rad-ChestCT datasets with significantly fewer parameters.

Conclusion: JPEG-compressed CT volumes can be effectively used for AI-based clinical diagnosis under resource constraints with minimal performance degradation.
...
TL;DR: This paper introduces an iERF-centric framework that unifies local, global, and mechanistic interpretability by using pointwise feature vectors and instance-specific Effective Receptive Fields, proposing SRD for saliency maps, CAFE for concept grounding, and ICAT for interlayer concept attribution.


<details>
  <summary>Details</summary>
Motivation: Modern vision models achieve high accuracy but lack a unified framework to explain where evidence arises, what the model encodes, and how internal computations assemble that evidence.

Method: Three components: (1) SRD (Sharing Ratio Decomposition) for local saliency maps via iERF propagation; (2) CAFE (Concept-Anchored Feature Explanation) for global concept grounding using iERFs as semantic labels; (3) ICAT (Interlayer Concept Attribution) with Integrated Gradients for quantifying concept-to-concept influence across layers.

Result: Outperforms baselines in fidelity and robustness across ResNet50, VGG16, and ViTs; successfully interprets dispersed SAE features; exposes dominant concept routes in correct, misclassified, and adversarial cases.

Conclusion: The iERF-grounded approach provides a coherent, evidence-backed map from pixels to concepts to decisions, unifying local, global, and mechanistic interpretability.
...
TL;DR: The paper proposes an active learning framework that reduces annotation cost by combining fine-grained human labels with coarse-grained VLM-generated weak labels via instance-wise label assignment, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To reduce human annotation cost in active learning by leveraging vision-language models (VLMs), which provide reliable coarse-grained but unreliable fine-grained labels.

Method: A framework combining fine-grained human annotations with coarse-grained VLM-generated weak labels through instance-wise label assignment, with systematic noise modeling using trusted full labels.

Result: Outperforms existing active learning methods on CUB200 and FGVC-Aircraft under the same annotation budget.

Conclusion: VLMs' varying reliability across label granularity can be effectively exploited to reduce human annotation cost in active learning for fine-grained tasks.
...
TL;DR: Introduces four jailbreak attacks exploiting visual modality of VLMs, demonstrating that text-based safety alignment fails to generalize to visually conveyed harmful intent.


<details>
  <summary>Details</summary>
Motivation: The visual modality of VLMs is an underexplored attack surface for bypassing safety alignment.

Method: Four visual jailbreak attacks: (1) visual symbol sequences with decoding legend, (2) harmful object substitution with benign terms, (3) benign text replacement in images preserving harmful visual context, (4) visual analogy puzzles inferring prohibited concepts.

Result: Visual attacks bypass safety alignment; e.g., visual cipher achieves 40.9% ASR vs 10.7% for textual cipher on Claude-Haiku-4.5.
...
TL;DR: BlenderRAG is a retrieval-augmented generation system that improves LLM-based Blender code generation by retrieving semantically similar examples from a curated multimodal dataset, boosting compilation success from 40.8% to 70.0% and CLIP similarity from 0.41 to 0.77 without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: LLMs produce frequent syntactic errors and geometrically inconsistent objects when generating Blender code from natural language, making automatic generation challenging.

Method: A retrieval-augmented generation (RAG) system operating on a curated multimodal dataset of 500 expert-validated examples (text, code, image) across 50 object categories, retrieving semantically similar examples during generation.

Result: Improves compilation success rates from 40.8% to 70.0% and semantic normalized alignment from 0.41 to 0.77 (CLIP similarity) across four state-of-the-art LLMs.

Conclusion: The approach improves performance without fine-tuning or specialized hardware, making it immediately accessible for deployment.
...
TL;DR: GOR-IS is a 3D object removal framework that decomposes scenes into intrinsic components to achieve physically consistent inpainting with proper global lighting and non-Lambertian surface handling, outperforming prior work by 13% LPIPS and 2dB PSNR.


<details>
  <summary>Details</summary>
Motivation: Existing 3D object removal methods neglect global lighting effects and struggle with view-dependent non-Lambertian surfaces, leading to physically implausible and visually inconsistent inpainting results.

Method: The framework decomposes scenes into intrinsic components (material & lighting), explicitly models light transport to maintain global lighting consistency, and uses an intrinsic-space inpainting module that operates directly in these domains to handle non-Lambertian surfaces.

Result: Outperforms existing methods by 13% in perceptual similarity (LPIPS) and 2dB in PSNR on synthetic and real-world datasets.

Conclusion: GOR-IS substantially improves physical consistency and visual coherence of 3D object removal, with code publicly available.
...
TL;DR: A training-free 3D inpainting method that optimizes initial noise in structured 3D latent diffusion to improve contextual consistency and prompt alignment.


<details>
  <summary>Details</summary>
Motivation: In structured 3D latent diffusion, the geometric structure is established early and is highly sensitive to initial noise, causing instability in inpainting/editing where strict context alignment is needed.

Method: Optimizes initial noise via backpropagation approximation using rectified flow model, with spectral parameterization for robust structured 3D latent optimization.

Result: Consistent improvements in contextual consistency and prompt alignment over training-free inpainting baselines.

Conclusion: Initial noise control is an independent, orthogonal dimension for 3D inpainting beyond conventional sampling trajectory manipulation.
...
### [36] [End-to-End Autoregressive Image Generation with 1D Semantic Tokenizer](https://arxiv.org/abs/2605.00503)
*Wenda Chu,Bingliang Zhang,Jiaqi Han,Yizhuo Li,Linjie Yang,Yisong Yue,Qiushan Guo*

Main category: cs.CV

TL;DR: This paper proposes an end-to-end training pipeline for autoregressive image modeling that jointly optimizes visual tokenizers and generative models, achieving state-of-the-art FID score of 1.48 on ImageNet 256x256.


<details>
  <summary>Details</summary>
Motivation: Traditional two-stage approaches train visual tokenizers and generative models separately, lacking direct feedback from generation quality to the tokenizer. This work aims to bridge this gap by enabling end-to-end optimization.

Method: An end-to-end training pipeline that jointly optimizes reconstruction and generation, allowing direct supervision from generation results to the tokenizer. Also explores using vision foundation models to improve 1D tokenizers.

Result: State-of-the-art FID score of 1.48 without guidance on ImageNet 256x256 generation.

Conclusion: Jointly optimizing tokenizer and generator in an end-to-end manner yields better generative performance than separate two-stage training, achieving new SOTA results.
...
TL;DR: LightKV reduces KV cache size in LVLMs by exploiting redundancy among vision tokens, compressing them via cross-modality message passing guided by text prompts, achieving 55% token retention while halving cache size and reducing computation by 40%.


<details>
  <summary>Details</summary>
Motivation: KV cache adoption in LVLMs causes substantial GPU memory overhead due to large numbers of vision tokens during prefill. Existing vision-only compression strategies lack prompt-aware guidance.

Method: LightKV employs cross-modality message passing guided by text prompts to aggregate informative messages across vision tokens and progressively compress them during prefill.

Result: With 55% of original vision tokens, LightKV halves KV cache size, reduces computation by up to 40%, preserves general performance, and outperforms baselines on 8 LVLMs across 8 benchmarks.

Conclusion: Prompt-aware, cross-modality compression effectively reduces KV cache memory overhead in LVLMs without sacrificing performance.
...
### [38] [PhysiGen: Integrating Collision-Aware Physical Constraints for High-Fidelity Human-Human Interaction Generation](https://arxiv.org/abs/2605.00517)
*Nan Lei,Yuan-Ming Li,Ling-An Zeng,Liang Xu,Zhi-Wei Xia,Hui-Wen Huang,Fa-Ting Hong,Wei-Shi Zheng*

Main category: cs.CV

TL;DR: PhysiGen is a plug-and-play optimization strategy that uses simplified geometric primitives and collision region guidance to reduce body inter-penetration in text-driven 3D human motion synthesis for multi-person interactions.


<details>
  <summary>Details</summary>
Motivation: Body inter-penetration in multi-person interaction sequences undermines realism and usability; previous methods either ignored this or used computationally expensive mesh-level loss functions.

Method: Simplifies high-resolution human body mesh into geometric primitives for efficient collision detection, identifies collision regions as optimization direction guidance, and is designed as a plug-and-play module.

Result: Cross-dataset and cross-model experiments show PhysiGen effectively reduces interpenetration and significantly improves visual coherence and physical plausibility over SOTA.

Conclusion: PhysiGen offers a general-purpose, computationally efficient solution for enforcing collision-aware physical constraints in human-human interaction generation.
...
TL;DR: The paper identifies "Visual Signal Dilution" in LVLMs where visual attention decays with longer text, and proposes Persistent Visual Memory (PVM)—a lightweight module integrated alongside FFN that creates a distance-agnostic retrieval pathway for sustained visual perception, improving performance across 4B and 8B Qwen3-VL models.


<details>
  <summary>Details</summary>
Motivation: To address the "Visual Signal Dilution" phenomenon in autoregressive Large Vision-Language Models (LVLMs), where visual attention decays as generated text sequences grow longer, impairing performance on tasks requiring sustained visual perception.

Method: Proposes Persistent Visual Memory (PVM), a lightweight learnable module integrated as a parallel branch alongside the Feed-Forward Network (FFN) in LVLMs. It establishes a distance-agnostic retrieval pathway that directly supplies visual embeddings, structurally preventing signal suppression during deep generation.

Result: Experiments on Qwen3-VL models (4B and 8B scales) show consistent accuracy gains with negligible parameter overhead, especially on complex reasoning tasks. PVM resists length-induced signal decay and accelerates internal prediction convergence.

Conclusion: PVM effectively mitigates visual signal dilution in LVLMs, offering a parameter-efficient solution for maintaining persistent visual perception across long generation sequences.
...
### [40] [IdentiFace: Multi-Modal Iterative Diffusion Framework for Identifiable Suspect Face Generation in Crime Investigations](https://arxiv.org/abs/2605.00526)
*Weichen Liu,Yixin Yang,Changsheng Chen,Alex Kot*

Main category: cs.CV

TL;DR: IdentiFace is a diffusion-based framework for generating identifiable suspect faces using multi-modal inputs and iterative refinement, outperforming existing methods in identity retrieval.


<details>
  <summary>Details</summary>
Motivation: Traditional sketch-drawing workflows suffer from low efficiency and quality, while existing diffusion models face conditional ambiguity and sampling variance issues, making suspect face generation challenging in crime investigations.

Method: (1) Multi-modal input design for stronger conditional control; (2) Iterative generation pipeline for adjustable facial features; (3) A facial identity loss function; (4) Two task-specific datasets.

Result: IdentiFace achieves superior performance over existing methods, especially in identity retrieval, validated on both synthetic datasets and real-world scenarios.

Conclusion: IdentiFace shows strong potential for practical applications in crime investigations, effectively addressing key limitations of prior approaches.
...
### [41] [Vesselpose: Vessel Graph Reconstruction from Learned Voxel-wise Direction Vectors in 3D Vascular Images](https://arxiv.org/abs/2605.00538)
...
TL;DR: A direction-vector-guided TEASAR algorithm jointly predicting vessel direction vectors and segmentation masks achieves state-of-the-art topological accuracy in 3D vascular graph reconstruction.


<details>
  <summary>Details</summary>
Motivation: The prevailing segment-then-fix paradigm for blood vessel segmentation is fundamentally limited in modeling complete and topologically accurate vascular network reconstruction.

Method: 1) Jointly predicts voxel-wise vessel direction vectors and segmentation masks; 2) Introduces direction-vector-guided extension of the TEASAR algorithm for graph extraction; 3) Proposes false splits and false merges as topological error measures.

Result: Achieves state-of-the-art performance on three benchmark datasets (synthetic and real imagery) and demonstrates applicability on 3D micro-CT scans of rat heart vasculature.

Conclusion: The approach substantially improves topological accuracy, effectively separates closely apposed vessel segments, and handles multiple vascular trees within a single volume.
...
### [42] [Colorful-Noise: Training-Free Low-Frequency Noise Manipulation for Color-Based Conditional Image Generation](https://arxiv.org/abs/2605.00548)
*Nadav Z. Cohen,Ofir Abramovich,Ariel Shamir*

Main category: cs.CV

TL;DR: This paper shows that low-frequency noise components primarily determine global structure and color in diffusion models, while high-frequency components control fine details. Simple manipulations of low-frequency noise can steer image generation without training.


<details>
  <summary>Details</summary>
Motivation: White Gaussian noise in diffusion models limits control over specific visual attributes because it is not human-interpretable. The authors aim to understand noise characteristics and leverage them for controllable generation.

Method: The authors analyze frequency characteristics of input noise in diffusion models, showing low frequencies control global structure/color and high frequencies control fine details. They propose a training-free method that manipulates low-frequency noise using image priors.

Result: Low-frequency noise manipulations can effectively condition generation to reconstruct low-frequency visual cues, steering overall image structure and color while preserving variability in fine details.

Conclusion: A simple, training-free method with minimal overhead can control global image attributes by modifying low-frequency noise components, while high-frequency components maintain output diversity.
...
### [43] [Depth-Guided Privacy-Preserving Visual Localization Using 3D Sphere Clouds](https://arxiv.org/abs/2605.00562)
*Heejoon Moon,Jongwoo Lee,Jeonggon Kim,Je Hyeong Hong*

Main category: cs.CV

TL;DR: The paper introduces "sphere cloud," a privacy-preserving 3D scene representation that lifts map points to lines through the centroid, blocking density-based attacks on point cloud geometry, while using ToF depth maps to resolve localization scale issues.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks can recover high-fidelity scene details from sparse 3D point clouds, raising privacy concerns in visual localization with private maps. Existing line-based privacy methods are vulnerable to density-based attacks that recover point cloud geometry.

Method: Lifts all points to 3D lines crossing the map centroid (sphere cloud), misleading density-based attacks to yield points at the centroid. A cloud construction strategy thwarts new image-recovery attacks, and an efficient localization framework uses on-device ToF depth maps to resolve translation scale ambiguity.

Result: Sphere cloud achieves competitive privacy-preserving ability and localization runtime on public RGB-D datasets, without excessively compromising pose estimation accuracy compared to other depth-guided localization methods.

Conclusion: Sphere cloud effectively neutralizes density-based attacks on line-based privacy representations, though it introduces new challenges (image recovery attacks, translation scale) that are addressed via tailored construction and ToF-guided localization.
...
### [44] [2D-SuGaR: Surface-Aware Gaussian Splatting for Geometrically Accurate Mesh Reconstruction](https://arxiv.org/abs/2605.00569)
*Prajwal Gupta C. R.,Divyam Sheth,Jinjoo Ha,Mirela Ostrek,Justus Thies*

Main category: cs.CV

TL;DR: This work improves 2D Gaussian Splatting by adding monocular depth and normal priors, a depth-guided initialization, and clustering-based pruning to achieve better surface reconstruction quality and robustness.


<details>
  <summary>Details</summary>
Motivation: 3DGS lacks accurate surface geometry, and 2DGS is sensitive to SfM initialization, leading to poor results on challenging image sets.

Method: Incorporates monocular depth and normal priors, proposes depth-guided initialization for Gaussians, and uses clustering-based pruning to remove degenerate Gaussians.

Result: Achieves state-of-the-art mesh reconstruction results on the DTU dataset while maintaining high-quality novel view synthesis.

Conclusion: The proposed enhancements improve geometric accuracy and robustness over 2DGS, especially when SfM initializations are poor.
...
### [45] [Federated Distillation for Whole Slide Image via Gaussian-Mixture Feature Alignment and Curriculum Integration](https://arxiv.org/abs/2605.00578)
*Luru Jing,Cong Cong,Yanyuan Chen,Yongzhi Cao*

Main category: cs.CV

TL;DR: FedHD is a federated learning framework for digital pathology that uses local Gaussian-mixture feature alignment and one-to-one synthetic distillation to handle heterogeneous MIL architectures across institutions, outperforming existing methods on cancer datasets.


<details>
  <summary>Details</summary>
Motivation: Real-world federated learning in digital pathology faces heterogeneity from diverse MIL architectures and feature extractors across institutions, which existing FL methods struggle to handle.

Method: FedHD performs local Gaussian-mixture feature alignment, uses one-to-one distillation to generate synthetic slide representations, applies curriculum-based integration of cross-site features, and includes an optional pseudo-patch reconstruction module for interpretability.

Result: Consistently outperforms state-of-the-art federated and distillation baselines on TCGA-IDH, CAMELYON16, and CAMELYON17 datasets.

Conclusion: FedHD is architecture-agnostic, privacy-preserving, and supports personalized collaborative training across heterogeneous institutions for WSI analysis.
...
### [46] [Intrinsic Gradient Suppression for Label-Noise Prompt Tuning in Vision-Language Models](https://arxiv.org/abs/2605.00591)
*Jiayu Li,Jiaxin Qi,Sheng Zhou,Jiaqiang Huang,Xiansheng Hua*

Main category: cs.CV

TL;DR: DSPT (Double-Softmax Prompt Tuning) is a hyperparameter-free method that uses sequential probabilistic normalization to suppress gradients from noisy samples, achieving state-of-the-art robustness for CLIP prompt tuning under label noise.


<details>
  <summary>Details</summary>
Motivation: Prompt tuning for CLIP is highly sensitive to label noise because mislabeled samples produce large gradients that overwhelm pre-trained priors, yet CLIP already provides near-optimal initialization requiring conservative adaptation.

Method: Double-Softmax Prompt Tuning (DSPT) applies sequential probabilistic normalization to create a self-adaptive saturation zone that suppresses gradients from high-error noisy samples while preserving informative updates—turning "gradient vanishing" into a noise-filtering shield.

Result: DSPT achieves state-of-the-art robustness across various noisy benchmarks, outperforming complex methods with handcrafted hyperparameters, despite being a simple drop-in design.

Conclusion: The proposed hyperparameter-free gradient suppression mechanism provides a principled solution for label-noise robustness in prompt tuning, transforming a traditionally problematic training behavior into a beneficial noise-filtering tool.
...
### [47] [Faithful Extreme Image Rescaling with Learnable Reversible Transformation and Semantic Priors](https://arxiv.org/abs/2605.00605)
*Hao Wei,Yanhui Zhou,Chenyang Ge,Saeed Anwar,Ajmal Mian*

Main category: cs.CV

TL;DR: FaithEIR is a diffusion-based framework for extreme image rescaling (16×+) that uses learnable reversible transformations (inspired by SVD), adaptive detail priors (high-frequency dictionary), and lightweight semantic conditioning to outperform state-of-the-art methods in both fidelity and perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Existing extreme rescaling methods fail to preserve semantically consistent structures and produce realistic details under scaling factors of 16× or higher due to the severely ill-posed nature of the task.

Method: (1) Learnable reversible transformation inspired by SVD for invertible downscaling/upscaling in latent space. (2) Adaptive detail prior—a high-frequency dictionary capturing empirical average of common structures from training data. (3) Lightweight pixel semantic embedder providing semantic conditioning for a pretrained diffusion model.

Result: FaithEIR consistently outperforms state-of-the-art methods, achieving superior reconstruction fidelity and perceptual quality on extreme rescaling benchmarks.

Conclusion: The proposed components (SVD-inspired reversible transform, adaptive detail prior, semantic conditioning) effectively address information loss and structure preservation in extreme rescaling, setting a new SOTA.
...
### [48] [CMTA: Leveraging Cross-Modal Temporal Artifacts for Generalizable AI-Generated Video Detection](https://arxiv.org/abs/2605.00630)
*Hang Wang,Chao Shen,Chenhao Lin,Minghui Yang,Lei Zhang,Cong Wang*

Main category: cs.CV

TL;DR: This paper identifies a new fingerprint called Cross-Modal Temporal Artifact (CMTA) in AI-generated videos, characterized by unnaturally stable semantic trajectories in cross-modal alignment. The authors propose a detection framework using BLIP and CLIP for visual-textual embeddings, with coarse-grained (GRU) and fine-grained (Transformer) temporal modeling branches, achieving SOTA across 40 subsets of four datasets.


<details>
  <summary>Details</summary>
Motivation: Existing AIGV detection methods overlook cross-modal temporal artifacts between visual and textual spaces, relying only on uni-modal or spatiotemporal cues.

Method: The CMTA framework uses BLIP for frame captions and CLIP for visual-textual embeddings, then employs a coarse-grained GRU branch for temporal fluctuation modeling and a fine-grained Transformer branch for inter-frame variations.

Result: Achieves state-of-the-art performance on 40 subsets across GenVideo, EvalCrafter, VideoPhy, and VidProM, with superior cross-generator generalization.

Conclusion: The CMTA framework effectively detects AI-generated videos by exploiting cross-modal temporal artifacts, outperforming existing methods and demonstrating strong generalization.
...
### [49] [UniVidX: A Unified Multimodal Framework for Versatile Video Generation via Diffusion Priors](https://arxiv.org/abs/2605.00658)
...
TL;DR: UniVidX is a unified video diffusion framework that handles multiple multimodal generation tasks (e.g., RGB↔intrinsic maps, RGB↔alpha layers) via stochastic condition masking, per-modality LoRAs, and cross-modal self-attention, achieving SOTA performance with fewer than 1,000 training videos.


<details>
  <summary>Details</summary>
Motivation: Existing video diffusion models for multimodal tasks require separate models for each task, fixing input-output mappings and limiting cross-modal correlation learning. UniVidX aims to unify these tasks into a single framework.

Method: Three key designs: (1) Stochastic Condition Masking (SCM) – random partition of modalities into conditions/targets for omni-directional generation; (2) Decoupled Gated LoRA (DGL) – per-modality LoRAs activated only when that modality is the target, preserving VDM priors; (3) Cross-Modal Self-Attention (CMSA) – shared K/V across modalities with modality-specific Q for inter-modal alignment.

Result: Competitive performance with SOTA across distinct tasks (intrinsic maps, RGBA layers), robust generalization to in-the-wild scenarios, even when trained on fewer than 1,000 videos.

Conclusion: UniVidX demonstrates that a single unified VDM framework can effectively handle diverse multimodal video generation tasks without task-specific training, opening possibilities for more flexible and efficient multimodal video processing.
...
TL;DR: Deep learning models can predict Alzheimer's risk factors (like age, smoking, blood pressure) from retinal fundus photos, and these retinal signatures differ in people who later develop AD, suggesting the retina may reflect preclinical AD vulnerability.


<details>
  <summary>Details</summary>
Motivation: To determine whether colored fundus photography contains retinal structural signatures corresponding to AD-related risk factors, and whether such signatures can be captured by deep learning models to assess AD vulnerability pathways.

Method: Using UK Biobank CFPs (62,876 images from 44,501 participants), DL models were trained to predict 12 AD-related risk factors (6 categorical, 6 continuous). Model performance, saliency maps, CAM-Scores, and comparisons with retinal morphometry and incident-AD cases (8.55 years before onset) were analyzed.

Result: DL achieved AUROC=0.5654-0.9480 for categorical and R²=-0.0291-0.7620 for continuous factors, outperforming morphometry-ML models. Saliency highlighted optic nerve head and retinal vasculature. Several saliency scores differed significantly between incident-AD cases and matched controls.

Conclusion: CFP encodes retinal signatures linked to AD risk factors; DL-derived retinal representations may uncover biologically meaningful risk-related structural changes that mirror potential AD vulnerability, though not yet diagnostic.
...
TL;DR: Proposes DMDSC, a dynamic margin approach for Open-Set Recognition in imbalanced medical imaging datasets, outperforming state-of-the-art methods on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Extreme class imbalance in medical imaging creates challenges for Open-Set Recognition; existing methods like DSC and UCDSC use a uniform margin that does not account for varying class densities.

Method: DMDSC framework with a dynamic margin approach that adapts class-specific margins based on label frequency, enforcing higher penalties and tighter feature clustering for rare pathologies.

Result: Outperforms state-of-the-art methods on BloodMNIST, OCTMNIST, DermaMNIST, and BreaKHis datasets.

Conclusion: Dynamic margin adaptation effectively counters data imbalance in medical OSR, improving both known class classification and unknown sample rejection.
...
TL;DR: ViTCG, a Vision Transformer with Channel-wise Grouping, achieves 62% MSE reduction over foundation models like Prithvi for aerosol optical depth retrieval from hyperspectral imagery by jointly modeling spatial and spectral information.


<details>
  <summary>Details</summary>
Motivation: Conventional physics-based AOD retrieval methods rely on pixel-wise inversion, memory-intensive LUTs, and auxiliary data, while existing data-driven approaches fail to exploit spatial-spectral coherence of hyperspectral imagery, leading to spatially inconsistent and noise-sensitive retrievals.

Method: ViTCG (Vision Transformer with Channel-wise Grouping) uses hyperspectral TOA radiance as input and jointly models spatial context and spectral information via a spatial regression framework.

Result: 62% reduction in mean squared error compared to state-of-the-art foundation models (including Prithvi) on PACE radiance observations, with spatially coherent AOD fields.

Conclusion: First study exploring Foundation AI models for AOD retrieval; ViTCG significantly outperforms existing methods in both accuracy and spatial consistency.
...
TL;DR: SDGAN jointly uses static and dynamic visual features with query-aware graph modeling and multi-granularity training to improve temporal video grounding.


<details>
  <summary>Details</summary>
Motivation: Existing GCN-based TVG methods suffer from: (1) using only static or dynamic features alone, (2) query-agnostic graph construction, and (3) single-granularity semantic matching leading to slow convergence and suboptimal precision.

Method: SDGAN proposes: (1) Static & Dynamic Graph Alignment with Position-wise Nodes Alignment for complementary visual representation, (2) Query-Clip Contrastive Learning & Adaptive Graph Modeling for query-aware visual representations, and (3) multi-granularity temporal proposals with Progressive Easy-to-Hard Training Strategy bridging coarse and fine-grained localization.

Result: State-of-the-art performance on three benchmark datasets for complex TVG scenarios.

Conclusion: SDGAN effectively addresses key bottlenecks in GCN-based TVG by integrating complementary visual features, query-aware graph modeling, and progressive multi-granularity training.
...
TL;DR: PhysEdit introduces two adaptive inference-time modules—CARD (for adaptive reasoning depth) and SRM (for spatial reasoning masking)—that adjust computational effort per instruction, achieving 1.18x speedup with slight quality improvements over fixed-recipe baselines.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning-based image editors apply a single fixed inference recipe to all instructions, ignoring that different edits (e.g., color swap vs. object insertion) require varying spatial coverage and reasoning depth.

Method: Two modules: (1) CARD predicts edit complexity from instruction+image to dynamically allocate reasoning steps and token length; (2) SRM uses cross-attention to confine reasoning to semantically relevant spatial regions. Both compose without retraining the backbone.

Result: 1.18x wall-clock speedup (64.3s vs. 76.1s per sample), +0.7% CLIP-T (0.2283 vs. 0.2266), matching CLIP-I (0.8246 vs. 0.8280). Up to 1.52x speedup on appearance-level edits.

Conclusion: Adaptive allocation along spatial and temporal axes is the missing degree of freedom in reasoning-based editing; CARD and SRM effectively trade efficiency for negligible quality loss, with category-dependent gains.
...
TL;DR: This paper investigates whether hierarchical supervision (coarse OA presence + fine-grained KL grading) using a simple dual-head model can improve knee osteoarthritis assessment over single-task approaches, finding backbone-dependent gains in KL metrics, more ordered latent representations, and better anatomical alignment.


<details>
  <summary>Details</summary>
Motivation: Knee OA assessment has a natural label hierarchy (binary OA + KL severity grades) that is often ignored; existing methods treat these as separate problems, missing potential representation-level supervisory benefits.

Method: Simple dual-head model with shared encoder and two task-specific heads; compared single-OA, single-KL, and dual-head training across multiple 3D backbones under same protocol with paired stats, latent geometry analysis, and saliency mapping.

Result: Dual-head supervision yields backbone-dependent gains, improving KL metrics for selected backbones, with more ordered coarse-to-fine latent organization and stronger cartilage saliency alignment.
...
TL;DR: RGSUD introduces a reward-guided self-reinforcement mechanism for unsupervised image deraining, using dynamic IQA-based reward recycling and self-reinforcement training to improve optimization and achieve SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Unsupervised deraining suffers from weak constraints due to lack of paired supervision, making network convergence difficult amid complex rain diversity. High-quality deraining results occasionally emerge during training but are not effectively leveraged.

Method: Two-stage framework: (1) Reward recycling with IQA-based dynamic reward mechanism to select and collect optimal derained outputs; (2) Self-reinforcement training incorporating these rewards into optimization via IQA-based self-reinforced loss to constrain the optimization space and improve deraining quality.

Result: SOTA performance across paired synthetic, paired real, and unpaired real datasets, outperforming existing unsupervised methods in both subjective and objective IQA metrics.

Conclusion: The self-reinforcement strategy is adaptable to other unsupervised deraining methods, and the framework demonstrates strong generalization across existing supervised deraining networks.
...
TL;DR: GSACP proposes an end-to-end single-point supervised IRSTD method that uses in-batch point-anchored feature-affinity propagation to generate pseudo-masks online, eliminating external label-evolution loops. It identifies and mitigates Self-Referential Propagation Drift through decoupling strategies, achieving competitive mIoU with 38% fewer false positives.


<details>
  <summary>Details</summary>
Motivation: To drastically reduce annotation costs for IRSTD while maintaining high performance, by replacing explicit offline pseudo-label construction (active learning, physics-driven mask generation) with a minimalist end-to-end online alternative.

Method: GSACP generates point-to-mask supervision online via hard-margin feature affinity propagation gated by local image priors, using in-batch point-anchored feature-affinity. A protocolized single-variable ablation (local EMA teacher decoupling, hard-background contrastive separation, adaptive support geometry) systematically addresses the Self-Referential Propagation Drift issue.

Result: On SIRST3, GSACP-Final achieves 0.6674 mIoU with a 38% relative reduction in false-positive artifacts (Fa) compared to PAL, establishing a new ultra-low false-alarm operating regime.

Conclusion: In-batch feature propagation provides a compact, end-to-end alternative for deployment scenarios where false-alarm suppression is paramount; the study systematically maps the performance boundaries of the end-to-end paradigm.
...
TL;DR: This paper presents quantum implementations of Sobel edge detection and Harris corner detection using FRQI and QPIE image encodings, with a quantum gradient computation scheme based on lag-2 differences. QPIE outperforms FRQI under limited measurement shots, though overall cost remains dominated by state preparation and post-processing.


<details>
  <summary>Details</summary>
Motivation: To explore quantum computing's potential for classical image processing tasks (edge and corner detection) by implementing quantum versions of Sobel and Harris operators, and to compare two quantum image encoding methods (FRQI vs. QPIE).

Method: Two quantum image encodings (FRQI and QPIE) are used. A lag-2 difference-based quantum gradient computation scheme enables superposition-based gradient evaluation. Classical post-processing reduces false positives in corner detection.

Result: Quantum circuits produce outputs consistent with classical Sobel/Harris operators. QPIE yields more stable results than FRQI under limited measurement shots. Gradient computation is efficient at circuit level, but total cost is dominated by state preparation, measurement, and classical post-processing.

Conclusion: A functional and scalable quantum realization of classical edge/corner detection is demonstrated, though it is not an end-to-end speedup. Performance on NISQ hardware may be affected by noise and measurement limitations.
...
TL;DR: This paper introduces Place Pulse-Gaze, an urban perception dataset with eye-tracking data, and proposes a Gaze-Guided Urban Perception Framework showing that gaze behavior improves prediction of subjective urban perception.


<details>
  <summary>Details</summary>
Motivation: Existing computational models for urban perception ignore the human perceptual process (e.g., gaze behavior) and directly predict from street view images.

Method: Created Place Pulse-Gaze dataset (street images + eye-tracking + perception labels); proposed Gaze-Guided Urban Perception Framework with three settings: gaze-only, gaze fusion with semantic scene representations, and gaze fusion with implicit visual representations.

Result: Gaze alone carries predictive signals for urban perception; integrating gaze with scene representations (both semantic and visual) improves prediction performance.

Conclusion: Incorporating human perceptual processes (gaze) is important for urban scene understanding, opening a direction for gaze-guided multimodal urban computing.
...
TL;DR: Map2World enables 3D world generation from user-defined segment maps of arbitrary shapes and scales, using a detail enhancer network for fine-grained details while maintaining global consistency, outperforming existing methods in controllability and coherence.


<details>
  <summary>Details</summary>
Motivation: Existing 3D world generation methods are constrained by grid layouts and suffer from inconsistencies in object scale across the entire world, limiting flexibility for expansive environments.

Method: Map2World: (1) conditioned on user-defined segment maps of arbitrary shapes/scales; (2) detail enhancer network that adds fine details using global structure information; (3) leverages strong priors from asset generators for robust generalization.

Result: Significantly outperforms existing approaches in user-controllability, scale consistency, and content coherence, enabling 3D world generation under more complex conditions.

Conclusion: Map2World provides flexible, globally consistent 3D world generation with fine-grained details, robust across diverse domains even with limited training data.
...
TL;DR: GMGaze proposes a multi-scale transformer for gaze estimation with early fusion of CLIP global embeddings, semantic prototype conditioning, and sparse Mixture-of-Experts, achieving SOTA results on four benchmarks (2.49°–10.16° mean angular error) and in cross-domain settings.


<details>
  <summary>Details</summary>
Motivation: Address three challenges in gaze estimation: late fusion of image features causing information loss, lack of factor-aware conditioning, and impractical capacity scaling of dense models.

Method: Multi-scale transformer with: (1) semantic prototype conditioning using four prototype banks (illumination, background, head pose, appearance) on CLIP global embeddings; (2) early unified fusion of CLIP patch tokens, CNN tokens, and context-biased global tokens at first layer; (3) sparse Mixture-of-Experts for conditional capacity scaling; (4) adversarial domain adaptation with feature separation loss for cross-domain.

Result: MPIIFaceGaze: 2.49°, EYEDIAP: 3.22°, Gaze360: 10.16°, ETH-XGaze: 1.44° mean angular error — outperforming all within-domain baselines. SOTA cross-domain on two transfer routes.

Conclusion: Early fusion, prototype-conditioned global tokens, and sparse MoE effectively overcome late fusion, lack of conditioning, and capacity scaling issues, setting new SOTA for gaze estimation in both within-domain and cross-domain settings.
...
TL;DR: GenLIP is a generative pretraining framework that trains ViTs to predict language tokens from visual tokens using language modeling, achieving strong multimodal performance with simplicity and scalability.


<details>
  <summary>Details</summary>
Motivation: Existing vision encoders for MLLMs are often trained with contrastive objectives (e.g., CLIP) which may not align well with the autoregressive nature of LLMs. GenLIP aims to bridge this gap by directly predicting language tokens from visual tokens.

Method: GenLIP trains a Vision Transformer (ViT) to autoregressively predict language tokens from visual tokens using a standard language modeling objective, without contrastive learning or an additional text decoder.

Result: Trained on 8B samples from Recap-DataComp-1B, GenLIP matches or surpasses strong baselines with less data. Continued pretraining on multi-resolution images with native aspect ratios further improves OCR and chart understanding tasks.

Conclusion: GenLIP offers a simple, scalable, and high-performing alternative for vision encoder pretraining in MLLMs, aligning better with autoregressive LLMs.
...
TL;DR: Posterior-Augmented Flow Matching (PAFM) improves standard flow matching by reducing training signal sparsity and variance through multi-target posterior supervision, achieving up to 3.4 FID improvement on image generation tasks.


<details>
  <summary>Details</summary>
Motivation: Standard flow matching suffers from sparse, high-variance training signals because each training sample supervises only a single trajectory, leading to flow collapse where the model memorizes specific source-target pairings and fails to generalize.

Method: PAFM replaces single-target supervision with an expectation over an approximate posterior of valid target completions, factorized into endpoint likelihood under the intermediate state and prior probability of the endpoint. It uses importance sampling to construct a mixture over multiple candidate targets, yielding an unbiased estimator of the original FM objective with reduced gradient variance.

Result: PAFM improves FID50K by up to 3.4 across different model scales (SiT-B/2, SiT-XL/2), architectures (SiT, MMDiT), and conditioning settings (class-conditioned ImageNet, text-conditioned CC12M), with negligible compute overhead.

Conclusion: PAFM is a theoretically grounded generalization of flow matching that effectively mitigates flow collapse and improves generation quality by aggregating information from multiple plausible continuation trajectories during training.
...
TL;DR: TADI is an agentic AI system that combines structured drilling data and unstructured reports via a dual-store architecture and 12 domain-specialized tools orchestrated by an LLM, achieving zero-error parsing of 1,759 reports and demonstrating that tool design, not model scale, drives analytical quality.


<details>
  <summary>Details</summary>
Motivation: To transform drilling operational data into evidence-based analytical intelligence by integrating heterogeneous data sources (reports, real-time measurements, production records) into a unified AI-driven system.

Method: Dual-store architecture (DuckDB for structured queries, ChromaDB for semantic search) with 12 domain-specialized tools orchestrated by LLM iterative function calling; formalized as sequential tool-selection problem with Evidence Grounding Score (EGS) for compliance.

Result: Zero errors parsing 1,759 DDR XML files; handled three incompatible well naming conventions; 95 automated tests and 130-question stress taxonomy passed; 6,084-line reproducible implementation.

Conclusion: Domain-specialized tool design, rather than model scale alone, is the primary driver of analytical quality in technical drilling operations.
...
TL;DR: The paper proposes AgentReputation, a decentralized three-layer reputation framework for agentic AI marketplaces that separates task execution, reputation services, and tamper-proof persistence, featuring verification regimes, context-conditioned reputation cards, and a policy engine for resource allocation and risk management.


<details>
  <summary>Details</summary>
Motivation: Existing reputation mechanisms fail in decentralized agentic AI marketplaces because: (1) agents can strategically optimize against evaluation procedures, (2) competence doesn't transfer across heterogeneous task contexts, and (3) verification rigor varies widely. Current approaches from federated learning, blockchain AI, and LLM safety cannot address these challenges collectively.

Method: A three-layer decentralized reputation framework: (1) task execution layer, (2) reputation services layer with verification regimes and context-conditioned reputation cards, and (3) tamper-proof persistence layer. Includes a decision-facing policy engine for resource allocation, access control, and adaptive verification escalation.

Result: The paper presents a conceptual framework design and outlines future research directions; no empirical results are reported.

Conclusion: AgentReputation addresses unique challenges in decentralized agentic AI reputation through layered separation, context-conditioned reputation, and adaptive policy, with future work needed on verification ontologies, privacy-preserving evidence, cold-start bootstrapping, and adversarial defenses.
...
TL;DR: LOCA identifies minimal, interpretable intermediate representation changes that causally induce refusal in jailbroken LLMs, providing local explanations for jailbreak success where prior global methods fail.


<details>
  <summary>Details</summary>
Motivation: Prior work only gives global explanations of jailbreaks (e.g., all jailbreaks reduce harmfulness), but different strategies and request categories may involve different mechanisms. We need local explanations for why a specific jailbreak succeeded.

Method: LOCA (Local, Causal explanations) identifies a minimal set of interpretable intermediate representation changes that causally induce model refusal on an otherwise successful jailbreak request.

Result: LOCA induces refusal with ~6 interpretable changes on average across Gemma and Llama models; prior methods fail even after 20 changes.

Conclusion: LOCA is a step toward mechanistic, local explanations of jailbreak success, outperforming prior global approaches.
...
TL;DR: Tool-augmented reasoning doesn't always outperform native CoT under semantic distractors due to a "tool-use tax" introduced by the tool-calling protocol; G-STEP, a lightweight gate, partially mitigates this.


<details>
  <summary>Details</summary>
Motivation: Challenge the prevailing assumption that tool-augmented reasoning always improves LLM reasoning and reliability, especially under semantic noise/distractors.

Method: A Factorized Intervention Framework that isolates three components: prompt formatting cost, tool-calling protocol overhead, and actual gain from tool execution. Plus G-STEP, a lightweight inference-time gate to mitigate protocol-induced errors.

Result: Tool-augmented reasoning underperforms native CoT with semantic distractors; the "tool-use tax" (protocol-induced degradation) often outweighs tool gains; G-STEP provides partial recovery but cannot fully close the gap.

Conclusion: Improving tool-augmented reasoning requires not only mitigating protocol errors but also strengthening the model's intrinsic reasoning and tool-interaction capabilities.
...
TL;DR: TUR-DPO improves DPO by incorporating reasoning topology quality and uncertainty weighting, achieving better alignment than DPO and matching PPO on reasoning tasks while staying RL-free.


<details>
  <summary>Details</summary>
Motivation: DPO treats preferences as flat winner/loser signals and is sensitive to noisy or brittle preferences from fragile chains of thought.

Method: TUR-DPO elicits lightweight reasoning topologies, combines semantic faithfulness, utility, and topology quality into a calibrated uncertainty signal, then uses a learnable reward factorized over these signals in an uncertainty-weighted DPO objective.

Result: Across 7-8B models on math reasoning, QA, summarization, and dialogue, TUR-DPO improves judge win-rates, faithfulness, and calibration over DPO, with consistent gains in multimodal/long-context settings, and matches/exceeds PPO on reasoning tasks.

Conclusion: TUR-DPO offers a topology- and uncertainty-aware DPO variant that improves alignment quality without sacrificing training simplicity or requiring online rollouts.
...
TL;DR: ARMOR 2025 is a safety benchmark grounded in military doctrines (Law of War, Rules of Engagement, Joint Ethics Regulation) with a 12-category taxonomy based on the OODA framework, used to evaluate 21 LLMs and reveal critical safety alignment gaps.


<details>
  <summary>Details</summary>
Motivation: Existing safety benchmarks focus on general social risks and don't test whether LLMs follow the legal and ethical rules governing real military operations, creating a gap for defense applications requiring reliable, legally compliant decision support.

Method: Extracted doctrinal text from three core military doctrines, generated multiple-choice questions preserving intended meanings, organized via an OODA-based taxonomy, and evaluated 21 commercial LLMs using 519 doctrinally grounded prompts.

Result: Evaluation of 21 commercial LLMs revealed critical gaps in safety alignment for military applications.

Conclusion: ARMOR 2025 fills a critical gap by providing a doctrinally grounded benchmark that reveals LLMs are not yet sufficiently aligned with military legal and ethical standards.
...
TL;DR: This paper proposes a behavioral framework using causal games and causal abstraction to determine when groups of agents can be viewed as unified collective agents, and applies it to analyze multi-agent incentives in actor-critic models and voting mechanisms.


<details>
  <summary>Details</summary>
Motivation: A key safety challenge for advanced AI is that multiple simpler agents might inadvertently form a collective agent with capabilities and goals distinct from any individual. Understanding when a group constitutes a unified collective agent is foundational for studying interactions in both biological and artificial systems.

Method: The authors adopt a behavioral perspective, ascribing collective agency when viewing joint actions as rational and goal-directed predicts behavior. They formalize this using causal games (causal models of strategic multi-agent interactions) and causal abstraction (formalizing when a simple high-level model captures a complex low-level model).

Result: The framework solves a puzzle regarding multi-agent incentives in actor-critic models and enables quantitative assessments of collective agency degree exhibited by different voting mechanisms.
...
TL;DR: The paper proposes an agentic AI framework for trip planning optimization that uses an orchestration agent to coordinate specialized agents, along with a new dataset (TOP Dataset) providing ground-truth optimal solutions. Their system achieves 77.4% accuracy on the TOP Benchmark, outperforming single-agent and workflow-based multi-agent baselines.


<details>
  <summary>Details</summary>
Motivation: Existing trip planning systems focus on feasibility (finding any valid route) rather than optimality, and current benchmarks lack ground-truth optimal solutions, making objective evaluation of optimization performance impossible.

Method: An agentic AI framework with an orchestration agent that dynamically coordinates specialized agents for traffic, charging, and points of interest, plus the Trip-planning Optimization Problems Dataset (TOP Dataset) with definitive optimal solutions and category-level task structure.

Result: 77.4% accuracy on the TOP Benchmark, significantly outperforming single-agent and workflow-based multi-agent baselines.

Conclusion: Orchestrated agentic reasoning is crucial for robust trip planning optimization, and the proposed framework + dataset enable objective evaluation of optimization performance in this domain.
...
TL;DR: TokenArena is a continuous benchmark that evaluates AI inference endpoints (provider, model, SKU tuples) across speed, latency, price, context, and quality, revealing that the same model can vary significantly across endpoints—up to 12.5 points in accuracy, 12 points in fingerprint similarity, an order of magnitude in tail latency, and 6.2× in energy per correct answer—and that workload-aware pricing substantially reorders leaderboards.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks compare AI systems at model/provider level, but real deployment decisions depend on endpoint-level granularity (provider, model, quantization, decoding, region, serving stack). This mismatch motivates TokenArena to measure at the correct unit of deployment.

Method: TokenArena evaluates endpoints along five axes: output speed, time-to-first-token, workload-blended price, effective context, and live-endpoint quality. It synthesizes these with a modeled energy estimate into three composite metrics: joules per correct answer, dollars per correct answer, and endpoint fidelity (distribution similarity to first-party reference).

Result: Across 78 endpoints and 12 model families: same model varies by up to 12.5 accuracy points (math/code), 12 points in fingerprint similarity, order-of-magnitude tail latency differences, and 6.2× energy per correct answer. Workload pricing presets (chat 3:1, RAG 20:1, reasoning 1:5) dramatically reorder leaderboards.

Conclusion: TokenArena provides a methodology (not a single ranking) for endpoint-granularity benchmarking, releasing framework, schema, probes, and leaderboard under CC BY 4.0 for external replication and continuous evaluation.
...
TL;DR: AgentFloor benchmark shows small open-weight models handle most routine agent tasks, while frontier models excel only on long-horizon planning; strongest 32B model matches GPT-5 overall at lower cost.


<details>
  <summary>Details</summary>
Motivation: Current evaluations don't address which parts of agent workflows require large frontier models vs. can be handled by smaller models, despite production systems making many routine model calls per request.

Method: Created AgentFloor: a 30-task, six-tier benchmark covering instruction following, tool use, multi-step coordination, and long-horizon planning. Evaluated 16 open-weight models (0.27B-32B) + GPT-5 across 16,542 scored runs.

Result: Small/mid open-weight models suffice for routine structured tool use. Strongest open-weight model matches GPT-5 overall at lower cost. Frontier advantage only on long-horizon planning tasks, but neither side is highly reliable there. The capability boundary is model-specific, not purely scale-driven.

Conclusion: Practical design principle: use smaller open-weight models for routine actions, reserve large frontier models for tasks requiring deeper planning and control.
...
TL;DR: This paper proposes Hamiltonian World Models that incorporate Hamiltonian dynamics into world modeling for physically meaningful, action-controllable, and long-horizon stable predictions in embodied intelligence.


<details>
  <summary>Details</summary>
Motivation: Current world model research is fragmented across 2D video generation, 3D scene reconstruction, and latent predictive models, all of which struggle to provide physically reliable and action-controllable predictions for embodied decision making.

Method: The method encodes observations into a structured latent phase space, evolves latent states via Hamiltonian-inspired dynamics with control, dissipation, and residual terms, decodes predicted trajectories into future observations, and uses rollouts for planning.

Result: The paper discusses potential improvements in interpretability, data efficiency, and long-horizon stability from the Hamiltonian structure, while acknowledging challenges in real-world robotics (friction, contact, non-conservative forces, deformable objects).

Conclusion: The bottleneck of world models is shifting from generating realistic futures to producing physically meaningful and actionable predictions; Hamiltonian World Models offer a physically grounded perspective to address this.
...
TL;DR: AEM is a supervision-free credit assignment method that adaptively modulates entropy dynamics during RL training to improve exploration-exploitation trade-off for LLM agents, achieving consistent gains including 1.4% improvement on SWE-bench-Verified.


<details>
  <summary>Details</summary>
Motivation: Sparse outcome-only rewards make credit assignment difficult for multi-turn LLM agent tasks; existing solutions (dense supervision like process reward models) increase complexity and generalize poorly across tasks.

Method: AEM adaptively modulates entropy dynamics during RL training. It elevates entropy analysis from token-level to response-level, derives that entropy drift under natural gradients is governed by advantage × relative response surprisal, and uses this as a practical proxy to reshape training dynamics for natural exploration-to-exploitation transition.

Result: Extensive experiments across benchmarks and models (1.5B-32B) show effectiveness, including a 1.4% gain when integrated into SOTA baseline on SWE-bench-Verified.

Conclusion: AEM provides a supervision-free, effective credit assignment mechanism for RL training of LLM agents that generalizes well across tasks and domains.
...
TL;DR: IVLR framework uses interleaved text-visual traces to improve long-horizon robotic manipulation, achieving 95.5% on LIBERO and 92.4% on LIBERO-Long, with ablations confirming both modalities are critical.


<details>
  <summary>Details</summary>
Motivation: Existing VLA policies fail to combine both logical coherence (text) and geometric grounding (visual) simultaneously for long-horizon tasks.

Method: IVLR introduces \trace{}, an intermediate representation alternating textual subgoals with visual keyframes. A multimodal transformer generates this trace from initial observation+instruction, caches it, and conditions an action decoder. Pseudo-supervision via temporal segmentation and VLM captioning.

Result: 95.5% average success on LIBERO, 92.4% on LIBERO-Long, 59.4% on SimplerEnv-WidowX. Text-only 62.0%, vision-only 68.4%, full interleaved 92.4% on LIBERO-Long.

Conclusion: Interleaved vision-language traces are essential for long-horizon manipulation; both modalities are necessary and complementary; traces tolerate local corruption but remain limited under stale global plans.
...
TL;DR: Proposes a method to trace and embed AI's functional role (assistive vs. creative) in generated text, enabling recovery of how AI participated in human-machine symbiosis.


<details>
  <summary>Details</summary>
Motivation: As AI and humans become more intertwined, it's difficult to define AI-generated content. The key question is not whether AI participated but how; however, once content is detached from context, AI's role becomes untraceable.

Method: Infers latent AI role from prompts, embeds this role into content during probabilistic generation, and recovers the nature of AI participation from the resulting text.

Result: Validates methodology across role discrimination, robustness against perturbations, and preservation of linguistic quality in scenarios where AI acts as assistive (editing) vs. creative (generating) agent.

Conclusion: This methodology may contribute to AI ethics research concerning fair, transparent, and appropriate AI use.
...
TL;DR: This paper proposes an instance-aware parameter configuration method for Bilevel Late Acceptance Hill Climbing in Electric Capacitated Vehicle Routing Problem, using offline tuning to create instance-specific parameters predicted via regression models, achieving 0.28% average objective improvement over globally tuned configurations.


<details>
  <summary>Details</summary>
Motivation: Algorithm performance in combinatorial optimization is highly sensitive to parameter settings, and a single global configuration fails to exploit instance heterogeneity, especially in Electric Capacitated Vehicle Routing Problem where instances differ in structure, demand patterns, and energy constraints.

Method: An offline tuning procedure generates instance-specific parameter labels, which are mapped from instance features via a regression model to enable parameter prediction for unseen instances prior to execution.

Result: Achieves an average objective value reduction of 0.28% across eight held-out test instances compared to a globally tuned configuration, corresponding to significant cost reduction in multimillion-dollar transportation operations.

Conclusion: Instance-aware parameter configuration improves metaheuristic performance for Electric Capacitated Vehicle Routing Problem, demonstrating practical value in real-world transportation cost reduction.
...
TL;DR: GUI-SD introduces the first on-policy self-distillation framework for GUI grounding, using visually enriched privileged context and entropy-guided distillation to outperform GRPO-based methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning methods (e.g., GRPO) for GUI grounding require expensive multiple rollouts and suffer from sparse signals on hard samples, motivating the need for a more efficient approach like on-policy self-distillation (OPSD).

Method: GUI-SD first constructs a visually enriched privileged context for the teacher using a target bounding box and Gaussian soft mask, then applies entropy-guided distillation that adaptively weights tokens based on digit significance and teacher confidence.

Result: GUI-SD consistently outperforms GRPO-based methods and naive OPSD on six representative GUI grounding benchmarks in both accuracy and training efficiency.

Conclusion: The proposed OPSD framework is effective and efficient for GUI grounding, providing a promising alternative to traditional reinforcement learning approaches.
...
TL;DR: This paper introduces a decision-making theory framework to evaluate when LLMs should use web search tools, finding misalignment between perceived and actual tool utility, and proposes lightweight estimators to improve tool-use decisions across multiple models and tasks.


<details>
  <summary>Details</summary>
Motivation: Tool use in LLMs is not always beneficial—some calls are redundant or harmful—yet the decision of whether to call a tool (especially web search) is challenging due to noisy responses and reliance on internal knowledge, motivating a principled evaluation framework.

Method: The authors propose a framework inspired by decision-making theory with three factors (necessity, utility, affordability), using two lenses: a normative perspective (true need/utility from optimal tool allocation) and a descriptive perspective (model's self-perceived need/utility from observed behaviors). They train lightweight estimators based on hidden states to improve decisions.

Result: Models' perceived need and utility of tool calls are often misaligned with their true need and utility. Lightweight estimators trained on hidden states enable simple controllers that improve decision quality and task performance across three tasks and six models compared to the self-perceived setup.

Conclusion: A principled decision-making framework can diagnose misalignment in tool-use decisions, and lightweight hidden-state-based estimators can effectively improve LLM tool-use behavior without complex modifications.
...
TL;DR: This position paper argues that while making LLMs themselves Bayesian is computationally intensive, applying Bayesian decision theory at the orchestration/control layer of agentic AI systems is both practical and beneficial for managing uncertainty in tool selection, expert consultation, and resource allocation.


<details>
  <summary>Details</summary>
Motivation: High-value LLM deployments require decisions under uncertainty (e.g., which tool to call, which expert to consult), but current systems lack coherent uncertainty management. Bayesian approaches are promising but remain unclear for LLM inference itself.

Method: Proposes Bayesian decision theory applied at the orchestration layer of agentic systems (not LLM parameters), enabling belief maintenance over latent quantities, belief updating from interactions, and utility-aware action selection. Provides design patterns and examples.

Result: Identifies practical properties for Bayesian control that fit modern agentic AI and human-AI collaboration, demonstrating how calibrated beliefs and utility-aware policies can improve orchestration.

Conclusion: Coherent decision-making under uncertainty requires Bayesian principles at the orchestration level of agentic systems, not necessarily within LLM parameters themselves.
...
### [82] [Dynamic-TD3: A Novel Algorithm for UAV Path Planning with Dynamic Obstacle Trajectory Prediction](https://arxiv.org/abs/2605.00059)
*Wentao Chen,Jingtang Chen,Mingjian Fu,Tiantian Li,Youfeng Su,Wenxi Liu,Yuanlong Yu*

Main category: cs.RO

TL;DR: Dynamic-TD3 is a physically enhanced DRL framework using CMDP, ATREM, and PAG-KF to achieve safe autonomous drone navigation with strict safety constraints, superior collision avoidance, reduced energy use, and smoother flight paths.


<details>
  <summary>Details</summary>
Motivation: DRL in drone navigation faces a safety-exploration dilemma: soft penalties encourage risky exploration, while constraint-based methods degrade under sensor noise and intent uncertainty.

Method: Models navigation as CMDP; integrates Adaptive Trajectory Relational Evolution Mechanism (ATREM) for long-range intentions; uses Physically Aware Gated Kalman Filter (PAG-KF) for noise mitigation; employs dual-criterion policy with Lagrangian relaxation.
...
Conclusion: Dynamic-TD3 effectively resolves the safety-exploration dilemma by enforcing hard safety constraints without sacrificing maneuverability or performance under noisy conditions.
...
### [83] [Do Open-Loop Metrics Predict Closed-Loop Driving? A Cross-Benchmark Correlation Study of NAVSIM and Bench2Drive](https://arxiv.org/abs/2605.00066)
*Yiru Wang,Anqing Jiang,Shuo Wang,Yuwen Heng,Hai Yang,Yang Chen,Hao Sun*

Main category: cs.RO

TL;DR: Open-loop metrics (especially NAVSIM's) show limited correlation with closed-loop driving performance. Ego Progress is the best predictor; safety metrics like NC are less reliable. A simpler 3-metric formula matches the full 5-metric PDM Score's predictive power. The snowball effect explains the residual gap.


<details>
  <summary>Details</summary>
Motivation: To determine whether safety-aware open-loop metrics (NAVSIM v2) can reliably predict real closed-loop driving performance, given that traditional metrics (ADE, FDE) fail to do so.

Method: Systematic cross-referencing of published results from 15 SOTA methods across NAVSIM (open-loop) and Bench2Drive (closed-loop), yielding 8 methods with complete paired data. Spearman correlation analysis between open-loop sub-metrics and closed-loop Driving Score.

Result: (1) NAVSIM PDM Score has strong but non-monotonic correlation with closed-loop Driving Score. (2) Ego Progress is the best single predictor, exceeding collision metric NC. (3) Safety-progress trade-off differs: safety-maximizing methods rank high in NAVSIM but underperform in closed-loop. A simpler 3-metric formula matches the full 5-metric PDMS (Spearman ρ=0.90).

Conclusion: Current open-loop metrics still have limited predictive power for closed-loop performance. The snowball effect (small open-loop deviations compounding into closed-loop failures) is a candidate mechanism for the residual gap. Simpler metrics may suffice; safety metrics near saturation add little information.
...
### [84] [Being-H0.7: A Latent World-Action Model from Egocentric Videos](https://arxiv.org/abs/2605.00078)
*Hao Luo,Wanpeng Zhang,Yicheng Feng,Sipeng Zheng,Haiweng Xu,Chaoyi Xu,Ziheng Xi,Yuhui Fu,Zongqing Lu*

Main category: cs.RO

TL;DR: Being-H0.7 is a latent world-action model that enhances VLA-style robot policies by inserting learnable latent queries trained via a dual-branch future alignment method, achieving predictive benefits without the cost of pixel-space video generation.


<details>
  <summary>Details</summary>
Motivation: Existing VLAs suffer from sparse action supervision leading to shortcut mappings, while world-action models using pixel-space video prediction are costly and indirect for control.

Method: Inserts learnable latent queries between perception and action; uses a dual-branch design where a prior branch infers latents from current context and a posterior branch uses future observations for alignment during training only.

Result: Achieves state-of-the-art or comparable performance across six simulation benchmarks and diverse real-world tasks.

Conclusion: Combines the predictive benefits of world models with the efficiency and deployability of direct VLA policies, without requiring visual rollout at inference.
...
### [85] [World Model for Robot Learning: A Comprehensive Survey](https://arxiv.org/abs/2605.00080)
...
TL;DR: A comprehensive survey reviewing world models for robot learning, covering their integration with policies, use as learned simulators, video generation advances, navigation/autonomous driving applications, and key benchmarks/datasets.


<details>
  <summary>Details</summary>
Motivation: The literature on world models for robot learning is fragmented across architectures, functional roles, and application domains. This survey aims to unify and systematically review these diverse works.

Method: The authors conduct a comprehensive literature review examining how world models couple with robot policies, serve as learned simulators for RL/evaluation, progress in video generation (from imagination to foundation-scale), and connect to navigation/autonomous driving, along with representative datasets and benchmarks.

Result: A structured taxonomy and systematic overview of world models in robot learning, clarifying key paradigms, applications, and available resources.

Conclusion: World models are central to robot learning with rapid advances driven by foundation models; the survey identifies major challenges and future directions, and provides a maintained GitHub repository for ongoing updates.
...
### [86] [Predictive Spatio-Temporal Scene Graphs for Semi-Static Scenes](https://arxiv.org/abs/2605.00121)
*Miguel Saavedra-Ruiz,Charlie Gauthier,Kumaraditya Gupta,Shima Shahfar,Kirsty Ellis,Steven Parkison,Liam Paull*

Main category: cs.RO

TL;DR: This paper introduces PredictiveGraphs, a method for tempo-spatio-semantic reasoning that uses Bayesian filters (Perpetua$^*$) within 3D scene graphs to predict future environment states based on observed cyclic temporal patterns.


<details>
  <summary>Details</summary>
Motivation: Existing spatio-semantic representations lack temporal reasoning capabilities, limiting robots' ability to predict structured changes in environments (e.g., cyclic movement of objects like a mug).

Method: Proposes PredictiveGraphs, a 3D scene graph where nodes represent objects and edges function as Perpetua$^*$ filters performing Bayesian reasoning on temporally observed environment states.

Result: Outperforms baselines in predicting future environment states in both simulation and real-world dynamic navigation tasks (bi-hourly changes over three weeks), even under distributional shifts.

Conclusion: Integrating temporal Bayesian filtering into scene graphs enables robust prediction of semi-static environmental changes, advancing robotic reasoning across time, space, and semantics.
...
### [87] [E$^2$DT: Efficient and Effective Decision Transformer with Experience-Aware Sampling for Robotic Manipulation](https://arxiv.org/abs/2605.00159)
*Kaiyan Zhao,Borong Zhang,Yiming Wang,Xingyu Liu,Xuetao Li,Yuyang Chen,Xiaoguang Niu*

Main category: cs.RO

TL;DR: E²DT is a DT-guided k-DPP sampling framework that improves robotic manipulation by actively selecting diverse, high-quality experiences for more efficient and effective long-horizon RL.


<details>
  <summary>Details</summary>
Motivation: Standard Decision Transformer lacks active exploration, relying on uniform replay that causes poor sample efficiency, limited exploration, and suboptimal effectiveness; excessive exploration also delays convergence.

Method: E²DT uses a k-Determinantal Point Process sampling framework guided by DT internal embeddings (for diversity) and a composite quality metric (RTG quantiles, predictive uncertainty, stage coverage) integrated via a quality-diversity joint kernel to prioritize informative trajectories.

Result: E²DT consistently outperforms prior methods on challenging robotic manipulation benchmarks in both simulation and real-robot settings.
...
### [88] [Lucid-XR: An Extended-Reality Data Engine for Robotic Manipulation](https://arxiv.org/abs/2605.00244)
...
TL;DR: Lucid-XR is a generative data engine that creates diverse multimodal synthetic data for training robotic systems using a web-based physics simulation (vuer) on XR headsets, with physics-guided video generation and human-to-robot pose retargeting, enabling zero-shot policy transfer to real-world cluttered environments.


<details>
  <summary>Details</summary>
Motivation: To generate diverse and realistic multimodal data for training real-world robotic systems without requiring specialized equipment, enabling scalable and accessible data generation for robot learning.

Method: Lucid-XR uses vuer (a web-based physics simulation on XR headsets) for immersive interactions, integrates on-device physics with human-to-robot pose retargeting, and amplifies data via a physics-guided, language-steerable video generation pipeline.

Result: Demonstrates zero-shot transfer of robot visual policies to unseen, cluttered, and poorly lit real-world environments after training exclusively on Lucid-XR's synthetic data, across dexterous manipulation tasks involving soft materials, particles, and rigid bodies.

Conclusion: Lucid-XR enables effective zero-shot sim-to-real transfer for robotic manipulation policies using only synthetic data generated through its integrated simulation and video generation pipeline.
...
### [89] [Task-Conditioned Uncertainty Costmaps for Legged Locomotion](https://arxiv.org/abs/2605.00261)
*Kartikeya Singh,Christo Aluckal,Romeo Orsolino,Karthik Dantu*

Main category: cs.RO

TL;DR: This paper shows that modeling epistemic uncertainty in learned foothold predictions helps detect out-of-distribution terrains, enabling more reliable path planning for legged robots with up to 37% reduction in feasibility error.


<details>
  <summary>Details</summary>
Motivation: Foothold prediction on highly unstructured terrain remains challenging even with repetitive gait cycles, and limited training data distributions cause poor generalization.

Method: The authors model epistemic uncertainty in predicted footholds conditioned on terrain observations and commanded motion, then use this uncertainty to detect OOD regions and incorporate them into a unified costmap-generation framework for uncertainty-aware path planning.

Result: Up to 37% reduction in simulation feasibility error, improved OOD detection, and more reliable planning behavior than geometry-only baselines.

Conclusion: Modeling epistemic uncertainty allows a single learned model trained on limited data to express uncertainty from missing coverage, improving legged robot planning in both in-distribution and OOD terrains.
...
### [90] [A Model-based Visual Contact Localization and Force Sensing System for Compliant Robotic Grippers](https://arxiv.org/abs/2605.00307)
*Kaiwen Zuo,Shuyuan Yang,Zonghe Chua*

Main category: cs.RO

TL;DR: A model-based visual force sensing approach for soft grippers using RGB-D wrist cameras, integrating iterative contact localization with inverse finite element analysis, achieving 0.23N RMSE during load phase.


<details>
  <summary>Details</summary>
Motivation: Grasp force estimation is crucial for preventing robot damage to delicate objects, but current methods face trade-offs between cost, robustness, and generalization. End-to-end deep learning approaches are brittle, while model-based methods don't suit modern gripper geometries.

Method: Extracts structural key points from RGB-D images of deforming fin-ray-shaped soft grippers, defines parameters for inverse FEA simulation in SOFA, and uses deep learning-based iterative contact localization for dynamic contact updates robust to occlusion.

Result: Average RMSE of 0.23N (NRMSD 2.11%) during load phase, and 0.48N (NRMSD 4.34%) over entire grasping process across various objects and conditions.

Conclusion: Demonstrates potential for real-time model-based indirect force sensing of soft grippers, offering improved generalization over end-to-end deep learning methods.
...
### [91] [Embodied Interpretability: Linking Causal Understanding to Generalization in Vision-Language-Action Models](https://arxiv.org/abs/2605.00321)
*Hanxin Zhang,Mingshuo Xu,Abdulqader Dhafer,Shigang Yue,Hongbiao Dong,Zhou Daniel Hao*

Main category: cs.RO

TL;DR: This paper proposes interventional attribution methods (ISS and NMR) to identify spurious correlations in VLA policies, showing they predict generalization failures and provide better explanations than existing methods.


<details>
  <summary>Details</summary>
Motivation: VLA policies fail under distribution shift due to spurious visual correlations rather than task-relevant causes.

Method: Formulates visual-action attribution as an interventional estimation problem, introducing Interventional Significance Score (ISS) for causal influence estimation and Nuisance Mass Ratio (NMR) for measuring task-irrelevant feature attribution.

Result: ISS admits unbiased estimation; NMR predicts generalization behavior; ISS yields more faithful explanations than existing interpretability methods.

Conclusion: Interventional attribution offers a simple diagnostic approach for identifying causal misalignment in embodied policies.
...
TL;DR: PrefMoE is a mixture-of-experts reward learning framework that handles heterogeneous and conflicting preference data in preference-based RL by learning multiple specialized reward experts and combining them via soft routing, improving robustness and downstream policy learning over single-model approaches.


<details>
  <summary>Details</summary>
Motivation: Large-scale preference datasets contain heterogeneous and conflicting supervision (annotator disagreement and inconsistency), but existing methods fit a single reward model that averages these incompatible signals, limiting robustness.

Method: PrefMoE uses a mixture-of-experts architecture with multiple specialized reward experts and trajectory-level soft routing to adaptively combine them, plus a load-balancing regularizer to prevent expert collapse.

Result: On D4RL locomotion and MetaWorld manipulation tasks, PrefMoE improves preference prediction robustness and leads to more reliable downstream policy learning compared to strong single-model baselines.

Conclusion: Mixture-of-experts reward modeling is an effective approach for handling noisy and heterogeneous preference supervision in preference-based RL.
...
TL;DR: MiniVLA-Nav v1 is a simulation dataset for language-conditioned object approach navigation, with 1,174 episodes of a robot navigating to objects based on natural-language instructions in four photorealistic environments. It provides synchronized RGB, depth, and segmentation data with expert action labels, supporting in-distribution, paraphrase-robustness, and OOD evaluation splits.


<details>
  <summary>Details</summary>
Motivation: To enable research on language-conditioned navigation by providing a high-quality, diverse simulation dataset that pairs natural-language instructions with synchronized sensor data and expert action trajectories for object approach tasks.

Method: The dataset collects 1,174 episodes in four Isaac Sim environments (Office, Hospital, Full Warehouse, Warehouse with Multiple Shelves) using an NVIDIA Nova Carter robot. It includes RGB images, depth maps, instance segmentation masks, and continuous/tokenized expert action labels from a vision-based proportional controller at 60 Hz. Trajectory diversity is achieved through three spawn-distance tiers, 12 object categories, 18 training templates, and 12 paraphrase-OOD templates.

Result: The dataset features a Pearson correlation of 0.94 between spawn distance and trajectory length, supports five evaluation splits, and is publicly released at https://huggingface.co/datasets/alibustami/miniVLA-Nav.

Conclusion: MiniVLA-Nav v1 provides a structured, reproducible benchmark for evaluating language-conditioned navigation agents across in-distribution and out-of-distribution settings.
...
TL;DR: LWD is a fleet-scale offline-to-online RL framework that continually improves generalist VLA robot policies using deployment data, achieving 95% average success across 16 robots on 8 real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Offline data alone is insufficient for robust robot deployment due to distribution shifts, long-tail failures, and task variations that fixed datasets cannot capture.

Method: LWD closes the loop between deployment, shared experience, and policy improvement using autonomous rollouts and human interventions. It combines Distributional Implicit Value Learning (DIVL) for robust value estimation with Q-learning via Adjoint Matching (QAM) for policy extraction in flow-based VLA action generators.

Result: Validated on a fleet of 16 dual-arm robots across 8 real-world tasks (e.g., grocery restocking, long-horizon tasks). Single generalist policy reached 95% average success rate, with largest gains on long-horizon tasks.

Conclusion: Continual online RL post-training from fleet deployment data significantly improves pretrained VLA policies, enabling robust real-world performance.
...
### [95] [Stereo Multistage Spatial Attention for Real-Time Mobile Manipulation Under Visual Scale Variation and Disturbances](https://arxiv.org/abs/2605.00471)
*Xianbo Cai,Hideyuki Ichiwara,Hyogo Hiruma,Masaki Yoshikawa,Hiroshi Ito,Tetsuya Ogata*

Main category: cs.RO

TL;DR: A stereo multistage spatial attention-based deep predictive learning method for real-time mobile manipulation, evaluated on four real-world tasks with improved robustness over baselines.


<details>
  <summary>Details</summary>
Motivation: Robots in open environments face visual scale variations due to changing camera viewpoints, which affects vision-based motion generation for mobile manipulation.

Method: Stereo multistage spatial attention-based deep predictive learning; extracts task-relevant spatial attention points from stereo images and integrates with robot states via hierarchical recurrent architecture for closed-loop action prediction.

Result: Improved robustness and task success rates on four mobile manipulation tasks (rigid placement, articulated object manipulation, deformable object interaction) compared to imitation learning and vision-language-action baselines.

Conclusion: Structured stereo spatial attention combined with predictive temporal modeling provides an effective solution for mobile manipulation scenarios.
...
### [96] [MSACT: Multistage Spatial Alignment for Stable Low-Latency Fine Manipulation](https://arxiv.org/abs/2605.00475)
*Xianbo Cai,Hideyuki Ichiwara,Masaki Yoshikawa,Tetsuya Ogata*

Main category: cs.RO

TL;DR: A multistage spatial attention module with temporal alignment loss improves bimanual manipulation stability and latency by extracting stable 2D attention points and predicting future sequences, built upon ACT with a pretrained ResNet.


<details>
  <summary>Details</summary>
Motivation: Real-world bimanual manipulation requires low-latency control and stable visual localization, but limited data causes localization drift, and existing methods trade off between latency, expressiveness, and computational cost.

Method: Introduces a multistage spatial attention module that extracts stable 2D attention points and jointly predicts future attention sequences with a temporal alignment loss, built upon ACT with pretrained ResNet visual prior.

Result: Experiments on simulated and real-world ALOHA bimanual platform show improvements in localization stability and task performance while maintaining low-latency inference.

Conclusion: The proposed approach effectively suppresses drift without keypoint annotations, improving vision-to-action mapping stability under limited data.
...
### [97] [Recovering Hidden Reward in Diffusion-Based Policies](https://arxiv.org/abs/2605.00623)
*Yanbiao Ji,Qiuchang Li,Yuting Hu,Shaokai Wu,Wenyuan Xie,Guodong Zhang,Qicheng He,Deyi Ji,Yue Ding,Hongtao Lu*

Main category: cs.RO

TL;DR: EnergyFlow unifies generative action modeling with inverse RL by using a scalar energy function whose gradient is the denoising field, achieving SOTA imitation performance while enabling reward extraction without adversarial training.


<details>
  <summary>Details</summary>
Motivation: To bridge generative action modeling and inverse reinforcement learning, eliminating the need for adversarial training while enabling principled reward extraction from expert demonstrations.

Method: Parameterizes a scalar energy function whose gradient serves as the denoising field; uses denoising score matching to recover the gradient of the expert's soft Q-function under max-entropy optimality; constrains the field to be conservative.

Result: Achieves SOTA imitation performance on manipulation tasks; provides effective reward signals for downstream RL that outperform adversarial IRL and likelihood-based methods; tighter OOD generalization bounds.

Conclusion: Structural constraints required for valid reward extraction also serve as beneficial inductive biases for policy generalization, demonstrating that principled design choices can simultaneously improve multiple objectives.
...
### [98] [Paired-CSLiDAR: Height-Stratified Registration for Cross-Source Aerial-Ground LiDAR Pose Refinement](https://arxiv.org/abs/2605.00634)
*Montana Hoover,Jing Liang,Tianrui Guan,Dinesh Manocha*

Main category: cs.RO

TL;DR: Paired-CSLiDAR provides a cross-source aerial-ground LiDAR benchmark for pose refinement, proposing RGSR—a training-free, geometry-only method achieving 86.0% success at 0.75m RMSE, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing registration methods fail under cross-source aerial-ground LiDAR due to minimal geometric overlap (primarily terrain only), causing convergence to metrically incorrect local minima.

Method: Residual-Guided Stratified Registration (RGSR): height-stratified ICP, reversed registration directions, and confidence-gated accept-if-better selection—training-free and geometry-only.
...
Conclusion: Height-stratified, geometry-only refinement effectively handles extreme partial overlap in cross-source LiDAR pose refinement; Fourier-Mellin BEV proposals can reduce RMSE but increase actual error under extreme conditions.
...
### [99] [Affordance Agent Harness: Verification-Gated Skill Orchestration](https://arxiv.org/abs/2605.00663)
*Haojian Huang,Jiahao Shi,Yinchuan Li,Yingcong Chen*

Main category: cs.RO

TL;DR: Affordance Agent Harness is a closed-loop runtime system that unifies heterogeneous skills, retrieves episodic memories, and uses adaptive routing with a verifier to improve affordance grounding accuracy while reducing computational cost compared to fixed-pipeline baselines.


<details>
  <summary>Details</summary>
Motivation: Existing affordance grounding systems use fixed pipelines poorly matched to per-instance difficulty, lack recovery from intermediate errors, and fail to reuse experience from recurring objects.

Method: A closed-loop runtime with an evidence store, episodic memory retrieval, adaptive Router for skill selection, a Verifier using self-consistency/cross-scale stability/evidence sufficiency for gating, and a final judge fusing evidence into predictions.

Result: Achieves stronger accuracy-cost Pareto frontier than fixed-pipeline baselines, improving grounding quality while reducing average skill calls and latency across multiple benchmarks.

Conclusion: Adaptive, closed-loop orchestration with memory and verification yields more efficient and robust affordance grounding than static pipelines.
