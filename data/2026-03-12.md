...
TL;DR: 4DEquine: A novel framework for 4D horse reconstruction from monocular video by disentangling motion and appearance reconstruction, achieving SOTA results with synthetic-only training.


<details>
  <summary>Details</summary>
Motivation: 4D reconstruction of horses is important for animal welfare, but existing methods require joint optimization of motion and appearance over entire videos, which is time-consuming and sensitive to incomplete observations.

Method: Disentangles 4D reconstruction into two sub-problems: 1) dynamic motion reconstruction using spatio-temporal transformer with post-optimization for smooth pose/shape sequences, and 2) static appearance reconstruction using feed-forward network for animatable 3D Gaussian avatar from single image. Created synthetic datasets VarenPoser (motion) and VarenTex (appearance) for training.

Result: Achieves state-of-the-art performance on real-world APT36K and AiM datasets despite training only on synthetic data, demonstrating superiority for both geometry and appearance reconstruction.
...
TL;DR: HG-Lane is a high-fidelity generation framework for lane scenes under adverse weather/lighting conditions, creating a 30K-image benchmark that improves lane detection models' robustness without requiring re-annotation.
...
Method: Propose HG-Lane, a high-fidelity generation framework for lane scenes under adverse weather and lighting conditions without requiring re-annotation. The framework creates synthetic but realistic adverse condition images from existing datasets.

Result: Created a benchmark with 30,000 images covering adverse weather and lighting scenarios. Using CLRNet, overall mF1 score increased by 20.87%, with F1@50 improvements across all categories: overall (19.75%), normal (8.63%), snow (38.8%), rain (14.96%), fog (26.84%), night (21.5%), and dusk (12.04%).

Conclusion: HG-Lane effectively addresses the data scarcity problem for adverse weather conditions in lane detection, significantly improving model robustness without costly re-annotation, making autonomous driving systems safer in challenging environments.
...
TL;DR: Using unbalanced Wasserstein barycenters for dictionary learning improves hyperspectral image clustering by handling noise better than balanced approaches.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral image labeling is labor-intensive and traditional methods struggle with high-dimensional data. Existing Wasserstein dictionary learning approaches require data balancing that blurs classes and reduces robustness to noise/outliers.
...
Result: The approach enables effective unsupervised learning of labels for hyperspectral images by better handling noise and outliers while preserving class distinctions.

Conclusion: Unbalanced Wasserstein barycenters provide a superior approach for hyperspectral image clustering compared to balanced methods, offering better robustness and clearer class separation.
...
TL;DR: ExeVRM: A video-based reward model that evaluates computer-using agents by analyzing execution videos, achieving 84.7% accuracy and outperforming proprietary models across multiple operating systems.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of computer-using agents is difficult to scale and often relies on internal reasoning or actions. Video-execution modeling offers a method-agnostic approach but faces challenges with redundant layouts and subtle success cues.

Method: Created ExeVR-53k dataset of video-task-reward triplets, used adversarial instruction translation for negative samples, developed spatiotemporal token pruning for efficient video processing, and fine-tuned an 8B parameter Execution Video Reward Model (ExeVRM).

Result: ExeVRM 8B achieves 84.7% accuracy and 87.7% recall on video-execution assessment, outperforming GPT-5.2 and Gemini-3 Pro across Ubuntu, macOS, Windows, and Android, with better temporal attribution.

Conclusion: Video-execution reward modeling provides a scalable, model-agnostic evaluator for computer-using agents, enabling effective assessment across different platforms with precise temporal understanding.
...
TL;DR: Delta-K is a training-free inference framework that addresses concept omission in diffusion models by extracting and injecting semantic signatures of missing concepts directly in cross-attention key space.


<details>
  <summary>Details</summary>
Motivation: Diffusion models often suffer from concept omission when synthesizing complex multi-instance scenes, and existing training-free methods that rescale attention maps only exacerbate unstructured noise without establishing coherent semantic representations.

Method: Delta-K operates directly in the shared cross-attention Key space, using a vision-language model to extract a differential key ΔK that encodes semantic signatures of missing concepts. This signal is injected during early semantic planning stages with a dynamically optimized scheduling mechanism.
...
Conclusion: Delta-K provides a backbone-agnostic, plug-and-play inference framework that effectively tackles concept omission in diffusion models by operating directly in the cross-attention key space, establishing coherent semantic representations for missing concepts.
...
TL;DR: FusionNet: A neural network that reconstructs high temporal resolution 4D cardiac motion from short-duration CMR scans to reduce patient discomfort while maintaining diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Standard CMR imaging requires patients to lie still for 40-60 minutes in a confined, loud machine, causing discomfort. Shorter scan times reduce temporal resolution, compromising diagnostic accuracy of cardiac motion analysis.

Method: Proposes FusionNet neural network that estimates intermediate 3D heart shapes from adjacent shapes to reconstruct 4D cardiac motion with high temporal resolution from short-duration CMR images.

Result: Experimental evaluation shows FusionNet achieved Dice coefficient performance over 0.897, confirming it can recover shapes more precisely than existing methods.

Conclusion: FusionNet successfully addresses the trade-off between scan time and temporal resolution in CMR imaging, enabling shorter scans while maintaining high-quality 4D cardiac motion reconstruction for improved patient comfort and diagnostic accuracy.
...
TL;DR: AI framework combining anatomy-aware segmentation and radiomics for predicting postoperative survival in colorectal liver metastasis patients using MRI.


<details>
  <summary>Details</summary>
Motivation: CRLM patient outcomes are highly heterogeneous after hepatectomy, making postoperative survival prediction crucial to avoid non-beneficial surgeries and guide personalized therapy.

Method: Two-stage framework: 1) Anatomy-aware segmentation pipeline using SAMONAI (prompt propagation algorithm extending Segment Anything Model to 3D) for liver, CRLMs, and spleen segmentation; 2) Radiomics pipeline using SurvAMINN (autoencoder-based multiple instance neural network) for time-to-event survival prediction from extracted tumor features.

Result: Segmentation achieved Dice scores: 0.96 (liver), 0.93 (spleen), 0.78 (CRLM), with detection F1-score of 0.79. Survival prediction achieved C-index of 0.69, outperforming established methods and biomarkers in Cox regression analysis.

Conclusion: Integration of segmentation algorithms with radiomics-based survival analysis enables accurate and automated CRLM outcome prediction, showing potential for clinical application.
...
TL;DR: Real-time deformation-aware CBCT updating framework using robotic ultrasound to track soft-tissue motion and update static CBCT slices without additional radiation exposure.


<details>
  <summary>Details</summary>
Motivation: Static intraoperative CBCT fails to capture continuous soft-tissue deformations caused by respiration, probe pressure, and surgical manipulation, leading to navigation inaccuracies during interventions.
...
Conclusion: Proposed framework successfully enables deformation-consistent CBCT visualization without repeated radiation exposure, addressing the limitation of static CBCT in tracking intraoperative soft-tissue dynamics.
...
TL;DR: OilSAM2: A memory-augmented segmentation framework for unordered SAR oil spill monitoring that addresses appearance variability and scale heterogeneity through hierarchical feature-aware memory banks.


<details>
  <summary>Details</summary>
Motivation: Segmenting oil spills from SAR imagery is challenging due to severe appearance variability, scale heterogeneity, and lack of temporal continuity in real-world monitoring scenarios. Existing SAM-based approaches operate on single images and cannot effectively reuse information across scenes, while memory-augmented variants assume temporal coherence and suffer from semantic drift when applied to unordered SAR image collections.

Method: OilSAM2 introduces a hierarchical feature-aware multi-scale memory bank that explicitly models texture, structure, and semantic level representations for robust cross-image information reuse. To mitigate memory drift, it employs a structure-semantic consistent memory update strategy that selectively refreshes memory based on semantic discrepancy and structural variation.
...
Conclusion: OilSAM2 provides an effective memory-augmented segmentation framework tailored for unordered SAR oil spill monitoring, overcoming limitations of existing approaches through hierarchical feature modeling and drift-resistant memory management.
...
TL;DR: I2X framework converts unstructured XAI interpretability into structured explanations by tracking prototype evolution during training, enabling both faithful model explanation and practical accuracy improvement.


<details>
  <summary>Details</summary>
Motivation: Current XAI methods provide unstructured interpretability (saliency maps/concepts) and often rely on auxiliary models that compromise faithfulness to original models, limiting transparency and trustworthiness.

Method: I2X builds structured explanations directly from unstructured interpretability by quantifying progress at selected checkpoints during training using prototypes extracted from post-hoc XAI methods (e.g., GradCAM), revealing both intra- and inter-class decision making.

Result: Experiments on MNIST and CIFAR10 demonstrate I2X effectively reveals prototype-based inference processes. I2X can identify uncertain prototypes and use targeted perturbation for fine-tuning, ultimately improving accuracy across different model architectures and datasets.

Conclusion: I2X not only faithfully explains model behavior but also provides a practical approach to guide optimization toward desired targets, bridging the gap between interpretability and actionable explainability.
...
TL;DR: One-A is an imbalance-aware framework for class-incremental learning that handles varying class counts per task using a single adapter with asymmetric subspace alignment and directional gating.


<details>
  <summary>Details</summary>
Motivation: Most CIL methods assume balanced task streams, but in practice, tasks often have significantly varying numbers of classes (step imbalance), causing large tasks to dominate learning and small tasks to inject unstable updates, degrading overall performance.

Method: One-A incrementally merges task updates into a single adapter using asymmetric subspace alignment to preserve dominant subspaces from large tasks while constraining low-information updates. It uses information-adaptive weighting to balance base and new adapters, and directional gating to selectively fuse updates along singular directions.
...
Conclusion: One-A provides a unified, imbalance-aware framework that effectively handles step-imbalanced CIL scenarios through asymmetric subspace fusion, maintaining both adaptability to varying task sizes and deployment efficiency with constant inference cost.
...
TL;DR: A unified cross-view contrastive framework that jointly learns global imaging and local ROI-graph representations for brain disorder classification, showing consistent performance gains through complementary pattern integration.


<details>
  <summary>Details</summary>
Motivation: Existing brain imaging classification approaches use either full-image volumes (global context) or ROI-based graphs (local topology), but their relative contributions and complementarity are poorly understood. Current fusion methods are task-specific and don't allow controlled evaluation of each representation.
...
Result: Extensive experiments on ADHD-200 and ABIDE datasets show joint learning consistently improves classification performance over imaging-only or ROI-only approaches across multiple backbone choices. Interpretability analyses reveal imaging and ROI branches emphasize distinct yet complementary discriminative patterns.

Conclusion: Explicit integration of global volumetric and ROI-level representations is a promising direction for neuroimaging-based brain disorder classification, as the two representations capture complementary information that jointly improves performance.
...
TL;DR: A robust Bangla License Plate Recognition system using YOLOv8 with adaptive training for localization and VisionEncoderDecoder (ViT+BanglaBERT) for text recognition, achieving 97.83% accuracy and strong performance on challenging real-world datasets.


<details>
  <summary>Details</summary>
Motivation: Bangla license plate recognition is challenging due to complex character schemes and uneven layouts, creating a need for robust systems for intelligent traffic management applications like automated law enforcement and access control.

Method: Two-stage approach: 1) Novel adaptive training strategy on YOLOv8 for license plate localization, compared with U-Net and other YOLO variants; 2) VisionEncoderDecoder architecture (ViT+BanglaBERT) for text recognition framed as sequence generation problem.

Result: Achieved 97.83% accuracy and 91.3% IoU for localization; ViT+BanglaBERT achieved Character Error Rate 0.1323 and Word Error Rate 0.1068; Consistent performance on external dataset with different environment/lighting conditions.

Conclusion: The proposed system provides robust and reliable Bangla license plate recognition effective across diverse real-world scenarios, making it suitable for deployment in intelligent transportation applications.
...
TL;DR: DeepIntuit: A framework that evolves video classification from imitation to intuition using vision-language models with intrinsic reasoning and reinforcement learning for open-instance scenarios.


<details>
  <summary>Details</summary>
Motivation: Real-world video classification faces open-instance challenges with vast intra-class variations beyond existing benchmarks. While VLMs offer generalization, they haven't fully leveraged reasoning capabilities for such tasks.

Method: Three-stage approach: 1) Cold-start supervised alignment to initialize reasoning, 2) Group Relative Policy Optimization (GRPO) refinement for reasoning coherence, 3) Intuitive calibration training classifier on reasoning traces for stable knowledge transfer.

Result: Extensive experiments show DeepIntuit significantly benefits from transcending feature imitation toward intrinsic reasoning for open-instance video classification.

Conclusion: DeepIntuit bridges the gap between imitation and intuition in video classification, enabling VLMs to better handle real-world open-instance scenarios through intrinsic reasoning capabilities.
...
TL;DR: Fuel Gauge predicts Chain-of-Thought length in Large Multi-modality Models to optimize memory allocation and reasoning accuracy.


<details>
  <summary>Details</summary>
Motivation: Current LMMs use unpredictable CoT processes that cause inefficient computational resource use (memory fragmentation) and sub-optimal accuracy (under-/over-thinking).

Method: Fuel Gauge extracts a hidden "fuel" parameter to predict CoT length ahead of time, enabling predictive KV cache allocation and CoT length modulation.

Result: On GPQA-Diamond benchmark, achieves <50% CoT length prediction error vs baseline, with 13.37x reduction in memory allocation frequency across text, image, and video QA benchmarks.

Conclusion: Fuel Gauge effectively addresses memory fragmentation and accuracy issues in LMM serving by predicting CoT length, demonstrating practical value for efficient reasoning systems.
...
TL;DR: CGVD is a training-free framework that addresses VLA models' precision-reasoning gap in cluttered environments by using concept-gated visual distillation to suppress semantic distractors while preserving spatial geometry.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models suffer from "Precision-Reasoning Gap" in cluttered environments due to background-induced feature dilution, where semantic noise corrupts geometric grounding needed for precise manipulation.

Method: Concept-Gated Visual Distillation (CGVD) parses instructions into safe/distractor sets, uses two-layer target refinement (cross-validation + spatial disambiguation) to penalize false positives, and applies Fourier-based inpainting to generate clean observations that suppress semantic distractors while preserving spatial geometry.

Result: CGVD prevents performance collapse in cluttered environments, achieving 77.5% success rate vs. baseline's 43.0% in dense semantic distractor settings, significantly outperforming state-of-the-art baselines.

Conclusion: CGVD establishes inference-time visual distillation as critical for robust robotic manipulation in clutter by enforcing strict attribute adherence and actively suppressing semantic distractors while preserving essential spatial information.
...
TL;DR: EmoStory is a new framework for emotion-aware story generation that creates subject-consistent visual stories with explicit emotional directions, outperforming existing methods in emotion accuracy and narrative coherence.


<details>
  <summary>Details</summary>
Motivation: Existing story generation methods focus on subject consistency and coherence but remain emotion-neutral, overlooking how emotions shape narrative interpretation and visual presentation. Since stories should engage audiences emotionally, there's a need for emotion-aware story generation.

Method: Two-stage framework: 1) Agent-based story planning with emotion agent and writer agent to transform target emotions into coherent story prompts, and 2) Region-aware story generation that preserves subject consistency while injecting emotion-related elements through visual composition.

Result: Evaluated on a new dataset with 25 subjects and 600 emotional stories. EmoStory outperforms state-of-the-art methods in emotion accuracy, prompt alignment, and subject consistency based on quantitative metrics, qualitative analysis, and user studies.

Conclusion: EmoStory successfully addresses the challenging task of emotion-aware story generation by grounding abstract emotions in concrete visual elements and maintaining consistent emotional expression across narratives through innovative agent-based planning and region-aware generation.
...
TL;DR: StyleGallery is a training-free, semantic-aware framework for image style transfer that addresses semantic gaps, eliminates need for extra constraints, and enables adaptive global-local alignment using arbitrary style references.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based style transfer methods have three key limitations: 1) semantic gap where style references may lack proper content semantics, 2) reliance on extra constraints like semantic masks that restrict applicability, and 3) rigid feature associations that fail to balance fine-grained stylization with global content preservation.

Method: StyleGallery uses three core stages: 1) semantic region segmentation via adaptive clustering on latent diffusion features, 2) clustered region matching using block filtering on extracted features for precise alignment, and 3) style transfer optimization with energy function-guided diffusion sampling and regional style loss.

Result: Experiments on the introduced benchmark show StyleGallery outperforms state-of-the-art methods in content structure preservation, regional stylization, interpretability, and personalized customization, especially when using multiple style references.

Conclusion: StyleGallery provides a training-free, semantic-aware framework that addresses fundamental limitations of existing style transfer methods, enabling flexible use of arbitrary style references for personalized customization without extra constraints.
...
TL;DR: Proposes a unified framework to reduce MLLM hallucinations by synergistically using vision tokens in two ways: strengthening visual representations with augmented images and creating negative samples with pruned tokens to correct model biases.


<details>
  <summary>Details</summary>
Motivation: Current training-free methods for reducing MLLM hallucinations use separate strategies (enhancing vision or suppressing text) that are insufficient due to critical trade-offs, and their naive combination is ineffective, necessitating a unified framework.

Method: A unified framework focusing on vision tokens with two modules: Synergistic Visual Calibration (SVC) incorporates augmented image tokens to strengthen visual representations, and Causal Representation Calibration (CRC) uses pruned vision tokens to create latent-space negative samples for correcting internal model biases.
...
Conclusion: By harmonizing two distinct uses of vision tokens (augmenting for stronger representations and pruning for negative samples), the framework effectively restores vision-language balance and reduces hallucinations in MLLMs with minimal computational overhead.
...
TL;DR: GAE is a principled autoencoder framework for latent diffusion models that optimizes semantic supervision from vision foundation models, replaces KL-divergence with latent normalization, and uses dynamic noise sampling to achieve state-of-the-art image generation with better compression-semantic-reconstruction balance.


<details>
  <summary>Details</summary>
Motivation: Existing latent designs for diffusion models are heuristic and struggle to unify semantic discriminability, reconstruction fidelity, and latent compactness. Current approaches lack systematic optimization of these competing objectives.

Method: GAE constructs optimized low-dimensional semantic supervision from Vision Foundation Models, replaces KL-divergence with latent normalization for stable manifolds, and incorporates dynamic noise sampling for robust reconstruction under high-intensity noise.

Result: Achieves gFID of 1.82 at 80 epochs and 1.31 at 800 epochs on ImageNet-1K 256×256 without Classifier-Free Guidance, significantly surpassing existing state-of-the-art methods. Establishes superior balance between compression, semantic depth, and reconstruction stability.

Conclusion: GAE provides a principled paradigm for latent diffusion modeling that validates design considerations for optimizing semantic supervision, latent stability, and noise robustness, offering promising improvements over heuristic approaches.
...
TL;DR: MLLMs gain spatial awareness by autonomously deciding when to use geometric features, improving spatial reasoning without compromising 2D capabilities.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs have limited spatial understanding and existing methods inefficiently inject geometry into all inputs, creating unnecessary computation overhead.

Method: Two-stage approach: 1) Add independent geometry input channel with alignment training, 2) Use spatial-aware fine-tuning dataset to activate model's ability to autonomously determine when geometric features are needed.

Result: Significant improvements on multiple spatial reasoning benchmarks while maintaining 2D visual reasoning capabilities, achieving more efficient and robust multimodal intelligence.

Conclusion: The framework enables MLLMs to become self-aware of perceptual insufficiency and autonomously engage geometric features, advancing toward more efficient and intelligent multimodal systems.
...
TL;DR: Proposes OCpose, a new metric for multi-person pose estimation that evaluates all detected poses equally regardless of confidence scores, using optimal transport for fair TP-FP tradeoff.


<details>
  <summary>Details</summary>
Motivation: Current metrics focus too much on high-confidence poses and ignore low-confidence false positives, leading to inflated scores even when many low-confidence false positives are detected. Need fair evaluation that properly balances true-positive and false-positive poses.

Method: OCpose evaluates detected poses against pose annotations using optimal transportation. It equally evaluates all detected poses regardless of confidence scores, but uses confidence scores to improve reliability of matching scores between estimated poses and annotations.

Result: OCpose provides a different perspective assessment than other confidence ranking-based metrics by offering fair evaluation of the TP-FP tradeoff.

Conclusion: OCpose is a novel metric that addresses limitations of current confidence-based metrics by using optimal transport to fairly evaluate all detected poses, providing more balanced assessment of multi-person pose estimation performance.
...
TL;DR: Motion Forcing is a video generation framework that stabilizes the trilemma of visual quality, physical consistency, and controllability in complex scenes through hierarchical Point-Shape-Appearance decomposition and masked point recovery training.


<details>
  <summary>Details</summary>
Motivation: Current video generation models maintain balance between visual quality, physical consistency, and controllability only in simple scenarios, but this equilibrium breaks down in complex scenes involving collisions or dense traffic.

Method: Hierarchical Point-Shape-Appearance paradigm: 1) Model dynamics as sparse geometric anchors (Point), 2) Expand to dynamic depth maps resolving 3D geometry (Shape), 3) Render high-fidelity textures (Appearance). Uses Masked Point Recovery training strategy where random masking of input anchors forces model to learn latent physical laws.

Result: Significantly outperforms state-of-the-art baselines on autonomous driving benchmarks, maintaining trilemma stability across complex scenes. Evaluations on physics and robotics confirm framework's generality.

Conclusion: Motion Forcing successfully addresses the video generation trilemma in complex scenarios by explicitly decoupling physical reasoning from visual synthesis and forcing models to learn underlying physical laws through structured training.
...
Conclusion: The proposed decoupling strategy allows F2R to effectively address the limitations of existing self-supervised video denoising methods by separating temporal consistency modeling from spatial texture recovery, achieving superior performance.
...
TL;DR: TractoRC is a unified probabilistic framework that jointly performs tractogram registration and streamline clustering in diffusion MRI tractography, learning a shared latent embedding space to improve both tasks over independent approaches.


<details>
  <summary>Details</summary>
Motivation: Current tractography analysis typically performs tractogram registration (aligning streamlines across individuals) and streamline clustering (grouping streamlines into fiber bundles) independently, despite both tasks sharing the goal of capturing geometrically similar white matter structures. A unified approach could leverage complementary information between these tasks.

Method: TractoRC learns a latent embedding space for streamline points as a shared representation for both tasks. Registration learns anatomical landmarks as probabilistic keypoints to align tractograms, while clustering learns streamline structural prototypes for grouping. A transformation-equivariant self-supervised strategy learns geometry-aware and transformation-invariant embeddings.
...
Conclusion: TractoRC demonstrates that a unified probabilistic framework for joint tractogram registration and clustering outperforms independent approaches, providing a more effective way to characterize consistent white matter organization in diffusion MRI tractography.
...
### [26] [UniPINN: A Unified PINN Framework for Multi-task Learning of Diverse Navier-Stokes Equations](https://arxiv.org/abs/2603.10466)
*Dengdi Sun,Jie Chen,Xiao Wang,Jin Tang*

Main category: cs.CV

TL;DR: UniPINN is a unified multi-flow Physics-Informed Neural Network framework that addresses challenges in extending PINNs to multiple flow scenarios, achieving superior accuracy across heterogeneous flow regimes while mitigating negative transfer.


<details>
  <summary>Details</summary>
Motivation: Existing PINN approaches are designed for single-flow settings and face three key challenges when extended to multi-flow scenarios: difficulty capturing both shared physics and flow-specific characteristics, susceptibility to negative transfer, and unstable training from disparate loss magnitudes across different flow regimes.
...
Conclusion: UniPINN provides a unified framework for multi-flow PINN learning that addresses key limitations of existing approaches, enabling effective simultaneous learning across multiple flow scenarios with improved accuracy and stability.
...
TL;DR: World2Act aligns VLA actions with world model latents instead of pixels, uses LLM-based skill decomposition for consistent video generation, and improves real-world performance by 6.7%.
...
Method: World2Act framework aligns VLA actions directly with WM video-dynamics latents using contrastive matching objective. Also proposes automatic LLM-based skill-decomposition pipeline that segments high-level instructions into low-level prompts, creating RoboCasa-Skill and LIBERO-Skill datasets for skill-compositional WMs.

Result: Achieves state-of-the-art results on RoboCasa and LIBERO benchmarks when applied to VLAs like GR00T-N1.6 and Cosmos Policy. Improves real-world performance by 6.7%, enhancing embodied agent generalization.

Conclusion: World2Act reduces dependence on pixels by aligning actions with WM latents, and the skill-decomposition pipeline enables temporally consistent video generation across diverse task horizons, leading to improved robustness and generalization in embodied agents.
...
TL;DR: Benchmark study of 11 promptable foundation models for medical image segmentation reveals performance varies significantly across models, prompting strategies, and anatomical structures, with human prompts yielding worse results than ideal prompts.


<details>
  <summary>Details</summary>
Motivation: The proliferation of promptable foundation models for medical image segmentation makes direct performance comparison difficult and complicates model selection for specific clinical tasks due to varying evaluations across datasets, metrics, and compared models.
...
Conclusion: Selection of optimal foundation models for human-driven medical image segmentation remains challenging, with even high-performing models being sensitive to variations in human input prompts, suggesting performance reported on ideal prompts may overestimate real-world clinical utility.
...
TL;DR: Novel keyframe-driven training paradigm for sign language production that uses sparse keyframes to capture human signing kinematics, enabling fluid motion synthesis while avoiding regression-to-the-mean issues.


<details>
  <summary>Details</summary>
Motivation: Current SLP frameworks face trade-offs: text-to-pose models suffer from regression-to-the-mean effects, while dictionary-retrieval methods produce robotic, disjointed transitions. Need to generate natural, linguistically accurate sign language avatars.

Method: 1) FAST: ultra-efficient sign segmentation model for automatic temporal boundary mining. 2) SignSparK: large-scale Conditional Flow Matching framework using extracted keyframe anchors to synthesize 3D signing sequences in SMPL-X and MANO spaces. 3) Keyframe-driven formulation enables Keyframe-to-Pose generation for spatiotemporal editing. 4) Reconstruction-based CFM objective allows high-fidelity synthesis in <10 sampling steps. 5) Integration with 3D Gaussian Splatting for photorealistic rendering.

Result: Establishes largest multilingual SLP framework across four distinct sign languages. Achieves new state-of-the-art across diverse SLP tasks and multilingual benchmarks. Enables precise spatiotemporal editing of signing sequences through KF2P generation.

Conclusion: The proposed keyframe-driven paradigm resolves the trade-off in SLP frameworks, enabling natural, fluid sign language production while maintaining linguistic accuracy. The approach scales effectively across multiple languages and enables practical editing capabilities.
...
TL;DR: Novel language-guided framework using vision-language models for zero-shot defect detection in carbon fiber composites via active infrared thermography, eliminating need for training datasets.


<details>
  <summary>Details</summary>
Motivation: AI-based active infrared thermography for CFRP inspection requires time-consuming and expensive dataset creation for training neural networks. Need for more efficient approach that doesn't require extensive training data.

Method: Proposes language-guided framework using pretrained vision-language models with lightweight AIRT-VLM adapter to bridge domain gap between thermographic data and natural images. Uses VLMs like GroundingDINO, Qwen-VL-Chat, and CogVLM for zero-shot defect understanding and localization.

Result: AIRT-VLM adapter achieves >10 dB SNR gains over conventional thermographic methods and enables zero-shot defect detection with IoU reaching 70%. Validated on 25 CFRP inspection sequences with realistic impact defects.

Conclusion: Framework successfully enables zero-shot subsurface defect analysis in CFRPs using pretrained VLMs, eliminating need for expensive training datasets while maintaining high detection performance.
...
TL;DR: LCAMV method corrects lateral chromatic aberration and fuses multi-channel phase data for accurate 3D reconstruction of colored objects using single projector-camera setup.
...
Method: LCAMV analytically models and pixel-wise compensates lateral chromatic aberration in both projector and camera, then adaptively fuses multi-channel phase data using Poisson-Gaussian noise model and minimum-variance estimation.
...
TL;DR: WanderBench is the first open-access global geolocation benchmark with 32K+ panoramas across six continents, enabling embodied navigation actions. GeoAoT is a novel framework that couples reasoning with physical actions for interactive geolocation, outperforming 19 large multimodal models.
...
Method: Created WanderBench benchmark with navigable graphs enabling rotation and movement actions. Proposed GeoAoT (Action of Thought) framework that generates actionable plans (approaching landmarks, adjusting viewpoints) instead of textual reasoning chains to actively reduce uncertainty.

Result: GeoAoT achieves superior fine-grained localization and stronger generalization in dynamic environments compared to 19 large multimodal models. The framework establishes a new evaluation protocol measuring both accuracy and difficulty-aware questioning ability.

Conclusion: WanderBench and GeoAoT define a new paradigm for actionable, reasoning-driven geolocation in embodied visual understanding, transforming geolocation from static recognition to interactive exploration through coupling reasoning with physical actions.
...
TL;DR: ROVA is a training framework that improves vision-language model robustness against real-world disturbances by using difficulty-aware online training with robustness-aware consistency rewards, evaluated on new benchmark PVRBench.
...
Conclusion: ROVA effectively addresses the robustness gap in vision-language models through adaptive training with robustness-aware consistency rewards, demonstrating significant improvements in both perturbed and clean evaluation settings.
...
TL;DR: CIPHER is a training-free method that reduces hallucinations in large vision-language models by using counterfactual image perturbations to identify and suppress vision-induced hallucination patterns in feature space.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models frequently generate hallucinations (unfaithful outputs misaligned with visual input), and existing training-free approaches primarily focus on text-induced hallucinations, leaving vision-induced hallucinations unaddressed.

Method: Two-phase approach: 1) Offline: Create OHC-25K counterfactual dataset with diffusion-edited images contradicting ground-truth captions, extract hallucination representations, identify low-rank subspace characterizing vision-induced hallucinations; 2) Inference: Project intermediate hidden states away from this hallucination subspace.
...
Conclusion: Counterfactual image perturbations provide an effective training-free approach to suppress vision-induced hallucinations in LVLMs by identifying and removing systematic hallucination patterns in feature space.
...
### [35] [RandMark: On Random Watermarking of Visual Foundation Models](https://arxiv.org/abs/2603.10695)
...
Method: Uses a small encoder-decoder network to embed digital watermarks into internal representations of a hold-out set of input images via random watermark embedding.

Result: Demonstrates low probability of false detection for non-watermarked models and low probability of false misdetection for watermarked models, both theoretically and experimentally.

Conclusion: Proposed watermarking approach provides effective ownership verification for visual foundation models while maintaining detection reliability.
...
### [36] [StructDamage:A Large Scale Unified Crack and Surface Defect Dataset for Robust Structural Damage Detection](https://arxiv.org/abs/2603.10484)
*Misbah Ijaz,Saif Ur Rehman Khan,Abd Ur Rehman,Sebastian Vollmer,Andreas Dengel,Muhammad Nabeel Asim*

Main category: cs.CV

TL;DR: StructDamage is a novel, large-scale dataset of 78,093 images across 9 surface types for crack detection, created by harmonizing 32 existing datasets, with baseline models achieving up to 98.62% accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing crack datasets lack geographic diversity, surface type variety, scale, and labeling consistency, making it difficult for trained algorithms to generalize effectively in real-world conditions. There's a need for a comprehensive, standardized dataset to support robust crack detection research.

Method: Created StructDamage by systematically aggregating, harmonizing, and reannotating images from 32 publicly available datasets covering concrete structures, asphalt pavements, masonry walls, bridges, and historic buildings. Organized images in folder-level classification hierarchy suitable for CNNs and Vision Transformers.

Result: Baseline classification results using 15 DL architectures from 6 model families showed strong performance, with 12 models achieving macro F1-scores over 0.96. DenseNet201 achieved the best performance with 98.62% accuracy.

Conclusion: StructDamage provides a comprehensive and versatile resource for crack damage detection research, designed to promote reproducible research and support the development and fair evaluation of robust approaches through thorough documentation and standard structure.
...
### [37] [UAV traffic scene understanding: A cross-spectral guided approach and a unified benchmark](https://arxiv.org/abs/2603.10722)
*Yu Zhang,Zhicheng Zhao,Ze Luo,Chenglong Li,Jin Tang*

Main category: cs.CV

TL;DR: CTCNet: Cross-spectral Traffic Cognition Network using optical-thermal fusion and traffic regulation knowledge for robust UAV traffic scene understanding, with new Traffic-VQA benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing UAV traffic monitoring methods degrade under adverse illumination (nighttime/fog) and lack domain-specific knowledge for complex traffic behavior assessment.

Method: CTCNet with two key modules: 1) Prototype-Guided Knowledge Embedding (PGKE) using Traffic Regulation Memory to inject domain knowledge, 2) Quality-Aware Spectral Compensation (QASC) for optical-thermal bidirectional feature compensation.

Result: Outperforms state-of-the-art methods in both cognition and perception scenarios, with new Traffic-VQA benchmark (8,180 image pairs, 1.3M QA pairs across 31 types).

Conclusion: CTCNet enables robust UAV traffic scene understanding in complex environments by combining cross-spectral fusion with domain knowledge embedding, addressing limitations of optical-only approaches.
...
### [38] [Spatial self-supervised Peak Learning and correlation-based Evaluation of peak picking in Mass Spectrometry Imaging](https://arxiv.org/abs/2603.10487)
*Philipp Weigand,Nikolas Ebert,Shad A. Mohammed,Denis Abu Sammour,Carsten Hopf,Oliver Wasenmüller*

Main category: cs.CV

TL;DR: Autoencoder-based spatial self-supervised neural network for peak picking in mass spectrometry imaging that outperforms existing methods by selecting spatially structured peaks using expert-annotated segmentation masks for evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing peak picking methods for MSI are inconsistent across heterogeneous datasets and their evaluation is limited to synthetic data or manually selected ion images, failing to represent real-world challenges.

Method: Proposed autoencoder-based spatial self-supervised peak learning neural network that selects spatially structured peaks by learning an attention mask leveraging both spatial and spectral information, with evaluation based on expert-annotated segmentation masks.
...
Conclusion: The spatial self-supervised network provides value over contemporary methods, and the evaluation procedure offers a consistent framework for comparing spatially structured peak picking methods across different datasets.
...
### [39] [Taking Shortcuts for Categorical VQA Using Super Neurons](https://arxiv.org/abs/2603.10781)
*Pierre Musacchio,Jaeyi Jeong,Dahun Kim,Jaesik Park*

Main category: cs.CV

TL;DR: Super Neurons (SNs) use scalar activations from early VLM layers as classifiers, enabling 5.10x speedup with improved accuracy compared to attention-based methods.


<details>
  <summary>Details</summary>
Motivation: To find more efficient and accurate training-free alternatives to Sparse Attention Vectors (SAVs) for Vision Language Models by exploring scalar activations instead of attention vectors.

Method: Probe raw scalar activations in VLMs, identify discriminative neurons (Super Neurons) in shallow layers, and use them for early exiting classification at the first generated token.

Result: SNs achieve up to 5.10x speedup while improving classification performance compared to original networks, with enough discriminative neurons found in the first layer.

Conclusion: Scalar activations (Super Neurons) provide superior training-free classification for VLMs, enabling extreme early exiting with both speed and accuracy improvements over attention-based methods.
...
### [40] [IMTBench: A Multi-Scenario Cross-Modal Collaborative Evaluation Benchmark for In-Image Machine Translation](https://arxiv.org/abs/2603.10495)
...
Result: Benchmarked commercial cascade systems and multi-modal models, revealing large performance gaps across scenarios and languages, especially on natural scenes and resource-limited languages, showing substantial headroom for improvement.

Conclusion: IMTBench establishes a standardized benchmark to accelerate progress in end-to-end in-image machine translation by addressing real-world complexity and introducing comprehensive multi-modal evaluation.
...
TL;DR: BALD-SAM introduces active prompting for SAM using Bayesian uncertainty to select optimal spatial prompts, achieving state-of-the-art cross-domain segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Real-world annotation workflows require iterative refinement where annotators strategically place prompts based on model outputs. Current approaches rely on visual assessment rather than principled methods to identify the most informative regions for next prompts.
...
Result: Achieves strong cross-domain performance across 16 datasets spanning natural, medical, underwater, and seismic domains, ranking first or second on 14 of 16 benchmarks. Surpasses human prompting and in several categories even oracle prompting, while consistently outperforming one-shot baselines, particularly on thin and structurally complex objects.

Conclusion: BALD-SAM establishes a principled active prompting framework that increases annotation efficiency by strategically selecting information-rich regions, demonstrating practical Bayesian uncertainty estimation for large foundation models with strong cross-domain generalization.
...
### [42] [UHD Image Deblurring via Autoregressive Flow with Ill-conditioned Constraints](https://arxiv.org/abs/2603.10517)
*Yucheng Xin,Dawei Zhao,Xiang Chen,Chen Wu,Pu Wang,Dianjie Lu,Guijuan Zhang,Xiuyi Jia,Zhuoran Zheng*

Main category: cs.CV

TL;DR: Proposes an autoregressive flow method with ill-conditioned constraint for UHD image deblurring, balancing detail recovery and inference efficiency through progressive coarse-to-fine refinement.


<details>
  <summary>Details</summary>
Motivation: UHD image deblurring faces challenges in balancing fine-grained detail recovery with practical inference efficiency. Current discriminative and generative methods struggle with the trade-off between computational cost and detail generation for UHD tasks.

Method: Decomposes UHD restoration into progressive coarse-to-fine process: sharp estimate formed by upsampling previous-scale result plus current-scale residual. Uses Flow Matching to model residual generation as conditional vector field with few-step ODE sampling. Introduces ill-conditioning suppression scheme with condition-number regularization on feature-induced attention matrix.

Result: Demonstrates promising performance on blurred images at 4K (3840×2160) or higher resolutions, achieving stable stage-wise refinement while maintaining affordable inference.

Conclusion: The proposed autoregressive flow method with ill-conditioned constraint effectively addresses UHD image deblurring challenges by enabling progressive refinement while controlling computational cost and numerical stability.
...
### [43] [On the Reliability of Cue Conflict and Beyond](https://arxiv.org/abs/2603.10834)
*Pum Jun Kim,Seung-Ah Lee,Seongho Park,Dongyoon Han,Jaejun Yoo*

Main category: cs.CV

TL;DR: REFINED-BIAS is a new framework that addresses limitations in current shape-texture bias evaluation methods, providing more reliable and interpretable diagnosis of neural network visual cue preferences.
...
Result: The framework enables fairer cross-model comparisons, more faithful diagnosis of shape and texture biases, and clearer empirical conclusions, resolving inconsistencies that prior cue-conflict evaluations could not reliably disambiguate across diverse training regimes and architectures.

Conclusion: REFINED-BIAS provides a more reliable and interpretable evaluation framework for shape-texture bias diagnosis that addresses fundamental limitations in current methods, leading to better understanding of neural network decision processes.
...
### [44] [Visually-Guided Controllable Medical Image Generation via Fine-Grained Semantic Disentanglement](https://arxiv.org/abs/2603.10519)
...
<details>
  <summary>Details</summary>
Motivation: Medical image synthesis faces challenges due to modality gaps between complex visual details and abstract clinical text, and semantic entanglement where coarse-grained text embeddings blur anatomical structures and imaging styles, weakening generation controllability.
...
### [45] [Sparse Task Vector Mixup with Hypernetworks for Efficient Knowledge Transfer in Whole-Slide Image Prognosis](https://arxiv.org/abs/2603.10526)
*Pei Liu,Xiangxiang Zeng,Tengfei Ma,Yucheng Xing,Xuanbai Ren,Yiping Liu*

Main category: cs.CV

TL;DR: STEPH uses sparse task vector mixup with hypernetworks to efficiently transfer prognostic knowledge from multiple cancers to a target cancer without large-scale joint training.


<details>
  <summary>Details</summary>
Motivation: Cancer-specific learning suffers from scarce training samples per cancer type, limiting model generalization on heterogeneous tumors. Existing multi-cancer approaches require computationally expensive joint training or multi-model inference.

Method: Proposes Sparse Task Vector Mixup with Hypernetworks (STEPH): 1) applies task vector mixup between each source cancer and target cancer, 2) sparsely aggregates these mixtures using hypernetworks to obtain improved target model via model merging.
...
Conclusion: STEPH provides an efficient solution for transferring prognostic knowledge across cancers without requiring large-scale joint training or extensive multi-model inference, addressing data scarcity and computational challenges in WSI analysis.
...
### [46] [DSFlash: Comprehensive Panoptic Scene Graph Generation in Realtime](https://arxiv.org/abs/2603.10538)
*Julian Lorenz,Vladyslav Kovganko,Elias Kohout,Mrunmai Phatak,Daniel Kienzle,Rainer Lienhart*

Main category: cs.CV

TL;DR: DSFlash is a fast, efficient panoptic scene graph generation model that achieves 56 FPS on RTX 3090 while maintaining SOTA performance and requiring minimal training resources.


<details>
  <summary>Details</summary>
Motivation: Existing Scene Graph Generation (SGG) methods lack practical deployment capabilities for real-world applications, especially on resource-constrained edge devices, where speed and efficiency are crucial but have received limited research attention.

Method: Introduces DSFlash, a low-latency model for panoptic scene graph generation designed to overcome computational limitations while maintaining comprehensive relationship extraction beyond just salient relationships.

Result: Achieves 56 frames per second on RTX 3090 GPU without compromising performance against state-of-the-art methods, computes comprehensive scene graphs, and requires less than 24 hours to train on a single GTX 1080 GPU.

Conclusion: DSFlash provides an accessible, efficient solution for researchers and practitioners with limited computational resources, enabling practical deployment of SGG models for specialized applications while maintaining rich contextual information.
...
### [47] [GroundCount: Grounding Vision-Language Models with Object Detection for Mitigating Counting Hallucinations](https://arxiv.org/abs/2603.10978)
*Boyuan Chen,Minghao Shao,Siddharth Garg,Ramesh Karri,Muhammad Shafique*

Main category: cs.CV

TL;DR: GroundCount framework improves VLM counting accuracy by 6.6pp using object detection models for spatial grounding, reducing hallucinations and inference time.


<details>
  <summary>Details</summary>
Motivation: VLMs have persistent hallucinations in counting tasks with much lower accuracy than other visual reasoning tasks, while object detection models excel at spatial localization and counting with minimal overhead.

Method: Propose GroundCount framework that augments VLMs with explicit spatial grounding from object detection models (like YOLO) using prompt-based augmentation strategies and structured prompts.

Result: Achieves 81.3% counting accuracy (6.6pp improvement) on best model, reduces inference time by 22%, positional encoding helps stronger models but hurts weaker ones, confidence scores add noise, explicit symbolic grounding outperforms implicit feature fusion.

Conclusion: Counting failures stem from fundamental spatial-semantic integration limitations in VLMs, not architecture-specific issues; architectural compatibility is crucial for augmentation strategies.
...
### [48] [Does AI See like Art Historians? Interpreting How Vision Language Models Recognize Artistic Style](https://arxiv.org/abs/2603.11024)
...
<details>
  <summary>Details</summary>
Motivation: While VLMs demonstrate increasing proficiency in art analysis tasks, there's a need to understand whether their mechanisms for predicting artistic style align with the criteria and reasoning processes used by art historians.

Method: Used latent-space decomposition approach to identify concepts driving art style prediction, combined with quantitative evaluations, causal analysis, and assessment by art historians.

Result: 73% of extracted concepts were judged by art historians to exhibit coherent and semantically meaningful visual features, and 90% of concepts used to predict style were judged relevant. When irrelevant concepts successfully predicted style, art historians identified alternative explanations like formal understanding of visual features.

Conclusion: VLMs demonstrate meaningful alignment with art historical reasoning in style prediction, though some concepts may operate through different formal mechanisms than human experts use, suggesting both convergence and divergence in how AI and humans analyze artistic style.
...
### [49] [P-GSVC: Layered Progressive 2D Gaussian Splatting for Scalable Image and Video](https://arxiv.org/abs/2603.10551)
*Longan Wang,Yuang Shi,Wei Tsang Ooi*

Main category: cs.CV

TL;DR: P-GSVC is a progressive 2D Gaussian splatting framework with layered representation and joint training for scalable image/video reconstruction.
...
Conclusion: P-GSVC provides an effective progressive Gaussian splatting framework with joint training that enables scalable reconstruction for both images and videos.
...
TL;DR: V2M-Zero generates time-aligned music for videos without paired training data by using intra-modal event curves to capture shared temporal structure.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-music models lack fine-grained temporal control for aligning music with video events, requiring a solution that doesn't rely on paired video-music data.

Method: Compute event curves from intra-modal similarity using pretrained encoders, fine-tune text-to-music model on music-event curves, then substitute video-event curves at inference without cross-modal training.

Result: Achieves 5-21% higher audio quality, 13-15% better semantic alignment, 21-52% improved temporal synchronization, and 28% higher beat alignment on dance videos compared to paired-data baselines.

Conclusion: Temporal alignment through within-modality features is more effective than paired cross-modal supervision for video-to-music generation, validated by objective metrics and subjective listening tests.
...
### [51] [PET-F2I: A Comprehensive Benchmark and Parameter-Efficient Fine-Tuning of LLMs for PET/CT Report Impression Generation](https://arxiv.org/abs/2603.10560)
*Yuchen Liu,Wenbo Zhang,Liling Peng,Yichi Zhang,Yu Fu,Xin Guo,Chao Qu,Yuan Qi,Le Xue*

Main category: cs.CV

TL;DR: Researchers create PET-F2I-41K benchmark for PET/CT impression generation, evaluate 27 LLMs, and develop PET-F2I-7B model that outperforms all baselines with specialized clinical metrics.


<details>
  <summary>Details</summary>
Motivation: PET/CT imaging requires labor-intensive summarization of complex findings into diagnostic impressions, and LLM capabilities in this specialized domain remain underexplored despite their promise in medical text generation.

Method: Created PET-F2I-41K benchmark from 41k+ real-world reports, evaluated 27 models (frontier LLMs, open-source generalists, medical-domain LLMs), developed PET-F2I-7B by fine-tuning Qwen2.5-7B-Instruct with LoRA, and introduced three clinical metrics (ECR, UER, FCR) beyond standard NLG metrics.
...
Conclusion: The work establishes PET-F2I-41K as a standardized evaluation framework to accelerate development of reliable, clinically deployable PET/CT reporting systems, demonstrating that domain-adapted models significantly outperform general-purpose LLMs in this specialized medical task.
...
### [52] [LiTo: Surface Light Field Tokenization](https://arxiv.org/abs/2603.11047)
...
<details>
  <summary>Details</summary>
Motivation: Prior works focus on either 3D geometry reconstruction or view-independent diffuse appearance, struggling to capture realistic view-dependent effects like specular highlights and Fresnel reflections.

Method: Encode random subsamples of RGB-depth surface light fields into compact latent vectors, creating unified 3D latent space for both geometry and appearance. Train latent flow matching model conditioned on single input images.

Result: Achieves higher visual quality and better input fidelity than existing methods, reproducing view-dependent effects under complex lighting and generating 3D objects with appearance consistent with input lighting/materials.

Conclusion: Proposed approach successfully creates unified 3D latent representation for joint geometry and view-dependent appearance modeling, enabling realistic view-dependent effects and single-image conditioned generation.
...
### [53] [UniStitch: Unifying Semantic and Geometric Features for Image Stitching](https://arxiv.org/abs/2603.10568)
*Yuan Mei,Lang Nie,Kang Liao,Yunqiu Xu,Chunyu Lin,Bin Xiao*

Main category: cs.CV

TL;DR: UniStitch unifies traditional geometric features and learning-based semantic features for image stitching via Neural Point Transformer and Adaptive Mixture of Experts modules, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Traditional image stitching uses hand-crafted geometric features while recent learning-based methods use semantic features, but these approaches have evolved separately without meaningful convergence. The paper aims to bridge this gap by unifying both feature types.

Method: Proposes UniStitch framework with two key modules: 1) Neural Point Transformer (NPT) transforms sparse geometric keypoints into dense semantic maps, and 2) Adaptive Mixture of Experts (AMoE) dynamically fuses geometric and semantic representations, shifting focus to more reliable features.

Result: UniStitch significantly outperforms existing state-of-the-art methods by a large margin, demonstrating that unified multimodal features deliver superior performance over single-feature approaches.

Conclusion: The work pioneers a unified paradigm between traditional and learning-based image stitching, showing that combining geometric and semantic features through proper transformation and adaptive fusion leads to substantial performance gains in complex scenes.
...
### [54] [COMIC: Agentic Sketch Comedy Generation](https://arxiv.org/abs/2603.11048)
*Susung Hong,Brian Curless,Ira Kemelmacher-Shlizerman,Steve Seitz*

Main category: cs.CV

TL;DR: AI system creates SNL-style comedy sketches using agent-based competition and LLM critics trained on YouTube comedy data


<details>
  <summary>Details</summary>
Motivation: To automate the creation of professional-quality comedic videos similar to sketch shows like Saturday Night Live, which traditionally requires extensive human creative effort and production resources

Method: Uses a population of agents based on real production studio roles that compete and collaborate through iterative cycles. Key innovation: LLM critics trained on YouTube comedy video corpus to automatically evaluate humor quality based on real viewer preferences

Result: System produces comedy sketches approaching professional quality while achieving state-of-the-art performance in video generation

Conclusion: The framework successfully automates comedy sketch production through agent-based competition and data-driven humor evaluation, demonstrating potential for AI-generated entertainment content
...
### [55] [R4-CGQA: Retrieval-based Vision Language Models for Computer Graphics Image Quality Assessment](https://arxiv.org/abs/2603.10578)
*Zhuangzi Li,Jian Jin,Shilv Cai,Weisi Lin*

Main category: cs.CV

TL;DR: This paper addresses CG quality assessment challenges by creating a dataset with systematic quality descriptions and proposing a retrieval-augmented framework to enhance VLM performance on fine-grained CG quality evaluation.


<details>
  <summary>Details</summary>
Motivation: Two main challenges exist: 1) existing CG datasets lack systematic quality descriptions, and 2) current CG quality assessment methods cannot provide reasonable text-based explanations for quality judgments.

Method: First identified six key perceptual dimensions of CG quality, constructed a dataset of 3500 CG images with quality descriptions covering style, content, and perceived quality. Then proposed a two-stream retrieval framework using retrieval-augmented generation to enhance VLM performance by leveraging descriptions of visually similar images.

Result: Found that current VLMs are not sufficiently accurate for fine-grained CG quality assessment, but descriptions of visually similar images significantly improve VLM understanding. The proposed retrieval-augmented framework substantially improves VLM performance on CG quality assessment tasks.

Conclusion: The paper successfully addresses CG quality assessment limitations by providing a systematic dataset and demonstrating that retrieval-augmented generation can effectively enhance VLM capabilities for fine-grained CG quality evaluation.
...
### [56] [Attribution as Retrieval: Model-Agnostic AI-Generated Image Attribution](https://arxiv.org/abs/2603.10583)
*Hongsong Wang,Renxi Cheng,Chaolei Han,Jie Gui*

Main category: cs.CV

TL;DR: LIDA is a model-agnostic framework for AI-generated image attribution that treats it as an instance retrieval problem, achieving SOTA performance in zero- and few-shot settings.


<details>
  <summary>Details</summary>
Motivation: Traditional image forensics methods can't handle increasingly realistic AI-generated images. Existing attribution methods are model-dependent, requiring access to generative models and lacking generality for new/unseen generators.

Method: Formulates attribution as instance retrieval rather than classification. Uses Low-Bit Fingerprint Generation for input, followed by Unsupervised Pre-Training and Few-Shot Attribution Adaptation. The LIDA framework is model-agnostic.
...
Conclusion: LIDA provides a scalable, generalizable solution to AI-generated image attribution that doesn't require access to generative models, addressing key limitations of existing methods.
...
### [57] [Need for Speed: Zero-Shot Depth Completion with Single-Step Diffusion](https://arxiv.org/abs/2603.10584)
*Jakub Gregorek,Paraskevas Pegios,Nando Metzger,Konrad Schindler,Theodora Kontogianni,Lazaros Nalpantidis*

Main category: cs.CV

TL;DR: Marigold-SSD is a single-step depth completion framework that uses diffusion priors without test-time optimization, achieving fast inference with only 4.5 GPU days training cost.


<details>
  <summary>Details</summary>
Motivation: To enable efficient and robust 3D perception under real-world latency constraints by eliminating the costly test-time optimization typically associated with diffusion-based depth completion methods.

Method: Single-step, late-fusion depth completion framework that leverages strong diffusion priors while shifting computational burden from inference to finetuning, eliminating test-time optimization.
...
Conclusion: Marigold-SSD provides an efficient diffusion-based depth completion solution that challenges common evaluation protocols by analyzing performance under varying input sparsity levels, making diffusion methods more practical for real-world applications.
...
TL;DR: LTD detects synthetic images by analyzing inter-layer consistency differences in latent representations, outperforming existing methods in accuracy and generalization.
...
Method: Proposes latent transition discrepancy (LTD) that captures inter-layer consistency differences - real images maintain consistent semantic attention and structural coherence across network layers, while synthetic ones show distinct patterns. LTD adaptively identifies discriminative layers and assesses transition discrepancies.
...
Conclusion: LTD provides an effective approach for synthetic image detection by leveraging inter-layer consistency patterns in latent representations, addressing generalization limitations of existing methods.
...
TL;DR: HyPER-GAN is a lightweight GAN for real-time photorealism enhancement of synthetic data, using hybrid training with real-world patches to improve visual quality and semantic consistency.
...
Method: HyPER-GAN uses a U-Net-style generator for image-to-image translation, trained with paired synthetic/photorealism-enhanced images plus a hybrid strategy incorporating matched patches from real-world data.
...
Conclusion: HyPER-GAN provides an effective lightweight solution for real-time photorealism enhancement of synthetic data, with hybrid training strategy proven beneficial for visual quality and semantic preservation.
...
TL;DR: Splat2Real: Novel-view scaling for monocular depth pretraining using imitation learning from 3DGS-rendered digital twins, with CN-Coverage curriculum for stable performance.


<details>
  <summary>Details</summary>
Motivation: Physical AI systems face viewpoint shift between training and deployment, requiring novel-view robustness for monocular RGB-to-3D perception. Current methods struggle with stability when scaling novel views.

Method: Real2Render2Real pretraining: student depth network imitates expert metric depth/visibility from scene mesh rendered by 3DGS. CN-Coverage curriculum greedily selects views by geometry gain + extrapolation penalty, with quality-aware guardrail fallback for unreliable teachers.

Result: Across 20 TUM RGB-D sequences: naive scaling unstable, CN-Coverage mitigates worst-case regressions, GOL-Gated CN-Coverage provides strongest medium-high-budget stability with lowest high-novelty tail error. Downstream control-proxy shows embodied-relevance via safety/progress trade-off shifts.

Conclusion: Novel-view scaling quality (which views added) matters more than raw view count. CN-Coverage curriculum enables stable monocular depth pretraining for viewpoint-robust physical AI systems.
...
TL;DR: SLiM is a unified skeleton-based action representation learning framework that combines masked modeling with contrastive learning using a shared encoder, eliminating the need for computationally heavy decoders while achieving state-of-the-art performance with 7.89x inference efficiency gains.


<details>
  <summary>Details</summary>
Motivation: Current skeleton-based action representation learning faces limitations: Contrastive Learning (CL) overlooks fine-grained local details, while Masked Auto-Encoder (MAE) suffers from computationally heavy decoders and severe computational asymmetry between pre-training and downstream tasks.

Method: Proposes SLiM (Skeleton Less is More), a unified framework that harmonizes masked modeling with contrastive learning via a shared encoder without reconstruction decoder. Introduces semantic tube masking to prevent trivial reconstruction from skeletal-temporal correlation, and skeletal-aware augmentations for anatomical consistency across temporal granularities.
...
Conclusion: SLiM successfully resolves the bottlenecks of both CL and MAE paradigms by eliminating computational redundancy while compelling the encoder to capture discriminative features directly, establishing a new efficient and effective approach for skeleton-based action representation learning.
...
TL;DR: Systematic analysis of embedding design in Geospatial Foundation Models for Earth observation workflows, showing how backbone architecture, pretraining, representation depth, spatial aggregation, and combination affect performance and scalability.


<details>
  <summary>Details</summary>
Motivation: Earth observation produces massive multispectral imagery analyzed with GeoFMs. As these models increasingly serve as feature extractors providing task-agnostic embeddings, understanding trade-offs in representation extraction, aggregation, and combination is crucial for scalable embedding-based workflows where compact embeddings can replace raw data.

Method: Systematic analysis using NeuCo-Bench to study how backbone architecture (transformers vs ResNet), pretraining strategy, representation depth, spatial aggregation, and representation combination influence EO task performance. Aggregating embeddings into fixed-size representations 500x smaller than raw input data.

Result: Consistent trends across models: transformer backbones with mean pooling provide strong default embeddings; intermediate ResNet layers can outperform final layers; self-supervised objectives show task-specific strengths; combining embeddings from different objectives improves robustness.

Conclusion: Understanding embedding design trade-offs in GeoFM-based workflows enables scalable Earth observation pipelines where compact, task-agnostic representations can effectively replace raw data while maintaining broad utility across downstream tasks.
...
TL;DR: A²-Edit is a unified inpainting framework that enables arbitrary object replacement using coarse masks, supported by a large-scale multi-category dataset and novel architectural components for cross-category generalization.


<details>
  <summary>Details</summary>
Motivation: Existing inpainting methods suffer from severe homogenization and limited category coverage, making them inadequate for arbitrary object editing tasks where users need to replace objects across diverse categories with only coarse masks.

Method: The approach includes: 1) Construction of UniEdit-500K dataset with 8 major categories and 209 subcategories; 2) Mixture of Transformer module for differentiated modeling through dynamic expert selection; 3) Mask Annealing Training Strategy (MATS) that progressively relaxes mask precision during training.

Result: Extensive experiments on VITON-HD and AnyInsertion benchmarks show that A²-Edit consistently outperforms existing approaches across all metrics, demonstrating superior performance in arbitrary object editing tasks.

Conclusion: A²-Edit provides an efficient solution for arbitrary object editing by addressing dataset limitations through UniEdit-500K, enabling cross-category generalization via Mixture of Transformer, and improving robustness with mask annealing training.
...
<details>
  <summary>Details</summary>
Motivation: To enhance CNN robustness to image occlusions by drawing inspiration from how the visual cortex handles border completion problems, addressing the challenge of recognizing objects when parts are obscured.

Method: Designs CNN filters based on mathematical modeling of border completion in the visual cortex, creating BorderNet architecture tested on occluded datasets (MNIST, Fashion-MNIST, EMNIST) with stripe and grid occlusions.

Result: BorderNet demonstrates improved performance across all tested datasets, with performance gains varying based on occlusion severity and dataset characteristics.

Conclusion: Modeling visual cortex border completion mechanisms can effectively enhance CNN robustness to image occlusions, providing a biologically-inspired approach to improving computer vision systems.
...
TL;DR: Lifelong imitation learning framework using multimodal latent space representations and incremental feature adjustment for continual policy refinement across sequential tasks with reduced forgetting.
...
Method: Operates in multimodal latent space storing compact visual, linguistic, and robot state representations; uses incremental feature adjustment with angular margin constraint to regularize task embedding evolution.

Result: Achieves 10-17 point gains in AUC and up to 65% less forgetting compared to previous leading methods on LIBERO benchmarks; ablation studies confirm component effectiveness.

Conclusion: The framework establishes new state-of-the-art in lifelong imitation learning by effectively addressing catastrophic forgetting through multimodal latent representations and feature regularization.
...
TL;DR: DynVLA is a driving VLA model that uses Dynamics CoT - forecasting compact world dynamics before action generation for more informed, physically grounded decision-making in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Existing CoT paradigms have limitations: Textual CoT lacks fine-grained spatiotemporal understanding, while Visual CoT introduces substantial redundancy due to dense image prediction. There's a need for a more compact, interpretable, and efficient way to model world dynamics for driving decisions.

Method: Introduces Dynamics CoT paradigm with a Dynamics Tokenizer that compresses future evolution into a small set of dynamics tokens. Decouples ego-centric and environment-centric dynamics for accurate modeling. Trains model using SFT and RFT to generate dynamics tokens before actions.

Result: Outperforms Textual CoT and Visual CoT methods on NAVSIM, Bench2Drive, and large-scale in-house datasets. Demonstrates improved decision quality while maintaining latency-efficient inference.

Conclusion: Dynamics CoT provides a compact, interpretable, and efficient way to capture world evolution for driving decisions, offering practical value for autonomous driving systems.
...
### [67] [UniCom: Unified Multimodal Modeling via Compressed Continuous Semantic Representations](https://arxiv.org/abs/2603.10702)
*Yaqi Zhao,Wang Lin,Zijian Zhang,Miles Yang,Jingyuan Chen,Wentao Zhang,Zhao Zhong,Liefeng Bo*

Main category: cs.CV

TL;DR: UniCom is a unified multimodal framework that uses compressed continuous representations instead of discrete tokens, achieving better visual understanding and generation through channel dimension reduction and attention-based compression.


<details>
  <summary>Details</summary>
Motivation: Current unified multimodal models face a dilemma: discrete visual tokenizers lose fine-grained semantic information, while continuous representations are challenging for high-dimensional generative modeling, causing slow convergence and instability.

Method: UniCom uses compressed continuous representations with channel dimension reduction (more effective than spatial downsampling), an attention-based semantic compressor to distill dense features into compact unified representations, and a transfusion architecture that outperforms query-based designs.

Result: UniCom achieves state-of-the-art generation performance among unified models, delivers exceptional controllability in image editing, and maintains image consistency without relying on VAE.

Conclusion: The UniCom framework successfully harmonizes multimodal understanding and generation by using compressed continuous representations, overcoming the limitations of both discrete tokenizers and traditional continuous representation approaches.
...
### [68] [WalkGPT: Grounded Vision-Language Conversation with Depth-Aware Segmentation for Pedestrian Navigation](https://arxiv.org/abs/2603.10703)
*Rafi Ibn Sultan,Hui Zhu,Xiangyu Zhou,Chengyin Li,Prashant Khanduri,Marco Brocanelli,Dongxiao Zhu*

Main category: cs.CV

TL;DR: WalkGPT is a pixel-grounded vision-language model that generates conversational navigation guidance with segmentation masks and depth estimation for accessibility, outperforming existing models on the new PAVE benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing Large Vision-Language Models struggle with reliable depth reasoning and object hallucinations when analyzing complex urban scenes for pedestrian navigation, limiting their usefulness for accessibility guidance.

Method: WalkGPT unifies language reasoning and segmentation in a single architecture with Multi-Scale Query Projector (MSQP) for hierarchical token aggregation and Calibrated Text Projector (CTP) with Region Alignment Loss for segmentation-aware language embeddings.
...
Conclusion: WalkGPT enables fine-grained grounding and depth inference for complete, realistic navigation guidance without requiring user-provided cues, advancing accessible pedestrian navigation through unified vision-language segmentation.
...
### [69] [eLasmobranc Dataset: An Image Dataset for Elasmobranch Species Recognition and Biodiversity Monitoring](https://arxiv.org/abs/2603.10724)
...
TL;DR: Researchers created a public image dataset of 7 elasmobranch species from Spanish Mediterranean coast to support fine-grained species identification and conservation monitoring.


<details>
  <summary>Details</summary>
Motivation: Existing visual datasets for elasmobranchs (sharks and rays) are inadequate for fine-grained morphological classification - they're mostly detection-oriented, underwater-acquired, or limited to coarse categories, which hinders conservation efforts like Important Shark and Ray Areas (ISRAs) initiatives.

Method: Created the eLasmobranc Dataset by collecting images from field campaigns, local fish markets, projects, and open-access sources, with standardized protocols for clear visualization of diagnostic morphological traits outside aquatic environments. Includes expert-validated species annotations, spatial/temporal metadata, and species-level information.

Result: A curated, publicly available dataset of 7 ecologically relevant elasmobranch species from the eastern Spanish Mediterranean coast (region with two ISRAs), designed specifically for supervised species-level classification, population studies, and AI system development for biodiversity monitoring.

Conclusion: The dataset addresses a critical gap in fine-grained elasmobranch identification by combining morphological clarity, taxonomic reliability, and public accessibility, promoting reproducible research in conservation-oriented computer vision for threatened shark and ray species.
...
### [70] [Just-in-Time: Training-Free Spatial Acceleration for Diffusion Transformers](https://arxiv.org/abs/2603.10744)
...
Method: JiT introduces spatially approximated generative ODE that evolves full latent state using computations from dynamically selected sparse anchor tokens. Uses deterministic micro-flow ODE to maintain structural coherence and statistical correctness when incorporating new tokens to expand latent state dimensions.
...
Conclusion: JiT addresses spatial redundancy in diffusion transformers through dynamic token selection and micro-flow ODE, enabling dramatic speedups while maintaining quality, representing a breakthrough in practical deployment of diffusion models.
...
TL;DR: Event-based photometric stereo using single moving light source and neural network for surface normal estimation without calibration.
...
Result: Achieves 7.12% reduction in mean angular error compared to existing event-based methods, with robustness in sparse event regions, strong ambient illumination, and specular scenes.

Conclusion: Event-based photometric stereo with moving light source and neural network provides more compact, scalable, and robust solution for surface normal estimation in challenging real-world conditions.
...
TL;DR: Scaling perception outperforms scaling reasoning for STEM visual reasoning in MLLMs. Code serves as a precise perceptual medium, enabled by ICC-1M dataset and STEM2Code-Eval benchmark.


<details>
  <summary>Details</summary>
Motivation: To determine whether failures in STEM visual reasoning by MLLMs stem from perceptual deficiencies or reasoning limitations, and to address perception as the primary bottleneck.

Method: Systematic scaling analysis of perception vs. reasoning components; creation of ICC-1M dataset with Image-Caption-Code triplets using Code-Grounded Caption Generation and STEM Image-to-Code Translation; introduction of STEM2Code-Eval benchmark for direct perception evaluation.

Result: Scaling perception consistently outperforms scaling reasoning, revealing perception as the true limiting factor. Code serves as an effective perceptual medium that aligns with structured STEM visuals and eliminates hallucinations.

Conclusion: Perception is the key bottleneck in current STEM visual reasoning for MLLMs, and using code as a perceptual medium with executable semantics provides a more precise and verifiable approach to enhance visual comprehension.
...
TL;DR: CDG replaces CFG's null prompt with a degraded condition to improve compositional accuracy in text-to-image models by creating "good vs. almost good" discrimination instead of "good vs. null" contrast.


<details>
  <summary>Details</summary>
Motivation: Classifier-Free Guidance (CFG) relies on a semantically vacuous null prompt that causes geometric entanglement and limits precision in complex compositional tasks, leading to well-documented failures in text-to-image generation.

Method: Proposes Condition-Degradation Guidance (CDG) which replaces the null prompt with a strategically degraded condition (c_deg). Leverages the observation that transformer text encoder tokens split into content tokens (object semantics) and context-aggregating tokens (global context), and selectively degrades only the content tokens without external models or training.
...
Conclusion: Challenges the reliance on static, information-sparse negative samples and establishes a new principle for diffusion guidance: adaptive, semantically-aware negative samples are critical for precise semantic control in text-to-image generation.
...
TL;DR: LGA-RCM-YOLO improves phase-interface segmentation in transparent glassware using local-global attention and boundary refinement, achieving state-of-the-art performance on the CTG 2.0 dataset for laboratory automation.
...
Method: Propose LGA-RCM-YOLO based on YOLO11m-seg, combining Local-Global Attention (LGA) for robust semantic representation and Rectangular Self-Calibration Module (RCM) for boundary refinement of thin, elongated interfaces.

Result: Achieves 84.4% AP@0.5 and 58.43% AP@0.5-0.95 on CTG 2.0 dataset, improving over YOLO11m baseline by 6.42 and 8.75 AP points respectively, with near real-time inference (13.67 FPS).
...
Method: Proposes Semantic Granularity Alignment (SGA) which engineers targeted interventions in the vector residual field to mitigate gradient conflicts. Uses Text-to-Image synthesis as a testbed and evaluates across DiT and U-Net architectures.

Result: SGA advances the efficiency-quality trade-off by accelerating convergence and improving structural integrity in generative fine-tuning tasks.

Conclusion: The geometric perspective reveals a latent Data Interaction Matrix in Flow Matching optimization, and SGA provides an effective method to explicitly control gradient conflicts, improving generative fine-tuning performance across different architectures.
...
TL;DR: PolGS++: A physically-guided polarimetric Gaussian Splatting framework for fast reflective surface reconstruction that integrates polarized BRDF modeling and depth-guided visibility masks to improve geometry recovery while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Accurate reconstruction of reflective surfaces is challenging but important for real-time VR and digital content creation. While 3D Gaussian Splatting enables efficient rendering, it underperforms on reflective surfaces compared to neural methods, especially for fine geometry and surface normals recovery.

Method: Integrates polarized BRDF (pBRDF) model into 3DGS to decouple diffuse and specular components for better reflectance modeling. Introduces depth-guided visibility mask acquisition for angle-of-polarization (AoP)-based tangent-space consistency constraints without costly ray-tracing, enabling physically-guided reconstruction.

Result: Method achieves improved reconstruction quality and efficiency, requiring only about 10 minutes of training. Validated through extensive experiments on both synthetic and real-world datasets.

Conclusion: PolGS++ successfully addresses the gap in reflective surface reconstruction by combining physical modeling with efficient Gaussian Splatting, providing a fast and accurate solution for reflective surface recovery.
...
TL;DR: The paper analyzes how backdoor attacks manifest in Vision Transformers, identifying a "trigger direction" in activations that causally controls backdoor behavior, revealing differences between patch vs. distributed triggers, and proposing weight-based detection methods.
...
Method: Identifies a specific "trigger direction" in model activations, performs interventions in activation and parameter space, traces backdoor feature processing across layers, analyzes differences between attack types, and proposes data-free weight-based detection for stealthy triggers.

Result: Found distinct qualitative differences between static-patch and distributed triggers, confirmed causal role of trigger direction through interventions, demonstrated PGD perturbations can (de-)activate trigger mechanism, and developed effective detection scheme for stealthy attacks.
...
TL;DR: HanMoVLM transforms large vision-language models into expert evaluators for Chinese paintings, achieving professional-grade assessment through specialized reasoning chains and validation.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack artistic expertise for professional evaluation in specific domains like Chinese painting, which requires extensive artistic training and understanding of abstract concepts.

Method: Introduces HanMo-Bench dataset with auction-grade masterpieces and AI-generated works, develops HanMoVLM with expert-validated Chain-of-Thought reasoning (content identification → RoI localization → professional evaluation), and designs reward functions to refine reasoning accuracy.

Result: HanMoVLM achieves high consistency with professional experts and significantly improves Chinese painting generation quality by serving as a high-quality verifier for test-time scaling in image generation.

Conclusion: The proposed approach successfully bridges the gap between general VLMs and domain-specific artistic expertise, enabling professional-grade evaluation of Chinese paintings and enhancing generative model outputs.
...
TL;DR: MEDISEG is a new pill image dataset with instance segmentation annotations for 32 pill types across 8262 images, addressing real-world complexities like overlapping pills and occlusions to improve AI-based medication recognition systems.


<details>
  <summary>Details</summary>
Motivation: Medication errors and adverse drug events pose significant patient safety risks due to difficulties in reliably identifying pharmaceuticals in real-world settings. Existing pill image datasets lack comprehensive coverage of real-world complexities like overlapping pills, varied lighting, and occlusions, hindering development of effective AI-based pill recognition models.

Method: Created MEDISEG dataset with instance segmentation annotations for 32 distinct pill types across 8262 images, capturing diverse conditions from individual pills to cluttered dosette boxes. Trained YOLOv8 and YOLOv9 models on this dataset and evaluated performance under both standard detection and few-shot learning protocols.
...
Conclusion: MEDISEG addresses critical gaps in existing pill recognition datasets by providing comprehensive real-world annotations. The dataset not only supports robust supervised training but also promotes transferable representations under limited supervision, making it a valuable resource for developing and benchmarking AI-driven systems for medication safety.
...
TL;DR: Few-shot pill recognition adapts quickly to new classes with minimal examples, but struggles with localization in complex real-world conditions like overlapping pills.


<details>
  <summary>Details</summary>
Motivation: Automated pill recognition systems face deployment challenges due to visually complex real-world conditions (cluttered scenes, overlapping pills, reflections, diverse environments), requiring better generalization under realistic domain shifts.

Method: Two-stage object detection framework: base training followed by few-shot fine-tuning using 1, 5, or 10 labeled examples per novel class, evaluated on separate deployment dataset with multi-object cluttered scenes.

Result: Semantic classification adapts rapidly with few-shot supervision (saturation with just 1 example), but localization and recall decline significantly under overlapping/occluded conditions. Models trained on realistic multi-pill data show greater robustness.

Conclusion: Training data realism is crucial for deployment readiness, and few-shot fine-tuning serves as a diagnostic tool to assess model robustness in real-world pill recognition scenarios.
...
TL;DR: UltrasoundAgents: A hierarchical multi-agent framework that mimics clinical workflow for breast ultrasound diagnosis, using main agent for lesion localization and sub-agent for attribute analysis, with decoupled progressive training to improve evidence traceability and diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods use end-to-end prediction or provide weakly grounded evidence, missing fine-grained lesion cues and limiting auditability. Need to align with clinical workflow (global localization → local sign assessment → evidence integration) and improve evidence traceability for clinical review.

Method: Hierarchical multi-agent framework: main agent localizes lesion and triggers crop-and-zoom; sub-agent analyzes local view to predict four clinical attributes (echogenicity pattern, calcification, boundary type, edge morphology); main agent integrates attributes for BI-RADS category and malignancy prediction. Uses decoupled progressive training: train attribute agent first, then main agent with oracle attributes, then corrective trajectory self-distillation with spatial supervision for supervised fine-tuning.
...
Conclusion: UltrasoundAgents framework successfully aligns with clinical workflow, improves evidence traceability, and achieves better diagnostic performance through hierarchical multi-agent design and progressive training strategy.
...
TL;DR: DIPE is a position encoding method that prevents visual fading in multimodal LLMs by decoupling intra-modal and inter-modal position encoding, ensuring visual tokens remain perceptually anchored regardless of text length.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs suffer from visual fading in long-context scenarios where attention to visual tokens diminishes as text sequences lengthen, causing text generation to detach from visual constraints.
...
Result: DIPE integrated with Multimodal RoPE maintains stable visual grounding in long-context scenarios, significantly alleviates visual fading while preserving performance on standard short-context benchmarks.

Conclusion: DIPE effectively mitigates the inter-modal distance-based penalty inherent in Multimodal RoPE, ensuring visual signals remain perceptually consistent regardless of context length.
...
TL;DR: Proposes haze-to-clear text-directed loss using CLIP for semantic alignment and BiLaLoRA strategy for targeted adaptation to address real image dehazing challenges.


<details>
  <summary>Details</summary>
Motivation: Learning-based real image dehazing methods face adaptation challenges in diverse real haze scenes due to lack of effective unsupervised mechanisms for unlabeled data and high cost of full model fine-tuning.

Method: 1) Haze-to-clear text-directed loss leveraging CLIP's cross-modal capabilities to reformulate dehazing as semantic alignment in latent space. 2) Bilevel Layer-positioning LoRA (BiLaLoRA) strategy that learns both LoRA parameters and automatically searches injection layers for targeted adaptation.
...
Conclusion: The proposed approach effectively addresses adaptation challenges in real image dehazing through cross-modal semantic guidance and targeted parameter-efficient adaptation, with code publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D representations like point clouds and 3D Gaussian Splatting suffer from non-photorealistic rendering and degrade significantly under sparse inputs, limiting practical applications.
...
Result: Achieves best consistency in novel view generation and first-tier sparse view reconstruction quality under different input sparsity, enabling stable scene reconstruction with minimal captures.

Conclusion: S2D bridges sparse and dense 3D representations, enabling high-quality 3DGS applications with minimal input requirements.
...
Conclusion: Design patterns and batch processing can significantly enhance scalability and reduce computational costs in oral cancer detection systems.
...
TL;DR: CBCTRepD is a bilingual oral/maxillofacial CBCT report generation system that improves radiologist performance across experience levels through AI collaboration.


<details>
  <summary>Details</summary>
Motivation: Limited application of generative AI in oral/maxillofacial CBCT reporting due to scarcity of high-quality paired CBCT-report data and complexity of volumetric CBCT interpretation.

Method: Curated large-scale dataset of 7,408 CBCT studies covering 55 oral diseases, developed CBCTRepD system, and established multi-level evaluation framework with automatic metrics and radiologist/clinician assessments.

Result: CBCTRepD achieves superior report generation with writing quality comparable to intermediate radiologists. In collaboration, it helps novices improve toward intermediate level, enables intermediates to approach senior performance, and assists seniors by reducing omission errors including missed lesions.

Conclusion: CBCTRepD shows strong potential as practical assistant for real-world CBCT reporting by improving report structure, reducing omissions, and promoting attention to co-existing lesions across multi-level care settings.
...
<details>
  <summary>Details</summary>
Motivation: To develop a more controlled, efficient approach to point cloud foundation models that doesn't rely heavily on cross-modal supervision from language or vision, demonstrating that careful architecture design and training setup can compensate for limited data.

Method: Introduces a lightweight transformer-based point cloud architecture trained exclusively on 39k point clouds, with a comprehensive replication study that standardizes training regimes and benchmarks across multiple architectures to isolate architectural impacts.
...
Conclusion: Simple backbones with carefully curated training setups can deliver competitive results to more complex or data-rich strategies, highlighting the value of architectural design and tokenizer-free approaches in point cloud processing.
...
TL;DR: SSL-V3 combines self-supervised video vision transformers with no-reference VQA to improve video classification by addressing video quality issues, achieving 94.87% accuracy on healthcare datasets.


<details>
  <summary>Details</summary>
Motivation: Video quality significantly impacts classification performance - the authors observed that Mild Cognitive Impairment classification worked well on clear videos but poorly on blurred ones, revealing the need to incorporate Video Quality Assessment (VQA) into classification systems.

Method: Proposed SSL-V3: Self-Supervised Learning-based Video Vision Transformer combined with No-reference VQA. Uses Combined-SSL mechanism to integrate VQA into video classification while addressing VQA label shortages. The method uses video quality scores to directly tune feature maps and links VQA and classification through supervised classification tasks to tune VQA parameters.

Result: Achieved robust experimental results on two datasets, including 94.87% accuracy on interview videos from the I-CONECT healthcare dataset, demonstrating the effectiveness of the approach.

Conclusion: Incorporating VQA through self-supervised learning improves video classification performance, especially when dealing with varying video quality, and successfully addresses the common problem of VQA label shortages in video datasets.
...
TL;DR: Med-DualLoRA: A federated learning framework for medical foundation models that uses dual LoRA modules (global+local) for efficient, privacy-preserving adaptation to multi-center cardiac MRI data.


<details>
  <summary>Details</summary>
Motivation: Foundation models need adaptation for medical imaging tasks, but centralized fine-tuning on clinical data is often infeasible due to privacy constraints. Federated learning offers privacy preservation but struggles with heterogeneous multi-center data and high communication costs for large models.

Method: Proposes Med-DualLoRA, a client-aware parameter-efficient fine-tuning framework that disentangles globally shared and local low-rank adaptations (LoRA) through additive decomposition. Only global LoRA modules are shared and aggregated across sites, keeping local adapters private. Adapts only two transformer blocks for efficiency.

Result: Achieves statistically significant improved performance (balanced accuracy 0.768, specificity 0.612) compared to other federated PEFT baselines on multi-center 3D CMR disease detection tasks, while maintaining communication efficiency.

Conclusion: Med-DualLoRA provides a scalable solution for local federated adaptation of medical foundation models under realistic clinical constraints, improving personalization while reducing communication costs.
...
TL;DR: VCR is a novel low-light image enhancement framework that addresses channel-level inconsistency and color misalignment through variance-driven channel recalibration and color distribution alignment.


<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from entangled luminance/color in sRGB space, noise artifacts in HSV space, and channel-level inconsistency between luminance and chrominance in HVI space, leading to unnatural enhancement results.

Method: VCR framework with two main components: 1) Channel Adaptive Adjustment (CAA) module using variance-guided feature filtering to focus on high-intensity/color regions, and 2) Color Distribution Alignment (CDA) module enforcing distribution alignment in color feature space.

Result: Experimental results on several benchmark datasets demonstrate state-of-the-art performance compared with existing low-light enhancement methods.

Conclusion: VCR effectively addresses channel-level inconsistency and color misalignment issues in low-light enhancement, achieving superior perceptual quality through variance-driven channel recalibration and color distribution alignment.
...
TL;DR: CFD dataset and CFM metric for evaluating color fidelity in realistic text-to-image generation, plus training-free CFR method to improve color authenticity.


<details>
  <summary>Details</summary>
Motivation: Current T2I evaluation biases favor visually vivid images with exaggerated saturation/contrast, making generations look unrealistic even when prompted for realistic-style images.

Method: 1) CFD dataset with 1.3M real/synthetic images with ordered color realism levels; 2) CFM metric using multimodal encoder to learn perceptual color fidelity; 3) CFR training-free method that adaptively modulates spatial-temporal guidance scale.

Result: Progressive framework for assessing and improving color fidelity in realistic-style T2I generation, with dataset and code publicly available.

Conclusion: The proposed CFD/CFM/CFR framework addresses evaluation biases and improves color authenticity in realistic T2I generation through objective assessment and refinement.
...
TL;DR: Agentar-Fin-OCR is a financial document parsing system that converts complex PDFs into structured data with auditing-grade accuracy, addressing finance-specific challenges like cross-page continuity and table parsing.


<details>
  <summary>Details</summary>
Motivation: Financial documents present unique challenges including complex layouts, structural discontinuities across pages, and the need for cell-level referencing capability. Existing document parsing systems struggle with these finance-specific requirements, creating a gap in reliable financial document processing.

Method: The system combines: (1) Cross-page Contents Consolidation algorithm and Document-level Heading Hierarchy Reconstruction module for structure-aware retrieval, and (2) difficulty-adaptive curriculum learning for table parsing with a CellBBoxRegressor module that uses structural anchor tokens to localize table cells without external detectors.

Result: The model shows high performance on OmniDocBench table parsing metrics. The authors also introduce FinDocBench, a new benchmark with six financial document categories and expert-verified annotations, evaluating state-of-the-art models to assess capabilities and limitations on financial documents.

Conclusion: Agentar-Fin-OCR and FinDocBench provide a practical foundation for reliable downstream financial document applications, addressing finance-specific parsing challenges and enabling realistic evaluation in the financial vertical.
...
### [93] [Agentic Control Center for Data Product Optimization](https://arxiv.org/abs/2603.10133)
...
<details>
  <summary>Details</summary>
Motivation: Creating useful data products is challenging and requires domain experts to manually craft supporting assets like question-SQL pairs
...
### [94] [Hybrid Self-evolving Structured Memory for GUI Agents](https://arxiv.org/abs/2603.10291)
*Sibo Zhu,Wenyi Wu,Kun Zhou,Stephen Wang,Biwei Huang*

Main category: cs.AI

TL;DR: HyMEM is a graph-based memory system for GUI agents that combines symbolic nodes with continuous embeddings, enabling multi-hop retrieval and self-evolution to improve performance on real-world computer tasks.


<details>
  <summary>Details</summary>
Motivation: Real-world computer-use tasks are challenging due to long-horizon workflows, diverse interfaces, and frequent errors. Existing approaches use flat retrieval over discrete summaries or continuous embeddings, lacking the structured organization and self-evolving characteristics of human memory.

Method: Hybrid Self-evolving Structured Memory (HyMEM) - a graph-based memory that couples discrete high-level symbolic nodes with continuous trajectory embeddings. It maintains a graph structure for multi-hop retrieval, self-evolution via node update operations, and on-the-fly working-memory refreshing during inference.
...
Conclusion: The brain-inspired HyMEM system successfully addresses limitations of existing memory approaches for GUI agents by providing structured, self-evolving memory that significantly enhances agent performance on complex computer-use tasks.
...
### [95] [HEAL: Hindsight Entropy-Assisted Learning for Reasoning Distillation](https://arxiv.org/abs/2603.10359)
*Wenjing Zhang,Jiangze Yan,Jieyun Huang,Yi Shen,Shuming Shi,Ping Chen,Ning Wang,Zhaoxiang Liu,Kai Wang,Shiguo Lian*

Main category: cs.AI

TL;DR: HEAL is a novel RL-free framework that overcomes the "Teacher Ceiling" limitation in distilling reasoning capabilities from large to small models by using entropy-guided repair, rigorous filtering, and progressive curriculum learning.


<details>
  <summary>Details</summary>
Motivation: Standard distillation methods create an artificial "Teacher Ceiling" by treating the teacher as a static filter that discards complex "corner-case" problems where the teacher fails to find solutions, limiting student learning potential.

Method: HEAL combines three modules: 1) GEAR detects reasoning breakpoints via entropy dynamics and injects hindsight hints to repair trajectories; 2) PURE filters genuine cognitive breakthroughs from shortcuts; 3) PACE implements a three-stage progressive curriculum from foundational alignment to frontier breakthrough.

Result: Extensive experiments on multiple benchmarks show HEAL significantly outperforms traditional SFT distillation and other baselines in distilling reasoning capabilities.

Conclusion: HEAL successfully bridges the reasoning gap between large and small models by overcoming the Teacher Ceiling limitation through an RL-free framework inspired by educational theory, enabling more effective knowledge transfer of complex reasoning capabilities.
...
### [96] [Beyond Scalars: Evaluating and Understanding LLM Reasoning via Geometric Progress and Stability](https://arxiv.org/abs/2603.10384)
*Xinyan Jiang,Ninghao Liu,Di Wang,Lijie Hu*

Main category: cs.AI

TL;DR: TRACED is a framework that evaluates LLM reasoning quality using geometric kinematics, analyzing reasoning traces through Progress (displacement) and Stability (curvature) to distinguish correct reasoning from hallucinations.


<details>
  <summary>Details</summary>
Motivation: Traditional scalar probability evaluations fail to capture the structural dynamics of reasoning processes in LLMs, necessitating a more nuanced approach to assess reasoning quality.

Method: Decomposes reasoning traces into Progress (displacement) and Stability (curvature) metrics, revealing topological patterns: correct reasoning shows high-progress stable trajectories while hallucinations show low-progress unstable patterns with high curvature fluctuations.

Result: Achieves competitive performance and superior robustness across diverse benchmarks by leveraging geometric signatures to distinguish reasoning quality.

Conclusion: TRACED bridges geometry and cognition by mapping high curvature to "Hesitation Loops" and displacement to "Certainty Accumulation," providing a physical lens to decode machine thought dynamics.
...
### [97] [Verbalizing LLM's Higher-order Uncertainty via Imprecise Probabilities](https://arxiv.org/abs/2603.10396)
*Anita Yang,Krikamol Muandet,Michele Caprio,Siu Lun Chau,Masaki Adachi*

Main category: cs.AI

TL;DR: LLMs' uncertainty doesn't fit classical probabilistic frameworks, causing systematic failures. The paper proposes imprecise probability-based prompting techniques to capture both first-order (uncertainty over responses) and second-order (uncertainty about uncertainty) uncertainty, improving LLM uncertainty reporting.


<details>
  <summary>Details</summary>
Motivation: Existing uncertainty elicitation techniques for LLMs, developed under classical probabilistic frameworks, fail to adequately capture LLM behavior, leading to systematic failure modes in ambiguous QA, in-context learning, and self-reflection settings.

Method: Propose novel prompt-based uncertainty elicitation techniques grounded in imprecise probabilities framework. Introduce general-purpose prompting and post-processing procedures to directly elicit and quantify both first-order uncertainty (over responses) and second-order uncertainty (uncertainty about the probability model itself).

Result: Demonstrate effectiveness across diverse settings. The approach enables more faithful uncertainty reporting from LLMs, improving credibility and supporting downstream decision-making.

Conclusion: Imprecise probability-based techniques provide a principled framework for better capturing LLM uncertainty, addressing the mismatch between classical probabilistic uncertainty frameworks and actual LLM behavior.
...
### [98] [Resource-constrained Amazons chess decision framework integrating large language models and graph attention](https://arxiv.org/abs/2603.10512)
*Tianhao Qian,Zhuoxuan Li,Jinde Cao,Xinli Shi,Hanjie Liu,Leszek Rutkowski*

Main category: cs.AI

TL;DR: Lightweight hybrid framework for Game of the Amazons combines graph-based learning with LLMs, achieving 15-56% accuracy improvement and competitive win rates under computational constraints.


<details>
  <summary>Details</summary>
Motivation: Resource-constrained environments challenge conventional deep learning methods that need extensive datasets and computational resources. Need for lightweight AI systems that can perform well in strategic games like Amazons without heavy resource requirements.

Method: Hybrid framework integrating graph-based learning with LLMs: Graph Attention Autoencoder informs multi-step Monte Carlo Tree Search, Stochastic Graph Genetic Algorithm optimizes evaluation signals, and GPT-4o-mini generates synthetic training data. Learns from noisy supervision rather than expert demonstrations.

Result: Achieves 15-56% improvement in decision accuracy over baselines. Outperforms teacher model (GPT-4o-mini) with competitive win rates: 45.0% at N=30 nodes and 66.5% at N=50 nodes on 10×10 Amazons board.

Conclusion: Demonstrates feasibility of evolving specialized, high-performance game AI from general-purpose foundation models under stringent computational constraints. Graph Attention mechanism effectively denoises LLM outputs, enabling weak-to-strong generalization.
...
### [99] [IH-Challenge: A Training Dataset to Improve Instruction Hierarchy on Frontier LLMs](https://arxiv.org/abs/2603.10521)
...
TL;DR: Instruction Hierarchy (IH) defines LLM priority rules for conflicting instructions. IH-Challenge dataset improves IH robustness via RL fine-tuning, reducing unsafe behavior while maintaining capabilities.
...
Method: Introduce IH-Challenge reinforcement learning training dataset with online adversarial example generation. Fine-tune GPT-5-Mini on this dataset to improve instruction hierarchy robustness.

Result: Fine-tuning improves IH robustness by +10.0% across 16 benchmarks (84.1% to 94.1%), reduces unsafe behavior from 6.6% to 0.7%, improves helpfulness on general safety evaluations, and saturates internal prompt injection evaluation with minimal capability regression.

Conclusion: IH-Challenge dataset enables effective training of robust instruction hierarchy in LLMs, significantly improving security against various attacks while maintaining model capabilities. The dataset is released to support future research.
...
### [100] [Adaptive RAN Slicing Control via Reward-Free Self-Finetuning Agents](https://arxiv.org/abs/2603.10564)
*Yuanhao Li,Haozhe Wang,Geyong Min,Nektarios Georgalas,Wang Miao*

Main category: cs.AI

TL;DR: Proposes self-finetuning framework for generative AI agents to enable continuous control in AI-native networks through experience internalization and autonomous feedback generation.


<details>
  <summary>Details</summary>
Motivation: Generative AI models face limitations in continuous control tasks due to finite context windows, lack of explicit rewards, and long-context degradation. Current approaches rely on prompt-based memory rather than parameter-based experience internalization.

Method: Self-finetuning framework with bi-perspective reflection mechanism that generates autonomous linguistic feedback to construct preference datasets from interaction history, followed by preference-based fine-tuning to distill long-horizon experiences into model parameters.

Result: Outperforms standard RL baselines and existing LLM-based agents in sample efficiency, stability, and multi-metric optimization on dynamic Radio Access Network slicing task.

Conclusion: Demonstrates potential of self-improving generative agents for continuous control tasks, paving way for future AI-native network infrastructure by enabling agents to internalize experience through parameter distillation.
...
### [101] [CUAAudit: Meta-Evaluation of Vision-Language Models as Auditors of Autonomous Computer-Use Agents](https://arxiv.org/abs/2603.10577)
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for Computer-Use Agents (CUAs) rely on static benchmarks, rule-based checks, or manual inspection, which are brittle, costly, and poorly aligned with real-world usage, creating a need for scalable and reliable assessment methods.

Method: Study Vision-Language Models (VLMs) as autonomous auditors for assessing CUA task completion by conducting large-scale meta-evaluation of five VLMs that judge task success given natural-language instructions and final environment states across three CUA benchmarks on macOS, Windows, and Linux.

Result: State-of-the-art VLMs achieve strong accuracy and calibration, but all auditors show performance degradation in complex/heterogeneous environments, and even high-performing models exhibit significant disagreement in their judgments.

Conclusion: Current model-based auditing approaches have fundamental limitations, highlighting the need to explicitly account for evaluator reliability, uncertainty, and variance when deploying autonomous CUAs in real-world settings.
...
### [102] [Does LLM Alignment Really Need Diversity? An Empirical Study of Adapting RLVR Methods for Moral Reasoning](https://arxiv.org/abs/2603.10588)
*Zhaowei Zhang,Xiaohan Liu,Xuekai Zhu,Junchao Huang,Ceyao Zhang,Zhiyuan Feng,Yaodong Yang,Xiaoyuan Yi,Xing Xie*

Main category: cs.AI

TL;DR: RLVR works well for logical reasoning, but alignment tasks may need different approaches. Study compares reward-maximizing vs distribution-matching methods on moral reasoning tasks, finding reward-maximizing methods work just as well despite expectations.


<details>
  <summary>Details</summary>
Motivation: While RLVR succeeds in logical reasoning, it's unclear if LLM alignment requires fundamentally different approaches. Moral reasoning appears to tolerate multiple valid responses, suggesting alignment might need diversity-seeking distribution-matching rather than reward-maximizing methods.

Method: Built a rubric-grounded reward pipeline using Qwen3-1.7B judge model for stable RLVR training. Conducted comprehensive empirical study comparing reward-maximizing policy-based methods vs distribution-matching approaches on MoReBench alignment tasks. Used semantic visualization to map high-reward responses to semantic space.

Result: Distribution-matching approaches showed no significant advantages over reward-maximizing methods on alignment tasks. Moral reasoning exhibits more concentrated high-reward distributions than mathematical reasoning. Mode-seeking optimization proved equally or more effective for alignment tasks.

Conclusion: Alignment tasks do not inherently require diversity-preserving algorithms. Standard reward-maximizing RLVR methods can effectively transfer to moral reasoning without explicit diversity mechanisms.
...
### [103] [Trajectory-Informed Memory Generation for Self-Improving Agent Systems](https://arxiv.org/abs/2603.10600)
*Gaodan Fang,Vatche Isahagian,K. R. Jayaram,Ritesh Kumar,Vinod Muthusamy,Punleuk Oum,Gegi Thomas*

Main category: cs.AI

TL;DR: Framework extracts actionable learnings from LLM agent execution trajectories to improve future performance through contextual memory retrieval.


<details>
  <summary>Details</summary>
Motivation: LLM-powered agents repeat inefficient patterns, fail to recover from similar errors, and miss opportunities to apply successful strategies from past executions.

Method: Four-component framework: Trajectory Intelligence Extractor (semantic analysis), Decision Attribution Analyzer (identifies key decisions), Contextual Learning Generator (produces strategy/recovery/optimization tips), and Adaptive Memory Retrieval System (injects relevant learnings).

Result: Up to 14.3 percentage point gains in scenario goal completion on held-out tasks, with particularly strong benefits on complex tasks (28.5pp improvement, 149% relative increase).

Conclusion: Framework enables agents to learn from execution experiences by extracting structured learnings with provenance and retrieving contextually relevant guidance, outperforming generic memory systems.
...
### [104] [FAME: Formal Abstract Minimal Explanation for Neural Networks](https://arxiv.org/abs/2603.10661)
*Ryma Boumazouza,Raya Elsaleh,Melanie Ducoffe,Shahaf Bassan,Guy Katz*

Main category: cs.AI

TL;DR: FAME is a new formal abductive explanation method using abstract interpretation that scales to large neural networks while reducing explanation size, outperforming VERIX+ in both size and runtime.


<details>
  <summary>Details</summary>
Motivation: Existing formal explanation methods struggle to scale to large neural networks while maintaining small explanation sizes. There's a need for scalable formal verification techniques that can provide minimal explanations without requiring traversal order.

Method: FAME uses dedicated perturbation domains in abstract interpretation to eliminate traversal order requirements. It progressively shrinks these domains and leverages LiRPA-based bounds to discard irrelevant features, converging to formal abstract minimal explanations.

Result: FAME consistently outperforms VERIX+ in both explanation size and runtime on medium- to large-scale neural networks. The method also includes a quality assessment procedure combining adversarial attacks with optional VERIX+ refinement.

Conclusion: FAME represents a significant advancement in formal explanation methods, achieving scalability to large neural networks while reducing explanation size through novel abstract interpretation techniques and perturbation domains.
...
### [105] [Emulating Clinician Cognition via Self-Evolving Deep Clinical Research](https://arxiv.org/abs/2603.10677)
...
Method: Developed DxEvolve, a self-evolving diagnostic agent with an interactive deep clinical research workflow that autonomously requisitions examinations and continually externalizes clinical experience from increasing encounter exposure as diagnostic cognition primitives.

Result: On MIMIC-CDM benchmark: improved diagnostic accuracy by 11.2% over backbone models, reached 90.4% on reader-study subset (comparable to clinician reference 88.8%). On external cohort: improved accuracy by 10.2% (covered categories) and 17.1% (uncovered categories) compared to competitive method.

Conclusion: DxEvolve transforms clinical experience into a governable learning asset, supporting an accountable pathway for the continual evolution of clinical AI that better aligns with real-world diagnostic practice.
...
### [106] [Nurture-First Agent Development: Building Domain-Expert AI Agents Through Conversational Knowledge Crystallization](https://arxiv.org/abs/2603.10808)
*Linghao Zhang*

Main category: cs.AI

TL;DR: NFD proposes nurturing AI agents through conversational interaction with experts rather than pre-engineering expertise, using knowledge crystallization cycles to evolve tacit domain knowledge.
...
Method: Nurture-First Development (NFD) initializes agents with minimal scaffolding and grows them through structured conversational interaction with domain practitioners, using Knowledge Crystallization Cycles to consolidate fragmented dialogue knowledge into reusable assets.

Result: The paper formalizes NFD through a Three-Layer Cognitive Architecture, Knowledge Crystallization Cycle with operations and metrics, and operational framework with Dual-Workspace Pattern and Spiral Development Model, illustrated through a financial research agent case study.

Conclusion: NFD offers a paradigm shift for human-agent co-evolution, better aligning with the evolving nature of domain expertise, though with specific conditions and limitations that need consideration.
...
### [107] [A Hybrid Knowledge-Grounded Framework for Safety and Traceability in Prescription Verification](https://arxiv.org/abs/2603.10891)
*Yichi Zhu,Kan Ling,Xu Liu,Hengrun Zhang,Huiqun Yu,Guisheng Fan*

Main category: cs.AI

TL;DR: PharmGraph-Auditor: A hybrid knowledge graph system for safe prescription auditing using LLMs as reasoning engines rather than unreliable generators.


<details>
  <summary>Details</summary>
Motivation: Medication errors threaten patient safety, but current LLMs are unreliable for pharmacist verification due to factual inaccuracies, lack of traceability, and poor complex reasoning.

Method: 1) Hybrid Pharmaceutical Knowledge Base (HPKB) combining relational (constraint satisfaction) and graph (topological reasoning) components via Virtual Knowledge Graph paradigm. 2) Iterative Schema Refinement algorithm for co-evolving schemas from medical texts. 3) KB-grounded Chain of Verification (CoV) that transforms LLMs into transparent reasoning engines by decomposing audit tasks into verifiable queries against HPKB.

Result: System demonstrates robust knowledge extraction capabilities and shows promise for enabling pharmacists to achieve safer and faster prescription verification.

Conclusion: PharmGraph-Auditor addresses LLM limitations for high-stakes medical applications by creating a trustworthy knowledge base and using LLMs as reasoning engines rather than unreliable generators, potentially improving prescription safety and verification efficiency.
...
### [108] [Daily-Omni: Towards Audio-Visual Reasoning with Temporal Alignment across Modalities](https://arxiv.org/abs/2505.17862)
*Ziwei Zhou,Rui Wang,Zuxuan Wu,Yu-Gang Jiang*

Main category: cs.AI

TL;DR: Daily-Omni: A new audio-visual QA benchmark requiring cross-modal temporal reasoning, with 684 videos and 1,197 questions across 6 task families, showing current MLLMs struggle with temporal alignment.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models perform well on individual visual and audio benchmarks, but their ability to process cross-modal information synchronously (especially temporal alignment) remains unexplored.

Method: Created Daily-Omni benchmark using semi-automatic pipeline for annotation, cross-modal consistency refinement, temporal alignment elicitation, and text-only leakage filtering. Evaluated 24 foundation models under 37 modality settings, plus a training-free modular diagnostic baseline using off-the-shelf unimodal models.

Result: Many end-to-end MLLMs struggle on alignment-critical questions, indicating that robust cross-modal temporal alignment remains a significant challenge despite good performance on individual modalities.

Conclusion: Cross-modal temporal alignment is an important open challenge for multimodal AI systems, and Daily-Omni provides a benchmark to measure progress on this critical capability.
...
### [109] [OmniGuide: Universal Guidance Fields for Enhancing Generalist Robot Policies](https://arxiv.org/abs/2603.10052)
...
TL;DR: OMNIGUIDE is a framework that enhances vision-language-action models on complex tasks by incorporating diverse guidance sources (3D models, semantic VLMs, human pose) as differentiable energy functions to influence action sampling.


<details>
  <summary>Details</summary>
Motivation: Current VLA models perform well on simple tasks but struggle with complex tasks requiring spatial/semantic understanding, clutter manipulation, or precise control. There's a need to leverage complementary guidance sources to improve VLA performance on challenging tasks.

Method: Proposes OMNIGUIDE framework that expresses various guidance sources as differentiable energy functions with task-specific attractors and repellers in 3D space. These energy functions influence VLA action sampling, allowing complementary guidance to enhance model performance.

Result: Extensive experiments in simulation and real-world show OMNIGUIDE significantly improves state-of-the-art generalist policies (π₀.₅, GR00T N1.6) across success and safety rates. The unified framework matches or surpasses prior methods designed for specific guidance sources.

Conclusion: OMNIGUIDE provides a flexible framework for incorporating arbitrary guidance sources into VLA models, enabling substantial performance improvements on complex tasks through complementary guidance expressed as differentiable energy functions.
...
### [110] [Model-Free Co-Optimization of Manufacturable Sensor Layouts and Deformation Proprioception](https://arxiv.org/abs/2603.10059)
*Yingjun Tian,Guoxin Fang,Aoran Lyu,Xilong Wang,Zikang Shi,Yuhu Guo,Weiming Wang,Charlie C. L. Wang*

Main category: cs.RO

TL;DR: A computational pipeline that co-optimizes flexible sensor layouts and shape prediction networks for soft robotics/wearables, improving accuracy without requiring physical simulation models.


<details>
  <summary>Details</summary>
Motivation: Current sensor layout design for flexible sensors in soft robotics and wearables relies on heuristics or trial-and-error, limiting prediction accuracy. There's a need for systematic optimization that doesn't require physical simulation models.

Method: A model-free, data-driven pipeline that jointly optimizes sensor number, length, placement, and neural network parameters using differentiable loss functions that consider both prediction accuracy and manufacturability constraints.
...
Conclusion: The approach provides a broadly applicable, model-free solution for optimizing flexible sensor layouts in soft robotics and wearables, enabling better proprioception through systematic co-optimization of sensing hardware and prediction algorithms.
...
### [111] [Decision-Aware Uncertainty Evaluation of Vision-Language Model-Based Early Action Anticipation for Human-Robot Interaction](https://arxiv.org/abs/2603.10061)
*Zhaoda Du,Michael Bowman,Qiaojie Zheng,Xiaoli Zhang*

Main category: cs.RO

TL;DR: First systematic evaluation of uncertainty in vision-language models for short-term action recognition in human-robot interaction, focusing on reliability under partial observations.


<details>
  <summary>Details</summary>
Motivation: Robots need trustworthy confidence estimates for human action predictions in shared workspaces, especially with partial/ambiguous egocentric observations where overconfident predictions can cause unsafe interactions. Current vision-language models lack reliability characterization for uncertainty in temporal-prefix scenarios.
...
Result: Provides missing reliability evidence needed for using vision-language model predictions in confidence-gated human-robot interaction modules. Characterizes how these models perform under partial observation conditions.

Conclusion: This study enables safer deployment of vision-language models in human-robot interaction by establishing evaluation protocols and reliability benchmarks for uncertainty estimation in short-term action recognition under partial observation.
...
### [112] [AR-VLA: True Autoregressive Action Expert for Vision-Language-Action Models](https://arxiv.org/abs/2603.10126)
*Yutong Hu,Jan-Nico Zaech,Nikolay Nikolov,Yuanqi Yao,Sombit Dey,Giuliano Albanese,Renaud Detry,Luc Van Gool,Danda Paudel*

Main category: cs.RO

TL;DR: AR-VLA: An autoregressive action expert that generates continuous action sequences using vision-language prefixes, maintaining long-term memory for context-aware robotic control instead of reactive prediction.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language-Action models and diffusion policies reset temporal context with each observation and predict actions reactively, lacking long-term context awareness and struggling with frequency mismatch between fast control and slow reasoning.

Method: Proposes a standalone autoregressive Action Expert that generates actions as continuous causal sequences conditioned on refreshable vision-language prefixes. Uses long-lived memory for context awareness, re-anchoring mechanism to synchronize asynchronous V-L-A modalities, and modular integration with perception backbones.

Result: Experiments on simulated and real-robot manipulation tasks show AR-VLA can replace traditional chunk-based action heads for both specialist and generalist policies. Exhibits superior history awareness and smoother action trajectories while maintaining or exceeding state-of-the-art reactive VLA task success rates.

Conclusion: Introduces a scalable, context-aware action generation schema that provides a robust structural foundation for training effective robotic policies, addressing frequency mismatch and enabling modular integration of kinematic syntax with perception backbones.
...
### [113] [Cross-Hand Latent Representation for Vision-Language-Action Models](https://arxiv.org/abs/2603.10158)
*Guangqi Jiang,Yutong Liang,Jianglong Ye,Jia-Yang Huang,Changwei Jing,Rocky Duan,Pieter Abbeel,Xiaolong Wang,Xueyan Zou*

Main category: cs.RO

TL;DR: XL-VLA introduces a unified latent action space for cross-embodiment dexterous manipulation, enabling efficient training across diverse robotic hands without costly per-hand data collection.


<details>
  <summary>Details</summary>
Motivation: Training vision-language-action models for dexterous manipulation requires large-scale demonstrations for each robotic hand, which is costly and impractical as new embodiments rapidly appear. There's a need for scalable cross-embodiment learning.
...
Result: Experimental results show XL-VLA consistently outperforms baseline VLA models operating in raw joint spaces, establishing it as an effective solution for scalable cross-embodiment dexterous manipulation.

Conclusion: XL-VLA provides a practical framework for scalable cross-embodiment learning in dexterous manipulation, addressing the data collection bottleneck for diverse robotic hands through a unified latent action representation.
...
### [114] [Dance2Hesitate: A Multi-Modal Dataset of Dancer-Taught Hesitancy for Understandable Robot Motion](https://arxiv.org/abs/2603.10166)
*Srikrishna Bangalore Raghu,Anna Soukhovei,Divya Sai Sindhuja Vankineni,Alexandra Bacula,Alessandro Roncone*

Main category: cs.RO

TL;DR: A multi-modal dataset of hesitant motion for human-robot collaboration, featuring dancer-generated and robot demonstrations across three hesitancy levels for two context-embodiment pairs.


<details>
  <summary>Details</summary>
Motivation: Hesitant robot motion is crucial for human-robot collaboration but challenging to design due to context- and embodiment-dependent inference by human observers.

Method: Created a dataset with: (1) kinesthetic teaching demonstrations on a Franka Emika Panda robot reaching a Jenga tower with three hesitancy levels, and (2) synchronized RGB-D motion capture of dancers performing the same reaching behavior with their upper limbs across three hesitancy levels, plus full-body sequences for extreme hesitancy.

Result: Collected 70 unique whole-body trajectories, 84 upper limb trajectories across three hesitancy levels, and 66 kinesthetic teaching trajectories across three hesitancy levels, with documentation for reproducible benchmarking.

Conclusion: The open-source dataset addresses the challenge of designing generalizable hesitant robot motion by providing multi-modal, context-embodiment-specific motion data for human-robot collaboration research.
...
### [115] [Characterizing Healthy & Post-Stroke Neuromotor Behavior During 6D Upper-Limb Isometric Gaming: Implications for Design of End-Effector Rehabilitation Robot Interfaces](https://arxiv.org/abs/2603.10173)
*Ajay Anand,Gabriel Parra,Chad A. Berghoff,Laura A. Hallock*

Main category: cs.RO

TL;DR: Analysis of how rehabilitation robot interface design affects user behavior, with methods to detect pathology in force data and classify neuromotor strategies using HMMs on sEMG signals.


<details>
  <summary>Details</summary>
Motivation: Designing effective robot-mediated rehabilitation requires understanding the complex interplay between user neuromotor behavior, gaming interfaces, and physical robots. Current challenges include characterizing what constitutes "healthy" versus pathological behaviors and designing system elements that promote healthy motor practice.

Method: Used OpenRobotRehab 1.0 dataset from 13 healthy and 2 post-stroke users performing isometric trajectory tracking tasks with an end-effector rehabilitation robot. Analyzed force output, muscle activations, and game performance. Developed: 1) assessment of interface design impact on behavior, 2) analysis of pathology in end-effector force dynamics, 3) novel HMM-based neuromotor behavior classification using sEMG signals during cyclic motions.

Result: Task specification shapes user behavior; pathology-related features detectable in 6D end-effector force data (significant differences in force error and average force production at p=0.05); healthy neuromotor strategies are heterogeneous and difficult to characterize; HMM-based models successfully discriminate healthy vs post-stroke dynamics where synergy-based decompositions fail.

Conclusion: The findings provide insights for designing adaptive end-effector rehabilitation robots that can promote healthier movement strategies across diverse user populations, highlighting the importance of interface design and advanced analysis methods for characterizing neuromotor behaviors.
...
### [116] [Autonomous Search for Sparsely Distributed Visual Phenomena through Environmental Context Modeling](https://arxiv.org/abs/2603.10174)
*Eric Chen,Travis Manderson,Nare Karapetyan,Peter Edmunds,Nicholas Roy,Yogesh Girdhar*

Main category: cs.RO

TL;DR: Using visual environmental context to guide AUVs for efficient coral species surveying when target detections are sparse, achieving 75% coverage in half the time of exhaustive search.


<details>
  <summary>Details</summary>
Motivation: AUVs surveying coral reefs struggle to efficiently locate sparsely distributed target species due to limited battery life and sparse target detections that don't provide directional guidance.

Method: Uses patch-level DINOv2 embeddings for one-shot detection of target species and surrounding environmental context online, then applies adaptive context modeling to guide robot navigation decisions.

Result: Method samples up to 75% of target species in roughly half the time required by exhaustive coverage, outperforming search strategies using only target detections.

Conclusion: Visual environmental context combined with one-shot detection enables efficient autonomous surveying of sparsely distributed coral species, significantly improving search efficiency over traditional methods.
...
### [117] [Octopus-inspired Distributed Control for Soft Robotic Arms: A Graph Neural Network-Based Attention Policy with Environmental Interaction](https://arxiv.org/abs/2603.10198)
*Linxin Hou,Qirui Wu,Zhihang Qin,Yongxin Guo,Cecilia Laschi*

Main category: cs.RO

TL;DR: SoftGM is an octopus-inspired distributed control architecture for soft robotic arms that learns target-reaching in contact-rich environments using online obstacle discovery without global geometry knowledge.


<details>
  <summary>Details</summary>
Motivation: Traditional soft robotic arm control struggles with complex contact-rich environments where global obstacle geometry is unknown. There's a need for distributed, resilient control that can discover obstacles online and coordinate effectively in unstructured environments.

Method: SoftGM formulates each arm section as a cooperative agent, represents arm-environment interaction as a graph, and uses a two-stage graph attention message passing scheme following CTDE paradigm with centralized critic and decentralized actors.

Result: SoftGM matches strong CTDE methods in simpler environments and achieves best performance in complex wall-with-hole task. It shows robustness to observation noise, actuation failure, and disturbances while maintaining bounded control effort.

Conclusion: SoftGM demonstrates resilient coordination through selective contact-relevant information routing, enabling effective target-reaching in contact-rich environments without prior knowledge of obstacle geometry.
...
### [118] [Perceptive Hierarchical-Task MPC for Sequential Mobile Manipulation in Unstructured Semi-Static Environments](https://arxiv.org/abs/2603.10227)
*Xintong Du,Jingxing Qian,Siqi Zhou,Angela P. Schoellig*

Main category: cs.RO

TL;DR: Perceptive HTMPC framework for sequential mobile manipulation in changing environments using Bayesian inference for object-level change modeling and lexicographic optimization.


<details>
  <summary>Details</summary>
Motivation: Sequential mobile manipulation requires robot awareness of environmental changes during long-term operations, but existing planners assume static environments with precomputed maps, which fails when objects are moved, removed, or introduced.

Method: Proposes perceptive hierarchical-task model predictive control (HTMPC) with Bayesian inference to model object-level changes and maintain temporally accurate 3D environment representations, embedded in lexicographic optimization for sequential task execution.

Result: Validated through simulated and real-robot experiments, showing systematic handling of moved and phantom obstacles, completing sequential tasks with higher efficiency and reactivity than baselines, without prior maps or external infrastructure.

Conclusion: The perceptive HTMPC framework enables efficient sequential mobile manipulation in unstructured, changing environments by maintaining accurate environmental awareness through Bayesian change modeling and optimization-based control.
...
### [119] [Hierarchical Task Model Predictive Control for Sequential Mobile Manipulation Tasks](https://arxiv.org/abs/2603.10232)
*Xintong Du,Siqi Zhou,Angela P. Schoellig*

Main category: cs.RO

TL;DR: Proposes Hierarchical-Task Model Predictive Control (HT-MPC) for mobile manipulators that improves sequential task execution by leveraging robot redundancy through nonlinear lexicographic optimization.


<details>
  <summary>Details</summary>
Motivation: Mobile manipulators need better decision-making algorithms to interface with high-level task planners from large language models, enabling efficient execution of sequential tasks in everyday environments.

Method: Uses nonlinear lexicographic optimization to create a hierarchical control framework that prioritizes tasks and leverages robot redundancy, building on Model Predictive Control principles.

Result: 42% average improvement in hierarchical trajectory tracking over state-of-the-art methods; 2.3x faster execution time for delivery tasks; shorter task-space paths; validated on 9-DOF mobile manipulator.

Conclusion: HT-MPC enables mobile manipulators to execute sequential tasks more efficiently and reactively by effectively managing task priorities and robot redundancy, bridging the gap between high-level planning and low-level control.
...
### [120] [Degeneracy-Resilient Teach and Repeat for Geometrically Challenging Environments Using FMCW Lidar](https://arxiv.org/abs/2603.10248)
*Katya M. Papais,Wenda Zhao,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: A degeneracy-resilient FMCW lidar Teach and Repeat navigation system that uses Doppler velocity-based odometry and degeneracy-aware localization to handle geometrically degenerate environments where conventional ICP-based systems fail.


<details>
  <summary>Details</summary>
Motivation: Teach and Repeat navigation is valuable for GPS-denied environments, but conventional ICP-based systems struggle in geometrically degenerate environments with sparsely structured terrain, leading to unreliable performance.

Method: Uses FMCW lidar with Doppler velocity measurements for geometry-independent motion estimation, incorporates principled pose uncertainty estimation, and develops degeneracy-aware localization with per-point curvature analysis and unified scale handling.

Result: The system successfully completes autonomous navigation in three environments with varying structural richness, including a challenging flat airport test field where conventional ICP-based systems fail.

Conclusion: The proposed FMCW lidar-based system with degeneracy-resilient capabilities provides reliable Teach and Repeat navigation in geometrically challenging environments where traditional methods are inadequate.
...
### [121] [From Prior to Pro: Efficient Skill Mastery via Distribution Contractive RL Finetuning](https://arxiv.org/abs/2603.10263)
*Zhanyi Sun,Shuran Song*

Main category: cs.RO

TL;DR: DICE-RL uses RL as a "distribution contraction" operator to refine pretrained generative robot policies, turning behavior priors into high-performing policies by amplifying successful behaviors from online feedback.


<details>
  <summary>Details</summary>
Motivation: To improve upon pretrained generative robot policies by selectively amplifying high-success behaviors while maintaining stability and sample efficiency during fine-tuning.

Method: Pretrain diffusion- or flow-based policy for broad coverage, then fine-tune with stable residual off-policy RL framework combining selective behavior regularization with value-guided action selection.

Result: DICE-RL reliably improves performance with strong stability and sample efficiency, enabling mastery of complex long-horizon manipulation skills from high-dimensional pixel inputs in simulation and real robots.

Conclusion: DICE-RL provides an effective framework for refining pretrained generative policies into high-performing robot policies through distribution contraction with RL.
...
### [122] [Design of a Robot-Assisted Chemical Dialysis System](https://arxiv.org/abs/2603.10264)
*Diane Jung,Caleb Escobedo,Noah Liska,Maitrey Gramopadhye,Daniel Szafir,Alessandro Roncone,Carson Bruns*

Main category: cs.RO

TL;DR: Robot-assisted system for dialysis procedures designed to reduce scientist workload in chemical labs


<details>
  <summary>Details</summary>
Motivation: Manual experimental procedures are tedious, time-consuming bottlenecks in scientific research, particularly in material science and medicine, creating burdens for scientists during multi-day purification methods like dialysis

Method: User-centered design approach with two usability studies to gather participant feedback and revise design requirements, developing a robot-assisted dialysis system for polymer and protein synthesis

Result: Final system satisfies scientists' needs and has potential for applications in other experimental workflows, with anticipated integration into real chemical wet lab synthesis procedures

Conclusion: The system can decrease scientist workload during long experimental procedures and provides an effective design approach for creating more systems that accelerate scientific discovery and liberate scientists from tedious labor
...
### [123] [Update-Free On-Policy Steering via Verifiers](https://arxiv.org/abs/2603.10282)
...
TL;DR: UF-OPS improves robot manipulation by predicting action success likelihood and steering policies at runtime without parameter updates.


<details>
  <summary>Details</summary>
Motivation: Behavior Cloning policies are brittle and struggle with precise manipulation, needing better adaptation during execution.

Method: Train verifier functions using policy rollout data, then use them to steer base policy toward higher-success actions at execution time.

Result: 49% average improvement in success rate over base policy across 5 real-world tasks, works with black-box diffusion policies.

Conclusion: UF-OPS provides lightweight, flexible improvement for BC policies without parameter changes, enabling better real-world robot manipulation.
...
### [124] [SteadyTray: Learning Object Balancing Tasks in Humanoid Tray Transport via Residual Reinforcement Learning](https://arxiv.org/abs/2603.10306)
*Anlun Huang,Zhenyu Wu,Soofiyan Atar,Yuheng Zhi,Michael Yip*

Main category: cs.RO

TL;DR: ReST-RL is a hierarchical RL architecture that decouples locomotion from payload stabilization for humanoid robots, achieving high success rates in simulation and zero-shot sim-to-real transfer on hardware.


<details>
  <summary>Details</summary>
Motivation: Stabilizing unsecured payloads against bipedal locomotion oscillations is a critical bottleneck for humanoids in unstructured environments, requiring a solution that maintains both payload stability and robot locomotion.
...
Result: 96.9% success rate in variable velocity tracking, 74.5% robustness against external force disturbances, outperforming end-to-end baselines in gait smoothness and orientation accuracy. Successfully deployed on Unitree G1 humanoid with zero-shot sim-to-real generalization.

Conclusion: The modular hierarchical approach effectively solves payload stabilization without degrading bipedal stability, demonstrating reliable sim-to-real transfer across various objects and disturbances.
...
### [125] [PC-Diffuser: Path-Consistent Capsule CBF Safety Filtering for Diffusion-Based Trajectory Planner](https://arxiv.org/abs/2603.10330)
*Eugene Ku,Yiwei Lyu*

Main category: cs.RO

TL;DR: PC-Diffuser is a safety-augmented diffusion planner that embeds certifiable barrier functions directly into the denoising loop to ensure safety while preserving learned trajectory distributions.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based trajectory planners show strong performance but lack safety certification and can fail catastrophically in rare or out-of-distribution scenarios, requiring a framework that integrates safety directly into the planning process.

Method: Embeds path-consistent barrier-function structure into diffusion denoising loop using: (i) capsule-distance barrier functions for collision risk, (ii) kinematic bicycle model conversion for dynamic feasibility, and (iii) path-consistent safety filtering without geometric distortion.

Result: Enables iterative, context-aware safeguarding during trajectory generation rather than post-hoc repair, maintaining safety while preserving the diffusion model's learned distribution.

Conclusion: PC-Diffuser provides a framework for making diffusion-based planners certifiably safe by integrating safety constraints directly into the denoising process, addressing critical limitations of current data-driven autonomous driving approaches.
...
### [126] [Adaptive Manipulation Potential and Haptic Estimation for Tool-Mediated Interaction](https://arxiv.org/abs/2603.10352)
*Lin Yang,Anirvan Dutta,Yuan Ji,Yanxin Zhou,Shilin Shan,Lv Chen,Etienne Burdet,Domenico Campolo*

Main category: cs.RO

TL;DR: A framework using Equilibrium Manifolds for tool-mediated manipulation integrates haptic SLAM, online planning, and adaptive stiffness control to handle visual occlusion and underdetermined haptic sensing.


<details>
  <summary>Details</summary>
Motivation: Human-level dexterity in contact-rich, tool-mediated manipulation is challenging due to visual occlusion and underdetermined haptic sensing, requiring better integration of perception and action.

Method: Introduces parameterized Equilibrium Manifolds as unified representation, uses physical-geometric duality with adaptive manipulation potential, reformulates haptic estimation as manifold parameter estimation via hybrid inference (haptic SLAM), and implements uncertainty-aware impedance control.

Result: Validated through simulation and 260+ real-world screw-loosening trials, demonstrating robust identification and manipulation success with accurate tracking, outperforming fixed impedance baselines and preventing jamming.

Conclusion: The Equilibrium Manifold framework successfully integrates haptic estimation, planning, and control for tool-mediated manipulation, closing the perception-action loop and enabling robust performance in contact-rich tasks.
...
### [127] [Few-Shot Adaptation to Non-Stationary Environments via Latent Trend Embedding for Robotics](https://arxiv.org/abs/2603.10373)
*Yasuyuki Fujii,Emika Kameda,Hiroki Fukada,Yoshiki Mori,Tadashi Matsuo,Nobutaka Shimada*

Main category: cs.RO

TL;DR: A latent Trend ID framework for few-shot adaptation in non-stationary robotic environments that estimates environmental states instead of modifying model parameters to avoid catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Robotic systems face concept shift from latent environmental factors, but conventional adaptation methods cause catastrophic forgetting and high computational costs by updating model parameters.

Method: Estimates low-dimensional environmental states (Trend IDs) via backpropagation while keeping model parameters fixed, with temporal regularization and state transition models to enforce smooth latent space evolution and prevent overfitting.

Result: Learned Trend IDs form distinct regions with temporally consistent trajectories in latent space, enabling few-shot adaptation to unseen environments without modifying model parameters in food grasping tasks.

Conclusion: The framework provides scalable and interpretable adaptation for robotics operating in diverse, evolving environments by avoiding parameter updates and catastrophic forgetting.
...
### [128] [ScanDP: Generalizable 3D Scanning with Diffusion Policy](https://arxiv.org/abs/2603.10390)
*Itsuki Hirako,Ryo Hakoda,Yubin Liu,Matthew Hwang,Yoshihiro Sato,Takeshi Oishi*

Main category: cs.RO

TL;DR: A data-efficient 3D scanning framework using Diffusion Policy to imitate human-like scanning strategies, with Occupancy Grid Mapping for robustness and hybrid sphere-based space representation with path optimization for safety and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning-based 3D scanning methods require large-scale training data and struggle to generalize to unseen object categories, needing more data-efficient and robust approaches.

Method: Uses Diffusion Policy for imitation learning of human scanning strategies, Occupancy Grid Mapping instead of direct point cloud processing for noise resilience, and hybrid sphere-based space representation with path optimization for safe and efficient scanning.

Result: Achieves higher coverage and shorter paths than baselines on diverse unseen objects in both shape and scale, remains robust to sensor noise, and demonstrates practical feasibility in real-world execution.

Conclusion: The proposed framework provides a data-efficient, robust, and generalizable solution for 3D scanning that outperforms existing methods while maintaining practical feasibility for real-world applications.
...
### [129] [Safe Probabilistic Planning for Human-Robot Interaction using Conformal Risk Control](https://arxiv.org/abs/2603.10392)
*Jake Gonzales,Kazuki Mizuta,Karen Leung,Lillian J. Ratliff*

Main category: cs.RO

TL;DR: Novel probabilistic safe control framework combining control barrier functions with conformal risk control for human-robot interaction with formal safety guarantees.
...
Conclusion: The framework provides formal probabilistic safety guarantees for human-robot interaction by effectively combining CBFs with conformal risk control, demonstrating practical improvements in safety and performance.
...
### [130] [Shape Control of a Planar Hyper-Redundant Robot via Hybrid Kinematics-Informed and Learning-based Approach](https://arxiv.org/abs/2603.10402)
*Yuli Song,Wenbo Li,Wenci Xin,Zhiqiang Tang,Daniela Rus,Cecilia Laschi*

Main category: cs.RO

TL;DR: SpatioCoupledNet: Hybrid kinematics-informed learning-based shape control for flexible hyper-redundant robots that combines model-based and data-driven approaches with confidence gating for improved accuracy and convergence.


<details>
  <summary>Details</summary>
Motivation: Hyper-redundant robots with flexible mechanisms offer high dexterity for confined environments but suffer from instability due to compliance, making them sensitive to uncertainties that limit control performance.

Method: Proposed SpatioCoupledNet - a hierarchical neural network that explicitly captures bidirectional spatial coupling between segments while modeling local disturbances, with a confidence-gating mechanism that integrates prior kinematic knowledge to adaptively balance model-based and learned components.

Result: On a five-segment planar hyper-redundant robot, the method reduces steady-state error by up to 75.5% vs analytical model and accelerates convergence by up to 20.5% vs data-driven baseline, with state-dependent authority fusion shifting toward data-driven predictions in unstable states.

Conclusion: The hybrid kinematics-informed learning approach enables robust shape control for flexible hyper-redundant robots, achieving precise tip-positioning (10.47 mm mean error) in dynamic obstacle avoidance tasks while outperforming both pure model-based and pure learning-based controllers.
...
### [131] [Rethinking Gaussian Trajectory Predictors: Calibrated Uncertainty for Safe Planning](https://arxiv.org/abs/2603.10407)
...
<details>
  <summary>Details</summary>
Motivation: Existing Gaussian trajectory predictors often produce unreliable confidence levels (over- or under-confident), which compromises safety when integrated with uncertainty-aware motion planners in crowded autonomous navigation scenarios.

Method: Introduces a calibration loss function that uses Kernel Density Estimation to estimate empirical confidence distribution and matches it to the Chi-squared distribution (consistent with Gaussian assumption). Combines with Mean Squared Error for accurate mean prediction.

Result: Significantly improves reliability of confidence levels in SOTA Gaussian trajectory predictors on real-world datasets. Integration with uncertainty-aware Model Predictive Control achieves improved planning performance and collision-free navigation.

Conclusion: Calibrated confidence levels are crucial for safe autonomous navigation, and the proposed loss function effectively addresses reliability issues in Gaussian trajectory predictors, enhancing downstream planning safety.
...
### [132] [COHORT: Hybrid RL for Collaborative Large DNN Inference on Multi-Robot Systems Under Real-Time Constraints](https://arxiv.org/abs/2603.10436)
*Mohammad Saeid Anwar,Anuradha Ravi,Indrajeet Ghosh,Gaurav Shinde,Carl Busart,Nirmalya Roy*

Main category: cs.RO

TL;DR: COHORT is a collaborative DNN inference framework for multi-robot systems that uses hybrid offline-online RL to dynamically schedule DNN execution across robots, reducing battery consumption by 15.4% and increasing GPU utilization by 51.67%.


<details>
  <summary>Details</summary>
Motivation: Large DNNs, especially transformer-based and multimodal architectures, are computationally demanding and challenging to deploy on resource-constrained edge platforms like field robots, particularly in mission-critical scenarios where robots must collaborate under tight constraints on bandwidth, latency, and battery life without infrastructure support.

Method: COHORT employs a hybrid offline-online reinforcement learning strategy: (a) Offline RL policy learning with Advantage-Weighted Regression trained on auction-based task allocation data, (b) Online policy adaptation via Multi-Agent PPO initialized from the offline policy and fine-tuned in real time, built on ROS framework.

Result: COHORT reduces battery consumption by 15.4%, increases GPU utilization by 51.67%, and satisfies frame-rate and deadline constraints 2.55 times more often compared to genetic algorithms and multiple RL baselines, evaluated on VLM inference tasks like CLIP and SAM.

Conclusion: COHORT effectively addresses the computational challenges of deploying large DNNs on resource-constrained multi-robot systems through collaborative inference and dynamic scheduling, demonstrating significant improvements in efficiency and constraint satisfaction for mission-critical applications.
...
### [133] [AsyncMDE: Real-Time Monocular Depth Estimation via Asynchronous Spatial Memory](https://arxiv.org/abs/2603.10438)
*Lianjie Ma,Yuquan Li,Bingzheng Jiang,Ziming Zhong,Han Ding,Lijun Zhu*

Main category: cs.RO

TL;DR: AsyncMDE: An asynchronous depth perception system that combines a foundation model with a lightweight model to enable real-time monocular depth estimation on edge platforms by amortizing computational cost over time through cross-frame feature reuse.
...
Method: Two-model asynchronous system: foundation model produces high-quality spatial features in background; lightweight model runs asynchronously in foreground, fusing cached memory with current observations through complementary fusion, outputting depth estimates, and autoregressively updating memory for cross-frame feature reuse.

Result: Achieves 237 FPS on RTX 4090 with only 3.83M parameters, recovering 77% of accuracy gap to foundation model with 25X parameter reduction. Validated across indoor static, dynamic, and synthetic extreme-motion benchmarks, degrades gracefully between refreshes, achieves 161 FPS on Jetson AGX Orin with TensorRT.

Conclusion: AsyncMDE demonstrates feasibility for real-time edge deployment by effectively amortizing foundation model computational cost over time while maintaining accuracy through asynchronous operation and cross-frame feature reuse.
...
### [134] [KnowDiffuser: A Knowledge-Guided Diffusion Planner with LM Reasoning and Prior-Informed Trajectory Initialization](https://arxiv.org/abs/2603.10441)
*Fan Ding,Xuewen Luo,Fengze Yang,Bo Yu,HwaHui Tew,Ganesh Krishnasamy,Junn Yong Loo*

Main category: cs.RO

TL;DR: KnowDiffuser integrates language models' semantic reasoning with diffusion models' trajectory generation for autonomous driving motion planning, outperforming existing methods on nuPlan benchmark.
...
Method: A knowledge-guided framework where language models infer context-aware meta-actions from structured scene representations, which are mapped to prior trajectories that anchor a two-stage truncated denoising process for trajectory refinement.

Result: Significantly outperforms existing planners in both open-loop and closed-loop evaluations on the nuPlan benchmark, establishing a robust and interpretable framework.

Conclusion: KnowDiffuser effectively bridges the semantic-to-physical gap in autonomous driving systems by tightly integrating semantic understanding with generative trajectory capabilities.
...
### [135] [DiT4DiT: Jointly Modeling Video Dynamics and Actions for Generalizable Robot Control](https://arxiv.org/abs/2603.10448)
*Teli Ma,Jia Zheng,Zifan Wang,Chuili Jiang,Andy Cui,Junwei Liang,Shuo Yang*

Main category: cs.RO

TL;DR: DiT4DiT: A unified Video-Action Model that couples video and action Diffusion Transformers, using intermediate video denoising features as temporal conditions for action prediction, achieving SOTA results with 10x better sample efficiency.


<details>
  <summary>Details</summary>
Motivation: Current VLA models inherit static image-text representations and struggle to learn physical dynamics from limited action data, while generative video models encode rich spatiotemporal structure and implicit physics that could benefit robotics but remain underexplored.

Method: End-to-end Video-Action Model with cascaded framework: couples video Diffusion Transformer with action Diffusion Transformer, extracts intermediate denoising features from video generation as temporally grounded conditions for action prediction, uses dual flow-matching objective with decoupled timesteps and noise scales.

Result: Achieves SOTA results: 98.6% success on LIBERO, 50.8% on RoboCasa GR1 with less training data; superior real-world performance on Unitree G1 with strong zero-shot generalization; 10x sample efficiency improvement and up to 7x faster convergence.

Conclusion: Video generation can serve as an effective scaling proxy for robot policy learning, bridging the gap between generative video models and robotic manipulation through unified video-action modeling with improved efficiency and performance.
...
### [136] [FAR-Dex: Few-shot Data Augmentation and Adaptive Residual Policy Refinement for Dexterous Manipulation](https://arxiv.org/abs/2603.10451)
...
Conclusion: FAR-Dex enables robust and precise arm-hand coordination through hierarchical integration of data augmentation and adaptive refinement, demonstrating effective dexterous manipulation in both simulation and real-world settings.
...
### [137] [SUBTA: A Framework for Supported User-Guided Bimanual Teleoperation in Structured Assembly](https://arxiv.org/abs/2603.10459)
*Xiao Liu,Prakash Baskaran,Songpo Li,Simon Manschitz,Wei Ma,Dirk Ruiken,Soshi Iba*

Main category: cs.RO

TL;DR: SUBTA is a bimanual assembly teleoperation system that combines intention estimation, task planning, and motion assists to improve precision and user experience.


<details>
  <summary>Details</summary>
Motivation: Human-robot collaboration needs accurate intention inference and task understanding to provide optimal support timing and methods in shared autonomy systems.

Method: SUBTA couples learned intention estimation, scene-graph task planning, and context-dependent motion assists for bimanual assembly teleoperation.

Result: User study (N=12) showed SUBTA significantly outperformed standard teleoperation in position accuracy (p<0.001, d=1.18) and orientation accuracy (p<0.001, d=1.75), while reducing mental demand (p=0.002, d=1.34).

Conclusion: SUBTA greatly improves both effectiveness and user experience in teleoperation through clearer visual feedback and predictable interventions.
...
### [138] [DepthCache: Depth-Guided Training-Free Visual Token Merging for Vision-Language-Action Model Inference](https://arxiv.org/abs/2603.10469)
*Yuquan Li,Lianjie Ma,Han Ding,Lijun Zhu*

Main category: cs.RO

TL;DR: DepthCache is a training-free framework that uses depth information to intelligently compress visual tokens in Vision-Language-Action models, achieving faster inference with minimal performance degradation.


<details>
  <summary>Details</summary>
Motivation: VLA models suffer from high inference latency due to processing massive visual tokens with large language backbones. Existing token compression methods degrade spatial reasoning essential for robotic control.

Method: Leverages depth as structural prior for visual token compression: partitions observations into depth-based regions, applies spatially differentiated merge ratios (preserving near-field, compressing background), distributes merging across frames for temporal consistency, and uses motion-adaptive pipeline for auxiliary view compression.

Result: Achieves up to 1.28x inference speedup with <1% average success rate degradation on LIBERO benchmark across three VLA models, outperforming baselines that suffer 4-24% degradation. Real-world experiments show faster task throughput and more responsive closed-loop control.

Conclusion: DepthCache provides an effective, training-free solution for reducing VLA inference latency while preserving spatial reasoning capabilities essential for robotic manipulation, generalizing across diverse VLA architectures without model modification.
...
### [139] [BinWalker: Development and Field Evaluation of a Quadruped Manipulator Platform for Sustainable Litter Collection](https://arxiv.org/abs/2603.10529)
...
TL;DR: A quadruped robot system for autonomous litter collection in challenging outdoor environments, combining legged mobility with robotic arm manipulation for detecting, grasping, and storing trash in difficult-to-access areas.


<details>
  <summary>Details</summary>
Motivation: Litter pollution is a growing environmental problem affecting ecosystems worldwide, with waste accumulating in difficult-to-access areas like uneven terrains, coasts, parks, and roadside vegetation. Current manual cleanup methods are labor-intensive, time-consuming, and costly, creating a need for robotic solutions to support large-scale environmental cleanup operations.

Method: Developed a quadruped robotic system that combines legged locomotion for navigating challenging outdoor terrains with a manipulation system consisting of a robotic arm and onboard litter container. The system integrates perception, locomotion, and manipulation capabilities to autonomously detect, grasp, and store litter items.

Result: Experimental evaluations in outdoor scenarios demonstrated the system's effectiveness and potential for assisting large-scale litter removal operations in environments that are difficult to reach with traditional robotic platforms.

Conclusion: The work successfully demonstrates the feasibility of integrating perception, locomotion, and manipulation on a legged robotic platform for environmental cleanup tasks, offering a promising solution for automating litter collection in challenging outdoor environments.
...
### [140] [Recover to Predict: Progressive Retrospective Learning for Variable-Length Trajectory Prediction](https://arxiv.org/abs/2603.10597)
*Hao Zhou,Lu Qi,Jason Li,Jie Zhang,Yi Liu,Xu Yang,Mingyu Fan,Fei Luo*

Main category: cs.RO

TL;DR: Progressive Retrospective Framework (PRF) addresses variable-length incomplete trajectory observations in autonomous driving by gradually aligning incomplete features with complete ones through cascade of retrospective units, improving prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: Real-world driving often yields variable-length, incomplete observations, but existing methods optimize for fixed-length observations. Direct one-shot mapping from incomplete to complete observations struggles with significant information gaps in short trajectories.

Method: PRF uses cascade of retrospective units (RDM for feature distillation + RPM for recovering previous timesteps) to gradually align features from incomplete observations with complete ones. Includes Rolling-Start Training Strategy for data efficiency.

Result: Extensive experiments on Argoverse 2 and Argoverse 1 datasets demonstrate effectiveness of PRF, which is plug-and-play with existing methods.

Conclusion: PRF successfully addresses the challenge of variable-length incomplete observations in trajectory prediction through progressive retrospective alignment, improving performance over one-shot mapping approaches.
...
### [141] [TacLoc: Global Tactile Localization on Objects from a Registration Perspective](https://arxiv.org/abs/2603.10565)
*Zirui Zhang,Boyang Zhang,Fumin Zhang,Huan Yin*

Main category: cs.RO

TL;DR: TacLoc is a tactile localization framework that formulates pose estimation as one-shot point cloud registration, using graph-theoretic partial-to-full matching with tactile data for robotic manipulation under occlusion.


<details>
  <summary>Details</summary>
Motivation: Pose estimation is crucial for robotic manipulation when visual perception is occluded during gripper-object interactions. Existing tactile-based methods rely on simulation or pre-trained models, limiting generalizability and efficiency.
...
Result: TacLoc is extensively evaluated on the YCB dataset and demonstrates improved performance. It's further validated on real-world objects across two different visual-tactile sensors.

Conclusion: TacLoc provides an efficient and accurate tactile localization framework that doesn't rely on simulation or pre-trained models, offering better generalizability for robotic manipulation under occlusion conditions.
...
### [142] [Interleaving Scheduling and Motion Planning with Incremental Learning of Symbolic Space-Time Motion Abstractions](https://arxiv.org/abs/2603.10651)
*Elisa Tosello,Arthur Bit-Monnot,Davide Lusuardi,Alessandro Valentini,Andrea Micheli*

Main category: cs.RO

TL;DR: Proposes an incremental learning framework that interleaves scheduling and motion planning to solve the Scheduling and Motion Planning problem for multi-object navigation in shared workspaces.


<details>
  <summary>Details</summary>
Motivation: In real-world domains like automated warehouses, tasks are predefined but execution requires safe, efficient coordination under resource, time, and motion constraints. The challenge shifts to determining if, when, and how to execute tasks.

Method: Novel solution framework interleaving off-the-shelf schedulers and motion planners in an incremental learning loop. Scheduler generates candidate plans, motion planner checks feasibility and returns symbolic feedback (spatial conflicts, timing adjustments) to guide scheduler toward motion-feasible solutions.

Result: Validated on logistics and job-shop scheduling benchmarks augmented with motion tasks using state-of-the-art schedulers and sampling-based motion planners. Framework effectively generates valid plans under complex temporal and spatial constraints where synchronized motion is critical.

Conclusion: Proposed framework successfully addresses the Scheduling and Motion Planning problem by integrating scheduling and motion planning through incremental learning and feedback, enabling safe and efficient execution in shared workspaces with complex constraints.
...
### [143] [Safety-critical Control Under Partial Observability: Reach-Avoid POMDP meets Belief Space Control](https://arxiv.org/abs/2603.10572)
*Matti Vahs,Joris Verhagen,Jana Tumova*

Main category: cs.RO

TL;DR: Layered certificate-based control architecture for POMDPs decouples goal reaching, information gathering, and safety into modular components using Belief Control Lyapunov Functions and Belief Control Barrier Functions for real-time control.


<details>
  <summary>Details</summary>
Motivation: Existing online POMDP solvers struggle with conflicting time scales when trying to coordinate goal reaching, safety, and active information gathering within a single belief tree search. There's a need for a more efficient approach that can handle these distinct behaviors while maintaining real-time performance.

Method: Proposes a layered, certificate-based control architecture operating directly in belief space. Uses Belief Control Lyapunov Functions (BCLFs) formalizing information gathering as Lyapunov convergence, learned via reinforcement learning. Implements Belief Control Barrier Functions (BCBFs) with conformal prediction for probabilistic safety guarantees. Control synthesis reduces to lightweight quadratic programs for real-time execution.

Result: Achieves real-time performance even for non-Gaussian belief representations with dimension >10^4. Experiments in simulation and on a space-robotics platform demonstrate improved safety and task success compared to state-of-the-art constrained POMDP solvers.

Conclusion: The decoupled, certificate-based approach effectively addresses the conflicting time scales in reach-avoid POMDPs, providing modular, real-time control with formal guarantees while outperforming existing unified POMDP solvers.
...
### [144] [Learning Bimanual Cloth Manipulation with Vision-based Tactile Sensing via Single Robotic Arm](https://arxiv.org/abs/2603.10609)
*Dongmyoung Lee,Wei Chen,Xiaoshuai Chen,Rui Zong,Petar Kormushev*

Main category: cs.RO

TL;DR: Touch G.O.G. is a vision-based tactile gripper system for single-arm bimanual cloth manipulation using tactile sensing and foundation models to handle fabric deformations and occlusions.


<details>
  <summary>Details</summary>
Motivation: Robotic cloth manipulation is challenging due to fabric's high-dimensional state space, deformable nature, and vision occlusions. Dual-arm systems help but increase complexity, so a single-arm solution is needed.

Method: Three components: 1) Novel gripper design for in-gripper cloth sliding with single arm, 2) Vision Transformer pipeline (PC-Net for classification, PE-Net for edge pose estimation) using real/synthetic tactile images, 3) Synthetic data generator (SD-Net) to reduce manual annotation.

Result: 96% accuracy in distinguishing edges, corners, interior regions, and grasp failures; sub-millimeter edge localization; 4.5° orientation error. Real-world demonstration of reliable cloth unfolding with single arm, even for crumpled fabrics.

Conclusion: Touch G.O.G. provides a compact, cost-effective solution for deformable object manipulation using tactile sensing and foundation models, enabling single-arm cloth manipulation without complex dual-arm systems.
...
### [145] [AdaClearGrasp: Learning Adaptive Clearing for Zero-Shot Robust Dexterous Grasping in Densely Cluttered Environments](https://arxiv.org/abs/2603.10616)
*Zixuan Chen,Wenquan Zhang,Jing Fang,Ruiming Zeng,Zhixuan Xu,Yiwen Hou,Xinke Wang,Jieqi Shi,Jing Huo,Yang Gao*

Main category: cs.RO

TL;DR: AdaClearGrasp: A closed-loop framework for adaptive clearing and zero-shot dexterous grasping in dense clutter using vision-language reasoning and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: In dense clutter, direct grasping often fails due to physical interference and occlusions, while aggressive singulation strategies compromise safety. Robots need adaptive decision-making to choose between clearing objects or directly grasping targets for robust manipulation.

Method: Closed-loop decision-execution framework with: 1) Vision-language model interprets visual observations and language tasks to reason about grasp interference and generate planning skeletons; 2) Structured atomic skills invoked through unified action interface; 3) Reinforcement learning policy with relative hand-object distance representation for zero-shot dexterous grasping; 4) Visual feedback monitoring and replanning upon failures.

Result: Significantly improves grasp success rates in densely cluttered environments. Evaluated on Clutter-Bench benchmark (7 objects × 3 clutter levels = 210 scenarios) and sim-to-real experiments (3 objects × 3 clutter levels = 18 scenarios).

Conclusion: AdaClearGrasp enables robust manipulation in dense clutter through adaptive decision-making between clearing and direct grasping, with zero-shot generalization across diverse objects and closed-loop correction mechanisms.
...
### [146] [STM32-Based Smart Waste Bin for Hygienic Disposal Using Embedded Sensing and Automated Control](https://arxiv.org/abs/2603.10660)
*Mohammed Aman Bhuiyan,Aritra Islam Saswato,Md. Misbah Khan,Anish Paul,Ahmed Faizul Haque Dhrubo,Mohammad Abdul Qayum*

Main category: cs.RO

TL;DR: Motion-sensing automatic waste bin using STM32 microcontroller, ultrasonic sensors, and servo motor for contactless waste disposal with waste level monitoring and OLED display.


<details>
  <summary>Details</summary>
Motivation: Increasing demand for hygienic and contactless solutions in public/private environments drives development of automated systems for everyday applications.

Method: Design and implementation using STM32 microcontroller, ultrasonic sensors for user presence detection and waste level monitoring, servo motor for automatic lid opening, and OLED display for real-time feedback.

Result: Experimental evaluation shows fast response time, stable sensing performance, smooth mechanical operation, and reliable touch-free waste disposal solution.

Conclusion: Low-cost, reliable, easily deployable system suitable for homes, educational institutions, hospitals, and public facilities to improve hygiene and user convenience.
...
### [147] [Dynamic Modeling and Attitude Control of a Reaction-Wheel-Based Low-Gravity Bipedal Hopper](https://arxiv.org/abs/2603.10670)
*Shriram Hari,M Venkata Sai Nikhil,R Prasanth Kumar*

Main category: cs.RO

TL;DR: Underactuated bipedal hopping robot uses reaction wheel for mid-air attitude stabilization in low-gravity environments like the Moon, reducing angular deviation by 65% and landing error to within 3.5 degrees.


<details>
  <summary>Details</summary>
Motivation: Low-gravity planetary bodies (Moon, asteroids) present locomotion challenges with diminished contact forces and extended airborne phases. Hopping locomotion is energy-efficient but prone to mid-flight attitude instability from asymmetric thrust and uneven terrain.

Method: Underactuated bipedal hopping robot with internal reaction wheel for posture regulation during ballistic flight. Modeled as gyrostat to analyze torso-wheel dynamic coupling. Three-phase locomotion cycle: leg-driven jump, mid-air stabilization via active momentum exchange controller, shock-absorbing landing. Reduced-order model captures critical coupling dynamics.

Result: Simulation in MuJoCo under lunar gravity (g=1.625 m/s²) shows reaction wheel controller reduces peak mid-air angular deviation by >65% and constrains landing attitude error to within 3.5 degrees. Also reduces actuator saturation per hop cycle, ensuring sufficient control authority.

Conclusion: The approach significantly mitigates in-flight attitude excursions and enables consistent upright landings, providing a practical and control-efficient solution for locomotion on irregular extraterrestrial terrains.
...
### [148] [Semantic Landmark Particle Filter for Robot Localisation in Vineyards](https://arxiv.org/abs/2603.10847)
*Rajitha de Silva,Jonathan Cox,James R. Heselden,Marija Popović,Cesar Cadena,Riccardo Polvara*

Main category: cs.RO

TL;DR: SLPF integrates semantic trunk/pole detections with LiDAR to solve perceptual aliasing in vineyards, reducing localization errors by 22-65% over baselines.
...
Method: Semantic Landmark Particle Filter (SLPF) integrates trunk and pole landmark detections with 2D LiDAR within a probabilistic framework, converting detected trunks into semantic walls that form structural row boundaries embedded in the measurement model, with GNSS as lightweight prior.

Result: Field experiments in 10-row vineyard show SLPF reduces Absolute Pose Error by 22-65% over AMCL, 61-65% over NoisyGNSS baseline; row correctness improves from 0.67 to 0.73; mean cross-track error decreases from 1.40m to 1.26m.

Conclusion: Embedding row-level structural semantics within measurement model enables robust localization in highly repetitive outdoor agricultural environments like vineyards.
...
### [149] [Cybo-Waiter: A Physical Agentic Framework for Humanoid Whole-Body Locomotion-Manipulation](https://arxiv.org/abs/2603.10675)
*Peng Ren,Haoyang Ge,Chuan Qi,Cong Huang,Hong Li,Jiang Zhao,Pei Chi,Kai Chen*

Main category: cs.RO

TL;DR: A humanoid robot framework that converts VLM plans into verifiable task programs with 3D geometric supervision, enabling robust long-horizon execution under partial observability through predicate-based verification and condition-level feedback.


<details>
  <summary>Details</summary>
Motivation: Robots need to execute open-ended natural language requests in human environments, requiring reliable long-horizon execution under partial observability, which is especially challenging for humanoids due to tight coupling between locomotion and manipulation through stance, reachability, and balance constraints.

Method: Uses VLM planner to compile instructions into typed JSON subtask sequences with explicit predicate-based preconditions and success conditions. Grounds task entities in 3D using SAM3 and RGB-D, estimates object centroids/extents, evaluates predicates over stable frames for condition-level diagnostics. Supervisor verifies subtask completion and provides feedback for progression/replanning. Executes subtasks by coordinating locomotion and whole-body manipulation with motion primitives under reachability and balance constraints.
...
Conclusion: The framework successfully enables humanoid robots to execute complex natural language instructions through verifiable task programs with geometric supervision, addressing the challenges of partial observability and tight locomotion-manipulation coupling in humanoid systems.
...
### [150] [GRACE: A Unified 2D Multi-Robot Path Planning Simulator & Benchmark for Grid, Roadmap, And Continuous Environments](https://arxiv.org/abs/2603.10858)
*Chuanlong Zang,Anna Mannucci,Isabelle Barz,Philipp Schillinger,Florian Lier,Wolfgang Hönig*

Main category: cs.RO

TL;DR: GRACE is a unified 2D simulator+benchmark for multi-agent pathfinding that enables transparent, reproducible comparisons across different abstraction levels (grid, roadmap, continuous) using a common evaluation protocol.


<details>
  <summary>Details</summary>
Motivation: Existing tools for multi-agent pathfinding either scale well under simplifying assumptions (grids, homogeneous agents) or offer higher fidelity but with less comparable instrumentation, creating a need for platforms that enable transparent, reproducible comparisons across different modeling choices.

Method: GRACE instantiates the same task at multiple abstraction levels (grid, roadmap, continuous) via explicit, reproducible operators and a common evaluation protocol, consolidating representation, execution, and evaluation in a unified 2D simulator+benchmark.

Result: Empirical results on public maps and representative planners enable commensurate comparisons on a shared instance set, quantifying expected representation-fidelity trade-offs (MRMP solves instances at higher fidelity but lower speed, while grid/roadmap planners scale farther).

Conclusion: GRACE aims to make cross-representation studies more comparable and provides a means to advance multi-robot planning research and its translation to practice by enabling transparent, reproducible comparisons across different abstraction levels.
...
### [151] [OnFly: Onboard Zero-Shot Aerial Vision-Language Navigation toward Safety and Efficiency](https://arxiv.org/abs/2603.10682)
*Guiyong Zheng,Yueting Ban,Mingjie Zhang,Juepeng Zheng,Boyu Zhou*

Main category: cs.RO

TL;DR: OnFly is a fully onboard, real-time framework for zero-shot aerial vision-language navigation that improves task success from 26.4% to 67.8% through dual-agent architecture, hybrid memory, semantic-geometric verification, and receding-horizon planning.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot AVLN methods suffer from unstable VLM decision-making, unreliable long-horizon progress monitoring, and trade-offs between safety and efficiency, limiting practical deployment of UAVs following natural-language instructions.

Method: OnFly uses: 1) shared-perception dual-agent architecture decoupling high-frequency target generation from low-frequency progress monitoring; 2) hybrid keyframe-recent-frame memory for global trajectory context; 3) semantic-geometric verifier refining VLM predictions; 4) receding-horizon planner for collision-free trajectories.

Result: In simulation, OnFly improves task success from 26.4% to 67.8% compared to strongest SOTA baseline. Fully onboard real-world flights validate feasibility for real-time deployment.

Conclusion: OnFly provides a practical solution for zero-shot AVLN with stabilized decision-making, reliable long-horizon monitoring, and improved safety-efficiency balance, enabling real-time deployment of UAVs following natural-language instructions.
...
### [152] [Contact Coverage-Guided Exploration for General-Purpose Dexterous Manipulation](https://arxiv.org/abs/2603.10971)
*Zixuan Liu,Ruoyi Qiao,Chenrui Tie,Xuanwei Liu,Yunfan Lou,Chongkai Gao,Zhixuan Xu,Lin Shao*

Main category: cs.RO

TL;DR: CCGE is a general exploration method for dexterous manipulation that uses contact coverage guidance to discover diverse contact patterns between fingers and object regions, improving training efficiency and real-world transfer.


<details>
  <summary>Details</summary>
Motivation: Dexterous manipulation lacks general-purpose reward formulations and typically requires task-specific, handcrafted priors, unlike domains like Atari games that have well-defined reward structures.

Method: CCGE represents contact states as intersections between object surface points and hand keypoints, maintains a contact counter conditioned on discretized object states via learned hash codes, and uses this counter for count-based contact coverage rewards and energy-based reaching rewards to explore novel contact patterns.

Result: CCGE substantially improves training efficiency and success rates over existing exploration methods on diverse dexterous manipulation tasks (cluttered object singulation, constrained object retrieval, in-hand reorientation, bimanual manipulation), and learned contact patterns transfer robustly to real-world robotic systems.

Conclusion: CCGE provides an effective general exploration method for dexterous manipulation that overcomes the need for task-specific priors by guiding agents to discover diverse contact patterns, with demonstrated real-world applicability.
...
### [153] [MapGCLR: Geospatial Contrastive Learning of Representations for Online Vectorized HD Map Construction](https://arxiv.org/abs/2603.10688)
*Jonas Merkert,Alexander Blumberg,Jan-Hendrik Pauls,Christoph Stiller*

Main category: cs.RO

TL;DR: Improving online HD map construction by enforcing geospatial consistency in BEV features through contrastive learning on overlapping traversals, using a semi-supervised approach that combines limited labeled data with extensive unlabeled multi-traversal data.


<details>
  <summary>Details</summary>
Motivation: Offline HD maps are costly to create and maintain, while online HD map construction requires extensive annotations. Self-supervised training can reduce annotation needs, and improving BEV feature representations through geospatial consistency can enhance map construction quality.

Method: Enforce geospatial consistency between overlapping BEV feature grids using contrastive loss. Analyze dataset traversal overlaps to generate multi-traversal data splits. Train model semi-supervised: supervised on reduced single-traversal labeled data and self-supervised on broader unlabeled multi-traversal data.

Result: Outperforms supervised baseline across all metrics: better vectorized map perception performance quantitatively and improved segmentation in PCA visualization of BEV feature space qualitatively.

Conclusion: Geospatial consistency enforcement through contrastive learning on overlapping traversals effectively improves BEV feature representations for online HD map construction, enabling better performance with reduced annotation requirements.
...
### [154] [Parallel-in-Time Nonlinear Optimal Control via GPU-native Sequential Convex Programming](https://arxiv.org/abs/2603.10711)
*Yilin Zou,Zhong Zhang,Fanghua Jiang*

Main category: cs.RO

TL;DR: GPU-native trajectory optimization framework using sequential convex programming with consensus ADMM achieves 4x speedup and 51% energy reduction over CPU, enabling 100Hz planning for autonomous systems.


<details>
  <summary>Details</summary>
Motivation: Real-time trajectory optimization for autonomous systems is typically CPU-based and limited by serial algorithms that cannot leverage massively parallel GPU architectures, creating a performance gap.
...
Result: Achieves 4x throughput speedup and 51% reduction in energy consumption over 12-core CPU baseline, maintains over 96% active GPU utilization for planning rates exceeding 100Hz, validated on quadrotor flight and Mars descent problems.

Conclusion: The GPU-native framework enables scalable real-time trajectory optimization, naturally extends to multi-trajectory optimization and robust MPC, providing efficient autonomy solutions for edge computing platforms.
...
### [155] [FutureVLA: Joint Visuomotor Prediction for Vision-Language-Action Model](https://arxiv.org/abs/2603.10712)
*Xiaoxu Xu,Hao Li,Jinhui Ye,Yilun Chen,Jia Zeng,Xinyi Chen,Linning Xu,Dahua Lin,Weixin Li,Jiangmiao Pang*

Main category: cs.RO

TL;DR: FutureVLA introduces a joint visuomotor predictive architecture that decouples visual and motor information to better anticipate future states in robotics, improving temporal continuity and generalization across VLA frameworks.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language-action models struggle with joint visuomotor predictive modeling - explicit methods get distracted by visual details, implicit methods disrupt temporal continuity, and both become visually dominated, entangling static scene context with dynamic action intent.

Method: Proposes FutureVLA with Joint Visuomotor Predictive Architecture: 1) Decouples visual and motor information, 2) Jointly encodes generalized physical priors, 3) Uses Joint Visuomotor Gating mechanism to separate visual state preservation from temporal action modeling, 4) Employs latent embeddings alignment strategy in post-training.

Result: Extensive experiments demonstrate that FutureVLA consistently improves various VLA frameworks by enabling better temporal continuity and generalization in visuomotor predictive modeling.

Conclusion: Effective joint visuomotor predictive modeling requires both temporal continuity and visually-conditioned supervision decoupling, which FutureVLA achieves through its novel architecture, leading to improved performance across diverse VLA models.
...
### [156] [MAVEN: A Meta-Reinforcement Learning Framework for Varying-Dynamics Expertise in Agile Quadrotor Maneuvers](https://arxiv.org/abs/2603.10714)
*Jin Zhou,Dongcheng Cao,Xian Wang,Shuo Li*

Main category: cs.RO

TL;DR: MAVEN is a meta-RL framework that enables a single quadrotor policy to adapt to large dynamic variations (mass changes up to 66.7% and rotor thrust losses up to 70%) through a predictive context encoder, achieving zero-shot sim-to-real transfer with robust online adaptation.


<details>
  <summary>Details</summary>
Motivation: Standard RL-trained quadrotor policies fail to generalize across significant dynamic variations, lacking adaptability needed for real-world deployment where conditions like mass changes and rotor failures occur.

Method: MAVEN uses meta-RL with a novel predictive context encoder that learns to infer latent representations of system dynamics from interaction history. The framework leverages GPU-vectorized simulation to distribute tasks across thousands of parallel environments for efficient training.
...
Conclusion: MAVEN demonstrates that meta-RL with predictive context encoding enables robust end-to-end navigation across wide dynamic variations, overcoming generalization limitations of standard RL for quadrotor control in real-world scenarios.
...
### [157] [ASTER: Attitude-aware Suspended-payload Quadrotor Traversal via Efficient Reinforcement Learning](https://arxiv.org/abs/2603.10715)
*Dongcheng Cao,Jin Zhou,Shuo Li*

Main category: cs.RO

TL;DR: ASTER is an RL framework that enables autonomous inverted flight for quadrotor cable-suspended systems using hybrid-dynamics-informed state seeding to overcome reward sparsity and achieve aggressive maneuvers.


<details>
  <summary>Details</summary>
Motivation: Agile maneuvering of quadrotor cable-suspended systems is hindered by non-smooth hybrid dynamics, and existing model-free RL approaches struggle with attitude-constrained or inverted flight due to extreme reward sparsity under strict orientation requirements.

Method: ASTER uses hybrid-dynamics-informed state seeding (HDSS), an initialization strategy that back-propagates target configurations through physics-consistent kinematic inversions across both taut and slack cable phases, enabling policies to discover aggressive maneuvers unreachable via standard exploration.

Result: The framework achieves the first successful autonomous inverted flight for cable-suspended systems, demonstrating remarkable agility, precise attitude alignment, and robust zero-shot sim-to-real transfer across complex trajectories in both simulations and real-world experiments.

Conclusion: ASTER's HDSS approach successfully overcomes the challenges of hybrid dynamics and reward sparsity, enabling previously unattainable aggressive maneuvers like inverted flight for quadrotor cable-suspended systems with robust real-world performance.
...
### [158] [FG-CLTP: Fine-Grained Contrastive Language Tactile Pretraining for Robotic Manipulation](https://arxiv.org/abs/2603.10871)
*Wenxuan Ma,Chaofan Zhang,Yinghao Cai,Guocai Yao,Shaowei Cui,Shuo Wang*

Main category: cs.RO

TL;DR: FG-CLTP is a fine-grained contrastive language tactile pretraining framework that integrates quantitative contact states (force, geometry, orientation) into tactile representations for robotic manipulation, achieving 95.9% classification accuracy and 52.6% MAE reduction.


<details>
  <summary>Details</summary>
Motivation: Existing tactile representations in vision-language-action models rely on qualitative descriptors (e.g., texture) and neglect quantitative contact states like force magnitude, contact geometry, and principal axis orientation, which are crucial for fine-grained manipulation tasks.
...
Result: Achieves 95.9% classification accuracy, reduces regression error (MAE) by 52.6% compared to SOTA, establishes sensor-agnostic foundation with minimal 3.5% sim-to-real gap, and significantly outperforms baselines in contact-rich manipulation tasks.

Conclusion: FG-CLTP provides a robust and generalizable foundation for tactile-language-action models by integrating quantitative contact states into multimodal representations, enabling fine-grained manipulation through explicit physical metrics in the feature space.
...
### [159] [RL-Augmented MPC for Non-Gaited Legged and Hybrid Locomotion](https://arxiv.org/abs/2603.10878)
*Andrea Patrizi,Carlo Rizzardo,Arturo Laurenzi,Francesco Ruscelli,Luca Rossini,Nikos G. Tsagarakis*

Main category: cs.RO

TL;DR: Hierarchical RL-MPC architecture learns acyclic gaits through simulation trial-and-error, enabling zero-shot sim-to-real transfer across diverse robotic platforms without domain randomization.


<details>
  <summary>Details</summary>
Motivation: To offload the combinatorial burden of contact timing from MPC controllers by learning adaptive acyclic gaits through RL, enabling robust locomotion across diverse terrains and robotic platforms.

Method: Contact-explicit hierarchical architecture with high-level RL agent providing gait/navigation commands to low-level locomotion MPC; learns acyclic gaits through trial-and-error in simulation with minimal rewards and limited tuning.

Result: Achieved zero-shot sim-to-sim transfer across platforms (50-120 kg) without domain randomization; demonstrated zero-shot sim-to-real transfer on 120 kg Centauro robot; observed emergence of acyclic gaits and timing adaptations for flat/hybrid locomotion and extensibility to non-flat terrains.

Conclusion: The proposed hierarchical RL-MPC architecture successfully learns adaptive locomotion policies with minimal tuning, enables robust zero-shot transfer across simulation and reality, and provides a scalable framework for diverse robotic platforms.
...
### [160] [A gripper for flap separation and opening of sealed bags](https://arxiv.org/abs/2603.10890)
*Sergi Foix,Jaume Oriol,Carme Torras,Júlia Borràs*

Main category: cs.RO

TL;DR: Novel gripper design with active dented-roller fingertip and compliant fingers enables reliable separation and grasping of thin flexible flaps for opening sterile hospital pouches.


<details>
  <summary>Details</summary>
Motivation: Addressing the physically demanding, repetitive task of opening sterile flat pouches in hospitals, where nurses manually open up to 240 bags per shift, leading to musculoskeletal injuries.

Method: Combines an active dented-roller fingertip with compliant fingers that exploit environmental constraints to robustly grasp thin flexible flaps.

Result: Gripper reliably grasps and separates sealed bag flaps and other thin-layered hospital materials, with normal force being the most sensitive performance variable; system withstands forces needed to open seals when using two grippers.

Conclusion: One of the first demonstrations of robotic assistance automating this repetitive but critical hospital task, addressing a significant ergonomic challenge for healthcare workers.
...
### [161] [Learning Adaptive Force Control for Contact-Rich Sample Scraping with Heterogeneous Materials](https://arxiv.org/abs/2603.10979)
*Cenk Cetin,Shreyas Pouli,Gabriella Pizzuto*

Main category: cs.RO

TL;DR: Adaptive control framework combining impedance control and RL enables robots to scrape diverse materials from vials, outperforming fixed-force methods by 10.9%.


<details>
  <summary>Details</summary>
Motivation: Scientific discovery acceleration requires AI-driven robotics for complex lab tasks like material scraping, which demands fine manipulation with diverse chemicals under varying conditions beyond traditional automation capabilities.

Method: Two-layer approach: low-level Cartesian impedance controller for stable interaction, plus high-level RL agent that dynamically adjusts end-effector forces using perception feedback. Materials modeled as spheres with procedurally-generated dislodgement thresholds using Perlin noise in simulation.

Result: Successfully transferred simulation-trained policy to real robot, achieving 10.9% average improvement over fixed-wrench baseline across five different material setups.

Conclusion: Adaptive control framework enables effective robotic material scraping in constrained lab environments, advancing autonomous scientific discovery capabilities.
...
### [162] [PPGuide: Steering Diffusion Policies with Performance Predictive Guidance](https://arxiv.org/abs/2603.10980)
*Zixing Wang,Devesh K. Jha,Ahmed H. Qureshi,Diego Romeres*

Main category: cs.RO

TL;DR: PPGuide is a lightweight classifier framework that steers pre-trained diffusion policies away from failure modes using self-supervised performance prediction and real-time gradient guidance.


<details>
  <summary>Details</summary>
Motivation: Diffusion policies are effective for robotic manipulation but suffer from compounding errors over time. Existing solutions like expert dataset augmentation or learning predictive world models are computationally expensive.

Method: PPGuide uses attention-based multiple instance learning to automatically identify relevant observation-action chunks from policy rollouts, trains a performance predictor on this self-labeled data, and provides real-time gradients during inference to guide the policy toward robust actions.
...
Conclusion: PPGuide provides an effective, lightweight solution to mitigate compounding errors in diffusion policies without expensive dataset augmentation or world model learning.
