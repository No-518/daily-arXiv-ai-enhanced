...
TL;DR: MARL-Rad is a multi-modal multi-agent RL framework for radiology report generation that coordinates region-specific agents with a global agent, achieving state-of-the-art clinical efficacy metrics.


<details>
  <summary>Details</summary>
Motivation: Prior approaches use single-model RL or post-hoc agentization of independently trained models, which don't fully optimize the entire agent system through reinforcement learning for better clinical efficacy.

Method: Multi-modal multi-agent RL framework with region-specific agents and a global integrating agent, jointly trained and optimized via clinically verifiable rewards.

Result: State-of-the-art clinical efficacy metrics (RadGraph, CheXbert, GREEN scores) on MIMIC-CXR and IU X-ray datasets, with improved laterality consistency and more accurate, detail-informed reports.

Conclusion: MARL-Rad demonstrates that jointly training multiple agents through RL significantly improves clinical efficacy in radiology report generation compared to prior approaches.
...
TL;DR: Online handwriting recognition using IMUs benefits from sub-word tokenization for inter-writer variability and concatenation-based augmentation for intra-writer sparsity.


<details>
  <summary>Details</summary>
Motivation: IMU-based handwriting recognition works across different surfaces but struggles with uneven character distributions and inter-writer variability. The paper aims to address these challenges through systematic investigation of tokenization and augmentation strategies.

Method: Two main strategies: 1) Sub-word tokenization (Bigram tokenization) for structural abstraction, and 2) Concatenation-based data augmentation as a regularizer. Experiments conducted on OnHW-Words500 dataset with both writer-independent and writer-dependent splits.

Result: Bigram tokenization reduced WER from 15.40% to 12.99% on writer-independent split (inter-writer variability). Concatenation-based augmentation reduced CER by 34.5% and WER by 25.4% on writer-dependent split (intra-writer sparsity). Short, low-level tokens benefit performance, and augmentation outperforms proportionally extended training.

Conclusion: Clear variance-dependent effect: sub-word tokenization primarily mitigates inter-writer stylistic variability, while concatenation-based data augmentation effectively compensates for intra-writer distributional sparsity. Different strategies are needed for different types of variability in handwriting recognition.
...
TL;DR: BHU framework enables communication-efficient multi-UAV cooperative perception using Top-K pixel selection, Swin-large-based BEV feature extraction, and diffusion-based DRL for joint optimization.


<details>
  <summary>Details</summary>
Motivation: Multi-UAV cooperative perception faces challenges with massive visual data causing communication latency and resource inefficiency in low-altitude economy applications.
...
Result: On Air-Co-Pred dataset: Improves perception performance by over 5% while reducing communication overhead by 85% compared to traditional CNN-based BEV fusion baselines.

Conclusion: BHU provides an effective solution for multi-UAV cooperative perception under resource-constrained wireless environments by balancing communication efficiency and perception utility.
...
TL;DR: Proposes E-BLS and ER-BLS methods combining Transfer Learning with Broad Learning System for facial beauty prediction, improving accuracy over existing BLS and CNN methods.


<details>
  <summary>Details</summary>
Motivation: Facial beauty prediction faces challenges: prone to overfitting due to limited data, difficulty building robust models due to facial variability and complex human perception. Need methods that reduce data dependence and enable quick model building.

Method: 1. E-BLS: Uses EfficientNets CNN with transfer learning as feature extractor, transfers extracted facial beauty features to BLS for prediction. 2. ER-BLS: Adds connection layer between feature extractor and BLS to enhance integration.

Result: Experimental results show E-BLS and ER-BLS improve facial beauty prediction accuracy compared to previous BLS and CNN methods, demonstrating effectiveness and superiority.

Conclusion: The fusion of transfer learning with broad learning system effectively addresses FBP challenges, with potential applications in pattern recognition, object detection, and image classification.
...
TL;DR: Proposes Script-to-Slide Grounding (S2SG) task and Text-S2SG method using LLMs to automatically ground script sentences to slide text objects for educational video generation.


<details>
  <summary>Details</summary>
Motivation: Slide-based videos with visual effects are widely used in education/research but require labor-intensive manual editing to ground spoken content to slide objects. There's a need to automate this process.

Method: Formalizes Script-to-Slide Grounding (S2SG) task, then proposes Text-S2SG method using large language models (LLMs) to ground script sentences to corresponding slide text objects.

Result: The proposed Text-S2SG method achieves high performance with F1-score of 0.924 in grounding script sentences to slide text objects.

Conclusion: Formalizes the previously implicit slide-based video editing process into a computable task, paving the way for automation of instructional video generation from slides and scripts.
...
TL;DR: AwaRes is a spatial-on-demand framework that uses tool-calling to selectively retrieve high-resolution image segments only when needed, balancing accuracy and computational efficiency in vision-language models.


<details>
  <summary>Details</summary>
Motivation: VLMs face a trade-off between accuracy (needing high-resolution images to capture fine details like small text) and computational efficiency (low-resolution is faster but misses critical information).

Method: Uses low-resolution global view with tool-calling to retrieve only needed high-resolution segments; creates supervised data automatically via judge comparing resolutions and oracle grounding model; trains with cold-start SFT followed by multi-turn GRPO with composite reward (semantic correctness + crop-cost penalties).

Result: The framework resolves the accuracy-efficiency trade-off by dynamically retrieving high-resolution segments only when necessary, reducing computational costs while maintaining accuracy for tasks requiring fine details.

Conclusion: AwaRes provides an effective spatial-on-demand approach that enables VLMs to balance accuracy and efficiency through selective high-resolution processing, with automated data generation and multi-turn training methodology.
...
TL;DR: Proposed V2VK pipeline generates AgriMM benchmark with 3k+ agricultural classes and 607k+ VQAs, used to train AgriChat MLLM that outperforms other models in agricultural tasks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack large-scale agricultural datasets and verified domain expertise, creating a trade-off that stalls agricultural AI deployment. Need for robust, verifiable agricultural data and models.

Method: V2VK pipeline integrates visual captioning with web-augmented scientific retrieval to autonomously generate AgriMM benchmark, eliminating biological hallucinations by grounding data in verified phytopathological literature.

Result: Created AgriMM benchmark with 3,000+ agricultural classes and 607k+ VQAs. AgriChat MLLM trained on this data demonstrates superior performance over other open-source models across diverse agricultural tasks.

Conclusion: Preserving visual detail combined with web-verified knowledge provides reliable pathway toward robust and trustworthy agricultural AI. Code and dataset publicly available.
...
TL;DR: GenLie is a video-based lie detection network that uses local feature modeling with global supervision to capture subtle deceptive cues while suppressing identity-related noise, achieving state-of-the-art performance across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Video-based lie detection faces challenges in learning sparse yet discriminative representations because deceptive signals are subtle and short-lived, easily overwhelmed by redundant information, while individual and contextual variations introduce strong identity-related noise.

Method: GenLie is a Global-Enhanced Lie Detection Network that performs local feature modeling under global supervision. It captures sparse and subtle deceptive cues at the local level, while global supervision and optimization ensure robust and discriminative representations by suppressing identity-related noise.
...
Conclusion: The proposed GenLie framework effectively addresses the core challenges in video-based lie detection by combining local feature modeling with global supervision, achieving superior performance in detecting deceptive behaviors across different scenarios.
...
TL;DR: First work to treat facial parameter modeling as a language problem, using foundation models to create large text-paired facial dataset for bidirectional facial motion understanding and generation.


<details>
  <summary>Details</summary>
Motivation: Facial animation lags behind body animation due to scarcity of well-annotated, text-paired facial corpora, creating a gap in text-guided facial animation capabilities.

Method: Leverage foundation generative models to synthesize 80+ hours of facial videos with emotion/head motion prompts, fit 3D facial parameters, create large-scale prompt-parameter pairs, then train language models for bidirectional Motion2Language and Language2Motion tasks using quantized motion tokens.

Result: Language models can effectively interpret and synthesize facial motion with strong generalization, establishing unified framework for text-conditioned facial animation and motion understanding.

Conclusion: First work to cast facial-parameter modeling as a language problem, creating a unified path for bidirectional facial animation and understanding through large-scale synthetic dataset and language model training.
...
TL;DR: Proposes divergence-based multimodal fusion to detect ambivalence/hesitancy in videos by measuring cross-modal conflict between visual (AUs), audio (Wav2Vec 2.0), and text (BERT) features, achieving 0.6808 Macro F1 vs baseline 0.2827.


<details>
  <summary>Details</summary>
Motivation: To address the Ambivalence/Hesitancy Video Recognition Challenge by capturing the inherent incongruence between different modalities (visual, audio, text) that characterizes ambivalent/hesitant behavior.

Method: Extracts visual features as Action Units via Py-Feat, audio via Wav2Vec 2.0, and text via BERT. Each modality processed by BiLSTM with attention pooling and projected to shared embedding space. Fusion computes pairwise absolute differences between modality embeddings to measure cross-modal conflict.

Result: Achieves Macro F1 of 0.6808 on BAH dataset validation test set, significantly outperforming baseline of 0.2827. Statistical analysis across 1,132 videos shows temporal variability of Action Units is the dominant visual discriminator of ambivalence/hesitancy.

Conclusion: Divergence-based multimodal fusion effectively captures cross-modal conflict for ambivalence/hesitancy recognition, with Action Unit temporal variability being the most important visual feature. The approach demonstrates strong performance on the ABAW challenge dataset.
...
TL;DR: KGS-GCN: Graph convolutional network using kinematics-driven Gaussian splatting and probabilistic topology to address sparse skeleton data limitations in action recognition.


<details>
  <summary>Details</summary>
Motivation: Current sensor devices generate sparse skeleton data that loses fine-grained spatiotemporal details during dynamic movements, and rigid predefined sensor topologies hinder modeling of latent long-range dependencies.

Method: 1) Kinematics-driven Gaussian splatting module using joint velocity vectors to create anisotropic covariance matrices, rendering sparse skeletons into multi-view continuous heatmaps. 2) Probabilistic topology construction using Bhattacharyya distance between joint Gaussian distributions to generate adaptive adjacency matrix. 3) GCN backbone modulated by rendered features via visual context gating.

Result: KGS-GCN significantly enhances modeling of complex spatiotemporal dynamics and offers robust solution for processing low-fidelity sensor data, improving perceptual reliability in real-world sensing applications.

Conclusion: The framework addresses inherent limitations of sparse sensor inputs by transforming discrete joints into continuous generative representations, establishing practical pathway for improved action recognition in sensor systems.
...
TL;DR: Omni IIE Bench is a new benchmark for Instruction-based Image Editing that diagnoses model consistency across semantic scales, revealing performance degradation when moving from low to high semantic complexity tasks.


<details>
  <summary>Details</summary>
Motivation: Existing IIE benchmarks use mixed evaluations that obscure critical failure modes in professional applications, particularly inconsistent performance across tasks of varying semantic scales.

Method: Created Omni IIE Bench with dual-track diagnostic design: (1) Single-turn Consistency with shared-context task pairs for attribute modification and entity replacement, and (2) Multi-turn Coordination with continuous dialogue tasks across semantic scales. Used rigorous multi-stage human filtering with CV graduate students and professional designers.

Result: Evaluation of 8 mainstream IIE models revealed a prevalent performance gap: nearly all models show significant performance degradation when transitioning from low-semantic-scale to high-semantic-scale tasks.

Conclusion: Omni IIE Bench provides critical diagnostic tools and insights for developing next-generation, more reliable and stable IIE models by quantifying previously obscured consistency issues.
...
TL;DR: Proposed .PcRecord format and parallel processing pipeline to accelerate large-scale point cloud data loading and processing, achieving significant speedups across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Large-scale point cloud data presents challenges in loading and processing due to its vast scale, complexity, and diverse storage formats (PLY, XYZ, BIN). Existing binary formats still don't fully address time-consuming data preparation phases.

Method: Developed .PcRecord format as unified storage solution to reduce storage occupation and accelerate processing. Created high-performance data processing pipeline with multi-stage parallel architecture to optimize computational resource usage.

Result: Achieved significant performance improvements: 6.61x (ModelNet40), 2.69x (S3DIS), 2.23x (ShapeNet), 3.09x (Kitti), 8.07x (SUN RGB-D), and 5.67x (ScanNet) with GPU. With Ascend: 6.9x, 1.88x, 1.29x, 2.28x, 25.4x, and 19.3x respectively.

Conclusion: The proposed .PcRecord format and parallel processing pipeline effectively address challenges in handling large-scale point cloud datasets by significantly improving processing speed and efficiency across multiple hardware platforms.
...
TL;DR: EmergeNav enables zero-shot vision-language navigation in continuous environments by adding structured execution to VLMs, achieving strong performance without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Current VLMs have semantic knowledge but lack execution structure for stable long-horizon embodied navigation in continuous environments (VLN-CE). The bottleneck is not missing knowledge but missing organization for instruction following, perceptual grounding, temporal progress, and stage verification.

Method: EmergeNav formulates continuous VLN as structured embodied inference with: 1) Plan-Solve-Transition hierarchy for stage-structured execution, 2) GIPE for goal-conditioned perceptual extraction, 3) contrastive dual-memory reasoning for progress grounding, and 4) role-separated Dual-FOV sensing for local control and boundary verification.

Result: Achieves 30.00 SR with Qwen3-VL-8B and 37.00 SR with Qwen3-VL-32B on VLN-CE without task-specific training, explicit maps, graph search, or waypoint predictors.

Conclusion: Explicit execution structure is crucial for turning VLM priors into stable embodied navigation behavior, enabling zero-shot performance in continuous environments.
...
TL;DR: PhysQuantAgent is a framework using VLMs with visual prompting for real-world object mass estimation, evaluated on a new benchmark dataset VisPhysQuant.


<details>
  <summary>Details</summary>
Motivation: VLMs are increasingly used in robotics but lack reliable mass reasoning capabilities needed for manipulation tasks like determining appropriate grasp force. Current benchmarks don't adequately evaluate physical quantity estimation under realistic sensing conditions.
...
Conclusion: The framework successfully enhances VLM capabilities for physical property estimation, suggesting promising directions for improving robotic perception and manipulation through better physical reasoning.
...
TL;DR: Standardized scenario extraction and domain-knowledge-guided clustering for automated vehicle testing from real-world highway data.


<details>
  <summary>Details</summary>
Motivation: Current methods for extracting and grouping traffic scenarios from real-world data for ADS testing lack standardization and interpretability, hindering efficient validation.

Method: Proposes standardized scenario extraction based on Scenario-as-Specification concept and domain-knowledge-guided clustering process, demonstrated on highD dataset.

Result: Scenarios can be extracted reliably and domain-knowledge can be effectively integrated into clustering, enabling more standardized scenario derivation.

Conclusion: The methodology supports standardized process for deriving scenario categories from highway data, enabling more efficient automated vehicle validation.
...
TL;DR: CineSRD is a multimodal framework for open-world speaker diarization in visual media like films/TV, using visual, acoustic, and linguistic cues to handle long-form content with many speakers.


<details>
  <summary>Details</summary>
Motivation: Traditional speaker diarization systems are limited to constrained scenarios (meetings/interviews) with few speakers and clean audio. Real-world visual media (films/TV) present open-world challenges: long-form content, many speakers, audio-visual asynchrony, and uncontrolled variability.

Method: CineSRD (Cinematic Speaker Registration & Diarization) is a unified multimodal framework that leverages visual, acoustic, and linguistic cues from video, speech, and subtitles. It first performs visual anchor clustering to register initial speakers, then integrates an audio language model for speaker turn detection to refine annotations and supplement unregistered off-screen speakers.

Result: CineSRD achieves superior performance on the proposed visual media benchmark (Chinese and English programs) and competitive results on conventional datasets, demonstrating robustness and generalizability in open-world settings.

Conclusion: The proposed CineSRD framework effectively addresses open-world speaker diarization challenges in visual media through multimodal integration, and the released benchmark facilitates further research in this domain.
...
TL;DR: MSRAMIE is a training-free agent framework that uses multimodal reasoning to handle complex multi-instruction image editing tasks without retraining existing models.


<details>
  <summary>Details</summary>
Motivation: Existing image editing models struggle with complex, multi-step instructions due to lack of training data with such annotations, and collecting this data is expensive.

Method: Uses MLLM-based Instructor and editing Actor with Tree-of-States and Graph-of-References reasoning topology to decompose complex instructions into steps, enabling state transitions, information aggregation, and input recall.

Result: Improves instruction following by over 15% as complexity increases, increases probability of finishing all modifications in single run by over 100%, while preserving quality and consistency.

Conclusion: MSRAMIE provides effective, interpretable solution for complex multi-instruction image editing without costly retraining, offering visualizable inference topology for control.
...
TL;DR: MAND: Modality-aware framework for multimodal egocentric open-world continual learning that improves novelty detection and classification by better utilizing complementary modality cues and preventing catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal egocentric activity recognition systems underutilize complementary evidence from individual modalities (especially IMU) for novelty detection, relying mainly on RGB-dominated logits. This imbalance worsens over time due to catastrophic forgetting in continual learning scenarios.

Method: MAND uses two key components: 1) Modality-aware Adaptive Scoring (MoAS) at inference to estimate modality reliability from energy scores and adaptively integrate modality logits for better novelty detection; 2) Modality-wise Representation Stabilization Training (MoRST) during training to preserve modality-specific discriminability via auxiliary heads and modality-wise logit distillation.

Result: On a public multimodal egocentric benchmark, MAND improves novel activity detection AUC by up to 10% and known-class classification accuracy by up to 2.8% over state-of-the-art baselines.

Conclusion: The proposed modality-aware framework effectively addresses the underutilization of complementary modality cues in multimodal egocentric open-world continual learning, leading to significant improvements in both novelty detection and classification performance.
...
TL;DR: Automatic pipeline enriches MMKGs by retrieving entity images, converting them to text descriptions, and fusing them with LLMs to generate entity-aligned summaries, improving KG completion without model changes.


<details>
  <summary>Details</summary>
Motivation: MMKGs lack comprehensive visual coverage and struggle with ambiguous images (logos, symbols, abstract scenes) that are hard to curate manually but contain relevant semantics.

Method: Three-stage pipeline: (1) large-scale retrieval of entity-related images, (2) conversion of all images to textual descriptions, (3) LLM-based fusion of multi-source descriptions to generate concise entity-aligned summaries that augment text modality.

Result: Consistent gains up to 7% Hits@1 across three MMKG datasets; dramatic improvements on ambiguous entities (201.35% MRR, 333.33% Hits@1); includes optional auditing interface for quality control.

Conclusion: Scaling image coverage and converting ambiguous visuals to text is a practical approach for stronger MMKG completion without architectural changes, with released tools for community use.
...
TL;DR: Compact VLMs underperform on inference speed despite smaller size; optimization recipes reduce latency by 53-93% while maintaining accuracy; new ArgusVLM family adds structured perception outputs.


<details>
  <summary>Details</summary>
Motivation: Compact vision-language models (VLMs) deployed in resource-constrained settings need low latency and high throughput, but current models don't achieve expected speedups from their smaller parameter counts.

Method: Conducted empirical end-to-end efficiency analysis and systematic inference profiling to identify bottlenecks; developed optimization recipes tailored to compact VLMs; extended compact VLMs with structured perception outputs to create ArgusVLM family.
...
Conclusion: The optimization recipes are broadly applicable across VLM architectures and serving frameworks, providing practical guidance for building efficient VLM systems; ArgusVLM demonstrates that compact VLMs can be both efficient and capable with structured perception outputs.
...
TL;DR: HopChain synthesizes multi-hop vision-language reasoning data to improve VLM performance on complex reasoning tasks, achieving broad benchmark gains without targeting specific benchmarks.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with fine-grained vision-language reasoning, especially in long CoT scenarios where errors compound across steps. Existing RLVR data lacks complex reasoning chains that rely on visual evidence throughout, leaving these weaknesses unexposed.

Method: HopChain framework synthesizes multi-hop vision-language reasoning data where each query forms logically dependent chains of instance-grounded hops. Earlier hops establish instances, sets, or conditions needed for later hops, with final answers being specific numbers for verifiable rewards.

Result: Adding HopChain data improved 20 out of 24 benchmarks across both Qwen3.5 models, showing broad gains. Removing full chained queries reduced average accuracy by 5.3-7.0 points. Multi-hop training boosted long-CoT reasoning by over 50 accuracy points in ultra-long-CoT scenarios.

Conclusion: HopChain is an effective, scalable framework for synthesizing multi-hop data that improves generalizable vision-language reasoning in VLMs, addressing compound error problems in complex reasoning chains.
...
TL;DR: OpenQlaw is an agentic orchestration system that uses a core LLM agent to coordinate domain-expert MLLMs for analyzing 2D quantum materials, enabling dynamic reasoning, scale-aware computations, and persistent memory for practical device fabrication.


<details>
  <summary>Details</summary>
Motivation: Current domain-specific MLLMs for 2D quantum material analysis produce accurate but verbose, step-by-step reasoning that causes cognitive overload and lacks immediate utility for real-world researcher interaction and device fabrication workflows.

Method: Built on NanoBot (lightweight agentic framework) and QuPAINT (physics-aware multimodal platform), OpenQlaw uses a core LLM agent to orchestrate domain-expert MLLMs, decoupling visual identification from reasoning, enabling dynamic query processing, scale-aware computations, and persistent memory for physical scale ratios and sample preparation methods.

Result: The system transforms isolated inferences into a context-aware assistant capable of accelerating high-throughput device fabrication by providing naturalistic responses, performing physical computations, generating visual annotations, and maintaining persistent memory for practical applications.

Conclusion: OpenQlaw's agentic architecture successfully bridges the gap between optical identification and practical device fabrication by orchestrating domain experts, enabling dynamic reasoning, and providing immediate utility for researchers through accessible messaging channels and persistent memory capabilities.
...
TL;DR: Astrolabe: Efficient online RL framework for aligning distilled AR video models with human preferences using forward-process fine-tuning, streaming training, and multi-reward stabilization.


<details>
  <summary>Details</summary>
Motivation: Distilled autoregressive video models enable efficient streaming generation but often misalign with human visual preferences. Existing RL frameworks are inefficient for these architectures, requiring expensive re-distillation or solver-coupled reverse-process optimization with high memory/computational overhead.

Method: 1) Forward-process RL formulation with negative-aware fine-tuning that contrasts positive/negative samples at inference endpoints without reverse-process unrolling. 2) Streaming training scheme with rolling KV-cache for progressive sequence generation, applying RL updates to local clip windows while conditioning on prior context. 3) Multi-reward objective stabilized by uncertainty-aware selective regularization and dynamic reference updates to mitigate reward hacking.

Result: Extensive experiments demonstrate consistent enhancement of generation quality across multiple distilled AR video models, serving as a robust and scalable alignment solution.

Conclusion: Astrolabe provides an efficient online RL framework tailored for distilled AR models, overcoming existing bottlenecks through forward-process optimization, streaming training, and stabilized multi-reward objectives for scalable video alignment.
...
<details>
  <summary>Details</summary>
Motivation: Existing image restoration agents lack insight summarization mechanisms for past interactions, leading to exhaustive searches for optimal restoration tools. The authors aim to create an agent that can intelligently select tools by learning from past restoration experiences.

Method: Proposes PaAgent with: 1) Self-evolving portrait bank that continuously summarizes IR tool characteristics using restored images, selected tools, and degraded images; 2) Retrieval-Augmented Generation (RAG) to select optimal tools by retrieving relevant insights; 3) Subjective-objective reinforcement learning strategy that considers both image quality scores and semantic insights for degradation perception in complex scenes.
...
Conclusion: PaAgent effectively addresses the limitations of existing IR agents by incorporating insight summarization and intelligent tool selection mechanisms, demonstrating superior performance in complex image restoration scenarios.
...
TL;DR: DesertFormer is a semantic segmentation model for off-road desert terrain that achieves 64.4% mIoU, a 24.2% improvement over baseline, enabling safety-aware path planning for autonomous vehicles.


<details>
  <summary>Details</summary>
Motivation: Desert landscapes present unique challenges for autonomous navigation due to low chromatic contrast between terrain categories, extreme lighting variability, and sparse vegetation that break standard road-scene segmentation assumptions.

Method: Uses SegFormer B2 with hierarchical Mix Transformer (MiT-B2) backbone to classify terrain into ten ecologically meaningful categories. Trained on 4,176 annotated off-road images at 512x512 resolution with class-weighted training and copy-paste augmentation for rare categories.

Result: Achieves 64.4% mIoU and 86.1% pixel accuracy, representing +24.2% absolute improvement over DeepLabV3 MobileNetV2 baseline (41.0% mIoU). Identified primary confusion patterns: Ground Clutter to Landscape and Dry Grass to Landscape.

Conclusion: DesertFormer enables reliable terrain perception for autonomous navigation in challenging desert environments, with code, checkpoints, and interactive dashboard released for community use.
...
TL;DR: TrackDeform3D: An affordable autonomous framework using RGB-D cameras to collect 3D datasets of deformable objects with keypoint tracking and motion consistency constraints.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with extracting structured 3D representations (keypoints, meshes) for deformable objects due to complex deformations and lack of large-scale 3D datasets. Existing approaches require expensive data collection (motion capture, manual annotation) or make simplifying assumptions that fail in unstructured environments.

Method: Proposes an affordable autonomous framework using only RGB-D cameras to collect 3D datasets. The method identifies 3D keypoints and robustly tracks their trajectories with motion consistency constraints to produce temporally smooth and geometrically coherent data.

Result: TrackDeform3D outperforms state-of-the-art tracking methods across diverse object categories with consistent improvements in geometric and tracking accuracy. The framework enabled creation of a high-quality, large-scale dataset of 6 deformable objects totaling 110 minutes of trajectory data.

Conclusion: The paper presents a practical solution to the data bottleneck in deformable object perception by providing an affordable autonomous data collection framework and a substantial dataset, advancing 3D representation learning for deformable objects.
...
TL;DR: A novel two-stream neural network for privacy-preserving fall detection in bathrooms using synchronized mmWave radar and vibration sensors, achieving high accuracy with low latency on edge devices.


<details>
  <summary>Details</summary>
Motivation: Existing fall detection systems treat motion and impact as loosely coupled streams, lacking explicit encoding of causal links between collapse and impact, and fail to address timing drift, object drop confounders, and edge device constraints.

Method: Two-stream architecture with Motion-Mamba branch for radar motion patterns and Impact-Griffin branch for vibration impact transients, using cross-conditioned fusion with low-rank bilinear interaction and Switch-MoE head to align tokens and suppress confounders.

Result: Achieves 96.1% accuracy, 94.8% precision, 88.0% recall, 91.1% macro F1, and 0.968 AUC on bathroom fall dataset, with 2.0% accuracy improvement, 15.8ms latency (vs 35.9ms baseline), and 10750 mJ energy consumption (vs 14200 mJ baseline) on Raspberry Pi 4B.

Conclusion: The proposed architecture effectively captures causal relationships between motion and impact for robust fall detection while meeting real-time edge computing constraints, significantly outperforming existing multimodal approaches.
...
TL;DR: ACE-LoRA is a parameter-efficient adaptation framework for generalist medical VLMs that maintains zero-shot generalization while capturing fine-grained diagnostic details through attention-based hypergraph networks and label-guided contrastive loss.


<details>
  <summary>Details</summary>
Motivation: Existing medical VLMs face a specialization-generalization trade-off: specialist models capture domain details but generalize poorly, while generalist models retain broad semantics but dilute fine-grained diagnostic cues. Current PEFT methods overlook these fine-grained details.

Method: Integrates LoRA modules into frozen image-text encoders with an Attention-based Context Enhancement Hypergraph Neural Network (ACE-HGNN) to capture higher-order contextual interactions. Uses label-guided InfoNCE loss to suppress false negatives between semantically related image-text pairs.

Result: Outperforms state-of-the-art medical VLMs and PEFT baselines across zero-shot classification, segmentation, and detection benchmarks spanning multiple domains, despite adding only 0.95M trainable parameters.

Conclusion: ACE-LoRA successfully bridges the specialization-generalization trade-off in medical VLMs, maintaining robust zero-shot generalization while capturing fine-grained diagnostic cues through parameter-efficient adaptation.
...
TL;DR: Proposes Gaussian-Hermite Sampling (GHS), a novel downsampling method that achieves accurate shift invariance in CNNs without architectural changes or extra training.


<details>
  <summary>Details</summary>
Motivation: Standard CNNs lack inherent shift invariance/equivariance due to downsampling operations, which break these properties while being essential for computational efficiency and receptive field coverage.

Method: Gaussian-Hermite Sampling (GHS) uses Gaussian-Hermite polynomials to perform shift-consistent sampling, enabling CNN layers to maintain invariance to arbitrary spatial shifts before training.

Result: Achieves 100% classification consistency under spatial shifts on CIFAR-10, CIFAR-100, and MNIST-rot datasets, while also improving classification accuracy compared to baseline CNNs.

Conclusion: GHS successfully embeds shift invariance directly at the layer level, addressing a fundamental limitation of CNNs without requiring architectural modifications or additional training procedures.
...
TL;DR: FloodLlama: A fine-tuned vision-language model using TikTok data for real-time, centimeter-resolution street-level flood depth estimation from single images, achieving high accuracy for EV/AV safety and transportation management.


<details>
  <summary>Details</summary>
Motivation: Urban flooding threatens transportation networks, but no existing system provides real-time, street-level flood depth information at centimeter resolution needed for dynamic routing, EV safety, and AV operations.

Method: Fine-tuned LLaMA 3.2-11B Vision using QLoRA on synthetic dataset (~190k images) with progressive curriculum training. Uses TikTok-based multimodal sensing pipeline and mechanistic interpretability framework for selective fine-tuning.

Result: Achieves MAE below 0.97 cm, Acc@5cm above 93.7% for deep flooding (96.8% for shallow). Tier 3 configuration gets 98.62% accuracy on real-world data. Identified layer L23 as critical depth-encoding transition, enabling 76-80% parameter reduction.

Conclusion: Provides scalable, infrastructure-free solution for real-time flood sensing with direct implications for EV safety, AV deployment, and resilient transportation management.
...
TL;DR: Proposes DVD-CL and MVD-CL methods combining counterfactual generation with dense contrastive learning for image segmentation, plus CHRO-map visualization, achieving ~94% DSC on challenging data.


<details>
  <summary>Details</summary>
Motivation: Image segmentation requires expensive manual annotations. Silver-standard (AI-generated) labels are cheaper but risk bias. Self-supervised learning is important for pre-training, but existing contrastive learning with counterfactuals doesn't extend well to pixel-level tasks.

Method: Proposes pipeline combining counterfactual generation with dense contrastive learning via Dual-View (DVD-CL) and Multi-View (MVD-CL) methods. Also introduces supervised variants using silver-standard annotations and CHRO-map visualization algorithm.
...
TL;DR: VLMs from same families have correlated errors that hurt ensemble performance. Proposed family-aware methods (HFV, QualRCCV, LCS) beat standard voting, with LCS achieving best gains without degrading any benchmark.


<details>
  <summary>Details</summary>
Motivation: Ensembling VLMs from different providers maximizes benchmark accuracy, but models from same architectural families share correlated errors that standard voting ignores, reducing effective ensemble dimensionality and creating a "Misleading tier" where correlated majority errors destroy accuracy.

Method: Three family-aware methods: 1) Hierarchical Family Voting (HFV) aggregates within families before voting across them; 2) QualRCCV weights models by calibration, family quality, and inverse family size; 3) Learned Candidate Scoring (LCS) trains cross-validated classifier to re-rank candidate answers using support breadth, family diversity, and model quality.

Result: Family-correlated errors reduce effective ensemble dimensionality to 2.5-3.6 independent voters. HFV recovers +18-26 pp on Misleading tier. QualRCCV beats calibrated voting on all three benchmarks (p<0.05). LCS achieves largest gains: +0.68% VQAv2, +0.61% TextVQA, +2.45% GQA, all significant, and never degrades any benchmark. On VQAv2 test-standard, LCS reaches 87.83% with 12 models.

Conclusion: Family-aware ensemble methods effectively address correlated errors in VLMs, with LCS being the most robust approach that consistently improves performance across benchmarks without degradation, demonstrating the importance of considering model family structure in ensemble design.
...
TL;DR: MosaicMem is a hybrid spatial memory system for video diffusion models that combines 3D patch lifting for reliable localization with native model conditioning for prompt-following, enabling consistent long-term video generation with accurate camera motion and dynamic object modeling.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models struggle with spatial memory: explicit 3D structures handle camera motion well but fail with moving objects, while implicit memory produces inaccurate camera motion even with correct poses. There's a need for a memory system that maintains consistency under camera motion, revisits, and interventions while preserving dynamic content.

Method: MosaicMem uses a hybrid spatial memory that lifts patches into 3D for reliable localization and targeted retrieval, while exploiting the model's native conditioning to preserve prompt-following generation. It features a patch-and-compose interface that spatially aligns patches in queried views, preserving persistent elements while allowing the model to inpaint evolving content. The method includes PRoPE camera conditioning and two new memory alignment techniques.

Result: Experiments show improved pose adherence compared to implicit memory and stronger dynamic modeling than explicit baselines. MosaicMem enables minute-level navigation, memory-based scene editing, and autoregressive rollout capabilities.

Conclusion: MosaicMem successfully addresses the spatial memory bottleneck in video diffusion models by combining the strengths of explicit 3D structures for localization with implicit conditioning for dynamic content, creating a robust hybrid memory system for long-term consistent video generation.
...
TL;DR: SMAL-pets is a framework that creates editable 3D dog avatars from single images using 3D Gaussian Splatting and SMAL parametric models, with multimodal editing via text prompts.


<details>
  <summary>Details</summary>
Motivation: Current 3D animal avatar creation faces challenges: lack of large annotated datasets, morphological diversity across breeds, difficulty capturing realistic fur textures, and labor-intensive manual editing/rigging for animation.

Method: Hybrid architecture combining 3D Gaussian Splatting with SMAL parametric model for anatomically grounded representation. Includes multimodal editing suite allowing refinement and animation via textual prompts.

Result: Generates high-quality, editable animal avatars from single input images that are both visually high-fidelity and anatomically accurate, with natural language control over appearance and behavior.

Conclusion: SMAL-pets provides a flexible, robust tool for animation and VR by bridging reconstruction and generative modeling, enabling user-friendly creation of animatable 3D dog avatars with natural language control.
...
TL;DR: BEV-SLD is a self-supervised LiDAR global localization method that discovers scene-specific patterns as landmarks in BEV images, achieving robust performance across diverse environments.


<details>
  <summary>Details</summary>
Motivation: Current scene-agnostic localization pipelines lack scene-specific adaptation. The authors aim to develop a method that can discover and leverage scene-specific patterns for more robust global localization across diverse environments.

Method: Uses BEV images to discover scene-specific patterns at prescribed spatial density as landmarks. Employs self-supervised learning with a consistency loss that aligns learnable global landmark coordinates with per-frame heatmaps to ensure consistent landmark detection across scenes.

Result: BEV-SLD delivers robust localization across campus, industrial, and forest environments and achieves strong performance compared to state-of-the-art methods.

Conclusion: The BEV-SLD approach successfully demonstrates that scene-specific landmark discovery through self-supervised learning can provide robust global localization across diverse real-world environments.
...
TL;DR: GazeOnce360 is an end-to-end model for multi-person 3D gaze estimation from a single upward-facing fisheye camera, addressing 360° scene coverage with novel architecture and synthetic dataset.


<details>
  <summary>Details</summary>
Motivation: Existing gaze estimation methods rely on forward-facing cameras with constrained viewpoints, leaving underexplored the challenge of estimating gaze for multiple people across a 360° scene from an upward fisheye perspective.

Method: Proposes GazeOnce360 model with rotational convolutions to handle fisheye distortion, eye landmark supervision, and dual-resolution architecture combining global low-resolution context with high-resolution local eye regions. Also introduces MPSGaze360 synthetic dataset created with Unreal Engine.

Result: Experimental results demonstrate effectiveness of each model component, showing feasibility of fisheye-based 360° gaze estimation in practical multi-person scenarios.

Conclusion: This work establishes the potential of using upward-facing fisheye cameras for comprehensive multi-person gaze estimation across full 360° scenes, enabled by specialized architecture and synthetic training data.
...
TL;DR: MLLMs with expert-informed prompts outperform specialized CNNs and human examiners for iris presentation attack detection while maintaining privacy constraints.


<details>
  <summary>Details</summary>
Motivation: Iris PAD faces practical barriers: collecting future unknown attack data is impossible, diverse data collection is expensive, and sharing biometric data raises privacy concerns. Need adaptable solutions for emerging attack vectors.

Method: Use general-purpose MLLMs augmented with human expert knowledge under strict privacy constraints. Analyze vision encoder embeddings, use structured prompts with human salience (verbal descriptions of attack indicators), test on IRB-restricted dataset of 224 iris images spanning 7 attack types using university-approved services (Gemini 2.5 Pro) or locally-hosted models (Llama 3.2-Vision).

Result: Pre-trained vision transformers in MLLMs inherently cluster many iris attack types despite no explicit training. With expert-informed prompts, Gemini outperforms specialized CNN baseline and human examiners. Locally-deployable Llama achieves near-human performance.

Conclusion: MLLMs deployable within institutional privacy constraints offer a viable path for iris PAD, combining pre-trained vision capabilities with human expert knowledge through structured prompts.
...
TL;DR: Patient4D improves 3D body mesh recovery from monocular surgical video by exploiting patient stationarity priors to handle occlusion from draping and moving camera viewpoints.


<details>
  <summary>Details</summary>
Motivation: Existing human mesh recovery methods fail under surgical conditions where patients are stationary under draping while cameras move continuously, requiring new approaches that leverage stationarity priors.

Method: A stationarity-constrained pipeline combining foundation models with geometric mechanisms: Pose Locking anchors pose using stable keyframes, and Rigid Fallback handles severe occlusion through silhouette-guided rigid alignment.

Result: Achieves 0.75 mean IoU under surgical drape occlusion, reducing failure frames from 30.5% to 1.3% compared to baselines, with evaluation on 4,680 synthetic sequences and three public benchmarks.

Conclusion: Exploiting stationarity priors substantially improves monocular reconstruction in clinical AR scenarios, demonstrating robust performance under challenging surgical conditions.
...
<details>
  <summary>Details</summary>
Motivation: Reliable product identification from images is critical in industrial/commercial applications where incorrect matches lead to costly failures. Need to evaluate how well modern embedding models perform for exact instance retrieval in production environments.

Method: Structured benchmark of visual embedding models including open-source foundation models, proprietary multi-modal systems, and domain-specific vision-only models. Evaluation uses unified image-to-image retrieval protocol on curated industrial datasets (Manufacturing, Automotive, DIY, Retail) and public benchmarks, without post-processing to isolate retrieval capability.

Result: Results provide insights into how contemporary foundation and unified embedding models transfer to fine-grained instance retrieval tasks, and how they compare to models explicitly trained for industrial applications.

Conclusion: Benchmark informs practitioners and researchers about strengths/limitations of current visual embedding approaches for production-level product identification, emphasizing realistic constraints and exact instance matching requirements. Interactive website available for detailed results.
...
TL;DR: SA-CycleGAN-2.5D is a domain adaptation framework that harmonizes multi-site neuroimaging by addressing scanner-induced covariate shifts through 2.5D tri-planar injection, global self-attention, and spectral normalization.


<details>
  <summary>Details</summary>
Motivation: Scanner-induced covariate shifts in multi-site neuroimaging confound analysis as acquisition variance often exceeds biological pathology variance. Existing methods either operate in feature space (precluding spatial tasks) or are limited by local receptive fields, failing to model global intensity correlations from field-strength bias.

Method: A domain adaptation framework with three innovations: (1) 2.5D tri-planar manifold injection preserving through-plane gradients at O(HW) complexity; (2) U-ResNet generator with dense voxel-to-voxel self-attention to model global scanner field biases; (3) Spectrally-normalized discriminator constraining Lipschitz constant for stable adversarial optimization.

Result: On 654 glioma patients across BraTS and UPenn-GBM domains: reduces Maximum Mean Discrepancy by 99.1% (1.729→0.015), degrades domain classifier accuracy to near-chance (59.7%). Ablation shows global attention is statistically essential (Cohen's d=1.32, p<0.001) for heterogeneous-to-homogeneous translation.

Conclusion: The framework bridges 2D efficiency and 3D consistency to yield voxel-level harmonized images that preserve tumor pathophysiology, enabling reproducible multi-center radiomic analysis by effectively addressing scanner-induced distribution shifts.
...
<details>
  <summary>Details</summary>
Motivation: Current Gaussian streaming pipelines use fixed anchor selection (8,192 anchors via FPS) regardless of scene complexity, leading to computational over-allocation under strict budgets.

Method: Proposes Efficient Gaussian Streaming (EGS) - a plug-in, reinforcement-learned policy that jointly selects anchor budget and informative anchors under discrete constraints, balancing reconstruction quality and runtime using spatial Gaussian features.

Result: At 256 anchors (32× fewer than 8,192), EGS improves PSNR by +0.52-0.61 dB while running 1.29-1.35× faster than baseline. In high-quality refinement, remains competitive with full-anchor baseline at substantially lower budgets.

Conclusion: EGS provides a budget-aware anchor sampling solution that consistently improves the quality-efficiency trade-off over traditional FPS sampling for dynamic Gaussian streaming reconstruction.
...
TL;DR: MLLMs have intrinsic segmentation capacity that emerges through attention mechanisms in LLM layers, with adapters causing initial representation drop-off that gets recovered via cross-token attention refinement.


<details>
  <summary>Details</summary>
Motivation: Despite MLLMs being applied to pixel-level vision tasks like segmentation, their intrinsic capacity for spatial understanding remains poorly understood. The paper aims to investigate how MLLMs process visual information for segmentation tasks.

Method: Used layerwise linear probing across the entire MLLM pipeline (vision encoder, adapter, LLM), conducted attention knockout analysis to test cross-token attention refinement, and evaluated bidirectional attention among image tokens for spatial consistency.

Result: Adapters introduce segmentation representation drop-off, but LLM layers progressively recover through attention-mediated refinement where correctly classified tokens steer misclassified neighbors toward correct labels. Bidirectional attention among image tokens alleviates limitations of causal attention at early positions.

Conclusion: MLLMs process visual information for segmentation through attention mechanisms that refine representations, with LLM layers recovering adapter-induced drop-offs. These findings provide mechanistic insights for designing future segmentation-capable models.
...
TL;DR: GigaWorld-Policy is a faster, more efficient World-Action Model that decouples action prediction from video generation, achieving 9x speedup and 7% higher success rates than prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing World-Action Models suffer from two bottlenecks: high inference overhead from joint reasoning over future visuals and actions, and entangled representations that make motion prediction dependent on video forecast quality.

Method: Action-centered WAM with two coupled components: predicts future actions from current observation, and optionally generates future videos conditioned on predicted actions. Uses causal design to prevent video tokens from influencing action tokens, making video generation optional at inference.

Result: 9x faster inference than leading baseline Motus, 7% higher task success rates, and 95% performance improvement over pi-0.5 on RoboTwin 2.0.

Conclusion: GigaWorld-Policy addresses efficiency bottlenecks in WAMs by decoupling action prediction from video generation, enabling faster deployment while maintaining or improving performance through richer learning signals.
...
TL;DR: LED is a new benchmark for evaluating structural reasoning in document layout analysis that goes beyond traditional overlap metrics to detect logical errors like region merging, splitting, and omission.


<details>
  <summary>Details</summary>
Motivation: Current document layout analysis models still make structural errors (region merging, splitting, omission) that conventional overlap-based metrics like IoU and mAP fail to capture, limiting proper assessment of structural reasoning capabilities.

Method: Proposes Layout Error Detection (LED) benchmark with eight standardized error types, provides quantitative rules and injection algorithms for realistic error simulation, constructs LED-Dataset, and designs three evaluation tasks for document-level and element-level error detection and classification.

Result: Experiments with state-of-the-art multimodal models show LED enables fine-grained, interpretable assessment of structural understanding, revealing clear weaknesses across modalities and architectures.

Conclusion: LED establishes a unified and explainable benchmark for diagnosing the structural robustness and reasoning capability of document understanding models, moving beyond surface-level accuracy to evaluate logical consistency.
...
TL;DR: Proposed ConfusionBench - a high-quality benchmark for student confusion recognition in educational videos, featuring multi-stage filtering pipeline, balanced datasets, and baseline evaluations showing proprietary models outperform open-source ones but with different error patterns.


<details>
  <summary>Details</summary>
Motivation: Existing confusion datasets have noisy labels, coarse temporal annotations, and limited expert validation, hindering reliable fine-grained recognition and temporally grounded analysis in educational AI.

Method: Multi-stage filtering pipeline integrating model-assisted screening, researcher curation, and expert validation to build ConfusionBench benchmark with balanced confusion recognition dataset and video localization dataset.
...
TL;DR: DANCE: A dynamic pruning framework for 3D CNNs that adapts computational complexity based on input content to maximize energy efficiency with minimal performance impact.


<details>
  <summary>Details</summary>
Motivation: Modern CNNs fail to adapt computational complexity to input samples dynamically, leading to inefficient energy consumption. There's a need for fine-grained, input-aware pruning that maintains performance while maximizing power efficiency.

Method: Two-step approach: 1) Activation Variability Amplification (AVA) - retrain 3D CNN to increase variance of neuron activation magnitudes; 2) Adaptive Activation Pruning (AAP) - train lightweight activation controller network to dynamically prune frames, channels, and features based on first-layer output statistics.

Result: Achieves substantial MAC operation and memory access savings through sparsity. Hardware validation shows 1.37X speedup on NVIDIA Jetson Nano GPU and 2.22X speedup on Qualcomm Snapdragon 8 Gen 1, with up to 1.47X higher energy efficiency than state-of-the-art.
...
### [48] [GMT: Goal-Conditioned Multimodal Transformer for 6-DOF Object Trajectory Synthesis in 3D Scenes](https://arxiv.org/abs/2603.17993)
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for 6-DOF object manipulation trajectory synthesis rely on 2D or partial 3D representations, limiting their ability to capture full scene geometry and achieve precise trajectory control in complex 3D environments.

Method: GMT uses a multimodal transformer framework that represents trajectories as continuous 6-DOF pose sequences and employs a tailored conditioning strategy to fuse geometric (3D bounding boxes, point clouds), semantic (object categories), contextual, and goal-oriented (target end poses) information.
...
Conclusion: GMT establishes a new benchmark for learning-based manipulation planning, demonstrating the effectiveness of multimodal 3D representations for generating realistic and goal-directed object manipulation trajectories in complex scenes.
...
<details>
  <summary>Details</summary>
Motivation: Existing story visualization methods struggle with subject inconsistency and identity drift, especially for complex narratives and character interactions.

Method: Two-stage framework: 1) Group-Shared Attention (GSA) for lossless cross-sample information flow to maintain identity consistency, 2) Direct Preference Optimization (DPO) to align outputs with human aesthetic and narrative preferences.
...
Conclusion: The proposed framework effectively addresses identity consistency challenges in story visualization through structural attention mechanisms and preference-based optimization, enabling robust and consistent narrative generation.
...
TL;DR: Proposes multimodal 3D CNN for Alzheimer's disease classification using volumetric MRI, achieving 72.34% accuracy with anatomically meaningful attention patterns.


<details>
  <summary>Details</summary>
Motivation: Clinical neuroimaging relies on full 3D brain structure, while many existing studies use 2D slices. Volumetric analysis may better capture spatial relationships relevant to Alzheimer's disease progression.

Method: Multimodal 3D convolutional neural network using raw OASIS 1 MRI volumes combined with gray matter, white matter, and cerebrospinal fluid probability maps from FSL FAST segmentation.

Result: 72.34% ± 4.66% accuracy and 0.7781 ± 0.0365 ROC AUC with 5-fold subject-level cross-validation. GradCAM shows model focuses on medial temporal lobe and ventricular areas associated with Alzheimer's.

Conclusion: The multimodal 3D framework establishes reproducible subject-level benchmark and highlights benefits of volumetric MRI analysis for Alzheimer's disease classification.
...
TL;DR: Symphony is a multi-agent system that improves long-form video understanding by decomposing tasks into fine-grained subtasks with deep reasoning collaboration and VLM-based video segment relevance analysis.


<details>
  <summary>Details</summary>
Motivation: Current MLLM agents struggle with long-form video understanding due to high information density and extended temporal spans. Simple task decomposition and collaboration are insufficient, and embedding-based retrieval loses key information for complex problems.

Method: Symphony emulates human cognition patterns by decomposing LVU into fine-grained subtasks with a deep reasoning collaboration mechanism enhanced by reflection. It also uses VLM-based grounding to analyze tasks and assess video segment relevance for locating complex problems with implicit intentions.

Result: Symphony achieves state-of-the-art performance on LVBench, LongVideoBench, VideoMME, and MLVU, with a 5.0% improvement over prior state-of-the-art on LVBench.

Conclusion: Symphony effectively addresses limitations in long-form video understanding through multi-agent collaboration and VLM-based grounding, demonstrating significant performance improvements across multiple benchmarks.
...
TL;DR: R²VLM: A recurrent reasoning VLM that processes local video snippets iteratively with evolving Chain of Thought to estimate long-horizon task progress efficiently.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based methods for task progress estimation primarily use video understanding capabilities but neglect complex reasoning potential, and processing long videos is computationally prohibitive for real-world deployment.

Method: Proposes R²VLM with recurrent reasoning framework that processes local video snippets iteratively while maintaining global context through evolving Chain of Thought (CoT) that records task decomposition, key steps, and completion status.

Result: Achieves state-of-the-art performance in long-horizon task progress estimation and demonstrates strong generalization in downstream applications including progress-enhanced policy learning, reward modeling for RL, and proactive assistance.

Conclusion: R²VLM effectively addresses computational challenges of long video processing while preserving essential reasoning capabilities, enabling efficient and accurate progress estimation for embodied agents in complex tasks.
...
TL;DR: Proposal-free Query-Guided Network (QGN) for Grounded Multimodal Named Entity Recognition that unifies multimodal reasoning through text guidance to overcome limitations of two-step approaches using pre-trained object detectors.


<details>
  <summary>Details</summary>
Motivation: Existing GMNER approaches use two-step methods with pre-trained object detectors that operate independently of textual entities, causing them to detect common objects while missing fine-grained regions needed for named entities, leading to imprecision and performance issues.

Method: Proposal-free Query-Guided Network (QGN) that unifies multimodal reasoning and decoding through text guidance and cross-modal interaction, enabling direct grounding without separate object detection steps.

Result: QGN achieves top performance among compared GMNER models on widely used benchmarks, demonstrating accurate grounding and robust performance in open-domain scenarios.

Conclusion: The proposed QGN approach effectively addresses limitations of traditional two-step GMNER methods by providing unified multimodal reasoning through text guidance, resulting in superior performance for grounded named entity recognition.
...
TL;DR: MedSAD-CLIP adapts CLIP for medical anomaly detection using fine-grained text-visual attention, lightweight adapters, and contrastive loss to improve lesion localization while preserving CLIP's generalization capabilities.


<details>
  <summary>Details</summary>
Motivation: Current CLIP-based medical anomaly detection methods rely on global representations and weak supervision, producing coarse localization and limited segmentation quality. The authors aim to improve this by leveraging supervised adaptation when limited labeled abnormal data is available.

Method: Proposes MedSAD-CLIP with Token-Patch Cross-Attention (TPCA) for fine-grained text-visual cues, lightweight image adapters and learnable prompt tokens to adapt CLIP to medical domain, and Margin-based image-text Contrastive Loss to enhance global feature discrimination between normal and abnormal representations.

Result: Superior performance on four diverse benchmarks (Brain, Retina, Lung, and Breast datasets) for both pixel-level segmentation and image-level classification compared to state-of-the-art methods.

Conclusion: Supervised CLIP adaptation shows potential as a unified and scalable paradigm for medical anomaly understanding, effectively leveraging limited labeled data while preserving CLIP's generalization capabilities.
...
TL;DR: FineViT is a new vision encoder for MLLMs that addresses CLIP's limitations in fine-grained perception through progressive training with dense recaptions and local caption alignment.


<details>
  <summary>Details</summary>
Motivation: Current CLIP-based visual encoders in MLLMs struggle with dense spatial tasks due to low-resolution pretraining and noisy web-crawled image-text pairs, losing visual details and limiting fine-grained perception.

Method: Two-stage progressive training: 1) Train from scratch at high native resolution on billions of global recaptioned image-text pairs for semantic foundation, 2) Enhance local perception through LLM alignment using curated FineCap-450M dataset with 450M+ high-quality local captions.
...
Conclusion: FineViT provides a powerful new baseline for fine-grained visual perception in MLLMs, demonstrating the effectiveness of progressive training with dense recaptions and local caption alignment to overcome CLIP's limitations.
...
TL;DR: EvoGuard is an agentic framework for AI-generated image detection that orchestrates multiple detectors through dynamic tool selection and multi-turn reasoning, achieving state-of-the-art accuracy without fine-grained annotations.


<details>
  <summary>Details</summary>
Motivation: Current AIGI detection methods have limited extensibility and require expensive training data annotations, while struggling to handle complex real-world environments where AI-generated images pose misinformation risks.

Method: EvoGuard uses an agentic framework that encapsulates various MLLM and non-MLLM detectors as callable tools, coordinates them through capability-aware dynamic orchestration with autonomous planning and reflection, and employs GRPO-based Agentic Reinforcement Learning optimized with only binary labels.

Result: EvoGuard achieves state-of-the-art accuracy while mitigating bias between positive and negative samples, allows plug-and-play integration of new detectors without retraining, and eliminates reliance on fine-grained annotations.

Conclusion: EvoGuard provides a highly practical, long-term solution to evolving AIGI threats through its agentic framework that leverages complementary strengths of heterogeneous detectors and requires only low-cost binary labels for optimization.
...
TL;DR: OnlineHMR: A fully online framework for 3D human mesh recovery from monocular videos that supports real-time streaming inference with causal processing, temporal consistency, and world-grounded alignment.


<details>
  <summary>Details</summary>
Motivation: Most existing human mesh recovery methods are offline and rely on future frames or global optimization, limiting their applicability in interactive scenarios like AR/VR and telepresence where real-time feedback is crucial.

Method: Two-branch architecture with causal key-value cache design and sliding-window learning for streaming inference, combined with human-centric incremental SLAM for online world-grounded alignment and physically plausible trajectory correction.

Result: Achieves performance comparable to existing chunk-based approaches on EMDB benchmark and highly dynamic custom videos while uniquely supporting online processing.

Conclusion: OnlineHMR successfully addresses the limitations of offline methods by providing a fully online framework that satisfies causality, faithfulness, temporal consistency, and efficiency requirements for interactive applications.
...
TL;DR: New synthetic dataset for benchmarking 3D reconstruction in asset inspection with challenging conditions like reflections, transparency, and dense capture trajectories.


<details>
  <summary>Details</summary>
Motivation: Asset management requires high-fidelity 3D models for maintenance and repair, but existing datasets lack the challenging conditions (reflections, transparency, dense trajectories) found in real inspection scenarios.

Method: Created synthetic dataset with ground truth depth maps, camera poses, and mesh models of three scenes with simulated inspection trajectories and varying surface conditions on non-Lambertian content.

Result: Evaluation shows state-of-the-art reconstruction methods struggle significantly with dense capture trajectories and complex surface conditions, revealing a critical scalability gap.

Conclusion: The dataset exposes limitations of current approaches and points toward new research directions for deployable 3D reconstruction in asset inspection applications.
...
### [59] [Understanding and Defending VLM Jailbreaks via Jailbreak-Related Representation Shift](https://arxiv.org/abs/2603.17372)
*Zhihua Wei,Qiang Li,Jian Ruan,Zhenxin Qin,Leilei Wen,Dongrui Liu,Wen Shen*

Main category: cs.CV

TL;DR: VLMs have weakened safety alignment with visual modality - images can jailbreak even when text contains harmful intent. Jailbreaks occur not from failure to recognize harm, but from visual modality shifting representations toward a specific jailbreak state that prevents refusal triggering.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models (VLMs) show weakened safety alignment when visual modality is integrated. Even with explicit harmful text prompts, adding images substantially increases jailbreak success rates, creating a critical safety vulnerability that needs understanding and mitigation.

Method: Analyze VLM representations to show jailbreak samples form distinct internal states separable from refusal samples. Identify a "jailbreak direction" in representation space and define "jailbreak-related shift" as image-induced shift along this direction. Propose JRS-Rem defense that removes this shift at inference time.

Result: Jailbreak-related shift reliably characterizes jailbreak behavior across diverse scenarios. JRS-Rem defense provides strong protection against jailbreaks while preserving performance on benign tasks, demonstrating effectiveness across multiple experimental scenarios.

Conclusion: Jailbreaks in VLMs occur due to visual modality shifting representations toward specific jailbreak states rather than failure to recognize harmful intent. The jailbreak-related shift provides unified explanation for diverse jailbreak scenarios, and removing it (JRS-Rem) effectively enhances VLM safety without compromising benign task performance.
...
TL;DR: MCoT-MVS: A novel CIR method using multi-modal chain-of-thought reasoning to guide multi-level vision feature selection for better semantic extraction from reference images.


<details>
  <summary>Details</summary>
Motivation: Existing CIR methods struggle to extract correct semantic cues from reference images that reflect user intent under textual modifications, often suffering from irrelevant visual noise interference.

Method: Uses MLLM for chain-of-thought reasoning to generate retained/removed/target-inferred texts, which guide patch-level and instance-level visual attention selection modules, followed by weighted hierarchical combination for embedding alignment.

Result: Achieves state-of-the-art performance on CIRR and FashionIQ benchmarks, consistently outperforming existing methods.

Conclusion: The proposed MCoT-MVS effectively integrates multi-modal reasoning with multi-level vision selection to improve composed image retrieval by better capturing user intent and reducing visual noise interference.
...
TL;DR: ASSR-EIC: A single-model extreme image compression framework using arbitrary-scale super-resolution for variable-rate compression with adaptive reconstruction across diverse bitrates.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based extreme compression methods require separate models for each bitrate (computationally expensive), and current super-resolution approaches struggle at ultra-low bitrates with fixed scaling factors that prevent flexible adaptation.

Method: Proposes ASSR-EIC with: 1) arbitrary-scale downsampling module for controllable rate reduction at encoder, 2) diffusion-based joint degradation-aware ASSR decoder for rate-adaptive reconstruction, 3) compression-rescaling aware diffusion prior with global adaptor and local modulator for fine-grained restoration, 4) dual semantic-enhanced design.

Result: State-of-the-art performance in extreme image compression with flexible bitrate control and adaptive rate-dependent reconstruction within a single model.

Conclusion: ASSR-EIC successfully addresses limitations of existing methods by enabling variable-rate extreme compression with adaptive reconstruction using arbitrary-scale super-resolution, offering practical deployment advantages over multi-model approaches.
...
TL;DR: Material Magic Wand: A tool for automatic material-aware part grouping in untextured 3D meshes using contrastive learning embeddings.


<details>
  <summary>Details</summary>
Motivation: Manual selection of repeated structures with same material in 3D meshes (like scales of pinecones or building windows) is tedious and time-consuming for artists during material assignment.

Method: Proposes a part encoder generating material-aware embeddings for 3D parts using both local geometry and global context. Trained with supervised contrastive loss to bring embeddings of material-consistent parts closer while separating different materials.

Result: Introduces benchmark dataset of 100 shapes with 241 part-level queries. Extensive experiments verify effectiveness and demonstrate practical value in interactive material assignment application.

Conclusion: Material Magic Wand provides an effective solution for automatic material-aware part grouping, reducing manual work for artists in 3D material assignment workflows.
...
TL;DR: AdaZoom-GUI: Adaptive zoom-based GUI grounding framework with instruction refinement and conditional zoom-in for precise element localization in high-resolution GUI screenshots.


<details>
  <summary>Details</summary>
Motivation: GUI grounding is challenging due to high-resolution images, small UI elements, and ambiguous user instructions. Current methods struggle with precise localization and understanding of natural language commands in complex GUI environments.

Method: 1) Instruction refinement module rewrites natural language commands into explicit descriptions; 2) Conditional zoom-in strategy performs second-stage inference on predicted small elements; 3) Trained using Group Relative Policy Optimization (GRPO) to predict click coordinates and bounding boxes; 4) Constructed high-quality GUI grounding dataset.
...
Conclusion: AdaZoom-GUI effectively addresses GUI grounding challenges through adaptive zooming and instruction refinement, enabling more accurate and efficient automated GUI interaction for vision-language models.
...
TL;DR: VirPro introduces visual-referred probabilistic prompt learning for monocular 3D detection, using adaptive prompts and multi-Gaussian modeling to capture visual uncertainties, achieving 4.8% AP improvement on KITTI.


<details>
  <summary>Details</summary>
Motivation: Hand-crafted textual descriptions fail to capture visual diversity across scenes, limiting scene-aware representation learning in weakly supervised monocular 3D detection.

Method: Proposes VirPro with Adaptive Prompt Bank (APB) storing learnable instance-conditioned prompts, Multi-Gaussian Prompt Modeling (MGPM) incorporating visual features into textual embeddings, and RoI-level contrastive matching for modality alignment.

Result: Extensive experiments on KITTI benchmark show consistent substantial performance gains, achieving up to 4.8% average precision improvement over baseline.

Conclusion: VirPro's adaptive multi-modal pretraining paradigm effectively addresses visual diversity challenges in weakly supervised monocular 3D detection, enhancing semantic coherence and scene-aware representations.
...
### [65] [Shot-Aware Frame Sampling for Video Understanding](https://arxiv.org/abs/2603.17374)
*Mengyu Zhao,Di Fu,Yongyu Xie,Jiaxing Zhang,Zhigang Yuan,Shirin Jalali,Yong Cao*

Main category: cs.CV

TL;DR: InfoShot is a task-agnostic frame sampler for long videos that selects complementary keyframes from each shot to balance overall coverage with critical brief events, improving VLM performance under frame constraints.


<details>
  <summary>Details</summary>
Motivation: Existing video frame samplers struggle to balance broad video coverage with brief but critical events when only a few frames can be retained, leading to unreliable predictions in long-video understanding with VLMs.

Method: InfoShot partitions videos into semantically consistent shots, then selects two complementary keyframes per shot: one representing main content and one capturing unusual within-shot changes, guided by an information-theoretic objective.

Result: InfoShot improves anomaly hit rate and downstream Video-QA accuracy under frame constraints, while matching or outperforming baselines on standard video understanding benchmarks. SynFlash benchmark enables better evaluation of short-lived events.

Conclusion: InfoShot effectively balances video coverage with critical events through shot-aware sampling, enhancing long-video understanding with VLMs without requiring retraining, and SynFlash provides better evaluation for short-lived anomalies.
...
### [66] [Revisiting Cross-Attention Mechanisms: Leveraging Beneficial Noise for Domain-Adaptive Learning](https://arxiv.org/abs/2603.17474)
*Zelin Zang,Yehui Yang,Fei Wang,Liangyu Li,Baigui Sun*

Main category: cs.CV

TL;DR: DACSM framework uses beneficial noise in cross-attention to handle domain/style gaps and cross-scale matching for scale variations, achieving SOTA UDA performance.


<details>
  <summary>Details</summary>
Motivation: UDA suffers from domain and scale gaps that degrade performance, and existing cross-attention transformers struggle to preserve content semantics under large appearance and scale variations.

Method: Proposes DACSM framework with Domain-Adaptive Transformer (DAT) that injects beneficial noise into cross-attention to disentangle domain-shared content from domain-specific style, and Cross-Scale Matching (CSM) module for adaptive multi-resolution feature alignment.

Result: Achieves state-of-the-art performance on VisDA-2017, Office-Home, and DomainNet, with up to +2.3% improvement over CDTrans on VisDA-2017 and +5.9% gain on challenging "truck" class.

Conclusion: Combining domain translation, beneficial-noise-enhanced attention, and scale-aware alignment effectively addresses domain/style gaps and scale discrepancies for robust cross-domain representation learning.
...
### [67] [Stereo World Model: Camera-Guided Stereo Video Generation](https://arxiv.org/abs/2603.17375)
*Yang-Tian Sun,Zehuan Huang,Yifan Niu,Lin Ma,Yan-Pei Cao,Yuewen Ma,Xiaojuan Qi*

Main category: cs.CV

TL;DR: StereoWorld is a camera-conditioned stereo world model that jointly learns appearance and binocular geometry for end-to-end stereo video generation, achieving better stereo consistency and faster generation than monocular approaches.


<details>
  <summary>Details</summary>
Motivation: Current approaches for stereo video generation often rely on monocular RGB or RGBD methods, then convert to stereo, which can lead to inconsistencies. There's a need for an end-to-end stereo generation approach that directly learns binocular geometry while maintaining efficiency.

Method: Two key designs: (1) unified camera-frame RoPE that augments latent tokens with camera-aware rotary positional encoding for view- and time-consistent conditioning, and (2) stereo-aware attention decomposition that factors 4D attention into 3D intra-view attention plus horizontal row attention using epipolar prior.

Result: StereoWorld improves stereo consistency, disparity accuracy, and camera-motion fidelity over monocular-then-convert pipelines, achieving more than 3x faster generation with 5% gain in viewpoint consistency. Enables end-to-end binocular VR rendering and enhances embodied policy learning.

Conclusion: StereoWorld demonstrates that end-to-end stereo video generation with joint appearance and geometry learning is feasible and superior to conversion-based approaches, enabling new applications in VR rendering and embodied AI while being more computationally efficient.
...
### [68] [UniSAFE: A Comprehensive Benchmark for Safety Evaluation of Unified Multimodal Models](https://arxiv.org/abs/2603.17476)
...
TL;DR: UniSAFE is the first comprehensive benchmark for evaluating system-level safety risks in Unified Multimodal Models across 7 modality combinations, revealing critical vulnerabilities in current models.


<details>
  <summary>Details</summary>
Motivation: Unified Multimodal Models introduce new safety risks not seen in single-task models, but existing safety benchmarks are fragmented across tasks and modalities, limiting comprehensive evaluation of complex system-level vulnerabilities.

Method: Created UniSAFE benchmark with shared-target design that projects common risk scenarios across task-specific I/O configurations, enabling controlled cross-task comparisons. Comprises 6,802 curated instances across 7 modality combinations including conventional tasks and novel multimodal-context image generation.
...
Conclusion: Findings highlight the need for stronger system-level safety alignment for Unified Multimodal Models. The benchmark provides a comprehensive tool for evaluating these complex safety risks.
...
### [69] [VisionNVS: Self-Supervised Inpainting for Novel View Synthesis under the Virtual-Shift Paradigm](https://arxiv.org/abs/2603.17382)
*Hongbo Lu,Liang Yao,Chenghao He,Fan Liu,Wenlong Liao,Tao He,Pai Peng*

Main category: cs.CV

TL;DR: VisionNVS reformulates novel view synthesis for autonomous driving from extrapolation to self-supervised inpainting using virtual shifts and pseudo-3D seam synthesis, achieving better results than LiDAR methods.


<details>
  <summary>Details</summary>
Motivation: The fundamental bottleneck in Novel View Synthesis for autonomous driving is the supervision gap - models need to synthesize unseen views during inference but lack ground truth images for these shifted poses during training.

Method: 1) Virtual-Shift strategy using monocular depth proxies to simulate occlusion patterns and map them onto original views, transforming the problem into self-supervised inpainting. 2) Pseudo-3D Seam Synthesis that integrates visual data from adjacent cameras to model real-world photometric discrepancies and calibration errors.

Result: VisionNVS achieves superior geometric fidelity and visual quality compared to LiDAR-dependent baselines, offering a robust solution for scalable driving simulation.

Conclusion: The camera-only VisionNVS framework fundamentally reformulates view synthesis from an ill-posed extrapolation problem into a self-supervised inpainting task, effectively eliminating the domain gap inherent in previous approaches.
...
### [70] [Toward Phonology-Guided Sign Language Motion Generation: A Diffusion Baseline and Conditioning Analysis](https://arxiv.org/abs/2603.17388)
*Rui Hong,Jana Kosecka*

Main category: cs.CV

TL;DR: A diffusion-based 3D sign language motion generation model using SMPL-X representation outperforms state-of-the-art CVAE methods, with analysis showing that translating symbolic phonological attributes to natural language is crucial for effective CLIP-based conditioning.


<details>
  <summary>Details</summary>
Motivation: Generating natural, correct, and visually smooth 3D avatar sign language motion from text inputs remains challenging, motivating exploration of phonological attribute conditioning using ASL-LEX 2.0 annotations.

Method: Established a diffusion baseline using Human Motion MDM-style diffusion model with SMPL-X representation, then systematically studied text conditioning using different encoders (CLIP vs. T5), conditioning modes (gloss-only vs. gloss+phonological attributes), and attribute notation formats (symbolic vs. natural language).

Result: The diffusion baseline outperforms SignAvatar (state-of-the-art CVAE method) on gloss discriminability metrics. Translating symbolic ASL-LEX notations to natural language is necessary for effective CLIP-based attribute conditioning, while T5 is largely unaffected. The best-performing variant (CLIP with mapped attributes) outperforms SignAvatar across all metrics.

Conclusion: Input representation is a critical factor for text-encoder-based attribute conditioning, motivating structured conditioning approaches where gloss and phonological attributes are encoded through independent pathways.
...
TL;DR: AdapTS is a unified Teacher-Student framework for multi-class continual visual anomaly detection that uses lightweight adapters and prototype-based task identification, achieving comparable performance with drastically reduced memory overhead (8MB vs 95-1120MB).


<details>
  <summary>Details</summary>
Motivation: Existing VAD methods are limited to single-category scenarios and fail to address multi-class and continual learning needs in real-world industrial environments. Teacher-Student architectures are efficient but unexplored for continual settings.

Method: AdapTS uses a single shared frozen backbone with lightweight trainable adapters injected into the student pathway. It employs segmentation-guided training objectives with synthetic Perlin noise, and a prototype-based task identification mechanism that dynamically selects adapters at inference.

Result: AdapTS matches existing TS methods' performance on MVTec AD and VisA datasets in multi-class and continual learning scenarios while reducing memory overhead dramatically. AdapTS-S requires only 8MB additional memory (13x less than STFPM, 48x less than RD4AD, 149x less than DeSTSeg).

Conclusion: AdapTS provides a highly scalable solution for edge deployment in complex industrial environments by addressing multi-class continual VAD with minimal memory overhead while maintaining competitive performance.
...
### [72] [Harnessing the Power of Foundation Models for Accurate Material Classification](https://arxiv.org/abs/2603.17390)
*Qingran Lin,Fengwei Yang,Chaolun Zhu*

Main category: cs.CV

TL;DR: A novel framework that uses vision-language foundation models to overcome data scarcity in material classification through synthetic data generation and prior incorporation.


<details>
  <summary>Details</summary>
Motivation: Material classification is crucial for computer vision/graphics but suffers from limited annotated data, and existing vision-language model solutions still underperform for material recognition tasks.

Method: Two key innovations: (1) image generation and auto-labeling pipeline creating diverse material-centric training data with fused object-material text prompts, (2) prior incorporation strategy distilling VLM information combined with joint fine-tuning of vision foundation models with VLM-derived priors.

Result: Significant improvements on multiple datasets; synthetic dataset effectively captures real-world material characteristics; VLM prior integration substantially enhances final performance.

Conclusion: The proposed framework successfully addresses data scarcity in material classification by leveraging foundation models through synthetic data generation and prior distillation, achieving superior performance while maintaining generalizability.
...
### [73] [Rel-Zero: Harnessing Patch-Pair Invariance for Robust Zero-Watermarking Against AI Editing](https://arxiv.org/abs/2603.17531)
*Pengzhen Chen,Yanwei Liu,Xiaoyan Gu,Xiaojun Chen,Wu Liu,Weiping Wang*

Main category: cs.CV

TL;DR: Rel-Zero is a novel zero-watermarking framework that uses editing-invariant patch relations for content authentication without modifying original images, achieving superior robustness against AI-based editing.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based image editing threatens digital content authenticity. Traditional watermarking methods compromise visual fidelity with perceptible perturbations, while existing zero-watermarking approaches using global features fail against sophisticated manipulations.

Method: Rel-Zero leverages the observation that while individual image patches change during AI editing, relational distances between patch pairs remain relatively invariant. The framework extracts zero-watermarks from these editing-invariant patch relations without modifying original images.

Result: Extensive experiments show Rel-Zero achieves substantially improved robustness across diverse editing models and manipulations compared to prior zero-watermarking approaches.

Conclusion: By grounding watermarks in intrinsic structural consistency rather than absolute appearance, Rel-Zero provides a non-invasive yet resilient mechanism for content authentication against AI-based editing threats.
...
### [74] [Gesture-Aware Pretraining and Token Fusion for 3D Hand Pose Estimation](https://arxiv.org/abs/2603.17396)
*Rui Hong,Jana Kosecka*

Main category: cs.CV

TL;DR: Gesture semantics improve 3D hand pose estimation via two-stage framework: gesture-aware pretraining with InterHand2.6M labels, followed by per-joint Transformer guided by gesture embeddings for MANO parameter regression.


<details>
  <summary>Details</summary>
Motivation: 3D hand pose estimation from monocular RGB is crucial for AR/VR, HCI, and sign language applications. Gesture labels provide semantic information that can serve as powerful inductive bias to improve pose estimation accuracy.

Method: Two-stage framework: 1) Gesture-aware pretraining learns informative embedding space using coarse and fine gesture labels from InterHand2.6M dataset. 2) Per-joint token Transformer uses gesture embeddings as intermediate representations for final regression of MANO hand parameters. Training uses layered objective over parameters, joints, and structural constraints.

Result: Experiments on InterHand2.6M show gesture-aware pretraining consistently improves single-hand accuracy over state-of-the-art EANet baseline. The benefit transfers across architectures without any modification.

Conclusion: Gesture semantics serve as effective inductive bias for 3D hand pose estimation, with gesture-aware pretraining providing consistent accuracy improvements that transfer across different model architectures.
...
### [75] [Learning Coordinate-based Convolutional Kernels for Continuous SE(3) Equivariant and Efficient Point Cloud Analysis](https://arxiv.org/abs/2603.17538)
*Jaein Kim,Hee Bin Yoo,Dong-Sig Han,Byoung-Tak Zhang*

Main category: cs.CV

TL;DR: ECKConv is an SE(3)-equivariant convolution method for 3D point clouds that achieves both rigorous symmetry and scalability through coordinate-based kernel design in double coset spaces.


<details>
  <summary>Details</summary>
Motivation: Existing group convolution methods struggle to maintain both rigorous SE(3) symmetry and scalability simultaneously for 3D point cloud learning, creating a trade-off that needs resolution.

Method: ECKConv uses an intertwiner framework with explicit kernel design in double coset spaces, employing coordinate-based networks to achieve SE(3) equivariance while enhancing learning capability and memory efficiency.

Result: ECKConv demonstrates rigid equivariance, memory scalability, and outperforms state-of-the-art equivariant methods on diverse tasks including classification, pose registration, part segmentation, and large-scale semantic segmentation.

Conclusion: ECKConv successfully resolves the trade-off between rigorous symmetry and scalability in 3D point cloud learning, providing an effective solution for SE(3)-equivariant feature extraction.
...
### [76] [Motion-Adaptive Temporal Attention for Lightweight Video Generation with Stable Diffusion](https://arxiv.org/abs/2603.17398)
*Rui Hong,Shuxue Quan*

Main category: cs.CV

TL;DR: Motion-adaptive temporal attention mechanism for efficient video generation using frozen Stable Diffusion, with only 25.8M trainable parameters (2.9% of base UNet).


<details>
  <summary>Details</summary>
Motivation: Current video generation methods treat all content uniformly, but motion varies across sequences. Need parameter-efficient approach that adapts to motion content while leveraging frozen diffusion models.

Method: Dynamic temporal attention that adjusts receptive fields based on motion: local attention for high-motion sequences, global attention for low-motion. Lightweight modules injected via cascaded strategy with motion-aware gating and temporally correlated noise initialization.

Result: Achieves competitive results on WebVid validation with only 100K training videos. Standard denoising objective alone provides sufficient temporal regularization, outperforming explicit consistency losses. Shows trade-off between noise correlation and motion amplitude for inference-time control.

Conclusion: Motion-adaptive temporal attention enables parameter-efficient video generation with frozen Stable Diffusion, offering practical inference control and demonstrating that implicit temporal regularization through denoising is sufficient for temporal consistency.
...
### [77] [FrescoDiffusion: 4K Image-to-Video with Prior-Regularized Tiled Diffusion](https://arxiv.org/abs/2603.17555)
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion-based I2V models struggle with ultra-high-resolution (4K) inputs, losing fine-grained structure at native resolution or breaking global layout consistency with tiled denoising, especially problematic for complex fresco animations with many distinct elements that need spatial coherence over time.

Method: Augments tiled denoising with precomputed latent prior: first generate low-resolution video at model's native resolution, upsample its latent trajectory as global reference, then fuse per-tile noise predictions with this reference at each diffusion timestep using weighted least-squares objective that combines tile-merging with regularization term for global coherence.

Result: Improved global consistency and fidelity over tiled baselines on VBench-I2V dataset and proposed fresco I2V dataset, computationally efficient, enables explicit control over creativity-consistency trade-off with spatial regularization for region-level motion control.

Conclusion: FrescoDiffusion effectively addresses the high-resolution I2V generation challenge by leveraging latent priors to maintain both local detail and global coherence, offering a practical solution for complex fresco animation and other large-format video generation tasks.
...
### [78] [Edit-As-Act: Goal-Regressive Planning for Open-Vocabulary 3D Indoor Scene Editing](https://arxiv.org/abs/2603.17583)
*Seongrae Noh,SeungWon Seo,Gyeong-Moon Park,HyeongYeop Kang*

Main category: cs.CV

TL;DR: Edit-As-Act: A framework for 3D indoor scene editing from natural language that treats editing as goal-regressive planning rather than generation, using symbolic predicates and action planning to achieve minimal, physically consistent changes.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary scene editing systems either regenerate large portions of scenes or rely on image-space edits that disrupt spatial structure, causing unintended global changes or physically inconsistent layouts. These limitations arise from treating editing as a generative task rather than as achieving a desired world state with minimal changes.

Method: Edit-As-Act frames scene editing as goal-regressive planning in 3D space. It predicts symbolic goal predicates from natural language instructions and plans using EditLang, a PDDL-inspired action language with explicit preconditions and effects for geometric relations. A language-driven planner proposes actions, and a validator enforces goal-directedness, monotonicity, and physical feasibility.
...
Conclusion: By separating reasoning from low-level generation and treating editing as goal-regressive planning rather than generation, Edit-As-Act achieves interpretable, physically coherent transformations that existing paradigms cannot deliver, satisfying all three key criteria for effective scene editing.
...
### [79] [Mutually Causal Semantic Distillation Network for Zero-Shot Learning](https://arxiv.org/abs/2603.17412)
...
<details>
  <summary>Details</summary>
Motivation: Prior ZSL methods use unidirectional attention in a weakly-supervised manner, learning spurious and limited latent semantic representations that fail to discover intrinsic semantic knowledge between visual and attribute features.

Method: Proposes MSDN++ with two mutual causal attention sub-nets: attribute→visual causal attention (learns attribute-based visual features) and visual→attribute causal attention (learns visual-based attribute features). Uses causal attention to learn causal vision-attribute associations and semantic distillation loss for collaborative learning.
...
Conclusion: MSDN++ effectively distills intrinsic and sufficient semantic representations for ZSL through mutually causal semantic distillation, outperforming previous methods on standard benchmarks.
...
### [80] [Anchoring and Rescaling Attention for Semantically Coherent Inbetweening](https://arxiv.org/abs/2603.17651)
*Tae Eun Choi,Sumin Shim,Junhyeok Kim,Seong Jae Hwang*

Main category: cs.CV

TL;DR: Proposes Keyframe-anchored Attention Bias and Rescaled Temporal RoPE to improve generative inbetweening with better frame consistency, semantic fidelity, and pace stability, evaluated on new TGI-Bench benchmark.
...
Method: Introduces Keyframe-anchored Attention Bias to provide semantic and temporal guidance from keyframes and text onto each intermediate frame. Also uses Rescaled Temporal RoPE to better enforce frame consistency by allowing self-attention to attend to keyframes more faithfully.

Result: Achieves state-of-the-art frame consistency, semantic fidelity, and pace stability for both short and long sequences across diverse challenges without additional training, as evaluated on the new TGI-Bench benchmark.

Conclusion: The proposed method effectively addresses key challenges in generative inbetweening by providing better guidance from keyframes and text, resulting in more consistent, semantically aligned, and temporally stable intermediate frame generation.
...
### [81] [Towards Motion-aware Referring Image Segmentation](https://arxiv.org/abs/2603.17413)
*Chaeyun Kim,Seunghoon Yi,Yejin Kim,Yohan Jo,Joonseok Lee*

Main category: cs.CV

TL;DR: The paper addresses the performance gap in Referring Image Segmentation (RIS) for motion-related queries by introducing data augmentation for motion phrases and multimodal contrastive learning, with a new motion-centric benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing RIS methods significantly underperform on motion-related queries compared to appearance-based ones, creating a need to improve motion understanding in visual-language models.

Method: Two main contributions: 1) Efficient data augmentation extracting motion-centric phrases from captions, 2) Multimodal Radial Contrastive Learning (MRaCL) on fused image-text embeddings rather than unimodal representations.

Result: Extensive experiments show substantial improvements on motion-centric queries across multiple RIS models while maintaining competitive results on appearance-based descriptions.

Conclusion: The proposed approach effectively addresses the motion-understanding gap in RIS, with the introduction of M-Bench benchmark for comprehensive evaluation of motion-centric queries.
...
TL;DR: CC-CDFSL method uses cycle consistency and semantic anchors to improve local vision-language alignment in CLIP-based cross-domain few-shot learning, enhancing interpretability and achieving SOTA performance.
...
Method: Proposes CC-CDFSL with cycle consistency that translates local visual features into text features and back (and vice versa), plus a Semantic Anchor mechanism that augments visual features for text-to-image mapping and shrinks them to filter noise for image-to-text mapping.

Result: Extensive experiments show the method effectively improves local vision-language alignment, enhances interpretability through patch visualization, and achieves state-of-the-art performance across various benchmarks, backbones, and fine-tuning methods.

Conclusion: The proposed CC-CDFSL method successfully addresses the local misalignment problem in CLIP-based cross-domain few-shot learning by leveraging self-supervision through cycle consistency and semantic anchors, enabling better fine-grained pattern recognition and interpretability.
...
### [83] [SHIFT: Motion Alignment in Video Diffusion Models with Adversarial Hybrid Fine-Tuning](https://arxiv.org/abs/2603.17426)
*Xi Ye,Wenjia Yang,Yangyang Xu,Xiaoyang Liu,Duo Su,Mengfei Xia,Jun Zhu*

Main category: cs.CV

TL;DR: SHIFT framework improves motion fidelity in video diffusion models through pixel-motion rewards and hybrid fine-tuning that combines supervised and advantage-weighted approaches.


<details>
  <summary>Details</summary>
Motivation: Video diffusion models achieve visual realism but suffer from weakened motion fidelity (reduced dynamics, degraded temporal coherence) after fine-tuning, especially motion alignment problems post-training.

Method: Introduces pixel-motion rewards based on pixel flux dynamics for instantaneous and long-term motion consistency. Proposes Smooth Hybrid Fine-tuning (SHIFT) - a scalable reward-driven framework that fuses supervised fine-tuning and advantage weighted fine-tuning with novel adversarial advantages.

Result: SHIFT improves convergence speed, mitigates reward hacking, and efficiently resolves dynamic-degree collapse in modern video diffusion models during supervised fine-tuning.

Conclusion: The proposed SHIFT framework effectively addresses motion alignment problems in video diffusion models through pixel-motion rewards and hybrid fine-tuning approach.
...
### [84] [FINER: MLLMs Hallucinate under Fine-grained Negative Queries](https://arxiv.org/abs/2603.17662)
*Rui Xiao,Sanghwan Kim,Yongqin Xian,Zeynep Akata,Stephan Alaniz*

Main category: cs.CV

TL;DR: FINER introduces fine-grained negative queries to benchmark MLLM hallucinations, revealing they occur when mismatches co-occur with present elements, and proposes FINER-Tuning using DPO to reduce hallucinations while improving general capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on coarse image-related questions, failing to capture MLLM hallucinations on fine-grained queries, which is a critical limitation for real-world applications requiring precise understanding.

Method: Introduces FINER benchmarks (FINER-CompreCap and FINER-DOCCI) with fine-grained negative queries across four settings, and proposes FINER-Tuning using Direct Preference Optimization on FINER-inspired data to reduce hallucinations.

Result: FINER reveals MLLMs hallucinate when fine-grained mismatches co-occur with genuinely present elements. FINER-Tuning yields up to 24.2% reduction in hallucinations on FINER benchmarks while improving performance on 8 existing hallucination suites and enhancing general multimodal capabilities across 6 benchmarks.

Conclusion: Fine-grained negative queries are crucial for evaluating MLLM hallucinations, and FINER-Tuning effectively reduces hallucinations while maintaining or improving general multimodal performance, providing a comprehensive solution for MLLM reliability.
...
### [85] [ECHO: Towards Emotionally Appropriate and Contextually Aware Interactive Head Generation](https://arxiv.org/abs/2603.17427)
*Xiangyu Kong,Xiaoyu Jin,Yihan Pan,Haoqin Sun,Hengde Zhu,Xiaoming Xu,Xiaoming Wei,Lu Liu,Siyang Song*

Main category: cs.CV

TL;DR: ECHO is a new Interactive Head Generation framework that improves avatar facial behaviors by modeling long-range context and decoupling dual-track signals to enhance contextual appropriateness, emotional rationality, and lip synchronization.


<details>
  <summary>Details</summary>
Motivation: Existing IHG methods have two main limitations: (1) they rely on short-clip behavioral cues without long-range contextual modeling, leading to facial behaviors lacking contextual appropriateness; (2) they use entangled, role-agnostic fusion of dual-track signals (user behaviors and avatar audio) which causes cross-signal interference and compromises lip synchronization.

Method: ECHO framework includes: (1) Long-range Contextual Understanding (LCU) component for contextual understanding of behavior-grounded dynamics and linguistic-driven affective semantics; (2) block-wise Spatial-aware Decoupled Cross-attention Modulation (SDCM) module that preserves self-audio-driven lip articulation while adaptively integrating user behavioral cues for non-lip regions; (3) two-stage training paradigm.

Result: Extensive experiments demonstrate the effectiveness of proposed components and ECHO's superior IHG performance compared to existing methods.

Conclusion: ECHO addresses key challenges in IHG by enabling long-range contextual understanding and decoupled signal processing, resulting in more lifelike avatar head videos with better contextual appropriateness, emotional rationality, and lip synchronization.
...
### [86] [WeatherReasonSeg: A Benchmark for Weather-Aware Reasoning Segmentation in Visual Language Models](https://arxiv.org/abs/2603.17680)
*Wanjun Du,Zifeng Yuan,Tingting Chen,Fucai Ke,Beibei Lin,Shunli Zhang*

Main category: cs.CV

TL;DR: WeatherReasonSeg is a new benchmark for evaluating vision-language models' reasoning-based segmentation capabilities under adverse weather conditions like rain, snow, and fog.


<details>
  <summary>Details</summary>
Motivation: Current VLM benchmarks use high-quality images under ideal conditions, but real-world applications often face degraded visual cues from adverse weather. The paper questions whether VLMs can maintain reliable reasoning segmentation when weather severely degrades visual information.

Method: 1) Constructed a controllable reasoning dataset by applying synthetic weather with varying severity levels to existing segmentation datasets. 2) Curated a real-world adverse-weather reasoning segmentation dataset with semantically consistent queries generated via mask-guided LLM prompting. 3) Broadened evaluation scope across five reasoning dimensions: functionality, application scenarios, structural attributes, interactions, and requirement matching.

Result: Extensive experiments across diverse VLMs revealed: 1) VLM performance degrades monotonically with increasing weather severity, and 2) different weather types induce distinct vulnerability patterns in the models.

Conclusion: WeatherReasonSeg serves as a foundation for advancing robust, weather-aware reasoning in vision-language models, addressing a critical gap in evaluating real-world performance under adverse conditions.
...
### [87] [Eye image segmentation using visual and concept prompts with Segment Anything Model 3 (SAM3)](https://arxiv.org/abs/2603.17715)
*Diederick C. Niehorster,Marcus Nyström*

Main category: cs.CV

TL;DR: SAM3 doesn't outperform SAM2 for eye image segmentation; SAM2 remains better and faster


<details>
  <summary>Details</summary>
Motivation: To evaluate whether the latest Segment Anything Model (SAM3) offers better eye image segmentation performance than SAM2, and explore its new text prompting capabilities

Method: Evaluated SAM3 vs SAM2 using diverse datasets including high-resolution lab videos and challenging in-the-wild TEyeD dataset, testing both visual and concept (text) prompting modes

Result: SAM3 with either visual or concept prompts did not perform better than SAM2 in most cases for both lab and in-the-wild datasets; SAM2 performed better and was faster

Conclusion: SAM2 remains the best option for eye image segmentation; researchers provide adapted SAM3 codebase for processing arbitrary duration videos
...
### [88] [FACE-net: Factual Calibration and Emotion Augmentation for Retrieval-enhanced Emotional Video Captioning](https://arxiv.org/abs/2603.17455)
*Weidong Chen,Cheng Ye,Zhendong Mao,Peipei Song,Xinyan Liu,Lei Zhang,Xiaojun Chang,Yongdong Zhang*

Main category: cs.CV

TL;DR: FACE-net is a retrieval-enhanced framework for Emotional Video Captioning that addresses factual-emotional bias through factual calibration and emotion augmentation in a unified architecture.


<details>
  <summary>Details</summary>
Motivation: Existing EVC methods struggle with factual-emotional bias - insufficient mining and coordination of factual and emotional cues leads to compromised descriptions that don't adapt to different samples' varying factual and emotional requirements.

Method: Proposes FACE-net with three key modules: 1) External repository retrieval for semantic augmentation, 2) Factual calibration via uncertainty estimation using subject-predicate-object triplets with self/cross-refinement, 3) Progressive visual emotion augmentation using calibrated semantics as experts with emotion dictionary interaction, and 4) Dynamic bias adjustment routing to predict and adjust sample bias.
...
Conclusion: FACE-net effectively addresses the factual-emotional bias problem in Emotional Video Captioning through a unified architecture that enhances both factual accuracy and emotional relevance in video descriptions.
...
### [89] [SARE: Sample-wise Adaptive Reasoning for Training-free Fine-grained Visual Recognition](https://arxiv.org/abs/2603.17729)
*Jingxiao Yang,DaLin He,Miao Pan,Ge Su,Wenqi Zhang,Yifeng Hu,Tangwei Li,Yuke Li,Xuhong Zhang*

Main category: cs.CV

TL;DR: SARE is a training-free fine-grained visual recognition framework that uses sample-adaptive reasoning with cascaded retrieval and self-reflective experience to improve accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current LVLM-based FGVR methods have two key limitations: (1) they use the same inference pipeline for all samples regardless of difficulty, leading to suboptimal accuracy and efficiency, and (2) they lack mechanisms to consolidate and reuse error-specific experience, causing repeated failures on similar challenging cases.

Method: SARE uses a cascaded design combining fast candidate retrieval with fine-grained reasoning, invoking reasoning only when necessary. It incorporates a self-reflective experience mechanism that leverages past failures to provide transferable discriminative guidance during inference without parameter updates.
...
Conclusion: SARE effectively addresses the limitations of existing training-free FGVR methods by introducing sample-adaptive reasoning and self-reflective experience, achieving superior performance and efficiency.
...
### [90] [AR-CoPO: Align Autoregressive Video Generation with Contrastive Policy Optimization](https://arxiv.org/abs/2603.17461)
*Dailan He,Guanlin Feng,Xingtong Ge,Yi Zhang,Bingqi Ma,Guanglu Song,Yu Liu,Hongsheng Li*

Main category: cs.CV

TL;DR: AR-CoPO adapts contrastive RLHF to streaming autoregressive video generation by using chunk-level forking and semi-on-policy training, improving alignment without reward hacking.


<details>
  <summary>Details</summary>
Motivation: Streaming AR video generators with few-step distillation achieve good quality but are hard to align via RLHF. Existing SDE-based methods struggle because few-step ODEs deviate from standard flow-matching, and short trajectories are too sensitive to initialization noise for effective exploration.

Method: AR-CoPO adapts Neighbor GRPO's contrastive approach to streaming AR generation. It uses chunk-level alignment via a forking mechanism that creates neighborhood candidates at random chunks, assigns sequence-level rewards, and performs localized GRPO updates. Also introduces semi-on-policy training combining on-policy exploration with replay buffer exploitation.
...
Conclusion: AR-CoPO successfully adapts contrastive RLHF to streaming AR video generation, overcoming limitations of existing SDE-based methods and achieving better alignment without reward hacking through chunk-level forking and semi-on-policy training.
...
TL;DR: QIG introduces fine-grained token-level quantization for LVLMs using integrated gradients to measure token sensitivity, improving accuracy with minimal latency overhead.


<details>
  <summary>Details</summary>
Motivation: LVLMs have high computational/memory costs hindering deployment. Existing quantization methods use coarse modality-level sensitivity measurement, failing to capture complex cross-token interactions and token-level quantization errors.

Method: Proposes Quantization-aware Integrated Gradients (QIG) that uses integrated gradients to quantitatively evaluate token sensitivity, pushing granularity from modality level to token level to reflect inter-modality and intra-modality dynamics.
...
Conclusion: Fine-grained token-level quantization using integrated gradients effectively addresses limitations of modality-level quantization, enabling more efficient LVLM deployment with minimal accuracy loss.
...
TL;DR: ChopGrad is a truncated backpropagation method for video diffusion models that reduces memory costs from linear to constant by limiting gradient computation to local frame windows while maintaining global consistency.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models use recurrent frame processing which requires storing activations across entire video sequences during training, leading to prohibitive memory costs that make fine-tuning with pixel-wise losses computationally intractable for long or high-resolution videos.

Method: ChopGrad introduces a truncated backpropagation scheme for video decoding that limits gradient computation to local frame windows while maintaining global consistency, with theoretical analysis of this approximation.

Result: ChopGrad reduces training memory from scaling linearly with the number of video frames to constant memory, and performs favorably compared to state-of-the-art video diffusion models across various conditional video generation tasks with pixel-wise losses.

Conclusion: The proposed ChopGrad method enables efficient fine-tuning of video diffusion models with frame-wise losses while dramatically reducing memory requirements, making it practical for long or high-resolution video generation tasks.
...
### [93] [UAV-CB: A Complex-Background RGB-T Dataset and Local Frequency Bridge Network for UAV Detection](https://arxiv.org/abs/2603.17492)
*Shenghui Huang,Menghao Hu,Longkun Zou,Hongyu Chi,Zekai Li,Feng Gao,Fan Yang,Qingyao Wu,Ke Chen*

Main category: cs.CV

TL;DR: LFBNet: A frequency-aware RGB-T fusion network for robust UAV detection in complex backgrounds and camouflage conditions, evaluated on new UAV-CB dataset.
...
Method: Proposes Local Frequency Bridge Network (LFBNet) that models features in localized frequency space to bridge both frequency-spatial fusion gap and cross-modality discrepancy gap in RGB-T fusion. Also introduces UAV-CB dataset specifically designed for complex backgrounds and camouflage.

Result: LFBNet achieves state-of-the-art detection performance and strong robustness under camouflaged and cluttered conditions on both the new UAV-CB dataset and public benchmarks.

Conclusion: The work provides a frequency-aware perspective on multimodal UAV perception, addressing critical real-world challenges in UAV detection through specialized dataset construction and novel frequency-space modeling approach.
...
### [94] [Differential Attention-Augmented BiomedCLIP with Asymmetric Focal Optimization for Imbalanced Multi-Label Video Capsule Endoscopy Classification](https://arxiv.org/abs/2603.17879)
*Podakanti Satyajith Chary,Nagarajan Ganapathy*

Main category: cs.CV

TL;DR: Multi-label classification framework for VCE using modified BiomedCLIP with differential attention and imbalance handling techniques, achieving 0.2456 mAP@0.5 on RARE-VISION test set.


<details>
  <summary>Details</summary>
Motivation: Address extreme class imbalance in Galar VCE dataset where pathological findings constitute less than 0.1% of annotated frames, requiring specialized techniques for effective multi-label classification.

Method: Modifies BiomedCLIP with differential attention mechanism, uses sqrt-frequency weighted sampler, asymmetric focal loss, mixup regularization, per-class threshold optimization, and temporal smoothing with median-filter and gap merging.

Result: Achieves overall temporal mAP@0.5 of 0.2456 and mAP@0.95 of 0.2353 on RARE-VISION test set (161,025 frames), with total inference time of ~8.6 minutes on single GPU.

Conclusion: The proposed framework effectively handles extreme class imbalance in VCE through architectural modifications and optimization strategies, demonstrating practical efficiency for real-world clinical applications.
...
### [95] [Omni-I2C: A Holistic Benchmark for High-Fidelity Image-to-Code Generation](https://arxiv.org/abs/2603.17508)
*Jiawei Zhou,Chi Zhang,Xiang Feng,Qiming Zhang,Haibo Qiu,Lihuo He,Dengpan Ye,Xinbo Gao,Jing Zhang*

Main category: cs.CV

TL;DR: Omni-I2C is a benchmark for evaluating Large Multimodal Models' ability to convert complex digital graphics into executable code, revealing significant performance gaps in current models.


<details>
  <summary>Details</summary>
Motivation: Current LMMs struggle with converting complex structured digital graphics to executable code, requiring unprecedented synergy between high-fidelity visual perception and precise generative expression. This task is non-trivial as minor errors lead to complete failure in visual reconstruction.

Method: Created Omni-I2C benchmark with 1080 curated samples spanning diverse subjects, image modalities, and programming languages. Includes authentic user-sourced cases from scientific visualizations to complex symbolic notations, each with executable reference code. Evaluation framework decouples performance into perceptual fidelity and symbolic precision.

Result: Evaluation reveals substantial performance gap among leading LMMs; even state-of-the-art models struggle to preserve structural integrity in complex scenarios, showing multimodal code generation remains a formidable challenge.

Conclusion: Omni-I2C provides a comprehensive benchmark that exposes granular structural failures and reasoning bottlenecks in current LMMs, demonstrating that converting complex digital graphics to executable code is a significant challenge requiring improved visual perception and code generation capabilities.
...
### [96] [VideoAtlas: Navigating Long-Form Video in Logarithmic Compute](https://arxiv.org/abs/2603.17948)
*Mohamed Eltahir,Ali Habibullah,Yazan Alshoibi,Lama Ayash,Tanveer Hussain,Naeemullah Khan*

Main category: cs.CV

TL;DR: VideoAtlas is a hierarchical grid representation for videos that enables lossless, scalable visual understanding without text conversion, combined with Video-RLM architecture for efficient long-context video processing.


<details>
  <summary>Details</summary>
Motivation: Existing video language models face two key challenges: lossy representations that approximate video content, and long-context limitations where caption-based pipelines collapse visual information into text, losing visual fidelity.

Method: VideoAtlas represents video as a hierarchical grid that is lossless, navigable, and scalable. Video-RLM uses a Master-Worker architecture where Master coordinates global exploration while Workers concurrently drill into regions to accumulate visual evidence.

Result: Three key findings: (1) logarithmic compute growth with video duration plus 30-60% multimodal cache hit rate, (2) environment budgeting provides compute-accuracy tradeoff, (3) emergent adaptive compute allocation scales with question granularity.

Conclusion: VideoAtlas with Video-RLM demonstrates structured environment navigation as a viable and scalable paradigm for video understanding, maintaining robustness from 1-hour to 10-hour benchmarks with minimal accuracy degradation.
...
### [97] [EI: Early Intervention for Multimodal Imaging based Disease Recognition](https://arxiv.org/abs/2603.17514)
*Qijie Wei,Hailan Lin,Xirong Li*

Main category: cs.CV

TL;DR: EI framework with MoR adaptation for multimodal medical imaging: early intervention tokens from reference modalities steer target modality embedding, plus parameter-efficient VFM fine-tuning with varied-rank adapters.


<details>
  <summary>Details</summary>
Motivation: Current multimodal medical imaging methods have two key limitations: 1) "fusion after embedding" paradigm fails to fully leverage complementary multimodal information, and 2) scarcity of labeled multimodal medical data and domain shift from natural images hinder use of Vision Foundation Models.

Method: Proposes Early Intervention (EI) framework where reference modality's high-level semantic tokens intervene early in target modality's embedding process, plus Mixture of Low-varied-Ranks Adaptation (MoR) - a parameter-efficient fine-tuning method using varied-rank adapters with weight-relaxed router for VFM adaptation.

Result: Extensive experiments on three public datasets (retinal disease, skin lesion, knee anomaly classification) demonstrate effectiveness against competitive baselines.

Conclusion: The proposed EI framework with MoR adaptation successfully addresses multimodal medical imaging challenges by enabling early cross-modal interaction and efficient VFM adaptation for improved disease recognition.
...
### [98] [UniSem: Generalizable Semantic 3D Reconstruction from Sparse Unposed Images](https://arxiv.org/abs/2603.17519)
...
<details>
  <summary>Details</summary>
Motivation: Existing feed-forward 3D Gaussian Splatting methods produce over-complete Gaussian sets with unstable geometry and rely on weak 2D segmenter features, leading to incomplete 3D semantics in novel scenes.

Method: Two key components: 1) Error-aware Gaussian Dropout (EGD) that uses rendering error cues to suppress redundancy-prone Gaussians, and 2) Mix-training Curriculum (MTC) that progressively blends 2D segmenter features with emergent 3D semantic priors using object-level prototype alignment.

Result: UniSem achieves superior performance on ScanNet and Replica datasets, reducing depth Rel by 15.2% and improving open-vocabulary segmentation mAcc by 3.7% with 16-view inputs compared to strong baselines.

Conclusion: UniSem provides a unified framework that jointly improves depth accuracy and semantic generalization for 3D reconstruction from sparse, unposed images through error-guided capacity control and progressive semantic blending.
...
### [99] [Loc3R-VLM: Language-based Localization and 3D Reasoning with Vision-Language Models](https://arxiv.org/abs/2603.18002)
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with spatial understanding and viewpoint-aware reasoning, as they lack explicit 3D reasoning capabilities despite having geometric cues in inputs.

Method: Framework with two joint objectives: global layout reconstruction for holistic scene structure, and explicit situation modeling to anchor egocentric perspective. Uses lightweight camera pose priors from pre-trained 3D foundation model for geometric consistency.
...
Conclusion: The spatial supervision framework enables strong 3D understanding in 2D Vision-Language Models, demonstrating that explicit spatial reasoning objectives can effectively ground perception and language in 3D context.
...
### [100] [PCA-Seg: Revisiting Cost Aggregation for Open-Vocabulary Semantic and Part Segmentation](https://arxiv.org/abs/2603.17520)
*Jianjian Yin,Tao Chen,Yi Chen,Gensheng Pei,Xiangbo Shu,Yazhou Yao,Fumin Shen*

Main category: cs.CV

TL;DR: PCA-Seg proposes a parallel cost aggregation paradigm with expert-driven perceptual learning and feature orthogonalization decoupling to improve vision-language alignment for open-vocabulary segmentation, achieving SOTA results with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs for open-vocabulary segmentation use serial aggregation that causes knowledge interference between class semantics and spatial context, limiting vision-language alignment quality.

Method: Parallel cost aggregation (PCA-Seg) with expert-driven perceptual learning module (multi-expert parser + coefficient mapper) and feature orthogonalization decoupling strategy to separate semantic and contextual streams.

Result: Achieves state-of-the-art performance on 8 benchmarks with only 0.35M additional parameters per parallel block, demonstrating superior vision-language alignment.

Conclusion: The parallel aggregation paradigm effectively addresses knowledge interference in VLMs, enabling richer vision-language alignment for open-vocabulary segmentation with minimal computational overhead.
...
### [101] [Unified Spatio-Temporal Token Scoring for Efficient Video VLMs](https://arxiv.org/abs/2603.18004)
*Jianrui Zhang,Yue Yang,Rohun Tripathi,Winson Han,Ranjay Krishna,Christopher Clark,Yong Jae Lee,Sangho Lee*

Main category: cs.CV

TL;DR: STTS is a simple, lightweight module that prunes 50% of vision tokens across both ViT and LLM in vision-language models, achieving 62% efficiency gains with minimal performance drop.


<details>
  <summary>Details</summary>
Motivation: Existing token pruning methods either prune only within ViT for unimodal tasks or only within LLM with complex text-conditioning, lacking unified architecture-wide pruning for vision-language models.

Method: STTS learns to score tokens temporally via auxiliary loss and spatially via LLM downstream gradients, using an efficient packing algorithm to prune tokens without text conditioning or merging.

Result: Prunes 50% of vision tokens, achieves 62% efficiency improvement during training/inference with only 0.7% performance drop across 13 video QA tasks. Efficiency increases with more frames.

Conclusion: STTS represents a novel, simple yet effective technique for unified, architecture-wide vision token pruning in vision-language models.
...
TL;DR: MM-OVSeg is a multimodal Optical-SAR fusion framework for open-vocabulary segmentation that works under adverse weather conditions like clouds and haze.


<details>
  <summary>Details</summary>
Motivation: Current open-vocabulary segmentation methods in remote sensing are limited to clear-sky optical data and struggle with cloudy/haze conditions, creating a need for weather-resilient solutions.

Method: Uses multimodal Optical-SAR fusion with cross-modal unification for representation alignment and dual-encoder fusion module integrating hierarchical features from multiple vision foundation models for text-aligned segmentation.

Result: Achieves superior robustness and generalization across diverse cloud conditions, demonstrating resilient performance under adverse weather.

Conclusion: MM-OVSeg effectively addresses weather limitations in remote sensing segmentation by leveraging complementary optical and SAR modalities through innovative cross-modal alignment and fusion techniques.
...
TL;DR: Video-SFT improves video performance in MLLMs but often hurts static image performance, creating an image-video trade-off tied to temporal budget (frame count).


<details>
  <summary>Details</summary>
Motivation: To understand how Video-SFT affects the balance between spatial (image) and temporal (video) understanding in multimodal LLMs, as current understanding is limited.

Method: Systematic study across architectures, parameter scales, and frame sampling settings; analysis of temporal budget effects; proposed instruction-aware Hybrid-Frame strategy for adaptive frame allocation.

Result: Video-SFT consistently improves video performance but yields limited gains or degradation on static images; trade-off correlates with temporal budget (more frames helps video but not images); Hybrid-Frame strategy partially mitigates the trade-off.

Conclusion: Video-SFT is not a free lunch - it creates an image-video trade-off, and preserving spatial understanding remains a key challenge in joint image-video training for MLLMs.
...
TL;DR: ProGVC is a progressive generative video compression framework that uses multi-scale residual token maps and transformer-based autoregressive context modeling for efficient low-bitrate perceptual video coding with progressive transmission capabilities.
...
Method: Encodes videos into hierarchical multi-scale residual token maps for progressive transmission, uses transformer-based multi-scale autoregressive context model for probability estimation, and predicts truncated fine-scale tokens at decoder to restore perceptual details.
...
Conclusion: ProGVC represents a new coding paradigm that unifies progressive transmission, efficient entropy coding, and detail synthesis within a single codec, addressing limitations of existing perceptual video compression approaches.
...
TL;DR: PF-RPN is a prompt-free universal region proposal network that identifies potential objects without external prompts, using sparse image-aware adapters, cascade self-prompting, and centerness-guided query selection.


<details>
  <summary>Details</summary>
Motivation: Existing object localization methods rely on exemplar images, predefined categories, or textual descriptions, which limits flexibility and adaptability in real-world scenarios where such prompts may not be available.

Method: Three key modules: 1) Sparse Image-Aware Adapter (SIA) for initial localization using learnable query embeddings, 2) Cascade Self-Prompt (CSP) for identifying remaining objects through self-prompted embeddings, and 3) Centerness-Guided Query Selection (CG-QS) for selecting high-quality queries using centerness scoring.

Result: The method can be optimized with limited data (5% of MS COCO) and applied directly to various domains (underwater, industrial defect, remote sensing) without fine-tuning, with experimental validation across 19 datasets.

Conclusion: PF-RPN provides a flexible, prompt-free approach to universal object proposal that works across diverse application domains without requiring external prompts or extensive fine-tuning.
...
TL;DR: A face anonymization framework that preserves facial expressions, lighting, and skin tone while concealing identity, with new evaluation metrics for expression fidelity and photometric consistency.


<details>
  <summary>Details</summary>
Motivation: Existing face anonymization methods focus on identity removal and image realism but neglect important facial features like expressions, illumination, and skin tone that are critical for downstream applications such as relighting, color constancy, and affective analysis.

Method: Extends DeepPrivacy by incorporating dense facial landmarks to preserve expressions and adding lightweight post-processing modules for lighting direction and skin color consistency. Also establishes new evaluation metrics for expression fidelity, lighting consistency, and color preservation.

Result: On CelebA-HQ dataset, the method produces anonymized faces with improved realism and significantly higher fidelity in expression, illumination, and skin tone compared to state-of-the-art baselines.

Conclusion: Feature-aware anonymization is important for creating more useful, fair, and trustworthy privacy-preserving facial data that maintains critical facial attributes beyond just identity removal.
...
TL;DR: PanoVGGT is a permutation-equivariant Transformer that jointly estimates camera poses, depth maps, and 3D point clouds from panoramas, addressing non-pinhole distortions with spherical-aware embeddings and SO(3) augmentation.


<details>
  <summary>Details</summary>
Motivation: Panoramic imagery introduces non-pinhole distortions that challenge existing perspective-camera models for joint pose estimation and 3D reconstruction, requiring specialized approaches for spherical domain reasoning.

Method: PanoVGGT uses permutation-equivariant Transformer with spherical-aware positional embeddings, panorama-specific three-axis SO(3) rotation augmentation, and stochastic anchoring to resolve global-frame ambiguity.

Result: The model achieves competitive accuracy, strong robustness, and improved cross-domain generalization on PanoCity dataset and standard benchmarks, outperforming existing perspective-camera models.

Conclusion: PanoVGGT effectively addresses panoramic 3D reconstruction challenges through spherical domain reasoning, with the release of PanoCity dataset and code enabling further research in panoramic vision.
...
TL;DR: LoGSAM: A parameter-efficient framework that transforms radiologist dictation into text prompts for foundation-model-based brain tumor localization and segmentation, achieving SOTA results with minimal parameter updates.


<details>
  <summary>Details</summary>
Motivation: Existing brain tumor localization approaches rely on task-specific supervised models with limited annotated data. Need for efficient, adaptable methods that can leverage radiologist expertise without extensive labeling.

Method: 1) Transcribe radiologist speech using Whisper ASR, 2) Apply negation-aware clinical NLP to extract tumor-specific text prompts, 3) Use LoRA-adapted Grounding DINO for text-conditioned tumor localization (5% parameter updates), 4) Feed bounding boxes to frozen MedSAM for pixel-level segmentation.

Result: Achieved state-of-the-art dice score of 80.32% on BRISC 2025. On 12 unseen German MRI scans with radiologist dictations, achieved 91.7% case-level accuracy. Demonstrates effective modular pipeline with minimal parameter updates.

Conclusion: Feasible to construct modular speech-to-segmentation pipeline by intelligently leveraging pretrained foundation models with minimal parameter updates, enabling efficient domain adaptation while preserving cross-modal knowledge.
...
TL;DR: DUCS is a coreset selection method for medical imaging that selects unreliable samples near decision boundaries by analyzing confidence fluctuations and forgetting patterns during training, outperforming SOTA methods at high compression rates.


<details>
  <summary>Details</summary>
Motivation: Large-scale medical imaging datasets are computationally expensive to process, and existing coreset selection methods are limited for medical data due to high intra-class variation and inter-class similarity. The authors observed that neural networks better remember samples near class centers, but unreliable samples near decision boundaries are actually more informative for model learning.

Method: Proposes Dynamic Unreliability-Driven Coreset Selection (DUCS) with two components: 1) Inward Self-Awareness - analyzes confidence evolution during training to quantify sample uncertainty; 2) Backward Memory Tracking - tracks frequency of forgetting samples to evaluate retention ability. Selects samples with substantial confidence fluctuations and repeated forgetting during training.
...
Conclusion: Unreliable samples near decision boundaries are more informative for model learning than stable samples near class centers. The proposed DUCS strategy effectively identifies these boundary samples through confidence fluctuation and forgetting pattern analysis, enabling efficient coreset selection for medical imaging with limited resources.
...
TL;DR: A framework for unified 3D perception and reasoning that builds hierarchical language-distilled Gaussian scenes and 3D semantic scene graphs without scene-specific training, enabling open-vocabulary 3D reasoning across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D perception methods are either object-centric or require costly training for inter-object reasoning, lacking unified approaches for segmentation, retrieval, and relation understanding.

Method: Constructs hierarchical language-distilled Gaussian scenes with Gaussian pruning for geometry refinement, multi-view language alignment for aggregating 2D features into 3D object embeddings, and builds open-vocabulary 3D scene graphs with Vision Language annotations and Graph Neural Network-based relational reasoning.
...
Conclusion: The framework achieves unified 3D perception and reasoning without scene-specific training, providing a scalable solution for complex 3D understanding tasks through hierarchical semantic modeling and relational reasoning.
...
TL;DR: S-VGGT reduces computational cost in 3D foundation models by addressing structural redundancy at the frame level rather than token level, enabling parallel processing and compounding with existing acceleration methods.


<details>
  <summary>Details</summary>
Motivation: Feed-forward 3D foundation models suffer from quadratic computational costs due to global attention, limiting scalability. Existing token-level acceleration methods fail to address structural redundancy in dense capture data and introduce overhead from nearest-neighbor searches.

Method: Builds a dense scene graph from initial features to characterize structural redundancy and guide scene partitioning. Softly assigns frames to balanced subscenes with smooth geometric transitions. Designs subscenes to share a common reference frame, creating a parallel geometric bridge for independent processing without explicit alignment.

Result: Provides strong intrinsic acceleration by cutting global attention costs at the source. The approach is orthogonal to token-level acceleration methods, allowing for compounded speedups without compromising reconstruction fidelity.

Conclusion: S-VGGT addresses the fundamental issue of structural redundancy in 3D foundation models at the frame level, enabling efficient parallel processing and compatibility with existing acceleration techniques for scalable 3D reconstruction.
...
TL;DR: Multi-agent LLM system fuses heterogeneous data sources to create building age ground truth, then uses BuildingAgeCNN satellite classifier with 90.69% accuracy for urban heat planning applications.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for determining urban building age distributions rely on sensor/remote sensing data with inconsistencies and gaps, creating challenges for sustainable municipal heat planning and upgrade prioritization.

Method: Three-agent LLM system (Zensus, OSM, Monument agents) fuses heterogeneous data sources, with data orchestrator for geocoding and deduplication. BuildingAgeCNN classifier uses ConvNeXt backbone with FPN, CoordConv, and SE blocks for satellite-only age classification.

Result: BuildingAgeCNN achieves 90.69% overall accuracy but modest 67.25% macro-F1 due to class imbalance and confusion between adjacent historical cohorts. System includes calibrated confidence estimates and manual review flags for low-confidence cases.

Conclusion: The multi-agent LLM system enables accurate building age classification for sustainable energy planning, with confidence calibration mitigating risks for practical applications in district-heating optimization and low-carbon energy systems.
...
TL;DR: Novel two-stage cross-modal framework for open-vocabulary 3D affordance grounding that enhances semantic and geometric representations using LLM-generated instructions and geometric consistency modeling.
...
Method: Two-stage framework: 1) LLMs generate part-aware instructions to recover missing semantics and link similar affordances; 2) Affordance Prototype Aggregation (APA) captures cross-object geometric consistency, and Intra-Object Relational Modeling (IORM) refines geometric differentiation within objects for precise semantic alignment.

Result: Demonstrates superior performance on a newly introduced benchmark and two existing benchmarks compared to existing methods, validating the effectiveness of the approach.

Conclusion: The proposed framework successfully addresses key challenges in open-vocabulary 3D affordance grounding by enhancing both semantic and geometric representations through cross-modal integration and geometric consistency modeling.
...
TL;DR: Proposes instance-aware discretization for diffusion/flow matching models that adapts timestep schedules per sample instead of using uniform global schedules, improving generation quality with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Existing discretization strategies use globally shared timestep schedules across all samples, which fails to account for instance-specific complexity in the generative process, potentially limiting performance. Controlled experiments on synthetic data reveal suboptimality of global schedules under instance-specific dynamics.

Method: An instance-aware discretization framework that learns to adapt timestep allocations based on input-dependent priors, extending gradient-based discretization search to the conditional generative setting.

Result: Empirical results across diverse settings (synthetic data, pixel-space diffusion, latent-space images, and video flow matching models) demonstrate consistent improvement in generation quality with marginal tuning cost compared to training and negligible inference overhead.

Conclusion: Instance-aware discretization addresses the limitations of uniform timestep schedules by adapting to sample-specific complexity, offering a practical improvement to diffusion/flow matching models with minimal computational cost.
...
TL;DR: DeepCORO-CLIP is a multi-view foundation model for coronary angiography that uses video-text contrastive learning to provide comprehensive coronary assessment across diagnostic, prognostic, and disease progression tasks with high accuracy and fast inference.


<details>
  <summary>Details</summary>
Motivation: Coronary angiography is the gold standard for evaluating coronary artery disease, but visual interpretation suffers from variability between readers. Existing AI methods are limited to single frames/projections and focus mainly on stenosis detection, lacking comprehensive assessment capabilities.

Method: Multi-view foundation model trained with video-text contrastive learning on 203,808 angiography videos from 28,117 patients across 32,473 studies. Uses attention-based pooling for study-level assessment and integrates multiple projections. Trained at Montreal Heart Institute and externally validated at UCSF.

Result: Achieved AUROC of 0.888 internally and 0.89 externally for significant stenosis detection. Mean absolute error against core lab QCA was 13.6% vs 19.0% for clinical reports. Strong performance for chronic total occlusion, thrombus, and calcification detection. Transfer learning enabled MACE prediction (AUROC 0.79) and LVEF estimation (MAE 7.3%). Mean inference time of 4.2 seconds.

Conclusion: DeepCORO-CLIP provides a foundation for automated coronary angiography interpretation at point of care with comprehensive assessment capabilities, high accuracy, and fast deployment. Public release of code, data, model weights, and infrastructure enables broader adoption.
...
TL;DR: Flash-non-flash paired imaging improves contactless fingerprint spoof detection by revealing material properties through lighting differences.


<details>
  <summary>Details</summary>
Motivation: Contactless fingerprint recognition lacks physical contact cues for spoof detection, and existing single-image methods generalize poorly across devices, conditions, and spoof materials.

Method: Uses paired flash-non-flash acquisition as active sensing, analyzes lighting-induced differences via interpretable metrics (inter-channel correlation, specular reflection, texture realism, differential imaging).

Result: Flash illumination accentuates material/structure properties (ridge visibility, subsurface scattering, micro-geometry, surface oils), helping discriminate genuine fingerprints from printed, digital, and molded spoofs.

Conclusion: Illumination-aware analysis improves robustness and interpretability in contactless fingerprint PAD, motivating future work on paired acquisition and physics-informed feature design.
...
TL;DR: AFSS speeds up YOLO training by dynamically sampling training images based on learning sufficiency, skipping easy images and focusing on hard ones.


<details>
  <summary>Details</summary>
Motivation: YOLO detectors have fast inference but slow training because they process every training image in every epoch, even when many images are already sufficiently learned, contradicting the "You Only Look Once" efficiency philosophy.

Method: Anti-Forgetting Sampling Strategy (AFSS) measures learning sufficiency as min(recall, precision), categorizes images into easy/medium/hard levels. Easy images are sparsely resampled with priority to unused ones, medium images are partially selected, hard images are fully sampled every epoch. Learning sufficiency is periodically updated.

Result: Achieves >1.43× training speedup for YOLO-series detectors while improving accuracy on MS COCO 2017, PASCAL VOC 2007, DOTA-v1.0, and DIOR-R datasets.

Conclusion: AFSS enables more efficient YOLO training by adaptively focusing on informative images and discarding redundant ones, aligning training efficiency with YOLO's inference efficiency.
...
TL;DR: SynRL: Post-training framework using synthetic videos to teach temporal primitives (direction, speed, state tracking) that transfer to real-world video understanding, outperforming larger real-world datasets.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with temporal reasoning due to datasets lacking true temporal-centricity and systematic errors in temporal perception from proprietary model-generated data.

Method: Decompose temporal understanding into short-term perceptual primitives (speed, direction) and long-term cognitive primitives, generate 7.7K CoT and 7K RL samples with frame-level annotations using code-based synthetic video generation with simple geometric shapes.

Result: Achieves substantial improvements across 15 benchmarks for temporal grounding, complex reasoning, and general video understanding; 7.7K synthetic samples outperform Video-R1 with 165K real-world samples.

Conclusion: Synthetic data teaching fundamental temporal skills provides a more cost-efficient scaling path for video post-training, establishing a new paradigm for video temporal learning.
...
TL;DR: MoBaNet: A parameter-efficient, modality-balanced symmetric fusion framework for multimodal remote sensing semantic segmentation that uses frozen VFM backbone with cross-modal prompt-injected adapters and difference-guided fusion.


<details>
  <summary>Details</summary>
Motivation: Adapting pretrained Vision Foundation Models to multimodal tasks often incurs high computational overhead and suffers from modality imbalance, where auxiliary modalities are suppressed during optimization.

Method: Uses frozen VFM backbone with symmetric dual-stream architecture; Cross-modal Prompt-Injected Adapter for deep semantic interaction; Difference-Guided Gated Fusion Module for adaptive feature fusion; Modality-Conditional Random Masking to mitigate modality imbalance.

Result: Achieves state-of-the-art performance on ISPRS Vaihingen and Potsdam benchmarks with significantly fewer trainable parameters than full fine-tuning.

Conclusion: MoBaNet provides an effective, parameter-efficient solution for robust and balanced multimodal fusion in remote sensing semantic segmentation.
...
TL;DR: DiffVP uses scan-to-reference differences rather than absolute visual features to generate CT reports, improving accuracy by focusing on diagnostically relevant changes.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based CT report generation methods encode 3D volumes holistically, failing to distinguish informative cues from redundant anatomical background. They don't leverage radiological cognitive subtraction principles.

Method: Proposes Differential Visual Prompting (DiffVP) with: 1) hierarchical difference extractor capturing global/local semantic discrepancies, 2) difference-to-prompt generator transforming signals into learnable visual prefix tokens for LLM conditioning.

Result: Outperforms prior methods on two benchmarks, improving average BLEU-1-4 by +10.98 and +4.36, and achieves F1 score 0.421 on RadGenome-ChestCT.

Conclusion: DiffVP effectively conditions report generation on semantic differences rather than absolute features, implicitly suppressing invariant anatomy while amplifying diagnostically relevant evidence without explicit lesion localization.
...
TL;DR: TAPESTRY generates high-fidelity 360° turntable videos from untextured 3D models using geometry-conditioned video diffusion, enabling automated creation of complete 3D assets.


<details>
  <summary>Details</summary>
Motivation: Existing video diffusion models struggle with geometric consistency and appearance stability across views, making them unsuitable for high-quality 3D reconstruction from untextured models.

Method: Reframes 3D appearance generation as geometry-conditioned video diffusion: renders multi-modal geometric features from 3D meshes to constrain video generation with pixel-level precision, plus a multi-stage reconstruction pipeline with 3D-Aware Inpainting for self-occluded regions.

Result: Outperforms existing approaches in both video consistency and final reconstruction quality, generating videos that serve as both dynamic previews and reliable intermediate representations for texture synthesis and neural rendering.

Conclusion: TAPESTRY enables automated creation of production-ready 3D assets from untextured meshes through high-quality, geometry-consistent turntable videos that can be back-projected into UV textures or used to supervise neural rendering methods.
...
TL;DR: C2P is a prompt-free universal medical image segmentation framework that disentangles anatomical knowledge into geometric and semantic representations using MLLMs, enabling robust cross-modal segmentation without manual prompts.


<details>
  <summary>Details</summary>
Motivation: Existing universal medical segmentation approaches rely too heavily on manual visual prompts or retrieved reference images, limiting automation and robustness. Naive joint training across modalities fails to address large domain shifts between different imaging techniques.

Method: C2P separates anatomical knowledge into Geometric and Semantic representations, uses MLLMs to distill medical concepts into learnable Semantic Tokens, introduces supervised Geometric Tokens for universal physical constraints, generates input-specific dynamic kernels for mask prediction, and employs a Geometry-Aware Inference Consensus mechanism to assess prediction reliability.

Result: Extensive experiments on eight diverse datasets across seven modalities demonstrate significant superiority over universe- or single-model approaches. The unified model shows strong generalization, achieving impressive results on zero-shot tasks with unseen cases and cross-modal transfers across similar tasks.

Conclusion: C2P provides an effective prompt-free framework for universal medical image segmentation that successfully addresses domain shifts across modalities through disentangled geometric and semantic representations, enabling robust automation without manual intervention.
...
TL;DR: PC-CrossDiff is a unified dual-task framework for 3D visual grounding that uses dual-level cross-modal differential attention to better handle complex multi-object scenes by parsing implicit localization cues and suppressing spatial interference.


<details>
  <summary>Details</summary>
Motivation: Existing 3D visual grounding methods work well in simple single-object scenes but suffer severe performance degradation in complex multi-object scenes common in real-world settings. They struggle with parsing implicit localization cues needed to disambiguate visually similar objects and suppressing dynamic spatial interference from co-occurring objects.

Method: PC-CrossDiff introduces a dual-level cross-modal differential attention architecture: (1) Point-Level Differential Attention (PLDA) modules apply bidirectional differential attention between text and point clouds to adaptively extract implicit localization cues via learnable weights; (2) Cluster-Level Differential Attention (CLDA) modules establish hierarchical attention to enhance localization-relevant spatial relationships while suppressing ambiguous/irrelevant relations through a localization-aware differential attention block.

Result: Achieves state-of-the-art performance on ScanRefer, NR3D, and SR3D benchmarks. Notably improves Overall@0.50 score by +10.16% on Implicit subsets of ScanRefer for 3DREC task, demonstrating strong ability to parse implicit spatial cues.

Conclusion: PC-CrossDiff effectively addresses key challenges in complex multi-object 3D visual grounding by parsing implicit localization cues and suppressing spatial interference through its dual-level differential attention architecture, enabling better performance in real-world scenarios.
...
TL;DR: SCEP is a training-free framework that uses evidence-driven reasoning for image deepfake detection, outperforming baselines without fine-tuning large vision-language models.


<details>
  <summary>Details</summary>
Motivation: Current LVLM-based deepfake detection methods require costly fine-tuning and generalize poorly to diverse, evolving manipulations. There's a need for training-free approaches that can effectively detect manipulated images.

Method: SCEP replaces whole-image inference with evidence-driven reasoning: 1) Uses vision encoder's CLS token as global reference, 2) Clusters patch features into coherent groups, 3) Scores patches with fused metric (CLS-guided semantic mismatch + frequency/noise anomalies), 4) Samples high-confidence patches per cluster with grid-based NMS, 5) Produces evidence pack to condition frozen LVLM for prediction.

Result: Experiments on diverse benchmarks show SCEP outperforms strong baselines without requiring LVLM fine-tuning.

Conclusion: SCEP provides an effective training-free framework for image deepfake detection that leverages evidence-driven reasoning to identify manipulation cues, offering better generalization to diverse manipulations without costly model fine-tuning.
...
TL;DR: CrowdGaussian: A unified framework for reconstructing multi-person 3D Gaussian Splatting representations from single images, handling occlusions and low clarity in crowd scenes.


<details>
  <summary>Details</summary>
Motivation: Existing 3D human reconstruction methods focus on clear, close-up single-person images and perform poorly in multi-person scenarios with extensive occlusions, low clarity, and varied appearances.

Method: Proposes CrowdGaussian framework with: 1) self-supervised adaptation pipeline to handle occlusions using pretrained large human models, and 2) Self-Calibrated Learning (SCL) training strategy that uses single-step diffusion models to refine coarse renderings and distill improvements back to 3DGS representations.

Result: Extensive experiments show CrowdGaussian generates photorealistic, geometrically coherent reconstructions of multi-person scenes from single-image inputs.

Conclusion: CrowdGaussian successfully addresses the challenging task of multi-person 3D reconstruction from single images, overcoming occlusion and clarity issues through innovative self-supervised adaptation and calibration techniques.
...
TL;DR: PEFT methods like QLoRA outperform traditional training approaches for livestock behavior classification, achieving 83% accuracy with only 2.7% of parameters while avoiding overfitting issues.


<details>
  <summary>Details</summary>
Motivation: Automated behavior classification for precision livestock farming faces challenges of high computational costs and limited labeled data, requiring efficient adaptation of large foundation models.

Method: Systematic comparison of three approaches: training from scratch (ResNet-18, ViT-Small), frozen feature extraction, and parameter-efficient fine-tuning (PEFT) using QLoRA and DoRA on DINOv3 foundation model with varying ranks and target modules.

Result: PEFT substantially outperformed alternatives, with best QLoRA achieving 83.16% test accuracy using only 2.72% parameters (183M) in 5.8 hours, compared to 72.87% for ResNet-18 and 76.56% for frozen DINOv3. Increasing adapter capacity improved generalization without overfitting.

Conclusion: Underfitting, not overfitting, is the primary challenge when adapting foundation models to agricultural imagery. PEFT provides efficient deployment guidelines for billion-parameter vision models in livestock applications.
...
TL;DR: Multi-label GI video analysis pipeline using ResNet-50 frame classifier with anatomy-guided temporal event decoding to predict 17 anatomy/pathology labels, addressing class imbalance via clipped positive weighting and improving temporal mAP from 0.3801 to 0.4303.


<details>
  <summary>Details</summary>
Motivation: To develop an effective gastrointestinal video analysis system that can accurately predict multiple anatomy and pathology labels from endoscopic videos, addressing challenges of severe class imbalance and temporal event fragmentation.

Method: ResNet-50 frame classifier on 336x336 frames with clipped class-wise positive weighting for imbalance, followed by anatomy-guided temporal decoding combining GT-style framewise event composition, anatomy vote smoothing, anatomy-based pathology gating, and conservative hysteresis decoder.

Result: Improved temporal mAP from 0.3801 to 0.4303 on challenge test set by addressing class imbalance and temporal fragmentation issues.

Conclusion: The proposed pipeline effectively handles multi-label GI video analysis through combined frame-level classification with weighted loss for imbalance and sophisticated temporal decoding strategies, achieving significant performance improvements.
...
TL;DR: M2P improves vision foundation models for point tracking using video object segmentation masks with three novel weakly-supervised constraints, achieving significant gains over DINOv2/DINOv3 on TAP benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current Vision Foundation Models rely on static image pre-training, which is suboptimal for capturing dense temporal correspondence in videos needed for Tracking Any Point tasks.

Method: Mask-to-Point learning with three mask-based constraints: 1) local structure consistency loss using Procrustes analysis, 2) mask label consistency loss for foreground matching, 3) mask boundary constraint for boundary points.

Result: Achieves 12.8% and 14.6% performance gains over DINOv2-B/14 and DINOv3-B/16 on TAP-Vid-DAVIS benchmark using only 3.6K VOS training videos; serves as effective pre-trained backbone for both test-time optimized and offline fine-tuned TAP tasks.

Conclusion: M2P provides an effective weakly-supervised approach to adapt static image VFMs for dense point tracking using video object segmentation masks, demonstrating potential as general pre-trained models for point tracking tasks.
...
TL;DR: STAS: Training-free method that steers rare high-magnitude hidden state spikes (Massive Activations) in video diffusion transformers to improve video quality and temporal coherence with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Despite progress in video diffusion transformers, leveraging their internal model signals with minimal overhead to enhance video generation quality remains underexplored. The paper investigates rare high-magnitude hidden state spikes (Massive Activations) that show structured patterns in video generation.

Method: Proposes Structured Activation Steering (STAS), a training-free self-guidance-like method that steers MA values at first-frame and boundary tokens toward a scaled global maximum reference magnitude, based on observed structured patterns in MA magnitudes across different token positions.
...
Conclusion: The structured patterns in Massive Activations reveal implicit temporal prioritization in video diffusion transformers, and steering these activations provides an effective, low-overhead approach to enhancing video generation quality.
...
TL;DR: TINA is a text-free inversion attack that bypasses text-centric concept erasure defenses in diffusion models by operating under null-text conditions, revealing that current erasure methods only obscure concepts rather than truly removing them.


<details>
  <summary>Details</summary>
Motivation: Current concept erasure techniques in text-to-image diffusion models focus on severing text-to-image mapping while leaving underlying visual knowledge intact. The adversarial co-evolution has converged on narrow text-centric defenses that can be bypassed, highlighting the need for approaches that directly address internal visual knowledge.

Method: TINA (Text-free INversion Attack) uses DDIM inversion under null-text conditions to probe for visual generative pathways of erased concepts. It includes an optimization procedure to overcome approximation errors that arise when operating without textual guidance, allowing it to bypass text-centric defenses.

Result: TINA successfully regenerates erased concepts from models treated with state-of-the-art unlearning methods, demonstrating that current erasure techniques merely obscure concepts rather than truly removing them.

Conclusion: The success of TINA proves that current concept erasure methods are insufficient and highlights an urgent need for new paradigms that operate directly on internal visual knowledge rather than just text-to-image mapping.
...
<details>
  <summary>Details</summary>
Motivation: Video understanding requires modeling temporal dynamics and evolving visual context, placing stronger demands on spatiotemporal reasoning than image understanding, making it a foundational problem in computer vision that needs systematic organization.

Method: Organizes literature into three complementary perspectives: 1) low-level video geometry understanding, 2) high-level semantic understanding, and 3) unified video understanding models. Highlights shift from task-specific pipelines to unified modeling paradigms.

Result: Provides coherent map of evolving video understanding landscape, summarizes key modeling trends and design principles, and outlines open challenges toward building robust, scalable, and unified video foundation models.

Conclusion: Survey consolidates three perspectives to provide systematic view of video understanding progress, emphasizing movement toward unified models that can adapt to diverse downstream objectives while identifying remaining challenges for future video foundation models.
...
TL;DR: Omni-3DEdit is a unified learning-based model that performs various 3D editing tasks in one forward pass, eliminating time-consuming iterative optimization.


<details>
  <summary>Details</summary>
Motivation: Current instruction-driven 3D editing methods have two main drawbacks: 1) lack of universal design for different editing tasks requiring task-dependent rules, and 2) time-consuming iterative optimization requiring thousands of 2D/3D updates.

Method: Constructed a data pipeline to synthesize paired multi-view editing samples, adapted pre-trained SEVA model by concatenating source view latents with conditional tokens, and introduced dual-stream LoRA module to disentangle view cues.

Result: The model reduces inference time from tens of minutes to approximately two minutes, can complete various 3D editing tasks in one forward pass, and shows effectiveness through extensive experiments.

Conclusion: Omni-3DEdit provides an efficient, unified solution for 3D editing that overcomes limitations of existing iterative optimization methods while maintaining high-quality results.
...
TL;DR: This paper comprehensively evaluates foundation models for cell segmentation (CellPoseSAM, CellSAM, μSAM) and general-purpose segmentation (SAM, SAM2, SAM3) on diverse microscopy datasets, introduces an automatic prompt generation (APG) strategy that improves μSAM performance, and provides lessons for adapting SAM-style models to microscopy.


<details>
  <summary>Details</summary>
Motivation: While several foundation models for cell segmentation have been introduced as extensions of Segment Anything Model (SAM), and newer versions SAM2/SAM3 have been released, there's a need for comprehensive evaluation of these models on diverse microscopy datasets and development of strategies to improve their performance for microscopy applications.

Method: The authors conduct comprehensive evaluation of foundation models (CellPoseSAM, CellSAM, μSAM, SAM, SAM2, SAM3) on diverse microscopy datasets for cell, nucleus and organoid segmentation. They introduce automatic prompt generation (APG) strategy to improve SAM-based microscopy foundation models, using μSAM as the base model.

Result: APG consistently improves segmentation results for μSAM and is competitive with state-of-the-art CellPoseSAM. The evaluation provides insights into model performance across different microscopy tasks and datasets.

Conclusion: The work provides important lessons for adaptation strategies of SAM-style models to microscopy and offers a strategy for creating more powerful microscopy foundation models. The APG approach effectively enhances SAM-based models for microscopy applications.
...
TL;DR: Eye tracking heatmaps outperform other human saliency methods for open-set iris presentation attack detection, showing best generalization in leave-one-attack-type experiments.


<details>
  <summary>Details</summary>
Motivation: While human perceptual priors show promise for iris PAD, the most effective form of human saliency (hand annotations vs eye tracking vs others) for open-set iris PAD remains underexplored.

Method: Compared hand annotations, eye tracking heatmaps, segmentation masks, and DINOv2 embeddings against state-of-the-art deep learning baseline for open-set iris PAD using leave-one-attack-type out paradigm.

Result: Denoised eye tracking heatmaps showed best generalization improvement over cross entropy in terms of AUROC and APCER at BPCER=1%.

Conclusion: Eye tracking heatmaps are the most effective human saliency method for open-set iris PAD, with models, code, and saliency maps released for reproducibility.
...
TL;DR: Edit spillover in image editing models reveals world understanding vs. attention leakage; new framework probes this via taxonomy, detection pipeline, and benchmark showing trade-offs between editing control and semantic knowledge.


<details>
  <summary>Details</summary>
Motivation: Instruction-following image editing models often modify semantically related but unspecified content (edit spillover), raising the question of whether this reflects genuine world understanding or just attention leakage. The paper aims to systematically investigate this phenomenon.

Method: Proposes EditSpilloverProbe framework with: 1) spillover taxonomy (spatial, semantic, mixed, random), 2) automated detection-and-classification pipeline, 3) EditSpilloverBench dataset from real-world Chinese text editing tasks. Evaluates 5 representative editing models systematically.

Result: Three key findings: 1) Spillover rates vary 3.3x across architectures (3.49% to 11.46%); 2) Semantic spillover quantity indicates world understanding - nano_banana has most (27.8 per image) while qwen_2511 has best control but lower semantic spillover; 3) Spatial decay analysis shows exponential area decay but constant semantic relevance (40%-58%), providing evidence spillover reflects genuine world understanding.

Conclusion: Edit spillover serves as a natural probe for world knowledge in image editing models, revealing a trade-off between editing control and world understanding capabilities. Semantic spillover reflects genuine implicit world knowledge rather than mere spatial diffusion.
...
<details>
  <summary>Details</summary>
Motivation: There's growing demand for identity-aware content creation, but no openly accessible framework exists for fine-grained control over facial appearance and voice timbre across multiple identities in generated videos.

Method: 1) Data curation pipeline that automatically extracts identity-bearing information with paired audio-visual annotations; 2) Flexible identity injection mechanism for single/multi-subject scenarios using facial appearance and vocal timbre as control signals; 3) Multi-stage training strategy to address modality disparity and enforce cross-modal coherence.
...
Conclusion: The paper presents a scalable framework for identity-aware joint audio-video generation that successfully addresses the lack of accessible tools for fine-grained control over facial appearance and voice timbre in generated content.
...
TL;DR: CAT framework enables reusable creative tokenization for T2I models, achieving 3.7× speedup and 4.8× cost reduction while improving creativity and alignment.


<details>
  <summary>Details</summary>
Motivation: Current T2I models struggle with fuzzy prompts requiring creative interpretation, and existing agent-driven approaches are computationally expensive and non-reusable, requiring repeated reasoning for each generation.

Method: Introduces Creative Agent Tokenization (CAT) with a Creative Tokenizer that generates reusable token templates from fuzzy prompt embeddings, trained via creative semantic disentanglement using overlapping concept pairs.

Result: Achieves 3.7× speedup and 4.8× computational cost reduction compared to state-of-the-art methods, with superior human preference and text-image alignment on Architecture Design, Furniture Design, and Nature Mixture tasks.

Conclusion: CAT provides a scalable and effective paradigm for enhancing creativity in T2I generation by encapsulating creative understanding in reusable tokens, eliminating the need for repeated reasoning or prompt augmentation.
...
TL;DR: SpiderCam is an FPGA-based snapshot depth-from-defocus camera that produces 480x400 sparse depth maps at 32.5 FPS over 52 cm range while consuming only 624 mW total power.


<details>
  <summary>Details</summary>
Motivation: To develop a low-power, real-time 3D camera system using passive depth sensing that achieves sub-Watt total power consumption, which hasn't been achieved before in FPGA-based 3D cameras.

Method: Uses a custom camera capturing two differently focused images simultaneously, processed with SystemVerilog implementation of depth from differential defocus (DfDD) on a low-power FPGA. Includes algorithmic improvements to DfDD for low-power sensors and memory-local implementation for streaming depth computation on resource-constrained hardware.

Result: Achieves 480x400 sparse depth maps at 32.5 FPS over 52 cm working range with total power consumption of 624 mW, making it the first sub-Watt total power measurement for passive FPGA-based 3D cameras in literature.

Conclusion: SpiderCam demonstrates that real-time passive 3D sensing with sub-Watt power consumption is achievable through custom hardware design, algorithmic improvements, and memory-efficient FPGA implementation, enabling new applications in power-constrained environments.
...
TL;DR: Proposes a VAE-based semi-gray-box anomaly detection framework for collaborative edge-AI that detects adversarial data injections in noisy environments by incorporating noise-aware features.


<details>
  <summary>Details</summary>
Motivation: Edge-offloading for collaborative DNN inference is vulnerable to stealthy adversarial attacks that are hard to detect, especially when environmental noise is present, creating a need for robust detection mechanisms.

Method: Develops a variational autoencoder (VAE)-based semi-gray-box anomaly detection framework with robust noise-aware features that capture characteristic environmental noise behavior to distinguish between legitimate and adversarial data.
...
Conclusion: The proposed noise-aware VAE framework provides effective detection of adversarial attacks in collaborative edge-AI systems, demonstrating robustness in noisy environments while identifying key limitations for future improvement.
...
TL;DR: Proposes SegFly, a large-scale UAV semantic segmentation benchmark with RGB and thermal images, using a geometry-driven 2D-3D-2D paradigm for automatic label propagation and cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Existing UAV RGB and RGB-T datasets are limited in scale, diversity, and annotation efficiency due to high manual labeling costs and difficulties with accurate RGB-T alignment on off-the-shelf UAVs.

Method: A scalable 2D-3D-2D paradigm that lifts <3% of manually annotated RGB images into a semantic 3D point cloud, then reprojects labels to all views for automatic pseudo ground-truth generation across RGB and thermal modalities, also enabling cross-modal registration via 3D geometry.

Result: Created SegFly benchmark with >20k RGB images and >15k aligned RGB-T pairs; achieved 91% RGB and 88% thermal annotation accuracy, 87% RGB-T registration accuracy; established Firefly baseline showing conventional and foundation models benefit from SegFly supervision.

Conclusion: Geometry-driven 2D-3D-2D pipelines enable scalable multi-modal scene understanding for UAVs, with SegFly providing a valuable large-scale benchmark for RGB and thermal semantic segmentation research.
...
TL;DR: Interpretable AI pipeline for legal age estimation from clavicle CT scans achieves state-of-the-art accuracy with uncertainty quantification, outperforming human experts and previous methods.


<details>
  <summary>Details</summary>
Motivation: Legal age estimation needs accurate, robust, reproducible methods with explicit uncertainty quantification for forensic contexts. Clavicle CT scans are effective but underexplored compared to hand radiographs and dental imaging.

Method: Multi-stage pipeline combining: (1) automatic clavicle detection via feature-based connected-component method with minimal manual annotation, (2) Integrated Gradients-guided slice selection for multi-slice CNN input, and (3) conformal prediction intervals for uncertainty-aware decisions.

Result: Achieves MAE of 1.55 ± 0.16 years on held-out test set, outperforming human experts (~1.90 years MAE) and previous methods (>1.75 years MAE). Conformal prediction enables configurable coverage levels, and attribution maps show focus on anatomically relevant medial clavicular epiphysis regions.

Conclusion: The interpretable pipeline provides accurate, uncertainty-aware age estimation from clavicle CT scans, currently being integrated into Skeleton-ID software as decision-support for multi-factorial forensic workflows.
...
<details>
  <summary>Details</summary>
Motivation: There's a gap between abundant dashcam video evidence and legal responsibility determination, which still relies heavily on human experts. Existing ego-view traffic accident studies focus on perception/semantic understanding, while LLM-based legal methods use only textual case descriptions, lacking video evidence integration.

Method: Two-stage framework: (1) Traffic accident understanding module generates textual descriptions from dashcam videos; (2) Legal multi-agent framework outputs responsibility modes, statute sets, and complete judgment reports based on Chinese traffic regulations.
...
Conclusion: The proposed C-TRAIL dataset and two-stage framework successfully bridge video evidence with legal responsibility determination, offering an effective solution for automated traffic accident legal analysis with interpretable reasoning.
...
TL;DR: TransText: First method for adapting image-to-video models to layer-aware text animation using Alpha-as-RGB paradigm without modifying pre-trained generative manifold.


<details>
  <summary>Details</summary>
Motivation: Existing approaches handle transparency as extra latent dimension requiring VAE reconstruction, which is computationally expensive and risks eroding semantic priors learned from RGB data. Need method for transparent glyph animation without these limitations.

Method: Proposes TransText framework with Alpha-as-RGB paradigm that embeds alpha channel as RGB-compatible visual signal through latent spatial concatenation, ensuring cross-modal consistency while preventing feature entanglement.
...
Conclusion: TransText enables effective layer-aware text animation without modifying pre-trained generative models, addressing computational and semantic preservation challenges in transparent content generation.
...
TL;DR: LaDe is a latent diffusion framework that generates editable layered design documents with flexible layer counts using natural language prompts, outperforming existing methods in text-to-layer alignment.


<details>
  <summary>Details</summary>
Motivation: Existing methods for layered media design generation either restrict outputs to fixed layer counts or require each layer to contain only spatially continuous regions, limiting flexibility and causing layer count to scale linearly with design complexity.

Method: LaDe combines three components: 1) LLM-based prompt expander that transforms user intent into structured per-layer descriptions, 2) Latent Diffusion Transformer with 4D RoPE positional encoding for joint generation of full media design and RGBA layers, and 3) RGBA VAE that decodes each layer with full alpha-channel support.

Result: LaDe outperforms Qwen-Image-Layered in text-to-layers generation on the Crello test set, improving text-to-layer alignment as validated by two VLM-as-a-judge evaluators (GPT-4o mini and Qwen3-VL).

Conclusion: LaDe provides a unified framework that supports three tasks (text-to-image, text-to-layers media design generation, and media design decomposition) while generating flexible numbers of semantically meaningful layers for fully editable design documents.
...
TL;DR: Robust-ComBat: MLP-based outlier compensation for diffusion MRI harmonization that handles pathological cases better than traditional statistical methods.


<details>
  <summary>Details</summary>
Motivation: Standard ComBat harmonization assumes Gaussian distributions, but neurological patients often have pathological outliers that distort site-effect estimation. Clinical practice includes undiagnosed patients who can't be excluded from harmonization cohorts.

Method: Evaluated 10 outlier rejection methods with 4 ComBat variants across 7 neurological conditions. Proposed Robust-ComBat using a simple MLP for robust outlier compensation while preserving disease-related signals.

Result: Robust-ComBat consistently outperformed conventional statistical baselines with lower harmonization error across all ComBat variants, even with up to 80% of subjects having neurological disorders.

Conclusion: MLP-based outlier compensation enables reliable harmonization in clinical settings where pathological cases are unavoidable, preserving both site-effect removal and disease-related signals.
...
TL;DR: AHOY reconstructs complete 3D Gaussian avatars from occluded monocular videos using hallucination supervision and a two-stage architecture.


<details>
  <summary>Details</summary>
Motivation: Existing methods require unoccluded input with fully visible subjects, excluding most real-world footage where people are routinely occluded by objects, furniture, or other people.

Method: Four key contributions: (1) hallucination-as-supervision using identity-finetuned diffusion models to generate supervision for unobserved regions, (2) two-stage canonical-to-pose-dependent architecture, (3) map-pose/LBS-pose decoupling to handle multi-view inconsistencies, (4) head/body split supervision to preserve facial identity.

Result: State-of-the-art reconstruction quality on YouTube videos and multi-view capture data with significant occlusion; avatars are robust enough for animation with novel poses and compositing into 3DGS scenes.

Conclusion: AHOY enables reconstruction of complete, animatable 3D Gaussian avatars from heavily occluded monocular video, addressing fundamental challenges of unobserved body regions and lack of multi-view supervision.
...
TL;DR: Proposes adaptive radar data compression using gradient descent on detection confidence to dynamically adjust compression ratio, achieving 100x size reduction with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Radar data is essential for autonomous driving but high-dimensional raw data saturates communication links; existing image compression methods are unsuitable as they operate at fixed ratios and can't adapt to varying conditions.

Method: Uses adaptive feedback compression with zeroth-order gradient approximation to adjust compression ratio based on detection confidence. Applies discrete cosine transform to radar cubes, selectively prunes coefficients, and preserves dynamic range through scaled quantization.
...
Conclusion: The proposed adaptive compression scheme effectively addresses radar data communication bottlenecks while maintaining detection performance, making it suitable for real-world autonomous driving applications.
...
TL;DR: Motion-MLLM enhances multimodal LLMs with egomotion data from IMUs to improve 3D spatial reasoning while reducing computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs for 3D spatial reasoning rely on expensive 3D representations (point clouds, BEV maps) or lack physical grounding for scale/size resolution, creating computational and accuracy limitations.

Method: Proposes Motion-MLLM with two key components: 1) cascaded motion-visual keyframe filtering using IMU data and visual features to select sparse representative keyframes, and 2) asymmetric cross-modal fusion where motion tokens channel egomotion cues and cross-frame visual context into visual representations.

Result: Motion-MLLM achieves similar or higher accuracy than SOTA methods using video frames and explicit 3D data, with significantly less overhead (1.40× and 1.63× higher cost-effectiveness respectively).

Conclusion: Incorporating egomotion modality data from IMUs enables more efficient and physically-grounded 3D scene understanding and spatial reasoning in MLLMs.
...
<details>
  <summary>Details</summary>
Motivation: Existing video editing methods struggle with complex edits involving action modification, dynamic events, or inserting objects that affect scene behaviors. Trained models lack relevant training data, while training-free methods are limited to structure- and motion-preserving edits.

Method: DynaEdit uses an inversion-free, model-agnostic approach with pretrained text-to-video flow models. It addresses low-frequency misalignment and high-frequency jitter issues through novel mechanisms, enabling complex editing without model training.

Result: DynaEdit achieves state-of-the-art results on complex text-based video editing tasks, successfully modifying actions, inserting interactive objects, and introducing global effects while maintaining video quality.

Conclusion: DynaEdit represents a significant advancement in training-free video editing, unlocking versatile capabilities for complex real-world video modifications that were previously challenging or impossible with existing methods.
...
TL;DR: LoST proposes semantic-level tokenization for 3D shapes, using semantic salience ordering instead of geometric LoD hierarchies, achieving SOTA reconstruction with 100-1000x fewer tokens than prior methods.


<details>
  <summary>Details</summary>
Motivation: Current 3D shape tokenization methods rely on geometric level-of-detail hierarchies designed for rendering/compression, which are token-inefficient and lack semantic coherence for autoregressive modeling. There's a need for more efficient, semantically-aware tokenization for 3D generative modeling.

Method: Proposes Level-of-Semantics Tokenization (LoST) that orders tokens by semantic salience, where early prefixes decode into complete plausible shapes with principal semantics, and later tokens refine details. Introduces Relational Inter-Distance Alignment (RIDA) loss to align 3D shape latent space with semantic DINO feature space structure.

Result: Achieves state-of-the-art reconstruction, surpassing previous LoD-based tokenizers by large margins on both geometric and semantic metrics. Enables efficient high-quality AR 3D generation using only 0.1%-10% of tokens needed by prior models, while enabling downstream tasks like semantic retrieval.

Conclusion: LoST provides a more efficient and semantically coherent tokenization approach for 3D shapes that enables high-quality autoregressive generation with dramatically fewer tokens, addressing limitations of traditional geometric hierarchies for generative modeling.
...
TL;DR: Training-free framework for continuous, controllable image editing using text-embedding steering without additional training or manual intervention.


<details>
  <summary>Details</summary>
Motivation: Prior approaches require additional training or manual user intervention for continuous image editing in text-conditioned generative models, which is inefficient and limits accessibility.

Method: Uses LLM to generate debiased contrastive prompt pairs, computes steering vector in text-encoder space, adds to input prompt, and employs elastic range search for continuous control across effective steering magnitudes.

Result: Method produces smooth, continuous edits comparable to training-based alternatives, outperforms other training-free methods, and generalizes across image and video generation modalities.

Conclusion: Simple text-embedding steering is sufficient for continuous, controllable image editing without training, offering lightweight yet effective alternative to complex training-based approaches.
...
TL;DR: EchoGen is a unified framework for layout-to-image generation and image grounding that uses progressive training strategies to overcome optimization challenges and achieve synergistic gains between both tasks.


<details>
  <summary>Details</summary>
Motivation: Image grounding has strong text and layout understanding that can compensate for layout-to-image generation limitations, while layout-generated images provide content diversity to enhance grounding robustness. Joint training should improve both tasks but faces optimization challenges.

Method: Three-stage progressive training: 1) Parallel Multi-Task Pre-training (PMTP) with shared tokens for basic abilities, 2) Dual Joint Optimization (DJO) exploiting task duality for unified optimization, 3) Cycle RL stage using consistency constraints as rewards via GRPO strategy to eliminate visual supervision dependence.

Result: State-of-the-art results on both layout-to-image generation and image grounding benchmarks, with clear synergistic gains from optimizing the two tasks together.

Conclusion: EchoGen demonstrates that unified training of layout-to-image generation and image grounding with progressive strategies effectively addresses optimization challenges and achieves superior performance through task synergy.
...
TL;DR: SkeletonLLM enables multimodal LLMs to understand skeleton data by converting skeletal sequences into visual representations that MLLMs can process, achieving strong performance across various skeleton understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs can't directly process structured non-visual data like human skeletons. Existing methods either compress skeleton data into lossy features or use discrete tokens that generalize poorly across different skeleton formats.

Method: SkeletonLLM uses DrAction, a differentiable format-agnostic renderer that converts skeletal kinematics into compact image sequences. It employs cooperative training with Causal Reasoning Distillation (transferring step-by-step reasoning from a teacher) and Discriminative Finetuning (sharpening decision boundaries).
...
Conclusion: The approach successfully enables MLLMs to understand skeleton data by translating it into their native visual modality, with code to be released upon acceptance.
...
TL;DR: Deploying foundation models in embodied edge systems requires system-level co-design across memory, scheduling, communication, and architecture, not just model compression.


<details>
  <summary>Details</summary>
Motivation: Real-time control in embodied edge systems must operate within strict size, weight, and power constraints, where memory traffic, compute latency, timing variability, and safety margins interact directly.

Method: The Deployment Gauntlet organizes constraints into eight coupled barriers to determine whether embodied foundation models can run reliably in practice, analyzing representative edge workloads.
...
TL;DR: Rewarding DINO: A language-conditioned reward modeling method that learns actual reward functions from visual inputs, not just specific trajectories, enabling generalization to new settings with low computational overhead.


<details>
  <summary>Details</summary>
Motivation: Dense reward design is challenging and often requires privileged simulation state information unavailable in real-world settings. Existing visual reward prediction methods are biased toward specific demonstration trajectories and fail in states not covered by demonstrations.

Method: Introduces Rewarding DINO, a language-conditioned reward model trained on 24 Meta-World+ tasks using rank-based loss. The compact model learns actual reward functions rather than specific trajectories, serving as a direct replacement for analytical reward functions with low computational overhead.

Result: Achieves competitive performance on training tasks and generalizes to new simulation and real-world settings. Shows good pairwise accuracy, rank correlation, and calibration. Successfully enables off-the-shelf RL algorithms to solve Meta-World+ tasks.

Conclusion: Rewarding DINO learns task semantics rather than just trajectories, enabling effective reward modeling that generalizes beyond demonstration coverage and works in both simulation and real-world environments with minimal computational cost.
...
TL;DR: Real-to-sim-to-real framework uses kNN human surrogate from <5 min teleoperation data to train RL copilot that improves fine-grained manipulation success for novices and efficiency for experts.


<details>
  <summary>Details</summary>
Motivation: Fine-grained teleoperation is slow and error-prone even for experts. Shared autonomy can help but requires accurate human behavior models, which are hard to obtain in practice.

Method: Real-to-sim-to-real framework: collect <5 min real teleoperation data, create kNN human surrogate model, train residual copilot policy with model-free RL in simulation, deploy to assist real operators.

Result: Improves task success for novices and execution efficiency for experts on nut threading, gear meshing, peg insertion tasks compared to direct teleoperation and baseline shared autonomy methods.

Conclusion: The framework enables effective shared autonomy with minimal real-world data, improves teleoperation performance, and produces better demonstrations for downstream imitation learning.
...
TL;DR: Learning-based Hamilton-Jacobi reachability with Fourier Neural Operators enables real-time contingency-aware navigation with formal safety guarantees.


<details>
  <summary>Details</summary>
Motivation: Traditional Hamilton-Jacobi reachability provides formal safety guarantees but is computationally expensive for high-dimensional systems, limiting real-time planning applications.

Method: Use Fourier Neural Operator to approximate HJ solution operator under varying obstacles, integrate certified reachable sets with incremental multi-goal planner, and enforce reachable-set constraints with recovery policy.

Result: Achieves asymptotically optimal navigation with provable contingency behavior, validated through real-time deployment on KUKA youBot in Webots simulation.

Conclusion: The framework successfully combines learning-based reachability with sampling-based planning to enable real-time contingency-aware navigation with formal safety guarantees.
...
TL;DR: TeleDex is an open-source phone-based teleoperation system for dexterous robot manipulation that enables quick demonstration collection for policy fine-tuning without external tracking hardware.


<details>
  <summary>Details</summary>
Motivation: Robot manipulation policies struggle to generalize beyond training distributions, requiring additional demonstrations for new environments/tasks/embodiments. Current solutions lack tools for quick, affordable, minimal-setup demonstration collection in real-world deployment.

Method: TeleDex streams low-latency 6-DoF wrist poses and 21-DoF hand state estimates from any phone, retargeting to robot arms and multi-fingered hands. Supports phone-only mode and optional 3D-printed hand-mounted interface for finger-level teleoperation without external tracking infrastructure.

Result: System evaluated across simulation and real-world manipulation tasks, demonstrating effectiveness as a unified scalable interface for robot teleoperation. All software/hardware designs and demonstration videos are open-sourced.

Conclusion: TeleDex lowers hardware/setup barriers to dexterous teleoperation, enabling users to quickly collect demonstrations during deployment to support policy fine-tuning, addressing generalization challenges in robot manipulation.
...
TL;DR: SLowRL combines Low-Rank Adaptation (LoRA) with safety enforcement for efficient and safe sim-to-real fine-tuning of locomotion policies, achieving 46.5% faster tuning with near-zero safety violations.


<details>
  <summary>Details</summary>
Motivation: Sim-to-real transfer often degrades performance due to reality gap, and naive hardware fine-tuning risks mechanical failure and suffers from sample inefficiency.

Method: SLowRL framework combines Low-Rank Adaptation (LoRA) with training-time safety enforcement via a recovery policy for safe fine-tuning of RL policies learned in simulation.

Result: 46.5% reduction in fine-tuning time and near-zero safety violations compared to PPO baselines; rank-1 adaptation alone recovers pre-trained performance while maintaining safe real-world fine-tuning.

Conclusion: SLowRL demonstrates practical, safe, and efficient fine-tuning for dynamic real-world robotic applications, enabling stable hardware adaptation with minimal safety risks.
...
TL;DR: A framework combining sequential control barrier functions with model-free RL to enforce complex spatio-temporal tasks specified in Signal Temporal Logic during learning.


<details>
  <summary>Details</summary>
Motivation: Real-world robotics applications need more than just safety constraints - they require complex spatio-temporal tasks like periodic recharging or time-bounded visits to specific regions. Current safe RL approaches struggle with these richer specifications, and STL provides a formal way to express such tasks but integrating it with RL remains challenging.

Method: Proposes a framework that leverages sequential control barrier functions combined with model-free reinforcement learning to ensure STL tasks are satisfied throughout the learning process. The approach extends beyond traditional safety constraints to enforce rich STL specifications, including visits to dynamic targets with unknown trajectories.

Result: Demonstrates effectiveness through various simulations, showing the framework can enforce complex spatio-temporal tasks specified in STL during RL training.

Conclusion: The proposed framework successfully bridges the gap between formal STL specifications and practical RL deployment by ensuring complex spatio-temporal constraints are satisfied throughout learning, enabling safer and more reliable real-world robotics applications.
...
TL;DR: SAL is a modular framework for evaluating visual SLAM systems under adversarial conditions like fog and rain by transforming datasets with severity-controlled perturbations.


<details>
  <summary>Details</summary>
Motivation: There's a need to systematically evaluate how visual SLAM systems perform under challenging real-world conditions like adverse weather, camera issues, and video transport problems to understand their robustness and failure points.

Method: SAL uses a modular architecture with common interfaces to decouple datasets, perturbations, and SLAM algorithms. It represents adversarial conditions as perturbations that transform existing datasets, supports severity levels with real-world units, and includes a search procedure to find failure points.
...
TL;DR: UMI gripper tools for robot learning from demonstration were tested on bandage opening tasks, showing concentrated load grippers outperform distributed ones but still lag far behind bare hands in speed and effectiveness.


<details>
  <summary>Details</summary>
Motivation: Handheld gripper tools like UMI enable efficient data collection for robot learning from demonstration, but their usability needs evaluation for healthcare applications like sterile packaging manipulation.

Method: Tested three conditions (distributed load grippers, concentrated load grippers, bare hands) with 8 participants performing timed bandage opening trials, measuring success rate, completion time, damage, and NASA-TLX workload.

Result: Concentrated load grippers improved performance over distributed load grippers but were substantially slower and less effective than bare hands.

Conclusion: Ergonomic and mechanical refinements in handheld grippers are crucial to reduce user burden and improve demonstration quality for healthcare robotics applications.
...
TL;DR: FastLoop: GPU-accelerated loop closing module for visual SLAM that achieves significant speedups (1.3-3.0x) while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Loop closure in visual SLAM is computationally expensive due to searching across the entire map for matches, creating a performance bottleneck that needs acceleration.

Method: Identifies key performance bottlenecks in loop closing pipeline and addresses them through GPU parallel optimizations using task-level and data-level parallelism, plus GPU-accelerated pose graph optimization. Built on ORB-SLAM3 using CUDA.
...
Conclusion: FastLoop successfully accelerates loop closure in visual SLAM through GPU optimization, providing significant speed improvements on both desktop and embedded platforms without sacrificing accuracy.
...
TL;DR: Stereo visual SLAM system for lunar rovers integrates learned feature detection/matching with DEM constraints to reduce long-term drift in challenging lunar terrain.


<details>
  <summary>Details</summary>
Motivation: Future lunar missions need autonomous rovers that can traverse tens of kilometers with accurate localization, but the Moon's lack of GPS, extreme illumination, and low-texture regolith cause visual-inertial odometry to accumulate drift over long distances.

Method: Stereo visual SLAM system with learning-based feature extraction/matching front-end for robustness to illumination extremes and repetitive terrain, plus back-end that incorporates DEM-derived height and surface-normal factors into a pose graph for absolute surface constraints.

Result: DEM anchoring consistently reduces absolute trajectory error compared to baseline SLAM methods, lowering drift in long-range navigation even in repetitive or visually aliased terrain, validated on simulated lunar data and real Moon/Mars analog data.

Conclusion: The integration of learned visual features with DEM constraints provides an effective solution for long-range lunar navigation by mitigating drift through absolute surface constraints while maintaining robustness to challenging lunar conditions.
...
TL;DR: A modular autonomy system for lunar navigation won first place in the Lunar Autonomy Challenge by integrating semantic segmentation, visual odometry, SLAM, and planning for GNSS-denied environments.


<details>
  <summary>Details</summary>
Motivation: To develop a full-stack autonomy system capable of operating in GNSS-denied, visually challenging lunar surface environments for the Lunar Autonomy Challenge competition.

Method: Integrated pipeline with semantic segmentation, stereo visual odometry, pose graph SLAM with loop closures, layered planning and control. Uses lightweight learning-based perception models, factor-graph backend for localization, high-level waypoint planning for coverage and loop closures, and local motion planning with arc sampling and geometric obstacle checks.

Result: Achieved centimeter-level localization accuracy, high-fidelity map generation, strong repeatability across random seeds and rock distributions, and first place in the final competition evaluation.

Conclusion: The modular full-stack autonomy system successfully addresses lunar surface navigation challenges in GNSS-denied environments, demonstrating robust performance and winning the Lunar Autonomy Challenge.
...
TL;DR: Using Neural Radiance Fields (NeRFs) to create efficient 3D maps for autonomous rover navigation, enabling better path planning by integrating local and global terrain information.


<details>
  <summary>Details</summary>
Motivation: Current Mars rover autonomy is limited by lack of global maps that can be easily constructed and stored for onboard re-planning, which slows down exploration and science objectives.

Method: Proposes using NeRFs to construct 3D maps from sparse 2D images, then uses kernel ridge regression over terrain features extracted from NeRF maps to interpolate local cost observations across global regions for integrated path planning.
...
Conclusion: NeRFs provide an effective 3D scene representation for autonomous navigation, enabling rovers to re-route around untraversable areas discovered during online operation and accelerating extraterrestrial exploration.
...
TL;DR: ReSteer improves task steerability in multitask robot policies by quantifying steerability issues, identifying problematic states, generating corrective data, and self-refining policies.


<details>
  <summary>Details</summary>
Motivation: Existing multitask robot policies often fail to respond to new instructions during execution (poor task steerability), even when they can complete tasks separately. This limits interactive use where users need to instruct robots to perform any task at any time.

Method: ReSteer framework includes: (1) steerability estimator to identify low-steerability states without full-rollout evaluation, (2) steerable data generator that synthesizes motion segments from these states, and (3) self-refinement pipeline that improves policy steerability using the generated data.

Result: In simulation on LIBERO, ReSteer improves steerability by 11% over 18k rollouts. Real-world experiments show improved steerability enables interactive use where users can instruct robots to perform any task at any time.

Conclusion: ReSteer successfully improves task steerability in multitask robot policies, and the work motivates further study on quantifying steerability and developing data collection strategies for large robot policies.
...
TL;DR: DexEXO is a comfortable, cross-user adaptable hand exoskeleton that aligns visual appearance and kinematics with robots at hardware level, enabling direct policy training from raw RGB observations and simplifying dexterous robot learning pipelines.


<details>
  <summary>Details</summary>
Motivation: Scaling dexterous robot learning is constrained by difficulties in collecting high-quality demonstrations across diverse operators, with existing wearable interfaces trading comfort and adaptability for kinematic fidelity, and requiring visual post-processing due to embodiment mismatch.

Method: Developed DexEXO, a wearability-first hand exoskeleton with pose-tolerant thumb mechanism and slider-based finger interface that supports hand lengths from 140-217mm, plus a passive hand that visually matches deployed robots, enabling direct policy training from raw wrist-mounted RGB observations.

Result: User studies show improved comfort and usability compared to prior systems. Using visually aligned observations alone, trained diffusion policies achieve competitive performance while substantially simplifying the end-to-end pipeline.

Conclusion: Prioritizing wearability and hardware-level embodiment alignment reduces both human and algorithmic bottlenecks in dexterous robot learning without sacrificing task performance.
...
TL;DR: OmniVLN is a zero-shot visual-language navigation framework that combines omnidirectional 3D perception with hierarchical reasoning to improve embodied navigation in real indoor environments by reducing prompt tokens and increasing success rates.


<details>
  <summary>Details</summary>
Motivation: Existing navigation systems struggle in real indoor environments due to narrow field-of-view sensing (forcing repeated rotations and delayed target discovery) and LLM context limitations when using dense 3D maps or exhaustive object lists.

Method: OmniVLN fuses rotating LiDAR and panoramic vision into a hardware-agnostic mapping stack, incrementally builds a five-layer Dynamic Scene Graph (DSG) from mesh to building-level structure, and uses persistent-homology-based room partitioning with hybrid geometric/VLM relation verification. For navigation, it transforms the global DSG into an agent-centric 3D octant representation with multi-resolution spatial attention prompting.
...
Conclusion: OmniVLN demonstrates that hierarchical scene representation with token-efficient reasoning enables more effective language-guided embodied navigation in complex indoor environments, with plans to release code and an omnidirectional multimodal dataset.
...
TL;DR: Proposes KODE: a Koopman operator-based data-driven dynamics modeling method for distributed electric-drive trucks, using dual-branch encoder and physics-informed supervision for accurate long-term state estimation.


<details>
  <summary>Details</summary>
Motivation: Accurate vehicle dynamics modeling is crucial for autonomous driving but challenging due to nonlinearities and coupled longitudinal/lateral dynamics. Existing physics-based models and neural networks struggle to balance identification efficiency, accuracy, and compatibility with linear control strategies.

Method: KODE method uses Koopman operator theory to represent nonlinear dynamics in lifted linear space. Features: 1) novel dual-branch encoder for dynamic states, 2) physics-informed supervision based on geometric consistency of temporal motion, 3) mixture-of-Koopman operator framework to handle diverse driving patterns of distributed electric-drive trucks.
...
Conclusion: The proposed KODE approach successfully addresses the challenges of vehicle dynamics modeling by combining Koopman operator theory with data-driven methods, achieving superior accuracy while maintaining compatibility with linear control strategies through the lifted linear representation.
...
TL;DR: SafeLand is a vision-based autonomous landing system for UAVs that uses semantic segmentation and Bayesian filtering to identify safe landing spots in unknown dynamic environments without prior maps or heavy sensors.


<details>
  <summary>Details</summary>
Motivation: Current UAV landing methods rely on prior maps, heavy sensors like LiDAR, static markers, or fail to handle non-cooperative dynamic obstacles like humans, limiting generalization and real-time performance for safe urban/rural operations.

Method: Uses camera + lightweight height sensor; constructs online semantic ground map via deep learning semantic segmentation (trained on 7 curated datasets, 70.22% mIoU across 20 classes); refines with Bayesian probabilistic filtering with temporal semantic decay; behavior tree governs adaptive landing with real-time obstacle reaction.

Result: Tested in 200 simulations and 60 field tests across industrial/urban/rural environments up to 100m altitude; zero false negatives for human detection; sub-second response latency (substantially lower than previous methods); 95% success rate.

Conclusion: SafeLand provides a lean, vision-based solution for safe autonomous landing in unknown dynamic environments without prior information, achieving high safety and real-time performance while being deployable on embedded systems.
...
TL;DR: FloorPlan-VLN introduces a new VLN paradigm using semantic floor plans as spatial priors, enabling navigation with concise instructions instead of verbose step-by-step guidance.


<details>
  <summary>Details</summary>
Motivation: Current VLN agents require verbose instructions and lack the ability to comprehend and utilize global spatial priors like floor plans, which are ubiquitous in real-world buildings. This limits their spatial reasoning capabilities.

Method: Proposes FP-Nav method that uses dual-view, spatio-temporally aligned video sequences and auxiliary reasoning tasks to align observations, floor plans, and concise instructions. Constructs FloorPlan-VLN dataset with 10k+ episodes across 72 scenes, pairing semantically annotated floor plans with Matterport3D trajectories.

Result: FP-Nav outperforms adapted state-of-the-art VLN baselines by more than 60% relative improvement in navigation success rate. Demonstrates robustness to actuation drift and floor plan distortions through comprehensive noise modeling and real-world deployments.

Conclusion: Floor plan guided navigation is effective and FloorPlan-VLN represents a promising step toward more spatially intelligent navigation agents that can leverage real-world spatial schematics.
...
TL;DR: P³Nav is a new VLN framework that integrates perception, prediction, and planning in a unified pipeline to improve scene understanding and navigation success by extracting multi-level visual cues and predicting future waypoints.


<details>
  <summary>Details</summary>
Motivation: Current VLN methods focus too much on visual-textual alignment for planning while neglecting comprehensive scene understanding, leaving agents with insufficient perception and prediction capabilities before planning.

Method: P³Nav integrates perception (extracting object-level and map-level cues), prediction (modeling potential future states via waypoints and forecasting semantic map cues), and planning in a unified end-to-end framework.

Result: Achieves state-of-the-art performance on REVERIE, R2R-CE, and RxR-CE benchmarks, demonstrating improved navigation success through better scene understanding.

Conclusion: Integrating perception, prediction, and planning in a unified pipeline significantly strengthens VLN agents' scene understanding and boosts navigation performance beyond current visual-textual alignment approaches.
...
<details>
  <summary>Details</summary>
Motivation: Realistic wireless channels in multi-robot systems introduce delays, erasures, and ordering stalls that degrade performance and compromise safety-critical decisions, while existing autonomy algorithms assume reliable communication.
...
Result: Coding-based communication significantly reduces in-order delivery stalls, preserves estimation consistency under delay, and improves deadline reliability relative to retransmission-based transport in cooperative localization and safety-critical overtaking case studies.

Conclusion: Joint design of autonomy algorithms and communication mechanisms is essential, with network coding serving as a principled tool for dependable multi-robot operation over wireless networks.
...
TL;DR: Proposes IMPACT framework for VICS testing with human-in-the-loop interactions to generate corner cases, extending digital twin taxonomy to L5 "Interactable" level.


<details>
  <summary>Details</summary>
Motivation: Existing corner-case generation methods are primarily AI-driven and limited to simulation, lacking human behavior uncertainty and real-world testing safety.

Method: Introduces L5 "Interactable" level in VICS-DT taxonomy and IMPACT framework enabling direct human interactions with VICS entities via mixed digital twin paradigm with "Physical-Virtual Action Interaction".

Result: Implemented on I-VIT testbed, demonstrates effectiveness in generating high-quality corner cases by incorporating unpredictable human behaviors and enabling safe testing with real-world environments.

Conclusion: IMPACT framework successfully extends VICS testing beyond simulation by incorporating human interactions, complementing AI-based methods and enabling safer corner-case testing with real-world elements.
...
TL;DR: A pipeline combining foundational models with MORL to enable robots to understand and apply context-dependent human navigation preferences expressed in natural language.


<details>
  <summary>Details</summary>
Motivation: Robots in human-shared environments need to adapt to human preferences, but these preferences are typically expressed in natural language and depend on environmental context, making direct integration into low-level control policies difficult.

Method: Combines VLM for environmental context extraction, LLM for converting natural language feedback into behavioral rules stored in updatable memory, and preference translation module mapping context/rules to numerical vectors that parameterize a pretrained MORL navigation policy.

Result: System reliably captures user intent, generates consistent preference vectors, and enables controllable behavior adaptation across diverse contexts, improving adaptability, transparency, and usability while maintaining safe real-time control.

Conclusion: The pipeline successfully integrates high-level semantic reasoning with low-level motion control, enabling robots to understand and apply context-dependent navigation preferences in human-shared environments.
...
TL;DR: KineVLA is a vision-language-action framework that handles fine-grained kinematic instructions for robotics, outperforming existing VLA methods on precise manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language-action (VLA) methods only capture kinematic attributes coarsely or partially, lacking support for fine-grained and personalized manipulation where task goals remain the same but execution trajectories must adapt to detailed kinematic specifications.

Method: Proposes KineVLA framework with bi-level action representation and bi-level reasoning tokens that explicitly decouple goal-level invariance from kinematics-level variability, using supervised intermediate variables to align language and action.

Result: KineVLA consistently outperforms strong VLA baselines on kinematics-sensitive benchmarks, achieving more precise, controllable, and generalizable manipulation behaviors on both LIBERO simulation and Realman-75 real-world robot platforms.

Conclusion: The proposed kinematics-rich VLA task and KineVLA framework enable fine-grained robotic manipulation by explicitly handling instruction-level kinematic variations, demonstrating superior performance over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action (VLA) models for robot control suffer from slow inference speeds. Existing speculative decoding methods are used in isolation without analyzing their relative advantages/disadvantages for VLA applications.

Method: Proposes HeiSD framework with hybrid speculative decoding: 1) retrieval-based SD optimization with verify-skip mechanism and sequence-wise relaxed acceptance, 2) kinematic-based fused metric to automatically determine hybrid boundary between drafter-based and retrieval-based SD.

Result: Achieves speedup of up to 2.45x in simulation benchmarks and 2.06x~2.41x in real-world scenarios while maintaining high task success rates.

Conclusion: Hybrid speculative decoding approach (HeiSD) effectively accelerates VLA models for robot control by combining strengths of both drafter-based and retrieval-based methods, addressing their individual limitations through novel optimization techniques.
...
TL;DR: VectorWorld is a streaming world model for real-time closed-loop evaluation of autonomous driving policies, addressing initialization mismatch, latency, and kinematic infeasibility issues.


<details>
  <summary>Details</summary>
Motivation: Existing generative world models degrade in closed-loop evaluation due to history-free initialization mismatches, multi-step sampling latency violating real-time constraints, and compounding kinematic infeasibility over long horizons.

Method: VectorWorld incrementally generates ego-centric vector-graph tiles using motion-aware gated VAE for policy-compatible initialization, one-step masked completion with edge-gated relational DiT for real-time outpainting, and ΔSim physics-aligned NPC policy with hybrid actions for stable long-horizon rollouts.
...
Conclusion: VectorWorld enables effective closed-loop evaluation of autonomous-driving policies by addressing key limitations of existing world models through streaming vector-graph generation and physics-aligned NPC policies.
...
TL;DR: REAL is an end-to-end learning framework for robust legged parkour that maintains agility even under severe visual degradation by combining cross-modal distillation, Mamba-based noise filtering, and physics-guided state estimation.


<details>
  <summary>Details</summary>
Motivation: Current learning-based legged systems achieve impressive agility but are fragile to perceptual degradation (visual noise/latency), which can cause catastrophic failures during extreme parkour maneuvers.

Method: REAL combines: 1) Cross-modal teacher policy distillation into deployable student with FiLM-modulated Mamba backbone for active visual noise filtering and short-term terrain memory, 2) Tight coupling of vision, proprioceptive history, and temporal memory, 3) Physics-guided Bayesian state estimator for rigid-body consistency during high-impact maneuvers.

Result: Validated on Unitree Go2 quadruped, REAL successfully traverses extreme obstacles even with 1-meter visual blind zone while maintaining real-time control with bounded 13.1 ms inference time.

Conclusion: REAL demonstrates that robust extreme agility is achievable under sensory corruption through tight cross-modal integration, active noise filtering, and physics-consistent state estimation, enabling reliable parkour in degraded perceptual conditions.
...
TL;DR: AgentVLN is a lightweight embodied navigation framework that uses VLMs for semantic reasoning with 3D perception, addressing spatial ambiguity and representation mismatch in VLN tasks.


<details>
  <summary>Details</summary>
Motivation: Current VLN systems suffer from limited spatial perception, 2D-3D representation mismatch, and monocular scale ambiguity, making them impractical for edge deployment despite strong 2D semantic understanding from VLMs.

Method: Formulates VLN as POSMDP with VLM-as-Brain paradigm, cross-space representation mapping for 3D-to-2D projection, context-aware self-correction, active exploration, and Query-Driven Perceptual Chain-of-Thought for geometric depth reasoning.

Result: Consistently outperforms prior SOTA methods on long-horizon VLN benchmarks and enables lightweight deployment for next-generation embodied navigation models.

Conclusion: AgentVLN provides an efficient framework that bridges 3D perception with 2D VLMs, addressing spatial ambiguity and representation inconsistency while being deployable on edge platforms.
...
TL;DR: Consistency-driven dual LSTM framework learns forward/inverse kinematics of wearable soft robotic arm, addressing nonlinear/hysteretic behaviors and one-to-many mapping challenges with cycle consistency loss.


<details>
  <summary>Details</summary>
Motivation: Need to accurately model kinematics of pneumatically actuated soft robotic arms in wearable devices, addressing nonlinear/hysteretic behaviors of soft pneumatic actuators and the one-to-many mapping challenge between actuation inputs and end-effector positions.

Method: Dual LSTM framework with cycle consistency loss to learn both forward and inverse kinematics simultaneously, enhancing physical realism and stability of inverse predictions.

Result: Consistency loss significantly boosts prediction accuracy and promotes physical consistency over conventional approaches. Wearable soft robotic arm demonstrates strong human-robot collaboration capabilities in tasks like object handover, obstacle-aware pick-and-place, and drawer operation.
...
TL;DR: AERR-Nav: A zero-shot object navigation framework with adaptive exploration-recovery-reminiscing strategy and fast/slow-thinking modes for better balancing exploration-exploitation in multi-floor environments.


<details>
  <summary>Details</summary>
Motivation: Current ZSON methods struggle to balance exploration and exploitation in unseen multi-floor environments, causing issues like robots getting stuck at narrow intersections, endless wandering, or failing to find stair entrances.

Method: Proposes AERR-Nav with: (1) Adaptive Exploration-Recovery-Reminiscing Strategy for dynamic state transitions, and (2) Adaptive Exploration State with Fast and Slow-Thinking modes for balancing exploration, exploitation, and higher-level reasoning.

Result: Achieves state-of-the-art performance among zero-shot methods on HM3D and MP3D benchmarks, with comprehensive ablation studies validating the proposed strategy and modules.

Conclusion: AERR-Nav effectively addresses the exploration-exploitation balance challenge in ZSON for multi-floor environments through its adaptive state-based framework.
...
TL;DR: VolumeDP is a 3D-aware policy architecture for visual imitation learning that lifts 2D image features into volumetric representations, selects task-relevant voxels, and uses multi-token decoding to predict actions, achieving state-of-the-art performance on robotic manipulation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing visual imitation methods suffer from a 2D-3D mismatch by mapping 2D image observations directly to 3D action outputs, which hinders spatial reasoning and degrades robustness in robotic manipulation tasks.
...
Result: Achieves state-of-the-art average success rate of 88.8% on LIBERO simulation benchmark (14.8% improvement over strongest baseline), large performance gains on ManiSkill and LIBERO-Plus benchmarks, and demonstrates higher success rates and robust generalization in real-world experiments with novel spatial layouts, viewpoints, and backgrounds.

Conclusion: VolumeDP's explicit 3D reasoning through volumetric representations and multi-token decoding effectively addresses the 2D-3D mismatch in visual imitation learning, leading to superior performance and robustness in robotic manipulation tasks across simulation and real-world environments.
...
TL;DR: MSH-MCCT is a novel CAV testbed that enables complex interactions between CAVs and human-driven vehicles using Mixed Digital Twin technology, combining physical, virtual, and mixed platforms with multi-source control inputs for enhanced experimental flexibility.


<details>
  <summary>Details</summary>
Motivation: In mixed traffic environments, CAVs need to interact with human-driven vehicles, but existing testbeds lack the capability to capture complex interactions between various CAVs and HDVs with human drivers in the loop.

Method: Developed MSH-MCCT testbed using Mixed Digital Twin concept (combining Mixed Reality with Digital Twin), integrating physical, virtual, and mixed platforms with multi-source control inputs, allowing human drivers and CAV algorithms to operate both physical and virtual vehicles across multiple fields of view.

Result: The testbed enables coexistence and real-time interaction of physical and virtual CAVs & HDVs, significantly enhancing experimental flexibility and scalability, as demonstrated through vehicle platooning experiments in mixed traffic with multi-source real human drivers using driving simulators of diverse fidelity.

Conclusion: MSH-MCCT shows potential for conducting comprehensive CAV testing with human drivers in the loop, addressing the need for realistic mixed traffic environment testing while providing enhanced flexibility and scalability compared to traditional approaches.
...
TL;DR: Decentralized algorithm for robots to form arbitrarily shaped assemblies using only local neighbor communication, without pose localization or motion constraints.


<details>
  <summary>Details</summary>
Motivation: To enable decentralized robot swarms to form complex assemblies without global coordination, avoiding common problems like unreachable states and gaps that plague existing approaches.

Method: Local interaction algorithm where in-assembly robots attract passing robots through signaling and alignment. Minimalistic approach requiring only communication between immediate attached neighbors, motion-agnostic, no pose localization needed, asynchronous and order-independent.

Result: Algorithm proven correct and demonstrated effective in forming a 107-robot assembly, showing it can create arbitrarily shaped assemblies without unreachable states or gaps.

Conclusion: The proposed decentralized algorithm enables robust, scalable assembly formation using only local interactions, overcoming limitations of previous approaches while maintaining simplicity and minimal requirements.
...
TL;DR: EVA is a reinforcement learning framework that aligns video world models for robotics by using an inverse dynamics model as a reward function to ensure generated videos produce physically executable actions.


<details>
  <summary>Details</summary>
Motivation: Current video world models for robotics lack explicit executability constraints, creating a mismatch between visually coherent rollouts and physically executable control commands (the "executability gap"). This leads to unstable or infeasible robot actions when decoded by inverse dynamics models.

Method: EVA trains an inverse dynamics model on real robot trajectories and repurposes it as a reward model that evaluates generated videos through the action sequences they induce. The reward encourages smooth motions (velocity, acceleration, jerk) while penalizing actions that violate embodiment constraints.

Result: Experiments on the RoboTwin benchmark and a real bimanual robot show that EVA reduces embodiment-specific artifacts in generated rollouts and improves downstream task execution success.

Conclusion: EVA effectively bridges the executability gap in video world models for robotics by using the executability gap itself as a training signal, enabling more physically consistent and actionable video predictions for robot control.
...
TL;DR: GeCO replaces fixed-schedule diffusion/flow matching with adaptive optimization, learning stationary velocity fields where expert actions are attractors, enabling early exit for simple states and longer refinement for complex ones.


<details>
  <summary>Details</summary>
Motivation: Current diffusion/flow matching models in robotic imitation learning use fixed integration schedules that waste computation on trivial motions while under-allocating to complex tasks, creating structural inefficiency.

Method: GeCO learns a stationary velocity field in action-sequence space where expert behaviors form stable attractors. Inference becomes iterative optimization that adaptively allocates computation based on convergence, with field norm providing OOD detection.

Result: GeCO improves success rates and efficiency on simulation benchmarks and scales to pi0-series VLA models as a plug-and-play replacement for flow-matching heads, with training-free safety signals for OOD detection.

Conclusion: GeCO transforms action synthesis from trajectory integration to optimization, enabling adaptive computation allocation and intrinsic safety monitoring, making it an efficient and safe alternative to standard flow-matching approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models with Flow Matching action heads suffer from inference latency due to multi-step iterative ODE solving, which hinders responsive physical robot control. While acceleration efforts focus on VLM backbones, the action head bottleneck remains unaddressed.

Method: ProbeFlow evaluates geometric trajectory complexity using cosine similarity between initial and lookahead velocity vectors, then dynamically schedules integration steps to prune redundant network evaluations. This training-free adaptive inference framework is tailored for continuous robotic control.

Result: On MetaWorld benchmark: 14.8x acceleration in action decoding (reducing average steps from 50 to 2.6), 2.8x reduction in end-to-end system latency without compromising manipulation success rate. On LIBERO benchmark: automatically allocates denser schedule for semantic bottlenecks, resolving flow solver delay. Real-world deployments confirm latency mitigation with execution stability.

Conclusion: ProbeFlow successfully mitigates action decoding latency in VLA models while ensuring execution stability, offering a practical solution for low-latency continuous generative policies in robotic control applications.
...
TL;DR: DexViTac is a portable human-centric data collection system for contact-rich dexterous manipulation that captures multimodal data (vision, tactile, poses, kinematics) and enables efficient dataset creation for robot learning.


<details>
  <summary>Details</summary>
Motivation: Existing human-centric data collection systems struggle to capture tactile information during physical interactions, which is essential for learning contact-rich dexterous manipulation tasks.

Method: Developed DexViTac system for high-fidelity multimodal data capture (vision, tactile, poses, kinematics) and proposed kinematics-grounded tactile representation learning algorithm to resolve semantic ambiguities in tactile signals.

Result: Collected over 2,400 demonstrations with efficiency exceeding 248 demonstrations/hour, robust against visual occlusions. Policies trained with this data achieved >85% success rate across four challenging tasks, significantly outperforming baselines.

Conclusion: DexViTac provides substantial improvement for learning contact-rich dexterous manipulation through efficient multimodal data collection and effective tactile representation learning.
...
TL;DR: A unified latent-driven framework bridges text to humanoid locomotion via bidirectional physical plausibility optimization, eliminating retargeting issues and improving deployment viability.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-motion pipelines for robots suffer from retargeting problems where kinematic motions become physically infeasible, with issues like contact errors, floating artifacts, and lack of real-world dynamical data.

Method: Introduces Physical Plausibility Optimization (PP-Opt) as a bidirectional coupling interface between motion generation and control. Forward: refines teacher-student policy with plausibility rewards. Backward: converts optimized simulation rollouts to fine-tune motion generator toward physically plausible latent distribution.

Result: Extensive experiments on Unitree G1 humanoid show improved tracking accuracy and success rates. The implicit latent-driven pipeline outperforms conventional explicit retargeting baselines in both precision and stability across IsaacLab and MuJoCo.

Conclusion: By coupling diffusion-based motion generation with physical plausibility optimization, the framework provides a practical path toward deployable text-guided humanoid intelligence through a self-improving cycle between generator and controller.
...
TL;DR: Framework that enforces Signal Temporal Logic constraints on robotics foundation models without retraining, ensuring safety and specification compliance during deployment.


<details>
  <summary>Details</summary>
Motivation: Robotics foundation models lack formal safety guarantees for time-dependent specifications like time-bounded goals, sequential objectives, and persistent safety conditions during real-world deployment.

Method: Specification-aware action distribution optimization framework that minimally modifies actions from pretrained models to satisfy STL constraints using forward dynamics propagation over remaining horizon.

Result: Validated in simulation with state-of-the-art robotics foundation model across multiple environments and complex specifications, showing constraint satisfaction.

Conclusion: Provides formal safety guarantees for robotics foundation models without modifying their parameters, enabling safe deployment with complex spatio-temporal requirements.
...
TL;DR: Novel OFDR-based shape sensing for CT-SDRs using integrated fiber in drilling instrument as protective sheath


<details>
  <summary>Details</summary>
Motivation: Traditional FBG-based shape sensing methods have limitations; need continuous strain measurement with enhanced spatial resolution for CT-SDRs

Method: Integrate single OFDR fiber with flat NiTi wire to create Shape Sensing Assembly (SSA), route through drilling instrument channel, use CT-SDR's pre-shaped NiTi tube for guidance

Result: Accurate and reliable shape sensing demonstrated under free-bending conditions and during drilling along J-shaped trajectories in synthetic phantoms

Conclusion: Feasible and robust integration strategy for shape sensing in CT-SDRs using OFDR technology with protective sheath approach
...
### [194] [Generative AI-assisted Participatory Modeling in Socio-Environmental Planning under Deep Uncertainty](https://arxiv.org/abs/2603.17021)
*Zhihao Pei,Nir Lipovetzky,Angela M. Rojas-Arevalo,Fjalar J. de Haan,Enayat A. Moallemi*

Main category: cs.AI

TL;DR: LLM-assisted workflow for participatory modeling in socio-environmental planning that translates stakeholder descriptions into quantitative models through iterative refinement.


<details>
  <summary>Details</summary>
Motivation: Participatory modeling for socio-environmental planning is complex and time-consuming, requiring translation of natural-language stakeholder descriptions into quantitative models for problem conceptualization.

Method: Proposed templated workflow using large language models (ChatGPT 5.2 Instant) to identify model components from stakeholder descriptions, explore diverse perspectives, assemble unified models, and implement in Python through iterative communication.

Result: Demonstrated workflow on lake problem and electricity market problem, obtaining acceptable outputs after few iterations with human verification and refinement.

Conclusion: Large language models can serve as effective tools for facilitating participatory modeling in problem conceptualization for socio-environmental planning under deep uncertainty.
...
### [195] [Transformers are Bayesian Networks](https://arxiv.org/abs/2603.17063)
*Gregory Coppola*

Main category: cs.AI

TL;DR: Transformers are Bayesian networks that implement weighted loopy belief propagation, with attention as AND and FFN as OR operations, forming Pearl's gather/update algorithm.


<details>
  <summary>Details</summary>
Motivation: Despite transformers being the dominant AI architecture, there's poor understanding of why they work. The paper aims to provide a precise mathematical characterization of transformers as Bayesian networks.

Method: Five formal approaches: 1) Prove sigmoid transformers implement weighted loopy belief propagation; 2) Constructive proof transformers implement exact belief propagation; 3) Prove uniqueness of BP weights for exact posteriors; 4) Analyze AND/OR boolean structure; 5) Experimental validation of formal results.

Result: Transformers are Bayesian networks implementing belief propagation, with attention as AND and FFN as OR operations. Loopy belief propagation works in practice despite theoretical convergence gaps. Verifiable inference requires finite concept spaces.

Conclusion: Transformers are Bayesian networks that implement belief propagation algorithms. Hallucination is not fixable by scaling but a structural consequence of operating without finite concepts. Formal verification requires finite concept spaces.
...
### [196] [Cascade-Aware Multi-Agent Routing: Spatio-Temporal Sidecars and Geometry-Switching](https://arxiv.org/abs/2603.17112)
*Davide Di Gioia*

Main category: cs.AI

TL;DR: Lightweight geometry-aware scheduler improves AI reasoning systems by modeling failure propagation patterns in different graph structures (tree-like vs cyclic), achieving 87.2% win rate vs 50.4% baseline.


<details>
  <summary>Details</summary>
Motivation: Current AI reasoning systems use symbolic graph networks but their schedulers are "geometry-blind" - they don't account for how failures propagate differently in tree-like structures (exponential cascade) versus cyclic graphs (self-limiting). This observability gap causes system-level inefficiencies.

Method: Proposes online geometry control with route-risk estimation on time-indexed execution graphs. Combines: 1) Euclidean spatio-temporal propagation baseline, 2) hyperbolic route-risk model with temporal decay, 3) learned geometry selector using topology statistics (6 features) and geometry-aware signals (BFS shell-growth slope, cycle-rank norm, Poincare curvature) via compact MLP (9→12→1).

Result: On Genesis 3 benchmark: adaptive switching improves win rate from 64-72% to 92% in hardest non-tree regimes, achieving 87.2% overall win rate. Baseline (no geometry awareness) achieves only 50.4% overall and 20% in tree-like regimes. Full system shows +36.8 percentage point overall improvement, with +48 to +68 pp gains in tree-like settings.

Conclusion: A lightweight 133-parameter sidecar effectively mitigates geometry-blind failure propagation in high-capability execution-graph systems, demonstrating significant performance improvements by accounting for structural differences in failure propagation patterns.
...
### [197] [How Clued up are LLMs? Evaluating Multi-Step Deductive Reasoning in a Text-Based Game Environment](https://arxiv.org/abs/2603.17169)
*Rebecca Ansell,Autumn Toney-Wails*

Main category: cs.AI

TL;DR: LLM agents struggle with deductive reasoning in Clue game simulations, achieving only 4 wins out of 18 games, and fine-tuning on logic puzzles doesn't reliably improve performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate LLM agents' multi-step deductive reasoning capabilities using a text-based version of Clue as a testbed, and to investigate whether fine-tuning on structured logic puzzles transfers to improved in-game reasoning.

Method: Implemented text-based multi-agent Clue game with six agents (GPT-4o-mini and Gemini-2.5-Flash), ran 18 simulated games, and investigated fine-tuning effects on structured logic puzzles.

Result: Agents achieved only 4 correct wins out of 18 games, showing difficulty maintaining consistent deductive reasoning. Fine-tuning didn't reliably improve performance and sometimes increased reasoning volume without improving precision.

Conclusion: LLM agents struggle with sustained deductive reasoning in complex games like Clue, and current fine-tuning approaches on logic puzzles don't effectively transfer to improved in-game reasoning performance.
...
### [198] [AI Scientist via Synthetic Task Scaling](https://arxiv.org/abs/2603.17216)
*Ziyang Cai,Harkirat Behl*

Main category: cs.AI

TL;DR: A pipeline for generating synthetic ML challenges to train AI research agents, improving performance on ML benchmarks by 9-12%.


<details>
  <summary>Details</summary>
Motivation: Current AI agents for scientific discovery lack principled training methods and often generate ineffective ideas, creating a need for better training environments for machine learning agents.

Method: Developed an automatic synthetic environment generation pipeline that creates ML challenges compatible with SWE-agent framework, including topic sampling, dataset proposal, and code generation, with verification through Huggingface API and self-debugging loops.

Result: Student models (Qwen3-4B and Qwen3-8B) trained with synthetic tasks from GPT-5 trajectories showed improved performance on MLGym benchmark, raising AUP metric by 9% for Qwen3-4B and 12% for Qwen3-8B.

Conclusion: The synthetic environment generation pipeline effectively trains machine learning agents, demonstrating that synthetic tasks grounded in real datasets can significantly improve agent performance on ML research benchmarks.
...
### [199] [Draft-and-Prune: Improving the Reliability of Auto-formalization for Logical Reasoning](https://arxiv.org/abs/2603.17233)
*Zhiyu Ni,Zheng Liang,Liangcheng Song,Chenrui Cao,Xian Zhang,Alberto Sangiovanni-Vincentelli,Pierluigi Nuzzo*

Main category: cs.AI

TL;DR: Draft-and-Prune (D&P) improves auto-formalization for logical reasoning by generating diverse natural-language plans, pruning incorrect formalizations, and aggregating predictions via majority voting, achieving state-of-the-art performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current auto-formalization pipelines are brittle - they often produce programs that fail to execute or encode incorrect semantics. While prior work addresses syntactic failures using solver feedback, reducing semantic failures remains a major bottleneck in AF-based logical reasoning.

Method: Draft-and-Prune (D&P) framework: 1) Drafts multiple natural-language plans for reasoning problems, 2) Conditions program generation on these plans, 3) Prunes executable but contradictory or ambiguous formalizations, and 4) Aggregates predictions from surviving paths using majority voting.

Result: D&P significantly improves AF-based reasoning without extra supervision. On AR-LSAT: 78.43% accuracy with GPT-4, 78.00% with GPT-4o, outperforming MAD-LOGIC and CLOVER. Achieves near-ceiling performance on other benchmarks: 100% on PrOntoQA and LogicalDeduction.

Conclusion: Draft-and-Prune effectively addresses semantic failures in auto-formalization through diversity and verification, substantially strengthening AF-based logical reasoning across diverse benchmarks without requiring additional supervision.
...
### [200] [Graph-Native Cognitive Memory for AI Agents: Formal Belief Revision Semantics for Versioned Memory Architectures](https://arxiv.org/abs/2603.17244)
*Young Bin Park*

Main category: cs.AI

TL;DR: Kumiho is a graph-native cognitive memory architecture for AI agents that unifies memory management with versionable asset management, formally grounded in AGM belief revision semantics, achieving state-of-the-art performance on cognitive memory benchmarks.


<details>
  <summary>Details</summary>
Motivation: While individual components for AI agent memory exist, their architectural synthesis and formal grounding remain underexplored. There's a need for a unified architecture that can handle both cognitive memory and versionable asset management with formal semantics.

Method: Presents Kumiho, a graph-native cognitive memory architecture using formal belief revision semantics (AGM framework). Uses a dual-store model (Redis working memory, Neo4j long-term graph) with hybrid fulltext and vector retrieval. Key innovations: prospective indexing (LLM-generated future-scenario implications), event extraction (structured causal events in summaries), and client-side LLM reranking.

Result: Achieves 0.565 overall F1 on LoCoMo (n=1,986) with 97.5% adversarial refusal accuracy. On LoCoMo-Plus (Level-2 cognitive memory benchmark), achieves 93.3% judge accuracy (n=401), substantially outperforming all published baselines (best baseline: Gemini 2.5 Pro at 45.7%). Architecture is model-decoupled - switching from GPT-4o-mini to GPT-4o improves accuracy without pipeline changes.

Conclusion: Kumiho demonstrates that a unified graph-native architecture can effectively serve both cognitive memory and versionable asset management needs for AI agents, with formal grounding in belief revision semantics enabling strong performance on cognitive memory benchmarks while maintaining model decoupling and cost-effectiveness.
...
### [201] [Contrastive Reasoning Alignment: Reinforcement Learning from Hidden Representations](https://arxiv.org/abs/2603.17305)
*Haozheng Luo,Yimin Wang,Jiahao Yu,Binghui Wang,Yan Chen*

Main category: cs.AI

TL;DR: CRAFT is a red-teaming alignment framework that improves jailbreak defense by aligning model reasoning through hidden representation optimization, achieving significant safety improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: Prior defenses operate primarily at output level, lacking robustness against jailbreak attacks. There's a need for deeper safety alignment that leverages model reasoning capabilities and hidden representations.

Method: Integrates contrastive representation learning with reinforcement learning to separate safe/unsafe reasoning trajectories. Optimizes objectives over hidden state space and incorporates latent-textual consistency into GRPO to eliminate superficially aligned policies.

Result: Outperforms state-of-the-art defenses (IPO, SafeKey) on multiple safety benchmarks. Achieves 79.0% improvement in reasoning safety and 87.7% improvement in final-response safety over base models using Qwen3-4B-Thinking and R1-Distill-Llama-8B.

Conclusion: Hidden-space reasoning alignment through CRAFT effectively improves robustness against jailbreak attacks, demonstrating the value of aligning reasoning processes rather than just final outputs.
...
### [202] [InfoDensity: Rewarding Information-Dense Traces for Efficient Reasoning](https://arxiv.org/abs/2603.17310)
*Chengwei Wei,Jung-jae Kim,Longyin Zhang,Shengkai Chen,Nancy F. Chen*

Main category: cs.AI

TL;DR: InfoDensity: A reward framework for RL training that reduces LLM verbosity by optimizing reasoning quality (entropy reduction & monotonicity) rather than just response length, achieving better accuracy-efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: LLMs generate verbose and redundant reasoning traces with unnecessary computational costs. Existing RL approaches only optimize final response length without considering intermediate reasoning quality, making them vulnerable to reward hacking. The authors argue verbosity stems from poor intermediate reasoning quality rather than just length.

Method: Empirical study tracking conditional entropy of answer distribution across reasoning steps reveals high-quality reasoning exhibits low uncertainty convergence and monotonic progress. Based on this, propose InfoDensity reward framework combining: 1) AUC-based reward for entropy reduction, 2) monotonicity reward for consistent progress, weighted by length scaling term favoring concise reasoning.
...
Conclusion: Verbosity in LLM reasoning is a symptom of poor intermediate reasoning quality, not just length. InfoDensity effectively optimizes reasoning quality through entropy reduction and monotonicity measures, enabling more efficient reasoning without sacrificing accuracy.
...
### [203] [Physics-informed offline reinforcement learning eliminates catastrophic fuel waste in maritime routing](https://arxiv.org/abs/2603.17319)
*Aniruddha Bora,Julie Chalfant,Chryssostomos Chryssostomidis*

Main category: cs.AI

TL;DR: PIER is an offline RL framework for shipping route optimization that reduces CO2 emissions by 10% and dramatically cuts catastrophic fuel waste from 4.8% to 0.5% of voyages.


<details>
  <summary>Details</summary>
Motivation: International shipping contributes 3% of global greenhouse gas emissions, but current voyage routing relies on heuristic methods that are inefficient and don't account for safety risks or weather uncertainties.

Method: Offline reinforcement learning framework using physics-calibrated environments from historical vessel tracking (AIS) and ocean reanalysis data, combining physics-informed state construction, demonstration-augmented offline data, and decoupled safety shield.

Result: 10% mean CO2 reduction vs great-circle routing, 9-fold reduction in catastrophic fuel waste (4.8% to 0.5%), 3.5x lower fuel variance, forecast-independent performance unaffected by weather uncertainty.

Conclusion: PIER provides substantial fuel savings and safety improvements for shipping, with architecture transferable to other domains like wildfire evacuation and autonomous navigation.
...
### [204] [ShuttleEnv: An Interactive Data-Driven RL Environment for Badminton Strategy Modeling](https://arxiv.org/abs/2603.17324)
*Ang Li,Xinyang Gong,Bozhou Chen,Yunlong Lu,Jiaming Ji,Yongyi Wang,Yaodong Yang,Wenxin Li*

Main category: cs.AI

TL;DR: ShuttleEnv is a data-driven badminton simulation environment for RL and strategy analysis, using elite-player match data and probabilistic models to simulate realistic rallies without physics-based simulation.


<details>
  <summary>Details</summary>
Motivation: To create an interactive simulation environment for badminton that supports reinforcement learning and strategic behavior analysis in fast-paced adversarial sports, addressing the need for realistic agent-opponent interactions without complex physics simulation.

Method: Uses elite-player match data and explicit probabilistic models to simulate rally-level dynamics, creating a data-driven environment that enables realistic and interpretable agent-opponent interactions.

Result: Developed ShuttleEnv with multiple trained agents, live step-by-step visualization of badminton rallies, and interactive analysis capabilities for exploring different play styles and emergent strategies.

Conclusion: ShuttleEnv serves as a reusable platform for research, visualization, and demonstration of intelligent agents in sports AI, providing a practical tool for studying strategic behavior in badminton.
...
### [205] [A Progressive Visual-Logic-Aligned Framework for Ride-Hailing Adjudication](https://arxiv.org/abs/2603.17328)
*Weiming Wu,Zi-Jian Cheng,Jie Meng,Peng Zhen,Shan Huang,Qun Li,Guobin Wu,Lan-Zhe Guo*

Main category: cs.AI

TL;DR: RideJudge is a visual-logic-aligned framework for automated ride-hailing responsibility disputes that addresses multimodal LLM limitations through trajectory synthesis, adaptive context optimization, and ordinal-sensitive reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Manual review of ride-hailing disputes is intractable due to exponential volume growth, while conventional automated methods lack reasoning transparency needed for quasi-judicial decisions. Multimodal LLMs struggle with bridging visual semantics and evidentiary protocols, leading to hallucinations and logical looseness.

Method: Progressive Visual-Logic-Aligned Framework with: 1) SynTraj synthesis engine to ground abstract liability concepts into concrete trajectory patterns, 2) Adaptive Context Optimization to distill expert knowledge within limited context windows, 3) Chain-of-Adjudication mechanism for active evidentiary inquiry, and 4) Ordinal-Sensitive Reinforcement Learning for hierarchical severity calibration.
...
Conclusion: The proposed framework successfully addresses systemic misalignments in automated adjudication by bridging semantic gaps, optimizing context usage, enforcing evidentiary protocols, and calibrating decision boundaries against hierarchical severity, enabling transparent and accurate responsibility assessment.
...
### [206] [Towards Safer Large Reasoning Models by Promoting Safety Decision-Making before Chain-of-Thought Generation](https://arxiv.org/abs/2603.17368)
*Jianan Chen,Zhifang Zhang,Shuo He,Linan Yue,Lei Feng,Minling Zhang*

Main category: cs.AI

TL;DR: LRMs suffer safety degradation when CoT is enabled; proposed method integrates safety decision signals before CoT generation to improve safety while maintaining reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Large reasoning models (LRMs) show remarkable reasoning performance via chain-of-thought (CoT), but this comes at the expense of significantly degraded safety capabilities. The degradation occurs only when CoT is enabled, not when it's disabled.

Method: Propose safety alignment method that encourages LRMs to make safety decisions BEFORE CoT generation. Use Bert-based classifier to extract safety decision signals from safe models (CoT-disabled LRMs), then integrate these signals as auxiliary supervision during safety alignment. This allows safety gradients to backpropagate to LRMs' latent representations, strengthening safety decision-making against CoT generation.
...
Conclusion: The proposed safety alignment approach successfully addresses the safety degradation problem in LRMs when CoT is enabled, allowing them to maintain both safety and reasoning capabilities.
...
### [207] [From Digital Twins to World Models:Opportunities, Challenges, and Applications for Mobile Edge General Intelligence](https://arxiv.org/abs/2603.17420)
*Jie Zheng,Dusit Niyato,Changyuan Zhao,Jiawen Kang,Jiacheng Wang*

Main category: cs.AI

TL;DR: Survey paper examining the transition from digital twins to world models for enabling edge general intelligence in 6G+ systems, covering conceptual differences, architectures, applications, and future directions.


<details>
  <summary>Details</summary>
Motivation: Traditional digital twins face limitations in autonomy, adaptability, and scalability in dynamic edge environments, necessitating a shift toward more flexible world models to enable edge general intelligence.

Method: Systematic survey approach: clarifies conceptual differences between digital twins and world models, reviews design principles and architectures (perception, latent representation, dynamics learning, planning, memory), examines integration in wireless EGI systems, and surveys applications.

Result: Provides comprehensive analysis of the transition from physics-based, centralized digital twins to data-driven, decentralized world models, with practical insights for designing world-model-driven edge intelligence systems and identification of key research challenges.

Conclusion: World models represent a crucial evolution beyond digital twins for enabling adaptive, autonomous, and resource-efficient edge general intelligence in 6G+ systems, with significant applications across wireless domains and ongoing research challenges in scalability, reliability, and interoperability.
...
### [208] [Proactive Knowledge Inquiry in Doctor-Patient Dialogue: Stateful Extraction, Belief Updating, and Path-Aware Action Planning](https://arxiv.org/abs/2603.17425)
*Zhenhai Pan,Yan Liu,Jia You*

Main category: cs.AI

TL;DR: Proactive knowledge-inquiry framework for doctor-patient dialogues that models uncertainty and guides information gathering, achieving 83.3% coverage in controlled pilot evaluation.


<details>
  <summary>Details</summary>
Motivation: Current EMR systems are output-oriented (transcribing, extracting, summarizing after consultation) but don't model what's known vs. unknown, uncertainty, or guide the inquiry process proactively.

Method: Formulates doctor-patient dialogue as proactive knowledge-inquiry under partial observability. Combines stateful extraction, sequential belief updating, gap-aware state modeling, hybrid retrieval over medical knowledge, and POMDP-lite action planner.

Result: In controlled pilot evaluation (10 standardized dialogues + 300-query benchmark): 83.3% coverage, 80.0% risk recall, 81.4% structural completeness, lower redundancy than baselines. Results are pilot-only, not clinical generalization.

Conclusion: Proactive inquiry is methodologically interesting under controlled conditions and conceptually appealing for dialogue-based EMR generation, but this is only a pilot concept demonstration, not evidence of clinical deployment readiness.
...
### [209] [When Only the Final Text Survives: Implicit Execution Tracing for Multi-Agent Attribution](https://arxiv.org/abs/2603.17445)
*Yi Nian,Haosen Cao,Shenzhe Zhu,Henry Peng Zou,Qingqing Luan,Yue Zhao*

Main category: cs.AI

TL;DR: IET enables token-level attribution and interaction topology reconstruction in multi-agent systems without execution logs, using embedded signals in generated text.


<details>
  <summary>Details</summary>
Motivation: When multi-agent systems produce harmful outputs, accountability is difficult without execution logs and agent identifiers. Current systems obscure interaction topology and individual contributions in final outputs.

Method: IET embeds agent-specific keyed signals into token distributions during generation, creating self-describing execution traces. A transition-aware scoring method detects agent handover points and reconstructs interaction graphs.

Result: Experiments show IET recovers agent segments and coordination structure with high accuracy while preserving generation quality.

Conclusion: IET enables privacy-preserving auditing for multi-agent language systems by allowing accountability without requiring execution logs.
...
### [210] [Informative Semi-Factuals for XAI: The Elaborated Explanations that People Prefer](https://arxiv.org/abs/2603.17534)
*Saugat Aryal,Mark T. Keane*

Main category: cs.AI

TL;DR: ISF method generates informative semi-factual explanations that reveal hidden features influencing decisions, outperforming current methods in quality and user preference.


<details>
  <summary>Details</summary>
Motivation: Current semi-factual XAI methods only show extreme value changes that don't alter outcomes but fail to explain WHY these changes don't matter, leaving users without understanding of underlying decision factors.

Method: Proposed Informative Semi-Factuals (ISF) algorithm that supplements semi-factuals with information about additional hidden features that influence automated decisions, providing more elaborated explanations.

Result: Experimental results on benchmark datasets show ISF computes semi-factuals that are both informative and high-quality on key metrics. User study demonstrates people prefer these elaborated explanations over simpler semi-factuals from current methods.

Conclusion: The ISF method advances XAI by generating more informative semi-factual explanations that reveal hidden decision factors, improving both technical quality and user understanding/preference.
...
### [211] [Per-Domain Generalizing Policies: On Learning Efficient and Robust Q-Value Functions (Extended Version with Technical Appendix)](https://arxiv.org/abs/2603.17544)
*Nicola J. Müller,Moritz Oster,Isabel Valera,Jörg Hoffmann,Timo P. Gros*

Main category: cs.AI

TL;DR: Learning Q-value functions instead of state-value functions for planning policies, with regularization to distinguish between taken vs. not-taken actions, outperforms state-value policies across 10 domains.


<details>
  <summary>Details</summary>
Motivation: Standard approaches learn state-value functions using supervised learning on optimal plans, but evaluating these policies is expensive as they need to process every successor state. Q-value functions are cheaper to evaluate as they only need to process the current state, but vanilla supervised learning of Q-values performs poorly.

Method: Propose learning Q-value functions instead of state-value functions, using regularization terms that enforce distinction between actions taken and those not taken by the teacher planner during supervised learning.

Result: Q-value policies consistently outperform state-value policies across a range of 10 domains and are competitive with the planner LAMA-first.

Conclusion: Learning Q-value functions with appropriate regularization is an effective approach for creating per-domain generalizing policies that are both efficient (cheaper to evaluate) and performant.
...
### [212] [VeriGrey: Greybox Agent Validation](https://arxiv.org/abs/2603.17639)
*Yuntong Zhang,Sungmin Kang,Ruijie Meng,Marcel Böhme,Abhik Roychoudhury*

Main category: cs.AI

TL;DR: VeriGrey is a grey-box testing approach that discovers security vulnerabilities in LLM agents by using tool invocation sequences as feedback and mutating prompts to create injection attacks, achieving 33% higher efficacy than black-box methods.


<details>
  <summary>Details</summary>
Motivation: LLM agents that autonomously interact with external tools introduce critical security risks, particularly indirect prompt injection vulnerabilities that traditional black-box testing methods fail to detect effectively.

Method: VeriGrey uses a grey-box approach that monitors tool invocation sequences as feedback to drive testing, mutates prompts to create malicious injection prompts by linking agent tasks to injection tasks, and employs mutational fuzz testing for conversation agents.

Result: VeriGrey achieves 33% higher efficacy than black-box baselines on AgentDojo benchmark, discovers attack scenarios in real-world agents like Gemini CLI and OpenClaw, and achieves 100% success rate on Kimi-K2.5 and 90% on Opus 4.6 for finding malicious skill variants.

Conclusion: VeriGrey demonstrates the value of dynamic grey-box testing for uncovering security risks in LLM agents, providing a foundation for developing comprehensive agent assurance frameworks.
...
### [213] [Sensi: Learn One Thing at a Time -- Curriculum-Based Test-Time Learning for LLM Game Agents](https://arxiv.org/abs/2603.17683)
*Mohsen Arjmandi*

Main category: cs.AI

TL;DR: Sensi is an LLM agent architecture for ARC-AGI-3 that achieves 50-94x sample efficiency improvement through structured test-time learning with two-player separation, curriculum learning, and database-as-control-plane, though it suffers from perceptual grounding issues.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents require thousands of interactions to learn task structure at test time in unknown environments, which is inefficient and impractical for real-world deployment.

Method: Three key mechanisms: (1) two-player architecture separating perception from action, (2) curriculum-based learning managed by external state machine, (3) database-as-control-plane for programmatic context window steering, plus LLM-as-judge with dynamic evaluation rubrics.

Result: Sensi v1 solves 2 game levels with two-player architecture alone. Sensi v2 adds curriculum learning and solves 0 levels but completes entire learning curriculum in ~32 attempts, achieving 50-94x greater sample efficiency than comparable systems (1600-3000 attempts).

Conclusion: The architectural bottleneck has shifted from learning efficiency to perceptual grounding (self-consistent hallucination cascade in perception layer), which is a more tractable problem than the original learning efficiency challenge.
...
### [214] [MALLES: A Multi-agent LLMs-based Economic Sandbox with Consumer Preference Alignment](https://arxiv.org/abs/2603.17694)
*Yusen Wu,Yiran Liu,Xiaotie Deng*

Main category: cs.AI

TL;DR: MALLES: A multi-agent LLM-based economic sandbox that uses preference learning and mean-field mechanisms to simulate high-dimensional, multimodal economic environments, improving product selection, purchase prediction, and simulation stability.


<details>
  <summary>Details</summary>
Motivation: Modern economic decision-making faces challenges from high-dimensional, multimodal environments with agent heterogeneity and combinatorial data sparsity, requiring better simulation frameworks that can handle cross-domain scenarios.

Method: Uses LLM post-training on heterogeneous transaction records for economic alignment, implements mean-field mechanism for simulation stability, and employs multi-agent discussion framework with specialized agents to process extensive product information collaboratively.

Result: Achieves significant improvements in product selection accuracy, purchase quantity prediction, and simulation stability compared to existing economic and financial LLM simulation baselines.

Conclusion: Demonstrates the potential of large language models as a foundational pillar for high-fidelity, scalable decision simulation and analysis in the real economy based on foundational databases.
...
### [215] [From Virtual Environments to Real-World Trials: Emerging Trends in Autonomous Driving](https://arxiv.org/abs/2603.17714)
*A. Humnabadkar,A. Sikdar,B. Cave,H. Zhang,N. Bessis,A. Behera*

Main category: cs.AI

TL;DR: Survey paper reviewing synthetic data and simulation technologies for autonomous driving, covering perception, planning, validation, and domain adaptation strategies.


<details>
  <summary>Details</summary>
Motivation: Real-world autonomous driving deployment faces data scarcity, safety constraints, and generalization challenges across diverse environments. Synthetic data and virtual environments offer scalable, controllable solutions with rich annotations.

Method: Comprehensive survey organized across three dimensions: synthetic data for perception/planning, digital twin-based simulation for validation, and domain adaptation strategies. Includes taxonomy of datasets, tools, simulation platforms, and benchmark design trends.

Result: Provides systematic organization of the field, highlighting key technologies including vision-language models and simulation realism for enhanced scene understanding and generalization.

Conclusion: Identifies critical challenges and open research directions: Sim2Real transfer, scalable safety validation, cooperative autonomy, and simulation-driven policy learning needed for safe, generalizable, globally deployable autonomous driving systems.
...
### [216] [Facts as First Class Objects: Knowledge Objects for Persistent LLM Memory](https://arxiv.org/abs/2603.17781)
*Oliver Zahn,Simran Chana*

Main category: cs.AI

TL;DR: Knowledge Objects (KOs) outperform in-context memory for LLMs with 100% accuracy, 252x lower cost, and better multi-hop reasoning, while revealing critical failure modes in traditional approaches.


<details>
  <summary>Details</summary>
Motivation: Current LLMs use in-context memory as default for persistent knowledge work, but this approach has fundamental limitations in capacity, reliability, and cost that need systematic benchmarking against alternative approaches like Knowledge Objects.

Method: Benchmarked in-context memory against Knowledge Objects (discrete hash-addressed tuples with O(1) retrieval), tested across Claude Sonnet 4.5 and four frontier models, evaluated capacity limits, compaction loss, goal drift, multi-hop reasoning, and introduced density-adaptive retrieval switching.

Result: KOs achieved 100% accuracy across all conditions at 252x lower cost, with 78.9% multi-hop reasoning vs 31.6% for in-context. In-context memory failed at 8,000 facts, lost 60% through summarization, and eroded 54% of constraints through goal drift. Cross-model replication showed compaction loss is architectural.

Conclusion: Knowledge Objects provide superior reliability, accuracy, and cost-effectiveness compared to in-context memory for persistent knowledge work in LLMs, with architectural advantages that overcome fundamental limitations of traditional approaches.
...
### [217] [Governed Memory: A Production Architecture for Multi-Agent Workflows](https://arxiv.org/abs/2603.17787)
*Hamed Taheri*

Main category: cs.AI

TL;DR: Governed Memory is a shared memory and governance layer for enterprise AI that solves memory governance gaps across autonomous agent workflows through dual memory models, tiered governance, reflection-bounded retrieval, and closed-loop schema lifecycle.


<details>
  <summary>Details</summary>
Motivation: Enterprise AI faces structural challenges from memory governance gaps: memory silos across agent workflows, governance fragmentation, unstructured memories unusable by downstream systems, redundant context delivery, and silent quality degradation without feedback loops.

Method: Four mechanisms: 1) dual memory model combining open-set atomic facts with schema-enforced typed properties; 2) tiered governance routing with progressive context delivery; 3) reflection-bounded retrieval with entity-scoped isolation; 4) closed-loop schema lifecycle with AI-assisted authoring and automated per-property refinement.

Result: 99.6% fact recall with complementary dual-modality coverage; 92% governance routing precision; 50% token reduction from progressive delivery; zero cross-entity leakage across 500 adversarial queries; 100% adversarial governance compliance; output quality saturation at ~7 governed memories per entity; 74.8% overall accuracy on LoCoMo benchmark.

Conclusion: Governed Memory effectively addresses enterprise AI's memory governance challenges, demonstrating that governance and schema enforcement impose no retrieval quality penalty while providing significant efficiency and security benefits. The system is production-ready at Personize.ai.
...
### [218] [RPMS: Enhancing LLM-Based Embodied Planning through Rule-Augmented Memory Synergy](https://arxiv.org/abs/2603.17831)
*Zhenhang Yuan,Shenghai Yuan,Lihua Xie*

Main category: cs.AI

TL;DR: RPMS architecture improves LLM agent performance in embodied environments by enforcing action feasibility through rule retrieval and managing state drift with belief-based memory filtering.


<details>
  <summary>Details</summary>
Motivation: LLM agents struggle in closed-world embodied environments due to strict action preconditions and sparse failure feedback, leading to coupled failure modes of invalid actions and state drift.

Method: RPMS uses conflict-managed architecture with structured rule retrieval for action feasibility, belief state for memory filtering, and rules-first arbitration to resolve conflicts between rules and memory.

Result: On ALFWorld, RPMS achieves 59.7% success with Llama 3.1 8B (+23.9 pp over baseline) and 98.5% with Claude Sonnet 4.5 (+11.9 pp); rule retrieval contributes +14.9 pp of the 8B gain. ScienceWorld adaptation shows consistent gains (54.0 vs 44.9 baseline).

Conclusion: Rule retrieval is the dominant factor in performance gains, and episodic memory is conditionally useful—harmful without grounding but beneficial when filtered by current state and constrained by explicit action rules.
...
### [219] [AgentFactory: A Self-Evolving Framework Through Executable Subagent Accumulation and Reuse](https://arxiv.org/abs/2603.18000)
*Zhang Zhang,Shuqi Lu,Hongjin Qian,Di He,Zheng Liu*

Main category: cs.AI

TL;DR: AgentFactory is a new self-evolution paradigm for LLM-based agents that preserves successful task solutions as executable subagent code instead of textual prompts, enabling continuous capability accumulation through reusable, portable Python subagents.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based agent self-evolution approaches record successful experiences as textual prompts or reflections, which cannot reliably guarantee efficient task re-execution in complex scenarios. There's a need for a more robust and reliable method for agent evolution.
...
Result: AgentFactory enables continuous capability accumulation where its library of executable subagents grows and improves over time, progressively reducing the effort required for similar tasks without manual intervention. The system demonstrates portability across any Python-capable system.

Conclusion: AgentFactory represents a significant advancement in LLM-based agent self-evolution by moving from textual experience recording to executable code preservation, creating a sustainable ecosystem of reusable, self-improving subagents that accumulate capabilities over time.
